...
describe('CalculatorPage smoke suite', () => {
  ...
  it('hydrates programming packages from the backend route instead of relying only on bundled options', async () => {
    vi.stubGlobal(
      'fetch',
    ...
    expect(screen.getByText(/live programming catalog/i)).toBeDefined();
  });

  it('shows all licensed Fusion toolpaths before filtering and narrows them by license tier', async () => {
    await renderCalculator();
  ...
  it('shows manual-programming cycles for lathe workflows', async () => {
    ...
    fireEvent.click(screen.getAllByRole('button', { name: /lathe/i })[0]!);
    fireEvent.click(screen.getByRole('button', { name: /programming package manual programming/i }));
