...
const finishPath = {
  label: 'Finish Pass',
  path: 'Manual > Finish Pass',
  operationId: 'finishing',
};

const profilePath = {
  label: 'Profile Mill',
  path: 'Manual > Profile Mill',
  operationId: 'shoulder_milling',
};

const chamferPath = {
  label: 'Chamfer Mill',
  path: 'Manual > Chamfer / Deburr',
  operationId: 'chamfer_milling',
};

const featureFlowRoughPath = {
...
describe('calculator tooling selection', () => {
  ...
  it('blocks chamfer cutters from generic finish and profile paths', () => {
    const chamferTool: ToolCatalogItem = {
      id: 'chamfer-mill',
      mode: 'mill',
      family: 'Chamfer Mill',
      label: '90 degree chamfer mill',
      description: 'Chamfer and edge-break cutter.',
      holder: 'Hydraulic holder',
      coating: 'AlTiN',
      defaultDiameter: 6,
      defaultFlutes: 2,
      operation: 'finishing',
      geometryClass: 'chamfer',
      supportedOperations: ['finishing', 'chamfer_milling'],
      toolpathKeywords: ['chamfer', 'deburr'],
    };

    expect(toolSupportsToolpath(chamferTool, finishPath)).toBe(false);
    expect(toolSupportsToolpath(chamferTool, profilePath)).toBe(false);
    expect(toolSupportsToolpath(chamferTool, chamferPath)).toBe(true);
  });

  it('treats FeatureFlow adaptive roughing as roughing instead of surface flow finishing', () => {
    const ballFinisher: ToolCatalogItem = {
      id: 'ball-finisher',
      mode: 'mill',
      family: 'Ball End Mill',
      label: '8 mm ball finisher',
      description: 'Ball endmill for 3D flowline and scallop finishing.',
      holder: 'Shrink-fit holder',
      coating: 'AlTiN',
      defaultDiameter: 8,
      defaultFlutes: 4,
      ...
      supportedOperations: ['finishing'],
      toolpathKeywords: ['flowline', 'scallop', 'finish'],
    };

    const variableHelixRougher: ToolCatalogItem = {
      id: 'vh-rougher',
      mode: 'mill',
      family: 'Variable Helix End Mill',
      label: '10 mm variable helix rougher',
      description: 'Adaptive roughing end mill.',
      holder: 'Hydraulic holder',
      coating: 'AlTiN',
      defaultDiameter: 10,
      defaultFlutes: 5,
      ...
      supportedOperations: ['roughing'],
      toolpathKeywords: ['adaptive', 'roughing'],
      variableHelix: true,
    };

    expect(toolSupportsToolpath(ballFinisher, featureFlowRoughPath)).toBe(false);
    expect(toolSupportsToolpath(variableHelixRougher, featureFlowRoughPath)).toBe(true);
    expect(selectPreferredToolForToolpath([ballFinisher, variableHelixRougher], featureFlowRoughPath)?.id).toBe('vh-rougher');
  });
});
