...
import {
  buildPrismShopFloorInsightFixture,
  buildPrismPromptAnalysisFixture,
  PRISM_INTELLIGENCE_WORKSPACE,
...
export const fixtureOperatingSystemServices: OperatingSystemServices = {
  ...
  async getPrismIntelligenceWorkspace() {
    return PRISM_INTELLIGENCE_WORKSPACE;
  },

  async analyzePrismPrompt(prompt) {
    return buildPrismPromptAnalysisFixture(prompt);
  },

  async analyzePrismShopFloor(input) {
    return buildPrismShopFloorInsightFixture(input);
  },
