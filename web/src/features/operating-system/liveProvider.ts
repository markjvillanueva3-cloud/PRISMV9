...
function normalizeCalculatorMeasuredPerformance(
  value: LiveCalculatorUserMachineProfileReadModel['profile']['measuredPerformance'] | null | undefined,
  ...
  if (Array.isArray(value.notes)) {
    measuredPerformance.notes = value.notes.filter((item): item is string => typeof item === 'string' && item.trim().length > 0);
  }
...
export const liveOperatingSystemServices: OperatingSystemServices = {
  ...
  async buildTrackedTasks(packet, department, role): Promise<ShopFloorTrackedTask[]> {
    return withFixtureFallback(
      ...
      () => fixtureOperatingSystemServices.buildTrackedTasks(packet, department, role),
    );
  },
  ...
  async registerJob(input): Promise<JobRegistrationResult> {
    return withFixtureFallback(
      ...
      () => fixtureOperatingSystemServices.registerJob(input),
    );
  },
  ...
  async checkIntoDepartment(input): Promise<DepartmentCheckInResult> {
    return withFixtureFallback(
      ...
      () => fixtureOperatingSystemServices.checkIntoDepartment(input),
    );
  },
  ...
  async buildRoiSignals(input): Promise<string[]> {
    return withFixtureFallback(
      ...
      () => fixtureOperatingSystemServices.buildRoiSignals(input),
    );
  },
  ...
  async getPrismIntelligenceWorkspace() {
    return withFixtureFallback(
      () => requestOperatingSystem<PrismIntelligenceWorkspace>('/intelligence/workspace'),
      () => fixtureOperatingSystemServices.getPrismIntelligenceWorkspace(),
    );
  },

  async analyzePrismPrompt(prompt: string) {
    return withFixtureFallback(
      () => requestOperatingSystem<PrismPromptAnalysis>('/intelligence/analyze', { prompt }),
      () => fixtureOperatingSystemServices.analyzePrismPrompt(prompt),
    );
  },

  async analyzePrismShopFloor(input: PrismShopFloorInsightInput) {
    return withFixtureFallback(
      () => requestOperatingSystem<PrismShopFloorInsight>('/intelligence/shop-floor', input),
      () => fixtureOperatingSystemServices.analyzePrismShopFloor(input),
    );
  },
