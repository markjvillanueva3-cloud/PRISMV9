...
const BUILT_IN_POSTS: PostEntry[] = [
  ...
  { id: 'prism-fanuc-31i', name: 'PRISM Fanuc 31i', description: 'Physics-optimized Fanuc 31i post — per-block S/F, macro variables, Kienzle force, Taylor tool life.', source: 'prism_native', vendor: 'PRISM', controller: 'fanuc_31i', machine_types: ['mill'], axes: 3, capabilities: ['probing', 'hsm', 'tcp', 'subprograms', 'per_block_sf', 'kienzle_force', 'tool_life', 'macro_variables'], version: '8.3', machine_profile: { max_rpm: 12000, max_power_kW: 15, rapid_x: 42000, rapid_y: 42000, rapid_z: 36000, volume_x: 560, volume_y: 410, volume_z: 460, coolant_types: ['flood', 'tsc'], recommended_features: ['probing_cycles', 'high_speed_smoothing', 'subprograms', 'macro_variables'] } },
  ...
  { id: 'prism-okuma-osp', name: 'PRISM Okuma OSP-P300', description: 'Physics-optimized Okuma OSP post — G270/G180, per-block S/F, G96/G97 CSS, Kienzle force model.', source: 'prism_native', vendor: 'PRISM', controller: 'okuma_osp_p300', machine_types: ['mill', 'lathe', 'mill_turn'], axes: 3, capabilities: ['probing', 'subprograms', 'per_block_sf', 'kienzle_force', 'tool_life', 'css_control'], version: '8.3', machine_profile: { max_rpm: 5000, max_power_kW: 30, rapid_x: 30000, rapid_y: 30000, rapid_z: 30000, volume_x: 660, volume_y: 400, volume_z: 610, coolant_types: ['flood', 'tsc'], recommended_features: ['probing_cycles', 'subprograms'] } },
...
export function PostLibraryUI({ onSelectPost, onGenerateForMachine }: PostLibraryUIProps) {
  ...
  // Load catalog from API, merge with built-in PRISM posts
  useEffect(() => {
  ...
  // PRISM-enhanced posts always on top, then catalog, then remaining built-in
  const posts = useMemo(() => {
    ...
    if (catalogPosts.length > 0) {
      // Merge: PRISM posts first, then catalog (which supersedes built-in CPS), then any built-in not in catalog
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex gap-1.5">
          ...
          <button onClick={() => setSourceFilter('prism_native')} className={`rounded-full px-2.5 py-0.5 text-[10px] font-semibold transition ${sourceFilter === 'prism_native' ? 'bg-emerald-500/20 text-emerald-200' : 'text-slate-500 hover:text-slate-300'}`}>PRISM ({prismCount})</button>
        ...
        {filteredPosts.map((post) => (
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <StatusPill label={post.source === 'prism_native' ? 'PRISM' : 'CPS'} tone={SOURCE_TONES[post.source] ?? 'sky'} />
            </div>
