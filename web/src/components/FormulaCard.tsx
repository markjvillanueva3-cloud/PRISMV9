...
export function FormulaCard({ formula }: Props) {
  ...
  const [drafts, setDrafts] = useState<Record<string, string>>(() => {
    ...
    for (const input of formula.inputs) {
      initial[input.name] = formatNumericExpressionValue(input.defaultValue);
    }
    return initial;
  });
  ...
  function handleChange(name: string, raw: string) {
    ...
    if (!raw.trim()) {
      setValues((prev) => ({ ...prev, [name]: Number.NaN }));
      setErrors((prev) => ({ ...prev, [name]: validate(name, Number.NaN) }));
      return;
    }
    ...
    if (value === null) {
      setValues((prev) => ({ ...prev, [name]: Number.NaN }));
      setErrors((prev) => ({ ...prev, [name]: 'Invalid expression' }));
      return;
    }
  ...
  function handleBlur(name: string) {
    ...
    setDrafts((prev) => ({ ...prev, [name]: formatNumericExpressionValue(value) }));
  }
          ...
          return (
            <div key={input.name}>
              ...
              <input
                ...
                title="Supports math like 1/8, 25.4*2, or 200/3"
              />
              {err && (
