...
const DOMAIN_BADGE_COLORS: Record<string, string> = {
  ...
  shop_practice: "bg-amber-500/20 text-amber-400",
  machine_operation: "bg-purple-500/20 text-purple-400",
};

function KnowledgeCard({ entry, onSelect }: { entry: KnowledgeEntry; onSelect: (e: KnowledgeEntry) => void }) {
  return (
    <button onClick={() => onSelect(entry)} className="text-left w-full p-4 rounded-xl border border-slate-700/60 hover:border-blue-300 hover:shadow-lg hover:shadow-black/20 transition-all bg-slate-800/70 backdrop-blur-sm">
      <div className="flex items-center gap-2 mb-1">
        <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${DOMAIN_BADGE_COLORS[entry.domain] ?? "bg-slate-700 text-slate-300"}`}>
          {entry.domain.replace("_", " ")}
        </span>
        <span className="text-[11px] text-slate-400">{entry.source}</span>
        <span className="text-[11px] text-slate-400 ml-auto">{entry.relevance_score}% match</span>
      </div>
      <h3 className="font-medium text-slate-100 text-sm">{entry.title}</h3>
      <p className="text-xs text-slate-400 mt-1 line-clamp-2">{entry.excerpt}</p>
      {entry.tags.length > 0 && (
...
function TribalCard({ entry }: { entry: TribalEntry }) {
  return (
    <div className="p-4 rounded-xl border border-slate-700/60 bg-slate-800/70 backdrop-blur-sm">
      <div className="flex items-center gap-2 mb-1">
        <span className="text-[11px] bg-orange-500/20 text-orange-400 px-2 py-0.5 rounded-full font-medium">Tribal</span>
        {entry.verified && <span className="text-[11px] text-green-400">Verified</span>}
        <span className="text-[11px] text-slate-400 ml-auto">{entry.upvotes} upvotes</span>
      </div>
      <h3 className="font-medium text-slate-100 text-sm">{entry.title}</h3>
...
function DetailModal({ entry, onClose }: { entry: KnowledgeEntry; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="knowledge-modal-title">
      <div className="bg-slate-800 rounded-xl max-w-2xl w-full max-h-[80vh] overflow-y-auto p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-start mb-4">
          <div>
            <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${DOMAIN_BADGE_COLORS[entry.domain] ?? "bg-slate-700 text-slate-300"}`}>
              {entry.domain.replace("_", " ")}
            </span>
            <h2 id="knowledge-modal-title" className="text-xl font-bold text-slate-100 mt-2">{entry.title}</h2>
          </div>
          <button onClick={onClose} aria-label="Close" className="text-slate-400 hover:text-slate-300 text-xl">&times;</button>
        </div>
        <div className="prose prose-sm max-w-none text-slate-300 whitespace-pre-wrap">{entry.content}</div>
        <div className="mt-4 pt-3 border-t border-slate-700/40 flex items-center gap-2 text-xs text-slate-400">
          <span>Source: {entry.source}</span>
          {entry.updated_at && <span>&middot; Updated: {new Date(entry.updated_at).toLocaleDateString()}</span>}
        </div>
        {entry.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {entry.tags.map((t) => <span key={t} className="text-xs bg-slate-700 text-slate-400 px-2 py-0.5 rounded">{t}</span>)}
          </div>
        )}
      </div>
    </div>
...
export default function KnowledgeSearch() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold text-slate-100 mb-1">Knowledge Search</h1>
      <p className="text-slate-400 text-sm mb-6">Search manufacturing knowledge base and tribal wisdom</p>
      ...
      <div className="flex gap-2 mb-4">
        <input
          ...
          className="flex-1 px-4 py-2.5 border border-slate-700/60 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm bg-slate-900/50 text-slate-200"
        />
        <button
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <div className="flex bg-slate-700 rounded-lg p-0.5">
          <button
            ...
            className={`px-3 py-1.5 text-sm rounded-md transition-colors ${tab === "knowledge" ? "bg-slate-800 shadow text-slate-100 font-medium" : "text-slate-400"}`}
          >
            Knowledge Base
          </button>
          <button
            onClick={() => setTab("tribal")}
            className={`px-3 py-1.5 text-sm rounded-md transition-colors ${tab === "tribal" ? "bg-slate-800 shadow text-slate-100 font-medium" : "text-slate-400"}`}
          >
            Tribal Knowledge
          </button>
        </div>
        {tab === "knowledge" && (
          <>
            <select
              ...
              className="px-3 py-1.5 border border-slate-700/60 rounded-lg text-sm bg-slate-900/50 text-slate-200"
            >
      ...
      {(knowledge.error || tribal.error) && (
        <div className="mt-4 text-sm text-red-400 bg-red-500/10 p-3 rounded-lg">{knowledge.error || tribal.error}</div>
      )}
