...
const FEATURED_COMMAND_ROUTES = new Set([
  '/dashboard',
  '/calculator',
  '/jobs',
  '/quote-builder',
...
export function Layout() {
  ...
  const workspaceCommands = useMemo(() => flattenWorkspaceCommands(), []);
  ...
  const featuredWorkspaces = useMemo(
    () => workspaceCommands.filter((command) => FEATURED_COMMAND_ROUTES.has(command.to)).slice(0, 6),
    [workspaceCommands],
  );
