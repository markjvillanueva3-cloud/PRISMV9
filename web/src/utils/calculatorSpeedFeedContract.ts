...
export function classifyCalculatorResultSafetyPosture(
  ...
  const setupCompletenessRatio =
    context.setupCompleteness != null ? clampRatio(context.setupCompleteness / 100) : undefined;
...
function parseTorqueNm(...parts: Array<string | undefined | null>) {
  for (const part of parts) {
    if (!part) continue;
    const match = part.replace(/,/g, '').match(/(\d+(?:\.\d+)?)\s*(?:n\.?m|nm)/i);
    if (match) return safeNumber(match[1]);
  }
  return undefined;
}
...
function inferSpindleTaper(input: CalculatorSpeedFeedContractInput): SpeedFeedParams['spindle_taper'] {
  ...
  if (/hsk[-\s]?e?40/.test(signature)) return 'HSK-E40';
...
function inferCutType(input: CalculatorSpeedFeedContractInput): SpeedFeedParams['cut_type'] {
  ...
  if (
    ...
    || /finish|parallel|scallop|swarf|skim|fine/.test(signature)
    || hasSurfaceFlowSignature(signature)
  ) {
    return 'finishing';
  }
...
function inferStrategy(input: CalculatorSpeedFeedContractInput): SpeedFeedParams['strategy'] {
  ...
  if (/adaptive|dynamic/.test(signature)) return 'adaptive';
  if (/high speed|hsm|parallel|scallop|swarf|multi-axis|multiaxis/.test(signature) || hasSurfaceFlowSignature(signature)) return 'hsm';
...
function machinePackageConfidenceScore(confidence?: MachineCatalogItem['packageProvenance']['confidence']) {
  switch (confidence) {
    case 'merged':
      return 0.92;
    case 'published':
...
function inferAxisCount(machine: MachineCatalogItem) {
  const signature = signatureOf(machine.taxonomy?.axisClass, machine.axes, machine.machineTypeId, machine.machineTypeLabel);
  if (/5/.test(signature)) return 5;
  if (/4/.test(signature)) return 4;
  if (/swiss|turning|lathe|2[-\s]?axis/.test(signature)) return 2;
  if (/wire|xyuv/.test(signature)) return 4;
  if (/laser|waterjet|router|3[-\s]?axis/.test(signature)) return 3;
  return undefined;
}

function buildMachinePackage(
  input: CalculatorSpeedFeedContractInput,
  args: {
    machineName: string;
    machineType: SpeedFeedParams['machine_type'];
    ...
    machineTorqueNm?: number;
    spindleTaper?: SpeedFeedParams['spindle_taper'];
    machineDynamics: ReturnType<typeof deriveMachineDynamics>;
  },
): SpeedFeedParams['machinePackage'] {
  const provenance = input.machine.packageProvenance;
  const baseConfidence = machinePackageConfidenceScore(provenance?.confidence);
  const measuredBoost = input.measuredMachineData ? 0.08 : 0;
  const overallConfidence = Math.min(0.98, roundMetric(baseConfidence + measuredBoost, 3) ?? baseConfidence);
  const spindleConfidence = args.spindleRpm && args.machinePowerKw
    ? Math.min(0.98, roundMetric(overallConfidence + 0.05, 3) ?? overallConfidence)
    : Math.max(0.35, roundMetric(overallConfidence - 0.2, 3) ?? 0.35);

  return {
    id: input.machine.packageId ?? `${input.machine.id}::calculator`,
    canonical_id: input.machine.canonicalMachineId ?? input.machine.id,
    manufacturer: input.machine.manufacturer,
    model: input.machine.model,
    name: args.machineName,
    mode: input.machine.mode,
    family: input.machine.family,
    machine_type: args.machineType,
    confidence: {
      overall: overallConfidence,
      spindle: spindleConfidence,
      controller: input.controllerOption ? overallConfidence : Math.max(0.45, roundMetric(overallConfidence - 0.16, 3) ?? 0.45),
      envelope: input.machine.envelope ? overallConfidence : Math.max(0.45, roundMetric(overallConfidence - 0.16, 3) ?? 0.45),
      axes: input.machine.taxonomy?.axisClass || input.machine.axes ? overallConfidence : 0.5,
    },
    spindle: {
      max_rpm: args.spindleRpm,
      power_kw: args.machinePowerKw,
      power_continuous_kw: args.machinePowerKw,
      power: args.machinePowerKw,
      max_torque_nm: args.machineTorqueNm,
      taper: args.spindleTaper,
    },
    controller: input.controllerOption
      ? {
          id: input.controllerOption.id,
          label: input.controllerOption.label,
          model: input.controllerOption.label,
        }
      : undefined,
    envelope: {
      description: input.machine.envelope,
    },
    axes: {
      count: inferAxisCount(input.machine),
      class: input.machine.taxonomy?.axisClass,
      description: input.machine.axes,
    },
    tooling: {
      layout: input.machine.toolingLayout?.kind,
      stations: input.machine.toolingLayout?.stations,
      interface: input.machine.toolingLayout?.interface,
      spindle_connection: input.machine.toolingLayout?.spindleConnectionLabel,
    },
    dynamics: {
      guideway: args.machineDynamics.guideway,
      natural_frequency_hz: args.machineDynamics.naturalFrequencyHz,
      system_stiffness_n_per_um: args.machineDynamics.systemStiffnessNPerUm,
      damping_ratio: args.machineDynamics.dampingRatio,
      axis_accel_m_s2: args.machineDynamics.axisAccelerationMps2,
      axis_jerk_m_s3: args.machineDynamics.axisJerkMps3,
    },
    provenance: provenance ?? {
      source: 'calculator-catalog',
      confidence: input.measuredMachineData ? 'measured' : 'inferred',
      sourceRecordIds: [input.machine.id],
      notes: ['Built by Calculator Studio speed/feed contract for machine safety gates.'],
    },
  };
}
...
export function buildCalculatorSpeedFeedParams(
  ...
  const machineName = inferMachineName(input.machine);
  const spindleTaper = inferSpindleTaper(input);
  const machinePackage = buildMachinePackage(input, {
    machineName,
    machineType,
    spindleRpm,
    machinePowerKw,
...
export function normalizeCalculatorSpeedFeedResult(
  ...
  const rpm = readAtomicNumber(raw, 'spindle_rpm', 'rpm');
  const cuttingSpeed = readAtomicNumber(raw, 'cutting_speed_mpm', 'cutting_speed_m_per_min');
  const feedRate = readAtomicNumber(raw, 'feed_rate_mmmin', 'feed_mm_min');
  const feedPerTooth = readAtomicNumber(raw, 'feed_per_tooth_mm', 'chipload_mm');
  ...
  return {
    ...
    feedRate,
    feedPerTooth,
    mrr: readAtomicNumber(raw, 'mrr_cm3min', 'mrr_cm3_min'),
    powerKw,
    ...
    axialDepthMm: readAtomicNumber(raw, 'axial_depth_mm', 'doc_mm'),
    radialDepthMm: readAtomicNumber(raw, 'radial_depth_mm', 'woc_mm'),
