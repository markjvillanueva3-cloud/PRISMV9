...
async function requestDataValue<T>(
  method: 'GET' | 'POST',
  path: string,
  body?: Record<string, unknown>,
): Promise<T> {
  const payload = await requestData<T>(method, path, body);
  if (!payload.ok) {
    throw new ApiError(500, payload.error || 'PRISM data request failed');
  }
  return payload.data;
}
...
export async function orderUpdateStatus(params: { order_id: string; status: string; job_id?: string }): Promise<PrismResponse> {
...
export async function milestoneCreateTimeline(params: {
  job_id: string;
  quote_id?: string;
  customer_id?: string;
  start_at_milestone?: MilestoneKey;
}): Promise<MilestoneTimelineRecord> {
  return requestDataValue('POST', '/portal/milestones', params);
}

export async function milestoneGetTimeline(jobId: string): Promise<MilestoneTimelineRecord> {
  return requestDataValue('GET', `/portal/milestones/${encodeURIComponent(jobId)}`);
}

export async function milestoneAdvance(jobId: string, params: {
  milestone_key?: MilestoneKey;
  notes?: string;
  advanced_by?: string;
}): Promise<MilestoneTimelineRecord> {
  return requestDataValue('POST', `/portal/milestones/${encodeURIComponent(jobId)}/advance`, params);
}

export async function milestoneSyncMutation(jobId: string, params: {
  ...
  return requestDataValue('POST', `/portal/milestones/${encodeURIComponent(jobId)}/sync`, params);
}

export async function milestoneGetSyncEvents(jobId: string, limit = 6): Promise<MilestoneSyncEvent[]> {
  return requestDataValue('GET', `/portal/milestones/${encodeURIComponent(jobId)}/sync-events?limit=${encodeURIComponent(String(limit))}`);
}
...
export async function quoteHistory(quoteId: string): Promise<DataResponse<InstantQuoteHistoryEntry[]>> {
