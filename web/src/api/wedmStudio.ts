...
export async function quickGenerate(
  ...
  try {
    ...
    const [classifyResult, feasibilityResult] = await Promise.all([
      ...
      wedmApi.feasibility({ features, material }, signal),
    ]);
    ...
    const [startHolesResult, toolpathResult] = await Promise.all([
      ...
      wedmApi.toolpath({ features, material }, signal),
    ]);
    ...
    progress(55);
    currentStepIdx = 3;
    const [tabsResult, sequenceResult] = await Promise.all([
    ...
    progress(75);
    const [flushingResult, wireBreakResult] = await Promise.all([
    ...
    progress(85);
    currentStepIdx = 5;
    const gcodeResult = await wedmApi.gcode({ controller, features, material }, signal);
    if (!gcodeResult.ok) return gcodeResult;

    // Stage 9: Cost + setup sheet (parallel)
    progress(95);
    const [costResult, setupResult] = await Promise.all([
      wedmApi.cost({ features, material }, signal),
      wedmApi.setupSheet({ features, material, controller }, signal),
    ]);
