...
export function EmployeeDirectoryPage() {
  ...
  const activeEmployee =
    ...
    ?? employees[0]
    ?? null;
  const workflowRecordType = activeEmployee ? 'Employee' : routeContext.origin.recordType;
  const workflowRecordId = activeEmployee?.id || routedEmployeeId || routeContext.origin.recordId;
      ...
      routeContext.origin.source
        ...
        : {
            ...
            note:
              ...
              || `Keep ${activeEmployee ? `${activeEmployee.first_name} ${activeEmployee.last_name}` : 'the selected employee'} attached while workforce work moves into timecards, payroll, and live floor execution.`,
            threadId: routeContext.origin.threadId,
          },
    [activeEmployee, launcherSource, routeContext.origin, workflowRecordId, workflowRecordType],
  );
    ...
    () =>
      activeEmployee
        ...
        : routeContext.focus.id
          ? routeContext.focus
          : undefined,
    [activeEmployee, routeContext.focus],
  );
    ...
    () =>
      buildWorkflowPath('/timecards', location.search, {
        ...
        extras: {
          ...
          employeeId: activeEmployee?.id || routedEmployeeId,
        },
      }),
    [activeEmployee?.id, effectiveFocus, effectiveOrigin, location.search, routedEmployeeId],
  );
  ...
  const shopFloorPath = useMemo(
    () =>
      buildShopFloorPath(location.pathname, location.search, {
        ...
        note:
          routeContext.origin.note
          || `Launch ${activeEmployee ? `${activeEmployee.first_name} ${activeEmployee.last_name}` : 'the selected employee'} into live shift and task tracking with workforce context intact.`,
        origin: effectiveOrigin,
        focus: effectiveFocus,
        extras: {
          employeeId: activeEmployee?.id || routedEmployeeId,
    ...
    [activeEmployee, effectiveFocus, effectiveOrigin, location.pathname, location.search, routeContext.origin.note, routedEmployeeId],
  );
  ...
  async function handleManagerCloseShift(employeeId: string) {
    setEditSaving(true);
    setError(null);
    try {
      await shiftClockOut({ employee_id: employeeId });
      setEditingEmployee(null);
      await loadEmployees();
    } catch (issue) {
      ...
      {tab === 'directory' ? (
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.08fr)_minmax(360px,0.92fr)]">
          ...
          <div className="space-y-6">
            ...
            <PanelCard title="Workforce handoff" subtitle="Push the active employee into time review, payroll, or live floor work without losing workforce context.">
              <div className="grid gap-3">
                <Link
                  to={timecardsPath}
                ...
                >
                  ...
                  <div className="mt-2 text-sky-50/80">
                    Carry the selected employee into weekly labor review and approval without reopening the roster cold.
                  </div>
                </Link>
                ...
                >
                  ...
                  <div className="mt-2 text-slate-300">
                    Launch the active employee into live shift and task tracking without dropping workforce provenance.
                  </div>
                </Link>
              </div>
            </PanelCard>
          ...
          <PanelCard title="Onboarding brief" subtitle="Keep the target employee and resulting skill state visible while you add new people.">
            {activeEmployee ? (
              <div className="space-y-3">
                <SummaryTile label="Selected employee" value={`${activeEmployee.first_name} ${activeEmployee.last_name}`} hint="The currently selected person for skill or utilization actions." accent="from-cyan-400/22 via-cyan-300/10 to-transparent" />
                <SummaryTile label="Department" value={activeEmployee.department} hint="Department assignment on the roster." accent="from-violet-400/22 via-violet-300/10 to-transparent" />
              </div>
            ) : (
              <div className="rounded-[22px] border border-white/8 bg-black/15 px-4 py-4 text-sm leading-6 text-slate-400">
            ...
            {activeEmployee ? (
              <div className="space-y-3">
                <div className="rounded-[22px] border border-white/8 bg-white/[0.03] px-4 py-4">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    <StatusPill label={activeEmployee.department} tone="violet" />
                    <StatusPill label={activeEmployee.role} tone="sky" />
                  </div>
                </div>
                <div className="rounded-[22px] border border-white/8 bg-black/15 px-4 py-4 text-sm leading-6 text-slate-300">
                  Skills: {(activeEmployee.skills ?? []).length > 0 ? activeEmployee.skills.join(', ') : 'No skill tags on file yet.'}
                </div>
              </div>
            ) : (
              <div className="rounded-[22px] border border-white/8 bg-black/15 px-4 py-4 text-sm leading-6 text-slate-400">
