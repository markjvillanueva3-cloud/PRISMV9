...
import { PrismMetricStat, type PrismGlowColor } from '../components/prism';
import {
  NotificationBell,
  NotificationPanel,
  ToastContainer,
...
/**
 * MetricTile — delegates to PrismMetricStat (U-Q-PRISM-METRIC-STAT, slot:quebec).
 * Calculator Studio language now applied uniformly across every Dashboard tile.
 * Accent string is mapped to a PrismGlowColor via inferAccent() so legacy
 * call-sites keep working without churn — `from-amber-400/...` → "amber", etc.
 */
function inferAccent(accent: string): PrismGlowColor {
  if (/amber|yellow|orange/.test(accent)) return 'amber';
  if (/emerald|green|teal/.test(accent)) return 'emerald';
  if (/red|rose|pink/.test(accent)) return 'red';
  if (/violet|purple|fuchsia|indigo/.test(accent)) return 'violet';
  return 'cyan';
}
          ...
          <div className="grid gap-4 xl:grid-cols-3">
            {hotJobs.slice(0, 3).map((hotJob) => (
              <HotPriorityCard
                ...
                releasePath={buildHotWorkflowPath('/print-to-cnc', hotJob, true)}
