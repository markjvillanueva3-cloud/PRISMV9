...
export function MessagesPage() {
  ...
  useEffect(() => {
    ...
    async function loadWorkspace() {
      ...
      try {
        ...
        if (!cancelled) {
          setError(issue instanceof Error ? issue.message : 'Messages workspace could not be loaded.');
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  const upstreamJobsPath = buildWorkflowPath('/jobs', location.search, {
    ...
    focus: downstreamJobFocusId ? { type: 'job', id: downstreamJobFocusId, jobId: downstreamJobFocusId } : undefined,
