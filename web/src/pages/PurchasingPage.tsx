...
export function PurchasingPage() {
    ...
    () =>
      routeContext.origin.source
        ...
        : {
            ...
            recordId: material,
            customer: '',
            note: `Carry sourcing posture for ${material} into receiving, capture, and supplier follow-up.`,
            threadId: '',
          },
    [material, routeContext.origin],
  );
  ...
  async function handleSearch() {
    ...
    try {
      ...
      setError(issue instanceof ApiError ? issue.message : 'Supplier search failed');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleRecommend() {
    ...
    try {
      ...
      setError(issue instanceof ApiError ? issue.message : 'Failed to get purchasing recommendations');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSummary() {
    ...
    try {
      ...
      setError(issue instanceof ApiError ? issue.message : 'Failed to load purchasing summary');
    } finally {
      setLoading(false);
    }
  }
            ...
            {summaryData ? (
              <div className="grid gap-3">
                <div className="rounded-[22px] border border-white/8 bg-white/[0.03] px-4 py-4">
                  <div className="text-[11px] uppercase tracking-[0.18em] text-slate-500">Summary payload</div>
                  <pre className="mt-3 overflow-auto whitespace-pre-wrap text-xs leading-6 text-slate-300">
