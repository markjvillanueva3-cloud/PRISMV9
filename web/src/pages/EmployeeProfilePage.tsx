              ...
              <div className="space-y-3">
                {(learningPath.courses ?? learningPath.path ?? []).length > 0 ? (
                  (learningPath.courses ?? learningPath.path ?? []).map((course: any, i: number) => (
                    <div key={course.id ?? i} className="rounded-lg border border-white/8 bg-white/[0.03] px-3 py-2 text-sm">
                      <div className="font-medium text-slate-100">{course.title ?? course.name ?? course.id}</div>
                      {course.status && <StatusPill label={course.status} tone={course.status === 'completed' ? 'emerald' : course.status === 'in_progress' ? 'sky' : 'slate'} />}
                    </div>
                  ))
                ) : (
