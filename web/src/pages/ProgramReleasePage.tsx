...
function chooseCatalogSelection(current: string, ids: string[]) {
...
function chooseCatalogSelectionWithRoutePreference(current: string, preferred: string | null, ids: string[]) {
...
function readRouteSelection(search: string) {
...
function findCadSourceId(catalog: ProgramReleaseCatalog | null, preference: 'compare' | 'prism') {
  if (!catalog) {
    return '';
  }
...
export function ProgramReleasePage() {
  ...
  const routeSelection = useMemo(() => readRouteSelection(location.search), [location.search]);
  ...
  useEffect(() => {
    ...
    operatingSystem
      ...
      .then(async (nextCatalog) => {
        ...
        setPartClassId((current) =>
          chooseCatalogSelectionWithRoutePreference(current, routeSelection.partClassId, mergedCatalog.partClasses.map((item) => item.id)),
        );
        setMachineId((current) =>
          chooseCatalogSelectionWithRoutePreference(current, preferredMachineId, mergedCatalog.machines.map((item) => item.id)),
        );
        setToolholderId((current) =>
          chooseCatalogSelectionWithRoutePreference(current, routeSelection.toolholderId, mergedCatalog.toolholders.map((item) => item.id)),
        );
        setToolingPackageId((current) =>
          chooseCatalogSelectionWithRoutePreference(current, routeSelection.toolingPackageId, mergedCatalog.toolingPackages.map((item) => item.id)),
        );
        setFixtureId((current) =>
          chooseCatalogSelectionWithRoutePreference(current, routeSelection.fixtureId, mergedCatalog.fixtures.map((item) => item.id)),
        );
        setStockId((current) =>
          chooseCatalogSelectionWithRoutePreference(current, routeSelection.stockId, mergedCatalog.stockProfiles.map((item) => item.id)),
        );
        setCadSourceId((current) =>
          chooseCatalogSelectionWithRoutePreference(current, routeSelection.cadSourceId, mergedCatalog.cadSources.map((item) => item.id)),
        );
      })
      .catch((error) => {
        if (active) {
  ...
  function applyFiles(files: FileList | null) {
    ...
    if (nextArtifacts.length > 0) {
      ...
      const compareSourceId = findCadSourceId(catalog, 'compare');
            ...
            <div className="grid gap-3">
              ...
              <ActionButton
                ...
                onClick={() => {
                  ...
                  setCadSourceId((current) => findCadSourceId(catalog, 'prism') || current);
                }}
              >
                Design in PRISM
              </ActionButton>
