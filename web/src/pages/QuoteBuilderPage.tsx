...
interface QuoteGenerateWorkspace {
  ...
  history: InstantQuoteHistoryEntry[];
    ...
    () =>
      releaseInput
        ? buildWorkflowPath('/print-to-cnc', location.search, {
            ...
            extras: {
              packetId: releaseInput.packetId,
              partClassId: releaseInput.partClassId,
              machineId: releaseInput.machineId,
              toolholderId: releaseInput.toolholderId,
              toolingPackageId: releaseInput.toolingPackageId,
              fixtureId: releaseInput.fixtureId,
              stockId: releaseInput.stockId,
              cadSourceId: releaseInput.cadSourceId,
            },
          })
        : '/print-to-cnc',
    [effectiveOrigin, location.search, releaseFocusId, releaseInput],
  );
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (instantResponse.status === 'fulfilled') {
        ...
        const history =
          downstreamSettled[2]?.status === 'fulfilled' ? downstreamSettled[2].value.data ?? [] : [];
        const shareToken =
                      ...
                      <div className="rounded-[22px] border border-white/8 bg-white/[0.03] px-4 py-4">
                        ...
                        <div className="mt-3 space-y-3 text-sm leading-6 text-slate-300">
                          ...
                          <div>
                            ...
                            {quoteGenerateWorkspace.history.length > 0
                              ? quoteGenerateWorkspace.history.map((entry) => `${entry.id} (${entry.status})`).join(' · ')
