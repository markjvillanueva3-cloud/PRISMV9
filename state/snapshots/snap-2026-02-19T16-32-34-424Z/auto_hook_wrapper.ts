      ...
      cadence.actions.push(cpResult.success ? `CHECKPOINT_AUTO_SAVED:${cpResult.checkpoint_id}` : "CHECKPOINT_FAILED");
          ...
          try {
            ...
            const hotContent = [
              ...
              "",
              "## Recent Activity",
              "Last 5 tool calls: " + (() => { try { const ra = JSON.parse(fs.readFileSync(RECENT_ACTIONS_FILE, "utf-8")).actions || []; return ra.slice(-5).map((a: any) => a.tool + ":" + a.action).join(", "); } catch { return "unavailable"; } })(),
              "",
