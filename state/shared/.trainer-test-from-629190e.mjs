...
// ---------------------------------------------------------------------------
// U-NN-TRAINER-EXPORT-FIX — stratified negative sampling
//
// graphsage-train-pipeline.mjs imports positiveTypeMarginal +
// sampleStratifiedNegativeEdges from this module (U-NNG-PIPELINE-STRATIFIED-
// WIRE wired the consumer side); the producer side was never implemented, so
// the pipeline crashed at import with a SyntaxError. These suites pin the new
// producers + their integration into train().
// ---------------------------------------------------------------------------

/** Trainer-internal NUL-separated undirected edge key (edgeKey is not exported). */
...
describe("positiveTypeMarginal", () => {
  it("counts endpoint types from [u,v] array edges", () => {
    ...
    // 2 edges: a-b (engine,engine), c-d (dispatcher,dispatcher) -> 2 each.
  ...
  it("skips malformed edge entries (adversarial: short array, null, scalar)", () => {
    ...
    // Only the well-formed ["a","b"] contributes; {u:"a"} has v=undefined ->
    // a counts, v skipped.
    assert.equal(m.get("t"), 3); // a,b from ["a","b"] + a from {u:"a"}
  });
});
...
describe("sampleStratifiedNegativeEdges", () => {
  ...
  it("happy path: draws the requested count, all real-edge-free, distinct", () => {
    ...
    const seen = new Set();
    for (const { u, v } of negs) {
      assert.notEqual(u, v, "no self-loop");
      assert.ok(!edgeSet.has(ek(u, v)), `${u}-${v} must not be a real edge`);
      const key = ek(u, v);
      assert.ok(!seen.has(key), `${u}-${v} must be distinct`);
      seen.add(key);
    }
  });
  ...
  it("pHard=1.0 -> every negative is intra-type (hard negatives only)", () => {
    ...
    assert.ok(negs.length > 0, "must produce negatives");
    for (const { u, v } of negs) {
      assert.equal(
        nodeType.get(u), nodeType.get(v),
        `pHard=1 negative ${u}-${v} must be same-type`,
      );
    }
  });
  ...
  it("rejects every pair already in edgeSet", () => {
    // Saturate type X completely: a-b, a-c, b-c are all real.
    const edgeSet = new Set([ek("a", "b"), ek("a", "c"), ek("b", "c")]);
    const negs = sampleStratifiedNegativeEdges(ids, edgeSet, 10, mulberry32(5), {
      nodeType, typeMarginal, pHard: 1.0,
    });
    for (const { u, v } of negs) {
      assert.ok(!edgeSet.has(ek(u, v)), `${u}-${v} is a real edge — must be rejected`);
    }
  });

  it("bounded: saturated graph returns fewer than count, never loops forever", () => {
    // 2-node single-type graph with the only non-self pair already real ->
    // ZERO valid negatives possible. Must terminate + return [].
    const tinyIds = ["a", "b"];
  ...
  it("NaN pHard -> falls back to 0.7 default (adversarial input)", () => {
    const negs = sampleStratifiedNegativeEdges(ids, new Set(), 5, mulberry32(4), {
      nodeType, typeMarginal, pHard: NaN,
    });
    assert.equal(negs.length, 5, "NaN pHard must not break sampling");
  });

  it("Infinity pHard -> not finite, falls back to 0.7 default (adversarial input)", () => {
    // Infinity is a number but Number.isFinite(Infinity) === false, so it
    // fails the finite-guard exactly like NaN and the safe 0.7 default is
    // used. The sampler must still fill the requested count.
    const negs = sampleStratifiedNegativeEdges(ids, new Set(), 6, mulberry32(8), {
      nodeType, typeMarginal, pHard: Infinity,
    });
    assert.equal(negs.length, 6, "Infinity pHard must not break sampling");
    // 0.7 default -> a mix of intra + cross type is expected; just confirm
    // every negative is a valid distinct non-self pair.
    for (const { u, v } of negs) assert.notEqual(u, v);
  });
  ...
  it("no ids carry a type -> legacy fallback (empty buckets)", () => {
    const orphanType = new Map([["zz", "Q"]]); // none of `ids` are in here
    const edgeSet = new Set();
    const legacy = sampleNegativeEdges(ids, edgeSet, 4, mulberry32(6));
    const strat = sampleStratifiedNegativeEdges(ids, edgeSet, 4, mulberry32(6), {
      nodeType: orphanType, typeMarginal: new Map([["Q", 1]]),
    });
    assert.deepEqual(strat, legacy);
  });
...
describe("train — stratified negative sampling integration", () => {
  /** A 2-community graph: X-clique {a,b,c}, Y-clique {d,e,f}, one bridge c-d. */
  function twoCommunitySetup() {
    const model = createModel({ inputDim: 3, hiddenDim: 6, embedDim: 4, seed: 5 });
    const adj = new Map([
      ["a", ["b", "c"]], ["b", ["a", "c"]], ["c", ["a", "b", "d"]],
      ["d", ["c", "e", "f"]], ["e", ["d", "f"]], ["f", ["d", "e"]],
    ]);
    const feat = new Map([
      ["a", [0.5, 0.1, 0.9]], ["b", [0.2, 0.8, 0.3]], ["c", [0.7, 0.4, 0.1]],
      ["d", [0.1, 0.6, 0.5]], ["e", [0.9, 0.2, 0.4]], ["f", [0.3, 0.7, 0.6]],
    ]);
    const nodeType = new Map([
      ["a", "X"], ["b", "X"], ["c", "X"],
      ["d", "Y"], ["e", "Y"], ["f", "Y"],
    ]);
    return { model, adj, feat, nodeType };
  }

  it("legacy parity: omitting nodeType is byte-identical to before", () => {
    // Two independent runs, one with nodeType:undefined explicitly — both must
    // match the bare-options run. This is the back-compat invariant.
    const baseOpts = { epochs: 8, batchSize: 6, seed: 3 };
    const { model: m1, adj, feat } = twoCommunitySetup();
    const bare = train(m1, adj, feat, { ...baseOpts });
    const { model: m2, adj: adj2, feat: feat2 } = twoCommunitySetup();
    const explicitUndef = train(m2, adj2, feat2, { ...baseOpts, nodeType: undefined });
    assert.deepEqual(explicitUndef.lossHistory, bare.lossHistory);
  });

  it("empty nodeType Map -> legacy uniform path (byte-identical)", () => {
    const baseOpts = { epochs: 8, batchSize: 6, seed: 3 };
    const { model: m1, adj, feat } = twoCommunitySetup();
    const bare = train(m1, adj, feat, { ...baseOpts });
    const { model: m2, adj: adj2, feat: feat2 } = twoCommunitySetup();
    const emptyType = train(m2, adj2, feat2, { ...baseOpts, nodeType: new Map() });
    assert.deepEqual(emptyType.lossHistory, bare.lossHistory);
  });

  it("with nodeType -> stratified path active, model trains (loss finite)", () => {
    const { model, adj, feat, nodeType } = twoCommunitySetup();
    const res = train(model, adj, feat, {
      epochs: 20, batchSize: 6, seed: 3, nodeType, negPHard: 0.7,
    });
    assert.equal(res.trained, true);
    assert.equal(res.epochs, 20);
    assert.ok(Number.isFinite(res.finalLoss), `finalLoss ${res.finalLoss} must be finite`);
    assert.equal(res.lossHistory.length, 20);
  });

  it("stratified path changes the trajectory vs uniform (the wiring fires)", () => {
    // Same seed, same graph — only nodeType differs. The negative-sample
    // sequence differs, so the loss trajectory must NOT be byte-identical.
    // (Proves train() actually routes through the stratified sampler.)
    const baseOpts = { epochs: 12, batchSize: 6, seed: 3 };
    const { model: m1, adj, feat } = twoCommunitySetup();
    const uniform = train(m1, adj, feat, { ...baseOpts });
    const { model: m2, adj: adj2, feat: feat2, nodeType } = twoCommunitySetup();
    const strat = train(m2, adj2, feat2, { ...baseOpts, nodeType });
    assert.notDeepEqual(strat.lossHistory, uniform.lossHistory);
  });

  it("stratified training still reduces loss (learning is not broken)", () => {
    const { model, adj, feat, nodeType } = twoCommunitySetup();
    const res = train(model, adj, feat, {
      epochs: 60, batchSize: 6, seed: 3, nodeType, negPHard: 0.7,
    });
    const first3 = mean(res.lossHistory.slice(0, 3));
    const last3 = mean(res.lossHistory.slice(-3));
    assert.ok(last3 < first3, `stratified loss must fall: first3=${first3} last3=${last3}`);
  });

  it("negPHard is honored: pHard=1 trains without error (intra-type negatives)", () => {
    const { model, adj, feat, nodeType } = twoCommunitySetup();
    const res = train(model, adj, feat, {
      epochs: 15, batchSize: 6, seed: 3, nodeType, negPHard: 1.0,
    });
    assert.equal(res.trained, true);
    assert.ok(Number.isFinite(res.finalLoss));
  });
});
