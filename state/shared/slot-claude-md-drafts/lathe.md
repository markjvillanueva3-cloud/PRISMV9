...
`threadingPipelineDispatcher.ts` — // UNVERIFIED (not found in `src/tools/dispatchers/`).

**MCP-down fallback:** `node scripts/lib/lathe-gcode-lint.mjs` // UNVERIFIED (script not found at that path).
...
2. **Boring-bar deflection scales L^4/D^4** — a 5:1 L/D steel bar deflects ~20× a 2:1 ratio.
...
- `threadingPipelineDispatcher.ts` — referenced in assessment; NOT found in `src/tools/dispatchers/`. Verify before use.
- `scripts/lib/lathe-gcode-lint.mjs` — cited in assessment with 28 tests; NOT found on disk. Verify or build.
