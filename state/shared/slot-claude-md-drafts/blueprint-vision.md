...
3. **VLM JSON dropout (~30-37% prints lost):** `qwen2.5vl:7b` hits `num_predict:4096` mid-structure ->
   malformed JSON -> dropout. Fix: `format:"json"` in `scripts/lib/ollama-vision-extract-lib.mjs:408`.
   NOT YET BUILT -- requires A/B gate + 3-of-3 scrutiny before merge (OCR = safety-relevant).
   `llama3.2-vision:11b` returns empty 100% (0/32 prints, tested 2026-06-10) -- DO NOT re-add it.
...
- **VLM JSON dropout (~30-37% prints):** `format:"json"` fix at `ollama-vision-extract-lib.mjs:408` NOT YET BUILT -- highest ROI open fix. [[reference_xray_ocr_yield_mechanics_2026_06_10]]
