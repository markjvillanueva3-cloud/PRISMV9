# Galaxy buildout — slot:lima (galaxy:academy)

> **Auto-fire trigger:** this file is THE first prompt slot:lima executes on fresh-launch when `mcp-server/src/engines/academy/` is missing OR incomplete.
> **Master protocol:** `state/shared/specs/PER-SLOT-GALAXY-BUILD-KIT.md`

## Your mission (per operator goal 2026-05-28)

Build slot:lima's **complete galaxy** so future sessions get the fullest possible context with the lowest possible token cost. The 11 artifacts below are mandatory. Sister slots will NOT build them for you.

| # | Artifact | Path |
|---|----------|------|
| 1 | Soul (realigned) | `state/shared/slot-souls/lima.md` |
| 2 | Galaxy CLAUDE.md | `mcp-server/src/engines/academy/CLAUDE.md` |
| 3 | Galaxy MEMORY.md | `mcp-server/src/engines/academy/MEMORY.md` |
| 4 | H:/-wide PATHS.md | `mcp-server/src/engines/academy/PATHS.md` |
| 5 | TOOLBELT.md | `mcp-server/src/engines/academy/TOOLBELT.md` |
| 6 | Wiki bridges (≥3) | `knowledge/wiki/architecture/<domain>-*.md` |
| 7 | Tribal tips (≥5) | via `prism_knowledge:tribal_capture` slot=lima |
| 8 | Custom skills (≥1) | `.claude/commands/<verb>-lima.md` |
| 9 | Custom hooks (≥1 if domain warrants) | `.claude/hooks/<slot>-*.mjs` + wire in `settings.json` |
| 10 | Master-brain memory (≥10 entries) | `C:/Users/wompu/.claude/projects/H--prism/memory/reference_lima_*.md` |
| 11 | PSN edges declared in CLAUDE.md `## Related galaxies` | symmetric — peer galaxies mention you back |

## Pre-flight

```bash
pwd                        # expect: H:/prism-slot-lima
git branch --show-current  # expect: slot/lima
```

If you're in shared `H:/prism` instead, `/checkin-lima` did NOT cut over. Run `cd H:/prism-slot-lima && git checkout slot/lima` first.

If `engines/academy/CLAUDE.md` already exists AND has all 11 artifacts above, skip this brief; run `/checkin-lima` for normal work pickup.

## STEP 1 — Soul realignment (5 min)

Read `state/shared/slot-souls/lima.md`. If frontmatter is generic-stub (`role: work`, `voice: direct`, `domain_filter: any`), **rewrite** with domain-specific values:
...
```bash
mkdir -p mcp-server/src/engines/academy
```
...
Verify `SLOT_GALAXY_MAP` in `.claude/hooks/slot-context-bundle-inject.mjs` includes `lima: 'academy',` — add if missing.

## STEP 3 — Domain inventory (15 min — DISPATCH PARALLEL AGENTS)

Send **one parallel `Explore` agent per surface** in a single message:

Keywords for slot:lima: `academy`, `course`, `mit-ocw`, `pdf-corpus`, `curriculum`, `lesson`

```
Agent({subagent_type: 'general-purpose', description: 'enumerate academy engines+dispatchers',
  prompt: 'Find every engine + dispatcher action related to academy. Use prism_session:master_index_query keyword="academy" + dispatcher_map_compact. Return: engine paths + dispatcher.action pairs + 1-line role each.'})
Agent({subagent_type: 'general-purpose', description: 'enumerate academy skills',
  prompt: 'Glob ~/.claude/commands/*.md and .claude/commands/*.md for skills matching keywords ["academy","course","mit-ocw","pdf-corpus","curriculum","lesson"]. Return: skill name + 1-line description.'})
Agent({subagent_type: 'general-purpose', description: 'enumerate academy hooks',
  prompt: 'Glob .claude/hooks/*.mjs filtered to keywords ["academy","course","mit-ocw","pdf-corpus","curriculum","lesson"]. Return: hook name + event (PreToolUse/UserPromptSubmit/Stop) + purpose.'})
Agent({subagent_type: 'general-purpose', description: 'enumerate academy memories+wiki+tribal',
  prompt: 'Run prism_memory:semantic_search query="academy course mit-ocw" topK=20 + prism_knowledge:search topic="academy" + prism_knowledge:tribal_search slot=lima. Return top 10 of each by relevance + recency.'})
```
...
This is THE HIGHEST-ROI artifact. Converts every future Grep/Glob from O(N) → O(1) for slot:lima.

Seed roots (start here, walk outward):
- `H:/prism/mcp-server/src/engines/academy/`
- `H:/PRISM/extracted/mit-ocw/`
- `H:/prism/mcp-server/src/engines/pdf-corpus/`
...
```
prism_memory:semantic_search query="academy course mit-ocw" topK=20
```
...
For each non-obvious learning your domain knows that ISN'T already in the memory store, write `C:/Users/wompu/.claude/projects/H--prism/memory/<type>_lima_<topic>.md` where `<type>` is one of: `feedback` (standing rule), `reference` (point-in-time fact), `project` (decision/why).
...
Add the top-10 pulled master memories under `## High-ROI memories` (galaxy side) so future slot:lima sessions find them fast.
...
```
[galaxy:academy] mcp-server/src/engines/academy/MEMORY.md — <one-line domain summary> (slot:lima, 2026-05-28)
```
...
Document the exact Grep/Glob/Bash/Read/git patterns slot:lima reaches for most. Memoize the regex + path so future sessions don't re-derive them.
...
Either write new entries OR cross-link existing ones in CLAUDE.md `## Wiki cross-refs`. Format: `[[architecture/<domain>-<topic>]]` or `[[lessons/<class>]]`. Query existing first: `prism_knowledge:search topic="academy"`.

If your domain lacks a wiki entry on a load-bearing topic, write one at `H:/prism/knowledge/wiki/architecture/academy-<topic>.md` (or `lessons/<class>.md`). Standard frontmatter + sections (see `H:/prism/WIKI_SCHEMA.md`).

### 7b. Tribal tip injection (≥5 entries)
Capture domain-specific tribal knowledge via:
```
prism_knowledge:tribal_capture {slot: 'lima', tip: '<one-line tip>', context: '<when applies>', citation: '<source>'}
```

These auto-surface via `tribal-by-domain-inject` hook on every UserPromptSubmit for slot:lima. Target: 5-15 tips covering the non-obvious wisdom only slot:lima knows.

## STEP 8 — Custom skills + hooks (10 min)

### 8a. Custom skill (≥1)
If slot:lima runs the same multi-step workflow repeatedly, capture it as `.claude/commands/<verb>-lima.md` (e.g. `/audit-lima`, `/sweep-lima`). Skill template: any short skill under `.claude/commands/` (e.g. `/reap-zombies` is 30 lines).

### 8b. Custom hook (≥1 if domain warrants)
If slot:lima would benefit from auto-firing on a tool-call event (e.g. inject a domain-specific reminder on every Bash), create `.claude/hooks/lima-<purpose>.mjs` and wire it in `.claude/settings.json` under the right event matcher.

Hooks for THIS slot's domain should be additive — NEVER disable a fleet-wide hook.

## STEP 9 — Master graphs + index (5 min)

Make slot:lima findable in the master indexes:
...
- Verify your galaxy appears in `prism_session:master_index_query keyword=academy` returns.
...
```bash
ls H:/prism/knowledge/memories/feedback/feedback_lima_*.md 2>/dev/null
ls H:/prism/knowledge/memories/reference/reference_lima_*.md 2>/dev/null
```

If empty, write at least one `feedback_lima_<topic>.md` in the auto-memory dir — Stop hook will feed it next session-end.

### 10c. PSN 11-leg verification (per [[feedback_psn_definition]])
Slot:lima's galaxy should be discoverable on each of the 11 PSN legs:

1. **Obsidian brain** — `grep -l "lima" H:/prism/knowledge/memories/**/*.md`
...
5. **Tribal** — ≥5 tribal tips slot-tagged lima
...
```bash
git add mcp-server/src/engines/academy/
git add state/shared/slot-souls/lima.md
...
git add .claude/hooks/lima-*.mjs                    # if custom hook added
git add .claude/settings.json                          # if hook wired
git add knowledge/wiki/architecture/                   # if new wiki entry

git commit -m "[lima] [PER-SLOT-GALAXY-BUILDOUT]/U-PSGB-LIMA: full galaxy — soul + 4 galaxy files + ≥10 memories + ≥3 wiki + ≥5 tribal + skill + PSN edges"
...
```bash
G=mcp-server/src/engines/academy
S=state/shared/slot-souls/lima.md
...
grep -q "^\s*lima:" .claude/hooks/slot-context-bundle-inject.mjs || echo "FAIL 6: not in SLOT_GALAXY_MAP"
...
ls C:/Users/wompu/.claude/projects/H--prism/memory/*_lima_*.md 2>/dev/null | head -1 || echo "FAIL 9: no auto-memory entries"
grep -rl "lima" knowledge/wiki/architecture/ knowledge/wiki/lessons/ 2>/dev/null | head -3 | wc -l | grep -q "^[3-9]\|^[1-9][0-9]" || echo "FAIL 10: <3 wiki refs"
ls .claude/commands/*lima*.md 2>/dev/null | head -1 || echo "FAIL 11: no custom skill"
grep -q "galaxy:academy" C:/Users/wompu/.claude/projects/H--prism/memory/MEMORY.md || echo "FAIL 12: master MEMORY.md has no [galaxy:academy] back-pointer — master index is blind to this brain (run STEP 5d)"
...
`slot-context-bundle-inject` auto-loads your galaxy on every future UserPromptSubmit for slot:lima. Future sessions skip this brief automatically (galaxy-buildout-detect sees `engines/academy/CLAUDE.md` exists).

Run `/checkin-lima` to pick the next normal-work unit.
