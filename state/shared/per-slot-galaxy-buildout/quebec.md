# Galaxy buildout — slot:quebec (galaxy:frontend-app)
...
Build slot:quebec's **complete galaxy** so future sessions get the fullest possible context with the lowest possible token cost. The 11 artifacts below are mandatory. Sister slots will NOT build them for you.

| # | Artifact | Path |
|---|----------|------|
| 1 | Soul (realigned) | `state/shared/slot-souls/quebec.md` |
| 2 | Galaxy CLAUDE.md | `mcp-server/src/engines/frontend-app/CLAUDE.md` |
| 3 | Galaxy MEMORY.md | `mcp-server/src/engines/frontend-app/MEMORY.md` |
| 4 | H:/-wide PATHS.md | `mcp-server/src/engines/frontend-app/PATHS.md` |
| 5 | TOOLBELT.md | `mcp-server/src/engines/frontend-app/TOOLBELT.md` |
| 6 | Wiki bridges (≥3) | `knowledge/wiki/architecture/<domain>-*.md` |
| 7 | Tribal tips (≥5) | via `prism_knowledge:tribal_capture` slot=quebec |
| 8 | Custom skills (≥1) | `.claude/commands/<verb>-quebec.md` |
| 9 | Custom hooks (≥1 if domain warrants) | `.claude/hooks/<slot>-*.mjs` + wire in `settings.json` |
| 10 | Master-brain memory (≥10 entries) | `C:/Users/wompu/.claude/projects/H--prism/memory/reference_quebec_*.md` |
| 11 | PSN edges declared in CLAUDE.md `## Related galaxies` | symmetric — peer galaxies mention you back |

## Pre-flight

```bash
pwd                        # expect: H:/prism-slot-quebec
git branch --show-current  # expect: slot/quebec
```

If you're in shared `H:/prism` instead, `/checkin-quebec` did NOT cut over. Run `cd H:/prism-slot-quebec && git checkout slot/quebec` first.

If `engines/frontend-app/CLAUDE.md` already exists AND has all 11 artifacts above, skip this brief; run `/checkin-quebec` for normal work pickup.

## STEP 1 — Soul realignment (5 min)

Read `state/shared/slot-souls/quebec.md`. If frontmatter is generic-stub (`role: work`, `voice: direct`, `domain_filter: any`), **rewrite** with domain-specific values:
...
```bash
mkdir -p mcp-server/src/engines/frontend-app
```
...
Verify `SLOT_GALAXY_MAP` in `.claude/hooks/slot-context-bundle-inject.mjs` includes `quebec: 'frontend-app',` — add if missing.

## STEP 3 — Domain inventory (15 min — DISPATCH PARALLEL AGENTS)

Send **one parallel `Explore` agent per surface** in a single message:

Keywords for slot:quebec: `frontend`, `react`, `nextjs`, `phone-app`, `web-app`, `ui`

```
Agent({subagent_type: 'general-purpose', description: 'enumerate frontend-app engines+dispatchers',
  prompt: 'Find every engine + dispatcher action related to frontend-app. Use prism_session:master_index_query keyword="frontend" + dispatcher_map_compact. Return: engine paths + dispatcher.action pairs + 1-line role each.'})
Agent({subagent_type: 'general-purpose', description: 'enumerate frontend-app skills',
  prompt: 'Glob ~/.claude/commands/*.md and .claude/commands/*.md for skills matching keywords ["frontend","react","nextjs","phone-app","web-app","ui"]. Return: skill name + 1-line description.'})
Agent({subagent_type: 'general-purpose', description: 'enumerate frontend-app hooks',
  prompt: 'Glob .claude/hooks/*.mjs filtered to keywords ["frontend","react","nextjs","phone-app","web-app","ui"]. Return: hook name + event (PreToolUse/UserPromptSubmit/Stop) + purpose.'})
Agent({subagent_type: 'general-purpose', description: 'enumerate frontend-app memories+wiki+tribal',
  prompt: 'Run prism_memory:semantic_search query="frontend react nextjs" topK=20 + prism_knowledge:search topic="frontend-app" + prism_knowledge:tribal_search slot=quebec. Return top 10 of each by relevance + recency.'})
```
...
This is THE HIGHEST-ROI artifact. Converts every future Grep/Glob from O(N) → O(1) for slot:quebec.

Seed roots (start here, walk outward):
- `H:/prism/mcp-server/src/engines/frontend-app/`
...
```
prism_memory:semantic_search query="frontend react nextjs" topK=20
```
...
For each non-obvious learning your domain knows that ISN'T already in the memory store, write `C:/Users/wompu/.claude/projects/H--prism/memory/<type>_quebec_<topic>.md` where `<type>` is one of: `feedback` (standing rule), `reference` (point-in-time fact), `project` (decision/why).
...
Add the top-10 pulled master memories under `## High-ROI memories` (galaxy side) so future slot:quebec sessions find them fast.
...
```
[galaxy:frontend-app] mcp-server/src/engines/frontend-app/MEMORY.md — <one-line domain summary> (slot:quebec, 2026-05-28)
```
...
Document the exact Grep/Glob/Bash/Read/git patterns slot:quebec reaches for most. Memoize the regex + path so future sessions don't re-derive them.
...
Either write new entries OR cross-link existing ones in CLAUDE.md `## Wiki cross-refs`. Format: `[[architecture/<domain>-<topic>]]` or `[[lessons/<class>]]`. Query existing first: `prism_knowledge:search topic="frontend-app"`.

If your domain lacks a wiki entry on a load-bearing topic, write one at `H:/prism/knowledge/wiki/architecture/frontend-app-<topic>.md` (or `lessons/<class>.md`). Standard frontmatter + sections (see `H:/prism/WIKI_SCHEMA.md`).

### 7b. Tribal tip injection (≥5 entries)
Capture domain-specific tribal knowledge via:
```
prism_knowledge:tribal_capture {slot: 'quebec', tip: '<one-line tip>', context: '<when applies>', citation: '<source>'}
```

These auto-surface via `tribal-by-domain-inject` hook on every UserPromptSubmit for slot:quebec. Target: 5-15 tips covering the non-obvious wisdom only slot:quebec knows.

## STEP 8 — Custom skills + hooks (10 min)

### 8a. Custom skill (≥1)
If slot:quebec runs the same multi-step workflow repeatedly, capture it as `.claude/commands/<verb>-quebec.md` (e.g. `/audit-quebec`, `/sweep-quebec`). Skill template: any short skill under `.claude/commands/` (e.g. `/reap-zombies` is 30 lines).

### 8b. Custom hook (≥1 if domain warrants)
If slot:quebec would benefit from auto-firing on a tool-call event (e.g. inject a domain-specific reminder on every Bash), create `.claude/hooks/quebec-<purpose>.mjs` and wire it in `.claude/settings.json` under the right event matcher.

Hooks for THIS slot's domain should be additive — NEVER disable a fleet-wide hook.

## STEP 9 — Master graphs + index (5 min)

Make slot:quebec findable in the master indexes:
...
- Verify your galaxy appears in `prism_session:master_index_query keyword=frontend-app` returns.
...
```bash
ls H:/prism/knowledge/memories/feedback/feedback_quebec_*.md 2>/dev/null
ls H:/prism/knowledge/memories/reference/reference_quebec_*.md 2>/dev/null
```

If empty, write at least one `feedback_quebec_<topic>.md` in the auto-memory dir — Stop hook will feed it next session-end.

### 10c. PSN 11-leg verification (per [[feedback_psn_definition]])
Slot:quebec's galaxy should be discoverable on each of the 11 PSN legs:

1. **Obsidian brain** — `grep -l "quebec" H:/prism/knowledge/memories/**/*.md`
...
5. **Tribal** — ≥5 tribal tips slot-tagged quebec
...
```bash
git add mcp-server/src/engines/frontend-app/
git add state/shared/slot-souls/quebec.md
...
git add .claude/hooks/quebec-*.mjs                    # if custom hook added
git add .claude/settings.json                          # if hook wired
git add knowledge/wiki/architecture/                   # if new wiki entry

git commit -m "[quebec] [PER-SLOT-GALAXY-BUILDOUT]/U-PSGB-QUEBEC: full galaxy — soul + 4 galaxy files + ≥10 memories + ≥3 wiki + ≥5 tribal + skill + PSN edges"
...
```bash
G=mcp-server/src/engines/frontend-app
S=state/shared/slot-souls/quebec.md
...
grep -q "^\s*quebec:" .claude/hooks/slot-context-bundle-inject.mjs || echo "FAIL 6: not in SLOT_GALAXY_MAP"
...
ls C:/Users/wompu/.claude/projects/H--prism/memory/*_quebec_*.md 2>/dev/null | head -1 || echo "FAIL 9: no auto-memory entries"
grep -rl "quebec" knowledge/wiki/architecture/ knowledge/wiki/lessons/ 2>/dev/null | head -3 | wc -l | grep -q "^[3-9]\|^[1-9][0-9]" || echo "FAIL 10: <3 wiki refs"
ls .claude/commands/*quebec*.md 2>/dev/null | head -1 || echo "FAIL 11: no custom skill"
grep -q "galaxy:frontend-app" C:/Users/wompu/.claude/projects/H--prism/memory/MEMORY.md || echo "FAIL 12: master MEMORY.md has no [galaxy:frontend-app] back-pointer — master index is blind to this brain (run STEP 5d)"
...
`slot-context-bundle-inject` auto-loads your galaxy on every future UserPromptSubmit for slot:quebec. Future sessions skip this brief automatically (galaxy-buildout-detect sees `engines/frontend-app/CLAUDE.md` exists).

Run `/checkin-quebec` to pick the next normal-work unit.
