# Galaxy buildout — slot:zebra (galaxy:hermes-zebra)

> **Auto-fire trigger:** this file is THE first prompt slot:zebra executes on fresh-launch when `mcp-server/src/engines/hermes-zebra/` is missing OR incomplete.
> **Master protocol:** `state/shared/specs/PER-SLOT-GALAXY-BUILD-KIT.md`

## Your mission (per operator goal 2026-05-28)

Build slot:zebra's **complete galaxy** so future sessions get the fullest possible context with the lowest possible token cost. The 11 artifacts below are mandatory. Sister slots will NOT build them for you.

| # | Artifact | Path |
|---|----------|------|
| 1 | Soul (realigned) | `state/shared/slot-souls/zebra.md` |
...
| 7 | Tribal tips (≥5) | via `prism_knowledge:tribal_capture` slot=zebra |
| 8 | Custom skills (≥1) | `.claude/commands/<verb>-zebra.md` |
| 9 | Custom hooks (≥1 if domain warrants) | `.claude/hooks/<slot>-*.mjs` + wire in `settings.json` |
| 10 | Master-brain memory (≥10 entries) | `C:/Users/wompu/.claude/projects/H--prism/memory/reference_zebra_*.md` |
| 11 | PSN edges declared in CLAUDE.md `## Related galaxies` | symmetric — peer galaxies mention you back |

## Pre-flight

```bash
pwd                        # expect: H:/prism-slot-zebra
git branch --show-current  # expect: slot/zebra
```

If you're in shared `H:/prism` instead, `/checkin-zebra` did NOT cut over. Run `cd H:/prism-slot-zebra && git checkout slot/zebra` first.

If `engines/hermes-zebra/CLAUDE.md` already exists AND has all 11 artifacts above, skip this brief; run `/checkin-zebra` for normal work pickup.

## STEP 1 — Soul realignment (5 min)

Read `state/shared/slot-souls/zebra.md`. If frontmatter is generic-stub (`role: work`, `voice: direct`, `domain_filter: any`), **rewrite** with domain-specific values:
...
Verify `SLOT_GALAXY_MAP` in `.claude/hooks/slot-context-bundle-inject.mjs` includes `zebra: 'hermes-zebra',` — add if missing.

## STEP 3 — Domain inventory (15 min — DISPATCH PARALLEL AGENTS)

Send **one parallel `Explore` agent per surface** in a single message:

Keywords for slot:zebra: `hermes`, `orchestrator`, `agent-chat`, `fleet`
...
Agent({subagent_type: 'general-purpose', description: 'enumerate hermes-zebra skills',
  prompt: 'Glob ~/.claude/commands/*.md and .claude/commands/*.md for skills matching keywords ["hermes","orchestrator","agent-chat","fleet"]. Return: skill name + 1-line description.'})
Agent({subagent_type: 'general-purpose', description: 'enumerate hermes-zebra hooks',
  prompt: 'Glob .claude/hooks/*.mjs filtered to keywords ["hermes","orchestrator","agent-chat","fleet"]. Return: hook name + event (PreToolUse/UserPromptSubmit/Stop) + purpose.'})
Agent({subagent_type: 'general-purpose', description: 'enumerate hermes-zebra memories+wiki+tribal',
  prompt: 'Run prism_memory:semantic_search query="hermes orchestrator agent-chat" topK=20 + prism_knowledge:search topic="hermes-zebra" + prism_knowledge:tribal_search slot=zebra. Return top 10 of each by relevance + recency.'})
```
...
This is THE HIGHEST-ROI artifact. Converts every future Grep/Glob from O(N) → O(1) for slot:zebra.

Seed roots (start here, walk outward):
- `H:/prism/mcp-server/src/engines/hermes-zebra/`
- `H:/prism/state/shared/zebra-orchestrator/`
...
```
prism_memory:semantic_search query="hermes orchestrator agent-chat" topK=20
```
...
For each non-obvious learning your domain knows that ISN'T already in the memory store, write `C:/Users/wompu/.claude/projects/H--prism/memory/<type>_zebra_<topic>.md` where `<type>` is one of: `feedback` (standing rule), `reference` (point-in-time fact), `project` (decision/why).
...
Add the top-10 pulled master memories under `## High-ROI memories` (galaxy side) so future slot:zebra sessions find them fast.
...
```
[galaxy:hermes-zebra] mcp-server/src/engines/hermes-zebra/MEMORY.md — <one-line domain summary> (slot:zebra, 2026-05-28)
```
...
Document the exact Grep/Glob/Bash/Read/git patterns slot:zebra reaches for most. Memoize the regex + path so future sessions don't re-derive them.
...
```
prism_knowledge:tribal_capture {slot: 'zebra', tip: '<one-line tip>', context: '<when applies>', citation: '<source>'}
```

These auto-surface via `tribal-by-domain-inject` hook on every UserPromptSubmit for slot:zebra. Target: 5-15 tips covering the non-obvious wisdom only slot:zebra knows.

## STEP 8 — Custom skills + hooks (10 min)

### 8a. Custom skill (≥1)
If slot:zebra runs the same multi-step workflow repeatedly, capture it as `.claude/commands/<verb>-zebra.md` (e.g. `/audit-zebra`, `/sweep-zebra`). Skill template: any short skill under `.claude/commands/` (e.g. `/reap-zombies` is 30 lines).

### 8b. Custom hook (≥1 if domain warrants)
If slot:zebra would benefit from auto-firing on a tool-call event (e.g. inject a domain-specific reminder on every Bash), create `.claude/hooks/zebra-<purpose>.mjs` and wire it in `.claude/settings.json` under the right event matcher.

Hooks for THIS slot's domain should be additive — NEVER disable a fleet-wide hook.

## STEP 9 — Master graphs + index (5 min)

Make slot:zebra findable in the master indexes:
...
```bash
ls H:/prism/knowledge/memories/feedback/feedback_zebra_*.md 2>/dev/null
ls H:/prism/knowledge/memories/reference/reference_zebra_*.md 2>/dev/null
```

If empty, write at least one `feedback_zebra_<topic>.md` in the auto-memory dir — Stop hook will feed it next session-end.

### 10c. PSN 11-leg verification (per [[feedback_psn_definition]])
Slot:zebra's galaxy should be discoverable on each of the 11 PSN legs:

1. **Obsidian brain** — `grep -l "zebra" H:/prism/knowledge/memories/**/*.md`
...
5. **Tribal** — ≥5 tribal tips slot-tagged zebra
...
git add state/shared/slot-souls/zebra.md
...
git add .claude/hooks/zebra-*.mjs                    # if custom hook added
git add .claude/settings.json                          # if hook wired
git add knowledge/wiki/architecture/                   # if new wiki entry

git commit -m "[zebra] [PER-SLOT-GALAXY-BUILDOUT]/U-PSGB-ZEBRA: full galaxy — soul + 4 galaxy files + ≥10 memories + ≥3 wiki + ≥5 tribal + skill + PSN edges"
...
S=state/shared/slot-souls/zebra.md
...
grep -q "^\s*zebra:" .claude/hooks/slot-context-bundle-inject.mjs || echo "FAIL 6: not in SLOT_GALAXY_MAP"
...
ls C:/Users/wompu/.claude/projects/H--prism/memory/*_zebra_*.md 2>/dev/null | head -1 || echo "FAIL 9: no auto-memory entries"
grep -rl "zebra" knowledge/wiki/architecture/ knowledge/wiki/lessons/ 2>/dev/null | head -3 | wc -l | grep -q "^[3-9]\|^[1-9][0-9]" || echo "FAIL 10: <3 wiki refs"
ls .claude/commands/*zebra*.md 2>/dev/null | head -1 || echo "FAIL 11: no custom skill"
...
`slot-context-bundle-inject` auto-loads your galaxy on every future UserPromptSubmit for slot:zebra. Future sessions skip this brief automatically (galaxy-buildout-detect sees `engines/hermes-zebra/CLAUDE.md` exists).

Run `/checkin-zebra` to pick the next normal-work unit.
