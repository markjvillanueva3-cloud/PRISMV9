# Galaxy buildout — slot:victor (galaxy:dormant-data)

> **Auto-fire trigger:** this file is THE first prompt slot:victor executes on fresh-launch when `mcp-server/src/engines/dormant-data/` is missing OR incomplete.
> **Master protocol:** `state/shared/specs/PER-SLOT-GALAXY-BUILD-KIT.md`

## Your mission (per operator goal 2026-05-28)

Build slot:victor's **complete galaxy** so future sessions get the fullest possible context with the lowest possible token cost. The 11 artifacts below are mandatory. Sister slots will NOT build them for you.

| # | Artifact | Path |
|---|----------|------|
| 1 | Soul (realigned) | `state/shared/slot-souls/victor.md` |
| 2 | Galaxy CLAUDE.md | `mcp-server/src/engines/dormant-data/CLAUDE.md` |
| 3 | Galaxy MEMORY.md | `mcp-server/src/engines/dormant-data/MEMORY.md` |
| 4 | H:/-wide PATHS.md | `mcp-server/src/engines/dormant-data/PATHS.md` |
| 5 | TOOLBELT.md | `mcp-server/src/engines/dormant-data/TOOLBELT.md` |
| 6 | Wiki bridges (≥3) | `knowledge/wiki/architecture/<domain>-*.md` |
| 7 | Tribal tips (≥5) | via `prism_knowledge:tribal_capture` slot=victor |
| 8 | Custom skills (≥1) | `.claude/commands/<verb>-victor.md` |
| 9 | Custom hooks (≥1 if domain warrants) | `.claude/hooks/<slot>-*.mjs` + wire in `settings.json` |
| 10 | Master-brain memory (≥10 entries) | `C:/Users/wompu/.claude/projects/H--prism/memory/reference_victor_*.md` |
| 11 | PSN edges declared in CLAUDE.md `## Related galaxies` | symmetric — peer galaxies mention you back |

## Pre-flight

```bash
pwd                        # expect: H:/prism-slot-victor
git branch --show-current  # expect: slot/victor
```

If you're in shared `H:/prism` instead, `/checkin-victor` did NOT cut over. Run `cd H:/prism-slot-victor && git checkout slot/victor` first.

If `engines/dormant-data/CLAUDE.md` already exists AND has all 11 artifacts above, skip this brief; run `/checkin-victor` for normal work pickup.

## STEP 1 — Soul realignment (5 min)

Read `state/shared/slot-souls/victor.md`. If frontmatter is generic-stub (`role: work`, `voice: direct`, `domain_filter: any`), **rewrite** with domain-specific values:
...
```bash
mkdir -p mcp-server/src/engines/dormant-data
```
...
Verify `SLOT_GALAXY_MAP` in `.claude/hooks/slot-context-bundle-inject.mjs` includes `victor: 'dormant-data',` — add if missing.

## STEP 3 — Domain inventory (15 min — DISPATCH PARALLEL AGENTS)

Send **one parallel `Explore` agent per surface** in a single message:

Keywords for slot:victor: `extracted`, `extracted_modules`, `dormant`, `unused`, `unwired-data`

```
Agent({subagent_type: 'general-purpose', description: 'enumerate dormant-data engines+dispatchers',
  prompt: 'Find every engine + dispatcher action related to dormant-data. Use prism_session:master_index_query keyword="extracted" + dispatcher_map_compact. Return: engine paths + dispatcher.action pairs + 1-line role each.'})
Agent({subagent_type: 'general-purpose', description: 'enumerate dormant-data skills',
  prompt: 'Glob ~/.claude/commands/*.md and .claude/commands/*.md for skills matching keywords ["extracted","extracted_modules","dormant","unused","unwired-data"]. Return: skill name + 1-line description.'})
Agent({subagent_type: 'general-purpose', description: 'enumerate dormant-data hooks',
  prompt: 'Glob .claude/hooks/*.mjs filtered to keywords ["extracted","extracted_modules","dormant","unused","unwired-data"]. Return: hook name + event (PreToolUse/UserPromptSubmit/Stop) + purpose.'})
Agent({subagent_type: 'general-purpose', description: 'enumerate dormant-data memories+wiki+tribal',
  prompt: 'Run prism_memory:semantic_search query="extracted extracted_modules dormant" topK=20 + prism_knowledge:search topic="dormant-data" + prism_knowledge:tribal_search slot=victor. Return top 10 of each by relevance + recency.'})
```
...
This is THE HIGHEST-ROI artifact. Converts every future Grep/Glob from O(N) → O(1) for slot:victor.

Seed roots (start here, walk outward):
- `H:/PRISM/extracted/`
- `H:/PRISM/extracted_modules/`
- `H:/prism/mcp-server/src/engines/dormant-data/`
...
```
prism_memory:semantic_search query="extracted extracted_modules dormant" topK=20
```
...
For each non-obvious learning your domain knows that ISN'T already in the memory store, write `C:/Users/wompu/.claude/projects/H--prism/memory/<type>_victor_<topic>.md` where `<type>` is one of: `feedback` (standing rule), `reference` (point-in-time fact), `project` (decision/why).
...
Add the top-10 pulled master memories under `## High-ROI memories` (galaxy side) so future slot:victor sessions find them fast.
...
```
[galaxy:dormant-data] mcp-server/src/engines/dormant-data/MEMORY.md — <one-line domain summary> (slot:victor, 2026-05-28)
```
...
Document the exact Grep/Glob/Bash/Read/git patterns slot:victor reaches for most. Memoize the regex + path so future sessions don't re-derive them.
...
Either write new entries OR cross-link existing ones in CLAUDE.md `## Wiki cross-refs`. Format: `[[architecture/<domain>-<topic>]]` or `[[lessons/<class>]]`. Query existing first: `prism_knowledge:search topic="dormant-data"`.

If your domain lacks a wiki entry on a load-bearing topic, write one at `H:/prism/knowledge/wiki/architecture/dormant-data-<topic>.md` (or `lessons/<class>.md`). Standard frontmatter + sections (see `H:/prism/WIKI_SCHEMA.md`).

### 7b. Tribal tip injection (≥5 entries)
Capture domain-specific tribal knowledge via:
```
prism_knowledge:tribal_capture {slot: 'victor', tip: '<one-line tip>', context: '<when applies>', citation: '<source>'}
```

These auto-surface via `tribal-by-domain-inject` hook on every UserPromptSubmit for slot:victor. Target: 5-15 tips covering the non-obvious wisdom only slot:victor knows.

## STEP 8 — Custom skills + hooks (10 min)

### 8a. Custom skill (≥1)
If slot:victor runs the same multi-step workflow repeatedly, capture it as `.claude/commands/<verb>-victor.md` (e.g. `/audit-victor`, `/sweep-victor`). Skill template: any short skill under `.claude/commands/` (e.g. `/reap-zombies` is 30 lines).

### 8b. Custom hook (≥1 if domain warrants)
If slot:victor would benefit from auto-firing on a tool-call event (e.g. inject a domain-specific reminder on every Bash), create `.claude/hooks/victor-<purpose>.mjs` and wire it in `.claude/settings.json` under the right event matcher.

Hooks for THIS slot's domain should be additive — NEVER disable a fleet-wide hook.

## STEP 9 — Master graphs + index (5 min)

Make slot:victor findable in the master indexes:
...
- Verify your galaxy appears in `prism_session:master_index_query keyword=dormant-data` returns.
...
```bash
ls H:/prism/knowledge/memories/feedback/feedback_victor_*.md 2>/dev/null
ls H:/prism/knowledge/memories/reference/reference_victor_*.md 2>/dev/null
```

If empty, write at least one `feedback_victor_<topic>.md` in the auto-memory dir — Stop hook will feed it next session-end.

### 10c. PSN 11-leg verification (per [[feedback_psn_definition]])
Slot:victor's galaxy should be discoverable on each of the 11 PSN legs:

1. **Obsidian brain** — `grep -l "victor" H:/prism/knowledge/memories/**/*.md`
...
5. **Tribal** — ≥5 tribal tips slot-tagged victor
...
```bash
git add mcp-server/src/engines/dormant-data/
git add state/shared/slot-souls/victor.md
...
git add .claude/hooks/victor-*.mjs                    # if custom hook added
git add .claude/settings.json                          # if hook wired
git add knowledge/wiki/architecture/                   # if new wiki entry

git commit -m "[victor] [PER-SLOT-GALAXY-BUILDOUT]/U-PSGB-VICTOR: full galaxy — soul + 4 galaxy files + ≥10 memories + ≥3 wiki + ≥5 tribal + skill + PSN edges"
...
```bash
G=mcp-server/src/engines/dormant-data
S=state/shared/slot-souls/victor.md
...
grep -q "^\s*victor:" .claude/hooks/slot-context-bundle-inject.mjs || echo "FAIL 6: not in SLOT_GALAXY_MAP"
...
ls C:/Users/wompu/.claude/projects/H--prism/memory/*_victor_*.md 2>/dev/null | head -1 || echo "FAIL 9: no auto-memory entries"
grep -rl "victor" knowledge/wiki/architecture/ knowledge/wiki/lessons/ 2>/dev/null | head -3 | wc -l | grep -q "^[3-9]\|^[1-9][0-9]" || echo "FAIL 10: <3 wiki refs"
ls .claude/commands/*victor*.md 2>/dev/null | head -1 || echo "FAIL 11: no custom skill"
grep -q "galaxy:dormant-data" C:/Users/wompu/.claude/projects/H--prism/memory/MEMORY.md || echo "FAIL 12: master MEMORY.md has no [galaxy:dormant-data] back-pointer — master index is blind to this brain (run STEP 5d)"
...
`slot-context-bundle-inject` auto-loads your galaxy on every future UserPromptSubmit for slot:victor. Future sessions skip this brief automatically (galaxy-buildout-detect sees `engines/dormant-data/CLAUDE.md` exists).

Run `/checkin-victor` to pick the next normal-work unit.
