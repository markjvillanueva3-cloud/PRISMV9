...
| 11 | **MEMORY SUBSTRATE (retain+recall) improvement** (assessed 2026-07-03) | A | **ASSESSED — 5 buildable units queued, none shipped** | none (M1/M2 buildable now) | Prior awareness work optimized the INJECT axis only; the RETAIN/RECALL axis was never measured. Live findings: 7196 memory files/16.9MB (96% `reference_`), MEMORY.md at 22/24KB ceiling, precompact handoff carries no working state (empty+padded), `post-ship-distill` floods 1 memory/commit. Build order: **M1** precompact handoff captures real in-flight state (highest ROI) → **M2** bound reference-store (consolidate `post_ship` → ledger + prune) → M3 keep master-index warm → M4 formalize 2-tier index → M5 convergence. | `state/shared/specs/MEMORY-SUBSTRATE-IMPROVEMENT-ASSESSMENT-2026-07-03.md` |
