# U-PSN-R3-SELF-07 — Self-Instruct / Evol-Instruct on tribal tips

> Auto-generated implementation plan by `scripts/psn-incorp-automate.mjs` from PSN-INCORPORATION-MS0.
> Source spec round: R3. Estimated cost: S. PSN legs touched: 4, 10.
> Slot-soul match: foxtrot

## Rationale

3919 tips become 30K+ training pairs
...
2. **Engine target:**
   ...
   - Candidate name: `R3SELF07Engine` (verify via dedup before committing).
   - File: `mcp-server/src/engines/<EngineName>.ts`
   - Singleton export pattern: `export const <camelName>Engine = new <EngineName>();`
   - Cost class S → expected size: <300 LOC, 1-2 days.

3. **Test target:**

   - File: `mcp-server/src/__tests__/R3SELF07Engine.test.ts` (per [[feedback_engine_tests_in_tests_dir]]).
...
5. **PSN-leg integration per `feedback_psn_definition`:**
   ...
   - Leg #4 — must verify integration (see leg map in feedback_psn_definition.md table)
   - Leg #10 — must verify integration (see leg map in feedback_psn_definition.md table)
...
_Generated: 2026-05-24T02:35:23.674Z_