# U-PSN-R3-ACT-01 — BALD/Query-by-Committee active learning

> Auto-generated implementation plan by `scripts/psn-incorp-automate.mjs` from PSN-INCORPORATION-MS0.
> Source spec round: R3. Estimated cost: M. PSN legs touched: 7, 10.
> Slot-soul match: hotel, juliett

## Rationale

5-10x labeling cost cut
...
2. **Engine target:**
   ...
   - Candidate name: `R3ACT01Engine` (verify via dedup before committing).
   - File: `mcp-server/src/engines/<EngineName>.ts`
   - Singleton export pattern: `export const <camelName>Engine = new <EngineName>();`
   - Cost class M → expected size: 300-800 LOC, 1-2 weeks.

3. **Test target:**

   - File: `mcp-server/src/__tests__/R3ACT01Engine.test.ts` (per [[feedback_engine_tests_in_tests_dir]]).
...
5. **PSN-leg integration per `feedback_psn_definition`:**
   ...
   - Leg #7 — must verify integration (see leg map in feedback_psn_definition.md table)
...
_Generated: 2026-05-24T02:35:23.704Z_