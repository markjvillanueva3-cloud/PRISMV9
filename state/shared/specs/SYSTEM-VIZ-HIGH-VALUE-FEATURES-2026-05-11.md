...
| C3 | **Closed-loop overlay** (`L` key / legend row) — colours engines green if they're part of a wired feedback loop (name-pattern match on `closedloop\|outcomecapture\|outcomecollect\|feedbackloop\|recalibrat\|bayesianposterior\|driftcanary\|actualvspredicted\|sfcclosedloop\|ppgclosedloop\|onlinelearning\|continuallearn` across L5 engines + L3 AI tiers); dims the rest to 7% so the open-loop majority is unmissable. Toast: "N engines in a wired feedback loop — the rest is open-loop." | ✅ | makes the closed-loop-learning gap visible — the SFC AI chain is the mature one; widening this is the roadmap. |
| C2 | **Action-surface overlay** — trace `disp.X → action → engine` using the `actEng` edges; "show the real call graph." | ⏳ blocked | **The current graph has only 14 L4a action nodes and 0 actEng edges** — the dispatcher-action atomizer + `generate-action-engine-edges.mjs` produce ~nothing without a `regen-viz --full` pass (~15-20 min). The viewer code is ~30 lines once the data exists. Re-open after the next `--full` regen. |
| C4 | **Drift flash** — pulse the milestone-envelope-drift nodes. | ⏳ blocked | **There are no milestone nodes in the system-graph** (`ms.*`/`roadmap.*` = 0) — the envelope-drift cases (`MF-MS1`, `MF-MS2`) aren't graph nodes, so there's nothing to flash. Would need a milestone layer/generator first. Genuinely low value (2 cases) — parked. |
