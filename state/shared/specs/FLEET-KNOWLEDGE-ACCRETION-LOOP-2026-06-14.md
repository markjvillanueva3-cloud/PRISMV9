...
- **Hard ceiling backstop (`maxIterations`, default 30):** a galaxy saturates regardless of novelty once it
  hits the ceiling. This guarantees physical termination even if the cron tier's planner hallucinates
  fresh-looking citations every iteration (which would otherwise reset the low-novelty streak forever). The
  novelty rule normally fires first; the ceiling is the anti-gaming guarantee (scrutiny P1-1).
- **Single source of truth:** scheduling (`nextGalaxies`/`fleetDone`/`summary`) computes saturation live from
  the loss function, never the cached `saturated` flag, so a config change can't desync them (scrutiny P1-2).
...
- **Honest planner provenance (scrutiny P0).** `ask-hermes` silently degrades to local Ollama on a Hermes
  outage. The driver calls it with `--json` and stamps each anchor with the ACTUAL answering source
  (`source=hermes` → "Hermes (xAI Grok)"; `source=ollama-fallback` → "Ollama local fallback"), never a
  hard-coded "Hermes" label — so an Ollama-drafted anchor is never passed off as Grok's. If both fail (exit 3)
  the iteration fails loud (no fabricated draft).
