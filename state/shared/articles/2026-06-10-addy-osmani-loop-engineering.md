...
captured: 2026-06-10
captured_by: slot:zulu (claude-4b1bbdf2) via playwright
topics: [agentic-coding, loops, harness, sub-agents, skills, worktrees, memory, connectors]
prism_relevance: KEYSTONE — names every PRISM loop primitive 1:1 and the 3 failure modes
---

# Loop Engineering — Addy Osmani

> Captured verbatim from the X long-form article for the PRISM master/orchestration galaxy.
> This is the conceptual frame behind PRISM's 26-slot fleet + /loop + /goal + slot worktrees + skills + hooks + Obsidian memory.

## TL;DR for PRISM
Loop engineering = replacing yourself as the person who prompts the agent; you design the system that prompts it instead. A loop is a recursive goal: define a purpose, the AI iterates until complete. **Five building blocks + one memory**, and Claude Code + Codex both ship all five:

1. **Automations** — scheduled discovery/triage that run themselves → PRISM: `/loop`, cron scheduled-tasks, lifecycle hooks, GitHub Actions.
2. **Worktrees** — parallel agents that don't collide → PRISM: SLOT-WORKTREE-MS0 (`H:/prism-slot-<nato>`), `isolation: worktree` on subagents.
3. **Skills** — project knowledge written down so the agent stops re-deriving it cold each run (intent debt) → PRISM: `.claude/commands/*.md` (~440 skills), SKILL.md format.
4. **Plugins / connectors** — MCP into your real tools (issue tracker, db, Slack, CI) → PRISM: prism_* MCP dispatchers + connectors.
5. **Sub-agents** — keep the maker away from the checker; a 2nd agent (diff model/instructions) catches what the 1st talked itself into → PRISM: per-file 2-arm scrutiny + 3-of-3 Stop gate + `/goal`'s separate-model done-check.
6. **Memory (the spine)** — a markdown file / Linear board OUTSIDE the conversation holding what's done + what's next; the model forgets between runs so memory lives on disk, not in context → PRISM: Obsidian vault + per-chat HANDOFF + MEMORY.md + the master brain.

## The 3 failure modes that get SHARPER as the loop improves (PRISM must heed)
- **Verification is still on you.** "A loop running unattended is also a loop making mistakes unattended." `/goal`'s split verifier makes "done" mean something, but "done is a claim, not a proof." → PRISM R12 fail-loud + the 3-of-3 scrutiny gate exist for exactly this.
- **Comprehension debt.** The faster the loop ships code you didn't write, the bigger the gap between what exists and what you understand. A smooth loop grows the gap faster unless you read what it made.
- **Cognitive surrender.** When the loop runs itself it's tempting to stop having an opinion. "Designing the loop is the cure when you do it with judgement and the accelerant when you do it to avoid thinking — same action, opposite result."

## Key quotes
- @steipete: "You shouldn't be prompting coding agents anymore. You should be designing loops that prompt your agents."
- @bcherny (head of Claude Code): "I don't prompt Claude anymore. I have loops running that prompt Claude... My job is to write loops."
...
## One loop shape (Addy's canonical example — already PRISM's pattern)
Morning automation → triage skill reads CI failures/open issues/recent commits → writes findings to a markdown/Linear spine → per finding, open an isolated worktree + maker sub-agent drafts the fix + checker sub-agent reviews against skills+tests → connectors open the PR + update the ticket → anything unhandled lands in a triage inbox → the state file remembers what was tried/passed/open so tomorrow picks up where today stopped. "You designed it one time. You did not prompt any of those steps."
