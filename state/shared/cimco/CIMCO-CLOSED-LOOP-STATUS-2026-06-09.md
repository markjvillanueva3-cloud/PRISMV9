...
## UPDATE 2026-06-26 (slot:echo, session ab21e9c9) — the `.mcfg` machine-load fidelity wire is now BUILT
The single open fidelity wire below (table row: "`.mcfg` machine-load per machine") is **BUILT + proven live**.
Two ops added to `PrismCimcoUI.exe` (`Program.cs`, +202 lines, compiles clean via `build.ps1`):
- `--op list-combo --name <page-hint> --cid <id>` — READ a Setup ComboBox's full option list (CB_GETCOUNT + per-index CB_GETLBTEXT).
- `--op set-combo --name <page-hint> --cid <id> --to <name|index> [--persist]` — WRITE: select by machine NAME (exact/unique-substring, fail-closed) or index (CB_SETCURSEL + WM_COMMAND/CBN_SELCHANGE notify), read-back-verify, **discard-by-default** (Cancel) unless `--persist`.

**Live-confirmed geography:** Setup **page 10** (hint "Highlight syntax errors when backplotting") holds `cid 14307`="Machine setup:" (86 machines) + `cid 14639`="Control Type:" (95 controllers). **Root cause of the header-only/`report-grid-not-found` sims, REFINED:** the persisted sim machine was **"CIMCO Lathe Default (Imperial)" + "Okuma Turning"** — a LATHE simulating MILL NC. `Haas VF-6/40`=combo idx 67 (exact `jm-fleet-sim-map` match), `Haas NGC Milling`=idx 11. **Proven:** read 86/95; write Okuma→Haas-NGC (verified+discarded); **persist** Machine→VF-6/40 + Control→Haas-NGC (`persisted:true`, independently re-read as current). Operator's CIMCO is now correctly configured for a VMC-03 mill sim.

**RESIDUAL (operator-confirmable; stopped at the 4th `report-grid-not-found` per R6):** even with VF-6/40 set, `invoke-read --launch` of the known-bad NC returned `report-grid-not-found` (frameRealized, 1813 nodes, `open=fired;run=fired`). A prior session DID read the grid (`found:true`, header-only, clean NC) so the pane discovery isn't a fundamental wall — this is a launch-frame/timing piece + the documented "report grid's live MSAA shape confirmed by the operator's first live run." NEXT: operator opens the committed `KNOWN-BAD-OVERTRAVEL-VMC03-HaasVF6.nc` in their now-VF-6/40 CIMCO + Simulate → over-travel rows read visually OR via `--op read-report` attach. Memory: [[reference_echo_cimco_baseline_live_2026_06_26]].
