...
## EFFICIENCY & TOOL DISCIPLINE (2026-07)

**Mandatory for Claude Code CLI, Claude Desktop, Codex, and Hermes:**

### 1. PRISM_FAST_MODE
- Set `PRISM_FAST_MODE=1` (or use `--fast`) during active development.
- Fast Mode makes heavy context injectors conditional/advisory and prefers summaries/node-cards.
- Core safety remains mandatory even in Fast Mode: no incomplete code, dedup before new assets, wiring to natural consumers, and real validation before claiming completion.
- Helpers: `scripts/lib/fast-mode.mjs` and `scripts/lib/smart-fast-mode.mjs`.

### 2. Node-card first
- Prefer `prism_node_card` over full graph or large file reads when identifying PRISM nodes/assets.
- Use `prism_search` → `prism_node_card` → focused `Read` only if needed.
- Helper: `scripts/lib/node-card-helper.mjs`.

### 3. Risk-tiered scrutiny
- `low` risk → light review.
- `medium` risk → standard review.
- `high` / `critical` risk → full 3-of-3 scrutiny.
- Helper: `scripts/lib/risk-tier.mjs`.

### 4. MCP enforcement bridge
- Use `prism_enforce` for cross-agent parity checks, scrutiny planning, lesson surfacing, Fast Mode state, context recommendations, helper manifest, and MCP health.
- This keeps Claude Code CLI, Claude Desktop, Codex, and Hermes aligned without duplicating behavior in each agent runtime.
...
- 2026-07-03 | **doc the auth-token storage-key dead-panel lesson** | observed-in: 65b02cf6c | fix: see commit | verify: `git -C H:/prism show 65b02cf6c`
- 2026-07-03 | **QuotingWorkbenchPage same wrong-key under-quote hazard** | observed-in: db9740e2f | fix: see commit | verify: `git -C H:/prism show db9740e2f`
- 2026-07-03 | **[MAIN-FORCE] [SIERRA-CODE-VAULT]/U-SIERRA-CODE-VAULT-FRESHNESS (slot:sierra, ultracode): make the code 2nd-brain SELF-MAINTAINING -- wire...** | observed-in: e5d85da6e | fix: see commit | verify: `git -C H:/prism show e5d85da6e`
- 2026-07-03 | **[MAIN-FORCE] [SIERRA-CODE-VAULT]/U-SIERRA-ENRICH-COMPLETE (slot:sierra, ultracode): COMPLETE the thin-note enrichment via Hermes -- 473 t...** | observed-in: 54f6cc5fb | fix: see commit | verify: `git -C H:/prism show 54f6cc5fb`
- 2026-07-03 | **[MAIN-FORCE] [SIERRA-CODE-VAULT]/U-SIERRA-ENRICH-PATH-FIX (slot:sierra, ultracode): fix 3 bugs that made --llm-synth produce empty/garbag...** | observed-in: 849917403 | fix: see commit | verify: `git -C H:/prism show 849917403`
- 2026-07-03 | **fix PS5.1 parse error in slot-worktree-migration-status installer** | observed-in: 1d94529ab | fix: see commit | verify: `git -C H:/prism show 1d94529ab`
- 2026-07-02 | **[MAIN-FORCE] [AI-SYNERGY]/U-CAD-ONESHOT-CONVERGENCE (slot:india): stage-4 MEASURE of CAD one-shot chain -- first-pass convergence + hones...** | observed-in: 0414609f5 | fix: see commit | verify: `git -C H:/prism show 0414609f5`
- 2026-07-02 | **[MAIN-FORCE] [ASSET-PRESERVE]/U-OSC-RESTORE-PEER-CODEVAULT (slot:oscar): restore 3 peer files my U-CW-16 commit damaged in history** | observed-in: 9d0995919 | fix: see commit | verify: `git -C H:/prism show 9d0995919`
- 2026-07-02 | **[MAIN-FORCE] [MASTER-UNIT-PLAN]/U-DELTA-CNC-RECONSTRUCT-WIRE (slot:delta): fix 100%-dead gcode_reverse_cad_reconstruct + 7 round-trip tests** | observed-in: ae64ecad2 | fix: see commit | verify: `git -C H:/prism show ae64ecad2`
- 2026-07-02 | **[MAIN-FORCE] [MASTER-UNIT-PLAN]/U-OSC-UNIT0011-BUE-MITIGATION (slot:oscar): BUEMitigationRecommenderEngine + bue_mitigation_recommend on ...** | observed-in: 252536c73 | fix: see commit | verify: `git -C H:/prism show 252536c73`
- 2026-07-02 | **[MAIN-FORCE] [KIENZLE-CAD]/U-INDIA-CAD-CORRECTION-PLAN (slot:india): direct target-correction planner -- id-free correspondence -> one-sh...** | observed-in: d833d4f07 | fix: see commit | verify: `git -C H:/prism show d833d4f07`
...
- 2026-07-05 | **MasterIndex utilization dashboard silently reported normal=0 while 123,444 nodes (58% of the 211,272-node graph) were mis-bucketed into a phantom "normals" key (slot:sierra)** | observed-in: U-SIERRA-MASTERINDEX-TOTALS-NORMAL-KEY (this commit) | root cause: `classifyAllNodes` derived each `totals` bucket by suffixing the class name with `s` (a template literal + `as keyof typeof totals` cast); five classes pluralize to real keys but the `totals` type used the SINGULAR key `normal`, so `cls="normal"` wrote a phantom `normals` property (absent from the type — the cast silenced the TS error) and `totals.normal` stayed permanently 0. No automated consumer reads the field (dispatcher passthrough + input-only schema) so nothing broke, but a human reading the dashboard saw 0 normal nodes out of 123,444. fix: explicit exhaustive `Record<UtilizationClass, Exclude<keyof typeof totals,"nodesScanned">>` bucket map (a new class without a bucket is now a COMPILE error, not a silent 0); dropped the cast; test strengthened to the exact partition `hubs+sinks+sources+orphans+ghosts+normal === nodesScanned` (FAILS against old code). Validated live: normal 0→123,444, sum===211,272. Lesson: never derive a typed-counter key by string-munging an enum member + `as keyof` cast — map explicitly with a `Record` so the compiler enforces the mapping. | verify: `git -C H:/prism show HEAD` -- [[reference_sierra_masterindex_totals_normal_key_2026_07_05]] · [[masterindex-degree-sidecar]]
- 2026-07-05 | **A shared-artifact schema bump reached only ONE of TWO readers → the master-index DAEMON silently degraded fleet search to the 59MB architecture graph (slot:sierra)** | observed-in: fc14c925cd (introduced) → f7863136df (fixed) | root cause: the degree-sidecar work (restore utilization/degree/orphan classification on the 915MB over-cap `system-graph.json` — `getNodeStatus`/`classifyAllNodes` were fail-loud degraded because the fallback `system-graph-index.json` sidecar had no edges) emitted a NEW additive `degrees:{in[],out[],maxIn}` block and bumped the sidecar schema 1.0.0→1.1.0. `MasterIndexEngine.ts` (per-prompt reader) was updated, but `scripts/lib/master-index-search-lib.mjs` — the SECOND independent reader, used by the large-heap `master-index-daemon.mjs` fleet search — strict-rejected `sc.schemaVersion !== "1.0.0"` and silently fell back to the 59MB architecture graph. Masked because its test fixture hard-coded "1.0.0". Caught by arm-C (`code-analyzer`) of the 3-of-3 scrutiny gate; arms A+B PASSed and missed it. fix: `degrees` is purely additive + this reader consumes only nodes+inverted, so the gate is now MAJOR-version compatible (`sidecarSchemaCompatible`: 1.x accepted, 2.x/garbage rejected). Tests 70/70 incl. a regression guard that FAILS against the old strict gate. LESSON: when you bump a shared artifact's schema version, grep EVERY reader and propagate — a strict `!==` gate turns a backward-compatible additive bump into a silent breaking change. | verify: `git -C H:/prism show f7863136df` -- [[masterindex-degree-sidecar]] · [[reference_sierra_masterindex_degree_sidecar_2026_07_05]]
