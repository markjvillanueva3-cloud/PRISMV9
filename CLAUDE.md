...
**Cross-tree commit-collision advisory (2026-05-15):** `stop-cross-tree-collision-advisory.mjs` (T3 Stop hook, NOT YET WIRED in settings.json — pending follow-up) fires once per 4h when a chat is in the shared `H:/prism` main tree with critical files dirty AND a sibling worktree matches its topic — emits a migration hint. Lighter touch than `worktree-commit-route.mjs` (default-OFF, opt-in via `PRISM_WORKTREE_ROUTE_ENABLE=1`). Knob: `PRISM_CROSS_TREE_ADVISORY_DISABLE=1`.
