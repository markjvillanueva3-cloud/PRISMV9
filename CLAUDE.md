...
36 slash-command wrappers `/{precompact,handoff,startup}-<slot>` × 12 NATO slots (alpha..lima) mirror the existing `/checkin-<slot>` pattern: force-take slot → bind topic `<slot>-work` → delegate to canonical `/precompact`, `/handoff`, or `/startup` pipeline. Each wrapper is ~30 lines, generated from a single template by `scripts/generate-per-slot-wrappers.mjs` (idempotent — re-runnable safely). Use when a chat-slot binding must be explicit (different terminal window, post-/compact drift, force-take from a dead peer). The wrappers are thin — the canonical pipeline body lives in the bare slash command.
...
**Fleet-design directive (2026-05-15, user):** every slot-aware design must accommodate **up to 12 concurrent chats** (expanded 2026-05-16 from 10 to support `/checkin-<slot>` for all 12 NATO letters through Lima — added `kilo` + `lima` to `SLOT_NAMES`, additive forward-compat, schemaVersion intentionally NOT bumped to avoid disrupting active peers). Current 12: alpha..foxtrot + golf (historically-hygiene; usable as work slot via `/checkin-golf` after bypassing `golf-slot-write-allowlist`) + hotel..lima. All new code reads `SLOT_NAMES` from `chat-slots.mjs` — never hard-code count.
