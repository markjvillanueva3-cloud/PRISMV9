...
- **Engines**: 95 WEDM/EDM engines (`src/engines/WEDM*.ts`, `EDM*.ts`, `WireEDM*.ts`) — Phase 4 added 9 autonomy/safety/RUL engines (WEDMAutonomyLevelEngine, WEDMAutonomyRulesEngine, WEDMHumanHandoffEngine, WEDMHeadClearanceEngine, WEDMSafetyEnvelopeEngine, WEDMFailsafeEngine, WEDMDegradationModelEngine, WEDMRULEngine, WEDMMaintenanceSchedulerEngine)
- **Skills**: 13 WEDM skills (`~/.claude/commands/wedm-*.md`) — verified against `data/state/WEDM_DIGEST.json` (MS-P0-V U-P0-V02)
- **Hooks**: 22 WEDM hooks (16 safety + 2 SVI coupling + 2 perception + 2 learning) — defined in `src/hooks/WEDM*Hooks.ts`, verified by digest
- **Playbooks**: 8 workflow playbooks (`data/playbooks/wedm_*.json`)
- **State Files**: 40 WEDM state/index files (`data/state/WEDM_*.json`, incl. WEDM_AUTONOMY_STATE v1 L0 cold-start seed, WEDM_TRANSFER_REGISTRY v2 with 24 curated mappings, WEDM_DRIFT_BASELINE v2 with 10 model heads, WEDM_RL_POLICY v1 LinUCB bandit snapshot, WEDM_DIGEST v1 baseline)
