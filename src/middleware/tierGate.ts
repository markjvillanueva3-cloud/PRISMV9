 ...
 *   - INFRA-3-2: Now reads actual usage from Redis-backed UsageCounter
 */
import type { Request, Response, NextFunction } from "express";
import type { Plan, TierLimits } from "../engines/AuthEngineV7.js";
import { getUsageCounter } from "./usageCounter.js";
...
export function requireTier(feature: string) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    const user = (req as any).user as { plan?: Plan; sub?: string; usage?: Record<string, number> } | undefined;
    const plan: Plan = user?.plan ?? "free";

    // INFRA-3-2: Read actual usage from Redis-backed counter
    const userId: string | undefined = (req as any).userId ?? user?.sub;
    let currentUsage: number = user?.usage?.[feature] ?? 0;
    if (userId) {
      try {
        const counter = await getUsageCounter();
        currentUsage = await counter.getUsage(userId, feature);
      } catch {
        // Non-fatal — fall through to static usage from request
      }
    }
