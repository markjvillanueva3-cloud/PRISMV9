...
describe("SAFETY: AC Python script E1120 alignment", () => {
  ...
  it("uses SetCfgParameters dict for speed/feed — NOT SetSpindleSpeed/SetFeedRate", () => {
    // Real hyperMILL API uses CfgParameters dict, not dedicated speed/feed methods
    expect(script).toContain('"spindle_speed": "5000"');
    expect(script).toContain('"feed": "800"');
  ...
  it("includes ap and ae in CfgParameters dict", () => {
    expect(script).toContain('"ap": "3"');
    expect(script).toContain('"ae": "6"');
    // Old pattern: SetParameter() does not exist in real om.cam.core.Job API
