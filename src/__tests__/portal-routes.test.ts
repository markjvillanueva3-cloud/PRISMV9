...
describe("Portal and milestone mounted routes", () => {
  ...
  it("routes canonical milestone intelligence sync and exposes recent sync memory", async () => {
    const jobId = nextId("JOB-SYNC");

    const syncResponse = await httpRequest("POST", `/api/v1/portal/milestones/${encodeURIComponent(jobId)}/sync`, {
      source: "jobs-desk",
      trigger: "job-created",
    ...
    expect(syncResponse.data.data).toMatchObject({
    ...
    expect(syncResponse.data.data.event.cli_command).toContain("prism milestone align");

    const eventsResponse = await httpRequest("GET", `/api/v1/portal/milestones/${encodeURIComponent(jobId)}/sync-events?limit=5`, undefined, internalAuthHeaders);
    expect(eventsResponse.status).toBe(200);
    expect(eventsResponse.data.data).toHaveLength(1);
    expect(eventsResponse.data.data[0]).toMatchObject({
      job_id: jobId,
      trigger: "job-created",
      outcome: "seeded",
    });
