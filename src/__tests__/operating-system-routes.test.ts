...
describe("Operating-system authority routes", () => {
  ...
  it("emits canonical PRISM sync memory for shop-floor registration and department check-in", async () => {
    const registerResponse = await httpRequest("POST", "/api/v1/operating-system/shop-floor/check-in", {
      action: "register",
      scanInput: "JOB-OS-SYNC",
      jobId: "JOB-OS-SYNC",
    ...
    expect(registerResponse.data.data).toMatchObject({
      jobId: "JOB-OS-SYNC",
      selectedDepartment: "Machining",
    });
    expect(registerResponse.data.data.prism_sync).toMatchObject({
      event: {
        job_id: "JOB-OS-SYNC",
    ...
    const checkInResponse = await httpRequest("POST", "/api/v1/operating-system/shop-floor/check-in", {
      action: "check-in",
      trackedJob: {
        jobId: "JOB-OS-SYNC",
        partNumber: "PART-101",
    ...
    expect(checkInResponse.data.data.entry).toMatchObject({
      jobId: "JOB-OS-SYNC",
      department: "Machining",
    });
    expect(checkInResponse.data.data.prism_sync).toMatchObject({
      event: {
        job_id: "JOB-OS-SYNC",
  ...
  it("emits canonical PRISM sync memory for shop-floor task events", async () => {
    const taskEventResponse = await httpRequest("POST", "/api/v1/operating-system/shop-floor/check-in", {
      action: "task-event",
      ...
      scrapQty: 1,
      note: "Completed the run cycle task on the floor tracker.",
    });

    expect(taskEventResponse.status).toBe(200);
    expect(taskEventResponse.data.data).toMatchObject({
      acknowledged: true,
      trigger: "shop-floor-task-completed",
      taskId: "run-cycle",
    });
    expect(taskEventResponse.data.data.prism_sync).toMatchObject({
      event: {
        job_id: "JOB-OS-TASK",
        trigger: "shop-floor-task-completed",
      },
