...
describe("Condition code resolution", () => {
  ...
  it("standard (1.6 < Ra ≤ 3.2) → condition 2", () => {
    expect(resolveEPackConditionCode(3.2)).toBe(2);
    expect(resolveEPackConditionCode(2.0)).toBe(2);
  });

  it("fine (0.8 < Ra ≤ 1.6) → condition 3", () => {
    expect(resolveEPackConditionCode(1.6)).toBe(3);
    expect(resolveEPackConditionCode(1.0)).toBe(3);
  });

  it("extra fine (0.4 < Ra ≤ 0.8) → condition 4", () => {
    expect(resolveEPackConditionCode(0.8)).toBe(4);
    expect(resolveEPackConditionCode(0.5)).toBe(4);
  });

  it("ultra fine (Ra ≤ 0.4) → condition 5", () => {
    expect(resolveEPackConditionCode(0.4)).toBe(5);
    expect(resolveEPackConditionCode(0.1)).toBe(5);
  });

  it("coated wire shifts condition code down by 1", () => {
    // Standard with coated wire → high speed (better capability)
    expect(resolveEPackConditionCode(3.2, "zinc_coated_0.25")).toBe(1);
    // Fine with coated wire → standard
    expect(resolveEPackConditionCode(1.6, "coated_0.25")).toBe(2);
  });
