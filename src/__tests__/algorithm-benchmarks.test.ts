...
const BENCHMARK_INPUTS: Record<string, Record<string, any>> = {
  ...
  "csp-setup-plan": { features: [{ id: "F1", access_directions: ["+Z", "+X"] as string[], machining_time: 5, tolerance_group: "TG1" }, { id: "F2", access_directions: ["+Z"] as string[], machining_time: 3, tolerance_group: "TG1", precedence_before: [] }, { id: "F3", access_directions: ["+X", "+Y"] as string[], machining_time: 4, tolerance_group: "TG2" }] },
  "stft-chatter": { signal: Array.from({ length: 512 }, (_, i) => Math.sin(2 * Math.PI * 800 * i / 10000) + 0.5 * Math.sin(2 * Math.PI * 1234 * i / 10000) + 0.1 * (Math.random() - 0.5)), sample_rate: 10000, spindle_speed: 12000, n_flutes: 4 },
  ...
  describe("Batch execution", () => {
    it("should batch-execute a representative set", () => {
      const result = algorithmEngine.batch({
        calculations: [
          { algorithm_id: "kienzle", params: { kc1_1: 1800, mc: 0.25, feed_per_tooth: 0.1, axial_depth: 3, radial_depth: 6, tool_diameter: 12, number_of_teeth: 4, cutting_speed: 200 } },
          { algorithm_id: "chip-thinning", params: { feed_per_tooth: 0.1, radial_depth: 3, tool_diameter: 12 } },
        ],
      });
