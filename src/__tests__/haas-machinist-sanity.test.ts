...
describe("Haas machinist sanity sweeps", () => {
  it("keeps prioritized material speed ordering sane on the Haas VF-2SS for the same roughing cutter", () => {
    const common = {
      tool_diameter_mm: 12,
      flutes: 5,
  ...
  it("keeps representative Haas mill and lathe jobs inside sane machine envelopes", () => {
    const jobs = [
      {
        label: "VF-2SS adaptive roughing in 4140 with a solid carbide end mill",
        maxRpm: 12000,
        minConfidence: 0.45,
        minRa: 0.0001,
        input: {
      ...
      {
        label: "VF-2SS face milling 6061 with an indexable cutter",
        maxRpm: 12000,
        minConfidence: 0.45,
        minRa: 0.0001,
        input: {
      ...
      {
        label: "UMC-750 finish milling H13 stays conservative on a ball end mill",
        maxRpm: 12000,
        minConfidence: 0.4,
        minRa: 0,
        input: {
      ...
      {
        label: "ST-20 OD roughing 4140 with a roughing insert stays in the spindle envelope",
        maxRpm: 4000,
        minConfidence: 0.35,
        minRa: 0.0001,
        input: {
      ...
      {
        label: "ST-20Y live milling 304 stainless keeps feed and confidence positive",
        maxRpm: 4000,
        minConfidence: 0.35,
        minRa: 0.0001,
        input: {
      ...
      {
        label: "ST-35 finish turning 316 stainless stays conservative but productive",
        maxRpm: 2400,
        minConfidence: 0.35,
        minRa: 0.0001,
        input: {
          machine_name: "Haas ST-35",
          machine_type: "lathe",
          material: "316 Stainless",
          operation: "turning",
          cut_type: "finishing",
          tool_diameter_mm: 12,
          workpiece_diameter_mm: 110,
          machine_power_kw: 22,
          machine_max_rpm: 2400,
          coolant_type: "flood",
          workholding_type: "chuck",
  ...
  it("keeps Haas turning jobs in turning mode and avoids mill-only setup chatter in the warnings", () => {
  ...
  it("keeps through-tool drilling on the VF-2SS stronger than flood for stainless deep-hole work", () => {
