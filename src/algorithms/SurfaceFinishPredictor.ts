...
export class SurfaceFinishPredictor implements Algorithm<SurfaceFinishInput, SurfaceFinishOutput> {
  ...
  calculate(input: SurfaceFinishInput): SurfaceFinishOutput {
    ...
    const hasMillingGeometry = !!(radial_depth && radial_depth > 0 && tool_diameter && tool_diameter > 0);
     ...
     * @returns void
     */
    if (is_milling && hasMillingGeometry) {
      Ra_theoretical = (feed * feed * radial_depth) / (32 * tool_diameter * nose_radius);
    } else {
      if (is_milling) {
        warnings.push("Missing radial_depth/tool_diameter for milling — falling back to turning-equivalent finish model");
      }
    ...
    return {
      ...
      iso_grade,
      warnings,
      calculation_method: is_milling && hasMillingGeometry
        ? "Surface Finish (milling Ra = f²·ae/(32·D·R) × process_factor)"
        : "Surface Finish (Ra = f²/(32R) × process_factor)",
    };
  }
