...
import { ACTION_SP_SCHEMAS } from "../../schemas/spActionSchemas.js";
...
export function registerSpDispatcher(server: any): void {
  server.tool(
    ...
    async ({ action, params: rawParams = {} }: { action: typeof ACTIONS[number]; params: Record<string,any> }) => {
      ...
      const validation = validateActionParams(action, params, ACTION_SP_SCHEMAS);
      if (!validation.valid) {
        return dispatcherError(`Invalid params for '${action}': ${validation.errorMessage}`, action, "prism_sp");
      }
      try {
        switch (action) {
          case "brainstorm": {
