...
const featureZ = z.object({
  type: z.enum([
    ...
    "hole", "impeller", "pocket", "ruled_surface", "slot", "steep_wall",
...
const toolZ = z.object({
  ...
  type: z.enum([
    ...
    "endmill", "face_mill", "insert", "tap", "turning_insert",
  ]).describe("Tool type"),
...
const categoryZ = z.enum([
  "drilling", "finishing", "multi_axis", "roughing", "turning",
]).optional().describe("Filter by strategy category");

const strategyNameZ = z.string().min(1).describe(
  "Strategy name, e.g. 'dynamic_mill', 'scallop', 'profit_turning'. Use mastercam_strategy_list to see all."
);

// ── Action Schemas ────────────────────────────────────────────────────────────

export const camxMs3U01ActionSchemas: ActionSchemaMap = {
  mastercam_strategy_recommend: z.object({
  ...
  mastercam_strategy_params: z.object({
    strategy_name: strategyNameZ,
  }),

  mastercam_strategy_dynamic_motion: z.object({}).passthrough(),

  mastercam_strategy_optirough: z.object({}).passthrough(),

  mastercam_strategy_profit_turning: z.object({}).passthrough(),

  mastercam_strategy_list: z.object({
    category: categoryZ,
  }),
};
