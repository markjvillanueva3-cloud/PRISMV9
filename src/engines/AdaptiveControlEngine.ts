...
function estimateWearKalman(params: Record<string, unknown>): WearResult & {
  ...
  try {
    ...
    // Fallback: RLS-based online estimation via SystemIdentificationEngine
    // Model: load_increase = wearToLoad * wear_pct → y = φ·θ
    // where θ = [wear_rate], φ = [cumulative_time], y = load_increase
    try {
      const rlsState = SystemIdentificationEngine.rlsInit(1, { lambda: 0.98, delta: 100 });
      rlsState.theta[0] = expectedLife > 0 ? (100 / expectedLife) * 0.4 : 0.8; // prior wear-to-load rate

      for (let i = 0; i < loadHistory.length; i++) {
        const loadIncrease = ((loadHistory[i] - baselineLoad) / baselineLoad) * 100;
        const t = (i + 1) * dt;
        SystemIdentificationEngine.rlsUpdate(rlsState, [t], loadIncrease);
      }

      // Recover wear from identified rate: wear = (rate / wearToLoad) * time
      const identifiedRate = rlsState.theta[0];
      const totalTime = loadHistory.length * dt;
      finalWear = Math.min(Math.max((identifiedRate / 0.4) * totalTime, 0), 100);
      finalRate = identifiedRate > 0 ? identifiedRate / 0.4 : (expectedLife > 0 ? 100 / expectedLife : 2);
      stateEstimate = [finalWear, finalRate];
      covDiag = [rlsState.P[0][0]];
    } catch {
      // Ultimate fallback: max of time/load/VB estimates
