...
import { physicsConstantsService } from "../services/PhysicsConstantsService.js";
...
/**
 * Thermal constants — sourced from PhysicsConstantsService (canonical).
 * ARCH-MS1 U-SHAD04: Migrated from inline values to service layer.
 *
 * Value snapshot at migration:
 *   CHIP_HEAT_FRACTION:        inline=0.75     canonical=0.75     — match
 *   TOOL_HEAT_FRACTION:        inline=0.15     canonical=0.15     — match
 *   WORK_HEAT_FRACTION:        inline=0.10     canonical=0.10     — match
 *   STEEL_THERMAL_EXPANSION:   inline=12e-6    canonical=12e-6    — match
 *   ALUMINUM_THERMAL_EXPANSION: inline=23e-6    canonical=23.6e-6  — corrected (+2.6%)
 *     Source: ASM Metals Handbook Vol.2, 6061-T6. Canonical value is more accurate.
 */
const THERMAL_CONSTANTS = {
  AMBIENT_TEMP: 20, // °C — environment constant, not a material property
  CHIP_HEAT_FRACTION: physicsConstantsService.getThermalConstant("CHIP_HEAT_FRACTION")!.value,
  TOOL_HEAT_FRACTION: physicsConstantsService.getThermalConstant("TOOL_HEAT_FRACTION")!.value,
  WORK_HEAT_FRACTION: physicsConstantsService.getThermalConstant("WORKPIECE_HEAT_FRACTION")!.value,
  STEEL_THERMAL_EXPANSION: physicsConstantsService.getThermalConstant("STEEL_THERMAL_EXPANSION")!.value,
  ALUMINUM_THERMAL_EXPANSION: physicsConstantsService.getThermalConstant("ALUMINUM_THERMAL_EXPANSION")!.value,
};
