 ...
 * planHistory moved from module-level to class-scoped storage.
 */
...
// Fallback cutting data — used when MaterialRegistry has no speed data for the material.
// Values from Sandvik Coromant General Turning Guide (2023 edition).
const FALLBACK_CUTTING_DATA: Record<string, MaterialCuttingData> = {
  "7075": {
...
// ─── Constants (externalized to ConfigService — ARCH-MS1 U-SHAD01) ─────────

const shopConfig = configService.getShopConfig();
const SHOP_RATE = shopConfig.shopRateUsd;
const SETUP_TIME_MIN = shopConfig.setupTimeMin;
const TOOL_CHANGE_SEC = shopConfig.toolChangeSec;
const WCS_LIST = ["G54", "G55", "G56", "G57", "G58", "G59"];

// ─── Plan Storage (ARCH-MS1: Map-based for key-indexed retrieval) ───────────

const planStore = new Map<string, ProcessPlan>();
...
/** Resolve material cutting data: try MaterialService first, fall back to built-in catalog. */
function findMaterial(name: string): MaterialCuttingData | undefined {
  // Try registry (synchronous getByIdOrName returns a promise — use cached result pattern)
  const key = name.toLowerCase();

  // Map from registry Material → engine-local MaterialCuttingData
  const fromRegistry = materialServiceCache.get(key);
  if (fromRegistry) return fromRegistry;

  // Fallback: match by keyword against built-in catalog
  if (key.includes("7075") || key === "aluminum" || key === "aluminium") return FALLBACK_CUTTING_DATA["7075"];
  if (key.includes("6061")) return FALLBACK_CUTTING_DATA["6061"];
  if (key.includes("4140") || key === "steel") return FALLBACK_CUTTING_DATA["4140"];
  if (key.includes("304") || key.includes("stainless")) return FALLBACK_CUTTING_DATA["304"];
  if (key.includes("ti") || key.includes("titanium")) return FALLBACK_CUTTING_DATA["ti64"];
  if (key.includes("cast") || key.includes("iron")) return FALLBACK_CUTTING_DATA["cast_iron"];
  return undefined;
}

// Warm cache from MaterialService on module load (non-blocking).
// Maps lowercase material name → MaterialCuttingData for fast synchronous lookup.
const materialServiceCache = new Map<string, MaterialCuttingData>();
void (async () => {
  try {
    for (const [alias, fallback] of Object.entries(FALLBACK_CUTTING_DATA)) {
      const mat = await materialService.getByIdOrName(fallback.name);
      if (mat) {
        const speedRange = mat.machining?.recommended_speed_range;
        materialServiceCache.set(alias, {
          name: mat.name,
          density_g_cm3: mat.physical?.density ?? fallback.density_g_cm3,
          cost_per_kg_usd: fallback.cost_per_kg_usd, // registry doesn't track cost
          vc_endmill_m_min: speedRange?.carbide_max ?? fallback.vc_endmill_m_min,
          vc_drill_m_min: speedRange?.carbide_min ?? fallback.vc_drill_m_min,
          vc_face_m_min: (speedRange?.carbide_max ?? fallback.vc_face_m_min) * 1.2,
          fz_endmill_mm: fallback.fz_endmill_mm, // feed per tooth not in registry yet
          fz_drill_mm: fallback.fz_drill_mm,
          machinability_factor: (mat.machining?.machinability_rating ?? (fallback.machinability_factor * 40)) / 40,
        });
        materialServiceCache.set(mat.name.toLowerCase(), materialServiceCache.get(alias)!);
      }
    }
  } catch { /* registry unavailable — fallback catalog is sufficient */ }
})();
...
export function generativeProcess(
  ...
  switch (action) {
    ...
    case "genplan_get": {
      ...
      if (!plan) return { error: `Plan ${planId} not found`, available: Array.from(planStore.keys()) };
