...
/**
 * Computation Dependency DAG — automatic cascade invalidation.
 * When action A depends on action B, invalidating B cascades to A.
 * Callers don't need to pass dependencies manually — this map auto-injects them.
 *
 * Key = downstream action (dependent), Value = upstream actions it depends on.
 * Example: tool_life depends on cutting_force and thermal — if either changes,
 * tool_life cache is invalidated.
 */
const COMPUTATION_DEPS: Record<string, string[]> = {
  // Tool life depends on force and temperature
  "prism_calc:tool_life":       ["prism_calc:cutting_force", "prism_calc:thermal"],
  // Surface finish depends on force, deflection, and vibration
  "prism_calc:surface_finish":  ["prism_calc:cutting_force", "prism_calc:deflection", "prism_calc:stability"],
  // Power depends on force
  "prism_calc:power":           ["prism_calc:cutting_force"],
  // Torque depends on force
  "prism_calc:torque":          ["prism_calc:cutting_force"],
  // Deflection depends on force
  "prism_calc:deflection":      ["prism_calc:cutting_force"],
  // Cycle time depends on speed/feed and MRR
  "prism_calc:cycle_time":      ["prism_calc:speed_feed", "prism_calc:mrr"],
  // MRR depends on speed/feed
  "prism_calc:mrr":             ["prism_calc:speed_feed"],
  // HSM depends on stability and speed/feed
  "prism_calc:hsm":             ["prism_calc:stability", "prism_calc:speed_feed"],
  // Trochoidal depends on engagement and speed/feed
  "prism_calc:trochoidal":      ["prism_calc:engagement", "prism_calc:speed_feed"],
  // Cost estimate depends on cycle time and tool life
  "prism_business:quote_estimate": ["prism_calc:cycle_time", "prism_calc:tool_life"],
};

/** Reverse map: upstream → Set of downstream dependents (built at module load) */
const UPSTREAM_TO_DEPENDENTS = new Map<string, Set<string>>();
for (const [dependent, upstreams] of Object.entries(COMPUTATION_DEPS)) {
  for (const upstream of upstreams) {
    if (!UPSTREAM_TO_DEPENDENTS.has(upstream)) {
      UPSTREAM_TO_DEPENDENTS.set(upstream, new Set());
    }
    UPSTREAM_TO_DEPENDENTS.get(upstream)!.add(dependent);
  }
}
...
class ComputationCache {
  ...
  set(action: string, params: Record<string, any>, value: any, dependencies?: string[]): void {
    ...
    // Auto-inject dependencies from COMPUTATION_DEPS DAG
    const autoDeps = COMPUTATION_DEPS[action] || [];
    const allDeps = dependencies ? [...new Set([...dependencies, ...autoDeps])] : autoDeps;
    ...
    // Register dependencies (both explicit and auto-injected)
  ...
  /**
   * Invalidate all entries for a specific action, then cascade to
   * downstream dependents via the COMPUTATION_DEPS DAG.
   * E.g., invalidating "prism_calc:cutting_force" also invalidates
   * tool_life, surface_finish, power, torque, deflection.
   */
  invalidateByAction(action: string): InvalidationResult {
    const result: InvalidationResult = { invalidated: 0, cascade_invalidated: 0, keys: [] };

    // 1. Invalidate direct entries for this action
    ...
    // 2. Cascade to downstream dependents via static DAG
    const dependents = UPSTREAM_TO_DEPENDENTS.get(action);
    if (dependents) {
      for (const depAction of dependents) {
        for (const [key, entry] of this.cache) {
          if (entry.source_action === depAction) {
            if (this.cache.has(key)) {
              this.cache.delete(key);
              this.stats.total_entries--;
              this.stats.entries_by_tier[entry.tier]--;
  ...
  /**
   * Get the dependency chain for an action (what it depends on + what depends on it)
   */
  getDependencyInfo(action: string): { dependsOn: string[]; dependedOnBy: string[] } {
    return {
      dependsOn: COMPUTATION_DEPS[action] || [],
      dependedOnBy: [...(UPSTREAM_TO_DEPENDENTS.get(action) || [])],
    };
  }
