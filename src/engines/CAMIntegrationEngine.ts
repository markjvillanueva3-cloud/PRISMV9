...
function calculateRecommendation(op: CAMOperation): CAMRecommendation {
  ...
  notes.push("Cycle time is a nominal benchmark estimate in this engine; actual feature geometry and toolpath length are not modeled here");
  ...
  switch (action) {
    case "cam_recommend": {
      ...
      return {
        ...
        } as CAMRecommendation["recommended"] & { rpm: number; sfm: number; fz_mm: number; ap_mm: number; ae_mm: number },
    ...
    case "cam_analyze_op": {
      ...
      if (op.parameters) {
        ...
        // RPM comparison
        const userRpm = (p as Record<string, unknown>).rpm as number ?? p.speed_rpm ?? 0;
        if (userRpm > 0) {
        ...
        // Depth comparison
        const userAp = (p as Record<string, unknown>).ap_mm as number ?? p.axial_depth_mm ?? 0;
        if (userAp > 0) {
        ...
        // Width comparison
        const userAe = (p as Record<string, unknown>).ae_mm as number ?? p.radial_depth_mm ?? 0;
        if (userAe > 0) {
