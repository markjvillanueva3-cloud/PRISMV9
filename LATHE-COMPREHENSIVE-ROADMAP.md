...
3-LOOP QUALITY PROTOCOL (MANDATORY for EVERY unit):
  ...
  Only after ALL 3 LOOPS pass → next unit
...
| | **→ 3-LOOP each unit: SCRUTINIZE → GAP FILL → TIE UP (see protocol at top)** |
...
| | **→ 3-LOOP each unit: SCRUTINIZE → GAP FILL → TIE UP** |
...
```
1. Final 3-LOOP pass on all units in this milestone (SCRUTINIZE → GAP FILL → TIE UP)
...
7. /compact (save what was done + 3-loop results + what's next)
...
> **3-LOOP GATE:** SCRUTINIZE (`/prism-review` + `/scrutinize`) → GAP FILL (`/test` + `/trace` wiring + edge cases) → TIE UP (no TODOs, reasoning[], golden snapshot). **`/compact` here if 3+ units done.**
...
See C:/PRISM/CAMX-CODE-REVIEW-PROTOCOL.md for full details.
See C:/PRISM/CAMX-ROADMAP-v22-QUALITY-FIXES.md for quality fix requirements.
