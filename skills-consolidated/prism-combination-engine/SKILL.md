...
async function loadResourceData() {
  const RESOURCE_REGISTRY = await load("H:/prism/data/coordination/RESOURCE_REGISTRY.json");
  const CAPABILITY_MATRIX = await load("H:/prism/data/coordination/CAPABILITY_MATRIX.json");
  const SYNERGY_MATRIX = await load("H:/prism/data/coordination/SYNERGY_MATRIX.json");
...
| File | Path | Contents |
|------|------|----------|
| RESOURCE_REGISTRY | H:\prism\data\coordination\RESOURCE_REGISTRY.json | 691 resources |
| CAPABILITY_MATRIX | H:\prism\data\coordination\CAPABILITY_MATRIX.json | Resource-task matching |
| SYNERGY_MATRIX | H:\prism\data\coordination\SYNERGY_MATRIX.json | 150+ pairwise synergies |
| FORMULA_REGISTRY | H:\prism\data\FORMULA_REGISTRY.json | 22 formulas |
