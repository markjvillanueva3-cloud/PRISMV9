...
### When To Use
- Trigger keywords: "tool", "selector", "instant", "selection", "development", "eliminates", "confusion"
- Task requires reference knowledge from this skill domain.

### How To Use
1. Load skill: `prism_skill_script→skill_content(id="prism-tool-selector")`
2. Apply relevant knowledge to current task context
3. Cross-reference with related dispatchers:
   - prism_skill_script→skill_content(id="prism-tool-selector") to load this skill
...
### Examples
**Example 1**: User asks about tool
→ Load skill: skill_content("prism-tool-selector") → Apply relevant knowledge → Provide structured response
...
// ✅ CORRECT - persistent
Filesystem:write_file({ path: "C:\\PRISM\\module.js", ... })
```

### WRONG: Using view for user's files
```javascript
// ❌ WRONG - view is for container
view({ path: "C:\\PRISM\\file.js" })

// ✅ CORRECT
Filesystem:read_file({ path: "C:\\PRISM\\file.js" })
```
