...
const REQUIRED_KEYS = [
  ...
  // U-CLT-TRAINING-SOURCES (schema 1.1.0, additive): the 3 new advisory blocks.
  "docustrata_invoices_match", "vendor_cost_index_advisory", "tool_purchases_advisory",
];
...
test("full cycle → complete, correctly-typed snapshot", () => {
  ...
  assert.equal(s.schemaVersion, "1.1.0"); // bumped minor for the 3 additive advisory blocks (U-CLT-TRAINING-SOURCES)
