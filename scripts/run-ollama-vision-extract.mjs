...
function appendEvent(eventsFile, event) {
  // Delegates to the canonical appender (U-BPA-EVENT-WRITER-LIB) -- preserves the
  // {success, error?} contract the caller reads (~line 277); the additive
  // written_to on success is harmless. buildPageEvent always sets a string type,
  // so the appender's fail-loud type guard never fires on this path.
  return appendAccuracyEvent(event, { path: eventsFile });
}
