...
import {
  ...
  offlineMfgLogic,
  isTransientOllamaError, ollamaGenerate,
} from "./cad-text-to-cadquery.mjs";

test("isTransientOllamaError: server-busy/timeout are transient; parse/normal are terminal (U-CAD-OLLAMA-RETRY)", () => {
  assert.equal(isTransientOllamaError("server busy, please try again. maximum pending requests exceeded"), true);
  assert.equal(isTransientOllamaError("curl exit 28 timed out"), true);
  assert.equal(isTransientOllamaError("connection reset"), true);
  assert.equal(isTransientOllamaError("ollama response parse: Unexpected token"), false);
  assert.equal(isTransientOllamaError(""), false);
});

test("ollamaGenerate: RETRIES a transient 'server busy' then succeeds", async () => {
  let n = 0;
  const runner = () => { n++; return n < 2 ? { status: 0, stdout: JSON.stringify({ error: "server busy, please try again. maximum pending requests exceeded" }) } : { status: 0, stdout: JSON.stringify({ response: "ok-code" }) }; };
  const res = await ollamaGenerate("p", "m", { runner, sleep: async () => {}, maxAttempts: 3 });
  assert.equal(res.ok, true);
  assert.equal(res.text, "ok-code");
  assert.equal(n, 2, "one retry after the busy response");
});

test("ollamaGenerate: a TERMINAL error fails fast (no retry); persistent transient exhausts retries", async () => {
  let nTerm = 0;
  const termRes = await ollamaGenerate("p", "m", { runner: () => { nTerm++; return { status: 0, stdout: "not-json" }; }, sleep: async () => {}, maxAttempts: 3 });
  assert.equal(termRes.ok, false);
  assert.equal(nTerm, 1, "parse error is terminal -> no retry");
  let nBusy = 0;
  const busyRes = await ollamaGenerate("p", "m", { runner: () => { nBusy++; return { status: 0, stdout: JSON.stringify({ error: "server busy" }) }; }, sleep: async () => {}, maxAttempts: 3 });
  assert.equal(busyRes.ok, false);
  assert.ok(busyRes.transient);
  assert.equal(nBusy, 3, "3 attempts then give up");
});
...
test("buildPrompt: a METRIC request gets a loud mm-native override (no /25.4 + radius=D/2); an INCH request does not (U-CAD-METRIC-PROMPT-FIX)", () => {
  // live finding: 41% of metric gens divided by 25.4 despite the generic rule -> a request-specific override
  const m = buildPrompt("a 31.68 mm diameter cylinder 19.81 mm long");
  assert.match(m, /THIS REQUEST IS IN MILLIMETRES/);
  assert.match(m, /circle\(D\/2\)/, "gives the correct radius=D/2 cylinder idiom (circle() takes the RADIUS)");
  assert.match(m, /Do NOT divide OR multiply/i, "explicit no-25.4 for the metric case");
  const inch = buildPrompt("a 2 inch cube");
  assert.ok(!/THIS REQUEST IS IN MILLIMETRES/.test(inch), "an inch request keeps the inch default -- no metric override");
});
...
test("classifyGenerationOutcome: offline CURVED-diameter fails a wrong-diameter cylinder; correct passes; kernel still wins (U-CAD-CURVED-DIM-LEARN)", () => {
  // curved part, Fusion-free, wrong diameter via the STEP radius entity -> FAIL offline (the bbox self-check must pass it by default)
  const bad = classifyGenerationOutcome({ status: { executed: true, analysisExit: 0, selfCheck: { accurate: false, bboxReliable: false, bboxAdvisory: true, curvedDim: { applicable: true, accurate: false, matchedDiaMm: 20, requestedDiaMm: 15.88, diaDeltaPct: 25.9, lenDeltaPct: null, deltaPct: 25.9 } } } });
  assert.equal(bad.status, "fail");
  assert.match(bad.reason, /curved dim off: dia 25\.9% \/ len n\/a%/);
  // correct diameter -> pass, EVEN THOUGH the bbox marked it advisory-inaccurate (this is the correction)
  assert.deepEqual(classifyGenerationOutcome({ status: { executed: true, analysisExit: 0, selfCheck: { accurate: false, bboxReliable: false, bboxAdvisory: true, curvedDim: { applicable: true, accurate: true } } } }), { status: "pass" });
  // no diameter request (not applicable) -> pass, no false fail (backward compat with the plain-curved case)
  assert.deepEqual(classifyGenerationOutcome({ status: { executed: true, analysisExit: 0, selfCheck: { accurate: false, bboxReliable: false, bboxAdvisory: true, curvedDim: { applicable: false } } } }), { status: "pass" });
  // authoritative kernel verdict present -> curved-dim path skipped (kernel wins)
  assert.deepEqual(classifyGenerationOutcome({ status: { executed: true, analysisExit: 0, kernelAccuracy: { accurate: true }, selfCheck: { curvedDim: { applicable: true, accurate: false } } } }), { status: "pass" });
});
