...
flags: --synth --model <n> --json --max-hits <n> --timeout <ms> --allow-unsafe`;
...
export async function runRequest(parsed, deps = {}) {
  ...
  // Host-aware model selection (BLACKWELL-TOKEN-SYNERGY-MS0/U-BW-SYNTH-CONSUMERS).
  // An explicit --model wins (operator intent); flags.model is "" when none was
  // passed (parseArgs sentinel — unambiguous), so otherwise we resolve the best
  // LOCAL model for THIS host: the 32B on the 96GB Blackwell, the conservative
  // DEFAULT_MODEL (3B) on a weak host or when Ollama is down. The 3b-avoids-page-
  // thrash rationale only held on a small-VRAM box; on the idle 96GB GPU the 32B
  // is free. resolveSynthesisModel is async + fail-soft (never throws here). The
  // resolver is injectable via deps so runRequest stays fully unit-testable.
  const model = flags.model
    ? pickModel(flags.model)
    : ((await (deps.resolveSynthesisModel || resolveSynthesisModel)({ fallback: DEFAULT_MODEL })).model || DEFAULT_MODEL);
  ...
  if (mode === "viz") {
    ...
    const footer = `↓ viz: scanned ${scanned} graph nodes locally → ${hits.length} hit(s) returned to Claude`;
  ...
  if (mode === "ask") {
    const gen = await (deps.callModel || callModel)(model, buildAskPrompt(input), {
      timeoutMs: flags.timeout,
      callOllamaImpl: deps.callOllama,
      callViaMcpImpl: deps.callViaMcp,
    });
  ...
  const gen = await (deps.callModel || callModel)(model, buildFilePrompt(mode, displayName, file.content), {
