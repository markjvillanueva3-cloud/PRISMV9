...
function runPerTaskSearches(taskNote, subagentType) {
  ...
  // FLEET-SEARCH-DAEMON-MS0 (opt-in PRISM_SUBAGENT_DAEMON_SEARCH=1): query the warm
  // search daemon -- full-coverage search with NO local graph load, so the OOM that
  // gated this OFF (PRISM_MASTER_INDEX_INJECT=0 fleet-default) no longer applies.
  // Zero cost when off (the default); falls back to the gated in-process path on a
  // daemon miss (down/disabled).
  let miFromDaemon = false;
  if (process.env.PRISM_SUBAGENT_DAEMON_SEARCH === "1") {
    try {
      const dm = searchViaDaemonSync(taskNote, { topK });
      if (dm) { mi = { tokens: dm.tokens, hits: dm.hits }; miFromDaemon = true; }
      const dt = searchViaDaemonSync(taskNote, { topK, kind: "tribal" });
      if (dt) { tribal = { tokens: dt.tokens, hits: dt.hits, prefDomain }; }
    } catch { /* fail-safe */ }
  }
  if (!miFromDaemon && process.env.PRISM_MASTER_INDEX_INJECT !== "0") {
