...
// --- resolveStartLogPath ---
test("resolveStartLogPath: default path when env unset", () => {
  assert.equal(resolveStartLogPath({}), "H:/prism/state/shared/hermes-proxy-start.log");
});

test("resolveStartLogPath: PRISM_HERMES_PROXY_LOG override wins", () => {
  assert.equal(resolveStartLogPath({ PRISM_HERMES_PROXY_LOG: "D:/x/h.log" }), "D:/x/h.log");
});

// --- buildStartStdio ---
test("buildStartStdio: a numeric log fd tees stdout+stderr to it (NOT silently swallowed)", () => {
  // The R12 fail-loud fix: stdout/stderr go to the SAME log fd so a start failure
  // (e.g. the missing-aiohttp dep) is captured, not lost to stdio:"ignore".
  assert.deepEqual(buildStartStdio(7), ["ignore", 7, 7]);
  assert.deepEqual(buildStartStdio(0), ["ignore", 0, 0]); // fd 0 is a valid (falsy-looking) descriptor
});

test("buildStartStdio: no fd (log open failed) falls back to 'ignore' so logging never blocks the keepalive", () => {
  assert.equal(buildStartStdio(null), "ignore");
  assert.equal(buildStartStdio(undefined), "ignore");
});
