...
/** Best-effort native recovery: reap orphan llama-server (dead parent) + restart the serve task. */
function recover() {
  const ps = `
...
try { Stop-ScheduledTask -TaskName '${SERVE_TASK}' -ErrorAction Stop } catch {}
Start-Sleep -Seconds 2
Get-Process ollama,'llama-server' -ErrorAction SilentlyContinue | ForEach-Object { try { Stop-Process -Id $_.Id -Force } catch {} }
Start-Sleep -Seconds 2
try { Start-ScheduledTask -TaskName '${SERVE_TASK}' -ErrorAction Stop; Write-Output 'serve-restarted' } catch { Write-Output ('start-fail: ' + $_.Exception.Message) }
`;
