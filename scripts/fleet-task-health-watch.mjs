...
// --- Heal-verify (U-GOLF-HEAL-VERIFY-LEG) constants -------------------------
// A G10 auto-re-enable reports a task `healed` the moment Enable-ScheduledTask
// returns OK -- but ENABLED != RAN. These bound the "did the heal actually
// take?" verification on the NEXT audit (see classifyHealEffectiveness).
//   GRACE_MULT       -- a healed task gets this many trigger intervals to fire
//                       before a still-never-ran is called INEFFECTIVE.
//   FALLBACK_GRACE   -- grace when the task carries no repetition interval
//                       (AtStartup-only) and an interval window cannot apply.
//   LOOKBACK         -- only verify heals this recent; an ancient heal that has
//                       been running fine for days is not worth re-litigating.
export const DEFAULT_HEAL_VERIFY_GRACE_MULT = 2;
export const DEFAULT_HEAL_VERIFY_FALLBACK_GRACE_MS = 60 * 60 * 1000;   // 1 h
export const DEFAULT_HEAL_VERIFY_LOOKBACK_MS = 24 * 60 * 60 * 1000;    // 24 h
...
export const CRASH_CRITICAL_TASKS = [
  ...
  // 2026-06-17 (slot:golf): added "PRISM MCP Server" + "PRISM MCP Server
  // Watchdog" -- THE most crash-critical service (without :3100 EVERY chat loses
  // prism). They were in KNOWN_PRISM_TASKS but NOT here, so when both landed in
  // the Disabled state mid-session (the live supervisor orphaned, durable restart
  // layer off) selectReenableTargets never selected them -> they stayed disabled
  // -> any server death was permanent = the operator's recurring "mcp server
  // disconnect". Adding them makes the self-heal cover :3100 too. EXPECTED_DISABLED
  // is empty, so a deliberate disable is still honored if ever added there. Ref
  // reference_mcp_durable_tasks_disabled_orphan_supervisor_2026_06_17.
  "PRISM MCP Server",
  "PRISM MCP Server Watchdog",
  // 2026-06-17 (slot:golf): added "PRISM Fleet Task Health" -- the cadence
  // self-healer (the --once watchdog that re-enables disabled crash-critical
  // tasks) was itself DISABLED and NOT crash-critical, so nothing re-enabled IT
  // and its 5-min self-heal was dormant (only chat-Stops triggered the audit).
  // Making it crash-critical SELF-PROTECTS the self-healer: the Stop-hook audit
  // re-enables it if ever disabled again. Also added to KNOWN_PRISM_TASKS below.
  "PRISM Fleet Task Health",
...
export const KNOWN_PRISM_TASKS = [
  ...
  "PRISM Fleet Task Health",               // 2026-06-17 (golf): self-healer in KNOWN so it satisfies the crash-critical subset invariant
...
export const EXPECTED_DISABLED_TASKS = [
  ...
  // NB: the permanently-deliberate set is now sourced DYNAMICALLY from the shared
  // ledger via readDeliberatelyDisabledTasks() (merged into the expectedDisabled
  // config below) -- keep this static array for any name NOT in that ledger.
];

/**
 * Read the deliberately-disabled ledger (state/shared/fleet/deliberately-disabled-tasks.json) --
 * the SAME ledger harden-prism-tasks.ps1 honors -- and return its task names. This puts the two
 * self-healers (the PS hardener that keeps tasks dark + this watcher that decides what is
 * "expected" dark) on ONE source of truth, so a permanently-deliberate task is never
 * false-flagged as degraded -- INCLUDING after the HW-migration freeze flag is deleted (which
 * the static list could not survive; scrutiny arm-C P1, U-CRON-LEDGER-GUARD).
 * Fail-soft: missing/corrupt ledger -> []. MIRRORS the hardener's crash-critical guard -- a
 * safety-net task in the ledger is IGNORED here too, so it stays degraded and gets re-enabled.
 */
export function readDeliberatelyDisabledTasks() {
  try {
    const p = join(SHARED_DIR, "fleet", "deliberately-disabled-tasks.json");
    if (!existsSync(p)) return [];
    const parsed = JSON.parse(readFileSync(p, "utf8"));
    const names = Array.isArray(parsed.tasks) ? parsed.tasks.map((t) => t && t.name).filter(Boolean) : [];
    const crit = new Set([...MUST_EXIST_TASKS, ...CRASH_CRITICAL_TASKS]);
    return names.filter((n) => !crit.has(n));
  } catch { return []; }
}
...
export function discoverInstallerTasks({ helpersDir = INSTALLERS_DIR, _io = {} } = {}) {
  ...
  for (const fname of entries) {
    ...
    for (const m of text.matchAll(NAME_RE)) {
      ...
      // Skip an unexpanded PowerShell template literal (e.g. "PRISM Galaxy Mine
      // ($Galaxy)") -- a `$` means a runtime variable, not a real task name. The
      // galaxy-mine installer registers the EXPANDED per-galaxy names ("PRISM
      // Galaxy Mine (mill)" ...) at runtime; capturing the literal `$Galaxy` is a
      // phantom that false-flags as installer-drift (U-FTH-DOLLAR-SKIP 2026-06-21).
...
export function classifyTask(t, nowMs, cfg) {
  ...
  // A task with a valid FUTURE NextRunTime is NOT stale -- it is correctly idle
  // between scheduled runs (e.g. an overnight-windowed task: a daily trigger with
  // a bounded intraday repetition window, waiting for tonight's window). Without
  // this gate every windowed task false-flags `stale` during its idle hours,
  // because lastRun legitimately exceeds interval*mult between windows. A past /
  // overdue NextRun is already caught above as `trigger-stalled`; a null / absent
  // NextRun (genuinely no upcoming run) still flags here, so real staleness -- a
  // task that ran long ago AND has nothing scheduled -- is not masked.
  if (Number.isFinite(t.intervalMs) && t.intervalMs > 0) {
    const waitingForFutureRun = Number.isFinite(t.nextRunTimeMs) && t.nextRunTimeMs > nowMs;
    const ageMs = nowMs - t.lastRunTimeMs;
    const limitMs = t.intervalMs * cfg.staleMultiplier;
    if (ageMs > limitMs && !waitingForFutureRun) {
      return {
        status: "stale",
        reason: `last ran ${Math.round(ageMs / 60000)}min ago with no upcoming scheduled run -- exceeds ${Math.round(limitMs / 60000)}min `
...
// --- Heal-verify (U-GOLF-HEAL-VERIFY-LEG) -----------------------------------
// THE GAP. The G10 auto-re-enable guard (selectReenableTargets -> reenableTasks)
// reports a task `healed` the instant Enable-ScheduledTask returns OK. But an
// ENABLED task is not a HEALED task: it can still never fire (a stalled trigger),
// fire-and-fail (a launch-failure LastTaskResult), or be re-disabled faster than
// the guard re-enables it (flapping). The Stop-hook advisory even says
// "auto-re-enabled N -- verify next audit", but nothing ever performed that
// verification -- so a crash-critical task could read "healed" every audit while
// never actually running, and the guard would blindly re-enable it forever (a
// destructive rekick loop on a root cause it cannot fix).
//
// This leg closes that: each audit reads the reenable-ledger's prior ok:true
// heals and checks each against the task's CURRENT LastRunTime. ran-after-heal =
// effective; still-never-ran past the grace window = INEFFECTIVE (surfaced so the
// operator fixes the root cause / re-registers elevated, instead of trusting a
// false "healed"). Pure assessment + reporting ONLY -- it NEVER kills, disables,
// or re-enables (golf-soul: the R12 backstop that runs BEFORE a destructive
// rekick, it does not become one).

/**
 * Classify whether a single auto-re-enable actually took effect, by comparing
 * the task's LastRunTime against when it was healed.
 *
 *   effective   -- the task ran STRICTLY after it was healed (lastRun > healedAt).
 *   pending     -- not yet run-after-heal, but still inside the grace window
 *                  (intervalMs * graceMult, or fallbackGraceMs when no interval).
 *   ineffective -- not run-after-heal AND the grace window has elapsed: the
 *                  re-enable did not take (stalled trigger / re-disabled / dead).
 *
 * Pure -- no IO, deterministic (R9). Defensive against missing/NaN inputs:
 * an absent healedAt or a future healedAt (clock skew) is `pending`, never a
 * false `ineffective` (over-reporting a still-fine heal is the unsafe direction).
 *
 * @param {{healedAtMs:number, lastRunTimeMs:number|null, intervalMs:number|null,
 *          nowMs:number, graceMult?:number, fallbackGraceMs?:number}} p
 * @returns {{outcome:"effective"|"pending"|"ineffective", reason:string}}
 */
export function classifyHealEffectiveness(p) {
  const healedAtMs = p && p.healedAtMs;
  const lastRunTimeMs = p && p.lastRunTimeMs;
  const intervalMs = p && p.intervalMs;
  const nowMs = p && p.nowMs;
  const graceMult = Number.isFinite(p && p.graceMult) && p.graceMult > 0
    ? p.graceMult : DEFAULT_HEAL_VERIFY_GRACE_MULT;
  const fallbackGraceMs = Number.isFinite(p && p.fallbackGraceMs) && p.fallbackGraceMs > 0
    ? p.fallbackGraceMs : DEFAULT_HEAL_VERIFY_FALLBACK_GRACE_MS;

  if (!Number.isFinite(healedAtMs)) return { outcome: "pending", reason: "no healedAt timestamp" };
  if (Number.isFinite(nowMs) && healedAtMs > nowMs) {
    return { outcome: "pending", reason: "healedAt is in the future (clock skew) -- cannot verify yet" };
  }
  // Effective: ran AFTER the heal. Strict `>` so the same-millisecond run that
  // triggered the heal (or a stale pre-heal run) is never counted as proof.
  if (Number.isFinite(lastRunTimeMs) && lastRunTimeMs > healedAtMs) {
    return { outcome: "effective", reason: `ran ${Math.round((lastRunTimeMs - healedAtMs) / 60000)}min after the re-enable` };
  }
  const graceMs = (Number.isFinite(intervalMs) && intervalMs > 0)
    ? intervalMs * graceMult : fallbackGraceMs;
  const sinceHealMs = Number.isFinite(nowMs) ? nowMs - healedAtMs : 0;
  if (sinceHealMs < graceMs) {
    return {
      outcome: "pending",
      reason: `re-enabled ${Math.round(sinceHealMs / 60000)}min ago -- within the ${Math.round(graceMs / 60000)}min grace window`,
    };
  }
  return {
    outcome: "ineffective",
    reason: `re-enabled ${Math.round(sinceHealMs / 60000)}min ago but has NOT run since `
      + `(${Math.round(graceMs / 60000)}min grace elapsed) -- the re-enable did not take`,
  };
}

/**
 * Parse the reenable-ledger JSONL text into the most-recent successful heal
 * timestamp per task, within the lookback window. A heal row is
 * {schemaVersion, ts, task, ok, by}; only ok:true rows are heals (an ok:false
 * row was a failed-needs-elevation enable, already reported failed, never claimed
 * healed -- so it is not subject to verify). Malformed lines are skipped, never
 * thrown. Pure -- no IO (the file read is readReenableLedgerText).
 *
 * @param {string} ledgerText
 * @param {{nowMs?:number, lookbackMs?:number}} [opts]
 * @returns {Map<string, number>} task -> healedAtMs (most recent ok:true in window)
 */
export function parseRecentHeals(ledgerText, opts = {}) {
  const nowMs = Number.isFinite(opts.nowMs) ? opts.nowMs : Date.now();
  const lookbackMs = Number.isFinite(opts.lookbackMs) && opts.lookbackMs > 0
    ? opts.lookbackMs : DEFAULT_HEAL_VERIFY_LOOKBACK_MS;
  const out = new Map();
  if (typeof ledgerText !== "string" || !ledgerText) return out;
  for (const line of ledgerText.split(/\r?\n/)) {
    const s = line.trim();
    if (!s) continue;
    let row;
    try { row = JSON.parse(s); } catch { continue; }
    if (!row || row.ok !== true || typeof row.task !== "string") continue;
    const tsMs = row.ts ? Date.parse(row.ts) : NaN;
    if (!Number.isFinite(tsMs)) continue;
    if (nowMs - tsMs > lookbackMs) continue;   // too old to re-litigate
    const prev = out.get(row.task);
    if (prev === undefined || tsMs > prev) out.set(row.task, tsMs);
  }
  return out;
}

/**
 * Read the reenable-ledger text (fail-soft). Missing file / read error -> "".
 * The ONLY IO in the heal-verify path; everything else is pure. Injectable for
 * tests via io.{existsSync,readFileSync}.
 */
export function readReenableLedgerText(path = REENABLE_LEDGER_PATH, io = {}) {
  const _exists = io.existsSync || existsSync;
  const _read = io.readFileSync || readFileSync;
  const readOne = (p) => {
    try { return _exists(p) ? (_read(p, "utf8") || "") : ""; } catch { return ""; }
  };
  // Read the rotated generation too. appendReenableLedger rotates the live file
  // to `<path>.1` at REENABLE_LEDGER_ROTATE_BYTES, so a recent heal can live in
  // `.1` while still inside the 24h lookback -- reading only the live file would
  // silently drop it (a fail-safe miss, but a miss). parseRecentHeals dedups to
  // the most-recent ts per task and drops out-of-window rows, so concatenating
  // older(.1)+live is order/overlap-safe.
  const older = readOne(path + ".1");
  const live = readOne(path);
  if (!older) return live;
  if (!live) return older;
  return older + "\n" + live;
}

/**
 * Assess every recently-healed task against the freshly-classified sample.
 * Returns the three-way partition the telemetry row + Stop advisory consume.
 * A healed task NOT present in this sample (vanished / unregistered) is skipped
 * -- missing-detection (aggregateHealth) already owns the vanished-task signal;
 * heal-verify only judges tasks it can actually see a current LastRunTime for.
 * Pure -- no IO, deterministic (sorted output).
 *
 * @param {Array<{name:string, lastRunTime:string|null, intervalMs:number|null}>} classified
 * @param {Map<string, number>} recentHeals  task -> healedAtMs (parseRecentHeals)
 * @param {number} nowMs
 * @param {{graceMult?:number, fallbackGraceMs?:number}} [cfg]
 * @returns {{effective:Array, pending:Array, ineffective:Array}}
 */
export function assessHealEffectiveness(classified, recentHeals, nowMs, cfg = {}) {
  const result = { effective: [], pending: [], ineffective: [] };
  if (!(recentHeals instanceof Map) || recentHeals.size === 0) return result;
  const byName = new Map();
  if (Array.isArray(classified)) {
    for (const t of classified) if (t && typeof t.name === "string") byName.set(t.name, t);
  }
  for (const [task, healedAtMs] of recentHeals) {
    const t = byName.get(task);
    if (!t) continue;   // healed but not in this sample -> missing-detection owns it
    const lrtMs = t.lastRunTime ? Date.parse(t.lastRunTime) : null;
    const { outcome, reason } = classifyHealEffectiveness({
      healedAtMs,
      lastRunTimeMs: Number.isFinite(lrtMs) ? lrtMs : null,
      intervalMs: t.intervalMs,
      nowMs,
      graceMult: cfg.graceMult,
      fallbackGraceMs: cfg.fallbackGraceMs,
    });
    result[outcome].push({
      name: task,
      healedAt: new Date(healedAtMs).toISOString(),
      lastRunTime: t.lastRunTime || null,
      reason,
    });
  }
  for (const k of ["effective", "pending", "ineffective"]) {
...
export function runOnce(opts = {}) {
  ...
  const agg = aggregateHealth(classified, {
    ...
    expectedDisabled: [...EXPECTED_DISABLED_TASKS, ...readDeliberatelyDisabledTasks()],
  ...
  // HEAL-VERIFY (U-GOLF-HEAL-VERIFY-LEG): verify PRIOR audits' auto-re-enables
  // actually took -- read the reenable-ledger's ok:true heals and compare each
  // against the freshly-sampled task's CURRENT LastRunTime. Read-only assessment
  // (safe in dryRun too); fail-soft so a ledger read error never aborts the audit
  // (R12) and never mutates/re-kicks (golf-soul: the backstop BEFORE a rekick).
  let healVerify = { effective: [], pending: [], ineffective: [] };
  try {
    const recentHeals = parseRecentHeals(
      readReenableLedgerText(opts.reenableLedgerPath, opts._healLedgerIo || {}),
      { nowMs, lookbackMs: opts.healVerifyLookbackMs },
    );
    healVerify = assessHealEffectiveness(classified, recentHeals, nowMs, {
      graceMult: opts.healVerifyGraceMult,
      fallbackGraceMs: opts.healVerifyFallbackGraceMs,
    });
  } catch { /* fail-soft -- empty verdict, audit continues */ }
  ...
  const row = {
    ...
    healVerify,     // {effective,pending,ineffective} (U-GOLF-HEAL-VERIFY-LEG): did prior heals take?
  };
  ...
  return { row, level: agg.level, aggregate: agg, classified, advisory: adv, writes, exitCode, cfg, dryRun, disabled, autoReenable, healVerify };
}
