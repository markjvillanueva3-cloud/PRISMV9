...
/* ---------------- runNliLint: confirm-resample / stochastic-verdict stabilization (U-VAULT-NLI-VOTE) ---------------- */
// scripted callImpl returns seq[n] in call order (primary, confirm1, confirm2, ...), then a default CONSISTENT.
const scripted = (seq) => { let n = 0; return async () => seq[n++] ?? { ok: true, text: "CONSISTENT\ndefault" }; };
const C = { ok: true, text: "CONTRADICT\nclaims conflict" };
const K = { ok: true, text: "CONSISTENT\nthey agree" };

test("confirmSamples=2: unanimous CONTRADICT (3/3) -> recorded with votes + confirm totals", async () => {
  const rep = await runNliLint(lintPages, { callImpl: scripted([C, C, C]), limit: 10, confirmSamples: 2 });
  assert.equal(rep.totals.contradictions, 1);
  assert.deepEqual(rep.contradictions[0].votes, { contradict: 3, total: 3 });
  assert.equal(rep.totals.confirmSamples, 2);
  assert.equal(rep.totals.confirmCalls, 2);   // 2 confirm calls on the single contradict pair
  assert.equal(rep.totals.pairsChecked, 1);
});

test("confirmSamples=2: a single-sample FLAKE (1/3) is NOT recorded (the false-positive class this fixes)", async () => {
  const rep = await runNliLint(lintPages, { callImpl: scripted([C, K, K]), limit: 10, confirmSamples: 2 });
  assert.equal(rep.totals.contradictions, 0);  // primary said contradict, 2 confirms disagreed -> dropped
  assert.equal(rep.totals.pairsChecked, 1);    // still counted as checked (we examined the pair)
  assert.equal(rep.totals.confirmCalls, 2);
});

test("confirmSamples=2: a bare majority (2/3) IS recorded", async () => {
  const rep = await runNliLint(lintPages, { callImpl: scripted([C, C, K]), limit: 10, confirmSamples: 2 });
  assert.equal(rep.totals.contradictions, 1);
  assert.deepEqual(rep.contradictions[0].votes, { contradict: 2, total: 3 });
});

test("confirmSamples=2: failed confirm calls -> conservative drop, never throws, breaker untouched", async () => {
  const rep = await runNliLint(lintPages, { callImpl: scripted([C, { ok: false }, { ok: false }]), limit: 10, confirmSamples: 2 });
  assert.equal(rep.totals.contradictions, 0);  // 1 contradict vote, confirms unusable -> not corroborated
  assert.equal(rep.totals.pairsChecked, 1);    // primary succeeded -> checked, NOT unchecked
  assert.equal(rep.totals.unchecked, 0);
  assert.equal(rep.aborted, undefined);        // confirm failures do NOT feed the circuit breaker
});

test("confirmSamples=0 (default): legacy single-sample shape -- no votes field, no confirm totals", async () => {
  const rep = await runNliLint(lintPages, { callImpl: scripted([C]), limit: 10 });
  assert.equal(rep.totals.contradictions, 1);
  assert.equal("votes" in rep.contradictions[0], false);   // byte-identical legacy contradiction entry
  assert.equal("confirmSamples" in rep.totals, false);
  assert.equal("confirmCalls" in rep.totals, false);
});

test("confirmSamples=1: even total -> needs BOTH votes (strict majority of 2)", async () => {
  // total=2, required=floor(2/2)+1=2 -> primary AND its one confirm must both contradict.
  const both = await runNliLint(lintPages, { callImpl: scripted([C, C]), limit: 10, confirmSamples: 1 });
  assert.equal(both.totals.contradictions, 1);
  assert.deepEqual(both.contradictions[0].votes, { contradict: 2, total: 2 });
  const split = await runNliLint(lintPages, { callImpl: scripted([C, K]), limit: 10, confirmSamples: 1 });
  assert.equal(split.totals.contradictions, 0);  // 1-of-2 is not a strict majority
});

/* ---------------- runNliLint: wall-clock budget / partial honest report (U-VAULT-NLI-BUDGET) ---------------- */
// 4 fully-overlapping pages -> 6 candidate pairs. A clock that advances per callImpl call
// drives elapsed time deterministically: nowFn READS it, callImpl STEPS it.
const budgetPages = [
  mkPage("lessons/a.md", ["t1", "t2", "t3"], "claim a"),
  mkPage("lessons/b.md", ["t1", "t2", "t3"], "claim b"),
  mkPage("lessons/c.md", ["t1", "t2", "t3"], "claim c"),
...
const stepClock = (stepMs) => { let t = 0; return { nowFn: () => t, step: () => { t += stepMs; } }; };

test("budgetMs: stops starting new pairs once the budget is spent -> partial honest report", async () => {
  const clk = stepClock(1000);
  const callImpl = async () => { clk.step(); return { ok: true, text: "CONSISTENT\nok" }; };
  // budget 2500ms, 1000ms/pair: pairs start at t=0,1000,2000 (checked); at t=3000 the 4th is skipped.
  const rep = await runNliLint(budgetPages, { callImpl, limit: 10, budgetMs: 2500, nowFn: clk.nowFn });
  assert.equal(rep.budgetExceeded, true);
  assert.equal(rep.totals.pairsChecked, 3);
  assert.equal(rep.totals.notAttempted, 3);   // 6 considered - 3 checked - 0 unchecked
  assert.equal(rep.totals.budgetMs, 2500);
  assert.match(rep.budgetReason, /budget 2500ms reached/);
});

test("budgetMs large enough to finish -> NOT budgetExceeded, no notAttempted", async () => {
  const clk = stepClock(1000);
  const callImpl = async () => { clk.step(); return { ok: true, text: "CONSISTENT\nok" }; };
  const rep = await runNliLint(budgetPages, { callImpl, limit: 10, budgetMs: 100000, nowFn: clk.nowFn });
  assert.equal(rep.budgetExceeded, undefined);  // completed within budget
  assert.equal(rep.totals.pairsChecked, 6);
  assert.equal(rep.totals.budgetMs, 100000);    // budget surfaced even when not hit
  assert.equal("notAttempted" in rep.totals, false);
});

test("budgetMs=0 (default): unbounded, no budget fields (back-compat)", async () => {
  const callImpl = async () => ({ ok: true, text: "CONSISTENT\nok" });
  const rep = await runNliLint(budgetPages, { callImpl, limit: 10 });
  assert.equal(rep.totals.pairsChecked, 6);
  assert.equal(rep.budgetExceeded, undefined);
  assert.equal("budgetMs" in rep.totals, false);
  assert.equal("notAttempted" in rep.totals, false);
});

test("budgetMs: per-call timeout shrinks to remaining budget, floored at minCallTimeoutMs", async () => {
  const clk = stepClock(1000);
  const seen = [];
  const callImpl = async (_m, _p, opts) => { seen.push(opts.timeoutMs); clk.step(); return { ok: true, text: "CONSISTENT\nok" }; };
  // budget 5000, full timeout 90000, floor 2000, 1000ms/pair. Remaining at each start:
  // 5000,4000,3000,2000,1000 -> the last floors to 2000; the 6th pair is skipped at t=5000.
  const rep = await runNliLint(budgetPages, { callImpl, limit: 10, budgetMs: 5000, timeoutMs: 90000, minCallTimeoutMs: 2000, nowFn: clk.nowFn });
  assert.deepEqual(seen, [5000, 4000, 3000, 2000, 2000]);
  assert.ok(seen.every((to) => to <= 90000 && to >= 2000), "every call timeout within [floor, full]");
  assert.equal(rep.totals.pairsChecked, 5);
  assert.equal(rep.budgetExceeded, true);
});

test("budgetMs + confirmSamples: budget spent before confirms -> finding conservatively DROPPED (not single-sample)", async () => {
  // Bounds the overshoot: a CONTRADICT primary on the boundary pair must NOT spawn its full
  // confirm fan-out past the budget. Here the budget is spent right after the primary, so the
  // confirm loop's guard fires immediately -> 0 confirms -> votes 1 < majority 2 -> dropped.
  const clk = stepClock(1000);
  let calls = 0;
  const callImpl = async () => { calls++; clk.step(); return { ok: true, text: "CONTRADICT\nx" }; };
  const rep = await runNliLint(lintPages, { callImpl, limit: 10, confirmSamples: 2, budgetMs: 500, nowFn: clk.nowFn });
  assert.equal(calls, 1, "only the primary ran; confirm resampling skipped once budget spent (overshoot bounded)");
  assert.equal(rep.totals.confirmCalls, 0);
  assert.equal(rep.totals.contradictions, 0, "an unconfirmed boundary CONTRADICT is dropped, not recorded single-sample");
  assert.equal(rep.totals.pairsChecked, 1);
});
