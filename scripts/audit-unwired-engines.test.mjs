...
import { engineReferencedInConsumer, applyConsumerClassification, applyDormantBridgeClassification, isTypeOnlyModule, wireExemptMarker } from "./audit-unwired-engines.mjs";
...
// ── wireExemptMarker -- full-file scan (grafted from slot/whiskey; was a head-only
// 2KB blind spot that mis-classified bottom-tagged engines as UNWIRED) ──
test("wireExemptMarker: head-of-file marker -> exempt with reason", () => {
  const c = `// WIRE-EXEMPT: wrapped by FooSingleton\nexport class FooEngine {}\n`;
  const r = wireExemptMarker(c);
  assert.equal(r.exempt, true);
  assert.match(r.reason, /FooSingleton/);
});

test("wireExemptMarker: BOTTOM-of-file marker (past 2KB) -> exempt (the fix)", () => {
  // Simulate a large engine with the marker adjacent to the singleton at the bottom,
  // far past the old 2KB head window. Fails-on-revert if the scan goes back to head-only.
  const filler = "// padding line to push the marker past the first 2KB\n".repeat(80);
  const c = `export class BarEngine {}\n${filler}// WIRE-EXEMPT: physics calc library, consumed internally\nexport const barEngine = new BarEngine();\n`;
  assert.ok(c.indexOf("WIRE-EXEMPT") > 2000, "marker must be past 2KB for this test to be meaningful");
  const r = wireExemptMarker(c);
  assert.equal(r.exempt, true);
  assert.match(r.reason, /physics calc library/);
});

test("wireExemptMarker: no marker -> not exempt", () => {
  assert.deepEqual(wireExemptMarker("export class BazEngine {}\n"), { exempt: false, reason: null });
  assert.deepEqual(wireExemptMarker(""), { exempt: false, reason: null });
});

test("E2E: real LatheThermodynamicsEngine bottom-tag is detected as WIRE-EXEMPT", () => {
  const eng = path.join(PRISM_ROOT, "mcp-server/src/engines/LatheThermodynamicsEngine.ts");
  if (!existsSync(eng)) {
    assert.fail(`expected engine not found: ${eng}`);
  }
  const content = readFileSync(eng, "utf8");
  // The marker sits near the singleton export (~line 2827), well past 2KB.
  assert.ok(content.indexOf("WIRE-EXEMPT") > 2000, "this engine's marker must be past 2KB (else the test is moot)");
  assert.equal(wireExemptMarker(content).exempt, true, "bottom-tagged physics library must classify WIRE-EXEMPT, not UNWIRED");
});

// ── Real-file E2E -- multi-line await-import fail-on-revert oracle (slot/whiskey graft).
// turningProgramDispatcher wires these engines via the multi-line `await import(` form;
// before the `\s*`-before-`)` fix they were reported UNWIRED (false positive that
// inflated the orphan count by hundreds fleet-wide). Fails-on-revert.
test("E2E: turningProgramDispatcher wires emitter/advanced-ops via multi-line await import", () => {
  const disp = path.join(
    PRISM_ROOT,
  ...
  const content = readFileSync(disp, "utf8");
  assert.equal(
    engineReferencedInConsumer("LathePrintProgramEmitterEngine", content),
    true,
    "wired via multi-line await import (U-CL9) -- must detect as wired",
  );
  assert.equal(
    engineReferencedInConsumer("LatheAdvancedOperationsEngine", content),
    true,
    "wired via multi-line await import (U-CL12) -- must detect as wired",
  );
});
