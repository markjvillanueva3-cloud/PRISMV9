...
import { pickHighestCapabilityGrok } from "./lib/grok-capability-rank.mjs";
...
/** Resolve the model id: explicit arg > PRISM_HERMES_PREFERRED_MODEL pin > HIGHEST-CAPABILITY
 *  of /v1/models (ranked, not first-listed -- operator "grok highest capability") > configured
 *  fallback. Impure. `deps.env` overridable for tests (defaults to process.env). */
export async function resolveModel(explicit, deps) {
  ...
  try {
    ...
    if (ok && body && Array.isArray(body.data) && body.data.length) {
      ...
      const best = pickHighestCapabilityGrok(ids);
      if (best) return best;
    }
...
export function makeServer(deps = {}) {
  ...
  server.tool(
    ...
    "Ask Hermes (xAI Grok via OAuth -- the stronger-than-Ollama managed lane, free, runs OUTSIDE the Claude context window). Use for a text question/instruction that wants a capable model but should not spend Claude tokens. Returns the model's text answer.",
    {
      ...
      model: z.string().optional().describe("override the upstream model id (default: first /v1/models entry, else the configured fallback)"),
