...
//
// Today this is a STAGING surface — the actual 7-voice dispatch is not
...
function parseArgs(argv) {
  const out = {
    ...
    localOnly: process.env.PRISM_OCTOPUS_LOCAL_ONLY === "1",
  };
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--local-only") { out.localOnly = true; continue; }
  }
...
// Knob: PRISM_OCTOPUS_LIVE_DISPATCH=1 fires the REAL 7-voice consensus
...
async function main() {
  ...
  // 3. DISPATCH — fire the real 7-voice consensus when LIVE_DISPATCH is on AND
  ...
  // Require a GENUINE consensus (>=2 voices) before feeding the galaxy learning
  // loop -- a 1-voice "consensus" (e.g. the curated panel collapsed under VRAM
  // pressure) is a single model's opinion, not fleet consensus, and must NOT be
  // consumed as one (scrutiny arm-C P2). The run is still recorded in the ledger
  // (honest), just not published as a learnable outcome.
  let consumePublish = null;
  if (dispatchInfo.dispatched && dispatchInfo.ok && (dispatchInfo.successCount || 0) < 2) {
    process.stderr.write(`note: consensus seated only ${dispatchInfo.successCount || 0} voice(s); not publishing a <2-voice run as galaxy consensus\n`);
  }
  if (dispatchInfo.dispatched && dispatchInfo.ok && (dispatchInfo.successCount || 0) >= 2 && opts.domain) {
  ...
  const result = {
    ...
    note: dispatchInfo.dispatched
      ? "live 7-voice consensus dispatched (see consensus.verdict in ledger)"
