...
import { streamDegreeZeroNodeIds } from "./lib/graph-stream-degree.mjs";
...
//
// --graph (default OFF, U-LINT-ORPHAN-OOM 2026-06-09): regenerates the
// _disconnected-graph-nodes page, which loads the 643MB system-graph.json.
// readGraphStreaming MATERIALIZES that graph (it only dodges the V8 string-cap
// during parse, not the in-memory result), so on the current graph it OOMs the
// default ~380-512MB heap. A heap-OOM is a process ABORT the try/catch below
// CANNOT catch -- so it used to kill the PRIMARY wiki-orphan lint too (which is
// light and had already written its outputs). Gated OFF so the orphan lint --
// the vault-value primary -- always completes; the disconnected-nodes page is
// opt-in and the caller must supply heap (node --max-old-space-size=4096 ...).
// The real fix (a non-materializing streaming degree pass in graph-io.mjs) is
// sierra's -- same 643MB graph OOM that also breaks system-viz-query.
const FLAGS = {
  write: args.has("--write") || args.size === 0,
  section: args.has("--section"),
...
`;
      ...
      if (FLAGS.graph) try {
        // Streaming degree pass -- never materializes the 643MB graph, so this no
        // longer OOMs (U-GRAPH-STREAM-DEGREE). Holds only an edge-endpoint Set +
        // minimal node records on-heap; the file Buffer is off-heap.
        const { degreeZero: z, totalNodes, withParent } = streamDegreeZeroNodeIds(GRAPH_PATH);
        const byLayer = {}, byKind = {};
        for (const n of z) {
          byLayer[n.layer] = (byLayer[n.layer] || 0) + 1;
          byKind[n.kind] = (byKind[n.kind] || 0) + 1; // kind already resolved by minimalNodeRecord
        }
...
> ${z.length} of ${totalNodes} system-viz nodes have **zero edges** —
...
**Degree-0 nodes:** ${z.length} / ${totalNodes} (${((z.length / Math.max(1, totalNodes)) * 100).toFixed(1)}%) · **${withParent}** carry a \`parent\` (fixable via parent→child edges).
...
`;
    ...
    console.log(`wrote ${OUT_PATH}${existsSync(WIKI_ARCH_DIR) ? ` + _orphans-rescue.md (${allOrphans.length})${FLAGS.graph ? " + _disconnected-graph-nodes.md" : " (disconnected-nodes: pass --graph with a bumped heap to regen)"}` : ""}`);
  }
