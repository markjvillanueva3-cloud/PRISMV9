...
export const ALL_STEPS = [
  ...
  // U-SIERRA-CODE-VAULT-FRESHNESS (2026-07-03, slot:sierra): keep the per-file CODE brain fresh so
  // coverage never rots. code-to-vault regenerates a note only for a CHANGED source file (idempotent:
  // matching sourceHash -> skip), so a new/edited engine/script/hook/etc auto-gets its vault note; the
  // MOC step then refreshes the navigation hubs. requires:none (pure local, NO Ollama -- these must
  // refresh even on an Ollama-down night; Hermes enrichment of any NEW thin note is a separate on-demand
  // `code-to-vault.mjs --llm-synth` run, kept out of the nightly to avoid mass calls on the shared lane).
  { id: "code-vault", label: "code-file vault notes (regen changed sources)", script: "code-to-vault.mjs", args: [], requires: "none", dependsOn: null, timeoutMs: 300_000 },
  { id: "code-vault-moc", label: "code-vault MOC navigation refresh", script: "code-vault-moc.mjs", args: [], requires: "none", dependsOn: "code-vault", timeoutMs: 120_000 },
