...
async function main() {
  ...
  if (args.includes("--kernel-dimprior")) {
    ...
    if (!noWrite) {
      ...
      // tmp+rename (atomic): the 23:04 gen loop's generator reads this dataset every 30 min while the
      // nightly chain rewrites it -- a plain truncating write leaves a torn/empty-read window.
      const tmpPath = `${outPath}.tmp`;
      fs.writeFileSync(tmpPath, pairs.map((p) => JSON.stringify({ instruction: p.instruction, output: p.output, partClass: p.partClass, files: p.files, source: p.source })).join("\n") + (pairs.length ? "\n" : ""));
