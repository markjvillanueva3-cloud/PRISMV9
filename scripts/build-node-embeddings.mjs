...
async function ollamaEmbedAttempt(prompt, url, laneFlag, timeoutMs) {
...
// high-concurrency batch corpus embed -- the shared :11434 starves embed
// requests under fleet inference load. A FAILED lane attempt retries MAIN
// once (withMainFallback), so a broken lane degrades to today's exact
// behavior. Lane-disabled/down -> resolveEmbedUrl returns MAIN, byte-identical
// to the pre-lane OLLAMA_URL request (same body shape, no options field).
async function ollamaEmbed(prompt, timeoutMs = OLLAMA_TIMEOUT_MS) {
  const lane = await resolveEmbedUrl();
  return withMainFallback(lane, (url, laneFlag) => ollamaEmbedAttempt(prompt, url, laneFlag, timeoutMs));
}
