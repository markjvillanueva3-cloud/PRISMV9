...
// clear the refs/age gate. Four classes, all R9/R12 (the wiki holds *verified,
// synthesized* knowledge -- never graph atoms, unverified text, run-logs, nor test junk):
//   1. GRAPH-NODE POINTERS (`node_kind` present) -- course/formula/engine node
//      stubs that exist only to be indexed by the master-graph. A single index hub
//      (e.g. an MIT-course index linking all ~22 course nodes) inflates their
//      inbound ref-count to ~10, so they clear minRefs without being genuinely
//      cross-referenced. They belong to the graph/master-index, not the wiki.
//   2. EXPLICITLY-UNVERIFIED content (`advisoryOnly` / `mustHumanVerify` truthy) --
//      LLM-generated syntheses + "audit-pending" stubs self-declare "verify before
//      trusting"; promoting them would launder advisory text as canonical wiki.
//   3. RUN-LOGS (`run_log` truthy) -- ephemeral, machine-generated, timestamped
//      operational records (one per NN-GRAPH retrain round, etc.). Their refs are
//      inflated by a shared index hub like (1); they are not synthesized knowledge.
//   4. TEST FIXTURES (`deadbeef` sentinel in sessionId/agent) -- smoke/integration
//      fixtures mirrored into the vault by a test run; zero knowledge value.
// Returns the exclusion reason (for reporting) or null when promotable.
export function nonPromotableReason(fm) {
  if (!fm || typeof fm !== "object") return null;
  if (typeof fm.node_kind === "string" && fm.node_kind.trim()) return "node-pointer";
  // Accept YAML-truthy spellings (true/1/yes/on) so a flag written `advisoryOnly: 1`
  // is not silently treated as promotable. Anything else (false/0/no/off/absent) promotes.
  const truthy = (v) =>
    v === true ||
    (typeof v === "string" && ["true", "1", "yes", "on"].includes(v.trim().toLowerCase()));
  if (truthy(fm.advisoryOnly) || truthy(fm.mustHumanVerify)) return "unverified-advisory";
  // Ephemeral machine-generated RUN-LOGS (e.g. per-retrain NN-GRAPH metric records
  // emitted by nn-feedback-to-memory.mjs) self-declare run_log:true -- timestamped
  // operational records, not durable synthesized knowledge; never wiki-promotable.
  if (truthy(fm.run_log)) return "run-log";
  // TEST FIXTURES -- smoke/integration tests mirror fixture memories into the vault
  // carrying a `deadbeef` sentinel session id (the classic fake-data marker, e.g.
  // sessionId c0f06deedeadbeefdeadbeefdeadbeef). They have no knowledge value and must
  // never graduate to the canonical wiki, however many dream-hub refs inflate them.
  const sentinel = (v) => typeof v === "string" && /deadbeef/i.test(v);
  if (sentinel(fm.sessionId) || sentinel(fm.agent) || sentinel(fm.originSessionId)) return "test-fixture";
  return null;
}
...
// Auto-generated AGGREGATOR / free-association files whose [[links]] are cataloging
// or brainstorming -- NOT genuine cross-referencing. A [[ref]] originating from one is
// not evidence a memory is genuinely cited by the human/synthesis layer, so it must not
// count toward the inbound-ref promotion gate. This is the STRUCTURAL de-inflation that
// generalizes the per-class nonPromotableReason() patches: it makes the ref-COUNT itself
// honest, so a future auto-generated junk class cannot clear minRefs via hub inflation
// alone (the recurring root cause behind classes 1/3/4 of nonPromotableReason).
// Three categories (segment-anchored so it holds under test temp-roots too):
//   - dreams/   : LLM free-association notes that liberally [[link]] many memories.
//   - _index/   : explicit memory aggregation hubs.
//   - index.md  : catalog/index files in any dir (e.g. the 722-entry wiki index).
// Empirically proven non-destructive on the live vault (2026-06-17): excluding all 15
// hub sources drops 0 of 55 genuine candidates below the gate.
// (The 3 MEMORY*.md aggregators are already dropped from memFiles; this extends that
//  exclusion to every aggregator that can appear as a SCAN source, incl. the wiki tree.)
...
export function runMemoryPromotion({
  ...
  const report = {
    ...
    totalMemories: 0, malformed: 0, hubSourcesSkipped: 0, skippedNonPromotable: 0, skippedType: 0, belowRefs: 0, belowAge: 0,
  ...
  for (const f of scanFiles) {
    // De-inflation: a ref from an auto-gen aggregator (dreams/_index/index.md) is
    // cataloging, not genuine cross-referencing -- it must not count toward the gate.
    if (isHubSource(f)) { report.hubSourcesSkipped++; continue; }
    let raw;
  ...
  for (const [f, info] of parsed) {
    ...
    // Hard exclusion BEFORE the refs/age gate: never promote graph-node pointers
    // or explicitly-unverified content, however popular+old (see nonPromotableReason).
    if (nonPromotableReason(info.fm)) { report.skippedNonPromotable++; continue; }
...
function main() {
  ...
  process.stdout.write(
    ...
    `memories=${report.totalMemories} malformed=${report.malformed} hubSrcSkip=${report.hubSourcesSkipped} ` +
    `skipNonProm=${report.skippedNonPromotable} skipType=${report.skippedType} belowRefs=${report.belowRefs} belowAge=${report.belowAge} ` +
