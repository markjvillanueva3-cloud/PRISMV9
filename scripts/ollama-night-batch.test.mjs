...
test("runJobs: one job failing never stops the next; accounting + log rows correct", async () => {
  const logged = [];
  const jobs = [
...
test("runJobs: between-jobs window re-check stops remaining jobs when the window closes mid-run (scrutiny P2)", async () => {
  let checks = 0;
  const logged = [];
  ...
  const noCheck = await runJobs([{ id: "j1", cmd: ["node", "scripts/a.mjs"], timeoutMs: 5 }],
    { runImpl: () => ({ status: 0, stdout: "", stderr: "" }), logImpl: () => {} });
  assert.equal(noCheck.skippedWindow, 0);
});

test("runJobs: ADVERSARIAL runImpl throw and timeout(null status) are contained as failures", async () => {
  const r = await runJobs(
    [
...
// ── spawnJob tree-kill (the 2026-06-15 13.6h-runaway regression) ─────────────

test("spawnJob: a child that outlives its timeout is killed and returns ~promptly with null status + ETIMEDOUT", async () => {
  const t0 = Date.now();
  // -e is fine here: the registry validator (not spawnJob) enforces the no-shell
  // confinement; this is a direct unit test of the runner's timeout-kill.
  const r = await spawnJob(["node", "-e", "setTimeout(() => {}, 60000)"], 400, process.cwd());
  const elapsed = Date.now() - t0;
  assert.equal(r.status, null, "killed child reports null status (-> exitCode -1 upstream)");
  assert.equal(r.error?.code, "ETIMEDOUT", "timeout kill is tagged ETIMEDOUT for forensics");
  assert.ok(elapsed < 8000, `must return shortly after the 400ms timeout, not after the 60s sleep (took ${elapsed}ms)`);
});

test("spawnJob: REGRESSION -- an orphaned GRANDCHILD holding the pipe no longer blocks the lane (was 13.6h)", async () => {
  // Reproduces the incident exactly: the direct child spawns a grandchild that
  // inherits its stdio (the pipe spawnJob reads) and sleeps far past the timeout.
  // The OLD spawnSync drained that inherited pipe until the grandchild died (hours);
  // the tree-kill reaps the grandchild so 'close' fires and the lane returns fast.
  const parentSrc =
    "const{spawn}=require('child_process');" +
    "spawn(process.execPath,['-e','setTimeout(()=>{},60000)'],{stdio:'inherit'});" +
    "setTimeout(()=>{},60000);";
  const t0 = Date.now();
  const r = await spawnJob(["node", "-e", parentSrc], 500, process.cwd());
  const elapsed = Date.now() - t0;
  assert.equal(r.status, null, "the timed-out tree reports null status");
  // Threshold MUST stay below KILL_GRACE_MS (5000ms): a tree-kill-only regression
  // (direct child killed, grandchild leaks) would still settle via the grace
  // backstop at ~5000ms, so a loose >5000 bound would NOT catch it. The real
  // tree-kill releases the grandchild-held pipe and returns in <1s; 3000ms isolates
  // "tree-kill works" from "grace backstop masked a broken tree-kill" (R9 oracle).
  assert.ok(elapsed < 3000, `tree-kill must release the grandchild-held pipe in <3s, below the ${500}ms timeout + grace; broken tree-kill leaks to ~5s (took ${elapsed}ms)`);
});

test("spawnJob: a fast child returns its real exit code + captured output (no false kill)", async () => {
  const ok = await spawnJob(["node", "-e", "process.stdout.write('hello'); process.exit(0)"], 10000, process.cwd());
  assert.equal(ok.status, 0);
  assert.match(ok.stdout, /hello/);
  assert.equal(ok.error, undefined, "a clean exit carries no spawn error");
  const bad = await spawnJob(["node", "-e", "process.exit(3)"], 10000, process.cwd());
  assert.equal(bad.status, 3, "a real non-zero exit is reported verbatim, never coerced");
});
