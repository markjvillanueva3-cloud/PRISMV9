...
/** Merge per-category counts from src into dst (in place); returns dst. Pure. */
...
/**
 * Generation-emission signature: a root CAD engine counts as a GENERATION engine (the kind whose
 * coverage this meter measures) ONLY if its body references the text->CAD generation substrate
 * (cadquery / build123d). This excludes the many CAD *infrastructure* engines that share the CAD*Engine
 * name but emit no geometry (RBAC/mTLS/transactions/bundle-signing/tenant/license/circuit-breaker) --
 * folding those would inflate capability.cad with non-generation op-marker noise and INVERT the metric
 * (mask real gaps). ~14 of ~109 root CAD*Engine.ts match (the 4 feature engines + the gen-pipeline
 * engines: codegen, operation-taxonomy, intent-decomposer, trial-error-learning, token-representation,
 * ...). NOTE: extend GEN_SIGNATURE if a NEW codegen backend is wired (direct STEP/BREP, OpenCascade,
 * a Fusion/SolidWorks API engine) -- a gen engine that never mentions cadquery/build123d would
 * otherwise be silently under-counted, re-introducing the false-low this fold fixes.
 */
const GEN_SIGNATURE = /cadquery|build123d/i;

/**
 * Root-level CAD GENERATION engines that live in engines/ ROOT, NOT in the engines/cad/ subdir (the
 * domain-galaxy migration has not physically moved them yet) AND carry the generation signature above.
 * Counting them is HONEST coverage: without this the per-galaxy scan saw only engines/<galaxy>/ subdirs
 * and UNDER-reported CAD coverage -> drove duplicate "capability unit" builds (slot:delta, 2026-06-26).
 * They are not in any subdir, so no double-count with the galaxy scan. readImpl injectable for tests.
 */
export function rootCadEngineFiles(enginesDir = ENGINES, readImpl = fs.readFileSync) {
  try {
    return fs.readdirSync(enginesDir, { withFileTypes: true })
      .filter((e) => e.isFile() && /^CAD.*Engine\.ts$/i.test(e.name))
      .map((e) => path.join(enginesDir, e.name))
      .filter((p) => { try { return GEN_SIGNATURE.test(readImpl(p, "utf8")); } catch { return false; } })
      .sort();
  } catch { return []; }
}
...
function buildHits() {
  ...
  // Fold root-level CAD*Engine.ts into the cad galaxy -- they are cad-domain engines not yet migrated
  // into engines/cad/; omitting them under-reported CAD coverage (the dup-build trap). 2026-06-26.
  const rootCadFiles = rootCadEngineFiles();
  if (rootCadFiles.length) {
    capability.cad = capability.cad || {};
    for (const f of rootCadFiles) mergeCounts(capability.cad, countKeywords(f, true));
  }
  ...
  return { capability, knowledge, galaxyCount: galaxies.length, rootCadFileCount: rootCadFiles.length };
}
