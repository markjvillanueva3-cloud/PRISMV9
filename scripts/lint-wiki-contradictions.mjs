...
export async function runNliLint(pages, {
  ...
  let checked = 0, unchecked = 0, consecutive = 0, aborted = false;
  for (const { i, j, shared } of pairs) {
    const a = pages[i], b = pages[j];
    const prompt = buildNliPrompt(a.claim, b.claim);
    let r;
    try {
      r = await callImpl(model, prompt, { numPredict, timeoutMs });
    } catch (err) {
    ...
    if (verdict === "contradict") {
      contradictions.push({ a: a.rel, b: b.rel, shared, reason });
    }
  }
  const report = {
    schemaVersion: 1,
