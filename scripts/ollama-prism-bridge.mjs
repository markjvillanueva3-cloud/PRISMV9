...
export async function embedViaOllama(query, opts = {}) {
  const { model = EMBEDDINGS_MODEL, ollamaUrl = OLLAMA_URL, timeoutMs = EMBEDDINGS_TIMEOUT_MS, fetchImpl = fetch } = opts;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { ok: false, error: `Ollama embeddings HTTP ${res.status}: ${String(txt).slice(0, 200)}` };
    }
    const json = await res.json();
    if (!json || !Array.isArray(json.embedding)) {
      return { ok: false, error: "Ollama embeddings response missing 'embedding' array" };
    }
    if (json.embedding.length !== EMBEDDING_DIM) {
      return { ok: false, error: `Ollama embeddings dim ${json.embedding.length} ≠ expected ${EMBEDDING_DIM} (model mismatch?)` };
    }
    return { ok: true, vector: json.embedding };
  } catch (e) {
    const why = e && e.name === "AbortError"
      ? `Ollama embeddings timed out after ${timeoutMs}ms`
      : `Ollama unreachable for embeddings: ${e && e.message}`;
