...
async function main(argv = process.argv.slice(2)) {
  ...
  // QDRANT-DENSE-ARM (U-INDIA-QDRANT-ARM-WIRE): inject async fetch-based 5th substrate
  // when PRISM_RAG_DENSE_QDRANT=1. Degrade gracefully if the dist/ singleton is absent.
  let qdrantDenseSearchFn;
  if ((process.env.PRISM_RAG_DENSE_QDRANT || "").trim() === "1") {
    try {
      const { qdrantDenseRetrievalEngine } = await import(
        "file:///H:/prism/mcp-server/dist/engines/ai-training/QdrantDenseRetrievalEngine.js"
      );
      qdrantDenseSearchFn = (opts) => qdrantDenseRetrievalEngine.search(opts);
    } catch {
      // Degrade -- hybrid-retrieval records "no-impl" skip for qdrantdense leg
    }
  }

  const result = await hybridSearch(a.query, {
    topK: a.topK,
...
if (invokedDirect) main().then((code) => process.exit(code ?? 0)).catch((err) => { process.stderr.write(String(err) + "\n"); process.exit(1); });
