...
export const DEFAULT_SPAWN_TIMEOUT_MS = 180_000;   // engine cap (synth is <2s; generous)
...
function runEngine(engineRelPath, { timeoutMs, engineArgs = [], _spawn = spawnSync, _exists = existsSync } = {}) {
  ...
  if (res.error) {
    return { ok: false, status: null, error: `spawn failed: ${res.error.code || res.error.message}` };
  }
  if (res.status !== 0) {
    return {
 ...
 *
 * @returns {{outcomes:Array, level:'clean'|'revived'|'failed', exitCode:number, dryRun:boolean}}
 */
export function runOnce(opts = {}) {
  ...
  for (const e of planInput) {
    ...
    outcomes.push({
      key: e.key,
      taskName: e.taskName,
      action: landed ? "revived" : "failed",
      reason: decision.reason,
      taskStatus: decision.taskStatus,
      engineExit: ran.status,
      error: landed ? undefined : (ran.error || "engine ran but output file did not land"),
    });
  }
...
Exit: 0 fresh-or-revived · 1 a revival failed · 2 measurement failure.`;
...
function fmtSummary(r) {
  const tag = r.level === "failed" ? "[FAIL]" : r.level === "revived" ? "[REVIVED]" : "[OK]";
