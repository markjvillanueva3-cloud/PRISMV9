...
const DEFAULT_PARAMETER_VARIANT = {
  id: 'optimizer-nominal',
  label: 'Optimizer nominal',
  docMm: Number.NaN,
  wocMm: Number.NaN,
};
const EXHAUSTIVE_FINISH_TARGETS = ['general', 'tight-finish', 'high-removal', 'prove-out'];
...
function parseArgs(argv: string[]) {
  ...
  return {
    count: intArg('count', DEFAULT_CASE_COUNT, 1, 1_000_000),
    concurrency: intArg('concurrency', DEFAULT_CONCURRENCY, 1, 32),
    shardIndex: intArg('shard-index', 0, 0, 999),
    shardCount: intArg('shard-count', 1, 1, 1_000_000),
    ...
    machineIds: listArg('machine-ids'),
    materialGroups: listArg('material-groups'),
    materialIds: listArg('material-ids'),
    coverageMode: (parsed.has('exhaustive') || ['exhaustive', 'cartesian'].includes((parsed.get('coverage-mode') ?? parsed.get('coverage') ?? '').toLowerCase())) ? 'exhaustive' : 'rotating',
    countOnly: parsed.has('count-only') || parsed.has('dry-run'),
  };
}
...
function selectAuditMachines(machines: any[], manufacturers: string[], machineIds: string[] = []) {
  ...
  const requestedMachineIds = new Set(machineIds);
  const machineFiltered = requestedMachineIds.size
    ? eligibleMachines.filter((machine) => requestedMachineIds.has(String(machine.id).toLowerCase()))
    : eligibleMachines;
  if (manufacturers.length === 0) return machineFiltered;
  const allowed = new Set(manufacturers);
  return machineFiltered.filter((machine) => allowed.has(String(machine.manufacturer).toLowerCase()));
}
...
function hasSurfaceFlowSignature(signature: string) {
...
function chooseFinishTarget(entry: any, token: number) {
  ...
  if (/finish|parallel|scallop|swarf|profile/.test(signature) || hasSurfaceFlowSignature(signature)) return token % 2 === 0 ? 'tight-finish' : 'general';
...
function finishTargetsForEntry(entry: any) {
  const signature = `${entry.toolpathType.id} ${entry.toolpath.label} ${entry.toolpath.path}`.toLowerCase();
  if (isThreadingAuditPath(signature)) return ['general', 'prove-out'];
  if (/finish|parallel|scallop|swarf|profile/.test(signature) || hasSurfaceFlowSignature(signature)) return ['tight-finish', 'general', 'prove-out'];
  if (/rough|adaptive|dynamic|pocket|face|slot/.test(signature)) return ['high-removal', 'general', 'prove-out'];
  return EXHAUSTIVE_FINISH_TARGETS;
}

function clampMetric(value: number, min: number, max: number, digits = 3) {
  return round(Math.max(min, Math.min(max, value)), digits);
}

function dedupeParameterVariants(variants: any[]) {
  const deduped = new Map<string, any>();
  for (const variant of variants) {
    const key = `${variant.docMm.toFixed(3)}__${variant.wocMm.toFixed(3)}`;
    if (!deduped.has(key)) deduped.set(key, variant);
  }
  return [...deduped.values()];
}

function buildParameterVariants(input: {
  mode: AuditMode;
  entry: any;
  ...
  const signature = `${input.entry.toolpathType.id} ${input.entry.toolpath.label} ${input.entry.toolpath.path}`.toLowerCase();
  const baseDoc = Math.max(input.optimization?.recommendedDocMm ?? input.currentDocMm, 0.02);
  const baseWoc = Math.max(input.currentWocMm, 0.01);
  const locLimit = Math.max(input.optimization?.recommendedLocMm ?? diameter, diameter * 0.2);
  const stockLimit = Math.max(input.stock.z * 0.92, 0.05);
  const maxDoc = Math.max(0.04, Math.min(stockLimit, locLimit * 0.96, diameter * (/adaptive|dynamic|trochoid|rough/.test(signature) ? 2.2 : /finish|parallel|scallop/.test(signature) ? 0.45 : 1.2)));
  const maxWoc = Math.max(0.02, Math.min(diameter, diameter * (/adaptive|dynamic|trochoid/.test(signature) ? 0.42 : /finish|parallel|scallop|flow/.test(signature) ? 0.28 : /slot/.test(signature) ? 1 : 0.82)));
  const minDoc = Math.min(maxDoc, Math.max(0.02, diameter * 0.015));
  const minWoc = Math.min(maxWoc, Math.max(0.01, diameter * 0.01));
  const scaled = (id: string, label: string, docScale: number, wocScale: number) => ({
    id,
    label,
    docMm: clampMetric(baseDoc * docScale, minDoc, maxDoc),
    wocMm: clampMetric(baseWoc * wocScale, minWoc, maxWoc),
  });

  if (/finish|parallel|scallop|swarf|flowline/.test(signature) || hasSurfaceFlowSignature(signature)) {
    return dedupeParameterVariants([
      scaled('optimizer-nominal', 'Optimizer nominal', 1, 1),
      scaled('finish-light', 'Finish light radial', 0.62, 0.52),
      scaled('spring-pass', 'Spring-pass skim', 0.38, 0.34),
      scaled('finish-margin', 'Finish margin check', 0.8, 0.75),
    ]);
  }

  if (/adaptive|dynamic|trochoid|featureflow/.test(signature)) {
    return dedupeParameterVariants([
      scaled('optimizer-nominal', 'Optimizer nominal', 1, 1),
      scaled('adaptive-deep-light', 'Adaptive deep/light radial', 1.45, 0.46),
      scaled('adaptive-balanced', 'Adaptive balanced', 1.12, 0.82),
      scaled('adaptive-aggressive', 'Adaptive aggressive radial', 1.22, 1.28),
      scaled('conservative-entry', 'Conservative entry', 0.68, 0.62),
    ]);
  }

  if (/slot/.test(signature)) {
    return dedupeParameterVariants([
      scaled('optimizer-nominal', 'Optimizer nominal', 1, 1),
      { id: 'full-slot-check', label: 'Full slot check', docMm: clampMetric(baseDoc * 0.72, minDoc, maxDoc), wocMm: clampMetric(diameter, minWoc, maxWoc) },
      scaled('slot-conservative', 'Slot conservative', 0.58, 0.82),
      scaled('slot-aggressive', 'Slot aggressive', 1.08, 1),
    ]);
  }

  if (/face|facing/.test(signature)) {
    return dedupeParameterVariants([
      scaled('optimizer-nominal', 'Optimizer nominal', 1, 1),
      scaled('face-wide-shallow', 'Face wide/shallow', 0.55, 1.38),
      scaled('face-balanced', 'Face balanced', 0.88, 1.08),
      scaled('face-cleanup', 'Face cleanup', 0.35, 0.62),
    ]);
  }

  return dedupeParameterVariants([
    scaled('optimizer-nominal', 'Optimizer nominal', 1, 1),
    scaled('conservative-entry', 'Conservative entry', 0.7, 0.68),
    scaled('balanced-high-mrr', 'Balanced high MRR', 1.18, 1.18),
    scaled('light-radial', 'Light radial check', 1.25, 0.5),
  ]);
}

function scenarioDocMm(scenario: any) {
  return Number.isFinite(scenario.parameterVariant?.docMm)
    ? scenario.parameterVariant.docMm
    : scenario.optimization.recommendedDocMm;
}

function scenarioWocMm(scenario: any) {
  return Number.isFinite(scenario.parameterVariant?.wocMm)
    ? scenario.parameterVariant.wocMm
    : scenario.currentWocMm;
}
...
async function buildScenario(unit: any, materials: any[], toolpathEntriesByMode: Record<AuditMode, any[]>, toolMapByMode: Record<AuditMode, Map<string, any[]>>, holderCache: Map<string, Promise<any[]>>) {
  ...
  for (let attempt = 0; attempt < entries.length; attempt += 1) {
    ...
    const optimization = buildCuttingParameterOptimization({
      machineMode: mode,
    ...
    return {
      ...
      currentWocMm,
      parameterVariant: {
        ...DEFAULT_PARAMETER_VARIANT,
        docMm: optimization.recommendedDocMm,
        wocMm: currentWocMm,
      },
      optimization,
    };
...
function buildScenarioKey(scenario: any) {
  return [scenario.mode, scenario.machine.id, scenario.config.controllerOption.id, scenario.config.spindleOption.id, scenario.controllerCapabilityProfileId ?? 'rotating', scenario.material.id, scenario.tool.id, scenario.holderPackage?.id ?? 'no-holder', scenario.entry.programmingId, scenario.entry.licenseTierId, scenario.entry.toolpath.id, scenario.coolantId, scenario.workholdingId, scenario.stockShape, scenario.stabilityId, scenario.finishTarget, scenario.parameterVariant?.id ?? 'optimizer-nominal'].join('__');
}

function buildComparisonKey(scenario: any) {
  return [scenario.mode, scenario.machine.id, scenario.config.controllerOption.id, scenario.config.spindleOption.id, scenario.controllerCapabilityProfileId ?? 'rotating', scenario.tool.id, scenario.holderPackage?.id ?? 'no-holder', scenario.entry.toolpath.id, scenario.coolantId, scenario.finishTarget, scenarioDocMm(scenario).toFixed(2), scenarioWocMm(scenario).toFixed(2)].join('__');
}

function sortByStableId<T>(items: T[], readId: (item: T) => string) {
  return [...items].sort((left, right) => readId(left).localeCompare(readId(right)));
}

function controllerCapabilitySets(config: any) {
  const allIds = (config.controllerCapabilityOptions ?? []).map((option: any) => option.id).filter(Boolean).sort();
  const defaultIds = (config.controllerCapabilityOptions ?? [])
    .filter((option: any) => option.defaultEnabled)
    .map((option: any) => option.id)
    .filter(Boolean)
    .sort();
  if (!allIds.length) return [{ id: 'none', ids: [] }];
  const sets = [{ id: 'default', ids: defaultIds }];
  if (allIds.join('|') !== defaultIds.join('|')) sets.push({ id: 'all', ids: allIds });
  return sets;
}

function availableCoolantIds(config: any) {
  const ids = (config.coolantOptionIds ?? []).filter((id: string) => COOLANT_OPTIONS.some((option) => option.id === id));
  return ids.length ? ids.sort() : ['flood'];
}

async function buildExhaustiveScenarioSet(args: any, selectedMaterials: any[], machineConfigRefs: any[], toolpathEntriesByMode: Record<AuditMode, any[]>, toolMapByMode: Record<AuditMode, Map<string, any[]>>, holderCache: Map<string, Promise<any[]>>) {
  const scenarios: any[] = [];
  let candidateIndex = 0;
  const materials = sortByStableId(selectedMaterials, (material) => material.id);
  const configs = sortByStableId(machineConfigRefs.filter((config) => args.modes.includes(config.machine.mode)), (config) => config.key);

  const maybeAddScenario = (scenario: any) => {
    const belongsToShard = candidateIndex % args.shardCount === args.shardIndex;
    candidateIndex += 1;
    if (belongsToShard && !args.countOnly && scenarios.length < args.count) scenarios.push(scenario);
    return !args.countOnly && scenarios.length >= args.count;
  };

  for (const config of configs) {
    const mode = config.machine.mode as AuditMode;
    const entries = sortByStableId(toolpathEntriesByMode[mode] ?? [], (entry) => `${entry.programmingId}__${entry.licenseTierId}__${entry.toolpath.id}`);
    const capabilitySets = controllerCapabilitySets(config);
    const coolantIds = availableCoolantIds(config);
    const workholdingIds = MODE_WORKHOLDING_IDS[mode].filter((id) => WORKHOLDING_OPTIONS.some((option) => option.id === id)).sort();
    const stockShapes = MODE_STOCK_SHAPES[mode].filter((id) => STOCK_SHAPES.some((option) => option.id === id)).sort();
    const stabilityIds = [...MODE_STABILITY_IDS[mode]].sort();

    for (const entry of entries) {
      if (!machineSupportsToolpath(mode, config.machine, entry.toolpath)) continue;
      const compatibleTools = sortByStableId(toolMapByMode[mode].get(entry.toolpath.id) ?? [], (tool) => tool.id);
      if (!compatibleTools.length) continue;
      for (const tool of compatibleTools) {
        const holderPackages = sortByStableId(await resolveHolderPackages(mode, config.machine, tool, holderCache), (holder) => holder.id);
        if (!holderPackages.length) continue;
        const toolDiameterMm = Math.max(tool.defaultDiameter, mode === 'lathe' ? 1.2 : 3);
        const defaults = entry.defaults ?? { docMm: mode === 'lathe' ? 1.5 : 0.2, wocMm: mode === 'lathe' ? 0.4 : 0.25, isAbsolute: true };
        const currentDocMm = round(defaults.isAbsolute ? defaults.docMm : toolDiameterMm * defaults.docMm, 3);
        const currentWocMm = round(defaults.isAbsolute ? defaults.wocMm : toolDiameterMm * defaults.wocMm, 3);
        const reachDefaults = deriveToolReachDefaults(tool, toolDiameterMm, mode);
        for (const material of materials) {
          for (const holderPackage of holderPackages) {
            for (const coolantId of coolantIds) {
              if (args.countOnly) {
                for (const stockShape of stockShapes) {
                  const stockToken = hashString(`${config.key}__${entry.toolpath.id}__${tool.id}__${material.id}__${holderPackage.id}__${stockShape}`);
                  const stock = buildStockDimensions(mode, stockShape, toolDiameterMm, stockToken);
                  for (const stabilityId of stabilityIds) {
                    const optimization = buildCuttingParameterOptimization({
                      machineMode: mode,
                    ...
                    candidateIndex += workholdingIds.length
                      * finishTargetsForEntry(entry).length
              ...
              for (const workholdingId of workholdingIds) {
                for (const stockShape of stockShapes) {
                  const stockToken = hashString(`${config.key}__${entry.toolpath.id}__${tool.id}__${material.id}__${holderPackage.id}__${stockShape}`);
                  const stock = buildStockDimensions(mode, stockShape, toolDiameterMm, stockToken);
                  for (const stabilityId of stabilityIds) {
                    ...
                    for (const finishTarget of finishTargetsForEntry(entry)) {
                      for (const capabilitySet of capabilitySets) {
                        for (const parameterVariant of parameterVariants) {
                          const scenario = {
                            mode,
                            machine: config.machine,
                            config,
                            ...
                            holderPackage,
                            insertOption: buildInsertOptionsForTool({ tool, material }).at(0) ?? null,
                            enabledControllerCapabilityIds: capabilitySet.ids,
                            controllerCapabilityProfileId: capabilitySet.id,
                            workholdingId,
                            stabilityId,
                            finishTarget,
                            desiredRaUm: desiredRaForFinishTarget(finishTarget),
                          ...
                          if (maybeAddScenario(scenario)) {
                            return { scenarios, candidateCount: candidateIndex, exhausted: false };
                          }
                        }
                      }
                    }
  ...
  return { scenarios, candidateCount: candidateIndex, exhausted: true };
}

async function buildRotatingScenarioSet(args: any, selectedMaterials: any[], machineConfigRefs: any[], toolpathEntriesByMode: Record<AuditMode, any[]>, toolMapByMode: Record<AuditMode, Map<string, any[]>>, holderCache: Map<string, Promise<any[]>>) {
  ...
  while (scenarios.length < args.count) {
    ...
    for (const unit of units) {
      const scenario = await buildScenario(unit, selectedMaterials, toolpathEntriesByMode, toolMapByMode, holderCache);
      unit.cursor += 1;
...
function detectAnomalies(scenario: any, live: any, finishPreview: any, setupPreview: any) {
  ...
  const namedFinishPath = /finish|parallel|scallop|swarf/.test(pathSignature) || hasSurfaceFlowSignature(pathSignature);
  ...
  const supportedSetupFail = (setupPreview.risks ?? []).some((risk: any) => (
    risk?.severity === 'fail'
    && /(stickout|deflection|interface|mismatch|rpm|exceed|too aggressive|too high|too deep|too wide|limit|overload|axis|coolant|engagement|load|radial|axial|doc|woc|tool envelope|chatter|rigidity|published)/i.test(
...
async function main() {
  ...
  const selectedMachinesByMode = {
    mill: selectAuditMachines(millMachines, args.manufacturers, args.machineIds),
    lathe: selectAuditMachines(latheMachines, args.manufacturers, args.machineIds),
  };
  ...
  if (machineConfigRefs.length === 0) {
    throw new Error(`No machine packages matched the requested modes/manufacturers/machine ids: modes=${args.modes.join(',')} manufacturers=${args.manufacturers.join(',') || 'all'} machineIds=${args.machineIds.join(',') || 'all'}.`);
  }
  ...
  const generation = args.coverageMode === 'exhaustive'
    ? await buildExhaustiveScenarioSet(args, selectedMaterials, machineConfigRefs, toolpathEntriesByMode as Record<AuditMode, any[]>, toolMapByMode as Record<AuditMode, Map<string, any[]>>, holderCache)
    : await buildRotatingScenarioSet(args, selectedMaterials, machineConfigRefs, toolpathEntriesByMode as Record<AuditMode, any[]>, toolMapByMode as Record<AuditMode, Map<string, any[]>>, holderCache);
  const scenarios = generation.scenarios;
  ...
  if (args.countOnly) {
    const summary = {
      ...
      viableCandidateCount: generation.candidateCount,
      exhaustedCandidateEnumeration: generation.exhausted,
      machineCount: selectedMachinesByMode.mill.length + selectedMachinesByMode.lathe.length,
      machineConfigCount: machineConfigRefs.length,
      materialCount: selectedMaterials.length,
      programmingPackageCount: new Set([...toolpathEntriesByMode.mill, ...toolpathEntriesByMode.lathe].map((entry) => entry.programmingId)).size,
      toolpathCount: new Set([...toolpathEntriesByMode.mill, ...toolpathEntriesByMode.lathe].map((entry) => entry.toolpath.id)).size,
    };
    await writeFile(path.join(args.outDir, 'calculator-live-audit-summary.json'), JSON.stringify(summary, null, 2), 'utf8');
    await writeFile(path.join(args.outDir, 'calculator-live-audit-summary.md'), ['# Calculator Live Audit Count', '', `- Coverage mode: ${summary.coverageMode}`, `- Viable candidates: ${summary.viableCandidateCount}`, `- Exhausted enumeration: ${summary.exhaustedCandidateEnumeration}`, `- Machine configs: ${summary.machineConfigCount}`, `- Materials: ${summary.materialCount}`, `- Toolpaths: ${summary.toolpathCount}`, ''].join('\n'), 'utf8');
  ...
  await runPool(scenarios, args.concurrency, async (scenario, index) => {
    try {
      const activeDocMm = scenarioDocMm(scenario);
      const activeWocMm = scenarioWocMm(scenario);
      ...
      const setupFailRisks = setupPreview.risks
        .filter((risk: any) => risk.severity === 'fail')
        .map((risk: any) => ({
          id: risk.id,
          title: risk.title,
          zone: risk.zone,
      ...
      const detectedAnomalies = detectAnomalies(scenario, live, finishPreview, setupPreview);
      const finishFeasibilityMiss = finishPreview.verdict === 'unlikely';
      const anomalies = args.coverageMode === 'exhaustive'
        ? {
          codes: detectedAnomalies.codes.filter((code) => code !== 'finish-unlikely'),
          details: detectedAnomalies.details.filter((_, detailIndex) => detectedAnomalies.codes[detailIndex] !== 'finish-unlikely'),
        }
        : detectedAnomalies;
      results[index] = {
        caseId: index + 1,
        ...
        controllerCapabilityProfileId: scenario.controllerCapabilityProfileId ?? 'rotating',
        ...
        finishTarget: scenario.finishTarget,
        desiredRaUm: scenario.desiredRaUm,
        parameterVariantId: scenario.parameterVariant?.id ?? 'optimizer-nominal',
        parameterVariantLabel: scenario.parameterVariant?.label ?? 'Optimizer nominal',
        docMm: activeDocMm,
        wocMm: activeWocMm,
      ...
      results[index] = {
        ...
        parameterVariantId: scenario.parameterVariant?.id ?? 'optimizer-nominal',
        parameterVariantLabel: scenario.parameterVariant?.label ?? 'Optimizer nominal',
        docMm: scenarioDocMm(scenario),
        wocMm: scenarioWocMm(scenario),
        optimizedDocMm: scenario.optimization.recommendedDocMm,
        optimizedLocMm: scenario.optimization.recommendedLocMm,
        optimizedStickoutMm: scenario.optimization.recommendedStickoutMm,
        formulasCount: 0,
  ...
  const summary = {
    ...
    requestedModes: args.modes,
    coverageMode: args.coverageMode,
    generatedCandidateCount: generation.candidateCount,
    exhaustedCandidateEnumeration: generation.exhausted,
    manufacturerFilter: args.manufacturers,
    machineIds: args.machineIds,
    materialGroups: args.materialGroups.length ? args.materialGroups : DEFAULT_MATERIAL_GROUPS,
    ...
    workholdingCount: new Set(validResults.map((result) => result.workholdingId)).size,
    coolantCount: new Set(validResults.map((result) => result.coolantId)).size,
    stockShapeCount: new Set(validResults.map((result) => result.stockShape)).size,
    stabilityCount: new Set(validResults.map((result) => result.stabilityId)).size,
    finishTargetCount: new Set(validResults.map((result) => result.finishTarget)).size,
    parameterVariantCount: new Set(validResults.map((result) => result.parameterVariantId)).size,
    finishFeasibilityMissCount: validResults.filter((result) => Boolean(result.finishFeasibilityMiss)).length,
  ...
  await writeFile(path.join(args.outDir, 'calculator-live-audit-summary.json'), JSON.stringify(summary, null, 2), 'utf8');
  await writeFile(path.join(args.outDir, 'calculator-live-audit-summary.md'), ['# Calculator Live Audit', '', `- Coverage mode: ${summary.coverageMode}`, `- Requested cases: ${summary.requestedCaseCount}`, `- Generated candidates: ${summary.generatedCandidateCount}`, `- Exhausted enumeration: ${summary.exhaustedCandidateEnumeration}`, `- Shard index: ${summary.shardIndex}`, `- Shard count: ${summary.shardCount}`, `- Completed cases: ${summary.completedCaseCount}`, `- Successful cases: ${summary.successfulCaseCount}`, `- Failed cases: ${summary.failedCaseCount}`, `- Mill cases: ${summary.modes.mill}`, `- Lathe cases: ${summary.modes.lathe}`, `- Machines covered: ${summary.machineCount}`, `- Materials covered: ${summary.materialCount}`, `- Tools covered: ${summary.toolCount}`, `- Holder packages covered: ${summary.holderPackageCount}`, `- Programming packages covered: ${summary.programmingPackageCount}`, `- Toolpaths covered: ${summary.toolpathCount}`, `- Workholding presets covered: ${summary.workholdingCount}`, `- Coolant modes covered: ${summary.coolantCount}`, `- Stock shapes covered: ${summary.stockShapeCount}`, `- Stability modes covered: ${summary.stabilityCount}`, `- Finish targets covered: ${summary.finishTargetCount}`, `- Parameter variants covered: ${summary.parameterVariantCount}`, `- Finish feasibility misses: ${summary.finishFeasibilityMissCount}`, '', '## Top Anomalies', ...Object.entries(summary.anomalyCounts).slice(0, 20).map(([code, count]) => `- ${code}: ${count}`), '', '## Suspicious Material Equivalence', ...summary.suspiciousMaterialEquivalence.map((entry: any) => `- ${entry.comparisonKey}: ${entry.materials.join(', ')} => ${entry.resultSignature}`), ''].join('\n'), 'utf8');
