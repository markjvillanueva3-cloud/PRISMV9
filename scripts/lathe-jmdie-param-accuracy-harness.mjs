...
function parseArgs(argv) {
  const a = { sample: 600, all: false, customer: null, out: OUT_DIR_DEFAULT, seed: 1337, root: null, allRoots: false };
  for (let i = 2; i < argv.length; i++) {
    const t = argv[i];
    if (t === "--all") a.all = true;
    else if (t === "--all-roots") a.allRoots = true;          // scan the WHOLE JM DIE tree (all 34,993 .MIN), not just CNC LATHE
...
function customerOf(fp) {
  // CNC LATHE/<customer>/.../file.MIN -> segment right after CNC LATHE (or JM DIE root for --all-roots)
  const parts = fp.split(/[\\/]/);
  let idx = parts.findIndex((p) => p.toUpperCase() === "CNC LATHE");
  if (idx < 0) idx = parts.findIndex((p) => p.toUpperCase() === "JM DIE");
...
function main() {
  ...
  const t0 = Date.now();
  const all = [];
  const corpusRoot = args.root || (args.allRoots ? "H:/PRISM/JM DIE" : CORPUS);
  walkMin(corpusRoot, all);
  let pool = all;
  if (args.customer) {
  ...
  // Dedicated SAFETY artifact: the FULL overspeed-risk list (G96 CSS without a
  // G50 max-RPM cap) so the shop can remediate every one -- not just the top 40
  // embedded in the main dashboard. Operator goal: "safety factored in".
  const overspeed = programs.filter((p) => p.g96NoCapRisk)
    .map((p) => ({ file: p.file, customer: p.customer, path: p.path, g96sfm: p.g96sfm }));
  const ovByCustomer = {};
  for (const p of overspeed) ovByCustomer[p.customer] = (ovByCustomer[p.customer] || 0) + 1;
  const ovTop = Object.entries(ovByCustomer).sort((a, b) => b[1] - a[1]).slice(0, 20);
  const ovJsonPath = join(args.out, "lathe-overspeed-risk.json");
  writeFileSync(ovJsonPath, JSON.stringify({
    schemaVersion: "1.0.0",
    generated_at: new Date(t0).toISOString(),
    corpus_root: corpusRoot,
    rule: "G96 (CSS) without a G50 max-RPM cap = spindle overspeed risk as diameter shrinks (SAFETY-CRITICAL)",
    analyzed: N,
    overspeed_risk_count: overspeed.length,
    by_customer_top20: ovTop.map(([customer, count]) => ({ customer, count })),
    programs: overspeed,
  }, null, 2));
  let ovMd = `# JM Die Lathe -- Overspeed-Risk Safety Report (G96 without G50 cap)\n\n`;
  ovMd += `> Rule: G96 constant-surface-speed without a G50 max-RPM cap lets the spindle overspeed as diameter shrinks. SAFETY-CRITICAL.\n\n`;
  ovMd += `**${overspeed.length} of ${N}** analyzed programs are overspeed-risk (corpus root: ${corpusRoot}).\n\n`;
  ovMd += `## By customer (top 20)\n\n| customer | overspeed-risk programs |\n|----|----|\n`;
  for (const [customer, count] of ovTop) ovMd += `| ${customer} | ${count} |\n`;
  ovMd += `\n## First 100 flagged programs (full list in the JSON)\n\n| customer | file |\n|----|----|\n`;
  for (const p of overspeed.slice(0, 100)) ovMd += `| ${p.customer} | ${p.file} |\n`;
  ovMd += `\n_Full machine-readable list: \`state/shared/dashboards/lathe-overspeed-risk.json\`_\n`;
  writeFileSync(join(args.out, "lathe-overspeed-risk.md"), ovMd);

  console.log(JSON.stringify({
    ok: true, analyzed: N, parse_errors: parseErrors, overspeed_risk_report: ovJsonPath,
