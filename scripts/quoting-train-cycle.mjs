...
} from "./lib/quoting-cost-source-consumers.mjs";

/**
 * U-QP-TSX-REEXEC (charlie 2026-06-22): the orchestrator engine is loaded SRC-FIRST
 * (a .ts file) -- see the SRC-FIRST rationale at the engine lazy-load below. Under a
 * TS-capable runtime (tsx) the .ts loads AND its own dynamic `import("./X.js")`
 * specifiers resolve to the sibling .ts. Under PLAIN `node` (Node 24 native TS
 * type-strip), the .ts orchestrator DOES load, but a DYNAMIC import of a ".js"
 * specifier where only the ".ts" exists fails with ERR_MODULE_NOT_FOUND -- so the
 * whole train-cycle dies opaquely on every bare-`node` launch (cron tsx-missing
 * fallback, ad-hoc runs, quoting-pipeline-verify). The comment at the engine load
 * already SAYS "invoke via tsx" but the script never ENFORCED it. This guards every
 * call path by self-re-exec'ing under tsx once, mirroring the proven
 * `shouldReexecForHeap` self-reexec pattern in scripts/nn-graph-retrain-lifecycle.mjs.
 * Pure + testable; the breaker env var prevents an infinite loop; a missing tsx binary
 * degrades to the pre-existing dist-fallback + honest error (never worse than today).
 */
const TSX_LOADER_RE = /tsx[\\/](?:dist[\\/])?(?:loader|preflight|cli)/i;

/** True iff this process is ALREADY running under the tsx loader (execArgv carries it). */
...
/** Resolve the tsx CLI entry under mcp-server; null if absent. */
export function resolveTsxCli(cwd = process.cwd()) {
  const cli = resolve(cwd, "mcp-server/node_modules/tsx/dist/cli.mjs");
  return existsSync(cli) ? cli : null;
}

/**
 * Decide whether to re-exec under tsx. Returns { reexec, reason, tsxCli }.
 * reexec only when: NOT already under tsx, NOT already re-exec'd (breaker), AND tsx present.
 * tsx absent -> DO NOT reexec; fall through to the dist-fallback + honest error.
 */
export function planTsxReexec({ execArgv = process.execArgv, env = process.env, cwd = process.cwd() } = {}) {
  ...
  if (env.PRISM_QTC_REEXEC === "1") return { reexec: false, reason: "reexec-breaker-set", tsxCli: null };
  if (env.PRISM_QTC_NO_REEXEC === "1") return { reexec: false, reason: "reexec-disabled", tsxCli: null };
...
// 1.1.0 (U-CLT-TRAINING-SOURCES): +docustrata_invoices_match, +vendor_cost_index_advisory,
// +tool_purchases_advisory top-level advisory blocks (additive minor -- no key removed/renamed).
export const TRAINING_STATUS_SCHEMA_VERSION = "1.1.0";
...
export function buildTrainingStatusSnapshot(result, opts = {}) {
  ...
  return {
    ...
    // U-CLT-TRAINING-SOURCES: the 3 newly-consumed price/cost sources. Every block is ADVISORY
    // (never alters the factor / provenance gate) and carries its own ok/reason so a frontend
    // can distinguish "ran and found a signal" from "skipped this cycle (stale/missing/empty)".
    docustrata_invoices_match: (() => {
      const im = opts.docustrataInvoicesMatch ?? null;
      if (!im) return null;
      return im.ok
        ? {
            ok: true,
            verdict: im.verdict ?? null,
            median_ratio: typeof im.median_ratio === "number" ? im.median_ratio : null,
            within_band_pct: typeof im.within_band_pct === "number" ? im.within_band_pct : null,
            deduped: typeof im.deduped === "number" ? im.deduped : null,
            duplicates_excluded: typeof im.duplicates_excluded === "number" ? im.duplicates_excluded : null,
            total_invoices: typeof im.total_invoices === "number" ? im.total_invoices : null,
            // true when the actuals dedupe guard could not run this cycle (actuals unreadable)
            // -- duplicates_excluded:0 is then "guard degraded", NOT "no overlap".
            dedupe_degraded: im.dedupe_degraded === true ? true : false,
            advisory: true,
          }
        : {
            ok: false,
            reason: im.reason ?? null,
            // provenance-gate refusal (charlie soul: never pass synthetic as real) -- lets a
            // frontend render "awaiting real invoice extraction", not a generic failure.
            synthetic: im.synthetic === true ? true : false,
            advisory: true,
          };
    })(),
    vendor_cost_index_advisory: (() => {
      const vm = opts.vendorCostIndexMatch ?? null;
      if (!vm) return null;
      return vm.ok
        ? {
            ok: true,
            reliable: Boolean(vm.reliable),
            reliability_reason: vm.reliability_reason ?? null,
            normalized_median_usd_per_in3: typeof vm.normalized_median_usd_per_in3 === "number" ? vm.normalized_median_usd_per_in3 : null,
            blended_index_median: typeof vm.blended_index_median === "number" ? vm.blended_index_median : null,
            normalized_grade_count: typeof vm.normalized_grade_count === "number" ? vm.normalized_grade_count : null,
            advisory: true,
          }
        : { ok: false, reason: vm.reason ?? null, advisory: true };
    })(),
    tool_purchases_advisory: (() => {
      const tm = opts.toolPurchasesMatch ?? null;
      if (!tm) return null;
      return tm.ok
        ? {
            ok: true,
            type_count: typeof tm.type_count === "number" ? tm.type_count : null,
            reconciled_with_consumption_ledger: typeof tm.reconciled_with_consumption_ledger === "number" ? tm.reconciled_with_consumption_ledger : null,
            advisory: true,
          }
        : { ok: false, reason: tm.reason ?? null, advisory: true };
    })(),
  };
}
...
export const QUOTING_DATA_SOURCES = [
  ...
  // U-CLT-TRAINING-SOURCES (2026-07-02, slot:charlie): all 3 formerly-unconsumed sources now
  // have a real consumption arm in scripts/lib/quoting-cost-source-consumers.mjs, gated by a
  // freshness preflight (checkFreshness) before the arm runs -- a stale source degrades to
  // "source-stale" and does NOT flip consumed:true for that cycle (see dataSourceCoverage()).
  { key: "vendor_cost_index", file: "jm-vendor-cost-index.json", role: "inbound cost basis -- what JM pays (units-blended; ADVISORY drift-check vs the normalized $/in3 basis, reliability-flagged)", consumed: "vendor_cost_index" },
  { key: "tool_purchases", file: "jm-tool-purchases.json", role: "tooling spend -- ADVISORY reconciliation vs the shop-floor consumption multiplier ledger", consumed: "tool_purchases" },
  { key: "docustrata_invoices", file: "docustrata-invoices.curated.json", role: "curated customer invoices -- price-side calibration TARGET, deduped against docustrata_actuals by customer|part_id; PROVENANCE-GATED (refused while the source's revenue values self-declare PLACEHOLDER -- never pass synthetic as real)", consumed: "docustrata_invoices" },
...
export function dataSourceCoverage(quotingDir, opts = {}) {
  ...
  // U-CLT-TRAINING-SOURCES: the 3 new conditional-consumed flags. Each is true ONLY when its
  // arm actually ran this cycle (fresh + present + non-empty result) -- mirrored 1:1 from the
  // proven outbound/docustrata_actuals mechanics above (R8: same pattern, not reinvented).
  const vendorCostIndexConsumed = opts.vendorCostIndexConsumed === true;
  const toolPurchasesConsumed = opts.toolPurchasesConsumed === true;
  const docustrataInvoicesConsumed = opts.docustrataInvoicesConsumed === true;
  const sources = QUOTING_DATA_SOURCES.map((s) => {
    ...
    const consumed = s.consumed === true ? true
      ...
      : s.consumed === "vendor_cost_index" ? vendorCostIndexConsumed
      : s.consumed === "tool_purchases" ? toolPurchasesConsumed
      : s.consumed === "docustrata_invoices" ? docustrataInvoicesConsumed
      : false;
...
async function main() {
  // U-QP-TSX-REEXEC (charlie 2026-06-22): if launched under bare `node` (which loads the
  // SRC-FIRST .ts orchestrator but cannot resolve its dynamic ".js" -> ".ts" imports),
  // re-exec this exact script under tsx ONCE so the engine import resolves. Every call
  // path (cron tsx-missing fallback, ad-hoc, quoting-pipeline-verify) is covered. Breaker:
  // PRISM_QTC_REEXEC=1 (set on the child) prevents an infinite loop; tsx-absent falls
  // through to the dist-fallback + honest error below (never worse than the prior behavior).
  {
    ...
    if (plan.reexec) {
      ...
      // Fail loud (R12): a spawn that never started (status null + no signal) or was
      // killed by a signal is an error, not a silent success.
      if (child.error) {
        process.stderr.write(`[quoting-train-cycle] FAIL: tsx re-exec could not spawn: ${String(child.error)}\n`);
        process.exit(1);
      }
      if (child.signal) {
        process.stderr.write(`[quoting-train-cycle] FAIL: tsx re-exec killed by signal ${child.signal}\n`);
        process.exit(1);
      }
  ...
  // RUNTIME NOTE (U-QP-TSX-REEXEC 2026-06-22): under Node 24 native TS type-strip, bare
  // `node` DOES load the SRC `.ts` orchestrator below -- but the orchestrator's OWN dynamic
  // `await import("./QuotingTrainingLoopEngine.js")` then throws ERR_MODULE_NOT_FOUND (only
  // the `.ts` sibling exists in src/). That is the real bare-node failure mode (NOT a guard
  // exit-2). The tsx self-reexec guard at the top of main() now re-execs under tsx so the
  // chained `.js`->`.ts` import resolves; this block's dist-fallback only runs when tsx is
  // genuinely absent (then plain `node` truly can't import the .ts and we fall to the fossil).
  ...
  // U-CLT-TRAINING-SOURCES (2026-07-02, slot:charlie): the 3 formerly-unconsumed quoting data
  // sources. Every arm is gated by a freshness preflight (checkFreshness -- R12: never silently
  // fold a stale source) BEFORE it runs; a stale/missing source degrades to an honest
  // "source-stale"/"source-missing" advisory result and its consumed flag stays false for this
  // cycle (data_source_coverage reports it as present-but-unconsumed, same as before this unit).
  const quotingDir = resolve(process.cwd(), "state/shared/quoting");
  const nowMs = Date.now();

  // Arm A: docustrata_invoices -- price-side, extends the actuals/calibration matching arm.
  // Dedupes against docustrata_actuals by customer|part_id join_key so the same real order is
  // never double-counted as two calibration targets.
  let docustrataInvoicesMatch = null;
  try {
    const invoicesPath = resolve(quotingDir, "docustrata-invoices.curated.json");
    const invoicesStat = existsSync(invoicesPath) ? statSync(invoicesPath) : null;
    const fresh = checkFreshness(invoicesStat ? invoicesStat.mtimeMs : null, nowMs);
    if (!fresh.present) {
      docustrataInvoicesMatch = { ok: false, reason: "source-missing", advisory: true, freshness: fresh };
    } else if (!fresh.fresh) {
      // fresh.reason is "source-stale" OR "future-mtime" (clock skew) -- pass the TRUE
      // verdict through, never collapse future-mtime into stale at the top level.
      docustrataInvoicesMatch = { ok: false, reason: fresh.reason, advisory: true, freshness: fresh };
    } else {
      const predictedAll = Array.isArray(result?.report?.predicted_fmv_usd_all) ? result.report.predicted_fmv_usd_all : [];
      const raw = JSON.parse(await fs.readFile(invoicesPath, "utf-8"));
      // PROVENANCE GATE (3-of-3 finding 2026-07-02; charlie soul: never pass synthetic as
      // real). The curated fixture's own note declares its revenue values PLACEHOLDER --
      // refusing here keeps consumed:false + coverage honest (5/6) until real invoice
      // extraction (U-QP-ACCOUNTING-WIRE / ERP credentials) replaces the fixture.
      const provenance = detectSyntheticProvenance(raw);
      if (provenance.synthetic) {
        docustrataInvoicesMatch = { ok: false, reason: "source-synthetic-placeholder", advisory: true, synthetic: true, provenance, freshness: fresh };
      } else {
        const invoices = Array.isArray(raw?.invoices) ? raw.invoices : [];
        // Build the actuals join_key set from the SAME orders-closed-actuals.jsonl already loaded
        // above for docustrataMatch -- re-read defensively (loadActualPrices only returns prices,
        // not join_keys) rather than plumb a second return shape through the shared helper (R8:
        // minimal surface change to a proven, already-tested library function).
        const actualsPath = resolve(quotingDir, "orders-closed-actuals.jsonl");
        let actualsJoinKeys = new Set();
        let dedupeDegraded = false;
        try {
          const actualsRaw = JSON.parse(await fs.readFile(actualsPath, "utf-8"));
          const actualsList = Array.isArray(actualsRaw?.actuals) ? actualsRaw.actuals : [];
          actualsJoinKeys = new Set(actualsList.map((a) => (typeof a?.join_key === "string" ? a.join_key : null)).filter(Boolean));
        } catch (dedupeErr) {
          // Fail-soft but LOUD: dedupe against an empty set means duplicates_excluded:0 is
          // indistinguishable from "no overlap" -- flag it so a consumer can tell the guard
          // was degraded this cycle, never silently disabled.
          dedupeDegraded = true;
          process.stderr.write(`[quoting-train-cycle] invoices dedupe guard DEGRADED (actuals unreadable, dedupe skipped): ${dedupeErr}\n`);
        }
        docustrataInvoicesMatch = matchDocustrataInvoices(predictedAll, invoices, actualsJoinKeys, matchPredictedToActuals);
        docustrataInvoicesMatch.freshness = fresh;
        if (dedupeDegraded) docustrataInvoicesMatch.dedupe_degraded = true;
      }
    }
  } catch (e) {
    process.stderr.write(`[quoting-train-cycle] docustrata-invoices match skipped (non-fatal): ${e}\n`);
  }

  // Arm B: vendor_cost_index -- cost-side ADVISORY drift-check. Compares the units-CORRECT
  // normalized material basis (jm-material-cost-basis.json) against the units-BLENDED vendor
  // cost index median -- reliability-flagged (R12), never folded into calibration as a ratio.
  let vendorCostIndexMatch = null;
  try {
    const vendorIndexPath = resolve(quotingDir, "jm-vendor-cost-index.json");
    const materialBasisPath = resolve(quotingDir, "jm-material-cost-basis.json");
    const vendorIndexStat = existsSync(vendorIndexPath) ? statSync(vendorIndexPath) : null;
    const materialBasisStat = existsSync(materialBasisPath) ? statSync(materialBasisPath) : null;
    // Both sources feed this arm -- freshness is gated on the OLDER of the two (R12: a stale
    // half of a comparison is a stale comparison, even if the other half is fresh).
    const vendorFresh = checkFreshness(vendorIndexStat ? vendorIndexStat.mtimeMs : null, nowMs);
    const basisFresh = checkFreshness(materialBasisStat ? materialBasisStat.mtimeMs : null, nowMs);
    if (!vendorFresh.present || !basisFresh.present) {
      vendorCostIndexMatch = { ok: false, reason: !vendorFresh.present ? "vendor-index-missing" : "material-basis-missing", advisory: true, freshness: { vendor_index: vendorFresh, material_basis: basisFresh } };
    } else if (!vendorFresh.fresh || !basisFresh.fresh) {
      // Distinguish real staleness from a future-mtime (clock-skew) refusal per side (R12).
      const staleSide = !vendorFresh.fresh
        ? (vendorFresh.reason === "future-mtime" ? "vendor-index-future-mtime" : "vendor-index-stale")
        : (basisFresh.reason === "future-mtime" ? "material-basis-future-mtime" : "material-basis-stale");
      vendorCostIndexMatch = { ok: false, reason: staleSide, advisory: true, freshness: { vendor_index: vendorFresh, material_basis: basisFresh } };
    } else {
      const vendorRaw = JSON.parse(await fs.readFile(vendorIndexPath, "utf-8"));
      const materialPrior = vendorRaw?.categories?.material ?? null;
      const basisRaw = JSON.parse(await fs.readFile(materialBasisPath, "utf-8"));
      const grades = basisRaw?.grades ?? {};
      vendorCostIndexMatch = vendorCostAdvisory(grades, materialPrior);
      vendorCostIndexMatch.freshness = { vendor_index: vendorFresh, material_basis: basisFresh };
    }
  } catch (e) {
    process.stderr.write(`[quoting-train-cycle] vendor-cost-index advisory skipped (non-fatal): ${e}\n`);
  }

  // Arm C: tool_purchases -- cost-side ADVISORY reconciliation vs the shop-floor consumption
  // multiplier ledger (ConsumableCostBasisEngine, shipped 2026-07-01). PURCHASES vs
  // CONSUMPTION are siblings, not duplicates -- this arm reconciles the vendor A/P purchase
  // spend against the ledger's reconciliation multipliers; it never writes the ledger.
  let toolPurchasesMatch = null;
  try {
    const toolPurchasesPath = resolve(quotingDir, "jm-tool-purchases.json");
    const toolPurchasesStat = existsSync(toolPurchasesPath) ? statSync(toolPurchasesPath) : null;
    const fresh = checkFreshness(toolPurchasesStat ? toolPurchasesStat.mtimeMs : null, nowMs);
    if (!fresh.present) {
      toolPurchasesMatch = { ok: false, reason: "source-missing", advisory: true, freshness: fresh };
    } else if (!fresh.fresh) {
      // fresh.reason passes "source-stale" vs "future-mtime" (clock skew) through honestly.
      toolPurchasesMatch = { ok: false, reason: fresh.reason, advisory: true, freshness: fresh };
    } else {
      const toolRaw = JSON.parse(await fs.readFile(toolPurchasesPath, "utf-8"));
      const byType = toolRaw?.byType ?? {};
      // SRC-FIRST, dist-fallback (mirrors the OutboundPriceIndexEngine loader above -- dist/engines
      // is a stale fossil). ConsumableCostBasisEngine.loadLedger() is a plain synchronous method;
      // a load failure fails soft (empty ledger) inside the engine itself.
      const ccbSrc = resolve(process.cwd(), "mcp-server/src/engines/ConsumableCostBasisEngine.ts");
      const ccbDist = resolve(process.cwd(), "mcp-server/dist/engines/ConsumableCostBasisEngine.js");
      let ccb;
      try {
        ccb = (await import(pathToFileURL(ccbSrc).href)).consumableCostBasisEngine;
      } catch {
        ccb = (await import(pathToFileURL(ccbDist).href)).consumableCostBasisEngine;
      }
      const ledger = ccb ? ccb.loadLedger() : { byType: {} };
      toolPurchasesMatch = toolPurchaseAdvisory(byType, ledger);
      toolPurchasesMatch.freshness = fresh;
    }
  } catch (e) {
    process.stderr.write(`[quoting-train-cycle] tool-purchases advisory skipped (non-fatal): ${e}\n`);
  }
  ...
  const dataCoverage = dataSourceCoverage(resolve(process.cwd(), "state/shared/quoting"), {
    ...
    // U-CLT-TRAINING-SOURCES: consumed only when the arm actually ran (ok:true) -- a
    // source-stale/source-missing/no-priceable-data result correctly stays unconsumed.
    vendorCostIndexConsumed: Boolean(vendorCostIndexMatch && vendorCostIndexMatch.ok),
    toolPurchasesConsumed: Boolean(toolPurchasesMatch && toolPurchasesMatch.ok),
    docustrataInvoicesConsumed: Boolean(docustrataInvoicesMatch && docustrataInvoicesMatch.ok),
  });
  ...
  try {
    ...
    const snapshot = buildTrainingStatusSnapshot(result, {
      realMatch,
      docustrataMatch,
      docustrataInvoicesMatch,
      vendorCostIndexMatch,
      toolPurchasesMatch,
      dataCoverage,
      baselineProvenance,
  ...
  if (jsonOut) {
    process.stdout.write(JSON.stringify({
      ...
      docustrata_invoices_match: docustrataInvoicesMatch,
      vendor_cost_index_advisory: vendorCostIndexMatch,
      tool_purchases_advisory: toolPurchasesMatch,
    ...
    // U-CLT-TRAINING-SOURCES: the 3 newly-consumed sources, same ok/reason reporting convention.
    if (docustrataInvoicesMatch && docustrataInvoicesMatch.ok) {
      const dim = typeof docustrataInvoicesMatch.median_ratio === "number" ? docustrataInvoicesMatch.median_ratio.toFixed(2) : "n/a";
      process.stdout.write(`[quoting-train-cycle] docustrata-invoices match (ADVISORY): verdict=${docustrataInvoicesMatch.verdict ?? "n/a"} median_ratio=${dim} (deduped=${docustrataInvoicesMatch.deduped}, duplicates_excluded_vs_actuals=${docustrataInvoicesMatch.duplicates_excluded}, total_invoices=${docustrataInvoicesMatch.total_invoices}) -- never alters the factor\n`);
    } else if (docustrataInvoicesMatch && docustrataInvoicesMatch.reason) {
      process.stdout.write(`[quoting-train-cycle] docustrata-invoices match skipped: ${docustrataInvoicesMatch.reason}\n`);
    }
    if (vendorCostIndexMatch && vendorCostIndexMatch.ok) {
      const nm = typeof vendorCostIndexMatch.normalized_median_usd_per_in3 === "number" ? vendorCostIndexMatch.normalized_median_usd_per_in3.toFixed(2) : "n/a";
      const bm = typeof vendorCostIndexMatch.blended_index_median === "number" ? vendorCostIndexMatch.blended_index_median.toFixed(2) : "n/a";
      process.stdout.write(`[quoting-train-cycle] vendor-cost-index advisory: normalized=$${nm}/in3 (n=${vendorCostIndexMatch.normalized_grade_count}) vs blended-index-median=$${bm}/unit reliable=${vendorCostIndexMatch.reliable} -- units-blended, context only, never a ratio\n`);
      if (!vendorCostIndexMatch.reliable) {
        process.stdout.write(`[quoting-train-cycle]   ⚠ vendor-cost-index NOT reliable: ${vendorCostIndexMatch.reliability_reason ?? ""}\n`);
      }
    } else if (vendorCostIndexMatch && vendorCostIndexMatch.reason) {
      process.stdout.write(`[quoting-train-cycle] vendor-cost-index advisory skipped: ${vendorCostIndexMatch.reason}\n`);
    }
    if (toolPurchasesMatch && toolPurchasesMatch.ok) {
      process.stdout.write(`[quoting-train-cycle] tool-purchases advisory: ${toolPurchasesMatch.type_count} tool type(s), ${toolPurchasesMatch.reconciled_with_consumption_ledger} reconciled with the consumption ledger -- advisory only\n`);
    } else if (toolPurchasesMatch && toolPurchasesMatch.reason) {
      process.stdout.write(`[quoting-train-cycle] tool-purchases advisory skipped: ${toolPurchasesMatch.reason}\n`);
    }
