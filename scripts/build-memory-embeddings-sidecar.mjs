...
function parseArgs(argv) {
  const a = { limit: Infinity, resume: false,
    ...
    model: DEFAULT_MODEL, url: DEFAULT_OLLAMA_URL, explicitUrl: false,
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    // U-INDIA-EMBED-LANE: explicitUrl marks operator intent -- an explicit --url
    // must never be silently redirected to the dedicated embed lane.
    else if (k === "--url") { a.url = argv[++i] || DEFAULT_OLLAMA_URL; a.explicitUrl = true; }
...
async function embedOneAttempt(text, url, laneFlag, model, timeoutMs) {
  let timer;
  try {
...
// batch sidecar build -- the shared :11434 starves embed requests under fleet
// inference load. Auto-resolve ONLY when the caller did not pin an explicit
// --url (explicitUrl=false, the default): an operator-chosen --url is intent,
// never silently redirected to the lane. A FAILED lane attempt retries MAIN
// once (withMainFallback) -- a broken lane degrades to today's exact behavior.
async function embedOne(text, { url, model, timeoutMs = PER_ITEM_TIMEOUT_MS, explicitUrl = false }) {
  if (explicitUrl) return embedOneAttempt(text, url, false, model, timeoutMs);
  const lane = await resolveEmbedUrl();
  return withMainFallback(lane, (u, laneFlag) => embedOneAttempt(text, u, laneFlag, model, timeoutMs));
}
...
async function main() {
  ...
  const probe = await embedOne("search_query: preflight", { url: args.url, model: args.model, timeoutMs: 15_000, explicitUrl: args.explicitUrl });
  if (!probe) {
  ...
  await runPool(todo, args.concurrency, async (item) => {
    const emb = await embedOne(item.docText, { url: args.url, model: args.model, explicitUrl: args.explicitUrl });
