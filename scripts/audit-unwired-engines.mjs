...
async function main() {
  ...
  // check WIRE-EXEMPT markers -- FULL-file scan (slot/whiskey graft): the CLAUDE.md
  // convention places `// WIRE-EXEMPT: <reason>` adjacent to whatever makes the engine
  // exempt, typically the singleton export at the file BOTTOM -- the prior 2KB head
  // window mis-classified bottom-tagged engines (e.g. LatheThermodynamicsEngine, tag
  // near its line-2827 singleton) as UNWIRED. fs.readFile already loads the whole
  // file, so scanning all of it costs no extra I/O.
  const exemptCount = { count: 0 };
  for (const [name, info] of engines) {
    try {
      const content = await fs.readFile(info.path, "utf8");
      const exempt = wireExemptMarker(content);
      if (exempt.exempt) {
        info.classified = "WIRE-EXEMPT";
        info.reasons.push(`exempt: ${exempt.reason ?? "no reason given"}`);
        exemptCount.count++;
      }
    } catch {}
  }
  ...
  const out = {
    ...
    notes: [
      ...
      "WIRE-EXEMPT engines have a `// WIRE-EXEMPT: <reason>` marker ANYWHERE in the file (full-content scan; the marker conventionally sits at the bottom singleton export).",
...
/**
 * Detect a `// WIRE-EXEMPT: <reason>` marker ANYWHERE in an engine's source
 * (grafted from slot/whiskey via U-GOLF-WHISKEY-WORKTREE-INTEGRATION). The
 * CLAUDE.md convention places the marker adjacent to whatever makes the engine
 * exempt -- for a singleton-wrapped engine that is typically the singleton export
 * at the file BOTTOM, not the head, so a head-only window cannot see it.
 * @param {string} content full engine source
 * @returns {{exempt: boolean, reason: string|null}}
 */
export function wireExemptMarker(content) {
  if (!content) return { exempt: false, reason: null };
  const m = content.match(/\/\/\s*WIRE-EXEMPT:\s*(.+)/);
  return m ? { exempt: true, reason: m[1].trim() } : { exempt: false, reason: null };
}
