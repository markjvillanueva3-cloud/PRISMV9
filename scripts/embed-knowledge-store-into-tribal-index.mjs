...
async function embedTextAttempt(text, fetchImpl, url, laneFlag, expectedDim) {
  const body = laneFlag ? withLaneOptions({ model: MODEL, prompt: text }) : { model: MODEL, prompt: text };
  ...
  if (!res.ok) {
    const respBody = typeof res.text === "function" ? await res.text() : "";
    throw new Error(`ollama embed ${res.status} @ ${url}: ${respBody}`);
  }
  ...
  if (expectedDim > 0 && j.embedding.length !== expectedDim) {
    throw new Error(`ollama embedding dim ${j.embedding.length} != index dim ${expectedDim} @ ${url} (R12 fail-loud)`);
  }
  return j.embedding;
}

// U-INDIA-EMBED-LANE: prefer the dedicated CPU embed lane (:11435) when it
// answers -- the shared :11434 starves embeds behind fleet inference load. A
// FAILED lane attempt (including wrong-dim -- a mis-served lane model must
// not poison the index) retries MAIN once. Auto-resolve ONLY on the real
// fetch: an injected fetchImpl (tests) keeps legacy single-URL behavior.
export async function embedText(text, fetchImpl = fetch, expectedDim = 0) {
  if (fetchImpl !== fetch) return embedTextAttempt(text, fetchImpl, OLLAMA_URL, false, expectedDim);
  const lane = await resolveEmbedUrl();
  if (lane.lane) {
    try { return await embedTextAttempt(text, fetchImpl, lane.url, true, expectedDim); }
    catch { /* broken lane -> one MAIN retry below; MAIN's error is the one thrown */ }
    return embedTextAttempt(text, fetchImpl, OLLAMA_URL, false, expectedDim);
  }
  return embedTextAttempt(text, fetchImpl, lane.url, false, expectedDim);
}
