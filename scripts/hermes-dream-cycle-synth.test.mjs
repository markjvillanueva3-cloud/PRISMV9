...
describe("runGalaxyCascade", () => {
  ...
  // ----- self-timeout (the real 267014 fix, slot:bravo 2026-06-17) -----

  it("self-timeout: an overrunning cascade (killed:true + ETIMEDOUT) is FAIL-SOFT, never throws", () => {
    // execFileSync kills the child on timeout and throws killed:true (+ ETIMEDOUT
    // on recent node). This is the graceful bound that REPLACES the OS task-kill
    // (267014): the dream synth must still exit 0, so the cascade must not propagate.
    let logged = "";
    let r;
    assert.doesNotThrow(() => {
      r = runGalaxyCascade({
        execImpl: () => { throw Object.assign(new Error("spawn ... ETIMEDOUT"), { killed: true, code: "ETIMEDOUT" }); },
        env: {},
        timeoutMs: 1000,
        logImpl: (m) => { logged += m; },
      });
    ...
    assert.equal(r.timeoutMs, 1000);
    assert.match(logged, /timed out/);
  });

  it("self-timeout: killed:true WITHOUT an ETIMEDOUT code (older node) is still a timeout, not a crash", () => {
    const r = runGalaxyCascade({
      execImpl: () => { throw Object.assign(new Error("terminated"), { killed: true }); },
      env: {},
      timeoutMs: 500,
    });
    assert.equal(r.timedOut, true);
  ...
  it("self-timeout: ETIMEDOUT code WITHOUT killed (node v22 shape) is a timeout -- ISOLATES the OR clause", () => {
    // Node v22.12.0 throws a timeout with {code:'ETIMEDOUT', killed:undefined}. This
    // pins the `code === "ETIMEDOUT"` half of the OR independently of `killed===true`
    // (scrutiny arm B P1 -- the other timeout tests set BOTH, so neither isolated the clause).
    const r = runGalaxyCascade({
      execImpl: () => { throw Object.assign(new Error("spawn ETIMEDOUT"), { code: "ETIMEDOUT" }); },
      env: {},
      timeoutMs: 750,
    });
    assert.equal(r.timedOut, true);
    assert.equal(r.ok, false);
    assert.equal(r.timeoutMs, 750);
  });

  it("bound: passes a timeout + SIGTERM kill to execImpl (default 20min < the 30min task limit)", () => {
    let opts = null;
    runGalaxyCascade({
      execImpl: (_bin, _args, o) => { opts = o; },
      env: {},
    });
    assert.equal(opts.timeout, 20 * 60 * 1000);
    assert.equal(opts.killSignal, "SIGTERM");
  });

  it("knob PRISM_DREAM_CASCADE_TIMEOUT_MS overrides the default cascade timeout", () => {
    let opts = null;
    runGalaxyCascade({
      execImpl: (_bin, _args, o) => { opts = o; },
      env: { PRISM_DREAM_CASCADE_TIMEOUT_MS: "123456" },
    });
    assert.equal(opts.timeout, 123456);
  });

  it("guard: a non-positive explicit timeoutMs falls through to the default, NOT unbounded (timeout:0 footgun)", () => {
    // Node treats timeout:0 as no-timeout. An explicit 0/negative/NaN must NOT pass
    // straight through (would silently un-bound the cascade) -- it coerces to the
    // 20min default (scrutiny arm A/B P2).
    for (const bad of [0, -5, NaN]) {
      let opts = null;
      runGalaxyCascade({ execImpl: (_b, _a, o) => { opts = o; }, env: {}, timeoutMs: bad });
      assert.equal(opts.timeout, 20 * 60 * 1000, `timeoutMs=${bad} should fall through to default`);
    }
  });
});

// ───────────── catalog-stub noise exclusion (U-DREAM-STUB-NOISE, slot:bravo) ─────────────

describe("isCatalogStub", () => {
  it("flags node_* catalog stubs and spares real knowledge memos", () => {
    assert.equal(isCatalogStub("node_course_mit_18_02.md"), true);
    assert.equal(isCatalogStub("node_formula_kienzle.md"), true);
    assert.equal(isCatalogStub("node_tribal_thinwall.md"), true);
    assert.equal(isCatalogStub("feedback_check_units_first.md"), false);
    assert.equal(isCatalogStub("reference_fanout_request.md"), false);
    assert.equal(isCatalogStub("project_jm_die.md"), false);
    assert.equal(isCatalogStub("a.md"), false);
  });
});

describe("run -- catalog-stub exclusion (noise-down + signal-preserved)", () => {
  // 2 real knowledge memos that connect to each other, + 2 node_* catalog stubs that connect to
  // EACH OTHER but share NO keyword with the real pair (disjoint -> Jaccard 0 cross-pair). So the
  // ONLY noise is the stub<->stub connection; the ONLY signal is the real<->real connection.
  const layout = () => ({
    "/r/reference/real_force_a.md": "kienzle force mill spindle titanium roughing strategy",
    "/r/reference/real_force_b.md": "kienzle force mill spindle titanium finishing strategy",
    "/r/reference/node_course_x.md": "course mit fall lecture syllabus problem set",
    "/r/reference/node_course_y.md": "course mit fall lecture syllabus exam grading",
  });

  it("DEFAULT excludes node_* stubs: only the real<->real connection survives, stubs reported", () => {
    const fs = makeFakeFs(layout());
    const r = run({ root: "/r", fsImpl: fs, date: "2026-01-01", now: 0 });
    assert.equal(r.ok, true);
    assert.equal(r.memo_count, 2);                 // only the 2 real knowledge memos walked
    assert.equal(r.catalog_stub_excluded, 2);      // R12: both stubs accounted for, not silently dropped
    assert.equal(r.connection_count, 1);           // the stub<->stub noise connection is gone
    const md = fs.readFileSync("/r/dreams/2026-01-01.md");
    assert.ok(md.includes("real_force_a") && md.includes("real_force_b"), "real signal preserved");
    assert.ok(!md.includes("node_course"), "no catalog-stub noise in the connections");
  });

  it("--include-catalog-stubs keeps the legacy all-corpus behavior (both connections, 0 excluded)", () => {
    const fs = makeFakeFs(layout());
    const r = run({ root: "/r", fsImpl: fs, date: "2026-01-01", now: 0, includeCatalogStubs: true });
    assert.equal(r.memo_count, 4);
    assert.equal(r.catalog_stub_excluded, 0);
    assert.equal(r.connection_count, 2);           // real<->real AND stub<->stub
    const md = fs.readFileSync("/r/dreams/2026-01-01.md");
    assert.ok(md.includes("node_course_x"), "stubs present when opted in");
    assert.ok(md.includes("real_force_a"), "real signal STILL present (exclusion never drops real memos)");
  });

  it("signal-preserved invariant: the real<->real connection is identical with and without stubs", () => {
    const def = run({ root: "/r", fsImpl: makeFakeFs(layout()), date: "2026-01-01", now: 0 });
    const inc = run({ root: "/r", fsImpl: makeFakeFs(layout()), date: "2026-01-01", now: 0, includeCatalogStubs: true });
    // Excluding stubs removes ONLY the stub connection (2 -> 1); the real connection count is unchanged.
    assert.equal(inc.connection_count - def.connection_count, 1);
    assert.equal(def.connection_count, 1); // the surviving connection is the real one
  });

  it("runWithSynth (default LLM-off path) applies the SAME stub exclusion as run() -- symmetric plumbing", async () => {
    // runWithSynth threads includeCatalogStubs identically to run(); pin the symmetry so the async
    // LLM variant can never silently diverge (no callFn -> behaves as the sync path, just async).
    const fs = makeFakeFs(layout());
    const r = await runWithSynth({ root: "/r", fsImpl: fs, date: "2026-01-01", now: 0 });
    assert.equal(r.ok, true);
    assert.equal(r.memo_count, 2);
    assert.equal(r.catalog_stub_excluded, 2);
    assert.equal(r.connection_count, 1);
    const fsInc = makeFakeFs(layout());
    const rInc = await runWithSynth({ root: "/r", fsImpl: fsInc, date: "2026-01-01", now: 0, includeCatalogStubs: true });
    assert.equal(rInc.memo_count, 4);
    assert.equal(rInc.catalog_stub_excluded, 0);
    assert.equal(rInc.connection_count, 2);
  });
});

describe("synthesizeDreamMarkdown -- catalog-stub transparency", () => {
  const base = { date: "2026-01-01", totalMemos: 100, connections: [], cluster: new Map(), params: { minJaccard: 0.15, topKKeywords: 20 } };
  it("surfaces the excluded-stub count in frontmatter + header when > 0", () => {
    const md = synthesizeDreamMarkdown({ ...base, catalogStubExcluded: 12858 });
    assert.ok(md.includes("catalog_stub_excluded: 12858"), "frontmatter records the exclusion");
    assert.ok(/excluded 12858 catalog-stub/.test(md), "header explains the exclusion");
    assert.ok(md.includes("100 knowledge memos"), "counts the real memos walked");
  });
  it("omits the exclusion note when none were excluded (legacy/opted-in run)", () => {
    const md = synthesizeDreamMarkdown({ ...base, catalogStubExcluded: 0 });
    assert.ok(md.includes("catalog_stub_excluded: 0"));
    assert.ok(!/excluded \d+ catalog-stub/.test(md), "no note when nothing excluded");
  });
});
