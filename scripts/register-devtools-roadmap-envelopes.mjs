#!/usr/bin/env node
/**
 * register-devtools-roadmap-envelopes.mjs — one-shot tool (idempotent).
 *
 ...
 *   headers with indented scalar fields (pillar/tier/ai_priority_score/
 *   leverage_score/why/depends_on/blocks/parallel_with/viz_node_id/loop_schedule)
 *   and verifies_via:/micro_steps:/adversarial_cases:/variability_axis: blocks.
 *
...
const trackOf = (msId) => msId.replace(/-MS\d+$/i, ""); // TOOL-INVENTORY-MS0 → TOOL-INVENTORY
const stripBackticks = (s) => String(s || "").replace(/`/g, "").trim();
function parseList(v) {
  // "[U-H1, U-H2]" → ["U-H1","U-H2"];  "[]" → [];  "U-H1" → ["U-H1"]
  const s = stripBackticks(v);
  if (!s || s === "[]" || s === "none" || s === "-") return [];
...
function parseAtomized(filePath) {
  ...
  // milestone id: first token in fm.milestone that matches XXX-MS<n>
  let msId = (fm.milestone || "").match(/[A-Z0-9][A-Z0-9-]*-MS\d+/i)?.[0]
  ...
  const title = titleM ? titleM[1].replace(/—.*$/, "").trim() || titleM[1].trim() : (msId || "untitled");

  // brief: first `> ...` blockquote (join wrapped lines)
  const briefLines = [];
  ...
  // units: `## U-<id> — <title>` sections (also accept `## H<n>` / `## K2-K<n>` style fallbacks)
  const unitRe = /^##\s+(U-[A-Za-z0-9._\-]+|[HK]\d[A-Za-z0-9._\-]*|K2-K\d+[A-Za-z0-9._\-]*)\s*[—\-:]+\s*(.+?)\s*$/gm;
  const headers = [];
  let m;
  ...
  for (let i = 0; i < headers.length; i++) {
    ...
    // scalar fields: `- key: value` lines (top-of-unit block, before verifies_via/micro_steps)
    const fields = {};
    for (const line of seg.split("\n")) {
      const fm2 = line.match(/^\s*-\s+([a-zA-Z_]+):\s*(.*)$/);
      if (fm2) { if (!(fm2[1] in fields)) fields[fm2[1]] = fm2[2].trim(); }
      if (/^(verifies_via|micro_steps|adversarial_cases|variability_axis):/.test(line.trim())) break;
    }
    const unitId = h.id.startsWith("U-") ? h.id : `U-${h.id}`;
    units.push({
      id: unitId,
      title: h.title.replace(/\s+\(.*\bsub-unit\b.*\)\s*$/, "").trim() || h.title.trim(),
      ...
      loop_schedule: fields.loop_schedule || null,
      status: "not_started",
      four_loop: FOUR_LOOP,
      exit_conditions: STD_EXIT,
      // pointer back to the canonical micro_step source — implementer reads this
      source_atomized: path.relative(PRISM, filePath).replace(/\\/g, "/"),
      atomized_anchor: `## ${h.id}`,
    });
  }
...
for (const fp of files) {
  ...
  const milestoneTier = units.some((u) => u.tier === 0) ? 0 : Math.min(...units.map((u) => u.tier));
  const track = trackOf(msId);
  ...
  const envelope = {
    ...
    track,                                   // milestone-group name (matches HOOK-SYNERGY-MS0.json convention)
    ...
    rationale: brief,
    phases: [
      { id: "P0", title, sessions: String(sessions), units },
    ],
  ...
  if (!opts["dry-run"]) {
    fs.writeFileSync(path.join(ENV_DIR, `${msId}.json`), JSON.stringify(envelope, null, 2) + "\n");
  }
  written.push({ id: msId, units: units.length, declared: frontmatter.declared_total_units, tier: milestoneTier, track, file: path.basename(fp) });

  indexAdds.push({
    id: msId, title, track, status: "not_started", tier: milestoneTier, roadmap_priority: 0,
    total_units: units.length, completed_units: 0, dependencies: [], blocks: [],
    envelope_path: `milestones/${msId}.json`,
    priority: milestoneTier === 0 ? "CRITICAL" : milestoneTier === 1 ? "HIGH" : "MEDIUM",
    sessions_p50: Math.max(2, Math.ceil(units.length / 4)),
    sessions_p90: Math.max(3, Math.ceil(units.length / 3)),
    viz_node_id: `ghost.ms.${slug(msId)}`,
    description: brief.slice(0, 280),
    source: sourceRel,
  });
}
