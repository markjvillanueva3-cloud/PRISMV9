...
export function generate(tsFiles, existingNodeIds = [], resolver = null) {
  ...
  let pivots = 0, leaves = 0, edges = 0, bridgesResolved = 0, bridgesDropped = 0;
  for (const f of tsFiles) {
    ...
    for (const engine of bridges) {
      ...
      // generation time so the augmentation never carries a dangling edge (class-name-node-resolver.mjs).
      // No resolver (standalone / back-compat) -> emit the bare name as before.
      let to = engine;
      if (resolver) {
  ...
  return { newNodes, newEdges, stats: { roostAdded: 1, pivots, leaves, edges, bridgesResolved, bridgesDropped, filesProcessed: tsFiles.length } };
}
...
function main() {
  ...
  const { newNodes, newEdges, stats } = generate(tsFiles, [], resolver);
  const result = {
