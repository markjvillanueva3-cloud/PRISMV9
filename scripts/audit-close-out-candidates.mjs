...
// HEAP GUARD (Blackwell doctrine "never fight a low default"). The milestone +
// transcript scan OOMs under portable-node's ~432MB default heap -- observed LIVE
// 2026-06-25 (slot:zulu) as a zlib `inflateValidate` crash that left
// CLOSE-OUT-CANDIDATES stale and the /goal goal-complete-gate blocking. Self-reexec
// ONCE with a generous heap so EVERY invocation succeeds (the /close-out-audit skill,
// the goal-gate pre-flight, the close-out-audit-suggest hook, manual runs) -- not only
// the rare caller that happens to pass --max-old-space-size. Idempotent: the child
// carries PRISM_CLOSE_OUT_REEXEC=1 (loop breaker); a caller-supplied heap flag skips it.
// Knobs: PRISM_CLOSE_OUT_AUDIT_HEAP_MB (default 4096), PRISM_CLOSE_OUT_REEXEC.
const CLOSE_OUT_HEAP_MB = Number(process.env.PRISM_CLOSE_OUT_AUDIT_HEAP_MB) || 4096;
if (
  process.env.PRISM_CLOSE_OUT_REEXEC !== "1" &&
  !process.execArgv.some((a) => a.startsWith("--max-old-space-size"))
) {
  const child = spawnSync(
    process.execPath,
    [`--max-old-space-size=${CLOSE_OUT_HEAP_MB}`, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
    { stdio: "inherit", env: { ...process.env, PRISM_CLOSE_OUT_REEXEC: "1" }, windowsHide: true },
  );
