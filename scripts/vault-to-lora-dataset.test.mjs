...
import {
  ...
  DEFAULT_OUT,
  DEFAULT_SYNTH_OUT,
  extractAiSynergySection,
  buildExamplesFromAiSynergy,
  collectGalaxyAiSynergyExamples,
  resolveAiSynOutPath,
  DEFAULT_AISYN_OUT,
  isOwnerAiSynergySection,
...
// ---- galaxy-ai-synergy source (U-LORA-GALAXY-AISYN, slot:bravo 2026-06-14) ----

// OWNER fixture (matches buildAiSynergySection's owner head wording).
const AISYN_CLAUDE = [
  "# foo galaxy",
  "## Some other section",
  "blah",
  ...
  "",
  "This galaxy is a first-class AI-substrate **participant** -- it OWNS 5 AI engine(s) (e.g. `FooEngine`).",
  "- Reasoning bridge: CAG + RAG hybrid reasoning via the local Ollama stack.",
  "- Vault -> LoRA: synthesis brain feeds the fleet LoRA dataset.",
  "- GNN (GraphSAGE) tier-5: neural wiring-inference; embedding recall.",
  "",
  "## Next section",
  "should not be captured",
].join("\n");

// CONSUMER fixture (matches buildAiSynergySection's consumer head wording -- bold **consumer**).
const AISYN_CONSUMER_CLAUDE = [
  "# bar galaxy",
  "## AI Synergy (PSN leg #10)",
  "",
...
test("extractAiSynergySection pulls ONLY the AI-Synergy section (stops at next ## heading)", () => {
  const s = extractAiSynergySection(AISYN_CLAUDE);
  assert.match(s, /Reasoning bridge/);
  assert.match(s, /Vault -> LoRA/);
  assert.doesNotMatch(s, /should not be captured/); // bounded at next ## heading
  assert.doesNotMatch(s, /Some other section/);      // earlier section not included
});

test("extractAiSynergySection returns '' when the marker is absent / input non-string", () => {
  assert.equal(extractAiSynergySection("# galaxy\nno ai section here"), "");
  assert.equal(extractAiSynergySection(null), "");
});

test("R9: buildExamplesFromAiSynergy emits a galaxy-tagged pair carrying real AI signal", () => {
  const ex = buildExamplesFromAiSynergy(AISYN_CLAUDE, "foo");
  assert.equal(ex.length, 1);
  assert.equal(ex[0]._galaxy, "foo");
  assert.match(ex[0].instruction, /foo domain/);
  // the answer must carry genuine AI substrate terms (not a hollow pair) -- fails if extraction breaks
  assert.match(ex[0].output, /Reasoning bridge/);
  assert.match(ex[0].output, /LoRA/);
  assert.match(ex[0].output, /GNN/);
});

test("R9: buildExamplesFromAiSynergy returns [] when no section or section too thin (no degenerate pair)", () => {
  assert.deepEqual(buildExamplesFromAiSynergy("# g\nno section", "foo"), []);
  // a present-but-thin section (< AISYN_MIN_CHARS) must NOT emit a pair
  assert.deepEqual(buildExamplesFromAiSynergy("## AI Synergy (PSN leg #10)\nshort", "foo"), []);
  assert.deepEqual(buildExamplesFromAiSynergy(AISYN_CLAUDE, ""), []); // no galaxy
});

// ---- OWNER-ONLY gate (U-LORA-OWNER-COVERAGE, slot:bravo 2026-06-14) ----

test("isOwnerAiSynergySection: owner section true, consumer section false, empty/non-string false", () => {
  assert.equal(isOwnerAiSynergySection(extractAiSynergySection(AISYN_CLAUDE)), true);          // participant head
  assert.equal(isOwnerAiSynergySection(extractAiSynergySection(AISYN_CONSUMER_CLAUDE)), false); // **consumer** head
  assert.equal(isOwnerAiSynergySection(""), false);
  assert.equal(isOwnerAiSynergySection(null), false);
});

test("R9: buildExamplesFromAiSynergy SKIPS a consumer-boilerplate section (owner-only, no padding)", () => {
  // the consumer section is non-thin (>80 chars) + carries AI terms, yet must NOT emit a pair --
  // proves the skip is the owner-gate, not the thin-section guard. Fails if the gate is removed.
  assert.ok(extractAiSynergySection(AISYN_CONSUMER_CLAUDE).length >= 80);
  assert.deepEqual(buildExamplesFromAiSynergy(AISYN_CONSUMER_CLAUDE, "bar"), []);
  // contrast: the owner fixture (same shape) DOES emit -- so the difference is owner-vs-consumer alone
  assert.equal(buildExamplesFromAiSynergy(AISYN_CLAUDE, "foo").length, 1);
});

test("resolveAiSynOutPath clobber-guard redirects the feedback default away (never overwrite verified set)", () => {
  assert.equal(resolveAiSynOutPath(DEFAULT_OUT), DEFAULT_AISYN_OUT);
  assert.equal(resolveAiSynOutPath("/tmp/custom.jsonl"), "/tmp/custom.jsonl");
  assert.equal(resolveAiSynOutPath(null), null);
});

test("R15 live: collectGalaxyAiSynergyExamples scans real galaxy CLAUDE.md -> OWNER-ONLY galaxy-tagged pairs", () => {
  const { examples, galaxies, scanned } = collectGalaxyAiSynergyExamples();
  assert.ok(scanned >= 30, `expected to scan the real galaxy set, scanned ${scanned}`);
  // owner-only (U-LORA-OWNER-COVERAGE): only AI-OWNER galaxies emit; the >=20 of the mixed era no longer holds.
  assert.ok(galaxies >= 5, `expected >=5 AI-owner galaxies with a section, got ${galaxies}`);
  assert.ok(examples.every((e) => e._galaxy && e.output.length >= 80)); // every pair tagged + non-thin
  // PURITY: not one extracted pair is a consumer-boilerplate section (the gate holds against live data)
  assert.ok(examples.every((e) => isOwnerAiSynergySection(e.output)), "every live pair must be an owner section");
});
