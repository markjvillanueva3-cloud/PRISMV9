...
import { buildDegradationWarnings } from "./lib/cross-substrate-warnings.mjs";
...
function main() {
  ...
  //     cross-substrate edge type; it materializes one edge per domain that owns an octopus
  //     outcomes jsonl -- 13 as of 2026-06-21 (U-XSUB-CONSENSUS-REFRESH), up from the 1
  //     (hermes-zulu only) present at U-XSUB-CONSENSUS-OF. AUTO-SCALES -- every domain that
  //     gains an outcomes jsonl links on the next regen with zero code change. The live count
  //     is whatever 13/13-style line main() prints; this number is a dated snapshot, not a
  //     contract. NOT a claim that the spine is "done" -- it is the mechanism (R13/R15).
  //
  ...
  //     never hard-fails -- embeds is optional + the oracle is a separate build step).
  out.warnings = buildDegradationWarnings(out.stats);
  for (const w of out.warnings) console.error(`XSUB-DEGRADED: ${w}`);
  ...
  // ATOMIC temp+rename: a concurrent reader (the test suite + merge-augmentations,
  // which read this gitignored artifact while a regen may be rewriting it) never
  // observes a torn/half-written file (P2 hermeticity, scrutiny arms B+C).
  atomicWriteText(OUT, JSON.stringify(out));
  ...
  try {
    ...
    // degradation sidecar (R12): a health check reads ONE file to know if this regen
    // silently dropped an edge type. Always written (empty warnings -> clean signal).
    fs.writeFileSync(
      path.join(VIZ_DIR, "cross-substrate-warnings.json"),
      JSON.stringify({ at: NOW, warnings: out.warnings, embedsEdges: out.stats.embedsEdges, oracleLoaded: !!out.stats.embed.oracleLoaded }),
    );
    if (out.drift.detected) {
