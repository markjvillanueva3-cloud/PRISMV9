...
// ── deepseek + glm: the two key-gated voices added with U-OCT-PROBE-GLM-DEEPSEEK ──
test("deepseek with a non-empty DEEPSEEK_API_KEY → ready", () => {
  const r = decideVoiceStatus({ voice: "deepseek", probe: { apiKey: FIXTURE_NONEMPTY } });
  assert.equal(r.status, "ready");
  assert.match(r.reason, /DEEPSEEK_API_KEY is set/);
});

test("deepseek with empty DEEPSEEK_API_KEY → missing + key-link remediation", () => {
  const r = decideVoiceStatus({ voice: "deepseek", probe: { apiKey: "" } });
  assert.equal(r.status, "missing");
  assert.match(r.remediation, /platform\.deepseek\.com/);
  assert.match(r.remediation, /DEEPSEEK_API_KEY=/);
});

test("glm with a non-empty key (GLM_API_KEY or ZHIPU_API_KEY) → ready", () => {
  const r = decideVoiceStatus({ voice: "glm", probe: { apiKey: FIXTURE_NONEMPTY } });
  assert.equal(r.status, "ready");
  assert.match(r.reason, /GLM_API_KEY \/ ZHIPU_API_KEY is set/);
});

test("glm with empty key → missing + key-link remediation naming both env vars", () => {
  const r = decideVoiceStatus({ voice: "glm", probe: { apiKey: "" } });
  assert.equal(r.status, "missing");
  assert.match(r.remediation, /bigmodel\.cn/);
  assert.match(r.remediation, /GLM_API_KEY=/);
});
...
test("script E2E: --json mode emits a parseable summary; cross-vendor keys forced missing", () => {
  // Force blank XAI/GEMINI/DEEPSEEK/GLM/ZHIPU in the subprocess env so the test
  // is hermetic for those voices regardless of the live host's state.
  const r = spawnSync(process.execPath, [SCRIPT, "--json"], {
    encoding: "utf-8",
    env: { ...process.env, XAI_API_KEY: "", GEMINI_API_KEY: "", DEEPSEEK_API_KEY: "", GLM_API_KEY: "", ZHIPU_API_KEY: "" },
    timeout: 30_000,
  });
  ...
  assert.equal(payload.totalCount, 7);
  assert.ok(Array.isArray(payload.voices));
  assert.equal(payload.voices.length, 7);
  const voiceNames = payload.voices.map((v) => v.voice).sort();
  assert.deepEqual(voiceNames, ["anthropic", "codex", "deepseek", "glm", "google", "ollama", "xai"]);
  // Every cross-vendor key was blanked -> those voices must classify missing.
  for (const name of ["xai", "google", "deepseek", "glm"]) {
    const v = payload.voices.find((x) => x.voice === name);
    assert.equal(v.status, "missing", `blank key for ${name} must classify missing`);
  }
});
