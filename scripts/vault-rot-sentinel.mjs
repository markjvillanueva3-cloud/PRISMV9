...
function main() {
  ...
  if (args.write) {
    ...
    // Atomic publish (temp + rename): this report now has TWO potential writers -- brain-refresh's
    // auto-fan-out (U-SIERRA-BRAIN-GAP-SENTINELS, every refresh) AND the standalone vault-rot cron
    // (deferred under the migration freeze today, but re-arms later). A bare writeFileSync to the
    // final path could interleave-corrupt the JSON for the vault-health consumer that JSON.parses it;
    // tmp+rename makes each writer's publish atomic. (scrutiny arm C P2, 2026-06-25 slot:sierra.)
    const tmp = `${DEFAULT_REPORT}.tmp.${process.pid}`;
    writeFileSync(tmp, JSON.stringify({ ...report, elapsedMs }, null, 2) + "\n", "utf8");
    renameSync(tmp, DEFAULT_REPORT);
  }
  if (args.json) {
