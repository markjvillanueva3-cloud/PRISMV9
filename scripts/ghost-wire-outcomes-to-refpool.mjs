 ...
 *
 * U-VIZ-GHOST-WIRE-STRENGTH (sierra 2026-06-16): "weak" confirmations (engine name found only in
 * a dispatcher COMMENT, graded by validate-ghost-wires.mjs) are EXCLUDED -- a comment mention is
 * not a verified wire and would inject a false ground-truth label. Rows without the strength field
 * (pre-grading history) are treated as non-weak for backward-compat.
 *
 * Usage:
...
// Significant content fields for an outcome ghost (volatile proposed_at + the constant
// scaffold excluded; note `sourceLedger`, not the vault feeder's `sourceMemory`). The
// merge itself is the shared scripts/lib/refpool-merge.mjs (R15 build-once with the
// vault feeder) -- a re-apply that only re-stamps proposed_at is a no-op (no write).
const OUTCOME_CONTENT_FIELDS = ["proposed_wiring", "confidence", "info", "reason", "sourceLedger", "label", "kind"];
const outcomeContentEqual = (a, b) => ghostContentEqual(a, b, OUTCOME_CONTENT_FIELDS);
...
export function extractConfirmedOutcomes(text) {
  ...
  for (const line of String(text || "").split("\n")) {
    ...
    // U-VIZ-GHOST-WIRE-STRENGTH: a "weak" confirmation means the engine name was
    // found ONLY in a dispatcher COMMENT, not in real code -- that is NOT a verified
    // wire ("existence != correct wiring"), so excluding it keeps a false label out
    // of the GNN ref pool. Backward-compat: rows predating the strength field
    // (confirmationStrength === undefined) are NOT weak, so historical confirmed
    // outcomes are unaffected.
    if (o.validation.confirmationStrength === "weak") continue;
...
export function main() {
  ...
  const g = readGraphStreaming(GRAPH_PATH);
  const merge = mergeGhostsIntoGraph(g, ghosts, outcomeContentEqual);
  if (!merge.changed) {
    // Idempotent no-op: every outcome ref is already present + current. SKIP the
    ...
    console.log(`UP-TO-DATE -- all ${ghosts.length} outcome refs already present + current; no write. graph nodes=${g.nodes.length}.`);
    return;
  }
