...
const DEFAULT_STATE_FILE = env.PRISM_BPA_STATE_FILE || join(REPO_ROOT, "state", "shared", "blueprint-accuracy-state.json");
...
function main() {
  ...
  // RESET path — wipe state.json back to clean v2 baseline. Opt-in only.
  if (RESET) {
    ...
    result.note = "reset complete — state.json reset to v2 baseline";
    return result;
  }
