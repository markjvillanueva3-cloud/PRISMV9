...
.DESCRIPTION
  ...
  knowledge/memories/generated/CONNECTIONS-YYYY-WW.md surfacing contradictions,
  echo patterns, and cross-domain analogies across the week's notes.

  Idempotent: re-running with no flags re-creates the task with the latest
  action string (so a path or model change is picked up on the next install).

  The engine ships as TypeScript; this task invokes the COMPILED .js under
  H:/prism/mcp-server/dist/engines/ConnectionFinderEngine.js. If dist is
...
.PARAMETER RunNow
  Register the task AND fire it immediately once (does not change the schedule).

.EXAMPLE
  pwsh -File H:/prism/scripts/cron/connection-finder-cron.ps1
  # Registers the task at Monday 08:00 local (or refreshes the action string).

.EXAMPLE
  pwsh -File H:/prism/scripts/cron/connection-finder-cron.ps1 -DryRun

.EXAMPLE
  pwsh -File H:/prism/scripts/cron/connection-finder-cron.ps1 -Uninstall

.NOTES
  Knob:  PRISM_CONNECTION_FINDER_VAULT_ROOT     (default H:/prism/knowledge)
  Knob:  PRISM_CONNECTION_FINDER_DATE           (override target date — backfill)
  Knob:  PRISM_CONNECTION_FINDER_OLLAMA_MODEL   (default qwen2.5-coder:7b)
  Knob:  PRISM_CONNECTION_FINDER_OLLAMA_URL     (default http://127.0.0.1:11434/api/generate)
#>
...
$TaskName    = 'PRISM Weekly Connection Brief'
$TaskFolder  = '\PRISM\'
$RunAt       = '08:00'   # 24h local time
$RunDay      = 'Monday'
$RepoRoot    = 'H:/prism'
$EnginePath  = "$RepoRoot/mcp-server/dist/engines/ConnectionFinderEngine.js"
...
function Write-Info($msg) { Write-Host "[connection-finder-cron] $msg" }
function Write-Warn($msg) { Write-Warning "[connection-finder-cron] $msg" }
...
$LogPath  = "$LogDir/connection-finder-cron.log"
if (-not $DryRun -and -not (Test-Path $LogDir)) {
  New-Item -ItemType Directory -Path $LogDir -Force | Out-Null
}

# PowerShell -Command wrapper: redirect both streams to the log.
$Argument = @(
  '-NoProfile',
  '-NonInteractive',
  '-Command',
  "& '$NodeExe' '$EnginePath' *>> '$LogPath'"
) -join ' '

$Action = New-ScheduledTaskAction -Execute 'powershell.exe' -Argument $Argument

# Weekly trigger: every Monday at $RunAt local.
$TriggerTime = [DateTime]::Today.AddHours( [int]$RunAt.Split(':')[0] ).AddMinutes( [int]$RunAt.Split(':')[1] )
$Trigger     = New-ScheduledTaskTrigger -Weekly -DaysOfWeek $RunDay -At $TriggerTime
...
if ($DryRun) {
  ...
  Write-Info "  Schedule : Weekly $RunDay at $RunAt local"
...
Register-ScheduledTask `
  ...
  -Principal $Principal `
  -Settings $Settings `
  -Description "PRISM OBSIDIAN-INTELLIGENCE-MS3/B2 — weekly cross-source connection brief (contradiction / echo / cross-domain) from the knowledge vault via Ollama qwen2.5-coder. Log: $LogPath" | Out-Null

Write-Info "Registered '$TaskName' — weekly $RunDay at $RunAt local, action logs to $LogPath"
