...
import {
  ...
  phantomPredictions, labelHealthReport, prunePhantomCeiling, growthCeiling,
...
describe("prunePhantomCeiling (honest prune BOUND -- FINDING 4 lever, no fabricated gain)", () => {
  // X: n=2 -> 1 correct, 1 miss to Z (phantom).  Y: n=1 -> miss to Z.  Z: predicted x2, never truth.
  // current macroF1 = (X F1 0.6667 + Y F1 0)/2 = 0.3333. Rewriting Z-preds to truth -> X,Y perfect -> 1.0.
  const PRUNE = [
    { truth: "X", predicted: "X", correct: true },
    { truth: "X", predicted: "Z", correct: false },
    { truth: "Y", predicted: "Z", correct: false },
  ];
  it("brackets the effect: floor = current macroF1, ceiling = optimistic re-inference of freed preds", () => {
    const r = prunePhantomCeiling(PRUNE);
    assert.deepEqual(r.prunedLabels, ["Z"]);
    assert.equal(r.freedPredictionMass, 2);
    assert.ok(Math.abs(r.macroF1Floor - 1 / 3) < 1e-3, "floor = (X 0.667 + Y 0)/2 = 0.333");
    assert.ok(Math.abs(r.macroF1Ceiling - 1.0) < 1e-9, "ceiling: both Z-preds re-inferred correct -> perfect");
    assert.ok(Math.abs(r.ceilingGain - (1.0 - 1 / 3)) < 1e-3);
  });
  it("floor never exceeds ceiling and equals the un-pruned macroF1 (prune ALONE moves nothing)", () => {
    const r = prunePhantomCeiling(PRUNE);
    assert.ok(r.macroF1Floor <= r.macroF1Ceiling);
    assert.ok(Math.abs(r.macroF1Floor - macroF1(perClassStats(PRUNE))) < 1e-12, "R12: no post-hoc gain claimed");
  });
  it("no phantom labels -> no-op: floor == ceiling, zero freed mass, empty prune list", () => {
    const r = prunePhantomCeiling(SAMPLES);
    assert.deepEqual(r.prunedLabels, []);
    assert.equal(r.freedPredictionMass, 0);
    assert.equal(r.ceilingGain, 0);
    assert.ok(Math.abs(r.macroF1Floor - r.macroF1Ceiling) < 1e-12);
    assert.ok(Math.abs(r.macroF1Floor - macroF1(perClassStats(SAMPLES))) < 1e-12);
  });
  it("adversarial: empty / null -> zeros, empty list, never throws", () => {
    assert.doesNotThrow(() => {
      const r = prunePhantomCeiling(null);
      assert.deepEqual(r.prunedLabels, []);
      assert.equal(r.macroF1Floor, 0);
      assert.equal(r.macroF1Ceiling, 0);
      assert.equal(prunePhantomCeiling([]).freedPredictionMass, 0);
    });
  });
});

describe("growthCeiling (FINDING 1 #1 lever -- honest projection of growing starved classes)", () => {
  const stats = perClassStats(SAMPLES); // A n=3 f1=0.6667, B n=1 f1=0.6667, C n=1 f1=0
  it("projects starved classes to AT LEAST the well-sampled mean F1 (floor -> ceiling upper bound)", () => {
    const g = growthCeiling(stats, { threshold: 2 }); // A well-sampled (n=3); B,C starved (n=1)
    assert.equal(g.wellSampledCount, 1);
    assert.equal(g.starvedCount, 2);
    assert.ok(Math.abs(g.meanWellSampledF1 - 2 / 3) < 1e-9, "mean well-sampled F1 = A's 0.6667");
    assert.ok(Math.abs(g.macroF1Floor - 4 / 9) < 1e-9, "floor = current macroF1 = (2/3+2/3+0)/3");
    assert.ok(Math.abs(g.macroF1Ceiling - 2 / 3) < 1e-9, "all 3 projected to >=2/3 -> 2/3");
    assert.ok(Math.abs(g.ceilingGain - 2 / 9) < 1e-3);
    assert.ok(g.macroF1Ceiling >= g.macroF1Floor, "ceiling is a true upper bound");
  });
  it("no well-sampled anchor -> REFUSES to project (R12: no fabricated target), floor==ceiling", () => {
    const g = growthCeiling(stats, { threshold: 5 }); // A n=3 < 5 -> nothing well-sampled
    assert.equal(g.wellSampledCount, 0);
    assert.equal(g.meanWellSampledF1, null);
    assert.equal(g.ceilingGain, 0);
    assert.ok(Math.abs(g.macroF1Floor - g.macroF1Ceiling) < 1e-12);
    assert.match(g.note, /fabricat/i);
  });
  it("no starved class -> growth no-op, ceiling == floor", () => {
    const allWell = perClassStats([
      { truth: "P", predicted: "P" }, { truth: "P", predicted: "P" },
      { truth: "Q", predicted: "Q" }, { truth: "Q", predicted: "Q" },
    ]);
    const g = growthCeiling(allWell, { threshold: 2 });
    assert.equal(g.starvedCount, 0);
    assert.ok(Math.abs(g.macroF1Ceiling - g.macroF1Floor) < 1e-12);
    assert.equal(g.ceilingGain, 0);
  });
  it("adversarial: null / non-Map -> zeros, never throws", () => {
    assert.doesNotThrow(() => {
      const g = growthCeiling(null);
      assert.equal(g.macroF1Floor, 0);
      assert.equal(g.macroF1Ceiling, 0);
      assert.equal(g.meanWellSampledF1, null);
    });
  });
});
...
describe("analyzeEval (full bundle)", () => {
  it("produces the bundle from an eval doc with samples", () => {
    ...
    // phantomPruneCeiling is wired into the bundle and holds the floor<=ceiling invariant.
    assert.ok(a.phantomPruneCeiling && a.phantomPruneCeiling.macroF1Floor <= a.phantomPruneCeiling.macroF1Ceiling);
    assert.ok(a.growthCeiling && a.growthCeiling.macroF1Floor <= a.growthCeiling.macroF1Ceiling);
  });
