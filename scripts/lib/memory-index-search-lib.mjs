...
import { resolveEmbedUrlSync, withLaneOptions } from "./embed-endpoint.mjs";
...
const EMBED_CIRCUIT_COOLDOWN_MS = 120_000; // skip dense for 2 min once the breaker OPENS
// U-BRAVO-EMBED-CIRCUIT-THRESHOLD (2026-07-02): require N *consecutive* embed
// failures before OPENING the breaker. The original 1-strike trip darkened the
// dense/semantic recall arm for a full 2-min window on a SINGLE transient blip,
// and because EMBED_CIRCUIT_PATH is ONE global file shared by all 26 fleet slots,
// one slot's blip blinded fleet-wide semantic recall (measured 2026-07-02: a warm
// n=50 recall-eval engaged hybrid on 2 queries then fell to BM25-only for the next
// 48 after one early embed timeout opened the breaker). A consecutive-failure
// threshold tolerates isolated transients while still protecting the 5 s hook
// budget when ollama is genuinely down (>=N failures in a row within the window).
// Knob: PRISM_MEMORY_EMBED_CIRCUIT_THRESHOLD (default 3; 1 = legacy 1-strike).
const DEFAULT_EMBED_CIRCUIT_THRESHOLD = 3;
export function embedCircuitFailureThreshold() {
  const raw = Number(process.env.PRISM_MEMORY_EMBED_CIRCUIT_THRESHOLD);
  return Number.isFinite(raw) && raw >= 1 ? Math.floor(raw) : DEFAULT_EMBED_CIRCUIT_THRESHOLD;
}
...
export function embedQueryViaOllamaSync(query, {
  ...
  // U-INDIA-EMBED-LANE: no explicit url -> prefer the dedicated CPU embed lane
  // (:11435) so the 2.5s-capped recall embed survives fleet inference load on
  // :11434 (live 2026-07-01: :11434 starved ALL embed requests >300s under
  // mining). Sidecar-cached probe (~0 amortized; curl-second-granular ~1s cap
  // on the cold miss). Lane requests carry the num_gpu:0 CPU pin; a FAILED
  // lane embed retries the legacy DEFAULT_OLLAMA_URL once (broken lane must
  // degrade to legacy, never dark recall / never trip the circuit breaker for
  // MAIN's sins). Lane-down keeps the legacy hardcoded DEFAULT_OLLAMA_URL --
  // byte-identical to pre-lane behavior. Only auto-resolve on the REAL
  // execFileSync: an injected execImpl (tests) keeps the legacy single-URL
  // behavior and never spawns a probe curl.
  let laneChosen = false;
  if (!url) {
    if (execImpl === execFileSync) {
      const lane = resolveEmbedUrlSync({ timeoutMs: 800 });
      laneChosen = lane.lane;
      url = laneChosen ? lane.url : DEFAULT_OLLAMA_URL;
    } else {
      url = DEFAULT_OLLAMA_URL;
    }
  }
  const baseBody = { model, prompt: buildEmbedQueryText(query), keep_alive: RECALL_EMBED_KEEP_ALIVE };
  const payload = JSON.stringify(laneChosen ? withLaneOptions(baseBody) : baseBody);
  ...
  const curlMaxSec = Math.max(1, Math.ceil(timeoutMs / 1000));
  const runAttempt = (attemptUrl, attemptPayload) => {
    try {
      const out = execImpl("curl", [
        "-s", "--max-time", String(curlMaxSec),
        `${attemptUrl}/api/embeddings`,
        "-H", "Content-Type: application/json",
        "-d", attemptPayload,
      ], { timeout: curlMaxSec * 1000 + 500, encoding: "utf8", maxBuffer: 8 * 1024 * 1024, windowsHide: true });
      if (!out) return null;
  ...
  const first = runAttempt(url, payload);
  if (first || !laneChosen) return first;
  // Broken-lane degrade: one legacy-MAIN retry with the undecorated body, so a
  // lane that answers /api/tags but cannot embed can never dark the dense arm
  // or trip the embed circuit while :11434 is healthy (scrutiny arm-B P1).
  return runAttempt(DEFAULT_OLLAMA_URL, JSON.stringify(baseBody));
}

// Read the breaker state {consecutiveFailures, lastFailureMs}. A legacy file that
// carries only {lastFailureMs} decodes as exactly ONE prior failure, so an old-shape
// file (written by the pre-threshold 1-strike code) can never by itself keep the
// breaker open under the new >=N rule. Fail-soft: any error -> pristine state.
export function readEmbedCircuit(readFileImpl, existsImpl) {
  try {
    if (!existsImpl(EMBED_CIRCUIT_PATH)) return { consecutiveFailures: 0, lastFailureMs: 0 };
    const j = JSON.parse(readFileImpl(EMBED_CIRCUIT_PATH, "utf8"));
    const lastFailureMs = Number(j && j.lastFailureMs);
    let consecutiveFailures = Number(j && j.consecutiveFailures);
    if (!Number.isFinite(consecutiveFailures)) {
      // legacy {lastFailureMs}-only shape -> treat as a single prior failure
      consecutiveFailures = Number.isFinite(lastFailureMs) && lastFailureMs > 0 ? 1 : 0;
    }
    return {
      consecutiveFailures: Math.max(0, consecutiveFailures),
      lastFailureMs: Number.isFinite(lastFailureMs) ? lastFailureMs : 0,
    };
  } catch { return { consecutiveFailures: 0, lastFailureMs: 0 }; }
}

// Breaker is OPEN only when >= threshold CONSECUTIVE failures have accrued AND the
// most recent one is still inside the cooldown window. One isolated blip (count 1)
// never opens it -> a single transient no longer darkens dense recall fleet-wide.
export function embedCircuitTripped(readFileImpl, existsImpl, now, threshold = embedCircuitFailureThreshold()) {
  const { consecutiveFailures, lastFailureMs } = readEmbedCircuit(readFileImpl, existsImpl);
  if (consecutiveFailures < threshold) return false;
  return (now - lastFailureMs) < EMBED_CIRCUIT_COOLDOWN_MS;
}

// Record ONE embed failure. Increments the consecutive counter when the previous
// failure is still inside the cooldown window (an unbroken streak); otherwise the
// streak had already lapsed, so this starts a fresh count at 1. Fail-soft.
export function recordEmbedFailure(readFileImpl, writeFileImpl, existsImpl, now) {
  try {
    const prev = readEmbedCircuit(readFileImpl, existsImpl);
    const streakLive = prev.lastFailureMs > 0 && (now - prev.lastFailureMs) < EMBED_CIRCUIT_COOLDOWN_MS;
    const consecutiveFailures = (streakLive ? prev.consecutiveFailures : 0) + 1;
    writeFileImpl(EMBED_CIRCUIT_PATH, JSON.stringify({ consecutiveFailures, lastFailureMs: now }), "utf8");
  } catch { /* fail-soft */ }
}

// A successful embed breaks the failure streak -> remove the file (reset count to 0).
export function clearEmbedCircuit(existsImpl, unlinkImpl) {
...
export function tryHybridFuse({ query, bm25Ranked, byKey, opts = {} }) {
  ...
  if (!qvec || qvec.length === 0) { recordEmbedFailure(readFileImpl, writeFileImpl, existsImpl, now); return null; }
