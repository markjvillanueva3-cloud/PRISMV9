...
export function salientSlice(body) {
  ...
  // Strip a leading UTF-8 BOM (U+FEFF): some editors prepend it, and a leading BOM makes
  // `startsWith("---\n")` (frontmatter) and the `# ` title scan both silently miss -> an empty
  // salient slice -> the memo is silently excluded from the semantic index. (Found via 2 vault
  // memos whose only defect was a BOM; a BOM-prefixed REAL memo would vanish from recall.)
  let rest = body.charCodeAt(0) === 0xFEFF ? body.slice(1) : body;
  let description = "";
  if (rest.startsWith("---\n")) {
...
export async function embedText(text, opts = {}) {
  ...
  const attempt = async (url, laneFlag) => {
    const body = laneFlag ? withLaneOptions({ model, prompt: text }) : { model, prompt: text };
  ...
  // (:11435, sidecar-cached 600ms probe) so recall embeds survive fleet
  // inference load on :11434; lane requests carry the num_gpu:0 CPU pin, and a
  // FAILED lane attempt retries MAIN once (a broken lane degrades to legacy,
  // never blackholes recall). Worst case is the rare broken-lane window:
  // 600ms probe + 2x timeoutMs. Explicit opts.url (tests, special callers)
  // keeps single-URL legacy behavior untouched.
  if (opts.url) return attempt(opts.url, false);
  const lane = await resolveEmbedUrl({ timeoutMs: 600 });
  return withMainFallback(lane, attempt);
}
...
export async function embedTextBatch(texts, opts = {}) {
  ...
  const attempt = async (url, laneFlag) => {
    const body = laneFlag ? withLaneOptions({ model, input }) : { model, input };
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), timeoutMs);
    try {
  ...
  // U-INDIA-EMBED-LANE: same lane-first resolution + broken-lane MAIN fallback
  // as embedText above (explicit opts.url = untouched legacy single-URL path).
  if (opts.url) return attempt(opts.url, false);
  const lane = await resolveEmbedUrl({ timeoutMs: 600 });
  return withMainFallback(lane, attempt);
}
