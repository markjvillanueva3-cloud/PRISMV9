...
test("salientSlice strips a leading UTF-8 BOM so a BOM-prefixed memo is NOT silently dropped from the index", () => {
  // A leading BOM (U+FEFF) used to break startsWith('---\n') + the '# ' title scan -> empty salient
  // -> the memo silently excluded from semantic recall. Found via 2 real vault memos (d2 smoke stubs).
  const bomTitle = "﻿# BOM smoke memo body";
  assert.equal(salientSlice(bomTitle), "BOM smoke memo body", "BOM-prefixed title-only memo yields its title, not empty");
  const bomFm = "﻿---\ndescription: real thing\n---\n# Title\n\nOpening paragraph here.";
  const s = salientSlice(bomFm);
  assert.ok(s.includes("real thing"), "BOM before frontmatter still parses the description");
  assert.ok(s.includes("Title"), "and the title");
});
