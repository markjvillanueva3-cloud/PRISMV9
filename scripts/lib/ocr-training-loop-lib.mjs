 ...
 * @returns {Set<string>}
 */
export function parseCursorDoneSet(text) {
  const done = new Set();
...
export function partitionByResumeCursor(worklist, done) {
  ...
  const skipped = [];
  const seen = new Set();
  let skippedDone = 0, skippedNullKey = 0;
  for (const item of list) {
    const k = printCursorKey(item);
    if (!k) { skippedNullKey++; continue; }
    if (seen.has(k)) { skippedDone++; skipped.push(item); continue; } // duplicate within worklist
    seen.add(k);
    if (doneSet.has(k)) { skippedDone++; skipped.push(item); continue; }
    todo.push(item);
  }
  return { todo, skipped, skippedDone, skippedNullKey };
}
