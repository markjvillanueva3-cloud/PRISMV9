...
describe("summarizeSweepEligibility (U-ZULU-SWEEP-HEARTBEAT)", () => {
  const optedLive = (pid) => ({ zuluOptIn: true, pid });
  const docOf = (slots) => ({ slots });

  it("counts eligible slots and reconciles EXACTLY with pickActionableSlots", () => {
    const doc = docOf({ alpha: optedLive(111), bravo: optedLive(222), zulu: optedLive(999) });
    ...
    assert.equal(elig.reason, "eligible-slots-present");
    // the invariant: the diagnostic eligible count must equal the real pick count
    assert.equal(elig.eligible, pickActionableSlots(doc, {}).length);
  });

  it("THE bug: all opted-in but transient pid missing -> all-opted-in-slots-missing-live-pid", () => {
    const doc = docOf({ alpha: { zuluOptIn: true, pid: null }, bravo: { zuluOptIn: true, pid: 0 }, charlie: { zuluOptIn: true } });
    const elig = summarizeSweepEligibility(doc, {});
    assert.equal(elig.eligible, 0);
    assert.equal(elig.missingPid, 3);
    assert.equal(elig.optedIn, 3);
    assert.equal(elig.reason, "all-opted-in-slots-missing-live-pid");
    // reconciles: a zero-eligible sweep is exactly what the live log freeze showed
    assert.equal(pickActionableSlots(doc, {}).length, 0);
  });

  it("nobody opted in -> no-slots-opted-in", () => {
    const doc = docOf({ alpha: { zuluOptIn: false, pid: 111 }, bravo: { pid: 222 } });
    const elig = summarizeSweepEligibility(doc, {});
    assert.equal(elig.eligible, 0);
    assert.equal(elig.optedIn, 0);
    assert.equal(elig.notOptedIn, 2);
    assert.equal(elig.reason, "no-slots-opted-in");
  });

  it("empty / malformed doc -> no-slots, never throws", () => {
    assert.equal(summarizeSweepEligibility({ slots: {} }, {}).reason, "no-slots");
    assert.equal(summarizeSweepEligibility(null, {}).reason, "no-slots");
    assert.equal(summarizeSweepEligibility({}, {}).reason, "no-slots");
  });

  it("--slot selfSlot is excluded like the self-exempt set", () => {
    const doc = docOf({ alpha: optedLive(111), bravo: optedLive(222) });
    const elig = summarizeSweepEligibility(doc, { selfSlot: "alpha" });
    assert.equal(elig.eligible, 1); // bravo only
    assert.equal(elig.selfExempt, 1); // alpha excluded as selfSlot
    assert.equal(elig.eligible, pickActionableSlots(doc, { selfSlot: "alpha" }).length);
  });

  it("mixed: one live + one pid-less -> partial, no-eligible reason only when zero", () => {
    const doc = docOf({ alpha: optedLive(111), bravo: { zuluOptIn: true, pid: null } });
    const elig = summarizeSweepEligibility(doc, {});
    assert.equal(elig.eligible, 1);
    assert.equal(elig.missingPid, 1);
    assert.equal(elig.reason, "eligible-slots-present"); // >=1 eligible -> not an idle reason
  });
});
