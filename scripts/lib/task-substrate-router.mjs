...
export function routeTask(taskType, phase, ctx = {}) {
  ...
  const substrates = [
    ...
    {
      name: "master-graph",
      when: "BEFORE every grep/glob/agent: where is X / is it built / wired / blast-radius",
      how: "MCP UP: prism_session master_index_query. MCP DOWN: `node scripts/system-viz-query.mjs find <q> --json` | `node-card <id>` | `blast-radius <id>`",
