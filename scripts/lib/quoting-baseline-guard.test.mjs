...
test("real baseline-records.json: degenerate => refuse, clean => ok (invariant)", () => {
  ...
  // degeneracy invariant -- a constant-revenue OR machine-heavy OR fixture-heavy baseline MUST refuse
  const constRev = res.degeneracy.revenueUniqueValues <= 1;
  const machineHeavy = res.degeneracy.machineNameShare > 0.2;
  const fixtureHeavy = res.degeneracy.fixtureNameShare > 0.2;
  if (recs.length >= 5 && (constRev || machineHeavy || fixtureHeavy)) {
    assert.equal(res.refuse, true, "degenerate production baseline must be refused");
  }
});

// ---------------------------------------------------------------------------
// U-CLT-BASELINE-GUARD-DEAD (2026-07-02): fixture-name customers -- the SECOND
// poisoning vocabulary. The builder-token detector was blind to the June-12
// regenerated baseline ("MACHINE MODELS FOR LEARNING ENGINE AND SIMULATION" 15/75,
// "CLAUDE - 3D MODELS" 4/75) -> guard said ok:true -> a 241%-MAPE factor was
// ACTIVATED from n=30 live on 2026-07-02 instead of the 47,905-record corpus.
// ---------------------------------------------------------------------------

test("fixture-name TP: the exact 2026-07-02 contamination names flag", () => {
  for (const n of [
    "MACHINE MODELS FOR LEARNING ENGINE AND SIMULATION",
    "CLAUDE -  3D MODELS",
    "CLAUDE - PDF PRINTS",
    "Learning Engine Simulation Batch",
  ]) {
    assert.equal(isFixtureNameCustomer(n), true, `${n} must flag as fixture`);
    assert.equal(isMachineNameCustomer(n), false, `${n} is invisible to the builder detector (why the guard was dead)`);
  }
});

test("fixture-name TN: real customer names NEVER flag (conservative double-gate)", () => {
  for (const n of [
    "Claude Fasteners Inc",          // a person's name alone is never fixture
    "XYZ Simulation Inc",            // simulation without a second fixture token
    "Machine Shop LLC",              // machine without models
    "General Bandages",              // odd but real-shaped -- name filter must not judge plausibility
    "Elite Fasteners",
    "Birmingham Fastener Mfg",
    "Engine Components International", // engine without learning
    "3D Model Makers Co",            // singular model, no claude/machine token
  ]) {
    assert.equal(isFixtureNameCustomer(n), false, `${n} must NOT flag`);
  }
});

test("fixture_name_customers flag: 27% fixture share refuses the whole baseline (the live 2026-07-02 shape)", () => {
  const recs = [];
  for (let i = 0; i < 15; i++) recs.push({ customer: "MACHINE MODELS FOR LEARNING ENGINE AND SIMULATION", actual_revenue_usd: 100 + i });
  for (let i = 0; i < 4; i++) recs.push({ customer: "CLAUDE -  3D MODELS", actual_revenue_usd: 300 + i });
  recs.push({ customer: "CLAUDE - PDF PRINTS", actual_revenue_usd: 555 });
  // 55 real-shaped rows with varied revenue + varied customers (defeats every OTHER flag)
  for (let i = 0; i < 55; i++) recs.push({ customer: `Real Customer ${i}`, actual_revenue_usd: 700 + i * 13 });
  const res = validateBaseline(recs);
  assert.equal(res.refuse, true, "fixture-heavy baseline must refuse");
  assert.ok(res.reasons.some((r) => r.startsWith("fixture_name_customers")), "refusal names the fixture flag");
  assert.equal(res.degeneracy.fixtureNameCount, 20);
  assert.equal(res.poisoned, 20, "poisoned now counts machine + fixture names");
  assert.equal(res.clean_records.length, 55, "clean_records excludes fixture rows");
});

test("fixture share UNDER the 20% threshold does not refuse (share framework, not per-record)", () => {
  const recs = [];
  for (let i = 0; i < 2; i++) recs.push({ customer: "CLAUDE -  3D MODELS", actual_revenue_usd: 100 + i });
  for (let i = 0; i < 18; i++) recs.push({ customer: `Real Customer ${i}`, actual_revenue_usd: 500 + i * 7 });
  const res = validateBaseline(recs);
  assert.equal(res.reasons.some((r) => r.startsWith("fixture_name_customers")), false);
});

test("real baseline-records.json: the live 2026-07-02 contamination is refused NOW (regression oracle)", () => {
  let payload = null;
  try {
    payload = JSON.parse(readFileSync("H:/prism/state/shared/quoting/baseline-records.json", "utf8"));
  } catch {
    return; // absent on this host -- skip
  }
  const recs = Array.isArray(payload.records) ? payload.records : [];
  if (recs.length === 0) return;
  const deg = detectDegeneracy(recs);
  // Oracle is CONDITIONAL so it survives a future cleaned regeneration: whenever the file
  // carries a >20% fixture share (as it does today: 20/75), the guard MUST refuse it.
  if (recs.length >= 5 && deg.fixtureNameShare > 0.2) {
    const res = validateBaseline(recs);
    assert.equal(res.refuse, true, "the fixture-contaminated production baseline must be refused");
    assert.ok(res.reasons.some((r) => r.startsWith("fixture_name_customers")));
  }
});
