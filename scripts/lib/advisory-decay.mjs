...
// ---- cross-bucket take-rate (OBSERVABILITY ONLY; decayDecision is NOT changed) ----
// U-ADVISORY-DECAY-XBUCKET (2026-06-24, slot:alpha): a PURE-ADVISORY hook only
// SUGGESTED++; the conversion (the user taking the nudge) is recorded by the EXECUTION
// it drives, in a DIFFERENT byHook bucket. classify() reads the advisory's OWN bucket --
// whose `offloaded` is always 0 -- so it reports take-rate 0. The TRUE conversion is
// `conversionBucket.offloaded / advisoryBucket.suggested`. This map + crossBucketTakeRate
// surface that real number for decayReport observability. decayDecision/classify are
// DELIBERATELY untouched here: changing the mute decision would re-judge every wired hook
// and needs its own gated re-validation unit. (For large-read-digest both readings agree
// "noise" today -- 0% own-bucket and ~1.4% cross-bucket both < the 5% threshold -- so this
// is a measurement fix, not yet a decision change.)
export const CONVERSION_BUCKET_MAP = Object.freeze({
  "large-read-digest-advisory": "ollama-file-digest", // suggests `node scripts/ollama-file-digest.mjs`
  "nav-rerank-advisory": "ollama-nav-rerank",          // suggests `node scripts/ollama-nav-rerank.mjs`
  "ollama-nav-enforce-inject": "ollama-prism-bridge",  // suggests the ollama-prism-bridge nav lever
  // wiki-read-offload-advisory suggests /route-to-obsidian, whose conversion bucket is not
  // yet instrumented -> intentionally omitted (an absent mapping yields takeRate:null =
  // UNMEASURED, never a manufactured 0 -- the false-mute direction this lib avoids).
});

/**
 * crossBucketTakeRate -- PURE. The TRUE conversion of a (pure-)advisory hook, read from
 * the EXECUTION bucket its suggestion drives via CONVERSION_BUCKET_MAP, not the advisory's
 * own always-0 `offloaded`. Never throws, never mutates, never feeds decayDecision.
 * @param {object} stats       parsed offload-stats ({byHook:{...}})
 * @param {string} advisoryKey the advisory hook's byHook key
 * @param {object} map         advisory->conversion bucket map (default CONVERSION_BUCKET_MAP)
 * @returns {{injected:number, taken:number|null, takeRate:number|null,
 *            conversionKey:string|null, status:'measured'|'unmeasured'}}
 */
export function crossBucketTakeRate(stats, advisoryKey, map = CONVERSION_BUCKET_MAP) {
  const byHook = stats && typeof stats === "object" && stats.byHook && typeof stats.byHook === "object" ? stats.byHook : {};
  const advStat = byHook[advisoryKey];
  const injected = advStat && typeof advStat === "object" ? Math.max(0, Number(advStat.suggested) || 0) : 0;
  const conversionKey = (map && map[advisoryKey]) || null;
  const convStat = conversionKey ? byHook[conversionKey] : null;
  // A FINITE NUMBER offloaded is the only valid taken-signal (mirrors classify's guard:
  // present-but-junk must stay UNMEASURED, never coerce to a false 0).
  const hasConv = convStat && typeof convStat === "object" && typeof convStat.offloaded === "number" && Number.isFinite(convStat.offloaded);
  const taken = hasConv ? Math.max(0, convStat.offloaded) : null;
  if (!conversionKey || !hasConv || injected <= 0) {
    return { injected, taken, takeRate: null, conversionKey, status: "unmeasured" };
  }
  return { injected, taken, takeRate: taken / injected, conversionKey, status: "measured" };
}
...
export function decayDecision(hookKey, {
  ...
  // confirmed noise by the OWN bucket. But a PURE-ADVISORY hook's own offloaded is
  // ALWAYS 0 (it only suggests); the real conversion lands in the EXECUTION bucket it
  // drives (CONVERSION_BUCKET_MAP). If THAT cross-bucket take-rate is measurably at/above
  // the bar, the advisory is NOT noise -> FIRE (un-mute). FAIL-SAFE: this can ONLY lift a
  // mute, never add one -- it lives in the noise branch (an own-bucket-healthy hook
  // already early-returned above and is never demoted), and a non-mapped hook ->
  // crossBucketTakeRate=unmeasured -> falls through to mute exactly as before.
  const x = crossBucketTakeRate(stats, hookKey);
  if (x.status === "measured" && x.takeRate >= opts.maxTakeRate) {
    return { fire: true, muted: false, probe: false, status: "healthy-xbucket", takeRate: c.takeRate, crossBucketTakeRate: x.takeRate, reason: "cross-bucket-converts" };
  }
  // genuinely noise (own AND cross both below the bar, or no cross signal) -> mute, but
  // keep an epsilon probe alive for self-revival.
  ...
  return {
    ...
    status: "noise",
    takeRate: c.takeRate,
    crossBucketTakeRate: x.takeRate,
...
export function decayReport({ statsPath = DEFAULT_STATS_PATH, readImpl = null, env = process.env, ...rawOpts } = {}) {
  ...
  return Object.entries(stats.byHook)
    .map(([hookKey, stat]) => {
      const c = classify(stat, opts);
      const x = crossBucketTakeRate(stats, hookKey);
      // ADDITIVE observability fields: the cross-bucket (true) take-rate + which execution
      // bucket it was read from. decayDecision/muting are unchanged -- existing consumers
      // that read takeRate/status are unaffected.
      return { hookKey, injected: c.injected, taken: c.taken, takeRate: c.takeRate, status: c.status, crossBucketTakeRate: x.takeRate, crossBucketKey: x.conversionKey };
    })
