...
test("buildShippedIdsUnion (production) — REGRESSION: U-BRIDGE-SFC-ESPRIT recovered from real repo", () => {
  // The literal 2026-05-17 bug: bridge units have no envelope so sources
  // (a)+(b) miss them; the picker re-served U-BRIDGE-SFC-ESPRIT after it
  // shipped in 76dc1b53cb. Source (c) must close this in the real repo.
  _resetShippedUnionCache();
  const u = buildShippedIdsUnion();
  assert.ok(
    u.has("U-BRIDGE-SFC-ESPRIT"),
    "U-BRIDGE-SFC-ESPRIT must be shipped via bridge-commit recovery (commit 76dc1b53cb)",
  );
  assert.ok(u.has("U-BRIDGE-SFC-SOLIDCAM"), "compound sibling recovered too");
});
