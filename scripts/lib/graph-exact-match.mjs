...
/**
 * Render an inline CARD block for a single high-confidence (non-exact) hit.
 * Shared home for the GAP-A inline card so every BM25 graph-inject surface
 * (pre-grep / pre-read / pre-write / pre-bash) can clone-by-import instead of
 * copy. Uses the hit's own fields (id, label, layer, status, info) plus
 * seekDocs for vault doc pointers. Returns null when the hit is falsy or has
 * no usable id/label -- the caller then falls back to names-only. Never throws
 * (vaultPathsLine is internally fail-soft).
 * @param {object} hit  master-index hit with id/label/layer/status/info/score
 * @param {((id:string)=>({wiki?:string[],mem?:string[]}|null))|undefined} seekDocs
 * @returns {string|null}
 */
export function renderTopCardBlock(hit, seekDocs) {
  if (!hit) return null;
  const nodeId = hit.id || hit.label;
  if (!nodeId) return null;
  const layer = hit.layer ? `[${hit.layer}/${hit.status || "?"}]` : "[?]";
  const label = hit.label || hit.id || "?";
  const info = (hit.info || "").slice(0, MAX_INFO);
  const score = typeof hit.score === "number" ? hit.score.toFixed(1) : "?";
  // vaultPathsLine is fail-soft: returns "" when seekDocs absent/throws/no docs.
  const docLine = vaultPathsLine(seekDocs, hit);
  const parts = [
    `  [card] ${layer} ${label} (score: ${score})`,
  ];
  if (info) parts.push(`    info: ${info}`);
  if (docLine) parts.push(docLine);
  return parts.join("\n");
}
