...
// Orphaned search/filter coreutils (grep/rg/find/...) left by DEAD chats. A chat
// runs `grep -rln X | head`; if its shell dies, the search keeps running with its
// stdout pointed at a dead pipe -- pure waste (observed 2026-06-14: grep/tail/find
// orphaned 22min-12.6h by dead chats, NOT covered by findStuckBashes/findFsmonitorOrphans).
export const DEFAULT_SEARCHTOOL_AGE_SEC = 300;     // 5 min: parent dead 5min => output orphaned
const MIN_SEARCHTOOL_AGE_SEC = 120;                // < 2 min risks a legit long recursive grep
const MAX_SEARCHTOOL_AGE_SEC = 86400;              // 24 h
const DEFAULT_SEARCHTOOL_NAMES = new Set([
  "grep.exe", "rg.exe", "find.exe", "head.exe", "tail.exe",
  "sort.exe", "wc.exe", "sed.exe", "awk.exe", "uniq.exe", "cut.exe",
]);
...
/**
 * Find orphaned search/filter coreutils (grep/rg/find/head/tail/...) left by DEAD
 * chats. Mirrors findFsmonitorOrphans: dead-parent AND age (a LIVE parent means an
 * active in-flight search -- always spared). Never bash (findStuckBashes owns that).
 * Protected-pid + missing-createdMs skips identical to the siblings. (2026-06-14:
 * 4 grep.exe + a 12.6h tail orphaned by dead chats slipped past every existing hunter.)
 *
 * @param {Array} procs -- normalized [{pid, ppid, name, cmd, createdMs, rssBytes}]
 * @param {Set<number>|null} livePidSet
 * @param {{now?: number, ageSec?: number, protectedPids?: Set<number>, searchToolNames?: Set<string>}} opts
 * @returns {Array<{pid, ppid, ageSec, rssBytes, reason, cmd}>}
 */
export function findOrphanedSearchTools(procs, livePidSet, opts = {}) {
  if (!Array.isArray(procs)) return [];
  const now = Number.isFinite(opts.now) ? opts.now : Date.now();
  const threshold = clamp(
    opts.ageSec ?? DEFAULT_SEARCHTOOL_AGE_SEC,
    MIN_SEARCHTOOL_AGE_SEC, MAX_SEARCHTOOL_AGE_SEC, DEFAULT_SEARCHTOOL_AGE_SEC,
  );
  const names = opts.searchToolNames instanceof Set ? opts.searchToolNames : DEFAULT_SEARCHTOOL_NAMES;
  const protectedPids = opts.protectedPids;
  const out = [];
  for (const p of procs) {
    if (!p || typeof p.name !== "string") continue;
    ...
    if (isProtected(p.pid, protectedPids)) continue;
    // A LIVE parent means an active in-flight search -- never reap it.
    if (isLive(p.ppid, livePidSet)) continue;
    const age = ageSec(p, now);
    if (age == null || age < threshold) continue;
    out.push({
      pid: p.pid, ppid: p.ppid, ageSec: age,
      rssBytes: p.rssBytes || 0,
      reason: `orphan-search-tool ${p.name.toLowerCase()} (age=${age}s, parent ${p.ppid} dead)`,
      cmd: typeof p.cmd === "string" ? p.cmd.slice(0, 200) : "",
    });
  }
  return out;
...
export function runStuckHunters({
  ...
  const fsmonitorOrphans = enableFsmonitor
    ...
    : [];
  const searchToolOrphans = enableSearchTool
    ? findOrphanedSearchTools(procs, livePidSet, { now, ageSec: searchToolAgeSec, protectedPids })
    : [];
  const staleSlotEntries = enableStaleSlot
    ? findStaleSlotPidEntries(slotsData, livePidSet)
    : [];
  return { stuckBashes, fsmonitorOrphans, searchToolOrphans, staleSlotEntries };
}
