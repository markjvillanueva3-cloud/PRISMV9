...
import { cagKey, corpusFingerprint, putCached, saveCache, loadCache, readCagStats, summarizeCagStats } from "./galaxy-cag-cache.mjs";
...
test("reasonForGalaxy: NEVER throws -- bad galaxy still resolves to a result object", async () => {
  ...
  // Hermetic: temp cagFile + cagStatsFile so this test never writes the REAL cache / CAG
  // hit-rate telemetry (pre-existing leak surfaced by U-CAG-HITRATE-TELEMETRY).
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "grb-badgalaxy-"));
  const r = await reasonForGalaxy("zzz-not-a-real-galaxy", "hi", { timeoutMs: 8000, cagFile: path.join(tmp, "cag.json"), cagStatsFile: path.join(tmp, "stats.json") });
...
// ─── U-CAG-HITRATE-HONESTY (slot:alpha 2026-06-15): the miss REASON the bridge records ───
// reasonForGalaxy now records WHY a CAG miss happened (novel vs invalidated) so cag-cache-stats
// can compute a warm hit-rate that cold-start misses no longer drown. The stat is recorded at the
// miss site BEFORE any dense/Ollama call -> these are hermetic (dense:false, no live service).

test("reasonForGalaxy records a NOVEL miss when the key was never cached (cold / new question)", async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "grb-novel-"));
  const gDir = path.join(root, "mcp-server/src/engines/nova");
  fs.mkdirSync(gDir, { recursive: true });
  fs.writeFileSync(path.join(gDir, "MEMORY.md"), "nova galaxy doctrine, no wiki links.");
  const cagStatsFile = path.join(root, "stats.json");
  // Empty cache + dense:false + includeWiki:false -> the key is absent -> NOVEL.
  await reasonForGalaxy("nova", "what is nova?", {
    root, model: "test-model", cagFile: path.join(root, "cag.json"),
    cagStatsFile, dense: false, includeWiki: false, timeoutMs: 4000,
  });
  const sum = summarizeCagStats(readCagStats(cagStatsFile));
  assert.equal(sum.byGalaxy.nova.missReasons.novel, 1, "absent key -> NOVEL miss");
  assert.equal(sum.byGalaxy.nova.missReasons.invalidated, 0, "absent key must NOT be classed invalidated");
  fs.rmSync(root, { recursive: true, force: true });
});

test("reasonForGalaxy records an INVALIDATED miss when the key exists but the fingerprint changed", async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "grb-inval-"));
  const gDir = path.join(root, "mcp-server/src/engines/vera");
  fs.mkdirSync(gDir, { recursive: true });
  fs.writeFileSync(path.join(gDir, "MEMORY.md"), "vera galaxy doctrine, no wiki links.");
  const cagFile = path.join(root, "cag.json");
  const cagStatsFile = path.join(root, "stats.json");
  const model = "test-model";
  const query = "what is vera?";
  // Seed the EXACT key (dense:false/wiki:false -> cacheModel === model) but with a STALE
  // corpusHash -> getCached returns null (fingerprint mismatch) while the raw key IS present ->
  // the bridge must classify this miss as INVALIDATED (the recoverable miss type).
  const key = cagKey("vera", model, query);
  saveCache(cagFile, putCached(loadCache(cagFile), key, { answer: "STALE", sources: ["seed"], corpusHash: "deadbeefdeadbeef", ts: 2 }, {}));
  await reasonForGalaxy("vera", query, { root, model, cagFile, cagStatsFile, dense: false, includeWiki: false, timeoutMs: 4000 });
  const sum = summarizeCagStats(readCagStats(cagStatsFile));
  assert.equal(sum.byGalaxy.vera.missReasons.invalidated, 1, "stale key (fingerprint changed) -> INVALIDATED miss");
  assert.equal(sum.byGalaxy.vera.missReasons.novel, 0, "a present-but-stale key must NOT be classed novel");
  fs.rmSync(root, { recursive: true, force: true });
});
