...
// U-DELTA-CADGEN-VERTEXBBOX-FIX (slot:delta 2026-07-01): a surface's AXIS2_PLACEMENT_3D origin
// CARTESIAN_POINT can sit OFF the solid (live repro: a CadQuery `.cutThruAll()` side-wall PLANE origin at
// Z=-76.2mm on a dimensionally-EXACT 101.6x76.2x25.4mm die plate -> naive full-scan envelope corrupted to
// [101.6,101.6,76.2], a FALSE "inaccurate" self-check verdict on a correct part). The fix scans only
// VERTEX_POINT-referenced points (true solid corners) when present, falling back to the full scan when a
// file has no VERTEX_POINTs. The contamination tests here FAIL pre-fix (off-solid origin inflated the envelope).
describe("vertex-only envelope (U-DELTA-CADGEN-VERTEXBBOX-FIX regression)", () => {
  // id-tagged point + a VERTEX_POINT referencing it (real STEP shape: #22 = VERTEX_POINT('',#23))
  const idPt = (id, x, y, z) => `#${id} = CARTESIAN_POINT ( 'NONE', ( ${x}, ${y}, ${z} ) )`;
  const vtx = (vid, pid) => `#${vid} = VERTEX_POINT ( 'NONE', #${pid} )`;
  it("off-solid placement origin does NOT inflate the envelope (die-plate cutThruAll repro)", () => {
    // solid corners: 101.6 x 76.2 x 25.4 mm box (the CORRECT envelope) ...
    const solid = [idPt(23, -50.8, -38.1, 0), vtx(22, 23), idPt(25, 50.8, 38.1, 25.4), vtx(24, 25)];
    // ... plus a cut side-wall PLANE placement origin FAR below the solid (NOT vertex-referenced)
    const offSolid = "#34 = CARTESIAN_POINT ( 'NONE', ( 19.0119, -9.4869, -76.2000002 ) )\n#33 = PLANE ( 'NONE', #34 )";
    const b = extractBboxMm(MM_HEADER + "\n" + solid.join("\n") + "\n" + offSolid);
    assert.deepEqual(b.dims, [101.6, 76.2, 25.4], "envelope from solid corners ONLY (pre-fix: [101.6,101.6,76.2] -- Z contaminated to 101.6)");
    assert.equal(b.pointCount, 2, "only the 2 vertex-referenced points counted");
  });
  it("no VERTEX_POINTs at all -> full-scan fallback preserved (older/minimal STEP)", () => {
    const b = extractBboxMm(MM_HEADER + "\n" + pt(0, 0, 0) + "\n" + pt(30, 20, 10));
    assert.deepEqual(b.dims, [30, 20, 10], "fallback identical to the historical behavior");
  });
  it("fewer than 2 vertex-referenced points -> falls back to full scan (never null on a valid file)", () => {
    // one vertex-referenced point + two bare points: vertex pass has n=1 (<2) -> full scan must win
    const txt = MM_HEADER + "\n" + idPt(23, 0, 0, 0) + "\n" + vtx(22, 23) + "\n" + pt(40, 20, 10) + "\n" + pt(0, 0, 0);
    const b = extractBboxMm(txt);
    assert.ok(b, "must not be null");
    assert.deepEqual(b.dims, [40, 20, 10]);
  });
  it("inch units: vertex-only envelope is unit-scaled like the full scan", () => {
    const txt = INCH_HEADER + "\n" + idPt(1, 0, 0, 0) + "\n" + vtx(2, 1) + "\n" + idPt(3, 4, 3, 1) + "\n" + vtx(4, 3)
      + "\n#9 = CARTESIAN_POINT ( 'NONE', ( 99, 99, 99 ) )"; // off-solid, not vertex-referenced
    assert.deepEqual(extractBboxMm(txt).dims, [101.6, 76.2, 25.4], "4x3x1 in -> mm; the 99,99,99 origin excluded");
  });
});
