...
// --- invalidRefBasenameSet (topology-audit exclusion, U-CADGEN-REF-EXCLUDE) -----------------
// The topology census found 2 broken reference parts; listReferenceParts must drop them from ground truth.

test("invalidRefBasenameSet: Windows abs-paths -> lowercased basenames (the 2 real broken refs)", () => {
  const s = invalidRefBasenameSet([
    "H:\\PRISM\\resources\\CAD FILES\\Body 1.step",
    "H:\\PRISM\\resources\\CAD FILES\\Full Part.step",
  ]);
  assert.equal(s.size, 2);
  assert.ok(s.has("body 1.step"), "backslash Windows path -> basename lowercased (POSIX basename() would NOT split \\)");
  assert.ok(s.has("full part.step"));
  assert.ok(!s.has("h:\\prism\\resources\\cad files\\body 1.step"), "stores the basename, not the abs path");
});

test("invalidRefBasenameSet: forward-slash paths + mixed case fold to one lowercase basename", () => {
  const s = invalidRefBasenameSet(["/x/y/BLISK.STP", "resources/CAD FILES/blisk.stp"]);
  assert.ok(s.has("blisk.stp"));
  assert.equal(s.size, 1, "same basename, different case+dir -> a single entry");
});

test("invalidRefBasenameSet: non-array / empty / non-step entries -> empty or filtered (fail-soft)", () => {
  assert.equal(invalidRefBasenameSet(null).size, 0);
  assert.equal(invalidRefBasenameSet(undefined).size, 0);
  assert.equal(invalidRefBasenameSet([]).size, 0);
  assert.equal(invalidRefBasenameSet(["/some/dir", "notes.txt", "readme"]).size, 0, "non-step basenames are dropped");
  assert.equal(invalidRefBasenameSet(["a.step", "b.STP"]).size, 2, ".step + .STP (case-insensitive) both kept");
});
