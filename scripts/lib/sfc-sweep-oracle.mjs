...
/**
 * Per-cell: engine power_kW must equal the PURE CUTTING POWER P = Fc[N]*Vc[m/min]/60000, within a
 * relative band + a rounding floor, UNLESS the MAX_POWER sanity clamp (default 500 kW) binds. Fc and
 * Vc are the engine's OWN reported cutting_force_N / cutting_speed_m_min, so a dropped 60000 unit
 * factor, a rev/min-vs-m/min slip, or power computed from a different force than the one reported all
 * surface as a divergence rather than passing silently. This is the load-bearing FORCE<->POWER<->SPEED
 * accuracy identity (ManufacturingCalculations.ts:391 -> power = min(Fc*cutting_speed/60000, MAX_POWER)).
 *
 * tolPct     - relative band for a genuine formula/unit divergence.
 * maxPowerKw - the SAFETY_LIMITS.MAX_POWER clamp; a reported power AT the clamp whose raw Fc*Vc/60000
 *   would meet/exceed it is a legitimate clamp, not a math bug, so it is forgiven.
 * The absolute rounding floor mirrors checkMrr/RpmConsistency: cutting_force_N is reported to +/-0.5 N
 * and cutting_speed_m_min to +/-0.5 m/min, but power_kW derives from the UNROUNDED Fc/vc, so
 * reconstructing expected power from the rounded fields carries a (0.5*Vc + 0.5*Fc)/60000 error plus
 * the +/-0.005 kW rounding of power_kW itself -- sub-floor divergence is display rounding, not a bug.
 */
export function checkPowerConsistency(params, v, tolPct = 0.03, maxPowerKw = 500) {
  const Fc = v.cutting_force_N, Vc = v.cutting_speed_m_min, P = v.power_kW;
  if (!(Fc > 0 && Vc > 0 && P > 0)) return null;
  const expected = (Fc * Vc) / 60000;
  // MAX_POWER clamp: engine clamped to maxPowerKw and the raw cutting power would meet/exceed it.
  if (P >= maxPowerKw - 1e-6 && expected >= maxPowerKw) return null;
  const absDiff = Math.abs(P - expected);
  const rel = absDiff / expected;
  const absFloor = (0.5 * Vc + 0.5 * Fc) / 60000 + 0.005;
  return rel > tolPct && absDiff > absFloor
    ? { kind: "power_inconsistent", expected, actual: P, relPct: rel, absDiff }
    : null;
}

/**
 * Per-cell: engine torque_Nm must equal Fc[N]*D[mm]/2000 (spindle torque = Fc * radius; radius = D/2 mm
 * = D/2000 m, so Fc*D/2000 gives N*m). ManufacturingCalculations.ts:393 torque = (Fc*tool_diameter)/2000,
 * NOT clamped. A radius-vs-diameter error, a mm-vs-m slip, or a torque decoupled from the reported force
 * surfaces as a divergence. This value FEEDS the spindle-torque safety gate, so a silent torque error is
 * safety-relevant. Rounding floor: cutting_force_N is reported to +/-0.5 N -> torque error 0.5*D/2000,
 * plus torque_Nm's own +/-0.005 N*m rounding; sub-floor divergence is display rounding, not a math bug.
 */
export function checkTorqueConsistency(params, v, tolPct = 0.03) {
  const D = params.tool_diameter, Fc = v.cutting_force_N, T = v.torque_Nm;
  if (!(D > 0 && Fc > 0 && T > 0)) return null;
  const expected = (Fc * D) / 2000;
  const absDiff = Math.abs(T - expected);
  const rel = absDiff / expected;
  const absFloor = (0.5 * D) / 2000 + 0.005;
  return rel > tolPct && absDiff > absFloor
    ? { kind: "torque_inconsistent", expected, actual: T, relPct: rel, absDiff }
    : null;
}
...
export function perCellOracle(params, v) {
  ...
  if (m) out.push(m);
  const p = checkPowerConsistency(params, v);
  if (p) out.push(p);
  const t = checkTorqueConsistency(params, v);
  if (t) out.push(t);
  return out;
}
