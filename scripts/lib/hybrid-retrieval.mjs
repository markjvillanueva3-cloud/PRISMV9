...
function defaultEmbedAttempt(query, sendImpl, baseUrl, model, laneFlag) {
  const body = JSON.stringify(laneFlag ? withLaneOptions({ model, prompt: query }) : { model, prompt: query });
  const r = sendImpl("POST", `${baseUrl}/api/embeddings`, body);
...
// U-INDIA-EMBED-LANE: no explicit opts.ollamaUrl -> prefer the dedicated CPU
// embed lane (:11435, sidecar-cached sync probe) so this hybrid-retrieval
// vector leg survives fleet inference load on :11434. An explicit ollamaUrl
// (an operator/dispatcher-pinned URL) keeps legacy single-URL behavior
// untouched -- note DEFAULT_OLLAMA_URL already carries the `/api/embeddings`
// suffix (legacy shape), so the explicit-url path preserves that exact URL
// while the lane path builds the URL from a bare origin + suffix itself.
export function defaultEmbed(query, opts = {}) {
  ...
  if (opts.ollamaUrl) {
    const r = sendImpl("POST", opts.ollamaUrl, JSON.stringify({ model, prompt: query }));
    if (!r || r.status !== 0) return null;
    try {
      const parsed = JSON.parse(r.stdout || "");
      return Array.isArray(parsed.embedding) && parsed.embedding.length > 0 ? parsed.embedding : null;
    } catch {
      return null;
    }
  }
  const lane = resolveEmbedUrlSync();
  const first = defaultEmbedAttempt(query, sendImpl, lane.url, model, lane.lane);
  if (first != null || !lane.lane) return first;
  // Broken-lane degrade: one legacy-MAIN retry -- MAIN's own /api/embeddings
  // suffix must match DEFAULT_OLLAMA_URL's legacy shape exactly.
  const mainOrigin = DEFAULT_OLLAMA_URL.replace(/\/api\/embeddings$/, "");
  return defaultEmbedAttempt(query, sendImpl, mainOrigin, model, false);
}
