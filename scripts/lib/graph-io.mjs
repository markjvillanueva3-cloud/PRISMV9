 ...
 * missing key / not-an-array (caller treats 0 as "couldn't verify").
 *
...
export function countGraphArrayStreaming(filePath, arrayKey = "nodes") {
  ...
  let count = 0, depth = 0, inStr = false, inElem = false;
  let i = arrStart;
  while (i < buf.length) {
    const c = buf[i];
    if (inStr) {
    ...
    if (c === RBRACKET) {
      if (depth === 0) break; // end of the target array
      depth--; i++; continue;
    }
...
export function streamGraphArray(filePath, arrayKey, onElement) {
  ...
  let i = arrStart, count = 0;
  while (i < buf.length) {
    while (i < buf.length && (isWS(buf[i]) || buf[i] === COMMA)) i++;
    if (i >= buf.length || buf[i] === RBRACKET) break; // end of the target array
    const elemStart = i;
