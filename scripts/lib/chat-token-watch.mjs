...
/** Window for the AUTHORITATIVE-CONTRADICTION guard (U-CHAT-TOKEN-AUTH-CONTRADICTION).
 *  The byte-estimate over-reports (transcript bloat); a byte-est "critical" must not
 *  ACTUATE auto-/compact when a recent per-turn-usage sidecar reading says clean/warn.
 *  6x SIDECAR_TTL_MS (30min): the sidecar refreshes every prompt, so staleness means a
 *  frozen tool-gap where context can't grow green->critical -> a reading this recent is
 *  trustworthy contradiction evidence. Beyond it we stay conservative (byte-est stands).
 *  Knob PRISM_CHAT_TOKEN_CONTRADICTION_DISABLE=1. */
export const CONTRADICTION_TTL_MS = 30 * 60_000;
...
export function readChatPressure(sessionId, opts = {}) {
  ...
  // AUTHORITATIVE-CONTRADICTION guard (U-CHAT-TOKEN-AUTH-CONTRADICTION, slot:alpha
  // 2026-06-21): a byte-estimate "critical" in the plausible band [critAt, 1.1xcap]
  // (i.e. NOT already SUSPECT) must NOT actuate auto-/compact when a recent per-turn
  // AUTHORITATIVE sidecar reading actively says clean/warn. Observed live this session:
  // zulu-advisory emitted "critical ~1004K" while slot-context-bundle (same sidecar)
  // said GREEN -> phantom /compact nudge. The sidecar-first block above already
  // returned if a reading was within SIDECAR_TTL_MS (180s); here we re-read with the
  // wider CONTRADICTION_TTL_MS to consult the LAST-KNOWN authoritative value. Downgrade
  // ONLY on active contradiction (authoritative exists + was clean/warn) -> ZERO
  // missed-critical risk: absent authoritative, or an authoritative-critical, leaves the
  // byte-est "critical" untouched. Safe because a stale sidecar = a frozen tool-gap
  // (context can't grow green->critical without a prompt refreshing the sidecar).
  let contradictedBy = null;
  if (level === "critical" && !suspect && _env.PRISM_CHAT_TOKEN_CONTRADICTION_DISABLE !== "1") {
    const lastAuth = readSidecarPressure(opts.slot, {
      sidecarDir: opts._sidecarDir,
      ttlMs: CONTRADICTION_TTL_MS,
      _io: opts._io,
      _now: opts._now,
    });
    if (lastAuth && (lastAuth.level === "clean" || lastAuth.level === "warn")) {
      level = "warn";
      contradictedBy = lastAuth.level;
    }
  }
  return {
    sessionId, slot: opts.slot || null,
