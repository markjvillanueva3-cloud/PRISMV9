...
export function embedText(text, opts = {}) {
  ...
  // U-INDIA-EMBED-LANE: no explicit opts.url + the REAL defaultRun (curl) in use ->
  // prefer the dedicated CPU embed lane (:11435) so this path-memoization embed
  // survives fleet inference load on :11434. An injected runImpl (tests) keeps the
  // legacy single-URL behavior untouched. A FAILED lane attempt retries MAIN once
  // (a broken lane degrades to today's exact behavior, never blackholes recall).
  if (!opts.url && run === defaultRun) {
    const lane = resolveEmbedUrlSync({ timeoutMs: 800 });
    const first = tryRun(run, { url: lane.url, model, text, timeoutMs, lane: lane.lane });
    if (first != null || !lane.lane) return parseEmbedRaw(first);
    return parseEmbedRaw(tryRun(run, { url: DEFAULT_URL(), model, text, timeoutMs, lane: false }));
  }
  const url = opts.url || DEFAULT_URL();
  return parseEmbedRaw(tryRun(run, { url, model, text, timeoutMs, lane: false }));
}

function tryRun(run, { url, model, text, timeoutMs, lane }) {
  try {
    return run({ url, model, text, timeoutMs, lane });
  } catch { return null; }
}

function parseEmbedRaw(raw) {
  if (!raw) return null;
  try {
...
function defaultRun({ url, model, text, timeoutMs, lane }) {
  // curl subprocess — reliable under localhost contention (vs node fetch/http).
  const sec = Math.max(1, Math.ceil(timeoutMs / 1000));
  const body = lane ? withLaneOptions({ model, prompt: text }) : { model, prompt: text };
  return execFileSync(
    "curl",
    ["-s", "--max-time", String(sec), `${url}/api/embeddings`, "-d", JSON.stringify(body)],
