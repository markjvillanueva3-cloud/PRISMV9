...
// --- detectCrashes: window-liveness gate (U-CRASHWATCH-LIVENESS-GATE) ---------
// A frozen heartbeat ALONE must NOT flag an idle-but-alive chat. When
// opts.deadSlots (Set of `${slot} ${chatId}`) is supplied, a crash ALSO requires
// the slot be confirmed window-pid-dead (heartbeat-crashed AND window-pid-dead).

const frozenCrash = (slot, chatId) => {
  const frozenHb = NOW - DEFAULT_CRASH_STALE_MS - 60_000;
  return {
    prev: mkSnap(NOW - 300_000, { [slot]: { chatId, lastHeartbeatMs: frozenHb } }),
    curr: mkSnap(NOW, { [slot]: { chatId, lastHeartbeatMs: frozenHb } }),
  };
};

test("detectCrashes gate: frozen + CONFIRMED window-pid-dead -> CRASH", () => {
  const { prev, curr } = frozenCrash("alpha", "claude-aa");
  const deadSlots = new Set(["alpha claude-aa"]);
  const r = detectCrashes(prev, curr, NOW, undefined, { deadSlots });
  assert.equal(r.length, 1);
  assert.equal(r[0].slot, "alpha");
});

test("detectCrashes gate: frozen but ALIVE (not in deadSlots) -> NO crash [THE FIX]", () => {
  // Idle-but-alive chat: heartbeat frozen 11min (operator away), window live.
  const { prev, curr } = frozenCrash("oscar", "claude-oo");
  const deadSlots = new Set(); // oscar NOT confirmed dead -> alive
  assert.equal(detectCrashes(prev, curr, NOW, undefined, { deadSlots }).length, 0);
  // Revert-proof: WITHOUT the gate the SAME input IS flagged a crash. If the gate
  // code were removed, the gated call above would return 1 and this test fails.
  assert.equal(detectCrashes(prev, curr, NOW).length, 1);
});

test("detectCrashes gate: deadSlots present but chatId MISMATCH (re-claim) -> NO crash", () => {
  const { prev, curr } = frozenCrash("alpha", "claude-aa");
  const deadSlots = new Set(["alpha claude-OLD"]); // stale chatId, not curr's
  assert.equal(detectCrashes(prev, curr, NOW, undefined, { deadSlots }).length, 0);
});

test("detectCrashes gate: empty deadSlots Set -> everything alive -> NO crash", () => {
  const { prev, curr } = frozenCrash("bravo", "claude-bb");
  assert.equal(detectCrashes(prev, curr, NOW, undefined, { deadSlots: new Set() }).length, 0);
});

test("detectCrashes gate: opts.deadSlots NOT a Set -> ignored (back-compat heartbeat-only)", () => {
  const { prev, curr } = frozenCrash("delta", "claude-dd");
  // array / null / absent are not a Set instance -> gate skipped -> frozen alone crashes
  assert.equal(detectCrashes(prev, curr, NOW, undefined, { deadSlots: ["delta claude-dd"] }).length, 1);
  assert.equal(detectCrashes(prev, curr, NOW, undefined, { deadSlots: null }).length, 1);
  assert.equal(detectCrashes(prev, curr, NOW, undefined, {}).length, 1);
});

test("detectCrashes gate: multi-slot -> only frozen+DEAD flagged, frozen+alive excluded", () => {
  const frozen = NOW - DEFAULT_CRASH_STALE_MS - 30_000;
  const prev = mkSnap(NOW - 300_000, {
    alpha: { chatId: "claude-aa", lastHeartbeatMs: frozen }, // dead
    oscar: { chatId: "claude-oo", lastHeartbeatMs: frozen }, // idle-but-alive
  });
  const curr = mkSnap(NOW, {
    alpha: { chatId: "claude-aa", lastHeartbeatMs: frozen },
    oscar: { chatId: "claude-oo", lastHeartbeatMs: frozen },
  });
  const deadSlots = new Set(["alpha claude-aa"]); // only alpha confirmed dead
  const r = detectCrashes(prev, curr, NOW, undefined, { deadSlots });
  assert.equal(r.length, 1);
  assert.equal(r[0].slot, "alpha");
});
