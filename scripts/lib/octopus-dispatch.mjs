...
export function mapConsensusToLedger(cr) {
  ...
  // Count per-vendor voices so a vendor that fields MULTIPLE distinct models gets each voice
  // tagged with its model id (diagnosability) -- the ollama diverse panel AND the new xai
  // multi-model Hermes panel (OCTOPUS-HERMES-MULTIMODEL). A single-voice cloud vendor stays bare.
  const vendorCounts = new Map();
  for (const r of responses) {
    const v = typeof r?.vendor === "string" ? r.vendor : "unknown";
  ...
  for (const r of responses) {
    ...
    const ok = r?.ok === true;
    voices.push({
      // Tag a voice with its model id when its vendor fields MULTIPLE distinct models so the
      // ledger shows WHICH voices answered (diagnosability). TWO such vendors now exist: the
      // ollama diverse local panel (ALWAYS tagged -- even a single panel voice names its model,
      // preserving the original 1-voice-regression diagnosis) AND the xai multi-model Hermes
      // panel (OCTOPUS-HERMES-MULTIMODEL -- tagged only when 2+ xai voices seat, so a single Grok
      // voice stays bare "xai", back-compat). Other 1:1 vendor:model voices stay bare. The cluster
      // signature is verdict-pattern based (octopus-record-lib buildOctopusEntry), NOT id-based,
      // so this does not affect cross-run comparability.
      id: (vendor === "ollama" || (vendorCounts.get(vendor) || 0) > 1) && model && model !== vendor
        ? `${vendor}:${model}`
        : vendor,
...
/**
 * Free local-only voice set: disable every PAID cloud vendor so the consensus
 * runs entirely on the local Blackwell Ollama panel ($0, no external send).
 * Enables the diverse local panel + dual-ollama so a single-machine run still
 * gets genuine multi-voice independent signal (gpt-oss:120b / gpt-oss:20b /
 * qwen2.5-coder:32b -- install-gated, absent models skipped). This is the
 * PSN-OCTOPUS local-offload mode: it lets the octopus consensus run un-dormant
 * on a cron / autonomous loop for free (operator goal: "octopus utilization +
 * ollama task offloading"). Builds on bravo's includeCodex flag (2026-06-10).
 * Pure -- returns the override object; the caller merges it UNDER any explicit
 * askOverrides so a deliberate cloud voice can still be re-seated by an
 * operator who passes one.
 *
 * @returns {object} ask() overrides that exclude all cloud vendors.
 */
export function localOnlyOverrides() {
  return {
    includeClaude: false,
    includeGrok: false,
    includeGemini: false,
    ...
    // Curated FAST local panel = the proven-seating general+coder pair. Kept in
    // deliberate sync with octopus-first-live-record.mjs `LOCAL_ONLY_PANEL`
    // (clone-don't-fork: that file is the standalone first-record tool, this is
    // the shared cron/loop DISPATCH preset; the value is intentionally the same
    // 2 voices -- consolidating into one SSOT is a follow-up). gpt-oss:20b
    // (general) + qwen2.5-coder:32b (coder) are the 2 distinct-family voices that
    // RELIABLY seat live (the engine's own default panel lists gemma4:31b, absent
    // on this box -> collapsed to 1). INSTALL-GATED: absent -> skipped (back-compat).
    //
    // CRITICAL (scrutiny arm-C P1, verified): the engine runs the Ollama panel
    // SEQUENTIALLY on one GPU (MultiModelConsensusEngine.ts ~672 `for (const
    // model of ollamaVoices) await callOllama`), so total wall time is the SUM of
    // resident voices' times, NOT a parallel ceiling. Big reasoners (120b /
    // deepseek-r1) are deliberately EXCLUDED -- they'd serialize behind the fast
    // pair and blow the cron budget. 2 fast voices bound the run to ~2x timeoutMs.
    diverseLocalModels: ["gpt-oss:20b", "qwen2.5-coder:32b"],
    // forceProbe so a stale 5-min capability-probe cache can't drop a now-resident
    // voice and collapse the panel to 1 (mirrors the sibling first-record tool).
    forceProbe: true,
    // Per-voice ceiling; SEQUENTIAL single-GPU => worst-case TOTAL ~= 2 x this for
    // the 2-voice panel. 90s gives a warm mid-size local model room while keeping
    // the cron/loop path bounded (~180s), not the ~600s a 5-model panel would risk.
    timeoutMs: 90000,
  };
}
 ...
 * @param {boolean} [args.localOnly]     - free local-only mode: disable every cloud vendor so
 *                                         only the local Ollama panel runs ($0). Applied UNDER
 *                                         askOverrides so an explicit cloud voice still wins.
 ...
 */
export async function dispatchOctopus({ prompt, context = "", askOverrides = {}, localOnly = false, engineFactory = null, distUrl = DEFAULT_CONSENSUS_DIST }) {
  ...
  const askInput = {
    ...
    // localOnly defaults sit BELOW the base (so includeClaude:false / persist:false hold)
    // but ABOVE askOverrides (so an explicit operator cloud voice still wins). R7 precedence.
