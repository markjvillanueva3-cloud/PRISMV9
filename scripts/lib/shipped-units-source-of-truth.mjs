...
// Source (e): explicit verified-shipped overrides written by the tango queue
// reconciler (scripts/tango-reconcile-queue.mjs). git+disk-confirmed unit-ids
// the other four sources structurally miss (e.g. a unit shipped under a commit
// subject that does not contain its U-ID, so the bridge/progress scans never see
// it). Same benign failure direction as the bridge source: a false-positive only
// hides a unit from pickup (operator-recoverable), never the reverse.
const DEFAULT_OVERRIDES_PATH = path.join(REPO_ROOT, "state/shared/verified-shipped-overrides.json");
...
/**
 * Source (e): explicit verified-shipped overrides -- unit-ids the tango reconciler
 * confirmed shipped on disk (git commit + asset) that sources (a)-(d) structurally
 * miss. File shape: { ids: ["U-...", ...], _evidence?: {...}, ... } or a bare array.
 * Only U-* shaped ids are honored (same gate as the other sources). Never throws.
 */
export function readVerifiedShippedOverrides(overridesPath = DEFAULT_OVERRIDES_PATH) {
  const out = new Set();
  const j = readJsonSafe(overridesPath);
  if (!j) return out;
  const ids = Array.isArray(j) ? j : (Array.isArray(j.ids) ? j.ids : []);
  for (const id of ids) {
    if (typeof id === "string" && UNIT_ID_RE.test(id.trim())) out.add(norm(id));
  }
  return out;
}
 ...
 * cached copy.
 */
export function buildShippedIdsUnion({ progressPath, envelopesDir, overridesPath } = {}) {
  const useCache = progressPath == null && envelopesDir == null && overridesPath == null;
  ...
  if (useCache) {
    ...
    const overMtime = statMtimeSafe(DEFAULT_OVERRIDES_PATH); // (e) verified-shipped overrides
    ...
    if (_unionCache
        ...
        && _unionCache.overMtime === overMtime
    ...
    for (const id of readVerifiedShippedOverrides()) out.add(id); // (e) git+disk-confirmed
    _unionCache = { progMtime, envMtime, overMtime, headSha, set: new Set(out) };
    return out;
  }
  ...
  const out = new Set(readShippedFromProgress(pp));
  for (const id of readShippedFromEnvelopes(ed)) out.add(id);
  if (overridesPath != null) for (const id of readVerifiedShippedOverrides(overridesPath)) out.add(id);
  return out;
}
