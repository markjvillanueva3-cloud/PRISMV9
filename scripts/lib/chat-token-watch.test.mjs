...
// ─── AUTHORITATIVE-CONTRADICTION guard (U-CHAT-TOKEN-AUTH-CONTRADICTION, alpha 2026-06-21) ───
// A byte-estimate "critical" must NOT actuate auto-/compact when a recent per-turn
// authoritative sidecar reading actively says clean/warn (the observed zulu "critical
// ~1004K" vs authoritative GREEN phantom). Downgrade ONLY on active contradiction ->
// zero missed-critical risk. Sidecar age 5min: STALE for the 180s sidecar-first (so we
// reach the byte path) but FRESH for the 30min contradiction window.
const NOW_AC = 1_700_000_000_000;
const AC_ENV = { PRISM_CHAT_TOKEN_WARN_AT: "100", PRISM_CHAT_TOKEN_CRITICAL_AT: "200" };
function contradictionIo(sidecarDoc) {
  const tbuf = Buffer.alloc(1000, "x".charCodeAt(0)); // ~286 tok -> critical at AC_ENV, far below suspect
  return {
    existsSync: (p) => (String(p).includes("token-budget-") ? sidecarDoc != null : true),
    statSync: () => ({ size: tbuf.length }),
    readFileSync: (p) => (String(p).includes("token-budget-") ? JSON.stringify(sidecarDoc) : tbuf),
  };
}
function acSidecar(zone, ageMs) {
  return { capturedAt: new Date(NOW_AC - ageMs).toISOString(), zone, ctx: { tokens: 400000, pct: 0.4 } };
}

test("CONTRADICTION: byte-est critical + recent authoritative GREEN (<30min) -> downgraded to warn", () => {
  const r = readChatPressure("sid", {
    slot: "alpha", _env: AC_ENV, _now: NOW_AC, _sidecarDir: "/fake",
    _io: contradictionIo(acSidecar("GREEN", 5 * 60_000)),
  });
  assert.equal(r.source, "byte-estimate", "sidecar >180s stale -> byte path");
  assert.equal(r.pressureLevel, "warn", "authoritative GREEN contradicts byte-est critical -> warn");
  assert.equal(r.contradictedBy, "clean");
});

test("CONTRADICTION: byte-est critical + NO authoritative sidecar -> stays critical (zero missed-critical)", () => {
  const r = readChatPressure("sid", {
    slot: "alpha", _env: AC_ENV, _now: NOW_AC, _sidecarDir: "/fake",
    _io: contradictionIo(null),
  });
  assert.equal(r.pressureLevel, "critical");
  assert.equal(r.contradictedBy, undefined);
});

test("CONTRADICTION: byte-est critical + authoritative RED (corroborates) -> stays critical", () => {
  const r = readChatPressure("sid", {
    slot: "alpha", _env: AC_ENV, _now: NOW_AC, _sidecarDir: "/fake",
    _io: contradictionIo(acSidecar("RED", 5 * 60_000)),
  });
  assert.equal(r.pressureLevel, "critical", "authoritative critical does not contradict -> no downgrade");
});

test("CONTRADICTION: byte-est critical + authoritative GREEN but ANCIENT (>30min) -> stays critical (conservative)", () => {
  const r = readChatPressure("sid", {
    slot: "alpha", _env: AC_ENV, _now: NOW_AC, _sidecarDir: "/fake",
    _io: contradictionIo(acSidecar("GREEN", 45 * 60_000)),
  });
  assert.equal(r.pressureLevel, "critical", "no authoritative reading within 30min -> byte-est critical stands");
});

test("CONTRADICTION: knob PRISM_CHAT_TOKEN_CONTRADICTION_DISABLE=1 -> no downgrade", () => {
  const r = readChatPressure("sid", {
    slot: "alpha", _now: NOW_AC, _sidecarDir: "/fake",
    _env: { ...AC_ENV, PRISM_CHAT_TOKEN_CONTRADICTION_DISABLE: "1" },
    _io: contradictionIo(acSidecar("GREEN", 5 * 60_000)),
  });
  assert.equal(r.pressureLevel, "critical", "knob disables the contradiction guard");
});
