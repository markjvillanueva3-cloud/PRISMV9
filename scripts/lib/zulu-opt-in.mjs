...
const PRISM = "H:/prism";
export const DEFAULT_OPTIN_FILE =
  process.env.PRISM_ZEBRA_OPTIN_FILE || `${PRISM}/state/shared/zebra-opt-in.json`;
export const OPTIN_SCHEMA_VERSION = 1;
const LOCK_TIMEOUT_MS = 3000;

// Canonical NATO slot set + the self-exempt set (zebra orchestrates work
...
/** The work slots zebra MAY manage: every canonical slot minus the exempt set. */
...
function cliStatus(file) {
  ...
  const lines = [`zebra-opt-in store: ${file}`];
