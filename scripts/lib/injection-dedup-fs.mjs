...
import { hashBlock, shouldEmit, recordEmit, formatDedupedMarker, pruneTag, DEFAULT_TTL_MS } from "./injection-dedup.mjs";
...
export function dedupeOrMarker(block, opts = {}) {
  ...
  // wrapper backs ~5 awareness/intent hooks, so the fix lands fleet-wide here.
  cache = pruneTag(cache, hookTag, now, ttlMs);
