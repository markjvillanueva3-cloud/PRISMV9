...
/**
 * isFixtureNameCustomer -- TRUE only when a "customer" is a LEARNING/SIMULATION/AI-fixture
 * seed masquerading as a customer (U-CLT-BASELINE-GUARD-DEAD, 2026-07-02). This module's own
 * header warned "the poisoning recurs whenever the source is dirty" -- and it did, with a
 * vocabulary the builder-token detector cannot see: the June-12 regenerated
 * baseline-records.json carries "MACHINE MODELS FOR LEARNING ENGINE AND SIMULATION" (15/75),
 * "CLAUDE - 3D MODELS" (4/75) and "CLAUDE - PDF PRINTS" (1/75) -- zero CNC-builder tokens, so
 * machineNameCount stayed 0, validateBaseline said ok:true, and a 241%-MAPE factor was
 * activated from n=30 (live, 2026-07-02) instead of falling back to the 47,905-record corpus.
 *
 * Same conservatism contract as isMachineNameCustomer (charlie soul:
 * non-conservative-customer-name-filter is REFUSED): every rule is a DOUBLE-token gate so a
 * real company name can never trip it -- "Claude Fasteners Inc" (a person's name, one token)
 * is safe; "CLAUDE - 3D MODELS" (claude + models) flags. Refusal is whole-baseline + loud via
 * detectDegeneracy's share threshold, never a silent per-record drop.
 */
export function isFixtureNameCustomer(name) {
  const tokens = new Set(tokenize(name));
  if (tokens.size === 0) return false;
  const has = (t) => tokens.has(t);
  // machine + models ("MACHINE MODELS FOR LEARNING...") -- a real shop is never both.
  if (has("machine") && has("models")) return true;
  // learning + engine ("...LEARNING ENGINE AND SIMULATION")
  if (has("learning") && has("engine")) return true;
  // simulation only with a second fixture token (a real "XYZ Simulation Inc" stays safe)
  if (has("simulation") && (has("machine") || has("models") || has("learning") || has("engine"))) return true;
  // claude + an artifact-type token ("CLAUDE - 3D MODELS" / "CLAUDE - PDF PRINTS")
  if (has("claude") && (has("models") || has("prints") || has("pdf") || has("3d") || has("simulation"))) return true;
  return false;
}
...
export function detectDegeneracy(records, opts = {}) {
  ...
  // U-CLT-BASELINE-GUARD-DEAD (2026-07-02): learning/simulation/AI-fixture seeds -- the
  // contamination vocabulary the builder-token detector is blind to. Same share-threshold
  // framework as machine names (whole-baseline refusal, never a per-record drop).
  const fixtureNameCount = recs.filter((r) => isFixtureNameCustomer(r?.customer)).length;
  const fixtureNameShare = total ? fixtureNameCount / total : 0;
  ...
  if (total >= minRecords && fixtureNameShare > maxMachineShare) {
    flags.push(
      `fixture_name_customers=${fixtureNameCount}/${total} ` +
        `(${(fixtureNameShare * 100).toFixed(0)}% > ${(maxMachineShare * 100).toFixed(0)}% threshold) ` +
        `-- learning/simulation/AI-fixture seeds as customers (e.g. "MACHINE MODELS FOR ` +
        `LEARNING ENGINE AND SIMULATION", "CLAUDE - 3D MODELS")`,
    );
  }
...
export function validateBaseline(records, opts = {}) {
  ...
  const cleanRecords = recs.filter((r) => !isMachineNameCustomer(r?.customer) && !isFixtureNameCustomer(r?.customer));
  ...
  return {
    ...
    total: deg.total,
    poisoned: deg.machineNameCount + deg.fixtureNameCount,
