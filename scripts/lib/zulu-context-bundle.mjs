...
// High-YELLOW gate: only recommend a proactive /compact past the MIDPOINT of the
// 25-65% YELLOW band, not at 25% where a reset would be wasteful (scrutiny arm C P2a:
// the producer emits action:"wrap-up" for ALL of YELLOW, so the consumer gates on pct).
const YELLOW_COMPACT_PCT = 0.5;
...
export function deriveZebraDecision({ soul, loop, tokenZone, bridgeUnits } = {}) {
  ...
  if (tokenZone && tokenZone.ok === true) {
    ...
    } else if (tokenZone.zone === "YELLOW") {
      // YELLOW (25-65%) is the PRUDENT proactive-compaction band -- the whole point
      // of model-initiated /compact is to act HERE, before the late RED/native-~95%
      // threshold. The "corroborating signal" the doc above promised is the
      // token-awareness writer's OWN `action` field: "wrap-up"/"compact" means it
      // already judged this slot should compact soon. Honor it -> recommend compact;
      // a mild yellow with no such action stays noop (keep building); a STALE sidecar
      // demotes to noop (a dead reading must never drive compaction).
      // (slot:bravo 2026-06-18 -- fixes the MISSING-YELLOW-BRANCH that made
      // self-compact never activate in the prudent band: previously YELLOW fell
      // through to the default noop, so ONLY RED/CRITICAL ever nudged a /compact.)
      if (tokenZone.stale === true) {
        decision.recommend = "noop";
        decision.rationale = "token-zone-yellow-but-stale";
      } else if ((tokenZone.action === "wrap-up" || tokenZone.action === "compact")
                 && (typeof tokenZone.worstPct !== "number" || tokenZone.worstPct >= YELLOW_COMPACT_PCT)) {
        // The token-awareness writer emits action:"wrap-up" for the ENTIRE YELLOW
        // zone (no sub-band), so gate on worstPct too: only nudge a proactive
        // /compact past the band MIDPOINT (~50%), not at 25% where compaction would
        // be wasteful. Below the midpoint stays "mild" -> keep building. (scrutiny
        // arm C P2a: makes the mild path actually reachable + the behavior prudent.)
        decision.recommend = "compact";
        decision.rationale = `token-zone-yellow-${tokenZone.action}`;
      } else {
        decision.recommend = "noop";
        decision.rationale = "token-zone-yellow-mild";
      }
