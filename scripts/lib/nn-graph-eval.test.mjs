...
test("assessHoldout -- holdoutGraph seam: holdout drawn from holdoutGraph, classified against graph (U-GNN-CODEBASE-WIRED-CONTROLLED)", () => {
  // base8 = the FIXED test population; the superset adds 4 reference-only ghosts (X*) that exist
  // ONLY in the classification graph. The seam must hold out from base8 (never an X), so a fixed
  // baseline holdout can be scored against an ENLARGED reference pool -- the controlled experiment.
  const base8 = makeGraph(8); // E0Engine..E7Engine
  const wirings = ["prism_cam", "prism_calc", "prism_turning"];
  const xrefs = [0, 1, 2, 3].map((i) => makeGhost(`ghost.unwired.X${i}`, `X${i}Engine`, wirings[i % 3], 0.9));
  const superset = { nodes: [...base8.nodes, ...xrefs], edges: [] };

  const seamed = assessHoldout(superset, makePredictor(), { holdoutGraph: base8, holdout: 4, seed: 3 });
  // The held-out engines must be EXACTLY what buildHoldout(base8) produces -- the seam routes the
  // holdout SOURCE to holdoutGraph, not the superset graph it classifies against.
  const expected = new Set(buildHoldout(base8, { holdout: 4, seed: 3 }).holdout.map((h) => h.label));
  const got = new Set(seamed.samples.map((s) => s.engine));
  assert.deepEqual([...got].sort(), [...expected].sort(), "holdout sourced from holdoutGraph");
  assert.ok([...got].every((e) => !e.startsWith("X")), "a reference-only ghost is NEVER a holdout test item");
});

test("assessHoldout -- default (no holdoutGraph) preserves legacy behavior: holdout drawn from the classification graph", () => {
  // Without the seam the holdout is drawn from the SAME graph that is classified, so the superset's
  // X refs ARE eligible holdout items -- byte-identical to pre-seam behavior.
  const base8 = makeGraph(8);
  const wirings = ["prism_cam", "prism_calc", "prism_turning"];
  const xrefs = [0, 1, 2, 3].map((i) => makeGhost(`ghost.unwired.X${i}`, `X${i}Engine`, wirings[i % 3], 0.9));
  const superset = { nodes: [...base8.nodes, ...xrefs], edges: [] };
  const legacy = new Set(assessHoldout(superset, makePredictor(), { holdout: 4, seed: 3 }).samples.map((s) => s.engine));
  const fromSuperset = new Set(buildHoldout(superset, { holdout: 4, seed: 3 }).holdout.map((h) => h.label));
  assert.deepEqual([...legacy].sort(), [...fromSuperset].sort(), "default holdout drawn from the classification graph (legacy)");
});
...
test("writeAssessment -- writes NN-EVAL.{md,json} + round-trips the result; fail-soft on an unwritable dir", () => {
  const result = runAssessment({ graphPath: path.join(os.tmpdir(), "wa-no-graph.json"), predictor: makePredictor() });
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "wa-"));
  const w = writeAssessment(result, dir);
  assert.equal(w.ok, true);
  assert.equal(fs.existsSync(path.join(dir, "NN-EVAL.json")), true, "JSON written");
  assert.equal(fs.existsSync(path.join(dir, "NN-EVAL.md")), true, "MD written");
  assert.deepEqual(JSON.parse(fs.readFileSync(path.join(dir, "NN-EVAL.json"), "utf8")), JSON.parse(JSON.stringify(result)), "JSON round-trips the assessment");
  fs.rmSync(dir, { recursive: true, force: true });
  // fail-soft (R12): a FILE used as a parent dir -> ENOTDIR; writeAssessment returns {ok:false}, NEVER throws.
  const tmp2 = fs.mkdtempSync(path.join(os.tmpdir(), "wa2-"));
  const filePath = path.join(tmp2, "afile");
  fs.writeFileSync(filePath, "x");
  const bad = writeAssessment(result, path.join(filePath, "sub"));
  assert.equal(bad.ok, false, "unwritable dir -> ok:false (no throw)");
  assert.ok(bad.error, "error message captured");
  fs.rmSync(tmp2, { recursive: true, force: true });
});

test("writeAssessment -- downgrade guard: NO non-direct write (model OR deferred) clobbers a deployed direct-mode NN-EVAL.json (force overrides)", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "wa-guard-"));
  const jsonPath = path.join(dir, "NN-EVAL.json");
  const readMode = () => JSON.parse(fs.readFileSync(jsonPath, "utf8")).embeddingMode;
  const direct = { embeddingMode: "direct", marker: "deployed" };
  const model = { embeddingMode: "model", marker: "degenerate-checkpoint" };
  const REPORT = "stub report"; // 3rd arg bypasses renderReport (synthetic results lack the full shape)

  // 1. No existing file: a model-mode write SUCCEEDS (nothing deployed to protect).
  let w = writeAssessment(model, dir, REPORT);
  assert.equal(w.ok, true);
  assert.ok(!w.skipped, "no existing file -> not skipped");
  assert.equal(readMode(), "model", "model written when no prior report");

  // 2. Write a deployed direct-mode report (the state the retrain lifecycle persists).
  w = writeAssessment(direct, dir, REPORT);
  assert.equal(w.ok, true);
  assert.equal(readMode(), "direct", "direct-mode report on disk");

  // 3. THE GUARD: a bare-CLI model-mode write over a direct-mode report is SUPPRESSED.
  w = writeAssessment(model, dir, REPORT);
  assert.equal(w.ok, true, "suppressed write still reports ok (fail-soft)");
  assert.ok(w.skipped, "model-over-direct -> skipped");
  assert.equal(readMode(), "direct", "deployed direct-mode report PRESERVED (not clobbered)");

  // 4. force is the authoritative-writer override. The retrain lifecycle (stage 4b) passes force:true so
  //    its direct -- or honest deferred-direct -- assessment always lands; this fences only bare-CLI callers.
  w = writeAssessment(model, dir, REPORT, { force: true });
  assert.equal(w.ok, true);
  assert.ok(!w.skipped, "force -> not skipped");
  assert.equal(readMode(), "model", "force lets a model-mode write through");

  // 5. direct-over-model is NEVER suppressed (deploying the good report must always land).
  w = writeAssessment(direct, dir, REPORT);
  assert.equal(w.ok, true);
  assert.ok(!w.skipped, "direct-mode never suppressed");
  assert.equal(readMode(), "direct");

  // 6. model-over-model writes (the guard protects ONLY a deployed direct report).
  fs.writeFileSync(jsonPath, JSON.stringify(model));
  w = writeAssessment(model, dir, REPORT);
  assert.equal(w.ok, true);
  assert.ok(!w.skipped, "model-over-model -> not suppressed");
  assert.equal(readMode(), "model");

  // 7. deferred-over-direct (a bare run with no checkpoint -> {deferred}, no embeddingMode) is ALSO
  //    suppressed: ANY non-direct write is fenced, not just model-mode.
  w = writeAssessment(direct, dir, REPORT); // restore the deployed direct-mode report
  assert.equal(readMode(), "direct");
  const deferred = { deferred: true, marker: "no-checkpoint" }; // no embeddingMode field
  w = writeAssessment(deferred, dir, REPORT);
  assert.equal(w.ok, true, "suppressed write still reports ok (fail-soft)");
  assert.ok(w.skipped, "deferred-over-direct -> skipped (a {deferred} result is non-direct)");
  assert.equal(readMode(), "direct", "deployed direct-mode report PRESERVED against a deferred bare run");

  // 8. force lets the AUTHORITATIVE lifecycle write an honest deferred-direct assessment over the prior
  //    report (R12: the deployed-state file must reflect that the direct path now defers, not a stale claim).
  w = writeAssessment(deferred, dir, REPORT, { force: true });
  assert.equal(w.ok, true);
  assert.ok(!w.skipped, "force -> deferred write lands");
  assert.equal(JSON.parse(fs.readFileSync(jsonPath, "utf8")).deferred, true, "honest deferred state persisted under force");

  fs.rmSync(dir, { recursive: true, force: true });
});

test("atomicWriteFileSync -- round-trips, leaves no .tmp sibling, overwrites in place, rethrows+cleans up on failure", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "wa-atomic-"));
  const target = path.join(dir, "NN-EVAL.json");
  const noTmp = () => assert.deepEqual(fs.readdirSync(dir).filter((f) => f.includes(".tmp-")), [], "no .tmp- sibling left behind");

  atomicWriteFileSync(target, JSON.stringify({ embeddingMode: "direct", n: 1 }));
  assert.deepEqual(JSON.parse(fs.readFileSync(target, "utf8")), { embeddingMode: "direct", n: 1 }, "data round-trips");
  noTmp(); // the torn-read window is closed -- the tmp must not linger after a successful rename

  atomicWriteFileSync(target, JSON.stringify({ embeddingMode: "model", n: 2 }));
  assert.equal(JSON.parse(fs.readFileSync(target, "utf8")).n, 2, "overwrite landed atomically (rename replaces in place)");
  noTmp();

  // failure path (R12): a target whose parent is a FILE -> writeFileSync(tmp) throws ENOTDIR; the helper
  // rethrows so writeAssessment's catch reports { ok:false }, and best-effort cleans the tmp up.
  const filePath = path.join(dir, "afile");
  fs.writeFileSync(filePath, "x");
  assert.throws(() => atomicWriteFileSync(path.join(filePath, "sub.json"), "data"), "rethrows on an unwritable target");
