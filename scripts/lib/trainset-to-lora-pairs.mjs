...
export function trainsetRowToPairs(row, rowIdx = 0) {
  if (!row || typeof row !== "object" || (!Array.isArray(row.labels) && !Array.isArray(row.gdt_labels))) return [];
  ...
  const pairs = [];
  let i = 0;
  for (const l of (Array.isArray(row.labels) ? row.labels : [])) {
  ...
  // GD&T training pairs (U-XRAY-GDT-LABEL-TIER): one per trainable GD&T frame. The ground truth is
  // the canonical FCF string (the GD&T analogue of a dimension's value_mm), stamped by buildFcfText.
  // calibration_basis is carried into context so a consumer knows the tier is dimension-calibrated.
  let gi = 0;
  for (const l of (Array.isArray(row.gdt_labels) ? row.gdt_labels : [])) {
    if (!l || typeof l !== "object" || l.trainable !== true) continue; // ONLY trainable labels
    const fcf = l.fcf_text != null ? String(l.fcf_text) : "";
    if (!fcf) continue;                                                 // no usable FCF ground truth → skip
    pairs.push({
      pairId: `${part}#${rowIdx}:gdt:${gi}`,
      customer: "na",
      partNumber: part,
      pdfPath: image,
      extractionType: "gdt",
      groundTruthValue: fcf,
      context: `type=gdt symbol=${l.symbol ?? "?"} fcf=${fcf} n_models=${l.n_models ?? "?"} `
        + `agreement_fraction=${l.agreement_fraction ?? "?"} corroboration=${l.corroboration ?? "?"} `
        + `expected_accuracy=${l.expected_accuracy ?? "?"} tier=${l.tier ?? "?"} `
        + `calibration_basis=${l.calibration_basis ?? "?"} src=${src}`,
    });
    gi++;
  }
  return pairs;
