...
// Sidecar schema version THIS loader understands. It consumes ONLY `nodes` +
// `inverted` (present since 1.0.0), so it accepts any sidecar with the SAME
// MAJOR version — an additive MINOR bump must never make this reader reject a
// valid sidecar and silently fall back to the architecture graph. A MAJOR bump
// (2.x = breaking node/inverted shape) is still rejected → safe legacy
// degradation, never a crash. Kept in step with build-graph-index.mjs
// SIDECAR_SCHEMA_VERSION. See U-MASTER-INDEX-SIDECAR + U-SIERRA-MASTERINDEX-DEGREE-SIDECAR
// (2026-07-05: the 1.0.0→1.1.0 `degrees` bump strict-rejected here, silently
// re-degrading the daemon's fleet search — fixed to major-compatible).
export const SIDECAR_SCHEMA_VERSION = "1.1.0";
const SIDECAR_SCHEMA_MAJOR = 1;

/**
 * True when a sidecar's schemaVersion shares this loader's MAJOR version, i.e.
 * its `nodes`/`inverted` shape is compatible. Additive minor bumps (new keys
 * like `degrees`) pass; a breaking major bump is rejected.
 * @param {*} v  raw schemaVersion string
 * @returns {boolean}
 */
export function sidecarSchemaCompatible(v) {
  if (typeof v !== "string") return false;
  const major = Number.parseInt(v.split(".")[0], 10);
  return Number.isFinite(major) && major === SIDECAR_SCHEMA_MAJOR;
}
...
function tryLoadSidecar(graphPath, graphStat) {
  ...
  if (!sc || !sidecarSchemaCompatible(sc.schemaVersion)) return rejected("schema mismatch");
