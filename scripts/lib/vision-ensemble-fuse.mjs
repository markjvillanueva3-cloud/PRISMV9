...
import { buildReadingGuidanceBlock } from "./blueprint-reading-knowledge.mjs";
...
// ------------------------------------------------------------------------------
// NON-DIMENSION FIELD UNION (recall-first) -- gdt / notes / profiles / surface_finishes.
//
// parseVisionResponse extracts these per model (goal-critical for CAD reconstruction +
// quoting, per the ollama-vision-extract-lib header), but the fuse used to keep ONLY
// dimensions and SILENTLY DROP them -- so the ensemble discarded every GD&T frame, note,
// profile, and surface-finish callout it read (R12 data loss; buildRegionRoutedFused's
// pass-through already ASSUMED the fused carried gdt/notes/...). This unions each field
// across the ensemble: every DISTINCT entry is KEPT (recall-first -- the same bias as keeping
// dimension singletons), de-duped by a per-field identity key, carrying the SAME
// corroboration / models / hallucination_candidate trust metadata the fused dimensions do, so
// a consumer applies one trust gate across all field types. Additive: existing consumers read
// dimensions/summary and are byte-unaffected.
// ------------------------------------------------------------------------------

/** Normalize a string for an identity key: lowercased, whitespace-collapsed, trimmed. */
function nonDimKeyNorm(s) {
  return s == null ? "" : String(s).toLowerCase().replace(/\s+/g, " ").trim();
}
/** Stringify a numeric key part (null/undefined/NaN -> "" so it never reads as a value). */
function numKeyPart(n) {
  return Number.isFinite(n) ? String(n) : "";
}

// FCF identity: geometric characteristic + tolerance + material condition + datum refs + feature.
// datum_references are order-insensitive (A,B == B,A), so they are sorted into the key.
function gdtKey(g) {
  const datums = (Array.isArray(g.datum_references) ? g.datum_references.map(nonDimKeyNorm) : []).sort().join(",");
  return [nonDimKeyNorm(g.symbol), numKeyPart(g.tolerance_value), nonDimKeyNorm(g.material_condition), datums, nonDimKeyNorm(g.applied_to)].join("|");
}
function noteKey(n) { return [nonDimKeyNorm(n.category), nonDimKeyNorm(n.text)].join("|"); }
function profileKey(p) {
  return [nonDimKeyNorm(p.name), nonDimKeyNorm(p.type), numKeyPart(p.width_mm), numKeyPart(p.height_mm), numKeyPart(p.diameter_mm)].join("|");
}
// A surface finish is identified by its ra value when present, else its verbatim text, plus location.
function surfaceFinishKey(s) {
  const id = Number.isFinite(s.ra_um) ? "ra:" + s.ra_um : "raw:" + nonDimKeyNorm(s.raw_text);
  return id + "|" + nonDimKeyNorm(s.location);
}

// Exported per-field identity keys so a cross-SOURCE union (e.g. region-glue-lib unioning the
// per-region + full-page-floor non-dim fields in the dense-rescue case) de-dupes by the SAME identity
// the ensemble fuse uses -- one identity definition, never a fork (R8). Keyed by fused field name.
export const NON_DIM_KEY_FNS = { gdt: gdtKey, notes: noteKey, profiles: profileKey, surface_finishes: surfaceFinishKey };

/**
 * Pure: union one non-dimension field across the ensemble runs, de-duped by keyFn, recall-first.
 * The representative of a de-duped group is the member with the highest confidence (ties -> the
 * first model that reported it -- deterministic, no Math.random). Each output entry preserves the
 * representative's fields and adds corroboration / n_models / models / hallucination_candidate
 * (mirroring the fused dimension trust metadata).
 *
 * @param {Array<{model:string, extraction:object}>} runs  the valid model extractions
 * @param {string} field  "gdt" | "notes" | "profiles" | "surface_finishes"
 * @param {(item:object)=>string} keyFn  identity key for the field
 * @param {number} nModels  ensemble size (drives hallucination_candidate = 1 of >=2)
 * @returns {Array<object>}  union entries, corroboration-desc then identity-key-asc (a content-
 *   derived tie-break, so the order is deterministic regardless of model input order)
 */
function fuseNonDimField(runs, field, keyFn, nModels) {
  const groups = new Map(); // key -> { rep, models:Set, maxConf }
  for (const r of runs) {
    const arr = r && r.extraction && Array.isArray(r.extraction[field]) ? r.extraction[field] : [];
    for (const item of arr) {
      if (!item || typeof item !== "object") continue;
      let key;
      try { key = keyFn(item); } catch { key = null; }
      if (key == null) continue;
      let g = groups.get(key);
      if (!g) { g = { rep: item, models: new Set(), maxConf: -1 }; groups.set(key, g); }
      g.models.add(r.model);
      const c = Number.isFinite(item.confidence) ? item.confidence : -1;
      if (c > g.maxConf) { g.maxConf = c; g.rep = item; } // strictly-greater -> first model wins ties
    }
  }
  const out = [];
  for (const [key, g] of groups) {
    const corroboration = g.models.size;
    out.push({
      key,
      entry: {
        ...g.rep,
        ...
        hallucination_candidate: corroboration === 1 && nModels >= 2,
      },
    });
  }
  // corroboration desc, then identity key asc -- a CONTENT-derived tie-break, so the order is
  // fully deterministic regardless of which model first reported each distinct entry.
  out.sort((a, b) => b.entry.corroboration - a.entry.corroboration || (a.key < b.key ? -1 : a.key > b.key ? 1 : 0));
  return out.map((o) => o.entry);
}

/**
 * Pure: fuse N model extractions into a corroborated consensus dimension set PLUS recall-first
 * unions of the non-dimension fields (gdt / notes / profiles / surface_finishes).
 *
 ...
 *   gdt: Array<object>,                   // union, corroboration-desc (each carries trust metadata)
 *   notes: Array<object>,                 // union, corroboration-desc
 *   profiles: Array<object>,              // union, corroboration-desc
 *   surface_finishes: Array<object>,      // union, corroboration-desc
...
export function fuseEnsemble(modelExtractions, opts = {}) {
  ...
  // Recall-first unions of the non-dimension fields the fuse used to DROP (see helpers above).
  const gdt = fuseNonDimField(runs, "gdt", gdtKey, nModels);
  const notes = fuseNonDimField(runs, "notes", noteKey, nModels);
  const profiles = fuseNonDimField(runs, "profiles", profileKey, nModels);
  const surface_finishes = fuseNonDimField(runs, "surface_finishes", surfaceFinishKey, nModels);
  ...
  const summary = {
    ...
    // Non-dimension coverage (previously silently zero -- the fuse dropped these fields).
    n_gdt: gdt.length,
    n_notes: notes.length,
    n_profiles: profiles.length,
    n_surface_finishes: surface_finishes.length,
    n_gdt_hallucination_candidates: gdt.filter((g) => g.hallucination_candidate).length,
  };

  return { dimensions, gdt, notes, profiles, surface_finishes, ambiguous_pairs, summary, per_model };
}
...
// HARD per-call ceiling: one VLM /api/generate call must FAIL FAST so a stalled/wedged Ollama (e.g.
// VRAM contention with a co-resident 65GB model) drops THAT page and the run moves on -- never hangs
// for the whole run budget. The training loop passes maxTimeSec = the 5h WINDOW (18000s); without this
// cap that 5h leaked into the per-call curl --max-time, so one wedged call froze the entire overnight
// run (observed 2026-06-16: cursor stuck at 0, GPU 1%, run dead for the night). 600s is ~5-20x a real
// 300dpi page extraction; anything beyond is a wedge, not slowness.
const PER_CALL_MAX_TIME_SEC = 600;
 ...
 *   ollamaUrl?:string, maxTimeSec?:number, format?:string, injectReadingGuidance?:boolean,
 *   deps?:{spawn?:Function, writeFile?:Function, unlink?:Function}
 * }} a
 *   injectReadingGuidance: OPT-IN (default off). When true, appends the curated tribal + ASME Y14.5
 *   reading-guidance block (blueprint-reading-knowledge.mjs) to the VLM prompt. Off => byte-identical prompt.
...
export async function ocrImageWithModelAsync(a) {
  ...
  // U-XRAY-READING-KNOWLEDGE: opt-in domain reading-guidance injection. Off (default) => empty block =>
  // buildVisionPrompt is byte-identical to the proven prompt (zero regression on the live path).
  const readingGuidance = a.injectReadingGuidance
    ? buildReadingGuidanceBlock({ partClass: a.partClass, wireEdm: !!a.wireEdm })
    : "";
  const prompt = buildVisionPrompt(a.partClass || "generic", { wireEdm: !!a.wireEdm, readingGuidance });
  // a.format (e.g. "json") -> Ollama grammar-constrained decode; undefined -> buildOllamaRequestBody
  // omits the key (byte-identical legacy body). Recovers the qwen2.5vl runaway-JSON dropout.
  const body = buildOllamaRequestBody(prompt, a.imageBase64, { model: a.model, format: a.format });
  ...
  const r = await curlAsync(["-s", "--max-time", String(perCallSec), url, "-d", "@" + reqFile], deps);
  ...
  const parsed = parseVisionResponse(raw, { assumeUnits: a.assumeUnits, forceUnits: a.forceUnits });
 ...
 *   forceUnits?:string, ollamaUrl?:string, maxTimeSec?:number, workDir?:string,
 *   format?:string, fuseOpts?:object, injectReadingGuidance?:boolean,
...
export async function runEnsembleOverImage(a) {
  ...
  const results = await Promise.all(models.map(async (model, idx) => {
    ...
    const ex = await ocrImageWithModelAsync({
      ...
      partClass: a.partClass, wireEdm: a.wireEdm, assumeUnits: a.assumeUnits, forceUnits: a.forceUnits,
      ollamaUrl: a.ollamaUrl, maxTimeSec: a.maxTimeSec, format: a.format,
      injectReadingGuidance: a.injectReadingGuidance, deps,
    });
    const ms = now() - t0;
