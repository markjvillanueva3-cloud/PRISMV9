...
test("body defaults: instruct model, SMALL num_predict, think:false, deterministic temp, image present", () => {
  ...
  assert.equal(b.options.num_ctx, 4096);
});
...
test("confident not-a-drawing → SKIP", () => {
  const v = decidePageVerdict({ is_drawing: false, confidence: 0.9 });
...
test("custom minConfidence floor shifts the skip threshold", () => {
  // conf 0.6 not-a-drawing: skips at floor 0.5, extracts at default 0.7
  assert.equal(decidePageVerdict({ is_drawing: false, confidence: 0.6 }, { minConfidence: 0.5 }).verdict, "skip");
  assert.equal(decidePageVerdict({ is_drawing: false, confidence: 0.6 }).verdict, "extract");
});
