...
import { exceedsStringParseCap } from "./graph-io.mjs";
...
test("generated system-graph.json: meta.coverage matches BUILD_STATE + L5 nodes", (t) => {
  ...
  // GRAPH-READ SAFETY (U-VIZ-COVERAGE-TEST-BIGREAD): system-graph.json is a SHARED
  // path -- when it holds the merged regen-viz product (372K nodes, ~765MB) it
  // exceeds Node's UTF-8 string cap (0x1fffffe8 ~512MB) and a single
  // readFileSync(...,"utf8") THROWS ERR_STRING_TOO_LONG *before* the schemaVersion
  // skip below can fire. The generate-system-viz 2.1.0 product this test verifies is
  // small, so a graph over the cap is definitionally the wrong product -- skip loudly
  // (R12) rather than crash. Reuses the canonical cap via exceedsStringParseCap().
  const graphBytes = fs.statSync(SYSTEM_GRAPH_PATH).size;
  if (exceedsStringParseCap(graphBytes)) {
    t.skip(
      `system-graph.json is ${Math.round(graphBytes / 1e6)}MB (> V8 string cap) -- holds the merged `
      + "regen-viz product, not the generate-system-viz 2.1.0 product; run scripts/generate-system-viz.mjs to verify meta.coverage",
    );
    return;
  }
