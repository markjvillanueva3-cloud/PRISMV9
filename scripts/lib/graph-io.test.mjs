...
// --- truncation guard (U-VIZ-GRAPHIO-TRUNCATION-GUARD): a graph cut off mid-array must
//     fail LOUD, not silently report a partial count / hand back a partial projection ---

test("countGraphArrayStreaming: a TRUNCATED array (no closing ']') -> 0, not a misleading partial count", () => {
  const p = tmpFile();
  try {
    // a non-atomic write crashed mid-array: 2 elements then EOF, no closing ']' / '}'.
    fs.writeFileSync(p, '{"schemaVersion":"1","nodes":[{"id":"a"},{"id":"b"}');
    assert.equal(countGraphArrayStreaming(p, "nodes"), 0, "truncated -> 0, never the misleading partial 2");
    // the SAME prefix, properly closed, still counts correctly (happy path unchanged).
    fs.writeFileSync(p, '{"schemaVersion":"1","nodes":[{"id":"a"},{"id":"b"}]}');
    assert.equal(countGraphArrayStreaming(p, "nodes"), 2);
  } finally { fs.unlinkSync(p); }
});

test("countGraphArrayStreaming: a TRUNCATED scalar array -> 0; a legit empty array stays 0", () => {
  const p = tmpFile();
  try {
    fs.writeFileSync(p, '{"nodes":["a","b","c"');      // truncated scalar array -> couldn't verify
    assert.equal(countGraphArrayStreaming(p, "nodes"), 0);
    fs.writeFileSync(p, '{"nodes":[]}');                // legitimately empty + closed -> 0
    assert.equal(countGraphArrayStreaming(p, "nodes"), 0);
  } finally { fs.unlinkSync(p); }
});

test("streamGraphArray: a TRUNCATED array THROWS (fail loud) after streaming the valid prefix", () => {
  const p = tmpFile();
  try {
    fs.writeFileSync(p, '{"schemaVersion":"1","nodes":[{"id":"a"},{"id":"b"}');  // no closing ']'
    const seen = [];
    assert.throws(
      () => streamGraphArray(p, "nodes", (el) => seen.push(el.id)),
      /unterminated array 'nodes'/);
    assert.deepEqual(seen, ["a", "b"], "the valid prefix streamed before truncation was detected -> the throw flags it incomplete");
  } finally { fs.unlinkSync(p); }
});

test("streamGraphArray: a properly-closed/empty/missing array does NOT throw (happy path unchanged)", () => {
  const p = tmpFile();
  try {
    fs.writeFileSync(p, '{"schemaVersion":"1","nodes":[{"id":"a"},{"id":"b"}],"edges":[]}');
    const seen = [];
    assert.equal(streamGraphArray(p, "nodes", (el) => seen.push(el.id)), 2);
    assert.deepEqual(seen, ["a", "b"]);
    assert.equal(streamGraphArray(p, "edges", () => {}), 0, "empty closed array -> 0, no throw");
    assert.equal(streamGraphArray(p, "missing", () => {}), 0, "missing key -> 0, no throw");
  } finally { fs.unlinkSync(p); }
});

test("readGraphStreaming: a TRUNCATED large array THROWS (the between-element cut its per-element parse misses)", () => {
  const p = tmpFile();
  try {
    // crashed write: ends cleanly right after the 2nd element -- no closing ']' /'}'. The
    // per-element JSON.parse never sees a partial slice, so only the closed-flag catches it.
    fs.writeFileSync(p, '{"schemaVersion":"1","nodes":[{"id":"a"},{"id":"b"}');
    assert.throws(() => readGraphStreaming(p), /unterminated array 'nodes'/);
    // a properly-closed graph reads fine (happy path unchanged).
    fs.writeFileSync(p, '{"schemaVersion":"1","nodes":[{"id":"a"},{"id":"b"}],"edges":[]}');
    const g = readGraphStreaming(p);
