...
export function computeSuppressSet(audit, { now = Date.now(), maxAgeMs = DEFAULT_MAX_AGE_MS } = {}) {
  ...
  for (const r of rows) {
    ...
    // LOAD-BEARING: keep the match strict `=== "suppress"`. The audit also emits an
    // ADVISORY `suppress-candidate` verdict (U-CLASSIFY-SUPPRESS-CANDIDATE) that must
    // NOT be muted -- a `.startsWith("suppress")` / `.includes("suppress")` would
    // wrongly catch it and silently kill the operator's graph-utilization nudges.
    // (The takes>0 clause is a 2nd guard; suppress-candidate is always takes===0.)
