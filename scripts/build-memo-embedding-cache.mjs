...
async function main() {
  ...
  for (let i = 0; i < work.length; i++) {
    ...
    let body;
    try { body = readFileSync(srcPath, "utf8"); } catch { failed++; continue; }
    const salient = salientSlice(body);
    if (!salient) { failed++; continue; }
    ...
    toEmbed.push({ name, salient, hash: h, path: srcPath });
  }
  ...
  for (let start = 0; start < toEmbed.length; start += BATCH_SIZE) {
    ...
    const batchOpts = { url: EMBED_URL, model: EMBED_MODEL };
  ...
  console.error(`[memo-embed] DONE — ${out.length} vectors (embedded ${embedded}, reused ${reused}, failed ${failed}) · dim ${out[0].vec.length} → ${EMBED_CACHE}`);
}
