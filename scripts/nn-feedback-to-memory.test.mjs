...
test("renderMemoryEntry: promoted round — frontmatter + concrete metrics", () => {
  ...
  // run-logs self-declare run_log:true so the wiki-promotion gate excludes them (vault hardening)
  assert.ok(e.markdown.includes("run_log: true"), "run_log marker for promotion exclusion");
