...
// U-LORA-CORPUS-GALAXY-CAP (slot:india): opt-in per-galaxy cap so one over-represented galaxy
// (the templated lathe archive hit ~54% of the corpus) cannot dominate the fine-tune.
test("capRowsPerGalaxy: caps each galaxy at N, preserves source order", () => {
  const rows = [
    { galaxy: "lathe", output: "a" }, { galaxy: "lathe", output: "b" }, { galaxy: "lathe", output: "c" },
    { galaxy: "mill", output: "d" }, { galaxy: "mill", output: "e" },
  ];
  const { rows: out, dropped } = capRowsPerGalaxy(rows, 2);
  assert.equal(dropped, 1);                                  // 1 lathe row over the cap
  assert.deepEqual(out.map((r) => r.output), ["a", "b", "d", "e"]); // first 2 lathe kept, order preserved
});
test("capRowsPerGalaxy: untagged/cross-cutting rows are NEVER capped", () => {
  const rows = [
    { output: "x1" }, { output: "x2" }, { output: "x3" },    // no galaxy -> cross-cutting
    { galaxy: "lathe", output: "a" }, { galaxy: "lathe", output: "b" },
  ];
  const { rows: out, dropped } = capRowsPerGalaxy(rows, 1);
  assert.equal(dropped, 1);                                  // 1 lathe dropped; the 3 untagged survive
  assert.equal(out.filter((r) => !r.galaxy).length, 3);
  assert.equal(out.filter((r) => r.galaxy === "lathe").length, 1);
});
test("capRowsPerGalaxy: when capping, KEEPS higher-weight (verified) rows over advisory (arm-B fix)", () => {
  // verified row appears LAST in source order; a weight-blind first-N cap would drop it.
  const rows = [
    { galaxy: "mill", output: "adv1", weight: 0.5 },
    { galaxy: "mill", output: "adv2", weight: 0.5 },
    { galaxy: "mill", output: "verified", weight: 1.0 },
  ];
  const { rows: out, dropped } = capRowsPerGalaxy(rows, 1);
  assert.equal(dropped, 2);
  assert.deepEqual(out.map((r) => r.output), ["verified"]);  // the w=1.0 row survives, not first adv
});
test("capRowsPerGalaxy: ties (equal/absent weight) keep source order -> first-N fallback unchanged", () => {
  const rows = [
    { galaxy: "mill", output: "a", weight: 0.5 }, { galaxy: "mill", output: "b", weight: 0.5 }, { galaxy: "mill", output: "c", weight: 0.5 },
  ];
  assert.deepEqual(capRowsPerGalaxy(rows, 2).rows.map((r) => r.output), ["a", "b"]); // original order
});
test("capRowsPerGalaxy: cap<=0 or non-finite is a no-op (default OFF -> zero behavior change)", () => {
  const rows = [{ galaxy: "lathe", output: "a" }, { galaxy: "lathe", output: "b" }];
  assert.equal(capRowsPerGalaxy(rows, 0).rows.length, 2);
  assert.equal(capRowsPerGalaxy(rows, 0).dropped, 0);
  assert.equal(capRowsPerGalaxy(rows, Infinity).rows.length, 2);
  assert.equal(capRowsPerGalaxy(rows, -5).rows.length, 2);
});
test("assembleCorpus threads capPerGalaxy: capped stats match the written set", () => {
  // hermetic inventory: one source, 3 lathe-tagged rows, cap=2 -> totalRows 2, byGalaxy.lathe 2
  const inv = { sources: [{ id: "s", kind: "lora-training-jsonl", status: "present", path: "/x.jsonl", advisory: true }] };
  const jsonl = [
    JSON.stringify({ instruction: "i1", output: "o1", galaxy: "lathe" }),
    JSON.stringify({ instruction: "i2", output: "o2", galaxy: "lathe" }),
    JSON.stringify({ instruction: "i3", output: "o3", galaxy: "lathe" }),
  ].join("\n");
  const r = assembleCorpus(inv, { readImpl: () => jsonl, capPerGalaxy: 2 });
  assert.equal(r.totalRows, 2);
  assert.equal(r.byGalaxy.lathe, 2);
  assert.equal(r.cappedPerGalaxy, 2);
  assert.equal(r.cappedDropped, 1);
  assert.equal(r.rows.length, 2);                            // written set == capped set
});
