...
describe("executeRefresh (sequential step loop)", () => {
  it("runs steps in order, all ok → exit 0", () => {
    ...
    assert.deepEqual(ran, ["mem-index", "mem-embed", "galaxy-synth", "wiki-tribal", "vault-rot", "supersession", "vault-links", "code-vault", "code-vault-moc", "vault-health"]);
  ...
  it("all ollama down → only no-requires steps run", () => {
    ...
    assert.deepEqual(ran, ["mem-index", "vault-rot", "supersession", "vault-links", "code-vault", "code-vault-moc", "vault-health"], "only no-requires steps run (mem-embed/wiki-tribal defer on embeddings, galaxy defers on generate; the gap-sentinels + link-doctor + the code-vault regen/MOC + the vault-health rollup are all Ollama-independent)");
    assert.equal(r.exitCode, 3);
  });
});
...
describe("vault gap-sentinels (U-SIERRA-BRAIN-GAP-SENTINELS)", () => {
  ...
  it("default refresh runs them in table order, after wiki-tribal and before regen-viz", () => {
    const ids = planSteps().map((s) => s.id);
    assert.deepEqual(ids, ["mem-index", "mem-embed", "galaxy-synth", "wiki-tribal", "vault-rot", "supersession", "vault-links", "code-vault", "code-vault-moc", "vault-health"]);
  });
