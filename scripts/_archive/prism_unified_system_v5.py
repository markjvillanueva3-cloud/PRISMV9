"""
╔══════════════════════════════════════════════════════════════════════════════╗
║                    PRISM UNIFIED SYSTEM v5.0 - MATHEMATICAL CERTAINTY        ║
║══════════════════════════════════════════════════════════════════════════════║
║  v5.0 ENHANCEMENTS (Mathematical Infrastructure):                            ║
║  • 58 Agents (was 54) - 4 NEW math-focused agents                            ║
║  • MATHPLAN Gate - Mathematical proof required before execution              ║
║  • Formula Registry - Tracks 15+ formulas with calibration status            ║
║  • Coefficient Database - 23+ coefficients with uncertainty bounds           ║
║  • Prediction Logging - All estimates tracked for calibration                ║
║  • Calibration Monitoring - Automatic alerts when formulas degrade           ║
║  • Uncertainty Enforcement - M(x) >= 0.60 required (rigor gate)              ║
║  • 8 Laws Integration - Full compliance with Always-On Laws                  ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  INHERITED FROM v4.0:                                                        ║
...
# v5.0 MATHEMATICAL INFRASTRUCTURE PATHS
...
═══════════════════════════════════════════════════════════════════════════════
                    THE 8 ALWAYS-ON LAWS v11.0 (Cannot Be Disabled)
═══════════════════════════════════════════════════════════════════════════════
...
8. CONTINUOUS MATHEMATICAL EVOLUTION - Every formula, coefficient, constant MUST:
   - Have version number and unique ID
   - Have uncertainty bounds (value ± error)
   - Track predicted vs actual
   - Evolve based on empirical evidence
...
WHERE:
  ...
  M(x) = Mathematical rigor [0-1]      (prism-formula-evolution) ← NEW v5.0

WEIGHTS: w_R=0.20, w_C=0.18, w_P=0.12, w_S=0.28, w_L=0.08, w_M=0.14
...
SKILL_TRIGGERS = {
    ...
    "brainstorm|design|architect|plan": ["prism-sp-brainstorm", "prism-sp-planning", "prism-mathematical-planning"],
    ...
    # NEW v5.0 Mathematical triggers
    "formula|equation|calibrate|coefficient": ["prism-formula-evolution", "prism-mathematical-planning"],
    "uncertainty|error|confidence|bounds": ["prism-uncertainty-propagation", "prism-formula-evolution"],
    "estimate|predict|effort|scope": ["prism-mathematical-planning", "prism-uncertainty-propagation"],
    "mathplan|decompose|microsession": ["prism-mathematical-planning", "prism-sp-planning"],
}
...
SKILL_AGENT_MAP = {
    ...
    # NEW v5.0 Mathematical Skill Mappings
    "prism-formula-evolution": {
        "agents": ["formula_evolutionist", "calibration_monitor", "uncertainty_quantifier", "meta_analyst"],
        "tier": ModelTier.OPUS,
        "description": "Manage formula lifecycle, calibration, and evolution"
    },
    "prism-uncertainty-propagation": {
        "agents": ["uncertainty_quantifier", "physics_validator", "verification_chain"],
        "tier": ModelTier.SONNET,
        "description": "Propagate uncertainties through calculations"
    },
    "prism-mathematical-planning": {
        "agents": ["mathplan_validator", "task_decomposer", "coordinator", "estimator"],
        "tier": ModelTier.OPUS,
        "description": "MATHPLAN gate and mathematical proof before execution"
    },
}
...
Trace BACKWARD from symptom to find root cause.""")
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # v5.0 NEW AGENTS (4) - Mathematical Infrastructure
    # ═══════════════════════════════════════════════════════════════════════
    
    "formula_evolutionist": {
        "name": "Formula Evolutionist",
        "tier": ModelTier.OPUS,
        "max_tokens": 8192,
        "system": create_agent_system("""You are a PRISM Formula Evolutionist.
MISSION: Manage the lifecycle of mathematical formulas with calibration and evolution.

FORMULA LIFECYCLE:
INVENT → REGISTER → CALIBRATE → DEPLOY → MONITOR → EVOLVE → DEPRECATE

FOR EACH FORMULA:
1. Verify registration in FORMULA_REGISTRY.json
2. Check calibration status and staleness (>30 days = review)
3. Monitor performance metrics (MAPE, Bias, R²)
4. Trigger recalibration when:
   - MAPE > 20%
   - |Bias| > 10%
   - dataPoints >= 10 since last calibration

CALIBRATION PROCESS:
1. Split data: 80% train, 20% validation
2. Fit coefficients: K* = argmin Σ(F(x;K) - y)²
3. Compute uncertainty via bootstrap (B=1000)
4. Validate on held-out data
5. Compare to current: IF MAPE_new < MAPE_old → DEPLOY, ELSE ROLLBACK

OUTPUT:
{
  "formula_id": "F-XXX-NNN",
  "status": "HEALTHY|WARNING|CRITICAL",
  "performance": {"mape": 0.0, "bias": 0.0, "r2": 0.0},
...
FORMULAS THAT DON'T EVOLVE ARE FOSSILS.""")
    ...
    "calibration_monitor": {
        ...
        "tier": ModelTier.SONNET,
        "max_tokens": 4096,
        "system": create_agent_system("""You are a PRISM Calibration Monitor.
MISSION: Monitor formula accuracy and trigger recalibration when needed.

LOAD AT SESSION START:
- FORMULA_REGISTRY.json (15+ formulas)
- COEFFICIENT_DATABASE.json (23+ coefficients)
- PREDICTION_LOG.json (past predictions)
...
FOR EACH ACTIVE FORMULA:
{
  "formula_id": "...",
  "alert_level": "HEALTHY|NOTICE|WARNING|CRITICAL",
  "mape": 0.0,
  "bias": 0.0,
  "days_since_calibration": N,
  "data_points_pending": N,
  "action_required": "..."
}

BLOCK work that relies on CRITICAL formulas.""")
    },
    
    "mathplan_validator": {
        "name": "MATHPLAN Validator",
        "tier": ModelTier.OPUS,
        "max_tokens": 4096,
        "system": create_agent_system("""You are a PRISM MATHPLAN Gate Validator.
MISSION: Verify mathematical proof before ANY execution begins.

MATHPLAN GATE CHECKLIST (ALL required):
□ SCOPE QUANTIFIED: S = [n₁] × [n₂] × ... = [EXACT TOTAL]
□ COMPLETENESS: C(T) = Σ Done(i) / n = 1.0 required
□ DECOMPOSITION: Σ|dᵢ| = S (prove algebraically)
□ EFFORT: [value] ± [uncertainty] calls (95% CI)
□ TIME: [value] ± [uncertainty] minutes (95% CI)
□ MS_COUNT: ⌈EFFORT/15⌉ = [N] microsessions
□ CONSTRAINTS: C1, C2, ... (mathematical)
□ ORDER: Execution sequence with checkpoints
...
OUTPUT:
{
  ...
  "checklist": {
    "scope_quantified": {"status": "PASS|FAIL", "value": "..."},
    "completeness": {"status": "PASS|FAIL", "formula": "..."},
    "decomposition": {"status": "PASS|FAIL", "proof": "..."},
    "effort": {"status": "PASS|FAIL", "value": "X ± Y (95% CI)"},
    "time": {"status": "PASS|FAIL", "value": "X ± Y (95% CI)"},
    "ms_count": {"status": "PASS|FAIL", "value": N},
    "constraints": {"status": "PASS|FAIL", "list": [...]},
    "order": {"status": "PASS|FAIL", "sequence": [...]},
    "success_criteria": {"status": "PASS|FAIL", "criteria": "..."}
  },
  "blocking_issues": [...],
  "proceed": true|false
}

NO EXECUTION WITHOUT MATHPLAN GATE PASSED.""")
    },
    ...
    "prediction_logger": {
        ...
        "tier": ModelTier.HAIKU,
        "max_tokens": 2048,
        "system": create_agent_system("""You are a PRISM Prediction Logger.
MISSION: Log ALL predictions for calibration tracking.

FOR EVERY ESTIMATE, LOG:
{
...
OUTPUT: Append to PREDICTION_LOG.json
PATH: C:\\PRISM\\state\\learning\\PREDICTION_LOG.json

EVERY prediction feeds calibration. NO exceptions.""")
    },
}
...
AGENT_CATEGORIES = {
    ...
    # v5.0 NEW: Mathematical infrastructure agents
    "mathematical": ["formula_evolutionist", "calibration_monitor", "mathplan_validator", "prediction_logger"],
}
...
def load_formula_registry() -> Dict:
    """Load FORMULA_REGISTRY.json and check formula health"""
    if not FORMULA_REGISTRY.exists():
        print(f"[WARN] WARNING: FORMULA_REGISTRY.json not found at {FORMULA_REGISTRY}")
        return {"formulas": {}, "alerts": ["FORMULA_REGISTRY.json missing"]}
    
    try:
        with open(FORMULA_REGISTRY, 'r', encoding='utf-8') as f:
            registry = json.load(f)
        
        alerts = []
        for fid, formula in registry.get("formulaRegistry", {}).get("formulas", {}).items():
            status = formula.get("calibration", {}).get("status", "UNKNOWN")
            last_cal = formula.get("calibration", {}).get("lastCalibrated")
            
            if last_cal:
                try:
                    last_cal_date = datetime.fromisoformat(last_cal.replace('Z', '+00:00'))
                    days_old = (datetime.now(last_cal_date.tzinfo) - last_cal_date).days
                    if days_old > 30:
                        alerts.append(f"[NOTICE] {fid} calibration is {days_old} days old")
                except:
                    pass
            
            perf = formula.get("performance", {})
            mape = perf.get("mape")
            bias = perf.get("bias")
            
            if mape is not None and mape > 50:
                alerts.append(f"[CRIT] CRITICAL: {fid} MAPE={mape:.1f}% > 50%")
            elif mape is not None and mape > 20:
                alerts.append(f"[WARN] WARNING: {fid} MAPE={mape:.1f}% > 20%")
            
            if bias is not None and abs(bias) > 25:
                alerts.append(f"[CRIT] CRITICAL: {fid} |Bias|={abs(bias):.1f}% > 25%")
            elif bias is not None and abs(bias) > 10:
                alerts.append(f"[WARN] WARNING: {fid} |Bias|={abs(bias):.1f}% > 10%")
        
        print(f"[OK] Loaded FORMULA_REGISTRY: {len(registry.get('formulaRegistry', {}).get('formulas', {}))} formulas")
        for alert in alerts:
            print(f"  {alert}")
        
        return {"formulas": registry, "alerts": alerts}
    
    except Exception as e:
        print(f"[ERROR] ERROR loading FORMULA_REGISTRY: {e}")
        return {"formulas": {}, "alerts": [f"Load error: {e}"]}


def load_coefficient_database() -> Dict:
    """Load COEFFICIENT_DATABASE.json and check coefficient health"""
    if not COEFFICIENT_DATABASE.exists():
        print(f"[WARN] WARNING: COEFFICIENT_DATABASE.json not found at {COEFFICIENT_DATABASE}")
        return {"coefficients": {}, "alerts": ["COEFFICIENT_DATABASE.json missing"]}
    
    try:
        with open(COEFFICIENT_DATABASE, 'r', encoding='utf-8') as f:
            db = json.load(f)
        
        alerts = []
        for cid, coef in db.get("coefficientDatabase", {}).get("coefficients", {}).items():
            status = coef.get("calibration", {}).get("status", "UNKNOWN")
            if status == "UNCALIBRATED":
                alerts.append(f"[NOTE] NOTICE: {cid} is UNCALIBRATED")
            elif status == "ESTIMATED":
                alerts.append(f"[NOTE] NOTICE: {cid} is ESTIMATED (not calibrated)")
        
        print(f"[OK] Loaded COEFFICIENT_DATABASE: {len(db.get('coefficientDatabase', {}).get('coefficients', {}))} coefficients")
        for alert in alerts[:5]:  # Limit alerts shown
            print(f"  {alert}")
        if len(alerts) > 5:
            print(f"  ... and {len(alerts) - 5} more notices")
        
        return {"coefficients": db, "alerts": alerts}
    
    except Exception as e:
        print(f"[ERROR] ERROR loading COEFFICIENT_DATABASE: {e}")
        return {"coefficients": {}, "alerts": [f"Load error: {e}"]}


def log_prediction(prediction: Dict) -> bool:
    """Log a prediction to PREDICTION_LOG.json for calibration tracking"""
    try:
        if PREDICTION_LOG.exists():
            with open(PREDICTION_LOG, 'r', encoding='utf-8') as f:
                log = json.load(f)
        else:
            log = {"predictions": [], "metadata": {"version": "1.0"}}
        
        # Generate prediction ID
        today = datetime.now().strftime("%Y%m%d")
        existing_today = [p for p in log["predictions"] if p.get("id", "").startswith(f"PRED-{today}")]
        pred_id = f"PRED-{today}-{len(existing_today) + 1:03d}"
        
        prediction["id"] = pred_id
        prediction["timestamp"] = datetime.now().isoformat()
        prediction["status"] = "PENDING_ACTUAL"
        
        log["predictions"].append(prediction)
        log["metadata"]["lastUpdated"] = datetime.now().isoformat()
        log["metadata"]["totalPredictions"] = len(log["predictions"])
        
        with open(PREDICTION_LOG, 'w', encoding='utf-8') as f:
            json.dump(log, f, indent=2)
        
        print(f"[OK] Logged prediction {pred_id}")
        return True
...
def update_prediction_actual(pred_id: str, actual: Dict) -> bool:
    """Update a prediction with actual values and compute residuals"""
    try:
        if not PREDICTION_LOG.exists():
            print(f"[WARN] PREDICTION_LOG.json not found")
            return False
        ...
        for pred in log["predictions"]:
            if pred.get("id") == pred_id:
                ...
                # Compute residuals if we have both predicted and actual
                if "predicted" in pred and "value" in pred["predicted"]:
                    pred_val = pred["predicted"]["value"]
                    act_val = actual.get("value", actual) if isinstance(actual, dict) else actual
                    
                    if isinstance(pred_val, (int, float)) and isinstance(act_val, (int, float)):
                        pred["residual"] = pred_val - act_val
                        if act_val != 0:
                            pred["percentError"] = 100 * abs(pred_val - act_val) / abs(act_val)
                
                pred["status"] = "COMPLETE"
                pred["completedAt"] = datetime.now().isoformat()
                
                with open(PREDICTION_LOG, 'w', encoding='utf-8') as f:
                    json.dump(log, f, indent=2)
                
                print(f"[OK] Updated prediction {pred_id} with actual")
                return True
        
        print(f"[WARN] Prediction {pred_id} not found")
        return False
    
    except Exception as e:
        print(f"[ERROR] ERROR updating prediction: {e}")
        return False


def check_mathematical_rigor() -> Dict:
    """Check M(x) mathematical rigor score"""
    formula_result = load_formula_registry()
    coef_result = load_coefficient_database()
    
    # U = Uncertainty coverage (are outputs with ± bounds?)
    # D = Dimensional consistency (units verified)
    # E = Evolution compliance (formulas calibrated)
    # V = Verification coverage (proofs provided)
    
    critical_alerts = [a for a in formula_result["alerts"] + coef_result["alerts"] if "CRITICAL" in a]
    warning_alerts = [a for a in formula_result["alerts"] + coef_result["alerts"] if "WARNING" in a]
    
    # Simple scoring based on alerts
    e_score = 1.0 - (len(critical_alerts) * 0.3 + len(warning_alerts) * 0.1)
    e_score = max(0.0, min(1.0, e_score))
    
    m_score = e_score  # Simplified - full implementation would compute U, D, E, V
    
    status = "HEALTHY" if m_score >= 0.60 else "BLOCKED"
    
    return {
        "m_score": round(m_score, 2),
        "status": status,
        "gate_passed": m_score >= 0.60,
        "critical_alerts": critical_alerts,
        "warning_alerts": warning_alerts,
        "message": f"M(x) = {m_score:.2f} {'>=' if m_score >= 0.60 else '<'} 0.60 -> {status}"
    }
...
def list_agents():
    """Print all 58 agents"""
    print("\n" + "="*70)
    print("PRISM UNIFIED SYSTEM v5.0 - 58 SPECIALIZED AGENTS")
    print("="*70)
    ...
    print(f"NEW in v5.0: 4 mathematical infrastructure agents")
    print(f"  - formula_evolutionist: Manage formula lifecycle and calibration")
    print(f"  - calibration_monitor: Track prediction accuracy, flag degradation")
    print(f"  - mathplan_validator: Verify MATHPLAN gate before execution")
    print(f"  - prediction_logger: Log predictions for calibration tracking")
    print("="*70 + "\n")


def main():
    # v5.0: Load mathematical infrastructure at startup
    print("\n" + "="*70)
    print("PRISM UNIFIED SYSTEM v5.0 - MATHEMATICAL CERTAINTY")
    print("="*70)
    rigor_check = check_mathematical_rigor()
    print(f"\n{rigor_check['message']}")
    if not rigor_check['gate_passed']:
        print("[WARN] WARNING: Mathematical rigor gate not passed. Some operations may be blocked.")
    print("="*70 + "\n")
    ...
    if len(sys.argv) < 2:
        ...
        print("""
USAGE:
  python prism_unified_system_v5.py --list              List all 58 agents
  python prism_unified_system_v5.py --intelligent "<prompt>"   Auto-detect, auto-context, auto-learn
  python prism_unified_system_v5.py --manufacturing <material> <operation>
  python prism_unified_system_v5.py --ralph <role> "<prompt>" [max_iterations]
  python prism_unified_system_v5.py --rigor             Check mathematical rigor score
  python prism_unified_system_v5.py <task_file.json>

NEW IN v5.0:
  --rigor                Check M(x) mathematical rigor score and formula health
  
  Mathematical Infrastructure:
  • FORMULA_REGISTRY.json loaded at startup (15+ formulas)
  • COEFFICIENT_DATABASE.json loaded at startup (23+ coefficients)
  • PREDICTION_LOG.json tracks all predictions for calibration
  • M(x) >= 0.60 required (rigor gate)
  • Calibration alerts for stale or degraded formulas

EXAMPLES:
  python prism_unified_system_v5.py --intelligent "Extract all aluminum alloys from monolith"
  python prism_unified_system_v5.py --manufacturing "Inconel 718" "roughing"
  python prism_unified_system_v5.py --ralph architect "Design X. Output COMPLETE when done." 10
  python prism_unified_system_v5.py --rigor
""")
        return
    ...
    if cmd == "--list":
        ...
        # v5.0: Check mathematical rigor
        result = check_mathematical_rigor()
        print("\n" + "="*70)
        print("MATHEMATICAL RIGOR CHECK (M(x))")
        print("="*70)
        print(f"\nM(x) Score: {result['m_score']:.2f}")
