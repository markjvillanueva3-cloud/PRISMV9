...
function makeStubAdapters() {
  return {
    ...
    // byte-identical inline impls (stub + live). Fail-loud type guard chains to
    // the pipeline's recordEvent try/catch -> record stage FAILED (no silent loss).
    recordEvent: async (event) => appendAccuracyEvent(event, { path: DEFAULT_EVENTS_FILE }),
  };
}
