...
function parseArgs(argv) {
  const out = { roots: [], ollama: false, noOllama: false, limit: 0, ollamaLimit: 200, dryRun: false, stateOnly: false, emitOnly: false, rebuildState: false,
  ...
  for (const a of argv.slice(2)) {
    ...
    else if (a === "--rebuild-state-from-index") out.rebuildState = true;
...
// --- Resumable state: JSONL store (streams line-by-line; immune to V8's 512MB
// single-string limit that killed the old monolithic CONTENT_INDEX_STATE.json at
// 1.34M entries -- JSON.stringify/JSON.parse of the whole object threw
// "Cannot create a string longer than 0x1fffffe8"). One line per entry:
// {"key":"<source>::<rel>", sig, note, summary, summarySource, role, domain}.
const STATE_JSONL = "CONTENT_INDEX_STATE.jsonl";
const STATE_LEGACY = "CONTENT_INDEX_STATE.json";
const STATE_MIGRATE_MAX = 480 * BYTES_PER_MB; // largest legacy .json we dare JSON.parse

/** Stream-load JSONL state into a Map (never materializes the file as one string). */
export function loadStateJsonl(file) {
  return new Promise((resolve, reject) => {
    const entries = new Map();
    if (!fs.existsSync(file)) { resolve(entries); return; }
    const rl = readline.createInterface({ input: fs.createReadStream(file), crlfDelay: Infinity });
    rl.on("line", (line) => {
      if (!line) return;
      let o; try { o = JSON.parse(line); } catch { return; }
      if (o && o.key) { const { key, ...rest } = o; entries.set(key, rest); }
    });
...
/** Atomically stream-write the entries Map to JSONL (temp + rename; awaits backpressure). */
export async function saveStateJsonl(file, entries) {
  const tmp = `${file}.tmp`;
  const ws = fs.createWriteStream(tmp);
  for (const [key, v] of entries) {
    if (!ws.write(JSON.stringify({ key, ...v }) + "\n")) await new Promise(res => ws.once("drain", res));
  }
...
/**
 * Load resumable state, preferring the JSONL store. One-time forward-migrate a
 * *readable* legacy monolith; a legacy .json past the string wall is skipped with
 * a hint to run --rebuild-state-from-index (which recovers from content-index.jsonl).
 */
export async function loadState(outDir) {
  const jsonl = path.join(outDir, STATE_JSONL);
  if (fs.existsSync(jsonl)) return loadStateJsonl(jsonl);
  const legacy = path.join(outDir, STATE_LEGACY);
  if (fs.existsSync(legacy)) {
    const sz = fs.statSync(legacy).size;
    if (sz < STATE_MIGRATE_MAX) {
      try {
        const obj = JSON.parse(fs.readFileSync(legacy, "utf8"));
        const m = new Map(Object.entries(obj.entries || {}));
        await saveStateJsonl(jsonl, m);
        return m;
      } catch { /* fall through to empty */ }
    } else {
      process.stderr.write(`[state] legacy ${STATE_LEGACY} is ${(sz / BYTES_PER_MB).toFixed(0)}MB (past V8 512MB string wall) -- run --rebuild-state-from-index to recover from content-index.jsonl\n`);
    }
  }
  return new Map();
}
...
/**
 * Rebuild all outputs from the accumulated state alone (no directory walk).
 * Iterates the entries Map and accumulates only the (small) per-role markdown line
 * buffers, streaming content-index.jsonl out, so even a 1.34M-entry corpus stays
 * well under a generous heap. Used after a batched/resumed population run.
 */
export async function emitFromState(entries, args) {
  ensureDir(args.out);
  ...
  const jsonlDone = new Promise((res, rej) => { jsonlStream.on("finish", res); jsonlStream.on("error", rej); });
  const roleBuf = {};
  ...
  for (const [key, v] of entries) {
    ...
    seen.add(abs);
    if (!jsonlStream.write(JSON.stringify({ source, path: rel, role: v.role, domain: v.domain, sizeMB: v.sizeMB ?? 0, note: v.note, summary: v.summary, summarySource: v.summarySource, sig: v.sig }) + "\n")) {
      await new Promise(res => jsonlStream.once("drain", res));
    }
  ...
  await jsonlDone;   // ensure content-index.jsonl is fully flushed before we report done
...
async function main() {
  ...
  // --rebuild-state-from-index: recover the resumable state.jsonl by STREAMING the
  // (readable) content-index.jsonl. The migration bridge -- the old 544MB monolithic
  // CONTENT_INDEX_STATE.json is itself past the 512MB string wall and unreadable, so
  // the jsonl index (which carries note/summary/role/domain per file) is the recovery
  // source. sig is preserved when present in the row, else "" (next run re-stats).
  if (args.rebuildState) {
    const idx = path.join(args.out, "content-index.jsonl");
    if (!fs.existsSync(idx)) { process.stderr.write("rebuild: no content-index.jsonl present\n"); process.exit(1); }
    const rebuilt = await new Promise((resolve, reject) => {
      const m = new Map();
      const rl = readline.createInterface({ input: fs.createReadStream(idx), crlfDelay: Infinity });
      rl.on("line", (line) => {
        if (!line) return;
        let r; try { r = JSON.parse(line); } catch { return; }
        if (!r || !r.path) return;
        m.set(`${r.source || ""}::${r.path}`, { sig: r.sig || "", note: r.note, summary: r.summary || null, summarySource: r.summarySource, role: r.role, domain: r.domain, sizeMB: r.sizeMB });
      });
      rl.on("close", () => resolve(m));
      rl.on("error", reject);
    });
    await saveStateJsonl(path.join(args.out, STATE_JSONL), rebuilt);
    process.stdout.write(JSON.stringify({ ok: true, rebuilt: rebuilt.size, to: STATE_JSONL }) + "\n");
    return;
  }

  // --emit-only: rebuild jsonl + vault notes from the accumulated state alone
  // (no re-walk). loadState STREAMS the JSONL store, so a 1.34M-entry corpus never
  // materializes a single giant string -- the exact wall that broke the old path.
  if (args.emitOnly) {
    const entries = await loadState(args.out);
    if (!entries.size) { process.stderr.write("emit-only: no state (run a population pass or --rebuild-state-from-index)\n"); process.exit(1); }
    await emitFromState(entries, args);
    return;
  }
  ...
  ensureDir(args.out);
  const stateFile = path.join(args.out, STATE_JSONL);
  const entries = await loadState(args.out);
  ...
  for (const root of roots) {
    ...
    for (const full of files) {
      ...
      entries.set(key, { sig, note, summary, summarySource, role, domain, sizeMB: Number(sizeMB) });
      rows.push({ source, path: rel, role, domain, sizeMB: Number(sizeMB), note, summary, summarySource, sig });

      if (!args.dryRun && counts.processed % CHECKPOINT_EVERY === 0) {
        try { await saveStateJsonl(stateFile, entries); } catch { /* keep going */ }
  ...
  if (args.stateOnly) { await saveStateJsonl(stateFile, entries); process.stdout.write(JSON.stringify(summaryObj) + "\n"); return; }

  // Write outputs
  await saveStateJsonl(stateFile, entries);
