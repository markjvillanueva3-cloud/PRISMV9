 ...
 *     degrees:{ in:[int], out:[int], maxIn:int } }
 *
 * DEGREES (schema ≥ 1.1.0, U-SIERRA-MASTERINDEX-DEGREE-SIDECAR 2026-07-05):
 * per-node in/out edge counts, index-ALIGNED to `nodes[]` (degrees.in[k] is the
 * in-degree of nodes[k].id). Computed here from the graph's edges[] — which the
 * streaming read HAS in memory — so MasterIndexEngine's sidecar-fallback path can
 * rank utilization + classify hubs/orphans/ghosts WITHOUT ever loading the >512MB
 * graph string. Before this, the fallback served the edgeless sidecar and every
 * degree/utilization/classification caller fail-loud degraded (all-zero would read
 * as "every node is an orphan"). `maxIn` is the graph-wide max in-degree (over ALL
 * edges), matching MasterIndexEngine.buildGraphCache so normalizeUtilization parity
 * holds across the two readers. Edge validity (from+to must be strings) mirrors the
 * engine EXACTLY — divergence would drift degree counts; the parity test pins it.
 *
...
import {
  writeFileSync, statSync, existsSync, renameSync,
} from "node:fs";
...
// 1.1.0 (2026-07-05): added the index-aligned `degrees` block (in/out/maxIn) so
// the master-index sidecar-fallback path can rank utilization + classify nodes
// without loading the >512MB graph. 1.0.0 sidecars (no `degrees`) still load —
// the engine degrades exactly as before until regenerated. See
// U-SIERRA-MASTERINDEX-DEGREE-SIDECAR.
export const SIDECAR_SCHEMA_VERSION = "1.1.0";
...
export function buildGraphIndex(graphObj, meta = {}) {
  ...
  // Degree pass (U-SIERRA-MASTERINDEX-DEGREE-SIDECAR): tally in/out-degree per
  // node id from edges[]. Edge validity mirrors MasterIndexEngine.buildGraphCache
  // EXACTLY (from+to must be strings) so the two readers can never disagree on a
  // degree count. `maxIn` is the graph-wide max in-degree (over every edge) — the
  // same quantity the engine derives while walking edges — so the sidecar path's
  // normalizeUtilization(inDeg, maxIn) matches the raw-graph path bit-for-bit.
  const edges = Array.isArray(graphObj.edges) ? graphObj.edges : [];
  const inDegMap = new Map();
  const outDegMap = new Map();
  let maxIn = 0;
  for (const e of edges) {
    // Shared canonical predicate (graph-edge-validity.mjs) — MUST stay identical
    // to MasterIndexEngine.buildGraphCache's edge guard, else degrees diverge.
    if (!isValidEdge(e)) continue;
    const inN = (inDegMap.get(e.to) ?? 0) + 1;
    inDegMap.set(e.to, inN);
    if (inN > maxIn) maxIn = inN;
    outDegMap.set(e.from, (outDegMap.get(e.from) ?? 0) + 1);
  }

  const nodes = [];
  // Per-node degree arrays, index-ALIGNED to nodes[] (degIn[k] ↔ nodes[k]). Pushed
  // in lockstep with nodes.push below so a skipped/malformed node never misaligns
  // the columns. Postings + the card-offset index already rely on this same order.
  const degIn = [];
  const degOut = [];
  ...
  for (const n of graphObj.nodes) {
    ...
    try {
      ...
      nodes.push(compact);
      // Lockstep with nodes.push — keeps degIn/degOut index-aligned to nodes[].
      degIn.push(inDegMap.get(n.id) ?? 0);
      degOut.push(outDegMap.get(n.id) ?? 0);
  ...
  return {
    ...
    nodes,
    inverted,
    degrees: { in: degIn, out: degOut, maxIn },
  };
}
...
export function writeSidecar(sidecar, outPath) {
  // Per-KEY streaming atomic write (writeGraphStreamingAtomic): `nodes` streams
  // per-element, every other key stringifies alone. A single JSON.stringify(sidecar)
  // hits V8's ~512MB max-string cap once the whole sidecar exceeds it (compact OR
  // pretty — the L230-era comment only covered pretty-print), which would make the
  // sidecar UN-REGENERATABLE past the cap (RangeError → the file freezes stale
  // forever). Streaming caps every intermediate string to its largest single key
  // (nodes[] streams to ~0; `inverted`/`degrees` stringify whole but each is far
  // under the cap), removing the write ceiling. See U-SIERRA-MASTERINDEX-SIDECAR-ROBUSTNESS.
  writeGraphStreamingAtomic(outPath, sidecar);
  try { return statSync(outPath).size; } catch { return 0; }
}
...
export function generate({ graphPath = DEFAULT_GRAPH_PATH, outPath = DEFAULT_OUT_PATH } = {}) {
  ...
  // Degree observability (R12): a live over-cap graph that still emits maxIn===0
  // OR 0 nodes-with-degree means the edges[] read/aligned wrong — surface it in
  // the summary + CLI so a silent degree regression can't hide behind a green run.
  const degIn = Array.isArray(sidecar.degrees?.in) ? sidecar.degrees.in : [];
  let nodesWithDegree = 0;
  for (let k = 0; k < degIn.length; k++) {
    if (degIn[k] || (sidecar.degrees.out[k] ?? 0)) nodesWithDegree++;
  }
  ...
  return {
    ...
    sidecarBytes: bytes,
    degreeMaxIn: sidecar.degrees?.maxIn ?? 0,
    nodesWithDegree,
    offsetIndex,
...
function main() {
  ...
  try {
    ...
    writeBuildBreadcrumb(args.outPath || DEFAULT_OUT_PATH, {
      ...
      degreeMaxIn: r.degreeMaxIn, nodesWithDegree: r.nodesWithDegree,
    });
    process.stdout.write(
      `[build-graph-index] ✓ ${r.nodeCount}/${r.rawNodeCount} nodes indexed `
      + `(${r.skipped} skipped) · ${r.tokenCount} tokens · `
      + `degrees: ${r.nodesWithDegree} nodes wired, maxIn ${r.degreeMaxIn} · `
