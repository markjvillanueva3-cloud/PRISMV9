...
import { deriveGalaxy } from "./tag-lora-corpus-galaxies.mjs";
 ...
 */
export function assembleCorpus(inventory, { readImpl = fs.readFileSync, capPerGalaxy = 0 } = {}) {
  ...
  for (const src of selected) {
    ...
    for (const r of parsed) {
      ...
      else { const g = deriveGalaxy({ source: src.id, instruction: r.instruction }); if (g) row.galaxy = g; } // root-cause tag recovery: galaxy-specific sources (bridge/cad/outcome-bus) get their galaxy; cross-cutting -> untagged (deriveGalaxy returns null)
      rows.push(row);
      added++;
    }
    bySource[src.id] = { rows: parsed.length, added, duplicates: dups, invalid, weight, advisory };
  }
  // Optional per-galaxy cap (opt-in; default 0 = OFF = no behavior change). Prevents one
  // over-represented galaxy from dominating the fine-tune -- e.g. the templated lathe archive
  // landed at ~54% of the corpus (1657/3126). Cross-cutting/untagged rows are NOT capped. All
  // summary stats below are computed on the CAPPED set so reporting matches what is written.
  const capRes = capPerGalaxy > 0 ? capRowsPerGalaxy(rows, capPerGalaxy) : { rows, dropped: 0 };
  const finalRows = capRes.rows;
  const totalRows = finalRows.length;
  const advisoryRows = finalRows.reduce((n, r) => n + (r.advisory ? 1 : 0), 0);
  // Per-galaxy coverage: how many rows carry each galaxy tag. deriveGalaxy recovers galaxy-SPECIFIC
  // tags that producers dropped (bridge/cad/outcome-bus); genuinely cross-cutting doctrine
  // (vault-feedback/wiki-canonical) is deliberately left UNTAGGED -> the splitter's _unclassified
  // track (NOT counted here). This is the "no dormant nodes across all galaxies" surface: a galaxy
  // with 0 rows has no training signal.
  const byGalaxy = {};
  for (const r of finalRows) { if (r.galaxy) byGalaxy[r.galaxy] = (byGalaxy[r.galaxy] || 0) + 1; }
  return {
    rows: finalRows,
    bySource,
    byGalaxy,
    ...
    cappedPerGalaxy: capPerGalaxy > 0 ? capPerGalaxy : null,
    cappedDropped: capRes.dropped,
...
/**
 * Cap each galaxy-tagged row group at `cap` rows; untagged/cross-cutting rows pass through
 * uncapped. When capping, KEEP the highest-`weight` rows (verified w=1.0 before advisory w=0.5)
 * -- a flat "first N by source order" is weight-blind and can drop a galaxy's lone verified row
 * before advisory ones (3-of-3 arm-B finding). Ties (incl. weightless rows) keep original source
 * order, so the no-weight path is byte-identical to a simple first-N cap. Output preserves the
 * original row order (only drops the over-cap lowest-weight rows). Returns { rows, dropped }. Pure.
 */
export function capRowsPerGalaxy(rows, cap) {
  if (!Number.isFinite(cap) || cap <= 0) return { rows, dropped: 0 };
  // Pass 1: per galaxy, pick the kept indices = top-`cap` by weight desc, ties by source order.
  const byGalaxy = new Map();
  rows.forEach((r, idx) => {
    const g = r && r.galaxy;
    if (!g) return;
    if (!byGalaxy.has(g)) byGalaxy.set(g, []);
    byGalaxy.get(g).push({ idx, weight: typeof r.weight === "number" ? r.weight : 0 });
  });
  const keep = new Set();
  for (const arr of byGalaxy.values()) {
    arr.sort((a, b) => (b.weight - a.weight) || (a.idx - b.idx)); // weight desc, then source order
    for (let i = 0; i < Math.min(cap, arr.length); i++) keep.add(arr[i].idx);
  }
  // Pass 2: emit in original order; untagged pass through, tagged kept iff in the top-`cap` set.
  const out = [];
  let dropped = 0;
  rows.forEach((r, idx) => {
    if (!(r && r.galaxy)) { out.push(r); return; }
...
function parseArgs(argv) {
  const out = { json: false, write: false, outPath: DEFAULT_OUT, capPerGalaxy: 0 };
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === "--json") out.json = true;
    else if (a === "--out") { out.write = true; const nxt = argv[i + 1]; if (nxt && !nxt.startsWith("--")) out.outPath = argv[++i]; }
    else if (a.startsWith("--cap-per-galaxy=")) out.capPerGalaxy = Math.max(0, parseInt(a.split("=")[1], 10) || 0);
    else if (a === "--help" || a === "-h") {
      console.error("usage: assemble-fleet-lora-corpus [--json] [--out [<path.jsonl>]] [--cap-per-galaxy=N]");
      process.exit(0);
    }
  }
  return out;
...
export function main() {
  ...
  const result = assembleCorpus(inventory, { capPerGalaxy: opts.capPerGalaxy });
  ...
  if (opts.json) {
    ...
    if (result.cappedPerGalaxy) console.log(`per-galaxy cap: ${result.cappedPerGalaxy} -> dropped ${result.cappedDropped} over-represented row(s)`);
