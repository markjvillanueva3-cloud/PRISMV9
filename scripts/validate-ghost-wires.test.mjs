...
test("validate() classifies a mixed batch and emits overlay + outcomes", () => {
  ...
  // U-VIZ-GHOST-WIRE-STRENGTH integration: the one confirmation matched a real
  // `import { MillEngine }` line -> graded strong, and nothing is weak.
  assert.equal(result.counts.weakConfirmed, 0);
  const confirmedOutcome = result.outcomes.find((o) => o.ghostId === "ghost.unwired.MillEngine");
  assert.equal(confirmedOutcome.validation.confirmationStrength, "strong");
...
// ============================================================================
// U-VIZ-GHOST-WIRE-STRENGTH: confirmation grading (comment-only = weak).
// A ghost marked "confirmed" only because the engine name appears in a COMMENT
// is a false ground-truth label that pollutes the GNN ref pool. These tests pin
// the strong/weak grading + the comment-aware stripper that powers it.
// ============================================================================

// ---------- stripComments: line / block / string-literal awareness ----------
test("stripComments removes line comments but keeps code tokens", () => {
  const out = stripComments("const a = 1; // FooEngine pending wire\nconst b = 2;");
  assert.ok(/\bconst\b/.test(out));
  assert.ok(/\bb\b/.test(out));
  assert.equal(/\bFooEngine\b/.test(out), false, "name inside a line comment must be stripped");
});

test("stripComments removes block comments (incl. multi-line)", () => {
  const out = stripComments("x /* BarEngine\n still in block */ y");
  assert.ok(/\bx\b/.test(out));
  assert.ok(/\by\b/.test(out));
  assert.equal(/\bBarEngine\b/.test(out), false, "name inside a block comment must be stripped");
});

test("stripComments does NOT strip a // that lives inside a string literal", () => {
  const src = 'const url = "http://x//y"; // RealComment';
  const out = stripComments(src);
  assert.ok(out.includes("http://x//y"), "string-literal slashes preserved");
  assert.equal(/\bRealComment\b/.test(out), false, "trailing real comment stripped");
});

test("stripComments handles escaped quotes inside strings", () => {
  const src = 'const s = "a\\"b // notacomment"; // gone';
  const out = stripComments(src);
  assert.ok(out.includes("notacomment"), "content after escaped quote stays inside the string");
  assert.equal(/\bgone\b/.test(out), false);
});

test("stripComments returns empty string on non-string input (R12 graceful)", () => {
  assert.equal(stripComments(null), "");
  assert.equal(stripComments(undefined), "");
  assert.equal(stripComments(42), "");
});

// ---------- analyzeConfirmationContext: strong vs weak ----------
test("analyzeConfirmationContext grades a comment-only mention as weak", () => {
  const ctx = analyzeConfirmationContext("FooEngine", "// planned: wire FooEngine here later\nconst x = 1;");
  assert.equal(ctx.strength, "weak");
  assert.equal(ctx.codeMatch, false);
  assert.equal(ctx.commentMatch, true);
  assert.equal(ctx.inImport, false);
});

test("analyzeConfirmationContext grades an import reference as strong + inImport", () => {
  const ctx = analyzeConfirmationContext("FooEngine", 'import { FooEngine } from "../engines/FooEngine.js";');
  assert.equal(ctx.strength, "strong");
  assert.equal(ctx.codeMatch, true);
  assert.equal(ctx.inImport, true);
});

test("analyzeConfirmationContext grades a bare code reference as strong (not import)", () => {
  const ctx = analyzeConfirmationContext("FooEngine", "result = FooEngine.run(params);");
  assert.equal(ctx.strength, "strong");
  assert.equal(ctx.codeMatch, true);
  assert.equal(ctx.inImport, false);
});

test("analyzeConfirmationContext: a code reference wins even if a comment also mentions it", () => {
  const ctx = analyzeConfirmationContext("FooEngine", "// see FooEngine\nimport { FooEngine } from \"x\";");
  assert.equal(ctx.strength, "strong");
  assert.equal(ctx.codeMatch, true);
  assert.equal(ctx.inImport, true);
});

test("analyzeConfirmationContext word-boundary: MillEngine not strong on WindMillEngine", () => {
  const ctx = analyzeConfirmationContext("MillEngine", 'import { WindMillEngine } from "x";');
  assert.equal(ctx.codeMatch, false, "substring of a longer token is not a match");
  assert.equal(ctx.strength, "weak");
});

// ---------- classifyGhostWire: confirmed carries the strength grade ----------
test("classifyGhostWire confirms a comment-only mention as WEAK", () => {
  const r = classifyGhostWire({
    engineName: "FooEngine",
    proposedAt: new Date(NOW).toISOString(),
    dispatcherText: "// FooEngine will be wired in a follow-up\nexport const x = 1;",
    now: NOW,
  });
  assert.equal(r.status, STATUS_CONFIRMED, "status stays confirmed for backward-compat");
  assert.equal(r.confirmationStrength, "weak");
  assert.equal(r.reason, "engine-referenced-comment-only");
  assert.equal(r.evidence.commentMatch, true);
  assert.equal(r.evidence.codeMatch, false);
});

test("classifyGhostWire confirms a real import as STRONG", () => {
  const r = classifyGhostWire({
    engineName: "FooEngine",
    proposedAt: new Date(NOW).toISOString(),
    dispatcherText: 'import { fooEngine } from "x"; case "a": { const { FooEngine } = await import("y"); }',
    now: NOW,
  });
  assert.equal(r.status, STATUS_CONFIRMED);
  assert.equal(r.confirmationStrength, "strong");
  assert.equal(r.reason, "engine-referenced");
  assert.equal(r.evidence.codeMatch, true);
});

// ---------- validate(): weakConfirmed subset counter + overlay strength ----------
test("validate() counts a comment-only confirmation as weakConfirmed (subset of confirmed)", () => {
  const graph = {
    nodes: [
      {
        id: "ghost.unwired.CommentOnlyEngine",
        kind: "ghost.unwired-engine",
        label: "CommentOnlyEngine",
        proposed_at: new Date(NOW - 5 * ONE_DAY_MS).toISOString(),
        proposed_wiring: "prism_calc",
        confidence: 0.7,
      },
      {
        id: "ghost.unwired.RealWiredEngine",
        kind: "ghost.unwired-engine",
        label: "RealWiredEngine",
        proposed_at: new Date(NOW - 5 * ONE_DAY_MS).toISOString(),
        proposed_wiring: "prism_calc",
        confidence: 0.8,
      },
  ...
  const dispatcherIndex = new Map([["calcdispatcher", "calcDispatcher.ts"]]);
  // CommentOnlyEngine appears ONLY in a comment; RealWiredEngine in a real import.
  const dispatcherReader = () =>
    `// CommentOnlyEngine is a candidate, not yet wired\nimport { RealWiredEngine } from "../engines/RealWiredEngine.js";`;
  const result = validate({ graph, dispatcherIndex, dispatcherReader, now: NOW });

  assert.equal(result.counts.confirmed, 2, "both still count as confirmed (status unchanged)");
  assert.equal(result.counts.weakConfirmed, 1, "exactly the comment-only one is weak");

  const byId = Object.fromEntries(result.outcomes.map((o) => [o.ghostId, o]));
  assert.equal(byId["ghost.unwired.CommentOnlyEngine"].validation.confirmationStrength, "weak");
  assert.equal(byId["ghost.unwired.RealWiredEngine"].validation.confirmationStrength, "strong");

  // Overlay annotation carries the strength for the client-side painter.
  assert.equal(
    result.augmentation.annotations["ghost.unwired.CommentOnlyEngine"].ghost_wire_confirmation_strength,
    "weak"
  );
  assert.equal(
    result.augmentation.annotations["ghost.unwired.RealWiredEngine"].ghost_wire_confirmation_strength,
    "strong"
  );
});

test("validate() sets ghost_wire_confirmation_strength=null for non-confirmed ghosts", () => {
  const graph = {
    nodes: [
      {
        id: "ghost.unwired.PendingEngine",
        kind: "ghost.unwired-engine",
        label: "PendingEngine",
        proposed_at: new Date(NOW - 3 * ONE_DAY_MS).toISOString(),
        proposed_wiring: "prism_calc",
      },
    ],
  };
  const dispatcherIndex = new Map([["calcdispatcher", "calcDispatcher.ts"]]);
  const dispatcherReader = () => "nothing relevant here";
  const result = validate({ graph, dispatcherIndex, dispatcherReader, now: NOW });
  assert.equal(result.counts.pending, 1);
  assert.equal(result.counts.weakConfirmed, 0);
  assert.equal(
    result.augmentation.annotations["ghost.unwired.PendingEngine"].ghost_wire_confirmation_strength,
    null
  );
});
