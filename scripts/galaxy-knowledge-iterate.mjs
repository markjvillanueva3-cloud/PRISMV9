...
function hermesResearch(entry, iter, prior) {
  ...
  // --json so we learn the ACTUAL answering source (hermes vs ollama-fallback) and label the anchor
  // truthfully (R12). Fallback stays ON for resilience, but the provenance is never faked.
  let out;
  try {
    out = execFileSync(process.execPath, [HERMES, "ask", q, "--json", "--timeout", String(HERMES_TIMEOUT)], {
      cwd: REPO,
    ...
    // FAIL LOUD -- planner unreachable (Hermes failed AND Ollama fallback failed => exit 3). Never
    // fabricate an iteration.
    throw new Error(`hermesResearch(${entry.galaxy}): planner call failed (exit ${e.status ?? "?"}): ${String(e.message).slice(0, 200)}`);
  }
  let parsed;
  try {
    ...
    throw new Error(`hermesResearch(${entry.galaxy}): unparseable planner --json output: ${out.slice(0, 200)}`);
  }
  const content = String(parsed.content || "");
  const source = parsed.source || "unknown"; // "hermes" | "ollama-fallback" | "unknown"
  if (!content.trim()) throw new Error(`hermesResearch(${entry.galaxy}): empty planner content (source=${source})`);
  return { knowledge: content, sources: parseHermes(content).sources, source };
}

function depositAnchor(entry, iter, knowledge, sources, source) {
  if (!existsSync(VAULT_REF)) mkdirSync(VAULT_REF, { recursive: true });
  const sourceLabel =
    source === "hermes" ? "Hermes (xAI Grok)" : source === "ollama-fallback" ? "Ollama local fallback (Hermes was unreachable)" : String(source || "unknown planner");
  const slug = `reference_${entry.galaxy}_iter${iter}_deepsource_2026_06_14`;
  const file = join(VAULT_REF, `${slug}.md`);
  const body = `---
name: ${slug}
description: "Knowledge-accretion iteration ${iter} for the ${entry.galaxy} galaxy (owner:${entry.owner}). ${sourceLabel}-DRAFTED next-layer reputable-source research (courses/textbooks/standards/gov-reports/seminars/articles). DRAFT: citations are model-recalled (source=${source}), PENDING WebFetch-promotion by a session deepen-workflow pass. ${entry.physics ? "Physics-safe: method/standards depth only, no cutting constants." : ""} Written 2026-06-14 by galaxy-knowledge-iterate.mjs (cron tier)."
metadata:
  node_type: memory
  type: reference
...
## Context (DRAFT -- ${sourceLabel} tier, pending WebFetch-promotion, R12)
Deep-research iteration **${iter}** for the **${entry.galaxy}** galaxy. Drafted by **${sourceLabel}** (source=\`${source}\`) via the
...
function main() {
  ...
  // Record an externally-confirmed (Workflow-tier) iteration into the ledger. lockedUpdate reloads the
  // ledger fresh INSIDE an exclusive lock so a concurrent cron tick can never lose this write.
  if (args.record) {
    const g = args.record;
    const sources = args.sources ? args.sources.split(",").map((x) => x.trim()).filter(Boolean) : [];
    const res = lockedUpdate(LEDGER_PATH, (fresh) => {
      if (!fresh.galaxies[g]) throw new Error(`--record: unknown galaxy '${g}'`);
      recordIteration(fresh, g, { sources, courseBookSources: args.course || 0, note: args.note || (args.confirmed ? "WebFetch-confirmed (Workflow tier)" : "recorded"), at });
      return { iter: fresh.galaxies[g].iterations, newSources: fresh.galaxies[g].history.at(-1).newSources, saturated: isSaturated(fresh, g) };
    }, { at, galaxies: GALAXIES });
    log(`recorded ${g} -> iter ${res.iter} (newSrc this iter ${res.newSources}) · saturated=${res.saturated}`);
    return;
  }
  ...
  for (const e of picks) {
    const iter = e.iterations + 1; // best-effort for the anchor filename (recordIteration re-derives from fresh)
    const iterAt = new Date().toISOString(); // per-galaxy timestamp (a --count run can span minutes)
    log(`iterate ${e.galaxy} (iteration ${iter})...`);
    // Hermes call is slow (~90s) -> kept OUTSIDE the lock. Only the short record+save is locked, and it
    // reloads the ledger fresh so a concurrent --record / cron tick is never lost (scrutiny P1).
    const { knowledge, sources, source } = hermesResearch(e, iter, e.sourceKeys);
    const file = depositAnchor(e, iter, knowledge, sources, source);
    const res = lockedUpdate(LEDGER_PATH, (fresh) => {
      recordIteration(fresh, e.galaxy, { sources, note: `${source} draft -> ${file}`, at: iterAt });
      const ent = fresh.galaxies[e.galaxy];
      return { iter: ent.iterations, newSources: ent.history.at(-1).newSources, saturated: isSaturated(fresh, e.galaxy) };
    }, { at: iterAt, galaxies: GALAXIES });
    log(`  ${e.galaxy}: iter ${res.iter}, ${res.newSources} new src [${source}], anchor ${file}, saturated=${res.saturated}`);
  }
  // Re-load for the closing summary: the top-level `ledger` is stale (each lockedUpdate mutated a
  // freshly-reloaded copy on disk, not this in-memory object), so summarize the authoritative on-disk state.
  log(JSON.stringify(summary(loadLedger(LEDGER_PATH) || ledger)));
}
