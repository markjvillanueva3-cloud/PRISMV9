...
const SOURCES = [
  ...
  // CAD manufacturing DECIPHER -> LoRA (U-DELTA-CAD-DECIPHER-LORA, slot:delta 2026-07-01). The corpus
  // decipher pipeline (cad-part-decipher.mjs deterministic 93% + cad-part-decipher-hermes.mjs residual)
  // turns every kernel-validated real part into {shape/family, stock, workholding}; this feeds it into
  // the fleet LoRA corpus so the model learns the manufacturing decipher (add finishing stock + choose
  // workholding). Grounded per REAL part (real name/class/kernel envelope + computed allowance / live
  // Hermes verdict); rows the pipeline could not decide (needsReasoning-without-Hermes, or 'unsure')
  // are SKIPPED, never trained on a guess. Advisory because ~7% of rows are unreviewed-cron Hermes
  // verdicts (down-weighted like the other LLM-asserted sources; the deterministic majority is rule-grounded).
  {
    id: 'cad-decipher-mfg-lora',
    kind: 'lora-training-jsonl',
    path: 'H:/prism/state/shared/lora/cad-decipher-mfg-dataset.jsonl',
    description: 'CAD manufacturing decipher (stock + workholding per real kernel-validated part) -> Alpaca pairs. 93% deterministic machining-allowance rules ($0) + Hermes residual (each row tagged source:cad-decipher-deterministic|-hermes). Producer: node scripts/build-cad-decipher-lora.mjs --write (U-DELTA-CAD-DECIPHER-LORA, slot:delta 2026-07-01).',
    domains: ['cad'],
    advisory: true, // contains unreviewed-cron Hermes verdicts (~7%) -> down-weight; deterministic majority is rule-grounded
  },
  // CAD text->code GEN outcomes -> LoRA (U-CADGEN-LORA-EMITTER, slot:delta 2026-07-03). The closed-loop
  // text->CadQuery generator (cad-text-to-cadquery.mjs) RUNS each generated script (cadquery 2.8.0 now
  // installed) and records the verified pass/fail/error verdict into cad-failure-ledger.jsonl (via
  // CADTrialErrorLearningEngine, which already reads it back into the next prompt). This turns every
  // VERIFIED-PASS generation into a request->working-code Alpaca pair -- so the model learns to WRITE
  // correct parametric CAD from its OWN successful runs. It is the missing GEN->LoRA hop (build-cad-
  // decipher-lora feeds the DECIPHER corpus, not gen). fail/error rows are skipped (no verified fix --
  // never train a guess); every emitted output passes codeInvalidReason, which keeps the known 25.4x-
  // undersize units bug (dimension divided by IN) OUT of the training set (live: 40/184 pass rows excluded).
  {
    id: 'cadgen-outcome-lora',
    kind: 'lora-training-jsonl',
    path: 'H:/prism/state/shared/lora/cadgen-outcome-dataset.jsonl',
    description: 'CAD text->CadQuery GEN outcomes: ledger-verified-pass generations -> request->working-code Alpaca pairs (fail/error rows + 25.4x-undersize-bug code excluded). Producer: node scripts/build-cadgen-lora-dataset.mjs --write (U-CADGEN-LORA-EMITTER, slot:delta 2026-07-03).',
    domains: ['cad'],
    advisory: false, // executed + ledger-verified WORKING code (NOT LLM-asserted knowledge) -- full weight
  },
  ...
  // Delta CAD vec2d DXF training lane (U-DELTA-VEC2D-TRAINING-LANE, slot:delta 2026-07-02, Domain 10).
  // Closes the vec2d gap: 9,280 readable .dxf drawings (of 9,527; 247 .dwg fail-loud) had a
  // live-validated reader but no training lane. DXF group-42 gives EXACT dims (with $INSUNITS units) --
  // a higher-quality signal than OCR weak-labels. Producer: scripts/build-cad-vec2d-dataset.mjs
  // (ledger from scripts/vec2d-to-training.mjs). ADVISORY: deterministic geometry extract, down-weighted 0.5.
  {
    id: 'cad-vec2d-training',
    kind: 'lora-training-jsonl',
    path: 'H:/prism/state/shared/lora/cad-vec2d-training-dataset.jsonl',
    description: 'Delta vec2d DXF dimension+units training pairs (raw DIMENSION values + $INSUNITS state -> normalized callouts + units-handling rule, advisory). Producer: scripts/build-cad-vec2d-dataset.mjs.',
    domains: ['cad', 'cad-vec2d'],
    advisory: true, // deterministic geometry extract (not hand-authored doctrine) -- down-weighted
  },
