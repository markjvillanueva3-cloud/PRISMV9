...
const GOAL_DOMAINS = ["mill", "post", "cad", "cam", "wedm", "lathe", "speed-feed", "academy", "business", "quoting"];

// ---- registry integrity ----

test("DOMAINS covers exactly the 10 domains, distinct, fully specified", () => {
  assert.equal(DOMAINS.length, 10);
  const keys = DOMAINS.map((d) => d.key);
  assert.deepEqual([...keys].sort(), [...GOAL_DOMAINS].sort());
  assert.equal(new Set(keys).size, 10, "domain keys distinct");
  ...
  assert.equal(new Set(DOMAINS.map((d) => d.hook)).size, 10, "hook filenames distinct");
  assert.ok(BUNDLE.includes("ups-domain-bundle"), "bundle path");
});

// ---- LIVE: the whole layer fires + is wired (the goal's core guarantee) ----

test("LIVE: all 10 domains import OK, fire live, and are bundle-wired", async () => {
  const cov = await buildCoverage();
  assert.equal(cov.rows.length, 10);
...
test("LIVE: only VERIFIED gates fire -- gatesVerified === gatesTotal per domain", async () => {
  ...
  assert.ok(cov.rollup.gatesVerifiedTotal >= 120, "fleet-wide verified-gate floor (10 domains, 170 as of 2026-07-01)");
});
...
test("fail-soft: one broken domain import yields ok:false, others unaffected", async () => {
  ...
  // the other 9 still imported + fired
  assert.equal(cov.rows.filter((r) => r.ok).length, 9);
...
test("renderReport: lists every domain + a rollup, and is null-safe", async () => {
  ...
  assert.ok(md.includes("all 10 fire live"));
...
test("renderReport surfaces a non-firing domain as NO (not silently passing)", () => {
  ...
  assert.ok(md.includes("all 1 fire live: **NO**"));
});
