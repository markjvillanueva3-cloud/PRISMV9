...
/**
 * Pure: should this corpus build RE-EXEC itself with a `--max-old-space-size` bump?
 *
 * The build loads the ~550MB system-viz graph AND accumulates ~568K corpus rows
 * IN-PROCESS, which OOMs at the default heap ceiling (regression 2026-06-25, slot:papa:
 * `node build-psn-training-corpus.mjs` -- and even `--dry-run` -- FATAL'd with
 * "Ineffective mark-compacts near heap limit" at ~378MB). We re-exec once with a heap
 * bump so EVERY launch path (cron / ad-hoc / loop) works without a manual flag. Cloned
 * from the proven nn-graph-retrain-lifecycle.mjs::shouldReexecForHeap guard (slot:charlie
 * 2026-06-11). The re-exec child sets PRISM_PSN_CORPUS_REEXEC=1 to break the loop;
 * PRISM_PSN_CORPUS_NO_REEXEC=1 opts out (a caller that already launched node with a heap
 * flag). NOTE: --dry-run STILL loads the full graph here, so it is NOT a cheap-mode skip.
...
export function shouldReexecForHeap(argv, env = {}, execArgv = []) {
  if (env.PRISM_PSN_CORPUS_REEXEC === "1") return false;    // already inside the bumped child
  if (env.PRISM_PSN_CORPUS_NO_REEXEC === "1") return false; // explicit opt-out
  if (hasHeapFlag(execArgv)) return false;                  // already launched WITH a heap bump
  return true;
}

const argv1 = process.argv[1] || "";
if (argv1.endsWith("build-psn-training-corpus.mjs")) {
  // Heap guard (regression 2026-06-25, slot:papa): the in-process graph load + ~568K-row
  // corpus accumulation OOMs at the default heap; re-exec once with a --max-old-space-size
  // bump so cron/ad-hoc/loop launches never need a manual flag (Blackwell: never fight a low
  // default). Knob: PRISM_PSN_CORPUS_HEAP_MB (default 16384).
  if (shouldReexecForHeap(process.argv.slice(2), process.env, process.execArgv)) {
    const heapMb = Number(process.env.PRISM_PSN_CORPUS_HEAP_MB) || 16384;
    const r = spawnSync(
      process.execPath,
      [`--max-old-space-size=${heapMb}`, process.argv[1], ...process.argv.slice(2)],
      { stdio: "inherit", cwd: PRISM_ROOT, env: { ...process.env, PRISM_PSN_CORPUS_REEXEC: "1" } }
    );
