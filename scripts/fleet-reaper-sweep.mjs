...
const _MCP_PROTECT_REGEX = "mcp-http-bridge|mcp-server-supervisor|dist[\\\\/]index\\.js|fleet-reaper-sweep|mcp-health-watchdog|mcp-server-watchdog"
  ...
  + "|master-index-daemon"  // FLEET-SEARCH-DAEMON-MS0: long-lived warm search index (:3101)
  + _PROTECT_EXTRA;
...
export function runSweep(opts = {}) {
  ...
  if (process.env.PRISM_FR_CRASH_WATCH_DISABLE !== "1" && actionsAllowed) {
    try {
      ...
      if (prev) {
        // Window-liveness gate (golf 2026-06-17, U-CRASHWATCH-LIVENESS-GATE):
        // a frozen heartbeat ALONE mis-flags an IDLE-but-alive chat as crashed
        // (operator away -> no prompts -> heartbeat frozen, but window still live).
        // Gate detection on the SAME canonical "heartbeat-crashed AND
        // window-pid-dead" set the RECLAIM side already uses (previewReclaimable),
        // so detection stops crying wolf for live windows just as reclaim does.
        // Fail-soft + injectable: any error -> deadSlots=null -> heartbeat-only.
        let deadSlots = null;
        try {
          const previewImpl = opts.previewReclaimableImpl || previewReclaimable;
          const pv = previewImpl(slotsPath);
          deadSlots = new Set(((pv && pv.reclaimable) || []).map((r) => `${r.slot} ${r.chatId}`));
        } catch { deadSlots = null; }
        const crashes = detectCrashes(prev, curr, now, undefined, { deadSlots });
        if (crashes.length > 0) {
  ...
  let stuckHunt = {
    ...
    searchToolReaped: 0, staleSlots: 0, freedMb: 0, error: null,
  };
  ...
  if (!isStatus && !disabled && !stuckHuntFullyDisabled) {
    try {
      ...
      const searchToolAgeSec = opts.searchToolAgeSec ?? envInt("PRISM_FR_HUNT_SEARCHTOOL_AGE_SEC");
      const report = runStuckHunters({
        procs, livePidSet, slotsData, now,
        stuckBashAgeSec, fsmonitorAgeSec, orphanGraceSec, searchToolAgeSec,
        ...
        enableSearchTool: process.env.PRISM_FR_HUNT_SEARCHTOOL_DISABLE !== "1",
      ...
      if (report.searchToolOrphans && report.searchToolOrphans.length > 0) {
        const pids = report.searchToolOrphans.map((o) => o.pid);
        const rss = report.searchToolOrphans.reduce((s, o) => s + (o.rssBytes || 0), 0);
        const freedMb = Math.round(rss / 1024 / 1024);
        const r = sumKills(reapProcesses(pids, { dryRun, killer }));
        stuckHunt.searchToolReaped = r.killed;
        stuckHunt.freedMb += freedMb;
        const verb = dryRun ? `would reap ${r.wouldKill}` : `reaped ${r.killed}/${r.wouldKill}`;
        caveats.push(
          `search-tool hunter: ${verb} orphaned grep/find/tail (~${freedMb}MB freed)` +
          (r.failed > 0 ? ` -- ${r.failed} kill failure(s)` : ""),
        );
      }
