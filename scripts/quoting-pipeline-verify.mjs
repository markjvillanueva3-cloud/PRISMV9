...
/**
 * Pure TAP-summary parser. node --test emits a summary block. The leading
 * sigil DIFFERS BY NODE VERSION (R12 -- a real silent bug, found live 2026-06-28):
 *   Node <=22 (classic TAP comment):   "# tests 21"
 *   Node >=24 (node:test info symbol):  "<U+2139> tests 21"  (the info source glyph)
 * The original regex matched ONLY "# ", so under Node 24 every file parsed to 0
 * tests -> the whole verify reported tests:0 (masked by the old vacuous ok:true,
 * now fail-loud via computeOk). Match BOTH sigils so the verify works across
 * Node versions. Returns { tests, pass, fail, skipped }; missing keys default 0.
 */
export function parseTapSummary(text) {
  ...
  for (const line of lines) {
    // Accept the classic "# " TAP-comment sigil OR the Node>=24 info sigil
    // (the U+2139 information-source glyph, a literal char in the regex below).
...
/**
 * Decide overall pass/fail from an aggregate. R12 fail-loud: "ok" requires that
 * tests actually RAN. A suite that discovers N files but collects ZERO tests
 * (e.g. every spawned `.test.mjs` emitted no `# tests N` line because it crashed
 * in setup, or a bare-node tsx-reexec path failed before any `it()` ran)
 * previously returned ok:true vacuously (`agg.fail===0` with `agg.tests===0`) --
 * a false "all pass" that masked a DEAD runner (observed live 2026-06-28: 28
 * files, tests:0, ok:true). Zero collected tests is a runner failure, not
 * success, so `agg.tests > 0` is a precondition of ok. Pure + exported for test.
 */
export function computeOk(agg) {
  if (!agg || typeof agg !== "object") return false;
  const failFiles = Array.isArray(agg.fail_files) ? agg.fail_files : [];
  return (agg.fail ?? 0) === 0 && failFiles.length === 0 && (agg.tests ?? 0) > 0;
}
...
async function main() {
  ...
  const noTestsRan = agg.tests === 0;
  const ok = computeOk(agg);

  if (jsonOut) {
    process.stdout.write(JSON.stringify({
      ok,
      ...(noTestsRan ? { reason: "no-tests-collected: discovered files ran but reported 0 tests (dead runner / tsx-reexec / setup crash) -- treated as FAIL, not vacuous pass" } : {}),
