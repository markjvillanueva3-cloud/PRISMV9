...
export function generate(items, existingNodeIds = [], resolver = null) {
  ...
  let pivotsEmitted = 0, pdfsEmitted = 0, edgesEmitted = 0, bridgesResolved = 0, bridgesDropped = 0;
  ...
  for (const [domain, group] of byDomain.entries()) {
    ...
    for (const pdf of group) {
      ...
      for (const engine of bridgeEngines) {
        ...
        // No resolver -> bare name (back-compat).
        let to = engine;
        if (resolver) {
...
export function main() {
  ...
  try {
    ...
    const { newNodes, newEdges, stats } = generate(items, [], resolver);
    result = {
