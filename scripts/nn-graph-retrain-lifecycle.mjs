...
import { runAssessment, writeAssessment } from "./lib/nn-graph-eval.mjs";
...
const GHOST_EMBED_PATH = path.join(NN_DIR, "ghost-node-embeddings.jsonl");

/**
 * Refresh the ghost-embedding source (the direct-embed DEPLOYED path) from the CURRENT graph
 * -- covers the durable ghost.outcome-wired/vault-wired refs that stages 1a/1b restore.
 * --ghosts-only embeds just the ghost.unwired-engine nodes (incl the ref-pool refs),
 * leak-stripped, via Ollama nomic (~25s). Heap-bumped (same flag as the trainer);
 * spawnSync-shaped { status, summary, stderr, error, signal }. Fail-soft: if Ollama is down
 * the spawn fails and the direct-embed assessment is simply skipped / left stale.
 */
function defaultRefreshGhostEmbeddings() {
  const heapMb = Number(process.env.PRISM_NN_RETRAIN_LIFECYCLE_HEAP_MB) || LIFECYCLE_DEFAULTS.heapMb;
  const r = spawnSync(
    process.execPath,
    nodeArgsWithHeap(path.join(__dirname, "build-node-embeddings.mjs"), heapMb, ["--graph", GRAPH_PATH, "--ghosts-only", "--out", GHOST_EMBED_PATH]),
    { cwd: ROOT, encoding: "utf8", timeout: 300000 },
  );
  const summary = ((r.stdout || "").trim().split("\n").pop() || "").slice(0, 200);
  return { status: r.status, summary, stderr: r.stderr, error: r.error ? String(r.error.message || r.error) : null, signal: r.signal || null };
}

/**
 * Assess the DEPLOYED direct-embed path (raw-768d cosine k-NN over GHOST_EMBED_PATH) -- the
 * checkpoint-independent inference the consumer actually uses, distinct from defaultEval
 * (which grades the trained 8-d candidate, degenerate under edgeless inference). Recorded
 * alongside the model-mode assessment; does NOT gate promotion (additive).
 */
function defaultDirectEmbedEval() {
  return runAssessment({ directEmbed: true, directEmbedPath: GHOST_EMBED_PATH });
}

/**
 * Re-apply CONFIRMED vault wirings to the GNN reference pool (durability for
 * vault-to-gnn-refpool, U-VAULT-REFPOOL-DURABLE). `--apply` is content-idempotent
 * (U-VAULT-REFPOOL-IDEMPOTENT): it adds 0 nodes + SKIPS the 542MB write when the
 * refs are already present (steady state), and only writes to restore them after a
 * regen-viz rebuild wiped them. We pass the heap flag explicitly so the child is
 * bumped on its first launch (its `shouldReexecForHeap` sees the flag in execArgv ->
 * no redundant second spawn). Returns spawnSync-shaped { status, summary, stderr }.
 */
function defaultApplyVaultRefpool() {
  const heapMb = Number(process.env.PRISM_NN_RETRAIN_LIFECYCLE_HEAP_MB) || LIFECYCLE_DEFAULTS.heapMb;
  const r = spawnSync(
    process.execPath,
    nodeArgsWithHeap(path.join(__dirname, "vault-to-gnn-refpool.mjs"), heapMb, ["--apply"]),
    { cwd: ROOT, encoding: "utf8", timeout: 300000 },
  );
  const summary = ((r.stdout || "").trim().split("\n").pop() || "").slice(0, 200);
  // r.status is null on a timeout / spawn failure (spawnSync sets r.error + r.signal
  // and does NOT throw); carry them so the stage can surface a silent restore-miss.
  return { status: r.status, summary, stderr: r.stderr, error: r.error ? String(r.error.message || r.error) : null, signal: r.signal || null };
}

/**
 * Re-apply CONFIRMED ghost-wire OUTCOME wirings (the sibling feeder) to the GNN reference
 * pool BEFORE the fingerprint. This is the BIGGER durable pool (~139 confirmed outcomes vs
 * ~14 vault). Same content-idempotent --apply via the shared scripts/lib/refpool-merge.mjs,
 * so a steady-state re-apply adds 0 nodes (count-based fingerprint unchanged -> no spurious
 * retrain); only a regen-viz wipe of the ghost.outcome-wired.* refs makes it write. Heap
 * flag passed explicitly (child sees it in execArgv -> no redundant second spawn). Returns
 * spawnSync-shaped { status, summary, stderr, error, signal }.
 */
function defaultApplyOutcomeRefpool() {
  const heapMb = Number(process.env.PRISM_NN_RETRAIN_LIFECYCLE_HEAP_MB) || LIFECYCLE_DEFAULTS.heapMb;
  const r = spawnSync(
    process.execPath,
    nodeArgsWithHeap(path.join(__dirname, "ghost-wire-outcomes-to-refpool.mjs"), heapMb, ["--apply"]),
    { cwd: ROOT, encoding: "utf8", timeout: 300000 },
  );
  const summary = ((r.stdout || "").trim().split("\n").pop() || "").slice(0, 200);
  return { status: r.status, summary, stderr: r.stderr, error: r.error ? String(r.error.message || r.error) : null, signal: r.signal || null };
...
export function runLifecycle(opts = {}) {
  ...
  try {
    // 1a (PRE-fingerprint, U-VAULT-REFPOOL-DURABLE slot:india). Restore CONFIRMED
    // vault wirings to the GNN reference pool BEFORE the fingerprint, so both the
    // count-based fingerprint AND the eval holdout pool see them. regen-viz rebuilds
    // the graph from canonical sources and WIPES the ghost.vault-wired.* refs; the
    // re-apply is content-idempotent, so in steady state (refs present) it adds 0
    // nodes -> the count-based fingerprint is unchanged and this does NOT trigger a
    // spurious retrain (a POST-fingerprint apply WOULD: the +N restored ghosts would
    // trip the NEXT tick's ghost-delta gate). Only a real regen-wipe makes it write.
    // Fail-soft + opt-out (PRISM_NN_RETRAIN_VAULT_REFPOOL_DISABLE=1): a failure NEVER
    // aborts the retrain (the pool falls back to the keyword + outcome seeders).
    if (env.PRISM_NN_RETRAIN_VAULT_REFPOOL_DISABLE === "1") {
      result.vaultRefpool = { ran: false, disabled: true };
    } else {
      const applyVaultRefpool = opts.applyVaultRefpool || defaultApplyVaultRefpool;
      try {
        const vr = applyVaultRefpool();
        result.vaultRefpool = { ran: true, status: vr ? vr.status : null, summary: vr ? vr.summary : "" };
        if (vr && Number.isFinite(vr.status) && vr.status !== 0) {
          errors.push(`vault ref-pool apply: exit ${vr.status} (retrain continues) ${((vr.stderr) || "").slice(0, 120)}`);
        } else if (vr && vr.status == null) {
          // null status = timeout / spawn-failure (R12: surface it -- errors[] is the
          // only signal persisted to the ledger; vaultRefpool telemetry is not).
          errors.push(`vault ref-pool apply: no exit status (timeout/spawn-fail${vr.signal ? " " + vr.signal : ""}), retrain continues ${((vr.error || vr.stderr) || "").slice(0, 120)}`);
        }
      } catch (e) {
        errors.push(`vault ref-pool apply failed: ${e && e.message ? e.message : e} (retrain continues)`);
      }
    }

    // 1b (PRE-fingerprint, U-OUTCOME-REFPOOL-DURABLE slot:india). Same durability for the
    // SIBLING outcome feeder -- the bigger ref pool (~139 confirmed-outcome refs vs ~14
    // vault). Idempotent --apply (shared refpool-merge) so steady state adds 0 nodes -> no
    // fingerprint churn / no spurious retrain; only a regen-viz wipe of ghost.outcome-wired.*
    // makes it write. Fail-soft + opt-out (PRISM_NN_RETRAIN_OUTCOME_REFPOOL_DISABLE=1): a
    // failure NEVER aborts the retrain (the pool still has the keyword + vault seeders).
    if (env.PRISM_NN_RETRAIN_OUTCOME_REFPOOL_DISABLE === "1") {
      result.outcomeRefpool = { ran: false, disabled: true };
    } else {
      const applyOutcomeRefpool = opts.applyOutcomeRefpool || defaultApplyOutcomeRefpool;
      try {
        const or = applyOutcomeRefpool();
        result.outcomeRefpool = { ran: true, status: or ? or.status : null, summary: or ? or.summary : "" };
        if (or && Number.isFinite(or.status) && or.status !== 0) {
          errors.push(`outcome ref-pool apply: exit ${or.status} (retrain continues) ${((or.stderr) || "").slice(0, 120)}`);
        } else if (or && or.status == null) {
          // null status = timeout / spawn-failure (R12: surface it -- errors[] is the only
          // signal persisted to the ledger; outcomeRefpool telemetry is not).
          errors.push(`outcome ref-pool apply: no exit status (timeout/spawn-fail${or.signal ? " " + or.signal : ""}), retrain continues ${((or.error || or.stderr) || "").slice(0, 120)}`);
        }
      } catch (e) {
        errors.push(`outcome ref-pool apply failed: ${e && e.message ? e.message : e} (retrain continues)`);
      }
    }
    ...
    // 4b (additive). Refresh ghost embeddings + assess the DEPLOYED direct-embed path.
    // The consumer's tier-5 uses direct-embed (raw-768d cosine k-NN over
    // ghost-node-embeddings.jsonl), NOT the trained 8-d candidate graded above (degenerate
    // under edgeless inference). regen-viz WIPES that embedding file, so refresh it from the
    // CURRENT graph (covers the durable ghost.outcome-wired/vault-wired refs from 1a/1b) --
    // THIS is the durable half (the embedding file is written to disk, so the deployed
    // direct-embed path survives the regen wipe). Then record the direct-embed assessment on
    // `result.directEmbedAssessment` as the deployed-path measurement (programmatic
    // runAssessment doesn't write NN-EVAL.json itself, so stage 4b PERSISTS the full
    // assessment via writeAssessment below -> NN-EVAL.{md,json} reflects the deployed path
    // each retrain + the PSN-leg hook (classifyGnn) auto-reads it). ADDITIVE: promotion still
    // gates on the model-mode `assessment`; this only refreshes + MEASURES. Fail-soft + opt-out
    // (PRISM_NN_RETRAIN_GHOST_EMBED_DISABLE=1).
    if (env.PRISM_NN_RETRAIN_GHOST_EMBED_DISABLE !== "1") {
      const refreshGhostEmbed = opts.refreshGhostEmbeddings || defaultRefreshGhostEmbeddings;
      try {
        const ge = refreshGhostEmbed();
        result.ghostEmbedRefresh = { ran: true, status: ge ? ge.status : null, summary: ge ? ge.summary : "" };
        if (ge && Number.isFinite(ge.status) && ge.status !== 0) {
          errors.push(`ghost-embedding refresh: exit ${ge.status} (direct-embed assessment may be stale) ${((ge.stderr) || "").slice(0, 120)}`);
        } else if (ge && ge.status == null) {
          errors.push(`ghost-embedding refresh: no exit status (timeout/spawn-fail${ge.signal ? " " + ge.signal : ""}; Ollama down?) ${((ge.error || ge.stderr) || "").slice(0, 120)}`);
        }
      } catch (e) {
        errors.push(`ghost-embedding refresh failed: ${e && e.message ? e.message : e} (direct-embed assessment skipped)`);
      }
      const directEval = opts.directEvalFn || defaultDirectEmbedEval;
      try {
        const da = directEval();
        result.directEmbedAssessment = summarizeAssessment(da);
        // Persist the DEPLOYED direct-embed state to NN-EVAL.{md,json} so the PSN-leg-state
        // hook (classifyGnn reads NN-EVAL.json) auto-reflects it post-retrain -- the last
        // mile that makes deploy-ready-selective self-updating. Fail-soft + injectable; a
        // write failure NEVER aborts the retrain.
        // force:true -- the lifecycle is the AUTHORITATIVE deployed-state writer; its direct-embed
        // (or honest {deferred} when ghost embeddings will not load) assessment MUST land over any
        // prior report (R12: never keep a stale "deploy-ready" claim after the deployed path defers).
        // writeAssessment's downgrade guard fences only NON-authoritative bare-CLI callers (no force).
        const writeEval = opts.writeAssessmentFn || writeAssessment;
        const w = writeEval(da, NN_DIR, undefined, { force: true });
        result.directEmbedWritten = !!(w && w.ok);
        if (w && !w.ok) errors.push(`direct-embed NN-EVAL write failed: ${w.error} (assessment recorded on result)`);
      } catch (e) {
        errors.push(`direct-embed assessment failed: ${e && e.message ? e.message : e} (model-mode gate unaffected)`);
      }
    }
...
/**
 * Pure: render the DEPLOYED tier-5 state from a parsed NN-EVAL.json doc (or null). The deployed
 * inference path is DIRECT-EMBED (embeddingMode "direct"); the trained 8-d checkpoint AUROC printed
 * above is only a link-pred PRETEXT metric, NOT this. Reads the verdict the eval already computed --
 * does NOT re-derive the deploy gate (R8: the canonical thresholds live in the eval/classifyGnn).
 * Returns an array of display lines so a reader never mistakes the degenerate pretext AUROC for the
 * deployed metric (the wrong-mode footgun this surfaces).
 * @param {object|null} evalDoc  parsed NN-EVAL.json, or null if absent/unreadable
 * @returns {string[]}
 */
export function formatDeployedStatus(evalDoc) {
  if (!evalDoc) {
    return ["Deployed (direct-embed): (no NN-EVAL.json -- run nn-graph-eval with PRISM_NNG_DIRECT_EMBED=1)"];
  }
  if (evalDoc.deferred) {
    return [`Deployed (direct-embed): DEFERRED -- ${evalDoc.reason || "no reason recorded"}`];
  }
  const mode = evalDoc.embeddingMode || "unknown";
  const m = evalDoc.metrics || {};
  const lines = [];
  if (mode !== "direct") {
    // A model/unknown-mode NN-EVAL.json is NOT the deployed path (a bare-CLI write, or a forced model write).
    lines.push(`Deployed report: embeddingMode=${mode} (NOT the deployed direct-embed path -- re-run eval with PRISM_NNG_DIRECT_EMBED=1)`);
  } else {
    lines.push("Deployed (direct-embed, NN-EVAL.json):");
  }
  lines.push(`  full-holdout: AUROC ${m.auroc ?? "n/a"} · macroF1 ${m.macroF1 ?? "n/a"} · Brier ${m.brier ?? "n/a"} (n=${evalDoc.holdoutN ?? "?"})`);
  const dg = evalDoc.selective && evalDoc.selective.deployGrade;
  if (dg) {
    const op = dg.operatingPoint || {};
    const cov = op.coverage != null ? (op.coverage * 100).toFixed(1) + "%" : "?";
    lines.push(`  selective @tau=${dg.productionGate ?? "?"}: ${dg.verdict} -- coverage ${cov} · Brier ${op.brier ?? "n/a"} · macroF1 ${op.macroF1 ?? "n/a"} · ${dg.robustAboveGate ? "robust" : "fragile"}`);
  }
  return lines;
}
...
function printStatus() {
  ...
  if (ckptMeta) {
    ...
    lines.push(`  AUROC ${ckptMeta.auroc ?? "n/a"} (link-pred PRETEXT, NOT the deploy gate) · epochs ${ckptMeta.epochs ?? "?"} · stratified ${ckptMeta.stratifiedNegatives ? "yes" : "no"}`);
  } else {
    lines.push("Live checkpoint: (none on disk)");
  }

  // The DEPLOYED tier-5 path is direct-embed (NN-EVAL.json), distinct from the trained checkpoint's
  // link-pred pretext AUROC above. Surface it so a reader never misreads the degenerate pretext metric
  // as leg #10's deployed state (the wrong-mode footgun).
  let deployedDoc = null;
  try { deployedDoc = JSON.parse(fs.readFileSync(path.join(NN_DIR, "NN-EVAL.json"), "utf8")); }
  catch { /* no deployed eval on disk -> formatDeployedStatus(null) renders the absent-file line */ }
  for (const l of formatDeployedStatus(deployedDoc)) lines.push(l);
...
export function main(argv) {
  ...
  // unaffected). Fail-soft; disable with PRISM_BATCH_NICE_DISABLE=1.
  nicifySelf();
