...
import { resolveEmbedUrl, withLaneOptions } from "./lib/embed-endpoint.mjs";
...
export async function embedText(text, fetchImpl = fetch, expectedDim = 0) {
  ...
  const attempt = async (url, laneFlag) => {
    const body = laneFlag
      ? withLaneOptions({ model: MODEL, prompt: clamped })
      : { model: MODEL, prompt: clamped };
    const res = await fetchImpl(`${url}/api/embeddings`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(body),
    });
    if (!res.ok) {
      const errBody = typeof res.text === "function" ? await res.text() : "";
      throw new Error(`ollama embed ${res.status} @ ${url}: ${errBody}`);
    }
    const j = await res.json();
    if (!Array.isArray(j.embedding) || j.embedding.length === 0) {
      throw new Error(`ollama returned no embedding @ ${url}`);
    }
    if (expectedDim > 0 && j.embedding.length !== expectedDim) {
      throw new Error(
        `ollama embedding dim ${j.embedding.length} != index dim ${expectedDim} @ ${url} ` +
        `(wrong model/endpoint? entry would be non-retrievable -- R12 fail-loud)`,
      );
    }
    return j.embedding;
  };
  // U-INDIA-EMBED-LANE: prefer the dedicated CPU embed lane (:11435) when it
  // answers -- the shared :11434 starves embeds behind fleet inference (this
  // script's cron exit-3'd every 30 min under mining load, 2026-07-01). The
  // resolution is sidecar-cached (~0 cost per call); a FAILED lane attempt
  // (including wrong-dim -- a mis-served lane model must not poison the index)
  // retries MAIN once, so a broken lane degrades to today's exact behavior.
  // Errors carry the resolved URL so operator triage probes the right port.
  // Auto-resolve ONLY on the real fetch: an injected fetchImpl (tests) keeps
  // legacy single-URL behavior unchanged.
  if (fetchImpl !== fetch) return attempt(OLLAMA_URL, false);
  const lane = await resolveEmbedUrl();
  if (lane.lane) {
    try { return await attempt(lane.url, true); }
    catch { /* broken lane -> one MAIN retry below; MAIN's error is the one thrown */ }
    return attempt(OLLAMA_URL, false);
  }
  return attempt(lane.url, false);
}
