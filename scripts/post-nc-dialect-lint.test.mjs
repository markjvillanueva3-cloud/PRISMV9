...
test("R9 dwell-dialect-okuma: a Fanuc G04 P<ms> dwell on an Okuma post warns", () => {
  // The JM Okuma corpus uses G4 F<sec> 25,000+ times and G04 P<ms> zero times.
  // A G04 P0.5 on an OSP control is the wrong dialect (~0.5 ms, i.e. no dwell where a
  // clamp-settle / chip-clear was intended). This is the U-PP-OKUMA-DWELL-DIALECT bug.
  const nc = "G50 S2000\nG96 S150 M03\nG01 X0 F0.05\nG04 P0.5 (DWELL AT CENTER)\nM30\n";
  const res = lintNc(nc, { dialect: "okuma" });
  assert.ok(rulesOf(res, "WARN").includes("dwell-dialect-okuma"),
    `Okuma G04 P dwell must warn: ${JSON.stringify(res.findings)}`);
});

test("R9 precision: the CORRECT Okuma G4 F<sec> dwell is NOT flagged", () => {
  const nc = "G50 S2000\nG96 S150 M03\nG01 X0 F0.05\nG4 F0.5 (DWELL AT CENTER)\nM30\n";
  const res = lintNc(nc, { dialect: "okuma" });
  assert.ok(!rules(res).includes("dwell-dialect-okuma"),
    `G4 F dwell wrongly flagged: ${JSON.stringify(res.findings)}`);
});

test("R9 precision: a Fanuc G04 P<ms> dwell is correct on Fanuc and must NOT be flagged", () => {
  // On Fanuc/Haas/etc., P is the dwell address (milliseconds) -- correct, not a bug.
  const nc = "G94\nS2000 M03\nG04 P500 (DWELL 0.5S)\nM30\n";
  const res = lintNc(nc, { dialect: "fanuc" });
  assert.ok(!rules(res).includes("dwell-dialect-okuma"));
});

test("R9 precision: G40/G43 (cutter-comp / tool-length) on Okuma must NOT trip the G4 detector", () => {
  // hasCode(G,4) must match G4/G04 but never G40/G41/G43/G45 -- a P word elsewhere
  // on such a line must not produce a false dwell-dialect finding.
  const nc = "G50 S2000\nG96 S150 M03\nG40 X10. P3\nG43 Z5. H1\nM30\n";
  const res = lintNc(nc, { dialect: "okuma" });
  assert.ok(!rules(res).includes("dwell-dialect-okuma"),
    `G40/G43 wrongly tripped the G4 dwell rule: ${JSON.stringify(res.findings)}`);
});

test("R9 thread-dialect-okuma: a Fanuc G76 threading cycle on an Okuma post warns", () => {
  // On Okuma OSP the threading cycle is the single-line G71 (verified vs Mark's JM programs
  // A05-LSC-25-B.MIN:149 + THREAD M8X125.MIN:60 + the JM Multus .cps thread-turning emit).
  // A Fanuc/Haas G76 multi-pass thread on an OSP control alarms/mis-cycles -- the
  // U-PP-OKUMA-THREAD-DIALECT bug.
  const nc = "G50 S2000\nG97 S800 M03\nG00 X40. Z2.\nG76 X38.16 Z-25. P920 Q100 F1.5\nM30\n";
  const res = lintNc(nc, { dialect: "okuma" });
  assert.ok(rulesOf(res, "WARN").includes("thread-dialect-okuma"),
    `Okuma G76 threading must warn: ${JSON.stringify(res.findings)}`);
});

test("R9 precision: the CORRECT Okuma G71 single-line threading cycle is NOT flagged", () => {
  // G71 IS the Okuma threading cycle (NOT Fanuc roughing) -- must never be flagged wrong-dialect.
  const nc = "G50 S2000\nG97 S800 M03\nG00 X40. Z2.\nG71 X38.16 Z-25. B60 D0.08 U0.025 H1.84 F1.5 M33 M73\nM30\n";
  const res = lintNc(nc, { dialect: "okuma" });
  assert.ok(!rules(res).includes("thread-dialect-okuma"),
    `Okuma G71 threading wrongly flagged: ${JSON.stringify(res.findings)}`);
});

test("R9 precision: a Fanuc G76 threading cycle is correct on Fanuc and must NOT be flagged", () => {
  // On Fanuc/Haas, G76 is the standard multi-pass threading cycle -- correct, not a bug.
  const nc = "G97 S800 M03\nG76 P010060 Q100 R0.05\nG76 X38.16 Z-25. P920 Q100 F1.5\nM30\n";
  const res = lintNc(nc, { dialect: "fanuc" });
  assert.ok(!rules(res).includes("thread-dialect-okuma"));
});

test("R9 precision: a coordinate value containing 76 (X76.) on Okuma must NOT trip the G76 thread detector", () => {
  // hasCode(G,76) must match the G76 word but never a coordinate like X76. or G760.
  const nc = "G50 S2000\nG96 S150 M03\nG0 X76. Z-5.\nM30\n";
  const res = lintNc(nc, { dialect: "okuma" });
  assert.ok(!rules(res).includes("thread-dialect-okuma"),
    `X76. coordinate wrongly tripped the G76 thread rule: ${JSON.stringify(res.findings)}`);
});

test("R9 precision: a VALID Okuma OSP G76 grooving cycle (G76 X.. L.., no P-word) is NOT flagged", () => {
  // On Okuma OSP, G76 X<dia> L<width> is a valid grooving / continuous-cut cycle used in ~984
  // real JM .MIN programs (e.g. JM DIE/CNC LATHE/ACME/A-11-10049-0.MIN "G76 X.216 L.012"). Only
  // the Fanuc THREADING G76 (which carries a P-word) is the wrong dialect; the P-word gate must
  // spare legitimate OSP G76 X/L grooving. (scrutiny arm A false-positive fix, 2026-06-29.)
  const nc = "G50 S2000\nG96 S150 M03\nG0 X.7 Z-1.\nG76 X.216 L.012\nM30\n";
  const res = lintNc(nc, { dialect: "okuma" });
  assert.ok(!rules(res).includes("thread-dialect-okuma"),
    `valid OSP G76 X/L grooving wrongly flagged as threading: ${JSON.stringify(res.findings)}`);
});

test("R9 groove-dialect-okuma: a Fanuc G75 grooving/peck cycle on an Okuma post warns", () => {
  // Okuma OSP grooving is explicit (G1 plunge + G4 dwell); the real JM corpus uses G75 0 times
  // (vs G85 LAP 12,103x). A Fanuc G75 on an OSP post is the wrong dialect (the U-PP-OKUMA-GROOVE bug).
  const nc = "G50 S2000\nG96 S84 M03\nG00 X40. Z-50.\nG75 R0.5\nG75 X30. Z-50. P2000 Q3000 F0.08\nM30\n";
  const res = lintNc(nc, { dialect: "okuma" });
  assert.ok(rulesOf(res, "WARN").includes("groove-dialect-okuma"),
    `Okuma G75 grooving must warn: ${JSON.stringify(res.findings)}`);
});

test("R9 precision: the CORRECT Okuma explicit groove (G1 plunge + G4 dwell) is NOT flagged", () => {
  const nc = "G50 S2000\nG96 S84 M03\nG00 X40. Z-50.\nG01 X30. F0.08\nG4 F0.5\nG00 X40.\nM30\n";
  const res = lintNc(nc, { dialect: "okuma" });
  assert.ok(!rules(res).includes("groove-dialect-okuma"),
    `Okuma explicit groove wrongly flagged: ${JSON.stringify(res.findings)}`);
});

test("R9 precision: a Fanuc G75 peck cycle is correct on Fanuc and must NOT be flagged", () => {
  const nc = "G97 S800 M03\nG75 R0.5\nG75 X30. Z-50. P2000 Q3000 F0.08\nM30\n";
  const res = lintNc(nc, { dialect: "fanuc" });
  assert.ok(!rules(res).includes("groove-dialect-okuma"));
});
