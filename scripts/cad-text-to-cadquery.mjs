...
export function buildPrompt(request, templateNames = [], enginePrompt = null, tribalTips = [], learnedRisk = [], classDimPriors = [], recipe = []) {
  ...
  // A METRIC request needs a LOUD, request-specific override: the generic "dimensions are INCHES unless
  // metric" rule + the `IN = 25.4` inch example prime the LLM to divide by 25.4 anyway (live: 41% of metric
  // gens did, a 25.4x-undersize scrap bug). This states the mm-native rule FIRST and gives the exact
  // radius=D/2 cylinder idiom (the other confirmed bug: circle() takes the RADIUS, not the diameter).
  const metric = requestIsMetric(request);
  return [
    ...
    metric ? "- ⚠ THIS REQUEST IS IN MILLIMETRES. cadquery/build123d are mm-native: use the stated numbers DIRECTLY as mm. Do NOT define `IN`. Do NOT divide OR multiply ANY dimension by 25.4 / IN (that is the #1 scrap bug -- `d = 31.68 / IN` makes the part 25.4x too small). A cylinder of diameter D, length L is EXACTLY `cq.Workplane('XY').circle(D/2).extrude(L)` -- circle() takes the RADIUS (= D/2), NOT the diameter." : null,
...
export function codeInvalidReason(code, { requestIsMetric: metric = false } = {}) {
  ...
  // A PARAMETRIC template (cad-parametric-templates.mjs) resolves EVERY dimension to mm as a named variable
  // (`base_dia = 38.1`; geometry is `circle(base_dia/2)`), so it legitimately carries NO inch->mm literal --
  // requiring one would false-reject a correct parametric script. It is deterministic (from parseRequestPrint,
  // which already converts units), so the LLM units bug cannot occur here; exempt check 6 like a metric part.
  // Checks 4/5 above still fire -- a resolved variable `= 1.5 / 25.4` or `= 38.1 * IN` is still a real bug.
  const isParametric = /\bdef\s+make_\w+\s*\(/.test(code) || /=====\s*PARAMETRIC\b/.test(code);
  ...
  if (!metric && !isParametric && !/25\.4|\bIN\b|\binch/i.test(code)) return "no inch->mm conversion evidence (JM units rule)";
  return null;
}
...
export function offlineSelfCheck(request, stepText) {
  try {
    ...
    const result = { accurate: v.accurate, score: v.score, dimAccuracy: v.dimAccuracy, completeness: v.completeness, missingCount: v.missingCount, extraCount: v.extraCount, bboxReliable: v.bboxReliable, bboxAdvisory: v.bboxAdvisory };
    // The bbox cannot measure a CURVED diameter (point cloud under-reads a circle), but the STEP radius
    // ENTITY can -- offline, no kernel (U-CAD-CURVED-DIM-LEARN). Attach the real curved-diameter verdict so
    // classifyGenerationOutcome can catch a wrong-diameter cylinder that the bbox check has to pass by default.
    try { const cd = curvedDimCheck(request, stepText); if (cd.applicable) result.curvedDim = cd; } catch { /* fail-soft */ }
    return result;
...
export function classifyGenerationOutcome({ invalidReason = null, status = null } = {}) {
  ...
  if (status.executed === true) {
    ...
    // Offline CURVED-diameter signal (U-CAD-CURVED-DIM-LEARN): a curved part whose requested diameter does NOT
    // match its STEP radius entity is a real defect the bbox self-check must otherwise pass by default -- now
    // catchable OFFLINE (no Fusion) via the radius literal. Only a clear, applicable mismatch fails.
    if ((!ka || ka.accurate == null) && sc && sc.curvedDim && sc.curvedDim.applicable && sc.curvedDim.accurate === false) {
      return { status: "fail", reason: `curved dim off: dia ${sc.curvedDim.diaDeltaPct ?? sc.curvedDim.deltaPct}% / len ${sc.curvedDim.lenDeltaPct ?? "n/a"}% (dia ${sc.curvedDim.matchedDiaMm}mm vs ${sc.curvedDim.requestedDiaMm}mm req)` };
    }
    return { status: "pass" };
  }
...
/** Is an Ollama failure TRANSIENT (worth a retry) vs terminal? Fleet contention returns a 200 with
 *  {"error":"server busy, please try again. maximum pending requests exceeded"} + no response field;
 *  a curl -m timeout is also transient. A parse error / genuine model error is NOT. Pure -> testable. */
export function isTransientOllamaError(s) {
  return /server busy|maximum pending requests|please try again|try again later|\btimeout\b|timed out|connection reset|temporarily/i.test(String(s || ""));
}

/** One Ollama /api/generate call. Returns {ok,text} | {ok:false,error,transient}. `runner` (the curl spawn)
 *  is injectable for tests. */
export function ollamaCallOnce(prompt, model, runner) {
  ...
  const call = runner || ((b) => spawnSync("curl", ["-s", "-m", String(Math.floor(GEN_TIMEOUT_MS / 1000)), "-X", "POST",
    `${OLLAMA_URL}/api/generate`, "-H", "Content-Type: application/json", "-d", b],
  { encoding: "utf8", timeout: GEN_TIMEOUT_MS + 10_000, maxBuffer: 32 * 1024 * 1024, windowsHide: true }));
  const r = call(body);
  if (r.status !== 0 || !r.stdout) return { ok: false, error: `ollama call failed (curl exit ${r.status})`, transient: true }; // timeout/network -> retry
  try {
    const j = JSON.parse(r.stdout);
    if (j.error) return { ok: false, error: `ollama error: ${String(j.error).slice(0, 120)}`, transient: isTransientOllamaError(j.error) };
    if (!j.response) return { ok: false, error: `no response field: ${r.stdout.slice(0, 160)}`, transient: isTransientOllamaError(r.stdout) };
    return { ok: true, text: j.response };
  } catch (e) { return { ok: false, error: `ollama response parse: ${e.message}`, transient: false }; }
}

/**
 * Generate via Ollama with BOUNDED retry-with-backoff on TRANSIENT failures (fleet "server busy" / timeout).
 * The overnight loop under fleet contention was failing hard (exit 4) on a transient the queue would clear;
 * retrying lets the complex-part gens survive it. keep_alive keeps the model GPU-resident across a batch.
 * `runner`/`sleep` injectable for tests. Knobs: PRISM_OLLAMA_GEN_RETRIES (default 3).
 */
export async function ollamaGenerate(prompt, model, { runner = null, maxAttempts = null, sleep = (ms) => new Promise((r) => setTimeout(r, ms)), backoffMs = [2000, 5000, 12000] } = {}) {
  const attempts = maxAttempts || Math.max(1, Number(process.env.PRISM_OLLAMA_GEN_RETRIES) || 3);
  let last;
  for (let i = 0; i < attempts; i++) {
    last = ollamaCallOnce(prompt, model, runner);
    if (last.ok || !last.transient) return last; // success, or a terminal error -> fail fast
    if (i < attempts - 1) {
      const wait = backoffMs[Math.min(i, backoffMs.length - 1)];
      console.error(`[cad-text] ollama TRANSIENT (${last.error.slice(0, 70)}) -- retry ${i + 1}/${attempts - 1} in ${wait}ms`);
      await sleep(wait);
    }
  }
  return last; // retries exhausted -> the last transient failure
}
...
export async function main(argv = process.argv.slice(2)) {
  ...
  // Prefer the CORPUS-GROUNDED cadquery recipe (U-CADGEN-ARCHETYPE-RECIPE-RECONCILE): the static
  // fusion360 recipe prescribes op.revolve for round parts, but 136/277 successful gens EXTRUDE a
  // circle. loadArchetypeRecipe falls back to fusion360 for any archetype without a cadquery section.
  const recipe = loadArchetypeRecipe(request, { platform: "cadquery" });
  const prompt = buildPrompt(request, loadTemplateNames(), await loadEnginePrompt(), tribalTips, learnedRisk, classDimPriors, recipe);
  // DETERMINISTIC-FIRST for simple primitives (U-CAD-PRIMITIVE-EMIT): a plain cube/cylinder/disc is a
  // one-line cadquery script -- emit it directly (dims already mm from parseRequestPrint, radius = D/2),
  // skipping the LLM. Dimensionally EXACT (immune to the 25.4x-undersize units bug the LLM hits on 41% of
  // metric requests) AND resilient to Ollama being down (curl exit 28 -> primitives still generate).
  // emitPrimitiveCode returns null for anything with a feature or non-primitive shape -> the LLM handles it.
  let code, genVia = "deterministic-primitive";
  // DETERMINISTIC-FIRST: plain primitive (cube/cylinder/tube), then a featured part drawn by engineering
  // convention (box + centered through-hole, U-CAD-FEATURE-EMIT), then the LLM for genuinely-ambiguous parts.
  const prim = emitPrimitiveCode(request);
  const feat = prim ? null : emitFeatureCode(request);
  const det = prim || feat;
  if (det) {
    code = det.code;
    genVia = prim ? "deterministic-primitive" : "deterministic-feature";
  } else {
    const gen = await ollamaGenerate(prompt, model);
    if (!gen.ok) { console.error(`[cad-text] OLLAMA UNREACHABLE/FAILED: ${gen.error}`); return 4; }
    code = extractPythonCode(gen.text);
    genVia = "ollama";
  ...
  // PARAMETRIC TEMPLATE sidecar (U-CAD-PARAMETRIC-WIRE): a deterministically-drawn part also emits its
  // EQUATION-BASED form -- named dimension variables + relationships (inner = outer - 2*wall, floor_r =
  // dia/2 - depth, ...) -- plus an alterable params.json, so the part is NOT hard-locked: a downstream
  // `cad-part-alter.mjs "<request>" --set var=val` regenerates it and the derived equations recompute.
  // Additive + fail-soft: model.py (the proven hard-coded form) is unchanged; the parametric form is
  // byte-validated to pass the same units gate before it is written. Only deterministic shapes with a template.
  let parametricShape = null;
  if (det && hasTemplate(det.shape)) {
    try {
      const params = paramsFromDims(det.shape, det.dimsMm);
      const pcode = renderParametricScript(det.shape, params);
      if (pcode && !codeInvalidReason(pcode, { requestIsMetric: requestIsMetric(request) })) {
        writeFileSync(join(dir, "model.parametric.py"), pcode, "utf8");
        writeFileSync(join(dir, "params.json"), JSON.stringify(templateSpec(det.shape, params), null, 2), "utf8");
        parametricShape = det.shape;
      }
    } catch { /* the parametric sidecar is additive -- never break generation over it */ }
  }
  writeFileSync(join(dir, "request.json"), JSON.stringify({ request, model, via: genVia, generatedAt: new Date().toISOString(), promptChars: prompt.length, tribalTipCount: tribalTips.length, learnedRiskCount: learnedRisk.length, archetype, recipeInjected: recipe.length > 0, parametricShape }, null, 2), "utf8");
  ...
  // OFFLINE geometric PRECEDENT-check (U-CAD-GEOMEMBED-LOOP-WIRE): featurize the generated STEP -> retrieve
  // the nearest proven corpus parts + flag geometric OUTLIERS. Wires the geometric pattern-recognition
  // system (featurizer/index/retrieve) INTO the generation loop as an advisory quality signal, distinct
  // from the dimensional self-check. Dynamic import + fail-soft: a missing index/module never affects
  // the generation outcome (mirrors the selfCheck contract above).
  if (status.executed && status.stepPath) {
    try {
      const { loadIndex, precedentCheck, defaultIndexPath } = await import("./cad-geometric-retrieve.mjs");
      const idx = loadIndex(defaultIndexPath());
      status.precedentCheck = idx.length
        ? precedentCheck(readFileSync(status.stepPath, "utf8"), idx)
        : { applicable: false, reason: "no geometric index (run build-cad-geometric-index --write)" };
    } catch (e) { status.precedentCheck = { applicable: false, reason: `precedent-check skipped: ${String(e?.message ?? e).slice(0, 80)}` }; }
  }
