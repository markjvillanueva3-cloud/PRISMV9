...
import {
  ...
  isGrokId, isNonChatModel, pickServedChatModel,
...
test("resolveModel: NON-grok lane (NVIDIA) prefers the configured SERVED model, never a blind first-listed id (the no-model-404 fix)", async () => {
  // REGRESSION (R9 -- verifies intent): a realistic NVIDIA-shaped list LEADS with 01-ai/yi-large and
  // contains the configured served fallback. The OLD resolver returned the first-listed id
  // (01-ai/yi-large -- a coin-flip that 404s the moment the lane leads with a non-chat model); the fix
  // returns the operator-configured SERVED model. This test FAILS on the old code, PASSES on the fix.
  const nvidiaList = ["01-ai/yi-large", "meta/llama-3.3-70b-instruct", "baai/bge-m3", "nvidia/embed-qa-4"];
  const d1 = { ...deps(mockFetch({ models: { body: { data: nvidiaList.map((id) => ({ id })) } } })), fallback: "meta/llama-3.3-70b-instruct", env: {} };
  assert.equal(await resolveModel(undefined, d1), "meta/llama-3.3-70b-instruct");

  // fallback NOT served + a non-chat model listed FIRST -> skip the embedding, pick the first served CHAT model
  const d2 = { ...deps(mockFetch({ models: { body: { data: [{ id: "baai/bge-m3" }, { id: "meta/llama-3.1-8b-instruct" }] } } })), fallback: "unserved/x", env: {} };
  assert.equal(await resolveModel(undefined, d2), "meta/llama-3.1-8b-instruct");

  // ONLY non-chat models served + fallback not among them -> last resort = configured fallback (never an embedding)
  const d3 = { ...deps(mockFetch({ models: { body: { data: [{ id: "baai/bge-m3" }, { id: "nvidia/embed-qa-4" }] } } })), fallback: "meta/llama-3.3-70b-instruct", env: {} };
  assert.equal(await resolveModel(undefined, d3), "meta/llama-3.3-70b-instruct");
});

test("resolveModel: PRISM_HERMES_PREFERRED_MODEL pin wins over the lane default", async () => {
  const withPin = { ...deps(mockFetch({ models: { body: { data: [{ id: "meta/llama-3.3-70b-instruct" }] } } })), env: { PRISM_HERMES_PREFERRED_MODEL: "pinned/model" } };
  assert.equal(await resolveModel(undefined, withPin), "pinned/model");
});

test("isGrokId / isNonChatModel: pure classifiers (chat models never flagged non-chat)", () => {
