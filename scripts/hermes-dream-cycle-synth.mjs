...
export function synthesizeDreamMarkdown({ date, totalMemos, connections, cluster, params }) {
  const lines = [];
  lines.push(`---`);
  ...
  lines.push(``);
  lines.push(`Walked ${totalMemos} memos · surfaced ${connections.length} cross-memo connections (Jaccard ≥ ${params.minJaccard}, top-${params.topKKeywords} keywords per memo).`);
  lines.push(``);
...
export function run({ root = DEFAULT_ROOT, out, date, minJaccard = 0.15, topKKeywords = 20, maxConnections = 200, now = Date.now(), fsImpl = fs } = {}) {
  const dateStr = date || new Date(now).toISOString().slice(0, 10);
  const memos = listAllMemos({ root, fsImpl });
  ...
  const md = synthesizeDreamMarkdown({ date: dateStr, totalMemos: memos.length, connections, cluster, params });
...
export async function runWithSynth(opts = {}) {
  const {
    ...
    maxConnections = 200, now = Date.now(), fsImpl = fs, callFn, topN, maxWords,
  } = opts;
  const dateStr = date || new Date(now).toISOString().slice(0, 10);
  const memos = listAllMemos({ root, fsImpl });
  const params = { minJaccard, topKKeywords, maxConnections };
...
export function runGalaxyCascade({
  ...
  try {
    execImpl(nodeBin, [script], { stdio: "ignore", env });
...
// CLI guard (matches hermes-self-reflect-populater pattern)
const argv1 = (process.argv[1] || "").replace(/\\/g, "/");
const thisUrl = import.meta.url.replace(/\\/g, "/");
if (thisUrl === `file:///${argv1}` || thisUrl.endsWith(argv1)) {
