...
### Triggered Updates
- When approach works well → add to patternsProven
- When approach fails → add to patternsFailed  
- When file/path changes → update criticalPaths
- When tool preference changes → update toolPreferences
...
```
materialCreation: "template → modify → validate → write"
largeFiles: "header(write) → chunks(append) → verify"
extraction: "index → read → parse → document → save"
```
...
### DNA Health Check
```
✓ lastUpdated within last session
✓ essence.position matches actual state
✓ criticalPaths are valid
