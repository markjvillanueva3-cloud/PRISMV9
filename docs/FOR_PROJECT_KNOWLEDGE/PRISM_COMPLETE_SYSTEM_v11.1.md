...
## Contains: ALL protocols, skills, HOOK SYSTEM, automation, enforcement, MATHEMATICAL CERTAINTY
...
**Hook: session:preStart fires automatically**
...
IF currentTask.status = "IN_PROGRESS":
    -> RESUME from checkpoint (SYS-LAW6-CONTINUITY enforces)
    -> DO NOT restart
...
CHECK: Any calibrations stale (>30 days)? (SYS-CALIBRATION-MONITOR alerts)
...
```
MATHPLAN GATE (MANDATORY - SYS-MATHPLAN-GATE enforces)
---------------------------------------------------------
...
ALL CHECKED? -> Proceed    ANY UNCHECKED? -> BLOCKED by SYS-MATHPLAN-GATE
```
...
# SECTION 3: HARD STOPS (NON-NEGOTIABLE - HOOKS ENFORCE)
...
147 hook points across 25 categories execute at every operation.
...
```
Fires: agent:*, swarm:* hooks

### Manufacturing Analysis (8 Experts)
```powershell
py -3 C:\PRISM\scripts\prism_orchestrator_v3.py --manufacturing "Material" "Operation"
```
Fires: agent:* hooks
...
U(x) = Uncertainty coverage [0-1]
D(x) = Dimensional consistency [0-1]
E(x) = Evolution compliance [0-1]
V(x) = Verification coverage [0-1]
```

---

# SECTION 9: PLANNING FORMULAS v2.0 (Hook-Aware)
...
COORD_FACTOR = 1 + (agents-1) x 0.05
VERIFY_FACTOR = 1 + (levels x 0.08)
```
...
BAD: 412 tool calls           <- BLOCKED by SYS-CMD5-UNCERTAINTY
BAD: About 400 calls          <- BLOCKED
```
...
## Level 0: Always-On (5)
prism-deep-learning, prism-formula-evolution, prism-uncertainty-propagation,
prism-mathematical-planning (MATHPLAN), **prism-hook-system** (NEW)
...
# SECTION 13: BUFFER ZONES (SYS-BUFFER-ZONE Enforced)

| Zone | Tool Calls | Required Action |
|------|------------|-----------------|
| GREEN | 0-8 | Work freely |
| YELLOW | 9-14 | Checkpoint after current unit |
| ORANGE | 15-18 | Checkpointing NOW |
| RED | 19+ | BLOCKED - Checkpoint required |
...
| uncertainty, confidence | prism-uncertainty-propagation |
| estimate, predict | prism-mathematical-planning |
...
**95% confidence required (SYS-LAW7-VERIFICATION enforces)**

---

# SECTION 16: SESSION PROTOCOLS

## Session Start (Hooks: session:preStart, session:postStart)
...
## During Execution (Hooks: task:*, microsession:*, db:*)
...
## Session End (Hooks: session:preEnd, learning:extract)
...
# SECTION 17: PREDICTION LOGGING (SYS-PREDICTION-LOG Enforced)
...
```json
{
  ...
  "task": "Description",
  "predicted": {"effort": {"value": 412, "uncertainty": 85, "ci": 0.95}},
  "actual": null,
...
## If Task Restarting (SYS-LAW6-CONTINUITY blocks)
1. STOP immediately
2. Read CURRENT_STATE.json
3. If IN_PROGRESS: Resume from checkpoint

## If S(x) < 0.70 (SYS-LAW1-SAFETY blocks)
1. BLOCKED automatically
2. Request additional verification
3. Do NOT proceed until S(x) >= 0.70

## If M(x) < 0.60 (SYS-LAW8-MATH-EVOLUTION blocks)
1. BLOCKED automatically
2. Add uncertainty to all outputs
3. Verify formulas calibrated
4. Check units consistent

## If Formula MAPE > 20%
1. Flag formula for recalibration
2. Increase uncertainty bounds x1.5
...
HOOKS:        147 (25 categories, 15 system hooks)
FORMULAS:     15 (2 at v2.0 hook-aware)
COEFFICIENTS: 32 (+9 hook-related)
MATERIALS:    1,540+ @ 127 parameters
MONOLITH:     986,621 lines | 831 modules

ENFORCEMENT:
* 8 Always-On Laws (auto-enforced by 15 hooks)
* 15 Commandments (3 auto-enforced)
* State verification gate
* MATHPLAN gate
* Microsession decomposition
* Resume enforcement
* Buffer zone checkpoints
* Safety S(x) >= 0.70
* Rigor M(x) >= 0.60
