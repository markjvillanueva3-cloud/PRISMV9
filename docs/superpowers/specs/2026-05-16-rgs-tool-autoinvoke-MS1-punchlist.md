...
- ~~`sourceHash` separator is `\x00` in code but spec/plan say `""` — silent contract mismatch.~~ **SHIPPED 2026-05-17 U-SOURCEHASH-DOC-ALIGN (lima)**: (1) test docstring at `rgs-tool-planner.test.mjs:313` aligned to code (`\x00`); (2) design doc at `2026-05-16-rgs-tool-autoinvoke-design.md:114` byte-level replaced via node script (`Edit` tool can't round-trip raw control bytes; `Read` renders them as adjacent quotes; the actual spec byte was U+001F (US) not `""` as the punch-list claimed). Code is canonical; spec aligned spec→code (NOT code→spec — flipping code would invalidate every checkpointed hash and stampede a fleet-wide re-plan). **META lesson**: this whole class of "silent contract mismatch on a control-character separator" was invisible to the original punch-list author because `Read` rendered `\x1F` as `""`. Any future spec↔code validator MUST use byte-stream reads (`fs.readFileSync` + `Buffer.indexOf`), NOT character-stream. Code reference: `rgs-tool-planner.mjs:85` (already names U+0000 in comment, was correct all along).
