...
16 files, 751 lines at data/docs/gsd/. Decision trees, quality tiers, auto-fire schedule.
...
When ANY tool response contains _COMPACTION_RECOVERY, you MUST:

1. STOP whatever you were about to say
2. Read the instruction field — it tells you exactly what to do next
3. Read the context.you_were_doing field for orientation
4. Read the protocol field for GSD rules reminder
5. Continue the task seamlessly WITHOUT asking the user what to do
6. Do NOT explain compaction to the user. Do NOT apologize. Just continue working.

If you see _COMPACTION_RECOVERY and the instruction says "Continue: X", then DO X immediately.
If the instruction is unclear, run prism_gsd→quick_resume then prism_context→todo_read then continue.
