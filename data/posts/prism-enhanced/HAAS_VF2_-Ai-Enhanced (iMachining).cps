/**
  ...
  HAAS VF-2 post processor configuration - Enhanced Edition

  $Revision: 44100 Enhanced for VF-2 $
  $Date: 2025-12-22 $
...
///////////////////////////////////////////////////////////////////////////////
//                        HAAS VF-2 ENHANCED POST
//
// DYNAMIC DEPTH FEED FOR 3D ADAPTIVE (KEY TO FAST ADAPTIVE!):
...
//   - "Adaptive base feedrate" - 0 = use Fusion programmed feed
//
// iMACHINING-STYLE VARIABLE FEED CONTROL:
//   Maintains constant chip thickness by adjusting feed based on:
...
//   - Axial depth adjustment (only increases for adaptive)
...
//   - Full G28/G53 still used for tool changes
//
// G187 SMOOTHING (Enhanced):
//   - Finishing face/contour operations → P3 (best surface finish)
//   - Adaptive roughing or any operation with stock to leave → P2 (medium)
//   - P1=Rough, P2=Medium, P3=Finish
//
// HAAS COOLANT CODES:
//   M8=Flood, M88/M89=TSC, M83/M84=Air blast, M73/M74=Air through spindle
//
// OTHER: Safe start, chip conveyor, probing, subroutines
//
///////////////////////////////////////////////////////////////////////////////
...
longDescription = "Enhanced post for HAAS VF-2 and similar pre-NGC mills. Features: iMachining-style variable feedrate (arc correction, direction change detection, feed ramping), dynamic depth feed adjustment for 3D adaptive (INCREASES feed for shallow cuts), advanced feed optimization with chip thinning compensation, minimum Z retract between work offsets, enhanced G187 smoothing (P3 for finishing face/contour, P2 for adaptive/roughing with stock to leave). Supports both standard and aggressive machining strategies.";
...
maximumCircularSweep = toRad(1000);
...
properties = {
  ...
  // =============== ENHANCED FEED OPTIMIZATION PROPERTIES ===============
  // Minimum Z Retract Between Work Offsets
  useMinimumZRetract: {
    title      : "Use minimum Z retract between WCS",
    description: "When changing work offsets (NOT tool changes), retract only to clearance above stock instead of full G28/G53. Saves cycle time on multi-fixture setups.",
    group      : "homePositions",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  minimumZRetractClearance: {
    ...
    description: "Distance above stock top for minimum retract between work offsets. Only used when 'Use minimum Z retract' is enabled.",
    group      : "homePositions",
    type       : "spatial",
    value      : 1.0,
    scope      : "post"
  ...
  useIMachiningFeed: {
    ...
    description: "Adjusts feedrate based on cutting conditions similar to SolidCAM iMachining. Includes arc feed correction, direction change detection, and feed ramping.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  iMachiningLevel: {
    ...
    description: "Similar to SolidCAM's slider. Level 1 is most conservative (50% of programmed feed), Level 8 is most aggressive (100% of programmed feed).",
    group      : "preferences",
    type       : "integer",
    range      : [1, 8],
    value      : 5,
  ...
  arcFeedCorrection: {
    ...
    description: "Percentage of programmed feed to use when cutting arcs. Smaller arcs get more reduction. 100 = apply full correction, 0 = no correction.",
    group      : "preferences",
    type       : "integer",
    range      : [0, 100],
    value      : 100,
  ...
  directionChangeFeedReduction: {
    ...
    description: "Maximum feed reduction at sharp direction changes. 25 = reduce up to 25% at sharp corners.",
    group      : "preferences",
    type       : "integer",
    range      : [0, 50],
    value      : 15,
  ...
  feedRampingDistance: {
    ...
    description: "Distance over which to smoothly transition feedrates. Prevents sudden load spikes. 0 = instant changes.",
    group      : "preferences",
    type       : "spatial",
    value      : 0,
    scope      : "post"
  ...
  minChipLoadFeed: {
    ...
    description: "Never reduce feedrate below this percentage of programmed feed. Prevents rubbing.",
    group      : "preferences",
    type       : "integer",
    range      : [20, 100],
    value      : 50,
  ...
  toolStickoutMultiplier: {
    ...
    description: "How aggressively to reduce feed for long stickout. 1.0 = standard, 0.5 = aggressive, 1.5 = conservative.",
    group      : "preferences",
    type       : "number",
    value      : 1.0,
    scope      : "post"
  ...
  maxStickoutRatio: {
    ...
    description: "Maximum stickout-to-diameter ratio before feed reduction. Typical: 3-4 for roughing.",
    group      : "preferences",
    type       : "number",
    value      : 4.0,
    scope      : "post"
  ...
  adaptiveDepthFeedAdjust: {
    ...
    description: "For adaptive: INCREASE feed when cutting shallow, maintain at full depth.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  finishingStickoutTolerance: {
    ...
    description: "For finishing, allow higher stickout before reducing feed. Typically 1.5x roughing.",
    group      : "preferences",
    type       : "number",
    value      : 6.0,
    scope      : "post"
  ...
  roughingOptimalWOC: {
    ...
    description: "Optimal width of cut for roughing as % of tool diameter.",
    group      : "preferences",
    type       : "integer",
    range      : [5, 50],
    value      : 15,
  ...
  finishingMaxWOC: {
    ...
    description: "Maximum width of cut for finishing as % of tool diameter.",
    group      : "preferences",
    type       : "integer",
    range      : [5, 100],
    value      : 35,
  ...
  // Dynamic Feed Adjustment for 3D Adaptive
  useDynamicDepthFeed: {
  ...
  // Variable Feedrate Controls
  forceFeedOutput: {
    title      : "Force feedrate on every line",
    description: "Output F on every motion line. Essential for adaptive toolpaths with variable feeds.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  showFeedComments: {
    ...
    description: "Add comments showing movement type and feed calculations.",
    group      : "formats",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
function initializeSmoothing() {
  ...
  // Get operation strategy and stock to leave values
  var strategy = "";
  if (hasParameter("operation-strategy")) {
    strategy = getParameter("operation-strategy").toLowerCase();
  }
  ...
  var hasStockToLeave = (stockToLeave > 0) || (verticalStockToLeave > 0);
  
  // Detect operation types
  var isAdaptiveOperation = strategy.indexOf("adaptive") >= 0;
  var isFaceOperation = strategy.indexOf("face") >= 0;
  var isContourOperation = strategy.indexOf("contour") >= 0 || strategy.indexOf("profile") >= 0;
  var isFinishingOperation = !hasStockToLeave; // No stock to leave = finishing

  // automatically determine smoothing level
  if (smoothing.level == 9999) {
    // CUSTOM SMOOTHING LOGIC:
    // P3 (Finish) = Finishing face or finishing contour (no stock to leave)
    // P2 (Medium) = Adaptive, roughing, or anything with stock to leave
    // P1 (Rough) = Reserved for heavy roughing if needed
    
    if (isFinishingOperation && (isFaceOperation || isContourOperation)) {
      // Finishing face or finishing contour -> P3 (best surface finish)
      smoothing.level = 3; // finishing
    } else if (isAdaptiveOperation || hasStockToLeave) {
      // Adaptive roughing OR any operation with stock to leave -> P2 (medium)
      smoothing.level = 2; // medium/semi
    } else if (isFinishingOperation) {
      // Other finishing operations (not face/contour) -> P3
      smoothing.level = 3; // finishing
    } else {
      // Default fallback -> P2 (medium)
      smoothing.level = 2; // medium/semi
    }
  }
...
//
// Similar to SolidCAM iMachining, these functions adjust feedrate based on
// cutting conditions to maintain constant chip thickness and prevent
// sudden load spikes.
///////////////////////////////////////////////////////////////////////////////
...
// Variables for tracking motion during cutting
...
/**
  ...
  Level 1 = 0.5 (50% feed), Level 8 = 1.0 (100% feed)
*/
...
/**
  ...
  Smaller arcs require slower feeds due to increased engagement.
*/
function calculateArcFeedFactor(arcRadius, toolRadius) {
  ...
  // Ratio of arc radius to tool radius
  var radiusRatio = arcRadius / toolRadius;
  
  // Very small arcs (ratio < 2) get significant reduction
  // Larger arcs (ratio > 5) get minimal reduction
  var reductionFactor;
  if (radiusRatio < 1) {
...
/**
  ...
  Sharp direction changes require slower feeds.
*/
function calculateDirectionChangeFactor(currentDir, prevDir) {
  ...
  // No reduction for angles less than 15 degrees
  if (angle < Math.PI / 12) {
    return 1.0;
  }
  
  // Progressive reduction based on angle
...
function applyIMachiningFeed(f, isArc, arcRadius, currentPos, moveDistance) {
  ...
  // Apply direction change correction (for linear moves)
...
var currentRadialDepth = 0;
var currentAxialDepth = 0;
var currentStickout = 0;
...
function getCuttingParameters() {
  ...
  // Get Z boundaries for dynamic depth
  ...
  // Classify operation - ADAPTIVE is always ROUGHING for feed purposes
...
function calculateChipThinningFactor(radialDepth, toolDiameter) {
  ...
  if (result.factor > 1.2) {
    ...
    result.suggestion = "Light chip thinning applied";
  }
...
/**
  Calculate axial depth factor - for adaptive, only INCREASE feed for shallow cuts.
*/
function calculateAxialDepthFactor(axialDepth, toolDiameter, fluteLength, isFinishing, isAdaptive) {
  ...
  if (isAdaptive) {
    // ADAPTIVE: Only INCREASE feed for shallow cuts, NEVER reduce
    if (result.depthRatio < optimalDepthRatio * 0.5) {
      ...
      var shallowRatio = result.depthRatio / optimalDepthRatio;
      result.factor = 1.0 + ((1.0 - shallowRatio) * 0.3);
    ...
    // Non-adaptive roughing
    var maxSafeDepthRatio = fluteLength > 0 ? (fluteLength / toolDiameter) * 0.9 : 2.5;
    if (result.depthRatio <= maxSafeDepthRatio) {
      result.factor = 1.0;
      result.suggestion = "Good DOC for roughing";
    } else {
      result.factor = maxSafeDepthRatio / result.depthRatio;
      result.suggestion = "Deep cut - feed reduced";
    }
  } else {
    // Finishing
    if (result.depthRatio <= optimalDepthRatio) {
      ...
      result.suggestion = "Deep finish cut - feed reduced";
    }
  }
...
/**
  Calculate 3D adaptive factor - only INCREASE, never reduce for adaptive.
*/
function calculate3DAdaptiveFactor(radialStepover, axialStepover, toolDiameter, isRoughing, isAdaptive) {
  var result = {
    factor: 1.0,
    radialPercent: 0,
    axialPercent: 0,
  ...
  if (toolDiameter <= 0 || radialStepover <= 0) {
  ...
  // Only INCREASE feed for light engagement - NEVER decrease for adaptive
  if (result.radialPercent < targetEngagement * 0.75) {
    var increaseRatio = Math.sqrt(targetEngagement / Math.max(result.radialPercent, 1));
    result.factor = Math.min(increaseRatio, getProperty("maxChipThinningMultiplier"));
    result.suggestion = "Light engagement - feed can increase +" + Math.round((result.factor - 1) * 100) + "%";
  } else {
    result.factor = 1.0;
    result.suggestion = "Normal engagement - using programmed feed";
  }
...
/**
  Calculate optimized feed combining all factors.
*/
function calculateOptimizedFeed(baseFeed, params) {
  ...
  if (stickout.warning) result.warnings.push(stickout.warning);
  if (stickout.suggestion) result.notes.push(stickout.suggestion);
  
  var chipThin = calculateChipThinningFactor(params.radialDepth, toolDia);
  result.chipThinningFactor = chipThin.factor;
  if (chipThin.suggestion) result.notes.push(chipThin.suggestion);
  ...
  if (axialF.suggestion) result.notes.push(axialF.suggestion);
  
  if (params.is3D || params.isAdaptive) {
    var adapt3D = calculate3DAdaptiveFactor(params.radialDepth, params.axialDepth, toolDia, params.isRoughing, params.isAdaptive);
    result.adaptive3DFactor = adapt3D.factor;
    if (adapt3D.suggestion) result.notes.push(adapt3D.suggestion);
  }
  
  // Combine factors - for adaptive, only apply INCREASES
  var increaseFactors = [];
  ...
  // For adaptive, ignore reductions
  var reductionProduct = 1.0;
  if (!params.isAdaptive) {
    reductionFactors.forEach(function(f) { reductionProduct *= f; });
  } else if (reductionFactors.length > 0) {
    result.notes.push("Feed reductions disabled for adaptive");
  }
...
/**
  Output optimization notes at start of operation.
*/
...
function onSection() {
  ...
  if (newWorkOffset && !insertToolCall && !isFirstSection() && getProperty("useMinimumZRetract")) {
    ...
    writeBlock(gAbsIncModal.format(90));
    zOutput.reset();
  } else if ((insertToolCall && !getProperty("fastToolChange")) || newWorkOffset || newWorkPlane || toolChecked) {
