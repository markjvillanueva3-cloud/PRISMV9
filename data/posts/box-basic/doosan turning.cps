...
description = "DN Solutions (Doosan) Turning";
...
if (!longDescription) {
  longDescription = subst("Generic post for %1. Use the property 'type' to switch the FANUC mode A, B, and C. The default mode is A. Use Turret 0 for Positional Turret, Turret 101 for QCTP on X- Post, Turret 102 for QCTP on X+ Post, Turret 103 for Gang Tooling on X- Post, Turret 104 for Gang Tooling on X+ Tool Post.", description);
}
...
properties = {
  ...
  type: {
    title      : "Type",
    description: "Specifies the type A, B or C.",
    group      : "formats",
    type       : "enum",
    values     : [
      "A",
  ...
  useWCS: {
    title      : "Output work offset codes",
    description: "Enable to allow work offset codes to be output (G54-G59)).",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
...
var g92QOutput = createVariable({prefix:"Q"}, threadPQFormat);
...
var fpmCode; // fpmCode is defined in onOpen
var fprCode; // fprCode is defined in onOpen
...
function onOpen() {
  fpmCode = getProperty("type") == "A" ? 98 : 94;
  fprCode = getProperty("type") == "A" ? 99 : 95;
...
function getFeed(f) {
  if (gFeedModeModal.getCurrent() == fprCode && currentSection.feedMode != FEED_PER_REVOLUTION) {
    f /= spindleSpeed;
  }
  if (activeMovements) {
...
function onSection() {
  ...
  var tapping = isTappingCycle();
  writeBlock(conditional(getProperty("type") != "A", gAbsIncModal.format(90)), formatFeedMode(tapping ? FEED_PER_REVOLUTION : currentSection.feedMode), gPlaneModal.format(18));
  ...
  if (insertToolCall || tool.getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED) {
    ...
    var modalCode = getProperty("type") != "A" ? gAbsIncModal.format(90) : "";
    if (getProperty("approachStyle") == "ZX") {
      writeBlock(modalCode, gMotionModal.format(0), zOutput.format(initialPosition.z));
      writeBlock(modalCode, gMotionModal.format(0), xOutput.format(initialPosition.x), yOutput.format(initialPosition.y));
    } else {
      writeBlock(modalCode, gMotionModal.format(0), xOutput.format(initialPosition.x), yOutput.format(initialPosition.y), zOutput.format(initialPosition.z));
    }
...
function onDwell(seconds) {
  ...
  writeBlock(gFeedModeModal.format(fpmCode), gFormat.format(4), "P" + milliFormat.format(milliseconds));
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      writeBlock(conditional(getProperty("type") != "A", gAbsIncModal.format(90)), gPlaneModal.format(17), gMotionModal.format(directionCode), iOutput.format(cx - start.x, 0), jOutput.format(cy - start.y, 0), getFeed(feed));
      break;
    case PLANE_ZX:
      writeBlock(conditional(getProperty("type") != "A", gAbsIncModal.format(90)), gPlaneModal.format(18), gMotionModal.format(directionCode), iOutput.format(cx - start.x, 0), kOutput.format(cz - start.z, 0), getFeed(feed));
      break;
    case PLANE_YZ:
      writeBlock(conditional(getProperty("type") != "A", gAbsIncModal.format(90)), gPlaneModal.format(19), gMotionModal.format(directionCode), jOutput.format(cy - start.y, 0), kOutput.format(cz - start.z, 0), getFeed(feed));
      break;
    default:
      linearize(tolerance);
    }
    ...
    switch (getCircularPlane()) {
    case PLANE_XY:
      writeBlock(conditional(getProperty("type") != "A", gAbsIncModal.format(90)), gPlaneModal.format(17), gMotionModal.format(directionCode), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x, 0), jOutput.format(cy - start.y, 0), getFeed(feed));
      break;
    case PLANE_ZX:
      writeBlock(conditional(getProperty("type") != "A", gAbsIncModal.format(90)), gPlaneModal.format(18), gMotionModal.format(directionCode), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x, 0), kOutput.format(cz - start.z, 0), getFeed(feed));
      break;
    case PLANE_YZ:
      writeBlock(conditional(getProperty("type") != "A", gAbsIncModal.format(90)), gPlaneModal.format(19), gMotionModal.format(directionCode), xOutput.format(x), yOutput.format(y), zOutput.format(z), jOutput.format(cy - start.y, 0), kOutput.format(cz - start.z, 0), getFeed(feed));
      break;
    default:
      linearize(tolerance);
    }
...
function onCyclePoint(x, y, z) {
  ...
  case "thread-turning":
    ...
    if (getProperty("useSimpleThread") ||
      ...
      writeBlock(
        gMotionModal.format(codes[getProperty("type")]),
  ...
  if (isFirstCyclePoint()) {
    ...
    case "counter-boring":
      if (P > 0) {
        writeBlock(
          conditional(getProperty("type") != "A", gAbsIncModal.format(90)), conditional(getProperty("type") != "A", gRetractModal.format(98)), gCycleModal.format(82),
          getCommonCycle(x, y, z, cycle.retract),
    ...
    case "chip-breaking":
      if ((cycle.accumulatedDepth < cycle.depth) || (P > 0)) {
        ...
        writeBlock(
          conditional(getProperty("type") != "A", gAbsIncModal.format(90)), conditional(getProperty("type") != "A", gRetractModal.format(98)), gCycleModal.format(73),
          getCommonCycle(x, y, z, cycle.retract),
    ...
    case "tapping":
      ...
      writeBlock(
        conditional(getProperty("type") != "A", gAbsIncModal.format(90)), conditional(getProperty("type") != "A", gRetractModal.format(98)), gCycleModal.format((tool.type == TOOL_TAP_LEFT_HAND) ? 74 : 84),
        getCommonCycle(x, y, z, cycle.retract),
        "P" + milliFormat.format(P),
        getFeed(cycle.feedrate)
      );
    ...
    case "left-tapping":
      ...
      writeBlock(
        conditional(getProperty("type") != "A", gAbsIncModal.format(90)), conditional(getProperty("type") != "A", gRetractModal.format(98)), gCycleModal.format(74),
        getCommonCycle(x, y, z, cycle.retract),
        "P" + milliFormat.format(P),
        getFeed(cycle.feedrate)
      );
    ...
    case "right-tapping":
      ...
      writeBlock(
        conditional(getProperty("type") != "A", gAbsIncModal.format(90)), conditional(getProperty("type") != "A", gRetractModal.format(98)), gCycleModal.format(84),
        getCommonCycle(x, y, z, cycle.retract),
        "P" + milliFormat.format(P),
        getFeed(cycle.feedrate)
      );
    ...
    case "fine-boring":
      writeBlock(
        conditional(getProperty("type") != "A", gAbsIncModal.format(90)), conditional(getProperty("type") != "A", gRetractModal.format(98)), gCycleModal.format(76),
        getCommonCycle(x, y, z, cycle.retract),
    ...
    case "reaming":
      ...
      if (P > 0) {
        writeBlock(
          conditional(getProperty("type") != "A", gAbsIncModal.format(90)), conditional(getProperty("type") != "A", gRetractModal.format(98)), gCycleModal.format(89),
          getCommonCycle(x, y, z, cycle.retract),
          "P" + milliFormat.format(P),
          getFeed(cycle.feedrate)
        );
        ...
        writeBlock(
          conditional(getProperty("type") != "A", gAbsIncModal.format(90)), conditional(getProperty("type") != "A", gRetractModal.format(98)), gCycleModal.format(85),
          getCommonCycle(x, y, z, cycle.retract),
          getFeed(cycle.feedrate)
        );
      }
    ...
    case "stop-boring":
      if (P > 0) {
        ...
        writeBlock(
          conditional(getProperty("type") != "A", gAbsIncModal.format(90)), conditional(getProperty("type") != "A", gRetractModal.format(98)), gCycleModal.format(86),
          getCommonCycle(x, y, z, cycle.retract),
          getFeed(cycle.feedrate)
        );
      }
...
function startSpindle(tappingMode, forceRPMMode, initialPosition) {
  ...
  if (tool.getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED) {
    ...
    if (forceRPMMode) { // RPM mode is forced until move to initial position
      ...
      writeBlock(gFormat.format(getProperty("type") == "A" ? 50 : 92), sOutput.format(maximumSpindleSpeed));
