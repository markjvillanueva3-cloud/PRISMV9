...
var chipConveyorOutput = createOutputVariable({}, mFormat);
...
var settings = {
  ...
  retract: {
    ...
    homeXY                    : {onIndexing:{axes:[X, Y]}, onToolChange:{axes:[X, Y]}, onProgramEnd:{axes:[X, Y]}} // Specifies when the machine should be homed in X/Y. Sample: onIndexing:{axes:[X, Y], singleLine:false}
  },
  parametricFeeds: {
...
function onOpen() {
  ...
  if (!receivedMachineConfiguration) { // set default home positions when no machine configuration is defined
...
function setSmoothing(mode) {
  ...
  if (mode) { // enable smoothing
    if (getProperty("smoothingCycle") == "32") {
      ...
      writeBlock("CYCL DEF 32.2 HSC-MODE:" + smoothing.level, conditional(tcp.isSupportedByOperation, "TA0.5")); // required for 5-axis support
    } else {
      var cycle332Mode = 0; // default
      if (smoothing.tolerance > toPreciseUnit(0.05, MM)) {
        cycle332Mode = 3;
      } else if (smoothing.tolerance < toPreciseUnit(0.002, MM)) {
        cycle332Mode = 1;
      }
      writeBlock("CYCL DEF 332", localize("PARAMETER SWITCHING"), "~" + EOL
      + "  Q1422=" + cycle332Mode, ";" + localize("MODE"), "~" + EOL
      + "  Q1423=" + toleranceFormat.format(smoothing.tolerance * scaleFactor), ";" + localize("PATH TOLERANCE"), "~" + EOL
      + "  Q1424=" + toleranceFormat.format(toPreciseUnit(0.1, MM)), ";" + localize("TA TOLERANCE"), "~" + EOL
      + "  Q1425=" + smoothing.level, ";" + localize("HSC-MODE")
      );
    }
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    // force XY home position after tool change
  ...
  if (getProperty("useChipConveyor") != "disabled") {
    ...
    case "M24":
      writeBlock(chipConveyorOutput.format(24)); // interval
      break;
    case "M25":
      writeBlock(chipConveyorOutput.format(25)); // continuous
      break;
    case "M26":
      writeBlock(chipConveyorOutput.format(26)); // stop
      break;
    }
  }
...
function onCommand(command) {
  ...
  case COMMAND_START_CHIP_TRANSPORT:
    writeBlock(chipConveyorOutput.format(24));
    return;
  case COMMAND_STOP_CHIP_TRANSPORT:
    writeBlock(chipConveyorOutput.format(26));
    return;
  case COMMAND_BREAK_CONTROL:
    return;
