...
function onOpen() {
  ...
  formatOutputVariables(); // format output variables based on machine configuration
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    ...
    if (outputClampCodes && machineConfiguration.isMultiAxisConfiguration()) {
      ...
      if (machineConfiguration.getNumberOfAxes() > 4) {
        writeBlock(fifthAxisClamp.format(110)); // lock 5th axis
      }
    }
    return;
  case COMMAND_UNLOCK_MULTI_AXIS:
    ...
    if (outputClampCodes && machineConfiguration.isMultiAxisConfiguration()) {
      ...
      if (machineConfiguration.getNumberOfAxes() > 4) {
        writeBlock(fifthAxisClamp.format(111)); // unlock 5th axis
      }
    }
    return;
