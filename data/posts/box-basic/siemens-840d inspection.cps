...
// >>>>> INCLUDED FROM inspection/common/siemens base inspection properties.cps
properties.variableStart = {
  title      : "Macro variable start",
  description: "Specify the starting value for the R macro variables to be used for inspection toolpaths." + EOL +
    "Inspection toolpaths require 15 variables to be available, 50 additional variables are required when using the live connection functionality.",
  group: "probing",
  type : "integer",
  value: 1,
  range: [1, 785], // R1 - R999 are available on the control, limited range to 1-785 since we need 15 variables for inspection toolpaths
  scope: "post"
};
...
var inspectionVariables = {
  ...
  availableVariableRange         : new Range(1, 999), // do not change, R1 - R999 are available on the control
  startVariableInspection        : properties.variableStart.value, // the start variable of the inspection toolpaths, overwritten by property 'variableStart'
  requiredVariablesInspection    : 15, // the number of variables required for inspection toolpaths, will be set to the correct value in the inspectionWriteVariables function
  reservedVariableRangeInspection: new Range(1, 799), // the reserved variable range for inspection toolpaths
...
// <<<<< INCLUDED FROM inspection/common/siemens base inspection properties.cps
// >>>>> INCLUDED FROM inspection/common/siemens base inspection.cps
...
function inspectionWriteVariables() {
  // assign R variables used for Inspection
  var macroCount = getProperty("variableStart");
  inspectionVariables.startVariableInspection = macroCount;
  var macroPrefix = inspectionVariables.localVariablePrefix;
  inspectionVariables.probeRadius = macroPrefix + macroCount; // R1
  ...
  inspectionVariables.macroVariable1 = macroPrefix + ++macroCount;
  inspectionVariables.macroVariable2 = macroPrefix + ++macroCount;
  inspectionVariables.macroVariable3 = macroPrefix + ++macroCount;
  inspectionVariables.macroVariable4 = macroPrefix + ++macroCount;
  inspectionVariables.macroVariable5 = macroPrefix + ++macroCount;
  inspectionVariables.macroVariable6 = macroPrefix + ++macroCount;
  inspectionVariables.macroVariable7 = macroPrefix + ++macroCount;
  inspectionVariables.requiredVariablesInspection = macroCount - inspectionVariables.startVariableInspection; // update the required number of variables
  ...
  for (var i = 0; i < getNumberOfSections(); ++i) {
    ...
    if (isInspectionOperation(section)) {
      ...
      if (!getProperty("useLiveConnection") && inspectionVariables.inspectionSections == 0 && getProperty("commissioningMode")) {
  ...
  if (inspectionVariables.inspectionSections > 0) {
    validate(
      inspectionVariables.reservedVariableRangeInspection.isWithin(inspectionVariables.startVariableInspection) &&
      inspectionVariables.reservedVariableRangeInspection.isWithin(macroCount),
      subst("The property '%1' value of '%2' is out of the allowed range. It must be within the range of %3 to %4.",
        properties.variableStart.title,
        inspectionVariables.startVariableInspection,
        inspectionVariables.reservedVariableRangeInspection[0],
        inspectionVariables.reservedVariableRangeInspection[1] - (macroCount - inspectionVariables.startVariableInspection)
      )
    );
  }
  if (getProperty("useLiveConnection")) {
    warning(localize("Live connection is in beta and not fully tested, please report any issues to Autodesk support."));
...
function inspectionCycleInspect(cycle, epx, epy, epz) {
  ...
  if (isFirstCyclePoint() || isLastCyclePoint()) {
    ...
    if (isFirstCyclePoint()) {
      ...
      forceXYZ();
      if (cycle.outOfPositionAction == "stop-message" && !getProperty("useLiveConnection")) {
...
// >>>>> INCLUDED FROM inspection/common/siemens live connection.cps
// Live Connection support
properties.useLiveConnection = {
  title      : "Live device connection (beta)",
  description: "This feature is in beta. Creates a live connection between the controller and Fusion, used for live importing of measurement results and toolpath tracking",
  group      : "probing",
  type       : "boolean",
  value      : false,
  scope      : "post"
...
inspectionVariables.startVariableLiveConnection = 800; // default start variable of the live connection buffer, will be set to the correct value after reading the machine definition
inspectionVariables.requiredVariablesLiveConnection = 49; // // the number of variables required for live connection
inspectionVariables.reservedVariableRangeLiveConnection = new Range(800, inspectionVariables.availableVariableRange[1]); // the reserved variable range for live connection

inspectionVariables.liveAlignmentCapability = 0; // 0 = translations only (3 axis), 1 = translation and rotation (4 axis), 2 = full translation and rotations (5 axis)
inspectionVariables.bufferVersion = 2;
inspectionVariables.bufferCapacity = 4;

function liveConnectionHeader() {
  if (hasGlobalParameter("machine-v2")) {
    try {
      inspectionVariables.startVariableLiveConnection = JSON.parse(getGlobalParameter("machine-v2")).connection.default.rpc.data_buffer;
    } catch (e) {
      warning(localize("For Live Connection functionality, a machine with connector capabilities must be in use."));
    }
  }

  validate(
    inspectionVariables.reservedVariableRangeLiveConnection.isWithin(inspectionVariables.startVariableLiveConnection) &&
    inspectionVariables.reservedVariableRangeLiveConnection.isWithin(inspectionVariables.startVariableLiveConnection + inspectionVariables.requiredVariablesLiveConnection),
    subst("The Machine connector Data Buffer value of %1 is outside the allowed range." + EOL +
      "Please enter a value between %2-%3 for the Machine Connector Data Buffer in the Machine Definition.",
    inspectionVariables.startVariableLiveConnection,
    inspectionVariables.reservedVariableRangeLiveConnection[0],
    inspectionVariables.reservedVariableRangeLiveConnection[1] - inspectionVariables.requiredVariablesLiveConnection
    )
  );
  ...
  if (getProperty("showSequenceNumbers")) {
    ...
    setProperty("showSequenceNumbers", "false");
    inspectionVariables.saveShowSequenceNumbers = "false"; // ensure showSequenceNumbers never gets turned back on again when using live connection
  }

  // initialize variables for live connection
  var prefix = inspectionVariables.localVariablePrefix;
  var addr = inspectionVariables.startVariableLiveConnection;
  inspectionVariables.liveConnectionVersion = prefix + addr; // R800
  inspectionVariables.liveConnectionCapacity = prefix + ++addr;
  inspectionVariables.liveConnectionReadPointer = prefix + ++addr;
  inspectionVariables.liveConnectionWritePointer = prefix + ++addr;
  inspectionVariables.liveConnectionStatus = prefix + ++addr;
  inspectionVariables.workplaneStartAddress = prefix + ++addr;
  inspectionVariables.liveConnectionWPA = prefix + ++addr;
  inspectionVariables.liveConnectionWPB = prefix + ++addr;
  inspectionVariables.liveConnectionWPC = prefix + ++addr;
  inspectionVariables.probeRadius = prefix + ++addr; // override
  inspectionVariables.commandID = prefix + ++addr; // R810
  inspectionVariables.commandArg1 = prefix + ++addr;
  inspectionVariables.commandArg2 = prefix + ++addr;
  inspectionVariables.commandArg3 = prefix + ++addr;
  inspectionVariables.commandArg4 = prefix + ++addr;
  inspectionVariables.commandArg5 = prefix + ++addr;
  inspectionVariables.commandArg6 = prefix + ++addr;
  inspectionVariables.commandArg7 = prefix + ++addr;
  inspectionVariables.commandArg8 = prefix + ++addr;
  inspectionVariables.commandArg9 = prefix + ++addr;
  inspectionVariables.probeResultsStartAddress = ++addr; // R820
  ...
  writeBlock("IF [" + inspectionVariables.liveConnectionStatus, "<> -1]");
  writeBlock(inspectionVariables.liveConnectionStatus, "= 1");
  writeBlock("ENDIF");
  writeBlock("IF [" + inspectionVariables.liveConnectionStatus, "== -1]");
  writeBlock(inspectionVariables.liveConnectionStatus, "= 3");
  writeBlock("ENDIF");
  ...
  writeBlock(inspectionVariables.commandArg1, "= 0");
  writeBlock(inspectionVariables.commandArg2, "= 0");
  writeBlock(inspectionVariables.commandArg3, "= 0");
  writeBlock(inspectionVariables.commandArg4, "= 0");
  writeBlock(inspectionVariables.commandArg5, "= 0");
  writeBlock(inspectionVariables.commandArg6, "= 0");
  writeBlock(inspectionVariables.commandArg7, "= 0");
  writeBlock(inspectionVariables.commandArg8, "= 0");
  writeBlock(inspectionVariables.commandArg9, "= 0");

  writeBlock("WHILE [" + inspectionVariables.liveConnectionStatus, "<> -1]");
...
function liveConnectionWriteData(type) {
  var value = inspectionVariables.probeResultsStartAddress + 6 * inspectionVariables.probeResultsBufferIndex;
  var pathTypeID = macroFormat.format(value); // path types 0=Information / 1=inspection / 2=macro probing / 3=milling / 4=additive
  var toolpathID = macroFormat.format(value + 1);
  var toolpathInfo1 = macroFormat.format(value + 2);
  var toolpathInfo2 = macroFormat.format(value + 3);
  var toolpathInfo3 = macroFormat.format(value + 4);
  ...
  case "toolpathStart":
    ...
    writeBlock(toolpathInfo2, "= [$A_HOUR*10000 + $A_MINUTE*100 + $A_SECOND]"); // HHMMSS
  ...
  case "toolpathEnd":
    ...
    writeBlock(toolpathInfo2, "= [$A_HOUR*10000 + $A_MINUTE*100 + $A_SECOND]");
    writeBlock(toolpathInfo3, "= 0");
    writeBlock(toolpathInfo4, "= 0");
    inspectionVariables.probeResultsBufferIndex += 1;
    break;
  case "toolpathAlignment":
    ...
    writeBlock(toolpathInfo2, "=", inspectionVariables.liveAlignmentCapability);
    writeBlock(toolpathInfo3, "= 0");
    writeBlock(toolpathInfo4, "= 0");
    inspectionVariables.probeResultsBufferIndex += 1;
    break;
  ...
  case "inspectSurfaceAlarm":
    onDwell(1);
    writeBlock("IF [" + inspectionVariables.commandID, "== 2]");
    writeBlock(inspectionVariables.commandID, "= -2");
    writeBlock("ENDIF");
    writeBlock("IF [[" + inspectionVariables.commandID, "== -2] AND [" + inspectionVariables.commandArg1, "== 1]]");
    writeBlock("SETAL(62990,\"OUT OF POSITION TOLERANCE\")");
    writeBlock("ENDIF");
    writeBlock("IF [[" + inspectionVariables.commandID, "== -2] AND [" + inspectionVariables.commandArg1, "== 2]]");
    writeBlock("SETAL(62990,\"-OUT OF POSITION TOLERANCE-\")"); // Point unprojected alarm
    writeBlock("ENDIF");
    writeBlock("IF [[" + inspectionVariables.commandID, "== -2] AND [" + inspectionVariables.commandArg1, "== 3]]");
    writeBlock("SETAL(62992,\"PART_ALIGNMENT_ALARM\")");
    writeBlock("ENDIF");
    writeBlock("IF [[" + inspectionVariables.commandID, "== -2] AND [" + inspectionVariables.commandArg1, "== 4]]");
    writeBlock("SETAL(62993,\"UNSPECIFIED\")");
    writeBlock("ENDIF");
    writeBlock("IF [" + inspectionVariables.commandID, "== -2]");
    writeBlock(inspectionVariables.commandID, "= 0");
    writeBlock("ENDIF");
  ...
  case "macroEnd":
    ...
    case "probing-xy-circular-partial-hole-with-island":
      macroType = 0.002;
      data1 = "_OVR [4]"; // diameter
      data2 = "_OVR [5]"; // x position
      data3 = "_OVR [6]"; // y position
      break;
    case "probing-xy-inner-corner":
    case "probing-xy-outer-corner":
      macroType = 0.003;
      data3 = "_OVR [5]"; // x position
      data4 = "_OVR [6]"; // y position
      break;
    case "probing-x-plane-angle":
    case "probing-y-plane-angle":
      macroType = 0.005;
      data4 = "_OVR [20]"; // angle error
      break;
    case "probing-xy-rectangular-boss":
    case "probing-xy-rectangular-hole":
    case "probing-xy-rectangular-hole-with-island":
      macroType = 0.009;
      data1 = "_OVR [4]"; // x size
      data2 = "_OVR [6]"; // x position
      data3 = "_OVR [7]"; // y position
      data4 = "_OVR [5]"; // y size
      break;
    default:
    ...
    writeBlock(pathTypeID, "=", (2 + macroType + 0.00001));
    writeBlock(toolpathID, "=", inspectionGetToolpathId(currentSection));
...
function liveConnectorInterface(type) {
  ...
  case "overwriteProtection":
    writeBlock(
      "WHILE [[" + inspectionVariables.liveConnectionStatus, "== -1" +
      "] AND [" + inspectionVariables.liveConnectionReadPointer, "==", inspectionVariables.liveConnectionWritePointer + "]]"
    );
    writeComment("WAITING FOR FUSION CONNECTION");
    writeBlock(gFormat.format(53));
    writeBlock("ENDWHILE");
  ...
  case "WORKPLANE":
    ...
    writeBlock(
      "WHILE [[" + inspectionVariables.workplaneStartAddress, "<> 0] AND [[" +
      inspectionVariables.liveConnectionWPA, "<>", abcFormat.format(abc.x) + "] OR [" +
      inspectionVariables.liveConnectionWPB, "<>", abcFormat.format(abc.y) + "] OR [" +
      inspectionVariables.liveConnectionWPC, "<>", abcFormat.format(abc.z) + "]]]"
    );
    ...
    writeBlock(inspectionVariables.workplaneStartAddress, "= -1");
    writeBlock("ENDIF");
    break;
  }
}
...
function onLiveAlignment() {
  if (!getProperty("useLiveConnection")) {
    writeBlock("MSG(" + "\"" + "Import the result file for part alignment first, and then regenerate the NC program of the following milling operations." + "\")");
    onCommand(COMMAND_STOP);
    writeBlock("MSG()");
    return;
  }

  function formatLiveAlignmentOffset(axis) {
    return "$P_UIFR[" + currentSection.workOffset + ", " + axis + ", TR]";
  }

  var liveAlignmentOffset = {
    x: formatLiveAlignmentOffset("X"),
    y: formatLiveAlignmentOffset("Y"),
    z: formatLiveAlignmentOffset("Z"),
    a: formatLiveAlignmentOffset("A"),
    b: formatLiveAlignmentOffset("B"),
    c: formatLiveAlignmentOffset("C")
  };
  ...
  writeBlock("WHILE [" + inspectionVariables.commandID, "<> 3]");
  ...
  writeBlock("ENDIF");
  writeBlock("IF [[" + inspectionVariables.commandID, "== -2] AND [" + inspectionVariables.commandArg1, "== 3]]");
  writeBlock("MSG(" + "\"" + "PART_ALIGNMENT_ALARM" + "\")");
  onCommand(COMMAND_STOP);
  writeBlock("MSG()");
  writeBlock("ENDIF");
  writeBlock("IF [[" + inspectionVariables.commandID, "== -2] AND [" + inspectionVariables.commandArg1, "== 3]]");
  ...
  writeBlock("ENDIF");
  writeBlock("IF [" + inspectionVariables.commandID, "== -2]");
  writeBlock(inspectionVariables.commandID, "= 0"); // Clear Alarm
  writeBlock("ENDIF");
  writeBlock("IF [[" + inspectionVariables.liveConnectionStatus, "== 2] AND [" + inspectionVariables.commandArg1, "== 3]]");
  writeBlock("M30");
  writeBlock("ENDIF");
  writeBlock("ENDWHILE");
  ...
  if (machineConfiguration.isMultiAxisConfiguration() && inspectionVariables.liveAlignmentCapability != 0) {
  ...
  writeBlock("IF [" + inspectionVariables.commandID, "== 3]");
  writeBlock(inspectionVariables.commandID, "= 0");
  writeBlock("ENDIF");
  writeBlock("STOPRE");
}
