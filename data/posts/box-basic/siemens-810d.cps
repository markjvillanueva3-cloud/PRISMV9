...
var settings = {
  ...
  parametricFeeds: {
    ...
    feedAssignmentVariable: "R", // specifies the syntax to define a parameter
    feedOutputVariable    : "F=R" // specifies the syntax to output the feedrate as parameter
  },
...
function onOpen() {
  ...
  writeBlock(gUnitModal.format(unit == IN ? 70 : 71)); // lengths
...
function writeDrillCycle(cycle, x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    case "tapping-with-chip-breaking":
      ...
      writeBlock(
        "MCALL CYCLE84(" + [RTP, RFP, SDIS, DP, DPR, DTB, SDAC, MPIT, PIT, POSS, SST, SST1, _AXN, _PITA, _TECHNO, _VARI, _DAM, _VRT].join(cycleSeparator) + ")"
      );
      break;
    case "reaming":
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    ...
    dOutput.reset();
    writeToolBlock("T" + (getProperty("toolAsName") ? "=" + "\"" + (tool.description.toUpperCase()) + "\"" : toolFormat.format(tool.number)), dOutput.format(toolLengthOffset));
