...
var gRotationModal = createOutputVariable({current:69}, gFormat); // modal group 16 // G68-G69
...
function onOpen() {
  ...
  settings.outputToolLengthOffset = getProperty("useSetting15"); // specifies if tool length offset code should be output (Hxx)
...
function onSection() {
  ...
  if (newWorkOffset || insertToolCall || newWorkPlane || state.tcpIsActive) {
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    if (getProperty("useSetting15")) {
      writeToolBlock("T" + toolFormat.format(tool.number), mFormat.format(getProperty("useM06") ? 6 : 0));
      if (!getProperty("useM06") && !tool.manualToolChange) {
