...
properties = {
  ...
  useG61: {
    ...
    description: "Output G61.1 Geometry compensation for milling.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
var geometryCompensationOutput = createOutputVariable({current:64}, gFormat);
...
var settings = {
  ...
  probing: {
    ...
    probeAngleVariables    : {x:"#135", y:"#136", z:0, i:0, j:0, k:1, r:"#144", baseParamG54x4:5801, baseParamAxisRot:5200, method:0}, // specifies variables for the angle compensation macros, method 0 = Fanuc, 1 = Haas
...
function onOpen() {
  ...
  // initialize formats
  fourthAxisClamp.format(44); // Default 4th axis modal code to be clamped
  fifthAxisClamp.format(47); // Default 5th axis modal code to be clamped
  ...
  writeComment("For best machining results the following");
  writeComment("parameter settings are recommended by Mazak");
  writeComment("F85 bit 2 is 0 Type of coordinate system for controlling tool tip point");
  writeComment("F86 bit 6 is 1 Table Rotary Axis Reference Position for Tool Tip Point Control");
  writeComment("F96 bit 1 is 1 automatic correction");
  writeComment("F114 bit 1 is 1 G49 allowed movement");
  writeComment("F144 bit 1 is 1 Table Rotary Axis Reference for Inclined Plane Machining");
  writeComment("F162 bit 0 is 1 Tool Tip Point Control movement");
  writeln("");
  writeProgramHeader();
  ...
  if (!machineConfiguration.isMultiAxisConfiguration() || tcp.isSupportedByMachine) {
    var isCancelTCPRequiredForPositioning = getSetting("headPositioningMethod", 0) == 1 && machineConfiguration.isHeadConfiguration() && !is3D() && tcp.isSupportedByMachine && getSetting("workPlaneMethod.useTiltedWorkplane", false);
    for (var i = 0; i < getNumberOfSections(); ++i) {
      var section = getSection(i);
      if (isTCPSupportedByOperation(section) || (isCancelTCPRequiredForPositioning && defineWorkPlane(section, false).isNonZero())) {
        writeComment("Warning, NC program requires parameter F114 Bit1 to be set to 1");
        writeComment("Ensure that the G49 command does not cause axis movement on your machine.");
        writeBlock("IF[[#806114AND2]NE2]THEN#3000=91", formatComment("Parameter_F114_Bit1_must_be_1"));
  ...
  writeBlock(gAbsIncModal.format(90), gFeedModeModal.format(94), gPlaneModal.format(17), toolLengthCompOutput.format(49));
...
function setGeometryCompensation(mode) {
  writeBlock(geometryCompensationOutput.format(mode ? 61.1 : 64));
}
...
function onSection() {
  ...
  if (getProperty("useG61")) {
    setGeometryCompensation(smoothing.isAllowed);
  }
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    if (machineConfiguration.isMultiAxisConfiguration()) {
      writeBlock(fourthAxisClamp.format(44)); // lock 4th axis
      if (machineConfiguration.getNumberOfAxes() > 4) {
        writeBlock(fifthAxisClamp.format(47)); // lock 5th axis
      }
    }
    return;
  case COMMAND_UNLOCK_MULTI_AXIS:
    ...
    if (outputClampCodes && machineConfiguration.isMultiAxisConfiguration()) {
      writeBlock(fourthAxisClamp.format(43)); // unlock 4th axis
      if (machineConfiguration.getNumberOfAxes() > 4) {
        writeBlock(fifthAxisClamp.format(46)); // unlock 5th axis
      }
    }
    return;
