/**
  ...
  Hermle post processor configuration.

  $Revision: 44207 38fb6f0f33c97b71d59abed37b94c48ffefee497 $
  $Date: 2025-12-17 14:33:04 $

  FORKID {909584E4-CF6-482A-8A57-F62BC099DBE1}
*/

description = "Hermle Heidenhain";
vendor = "Hermle";
vendorUrl = "https://www.hermle.de/en/home_en";
legal = "Copyright (C) 2012-2025 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic post for Hermle machines with Heidenhain controls like TNC 620, TNC 640 and TNC 7.";
...
properties = {
  ...
  safePositionMethod: {
    title      : "Safe Retracts",
    description: "Select your desired retract option. 'Clearance Height' retracts to the operation clearance height." + EOL +
      "The 'Q Parameter' option will read the machine's actual XY axis ranges from the NC program to define the home XY positions" + EOL +
      "and retract to those values (if Home XY is enabled) instead of using the XY home positions defined in the machine configuration.",
    group : "homePositions",
    type  : "enum",
    values: [
      ...
      {title:"M91", id:"M91"},
      {title:"M92", id:"M92"},
      {title:"Q Parameter", id:"qParameter"}
    ],
    value: "qParameter",
    scope: "post"
  },
  ...
  useSmoothing: {
    title      : "Use smoothing",
    description: "Specifies whether CYCL DEF 326 should be used.",
    group      : "preferences",
    type       : "enum",
    values     : [
      {title:"Off", id:"-1"},
  ...
  homeXY: {
    title      : "Home XY",
    description: "Specifies when the machine should move to its XY home positions.",
    group      : "homePositions",
    type       : "enum",
    values     : [
      {title:"Disabled", id:"disabled"},
      {title:"On tool change", id:"toolChange"},
      {title:"On indexing", id:"indexing"},
      {title:"On tool change + indexing", id:"toolIndexing"},
      {title:"Use postprocessor default", id:"default"},
    ],
    value: "default",
    scope: "post"
  },
  ...
  outputWCS: {
    title      : "Output WCS",
    description: "Specifies if CYCL DEF 247, CYCL DEF 7 or none WCS should be output. If 'Disabled' is selected, the machine will use the datum that is currently active.",
    group      : "preferences",
    type       : "enum",
    values     : [
      {title:"CYCL DEF 247", id:"247"},
      {title:"CYCL DEF 7", id:"7"},
      {title:"Disabled", id:"-1"}
    ],
    value: "247",
    scope: "post"
  ...
  useMistCollector: {
    title      : "Use mist collector",
    description: "Specifies whether to use the mist collector.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  useSkipLabels: {
    title      : "Use skip Labels",
    description: "If enabled, the program will have additional jump marks which allows to call the output program by another main program.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
var settings = {
  coolant: {
    ...
    coolants: [
      ...
      {id:COOLANT_AIR, on:7},
      {id:COOLANT_AIR_THROUGH_TOOL, on:12},
      {id:COOLANT_SUCTION},
      {id:COOLANT_FLOOD_MIST},
      {id:COOLANT_FLOOD_THROUGH_TOOL, on:[8, 51], off:[9, 52]},
      {id:COOLANT_OFF, off:9}
    ],
    singleLineCoolant: true, // specifies to output multiple coolant codes in one line rather than in separate lines
  },
  retract: {
    ...
    homeXY                    : {onIndexing:{axes:[X, Y], singleLine:false}, onToolChange:{axes:[X, Y], singleLine:false}, onProgramEnd:{axes:[X, Y], singleLine:false}} // Specifies when the machine should be homed in X/Y. Sample: onIndexing:{axes:[X, Y], singleLine:false}
  },
  parametricFeeds: {
...
function onOpen() {
  ...
  var homeXY = getProperty("homeXY");
  if (homeXY != "default") {
    var toolChangeSettings = {
      axes      : settings.retract.homeXY.onToolChange.axes != undefined ? settings.retract.homeXY.onToolChange.axes : [X, Y],
      singleLine: settings.retract.homeXY.onToolChange.singleLine != undefined ? settings.retract.homeXY.onToolChange.singleLine : false
    };
    var indexingSettings = {
      axes      : settings.retract.homeXY.onIndexing.axes != undefined ? settings.retract.homeXY.onIndexing.axes : [X, Y],
      singleLine: settings.retract.homeXY.onIndexing.singleLine != undefined ? settings.retract.homeXY.onIndexing.singleLine : false
    };
    switch (homeXY) {
    case "disabled":
      settings.retract.homeXY.onToolChange = false;
      settings.retract.homeXY.onIndexing = false;
      break;
    case "toolChange":
      settings.retract.homeXY.onToolChange = toolChangeSettings;
      settings.retract.homeXY.onIndexing = false;
      break;
    case "indexing":
      settings.retract.homeXY.onToolChange = false;
      settings.retract.homeXY.onIndexing = indexingSettings;
      break;
    case "toolIndexing":
      settings.retract.homeXY.onToolChange = toolChangeSettings;
      settings.retract.homeXY.onIndexing = indexingSettings;
  ...
  useCycl247 = getProperty("outputWCS") == "247";
  useCycl7 = getProperty("outputWCS") == "7";

  writeBlock("BEGIN PGM" + (programName ? (SP + programName) : "") + ((unit == MM) ? " MM" : " INCH"));
  writeComment(programComment);
  if (getProperty("useSkipLabels")) {
    writeBlock("FN 9: IF +Q1999 EQU +987654321 GOTO LBL \"START\"");
  }
  writeWorkpiece();
  if (getProperty("useSkipLabels")) {
    writeBlock("LBL \"START\"");
  }
  if (getProperty("safePositionMethod") == "qParameter") {
    writeBlock("CALL LBL \"GET_SAFE_POSITIONS\"; GET SAFE POSITIONS FOR TOOLCHANGE AND INDEXING");
  }
  ...
  if (getProperty("useMistCollector")) {
    writeBlock(mFormat.format(10)); // mist collector on
  }
...
function setSmoothing(mode) {
  ...
  if (mode) { // enable smoothing
    var Q1512 = formatQword("Q1512", CYCL326.Q1512, "SELECTION OF MACHINING SETUP");
    var Q1519 = formatQword("Q1519", CYCL326.Q1519, "TOLERANCE PERCENTAGE");
    var Q1520 = formatQword("Q1520", CYCL326.Q1520, "ANGLE TOLERANCE PERCENTAGE");
    writeBlock(["CYCL DEF 326 " + localize("DYN. MACHINING SETUP"), Q1512, Q1519, Q1520].join(EOL_));
  } else { // disable smoothing
    var Q1512 = formatQword("Q1512", 0, "SELECTION OF MACHINING SETUP");
    var Q1519 = formatQword("Q1519", 100, "TOLERANCE PERCENTAGE");
    var Q1520 = formatQword("Q1520", 100, "ANGLE TOLERANCE PERCENTAGE");
    writeBlock(["CYCL DEF 326 " + localize("DYN. MACHINING SETUP"), Q1512, Q1519, Q1520].join(EOL_));
  }
}

var CYCL326 = {};
function initializeCYCL326Parameters() {
  var result;
  switch (currentSection.strategy) {
  case "drill":
    result = {Q1512:2, Q1519:100, Q1520:100};
    break;
  case "adaptive2d":
  case "adaptive": // 3D Adaptive
  case "pocket_new": // 3D Pocket Clearing
  case "moduleworks_4axis_roughing": // Rotary pocket
  case "moduleworks_automatic_deburring": // Deburr
  case "pocket2d":
    result = {Q1512:6, Q1519:100, Q1520:100};
    break;
  case "steep_and_shallow":
  case "parallel":
  case "parallel_new":
  case "scallop":
  case "scallop_new":
  ...
  case "blend":
  case "morph":
  case "rest_finishing": // Corner
  case "flow2":
  case "swarf5d":
  case "multiAxisContour":
  case "rotary_finishing": // Rotary Parallel
  case "moduleworks_geodesic": // Geodesic
  case "moduleworks_swarf": // Advanced Swarf
  case "moduleworks_4axis_finishing": // Rotary Contour
    result = {Q1512:5, Q1519:100, Q1520:100};
    break;
  case "face":
  case "contour2d":
  case "slot":
  ...
  case "horizontal":
  case "horizontal_new":
    result = {Q1512:0, Q1519:100, Q1520:100};
    break;
  default:
    result = {Q1512:0, Q1519:100, Q1520:100}; // default case if the toolpath strategy does not match any case
  }
  result.isDifferent = false;
  if (result.Q1512 != CYCL326.Q1512 || result.Q1519 != CYCL326.Q1519 || result.Q1520 != CYCL326.Q1520) {
    result.isDifferent = true;
  }
  CYCL326 = Object.create(result);
  return result;
}
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane || state.tcpIsActive) {
    ...
    if (isFirstSection() || insertToolCall) {
      setWorkPlane(new Vector(0, 0, 0)); // for safety reasons we must set the rotary axes to 0 for a tool change
      forceABC();
    } else if (newWorkPlane) {
      cancelWorkPlane();
    }
  }
  ...
  if (insertToolCall) {
    ...
    // force retract to home XY for tool changes
    if (getSetting("retract.homeXY.onToolChange", false) || getSetting("retract.homeXY.onIndexing", false)) {
  ...
  var newDeltaRadius = getWearCompensationDeltaRadius();
  if (currentDeltaRadius != newDeltaRadius) {
    currentDeltaRadius = newDeltaRadius;
    writeBlock("TOOL CALL DR" + xyzFormat.format(currentDeltaRadius));
  }
  ...
  if (useCycl247 || useCycl7) {
    if (currentSection.workOffset != 0) {
      currentWorkOffset = undefined;
    }
    writeWCS();
  } else {
    if (!isFirstSection() && isNewWorkOffset()) {
      error(subst(localize("Using multiple WCS is not supported when property '%1' is set to 'Disabled'."), properties.outputWCS.title));
    }
  }
  forceXYZ();
  ...
  if (CYCL326.isDifferent) {
    setSmoothing(true); // writes the required smoothing codes
  }
...
var currentDeltaRadius = undefined;
function getWearCompensationDeltaRadius() {
  // returns the delta radius for wear compensation, "DR" value
  return currentSection.getParameter("operation:compensationType", undefined) == "wear" ? -1 * (tool.diameter / 2) : 0;
}
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    ...
    currentDeltaRadius = getWearCompensationDeltaRadius();
    writeToolBlock(
      ...
      tool.type == TOOL_PROBE ? getProperty("outputSpindleSpeedForProbing") ? sOutput.format(50) : "" : sOutput.format(spindleSpeed),
      currentDeltaRadius != 0 ? "DR" + xyzFormat.format(currentDeltaRadius) : ""
    );
    ...
    if (getSetting("retract.homeXY.onIndexing", false)) {
      if (currentSection.isMultiAxis() || !isSameDirection(currentSection.workPlane.forward, new Vector(0, 0, 1))) {
        state.retractedX = false; // force retract in X after tool change
        state.retractedY = false; // force retract in Y after tool change
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    if (machineConfiguration.isMultiAxisConfiguration()) {
      // writeBlock(fourthAxisClamp.format(28)); // lock 4th axis
      if (machineConfiguration.getNumberOfAxes() > 4) {
        writeBlock(fifthAxisClamp.format(26)); // lock 5th axis
      }
    }
    return;
  case COMMAND_UNLOCK_MULTI_AXIS:
    if (machineConfiguration.isMultiAxisConfiguration()) {
      // writeBlock(fourthAxisClamp.format(27)); // unlock 4th axis
      if (machineConfiguration.getNumberOfAxes() > 4) {
        writeBlock(fifthAxisClamp.format(25)); // unlock 5th axis
      }
    }
    return;
...
*/
  ...
  if (getProperty("useParkPosition") && getSetting("retract.homeXY.onProgramEnd", false)) {
    state.retractedX = false; // force retract in X on program end
    state.retractedY = false; // force retract in Y on program end
  ...
  if (getProperty("useMistCollector")) {
    writeBlock(mFormat.format(11)); // mist collector off
  }
  if (getProperty("useSkipLabels")) {
    writeBlock("FN 9: IF +Q1999 EQU +987654321 GOTO LBL \"END\"");
  }
  ...
  if (getProperty("safePositionMethod") == "qParameter") {
    writeln("");
    writeSeparator();
    writeStructureComment("GET SAFE POSITIONS");
    writeBlock("LBL \"GET_SAFE_POSITIONS\"");
    writeBlock("FN 20: WAIT FOR SYNC");
    writeBlock("FN 18: SYSREAD Q1703 = ID230 NR3 IDX1 ; READ MAX X TRAVEL");
    writeBlock("FN 18: SYSREAD Q1706 = ID230 NR3 IDX2 ; READ MAX Y TRAVEL");
    writeBlock("FN 20: WAIT FOR SYNC");
    if (unit == MM) {
      writeBlock("Q1703 = Q1703 - 0.1 ; SUBTRACT 0.1MM FROM X LIMIT");
      writeBlock("Q1706 = Q1706 - 0.1 ; SUBTRACT 0.1MM FROM Y LIMIT");
    } else {
      writeBlock("Q1703 = (Q1703 - 0.1) / 25.4 ; SUBTRACT 0.1MM FROM X LIMIT AND CONVERT TO INCHES");
      writeBlock("Q1706 = (Q1706 - 0.1) / 25.4; SUBTRACT 0.1MM FROM Y LIMIT AND CONVERT TO INCHES");
    }
    writeBlock("Q1705 = Q1706 / 2 ; DIVIDE Y LIMIT BY 2");
    writeBlock("LBL 0");
    writeSeparator();
  }
  if (getProperty("useSkipLabels")) {
      ...
      case "qParameter":
        ...
        for (var i in words) {
          if (words[i].indexOf("X") != -1) {
            wordsXY.push("X+Q1703");
          }
          if (words[i].indexOf("Y") != -1) {
            wordsXY.push(arguments[0] === settings.retract.homeXY.onIndexing ? "Y+Q1705" : "Y+Q1706"); // Q1705 is half the Y-axis range and used as safe position for indexing
          }
        }
        if (wordsXY.length > 1 && getSimulationStreamPath() != "") {
          warningOnce(
            "The 'Safe Retracts' property is set to 'Q Parameter', which reads the machine's XY travel ranges from the NC program to define the home XY positions." + EOL +
            "Ensure that the XY axis ranges specified in your machine definition match the actual machine's ranges and positions to obtain accurate simulation results."
            , WARNING_QRETRACT);
        }
        wordsXY.push("R0 FMAX " + mFormat.format(91));
        if (arguments[0] === settings.retract.homeXY.onToolChange) {
          writeBlock(wordsXY, formatComment("TOOL CHANGE POSITION"));
        } else if (arguments[0] === settings.retract.homeXY.onIndexing) {
          writeBlock(wordsXY, formatComment("INDEXING POSITION"));
        } else if (arguments[0] === settings.retract.homeXY.onProgramEnd) {
          writeBlock(wordsXY, formatComment("PROGRAM END POSITION"));
        } else { // Z retract
          writeBlock("L", words, "R0 FMAX", mFormat.format(91));
        }
        break;
      default:
        error(subst(localize("Unsupported safe position method '%1'"), retract.method));
      }
      var posX = getProperty("safePositionMethod") == "qParameter" ? getAxisRange(machineConfiguration.getAxisX(), "MAX") : retract.positions.x;
      var posY = getProperty("safePositionMethod") == "qParameter" ? getAxisRange(machineConfiguration.getAxisY(), "MAX") : retract.positions.y;
      if (arguments[0] === settings.retract.homeXY.onIndexing && getProperty("safePositionMethod") == "qParameter") {
        posY /= 2; // half the Y-axis range to match Q1705
      }
      var posZ = retract.positions.z;
      machineSimulation({
...
function getAxisRange(axis, limit) {
  var range = axis.getRange();
  if (range.isNonRange()) {
    return 0;
  }
...
// <<<<< INCLUDED FROM include_files/rewind.cpi
properties.useParametricFeed.value = true;
