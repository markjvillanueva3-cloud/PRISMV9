...
function setAxisMode(_format, _value, _isInc) {
  _format.setType(_isInc ? TYPE_INCREMENTAL : TYPE_ABSOLUTE);
  _format.setCurrent(_value);
}

function setAbsIncMode(isIncremental, xyz, abc) {
  if (isIncremental) {
...
function onSection() {
  ...
  if (getProperty("useIncremental") && tool.type != TOOL_PROBE) {
    if (xyzFormat.areDifferent(xOutput.getCurrent(), initialPosition.x) ||
    xyzFormat.areDifferent(yOutput.getCurrent(), initialPosition.y) ||
    xyzFormat.areDifferent(zOutput.getCurrent(), initialPosition.z)) {
      writeBlock(gAbsIncModal.format(90), xOutput.format(initialPosition.x), yOutput.format(initialPosition.y), zOutput.format(initialPosition.z));
    }
    setAbsIncMode(true, initialPosition, abc);
  } else {
    setAbsIncMode(false, initialPosition, abc);
  }
}
    ...
    if (isFirstCyclePoint()) {
      ...
      case "tapping":
        ...
        if (getProperty("usePitchForTapping")) {
          writeBlock(
            gRetractModal.format(98), gAbsIncModal.format(useIncremental ? 91 : 90), gFeedModeModal.format(95), gCycleModal.format((tool.type == TOOL_TAP_LEFT_HAND) ? 74 : 84),
            getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
            "P" + milliFormat.format(P),
      ...
      case "left-tapping":
        ...
        if (getProperty("usePitchForTapping")) {
          writeBlock(
            gRetractModal.format(98), gAbsIncModal.format(useIncremental ? 91 : 90), gFeedModeModal.format(95), gCycleModal.format(74),
            getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
            "P" + milliFormat.format(P),
            pitchOutput.format(tool.threadPitch)
          );
      ...
      case "right-tapping":
        ...
        if (getProperty("usePitchForTapping")) {
          writeBlock(
            gRetractModal.format(98), gAbsIncModal.format(useIncremental ? 91 : 90), gFeedModeModal.format(95), gCycleModal.format(84),
            getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
            "P" + milliFormat.format(P),
            pitchOutput.format(tool.threadPitch)
          );
...
function onSectionEnd() {
  ...
  setAbsIncMode(false, getCurrentPosition(), getCurrentDirection()); //set to absolute mode
...
function writeRetract() {
  settings.retract.methodXY = (machineConfiguration.hasHomePositionX() && machineConfiguration.hasHomePositionY()) ? "G53" : "G28"; // force G53 for X&Y if it has Home position
  ...
  if (retract && retract.words.length > 0) {
    ...
    for (var i in retract.words) {
      ...
      case "G28":
        ...
        writeBlock(gAbsIncModal.format(useIncremental ? 91 : 90));
        break;
      case "G30":
        forceModals(gMotionModal, gAbsIncModal);
