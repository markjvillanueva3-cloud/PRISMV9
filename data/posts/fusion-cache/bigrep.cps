/**
  ...
  bigrep post processor configuration.
...
highFeedrate = toPreciseUnit(15000, MM);
...
var settings = {
  useG0              : false, // specifies to either use G0 or G1 commands for rapid movements
...
function onClose() {
  if (getProperty("printerModel") == "studio") {
    writeBlock(getCode(commands.setExtruderTemperature), sOutput.format(0), "H0");
    writeBlock(getCode(commands.setExtruderTemperature), sOutput.format(0), "H1");
...
function onExtruderTemp(temp, wait, id) {
  ...
  if (wait) {
    ...
    writeBlock(getCode(commands.waitExtruder), sOutput.format(temp), extruderString);
  } else {
    writeBlock(getCode(commands.setExtruderTemperature), sOutput.format(temp), extruderString);
  }
}
