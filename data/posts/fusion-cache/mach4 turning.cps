/**
  ...
  Mach4 Lathe post processor configuration.
...
longDescription = "Generic turning post for Mach4. Use Turret 0 for Positional Turret, Turret 101 for QCTP on X- Post, Turret 102 for QCTP on X+ Post, Turret 103 for Gang Tooling on X- Post, Turret 104 for Gang Tooling on X+ Tool Post.";
...
var jOutput = createReferenceVariable({prefix:"J"}, yFormat);
...
function onOpen() {
  ...
  if (getProperty("writeTools")) {
    ...
    if (tools.getNumberOfTools() > 0) {
      for (var i = 0; i < tools.getNumberOfTools(); ++i) {
        ...
        var comment = "T" + toolFormat.format(tool.number * 100 + compensationOffset) + " " +
  ...
  // incremental IJK's and feed per min
  writeBlock(getCode("FEED_MODE_UNIT_MIN"), gPlaneModal.format(18));
  writeBlock(gFormat.format(91.1));
...
*/
    ...
    case "drilling":
      writeBlock(
        gRetractModal.format(98.1), gCycleModal.format(81.1),
    ...
    case "counter-boring":
      if (P > 0) {
        writeBlock(
          gRetractModal.format(98.1), gCycleModal.format(82.1),
          getCommonCycle(x, y, z, cycle.retract),
    ...
    case "chip-breaking":
      if ((cycle.accumulatedDepth < cycle.depth) || (P > 0)) {
        ...
        writeBlock(
          gRetractModal.format(98.1), gCycleModal.format(73.1),
          getCommonCycle(x, y, z, cycle.retract),
    ...
    case "deep-drilling":
      if (P > 0) {
        ...
        writeBlock(
          gRetractModal.format(98.1), gCycleModal.format(83.1),
          getCommonCycle(x, y, z, cycle.retract),
          "Q" + spatialFormat.format(cycle.incrementalDepth),
          feedOutput.format(F)
        );
    ...
    case "tapping":
      ...
      writeBlock(
        gRetractModal.format(98.1), gCycleModal.format((tool.type == TOOL_TAP_LEFT_HAND) ? 84.3 : 84.2),
        getCommonCycle(x, y, z, cycle.retract),
        "P" + secFormat.format(P),
        feedOutput.format(F)
      );
    ...
    case "left-tapping":
      ...
      // writeBlock(mFormat.format(29), sOutput.format(spindleSpeed));
      writeBlock(
        gRetractModal.format(98.1), gCycleModal.format(84.3),
        getCommonCycle(x, y, z, cycle.retract),
        "P" + secFormat.format(P),
        feedOutput.format(F)
      );
    ...
    case "right-tapping":
      ...
      // writeBlock(mFormat.format(29), sOutput.format(spindleSpeed));
      writeBlock(
        gRetractModal.format(98.1), gCycleModal.format(84.2),
        getCommonCycle(x, y, z, cycle.retract),
        "P" + secFormat.format(P),
        feedOutput.format(F)
      );
    ...
    case "right-tapping-with-chip-breaking":
      error(localize("tapping-with-chip-breaking cycle is not supported."));
      return;
    case "fine-boring":
      writeBlock(
        gRetractModal.format(98.1), gCycleModal.format(76.1),
        getCommonCycle(x, y, z, cycle.retract),
        "P" + secFormat.format(P), // not optional
        "I" + xFormat.format(-cycle.shift / 2),
        feedOutput.format(F)
      );
      break;
    case "reaming":
      ...
      writeBlock(
        gRetractModal.format(98.1), gCycleModal.format(85.1),
        getCommonCycle(x, y, z, cycle.retract),
        feedOutput.format(F)
      );
      break;
    case "stop-boring":
      if (P > 0) {
        ...
        writeBlock(
          gRetractModal.format(98.1), gCycleModal.format(86.1),
          getCommonCycle(x, y, z, cycle.retract),
          feedOutput.format(F)
        );
      }
    ...
    case "boring":
      ...
      if (P > 0) {
        writeBlock(
          gRetractModal.format(98.1), gCycleModal.format(89.1),
          getCommonCycle(x, y, z, cycle.retract),
          "P" + secFormat.format(P), // not optional
          feedOutput.format(F)
        );
