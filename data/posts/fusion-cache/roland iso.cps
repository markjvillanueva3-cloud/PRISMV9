...
longDescription = "Generic milling post for Roland ISO/NC-Code mode.";
...
function onSection() {
  ...
  if (tool.coolant == COOLANT_AIR) { // only air coolant is supported
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    if (getProperty("useToolCall")) {
      ...
      writeToolBlock(formatComment(comment));
    }
