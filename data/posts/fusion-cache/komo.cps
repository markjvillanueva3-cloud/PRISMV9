/**
  ...
  KOMO post processor configuration.

  $Revision: 44107 fdbddcd9a1d3cf71c49c0d31d4963c1877e5a756 $
  $Date: 2024-01-11 15:12:03 $

  FORKID {B35D4152-74B3-4B46-9CB9-9D6DE2ABBD69}
*/

description = "KOMO CNC Machining Center";
vendor = "KOMO";
vendorUrl = "http://www.komo.com";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic post for KOMO CNC Machining Centers.";
...
properties = {
  ...
  useSmoothing: {
    title      : "Use smoothing",
    description: "Defines the smoothing control mode. 'On' outputs G08 P0/P1 only",
    group      : "preferences",
    type       : "enum",
    values     : [
      {title:"Off", id:"-1"},
...
wcsDefinitions = {
  useZeroOffset: true,
  wcs          : [
    {name:"Default", format:"G", range:[55, 55]},
...
var gTappingModal = createOutputVariable({}, gFormat); // G63-G64
...
var settings = {
  ...
  workPlaneMethod: {
    ...
    eulerConvention       : EULER_ZYX_R, // specifies the euler convention (ie EULER_XYZ_R), set to undefined to use machine angles for TWP commands ('undefined' requires machine configuration)
  ...
  subprograms: {
    ...
    callBlock              : {files:[gFormat.format(65) + " P"], embedded:[gFormat.format(65) + " P"]} // specifies the command for calling a subprogram followed by the subprogram number
  },
  comments: {
...
function onOpen() {
  ...
  // absolute coordinates and feed per min or feed per revolution
  writeBlock(
    ...
    gFormat.format(40),
    gFormat.format(80),
    gFeedModeModal.format(getProperty("useG95") ? 95 : 94),
    gAbsIncModal.format(91),
    "Z" + xyzFormat.format(0),
    mFormat.format(5)
  );
  writeBlock(gAbsIncModal.format(90));
  writeBlock(gFormat.format(52), "X" + xyzFormat.format(0), "Y" + xyzFormat.format(0));
...
function setSmoothing(mode) {
  ...
  if (mode && getProperty("useSmoothing") == "0") { // enable smoothing
...
function writeDrillCycle(cycle, x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    case "deep-drilling":
      ...
      break;
    case "tapping":
      writeBlock(gTappingModal.format(63));
...
function onCycleEnd() {
  if (isProbeOperation()) {
    ...
    if (!cycleExpanded) {
      writeBlock(
        !getProperty("useG95") ? gFeedModeModal.format(94) : "",
        isTappingCycle() ? gTappingModal.format(64) : "",
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    ...
    writeBlock("T" + toolFormat.format(102), formatComment(tool.comment));
...
function onClose() {
  ...
  forceModals(gTappingModal);
  writeBlock(
    gMotionModal.format(0),
    gFormat.format(80),
    gFormat.format(28),
    gAbsIncModal.format(91),
    gTappingModal.format(64),
    "Z" + xyzFormat.format(0)
  );
  writeBlock(gAbsIncModal.format(90));
  ...
  // }
  writeBlock(gFormat.format(53), "Y" + xyzFormat.format(0));
