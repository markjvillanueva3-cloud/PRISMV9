...
var gPlaneModal = createOutputVariable({onchange:function () {gMotionModal.reset();}}, gFormat); // modal group 12 // G17-19
var gAbsIncModal = createOutputVariable({}, gFormat); // modal group 3 // G90-91
var gFeedModeModal = createOutputVariable({}, gFormat); // modal group 5 // G93-94
var gUnitModal = createOutputVariable({}, gFormat); // modal group 2 // G70-71
var gCycleModal = createOutputVariable({}, gFormat); // G81, ...
...
var settings = {
  ...
  maximumSequenceNumber       : 1000000, // the maximum sequence number (Nxxx), use 'undefined' for unlimited
...
function onDwell(seconds) {
  ...
  writeBlock(gFormat.format(4), "F" + milliFormat.format(milliseconds));
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    writeToolBlock(mFormat.format(16), "T" + toolFormat.format(tool.number));
    writeBlock("Q6");
    writeComment(tool.comment);

    var preloadTool = getNextTool(tool.number != getFirstTool().number);
    if (getProperty("preloadTool") && preloadTool) {
      writeBlock(mFormat.format(16), "T" + toolFormat.format(preloadTool.number)); // preload next/first tool
    }
    return;
