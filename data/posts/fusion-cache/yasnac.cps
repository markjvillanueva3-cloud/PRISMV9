...
properties = {
  ...
  o5: {
    title      : "5 Digit program number",
    description: "Specifies that a 5 digit program number is needed.",
    group      : "formats",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  dOffset: {
    title      : "Radius compensation register base",
    description: "The amount to add to the radius compensation register (D).",
    group      : "preferences",
    type       : "integer",
    value      : 0,
    scope      : "post"
  ...
  usePitchForTapping: {
    ...
    description: "Enables the use of pitch instead of feed for tapping cycles.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  useSolidTapping: {
    title      : "Use solid tapping",
    description: "Enables solid tapping mode (G93).",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  useExtendedWCS: {
    title      : "Use G54 Pxx Extended WCS",
    description: "Specifies that the address is \"P\" instead of \"J\" for G54->G59.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
function writeProgramNumber() {
  if (programName) {
    ...
    if (programId > settings.maximumSequenceNumber) {
      if (!((programId >= 1) && (programId <= settings.maximumSequenceNumber))) {
        error(localize("Program number is out of range."));
        return;
      }
    }
    var oFormat = createFormat({width:(getProperty("o5") ? 5 : 4), zeropad:true, decimals:0});
...
function onOpen() {
  ...
  if (getProperty("o5")) {
    settings.maximumSequenceNumber = 99999;
  }
  ...
  writeBlock(gAbsIncModal.format(90), conditional(getProperty("useSolidTapping"), gFeedModeModal.format(94)), gPlaneModal.format(17));
...
function onSection() {
  ...
  writeBlock(gPlaneModal.format(17), gAbsIncModal.format(90), conditional(getProperty("useSolidTapping"), gFeedModeModal.format(94)));
  gFeedModeModal.format(94);
  ...
  if (workOffset > 0) {
    var address = getProperty("useExtendedWCS") ? "P" : "J";
...
function onDwell(seconds) {
  ...
  writeBlock(gFeedModeModal.format(94), gFormat.format(4), "P" + (seconds >= 1 ? secFormat.format(seconds) : milliFormat.format(seconds * 1000)));
}
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    case "tapping":
      ...
      if (getProperty("useSolidTapping")) {
      ...
      writeBlock(
        ...
        getCommonCycle(x, y, z, cycle.retract),
        getProperty("usePitchForTapping") ? pitchOutput.format(tool.threadPitch) : feedOutput.format(F),
        conditional(getProperty("useSolidTapping"), sOutput.format(spindleSpeed))
      );
      break;
    case "left-tapping":
      if (!F) {
...
function onLinear(_x, _y, _z, feed) {
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      ...
      var d = diameterOffsetFormat.format(tool.diameterOffset + getProperty("dOffset"));
      if (d > 1199) {
        warning(localize("The diameter offset exceeds the maximum value of 1199."));
      }
      writeBlock(gPlaneModal.format(17));
