/**
  ...
  DMG Mori CMX post processor configuration.

  $Revision: 44207 3c29ebc507e8e59a0898c47558016451f197da94 $
  $Date: 2025-12-17 08:29:21 $

  FORKID {78638A63-E6E0-4F95-B886-32D2EC38D9AF}
*/

description = "DMG Mori CMX with FANUC control";
vendor = "DMG MORI";
vendorUrl = "http://en.dmgmori.com";
legal = "Copyright (C) 2012-2025 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;
longDescription = "DMG Mori CMX series horizontal machining center with optional rotary table and a FANUC control.";
...
properties = {
  ...
  rotaryTable: {
    title      : "Rotary table",
    description: "Select rotary table setup. Select 'No rotary' 'A-axis Contouring', or '5-Axis Positioning.",
    group      : "configuration",
    type       : "enum",
    values     : [
      {title:"No rotary", id:"none"},
      {title:"A-axis Contouring", id:"4axis"},
      {title:"5-axis Positioning", id:"5axis"}
    ],
    value: "none",
    scope: "post"
  },
...
var inverseTimeFormat = createFormat({decimals:(unit == MM ? 0 : 1), type:FORMAT_REAL});
...
var settings = {
  ...
  retract: {
    ...
    homeXY                    : {onIndexing:false, onToolChange:false, onProgramEnd:[X, Y]} // Specifies when the machine should be homed in XY (sample: onIndexing:[X,Y]). Options can be combined
  },
  parametricFeeds: {
  ...
  comments: {
    ...
    outputFormat         : "upperCase", // can be set to "upperCase", "lowerCase" and "ignoreCase". Set to "ignoreCase" to write comments without additional formatting
...
function defineMachine() {
  if (getProperty("rotaryTable") != "none") { // note: setup your machine here
    var useTCP = false;
    if (getProperty("rotaryTable") == "4axis") {
      var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], cyclic:true, preference:0, tcp:useTCP});
      machineConfiguration = new MachineConfiguration(aAxis);
    } else {
      var aAxis = createAxis({coordinate:0, table:true, axis:[0, 0, 1], cyclic:true, preference:0, tcp:useTCP});
      var bAxis = createAxis({coordinate:1, table:true, axis:[0, 1, 0], cyclic:false, range:[-30, 110], preference:0, tcp:useTCP});
...
function onOpen() {
  ...
  fifthAxisClamp.format(68); // Default 5th axis modal code to be clamped
  ...
  writeBlock(gAbsIncModal.format(90), gFeedModeModal.format(94), gPlaneModal.format(17), toolLengthCompOutput.format(49), gFormat.format(40), gFormat.format(80));
...
function setSmoothing(mode) {
  ...
  writeBlock(gFormat.format(332), mode ? "R" + smoothing.level + "." : "R3.");
...
function onSection() {
  ...
  writeBlock(gAbsIncModal.format(90), gFeedModeModal.format(94), gPlaneModal.format(17), insertToolCall ? formatWords(toolLengthCompOutput.format(49), gFormat.format(40), gFormat.format(80)) : "");
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    if (machineConfiguration.isMultiAxisConfiguration()) {
      ...
      if (machineConfiguration.getNumberOfAxes() > 4) {
        writeBlock(fifthAxisClamp.format(68)); // lock 5th axis
      }
    }
    return;
  case COMMAND_UNLOCK_MULTI_AXIS:
    if (machineConfiguration.isMultiAxisConfiguration()) {
      ...
      if (machineConfiguration.getNumberOfAxes() > 4) {
        writeBlock(fifthAxisClamp.format(69)); // unlock 5th axis
      }
    }
    return;
  ...
  case COMMAND_BREAK_CONTROL:
    writeBlock(gFormat.format(325));
    return;
  case COMMAND_TOOL_MEASURE:
    writeBlock(gFormat.format(324));
    return;
  }
...
function onClose() {
  ...
  if (getProperty("safePositionMethod") == "G28") { // retract for Y needs to be G30
    settings.retract.methodXY = "G30";
    writeRetract(Y); // return to home
    settings.retract.methodXY = undefined; // resetting retract method to default
  } else {
    writeRetract(Y); // return to home
  }
