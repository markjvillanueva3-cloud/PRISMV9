var modelType = "TMX10MY";
description = "Hurco TMX10MY";
...
function activateMachine(section) {
  ...
  if (section.getSpindle() == SPINDLE_PRIMARY) {
    ...
    if (getProperty("controllerType") == "v9") {
      cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, -1], range:[0, 360], cyclic:true, preference:1});
    } else {
      cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], range:[-36000, 36000], cyclic:false, preference:0, reset:1});
    }
  } else {
  ...
  if (section.isMultiAxis()) {
    section.optimizeMachineAnglesByMachine(machineConfiguration, OPTIMIZE_BOTH);
  }
...
function getSpindle(whichSpindle) {
  ...
  if (getCurrentSectionId() < 0) {
    if (machineState.liveToolIsActive && !whichSpindle) {
      return SPINDLE_LIVE;
    } else {
  ...
  if (machineState.isTurningOperation || machineState.axialCenterDrilling || whichSpindle) {
...
// Only needed if C-axis is [0, 0, -1]
...
function onCycle() {
  if ((typeof isSubSpindleCycle == "function") && isSubSpindleCycle(cycleType)) {
    ...
    if (!machineState.stockTransferIsActive) {
      ...
      //writeBlock(gMotionModal.format(0), gFormat.format(53 + workOffset), gFeedModeModal.format(getCode("FEED_MODE_UNIT_MIN", getSpindle(TOOL))));
    }
    onCommand(COMMAND_OPTIONAL_STOP);
    writeln("");
    if (hasParameter("operation-comment")) {
