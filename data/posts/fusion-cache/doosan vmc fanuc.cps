/**
  ...
  FORKID {ECCF493A-8FAE-4803-A465-19049BA8E985}
*/
...
//
//     smooth:on,off,auto,nano,1-10 - Defines the smoothing mode for the next operation
//
///////////////////////////////////////////////////////////////////////////////

description = "DN Solutions (Doosan) 3/5-axis VMC (FANUC)";
...
longDescription = "Post for DN Solutions (Doosan) 3/5-axis VMC post with Fanuc control.";
...
properties = {
  ...
  useSmoothing: {
    title      : "Use smoothing",
    description: "Defines the smoothing control mode (AICC/AIAPC). 'On' outputs G05.1 Q0/Q1 only, 'Nano' enables Nano Smoothing, 'Automatic' or 'Level 1-10' outputs G05.1 Q1 with the R value for the desired level.",
    group      : "preferences",
    type       : "enum",
    values     : [
      {title:"Off", id:"-1"},
  ...
  useRotaryScale: {
    title      : "Table uses rotary scale",
    description: "Enable to output the rotary table on a rotary scale (0-360), or disable to output it on a linear scale (-99999, 99999)",
    group      : "configuration",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  safeRetractDistance: {
    ...
    description: "Specifices the distance to add to retract distance when rewinding rotary axes.",
    group      : "multiAxis",
    type       : "spatial",
    value      : 0,
    scope      : "post"
...
var settings = {
  ...
  smoothing: {
    ...
    semi                  : 6, // semi-roughing level for smoothing in automatic mode
  ...
  probing: {
    macroCall              : mFormat.format(165), // specifies the command to call a macro
...
function defineMachine() {
  ...
  if (getProperty("fifthAxis")) {
    ...
    var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-120, 30], preference:-1, tcp:useTCP});
...
function onOpen() {
  ...
  saveUseSmoothing = getProperty("useSmoothing");
...
function setSmoothing(mode) {
  ...
  if (mode) { // enable smoothing
    if (getProperty("useSmoothing") == "0") {
      ...
      if (getProperty("useSmoothing") != "9998") {
...
function onParameter(name, value) {
  if (name == "action") {
    ...
    if (sText2[0] == "SMOOTH") {
      var sText3 = new Array();
      sText3 = sText2[1].split(",");
      if (sText3[0].toUpperCase() == "ON") {
        if (sText3[1]) {
          var num = parseFloat(sText3[1]);
          if (num < 0 || num > 10) {
            error(localize("SMOOTH value must be between 0-10" + ": " + value));
            return;
          } else {
            setProperty("useSmoothing", num.toString());
          }
        } else {
          setProperty("useSmoothing", "0");
        }
      } else if (sText3[0].toUpperCase() == "OFF") {
        setProperty("useSmoothing", "-1");
      } else if (sText3[0].toUpperCase() == "NANO") {
        setProperty("useSmoothing", "9998");
      } else if (sText3[0].toUpperCase() == "AUTO") {
        setProperty("useSmoothing", "9999");
      } else {
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane  || smoothing.cancel  || toolChecked) {
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    if (machineConfiguration.isMultiAxisConfiguration() && (machineConfiguration.getNumberOfAxes() >= 4)) {
      ...
      if (machineConfiguration.getNumberOfAxes() == 5) {
        writeBlock(mClampModal.format(38)); // lock 5th-axis motion
      }
    }
    return;
  case COMMAND_UNLOCK_MULTI_AXIS:
    if (machineConfiguration.isMultiAxisConfiguration() && (machineConfiguration.getNumberOfAxes() >= 4)) {
      ...
      if (machineConfiguration.getNumberOfAxes() == 5) {
        writeBlock(mClampModal.format(39)); // unlock 5th-axis motion
      }
    }
    return;
...
function onSectionEnd() {
  ...
  forceAny();
  setProperty("useSmoothing", saveUseSmoothing);
}
...
function onReturnFromSafeRetractPosition(_x, _y, _z) {
  ...
  if (getSetting("workPlaneMethod.prepositionWithTWP", true) && settings.workPlaneMethod.useTiltedWorkplane && tcp.isSupportedByOperation) {
    forceXYZ();
    var isRequired = true; // never use optional state
    writeInitialPositioning(new Vector(_x, _y, _z), isRequired); // initial positioning
    forceXYZ();
    invokeOnRapid5D(_x, _y, _z, getCurrentDirection().x, getCurrentDirection().y, getCurrentDirection().z);
  } else {
    ...
    if (tcp.isSupportedByOperation) {
      ...
      yOutput.reset();
      zOutput.disable();
      invokeOnRapid(_x, _y, _z); // position in XY
      zOutput.enable();
      invokeOnRapid(_x, _y, _z); // position in Z
    }
  }
}
...
function onClose() {
  ...
  if (gRotationModal.getCurrent() == 68.2) { // cancel G68 before retracting
    cancelWorkPlane();
  }
  ...
  writeln("");
  writeln("N" + Math.max(sequenceNumber, settings.maximumSequenceNumber));
  writeln("");
  writeln("%");
}
