/**
  ...
  KIMLA post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {0AAD8622-5765-AB1C-D898-8F32F67B8C49}
*/

description = "KIMLA";
vendor = "KIMLA";
vendorUrl = "https://kimla.pl/en";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic post for KIMLA.";
...
// var lengthCompensationActive = true; // tool length compensation is always enabled with the tool call
...
function onSection() {
  ...
  // stop spindle between sections
  if (!isFirstSection()) {
    onCommand(COMMAND_STOP_SPINDLE);// if the next section is executed with the same tool, the controller will not switch off the spindle
  ...
  // tool change
  if (!insertToolCall) {
    onCommand(COMMAND_LOAD_TOOL); // output tool call command which is used to create an operation list on the control
  } else {
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    writeToolBlock("T" + toolFormat.format(tool.number), formatComment(getParameter("operation-comment", "")));
