...
description = "Tsugami MO8SY/MO6SY Mill/Turn with Fanuc 0i control";
vendor = "Tsugami";
...
longDescription = "Tsugami MO8SY/MO6SY lathe (Fanuc 0i) with support for mill-turn.";
...
properties = {
  ...
  cAxisDesignation: {
    title      : "Use C/B for C-axis",
    description: "Enable to use C (main) and B (sub) for the C-axis.  Disable to use CM (main) and CB (sub).",
    group      : "multiAxis",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
var cFormat = createFormat({decimals:4, type:FORMAT_REAL, scale:DEG});
...
var yAxisMinimum = toPreciseUnit(-50.0, MM); // specifies the minimum range for the Y-axis
var yAxisMaximum = toPreciseUnit(50.0, MM); // specifies the maximum range for the Y-axis
...
function getCode(code, spindle) {
  ...
  case "START_SPINDLE_CW":
    ...
    case SPINDLE_LIVE:
      ...
      return (getMachiningDirection(currentSection) == MACHINING_DIRECTION_RADIAL) ? 204 : (getSpindle(PART) == SPINDLE_MAIN) ? 204 : 203;
    }
    break;
  case "START_SPINDLE_CCW":
    ...
    case SPINDLE_LIVE:
      machineState.liveToolIsActive = true;
      return (getMachiningDirection(currentSection) == MACHINING_DIRECTION_RADIAL) ? 203 : (getSpindle(PART) == SPINDLE_MAIN) ? 203 : 204;
    }
    break;
...
function invertAxes(activate, polarMode) {
  ...
  if (getProperty("cAxisDesignation")) {
    _prefix = activate ? "B" : "C";
  } else {
    _prefix = activate ? "CB" : "CM";
  }
  ...
  if (polarMode) {
    cOutput.disable();
    yOutput.setPrefix(_prefix);
  } else {
    cOutput.setPrefix(_prefix);
  }
...
function onSection() {
  ...
  if (spindleChange) {
    ...
    if (!tapping || (getParameter("operation:cycleType", "") == "tapping-with-chip-breaking")) {
...
function setPolarInterpolation(activate) {
  if (activate) {
    ...
    if (getSpindle(PART) == SPINDLE_SUB) {
      ...
      var _prefix = getProperty("cAxisDesignation") ? "C" : "CM";
      yOutput.setPrefix(_prefix);
    ...
    case "secondary-spindle-pull":
      ...
      if (pullMethod.home) {
        ...
        if ((transferCodes != undefined) && !transferCodes.cutoff) {
          ...
          writeBlock(mFormat.format(getCode("SPINDLE_SYNCHRONIZATION_PHASE_OFF", getSpindle(PART))), formatComment("CANCEL PHASE SYNCHRONIZATION"));
        } else {
...
function ejectPart() {
  ...
  if (getProperty("usePartCatcher")) {
    writeBlock(mFormat.format(getCode("PART_CATCHER_ON", getSpindle(PART))) + " (CATCHER ADVACE)");
  }
  writeBlock(gFormat.format(1), barOutput.format(0), formatComment("ENTER A POSITION FOR EJECT"));
  writeBlock(mFormat.format(getCode("UNCLAMP_CHUCK", getSpindle(PART))), formatComment("OPEN SUB"));
  writeBlock(mFormat.format(getCode("CYCLE_PART_EJECTOR")), formatComment("EJECT PART"));
  onDwell(0.5);
  if (getProperty("usePartCatcher")) {
    writeBlock(mFormat.format(getCode("PART_CATCHER_OFF", getSpindle(PART))), formatComment("CATCHER RETRACT"));
  }
  // clean out chips
  if (airCleanChuck) {
