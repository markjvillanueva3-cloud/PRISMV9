/**
  ...
  VOLTER CNC post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {A30957EA-A8CD-49D8-9210-0470A1B2D05D}
*/

description = "VOLTER ACP Series CNC Routers";
vendor = "VOLTER";
vendorUrl = "https://volter.com/";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic post for VOLTER ACP Series CNC Routers.";
...
allowedCircularPlanes = 1 << PLANE_XY; // allow XY plane circular
...
var ijkFormat = createFormat({decimals:(unit == MM ? 3 : 4), trim:false, forceSign:true});
var feedFormat = createFormat({decimals:(unit == MM ? 1 : 2), trim:false});
...
var kOutput = createOutputVariable({prefix:"K", control:CONTROL_FORCE}, ijkFormat);
...
function onOpen() {
  ...
  if (!machineConfiguration.isReceived()) {
    ...
    machineConfiguration.setRetractPlane(15); // always MM
  }
  ...
  writeProgramHeader();
  unit = MM; // output is always in MM
...
function writeInitialPositioning(initialPosition, isRequired) {
  ...
  if (!isRequired) {
    if (!state.retractedZ && xyzFormat.getResultingValue(getCurrentPosition().z) < xyzFormat.getResultingValue(initialPosition.z)) {
      writeBlock(gMotionModal.format(motionCode.single), zOutput.format(initialPosition.z), feed);
    }
    writeBlock(gMotionModal.format(motionCode.multi), xOutput.format(initialPosition.x), yOutput.format(initialPosition.y), feed);
  } else {
    writeBlock(
      gMotionModal.format(motionCode.multi), xOutput.format(initialPosition.x), yOutput.format(initialPosition.y), zOutput.format(initialPosition.z), feed
    );
  }
}
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    if (insertToolCall && !isFirstSection()) {
      writeBlock(getCoolantCodes(COOLANT_OFF), mFormat.format(5)); // stop spindle before retract during tool change
    }
...
function onClose() {
  ...
  if (getProperty("safePositionMethod") == "clearanceHeight") {
    forceXYZ();
    gMotionModal.reset();
    var retractPlane = machineConfiguration.getRetractPlane();
    writeBlock(gMotionModal.format(0),
      xOutput.format(machineConfiguration.getHomePositionX()),
      yOutput.format(machineConfiguration.getHomePositionY()),
      conditional(retractPlane >= getCurrentPosition().z, zOutput.format(retractPlane))
    );
  } else {
    writeRetract(Z);
  ...
  writeBlock(getCoolantCodes(COOLANT_OFF), mFormat.format(5));
}
