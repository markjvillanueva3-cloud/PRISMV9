...
properties = {
  ...
  gotSecondarySpindle: {
    title      : "Secondary spindle",
    description: "Specifies that the machine has a secondary spindle.",
    group      : "configuration",
    type       : "boolean",
    value      : true,
    scope      : "post"
 ...
 */
  ...
  if (spindleChange || operationNeedsSafeStart) {
    ...
    // Turn spindle on
    writeStartBlocks(spindleChange, function () {
  ...
  if (x || y || z) {
    ...
    if (pendingRadiusCompensation >= 0) {
      ...
      case RADIUS_COMPENSATION_LEFT:
        writeBlock(gMotionModal.format(linearCode), gFormat.format(41), x, y, z, getFeed(feed));
        break;
      case RADIUS_COMPENSATION_RIGHT:
        writeBlock(gMotionModal.format(linearCode), gFormat.format(42), x, y, z, getFeed(feed));
        break;
      default:
...
function onParameter(name, value) {
  ...
  case "action":
    ...
    } else if (String(value).toUpperCase() == "usePolarInterpolation") {
      forcePolarInterpolation = true;
      forcePolarCoordinates = false;
    } else {
      invalid = true;
...
function startSpindle(tappingMode, forceRPMMode, initialPosition, rpm) {
  ...
  if ((getSpindle(PART) == SPINDLE_SUB) && !getProperty("gotSecondarySpindle")) {
    error(localize("Secondary spindle is not available."));
    return;
  }
