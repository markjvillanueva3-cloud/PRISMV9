/**
  ...
  NewCNC router post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {CB457F2F-3459-4C28-B4F1-6987B2099136}
*/

description = "NewCNC Router";
vendor = "NewCNC";
vendorUrl = "https://newcnc.com/";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic post for NewCNC router.";
...
allowedCircularPlanes = 0; // allow no circular output
...
wcsDefinitions = {
  useZeroOffset: false,
  wcs          : [
    {name:"Standard", format:"(UAO,#)", range:[1, 6]}
  ]
};
...
function onOpen() {
  ...
  if (machineConfiguration.isHeadConfiguration()) {
    settings.outputToolLengthCompensation = true;
    settings.outputToolLengthOffset = true;
  }

  writeComment(formatComment("Program name: " + programName));
  writeComment(formatComment("Program comment: " + programComment));
  writeProgramHeader();
  ...
  if (xyzFormat.getResultingValue(workpiece.upper.z) > 0 && xyzFormat.getResultingValue(workpiece.lower.z) < 0) {
    writeComment("[Zero tool on PART MATERIAL SURFACE]");
  } else if (xyzFormat.getResultingValue(workpiece.upper.z) == 0 && xyzFormat.getResultingValue(workpiece.lower.z) < 0) {
    writeComment("[Zero tool on STOCK MATERIAL SURFACE]");
  } else {
    writeComment("[Zero tool on MACHINE BED]");
  }
  var xCenCheck = workpiece.upper.x + workpiece.lower.x;
  var yCenCheck = workpiece.upper.y + workpiece.lower.y;
  if (xyzFormat.getResultingValue(workpiece.lower.x) == 0 && xyzFormat.getResultingValue(workpiece.lower.y) == 0) {
    writeComment("X/Y Zero is at MATERIAL LEFT LOWER CORNER");
  } else if (xyzFormat.getResultingValue(xCenCheck) == 0 && xyzFormat.getResultingValue(yCenCheck) == 0) {
    writeComment("X/Y Zero is at MATERIAL CENTER");
  } else {
    writeComment("X/Y Zero is at NON-STANDARD LOCATION");
  }
  var xStockDim = workpiece.upper.x - workpiece.lower.x;
  var yStockDim = workpiece.upper.y - workpiece.lower.y;
  var zStockDim = workpiece.upper.z - workpiece.lower.z;
  writeComment("Stock size is X(" + xyzFormat.format(xStockDim) + "), Y(" + xyzFormat.format(yStockDim) + "), Z(" + xyzFormat.format(zStockDim) + ")");
...
function writeRetract() {
  ...
  if (retract && retract.words.length > 0) {
    ...
    for (var i in retract.words) {
      ...
      case "G79":
        gMotionModal.reset();
        writeBlock(gFormat.format(79), gMotionModal.format(0), words);
        break;
      default:
