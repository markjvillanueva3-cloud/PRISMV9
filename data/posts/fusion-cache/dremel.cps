/**
  ...
  DREMEL post processor configuration.

  $Revision: 44106 85222cccf9cee3e216d9ee111aa5823d026a774f $
  $Date: 2024-01-03 15:11:55 $

  FORKID {5E2D9156-D6BE-433B-ABBF-B2FE755099E4}
*/

description = "Dremel";
vendor = "Dremel";
vendorUrl = "https://www.dremel.com/gn/en/digilab";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Post for exporting toolpaths to the Dremel range of printers, 3D40, 3D45";
...
var settings = {
  ...
  maximumExtruderTemp: 230, // specifies the maximum extruder temperature
...
function onOpen() {
  ...
  writeComment("TIME:" + integerFormat.format(printTime)); // for count down timer
...
function onSection() {
  ...
  writeBlock(getCode(getProperty("relativeExtrusion") ? commands.extrusionMode.relative : commands.extrusionMode.absolute)); // extrusion mode
  writeBlock(mFormat.format(132), "X Y Z A"); // recall stored home offsets
  writeBlock(mFormat.format(907), xOutput.format(100), yOutput.format(100), zOutput.format(60), "A" + xyzFormat.format(100));
  if (getProperty("useEnclosureFan")) {
    writeBlock(mFormat.format(651)); // start enclosure fan if enabled
  }
...
function onExtruderTemp(temp, wait, id) {
  ...
  if (wait) {
    ...
    writeBlock(getCode(commands.waitExtruder), sOutput.format(temp));
  } else {
...
function onClose() {
  ...
  writeBlock(gFormat.format(92), eOutput.format(toPreciseUnit(1, MM)));
  writeBlock(gMotionModal.format(1), eOutput.format(toPreciseUnit(-1, MM)), feedOutput.format(300));
  writeBlock(gFormat.format(162), "Z", feedOutput.format(toPreciseUnit(600, MM)));
  writeBlock(gFormat.format(162), "X Y", feedOutput.format(toPreciseUnit(2000, MM)));
  writeBlock(mFormat.format(84)); // disable steppers
...
// <<<<< INCLUDED FROM ../common/onBedTemp.cpi
//include("../common/onExtruderTemp.cpi");
//include("../common/onExtruderChange.cpi");  // commented as the T0 command is erroring on the printer
