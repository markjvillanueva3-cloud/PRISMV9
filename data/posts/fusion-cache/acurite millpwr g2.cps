...
properties = {
  ...
  useM06: {
    title      : "Use tool change codes",
    description: "Specifies that the M6, Lxx, and Dxx codes should be output.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
...
function onOpen() {
  ...
  if (machineConfiguration.getMaximumSpindleSpeed() == 0) {
    settings.maximumSpindleRPM = getProperty("maximumSpindleSpeed");
  }
...
function onSection() {
  ...
  // tool change
  if (insertToolCall) {
    if (getProperty("useExactStop") && (isDrillingCycle() ||
      !isFirstSection() && isDrillingCycle(getPreviousSection()))) {
      setExactStop(false);
    }
    forceWorkPlane();
    onCommand(COMMAND_COOLANT_OFF);
    if (tool.manualToolChange) {
      writeToolBlock(formatComment("T" + toolFormat.format(tool.number) + " D" + xyzFormat.format(tool.diameter)));
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    if (getProperty("useM06")) {
      writeToolBlock("T" + toolFormat.format(tool.number), "L" + xyzFormat.format(tool.bodyLength), "D" + xyzFormat.format(tool.diameter), mFormat.format(6));
    } else {
