/**
  ...
  Anet 3D post processor configuration.

  $Revision: 44106 85222cccf9cee3e216d9ee111aa5823d026a774f $
  $Date: 2024-01-03 15:11:55 $

  FORKID {956EFBAF-8459-40E1-B65B-2FD62E1E64BC}
*/

description = "Anet 3D";
vendor = "Anet";
vendorUrl = "http://www.anet3d.com/";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Post for exporting toolpath to the Anet 3D range of printers in gcode format (A6, A8, ET4, ET5...)";
...
highFeedrate = toPreciseUnit(9000, MM);
...
properties = {
  ...
  meshLeveling: {
    title      : "Leveling using current mesh",
    description: "Specifies if the leveling is activated, using the current mesh.",
    type       : "boolean",
    value      : true,
    scope      : "post"
  },
  compensatedHeight: {
    title      : "Maximum height for compensation",
  ...
  moveBedToFront: {
    title      : "Move the bed to the front when finished",
    description: "Specifies if we want to move the bed to the front, at the program end.",
    type       : "boolean",
    value      : true,
    scope      : "post"
  }
...
function onSection() {
  writeBlock(gMotionModal.format(1), feedOutput.format(highFeedrate));
  ...
  if (bedTemp > 0) {
    writeBlock(mFormat.format(155), sOutput.format(5)); // activate temperature sensor
  }

  // homing X Y
  writeBlock(gFormat.format(28), xOutput.format(0), yOutput.format(0));
  // homing Z, for bed probing function, it should be after XY
  writeBlock(gFormat.format(28), zOutput.format(0));
  forceXYZE();

  if (getProperty("printerModel") == "a6") {
    writeBlock(mFormat.format(84)); // stepper off
    writeBlock(mFormat.format(0), sOutput.format(12)); // dwell
    writeBlock(mFormat.format(17)); // stepper on
  }

  if (getProperty("meshLeveling") == true) {
    writeBlock(mFormat.format(420), sOutput.format(1));
    sOutput.reset();
    writeBlock(mFormat.format(420), zOutput.format(getProperty("compensatedHeight")));
    writeBlock(mFormat.format(900), "K0");
  }

  // lower build plate
  feedOutput.reset();
  var initialPosition = getFramePosition(currentSection.getInitialPosition());
  writeBlock(gMotionModal.format(1), zOutput.format(initialPosition.z), feedOutput.format((unit == MM ? highFeedrate : highFeedrate / 25.4)));
...
function onClose() {
  if (getProperty("printerModel") == "a6") {
    ...
    writeBlock(gFormat.format(28), xOutput.format(0), yOutput.format(0));
  } else if (getProperty("printerModel") == "others") {
    writeBlock(gFormat.format(28), xOutput.format(0), yOutput.format(0));
  }
  if (getProperty("moveBedToFront")) {
    if (getProperty("printerModel") == "a8") {
      writeBlock(gAbsIncModal.format(90));
      writeBlock(gMotionModal.format(1), yOutput.format((unit == MM ? 190 : 7.48)), feedOutput.format((unit == MM ? highFeedrate : highFeedrate / 25.4)));
    } else if (getProperty("printerModel") == "a6") {
      writeBlock(gMotionModal.format(1), yOutput.format((unit == MM ? 210 : 8.26)), feedOutput.format((unit == MM ? highFeedrate : highFeedrate / 25.4)));
    } else if (getProperty("printerModel") == "others") {
      var pos = machineConfiguration.getDepth() - machineConfiguration.getCenterPositionY();
      pos -= (unit == MM) ? 10 : 10 / 25.4;
      writeBlock(gMotionModal.format(1), yOutput.format(pos), feedOutput.format((unit == MM ? highFeedrate : highFeedrate / 25.4)));
    }
  }
