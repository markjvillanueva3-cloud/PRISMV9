...
longDescription = "Generic milling post for FIDIA.";
...
properties = {
  ...
  useABCPrepositioning: {
    ...
    description: "Enable to preposition rotary axes prior to G21 blocks.",
    group      : "multiAxis",
    type       : "boolean",
    value      : true,
    scope      : ["machine", "post"]
...
var feedFormat = createFormat({decimals:0, forceDecimal:false, scale:(unit == MM ? 1 : 100)});
...
var gAbsIncModal = createOutputVariable({}, gFormat); // modal group 1 // G90-91
...
var gRotationModal = createOutputVariable({}, gFormat); // modal group 16 // G20-G21
...
var settings = {
  ...
  workPlaneMethod: {
    ...
    // useABCPrepositioning  : true, // controlled by property - position ABC axes prior to tilted workplane blocks
    prepositionWithTWP    : false, // Use tilted workplane commands for multi axis toolpath prepositioning
...
function onOpen() {
  ...
  writeProgramHeader();
  // feed per min is the only feedrate mode on the Fidia controller
...
function cancelWorkPlane(force) {
  ...
  writeBlock(gRotationModal.format(20)); // cancel frame
  forceWorkPlane();
}
...
function onSection() {
  ...
  if (currentSection.isMultiAxis() && tcp.isSupportedByOperation) {
    for (var i in tcpCommands) {
      writeBlock(gFormat.format(tcpCommands[i].on));
    }
  }
  // prepositioning
...
function onCyclePoint(x, y, z) {
  ...
  if (currentSection.isOptimizedForMachine()) {
    forward = machineConfiguration.getOptimizedPosition(currentSection.workPlane.forward, getCurrentDirection(), TCP_XYZ, OPTIMIZE_BOTH, false);
  } else {
...
function onLinear5D(_x, _y, _z, _a, _b, _c, feed, feedMode) {
  ...
  if (feedMode == FEED_INVERSE_TIME) {
    error(localize("This controller does not support inverse time feed rate mode."));
    return;
  }
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    writeBlock(mFormat.format(10)); // depending on the machine parameters AXSLCKM10, or AXSLCKM12 it can be M10, or M12
    return;
  case COMMAND_UNLOCK_MULTI_AXIS:
    writeBlock(mFormat.format(11)); // M11 unlock all axis
    return;
...
function onSectionEnd() {
  if (currentSection.isMultiAxis() && tcp.isSupportedByOperation) {
    for (var i in tcpCommands) {
      writeBlock(gFormat.format(tcpCommands[i].off));
    }
  }
