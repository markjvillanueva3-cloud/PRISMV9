...
var forcePolarCoordinates = false; // forces XZC output, activated by Action:usePolarInterpolation
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint()) { // first cycle point
    if (getProperty("cycleFormat") == "f15") {
      ...
      rapto = (getSpindle(PART) == SPINDLE_SUB && getMachiningDirection(currentSection) == MACHINING_DIRECTION_AXIAL) ? cycle.clearance - cycle.retract :  cycle.retract - cycle.clearance; // radius mode
    }
