/**
  Copyright (C) 2017-2023 by Autodesk, Inc.
  All rights reserved.

  RosettaCNC post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {34100C56-9423-4797-AC49-93C271621785}
*/

description = "Rosetta Mill";
vendor = "RosettaCNC Motion";
vendorUrl = "http://rosettacnc.com";
legal = "Copyright (C) 2012-2023 Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic post for RosettaCNC.";
...
properties = {
  ...
  axes: {
    title      : "Machine axes",
    description: "Sets the kinematics of the machine",
    group      : "configuration",
    type       : "enum",
    values     : [
      {id:"3", title:"3 Axis"},
      {id:"x", title:"4 Axis (Ax)"},
      {id:"y", title:"4 Axis (Ay)"},
      {id:"ac", title:"5 Axis (AxCz)"}
    ],
    value: "3",
    scope: "post"
  },
...
function defineMachine() {
  var useTCP = false;
  if (getProperty("axes") != "3") {
    var axis = getProperty("axes");
    var aAxis = createAxis({coordinate:0, table:true, axis:[axis != "y" ? 1 : 0, axis == "y" ? 1 : 0, 0], cyclic:axis != "ac", range:[-90, 90], preference:0, tcp:useTCP});
    if (getProperty("axes") == "ac") {
      var cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], cyclic:true, range:[-9999, 9999], preference:0, tcp:useTCP});
  ...
  if (!receivedMachineConfiguration) {
    ...
    machineConfiguration.setRetractPlane(toPreciseUnit(300, MM));
  }
}
...
function onOpen() {
  ...
  writeComment(" ==                        == ");
  writeComment(" == Created for RosettaCNC == ");
  writeComment(" ==                        == ");
  switch (getProperty("axes")) {
  case "3":
    writeComment(" ==    3 Axis Mill - v2    == ");
    break;
  case "x":
    writeComment(" ==    4 Axis Mill - v2    == ");
    writeComment(" ==   XYZA[x] - RTCP OFF   == ");
    break;
  case "y":
    writeComment(" ==    4 Axis Mill - v2    == ");
    writeComment(" ==   XYZA[y] - RTCP OFF   == ");
    break;
  case "ac":
    writeComment(" ==    5 Axis Mill - v2    == ");
    writeComment(" == XYZA[x]C[z] - RTCP OFF == ");
    break;
  }
  writeComment(" ==                        == ");
  writeln("");
  ...
  writeBlock(gFormat.format(49));  // disable tool compensation
}
