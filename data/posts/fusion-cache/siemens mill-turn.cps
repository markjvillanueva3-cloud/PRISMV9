...
var cFormat = createFormat({decimals:3, forceDecimal:true, scale:DEG, cyclicLimit:Math.PI * 2, prefix:"="});
...
function formatFeedMode(mode) {
  var fMode = (mode == FEED_PER_REVOLUTION) || tapping || isDrillingCycle() ? getCode("FEED_MODE_UNIT_REV") : getCode("FEED_MODE_UNIT_MIN");
  if (fMode) {
...
function getCycleFeed(f) {
  if (currentSection.feedMode != FEED_PER_REVOLUTION && machineState.feedPerRevolution) {
    f /= spindleSpeed;
  }
  return feedFormat.format(f);
}
...
function setPolarCoordinates(mode) {
  ...
  activatePolarMode(unit == IN ? 0.0004 : (getTolerance() / 2), 0, direction); // TAG: failure if using tolerance / 2 in inch mode
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      ...
      if (turns > 0) {
        writeBlock(gMotionModal.format(directionCode), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x, 0), jOutput.format(cy - start.y, 0), getFeed(feed), "TURN=" + turns);
      } else {
    ...
    case PLANE_ZX:
      ...
      if (turns > 0) {
        writeBlock(gMotionModal.format(directionCode), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x, 0), kOutput.format(cz - start.z, 0), getFeed(feed), "TURN=" + turns);
      } else {
    ...
    case PLANE_YZ:
      ...
      if (turns > 0) {
        writeBlock(gMotionModal.format(directionCode), xOutput.format(x), yOutput.format(y), zOutput.format(z), jOutput.format(cy - start.y, 0), kOutput.format(cz - start.z, 0), getFeed(feed), "TURN=" + turns);
      } else {
...
*/
  ...
  case "reaming":
    writeCycleClearance();
    forceFeed();
    var FFR = getCycleFeed(cycle.feedrate);
    forceFeed();
    var RFF = getCycleFeed(cycle.retractFeedrate);
    writeBlock(
