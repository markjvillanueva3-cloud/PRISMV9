...
longDescription = "Generic milling post for FAGOR controls, such as 8037 and 8055, with an optional rotary table.";
...
var gPlaneModal = createOutputVariable({onchange:function () {gMotionModal.reset();}}, gFormat); // modal group 2 // G16-19
    ...
    case "tapping":
      if (tool.type == TOOL_TAP_LEFT_HAND) {
        ...
        writeBlock(
          ...
          getCommonCycle(x, y, cycle.retract, cycle.bottom),
          ((K > 0) ? "K" + kFormat.format(K) : ""), "R" + getProperty("tappingMode"),
          feedOutput.format(F)
        );
      }
      break;
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      ...
      writeBlock(gAbsIncModal.format(90), conditional(!getProperty("useG16"), gPlaneModal.format(17)), gFeedModeModal.format(94), gMotionModal.format(clockwise ? 2 : 3), iOutput.format(cx - start.x), jOutput.format(cy - start.y), feedOutput.format(feed));
      break;
    case PLANE_ZX:
      if (getProperty("useG16")) {
        writeBlock(getG16("ZX"));
      }
      writeBlock(gAbsIncModal.format(90), conditional(!getProperty("useG16"), gPlaneModal.format(18)), gFeedModeModal.format(94), gMotionModal.format(clockwise ? 2 : 3), iOutput.format(cx - start.x), kOutput.format(cz - start.z), feedOutput.format(feed));
      break;
    case PLANE_YZ:
      if (getProperty("useG16")) {
        writeBlock(getG16("YZ"));
      }
      writeBlock(gAbsIncModal.format(90), conditional(!getProperty("useG16"), gPlaneModal.format(19)), gFeedModeModal.format(94), gMotionModal.format(clockwise ? 2 : 3), jOutput.format(cy - start.y), kOutput.format(cz - start.z), feedOutput.format(feed));
      break;
    default:
      linearize(tolerance);
    }
    ...
    case PLANE_XY:
      ...
      writeBlock(gAbsIncModal.format(90), conditional(!getProperty("useG16"), gPlaneModal.format(17)), gFeedModeModal.format(94), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), iOutput.format(cx - start.x), jOutput.format(cy - start.y), zOutput.format(z), feedOutput.format(feed));
      break;
    case PLANE_ZX:
      if (isHelical()) {
        linearize(tolerance);
      ...
      writeBlock(gAbsIncModal.format(90), conditional(!getProperty("useG16"), gPlaneModal.format(18)), gFeedModeModal.format(94), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x), kOutput.format(cz - start.z), feedOutput.format(feed));
      break;
    case PLANE_YZ:
      if (isHelical()) {
        linearize(tolerance);
      ...
      writeBlock(gAbsIncModal.format(90), conditional(!getProperty("useG16"), gPlaneModal.format(19)), gFeedModeModal.format(94), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), jOutput.format(cy - start.y), kOutput.format(cz - start.z), feedOutput.format(feed));
      break;
    default:
      linearize(tolerance);
    }
