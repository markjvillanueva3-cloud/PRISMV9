...
longDescription = "Generic milling post for the FESTO LabVolt series.";
...
function writeInitialPositioning(position, isRequired, codes1, codes2) {
  ...
  writeStartBlocks(isRequired, function() {
    ...
    if (machineConfiguration.isHeadConfiguration()) {
      writeBlock(gMotionModal.format(motionCode.multi));
      writeBlock(xOutput.format(position.x), yOutput.format(position.y), zOutput.format(position.z),  gAbsIncModal.format(90), gAbsIncModal.format(90), feed);
    } else {
      writeBlock(gMotionModal.format(motionCode.multi));
      writeBlock(xOutput.format(position.x), yOutput.format(position.y), gAbsIncModal.format(90), feed);
      writeBlock(gMotionModal.format(motionCode.single));
  ...
  if (!isRequired) { // simple positioning
    ...
    if (!state.retractedZ && xyzFormat.getResultingValue(getCurrentPosition().z) < xyzFormat.getResultingValue(position.z)) {
      writeBlock(gMotionModal.format(motionCode.single));
      writeBlock(zOutput.format(position.z), feed);
    }
    forceXYZ();
    writeBlock(gMotionModal.format(motionCode.multi));
    writeBlock(xOutput.format(position.x), yOutput.format(position.y),  gAbsIncModal.format(90), feed);
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      ...
      writeBlock(xOutput.format(cx), yOutput.format(cy), zOutput.format(cz)); // arc center point
      forceXYZ();
      writeBlock(xOutput.format(x), yOutput.format(y), zOutput.format(z), f); // arc end point
      break;
    case PLANE_ZX:
      writeBlock(gPlaneModal.format(18));
      writeBlock(feedOutput.format(feed));
      ...
      writeBlock(xOutput.format(cx), yOutput.format(cy), zOutput.format(cz));
      forceXYZ();
      writeBlock(xOutput.format(x), yOutput.format(y), zOutput.format(z), f);
      break;
    case PLANE_YZ:
      writeBlock(gPlaneModal.format(19));
      writeBlock(feedOutput.format(feed));
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    writeToolBlock(mFormat.format(6), toolFormat.format(tool.number));
    if (tool.comment) {
