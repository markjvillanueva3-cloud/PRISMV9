/**
  ...
  $Revision: 44110 69fa9f39764e7c39c6bb68b2d037452ab01a8a71 $
  $Date: 2024-02-01 15:33:04 $
...
var programFilename = FileSystem.replaceExtension(FileSystem.getFilename(getOutputPath()), "");
...
function onCycle() {
  ...
  case "boring":
    ...
    break;
  case "fine-boring":
    statement = "CYCLE/BORE, FEDTO, " + d + ", " + feedUnit + ", " + f + ", CLEAR, " + c + ", NODRAG, " + cycle.shift + ", RETURN, " + RETURN;
    if (mainFormat.getResultingValue(cycle.retract - cycle.stock) > 0) {
      statement += ", RAPTO, " + RAPTO;
    }
    statement += ", ORIENT, " + 0; // unknown orientation
...
function createProjectFile() {
  ...
  file.writeln(" </SOURCE>");
  file.writeln(" <NAME>" + programFilename + "</NAME>");
  file.writeln(" <TOOLING>");

  file.writeln("  <TOOLLIBRARY LOADER=" + "\"" + "AUTODESKCAM_CAMPLETE_XML_TOOLING" + "\"" + ">" + ".\\" + programFilename + ".tdb</TOOLLIBRARY>");
  file.writeln(" </TOOLING>");
  file.writeln(" <TOOLPATHS>");
  file.writeln("  <TOOLPATH LOADER=" + "\"" + "AUTODESKCAM_CAMPLETE_APT" + "\"" + " UNITS=" + "\"" + ((unit == IN) ? "INCHES" : "MM") + "\"" + ">" + ".\\" + programFilename + "." + extension + "</TOOLPATH>");
  file.writeln(" </TOOLPATHS>");
  ...
  if (getProperty("exportPart")) {
    file.writeln("  <TARGETMODEL LOADER=" + "\"" + "GENERIC_STL" + "\"" + " UNITS=" + "\"" + ((unit == IN) ? "INCHES" : "MM") + "\"" + ">" + ".\\" + programFilename + "_PART.stl</TARGETMODEL>");
  }
  if (getProperty("exportStock") && hasGlobalParameter("autodeskcam:stock-path")) {
    file.writeln("  <STOCK LOADER=" + "\"" + "GENERIC_STL" + "\"" + " UNITS=" + "\"" + ((unit == IN) ? "INCHES" : "MM") + "\"" + ">" + ".\\" + programFilename + "_STOCK.stl</STOCK>");
  }
  if (getProperty("exportFixture") && hasGlobalParameter("autodeskcam:fixture-path")) {
    file.writeln("  <FIXTURE LOADER=" + "\"" + "GENERIC_STL" + "\"" + " UNITS=" + "\"" + ((unit == IN) ? "INCHES" : "MM") + "\"" + ">" + ".\\" + programFilename + "_FIXTURE.stl</FIXTURE>");
  }
  file.writeln(" </PARTINFO>");
...
function createVerificationJob() {
  ...
  if (getProperty("exportStock") && FileSystem.isFile(stockPath)) {
    destStockPath = FileSystem.getCombinedPath(destPath, programFilename + "_STOCK.stl");
    FileSystem.copyFile(stockPath, destStockPath);
  }

  if (getProperty("exportPart") && FileSystem.isFile(partPath)) {
    destPartPath = FileSystem.getCombinedPath(destPath, programFilename + "_PART.stl");
    FileSystem.copyFile(partPath, destPartPath);
  }

  if (getProperty("exportFixture") && FileSystem.isFile(fixturePath)) {
    destFixturePath = FileSystem.getCombinedPath(destPath, programFilename + "_FIXTURE.stl");
