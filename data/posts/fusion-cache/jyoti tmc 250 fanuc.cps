var modelType = "TMC 250";
description = "Jyoti TMC-250";
// >>>>> INCLUDED FROM ../common/jyoti tmc-ax fanuc mill-turn.cps
...
/**
  ...
  Jyoti Mill turn post processor configuration.

  $Revision: 44113 9e69e99eb0ed7579612c8e0c6b12203ef827eb3a $
  $Date: 2024-02-28 23:08:43 $

  FORKID {0BC635A1-197E-4C32-B90B-9EC8356A9E9B}
*/
...
if (!longDescription) {
  longDescription = subst("Preconfigured %1 post (Fanuc control) with support for mill-turn. Enter the Tool ID Code in the Product ID of the Tool. Leave Blank if not used", description);
}
...
maximumCircularSweep = toRad(120); // reduced sweep due to G12.1 support
...
properties = {
  ...
  torqueValue: {
    title      : "Torque Value",
    description: "Torque Value in % for part transfer ",
    group      : "preferences",
    type       : "integer",
    value      : 30,
    scope      : "post"
...
var gSelectSpindleModal = createModal({}, mFormat); // G92, G93
...
*/
  case "ENABLE_C_AXIS":
    ...
    return (spindle == SPINDLE_MAIN) ? 36 : 46;
  case "DISABLE_C_AXIS":
    machineState.cAxisIsEngaged = false;
    return (spindle == SPINDLE_MAIN) ? 37 : 47;
  ...
  case "LOCK_MULTI_AXIS":
    return (spindle == SPINDLE_MAIN) ? 71 : 73;
  case "UNLOCK_MULTI_AXIS":
    return (spindle == SPINDLE_MAIN) ? 72 : 74;
  /*
  case "CLAMP_CHUCK":
...
*/
  case "ACTIVATE_SPINDLE":
    return (spindle == SPINDLE_MAIN) ? 92 : 93;
  case "SELECT_SPINDLE":
    switch (spindle) {
    case SPINDLE_MAIN:
...
function onSection() {
  ...
  if (gotSecondarySpindle && !machineState.isTurningOperation) {
...
function onCycle() {
  if ((typeof isSubSpindleCycle == "function") && isSubSpindleCycle(cycleType)) {
    ...
    if (cycleType != "secondary-spindle-return") {
      ...
      gSelectSpindleModal.reset();
      // Customized part transfer code for Jyoti TM machine
      // G119  A F Q R  S  T
      // A - Job Clamp Position
      // F - Feed rate Reduced
      // Q - Job Transfer Type  if 12 - from spindle 1 to spindle 2 and if 21 - from spindle 2 to spindle 1
      // R - Retraction position after  Job transfer
      // S - Spindle Rpm when Job Transfer  if S0 then no Rpm
      // T - Torque Value in %
      writeBlock(
        gFormat.format(119),
        "A" + spatialFormat.format(cycle.chuckPosition),
        getFeed(cycle.feedrate),
        "Q" + (getSpindle(true) == SPINDLE_MAIN ? 12 : 21),
        "R" + spatialFormat.format(cycle.feedPosition),
        sOutput.format(cycle.spindleSpeed),
        ("T" + getProperty("torqueValue")));
    } else {
      error(localize("Secondary Pull and Return is not supported. Use secondary spindle chuck operation to perform part transfer"));
      return;
    }
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint()) { // first cycle point
    ...
    case "drilling":
      ...
      localZOutput.reset();
      writeBlock(
        gRetractModal.format(98), gCycleModal.format(plane == 19 ? 87 : 83),
    ...
    case "left-tapping":
      ...
      if (getProperty("useRigidTapping")) {
        ...
        writeBlock(
          gRetractModal.format(98), gCycleModal.format(plane == 19 ? 88 : 84),
          getCommonCycle(x, y, z, rapto, true),
          getFeed(cycle.feedrate),
          lockCode
        );
    ...
    case "boring":
      ...
      localZOutput.reset();
      writeBlock(
        gRetractModal.format(98), gCycleModal.format(plane == 19 ? 89 : 85),
        getCommonCycle(x, y, z, rapto, true),
        conditional(P > 0, pOutput.format(P)),
        getFeed(cycle.feedrate),
        lockCode
...
function onCycleEnd() {
  ...
  if (getProperty("useRigidTapping") &&  tapping)  {
    writeBlock(mFormat.format(49));
  }
}
...
function ejectPart() {
  ...
  writeBlock(mFormat.format(getCode("STOP_SPINDLE", SPINDLE_SUB))/*, pOutput.format(getCode("SELECT_SPINDLE", getSpindle(PART))));
  setCoolant(COOLANT_OFF);
  if (getProperty("optionalStop")) {
    onCommand(COMMAND_OPTIONAL_STOP);
    gMotionModal.reset();
...
// <<<<< INCLUDED FROM ../common/jyoti tmc-ax fanuc mill-turn.cps
properties.maximumSpindleSpeed.value = 2500;
