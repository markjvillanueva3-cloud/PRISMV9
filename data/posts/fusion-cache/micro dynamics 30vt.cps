/**
  ...
  Micro Dynamics Mega 30VT post processor configuration.

  $Revision: 44107 fdbddcd9a1d3cf71c49c0d31d4963c1877e5a756 $
  $Date: 2024-01-11 15:12:03 $

  FORKID {B2E09FC3-593F-4E86-ABBF-D4FF02C9B0FE}
*/

description = "Micro Dynamics Mega 30VT";
vendor = "Mitsubishi";
vendorUrl = "https://www.mdxfa.com/";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic post for Micro Dynamics Mega 30VT with a Mitsubishi M830W control with 5-axis capabilities.";
...
properties = {
  ...
  useSmoothingType: {
    ...
    description: "High accuracy control type I (G5.1 Q1), type II (G5 P10000), or type III (G5 P20000).",
    group      : "preferences",
    type       : "enum",
    values     : [
      {title:"Type I", id:"I"},
      {title:"Type II", id:"II"},
      {title:"Type III", id:"III"}
    ],
    value: "I",
    scope: "post"
  },
  ...
  fifthAxis: {
    title      : "Machine has 5-axis table",
    description: "Enable to output table positions for C-axis table on an A-axis table.",
    group      : "configuration",
    type       : "boolean",
    value      : true,
    scope      : "post"
  },
  useSynchronizedTapping: {
    title      : "Use synchronized tapping",
    description: "Synchronized tapping uses the thread pitch to control the feed.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
var mClampModal = createModalGroup(
  {strict:false},
  [[68, 69], // 4th axis LOCK / UNLOCK
    [10, 11]], // 5th axis LOCK / UNLOCK
  mFormat
);
...
function defineMachine() {
  ...
  if (getProperty("fifthAxis")) { // note: setup your machine here
    var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-120, 40], preference:-1, tcp:useTCP});
...
function setSmoothing(mode) {
  ...
  // set cutting mode, not supported on all controls
  if (false) {
    setHighAccuracy(mode);
  }

  if (getProperty("useSmoothing") == "0") {
    var gType =  getProperty("useSmoothingType") == "II" ? 2 : getProperty("useSmoothingType") == "III" ? 3 : 1;
    if (gType == 1) {
      writeBlock(gFormat.format(5.1), mode ? "Q1" : "Q0");
    } else  if (gType == 2) {
      writeBlock(gFormat.format(5), mode ? "P10000" : "P0");
    } else {
      writeBlock(gFormat.format(5), mode ? "P20000" : "P0");
    }
  } else {
    if (getProperty("useSmoothingType") == "I") {
      writeBlock(gFormat.format(5.1), mode ? formatWords("Q1", "R" + smoothing.level) : "Q0");
    }
  }
...
var currentHighSetting = 0;
function setHighAccuracy(mode) {
  var setting;
  if (isTappingCycle()) {
    setting = 63; // tapping mode
  } else if (!mode) {
    setting = 64; // cutting mode
  } else if (hasParameter("operation-strategy") && (getParameter("operation-strategy") == "drill")) {
    setting = 64; // machine default
    ...
    if (tolerance > 0) {
      ...
      if ((stockToLeave > stockToLeaveThreshold) && (verticalStockToLeave > stockToLeaveThreshold)) {
        ...
        if ((stockToLeave > 0) || (verticalStockToLeave > 0)) {
          ...
          setting = 64.1; // high accuracy mode
        }
      }
    } else {
      setting = 64; // cutting mode
  ...
  if (setting == currentHighSetting) {
    return false;
  }
  currentHighSetting = setting;
  writeBlock(gFormat.format(setting));
  return true;
}
  ...
  if (isFirstCyclePoint()) {
    ...
    case "tapping":
      ...
      writeBlock(
        ...
        getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
        conditional(P > 0, "P" + milliFormat.format(P)),
        getProperty("useSynchronizedTapping") ? pitchOutput.format(tool.getThreadPitch()) : feedOutput.format(F),
        conditional(getProperty("useSynchronizedTapping"), ",R1")
      );
      break;
    case "left-tapping":
      F = tool.getThreadPitch() * rpmFormat.getResultingValue(spindleSpeed);
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      writeBlock(
        gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3),
        xOutput.format(circle.end.x), yOutput.format(circle.end.y), zOutput.format(circle.end.z), "R" + rFormat.format(r), getFeed(feed));
      break;
    case PLANE_ZX:
      writeBlock(
        gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3),
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    var outputClampCodes = getProperty("useClampCodes") && (currentSection.isMultiAxis() || isPolarModeActive() || isNewWorkPlane());
    if (outputClampCodes && machineConfiguration.isMultiAxisConfiguration() && (machineConfiguration.getNumberOfAxes() >= 4)) {
      writeBlock(mClampModal.format(68)); // lock 4th-axis
      if (machineConfiguration.getNumberOfAxes() == 5) {
        writeBlock(mClampModal.format(10)); // lock 5th-axis
      }
    }
    return;
  case COMMAND_UNLOCK_MULTI_AXIS:
    var outputClampCodes = getProperty("useClampCodes") && (currentSection.isMultiAxis() || isPolarModeActive() || isNewWorkPlane());
    if (outputClampCodes && machineConfiguration.isMultiAxisConfiguration() && (machineConfiguration.getNumberOfAxes() >= 4)) {
      writeBlock(mClampModal.format(69)); // unlock 4th-axis
      if (machineConfiguration.getNumberOfAxes() == 5) {
        writeBlock(mClampModal.format(11)); // unlock 5th-axis
      }
    }
    return;
...
function onClose() {
  ...
  if (getProperty("homePositionCenter")) {
    ...
    writeBlock(gMotionModal.format(0), "X" + xyzFormat.format(_xHome)); // only desired when X is in the table
    writeRetract(Y);
  } else {
