/**
  ...
  MachMotion post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {E6025028-6737-40A8-A73D-33BFF4EAB87C}
*/

description = "MachMotion Control";
vendor = "MachMotion";
vendorUrl = "https://support.machmotion.com/";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic milling post for MachMotion control.  This post will support a routing machine with XZC-axes, but no Y-axis." +
  " You must use a Machine Configuration with the Y-axis limits set to 0,0 to enable this type of machine.";
extension = "tap";
setCodePage("ascii");
...
properties = {
  ...
  useRotaryScale: {
    ...
    description: "Enable to output the rotary table on a rotary scale (0-360), or disable to output it on a linear scale (-99999, 99999)." +
      "'Directional' will use a directional rotary scale where the sign of the angle determines the direction.",
    group : "configuration",
    type  : "enum",
    values: [
      {title:"No", id:"false"},
      {title:"Yes", id:"true"},
      {title:"Directional", id:"directional"}
    ],
    value: "false",
    scope: "post"
  }
...
var settings = {
  ...
  retract: {
    ...
    homeXY                    : {onIndexing:false, onToolChange:false, onProgramEnd:{axes:[X, Y]}} // Specifies when the machine should be homed in XY (sample: onIndexing:[X,Y]). Options can be combined
  },
  unwind: {
...
function onOpen() {
  ...
  var yRange = machineConfiguration.getAxisY().getRange();
  if (!machineConfiguration.getAxisY().isEnabled() || (!yRange.isNonRange() && xyzFormat.getResultingValue(yRange.minimum) == 0 &&
      xyzFormat.getResultingValue(yRange.maximum) == 0)) {
    if (!machineConfiguration.getAxisU().isEnabled() || Math.abs(Vector.dot(machineConfiguration.getAxisU().getAxis(), new Vector(1, 0, 0))) < 1e-7) {
      error(localize("You must define an X-axis table when the Y-axis is disabled."));
    }
    gotYAxis = false;
    yOutput.disable();
    var yIndex = settings.retract.homeXY.onProgramEnd.axes.indexOf(Y);
    if (settings.retract.homeXY.onProgramEnd != false && yIndex > -1) {
      settings.retract.homeXY.onProgramEnd.axes.splice(yIndex, 1); // remove Y-axis from homeXY if exists
    }
  }

  if (getProperty("useRotaryScale", "false") != "false") {
    var axis = -1;
    ...
    if (axis != -1) {
      ...
      output.setType(getProperty("useRotaryScale") == "true" ? TYPE_ABSOLUTE : TYPE_DIRECTIONAL);
...
function onSection() {
  ...
  if (!gotYAxis) {
    var yTolerance = xyzFormat.getResultingValue(toUnit(0.05, IN)); // TAG: tolerance is based on observed fluctuations in Y during simple and complex toolpaths, may need to be increased
    var box = currentSection.getBoundingBox();
    if (xyzFormat.getResultingValue(box.lower.y) < -yTolerance || xyzFormat.getResultingValue(box.upper.y) > yTolerance) {
      var msg = subst(localize("There is Y-axis movement of [%1, %2] when a Y-axis is not supported in operation '%3'."),
        xyzFormat.format(box.lower.y), xyzFormat.format(box.upper.y), getParameter("operation-comment")) + EOL +
        "** This could be due to the lead-in/lead-out moves, operation tolerance, or operation type chosen." + EOL +
        "** If the Y-axis deviation is acceptable you can increase the 'yTolerance' variable in the post processor." + EOL +
        subst("** It is currently set to %1.", xyzFormat.format(yTolerance));
      error(msg);
    }
  }
