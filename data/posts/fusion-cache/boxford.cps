...
description = "BOXFORD V10";
vendor = "BOXFORD";
vendorUrl = "http://www.boxford.co.uk";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic milling post for BOXFORD V10.";
...
allowedCircularPlanes = 0; // circular motion not supported
...
function onOpen() {
  ...
  var materialName = "Fusion360";

  writeln("[GM HEADER]");
  writeln("Src=Fusion360");
  writeln("type=mill");
  writeln("machine=M_Fusion");
  writeln("machine serial no=M_Fusion");
  writeln("units=" + ((unit == IN) ? "in" : "mm"));
  writeln("Material=" + materialName + ",1.0");
  writeln("last operation=WRITE");

  if (delta.isNonZero()) {
    writeln("bar shape=BOX");
    writeln("bar size=" + xyzFormat.format(delta.x) + "," + xyzFormat.format(delta.y) + "," + xyzFormat.format(delta.z));
  }

  if (tools.getNumberOfTools() > 0) {
    for (var i = 0; i < tools.getNumberOfTools(); ++i) {
      var tool = tools.getTool(i);
      var toolnum = tool.number + 1;
      var comment = "T" + toolFormat.format(tool.number) + "=" + toolFormat.format(toolnum) + "= ";
      var shankHeight = tool.getBodyLength() - tool.getShoulderLength();
      var shankDia = tool.shaftDiameter;
      var shankInfo = "SD=" + xyzFormat.format(shankDia) + ",SH=" + xyzFormat.format(shankHeight);
      var toolAddParams = "";

      var toolpos = tool.turret;
      if (toolpos < 1) {
        toolpos = 1;
      }

      switch (tool.getType()) {
      case TOOL_UNSPECIFIED: // - Unspecified tool.
        comment += "T=1,";
        break;
      case TOOL_DRILL: // - Drill.
        comment += "T=1,";
        if ((tool.taperAngle > 0) && (tool.taperAngle < Math.PI)) {
          var Angle = 180 - (tool.taperAngle / 2.0);
          toolAddParams = ",A=" + xyzFormat.format(Angle);
        }
        break;
      case TOOL_DRILL_CENTER: // - Center drill.
        comment += "T=1,";
        break;
      case TOOL_DRILL_SPOT:// - Spot drill.
        comment += "T=1,";
        break;
      case TOOL_DRILL_BLOCK:// - Block drill.
        comment += "T=1,";
        break;
      case TOOL_MILLING_END_FLAT:// - Flat end-mill.
        comment += "T=10,";
        break;
      case TOOL_MILLING_END_BALL:// - Ball end-mill.
        comment += "T=5,";
        toolAddParams = ",R=" + xyzFormat.format(tool.cornerRadius);
        break;
      case TOOL_MILLING_CHAMFER:// - Chamfer mill.
        comment += "T=23,";
        var Angle = 90 - tool.taperAngle;
        toolAddParams = ",A=" + xyzFormat.format(Angle);
        break;
      case TOOL_MILLING_FACE:// - Face mill.
        comment += "T=1,";
        break;
      case TOOL_MILLING_SLOT:// - Slot mill.
        comment += "T=2,";
        break;
      case TOOL_MILLING_RADIUS:// - Radius mill.
        comment += "T=5,";
        toolAddParams = ",R=" + xyzFormat.format(tool.cornerRadius);
        break;
      case TOOL_MILLING_DOVETAIL:// - Dovetail mill.
        comment += "T=7,";
        toolAddParams = ",A=" + xyzFormat.format(tool.taperAngle);
        break;
      case TOOL_MILLING_TAPERED:// - Tapered mill.
        comment += "T=23,";
        toolAddParams = ",A=" + xyzFormat.format(tool.taperAngle);
        break;
      case TOOL_REAMER:
        comment += "T=10,";
        warning(localize("Unsupported tool type - REAMER"));
        break;
      case TOOL_MILLING_END_BULLNOSE:
        comment += "T=10,";
        warning(localize("Unsupported tool type - BULL NOSE"));
        break;
      ...
      case TOOL_COUNTER_BORE:
        comment += "T=10,";
        warning(localize("Unsupported tool type - COUNTER_BORE"));
        break;
      ...
      case TOOL_HOLDER_ONLY:
        comment += "T=10,";
        warning(localize("Unsupported tool type - HOLDER ONLY"));
        break;
      default:
        comment += "T=10,";
        warning(localize("Unsupported unknown tool type"));
        break;
      }

      comment += "P=" + toolFormat.format(toolpos);
      comment += ",D=" + xyzFormat.format(tool.diameter) + ",C=" + xyzFormat.format(tool.getShoulderLength());

      if (toolAddParams != "") {
        comment += toolAddParams;
      }

      comment += "," + shankInfo;
      comment += ",FIT=YES,Z=0.000";
      writeln(comment);
    }
  }
...
function onSection() {
  ...
  */
  ...
  var workpiece = getWorkpiece();
  offset = new Vector(workpiece.lower.x * -1, workpiece.lower.y * -1, 0);
  writeBlock(gFormat.format(92),
    xOutput.format(offset.x),
    yOutput.format(offset.y)
  );
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    // In .box format use below
    // writeBlock(mFormat.format(6), "T" + toolFormat.format(tool.number)+"="+ xyzFormat.format(tool.diameter)+ "mm " + getToolTypeName(tool.type));
    // otherwise, use the code that is being output below
