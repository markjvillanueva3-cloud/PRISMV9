/**
  ...
  Faro CNC Multi-axis post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {C15E7AE2-2DDE-4980-80DA-F3A31D64C8CD}
*/

description = "FARO CNC";
vendor = "Faro Produzione Centri CNC";
vendorUrl = "http://www.faro.ve.it";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Postprocessor for Faro machines with Faro CNC control.";
...
allowedCircularPlanes = 1; // allow circular motion on G17 plane
...
var wcsDefinitions = {
  ...
  wcs          : [
    {name:"Standard", format:"G", range:[54, 56]} // standard WCS, output as G54-G59
  ]
};
...
var settings = {
  ...
  maximumToolLengthOffset     : 9999, // specifies the maximum allowed tool length offset number
  maximumToolDiameterOffset   : 9999, // specifies the maximum allowed tool diameter offset number
...
function onOpen() {
  if (unit != MM) {// machine requires input code in MM
    warning(localize("Outputting program in millimeters since the machine does not support inches."));
    unit = MM;
  }
...
function onSection() {
  ...
  if (insertToolCall || newWorkPlane) {
    ...
    writeRetract(Z);
    writeBlock(mFormat.format(95) + "B"); // Clear rotary axis turns counter
    zOutput.reset();
  }
  ...
  // just one origin, machine table-table with no TCP
  if (insertToolCall) {
    writeComment(currentSection.wcs);
  }
  ...
  if (comment) {
    writeComment(localize("toolpath: ") + comment);
  }
...
function writeRetract() {
  ...
  if (retract && retract.words.length > 0) {
    for (var i in retract.words) {
      ...
      gMotionModal.reset();
      writeBlock(gFormat.format(52), words);
