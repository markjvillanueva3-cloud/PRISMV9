/**
  ...
  Bantam Tools post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {79F7A930-8EF6-4E35-9401-A6FE752ABBF3}
*/

description = "Bantam Tools";
vendor = "Bantam Tools";
vendorUrl = "https://www.bantamtools.com/";

var gotWCS = false; // 'true' = has support for multiple WCS, 'false' = uses G55 only
...
var diameterOffsetFormat = createFormat({prefix:"D", decimals:0}); // not supported at the moment
...
var settings = {
  ...
  unwind: {
    ...
    codes         : [gFormat.format(10), "L20"], // formatted code(s) that will (virtually) unwind axis (G90 G28), (G92), etc.
    workOffsetCode: "P", // prefix for workoffset number if it is required to be output
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    if (insertToolCall && getProperty("useActiveSpindle")) {
  ...
  if (gotWCS) {
    ...
    var workOffset = currentSection.workOffset == 0 && !wcsDefinitions.useZeroOffset ? 1 : currentSection.workOffset;
...
function onCommand(command) {
  ...
  case COMMAND_START_SPINDLE:
    ...
    if (getProperty("useActiveSpindle")) {
      ...
      writeComment(mFormat.format(tool.clockwise ? 3 : 4) + " " + "S" + rpmFormat.format(spindleSpeed));
      wait = true;
    }
    return;
  case COMMAND_LOAD_TOOL:
    ...
    if (getProperty("preloadTool") && preloadTool && getProperty("useM06")) {
...
properties.useRadius.value = true;
