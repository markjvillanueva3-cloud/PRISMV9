// src/tools/autoHookWrapper.ts
var telemetryEngine2 = null;
var AUTO_HOOK_CONFIG = {
  enabled: true,
...
function getHookHistory(limit = 50) {
...
function validateSafetyProof(context) {
  ...
  let validityScore = 1;
  if (inputs) {
    if (inputs.cutting_speed !== void 0) {
    ...
    if (inputs.feed_per_tooth !== void 0) {
    ...
    if (inputs.tool_diameter !== void 0) {
  ...
  if (result) {
    ...
    if (result.tool_life !== void 0 && result.tool_life <= 0) {
...
var recentToolCalls = [];
var RECENT_ACTIONS_FILE = path24.join("C:\\PRISM\\state", "RECENT_ACTIONS.json");
...
function recordFlightAction(callNum, toolName, action2, params2, success, durationMs, result, errorMsg) {
  try {
    let actions = [];
    if (fs21.existsSync(RECENT_ACTIONS_FILE)) {
      try {
        actions = JSON.parse(fs21.readFileSync(RECENT_ACTIONS_FILE, "utf-8")).actions || [];
      } catch {
      }
    }
    let paramsSummary = "{}";
    ...
    fs21.writeFileSync(RECENT_ACTIONS_FILE, JSON.stringify({
...
function wrapWithUniversalHooks(toolName, handler) {
  const wrapped = async function(...args) {
    ...
    const journalPath = path24.join(STATE_DIR12, "SESSION_JOURNAL.jsonl");
    ...
    try {
      ...
      fs21.appendFileSync(journalPath, entry);
      try {
        const stat3 = fs21.statSync(journalPath);
        if (stat3.size > 5e4) {
          const all = fs21.readFileSync(journalPath, "utf-8").trim().split("\n");
          if (all.length > 200) {
            fs21.writeFileSync(journalPath, all.slice(-100).join("\n") + "\n");
          }
        }
      } catch {
      }
    ...
    if (!firstCallReconDone) {
      ...
      try {
        ...
        globalThis.__prism_recon = {
          ...
          knowledge: knowledgeHints?.total_enrichments ? knowledgeHints : void 0,
          scripts: scriptRecs?.recommendations?.length ? scriptRecs : void 0,
          rehydrated: rehydrated ? cadence.rehydrated : void 0
        };
      } catch {
      }
    }
    ...
    try {
      ...
      const errorClass = error ? error.constructor?.name || "UnknownError" : void 0;
    ...
    if (toolName === "prism_dev" && action2 === "build" && !error) {
      try {
        ...
        if (typeof resultText === "string" && resultText.includes("SUCCESS")) {
          ...
          const checklistPath = path24.join("C:\\PRISM\\state", "build_checklist.json");
          fs21.writeFileSync(checklistPath, JSON.stringify({
          ...
          try {
            const syncOut = (0, import_child_process2.execSync)(`py -3 "${path24.join("C:\\PRISM\\scripts\\core", "gsd_sync_v2.py")}" --apply --json`, {
              encoding: "utf-8",
              timeout: 15e3,
    ...
    if (toolName === "prism_safety" && result) {
      try {
        ...
        if (resultText?.safety_score !== void 0 && resultText.safety_score < 0.7) {
          await fireHook("CALC-SAFETY-VIOLATION-001", {
            tool_name: toolName,
            action: action2,
    ...
    if (!error && result?.content?.[0]?.text) {
      try {
        ...
        if (parsed) {
          let scoreField = null;
          let scoreValue = null;
          if (toolName === "prism_omega" && action2 === "compute" && parsed.omega !== void 0) {
            scoreField = "omega_score";
            scoreValue = parsed.omega;
          }
          if (toolName === "prism_validate" && action2 === "safety" && parsed.safety_score !== void 0) {
            scoreField = "safety_score";
            scoreValue = parsed.safety_score;
          }
          if (toolName === "prism_safety" && parsed.safety_score !== void 0) {
            scoreField = "safety_score";
            scoreValue = parsed.safety_score;
          }
          if (scoreField && scoreValue !== null) {
            const stateFile = path24.join("C:\\PRISM\\state", "CURRENT_STATE.json");
            if (fs21.existsSync(stateFile)) {
              const state = JSON.parse(fs21.readFileSync(stateFile, "utf-8"));
              ...
              fs21.writeFileSync(stateFile, JSON.stringify(state, null, 2));
            }
          }
        }
      } catch {
    ...
    if (resultBytes > MAX_RESULT_BYTES && result?.content?.[0]?.text) {
      try {
        ...
        const extDir = path24.join("C:\\PRISM\\state", "externalized");
        if (!fs21.existsSync(extDir)) fs21.mkdirSync(extDir, { recursive: true });
        const extFile = path24.join(extDir, `result_${toolName}_${action2}_${Date.now()}.json`);
        fs21.writeFileSync(extFile, fullText);
        ...
        const truncated = {
          ...
          ...parsed.success !== void 0 ? { success: parsed.success } : {},
    ...
    if (!error && !result?.isError) {
      try {
        ...
        if (!hasError) {
          const fpPath = path24.join("C:\\PRISM\\state", "failure_patterns.jsonl");
          if (fs21.existsSync(fpPath)) {
            const lines = fs21.readFileSync(fpPath, "utf-8").trim().split("\n").filter(Boolean);
            ...
            if (match && match.occurrences >= 2) {
              ...
              fs21.writeFileSync(fpPath, patterns.map((p) => JSON.stringify(p)).join("\n") + "\n");
    ...
    if (toolName === "prism_dev" && action2 === "file_write" && args[0]?.params?.content && args[0]?.params?.path) {
      try {
        const fullPath = path24.resolve(args[0].params.path);
    ...
    if (isFileWrite && args[0]?.params?.content && (args[0]?.params?.path || args[0]?.params?.name)) {
      try {
        const docPath = args[0].params.path || path24.join("C:\\PRISM\\mcp-server\\data\\docs", args[0].params.name);
    ...
    if (callNum % 15 === 0) {
      ...
      try {
        const { synergyCrossEngineHealth: synergyCrossEngineHealth2 } = await Promise.resolve().then(() => (init_synergyIntegration(), synergyIntegration_exports));
    ...
    if (callNum > 0 && callNum % 25 === 0) {
      try {
        const { synergyComplianceAudit: synergyComplianceAudit2 } = await Promise.resolve().then(() => (init_synergyIntegration(), synergyIntegration_exports));
    ...
    if (callNum > 0 && callNum % 8 === 0) {
      try {
        const nlResult = autoNLHookEvaluator(callNum, toolName, action);
    ...
    if (callNum >= 41) {
      ...
      try {
        ...
        if (fs21.existsSync(stateFile)) {
          try {
            Object.assign(autoSaveState, JSON.parse(fs21.readFileSync(stateFile, "utf-8")));
          } catch {
          }
        }
        ...
        fs21.writeFileSync(stateFile, JSON.stringify(autoSaveState, null, 2));
        ...
        try {
          ...
          if (fs21.existsSync(atcsDir)) {
            const taskDirs = fs21.readdirSync(atcsDir).filter((d) => {
              try {
                return fs21.statSync(path24.join(atcsDir, d)).isDirectory();
              } catch {
                return false;
              }
            });
            for (const dir of taskDirs) {
              const mfPath = path24.join(atcsDir, dir, "TASK_MANIFEST.json");
              if (fs21.existsSync(mfPath)) {
                try {
                  const mf = JSON.parse(fs21.readFileSync(mfPath, "utf-8"));
                  if (mf.status === "IN_PROGRESS") {
                    ...
                    fs21.writeFileSync(mfPath, JSON.stringify(mf, null, 2));
    ...
    if (cadence.actions.length > 0 && result?.content?.[0]?.text) {
      try {
        ...
        try {
          const cadenceFiresPath = path24.join("C:\\PRISM\\state", "CADENCE_FIRES.json");
          let fires = {};
          if (fs21.existsSync(cadenceFiresPath)) {
            try {
              fires = JSON.parse(fs21.readFileSync(cadenceFiresPath, "utf-8"));
            } catch {
              fires = {};
            }
          }
          ...
          fs21.writeFileSync(cadenceFiresPath, JSON.stringify(fires, null, 2));
        } catch {
        }
        try {
          const survivalPath = path24.join("C:\\PRISM\\state", "COMPACTION_SURVIVAL.json");
          const actionsPath = path24.join("C:\\PRISM\\state", "RECENT_ACTIONS.json");
          const trackerPath = path24.join("C:\\PRISM\\mcp-server\\data\\docs", "ACTION_TRACKER.md");
          let ctx = { call: callNum };
          if (fs21.existsSync(survivalPath)) {
            try {
              const surv = JSON.parse(fs21.readFileSync(survivalPath, "utf-8"));
              const ageMin = Math.round((Date.now() - new Date(surv.captured_at).getTime()) / 6e4);
              if (ageMin < 240) {
                ctx.task = surv.current_task !== "unknown" ? surv.current_task : void 0;
          ...
          if (!ctx.task && fs21.existsSync(trackerPath)) {
            try {
              const tracker = fs21.readFileSync(trackerPath, "utf-8");
          ...
          try {
            const wfPath = path24.join(STATE_DIR12, "WORKFLOW_STATE.json");
            if (fs21.existsSync(wfPath)) {
              const wf = JSON.parse(fs21.readFileSync(wfPath, "utf-8"));
        ...
        if (compactionDetectedThisCall) {
          ...
          try {
            const manifestPath = path24.join("C:\\PRISM\\state", "RECOVERY_MANIFEST.json");
            if (fs21.existsSync(manifestPath)) {
              recoveryManifest = JSON.parse(fs21.readFileSync(manifestPath, "utf-8"));
              const ageMs = Date.now() - new Date(recoveryManifest.captured_at).getTime();
              if (ageMs > 4 * 60 * 60 * 1e3) recoveryManifest = null;
            }
          } catch {
          }
          let survivalInfo = {};
          try {
            const survPath = path24.join("C:\\PRISM\\state", "COMPACTION_SURVIVAL.json");
            if (fs21.existsSync(survPath)) {
              survivalInfo = JSON.parse(fs21.readFileSync(survPath, "utf-8"));
            }
          } catch {
          }
          let recentActionsInfo = {};
          try {
            const raPath = path24.join("C:\\PRISM\\state", "RECENT_ACTIONS.json");
            if (fs21.existsSync(raPath)) {
              recentActionsInfo = JSON.parse(fs21.readFileSync(raPath, "utf-8"));
            }
          } catch {
          }
          let trackerNext = "";
          try {
            ...
            if (fs21.existsSync(tkPath)) {
              const tk = fs21.readFileSync(tkPath, "utf-8");
          ...
          let reasoningTrail = [];
          try {
            const jPath = path24.join(STATE_DIR12, "SESSION_JOURNAL.jsonl");
            if (fs21.existsSync(jPath)) {
              const lines = fs21.readFileSync(jPath, "utf-8").trim().split("\n").filter(Boolean);
          ...
          const hijacked = {
            ...
            _MANDATORY_RECOVERY: {
              ...
              step_1: recoveryManifest ? "READ recovery_manifest below \u2014 it has EVERYTHING you need in one place." : "READ reasoning_trail below FIRST \u2014 these are YOUR OWN NOTES from before compaction happened.",
            ...
            recovery_manifest: recoveryManifest ? {
              ...
              age_seconds: Math.round((Date.now() - new Date(recoveryManifest.captured_at).getTime()) / 1e3)
            } : null,
            reasoning_trail: {
              ...
              recent_calls: reasoningTrail.slice(-8).map((e) => `[${e.call}] ${e.tool}:${e.action}${e.notes ? " \u{1F4DD} " + e.notes : ""}`),
              _hint: notesEntries.length > 0 ? "\u26A1 You left notes for yourself. READ THEM. They contain your reasoning from before compaction." : "No notes found. Check WIP_NOTES.md and /mnt/transcripts/ for context."
            },
            workflow_recovery: (() => {
              try {
                const wfPath = path24.join(STATE_DIR12, "WORKFLOW_STATE.json");
                if (fs21.existsSync(wfPath)) {
                  ...
                  if (wf.status === "active") {
                    const cur = wf.steps[wf.current_step - 1];
                    const done = wf.steps.filter((s) => s.status === "done").map((s) => `${s.name} \u2705`);
                    const remaining = wf.steps.filter((s) => s.status !== "done").map((s) => `Step ${s.id}: ${s.name} \u2014 ${s.intent}`);
                    return {
                      active: true,
                      ...
                      remaining,
                      files_touched: [...new Set(wf.steps.flatMap((s) => s.files_touched || []))]
                    };
                  }
                }
              } catch {
            ...
            recovery_context: {
              ...
              phase: (() => {
                ...
                try {
                  const cs = JSON.parse(fs21.readFileSync(path24.join(STATE_DIR12, "CURRENT_STATE.json"), "utf-8"));
            ...
            recent_tool_calls: (recentActionsInfo.actions || []).slice(-5).map(
              (a) => `${a.tool}:${a.action} (${a.success ? "OK" : "FAIL"}) ${(a.params_summary || "").slice(0, 60)}`
            ),
            transcripts: transcriptHint,
            original_response: parsed,
            // Include the actual tool response too
