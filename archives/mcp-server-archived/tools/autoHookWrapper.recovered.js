...
var DISPATCHER_HOOK_MAP = {
  ...
  prism_orchestrate: { before: "before_spawn", after: "on_complete", category: "AGENT" },
...
var RECENT_ACTIONS_FILE = path17.join("C:\\PRISM\\state", "RECENT_ACTIONS.json");
...
function recordFlightAction(callNum, toolName, action2, params, success, durationMs, result, errorMsg) {
  try {
    let actions = [];
    if (fs15.existsSync(RECENT_ACTIONS_FILE)) {
      try {
        actions = JSON.parse(fs15.readFileSync(RECENT_ACTIONS_FILE, "utf-8")).actions || [];
      } catch {
      }
    }
    let paramsSummary = "{}";
    ...
    fs15.writeFileSync(RECENT_ACTIONS_FILE, JSON.stringify({
...
function wrapWithUniversalHooks(toolName, handler) {
  const wrapped = async function(...args) {
    ...
    if (lastCallTimestamp > 0 && compactionRecoveryCallsRemaining <= 0) {
      ...
      if (gapSeconds > 300 && firstCallReconDone) {
        try {
          ...
          if (rehydrateResult.rehydrated && rehydrateResult.survival_data) {
            ...
            compactionRecoveryCallsRemaining = 3;
    ...
    if (toolName === "prism_dev" && action2 === "build" && !error) {
      try {
        ...
        if (typeof resultText === "string" && resultText.includes("SUCCESS")) {
          ...
          const checklistPath = path17.join("C:\\PRISM\\state", "build_checklist.json");
          fs15.writeFileSync(checklistPath, JSON.stringify({
      ...
      try {
        ...
        if (parsed) {
          ...
          if (scoreField && scoreValue !== null) {
            const stateFile = path17.join("C:\\PRISM\\state", "CURRENT_STATE.json");
            if (fs15.existsSync(stateFile)) {
              const state = JSON.parse(fs15.readFileSync(stateFile, "utf-8"));
              ...
              fs15.writeFileSync(stateFile, JSON.stringify(state, null, 2));
            }
          }
        }
      } catch {
    ...
    if (resultBytes > MAX_RESULT_BYTES && result?.content?.[0]?.text) {
      try {
        ...
        const extDir = path17.join("C:\\PRISM\\state", "externalized");
        if (!fs15.existsSync(extDir)) fs15.mkdirSync(extDir, { recursive: true });
        const extFile = path17.join(extDir, `result_${toolName}_${action2}_${Date.now()}.json`);
        fs15.writeFileSync(extFile, fullText);
    ...
    if (!error && ["prism_calc", "prism_safety", "prism_thread", "prism_toolpath", "prism_atcs"].includes(toolName)) {
      try {
        const skillHint = autoSkillHint(callNum, toolName, action2, args[0]?.params || {});
    ...
    if (callNum - lastCheckpointReminder >= 10) {
      ...
      try {
        const currentTask = cadence.todo?.currentFocus || cadence.todo?.taskName || cadence.todo?.todo_preview || "Active session";
    ...
    if (!error && !result?.isError) {
      try {
        ...
        if (!hasError) {
          const fpPath = path17.join("C:\\PRISM\\state", "failure_patterns.jsonl");
          if (fs15.existsSync(fpPath)) {
            const lines = fs15.readFileSync(fpPath, "utf-8").trim().split("\n").filter(Boolean);
            ...
            if (match && match.occurrences >= 2) {
              ...
              fs15.writeFileSync(fpPath, patterns.map((p) => JSON.stringify(p)).join("\n") + "\n");
    ...
    if (toolName === "prism_dev" && action2 === "file_write" && args[0]?.params?.content && args[0]?.params?.path) {
      try {
        const fullPath = path17.resolve(args[0].params.path);
    ...
    if (callNum >= 19) {
      ...
      try {
        ...
        if (fs15.existsSync(stateFile)) {
          try {
            Object.assign(autoSaveState, JSON.parse(fs15.readFileSync(stateFile, "utf-8")));
          } catch {
          }
        }
        autoSaveState.auto_saved = true;
        autoSaveState.auto_save_reason = "BLACK_ZONE_19+_calls";
        ...
        fs15.writeFileSync(stateFile, JSON.stringify(autoSaveState, null, 2));
        cadence.actions.push("STATE_AUTO_SAVED:BLACK_ZONE");
        try {
          const atcsDir = "C:\\PRISM\\autonomous-tasks";
          if (fs15.existsSync(atcsDir)) {
            const taskDirs = fs15.readdirSync(atcsDir).filter((d) => {
              try {
                return fs15.statSync(path17.join(atcsDir, d)).isDirectory();
              } catch {
                return false;
              }
            });
            for (const dir of taskDirs) {
              const mfPath = path17.join(atcsDir, dir, "TASK_MANIFEST.json");
              if (fs15.existsSync(mfPath)) {
                try {
                  const mf = JSON.parse(fs15.readFileSync(mfPath, "utf-8"));
                  if (mf.status === "IN_PROGRESS") {
                    ...
                    fs15.writeFileSync(mfPath, JSON.stringify(mf, null, 2));
      ...
      cadence.actions.push("\u{1F534} WARNING: 15+ calls \u2014 checkpoint NOW, prepare handoff");
    } else if (callNum >= 9) {
      cadence.actions.push("\u{1F7E1} NOTICE: 9+ calls \u2014 plan checkpoint soon");
    }
      ...
      try {
        ...
        if (recoveryData && compactionRecoveryCallsRemaining > 0) {
          ...
          parsed._COMPACTION_RECOVERY = {
            alert: `\u26A0\uFE0F COMPACTION RECOVERY (${compactionRecoveryCallsRemaining} reminders left) \u2014 You are mid-task.`,
            instruction: `IMMEDIATELY do this: ${nextAction}`,
            context: {
