...
OLD_PATTERNS = [
    r"C:\\C:/PRISM \(UPLOAD TO BOX OCCASSIONALLY\)",
    r"C:/PRISM \(UPLOAD TO BOX OCCASSIONALLY\)",
    r"C:\\\\C:/PRISM \\(UPLOAD TO BOX OCCASSIONALLY\\)",  # Escaped
    r'C:\\C:/PRISM',  # Short form
    r'C:/PRISM',   # Forward slash short
    r"C:/PRISM \(UPLOAD TO BOX OCCASSIONALLY\)",  # Without drive
    r"C:/PRISM",      # Shortest form
]
...
def create_backup(filepath: Path) -> bool:
    ...
    try:
        ...
        print(f"  [WARN] Backup failed for {filepath}: {e}")
        return False
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            if result['status'] == 'updated':
                ...
                print(f"  [OK] Updated: {result['path']} ({result['replacements']} replacements)")
            elif result['status'] == 'error':
                print(f"  [ERR] Error: {result['path']} - {result['error']}")
