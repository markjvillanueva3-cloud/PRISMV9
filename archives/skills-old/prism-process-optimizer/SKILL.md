...
description: |
  ...
  Produces P(x) component for master equation Ω(x) = R×C×P×S×L.
...
TO prism-skill-orchestrator:
  - getSkillActivation(skill, context) → number
  - selectAgents(task, budget) → Agent[]
```
...
|------------------|----------|------|
| All 12 process metrics | ✅ | - |
| 39 skill formulas | ✅ | ⚠️ Need examples |
| 57 agent criteria | ✅ | ⚠️ Need selection algorithm |
| Manufacturing examples | ❌ | GAP |
| Cost optimization | ❌ | GAP |
| Ralph loop integration | ❌ | GAP |
...
## Coverage:
- 12/12 process metrics with formulas ✅
- 39/39 skill activation formulas ✅
- 57/57 agent selection criteria ✅
