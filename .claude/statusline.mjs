...
// ─── Global fail-safe (HARDEN 2026-07-04) ──────────────────────────────────
// A statusLine must NEVER emit nothing — a blank line is the exact failure mode
// that makes the chat-slot UI "disappear". Two guards:
//  1. Fail-safe dynamic import: a broken/missing lib (a STATIC import error aborts
//     the whole module before any fallback can render) degrades to "no compact
//     marker" (-1 -> whole-file count), the pre-existing fallback behavior.
//  2. Process-level uncaught handlers below emit a minimal always-valid line on
//     ANY throw anywhere in the script, so the terminal is never left blank.
let lastCompactMarkerOffset = () => -1;
try { ({ lastCompactMarkerOffset } = await import('../scripts/lib/transcript-token-counter.mjs')); } catch { /* keep the -1 fallback */ }

let __slotForFallback = '?';
let __emitted = false;
function __emitFallback(reason) {
  if (__emitted) return;
  __emitted = true;
  try {
    const s = (__slotForFallback && __slotForFallback !== '?') ? String(__slotForFallback).toUpperCase() : '?';
    process.stdout.write(`\x1b[101m\x1b[1m\x1b[30m ${s} \x1b[0m \x1b[2m⚔ PRISM · statusline degraded (${reason})\x1b[0m`);
  } catch { try { process.stdout.write('⚔ PRISM'); } catch { /* give up quietly */ } }
}
process.on('uncaughtException', () => { __emitFallback('exc'); process.exit(0); });
process.on('unhandledRejection', () => { __emitFallback('rej'); process.exit(0); });
...
for (const [name, data] of Object.entries(slotsObj)) {
  if (data && data.chatId && sid && (data.chatId === sid || sid.includes(data.chatId.replace(/^claude-/, '')))) {
    mySlot = name;
    __slotForFallback = name; // fail-safe line shows the real slot badge even on a later throw
    break;
  }
}
...
// ENHANCE 2026-07-04: derive from NATO index % 6 so ALL 26 slots (alpha..zulu) get a
// deterministic color — the old literal map covered only alpha..lima (mike..zulu all
// fell through to bgRed, visually indistinguishable).
const BG_CYCLE = [C.bgRed, C.bgBlu, C.bgMag, C.bgGrn, C.bgYlw, C.bgCyn];
const NATO_ORDER = ['alpha','bravo','charlie','delta','echo','foxtrot','golf','hotel','india','juliett','kilo','lima','mike','november','oscar','papa','quebec','romeo','sierra','tango','uniform','victor','whiskey','xray','yankee','zulu'];
const __natoIdx = NATO_ORDER.indexOf(mySlot);
const badgeColor = __natoIdx >= 0 ? BG_CYCLE[__natoIdx % 6] : C.bgRed;
...
__emitted = true; // normal render succeeded — suppress the fail-safe handlers
