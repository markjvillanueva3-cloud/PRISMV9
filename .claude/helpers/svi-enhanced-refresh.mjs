...
function readHealthSignals() {
  ...
  let mcp = false;
  try {
    execFileSync("curl", ["-s", "-o", "/dev/null", "-w", "%{http_code}", "--max-time", "3", "http://127.0.0.1:3100"], { windowsHide: true, encoding: "utf8" });
    mcp = true;
  } catch { mcp = false; }
  let ollama = false;
  try {
    const out = execFileSync("curl", ["-s", "-o", "/dev/null", "-w", "%{http_code}", "--max-time", "3", "http://localhost:11434/api/tags"], { windowsHide: true, encoding: "utf8" });
    ollama = out === "200";
  } catch { ollama = false; }
