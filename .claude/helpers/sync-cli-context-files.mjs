...
const MIRRORS = [
  ...
  { src: PROJECT_CLAUDE, dst: "H:/PRISM/AGENTS.md", target: "Codex CLI" },
  { src: GLOBAL_CLAUDE,  dst: `${HOME}/.gemini/GEMINI.md`, target: "Gemini CLI (global)" },
  { src: GLOBAL_CLAUDE,  dst: `${HOME}/.codex/AGENTS.md`,  target: "Codex CLI (global)" },
...
async function syncOne({ src, dst, target }) {
  if (!existsSync(src)) {
  ...
  const mirrorContent = header + sourceContent;

  if (existsSync(dst)) {
    const existing = await readFile(dst, "utf-8");
    if (stripExistingHeader(existing) === sourceContent) {
