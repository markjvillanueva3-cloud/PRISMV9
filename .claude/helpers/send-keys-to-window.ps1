...
.DESCRIPTION
  ...
  the operator to opt-in per call via the -Confirm flag. The orchestrator
  main loop sets -Confirm:$true based on per-slot env / config (each slot
  must explicitly allow being sent-keys-into).
...
.PARAMETER Confirm
  Required boolean. False (default) is dry-run — script reports what it
  WOULD type but does not synthesize keystrokes. True actually types.
  Operator gate: the orchestrator must pass -Confirm:$true based on the
  target slot's opt-in config.
...
.EXAMPLE
  # Actually send /clear into the window.
  powershell.exe -File send-keys-to-window.ps1 -Hwnd 0x12345 -Text "/clear" -Confirm:$true
