...
export function detectUncommittedShared(deps = {}) {
  const runGit = deps.runGit || ((args) => execFileSync("git", args, { windowsHide: true, cwd: PRISM_ROOT, encoding: "utf8" }));
  try {
...
export function gitWorktreeAdd({ slot, worktreeDir, baseRef, deps = {} }) {
  const runGit = deps.runGit || ((args) => execFileSync("git", args, { windowsHide: true, cwd: PRISM_ROOT, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }));
