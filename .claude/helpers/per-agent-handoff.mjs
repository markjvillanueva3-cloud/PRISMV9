...
// SESSION-CONTINUITY-FIX/U-HANDOFF-READ-SLOT-AWARE (2026-06-18, slot:alpha):
...
/**
 * SESSION-CONTINUITY-FIX/U-HANDOFF-READ-SLOT-AWARE (2026-06-18, slot:alpha):
 * which slot does `instance` CURRENTLY own? Reads chat-slots.json and returns
 * the canonical slot name whose `chatId` equals `instance`, else null.
 * Fail-soft: a missing/corrupt chat-slots file -> null (the caller's resolution
 * then falls through to the slot-blind tiers unchanged).
 */
function slotOwnedByInstance(instance) {
  if (!instance) return null;
  try {
    const doc = JSON.parse(fs.readFileSync(chatSlotsPath(), "utf-8"));
    const slots = doc && doc.slots ? doc.slots : {};
    for (const [name, st] of Object.entries(slots)) {
      if (st && typeof st === "object" && st.chatId === instance && SLOT_NAMES.has(name)) {
        return name;
      }
    }
  } catch { /* fail-soft -- no ground truth -> no slot preference */ }
  return null;
}

/**
 * SESSION-CONTINUITY-FIX/U-HANDOFF-READ-SLOT-AWARE (2026-06-18, slot:alpha):
 * among THIS chat's own handoffs (`sameInstanceFiles`, pre-sorted newest-first),
 * return the newest one BOUND to `slot` -- by a `<slot>`/`<slot>-` topic prefix
 * in the filename (cheap; no read to decide) OR the durable `slot:` frontmatter
 * field (authoritative; covers non-slot-prefixed topics). Returns
 * {file, path, mtime, content} or null. Used to prefer the current-slot handoff
 * over the global newest when a terminal carries handoffs under multiple slots.
 */
function newestSameInstanceForSlot(sameInstanceFiles, instance, slot) {
  if (!slot || !Array.isArray(sameInstanceFiles)) return null;
  const base = `HANDOFF-${sanitizeFilename(instance)}-`;
  // Pass 1 -- filename topic-prefix (decide the match without reading content).
  for (const f of sameInstanceFiles) {
    if (!f.file.startsWith(base) || !f.file.endsWith(".md")) continue;
    const topic = f.file.slice(base.length, -3); // strip the trailing ".md"
    if (topic === slot || topic.startsWith(`${slot}-`)) {
      try { return { ...f, content: fs.readFileSync(f.path, "utf-8") }; } catch { /* skip unreadable */ }
    }
  }
  // Pass 2 -- durable `slot:` frontmatter (handoffs whose topic is not slot-prefixed).
  for (const f of sameInstanceFiles) {
    try {
      const content = fs.readFileSync(f.path, "utf-8");
      if (handoffSlot(content) === slot) return { ...f, content };
...
function cmdRead(identity, args) {
  ...
  // Same-instance handoff list (newest-first) -- shared by tiers (0.4) + (0.5).
  ...
  // (0.4) Same-instance CURRENT-SLOT preference (SESSION-CONTINUITY-FIX/
  //       U-HANDOFF-READ-SLOT-AWARE, 2026-06-18, slot:alpha). When this chat
  //       CURRENTLY owns a slot in chat-slots.json, prefer the newest of ITS
  //       OWN handoffs bound to THAT slot over the global newest (0.5). A
  //       terminal that churned papa->alpha accumulates handoffs under BOTH
  //       slots; slot-blind same-instance-newest then resumes
  //       /startup-<wrong-slot> ("keep checking back into papa"). Fail-soft:
  //       no owned slot / no slot-matching handoff -> falls through to (0.5)
  //       unchanged (so a chat with a single slot's handoffs is byte-identical).
  const ownedSlot = slotOwnedByInstance(targetInstance);
  if (ownedSlot) {
    const slotPick = newestSameInstanceForSlot(sameInstanceFiles, targetInstance, ownedSlot);
    if (slotPick) {
      const ageMin = Math.round((Date.now() - slotPick.mtime) / 60000);
      return { ok: true, content: slotPick.content, file: slotPick.file, matchedBy: "same-instance-current-slot", slot: ownedSlot, age_minutes: ageMin };
    }
  }
