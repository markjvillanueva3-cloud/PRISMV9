...
// LOOP CAP REMOVED FLEET-WIDE (operator 2026-06-16: "remove the iteration cap permanently for
// all galaxies; update the system and settings to fully accommodate"). Loops are now UNBOUNDED by
// default -- they run until explicitly `end`ed or a genuine spiral, NOT until an arbitrary iteration
// count. An explicit `--target N` still opts into a bound (and keeps the 2x-target runaway-abandon
// backstop at line ~192, which only fires for an explicitly-bounded loop). "Unbounded" is expressed
// as an arithmetic-safe huge default (so budget/abandon math never overflows), not 0/null. Knob:
// PRISM_LOOP_DEFAULT_TARGET overrides the default if a finite bound is ever wanted fleet-wide.
const DEFAULT_TARGET = Number(process.env.PRISM_LOOP_DEFAULT_TARGET) || 1_000_000_000;
...
// U-LOOP-SPIRAL-GATE (slot:bravo, 2026-06-18, harness/loop/cron upgrade #4): a WITHIN-UNIT spiral
// signal keyed on consecutive eval FAILURES (NOT iteration count). The operator removed the iteration
// COUNT cap fleet-wide (DEFAULT_TARGET=1e9 above) but EXPLICITLY preserved spiral-stops ("won't stop
// on a count, never uninterruptible; STOPS on a genuine spiral"). Today the STUCK detector only
// catches NO-PROGRESS (iter not advancing) and the planning-loop fail-stop only fires at `next`/roll
// -- so a tick stream that ADVANCES iter every tick but FAILS every eval has no within-unit spiral
// signal. This closes that gap. Keyed on FAILURE so a HEALTHY long loop (passing evals,
// consecutiveFails 0) NEVER trips it -- it cannot re-impose the operator-removed count cap. Knobs:
// PRISM_LOOP_SPIRAL_WARN_FAILS (default 8, advisory), PRISM_LOOP_SPIRAL_FAILS (default 25; 0 disables
// the halt level, WARN still emits).
const SPIRAL_WARN_FAILS = (() => { const n = Number(process.env.PRISM_LOOP_SPIRAL_WARN_FAILS); return Number.isFinite(n) && n >= 0 ? n : 8; })();
const SPIRAL_CRIT_FAILS = (() => { const n = Number(process.env.PRISM_LOOP_SPIRAL_FAILS); return Number.isFinite(n) && n >= 0 ? n : 25; })();

/** PURE: classify within-unit spiral risk from a CONSECUTIVE-FAIL count -> "none" | "warn" |
 *  "critical". crit/warn=0 disables that level; critical takes precedence. Keyed on FAILURE, never on
 *  iteration count (the operator removed the count cap) -- a healthy passing loop has consecutiveFails
 *  0 and never trips. Non-finite input -> "none" (fail-soft). */
export function classifySpiralRisk(consecutiveFails, { warn = SPIRAL_WARN_FAILS, crit = SPIRAL_CRIT_FAILS } = {}) {
  const cf = Number.isFinite(consecutiveFails) ? consecutiveFails : 0;
  if (crit > 0 && cf >= crit) return "critical";
  if (warn > 0 && cf >= warn) return "warn";
  return "none";
}
...
function cmdStart(flags) {
  ...
  const state = {
    ...
    target: Number(flags.target) || DEFAULT_TARGET,
...
function cmdTick(flags) {
  ...
  // U-LOOP-SPIRAL-GATE (upgrade #4): within-unit failing-streak spiral signal. consecutiveFails
  // reuses deriveLoopSignals' SAME fail definition (R8 -- no forked fail rule). WARN is advisory;
  // CRITICAL flips status to "spiral" so a failing runaway self-halts -- the force-loop Stop hook
  // RELEASES on status "spiral" (stop-force-loop-continue.mjs::spiralReleaseReason) instead of forcing
  // continuation; the recorded spiralReason + WARN carry the why. Keyed on FAILURE, never iteration
  // count -- it cannot re-impose the operator-
  // removed count cap, and a healthy passing loop (consecutiveFails 0) never trips it. Runs AFTER the
  // explicit-target abandon above so that opt-in finite bound still wins; only flips a still-running loop.
  const spiralSig = deriveLoopSignals(state);
  state.consecutiveFails = spiralSig.consecutiveFails;
  state.spiralRisk = classifySpiralRisk(spiralSig.consecutiveFails);
  if (state.spiralRisk === "critical" && state.status === "running") {
    state.status = "spiral";
    state.spiralReason = `${spiralSig.consecutiveFails} consecutive failing iterations (>= PRISM_LOOP_SPIRAL_FAILS=${SPIRAL_CRIT_FAILS}) -- within-unit spiral backstop`;
  }
  if (state.spiralRisk !== "none") {
    process.stderr.write(`[loop-state] SPIRAL ${state.spiralRisk}: ${spiralSig.consecutiveFails} consecutive failing iterations on "${state.task}"\n`);
  }
  write(flags.session, state);
  process.stdout.write(JSON.stringify({ ok: true, iter: state.iter, target: state.target, status: state.status, evalScore, spiralRisk: state.spiralRisk, consecutiveFails: state.consecutiveFails }) + "\n");
}
...
const HANDOFF_OWN_MATCH = new Set([
  ...
  // U-HANDOFF-READ-SLOT-AWARE (2026-06-18, slot:alpha): the slot-aware read tier
  // in per-agent-handoff.mjs returns this when it prefers the chat's CURRENT-SLOT
  // handoff over the global newest -- it ONLY ever picks this instance's own
  // HANDOFF-<instance>-* files (same family as same-instance-newest), so it is an
  // own-instance match. Omitting it silently dropped /loop auto-resume for any
  // terminal that churned slots (the exact case the read fix targets).
  "same-instance-current-slot",
]);
...
function handoffResume(terminal) {
  ...
  try {
    const r = spawnSync(
      ...
      { windowsHide: true, encoding: "utf-8", timeout: 15000, cwd: REPO },
    );
...
/** Build the pick-unit.mjs argv. Exported PURE for test (R9): `--all-lanes` is
 *  emitted IFF opts.allLanes, and the real slot/chatId are passed through.
 *  --all-lanes requests pick-unit's TRUE full-roadmap pool for the fleet-fallback;
 *  the old fallback passed a bare empty --slot, which pick-unit silently defaulted
 *  to ALPHA's lane (never the full roadmap) -- see
 *  reference_loop_fallback_live_peer_poach_risk_2026_06_18. */
export function buildPickUnitArgs(repo, slot, chatId, opts = {}) {
  ...
  if (opts.allLanes) args.push("--all-lanes");
  return args;
}

function pickUnitOnce(slot, chatId, opts = {}) {
  if (process.env.PRISM_LOOP_NEXT_NO_PICKUNIT === "1") return "";
  try {
    const args = buildPickUnitArgs(REPO, slot, chatId, opts);
    const r = spawnSync(process.execPath, args, { windowsHide: true, encoding: "utf-8", timeout: 30000, cwd: REPO });
    if (r.status !== 0 || !r.stdout) return "";
...
function pickUnitTop(slot, chatId) {
  ...
  // Own lane empty -> fleet-wide fallback over a TRUE all-lanes pool (--all-lanes).
  // FAIL-CLOSED: only fall back when we have a chatId, so pick-unit's PER-SLOT-CLAIM
  // filter can exclude units a peer slot has formally claimed -- auto-rolling onto
  // peer work is a wrong-work hazard (scrutiny P1). Pass the REAL slot (not "") so
  // the claim filter self-detects correctly; --all-lanes makes the pool the whole
  // roadmap instead of the old empty-slot default that silently served ALPHA's lane.
  // FOLLOW-UP (reference_loop_fallback_live_peer_poach_risk_2026_06_18): the
  // 9-vs-17 any-domain gating + a peer-LIVENESS filter (exclude units a LIVE peer is
  // informally working) + SLOT_TO_CHAT 26-slot coverage remain a scoped milestone.
  if (slot && chatId) {
    const fleet = pickUnitOnce(slot, chatId, { allLanes: true });
...
// Roll-chain cap. A roll RESETS per-unit iter to 0, so the cmdTick `iter > 2x target`
// guard cannot bound TOTAL units advanced -- maxRolls is the only count-based bound on
// how many units a /loop auto-advances through.
//
// UNBOUNDED FLEET-WIDE (operator directive 2026-06-17: "remove the iteration cap
// permanently for all galaxies ... run autonomously all night"). The earlier default
// (8) handed back for a human checkpoint after 8 units -- the operator has now EXPLICITLY
// opted into auto-advance-forever for every galaxy's /loop, so the default is unbounded
// (1e9, arithmetic-safe -- not 0/Infinity, so rollsTotal>=maxRolls math never misbehaves).
// SAFETY IS PRESERVED, not removed: a /loop still STOPS on a genuine SPIRAL, never on a
// healthy count (R6 -- context/iteration growth is NOT a stop signal, a spiral IS):
//   - decidePlanningAction / MAX_REPLANS demote-to-stop on a persistent FAIL streak,
//   - the force-loop hook's no-progress STUCK detector releases a wedged loop,
//   - its TOKEN_CEILING releases near context exhaustion for compaction,
//   - per-unit `iter > 2x target` runaway abandon still fires on an explicit finite target.
// So "unbounded" means "won't stop on a count," NEVER "uninterruptible."
// Re-impose a finite bound fleet-wide via PRISM_LOOP_MAX_ROLLS (read in maxRolls() below).
const DEFAULT_MAX_ROLLS = 1_000_000_000;
function maxRolls() {
...
function cmdNext(flags) {
  ...
  // Roll inherits the explicit flag, else the prior loop's target, else the UNBOUNDED
  // default (was a stale hardcoded 20 -- a residual cap that would re-bound a rolled
  // loop to 20 iters if prev.target were ever unset; aligned to DEFAULT_TARGET per the
  // 2026-06-17 unbound-all-galaxies directive).
  const target = Number(flags.target) || prev?.target || DEFAULT_TARGET;
  const state = {
    schemaVersion: "1.0.0",
    sessionId: flags.session,
    task: nextTask,
...
/**
 * Read ALL fleet loop-state files -> { count, loops[] } sorted freshest-first.
 * The DATA half of cmdList, exported so dispatchers/hooks/scripts can query loop state
 * programmatically (U-LOOP-STATE-READ-API, slot:bravo 2026-06-14 -- the proven .mjs
 * foundation that a future prism_session:loop_state_query action wraps; the CLI `list`
 * delegates here so there is ONE read path). Fail-soft: a missing dir -> empty; a corrupt
 * file is skipped. `now` is injectable so staleMs is deterministic in tests.
 */
export function readFleetLoops({ dir = STATE_DIR, now = Date.now() } = {}) {
  let files = [];
  try { files = fs.readdirSync(dir).filter((f) => f.startsWith("loop-") && f.endsWith(".json")); }
  catch { return { count: 0, loops: [] }; }
  const out = [];
  for (const f of files) {
    try {
      ...
      out.push({
        ...
        staleMs: now - new Date(s.lastTickAt).getTime(),
      });
    } catch { /* skip corrupt */ }
  }
  out.sort((a, b) => a.staleMs - b.staleMs);
  return { count: out.length, loops: out };
}

function cmdList() {
  ensureDir();
  const r = readFleetLoops();
