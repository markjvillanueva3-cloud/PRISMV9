...
$Script = (Resolve-Path -Path (Join-Path $PSScriptRoot '..' '..' 'scripts' 'slot-worktree-migration-status.mjs')).Path
$NodePath = $null
foreach ($candidate in @(
    (Join-Path 'H:\Tools\nodejs' 'node.exe'),
    (Join-Path $env:USERPROFILE '.claude' 'bin' 'portable-node' 'node.exe'),
