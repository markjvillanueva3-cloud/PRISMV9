...
import { deriveLoopSignals, attemptAtcsReplan, classifySpiralRisk } from "./loop-state.mjs";
...
// ---- classifySpiralRisk (pure): within-unit failing-streak spiral signal (upgrade #4) ----
// Keyed on CONSECUTIVE FAILURES, never iteration count -- so it can never re-impose the operator-
// removed count cap. R9: real thresholds, fails if the level boundaries or the disable knobs regress.
test("classifySpiralRisk: <warn -> none; >=warn -> warn; >=crit -> critical (critical wins)", () => {
  assert.equal(classifySpiralRisk(0, { warn: 3, crit: 6 }), "none");
  assert.equal(classifySpiralRisk(2, { warn: 3, crit: 6 }), "none");
  assert.equal(classifySpiralRisk(3, { warn: 3, crit: 6 }), "warn");
  assert.equal(classifySpiralRisk(5, { warn: 3, crit: 6 }), "warn");
  assert.equal(classifySpiralRisk(6, { warn: 3, crit: 6 }), "critical");
  assert.equal(classifySpiralRisk(99, { warn: 3, crit: 6 }), "critical");
});
test("classifySpiralRisk: crit=0 disables the HALT level (warn still classifies)", () => {
  assert.equal(classifySpiralRisk(100, { warn: 5, crit: 0 }), "warn");
  assert.equal(classifySpiralRisk(4, { warn: 5, crit: 0 }), "none");
});
test("classifySpiralRisk: warn=0 + crit=0 -> always none (fully disabled knob)", () => {
  assert.equal(classifySpiralRisk(1000, { warn: 0, crit: 0 }), "none");
});
test("classifySpiralRisk: non-finite consecutiveFails -> none (fail-soft, never a phantom spiral)", () => {
  assert.equal(classifySpiralRisk(NaN, { warn: 3, crit: 6 }), "none");
  assert.equal(classifySpiralRisk(undefined, { warn: 3, crit: 6 }), "none");
});

// ---- CLI round-trip: cmdTick within-unit spiral backstop (the operator-endorsed spiral-stop) ----
test("CLI: N consecutive fail ticks (>= PRISM_LOOP_SPIRAL_FAILS) flip status to 'spiral'", () => {
  cleanup();
  const env = { PRISM_LOOP_SPIRAL_FAILS: "3", PRISM_LOOP_SPIRAL_WARN_FAILS: "2" };
  try {
    runCli(["start", "--session", TEST_SID, "--task", "RGS-X-MS0/U1: x", "--target", "100"], env);
    let t = runCli(["tick", "--session", TEST_SID, "--status", "fail"], env);
    assert.equal(t.spiralRisk, "none", "1 fail < warn(2) -> none");
    t = runCli(["tick", "--session", TEST_SID, "--status", "fail"], env);
    assert.equal(t.spiralRisk, "warn", "2 fails == warn -> advisory warn");
    assert.equal(t.status, "running", "warn does NOT halt -- advisory only");
    t = runCli(["tick", "--session", TEST_SID, "--status", "fail"], env);
    assert.equal(t.spiralRisk, "critical", "3 fails == crit -> critical");
    assert.equal(t.status, "spiral", "critical fail-streak flips status to spiral (within-unit halt)");
    assert.equal(t.consecutiveFails, 3);
    const after = runCli(["read", "--session", TEST_SID], env);
    assert.equal(after.status, "spiral");
    assert.match(after.spiralReason, /consecutive failing iterations/);
  } finally { cleanup(); }
});
test("CLI: a PASS tick resets the fail streak -> spiralRisk none, never halts (healthy loop, R6)", () => {
  cleanup();
  const env = { PRISM_LOOP_SPIRAL_FAILS: "5", PRISM_LOOP_SPIRAL_WARN_FAILS: "2" };
  try {
    runCli(["start", "--session", TEST_SID, "--task", "RGS-X-MS0/U1: x", "--target", "100"], env);
    runCli(["tick", "--session", TEST_SID, "--status", "fail"], env);
    let t = runCli(["tick", "--session", TEST_SID, "--status", "fail"], env);
    assert.equal(t.spiralRisk, "warn");
    t = runCli(["tick", "--session", TEST_SID, "--status", "ok"], env); // a real pass
    assert.equal(t.consecutiveFails, 0, "a pass resets the consecutive-fail run");
    assert.equal(t.spiralRisk, "none", "healthy progress clears the spiral signal");
    assert.equal(t.status, "running", "never halted -- iteration growth alone is not a spiral");
  } finally { cleanup(); }
});
