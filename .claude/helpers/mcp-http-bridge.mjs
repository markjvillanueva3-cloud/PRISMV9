 ...
 *   3. INIT BUDGET  - the `initialize` handshake gets a long retry budget
 *      (PRISM_MCP_INIT_RETRY_MS, default 60s) so it survives the server's
 *      ~30s cold start. A failed `initialize` is what makes a chat drop the
 *      `prism` server for its whole session - this is the bug this closes.
 *
 * Usage:
...
// Retry budgets: initialize gets long enough to outlast a cold start.
const INIT_RETRY_BUDGET_MS = parseInt(process.env.PRISM_MCP_INIT_RETRY_MS || "60000", 10);
const REQUEST_RETRY_BUDGET_MS = parseInt(process.env.PRISM_MCP_REQUEST_RETRY_MS || "15000", 10);
...
// matches INIT_RETRY_BUDGET_MS for symmetry. Knob: PRISM_MCP_READY_BUDGET_MS.
// PRISM_MCP_WAIT_FOR_READY=0 disables (fail-open if /ready route is missing).
const READY_BUDGET_MS = parseInt(process.env.PRISM_MCP_READY_BUDGET_MS || "60000", 10);
...
async function main() {
  ...
  // U-MCPR01 (alpha 2026-05-28): block until /ready 200 so the first MCP
  // initialize doesn't hit a lazy-load crash on a not-yet-fully-warmed server.
  // Fail-open after 60s so a missing /ready route (old server) doesn't wedge
  // bridges fleet-wide.
  await waitForReady();
