...
# Prefer the canonical PRISM portable node FIRST: a scheduled task (esp. -AsSystem / logged-off) may have a
# different PATH than the interactive installer shell, so a bare `node` resolved at install time can be a
# binary the TASK cannot see -> silent HRESULT launch-failure (the fleet-task-health class). The portable
# node is the absolute path every PRISM hook already uses, present regardless of principal PATH.
$Node = $null
