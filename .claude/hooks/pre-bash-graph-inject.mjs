 ...
 *   PRISM_PRE_BASH_INLINE_CARD_MIN_SCORE=N    -- GAP-A high-conf threshold (default 10; 0=disable)
 */
...
import { exactMatchHit, navPathLine, renderTopCardBlock } from "../../scripts/lib/graph-exact-match.mjs";
...
// 10 is a confident floor. Knob PRISM_PRE_BASH_INLINE_CARD_MIN_SCORE (0=disable).
 ...
 *        When omitted, reads PRISM_PRE_BASH_INLINE_CARD_MIN_SCORE env or uses 10.
 * @returns {string|null}
 */
export function renderInject(keys, hits, resolve, seekDocs, inlineCardMinScore) {
  ...
  // GAP-A: the top non-exact hit is high-confidence (score >= threshold > 0) ->
  // inject its node CARD inline so the model needs zero follow-up node-card/Read.
  const topHit = hits[0];
  const topScore = (topHit && typeof topHit.score === "number") ? topHit.score : 0;
  const threshold = (typeof inlineCardMinScore === "number")
    ? inlineCardMinScore
    : (() => {
        const env = Number(process.env.PRISM_PRE_BASH_INLINE_CARD_MIN_SCORE);
