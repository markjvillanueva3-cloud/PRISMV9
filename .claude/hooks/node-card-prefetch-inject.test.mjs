...
test("BUDGET INVARIANT: node in the full sidecar but NO offset index -> null (hook never bulk-parses)", () => {
  ...
  try {
    ...
    assert.equal(
      buildPrefetchContext("explain eng.mill", { paths }), null,
      "node exists in the full sidecar but the hook stays seek-only -> no injection",
    );
  } finally {
    fs.rmSync(d, { recursive: true, force: true });
  }
