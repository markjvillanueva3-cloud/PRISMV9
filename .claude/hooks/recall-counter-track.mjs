...
function emitContinue(msg) {
  ...
  if (msg) out.hookSpecificOutput = { hookEventName: "PostToolUse", additionalContext: `recall-counter: ${msg}` };
...
function main() {
  ...
  if (!derived) return emitContinue("could-not-derive-key");
  ...
  try {
    ...
    emitContinue(`+1 ${derived.key} (count=${updated.count})`);
  } catch (e) {
