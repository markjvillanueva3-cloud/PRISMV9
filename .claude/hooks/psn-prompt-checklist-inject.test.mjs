...
describe("shouldInject — gating heuristic", () => {
  it("FIRES on a normal substantive prompt", () => {
    assert.equal(shouldInject("can you update the system-viz skill to surface PSN legs"), true);
  });
  ...
  it("HONORS custom opts.minLen", () => {
    assert.equal(shouldInject("short!", { minLen: 5 }), true); // 6 > 5
    assert.equal(shouldInject("longer prompt!", { minLen: 100 }), false); // < 100
  });
  ...
  it("FIRES on a slash-command WITH work-order args", () => {
    assert.equal(shouldInject("/checkin-golf /loop [5m] /goal build the auto-invoke hook"), true);
    assert.equal(shouldInject("/loop 5m /goal high-roi-build"), true);
  });

  it("BOUNDARY: exact minLen passes the threshold check", () => {
    // length 15, default minLen 15 — passes (the rule is `< minLen` skips)
    assert.equal(shouldInject("123456789012345"), true);
  });
});
