...
test("runCheck: server UP + 0 bridges -> OVERRIDE silent with the /mcp directive", async () => {
  const r = await runCheck({
    probeFn: async () => ({ ok: true, status: 200, latencyMs: 5 }),
...
test("runCheck: sentinel no-signal -> FALLS BACK to golf countBridges (0 -> degraded)", async () => {
  const r = await runCheck({
    env: {},
  ...
  assert.match(r.hookSpecificOutput.additionalContext, /MCP BRIDGE DOWN/); // golf's degraded banner
});
