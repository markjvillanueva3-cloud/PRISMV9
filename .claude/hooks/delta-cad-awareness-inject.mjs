...
export function buildContext() {
  return [
    ...
    "FUSION LIVE PORT (OPERATOR-AUTHORITATIVE 2026-06-02 -- [[reference_fusion_port_assignment_kilo_18361_2026_06_02]]): delta = :18362 (CAD) · kilo = :18361 (CAM). NEVER assume :18360 is delta's port -- :18360 is ONLY the generic Fusion360LiveBridgeEngine default; delta's DESIGNATED window is :18362. Point the bridge at delta via PRISM_FUSION_DELTA_PORT=18362 (or new Fusion360LiveBridgeEngine('http://127.0.0.1:18362')). Probe/reload delta's add-in on :18362; if /extrude returns a CONSTANT Z regardless of distance_mm the add-in is stale-in-memory -> operator Stop then Run reload (GUI-only).",
...
async function main(injected) {
  ...
  // TASK-AWARE firing (CAD-APPROACH-KNOWLEDGE, slot:zulu 2026-06-29): fire the SPECIFIC
  // verified gates (delta's 5 cited KNOWN FAILURES + collision SAFETY) for the CAD op(s)
  // named in the prompt. GUARDED DYNAMIC import so a failure in the optional lib can NEVER
  // kill the static CAD block above (the arm-C P1 lesson). Fires only when an op is
  // detected -> ~0 added tokens otherwise.
  let taskBlock = "";
  try {
    const { detectOperations, fireForApproach } = await import("../../scripts/lib/cad-approach-knowledge.mjs");
    const ops = detectOperations(prompt);
    if (ops.length) {
      const seats = (prompt.match(/\b(fusion|hypermill|hypercad|mastercam|freecad|cadquery)\b/gi) || []);
      const fired = fireForApproach({ operations: ops, seats });
      const lines = [`\n\n### 🎯 CAD approach firing -- ${ops.join(", ")}${fired.seats.length ? ` (seat: ${fired.seats.join("/")})` : ""}`];
  ...
  } catch { /* fail-soft: static CAD block still ships even if the task-lib breaks */ }
  approve(buildOutput(dedupedContext("delta-cad-awareness", buildContext() + taskBlock, sid)));
}
