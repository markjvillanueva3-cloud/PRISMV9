...
import { seekCard, offsetIndexStatus } from "../../scripts/lib/node-card-read.mjs";
...
export const TRIGGER_PREFIXES = Object.freeze([
  ...
  "memory_feedback", "memory_reference", "memory_patterns", "tribal-tip", "ms-envelope",
]);
...
export function buildPrefetchContext(prompt, opts = {}) {
  ...
  if (!cards.length) {
    // ids were named but NONE seeked. Distinguish a DEGRADED offset index (absent/stale --
    // a real substrate problem) from genuine non-node ids (a normal miss). seekCard alone
    // cannot tell these apart (both -> null); offsetIndexStatus surfaces the distinction
    // _seekCard already computes (miss vs unavailable). R12: fail-loud on the degradation
    // (a one-line advisory instead of silent zero-injection), silent on the normal miss.
    let st = null;
    try { st = offsetIndexStatus({ paths: opts.paths }); } catch { return null; }
    if (st && (!st.available || st.stale)) {
      const why = st.available ? `stale (${st.staleReason})` : "absent";
      return "## Node card(s) -- offset index unavailable\n" +
        "_Your prompt named system-viz node id(s) but the offset index is " + why +
        ", so no card could be prefetched. Refresh it: `node scripts/build-card-offset-index.mjs`. " +
        "CLI fallback: `node scripts/system-viz-query.mjs node-card <id>`. " +
        "Disable: PRISM_NODECARD_PREFETCH_DISABLE=1._";
    }
    return null;
  }
  const header =
