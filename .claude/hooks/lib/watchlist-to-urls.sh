...
# Use portable Python wrapper (tries H: drive first, then falls back to C: drive)
PYTHON="H:/.claude/bin/python.cmd"
YTDLP="$PYTHON -m yt_dlp"
CATEGORY="${1:?Usage: watchlist-to-urls.sh <search-terms-file|category-keyword>}"
MAX_PER="${2:-1}"
WATCHLIST="H:/prism/mcp-server/data/docs/VIDEO_WATCHLIST.md"
OUT_DIR="H:/prism/mcp-server/data/video-learned/url-lists"
