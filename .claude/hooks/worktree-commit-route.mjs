...
// Explicit cwd override via `git -C <path>` or `cd <path> && git commit`
// is a deliberate user choice to commit to a specific worktree. The
// routing logic below uses process.cwd() (which Claude Code reports as
// the SESSION cwd, not the redirected target) so without this early-out
// the hook false-positive blocks legit cross-worktree commits.
// Same session-cwd bug as commit-ownership-guard.mjs (fixed 2026-05-07).
if (/\bgit\s+-C\s+\S+/.test(cmd)) exit(0);
if (/\bcd\s+\S+\s*&&[^&]*\bgit\s+commit\b/.test(cmd)) exit(0);
...
const CROSS_CUTTING_SCOPES = [
  ...
  /^\s*\[\s*MERGE-STAGING\s*\]/i,  // WORKTREE-CONSOLIDATE-MS0: cherry-pick consolidation lane
];
