...
/**
 * Build the PreCompact verdict object. ALWAYS {continue:true} -- never blocks
 * /compact. An informational message goes in the top-level `systemMessage`
 * field: PreCompact has NO `hookSpecificOutput` contract (that shape is valid
 * only for PreToolUse/UserPromptSubmit/PostToolUse/PostToolBatch/Stop), so the
 * earlier `hookSpecificOutput.additionalContext` shape was rejected by the
 * harness ("Invalid input") on every /compact, silently dropping the message.
 * Sibling PreCompact hooks (claude-brief-precompact, precompact-handoff) use
 * `systemMessage` for exactly this. Pure -- no IO, so the contract is testable.
 */
export function buildVerdict(additionalContext) {
  return additionalContext
    ? { continue: true, systemMessage: additionalContext }
    : { continue: true };
}

/** Emit the verdict to stdout. ALWAYS {continue:true} -- never blocks /compact. */
function emitContinue(additionalContext) {
  try { process.stdout.write(JSON.stringify(buildVerdict(additionalContext))); } catch { /* stdout gone */ }
}
