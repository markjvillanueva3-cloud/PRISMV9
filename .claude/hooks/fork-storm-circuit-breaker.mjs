...
// Live system bash.exe count, TTL-cached so a burst of gated calls pays the enumeration ONCE.
// Returns -1 (=> fail-open) if it cannot be determined.
export function liveBashCount({ ttlMs = DEFAULT_TTL_MS, now = Date.now() } = {}) {
  try {
    if (existsSync(CACHE)) {
      const c = JSON.parse(readFileSync(CACHE, "utf8"));
      if (c && typeof c.count === "number" && (now - c.ts) <= ttlMs) return c.count;
    }
  ...
  try {
    ...
    // "tasklist" throw ENOENT -> -1 -> fail-open (the breaker would then SILENTLY never fire). The
    // Claude Code hook runner has the full PATH, but defense-in-depth: never trust PATH for the
    ...
    const count = (out.match(/bash\.exe/gi) || []).length;  // "INFO: No tasks running..." -> 0
...
function main() {
  ...
  const bashCount = liveBashCount({ ttlMs });
