...
//   3. the actual bash cwd the tool runs in (PreToolUse input.cwd)
//   4. fallback: H:/prism (only when input.cwd is absent - fail-soft)
function resolveGitCwd(cmd, input) {
  ...
  if (input && typeof input.cwd === "string" && input.cwd.trim()) return input.cwd.trim();
...
function resolveSessionId(input) {
  ...
  try {
    const r = spawnSync(process.execPath, ["H:/prism/.claude/helpers/stable-session-id.mjs"], { windowsHide: true,
      encoding: "utf8",
...
function liveBranch(cwd) {
  try {
    const r = spawnSync("git", ["-C", cwd, "rev-parse", "--abbrev-ref", "HEAD"], { windowsHide: true,
      encoding: "utf8",
      timeout: SPAWN_TIMEOUT_MS,
    });
...
async function main() {
  ...
  const gitCwd = resolveGitCwd(cmd, input);
