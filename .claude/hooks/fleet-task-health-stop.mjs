...
export function buildAdvisory(row, nowMs) {
  ...
  // U-GOLF-HEAL-VERIFY-LEG: an ineffective heal (a task re-enabled by the G10
  // guard but still NOT running past its grace window) is the honest counter to
  // the "[auto-re-enabled N -- verify next audit]" note below -- the verify ran
  // and these N did not take. Computed here so it is surfaced even when the
  // overall level is clean (a heal that silently never took is the exact failure
  // this leg exists to catch). row.healVerify is written by the watchdog audit.
  let verifyNote = "";
  const hv = (row.healVerify && typeof row.healVerify === "object") ? row.healVerify : null;
  ...
  if (ineffective.length) {
    ...
    verifyNote = ` [heal-INEFFECTIVE: ${ineffective.length} re-enabled task(s) still NOT running`
      ...
      + `; the re-enable did not take -- fix the root cause / re-register from an ELEVATED shell, do not just re-enable again]`;
  }

  if (row.level !== "warn" && row.level !== "critical") {
    // Clean/stale safety net, but an ineffective self-heal still needs surfacing.
    return verifyNote ? `PRISM scheduled-task self-heal INEFFECTIVE.${verifyNote}` : null;
  }
