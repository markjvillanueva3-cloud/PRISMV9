...
function readHeadCommit() {
  try {
    const sha = execFileSync("git", ["-C", REPO, "log", "-1", "--format=%H"], { windowsHide: true,
      encoding: "utf8", timeout: TIMEOUT_MS, stdio: ["ignore", "pipe", "ignore"],
    }).trim();
    if (!sha) return null;
    const subject = execFileSync("git", ["-C", REPO, "log", "-1", "--format=%s"], { windowsHide: true,
      encoding: "utf8", timeout: TIMEOUT_MS, stdio: ["ignore", "pipe", "ignore"],
    }).trim();
    const body = execFileSync("git", ["-C", REPO, "log", "-1", "--format=%b"], { windowsHide: true,
      encoding: "utf8", timeout: TIMEOUT_MS, stdio: ["ignore", "pipe", "ignore"],
    }).trim();
    const dateRaw = execFileSync("git", ["-C", REPO, "log", "-1", "--format=%cI"], { windowsHide: true,
      encoding: "utf8", timeout: TIMEOUT_MS, stdio: ["ignore", "pipe", "ignore"],
    }).trim();
