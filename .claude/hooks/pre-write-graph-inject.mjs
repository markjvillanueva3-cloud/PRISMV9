 ...
 *   PRISM_PRE_WRITE_INLINE_CARD_MIN_SCORE=N    -- GAP-A high-conf threshold (default 10; 0=disable)
 */
...
// 10 is a confident floor. Knob PRISM_PRE_WRITE_INLINE_CARD_MIN_SCORE (0=disable).
 ...
 *        optional node-id->doc-pointer resolver (seekCard wrapper) for vault paths
 *        on the exact-match banner and the GAP-A inline card.
 * @param {number} [inlineCardMinScore]
 *        GAP-A threshold override; 0 or negative disables the inline card.
 *        When omitted, reads PRISM_PRE_WRITE_INLINE_CARD_MIN_SCORE env or uses 10.
 * @returns {string|null}
 */
export function renderInject(keys, hits, resolve, seekDocs, inlineCardMinScore) {
  ...
  const threshold = (typeof inlineCardMinScore === "number")
    ? inlineCardMinScore
    : (() => {
        const env = Number(process.env.PRISM_PRE_WRITE_INLINE_CARD_MIN_SCORE);
