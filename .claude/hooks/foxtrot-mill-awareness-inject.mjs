...
export function buildContext() {
  return [
    ...
    "  5. trochoidal entry < 90deg air-cuts -- 90deg flat default (TrochoidalMillingEngine.ts)",
...
async function main(injected) {
  ...
  // TASK-AWARE firing (MILL-APPROACH-KNOWLEDGE, slot:zulu 2026-06-29): fire the SPECIFIC
  // verified gates for the milling op(s) named in the prompt, conditioned on the JM mill
  // fleet (5-axis/Hurco-aware). Loaded via a GUARDED DYNAMIC import so a failure in the
  // optional lib can NEVER kill the static mill block above (the arm-C P1 lesson from the
  // lathe sibling). Fires only when an op is detected -> ~0 added tokens otherwise.
  let taskBlock = "";
  try {
    const { detectOperations, fireForApproach, JM_MILL_FLEET } = await import("../../scripts/lib/mill-approach-knowledge.mjs");
    const ops = detectOperations(prompt);
    if (ops.length) {
      const fired = fireForApproach({ operations: ops, machines: JM_MILL_FLEET.map((m) => m.id) });
      const lines = [`### 🎯 Mill approach firing -- ${ops.join(", ")} (JM fleet caps: ${fired.fleetCaps.join("/") || "?"})`];
      for (const o of fired.operations) {
        const tips = o.gates.slice(0, 4).map((g) => `${g.rule} [${g.cite}]`).join("  ·  ");
        lines.push(`- **${o.operation}**: ${tips}`);
        for (const l of o.landmines.slice(0, 1)) lines.push(`  - LANDMINE: ${l.rule}`);
      }
      taskBlock = "\n\n" + lines.join("\n");
    }
  } catch { /* fail-soft: static mill block still ships even if the task-lib breaks */ }
  approve(buildOutput(dedupedContext("foxtrot-mill-awareness", buildContext() + taskBlock, sid)));
}
