...
describe("readActiveClaim", () => {
  ...
  const claimsRoot = (c) => JSON.stringify({ claims: { [typeof c.unitId === "string" ? c.unitId : "MS::U-X"]: { slot: "charlie", ...c } } });
  ...
  it("REGRESSION: resolves a unitId-keyed claim via its slot field; the old slot-name-keyed shape is NOT matched", () => {
    // real slot-task-claim.mjs shape: claims keyed by unitId, each row carries its slot
    const realShape = JSON.stringify({ claims: { "MS::U-REAL": { unitId: "MS::U-REAL", slot: "charlie", lastHeartbeat: new Date(now - 60_000).toISOString() } } });
    assert.equal(readActiveClaim("charlie", "x", () => realShape, now)?.unitId, "MS::U-REAL");
    // pre-fix bug shape (keyed by slot name, no slot field) must not be matched by accident
    const buggyShape = JSON.stringify({ claims: { charlie: { unitId: "MS::U-OLD", lastHeartbeat: new Date(now - 60_000).toISOString() } } });
    assert.equal(readActiveClaim("charlie", "x", () => buggyShape, now), null);
  });
});
...
describe("runHook end-to-end", () => {
  ...
  const claimsBody = (unitId) => JSON.stringify({
    claims: { [unitId]: { unitId, slot: "charlie", lastHeartbeat: new Date().toISOString() } },
  });
