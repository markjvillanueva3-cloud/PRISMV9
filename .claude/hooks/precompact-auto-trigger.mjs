#!/usr/bin/env node
/**
 * precompact-auto-trigger.mjs — Enforce /precompact at 900K tokens.
 *
 * Goal:
 *   Claude runs with a 1M token context window. At 900K tokens we MUST run
 ...
 *   PRECOMPACT_SOFT_TOKENS  (default 900_000) — soft inject
 *   PRECOMPACT_HARD_TOKENS  (default 950_000) — hard block (buffer for
...
// Thresholds are paired with CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=95 (950K):
//   SOFT 900K — nudge Claude to run /precompact (non-blocking)
...
const SOFT = Number(process.env.PRECOMPACT_SOFT_TOKENS || 900_000);
...
function main() {
  ...
  if (event === "PreToolUse" && tokens >= HARD && !precompactAlreadyArmed) {
    emit({
      ...
      reason: [
        ...
        ``,
        `You are ${Math.max(0, 1_000_000 - tokens).toLocaleString()} tokens from the 1M hard cap.`,
  ...
  if ((event === "PostToolUse" || event === "UserPromptSubmit") &&
    ...
    const remaining = Math.max(0, 1_000_000 - tokens);
    const msg = [
