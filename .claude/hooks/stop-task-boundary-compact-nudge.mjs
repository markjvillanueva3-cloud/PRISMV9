...
// U-SLOT-RESOLVE-UNIFY-2 (2026-06-18): delegate to the canonical shared
// resolver -- it derives claude-<8hex> from the full UUID and EXACT-matches it
// before any lenient substring, so a peer's 8hex inside this chat's UUID can no
// longer mis-attribute the nudge (the prior substring fallback's ~1/4B risk).
function resolveSlotChat(sessionId) {
  ...
  return r ? { slot: r.slot, chatId: r.entry.chatId } : { slot: "unknown", chatId: null };
}
...
function countSlotBatch(slot) {
  ...
  try {
    ...
    ], { windowsHide: true, encoding: "utf-8", timeout: GIT_TIMEOUT_MS });
