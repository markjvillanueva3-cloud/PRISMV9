...
function main() {
  ...
  try {
    ...
    console.log(`⚠ Cannot read installed_plugins.json: ${e.message}`);
    return;
  }
  ...
  try {
    ...
    console.log(`⚠ Invalid JSON in installed_plugins.json: ${e.message}`);
    return;
  }
  ...
  try {
    ...
    console.log(`✓ Fixed ${fixCount} plugin path(s): ${fixedPlugins.slice(0, 5).join(', ')}${fixedPlugins.length > 5 ? '...' : ''}`);
  } catch (e) {
    console.log(`⚠ Cannot write installed_plugins.json: ${e.message}`);
  }
}
