...
function main() {
  ...
  if (!corpus || !corpus.entries.length) { tele("error_no_corpus"); return out({}); }
  const ranked = corpus.entries
    .map(e => { const { s, matches } = score(promptToks, e, corpus.idf); return { e, s, matches }; })
    .filter(x => x.s >= MIN_SCORE && x.matches >= MIN_MATCHES)
    .sort((a, b) => b.s - a.s)
    .slice(0, TOP_K);
  if (!ranked.length) { tele("noop_no_matches", { tokens: promptToks.length }); return out({}); }
  tele("matched", { hits: ranked.length, top_score: Math.round(ranked[0].s * 10) / 10 });
  const lines = [
