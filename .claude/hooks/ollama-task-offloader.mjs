...
async function main() {
  ...
  if (!ollama.available) {
    ...
    recordOllamaEvent({
      ...
      extras: { mode: "ollama-down", fallbackClaudeModel, snippet: prompt.slice(0, 80) },
    });
    console.log(JSON.stringify({
      continue: true,
      hookSpecificOutput: {
        hookEventName: "UserPromptSubmit",
        additionalContext: buildClaudeFallbackDirective(classification.category, fallbackClaudeModel),
      },
    }));
    return;
  }
