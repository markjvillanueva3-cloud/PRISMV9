...
test("formatBanner: zero takeups + many fires → tags as 'below target' (R12 fail-loud)", () => {
  const banner = formatBanner({
    totalFires: 100,
    takeupTotals: { totalTakeups: 0 },
  });
  assert.ok(banner.includes("below"), "below-target tag must be present");
  assert.ok(banner.includes("30%"), "target percentage must be cited so operator sees the gap");
});
