...
test("appendActionHints: real broad-Grep nudge gets master_index_query suffix", () => {
  const msg = "TOKEN-SAVE — broad Grep (output_mode='content', no glob/type, prism scope) returns full match lines for every file. Cheaper alternatives: `prism_session:master_index_query` (ranked top-K across the 110K-node graph + wiki + memory) or `prism_dev:code_search`.";
  const out = appendActionHints([msg]);
  assert.equal(out.length, 1);
  assert.ok(out[0].includes(msg), "original message preserved");
  assert.ok(out[0].includes("→ Take this route now:"), "hint appended");
  assert.ok(out[0].includes("`prism_session:master_index_query`"));
});
