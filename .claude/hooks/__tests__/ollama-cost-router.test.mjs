...
//
// U-BW-RESEARCH-REFINE (2026-06-04) + U-ALPHA-OLLAMA-ROSTER-SYNC (2026-06-25):
// qwen2.5-coder:3b stays absent (never re-pulled). qwen2.5-coder:7b was RE-PULLED
// (verified live via /api/tags 2026-06-25) + RESTORED to balanced as the measured
// mechanical sweet spot, so it is NO LONGER retired below. qwen2.5-coder:14b +
// deepseek-r1:14b are re-installed too but DELIBERATELY not tier-preferred
// (gpt-oss:20b measured faster+stronger), so they stay in the do-not-tier guard.
// These fixtures no longer use those tags as tier stand-ins; the
...
test("no tier preference references a retired model tag", () => {
  // DO-NOT-TIER guard (was "RETIRED"): :3b is genuinely not installed; :14b +
  // deepseek-r1:14b are installed but deliberately not tier-preferred (gpt-oss:20b
  // measured faster+stronger). qwen2.5-coder:7b was REMOVED here 2026-06-25 -- it was
  // re-pulled and is now the legitimate balanced sweet-spot pick (U-ALPHA-OLLAMA-ROSTER-SYNC).
  const RETIRED = ["qwen2.5-coder:3b", "qwen2.5-coder:14b", "deepseek-r1:14b"];
  for (const tier of TIER_ORDER) {
    for (const m of TIER_PREFERENCES[tier]) {
