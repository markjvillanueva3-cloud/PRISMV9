...
function fmtSummary(ctx, verbose, liveBrain) {
  ...
  if (ctx.decision) {
    ...
    // When the bundle RECOMMENDS compact (prudent band -- not yet the native ~95%
    // autocompact), make it ACTIONABLE: surface the exact self-compact command so the
    // model self-initiates instead of just seeing "compact" and continuing. Closes the
    // "recommendation surfaced but no actuation" half of why self-compact never fired.
    // (slot:bravo 2026-06-18)
    if (d.recommend === "compact") {
      lines.push("  - PRUDENT COMPACT -> self-initiate now (preferred over waiting for the native ~95% autocompact): run `node scripts/self-compact.mjs --session-id <id> --reason \"<why>\" --resume \"<next-action>\"` -- it writes a live handoff + SendKeys /compact into THIS chat's terminal. (Disable: PRISM_SELF_COMPACT_DISABLE=1.)");
    }
  }
