...
import { pickModel } from "./lib/model-router-bridge.mjs";

// ── Config ────────────────────────────────────────────────────────────
const OLLAMA_URL = process.env.OLLAMA_URL || "http://localhost:11434";
const TIMEOUT_MS = parseInt(process.env.CLAUDEMD_ENFORCER_TIMEOUT || "2000", 10);
// P20-U04: model selection routes through ModelRouterEngine. The env
// override CLAUDEMD_ENFORCER_MODEL still wins (caller-control), then
// the engine routes (tier-1 default), then qwen2.5-coder:7b fallback.
const MODEL_OVERRIDE = process.env.CLAUDEMD_ENFORCER_MODEL || "";
...
async function queryOllama(prompt, claudeMd) {
  ...
  try {
    ...
    const model = MODEL_OVERRIDE.length > 0
      ? MODEL_OVERRIDE
      : await pickModel({ kind: "general", complexity: "simple" });
