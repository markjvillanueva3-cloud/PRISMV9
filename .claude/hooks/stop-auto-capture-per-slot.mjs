...
try {
  ...
  const myEntry = ledger3way[env.session_id || ""];
  if (myEntry && myEntry.opusReviewed === "pass" && myEntry.claudeReviewed === "pass" && myEntry.codexReviewed === "pass") {
    signals.push({
      topic: `clean_ship_${(myEntry.notes ?? "session").replace(/[^\w]+/g, "_").slice(0, 40)}`,
      type: "reference",
      body: `3-of-3 PASS verdict for session. Arms: A=${myEntry.opusNotes ?? "n/a"} · B=${myEntry.claudeNotes ?? "n/a"} · C=${myEntry.codexNotes ?? "n/a"}`,
