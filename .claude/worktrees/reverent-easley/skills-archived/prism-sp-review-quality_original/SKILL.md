...
description: |
  Code quality gate - enforces S(x)≥0.70 HARD BLOCK via hooks.
  HOOK-FIRST v8.0: Auto-fires CALC-SAFETY-VIOLATION-001 (blocks S(x)<0.70),
  STATE-ANTI-REGRESSION-001 (verifies New≥Old), FORMULA-MAPE-EXCEED-001.
  Use when: after spec review approved, enforce safety gate, check quality.
  Part of SP.1 Core Development Workflow.
---
# PRISM-SP-REVIEW-QUALITY v8.0
## Code Quality Gate with Hook-Enforced Safety
### Version 8.0 | Hook-First Architecture | Development Workflow

---

# HOOK INTEGRATION (v8.0) - SAFETY GATE ENFORCEMENT

## CRITICAL: S(x)≥0.70 HARD BLOCK
The CALC-SAFETY-VIOLATION-001 hook automatically fires when:
- Safety score S(x) drops below 0.70
- G-code contains dangerous operations without guards
- Material limits exceeded
- Tool engagement unsafe

## Hooks Fired Automatically
| Phase | Hook | Purpose |
|-------|------|---------|
| Start | CALC-BEFORE-EXEC-001 | Validate review inputs |
| Safety Check | CALC-SAFETY-VIOLATION-001 | **HARD BLOCK if S(x)<0.70** |
| Anti-Regression | STATE-ANTI-REGRESSION-001 | Verify New≥Old |
| Accuracy | FORMULA-MAPE-EXCEED-001 | Flag accuracy issues |
| Complete | FORMULA-AFTER-APPLY-001 | Log Ω(x) computation |

## Manual Hook Usage
```javascript
// Check safety score explicitly
prism_hook_fire("CALC-SAFETY-VIOLATION-001", {
  safetyScore: 0.65,  // Will trigger HARD BLOCK
  component: "spindle_speed_calculation",
  violation: "exceeds_material_limit"
});

// Verify anti-regression
prism_hook_fire("STATE-ANTI-REGRESSION-001", {
  oldCount: 146,
  newCount: 148,
  resourceType: "skills"
});
...
prism_hook_fire("FORMULA-MAPE-EXCEED-001", {
  mape: 0.12,  // 12% error - may flag
  threshold: 0.10,
  formula: "Kienzle_force"
});
```

## Hook-Enforced Quality Flow
```
LOAD_STANDARDS → CODE_STYLE → PATTERNS → SAFETY_GATE → VERDICT
       ↓              ↓           ↓           ↓            ↓
   CALC-001      CALC-001    BATCH-001   **SAFETY-001**  Ω(x)
                                         (HARD BLOCK)
```

## Safety Gate Hooks (BLOCKING)
- CALC-SAFETY-VIOLATION-001: S(x)<0.70 → **OUTPUT BLOCKED**
- STATE-ANTI-REGRESSION-001: New<Old → **UPDATE BLOCKED**
- FILE-GCODE-VALIDATE-001: Dangerous codes → **FILE BLOCKED**
