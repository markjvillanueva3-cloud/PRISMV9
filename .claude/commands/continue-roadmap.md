...
policy:
  ...
  triggers:
    - "continue-roadmap"
consumes:
  - "prism_orchestrate:roadmap_advance"
  - "prism_orchestrate:roadmap_gate"
  - "prism_orchestrate:roadmap_list"
  - "prism_orchestrate:roadmap_load"
  - "prism_orchestrate:roadmap_next_batch"
  - "prism_orchestrate:roadmap_plan"
...
```
prism_session:context_boot           -- restore full context from prior session
prism_session:dispatcher_map         -- discover all all dispatchers + actions (live count from PRISM-INVENTORY-LATEST.md)
prism_session:memory_recall          -- load cross-session knowledge
prism_session:system_snapshot        -- baseline before changes
prism_session:action_search "<goal>" -- find right tool for this unit's work
```

## Advisor Strategy (`advisor_20260301`)
- **Executor**: Sonnet 4.6 (executes roadmap units, calls MCP tools)
- **Advisor**: Opus 4.6, `max_uses: 2`, `caching: {"type": "ephemeral", "ttl": "5m"}`
- **When Sonnet should call advisor**: (1) before starting each unit — to plan approach, (2) before declaring unit complete — to verify deliverables are durable
...
- **Tool references** -- all tools in unit.tools must resolve to known prism_* dispatchers (all dispatchers + actions (live count from PRISM-INVENTORY-LATEST.md))
...
## MCP Utilization During Execution
Every 5-10 tool calls during unit work:
```
prism_session:auto_checkpoint       -- save incremental state
prism_session:action_search "<need>"-- find optimal tool for current task
prism_session:wip_capture           -- snapshot work at natural breakpoints
```
...
prism_session:system_snapshot       -- capture post-work state
prism_session:checkpoint_enhanced   -- detailed checkpoint with artifact list
```
The PostCompact hook auto-writes SESSION_ARTIFACTS.json (Feature Cascade) for the next session.

## Target Quality
- Omega = 1.0 at every gate
- All tests passing, no regression
- Build clean
- MCP utilization: context_boot at start, memory_save at end (NON-NEGOTIABLE)
