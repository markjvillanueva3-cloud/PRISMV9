...
// @ts-expect-error — three-cad-viewer ships no .d.ts; treat as `any` import
...
export default function CadViewer({ cadShapes }: CadViewerProps) {
    ...
    useEffect(() => {
        ...
        if (cadShapes && cadShapes.length > 0) {
            ...
            const render = (_name: string, shapes: unknown, states: unknown) => {
                viewer?.clear()
            ...
            const [shapesArg, statesArg] = cadShapes as [unknown, unknown]
            render("input", shapesArg, statesArg)
        }
