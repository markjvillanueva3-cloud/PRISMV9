...
const HOTKEY_RE = /\[?\b(?:Ctrl|Alt|Shift|Cmd|Meta)(?:\s*\+\s*(?:Ctrl|Alt|Shift|Cmd|Meta))*\s*\+\s*[A-Za-z0-9]+\]?/gi;
/**
...
export class ActionSequenceExtractorEngine {
     ...
     */
    static detectUITargets(sentence) {
        if (typeof sentence !== "string" || sentence.length === 0)
            return [];
        const found = new Set();
     ...
     */
    static extractFromTip(tip, sourceDoc) {
        ...
        if (!parsed.success) {
            ...
            const issues = parsed.error.issues ??
                parsed.error.errors ??
                [];
            throw new Error(`ActionSequenceExtractor: invalid tip shape — ${issues
        ...
        if (steps.length === 0)
            return null;
        return {
            id: this.stableId(sourceDoc, parsed.data),
     ...
     */
    static extractBatch(tips, sourceDoc) {
