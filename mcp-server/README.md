...
PRISM exposes **103 dispatchers / 12 000+ MCP actions** in a machine-readable manifest at
`mcp-server/MANIFEST.json`. Any MCP-capable AI coding agent can connect and consume the full
manufacturing intelligence surface. Regenerate after dispatcher changes:

```bash
node H:/prism/scripts/build-mcp-manifest.mjs
```

### Cline (VS Code extension, 58k stars)

Add to `.cline/mcp.json` in your workspace:
...
```json
{
  "mcpServers": {
    "prism": {
      ...
      "env": { "LOG_LEVEL": "info" },
      "manifestPath": "H:/prism/mcp-server/MANIFEST.json"
    }
  }
}
```

Cline's Plan phase can read `MANIFEST.json` to surface the right dispatcher before acting.
PRISM's `/plan-build` skill + per-file scrutiny gate mirrors Cline's Plan/Act architecture.

### Continue.dev (JetBrains + VS Code)
...
For HTTP transport (shared server already running on `:3100`):
...
```bash
# Start PRISM MCP server in a separate terminal
node H:/prism/mcp-server/dist/index.js

# Launch Aider with MCP endpoint
aider --mcp-server http://localhost:3100/mcp
```

Reference `MANIFEST.json` to discover available actions before prompting Aider.

### Gemini CLI

```bash
# In gemini CLI config (~/.gemini/config.json):
{
  "mcpServers": {
    "prism": {
      "url": "http://localhost:3100/mcp",
      "manifest": "H:/prism/mcp-server/MANIFEST.json"
    }
  }
}
```
...
```bash
codex --mcp-server http://localhost:3100/mcp \
      --mcp-manifest H:/prism/mcp-server/MANIFEST.json
```
...
### Claude Code (canonical internal client)

Already wired. PRISM's 26-chat fleet uses stdio transport via `.claude/settings.json`.
The manifest serves as the discoverability surface for all other agents.

### Dispatcher Quick-Reference

The manifest groups all 103 dispatchers by PSN leg and use-case. Key entry points:

| Dispatcher | MCP Name | Actions | Primary Use |
|---|---|---|---|
| `calcDispatcher` | `prism_calc` | 1360 | Physics calculations (force, speed/feed, tool life) |
| `camDispatcher` | `prism_cam` | 2359 | CAM toolpath generation & simulation |
| `turningDispatcher` | `prism_turning` | 269 | Lathe/turning programs — SAFETY CRITICAL |
| `safetyDispatcher` | `prism_safety` | 80 | Collision detection, safety validation |
| `ppDispatcher` | `prism_pp` | 801 | Post-processing & G-code generation |
| `cadDispatcher` | `prism_cad` | 548 | CAD geometry operations |
| `intelligenceDispatcher` | `prism_intelligence` | 366 | Manufacturing intelligence & job planning |
| `sessionDispatcher` | `prism_session` | 318 | Session state, memory, handoffs |
| `knowledgeDispatcher` | `prism_knowledge` | 253 | Unified knowledge query (9 registries) |
| `businessDispatcher` | `prism_business` | 549 | Quoting, costing, ERP, scheduling |

Full action list: query `MANIFEST.json` `.dispatchers[].actionCount` or connect any MCP client.
