...
// The PRISM MCP HTTP backend ("HTTP bridge") runs on :3100 canonically (fleet-wide; see
// src/index.ts + every "HTTP bridge port 3100" doc). The dev server proxies /api + /ws to it.
// Was defaulted to :3000 (stale) -> the proxy hit a dead port so every dispatcher call failed.
// Override with PRISM_API_PORT / PRISM_API_HOST if the backend runs elsewhere.
const prismApiPort = process.env.PRISM_API_PORT || '3100';
...
// Use 127.0.0.1 (IPv4), NOT 'localhost'. The backend binds 127.0.0.1 (IPv4-only, see
// src/index.ts PRISM_BIND_HOST default); on Windows 'localhost' resolves to ::1 (IPv6) first,
// so the proxy hit a non-PRISM listener and 405'd every POST. Override via PRISM_API_HOST.
const prismApiHost = process.env.PRISM_API_HOST || '127.0.0.1';
...
export default defineConfig({
  ...
  server: {
    // Dev server on :3101 -- the canonical backend (proxy target above) owns :3100, so the
    // dev server cannot share it; /api + /ws are proxied to the :3100 backend. (Playwright
    // overrides this via its own --port flag, so e2e is unaffected.)
    port: 3101,
