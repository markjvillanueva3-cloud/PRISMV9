...
describe('ShopFloorClockPage', () => {
  ...
  it('shows clock in button after selecting employee', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Select Employee'), { target: { value: 'EMP-001' } });
  ...
  it('shows timer display with 00:00:00 format', async () => {
    ...
    await waitFor(() => screen.getByText('Job Timer'));
...
describe('TimecardPage', () => {
  ...
  it('loads timecard with job breakdown', async () => {
    wrap(<TimecardPage />);
    await waitFor(() => screen.getByText(/John Doe/));
    fireEvent.change(screen.getByLabelText('Employee'), { target: { value: 'EMP-001' } });
    fireEvent.click(screen.getByText('Load Timecard'));
    await waitFor(() => {
...
describe('PayrollPage', () => {
  it('renders title and period inputs', () => {
    ...
    expect(screen.getByText('Run Payroll')).toBeDefined();
  });
...
describe('InvoicesPage', () => {
  ...
  it('toggles create invoice form', () => {
    ...
    expect(screen.getByText('Create Invoice from Job')).toBeDefined();
  });
});
...
describe('ToolingCostPage', () => {
  ...
  it('shows reorder alerts for high wear tools', async () => {
    wrap(<ToolingCostPage />);
    await waitFor(() => {
      expect(screen.getByText('Reorder Required')).toBeDefined();
    });
    expect(screen.getAllByText(/6mm Ball Nose/).length).toBeGreaterThan(0);
  });
