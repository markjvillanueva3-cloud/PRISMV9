...
describe('CalculatorPage machine selection', () => {
  ...
  it('renders media-backed previews for every machine mode card', async () => {
    await act(async () => {
      renderCalculator();
    });

      const expectedSources = {
        mill: '/media/machine-modes/real/mill-5axis-cut-loop.mp4',
        lathe: '/media/machine-modes/real/lathe-cnc-real.webm',
        edm: '/media/machine-modes/real/edm-sinker-running-loop.mp4',
        wire_edm: '/media/machine-modes/real/wire-edm-running-loop.mp4',
        laser: '/media/machine-modes/real/laser-real.webm',
        waterjet: '/media/machine-modes/real/waterjet-cutting-loop.mp4',
      } as const;

    (Object.entries(expectedSources) as Array<[keyof typeof expectedSources, string]>).forEach(([mode, expectedSrc]) => {
      const preview = screen.getByTestId(`machine-mode-media-${mode}`);
      const media = preview.querySelector('img, video');
      expect(media).not.toBeNull();
      expect(media?.getAttribute('src')).toContain(expectedSrc);
    });

    expect(screen.getByRole('button', { name: /switch calculator mode to mill/i })).toHaveAttribute('aria-pressed', 'true');
    await act(async () => {
      fireEvent.click(screen.getByRole('button', { name: /switch calculator mode to wire edm/i }));
    });
    await waitFor(() => {
  ...
  it('hydrates live-only machine brands into the machine selector when the expanded catalog route responds', async () => {
    vi.stubGlobal(
      'fetch',
    ...
    await waitFor(() => {
      expect(screen.getAllByText(/2 live/i).length).toBeGreaterThan(0);
    });

    const manufacturerSelect = screen.getByRole('combobox', { name: /manufacturer/i });
    expect(within(manufacturerSelect).getByRole('option', { name: 'Awea' })).toBeDefined();
    expect(within(manufacturerSelect).getByRole('option', { name: 'Toyoda' })).toBeDefined();
  });
  ...
  it('surfaces the Hurco TM10i package with a populated turning configuration and the JM Die VDI baseline holder', async () => {
    await act(async () => {
      renderCalculator();
    });
    ...
    expect(within(holderPackageSelect).getByRole('option', { name: /VDI30 Turning Baseline/i })).toBeDefined();
  });

  it('surfaces the DN Solutions VTL package with a populated turning configuration and the JM Die VDI baseline holder', async () => {
    await act(async () => {
      renderCalculator();
    });
  ...
  it('keeps the JM Die VDI baseline holder package available for VDI80 lathes like the DN PUMA 700LM', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn((input: RequestInfo | URL) => {
        const url = typeof input === 'string' ? input : input instanceof URL ? input.toString() : input.url;
