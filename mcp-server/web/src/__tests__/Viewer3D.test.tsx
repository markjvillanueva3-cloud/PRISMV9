...
describe('ViewerPage', () => {
  ...
  it('displays scene name and segment count in header', () => {
    renderViewer();
    // "Demo Scene" appears in header subtitle and side panel Scene row
    expect(screen.getAllByText(/Demo Scene/).length).toBeGreaterThanOrEqual(1);
    expect(screen.getByText(/3 segments/)).toBeDefined();
  });

  it('renders the 3D canvas mock', () => {
    renderViewer();
    expect(screen.getByTestId('viewer-3d-canvas')).toBeDefined();
  });
  ...
  it('displays Scene Info panel with segment stats', () => {
    renderViewer();
    expect(screen.getByText('Scene Info')).toBeDefined();
  ...
  it('shows the operation list in side panel', () => {
    renderViewer();
