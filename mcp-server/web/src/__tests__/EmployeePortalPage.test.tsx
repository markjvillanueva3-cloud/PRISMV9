...
function LocationProbe() {
  ...
  return <div data-testid="portal-location">{`${location.pathname}${location.search}`}</div>;
}
...
describe('EmployeePortalPage', () => {
  ...
  it('preserves unrelated workflow query params when switching profiles', async () => {
    renderPortal('/employee?profile=machinist&source=timecards&employeeId=EMP-0042');
    ...
    await act(async () => {
      screen.getByRole('button', { name: /Jordan Vale/i }).click();
    });
    ...
    expect(screen.getByTestId('portal-location').textContent).toContain('/employee?');
    expect(screen.getByTestId('portal-location').textContent).toContain('profile=planner');
    expect(screen.getByTestId('portal-location').textContent).toContain('source=timecards');
    expect(screen.getByTestId('portal-location').textContent).toContain('employeeId=EMP-0042');
  });
});
