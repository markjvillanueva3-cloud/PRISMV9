  ...
  mockMachineRateCompare.mockResolvedValue({
    result: {
      machines: [
        ...
        { id: 'UMC750', name: 'Haas UMC-750', hourly_rate: 105, setup_multiplier: 1.25, annual_cost: 260000 },
      ],
      cheapest: 'VF2',
...
describe('manufacturing utility pages', () => {
  ...
  it('renders the rebuilt machine rates desk and effective-rate lane', async () => {
    ...
    fireEvent.change(screen.getByLabelText('OEE posture'), { target: { value: 'typical' } });
    fireEvent.click(screen.getByRole('button', { name: 'Lookup' }));

    await waitFor(() => {
      expect(screen.getByText('Effective / hr')).toBeDefined();
      expect(screen.getByText('$114.50')).toBeDefined();
