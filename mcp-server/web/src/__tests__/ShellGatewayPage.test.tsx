    ...
    <OperatingSystemProvider services={services}>
      <MemoryRouter initialEntries={[initialEntry]}>
        <Routes>
          ...
          <Route path="/shop-clock" element={<div>Shop clock target</div>} />
          <Route path="/employees" element={<div>People ops target</div>} />
...
describe('ShellGatewayPage', () => {
  ...
  it('renders role-aware sign-in options when no shell session exists', async () => {
    ...
    expect(screen.getByRole('link', { name: /Employee workspace/i })).toBeDefined();
    expect(screen.getByRole('link', { name: /Shop clock kiosk/i })).toBeDefined();
    expect(screen.getByRole('link', { name: /People ops/i })).toBeDefined();
  });
