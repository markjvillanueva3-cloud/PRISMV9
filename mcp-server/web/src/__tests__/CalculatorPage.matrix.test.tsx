...
describe('CalculatorPage logical mill/lathe matrix', () => {
  ...
  it('keeps desired-finish forecasting logical across thousands of ferrous calculator combinations', () => {
    ...
    const machineById = new Map(MACHINE_CATALOG.map((item) => [item.id, item]));
    const materialById = new Map(MATERIAL_CATALOG.map((item) => [item.id, item]));
    const toolById = new Map(TOOL_CATALOG.map((item) => [item.id, item]));
    const programmingById = new Map(PROGRAMMING_ENVIRONMENTS.map((item) => [item.id, item]));
    const toolpathsByProgramId = new Map(
      PROGRAMMING_ENVIRONMENTS.map((item) => [item.id, new Map(item.toolpaths.map((toolpath) => [toolpath.id, toolpath]))]),
    );
    ...
    matrix.rows.forEach((row, index) => {
      ...
      const programming = programmingById.get(row.programmingId);
      const toolpath = toolpathsByProgramId.get(row.programmingId)?.get(row.toolpathId);
