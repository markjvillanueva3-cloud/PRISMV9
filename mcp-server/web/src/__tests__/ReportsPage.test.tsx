...
export class LeafSpringEngine {
    calculate(input) {
        ...
        // Warnings
        if (sigma > sigUlt * 0.5) {
            warnings.push(`Stress ${r0(sigma)} MPa > 50% Sut — fatigue risk`);
        }
        if (fatigueN < 1.5) {
            warnings.push(`Fatigue SF ${r1(fatigueN)} < 1.5 — reduce load or add leaves`);
        }
        if (t > b / 4) {
            warnings.push(`Thickness ${t}mm > b/4 — non-standard aspect ratio`);
        }
        if (L / t > 100) {
            warnings.push(`L/t ratio ${r0(L / t)} > 100 — excessive flexibility`);
        }
        if (delta > span * 0.1) {
            warnings.push(`Deflection ${r1(delta)}mm > 10% span — may bottom out`);
        }
        if (n > 12) {
        ...
        return {
            bending_stress: mkAv(r0(sigma), "MPa", sigma * 0.05, `6FL/(nEff×b×t²)`),
            deflection: mkAv(r1(delta), "mm", delta * 0.1, `6FL³/(nEff×E×b×t³)`),
            spring_rate: mkAv(r0(k), "N/mm", k * 0.05, `F/δ`),
            effective_leaves: mkAv(r2(nEff), "leaves", 0, `(3nf+2ng)/3(nf+ng)×n`),
            nip_load: mkAv(r0(Fnip), "N", Fnip * 0.1, "stress equalization"),
            full_leaf_stress: mkAv(r0(sigFull), "MPa", sigFull * 0.05, "full leaves"),
            graduated_leaf_stress: mkAv(r0(sigGrad), "MPa", sigGrad * 0.05, "graduated leaves"),
            fatigue_safety: mkAv(r1(fatigueN), "×", fatigueN * 0.1, "Goodman"),
            energy_stored: mkAv(r1(U), "J", U * 0.1, `½Fδ`),
            free_camber: mkAv(r0(freeCamber), "mm", freeCamber * 0.05, src),
            warnings,
        };
    }
}
// ── Helpers ───────────────────────────────────────────────────────
function mkAv(value, unit, uncertainty, source) {
