...
describe('SchedulingPage', () => {
  ...
  it('deep-links shop floor and capture ops from the selected study', async () => {
    ...
    const releaseLink = screen.getByRole('link', { name: /Open Print to CNC/i });
    ...
    expect(releaseParams.get('source')).toBe('scheduling-desk');
    expect(releaseParams.get('originSource')).toBe('scheduling-desk');
    expect(releaseParams.get('focusJobId')).toBe('BRKT-01');
    ...
    expect(releaseParams.get('partClassId')).toBeNull();
    expect(releaseParams.get('toolholderId')).toBeNull();
    expect(releaseParams.get('stockId')).toBeNull();
  ...
  it('shows the hot release queue and keeps release links record-aware', async () => {
    ...
    expect(releaseParams.get('source')).toBe('scheduling-desk');
    expect(releaseParams.get('originSource')).toBe('scheduling-desk');
    expect(releaseParams.get('focusJobId')).toBe('JOB-HOT-991');
    expect(releaseParams.get('partClassId')).toBe('prismatic-bracket');
    expect(releaseParams.get('machineId')).toBeNull();
    expect(releaseParams.get('toolholderId')).toBeNull();
    expect(releaseParams.get('stockId')).toBeNull();
  });
});
