...
function installFetchMock(handlers: {
  ...
  resolveMachine?: (body: Record<string, unknown>) => unknown;
  resolveTool?: (body: Record<string, unknown>) => unknown;
  inventorySelect?: (body: Record<string, unknown>) => unknown;
  ...
  vi.stubGlobal('fetch', vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.includes('/api/v1/speed-feed/resolve/machine')) return okJson(handlers.resolveMachine?.(body ?? {}) ?? { result: { value: { limiting_factors: [] } } });
    if (url.includes('/api/v1/speed-feed/resolve/tool')) return okJson(handlers.resolveTool?.(body ?? {}) ?? { result: { value: { limiting_factors: [] } } });
    if (url.includes('/api/v1/speed-feed/inventory-select')) return okJson(handlers.inventorySelect?.(body ?? {}) ?? { result: { value: { inventory_coverage_pct: 100, missing_tools: [] } } });
...
describe('CalculatorPage machinist workflow solves', () => {
  it.each([
    ...
    const requests = installFetchMock({
      speedFeed: () => response,
      resolveMachine: () => ({ result: { value: { power_kw: 22, max_rpm: 12000, limiting_factors: [{ parameter: 'machine', constraint: 'torque knee reached near the requested speed window' }] } } }),
      resolveTool: () => ({ result: { value: { tool_life_min: 8.5, deflection_um: 31.2, limiting_factors: [{ parameter: 'tool', constraint: 'stickout dominates the current engagement' }] } } }),
      inventorySelect: () => ({ result: { value: { inventory_coverage_pct: 50, missing_tools: [{ recommended_type: 'face_mill', recommended_diameter_mm: 50 }] } } }),
    });
    ...
    await waitFor(() => expect(screen.getByText(/Backend preflight/i)).toBeDefined());
    expect(screen.getByText(/Machine preflight limits:/i)).toBeDefined();
    expect(screen.getByText(/Tool preflight/i)).toBeDefined();
    expect(screen.getByText(/Inventory preflight:/i)).toBeDefined();
    expect(requests.find((entry) => entry.url.includes('/api/v1/speed-feed/orchestrate'))?.body).toMatchObject(expectRequest);
    expect(requests.some((entry) => entry.url.includes('/api/v1/speed-feed/resolve/machine'))).toBe(true);
    expect(requests.some((entry) => entry.url.includes('/api/v1/speed-feed/resolve/tool'))).toBe(true);
    expect(requests.some((entry) => entry.url.includes('/api/v1/speed-feed/inventory-select'))).toBe(true);
  }, 90000);
