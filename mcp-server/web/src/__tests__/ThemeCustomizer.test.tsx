...
describe('ThemeCustomizer — accent color', () => {
  it('renders every ACCENT_PRESET as a radio, with System Blue selected by default', () => {
    ...
    // KIENZLE-MERGE U-KZ-THEME: ACCENT_PRESETS grew 5->7 (added Kienzle Green +
    // Kienzle Orange); the customizer renders one radio per preset.
    expect(screen.getAllByRole('radio')).toHaveLength(7);
    expect(screen.getByRole('radio', { name: 'System Blue' })).toHaveAttribute('aria-checked', 'true');
    expect(screen.getByRole('radio', { name: 'Kienzle Cyan' })).toHaveAttribute('aria-checked', 'false');
    expect(screen.getByRole('radio', { name: 'Kienzle Green' })).toHaveAttribute('aria-checked', 'false');
    expect(screen.getByRole('radio', { name: 'Kienzle Orange' })).toHaveAttribute('aria-checked', 'false');
  });

  it('clicking a swatch writes its rgb to --accent-rgb on document.body and flips aria-checked', () => {
    render(<ThemeCustomizer />);
    fireEvent.click(screen.getByRole('radio', { name: 'Kienzle Cyan' }));
    expect(document.body.style.getPropertyValue('--accent-rgb')).toBe('34 211 238');
    expect(screen.getByRole('radio', { name: 'Kienzle Cyan' })).toHaveAttribute('aria-checked', 'true');
...
describe('ThemeCustomizer — reset + the deliberate no-density-dial', () => {
  it('Reset clears every body override and localStorage', () => {
    ...
    expect(document.body.style.getPropertyValue('--radius-md')).toBe('');
    expect(window.localStorage.getItem('kienzle-theme-tokens-v1')).toBeNull();
  });
