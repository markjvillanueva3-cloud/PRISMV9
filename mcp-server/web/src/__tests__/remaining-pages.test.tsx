  ...
  buildDemoScene: () => ({
    ...
    root: {
      ...
      children: [
        ...
        { id: 'tp', type: 'toolpath', name: 'Toolpath', segments: [] },
      ],
    },
  }),
}));
...
describe('AlarmPage', () => {
  ...
  it('has alarm code input field', async () => {
    const { AlarmPage } = await import('../pages/AlarmPage');
    renderPage(AlarmPage);
    expect(screen.getByPlaceholderText('e.g., 1020')).toBeDefined();
  });
});
...
describe('CalculatorPage', () => {
  it('renders calculator with formula list', async () => {
    ...
    expect(screen.getByText(/Material Removal Rate/i)).toBeDefined();
  });
});
...
describe('DashboardPage', () => {
  it('renders dashboard title', async () => {
    ...
    expect(screen.getByText(/PRISM/i)).toBeDefined();
  });
});
...
describe('FinancialAnalysisPage', () => {
  it('renders title and tabs', async () => {
    ...
    expect(screen.getByText('NPV Analysis')).toBeDefined();
    expect(screen.getByText('IRR Calculator')).toBeDefined();
  });

  it('switches to machine investment tab', async () => {
    const { FinancialAnalysisPage } = await import('../pages/FinancialAnalysisPage');
    renderPage(FinancialAnalysisPage);
    const btn = screen.getByRole('button', { name: 'Machine Investment' });
    fireEvent.click(btn);
    expect(btn.className).toContain('bg-prism');
  });

  it('switches to breakeven tab', async () => {
    const { FinancialAnalysisPage } = await import('../pages/FinancialAnalysisPage');
    renderPage(FinancialAnalysisPage);
    const btn = screen.getByRole('button', { name: 'Breakeven' });
    fireEvent.click(btn);
    expect(btn.className).toContain('bg-prism');
  });
});
...
describe('ToolpathAdvisorPage', () => {
  it('renders toolpath advisor', async () => {
    ...
    expect(screen.getByText(/Toolpath/i)).toBeDefined();
  });
});
