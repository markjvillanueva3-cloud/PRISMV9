...
describe('calculatorSpeedFeedContract', () => {
  it('builds a mill payload with canonical machine, holder, coolant, and DOC/WOC fields', () => {
    ...
    expect(params.machinePackage?.spindle?.power_kw).toBeCloseTo(22.371, 3);
    expect(params.machinePackage?.spindle?.power_continuous_kw).toBeCloseTo(22.371, 3);
  ...
  it('prefers measured machine posture and verified features when deriving machine dynamics', () => {
    ...
    expect(params.machinePackage?.spindle?.max_rpm).toBe(15000);
    expect(params.machinePackage?.spindle?.torque_max_nm).toBe(102);
  ...
  it('maps vacuum and tombstone workholding selections into the correct backend workholding types', () => {
    const vacuumParams = buildCalculatorSpeedFeedParams({
      machineMode: 'mill',
      machine: okumaMill,
      controllerOption: okumaMill.controllerOptions[0],
      spindleOption: okumaMill.spindleOptions[0],
      ...
      toolpath: {
        ...
        path: 'Roughing / Adaptive',
        operationId: 'roughing',
      },
      programming: { id: 'fusion-mill', label: 'Fusion Mill', vendor: 'Fusion 360' },
      toolDiameterMm: 10,
      docMm: 4,
      wocMm: 1.8,
      flutes: 4,
      ...
      stockZm: 16,
      coolantId: 'flood',
      workholdingId: 'vacuum-fixture',
      workholdingCategoryId: 'fixture',
      stabilityId: 'detail-control',
      finishTarget: 'balanced',
    });

    const tombstoneParams = buildCalculatorSpeedFeedParams({
      machineMode: 'mill',
      machine: okumaMill,
      controllerOption: okumaMill.controllerOptions[0],
      spindleOption: okumaMill.spindleOptions[0],
    ...
    expect(vacuumParams.workholding_type).toBe('vacuum');
    expect(vacuumParams.workholding_stiffness).toBe('low');
    expect(tombstoneParams.workholding_type).toBe('tombstone');
    expect(tombstoneParams.workholding_stiffness).toBe('high');
  });
  ...
  it('reconstructs milling speed, feed, and MRR identities when primary outputs are missing or zero', () => {
    const normalized = normalizeCalculatorSpeedFeedResult({
      result: {
        value: {
          cutting_speed_mpm: 188.4956,
          spindle_rpm: 0,
          feed_rate_mmmin: 0,
    ...
    expect(normalized.rpm).toBeCloseTo(6000, 0);
    expect(normalized.cuttingSpeed).toBeCloseTo(188.4956, 4);
    expect(normalized.feedPerTooth).toBe(0.03);
    expect(normalized.feedRate).toBeCloseTo(720, 3);
    expect(normalized.mrr).toBeCloseTo(6.48, 3);
  });

  it('normalizes legacy SFM and chip-load response fields into the metric calculator contract', () => {
    const normalized = normalizeCalculatorSpeedFeedResult({
      result: {
        value: {
          sfm: 600,
    ...
    expect(normalized.cuttingSpeed).toBeCloseTo(182.88, 3);
    expect(normalized.rpm).toBe(6111);
    expect(normalized.feedPerTooth).toBe(0.0254);
    expect(normalized.feedRate).toBe(621.1);
    expect(normalized.mrr).toBe(23.5);
  });
