...
describe('callBusinessAction', () => {
  ...
  // U-WIRE-AUTH-KEY-FIX regression: AuthContext writes the CANONICAL key
  // 'prism-auth-token' (hyphen) as a JSON session object {token}. The pre-fix
  // client read only 'prism_auth_token' (underscore) as a raw string, so a
  // real logged-in user's token was MISSED -> no Bearer -> 401 -> dead panel.
  // These two cases fail against the old code and pass against getStoredAuthToken().
  it('attaches a Bearer from the canonical prism-auth-token key (JSON session object)', async () => {
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem('prism-auth-token', JSON.stringify({ token: 'canonical-abc' }));
    }
    const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => ({}) });
    globalThis.fetch = fetchMock as unknown as typeof fetch;
    ...
    const [, init] = fetchMock.mock.calls[0];
    expect(init.headers.Authorization).toBe('Bearer canonical-abc');
  });

  it('prefers the canonical key over a legacy underscore key', async () => {
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem('prism-auth-token', JSON.stringify({ token: 'canonical-win' }));
      localStorage.setItem('prism_auth_token', 'legacy-lose');
    }
    const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => ({}) });
    globalThis.fetch = fetchMock as unknown as typeof fetch;
    ...
    const [, init] = fetchMock.mock.calls[0];
    expect(init.headers.Authorization).toBe('Bearer canonical-win');
  });
