...
function splitRequestedChanges(value: string) {
  return value
    .split(/[\n;]+/)
...
export function CustomerPortalPage() {
  ...
  const [quoteResponseForm, setQuoteResponseForm] = useState<QuoteResponseState>({
    response: 'accept',
    customer_name: workflow.origin.customer,
    message: '',
    requested_changes: '',
  });
  const [portalMessageForm, setPortalMessageForm] = useState<PortalMessageDraftState>({
    sender_name: workflow.origin.customer,
    message: '',
  });
  ...
  useEffect(() => {
    ...
    setQuoteResponseForm({
      response: 'accept',
      customer_name: workflow.origin.customer,
      message: '',
      requested_changes: '',
    });
    setPortalMessageForm({
      sender_name: workflow.origin.customer,
      message: '',
    });
  ...
  const selectedTokenCanRespond = selectedToken?.scope.includes('respond') ?? false;
  const selectedTokenCanMessage = selectedToken?.scope.includes('messages') ?? false;
  ...
  async function handleRespondToQuote(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!selectedToken || selectedToken.token_type !== 'quote' || !selectedTokenCanRespond) return;
    ...
    try {
      const requestedChanges =
        quoteResponseForm.response === 'request_changes'
          ? splitRequestedChanges(quoteResponseForm.requested_changes)
          : undefined;

      const result = await respondToPortalQuote(selectedToken.token, {
        response: quoteResponseForm.response,
        customer_name: quoteResponseForm.customer_name.trim(),
        message: quoteResponseForm.message.trim() || undefined,
        requested_changes: requestedChanges,
      });
      const refreshedPreview = await getPortalQuoteView(selectedToken.token);
      setQuotePreview(refreshedPreview);
      setQuoteResponseForm((current) => ({
        ...current,
        message: '',
        requested_changes: '',
      }));
      setMessage(`Recorded quote response: ${result.response}.`);
    } catch (issue) {
      setError(issue instanceof Error ? issue.message : 'Unable to record quote response.');
    }
  }

  async function handleSendPortalMessage(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!selectedToken || selectedToken.token_type !== 'order' || !selectedTokenCanMessage) return;

    setError('');
    setMessage('');

    try {
      await sendPortalOrderMessage(selectedToken.token, {
        sender_name: portalMessageForm.sender_name.trim(),
        message: portalMessageForm.message.trim(),
      });
      const refreshedMessages = await listPortalOrderMessages(selectedToken.token);
      setPortalMessages(refreshedMessages);
      setPortalMessageForm((current) => ({
        ...current,
        message: '',
      }));
      setMessage(`Sent portal message for ${selectedToken.entity_id}.`);
    } catch (issue) {
      setError(issue instanceof Error ? issue.message : 'Unable to send portal message.');
    }
  }
          ...
          <Panel title={selectedToken?.token_type === 'order' ? 'Order portal preview' : 'Quote portal preview'} subtitle="Preview the same mounted customer portal payloads the token exposes.">
            ...
            {quotePreview ? (
              <div className="space-y-4">
                ...
                <div className="rounded-[22px] border border-white/8 bg-white/[0.03] px-4 py-4">
                  <div className="text-sm font-semibold text-slate-50">Customer quote response</div>
                  {!selectedTokenCanRespond ? (
                    <div className="mt-3 text-sm text-slate-400">
                      This token is view-only. Generate or select a token with response scope to capture customer acceptance, rejection, or requested changes.
                    </div>
                  ) : (
                    <form className="mt-3 grid gap-3" onSubmit={handleRespondToQuote}>
                      <div className="grid gap-3 sm:grid-cols-2">
                        <select
                          aria-label="Quote response action"
                          value={quoteResponseForm.response}
                          onChange={(event) =>
                            setQuoteResponseForm((current) => ({
                              ...current,
                              response: event.target.value as QuoteResponseState['response'],
                            }))
                          }
                          className="rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-slate-100 outline-none"
                        >
                        ...
                        <input
                          value={quoteResponseForm.customer_name}
                          onChange={(event) =>
                            setQuoteResponseForm((current) => ({
                              ...current,
                      ...
                      <textarea
                        value={quoteResponseForm.message}
                        onChange={(event) =>
                          setQuoteResponseForm((current) => ({
                            ...current,
                        ...
                        placeholder="Optional message to the quoting desk"
                        rows={3}
                        className="rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-slate-100 outline-none"
                      />
                      {quoteResponseForm.response === 'request_changes' ? (
                        <textarea
                          value={quoteResponseForm.requested_changes}
                          onChange={(event) =>
                            setQuoteResponseForm((current) => ({
                              ...current,
                              requested_changes: event.target.value,
                            }))
                          }
                          placeholder="Requested changes (one per line or separated by semicolons)"
                          rows={3}
                          className="rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-slate-100 outline-none"
                        />
                      ) : null}
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  ...
                  <div className="rounded-[22px] border border-white/8 bg-white/[0.03] px-4 py-4">
                    ...
                    {!selectedTokenCanMessage ? (
                      <div className="mt-4 text-sm text-slate-400">
                        This token does not include customer messaging scope yet.
                      </div>
                    ) : (
                      <form className="mt-4 grid gap-3 rounded-[20px] border border-white/8 bg-slate-950/40 px-4 py-4" onSubmit={handleSendPortalMessage}>
                        <input
                          value={portalMessageForm.sender_name}
                          onChange={(event) =>
                            setPortalMessageForm((current) => ({
                              ...current,
                        ...
                        />
                        <textarea
                          value={portalMessageForm.message}
                          onChange={(event) =>
                            setPortalMessageForm((current) => ({
                              ...current,
                              message: event.target.value,
                          ...
                          placeholder="Write a customer-visible portal message"
                          rows={3}
                          className="rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-slate-100 outline-none"
                        />
