...
import { AppwCalculatorCopilot } from '../components/puoa/AppwCalculatorCopilot';
...
import { CalculatorBackendAiReview } from '../components/calculator/CalculatorBackendAiReview';
import {
  WireBreakRiskCard,
...
import { FeatureEditorPanel, contoursToPartFeatures, type PartFeature, type FeatureDimension, type SecondaryOp, type ContourData } from '../components/calculator/FeatureEditorPanel';
...
import {
  STABILITY_OPTIONS,
  WORKHOLDING_BRAND_OPTIONS,
  WORKHOLDING_CATEGORY_OPTIONS,
  WORKHOLDING_PRESET_LIBRARY,
  type WorkholdingPresetOption,
} from '../data/calculatorWorkholding';
...
import {
  buildProgrammingSelectionContext as buildProgrammingSelectionContextFromAuthority,
  buildToolpathTypeOptions as buildToolpathTypeOptionsFromAuthority,
  classifyToolpathType as classifyToolpathTypeFromAuthority,
  filterToolpathsForLicense as filterToolpathsForLicenseFromAuthority,
  getToolpathDefaults as getToolpathDefaultsFromAuthority,
  licenseOptionsFor as licenseOptionsForFromAuthority,
  resolveProgrammingSelectionState as resolveProgrammingSelectionStateFromAuthority,
  type ToolpathDefaults as ProgrammingAuthorityToolpathDefaults,
...
const CalculatorSetupPreview3D = lazyNamed(
  () => import('../components/calculator/CalculatorSetupPreview3D'),
  'CalculatorSetupPreview3D',
);
const CalculatorProgramWorkbench = lazyNamed(
  () => import('../components/calculator/CalculatorProgramWorkbench'),
  'CalculatorProgramWorkbench',
);
const CalculatorSectionPurchaseModal = lazyNamed(
  () => import('../components/calculator/CalculatorSectionPurchaseModal'),
  'CalculatorSectionPurchaseModal',
);
const LatheThreadingPanel = lazyNamed(
  () => import('../components/calculator/LatheThreadingPanel'),
  'LatheThreadingPanel',
);
const LatheInsertSelectorPanel = lazyNamed(
  () => import('../components/calculator/LatheInsertSelectorPanel'),
  'LatheInsertSelectorPanel',
);
const LatheWorkholdingPanel = lazyNamed(
  () => import('../components/calculator/LatheWorkholdingPanel'),
  'LatheWorkholdingPanel',
);
const LatheGroovingPanel = lazyNamed(
  () => import('../components/calculator/LatheGroovingPanel'),
  'LatheGroovingPanel',
);
const LatheHardTurningPanel = lazyNamed(
  () => import('../components/calculator/LatheHardTurningPanel'),
  'LatheHardTurningPanel',
);
const LatheToolLifePanel = lazyNamed(
  () => import('../components/calculator/LatheToolLifePanel'),
  'LatheToolLifePanel',
);
const LatheChatterPanel = lazyNamed(
  () => import('../components/calculator/LatheChatterPanel'),
  'LatheChatterPanel',
);
const LatheCostPanel = lazyNamed(
  () => import('../components/calculator/LatheCostPanel'),
  'LatheCostPanel',
);
...
const WireEdmSurfaceIntegrityPanel = lazyNamed(
  ...
  'WireEdmSurfaceIntegrityPanel',
);
const WireEdmCostBreakdownPanel = lazyNamed(
  () => import('../components/calculator/WireEdmCostBreakdownPanel'),
  'WireEdmCostBreakdownPanel',
);
const WireEdmFeasibilityPanel = lazyNamed(
  () => import('../components/calculator/WireEdmFeasibilityPanel'),
  'WireEdmFeasibilityPanel',
);
const PurchaseRecommendationModal = lazyNamed(
  () => import('../components/shell/PurchaseRecommendationModal'),
  'PurchaseRecommendationModal',
);

function DeferredCalculatorSurface({
  children,
  label = 'Loading panelâ€¦',
  className = '',
}: {
  children: ReactNode;
  label?: string;
  ...
  return (
    <Suspense
      fallback={
        <div
          className={`rounded-[22px] border border-white/10 bg-[#081018] px-4 py-4 text-sm text-slate-400 ${className}`.trim()}
        >
          {label}
        </div>
      }
...
type CalculatorWizardLaunchState = {
  sourceLabel: string;
  fileName?: string;
  fileRoute?: string;
...
const CALCULATOR_HELP_TOPICS: Record<HelpTopicId, CalculatorHelpTopic> = {
  ...
  'prism-flow': {
    ...
    bullets: [
      ...
      'The goal is a practical process plan built from the live setup posture, not just a feed-and-speed guess.',
    ],
  },
  'machine-selection': {
    badge: 'Machine truth',
  ...
  programming: {
    ...
    bullets: [
      ...
      'Use the package that is closest to the one you will actually post or prove out, and treat curated matches as advisory until the live programming registry is wired.',
    ],
  },
  'tooling-fixture': {
    badge: 'Tooling stack',
  ...
  'cutting-results': {
    ...
    summary: 'This is the calculatorâ€™s release lane for speed, feed, load, finish, and warnings.',
    bullets: [
      ...
      'This panel should answer â€œcan I trust this?â€ before it answers â€œhow fast can I run?â€',
    ],
  },
  'my-shop': {
    badge: 'Shop memory',
...
interface CalculatorSolvePreflightState {
  notes: string[];
...
const EMPTY_SOLVE_PREFLIGHT_STATE: CalculatorSolvePreflightState = {
  notes: [],
  machineLimitingFactors: [],
  toolLimitingFactors: [],
  inventoryCoveragePct: null,
  inventoryMissingCount: 0,
};

function asPreflightRecord(value: unknown): Record<string, unknown> | null {
  return value != null && typeof value === 'object' && !Array.isArray(value)
    ? (value as Record<string, unknown>)
    : null;
}

function unwrapPreflightPayload(payload: unknown) {
  const envelope = asPreflightRecord(payload);
  if (!envelope) return null;
  const nested = asPreflightRecord(envelope.result) ?? envelope;
  return asPreflightRecord(nested.value) ?? nested;
}

function readPreflightNumber(record: Record<string, unknown> | null, ...keys: string[]) {
  if (!record) return undefined;
  for (const key of keys) {
    const raw = record[key];
    if (typeof raw === 'number' && Number.isFinite(raw)) return raw;
    const nested = asPreflightRecord(raw);
    if (nested && typeof nested.value === 'number' && Number.isFinite(nested.value)) {
      return nested.value;
    }
  }
  return undefined;
...
function readPreflightStrings(value: unknown) {
  if (!Array.isArray(value)) return [];
  return value
    .map((entry) => {
      if (typeof entry === 'string') return entry;
      const record = asPreflightRecord(entry);
      if (!record) return null;
      if (typeof record.constraint === 'string' && typeof record.parameter === 'string') {
        return `${record.parameter}: ${record.constraint}`;
      }
...
function mapInventoryFeatureType(
  featureType: ToolRoiAnalysisParams['feature']['type'],
): 'hole' | 'pocket' | 'slot' | 'face' | 'contour' | 'thread' | 'drill' | 'bore' | 'chamfer' {
  switch (featureType) {
    case 'drill':
      return 'drill';
    case 'thread':
...
function buildInventorySelectionFeature(input: {
  featureType: ToolRoiAnalysisParams['feature']['type'];
  ...
  const widthMm = Math.max(input.wocMm || 0, input.toolDiameterMm || 0, 0.5);
  const depthMm = Math.max(input.docMm || 0, 0.5);
  const lengthMm = Math.max(input.stockXm || 0, input.stockYm || 0, input.stockZm || 0, input.toolDiameterMm || 0, 1);
  const mappedType = mapInventoryFeatureType(input.featureType);

  return {
    id: 'active-operation',
    type: mappedType,
    diameter_mm: mappedType === 'drill' || mappedType === 'thread' || mappedType === 'bore' ? Math.max(input.toolDiameterMm || 0, 0.5) : undefined,
    width_mm: mappedType === 'pocket' || mappedType === 'slot' || mappedType === 'contour' || mappedType === 'face' ? widthMm : undefined,
    depth_mm: depthMm,
    length_mm: lengthMm,
    tolerance_mm: inferToleranceMm(input.finishTarget, input.desiredRaUm),
    surface_finish_Ra: input.desiredRaUm,
    material: input.materialName,
  };
}

function buildSolvePreflightState(input: {
  machinePayload: unknown;
  toolPayload: unknown;
  inventoryPayload: unknown;
}): CalculatorSolvePreflightState {
  const machineRecord = unwrapPreflightPayload(input.machinePayload);
  const toolRecord = unwrapPreflightPayload(input.toolPayload);
  const inventoryRecord = unwrapPreflightPayload(input.inventoryPayload);

  const machineLimitingFactors = readPreflightStrings(machineRecord?.limiting_factors).slice(0, 2);
  const toolLimitingFactors = readPreflightStrings(toolRecord?.limiting_factors).slice(0, 2);
  const toolLifeMin = readPreflightNumber(toolRecord, 'tool_life_min');
  const deflectionUm = readPreflightNumber(toolRecord, 'deflection_um');
  const inventoryCoveragePct = readPreflightNumber(inventoryRecord, 'inventory_coverage_pct') ?? null;
  const missingTools = Array.isArray(inventoryRecord?.missing_tools) ? inventoryRecord.missing_tools : [];
  const missingToolLabels = missingTools
    .map((entry) => {
      const record = asPreflightRecord(entry);
      if (!record) return null;
      const recommendedType = typeof record.recommended_type === 'string'
        ? humanizeToken(record.recommended_type)
        : 'compatible tooling';
      const diameterMm = typeof record.recommended_diameter_mm === 'number'
        ? ` ${record.recommended_diameter_mm.toFixed(1)} mm`
        : '';
      return `${recommendedType}${diameterMm}`;
    })
    .filter((entry): entry is string => Boolean(entry));

  const notes: string[] = [];

  if (machineLimitingFactors.length) {
    notes.push(`Machine preflight limits: ${machineLimitingFactors.join(' / ')}.`);
  }

  if (toolLimitingFactors.length) {
    notes.push(`Tool preflight limits: ${toolLimitingFactors.join(' / ')}.`);
  } else if ((deflectionUm ?? 0) >= 25) {
    notes.push(`Tool preflight: predicted deflection ${deflectionUm?.toFixed(1)} um is high for the current reach and load.`);
  } else if ((toolLifeMin ?? Number.POSITIVE_INFINITY) < 10) {
    notes.push(`Tool preflight: predicted tool life ${toolLifeMin?.toFixed(1)} min is short for production use.`);
  }

  if (inventoryCoveragePct != null) {
    if (missingToolLabels.length) {
      const lead = missingToolLabels.slice(0, 2).join(' / ');
      notes.push(`Inventory preflight: ${inventoryCoveragePct.toFixed(1)}% of this operation is covered by active tooling; add ${lead}${missingToolLabels.length > 2 ? ' +' : ''}.`);
    } else if (inventoryCoveragePct < 100) {
      notes.push(`Inventory preflight: ${inventoryCoveragePct.toFixed(1)}% of this operation is covered by active tooling.`);
    } else {
      notes.push('Inventory preflight: the active tooling stack covers this operation end to end.');
    }
  }
  ...
  return {
    ...
    inventoryMissingCount: missingToolLabels.length,
  };
}
...
type CalculatorCatalogStatus = Pick<
  ...
  'source' | 'liveCount' | 'fallbackCount' | 'note' | 'authority' | 'sampled'
>;
...
function toCatalogStatus(
  state: Pick<CalculatorCatalogLoadState<unknown>, 'source' | 'liveCount' | 'fallbackCount' | 'note' | 'authority' | 'sampled'>,
): CalculatorCatalogStatus {
  return {
    source: state.source,
...
function CalculatorSourcePostureCard({
  label,
  detail,
  statusLabel,
  toneClass,
  ...
  return (
    <div className="rounded-2xl border border-slate-700/50 bg-[#081320] px-4 py-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
      ...
      <div className="mt-3 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">{metaLabel}</div>
    </div>
  );
}
...
// â”€â”€ Feature Editor Section (lathe + wire modes) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

function LatheFeatureEditorSection({ mode, contours }: { mode: string; contours?: ContourData[] }) {
  const defaultLatheFeatures: PartFeature[] = useMemo(() => [
    {
      id: 'f1', type: 'OD_STRAIGHT', label: 'OD Step 1', icon: '\u2B24',
      dimensions: [
        { key: 'od_mm', label: 'OD', value: 50, unit: 'mm', min: 1, step: 0.1, editable: true },
        { key: 'length_mm', label: 'Length', value: 40, unit: 'mm', min: 0.5, step: 0.1, editable: true },
      ],
      secondary_ops: [], position_z_mm: 0, tolerance_mm: 0.05, surface_finish_Ra_um: 3.2,
    },
    {
      id: 'f2', type: 'OD_STEP', label: 'OD Step 2', icon: '\u2592',
      dimensions: [
        { key: 'od_mm', label: 'OD', value: 35, unit: 'mm', min: 1, step: 0.1, editable: true },
        { key: 'length_mm', label: 'Length', value: 30, unit: 'mm', min: 0.5, step: 0.1, editable: true },
      ],
      secondary_ops: [], position_z_mm: 40, tolerance_mm: 0.02, surface_finish_Ra_um: 1.6,
    },
  ], []);

  const initialFeatures = useMemo(() => {
    if (mode === 'wire_edm' && contours && contours.length > 0) {
      return contoursToPartFeatures(contours);
    }
    return defaultLatheFeatures;
  }, [mode, contours, defaultLatheFeatures]);

  const [features, setFeatures] = useState<PartFeature[]>(initialFeatures);
  const [selectedFeatureId, setSelectedFeatureId] = useState<string | null>(null);

  // Sync features when contours change (e.g., user uploads a new DXF)
  const prevContoursRef = useRef(contours);
  useEffect(() => {
    if (contours !== prevContoursRef.current && mode === 'wire_edm' && contours && contours.length > 0) {
      setFeatures(contoursToPartFeatures(contours));
      setSelectedFeatureId(null);
      prevContoursRef.current = contours;
    }
  }, [contours, mode]);

  const handleFeatureUpdate = useCallback((featureId: string, dims: FeatureDimension[]) => {
    setFeatures(prev => prev.map(f => f.id === featureId ? { ...f, dimensions: dims } : f));
  }, []);
  const handleSecondaryOpChange = useCallback((featureId: string, ops: SecondaryOp[]) => {
    setFeatures(prev => prev.map(f => f.id === featureId ? { ...f, secondary_ops: ops } : f));
  }, []);
  const handleFeatureAdd = useCallback((feature: PartFeature) => {
    setFeatures(prev => [...prev, feature]);
  }, []);
  const handleFeatureRemove = useCallback((featureId: string) => {
    setFeatures(prev => prev.filter(f => f.id !== featureId));
    setSelectedFeatureId(null);
  }, []);

  const stockOD = Math.max(...features.map(f => f.dimensions.find(d => d.key === 'od_mm')?.value ?? 50), 50);
  const partLen = features.reduce((sum, f) => sum + (f.dimensions.find(d => d.key === 'length_mm')?.value ?? 20), 0);

  return (
    <div className="col-span-full mt-3 space-y-3">
      {mode === 'lathe' && (
        <DeferredCalculatorSurface label="Loading lathe sketchâ€¦" className="min-h-[220px]">
          <LatheSketch2D
            features={features}
            selectedFeatureId={selectedFeatureId}
            onFeatureSelect={setSelectedFeatureId}
            stockOD_mm={stockOD + 5}
            partLength_mm={partLen}
          />
        </DeferredCalculatorSurface>
      )}
      {mode === 'wire_edm' && features.length > 0 && (
        <div className="bg-zinc-900/80 border border-zinc-700/50 rounded-lg p-3">
          <div className="text-[10px] text-zinc-500 uppercase tracking-wide mb-2">
            Wire EDM Contours ({features.length} features mapped)
          </div>
          <div className="flex gap-2 flex-wrap">
            {features.map(f => (
              ...
              >
                {f.label} â€” {f.dimensions.find(d => d.key === 'perimeter_mm')?.value?.toFixed(1) ?? '?'}mm
              </button>
            ))}
          </div>
        </div>
      ...
      <FeatureEditorPanel
        ...
        onSecondaryOpChange={handleSecondaryOpChange}
...
const LATHE_OPERATIONS = [
  ...
  { id: 'hard_turning', label: 'Hard turning' },
  { id: 'parting', label: 'Part-off / cutoff' },
];
...
function toolingInterfaceAliases(value?: string) {
  ...
  switch (normalized) {
    ...
    case 'hsk-a50':
    case 'hsk_a50':
      return ['hsk-a50', 'hsk_a50'];
    case 'hsk-c40':
    case 'hsk_c40':
      return ['hsk-c40', 'hsk_c40'];
    case 'hsk-e40':
    case 'hsk_e40':
      return ['hsk-e40', 'hsk_e40'];
    case 'bmt45':
    case 'bmt-45':
      return ['bmt45', 'bmt-45'];
    case 'bmt55':
...
function normalizeToolingInterfaceId(value: string, fallback: string): string {
  ...
  if (/iso\s*20/i.test(normalized)) return 'iso20';
  if (/bt\s*30/i.test(normalized)) return 'bt30';
  if (/bt\s*50/i.test(normalized)) return 'bt50';
  if (/hsk[\s-]*a\s*50/i.test(normalized)) return 'hsk-a50';
  if (/hsk[\s-]*a\s*63/i.test(normalized)) return 'hsk-a63';
  if (/hsk[\s-]*c\s*40/i.test(normalized)) return 'hsk-c40';
  if (/hsk[\s-]*e\s*40/i.test(normalized)) return 'hsk-e40';
...
export function holderPackageMatchesTool(
  holderPackage: HolderPackageOption,
  tool?: Pick<ToolCatalogItem, 'id' | 'geometryClass' | 'bodyType' | 'defaultDiameter'> | string | null,
) {
  const selectedToolId = typeof tool === 'string' ? tool : tool?.id;
  if (holderPackage.toolId && selectedToolId && holderPackage.toolId !== selectedToolId) return false;

  const toolRecord = typeof tool === 'string' ? null : tool;
  if (
    holderPackage.compatibleGeometryClasses?.length
    && toolRecord?.geometryClass
    && !holderPackage.compatibleGeometryClasses.includes(toolRecord.geometryClass)
  ) {
    return false;
  }
  if (
    holderPackage.maxToolDiameterMm !== undefined
    && toolRecord?.defaultDiameter !== undefined
    && toolRecord.defaultDiameter > holderPackage.maxToolDiameterMm
  ) {
    return false;
  }
  ...
  const holderSignature = [
    ...
    holderPackage.label,
    holderPackage.detail,
    holderPackage.toolInterface,
  ]
    .filter(Boolean)
    .join(' ')
    .toLowerCase();
  const isShellArborHolder = /shell|arbor|face\s*mill|indexable/.test(holderSignature);

  if (toolRecord?.geometryClass === 'face-mill') {
    return isShellArborHolder;
  }
  if (isShellArborHolder && toolRecord?.geometryClass) {
    return false;
  }
...
function defaultHolderPackageForMachine(
  ...
  if (!profile) {
    ...
    if (machineMode === 'wire_edm') return 'fanuc-wire-standard';
    if (machineMode === 'edm') return 'th-jmd-system3r-er32';
    return '';
  }
  ...
  if (machineMode === 'mill') {
    switch (profile.spindleConnectionTypeId) {
      case 'bt40':
        ...
        return 'regofix-er-collet';
      case 'bt30':
        return 'haimer-bt30-micro-collet';
      case 'iso20':
        return toolId === 'micro-endmill' ? 'haas-iso20-micro-collet' : 'haas-iso20-er-utility';
      case 'cat50':
        if (toolId === 'face-mill') return 'sandvik-cat50-shell-arbor';
        if (toolId === 'finisher') return 'haimer-shrink-mill';
        if (toolId === 'adaptive-endmill') return 'haimer-hydraulic-rough';
        return 'regofix-er-collet';
      case 'bt50':
      ...
      case 'hsk-a50':
        return '';
      case 'hsk-c40':
        return toolId === 'micro-endmill' ? 'rokuroku-hsk-c40-micro-collet' : 'rokuroku-hsk-c40-high-speed-collet';
      case 'hsk-e40':
        return '';
      case 'hsk-a63':
  ...
  if (machineMode === 'wire_edm') {
    return 'fanuc-wire-standard';
  }

  if (machineMode === 'edm') {
    return 'th-jmd-system3r-er32';
  }
...
const MACHINE_PREVIEW_PALETTE: Record<
  MachineMode,
  {
    accent: string;
    accentSoft: string;
...
function PrismHeaderMark({
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={`${idPrefix}-endmill-body`} x1="0%" y1="0%" x2="100%" y2="0%">
          <stop offset="0%" stopColor="#8a4a04" />
        ...
        <linearGradient id={`${idPrefix}-wedge-main`} x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#22d3ee" />
          <stop offset="58%" stopColor="#60a5fa" />
          <stop offset="100%" stopColor="#7c3aed" />
        </linearGradient>
        <linearGradient id={`${idPrefix}-wedge-lower`} x1="0%" y1="0%" x2="100%" y2="100%">
      ...
      <g opacity="0.99">
        <rect x="20.8" y="12.8" width="9.8" height="45.4" rx="0.7" fill={`url(#${idPrefix}-endmill-body)`} stroke="rgba(125, 211, 252, 0.82)" strokeWidth="0.82" />
        <path d="M21 34.2H30.4" stroke="rgba(125, 211, 252, 0.78)" strokeWidth="0.72" strokeLinecap="square" />
        <path d="M21.1 37.3C24.4 37.5 27.2 38.5 30.2 41.8" stroke="rgba(125, 211, 252, 0.98)" strokeWidth="1.02" strokeLinecap="round" fill="none" />
        <path d="M21.1 43C24.4 43.2 27.2 44.2 30.2 47.5" stroke="rgba(125, 211, 252, 0.98)" strokeWidth="1.02" strokeLinecap="round" fill="none" />
        <path d="M21.1 48.7C24.4 48.9 27.2 49.9 30.2 53.2" stroke="rgba(125, 211, 252, 0.98)" strokeWidth="1.02" strokeLinecap="round" fill="none" />
        <path d="M21.1 54.4C24.4 54.6 27.2 55.6 30.2 58.9" stroke="rgba(125, 211, 252, 0.98)" strokeWidth="1.02" strokeLinecap="round" fill="none" />
        <path d="M24.6 17.3v12.7" stroke="rgba(255,255,255,0.16)" strokeWidth="0.56" strokeLinecap="round" />
      </g>
      <g opacity="0.99">
        <path d="M36.5 15.1 36.5 35.7 53.8 26.7Z" fill={`url(#${idPrefix}-wedge-main)`} stroke="rgba(125, 211, 252, 0.92)" strokeWidth="0.94" strokeLinejoin="round" />
        <path d="M36.5 35.7 53.8 26.7 42.9 44.7Z" fill={`url(#${idPrefix}-wedge-lower)`} stroke="rgba(125, 211, 252, 0.9)" strokeWidth="0.94" strokeLinejoin="round" />
        <path d="M36.5 15.1 36.5 35.7" stroke="rgba(224,242,254,0.94)" strokeWidth="1" strokeLinecap="round" />
        <path d="M36.5 25.4 53.8 26.7" stroke="rgba(224,242,254,0.16)" strokeWidth="0.7" strokeLinecap="round" />
      </g>
    </svg>
  );
}

function MachinePreviewIllustration({ mode }: { mode: MachineMode }) {
...
const MACHINE_MODE_PREVIEW_MEDIA: Record<MachineMode, MachineModePreviewMedia> = {
  mill: {
    kind: 'video',
    src: '/media/machine-modes/real/mill-5axis-cut-loop.mp4',
    mimeType: 'video/mp4',
    poster: '/media/machine-modes/real/mill-5axis-cut-loop-poster.jpg',
  },
  lathe: {
    kind: 'video',
    src: '/media/machine-modes/real/lathe-cnc-real.webm',
    mimeType: 'video/webm',
    poster: '/media/machine-modes/real/lathe-bore-real.jpg',
  },
  edm: {
    kind: 'video',
    src: '/media/machine-modes/real/edm-sinker-running-loop.mp4',
    mimeType: 'video/mp4',
    poster: '/media/machine-modes/real/edm-sinker-running-loop-poster.jpg',
  },
  wire_edm: {
    kind: 'video',
    src: '/media/machine-modes/real/wire-edm-running-loop.mp4',
    mimeType: 'video/mp4',
    poster: '/media/machine-modes/real/wire-edm-running-loop-poster.jpg',
  },
  laser: {
    kind: 'video',
    src: '/media/machine-modes/real/laser-real.webm',
    mimeType: 'video/webm',
    poster: '/media/machine-modes/real/laser-real.jpg',
  },
  waterjet: {
    kind: 'video',
    src: '/media/machine-modes/real/waterjet-cutting-loop.mp4',
    mimeType: 'video/mp4',
    poster: '/media/machine-modes/real/waterjet-cutting-loop-poster.jpg',
  },
};

function MachineModeMediaPreview({ mode }: { mode: MachineMode }) {
  const label = MACHINE_MODE_OPTIONS.find((item) => item.id === mode)?.label ?? mode;
  const media = MACHINE_MODE_PREVIEW_MEDIA[mode];

  return (
    <div className="calculator-machine-mode-preview-media-frame" data-testid={`machine-mode-media-${mode}`}>
      {media.kind === 'video' ? (
        <video
          className="calculator-machine-mode-preview-media"
          src={media.src}
          poster={media.poster}
          autoPlay
          muted
          ...
          aria-label={`${label} machine preview`}
        >
...
export function CalculatorPage() {
  ...
  const programWorkbenchPanelRef = useRef<HTMLDivElement | null>(null);
  ...
  const [solvePreflight, setSolvePreflight] = useState<CalculatorSolvePreflightState>(EMPTY_SOLVE_PREFLIGHT_STATE);
  ...
  const [workholdingCatalogStatus, setWorkholdingCatalogStatus] = useState<CalculatorCatalogStatus>(DEFAULT_CATALOG_STATUS);
  ...
  const [liveWorkholdingCategoryOptions, setLiveWorkholdingCategoryOptions] = useState<SelectionOption[]>([]);
  const [liveWorkholdingBrandOptions, setLiveWorkholdingBrandOptions] = useState<SelectionOption[]>([]);
  const [liveWorkholdingPresetOptions, setLiveWorkholdingPresetOptions] = useState<WorkholdingPresetOption[]>([]);
  const [liveStabilityOptions, setLiveStabilityOptions] = useState<SelectionOption[]>(STABILITY_OPTIONS);
  ...
  const invalidateSolveState = useCallback((nextError: string | null = null) => {
    ...
    setSolvePreflight(EMPTY_SOLVE_PREFLIGHT_STATE);
    setError(nextError);
  }, []);
  ...
  useEffect(() => {
    ...
    fetchProgrammingCatalogState(machineMode).then((catalogState) => {
      ...
      if (!cancelled) {
        ...
        const authority = buildJMDieProgrammingCatalogAuthority({
          mode: machineMode,
          items: fallbackItems,
          liveCount: 0,
          fallbackCount: fallbackItems.length,
          stage: fallbackItems.length ? 'database-unavailable' : 'empty',
        });
    ...
    fetchWorkholdingCatalogState(machineMode).then((catalogState) => {
      if (!cancelled) {
        setLiveWorkholdingCategoryOptions(catalogState.bundle.categoryOptions);
        setLiveWorkholdingBrandOptions(catalogState.bundle.brandOptions);
        setLiveWorkholdingPresetOptions(catalogState.bundle.presetOptions);
        setLiveStabilityOptions(catalogState.bundle.stabilityOptions);
        setWorkholdingCatalogStatus(toCatalogStatus(catalogState));
      }
    }).catch(() => {
      if (!cancelled) {
        setLiveWorkholdingCategoryOptions(WORKHOLDING_CATEGORY_OPTIONS[machineMode] ?? []);
        setLiveWorkholdingBrandOptions(WORKHOLDING_BRAND_OPTIONS[machineMode] ?? []);
        setLiveWorkholdingPresetOptions(WORKHOLDING_PRESET_LIBRARY.filter((item) => item.modes.includes(machineMode)));
        setLiveStabilityOptions(STABILITY_OPTIONS);
        setWorkholdingCatalogStatus({
          source: 'fallback',
          liveCount: 0,
          fallbackCount: WORKHOLDING_PRESET_LIBRARY.filter((item) => item.modes.includes(machineMode)).length,
          note: 'Workholding route is unavailable, so the local curated workholding catalog is active.',
          sampled: false,
        });
      }
    });
  ...
  const workholdingCatalogBadge = workholdingCatalogStatus.note.includes('Backend-served')
    ? 'Curated workholding service'
    : workholdingCatalogStatus.source === 'fallback'
      ? 'Fallback workholding catalog'
      : workholdingCatalogStatus.source === 'live'
        ? 'Live workholding catalog'
        : 'No workholding catalog';
  const workholdingCatalogSummary = workholdingCatalogStatus.source === 'hybrid'
    ? `${workholdingCatalogStatus.liveCount.toLocaleString()} live + ${workholdingCatalogStatus.fallbackCount.toLocaleString()} curated`
    : workholdingCatalogStatus.source === 'live'
      ? `${workholdingCatalogStatus.liveCount.toLocaleString()} live workholding packages loaded`
      : `${(workholdingCatalogStatus.fallbackCount || liveWorkholdingPresetOptions.length || WORKHOLDING_PRESET_LIBRARY.filter((item) => item.modes.includes(machineMode)).length).toLocaleString()} curated presets loaded`;
  ...
  const holderCompatibilityTool =
    liveTools.find((item) => item.id === toolId)
    ?? TOOL_CATALOG.find((item) => item.id === toolId)
    ?? toolId;
  const fallbackCompatibleHolderPackages = HOLDER_PACKAGE_LIBRARY.filter(
    (item) =>
      item.mode === machineMode
      && holderPackageMatchesMachine(item, holderCompatibilityMachine)
      && holderPackageMatchesTool(item, holderCompatibilityTool),
  );
  ...
  const holderCatalogSource: CalculatorCatalogSourceState = catalogCompatibleHolderPackages.length
    ...
    : compatibleHolderPackages.length
      ? 'fallback'
      : 'empty';
  const holderCatalogUsesJMDieSeed = compatibleHolderPackages.some((item) => isJMDieCanonicalHolderPackage(item));
  ...
  const compatibleHolderPackageSignature = compatibleHolderPackages.map((item) => item.id).join('|');
  const holderPackagesForModeSignature = holderPackagesForMode.map((item) => item.id).join('|');
  ...
  const holderCatalogBadge = holderCatalogSource === 'live'
    ...
    : holderCatalogSource === 'hybrid'
      ? (holderCatalogUsesJMDieSeed ? 'Hybrid + JM Die seed' : 'Hybrid holder catalog')
      : holderCatalogSource === 'fallback'
        ? (holderCatalogUsesJMDieSeed ? 'JM Die seeded holders' : 'Fallback holder library')
  ...
  const holderCatalogSummary = holderCatalogSource === 'hybrid'
    ? holderCatalogUsesJMDieSeed
      ? `${holderCatalogStatus.liveCount.toLocaleString()} live + ${holderCatalogStatus.fallbackCount.toLocaleString()} JM Die seeded`
    ...
    : holderCatalogSource === 'live'
      ...
      : holderCatalogUsesJMDieSeed
        ? `${(compatibleHolderPackages.length || holderCatalogStatus.fallbackCount).toLocaleString()} JM Die seeded compatible packages`
  ...
  const programmingCatalogBadge = programmingCatalogStatus.authority?.badge ?? (
  ...
  const programmingCatalogSummary = programmingCatalogStatus.authority?.summary ?? (
    programmingCatalogStatus.source === 'hybrid'
  ...
  const programmingSelectionContext = buildProgrammingSelectionContextFromAuthority({
  ...
  const activeLicenseTierId = programmingSelectionContext.licenseTierId;
  const activeToolpathTypeId = programmingSelectionContext.toolpathTypeId;
  const activeToolpathId = programmingSelectionContext.toolpathId;
  const licensedToolpathOptions = programmingSelectionContext.licensedToolpathOptions;
  const toolpathTypes = programmingSelectionContext.toolpathTypes;
  const selectedToolpathType = programmingSelectionContext.selectedToolpathType;
  const filteredToolpathOptions = programmingSelectionContext.filteredToolpathOptions;
  const selectedToolpath = programmingSelectionContext.selectedToolpath;
  ...
  const backendWiringStatusCards: Array<{
    ...
    },
    {
      id: 'workholding',
      label: 'Workholding',
      badge: workholdingCatalogBadge,
      summary: workholdingCatalogSummary,
      note: workholdingCatalogStatus.note,
      source: effectiveCatalogSource(workholdingCatalogStatus),
    },
    {
      id: 'tooling',
      label: 'Tooling',
  ...
  const liveBackboneCount = backendWiringStatusCards.filter((item) => item.source === 'live').length;
  const hybridBackboneCount = backendWiringStatusCards.filter((item) => item.source === 'hybrid').length;
  const fallbackBackboneCount = backendWiringStatusCards.filter(
  ...
  const workholdingCategoryOptions = liveWorkholdingCategoryOptions.length
    ? liveWorkholdingCategoryOptions
    : WORKHOLDING_CATEGORY_OPTIONS[machineMode];
  const workholdingBrandOptions = liveWorkholdingBrandOptions.length
    ? liveWorkholdingBrandOptions
    : WORKHOLDING_BRAND_OPTIONS[machineMode];
  const workholdingPresetLibraryForMode = liveWorkholdingPresetOptions.length
    ? liveWorkholdingPresetOptions
    : WORKHOLDING_PRESET_LIBRARY.filter((item) => item.modes.includes(machineMode));
  const workholdingPresetsForMode = workholdingPresetLibraryForMode.filter(
    (item) =>
  ...
  const selectedWorkholdingPreset =
    ...
    workholdingPresetLibraryForMode.find((item) => item.id === workholdingPresetId && item.modes.includes(machineMode)) ??
    workholdingPresetsForMode[0];
  const stabilityOptions = liveStabilityOptions.length ? liveStabilityOptions : STABILITY_OPTIONS;
  const selectedStability = stabilityOptions.find((item) => item.id === stabilityId) ?? stabilityOptions[0];
  ...
  const calculatorRouteAuthority = useMemo(
    () => buildJMDieCalculatorRouteAuthority({
      machineMode,
      machine: selectedMachine,
      material: selectedMaterial,
    }),
  ...
  const programReleasePath = buildWorkflowPath('/print-to-cnc', location.search, {
    ...
    extras: {
      machineId: calculatorRouteAuthority.routeSelection.machineId,
      machineFamilyId: calculatorRouteAuthority.routeSelection.machineFamilyId,
      machineManufacturer: calculatorRouteAuthority.routeSelection.machineManufacturer,
      toolholderId: calculatorRouteAuthority.routeSelection.toolholderId,
      toolingPackageId: calculatorRouteAuthority.routeSelection.toolingPackageId,
      fixtureId: calculatorRouteAuthority.routeSelection.fixtureId,
      stockId: calculatorRouteAuthority.routeSelection.stockId,
      cadSourceId: calculatorRouteAuthority.routeSelection.cadSourceId,
      partClassId: calculatorRouteAuthority.routeSelection.partClassId,
      material: selectedMaterial?.name,
    },
  });
  const toolpathAdvisorPath = calculatorRouteAuthority.toolpathSupported
    ...
    : null;
  const pipelinePath = buildWorkflowPath('/pipeline', location.search, {
    origin: calculatorOrigin,
    focus: {
      type: 'packet',
      id: calculatorPacketId,
  ...
  const guidedWizardPath =
    machineMode === 'mill'
      ? buildWorkflowPath('/milling/wizard', location.search, {
        origin: calculatorOrigin,
        focus: {
          type: 'packet',
          id: calculatorPacketId,
      ...
      : machineMode === 'lathe'
        ? buildWorkflowPath('/lathe/wizard', location.search, {
          origin: calculatorOrigin,
          focus: {
            type: 'packet',
            id: calculatorPacketId,
        ...
        : machineMode === 'wire_edm'
          ? buildWorkflowPath('/wire-edm/wizard', location.search, {
            origin: calculatorOrigin,
            focus: {
              type: 'packet',
              id: calculatorPacketId,
          ...
          : null;
  const calculatorWizardLaunchState = useMemo<CalculatorWizardLaunchState>(() => ({
    sourceLabel: 'calculator studio',
    fileName: loadedFileName ?? undefined,
    fileRoute: '/calculator',
    extractedData: undefined,
    materialId: selectedMaterial?.id ?? materialId,
    materialName: selectedMaterial?.name ?? selectedMaterial?.id ?? materialId,
    machineId: selectedMachine?.id,
    machinePreference: selectedMachine?.model ?? selectedMachine?.id ?? MACHINE_MODE_OPTIONS.find((option) => option.id === machineMode)?.label ?? machineMode,
    controllerId: selectedControllerOption?.id ?? controllerOptionId,
    operation,
    stockDiameterMm: stockX,
    stockLengthMm: stockY,
    stockThicknessMm: Math.max(doc || 0, stockZ || 0),
    notes: [
      'Launched from Calculator Studio.',
      `Machine: ${selectedMachine?.model ?? MACHINE_MODE_OPTIONS.find((option) => option.id === machineMode)?.label ?? machineMode}.`,
      `Material: ${selectedMaterial?.name ?? materialId}.`,
      `Programming: ${selectedProgramming?.label ?? 'unresolved'}${selectedToolpath?.label ? ` / ${selectedToolpath.label}` : ''}.`,
      `Workflow packet: ${calculatorPacketId}.`,
    ].join(' '),
    workspaceContext: {
      machineMode,
      machineLabel: selectedMachine?.model ?? selectedMachine?.id,
      machineId: selectedMachine?.id,
      machineManufacturer: selectedMachine?.manufacturer ?? manufacturer,
      controllerId: selectedControllerOption?.id ?? controllerOptionId,
      controllerLabel: selectedControllerOption?.label ?? selectedMachine?.controller,
      materialName: selectedMaterial?.name ?? selectedMaterial?.id ?? materialId,
      materialGroup,
      programmingLabel: selectedProgramming?.label,
      toolpathLabel: selectedToolpath?.label,
      packetId: calculatorPacketId,
      focusId: calculatorFocusId,
    },
  }), [
    calculatorFocusId,
    calculatorPacketId,
    ...
    materialId,
    operation,
    selectedControllerOption?.id,
    selectedControllerOption?.label,
    selectedMachine?.controller,
    selectedMachine?.id,
    selectedMachine?.manufacturer,
    selectedMachine?.model,
    selectedMaterial?.id,
    selectedMaterial?.name,
    selectedProgramming?.label,
    selectedToolpath?.label,
    stockX,
    stockY,
    stockZ,
  ]);
  ...
  useEffect(() => {
    const nextHolderPackageId = holderPackagesForMode[0]?.id ?? '';
    if (!holderPackagesForMode.some((item) => item.id === holderPackageId) && holderPackageId !== nextHolderPackageId) {
      setHolderPackageId(nextHolderPackageId);
    }
  }, [machineMode, holderBrand, holderTypeFilter, holderInterfaceFilter, holderPackageId, holderPackagesForModeSignature]);
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬ Toolpath-aware auto-adjustment Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // Mill uses multiplier Ãƒâ€” toolDiameter; lathe uses absolute mm values.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const nextHolderPackageId = defaultHolderPackageForMachine(machineMode, holderCompatibilityMachine, selectedTool);
      ...
      setSetupSource('recommended');
      setHolderBrand('all');
      setHolderPackageId(nextHolderPackageId || 'fanuc-wire-standard');
      setHolderStyle(holderPackagePrimaryStyle(nextHolderPackage));
      ...
      setWorkholdingCategory('all');
      setWorkholdingBrand('all');
      setWorkholdingPresetId('system3r-reference-pallet');
      setStockShape('plate');
      setStockSource('from-model');
      ...
      setSetupSource('recommended');
      setHolderBrand('three-r');
      setHolderPackageId(nextHolderPackageId || 'th-jmd-system3r-er32');
      setHolderStyle(holderPackagePrimaryStyle(nextHolderPackage));
      setStabilityId('detail-control');
      setDoc(0.15);
      setWoc(1.5);
  ...
  useEffect(() => {
    ...
    const nextHolderPackage = compatibleHolderPackages.find((item) => item.id === nextHolderPackageId);
    ...
    setHolderStyle(holderPackagePrimaryStyle(nextHolderPackage));
  }, [machineMode, selectedMachine?.id, selectedTool?.id, selectedToolpath?.id, holderPackageId, compatibleHolderPackageSignature]);
  ...
  async function runCalculation() {
    ...
    try {
      // Ã¢â€â‚¬Ã¢â€â‚¬ Wire EDM mode: use dedicated 6-engine orchestrator Ã¢â€â‚¬Ã¢â€â‚¬
      if (machineMode === 'wire_edm') {
      ...
      // Ã¢â€â‚¬Ã¢â€â‚¬ Milling / Lathe / other modes: standard speed-feed path Ã¢â€â‚¬Ã¢â€â‚¬
      ...
      const activeToolInventory = selectedTool
        ? [{
            id: `active-${selectedTool.id}`,
            name: selectedTool.label,
            type: inferToolInventoryType(selectedTool),
            diameter_mm: Math.max(toolDiameter || 0, selectedTool.defaultDiameter || 0, 0.5),
            flutes: Math.max(flutes || 0, selectedTool.defaultFlutes || 0, 1),
            material: inferToolInventoryMaterial(selectedTool),
            coating: selectedTool.coating,
            max_depth_mm: Math.max(doc || 0, selectedTool.defaultDiameter * 1.5, 0.5),
            corner_radius_mm: selectedTool.cornerRadiusMm,
      ...
      for (const item of activeToolInventory) {
        if (!inventoryCandidates.some((candidate) => candidate.id === item.id || candidate.name === item.name)) {
          inventoryCandidates.push(item);
        }
      }
      const preflightPromise = Promise.all([
        sfResolveMachine(params).catch(() => null),
        sfResolveTool(params).catch(() => null),
        inventoryCandidates.length
          ? sfInventoryToolSelect({
              machinePackage: params.machinePackage,
              machine_name: params.machine_name,
              machine_power_kw: params.machine_power_kw,
              machine_max_rpm: params.machine_max_rpm,
              machine_max_torque_nm: params.machine_max_torque_nm,
              features: [
                buildInventorySelectionFeature({
                  featureType,
                  toolDiameterMm: toolDiameter,
                  docMm: doc,
                  wocMm: woc,
      ...
      try {
        ...
        if (!isSolveCurrent()) return;
        const [machinePreflight, toolPreflight, inventoryPreflight] = await preflightPromise;
        if (!isSolveCurrent()) return;
        setSolvePreflight(
          buildSolvePreflightState({
            machinePayload: machinePreflight,
            toolPayload: toolPreflight,
            inventoryPayload: inventoryPreflight,
          }),
        );
  ...
  function saveCurrentSetupSnapshot() {
    const snapshot = createSetupSnapshot({
      ...
      licenseTierId: activeLicenseTierId,
      toolpathTypeId: selectedToolpathType?.id ?? activeToolpathTypeId,
      toolpathId: selectedToolpath?.id ?? activeToolpathId,
      stockShape,
      stockSource,
      stockX,
      stockY,
  ...
  const workflowSnapshot = [
    ...
    },
    {
      ...
      detail: selectedProgramming
        ? `${selectedProgramming.vendor} / ${labelFor(programmingLicenseOptions, activeLicenseTierId)}`
  ...
  const selectedMachineSummaryMeta = selectedMachine
    ? `${selectedMachine.axes} Â· ${selectedMachine.machineTypeLabel}`
    : 'The machine portrait updates when the selected package changes.';
  const selectedMachineSummaryDetail = selectedMachine
    ? `${selectedMachineConnectionLabel} Â· ${selectedMachinePrimaryStat}`
  ...
  const postProcessorState = useMemo(
    () => buildJMDieCalculatorPostWorkflowState({
      machineMode,
      machine: selectedMachine,
      material: selectedMaterial,
      controllerOption: selectedControllerOption,
      ...
      stockLengthMm: stockY,
      stockThicknessMm: Math.max(doc || 0, stockZ || 0),
      targetRaUm: effectiveDesiredRaUm ?? 0.8,
      programReleasePath,
      routeAuthority: calculatorRouteAuthority,
      programmingAuthority: programmingCatalogStatus.authority,
      programming: selectedProgramming,
      licenseLabel: programmingLicenseOptions.find((option) => option.id === activeLicenseTierId)?.label ?? null,
      toolpathTypeLabel: selectedToolpathType?.label ?? null,
      toolpath: selectedToolpath,
    }),
    [
      activeLicenseTierId,
      calculatorRouteAuthority,
      doc,
      effectiveDesiredRaUm,
      holderStyle,
      machineMode,
      programmingCatalogStatus.authority,
      programmingLicenseOptions,
      programReleasePath,
      selectedControllerOption,
      selectedMachine,
      selectedMaterial,
  ...
  const postProcessorPath = buildWorkflowPath('/ppg', location.search, {
    ...
    extras: {
      machine: selectedMachine?.model,
      controller: selectedControllerOption?.id ?? controllerOptionId,
      machinePosture: postProcessorState.machinePosture,
      camSystem: postProcessorState.camSystem,
      operation: postProcessorState.operation ?? operation,
    },
  });
  ...
  const toolbarOptimizationTone =
    toolbarOptimizationScore >= 90
      ? {
          ...
          detail: 'Inputs, machine posture, and active cut state are aligned enough for a strong handoff.',
  ...
  const calculatorMyShopTruthDetail = loadedFileName
    ...
    : `No active uploaded file. My Shop currently has ${toolCribImportCount} imports and ${toolCribLibraryCount} linked libraries.${toolCribImportSummary ? ` ${toolCribImportSummary}` : ''}${toolCribImportError ? ` Import issue: ${toolCribImportError}` : ''}${toolCribScanError ? ` Scan issue: ${toolCribScanError}` : ''}`;
  const calculatorBackendAiReady = Boolean(
    resultSolveSource
    || solvePreflight.notes.length
    || result
    || wedmResult,
  );
  const calculatorBackendBuildLabel =
    machineMode === 'wire_edm' && wedmResult
      ? `${wedmResult.first_cut_speed_mm_min?.toFixed(1) ?? 'Unavailable'} mm/min first cut`
      : result?.rpm != null
        ? `${Math.round(result.rpm).toLocaleString()} RPM live solve`
        : 'Awaiting live backend solve';
  const calculatorBackendBuildDetail =
    machineMode === 'wire_edm' && wedmResult
      ? `${wedmResult.passes?.length ?? 0} passes planned for ${selectedMachine?.model ?? 'the selected machine'} with ${
          wedmResult.estimated_cost?.total_usd != null
            ? `$${wedmResult.estimated_cost.total_usd.toFixed(0)} estimated cost`
            : 'cost still pending'
        }.`
      : result
  ...
  const machineProgramLaunchPlan = useMemo(() => {
    if (machineMode === 'mill') {
      return {
        summary: 'Guided milling wizard',
        body: 'Launch the full milling wizard with the current machine, material, and CAM posture preloaded so the backend program route starts from the live calculator setup.',
        primaryLabel: 'Launch Milling Wizard',
    ...
    if (machineMode === 'wire_edm') {
      return {
        summary: 'Wire wizard + contour studio',
        body: 'Launch the guided Wire EDM wizard for solver-first planning, or open the embedded contour/CAD studio when you are ready to generate code from the staged geometry.',
        primaryLabel: 'Launch Wire EDM Wizard',
        secondaryLabel: 'Open Wire EDM CAD / Program Studio',
      };
    }
    if (machineMode === 'edm') {
      return {
        summary: 'Electrode CAD/program studio',
        body: 'Sinker EDM stays anchored to the embedded electrode-prep studio so the Roku-Roku packet, trilobe macro path, and burn handoff remain attached to the live calculator posture.',
        primaryLabel: 'Open Sinker EDM CAD / Program Studio',
        secondaryLabel: 'Open full Print to CNC',
      };
    }
    ...
    return {
      summary: 'Waterjet print-to-program pipeline',
      body: 'Waterjet programming currently hands off through the shared pipeline with the active machine, material, and path posture attached so the next desk can pick up without reinterpreting the calculator setup.',
      primaryLabel: 'Open Waterjet Program Pipeline',
      secondaryLabel: 'Open full Print to CNC',
    };
  }, [machineMode]);
  const scrollToProgramWorkbench = useCallback(() => {
    if (!programWorkbenchPanelRef.current) return;
    programWorkbenchPanelRef.current.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }, []);
  const handleLaunchGuidedWizard = useCallback(() => {
    if (!guidedWizardPath) return;
    navigate(guidedWizardPath, { state: calculatorWizardLaunchState });
  }, [calculatorWizardLaunchState, guidedWizardPath, navigate]);
  const handleOpenProgramStudio = useCallback(() => {
    if (hasEmbeddedProgramStudio) {
      scrollToProgramWorkbench();
      return;
    }
    navigate(pipelinePath, {
      state: {
        sourceLabel: 'calculator studio',
        launchIntent: 'program-pipeline',
        workspaceContext: calculatorWizardLaunchState.workspaceContext,
      },
    });
  }, [
    calculatorWizardLaunchState.workspaceContext,
    hasEmbeddedProgramStudio,
    navigate,
    pipelinePath,
    scrollToProgramWorkbench,
  ]);
  const prismModeExplainerCards = [
    ...
    },
    {
      ...
      body: 'It blends constraint logic, machine legality, physics-informed cutting models, finish estimation, and weighted readiness scoring so the advice stays grounded instead of generic.',
  ...
  useEffect(() => {
    ...
    const params: ToolRoiAnalysisParams = {
      ...
      machine: {
        ...
        spindle: {
          max_rpm: selectedMachine.spindleRpm,
          power_kw: selectedMachine.powerHp / HP_PER_KW,
          power_continuous_kw: selectedMachine.powerHp / HP_PER_KW,
          power: selectedMachine.powerHp / HP_PER_KW,
        },
        confidence: {
          overall: selectedMachine.packageProvenance?.confidence === 'fallback' ? 0.5 : 0.8,
          spindle: selectedMachine.packageProvenance?.confidence === 'fallback' ? 0.5 : 0.8,
        },
        provenance: {
          source: selectedMachine.packageProvenance?.source ?? 'calculator-workspace',
          confidence: selectedMachine.packageProvenance?.confidence ?? 'catalog',
          sourceRecordIds: selectedMachine.packageProvenance?.sourceRecordIds ?? [selectedMachine.id],
        },
      },
      current_tool: {
        id: selectedTool.id,
  ...
  return (
    ...
    <div className="calculator-workspace-root relative w-full text-slate-200 bg-[#0a1224] min-h-screen lg:min-h-full">
      {/*
       * Cross-link: this is the full Studio — focused Codex SFC is at /speed-feed-calc.
       * Inline (NOT absolute) so it cannot overlap the brand/header grid at lg+ viewports.
       * Hidden below lg because the focused-SFC link is most useful when the Studio
       * has room to render its full header (mobile users land here intentionally).
       */}
      <div className="hidden justify-end border-b border-white/5 bg-[#0a1224]/80 px-4 py-2 lg:flex">
        <SurfaceCrossLink
          to="/speed-feed-calc"
          label="Lite SFC"
          note="Focused S/F with smart selectors + PDF export"
          accent="cyan"
        />
      </div>
      ...
      {guidedModeEnabled
        ...
        : null}
      {/* Ã¢â€â‚¬Ã¢â€â‚¬ Compact header bar Ã¢â€â‚¬Ã¢â€â‚¬ */}
                    ...
                    >
                      ...
                      <div className="calculator-toolbar-prism-statgrid">
                        ...
                        <span className="calculator-toolbar-prism-badge calculator-toolbar-prism-badge-secondary">
                          {`${prismModePlan.confidenceLabel} Â· ${prismModePlan.inventoryCoverageLabel}`}
                        </span>
                      </div>
                    </button>
      ...
      {/* Ã¢â€â‚¬Ã¢â€â‚¬ Experience + workflow launchers + machine mode row Ã¢â€â‚¬Ã¢â€â‚¬ */}
          ...
          <div className="calculator-machine-mode-row">
            {MACHINE_MODE_OPTIONS.map((mode) => (
              <button
                ...
                aria-label={`Switch calculator mode to ${mode.label}`}
                aria-pressed={machineMode === mode.id}
              ...
              >
                ...
                <div className="calculator-machine-mode-preview-shell" aria-hidden="true">
                  <MachineModeMediaPreview mode={mode.id} />
                </div>
              </button>
            ))}
          </div>

          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-6" aria-label="Calculator data backbone status">
            ...
            <div className="calculator-toolbar-flow-group">
              <div className="relative group/upload">
                ...
                >
                  ...
                  <span className="calculator-toolbar-flow-arrow" aria-hidden="true">â†—</span>
                </button>
      ...
      <div className="border-b border-slate-700/50 bg-[linear-gradient(180deg,rgba(8,17,31,0.98)_0%,rgba(7,16,29,0.98)_100%)] px-4 py-4">
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)]">
          <AppwCalculatorCopilot
            deskLabel={calculatorStudioLabel}
            activeLaneLabel={activeCalculatorLaneLabel}
            activeLaneDetail={activeCalculatorLaneDetail}
            machineModeLabel={machineModeLabel}
            experienceLabel={experienceProfile.badge}
            overallCatalogLabel={overallCatalogLabel}
            liveBackboneCount={liveBackboneCount}
            hybridBackboneCount={hybridBackboneCount}
            fallbackBackboneCount={fallbackBackboneCount}
            selectedMachineTitle={selectedMachineSummaryTitle}
            selectedMachineConnection={selectedMachineConnectionLabel}
            selectedMaterial={selectedMaterial?.name}
            selectedTool={selectedTool?.name}
            selectedToolpath={selectedToolpath?.label}
            selectedProgramming={selectedProgramming?.label}
            setupCompletenessPct={setupCompleteness}
            optimizationScore={toolbarOptimizationScore}
            optimizationSummary={toolbarOptimizationSummary}
            resultSafetyLabel={resultSafety.label}
            resultSafetySummary={resultSafety.summary}
            resultSolveSourceLabel={resultSolveSourceLabel}
            resultSignalCount={resultSignalStack.length}
            solverConfidencePct={liveSolverConfidenceScore}
            prismModeEnabled={prismModeEnabled}
            prismConfidenceLabel={prismModePlan.confidenceLabel}
            prismConfidenceScore={prismModePlan.confidenceScore}
            inventoryCoverageLabel={prismModePlan.inventoryCoverageLabel}
            ...
            toolCribImportCount={toolCribImportCount}
            toolCribLibraryCount={toolCribLibraryCount}
            toolCribImportSummary={toolCribImportSummary}
            toolCribImportError={toolCribImportError}
            toolCribScanError={toolCribScanError}
          />

          <section className="rounded-[28px] border border-slate-700/50 bg-[#081320] px-5 py-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
            <div className="mb-4 space-y-1">
              <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-cyan-200">Command-surface truth</div>
              <div className="text-xl font-semibold text-slate-50">Calculator posture</div>
              <div className="text-sm leading-6 text-slate-400">
                Keep authority, solve confidence, downstream workflow routing, and My Shop evidence explicit so the calculator behaves like a trustworthy operating desk instead of a generic calculator.
              </div>
            </div>
            <div className="grid gap-3">
              <CalculatorSourcePostureCard
                label="Catalog backbone authority"
                detail={calculatorCatalogTruthDetail}
                statusLabel={overallCatalogLabel}
                toneClass={overallCatalogSource === 'live' ? 'border-emerald-400/25 bg-emerald-400/10 text-emerald-100' : overallCatalogSource === 'hybrid' ? 'border-cyan-300/25 bg-cyan-300/10 text-cyan-100' : 'border-amber-300/25 bg-amber-300/10 text-amber-100'}
                metaLabel="Machines, materials, programming, workholding, tooling, holders"
              />
              <CalculatorSourcePostureCard
                label="Solve and release posture"
                detail={calculatorSolveTruthDetail}
                statusLabel={resultSafety.label}
                toneClass={resultSafety.tone === 'emerald' ? 'border-emerald-400/25 bg-emerald-400/10 text-emerald-100' : resultSafety.tone === 'amber' ? 'border-amber-300/25 bg-amber-300/10 text-amber-100' : resultSafety.tone === 'rose' ? 'border-rose-400/25 bg-rose-400/10 text-rose-100' : 'border-slate-400/20 bg-slate-400/8 text-slate-200'}
                metaLabel={`Solve ${resultSolveSourceLabel} • ${resultSignalStack.length} active signals`}
              />
              <CalculatorSourcePostureCard
                label="Workflow continuity"
                detail={calculatorWorkflowTruthDetail}
                statusLabel={calculatorRouteAuthority.releaseSupported && calculatorRouteAuthority.toolpathSupported ? 'Ready' : 'Constrained'}
                toneClass={calculatorRouteAuthority.releaseSupported && calculatorRouteAuthority.toolpathSupported ? 'border-cyan-300/25 bg-cyan-300/10 text-cyan-100' : 'border-amber-300/25 bg-amber-300/10 text-amber-100'}
                metaLabel={`Packet ${calculatorPacketId}`}
              />
              <CalculatorSourcePostureCard
                label="My Shop and upload continuity"
                detail={calculatorMyShopTruthDetail}
                statusLabel={loadedFileName || toolCribImportCount > 0 ? 'Staged' : 'Standing by'}
                toneClass={toolCribImportError || toolCribScanError ? 'border-rose-400/25 bg-rose-400/10 text-rose-100' : loadedFileName || toolCribImportCount > 0 ? 'border-violet-300/25 bg-violet-300/10 text-violet-100' : 'border-slate-400/20 bg-slate-400/8 text-slate-200'}
                metaLabel={`Imports ${toolCribImportCount} • Libraries ${toolCribLibraryCount}`}
              />
            </div>
          </section>
        </div>
      ...
      {/* Ã¢â€â‚¬Ã¢â€â‚¬ Calculator Workspace: Balanced rails with a wider center cutting lane Ã¢â€â‚¬Ã¢â€â‚¬ */}
        ...
        <aside className="space-y-3 lg:sticky lg:top-2 lg:self-start lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto lg:pr-1 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-700">
          ...
          >
            ...
            <div className="calculator-rail-experience-button-row">
              ...
              <Panel icon="Ã°Å¸ÂÂ­" title={machineSelectionTitle} guideTargetId="machine-selection" helpTopicId="machine-selection" onOpenHelp={handleOpenHelpTopic} summary={selectedMachine ? `${selectedMachine.manufacturer} ${selectedMachine.model}` : undefined} collapsible>
            <div className="space-y-4">
              ...
              <Panel icon="Ã¢Å¡â„¢Ã¯Â¸Â" title={machineFeaturesTitle} guideTargetId="machine-features" helpTopicId="machine-features" onOpenHelp={handleOpenHelpTopic} summary={selectedFeatureDetails.length ? t('machine.verifiedCount').replace('{count}', String(selectedFeatureDetails.length)) : undefined} collapsible>
            <div className="space-y-4">
              <div className="grid gap-3">
              ...
              <Panel icon="Ã°Å¸Â§Â±" title={materialTitle} guideTargetId="material" helpTopicId="material" onOpenHelp={handleOpenHelpTopic} summary={selectedMaterial?.name} collapsible>
            <div className="space-y-4">
              <div className="rounded-xl border border-slate-700/50 bg-[#0a1628] px-3 py-3 text-xs text-slate-400">
                <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="min-w-0 flex flex-col gap-6 xl:w-full xl:max-w-none">
          <div className={CALCULATOR_MAIN_RAIL_GRID_CLASS}>
            <div className="min-w-0 flex flex-col gap-6 xl:pr-2">
              {showUploadWorkflowDialog ? (
                ...
                <button
                  ...
                  aria-label={loadedFileName ? `Replace uploaded file ${loadedFileName}` : t('upload.dropPrompt')}
              ...
              <Panel icon="Ã°Å¸â€™Â»" title={programmingTitle} guideTargetId="programming" helpTopicId="programming" onOpenHelp={handleOpenHelpTopic} summary={selectedProgramming?.label}
                className="order-2"
                collapsible
              >
                <div className="space-y-4">
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="mt-4 flex items-center justify-between gap-3 text-[11px] font-semibold uppercase tracking-[0.18em]">
                              ...
                              <span className={`transition group-hover:translate-x-1 ${tone.accent}`}>Inspect option Ã¢â€ â€™</span>
                            </div>
                          </button>
                        );
                      })}
              ...
              <Panel
                icon="Ã°Å¸â€Â©" title={cuttingParametersTitle} guideTargetId="cutting-parameters" summary={selectedOperationLabel}
                className="order-3"
                collapsible
              >
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="rounded-2xl border border-slate-700/50 bg-[#0a1628] px-4 py-4">
                        ...
                        <div className="mt-4 grid gap-3 md:grid-cols-2">
                          ...
                          {/* Feature Editor + 2D Sketch (lathe + wire) */}
                          {(machineMode === 'lathe' || machineMode === 'wire_edm') && (
                            <LatheFeatureEditorSection mode={machineMode} />
                          )}

                          {/* Lathe-specific operation panels */}
                          {machineMode === 'lathe' && operation === 'threading' && (
                            <div className="col-span-full mt-2">
                              <DeferredCalculatorSurface label="Loading threading controlsâ€¦">
                                <LatheThreadingPanel />
                              </DeferredCalculatorSurface>
                            </div>
                          )}
                          {machineMode === 'lathe' && (operation === 'turning_rough' || operation === 'turning_finish' || operation === 'hard_turning') && (
                            <div className="col-span-full mt-2">
                              <DeferredCalculatorSurface label="Loading insert selectorâ€¦">
                                <LatheInsertSelectorPanel />
                              </DeferredCalculatorSurface>
                            </div>
                          )}
                          {machineMode === 'lathe' && operation === 'hard_turning' && (
                            <div className="col-span-full mt-2">
                              <DeferredCalculatorSurface label="Loading hard-turning controlsâ€¦">
                                <LatheHardTurningPanel />
                              </DeferredCalculatorSurface>
                            </div>
                          )}
                          {machineMode === 'lathe' && (operation === 'grooving' || operation === 'parting') && (
                            <div className="col-span-full mt-2">
                              <DeferredCalculatorSurface label="Loading grooving controlsâ€¦">
                                <LatheGroovingPanel />
                              </DeferredCalculatorSurface>
                            </div>
                          )}
                          {machineMode === 'lathe' && (
                            <div className="col-span-full mt-2">
                              <DeferredCalculatorSurface label="Loading tool life panelâ€¦">
                                <LatheToolLifePanel />
                              </DeferredCalculatorSurface>
                            </div>
                          )}
                          {machineMode === 'lathe' && (
                            <div className="col-span-full mt-2">
                              <DeferredCalculatorSurface label="Loading workholding checksâ€¦">
                                <LatheWorkholdingPanel />
                              </DeferredCalculatorSurface>
                            </div>
                          )}
                          {machineMode === 'lathe' && (
                            <div className="col-span-full mt-2">
                              <DeferredCalculatorSurface label="Loading chatter panelâ€¦">
                                <LatheChatterPanel />
                              </DeferredCalculatorSurface>
                            </div>
                          )}
                          {machineMode === 'lathe' && (
                            <div className="col-span-full mt-2">
                              <DeferredCalculatorSurface label="Loading cost modelâ€¦">
                                <LatheCostPanel />
                              </DeferredCalculatorSurface>
                            </div>
                          )}
                          ...
                          {machineMode === 'wire_edm' && (
                            <div className="col-span-full mt-2">
                              <DeferredCalculatorSurface label="Loading feasibility checkâ€¦">
                                <WireEdmFeasibilityPanel />
                              </DeferredCalculatorSurface>
                            </div>
                          )}
                          {machineMode === 'wire_edm' && (
                            <div className="col-span-full mt-2">
                              <DeferredCalculatorSurface label="Loading surface integrityâ€¦">
                                <WireEdmSurfaceIntegrityPanel />
                              </DeferredCalculatorSurface>
                            </div>
                          )}
                          {machineMode === 'wire_edm' && (
                            <div className="col-span-full mt-2">
                              <DeferredCalculatorSurface label="Loading cost breakdownâ€¦">
                                <WireEdmCostBreakdownPanel />
                              </DeferredCalculatorSurface>
                            </div>
                          )}
              ...
              <Panel
                icon="🧭"
                title="Guided Program Launch"
                summary={machineProgramLaunchPlan.summary}
                collapsible
                className="border-violet-300/20 bg-[#09111f]"
                headerClassName="border-b border-violet-400/15 bg-[linear-gradient(135deg,rgba(20,16,38,0.98)_0%,rgba(13,27,49,0.96)_52%,rgba(8,20,38,0.96)_100%)]"
              >
                <div className="rounded-[22px] border border-violet-300/18 bg-violet-300/[0.07] px-4 py-4 text-sm leading-6 text-violet-50/92">
                  {machineProgramLaunchPlan.body}
                </div>

                <div className="grid gap-3 md:grid-cols-3">
                  <div className="rounded-2xl border border-white/10 bg-slate-950/50 px-4 py-4">
                    <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Machine focus</div>
                    <div className="mt-2 text-sm font-semibold text-slate-100">{selectedMachine?.model ?? selectedMachineModeOption.label}</div>
                    <div className="mt-1 text-xs leading-5 text-slate-400">{selectedMachine?.manufacturer ?? manufacturer}</div>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-slate-950/50 px-4 py-4">
                    <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Backend posture</div>
                    <div className="mt-2 text-sm font-semibold text-slate-100">
                      {calculatorBackendAiReady ? 'Live backend evidence staged' : 'Solve still needed'}
                    </div>
                    <div className="mt-1 text-xs leading-5 text-slate-400">{calculatorBackendBuildLabel}</div>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-slate-950/50 px-4 py-4">
                    <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Programming path</div>
                    <div className="mt-2 text-sm font-semibold text-slate-100">{selectedProgramming?.label ?? 'Programming unresolved'}</div>
                    <div className="mt-1 text-xs leading-5 text-slate-400">
                      {selectedToolpath?.label ?? calculatorRouteAuthority.toolpathNote}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                  <button
                    type="button"
                    onClick={guidedWizardPath ? handleLaunchGuidedWizard : handleOpenProgramStudio}
                    className="inline-flex items-center rounded-full border border-violet-300/40 bg-violet-500/[0.14] px-5 py-3 text-sm font-semibold text-violet-50 transition hover:border-violet-200/60 hover:bg-violet-500/[0.2]"
                  >
                    {machineProgramLaunchPlan.primaryLabel}
                  </button>
                  <button
                    type="button"
                    onClick={hasEmbeddedProgramStudio ? handleOpenProgramStudio : () => navigate(programReleasePath)}
                    className="inline-flex items-center rounded-full border border-cyan-300/35 bg-cyan-950/35 px-5 py-3 text-sm font-semibold text-cyan-100 transition hover:border-cyan-200/60 hover:bg-cyan-950/55"
                  >
                    {machineProgramLaunchPlan.secondaryLabel}
                  </button>
                </div>
              </Panel>

              {(machineMode === 'lathe' || machineMode === 'wire_edm' || machineMode === 'edm') ? (
                <div ref={programWorkbenchPanelRef}>
                  <Panel
                    icon="ðŸ§ "
                    title="Print / CAD to Program"
                    summary={
                      machineMode === 'lathe'
                        ? 'Lathe auto-programming intake'
                        : machineMode === 'wire_edm'
                          ? 'Wire auto-programming intake'
                          : 'Sinker electrode machining intake'
                    }
                    collapsible
                    className="border-cyan-300/20 bg-[#07111d]"
                    headerClassName="border-b border-cyan-400/15 bg-[linear-gradient(135deg,rgba(6,20,36,0.96)_0%,rgba(9,34,58,0.96)_42%,rgba(14,55,88,0.94)_100%)]"
                  >
                    <DeferredCalculatorSurface label="Loading auto-programming workbenchâ€¦" className="min-h-[240px]">
                      <CalculatorProgramWorkbench
                        mode={machineMode}
                        machineLabel={selectedMachine?.model ?? selectedMachine?.id}
                        machineId={selectedMachine?.id}
                        machineManufacturer={selectedMachine?.manufacturer ?? manufacturer}
                        controllerId={selectedControllerOption?.id ?? controllerOptionId}
                        materialLabel={selectedMaterial?.name ?? selectedMaterial?.id ?? materialId}
                        ...
                        programReleasePath={programReleasePath}
                        wizardPath={guidedWizardPath}
                        wizardState={guidedWizardPath ? calculatorWizardLaunchState : undefined}
                        wizardLabel={guidedWizardPath ? machineProgramLaunchPlan.primaryLabel : undefined}
                      />
                    </DeferredCalculatorSurface>
                  </Panel>
                </div>
              ...
              >
                <div className="relative overflow-hidden rounded-[24px] bg-[linear-gradient(135deg,rgba(103,232,249,0.98)_0%,rgba(59,130,246,0.44)_22%,rgba(34,211,238,1)_48%,rgba(96,165,250,0.42)_76%,rgba(45,212,191,0.96)_100%)] p-[2px] shadow-[0_0_0_1px_rgba(165,243,252,0.24),0_0_34px_rgba(34,211,238,0.18),0_0_92px_rgba(14,165,233,0.16)]">
                  ...
                  <div className="relative rounded-[22px] bg-[radial-gradient(circle_at_top,rgba(34,211,238,0.22),transparent_34%),linear-gradient(180deg,#07111d_0%,#0b1626_100%)] px-5 py-5 md:px-6">
                    ...
                    {solvePreflight.notes.length ? (
                      <div className="mb-4 rounded-2xl border border-amber-400/25 bg-amber-950/20 px-4 py-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.02)]">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          <div>
                            <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-amber-200/80">Backend preflight</div>
                            <div className="mt-1 text-sm font-semibold text-amber-50">Machine, tool, and on-hand inventory lanes checked before the final solve.</div>
                          </div>
                          <div className="flex flex-wrap items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em]">
                            {solvePreflight.machineLimitingFactors.length ? (
                              <span className="rounded-full border border-amber-300/35 bg-amber-500/10 px-3 py-1 text-amber-200">
                                {solvePreflight.machineLimitingFactors.length} machine limit{solvePreflight.machineLimitingFactors.length === 1 ? '' : 's'}
                              </span>
                            ) : null}
                            {solvePreflight.toolLimitingFactors.length ? (
                              <span className="rounded-full border border-amber-300/35 bg-amber-500/10 px-3 py-1 text-amber-200">
                                {solvePreflight.toolLimitingFactors.length} tool limit{solvePreflight.toolLimitingFactors.length === 1 ? '' : 's'}
                              </span>
                            ) : null}
                            {solvePreflight.inventoryCoveragePct != null ? (
                              <span className="rounded-full border border-sky-300/35 bg-sky-500/10 px-3 py-1 text-sky-200">
                                {solvePreflight.inventoryCoveragePct.toFixed(1)}% crib coverage
                              </span>
                            ) : null}
                          </div>
                        </div>
                        <div className="mt-3 grid gap-2 xl:grid-cols-3">
                          {solvePreflight.notes.map((note) => (
                            <div
                              key={note}
                              className="rounded-2xl border border-amber-300/15 bg-[linear-gradient(180deg,rgba(42,21,12,0.58)_0%,rgba(28,15,10,0.82)_100%)] px-3 py-2.5 text-sm leading-6 text-amber-50/92"
                            >
                              {note}
                            </div>
                          ))}
                    ...
                    <CalculatorBackendAiReview
                      solveStateReady={calculatorBackendAiReady}
                      machineModeLabel={selectedMachineModeOption.label}
                      solveSourceLabel={resultSafety.solveSourceLabel}
                      resultHeading={resultSafety.heading}
                      resultSummary={resultSafety.summary}
                      resultGuidance={resultSafety.guidance}
                      resultConfidencePct={resultSafety.confidencePct}
                      resultSignals={resultSafety.signals}
                      solveWarnings={resultDisplayWarnings}
                      selectedMachineTitle={selectedMachineSummaryTitle}
                      selectedMachineConnection={selectedMachineConnectionLabel}
                      selectedMaterial={selectedMaterial?.name}
                      selectedTool={selectedTool?.name}
                      ...
                      releaseNote={calculatorRouteAuthority.releaseNote}
                      toolpathNote={calculatorRouteAuthority.toolpathNote}
                      preflightNotes={solvePreflight.notes}
                      machineLimitingFactors={solvePreflight.machineLimitingFactors}
                      toolLimitingFactors={solvePreflight.toolLimitingFactors}
                      inventoryCoveragePct={solvePreflight.inventoryCoveragePct}
                      backendResultLabel={calculatorBackendBuildLabel}
                      backendResultDetail={calculatorBackendBuildDetail}
                    />
            ...
            <div className="space-y-6 xl:sticky xl:top-6 xl:self-start xl:max-h-[calc(100vh-7.5rem)] xl:overflow-y-auto xl:pr-2 xl:[scrollbar-gutter:stable]">
              <Panel icon="Ã°Å¸â€Â§" title={toolingFixtureTitle} guideTargetId="tooling-fixture" helpTopicId="tooling-fixture" onOpenHelp={handleOpenHelpTopic} summary={selectedTool?.family ?? selectedTool?.label} collapsible>
                <div className="space-y-3">
                  ...
                  <div className="rounded-xl border border-slate-700/50 bg-[#0f1f36] px-3 py-3">
                    ...
                    <div className="mt-4 rounded-xl border border-violet-500/30 bg-[#0a1628] px-3 py-3">
                      <div className="mb-3 flex flex-wrap items-center gap-2 text-[11px]">
                        <span className={`rounded-full border px-3 py-1 font-bold uppercase tracking-[0.18em] ${
                          catalogStatusTone(workholdingCatalogStatus.source)
                        }`}>
                          {workholdingCatalogBadge}
                        </span>
                        <span className="rounded-full border border-slate-700/50 bg-[#0f1f36] px-3 py-1 font-semibold text-slate-400">
                          {workholdingCatalogSummary}
                        </span>
                      </div>
                      <div className="mb-3 rounded-xl border border-violet-500/20 bg-violet-950/10 px-3 py-2 text-[11px] leading-5 text-slate-400">
                        {workholdingCatalogStatus.note}
                      </div>
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
              ...
              <Panel
                ...
                collapsible
              >
                <DeferredCalculatorSurface label="Loading setup previewâ€¦" className="min-h-[260px]">
                  <CalculatorSetupPreview3D preview={setupPreview} />
                </DeferredCalculatorSurface>
              </Panel>

              <Panel icon="Ã°Å¸Ââ€”Ã¯Â¸Â" title={hardwarePanelTitle} guideTargetId="hardware" summary={hardwareDigest(effectiveSelectedMachine, selectedStation)} collapsible>
                <HardwarePanel
                  machine={effectiveSelectedMachine}
              ...
              <Panel icon="Ã°Å¸â€™Â¡" title={processNotesTitle} guideTargetId="process-notes" collapsible>
                <div className="space-y-4">
                  <Insight
                    title="Coolant fit"
              ...
              <Panel
                icon="Ã°Å¸ÂÂ­" title={myShopTitle} guideTargetId="my-shop" summary={myShopPanelSummary}
                collapsible
              >
                <div className="space-y-4">
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    {[
                      {
                        title: 'Stage release desk',
                        body: calculatorRouteAuthority.releaseNote,
                        href: programReleasePath,
                      },
                      {
                        title: 'Stage Toolpath Advisor',
                        body: calculatorRouteAuthority.toolpathNote,
                        href: toolpathAdvisorPath,
                      },
                      {
                      ...
                      },
                      {
                        ...
                        body:
                          postProcessorState.unsupportedReason
                          ?? 'Send the current machine and operation posture into the post-processor desk before release.',
                      ...
                      action.href ? (
                        ...
                        <div
                          key={action.title}
                          className="rounded-xl border border-amber-400/20 bg-amber-950/15 px-4 py-4 text-left"
                        >
                          <div className="text-sm font-semibold text-amber-200">{action.title}</div>
                          <div className="mt-2 text-sm leading-6 text-amber-100/80">{action.body}</div>
                        </div>
                      )
                    ))}
                  </div>
          ...
          <Panel
            icon="Ã°Å¸â€œÂ" title={formulaLibraryTitle} guideTargetId="formula-library" summary={`${FORMULAS.length} formulas`}
            collapsible
            defaultCollapsed
          >
      ...
      >
        {collapsible ? (
          <button
            ...
            className="calculator-panel-header-main flex min-w-0 flex-1 items-center gap-2 bg-transparent text-center"
          >
            ...
            {calculatorGuide?.enabled && guideMeta ? (
              ...
              >
                Step {guideMeta.index + 1} Â· {guideMeta.statusLabel}
              </span>
            ) : null}
          </button>
        ) : (
          <div className="calculator-panel-header-main flex min-w-0 flex-1 items-center gap-2">
            <div className={`calculator-panel-title-chip inline-flex min-w-0 items-center rounded-xl border px-3 py-1.5 ${tone.titleWrap} ${resolvedGuideComplete ? 'calculator-guide-panel-title-complete' : ''} ${resolvedGuideActive ? 'calculator-guide-panel-title-attention' : ''}`}>
              <h2 className={`calculator-panel-title-text truncate text-[19px] font-black tracking-[0.01em] sm:text-[21px] ${tone.titleTone}`}>{title}</h2>
            </div>
            {calculatorGuide?.enabled && guideMeta ? (
        ...
        <div className="flex items-center gap-2 shrink-0">
          {summary && (
            <span className="calculator-panel-summary hidden max-w-[220px] truncate text-[11px] font-semibold text-slate-300/80 xl:inline-flex">{summary}</span>
          )}
...
function NumberField({
  ...
  return (
    <label className="block">
      <span className="mb-1 flex items-center gap-1.5 text-[10px] font-semibold text-slate-400">
        <span>{label}</span>
        {guideHint ? <CalculatorInfoHint label={label} body={guideHint} /> : null}
      </span>
...
function SelectField({
  ...
  const hintBody = guideHint ?? selectedOption?.detail;
  return (
    <label className="block">
      <span className="mb-1.5 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-[0.16em] text-slate-300">
        <span>{label}</span>
        {hintBody ? <CalculatorInfoHint label={label} body={hintBody} /> : null}
      </span>
      <select
        data-guide-managed="true"
...
function OptionGrid({
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">
        <span>{label}</span>
        {guideHint ? <CalculatorInfoHint label={label} body={guideHint} /> : null}
      </div>
      <div className="grid gap-2">
        {options.map((option) => (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <div className="text-sm font-semibold">{option.label}</div>
              {option.detail ? <CalculatorInfoHint label={`${label} ${option.label}`} body={option.detail} /> : null}
            </div>
          </button>
        ))}
      </div>
...
function WireEdmResultCards({ wedmResult, controller, calcParams }: { wedmResult: WireEdmCalcResult; controller: string; calcParams?: { material: string; thickness_mm: number; target_ra_um: number; wire_type: string } }) {
  ...
  const [generatedProgram, setGeneratedProgram] = useState<{ text: string; filename: string; result: WedmProgramResult } | null>(null);
  ...
  // Backplot path verdict â€” gates download when RED issues detected
  ...
  async function generateProgram() {
    if (dxfContent) {
      if (selectedContourIndices.length > 0) {
        await generateFromSelection();
        return;
      }

      await generateFromDxf(dxfContent, dxfFilename || 'wedm-part');
      return;
    }
  ...
  async function handleDxfUpload(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (parseResp?.data?.contours?.length) {
        ...
        // No contours parsed â€” generate directly
  ...
  async function generateFromSelection() {
    ...
    try {
      const resp = await wePhotoToProgram({
        ...
        controller: controller as WedmController,
  ...
  function exportSetupSheet() {
    const lines: string[] = [
      ...
      'Ã¢â€¢â€˜          WIRE EDM SETUP SHEET                Ã¢â€¢â€˜',
      ...
      '',
      'Ã¢â€â‚¬Ã¢â€â‚¬ Process Summary Ã¢â€â‚¬Ã¢â€â‚¬',
      ...
      '',
      'Ã¢â€â‚¬Ã¢â€â‚¬ Per-Pass Parameters Ã¢â€â‚¬Ã¢â€â‚¬',
    ...
    if (wedmResult.estimated_cost) {
      lines.push('', 'Ã¢â€â‚¬Ã¢â€â‚¬ Cost Estimate Ã¢â€â‚¬Ã¢â€â‚¬');
    ...
    <div className="space-y-4">
      ...
      {passes.length > 0 && (
        <div className="rounded-2xl border border-slate-700/40 bg-[#0c1522] px-1 py-3 md:px-4">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2 px-3">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                disabled={programGenState === 'generating' || !dxfContent}
                title={!dxfContent ? 'Upload DXF geometry to generate NC from the actual contour.' : undefined}
              ...
              >
                {programGenState === 'generating'
                  ? 'Generating...'
                  : selectedContourIndices.length > 0
                    ? `Generate program (${selectedContourIndices.length} profiles)`
                    : 'Generate NC program'}
              </button>
                  <button
                    type="button"
                    ...
                    className="rounded-lg border border-amber-500/30 bg-amber-950/40 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-widest text-amber-300 transition hover:bg-amber-900/50 disabled:opacity-50"
                  >
                  ...
                  <input
                    ...
                    disabled={uploadState !== 'idle'}
                    aria-label="Upload wire EDM DXF file"
                  />
                  <button
                    type="button"
                    onClick={() => imageFileInputRef.current?.click()}
                    disabled={uploadState !== 'idle'}
                    className="rounded-lg border border-violet-500/30 bg-violet-950/40 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-widest text-violet-300 transition hover:bg-violet-900/50 disabled:opacity-50"
                  >
                  ...
                  <input
                    ...
                    disabled={uploadState !== 'idle'}
                    aria-label="Upload wire EDM photo or PDF"
                  />
                  {generatedProgram && (
                    <button
                      type="button"
          ...
          {/* Wire Path Backplot â€” auto-displayed after program generation (U-W100-22) */}
          {generatedProgram && (
            <div className="mb-3">
              <WireEdmBackplot
              ...
              {backplotVerdict && (
                <div className={`mt-2 rounded-lg border px-4 py-2 text-xs font-semibold ${
                  ...
                  {!backplotVerdict.canDownload && (
                    <span className="ml-2 font-normal text-red-400">â€” Fix critical issues before downloading</span>
                  )}
                </div>
              )}
            </div>
          ...
          {/* Shop Settings Panel â€” collapsible */}
          {calcParams && (
            <div className="mb-3">
              <button
                type="button"
                ...
                <div className="mt-2 grid gap-3 rounded-xl border border-slate-700/20 bg-[#0a1018] p-4 md:grid-cols-2 xl:grid-cols-3">
                  ...
                  <div>
                    ...
                    <input type="number" step="0.01" placeholder={calcParams.wire_type?.includes('0.20') ? '0.20' : '0.25'} value={actualWireDia} onChange={e => setActualWireDia(e.target.value)}
                      className="w-full rounded border border-slate-700/40 bg-slate-900/60 px-2 py-1.5 text-xs text-slate-300 placeholder:text-slate-600" />
                    <div className="mt-0.5 text-[9px] text-slate-600">If different from planned â€” compensates offsets</div>
                  </div>
          ...
          {/* Contour Picker (2D/3D) â€” shown after DXF upload parses geometry */}
      ...
      {/* Surface Integrity Safety Card â€” ALWAYS VISIBLE, CANNOT BE HIDDEN */}
...
export function licenseOptionsFor(
  ...
  return licenseOptionsForFromAuthority(programming);
}
...
export function resolveProgrammingSelectionState({
  ...
  return resolveProgrammingSelectionStateFromAuthority({
    programming,
    machine,
    requestedLicenseTierId,
    requestedToolpathTypeId,
...
export function filterToolpathsForLicense(
  ...
  return filterToolpathsForLicenseFromAuthority(programming, toolpaths, licenseTierId, machine);
}
            ...
            {Array.from({ length: stationCount }).map((_, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-2 text-lg">Ã¢â€“Â®</div>
                </button>
              );
            })}
          </div>
...
function programmingLogo(label: string) {
  ...
  if (key.includes('camworks')) return { mark: 'CW', wordmark: 'CAMWorks' };
  ...
  if (key.includes('sprutcam')) return { mark: 'SP', wordmark: 'SprutCAM' };
  if (key.includes('surfcam')) return { mark: 'SF', wordmark: 'SURFCAM' };
...
export function classifyToolpathType(toolpath: { label: string; path: string; operationId: string }) {
  return classifyToolpathTypeFromAuthority(toolpath);
}
 ...
 */
export type ToolpathDefaults = ProgrammingAuthorityToolpathDefaults;

export function getToolpathDefaults(
  toolpath: { id: string; label: string; path: string; operationId: string } | undefined,
  machineMode: MachineMode,
): ToolpathDefaults | null {
  return getToolpathDefaultsFromAuthority(toolpath, machineMode);
}

export function buildToolpathTypeOptions(toolpaths: Array<{ label: string; path: string; operationId: string; summary?: string }>) {
  return buildToolpathTypeOptionsFromAuthority(toolpaths);
}
