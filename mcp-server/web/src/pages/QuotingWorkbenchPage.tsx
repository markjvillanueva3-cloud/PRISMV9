...
// U-QP-SECONDARY-OPS-AUTH-MIGRATION (2026-07-01): secondary-ops price returns the shop's real per-op
// cost basis, so it is NO LONGER dispatchable on the anon generic surface (403). Call the typed
// verifyToken verb POST /api/v1/quoting/secondary-ops-price with the operator's Bearer token. When no
// token is present (anon), the endpoint 401s -> we degrade to $0 (the same benign state as no ops
// selected), never leaking cost to an unauthenticated viewer.
type SecOpsData = { total_secondary_ops_usd: number; total_lead_days_delta: number; any_outsourced: boolean; needs_auth?: boolean };
// needs_auth distinguishes "the operator must sign in to price these ops" from a genuine $0 (no ops
// selected). Without it the FE would render $0 for a token-expired session -> an OPERATOR UNDER-QUOTE
// (charlie soul: under-quote is a cardinal sin). The render shows a "Sign in to price" state instead.
const SEC_OPS_NEEDS_AUTH: SecOpsData = { total_secondary_ops_usd: 0, total_lead_days_delta: 0, any_outsourced: false, needs_auth: true };
async function callSecondaryOpsAuthed(params: Record<string, unknown>): Promise<Dispatch<SecOpsData>> {
  try {
    // getStoredAuthToken() reads the canonical 'prism-auth-token' (hyphen) key
    // AuthContext writes + JSON-unwraps the {token} session. The old bare
    // getItem('prism_auth_token') missed both -> a signed-in salesman saw the
    // "sign in to price" state instead of a real price (charlie-soul under-quote hazard).
    const token = getStoredAuthToken();
    if (!token) return { ok: true, data: SEC_OPS_NEEDS_AUTH }; // anon: sign in to price (never a misleading $0)
    const res = await fetch('/api/v1/quoting/secondary-ops-price', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify(params),
    });
    if (!res.ok) return { ok: true, data: SEC_OPS_NEEDS_AUTH }; // 401/403 token rejected (or 5xx) -> sign-in state, never a raw error or a misleading $0
    const payload = await res.json();
    const text = payload?.content?.[0]?.text;
    if (typeof text === 'string') {
      try {
        const parsed = JSON.parse(text);
        if (parsed && typeof parsed === 'object' && 'error' in parsed) return { ok: true, data: SEC_OPS_NEEDS_AUTH };
        return { ok: true, data: parsed as SecOpsData };
      } catch {
        return { ok: true, data: SEC_OPS_NEEDS_AUTH };
      }
    }
    return { ok: true, data: payload as SecOpsData };
  } catch {
    return { ok: true, data: SEC_OPS_NEEDS_AUTH };
  }
}
...
export function QuotingWorkbenchPage() {
  ...
  const [secOpsResult, setSecOpsResult] = useState<{ total: number; lead_days: number; outsourced: boolean; needs_auth?: boolean } | null>(null);
  ...
  const generateQuote = useCallback(async () => {
    ...
    const [tierR, secR, tolR, freightR] = await Promise.all([
      ...
      selectedOps.size > 0
        ? callSecondaryOpsAuthed({ ops: [...selectedOps], quantity: qty, base_material_cost_usd: 50 })
    ...
    if (secR.ok && secR.data) setSecOpsResult({
      ...
      needs_auth: (secR.data as any).needs_auth === true,
    });
  ...
  return (
    <div className="min-h-screen bg-[#0f1014] text-slate-100 p-4 md:p-6"
      ...
      {/* ADDERS */}
      {(secOpsResult && (secOpsResult.total > 0 || secOpsResult.needs_auth)) || tolResult || freightResult ? (
        <Section title="6. Adders" subtitle="Secondary ops + tolerance impact + freight">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {secOpsResult && secOpsResult.needs_auth ? (
              // Sign-in state -- NEVER a misleading $0 that could cause an under-quote (charlie soul).
              <Adder label="Secondary Ops" value="Sign in"
                     hint="Sign in to price selected secondary ops" tone="amber" />
