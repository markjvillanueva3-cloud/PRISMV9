...
import { ApiError, getLatheResult } from '../api/client';
import {
  ActionButton,
  PanelCard,
  StatusPill,
...
import type { LatheWizardResult } from '../components/LatheInputWizard';
import { SetupInstructionPanel, type SetupStep, type ToolStation } from '../components/SetupInstructionPanel';
import { LatheAIPanel } from '../components/LatheAIPanel';
...
function parseToolDiameterMm(toolName: string): number {
  const match = toolName.match(/(\d+(?:\.\d+)?)/);
  return match ? Number(match[1]) : 10;
}

function classifyLatheOperation(operation: string): string {
  const normalized = operation.toLowerCase();
  if (normalized.includes('rough')) return 'rough';
  if (normalized.includes('finish') || normalized.includes('smooth')) return 'finish';
  if (normalized.includes('cut') || normalized.includes('part')) return 'partoff';
  if (normalized.includes('drill')) return 'drill';
  return 'turning';
}

function buildLatheToolOptimizationState(
  result: LatheResultData,
  workspaceContext?: MachineWorkspaceContext,
) {
  const tools = result.toolStations.map((station) => ({
    tool_id: `T${station.station}`,
    diameter_mm: parseToolDiameterMm(station.toolName),
    length_mm: 80,
    is_sister: false,
    sister_of: '',
  }));

  const diameterByToolId = new Map(tools.map((tool) => [tool.tool_id, tool.diameter_mm]));

  const operations = result.toolsUsed.map((tool, index) => {
    const toolId = `T${tool.station}`;
    return {
      id: `lathe-op-${index + 1}`,
      tool_id: toolId,
      tool_diameter_mm: diameterByToolId.get(toolId) ?? parseToolDiameterMm(tool.name),
      operation_type: classifyLatheOperation(tool.operation),
      cutting_time_min: Number((tool.timeSeconds / 60).toFixed(2)),
    };
  });
...
function buildLathePostProcessorState(
  result: LatheResultData,
  fileName: string | undefined,
  workspaceContext?: MachineWorkspaceContext,
) {
  const baseName = fileName ? fileName.replace(/\.[^.]+$/, '') : result.partName;

  return {
    sourceLabel: 'lathe results',
    workspaceContext,
    fileName: fileName ?? `${result.partName}.nc`,
    gcode: result.gcode,
    machineModel: workspaceContext?.machineLabel ?? result.machineName,
    controller: workspaceContext?.controllerLabel ?? workspaceContext?.controllerId,
    machinePosture: 'lathe',
    operation: 'turning_profile',
    camSystem: 'fusion_360',
    programName: baseName.replace(/[^a-z0-9]+/gi, '_').replace(/^_+|_+$/g, '').toUpperCase(),
    notes:
      'Routed from lathe results so post generation stays aligned with the JM Die turning authority spine.',
  };
}

function buildLatheOptimizationState(
  result: LatheResultData,
  fileName: string | undefined,
  workspaceContext?: MachineWorkspaceContext,
) {
  return {
    fileName: fileName ?? `${result.partName}.nc`,
    gcode: result.gcode,
    materialName: workspaceContext?.materialLabel ?? result.material,
...
function buildLatheCycleTimeState(
  result: LatheResultData,
  fileName: string | undefined,
  workspaceContext?: MachineWorkspaceContext,
) {
...
function buildLatheFeatureSelectionState(
  result: LatheResultData,
  workspaceContext?: MachineWorkspaceContext,
) {
  return {
...
function buildLatheReleasePacketId(partName: string) {
  const normalized = partName.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '') || 'lathe';
  return `pkt__lathe__${normalized}`;
}

function buildLatheProgramReleasePath(
  result: LatheResultData,
  currentSearch = '',
  workspaceContext?: MachineWorkspaceContext,
) {
  const packetId = buildLatheReleasePacketId(result.partName);
  return buildWorkflowPath('/print-to-cnc', currentSearch, {
    origin: {
      source: 'lathe-results',
      recordType: 'Lathe Result',
      recordId: result.partName,
    ...
    extras: {
      ...
      machineManufacturer: (workspaceContext?.machineManufacturer ?? 'haas').toLowerCase(),
...
function mapLatheSetupSteps(rawValue: unknown): SetupStep[] {
  ...
  return rawValue.flatMap((entry, index) => {
    if (typeof entry === 'string' && entry.trim().length > 0) {
      return [{
        id: `setup-${index + 1}`,
        number: index + 1,
        title: `Setup step ${index + 1}`,
        description: entry,
      }];
    }

    const rawStep = asRecord(entry);
    if (!rawStep) {
      return [];
    }
    ...
    const title = typeof rawStep.title === 'string' && rawStep.title.trim().length > 0
      ? rawStep.title
      : `Setup step ${index + 1}`;
    const description = typeof rawStep.description === 'string' && rawStep.description.trim().length > 0
      ? rawStep.description
      : typeof rawStep.note === 'string' && rawStep.note.trim().length > 0
        ? rawStep.note
        : title;
    const safetyNote = typeof rawStep.safetyNote === 'string'
      ? rawStep.safetyNote
      : typeof rawStep.safety_note === 'string'
        ? rawStep.safety_note
        : undefined;

    return [{
      id: typeof rawStep.id === 'string' && rawStep.id.trim().length > 0 ? rawStep.id : `setup-${index + 1}`,
      number: typeof rawStep.number === 'number' ? rawStep.number : index + 1,
      title,
      description,
      safetyNote,
    }];
...
function mapLatheResultPayload(
  payload: unknown,
  options: {
    fallbackBatchQuantity?: number;
    fallbackFileName?: string;
    fallbackMachineName?: string;
    fallbackMaterial?: string;
  } = {},
): LatheResultData | null {
  const envelope = asRecord(payload);
  const rawResult = asRecord(envelope?.result) ?? envelope;
  if (!rawResult) {
    return null;
  }

  const gcode = typeof rawResult.program_text === 'string'
    ? rawResult.program_text
    : typeof rawResult.gcode === 'string'
      ? rawResult.gcode
      : '';
      ...
      return {
        ...
        name:
          typeof rawTool.tool_name === 'string'
            ? rawTool.tool_name
            : typeof rawTool.insert === 'string'
              ? rawTool.insert
              : `T${String(index + 1).padStart(2, '0')}`,
        operation:
          typeof rawTool.operation === 'string'
            ? rawTool.operation
            : typeof rawTool.description === 'string'
              ? rawTool.description
              : 'Turning operation',
        timeSeconds:
          typeof rawTool.cycle_time_sec === 'number'
            ? rawTool.cycle_time_sec
            : typeof rawTool.time_seconds === 'number'
              ? rawTool.time_seconds
              : 0,
      };
    })
    : [];

  const safetyChecks: SafetyCheck[] = Array.isArray(rawResult.safety_checks)
    ? rawResult.safety_checks.flatMap((entry, index) => {
      const rawCheck = asRecord(entry);
      if (!rawCheck) {
        return [];
      }
      ...
      return [{
        id:
          typeof rawCheck.id === 'string' && rawCheck.id.trim().length > 0
            ? rawCheck.id
            : `safety-${index + 1}`,
        label:
          typeof rawCheck.label === 'string'
            ? rawCheck.label
            : typeof rawCheck.name === 'string'
              ? rawCheck.name
              : `Safety check ${index + 1}`,
        passed:
          typeof rawCheck.passed === 'boolean'
            ? rawCheck.passed
            : typeof rawCheck.safe === 'boolean'
              ? rawCheck.safe
              : true,
        detail:
          typeof rawCheck.detail === 'string'
            ? rawCheck.detail
            : typeof rawCheck.message === 'string'
              ? rawCheck.message
              : undefined,
      }];
    })
  ...
  const toolStations: ToolStation[] = Array.isArray(rawResult.tool_stations)
    ? rawResult.tool_stations.flatMap((entry, index) => {
      const rawTool = asRecord(entry);
      if (!rawTool) {
        return [];
      }
      ...
      return [{
        station: typeof rawTool.station === 'number' ? rawTool.station : index + 1,
        toolName:
          typeof rawTool.toolName === 'string'
            ? rawTool.toolName
            : typeof rawTool.tool_name === 'string'
              ? rawTool.tool_name
              : `Tool ${index + 1}`,
        holderType:
          typeof rawTool.holderType === 'string'
            ? rawTool.holderType
            : typeof rawTool.holder_type === 'string'
              ? rawTool.holder_type
              : 'Holder pending',
        insertGrade:
          typeof rawTool.insertGrade === 'string'
            ? rawTool.insertGrade
            : typeof rawTool.insert_grade === 'string'
              ? rawTool.insert_grade
              : 'Grade pending',
      }];
    })
    : toolsUsed.map((tool) => ({
      ...
      toolName: tool.name,
      holderType: 'Holder pending',
      insertGrade: 'Grade pending',
    }));

  const measurementPoints = Array.isArray(rawResult.measurement_points)
    ? rawResult.measurement_points.flatMap((entry, index) => {
      const rawPoint = asRecord(entry);
      if (!rawPoint) {
        return [];
      }
      ...
      return [{
        label:
          typeof rawPoint.label === 'string'
            ...
            : `Measurement ${index + 1}`,
        nominal:
          typeof rawPoint.nominal === 'string'
            ? rawPoint.nominal
            : typeof rawPoint.nominal_value === 'string'
              ? rawPoint.nominal_value
              : 'See print',
        tolerance:
          typeof rawPoint.tolerance === 'string'
            ? rawPoint.tolerance
            : typeof rawPoint.tolerance_value === 'string'
              ? rawPoint.tolerance_value
              : 'Per print',
        tool:
          typeof rawPoint.tool === 'string'
            ? rawPoint.tool
            : typeof rawPoint.instrument === 'string'
              ? rawPoint.instrument
              : 'Inspection tool pending',
      }];
    })
    : [];

  const fileBaseName = options.fallbackFileName?.replace(/\.[^.]+$/, '');
  const partName =
    typeof rawResult.part_name === 'string' && rawResult.part_name.trim().length > 0
      ? rawResult.part_name
      : typeof rawResult.partNumber === 'string' && rawResult.partNumber.trim().length > 0
        ? rawResult.partNumber
        : fileBaseName && fileBaseName.trim().length > 0
          ? fileBaseName
          : 'Lathe result';
  const machineName =
    typeof rawResult.machine_name === 'string' && rawResult.machine_name.trim().length > 0
      ? rawResult.machine_name
      : typeof rawResult.machineName === 'string' && rawResult.machineName.trim().length > 0
        ? rawResult.machineName
        : options.fallbackMachineName ?? 'Selected lathe';
  const material =
    typeof rawResult.material === 'string' && rawResult.material.trim().length > 0
      ? rawResult.material
      : options.fallbackMaterial ?? 'Material pending';
  const confidenceExplanation =
    typeof rawResult.confidence_explanation === 'string' && rawResult.confidence_explanation.trim().length > 0
      ? rawResult.confidence_explanation
      : typeof rawResult.confidenceExplanation === 'string' && rawResult.confidenceExplanation.trim().length > 0
        ? rawResult.confidenceExplanation
        : 'Live result from the selected turning endpoint.';

  return {
    cycleTimeSeconds:
      typeof rawResult.estimated_cycle_time_sec === 'number'
        ? rawResult.estimated_cycle_time_sec
        : typeof rawResult.cycle_time_sec === 'number'
          ? rawResult.cycle_time_sec
          : 0,
    toolsUsed,
    costPerPart:
      typeof rawResult.cost_per_part === 'number'
        ? rawResult.cost_per_part
        : typeof rawResult.costPerPart === 'number'
          ? rawResult.costPerPart
          : 0,
    batchQuantity:
      typeof rawResult.batch_quantity === 'number'
        ? rawResult.batch_quantity
        : typeof rawResult.batchQuantity === 'number'
          ? rawResult.batchQuantity
          : options.fallbackBatchQuantity ?? 1,
    safetyChecks,
    confidenceScore: normalizeConfidence(rawResult.confidence_score ?? rawResult.confidence),
    confidenceExplanation,
    material,
    partName,
    machineName,
    gcode,
    moves: Array.isArray(rawResult.backplot_moves) ? rawResult.backplot_moves as BackplotMove[] : [],
    setupSteps: mapLatheSetupSteps(rawResult.setup_notes),
    toolStations,
    measurementPoints,
  };
}
...
type ResultTab = 'summary' | 'backplot' | 'setup' | 'gcode' | 'ai';
type ResultStatus = 'loading' | 'live' | 'unavailable';

type LatheResultsLocationState = {
  result?: LatheResultData;
  jobId?: string;
  wizardResult?: LatheWizardResult;
  workspaceContext?: MachineWorkspaceContext;
  fileName?: string;
  fileRoute?: 'photo' | 'cad' | 'pdf';
};
...
export function LatheResultsPage() {
  ...
  const [fetchedResult, setFetchedResult] = useState<LatheResultData | null>(null);
  const [resultStatus, setResultStatus] = useState<ResultStatus>('unavailable');
  const [fetchError, setFetchError] = useState<string | null>(null);

  const locationState = (location.state as LatheResultsLocationState | null) ?? null;
  const workspaceContext = locationState?.workspaceContext;
  const fallbackMaterial = locationState?.wizardResult?.material ?? workspaceContext?.materialLabel;
  ...
  useEffect(() => {
    ...
    if (!jobId) {
      ...
      setFetchError('No mounted lathe result was provided for this route.');
      return;
    }
    ...
    const poll = async () => {
      try {
        ...
        setFetchError(null);
        const response = await getLatheResult(jobId);
        if (cancelled) {
          return;
        }
        ...
        const mapped = mapLatheResultPayload(response.payload, {
          fallbackBatchQuantity: locationState?.wizardResult?.batchQuantity,
          fallbackFileName: locationState?.fileName,
          fallbackMachineName: workspaceContext?.machineLabel,
          fallbackMaterial,
        });
        ...
        setResultStatus('unavailable');
        setFetchError(
          'The live /api/v1/lathe/result route returned no usable result payload, so this screen stays unavailable instead of implying a generated CNC program.',
        );
      } catch (issue) {
        if (cancelled) {
          return;
        ...
        setFetchError(
          issue instanceof ApiError
            ? issue.message
            : 'The lathe result route is unavailable right now, so this screen stays unavailable instead of implying a generated CNC program.',
        );
      }
    };
    ...
    locationState?.wizardResult?.batchQuantity,
    workspaceContext?.machineLabel,
  ]);

  const result = useMemo(
    () => locationState?.result ?? fetchedResult,
  ...
  if (!result) {
    const unavailableDescription = resultStatus === 'loading'
      ? 'Waiting for the live /api/v1/lathe/result route to finish mounting this turning program.'
      : fetchError ?? 'No mounted lathe result is available for this route right now.';

    return (
      <div className="mx-auto flex w-full max-w-[1520px] flex-col gap-6">
        <WorkspaceHero
          eyebrow={resultStatus === 'loading' ? 'Fetching live lathe result' : 'Lathe result unavailable'}
          title={locationState?.fileName ?? 'Lathe Result'}
          description={unavailableDescription}
          metrics={
            <>
              <SummaryTile
                label="Result posture"
                value={resultStatus === 'loading' ? 'Loading' : 'Unavailable'}
                hint={locationState?.jobId ? `Job ${locationState.jobId}` : 'No live job mounted'}
                accent={resultStatus === 'loading' ? 'from-amber-400/22 via-amber-300/8 to-transparent' : 'from-rose-400/22 via-rose-300/8 to-transparent'}
              />
              <SummaryTile
                label="Material"
                value={fallbackMaterial ?? 'Pending'}
                hint={workspaceContext?.machineLabel ?? 'Machine context pending'}
              />
            </>
          }
        />
        ...
        {workspaceContext ? (
          <MachineWorkspaceAuthorityCard
            ...
            subtitle="The routed JM Die machine, controller, and programming posture stay attached even when the live lathe result feed is unavailable."
          />
        ) : null}

        <PanelCard title={resultStatus === 'loading' ? 'Waiting on live result' : 'Lathe result unavailable'}>
          <p className="text-sm text-slate-300">{unavailableDescription}</p>
          <p className="mt-2 text-xs text-slate-500">
            This page now fails closed instead of showing a seeded sample result when the live route has not mounted usable data yet.
          </p>
        </PanelCard>
          ...
          <ActionButton
            onClick={() =>
              navigate('/lathe/wizard', {
                state: {
                  fileName: locationState?.fileName ?? 'Re-edit',
                  fileRoute: locationState?.fileRoute ?? 'cad',
                  extractedMaterial: fallbackMaterial,
                  workspaceContext,
                },
              })
            }
          >
            Return to Turning Wizard
          </ActionButton>
    ...
    <div className="mx-auto flex w-full max-w-[1520px] flex-col gap-6">
      ...
      {activeTab === 'ai' && (
        <div role="tabpanel" id="tabpanel-ai" aria-labelledby="tab-ai">
          <LatheAIPanel
            controller={workspaceContext?.controllerId ?? 'fanuc_oi_tf'}
            gcode={result.gcode}
            material={result.material}
            machineName={result.machineName}
          />
      ...
      {/* Navigation */}
      <div className="flex gap-3">
        <ActionButton onClick={() => navigate(buildLatheProgramReleasePath(result, location.search, workspaceContext))}>
        ...
        <ActionButton onClick={() => navigate('/optimize', {
          state: buildLatheOptimizationState(
            result,
            locationState?.fileName ?? `${result.partName}.nc`,
            workspaceContext,
          ),
        })}>
        ...
        <ActionButton onClick={() => navigate('/cycle-time', {
          state: buildLatheCycleTimeState(
            result,
            locationState?.fileName ?? `${result.partName}.nc`,
            workspaceContext,
          ),
        ...
        <ActionButton onClick={() => navigate('/features', {
          state: buildLatheFeatureSelectionState(result, workspaceContext),
        })}>
          Open Feature Auto-Selection
        </ActionButton>
        <ActionButton onClick={() => navigate('/tool-optimization', {
          state: buildLatheToolOptimizationState(result, workspaceContext),
        })}>
        ...
        <ActionButton
          onClick={() =>
            navigate(
              `/ppg?source=lathe&recordType=Lathe%20Result&recordId=${encodeURIComponent(
                result.partName,
              )}`,
              {
                state: buildLathePostProcessorState(
                  result,
                  locationState?.fileName ?? `${result.partName}.nc`,
                  workspaceContext,
                ),
        ...
        <ActionButton onClick={() => navigate('/lathe/wizard', {
          state: {
            ...
            extractedData: {
              features: result.toolsUsed.map((t: any, i: number) => ({
                id: `feat-${i}`,
