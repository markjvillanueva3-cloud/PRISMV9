...
function normalizeFollowUp(value: Record<string, unknown>, index: number): FollowUpRecord {
  ...
  return {
    id: readString(value.id) || `${customerId || customerName}-${index}`,
    customerId,
    customerName,
    subject:
    ...
    details:
      ...
      readString(value.notes) ||
      'No additional follow-up details recorded.',
    ...
    status:
      ...
      (value.follow_up_done === true ? 'done' : 'pending'),
...
export function CustomersPage() {
    ...
    try {
      if (tab === 'list') {
        ...
        const rawFollowUps =
          ...
          [];
        setFollowUps(
          rawFollowUps.map((entry, index) => normalizeFollowUp(entry, index)),
        );
      }
    } catch (issue) {
  ...
  async function doCommHistory() {
    ...
    try {
      ...
      const rawHistory =
        ...
        [];
      setCommHistory(
        rawHistory.map((entry, index) => normalizeCommunication(entry, index)),
      );
    } catch (issue) {
  ...
  const activeCustomerContext = useMemo(() => {
    if (routeContext.origin.recordType === 'Customer' && routeContext.origin.recordId) {
      return {
        id: routeContext.origin.recordId,
        name: routeContext.origin.customer,
        lane: 'Workflow context',
    ...
    if (commCustId) {
      return { id: commCustId, name: '', lane: 'Communications', intent: 'bridge' as const, quoteId: routeContext.focus.quoteId };
    }

    if (creditCustId) {
      return { id: creditCustId, name: '', lane: 'Credit + Analytics', intent: 'bridge' as const, quoteId: routeContext.focus.quoteId };
    }

    if (analyticsCustId) {
      return { id: analyticsCustId, name: '', lane: 'Credit + Analytics', intent: 'production' as const, quoteId: routeContext.focus.quoteId };
    }
    ...
    if (tab === 'list' && customers[0]) {
      return {
        ...
        lane: 'Customer List',
        intent: customers[0].status === 'prospect' ? ('prototype' as const) : ('bridge' as const),
        quoteId: routeContext.focus.quoteId,
      };
    }
    ...
    if (tab === 'top' && topCustomers[0]) {
      return {
        id: topCustomers[0].customer_id,
        name: topCustomers[0].customer_name,
        lane: 'Top Customers',
    ...
    if (tab === 'followups' && followUps[0]) {
      return {
        id: followUps[0].customerId,
        name: followUps[0].customerName,
        lane: 'Follow-ups',
        intent: 'bridge' as const,
        quoteId: followUps[0].quoteId || routeContext.focus.quoteId,
      };
    }
  ...
  const effectiveOrigin = useMemo(
    () =>
      routeContext.origin.source
        ? {
            ...routeContext.origin,
            customer: routeContext.origin.customer || activeCustomerContext.name,
            note:
              routeContext.origin.note ||
              `Carry ${activeCustomerReference} context through quoting, follow-up, and evidence capture.`,
          }
        : {
            source: 'customers',
            recordType: 'Customer',
    ...
    [
      activeCustomerContext.id,
      activeCustomerContext.name,
  ...
  const workflowFocus = useMemo(
    () =>
      activeCustomerContext.quoteId || routeContext.focus.packetId
        ? {
            type: activeCustomerContext.quoteId ? 'quote' : 'packet',
            id: activeCustomerContext.quoteId || routeContext.focus.packetId,
            quoteId: activeCustomerContext.quoteId || '',
  ...
  }, [activeCustomerContext.intent, activeCustomerContext.lane, activeCustomerReference, effectiveOrigin, location.search, workflowFocus]);
  ...
  }, [activeCustomerReference, effectiveOrigin, location.search, workflowFocus]);
  const customerPortalPath = useMemo(() => {
    return buildWorkflowPath('/customer-portal', location.search, {
      origin: {
        ...effectiveOrigin,
        note: `Carry ${activeCustomerReference} into customer portal response and status review.`,
      },
      focus: workflowFocus,
      extras: {
        source: 'customers',
  ...
  const customerQuoteFollowUpPath = useMemo(() => {
    if (!activeCustomerContext.quoteId && !routeContext.focus.quoteId) {
      return '';
    }

    return buildWorkflowPath('/quote-follow-up', location.search, {
      origin: {
        ...effectiveOrigin,
        note: `Work the active quote follow-up for ${activeCustomerReference}.`,
      },
      focus: {
        type: 'quote',
        id: activeCustomerContext.quoteId || routeContext.focus.quoteId,
        quoteId: activeCustomerContext.quoteId || routeContext.focus.quoteId,
        packetId: routeContext.focus.packetId,
      },
      extras: {
        source: 'customers',
  ...
  }, [activeCustomerContext.quoteId, activeCustomerReference, effectiveOrigin, location.search, routeContext.focus.packetId, routeContext.focus.quoteId]);
            ...
            <Card title="Follow-ups" subtitle="Outstanding customer follow-up work stays visible in one simple lane.">
              <div className="grid gap-3">
                {followUps.map((followUp, index) => (
                  <div key={`${followUp.id}-${index}`} className="rounded-[22px] border border-white/8 bg-white/[0.03] px-4 py-4">
                    <div className="grid gap-3 md:grid-cols-[1.1fr_120px_140px_1.2fr_100px]">
                      <div>
                        <div className="text-sm font-semibold text-slate-50">{followUp.customerName}</div>
                        <div className="mt-2 text-xs text-slate-400">{followUp.customerId || 'No customer ID'} · {followUp.subject}</div>
                      </div>
                      <div className="capitalize text-slate-300">{followUp.type}</div>
                      <div className="font-mono text-slate-300">{formatDate(followUp.dueDate)}</div>
                      <div className="truncate text-slate-300">{followUp.details}</div>
                      <div className="text-slate-100">{followUp.status}</div>
                    </div>
                    <div className="mt-4 flex flex-wrap gap-3">
                      {followUp.quoteId ? (
                        <Link
                          to={buildWorkflowPath('/quote-follow-up', location.search, {
                            origin: {
                              ...effectiveOrigin,
                              recordId: followUp.customerId || effectiveOrigin.recordId,
                              customer: followUp.customerName || effectiveOrigin.customer,
                              note: `Carry CRM follow-up context into quote follow-up for ${followUp.customerName || followUp.customerId}.`,
                            },
                            focus: {
                              type: 'quote',
                              id: followUp.quoteId,
                          ...
                          className="inline-flex items-center justify-center rounded-2xl bg-rose-300 px-4 py-3 text-sm font-semibold text-slate-950 transition hover:bg-rose-200"
                        >
                          Open Quote Follow-Up
                        </Link>
                      ) : null}
                      <Link
                        to={buildWorkflowPath('/customer-portal', location.search, {
                          origin: {
                            ...effectiveOrigin,
                            recordId: followUp.customerId || effectiveOrigin.recordId,
                            customer: followUp.customerName || effectiveOrigin.customer,
                            note: `Carry CRM follow-up context into customer portal review for ${followUp.customerName || followUp.customerId}.`,
                          },
                          focus: followUp.quoteId
                            ? {
                                type: 'quote',
                                id: followUp.quoteId,
                                quoteId: followUp.quoteId,
              ...
              {commHistory.length > 0 ? (
                <div className="mt-5 grid gap-3">
                  {commHistory.map((entry) => (
                    <div key={entry.id} className="rounded-[22px] border border-white/8 bg-black/20 px-4 py-4 text-sm text-slate-200">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div className="font-semibold text-slate-50">{entry.subject}</div>
                        <div className="text-xs uppercase tracking-[0.18em] text-slate-500">{entry.type}</div>
                      </div>
                      <div className="mt-2 text-xs text-slate-400">{formatDate(entry.date)} · {entry.loggedBy}</div>
                      <div className="mt-3 leading-6 text-slate-300">{entry.details}</div>
                    </div>
                  ))}
                </div>
              ) : null}
          ...
          <Card title="Customer handoff" subtitle="Keep customer and RFQ context attached when commercial work moves into quote review or evidence capture.">
            <div className="space-y-3">
              ...
              <div className="grid gap-3">
                ...
                <Link
                  to={customerPortalPath}
                  className="inline-flex items-center justify-center rounded-2xl border border-white/12 bg-white/[0.03] px-5 py-3 text-sm font-semibold text-slate-100 transition hover:border-white/20 hover:bg-white/[0.05]"
                >
                  Open Customer Portal
                </Link>
                {customerQuoteFollowUpPath ? (
                  <Link
                    to={customerQuoteFollowUpPath}
                    className="inline-flex items-center justify-center rounded-2xl border border-white/12 bg-white/[0.03] px-5 py-3 text-sm font-semibold text-slate-100 transition hover:border-white/20 hover:bg-white/[0.05]"
                  >
                    Open Quote Follow-Up
                  </Link>
