...
import {
  ...
  chooseToolpathSelectorValue,
  readProgramReleaseRouteSelection,
  resolveToolpathAdvisorReleaseContext,
  resolveToolpathAdvisorRouteAuthority,
  type ToolpathAdvisorAxisProfile as AxisProfile,
  type ToolpathAdvisorMaterialProfile as MaterialProfile,
...
function programmingModeForAxis(axis: AxisProfile) {
  return axis === 'lathe' ? 'lathe' : 'mill';
}
...
export function ToolpathAdvisorPage() {
  ...
  const [programReleaseCatalog, setProgramReleaseCatalog] = useState(PROGRAM_RELEASE_CATALOG);
  const [selectorAuthority, setSelectorAuthority] = useState(() => buildToolpathAdvisorSelectorAuthority(PROGRAM_RELEASE_CATALOG));
  const [feature, setFeature] = useState<FeatureType>('pocket');
  const [material, setMaterial] = useState<MaterialProfile>(selectorAuthority.defaultMaterial);
  const [axis, setAxis] = useState<AxisProfile>(selectorAuthority.defaultAxis);
  ...
  useEffect(() => {
    ...
    operatingSystem
      ...
      .then((catalog) => {
        ...
        const nextAuthority = buildToolpathAdvisorSelectorAuthority(catalog);
        startTransition(() => {
          setProgramReleaseCatalog(catalog);
          setSelectorAuthority(nextAuthority);
          setAxis((current) =>
            chooseToolpathSelectorValue(
              current,
              nextAuthority.machineOptions.map((option) => option.value),
              nextAuthority.defaultAxis,
            ),
          );
          setMaterial((current) =>
            chooseToolpathSelectorValue(
              current,
              nextAuthority.materialOptions.map((option) => option.value),
              nextAuthority.defaultMaterial,
            ),
          );
        });
      })
      .catch(() => {
        if (active) {
          startTransition(() => {
            setProgramReleaseCatalog(PROGRAM_RELEASE_CATALOG);
            setSelectorAuthority(buildToolpathAdvisorSelectorAuthority(PROGRAM_RELEASE_CATALOG));
          });
        }
      });
  ...
  const routedToolpathAuthority = useMemo(
    () => resolveToolpathAdvisorRouteAuthority(programReleaseCatalog, routeSelection),
    [programReleaseCatalog, routeSelection],
  );

  useEffect(() => {
    if (!routedToolpathAuthority) {
      return;
    }
    ...
    startTransition(() => {
      setMaterial(
        chooseToolpathSelectorValue(
          routedToolpathAuthority.material,
          selectorAuthority.materialOptions.map((option) => option.value),
          selectorAuthority.defaultMaterial,
        ),
      );
      if (!routedToolpathAuthority.unsupportedReason) {
        setAxis(
          chooseToolpathSelectorValue(
            routedToolpathAuthority.axis,
            selectorAuthority.machineOptions.map((option) => option.value),
            selectorAuthority.defaultAxis,
          ),
        );
      }
    });
    ...
    selectorAuthority.defaultAxis,
    selectorAuthority.defaultMaterial,
    selectorAuthority.machineOptions,
    selectorAuthority.materialOptions,
  ]);
  ...
  const selectedMaterial = useMemo(
    () => selectorAuthority.materialOptions.find((option) => option.value === material) ?? selectorAuthority.materialOptions[0],
    [material, selectorAuthority.materialOptions],
  );

  const selectedAxis = useMemo(
    () => selectorAuthority.machineOptions.find((option) => option.value === axis) ?? selectorAuthority.machineOptions[0],
    [axis, selectorAuthority.machineOptions],
  );
  ...
  const releaseContext = useMemo(
    () => routedToolpathAuthority?.releaseContext ?? resolveToolpathAdvisorReleaseContext(programReleaseCatalog, axis, material),
    [axis, material, programReleaseCatalog, routedToolpathAuthority],
  );
  ...
  useEffect(() => {
    ...
    const mode = programmingModeForAxis(axis);
    const buildAuthority = (programmingCatalogState?: Awaited<ReturnType<typeof fetchProgrammingCatalogState>>) =>
      buildMachineWorkspaceProgrammingAuthority({
        mode,
        machineFamilyId: axis === 'lathe' ? 'lathe' : axis,
        machineLabel: releaseContext.machineLabel ?? selectedAxis.label,
        machineKinematics: selectedAxis.label,
        controllerLabel: releaseContext.controller ?? selectedAxis.controller,
        programmingCatalogState,
      }) ?? null;

    const fallbackAuthority = buildAuthority();
    startTransition(() => {
      setProgrammingAuthority(fallbackAuthority);
    });

    fetchProgrammingCatalogState(mode)
      .then((programmingCatalogState) => {
        ...
        startTransition(() => {
          setProgrammingAuthority(buildAuthority(programmingCatalogState));
        });
      })
      .catch(() => {
        if (!active) {
  ...
  }, [axis, releaseContext.controller, releaseContext.machineLabel, routedToolpathAuthority?.unsupportedReason, selectedAxis.controller, selectedAxis.label]);
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.75fr)]">
          <div className="space-y-5">
            ...
            <div className="space-y-3">
              ...
              <p className="max-w-3xl text-sm leading-6 text-sky-100/80">
                {routedToolpathAuthority?.authorityNote ?? selectorAuthority.authorityNote}
              </p>
            </div>
            {routedToolpathAuthority ? (
              <div className={`max-w-3xl rounded-[22px] border px-4 py-4 text-sm leading-6 ${
                routedToolpathAuthority.unsupportedReason
                  ? 'border-amber-300/18 bg-amber-300/[0.08] text-amber-50'
                  : 'border-cyan-300/18 bg-cyan-300/[0.08] text-cyan-50'
              }`}>
                <div className="font-semibold">
                  {routedToolpathAuthority.unsupportedReason
                    ? routedToolpathAuthority.unsupportedReason
                    : 'Toolpath Advisor is honoring the routed JM Die release packet instead of falling back to a generic machine/material guess.'}
                </div>
                <div className="mt-2 text-slate-200">{releaseContext.authorityNote}</div>
              </div>
            ) : null}
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <div className="rounded-[22px] border border-white/8 bg-white/[0.03] px-4 py-4">
                <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500">Programming posture</div>
                {programmingAuthority ? (
                  <>
                    <div className="mt-2 flex items-center gap-2">
                      ...
                      <StatusPill
                        label={programmingAuthority.posture}
                        tone={programmingAuthorityTone(programmingAuthority.posture)}
                      />
                    </div>
                    <div className="mt-1 text-sm text-slate-400">{programmingAuthority.summary}</div>
                  ...
                  <div className="mt-2 text-sm text-slate-400">
                    {routedToolpathAuthority?.unsupportedReason
                      ? 'Programming posture stays on the routed Print to CNC packet until this unsupported machine family gets a dedicated toolpath contract.'
                      : 'Resolving the shared JM Die programming spine for this posture.'}
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="mt-4 space-y-4">
                ...
                <div className="space-y-2 rounded-[22px] border border-cyan-300/12 bg-cyan-300/[0.05] px-4 py-4">
                  ...
                  <div className="text-xs text-slate-300/80">
                    {releaseContext.authorityNote}
                  </div>
                  {programmingAuthority ? (
                    <div className="rounded-[18px] border border-violet-300/12 bg-violet-300/[0.05] px-3 py-3 text-xs text-slate-200">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-semibold uppercase tracking-[0.16em] text-violet-100">Programming authority</span>
                        <StatusPill
                          label={programmingAuthority.badge}
                          tone={programmingAuthorityTone(programmingAuthority.posture)}
                        />
                      </div>
                  ...
                  {releaseContext.machineLabel || releaseContext.toolholderLabel || releaseContext.toolingPackageLabel || releaseContext.stockLabel ? (
                    <div className="text-xs text-cyan-100/80">
                      Release spine: {[releaseContext.machineLabel, releaseContext.toolholderLabel, releaseContext.toolingPackageLabel, releaseContext.stockLabel].filter(Boolean).join(' / ')}
                    </div>
                  ) : null}
                  {backendSf && (
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(320px,0.7fr)_minmax(0,1.3fr)]">
        <div className="rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(10,17,24,0.96)_0%,rgba(5,10,16,0.96)_100%)] p-5 shadow-[0_28px_90px_rgba(0,0,0,0.28)]">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-slate-100 outline-none transition focus:border-cyan-300/35 focus:ring-2 focus:ring-cyan-400/15"
              >
                {selectorAuthority.materialOptions.map((option) => (
                  <option key={option.value} value={option.value}>
                    {option.label}
                  </option>
                ))}
            ...
            <div>
              ...
              <select
                ...
                className="w-full rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-slate-100 outline-none transition focus:border-cyan-300/35 focus:ring-2 focus:ring-cyan-400/15"
              >
                {selectorAuthority.machineOptions.map((option) => (
                  <option key={option.value} value={option.value}>
                    {option.label}
                  </option>
                ))}
          ...
          <button
            ...
            disabled={Boolean(routedToolpathAuthority?.unsupportedReason)}
          ...
          >
            {routedToolpathAuthority?.unsupportedReason ? 'Toolpath Advisor unavailable for this routed posture' : 'Get Strategies'}
          </button>
        </div>
