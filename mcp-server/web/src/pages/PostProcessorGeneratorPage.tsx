...
import { MaterialSearchPanel, type MaterialSelection } from '../components/ppg/MaterialSearchPanel';
...
export function PostProcessorGeneratorPage() {
  ...
  const routedProgramName = useMemo(
    () =>
  ...
  const [pipelineStages, setPipelineStages] = useState<Array<{stage: string; status: string; data: Record<string, unknown>}>>([]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const r = (res as Record<string, unknown>).result as Record<string, unknown> | undefined;
        if (r) {
                      ...
                      <ActionButton
                        onClick={async () => {
                          try {
                            ...
                            const blob = new Blob([String((res as Record<string, unknown>).result ?? generated.preview)], { type: 'text/plain' });
                            const url = URL.createObjectURL(blob);
                            const a = document.createElement('a');
                            a.href = url;
              ...
              <PostLibraryUI
                ...
                onGenerateForMachine={(post) => {
                  ...
                  if ((post as any).machine_profile?.max_rpm) {
                    const profile = (post as any).machine_profile;
