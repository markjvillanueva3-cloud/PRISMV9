...
import type { HotJobRecord, ProgramReleaseCatalog, SchedulingStudyRecord, StudyExceptionRecord, StudyTone } from '../features/operating-system/contracts';
...
export function SchedulingPage() {
  ...
  const releaseMachineSeed = useMemo(
    () => resolveProgramReleaseMachineRouteSeed(releaseCatalog, [selectedPrimaryMachine]),
    [releaseCatalog, selectedPrimaryMachine],
  );
  const plannerReleaseExtras = useMemo(
    () =>
  ...
  const plannerReleasePath = useMemo(
    () =>
      buildWorkflowPath('/print-to-cnc', location.search, {
        origin: effectiveOrigin,
        focus: selectedPrimaryJobId ? { type: 'job', id: selectedPrimaryJobId, jobId: selectedPrimaryJobId, packetId: selectedPrimaryJobId } : effectiveFocus,
        extras: {
          source: 'scheduling-desk',
          ...plannerReleaseExtras,
        },
      }),
    [effectiveFocus, effectiveOrigin, location.search, plannerReleaseExtras, selectedPrimaryJobId],
  );
  const hotReleaseItems = useMemo(
    () =>
      hotJobs.slice(0, 3).map((hotJob) => ({
        ...
        releasePath: buildWorkflowPath('/print-to-cnc', location.search, {
          ...
          extras: buildProgramReleaseRouteExtras({
            catalog: releaseCatalog,
            routeSelection: {
              partClassId: inferProgramReleasePartClassId(hotJob.partNumber, hotJob.note),
            },
          }),
        }),
    ...
    [hotJobs, location.search, releaseCatalog],
  );
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <div className="mt-2 text-sm font-normal leading-6 text-sky-50/80">
                    Carry the selected study’s primary job and machine posture into release review so the planner handoff does not reopen the packet cold.
                  </div>
                </Link>
                <Link
