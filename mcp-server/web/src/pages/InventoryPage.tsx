...
import { AppwInventoryCopilot } from '../components/puoa/AppwInventoryCopilot';
...
function InventorySourcePostureCard({
  label,
  detail,
  refreshedAt,
  issue,
...
export function InventoryPage() {
  ...
  const [eoqRefreshedAt, setEoqRefreshedAt] = useState<number | null>(null);
  const [abcRefreshedAt, setAbcRefreshedAt] = useState<number | null>(null);
  const [safetyRefreshedAt, setSafetyRefreshedAt] = useState<number | null>(null);
  const [toolOptRefreshedAt, setToolOptRefreshedAt] = useState<number | null>(null);
  const [workspaceIssue, setWorkspaceIssue] = useState<string | null>(null);
  const [eoqIssue, setEoqIssue] = useState<string | null>(null);
  const [abcIssue, setAbcIssue] = useState<string | null>(null);
  const [safetyIssue, setSafetyIssue] = useState<string | null>(null);
  const [toolOptIssue, setToolOptIssue] = useState<string | null>(null);
  ...
  const launcherSourceLabel = useMemo(
    () => (sourceContext ? formatWorkflowSourceLabel(sourceContext) : null),
    [sourceContext],
  );
  ...
  const toolingFocusId =
    tab === 'toolopt' && seededToolId
      ? seededToolId
      : activeCheckoutRecord?.toolId ?? activeUsagePulse?.id ?? (tab === 'toolopt' ? seededToolId : '');
  const toolingFocusLabel =
    tab === 'toolopt' && seededToolId
      ? seededToolId
      : activeCheckoutRecord?.label ?? activeUsagePulse?.label ?? 'No active tool record';
      ...
      buildCapturePath(location.pathname, location.search, {
        ...
        note:
          tab === 'receiving' || tab === 'documents'
            ...
            : `Capture tooling condition, insert indexing, and checkout proof for ${toolingFocusLabel || 'the active tooling record'}.`,
        origin: effectiveOrigin,
        focus: effectiveFocus,
      }),
    [activeDepartment, activeReceivingRecord?.reference, activeUsagePulse?.machine, effectiveFocus, effectiveOrigin, location.pathname, location.search, tab, toolingFocusId, toolingFocusLabel],
  );
        ...
        extras: {
          ...
          note:
            tab === 'receiving' || tab === 'documents'
              ...
              : `Follow up on tooling checkout, insert indexing, and cost-per-part posture for ${toolingFocusLabel || 'the active tooling record'} without losing the upstream commercial origin.`,
        },
      }),
    [activeReceivingRecord?.reference, effectiveFocus, effectiveOrigin, location.search, tab, toolingFocusLabel],
  );
  ...
  useEffect(() => {
    ...
    operatingSystem.getInventoryOperationsWorkspace().then((workspace) => {
      ...
      if (active) {
        setOperationsWorkspace(null);
        setWorkspaceIssue('Inventory operations workspace unavailable.');
      }
    });
    return () => {
      active = false;
  ...
  async function calcEOQ() {
    setLoading(true); setError(null); setEoqIssue(null);
    try {
      ...
      setEoqResult(response.result as unknown as EOQResult); setEoqRefreshedAt(Date.now()); setTab('eoq');
    } catch (issue) {
      const message = issue instanceof ApiError ? issue.message : 'Failed to calculate EOQ';
      setError(message);
      setEoqIssue(message);
  ...
  async function calcABC() {
    setLoading(true); setError(null); setAbcIssue(null);
    try {
      ...
      setAbcResult(response.result as unknown as ABCClassification); setAbcRefreshedAt(Date.now()); setTab('abc');
    } catch (issue) {
      const message = issue instanceof ApiError ? issue.message : 'Failed to run ABC analysis';
      setError(message);
      setAbcIssue(message);
    } finally { setLoading(false); }
  }
  ...
  async function calcSafetyStock() {
    setLoading(true); setError(null); setSafetyIssue(null);
    try {
      ...
      setSafetyResult((response.result as Record<string, number>) ?? null); setSafetyRefreshedAt(Date.now()); setTab('safety');
    } catch (issue) {
      const message = issue instanceof ApiError ? issue.message : 'Calculation failed';
      setError(message);
      setSafetyIssue(message);
    } finally { setLoading(false); }
  }

  async function calcToolOptimize() {
    setLoading(true); setError(null); setToolOptIssue(null);
    try {
      ...
      setToolOptResult(response.result); setToolOptRefreshedAt(Date.now()); setTab('toolopt');
    } catch (issue) {
      const message = issue instanceof ApiError ? issue.message : 'Optimization failed';
      setError(message);
      setToolOptIssue(message);
    } finally { setLoading(false); }
  }
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1520px] flex-col gap-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.9fr)]">
        ...
        <div className="space-y-6">
          <AppwInventoryCopilot
            deskLabel="Inventory"
            activeLaneLabel={activeTab.label}
            activeLaneDetail={activeTab.detail}
            workflowReference={workflowReference}
            launcherSourceLabel={launcherSourceLabel}
            upstreamSourceLabel={upstreamSourceLabel}
            workspaceSummary={operationsWorkspace?.summary ?? null}
            documentTemplateCount={operationsWorkspace?.documentTemplates.length ?? 0}
            receivingCount={receivingQueue.length}
            checkoutCount={checkoutQueue.length}
            usagePulseCount={usagePulses.length}
            departmentRouteCount={operationsWorkspace?.departmentRoutes.length ?? 0}
            formulaNoteCount={operationsWorkspace?.formulaNotes.length ?? 0}
            activeReceivingReference={activeReceivingRecord?.reference ?? null}
            activeReceivingSupplier={activeReceivingRecord?.supplier ?? null}
            activeReceivingDepartment={activeReceivingRecord?.department ?? null}
            activeReceivingStatus={activeReceivingRecord?.status ?? null}
            activeCheckoutToolId={activeCheckoutRecord?.toolId ?? null}
            activeCheckoutLabel={activeCheckoutRecord?.label ?? null}
            activeCheckoutLocation={activeCheckoutRecord?.location ?? null}
            activeCheckoutStatus={activeCheckoutRecord?.status ?? null}
            activeUsageMachine={activeUsagePulse?.machine ?? null}
            activeUsageOperator={activeUsagePulse?.operator ?? null}
            activeUsageState={activeUsagePulse?.state ?? null}
            activeUsageEdgesLabel={activeUsagePulse ? renderUsageLabel(activeUsagePulse) : null}
            activeUsageCostPerPart={activeUsagePulse?.costPerPart ?? null}
            eoqValue={eoqResult?.eoq ?? null}
            abcClassCount={abcResult?.summary.length ?? 0}
            topAbcClass={abcResult?.summary[0]?.class ?? null}
            safetyStockValue={safetyResult?.safety_stock ?? null}
            reorderPointValue={safetyResult?.reorder_point ?? null}
            toolOptimizationReady={Boolean(toolOptResult)}
            toolOptimizationFocus={toolOptForm.tool_id || activeCheckoutRecord?.toolId || activeUsagePulse?.id || null}
            workspaceRefreshedAt={workspaceRefreshedAt}
            eoqRefreshedAt={eoqRefreshedAt}
            abcRefreshedAt={abcRefreshedAt}
            safetyRefreshedAt={safetyRefreshedAt}
            toolOptRefreshedAt={toolOptRefreshedAt}
            workspaceIssue={workspaceIssue}
            eoqIssue={eoqIssue}
            abcIssue={abcIssue}
            safetyIssue={safetyIssue}
            toolOptIssue={toolOptIssue}
          />

          <PanelCard title="Source posture" subtitle="Keep mounted workspace and analysis routes explicit so the desk never hides whether its intelligence is live, stale, or degraded.">
            <div className="space-y-3">
              <InventorySourcePostureCard
                label="Mounted inventoryOperations workspace"
                detail="Canonical receiving, routing, tooling custody, and formula-note workspace for the desk."
                refreshedAt={workspaceRefreshedAt}
                issue={workspaceIssue}
              />
              <InventorySourcePostureCard
                label="Mounted inventoryEOQ route"
                detail="Economic order quantity posture for replenishment cadence and order batching."
                refreshedAt={eoqRefreshedAt}
                issue={eoqIssue}
              />
              <InventorySourcePostureCard
                label="Mounted inventoryABC route"
                detail="Spend-priority classification so high-value inventory attention stays visible."
                refreshedAt={abcRefreshedAt}
                issue={abcIssue}
              />
              <InventorySourcePostureCard
                label="Mounted inventorySafetyStock route"
                detail="Service-level buffer posture for reorder timing and stockout resilience."
                refreshedAt={safetyRefreshedAt}
                issue={safetyIssue}
              />
              <InventorySourcePostureCard
                label="Mounted inventoryToolOptimize route"
                detail="Tool-life and regrind economics lane for consumable and tooling decisions."
                refreshedAt={toolOptRefreshedAt}
                issue={toolOptIssue}
              />
            </div>
          </PanelCard>
            ...
            <div className="space-y-3">
              <div className="rounded-[20px] border border-white/8 bg-white/[0.03] px-4 py-4 text-sm leading-6 text-slate-300">
                {tab === 'receiving' || tab === 'documents' ? (
                  ...
                  <>
                    Tooling focus: <span className="font-semibold text-slate-100">{toolingFocusLabel}</span>
