...
export function GeneralLedgerPage() {
  ...
  const routedPayrollPeriod = routedPeriodId || (routedPeriodStart && routedPeriodEnd ? `${routedPeriodStart} to ${routedPeriodEnd}` : '');
  const [recordType, setRecordType] = useState<RecordType>(
    launcherSource === 'payroll' || Boolean(routedPayrollPeriod) ? 'payroll' : 'invoice',
  );
  const [recordForm, setRecordForm] = useState({
    ...
    vendor_id: '',
    employee_count: '',
    period: routedPayrollPeriod,
    gross: '',
    taxes: '',
    net: '',
    date: routedPeriodEnd || new Date().toISOString().slice(0, 10),
    debit_account: '',
    credit_account: '',
    reference: '',
  });
    ...
    try {
      ...
      if (recordType === 'invoice') {
        ...
        response = await glRecordPayroll({
          period: recordForm.period || routedPayrollPeriod,
          gross: parseFloat(recordForm.gross) || 0,
          taxes: parseFloat(recordForm.taxes) || 0,
          net: parseFloat(recordForm.net) || 0,
          ...
          {recordType === 'payroll' ? (
            <>
              <Field label="Payroll period">
                <Input type="text" value={recordForm.period} onChange={(event) => setRecordForm((current) => ({ ...current, period: event.target.value }))} placeholder="2026-03-16 to 2026-03-29" />
              </Field>
              <Field label="Gross">
                <Input type="number" value={recordForm.gross} onChange={(event) => setRecordForm((current) => ({ ...current, gross: event.target.value }))} />
              </Field>
              <Field label="Taxes">
                <Input type="number" value={recordForm.taxes} onChange={(event) => setRecordForm((current) => ({ ...current, taxes: event.target.value }))} />
              </Field>
              <Field label="Net">
                <Input type="number" value={recordForm.net} onChange={(event) => setRecordForm((current) => ({ ...current, net: event.target.value }))} />
              </Field>
