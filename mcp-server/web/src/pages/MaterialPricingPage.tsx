...
import { AppwMaterialPricingCopilot } from '../components/puoa/AppwMaterialPricingCopilot';
...
function PricingSourcePostureCard({
  label,
  source,
  refreshedAt,
  selection,
...
export function MaterialPricingPage() {
  ...
  const [lookupRefreshedAt, setLookupRefreshedAt] = useState<number | null>(null);
  const [compareRefreshedAt, setCompareRefreshedAt] = useState<number | null>(null);
  const [surchargeRefreshedAt, setSurchargeRefreshedAt] = useState<number | null>(null);
  const [lookupIssue, setLookupIssue] = useState<string | null>(null);
  const [compareIssue, setCompareIssue] = useState<string | null>(null);
  const [surchargeIssue, setSurchargeIssue] = useState<string | null>(null);
  ...
  const cheapestComparison = useMemo(
    () =>
      comparisons.reduce<PriceComparison | null>(
        (current, candidate) => (current == null || candidate.total_per_kg < current.total_per_kg ? candidate : current),
        null,
      ),
    [comparisons],
  );
  const surchargeSummary = useMemo(() => {
    if (!surchargeResult || typeof surchargeResult !== 'object') {
      return '';
    }

    const entries = Object.entries(surchargeResult as Record<string, unknown>)
  ...
  async function handleLookup() {
    ...
    try {
      ...
      setLookupRefreshedAt(Date.now());
    } catch (issue) {
      const message = issue instanceof ApiError ? issue.message : 'Lookup failed';
      setError(message);
      setLookupIssue(message);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleCompare() {
    ...
    try {
      ...
      const message = issue instanceof ApiError ? issue.message : 'Comparison failed';
      setError(message);
  ...
  async function handleSurchargeLookup() {
    setLoading(true);
    setError(null);
    setSurchargeIssue(null);
    try {
      const response = await materialSurcharge({ material: surchargeMaterial });
      setSurchargeResult(response.result);
      setSurchargeRefreshedAt(Date.now());
    } catch (issue) {
      const message = issue instanceof ApiError ? issue.message : 'Failed to load surcharge data';
      setError(message);
      setSurchargeIssue(message);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1520px] flex-col gap-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.08fr)_minmax(360px,0.92fr)]">
        ...
        <div className="space-y-6">
          <AppwMaterialPricingCopilot
            deskLabel="Material Pricing desk"
            activeLaneLabel={activeTab.label}
            activeLaneDetail={activeTab.detail}
            lookupMaterial={lookupForm.material}
            lookupForm={lookupForm.form}
            lookupRegion={lookupForm.region}
            lookupBasePrice={lookupResult?.base_price_kg ?? null}
            lookupAdjustedPrice={lookupResult?.adjusted_price_kg ?? null}
            lookupTotalPrice={lookupResult?.total_per_kg ?? null}
            lookupSurchargeCount={lookupResult?.surcharges?.length ?? 0}
            comparisonMaterialCount={comparisonCount}
            comparisonResultCount={comparisons.length}
            comparisonCheapestMaterial={cheapestComparison?.material ?? null}
            comparisonCheapestPrice={cheapestComparison?.total_per_kg ?? null}
            surchargeMaterial={surchargeMaterial}
            surchargeSummary={surchargeSummary || null}
            lookupRefreshedAt={lookupRefreshedAt}
            compareRefreshedAt={compareRefreshedAt}
            surchargeRefreshedAt={surchargeRefreshedAt}
            lookupIssue={lookupIssue}
            compareIssue={compareIssue}
            surchargeIssue={surchargeIssue}
          />
          ...
          <PanelCard title="Source posture" subtitle="Expose route freshness and degraded-lane posture directly inside the pricing desk before a buy decision leaves the quote flow.">
            <div className="grid gap-3">
              <PricingSourcePostureCard
                label="Price lookup"
                source="Mounted materialPriceLookup route"
                refreshedAt={lookupRefreshedAt}
                selection={`${lookupForm.material} / ${lookupForm.form} / ${lookupForm.region}`}
                unavailableReason={lookupIssue}
              />
              <PricingSourcePostureCard
                label="Compare materials"
                source="Mounted materialPriceCompare route"
                refreshedAt={compareRefreshedAt}
                selection={`${comparisonCount} alloy${comparisonCount === 1 ? '' : 's'} / ${compareForm.form}`}
                unavailableReason={compareIssue}
              />
              <PricingSourcePostureCard
                label="Surcharges"
                source="Mounted materialSurcharge route"
                refreshedAt={surchargeRefreshedAt}
                selection={surchargeMaterial}
                unavailableReason={surchargeIssue}
              />
            </div>
          </PanelCard>
        </div>
