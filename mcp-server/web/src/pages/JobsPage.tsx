...
import { AppwJobsCopilot } from '../components/puoa/AppwJobsCopilot';
...
import type { HotJobRecord, JobDeskRecord, ProgramReleaseCatalog } from '../features/operating-system/contracts';
...
import { inferProgramReleasePartClassId } from '../utils/programReleasePartClassInference';
...
export function JobsPage() {
  ...
  const downstreamWorkflowFocus = useMemo(
    () =>
      routeContext.focus.id
        ? routeContext.focus
        : activeWorkflowJobId
          ? { type: 'job', id: activeWorkflowJobId, jobId: activeWorkflowJobId }
          : undefined,
    [activeWorkflowJobId, routeContext.focus],
  );
  const downstreamProgramReleaseFocus = useMemo(
    () =>
      routeContext.focus.id
        ? routeContext.focus
        : activeWorkflowJobId
  ...
  }, [downstreamWorkflowFocus, effectiveOrigin, location.search]);
  ...
  const releaseMachineSeed = useMemo(
    () =>
      resolveProgramReleaseMachineRouteSeed(releaseCatalog, [
        mountedMachineId,
        selectedMachineQueue?.machine_id,
        selectedMachineQueue?.machine_name,
        selectedPacketMachine,
      ]),
    [mountedMachineId, releaseCatalog, selectedMachineQueue?.machine_id, selectedMachineQueue?.machine_name, selectedPacketMachine],
  );
    ...
    () =>
      buildProgramReleaseRouteExtras({
        ...
        routeSelection: {
          ...
          machineFamilyId: releaseMachineSeed?.machineFamilyId,
          machineManufacturer: releaseMachineSeed?.machineManufacturer,
          partClassId: inferProgramReleasePartClassId(
            selectedJob?.part_number,
            selectedJob?.description,
            selectedPacket?.partNumber,
            selectedPacket?.jobName,
          ),
        },
      }),
    [
  ...
  }, [downstreamProgramReleaseFocus, effectiveOrigin, location.search, releaseRouteExtras]);
  ...
  const selectedTravelerProgress = liveTravelerSummary
    ? `${liveTravelerSummary.completed_steps}/${liveTravelerSummary.total_steps} complete`
    : selectedDesk?.traveler.length
      ? `${selectedDesk.traveler.length} desk steps staged`
      : 'No direct traveler record yet';
  const selectedTravelerStep = liveTravelerSummary?.current_step
    ? `Op ${liveTravelerSummary.current_step.step_number} · ${liveTravelerSummary.current_step.operation}`
    : fallbackTravelerStep?.title ?? 'Traveler step pending';
  const selectedTravelerVariance = liveTravelerSummary
    ? `Setup ${liveTravelerSummary.setup_variance_pct}% · Cycle ${liveTravelerSummary.cycle_variance_pct}%`
    : selectedDesk?.traveler.length
      ? 'Desk fallback is active until mounted traveler timing exists.'
      : 'Traveler timing unavailable';
  const selectedDispatchQueueStatus = selectedMachineQueue
  ...
  const selectedQueueLoad = selectedMachineQueue
  ...
  const selectedShortageCount = selectedDesk?.shortages.length ?? 0;
  const selectedBlockedApprovalCount = selectedDesk?.approvals.filter((approval) => approval.status === 'blocked').length ?? 0;
  const dispatchLaneSnapshot = useMemo(
    () => dispatchLanes.map((lane) => ({ label: lane.label, count: lane.items.length })),
    [dispatchLanes],
  );
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const createdId = (response.result as { id?: string } | undefined)?.id;
      if (createdId) {
  ...
  async function handleToggleHotJob(job: Job) {
    setError(null);
    try {
      const nextHotJobs = services.isJobHot(job.id)
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.12fr)_minmax(360px,0.88fr)]">
        ...
        <div className="space-y-6">
          <AppwJobsCopilot
            ...
            partNumber={selectedJob?.part_number ?? null}
            customer={selectedJob?.customer ?? null}
            jobStatus={selectedJob ? formatStatus(selectedJob.status) : null}
            priority={selectedJob ? formatPriority(selectedJob.priority) : null}
            dueDate={selectedJob?.due_date ?? null}
            workcenter={selectedDesk?.workcenter ?? null}
            queueSlot={selectedDesk?.queueSlot ?? null}
            travelerProgress={selectedTravelerProgress}
            travelerStep={selectedTravelerStep}
            travelerVariance={selectedTravelerVariance}
            dispatchQueueStatus={selectedDispatchQueueStatus}
            queueLead={queueLead(selectedMachineQueue)}
            queueLoad={selectedQueueLoad}
            shortageCount={selectedShortageCount}
            blockedApprovalCount={selectedBlockedApprovalCount}
            hotJobCount={hotJobCount}
            hotJobNote={selectedHotJob?.note ?? selectedDesk?.hotNote ?? null}
            nextActions={selectedDesk?.nextActions ?? []}
            purchasingActions={selectedDesk?.purchasingActions ?? []}
            ...
            upstreamContextNote={upstreamContextNote || null}
            wsConnected={wsConnected}
            onSchedule={dashboard?.on_schedule}
            atRisk={dashboard?.at_risk}
            overdue={dashboard?.overdue}
            revenuePipeline={dashboard?.revenue_pipeline}
          />
