...
import { AppwOrderTrackingCopilot } from '../components/puoa/AppwOrderTrackingCopilot';
...
export function OrderTrackingPage() {
  ...
  const effectiveOrigin = useMemo(
    ...
    [effectiveContinuityNote, launcherSource, routeContext.origin, workflowRecordId, workflowRecordType],
  );
  ...
  async function refreshWorkspaceAfterMutation() {
    ...
    const issue = rejected.reason;
    throw issue instanceof Error ? issue : new Error('Failed to refresh live order workspace.');
  }
  ...
  async function handleCreate(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setCreateResult((response.result as Record<string, unknown>) ?? { created: true });
  ...
  async function handleExecution(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setExecutionResult((response.result as Record<string, unknown>) ?? { saved: true });
      await refreshWorkspaceAfterMutation();
    } catch (issue) {
      setError(
        issue instanceof ApiError
          ...
          : issue instanceof Error
            ? issue.message
            : 'Failed to save execution update',
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1520px] flex-col gap-6">
          ...
          <div className="space-y-4">
            ...
            {launcherSource ? (
              <div className="rounded-[22px] border border-cyan-300/18 bg-cyan-300/[0.08] px-4 py-4 text-sm leading-6 text-cyan-50">
                ...
                {effectiveOrigin.note || effectiveContinuityNote}
                {orderReference ? (
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)]">
        ...
        <div className="space-y-6">
          <AppwOrderTrackingCopilot
            deskLabel="Order Tracking desk"
            activeLaneLabel={activeTab.label}
            activeLaneDetail={activeTab.detail}
            selectedOrderId={activeOrder?.id ?? null}
            selectedJobId={activeOrder?.job_id ?? routedJobId ?? null}
            partNumber={activeOrder?.part_number ?? null}
            machine={activeOrder?.machine ?? null}
            status={activeOrder?.status?.replace(/_/g, ' ') ?? null}
            quantity={activeOrder?.quantity ?? null}
            estimatedHours={activeOrder?.est_hours ?? null}
            actualHours={activeOrder?.actual_hours ?? null}
            activeOrderCount={activeOrderCount}
            queueDepth={queueDepth || metrics?.queue_depth || 0}
            queueRowCount={queue.length}
            onTimePct={metrics?.on_time_pct ?? null}
            avgLeadDays={metrics?.avg_lead_days ?? null}
            activeWorkOrders={metrics?.active_work_orders ?? null}
