...
function DetailPanel({ item }: { item: InboxItem }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4 space-y-4">
      ...
      {/* Part Numbers */}
      {Array.isArray(item.part_numbers) && item.part_numbers.length > 0 && (
        <div>
      ...
      {/* Line Items */}
      {Array.isArray(data.line_items) && (data.line_items as Array<Record<string, unknown>>).length > 0 && (
        <div>
          ...
          <div className="text-xs space-y-1">
            {(data.line_items as Array<Record<string, unknown>>).map((li, i) => (
              <div key={i} className="flex justify-between bg-gray-50 px-2 py-1 rounded">
                <span>{li.part_number ? `${String(li.part_number)} — ` : ''}{String(li.description)}</span>
