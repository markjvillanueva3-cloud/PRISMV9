...
export function WorkspaceHero({
  ...
  return (
    // KIENZLE-MERGE U-KZ-WORKSPACE-HERO: shared hero retargeted to the brand-aware
    // `accent` token (cyan in PRISM mode, Kienzle green under body[data-brand='kienzle'])
    // + Space-Grotesk display font on the title. One edit restyles every WorkspaceHero
    // page (7 quoting + ~49 fleet) consistently. Mirrors the QuoteBuilderPage template.
    <section className="overflow-hidden rounded-ios-xl border border-accent/10 bg-[linear-gradient(135deg,rgba(7,14,22,0.98)_0%,rgba(5,10,16,0.98)_42%,rgba(18,32,48,0.96)_100%)] p-6 shadow-[0_40px_120px_rgba(0,0,0,0.34)]">
      <div className="grid gap-8 xl:grid-cols-[minmax(0,1.65fr)_minmax(320px,0.85fr)]">
        <div className="space-y-5">
          ...
          <div className="space-y-3">
            <h1 className="text-4xl font-semibold tracking-[-0.02em] text-slate-50 sm:text-5xl [font-family:var(--font-display)]">{title}</h1>
