...
export function buildSfcCalcRequest(
  ...
  tool: CuttingToolEntry | null = null,
): SfcCalculateRequest {
  ...
  // Forward the selected tool's rated ceilings (catalog maxRpm in rev/min, maxDoc in mm) so the engine
  // clamps the recommendation to what the physical tool can run -- min(machine, tool). Omit-when-absent
  // (<=0 or missing), matching the machine-limit pattern; the engine treats an absent value as unclamped.
  if (tool) {
    if (typeof tool.maxRpm === "number" && tool.maxRpm > 0) {
      req.tool_max_rpm = tool.maxRpm;
    }
    if (typeof tool.maxDoc === "number" && tool.maxDoc > 0) {
      req.tool_max_doc = tool.maxDoc;
    }
  }
...
/**
 * Derive the parameter-panel updates from a newly selected catalog tool. Kept pure + exported so the
 * tool -> params mapping is unit-testable and shared by every SFC surface that selects a tool (the
 * page's handleToolChange previously inlined this and DROPPED `coating`, so a coated tool computed as
 * uncoated and the built coating Vc/life derate never fired -- romeo-2 wiring gap, the operator's #1
 * "auto-adjust for coating" ask). Copies geometry (diameter, fluteCount), the substrate as tool
 * material, and the coating; all other panel fields (cut geometry, coolant) are preserved from `prev`.
 * The coating string flows through buildSfcCalcRequest -> ProductEngine.normalizeCoatingKey, which
 * lowercases + strips separators (so catalog "TiAlN"/"AlTiN"/"DLC"/"AlCrN" hit real derate cells and
 * any unrecognized coating falls through to a neutral 1.0 no-op -- never a wrong number).
 */
export function applyToolToParams(prev: SfcParams, t: CuttingToolEntry): SfcParams {
  return {
    ...prev,
