import type { Employee, Job, ProcessType } from '../api/types';
...
function inferProcessType(operationCode: string, title: string, department: string): ProcessType {
  const code = normalizedText(operationCode);
  const taskTitle = normalizedText(title);
  const taskDepartment = normalizedText(department);

  if (code === 'cad' || code === 'cam' || taskTitle.includes('program') || taskDepartment.includes('cam')) {
    return 'programming';
  }
  if (code === 'op10' || taskTitle.includes('setup') || taskTitle.includes('prove')) {
    return 'setup';
  }
  if (code === 'op30' || taskDepartment.includes('quality') || taskTitle.includes('inspection') || taskTitle.includes('first article')) {
    return 'inspection';
  }
  if (code === 'op25' || taskTitle.includes('secondary') || taskTitle.includes('deburr')) {
    return 'secondary_ops';
  }
  if (code === 'op40' || taskDepartment.includes('shipping') || taskTitle.includes('packout') || taskTitle.includes('ship')) {
    return 'material_handling';
  }
  return 'production_run';
}

function createMobileTaskTemplate(task: {
  id: string;
  title: string;
  department: string;
  operationCode: string;
  ...
  return {
    ...
    processType: inferProcessType(task.operationCode, task.title, task.department),
  };
}
...
export function buildMobileTaskTemplates({
  ...
  if (normalizedDepartment.includes('cad') || normalizedRole.includes('engineer')) {
    return [
      createMobileTaskTemplate({ id: 'cad-work', title: 'CAD work', department: 'CAD work', operationCode: 'CAD', quantityTarget, cycleSeconds: 0, note: 'Model changes, print cleanup, and revision release.' }),
      createMobileTaskTemplate({ id: 'cam-programming', title: 'CAM programming', department: 'CAM programming', operationCode: 'CAM', quantityTarget, cycleSeconds: 0, note: 'Toolpath updates, simulation, and setup-sheet alignment.' }),
    ];
  }

  if (normalizedDepartment.includes('quality') || normalizedRole.includes('quality')) {
    return [
      createMobileTaskTemplate({ id: 'first-article', title: 'First article', department: 'Quality', operationCode: 'OP30', quantityTarget, cycleSeconds: 0, note: 'First-piece dimensional release and setup validation.' }),
      createMobileTaskTemplate({ id: 'final-inspection', title: 'Final inspection', department: 'Quality', operationCode: 'OP30', quantityTarget, cycleSeconds: 0, note: 'Final quantity, cert packet, and ship release checks.' }),
    ];
  }

  if (normalizedDepartment.includes('shipping') || normalizedRole.includes('ship')) {
    return [
      createMobileTaskTemplate({ id: 'packout', title: 'Packout', department: 'Shipping', operationCode: 'OP40', quantityTarget, cycleSeconds: 0, note: 'Label, box, and document packout preparation.' }),
      createMobileTaskTemplate({ id: 'ship-release', title: 'Ship release', department: 'Shipping', operationCode: 'OP40', quantityTarget, cycleSeconds: 0, note: 'Final scan, carrier handoff, and shipment confirmation.' }),
    ];
  }
  ...
  return [
    createMobileTaskTemplate({ id: 'setup', title: 'Job setup', department: 'Job setup', operationCode: 'OP10', quantityTarget, cycleSeconds: 0, note: 'Fixture, offsets, tool prove-out, and first-piece setup.' }),
    createMobileTaskTemplate({ id: 'run-cycle', title: 'Run cycle', department: 'Run cycle', operationCode: 'OP20', quantityTarget, cycleSeconds: runCycleSeconds, note: 'Cycle production time with parts completed and extras captured.' }),
    createMobileTaskTemplate({ id: 'secondary-op', title: 'Secondary op / deburr', department: 'Run cycle', operationCode: 'OP25', quantityTarget, cycleSeconds: Math.max(Math.round(runCycleSeconds * 0.35), 16), note: 'Secondary handling, deburr, or cell-side finishing.' }),
  ];
}
