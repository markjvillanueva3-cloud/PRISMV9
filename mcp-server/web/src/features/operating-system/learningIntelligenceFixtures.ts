...
export const PLATFORM_LEARNING_SNAPSHOT: PlatformLearningSnapshot = {
  shopProfile: {
    ...
    captures: [
      ...
      },
      {
        ...
        scope: 'hybrid',
        status: 'warming',
        detail: 'Actual cycle time, tooling wear, setup burden, and expedite pressure should refine the next JM Die quote instead of dying inside one job.',
      ...
      },
      {
        ...
        scope: 'shop',
        status: 'planned',
        detail: 'Document learning, tribal knowledge, and operator handoffs should continuously harden JM Die-specific recommendations.',
    ...
    improvements: [
      ...
      },
      {
        ...
        source: 'hybrid',
        detail: 'Approved local outcomes should sharpen speeds/feeds, holder choice, and setup-sheet defaults for the JM Die machine mix.',
  ...
  rolloutActions: [
    'Keep every new JM Die workflow, engine, and data surface represented in the learning layer so SVI does not drift behind platform reality.',
    'Let JM Die tune recommendations locally first, then promote only privacy-safe deltas into the network layer after validation.',
    'Surface learning posture on command-center, employee, planning, and release desks so JM Die adaptation is visible instead of hidden in one academy module.',
  ],
};
