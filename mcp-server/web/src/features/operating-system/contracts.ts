import type { Employee, Job, ScheduleResult } from '../../api/types';
...
export interface ShopFloorTrackedTask {
  ...
  processType: import('../../api/types').ProcessType;
...
export interface DepartmentCheckInResult {
  duplicate: boolean;
  message: string;
  checkIns: ShopFloorDepartmentCheckIn[];
  entry?: ShopFloorDepartmentCheckIn;
  tasks: ShopFloorTrackedTask[];
}

export type ShopFloorTaskEventAction = 'start' | 'pause' | 'complete';
...
export interface ShopFloorProvider {
  ...
  recordTaskEvent(input: {
    tasks: ShopFloorTrackedTask[];
    taskId: string;
    action: ShopFloorTaskEventAction;
    nowMs?: number;
    partsCompleted?: number;
    extraParts?: number;
