...
export interface PortalQuoteResponseRecord {
  recorded: boolean;
  response: 'accept' | 'reject' | 'request_changes' | string;
  message_id?: string;
}
...
export async function respondToPortalQuote(token: string, params: {
  response: 'accept' | 'reject' | 'request_changes';
  customer_name: string;
  message?: string;
  requested_changes?: string[];
}) {
  return requestPortalApi<PortalQuoteResponseRecord>('POST', `/quote/${encodeURIComponent(token)}/respond`, params);
}
...
export async function sendPortalOrderMessage(token: string, params: {
  sender_name: string;
  message: string;
}) {
  return requestPortalApi<PortalMessageRecord>('POST', `/order/${encodeURIComponent(token)}/messages`, params);
}
