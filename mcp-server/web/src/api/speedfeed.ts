...
export interface SpeedFeedParams {
  ...
  spindle_taper?: 'ISO20' | 'BT30' | 'BT40' | 'BT50' | 'CAT40' | 'CAT50' | 'HSK-A50' | 'HSK-A63' | 'HSK-A100' | 'HSK-C40' | 'HSK-E40';
...
export async function sfToolRoiAnalysis(params: ToolRoiAnalysisParams) {
  if (!shouldCacheSpeedFeedRequests()) {
    return sfRequest<ToolRoiAnalysisResult>('/tool-roi', params);
  }
  ...
  const promise = sfRequest<ToolRoiAnalysisResult>('/tool-roi', params).catch((error) => {
...
export const speedFeedApi = {
  orchestrate: sfOrchestrate,
  quick: sfQuick,
  stochastic: sfStochastic,
  resolveMachine: sfResolveMachine,
