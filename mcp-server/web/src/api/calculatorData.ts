...
import { describeJMDieHolderCatalogAuthority } from '../utils/jmDieCalculatorAuthority';
import {
  buildJMDieProgrammingCatalogAuthority,
  type ProgrammingCatalogAuthoritySummary,
...
const MACHINE_AGGREGATE_PAGE_SIZE = 5000;
...
const workholdingCatalogPromiseCache = new Map<MachineMode, Promise<WorkholdingCatalogState>>();
...
export function resetCalculatorDataCachesForTest() {
  ...
  workholdingCatalogPromiseCache.clear();
...
export interface CalculatorCatalogLoadState<T> {
  ...
  note: string;
  authority?: ProgrammingCatalogAuthoritySummary;
  sampled?: boolean;
}

export interface WorkholdingCatalogState {
  bundle: WorkholdingCatalogBundle;
  source: CalculatorCatalogSourceState;
  liveCount: number;
  fallbackCount: number;
  note: string;
...
function normalizeManufacturerName(value: string): string {
  ...
  const manufacturerAliases: Record<string, string> = {
    ...
    'mori seiki': 'DMG MORI',
    'grob': 'GROB',
    'nakamura tome': 'Nakamura-Tome',
    'roku roku': 'Roku-Roku',
    'mhi machine tool': 'MHI Machine Tool',
    'okk': 'OKK',
    'ycm': 'YCM',
  };
...
async function fetchAllMachineRows(): Promise<unknown[]> {
  const loadRows = async (
    buildRequestBody: (offset: number) => Record<string, unknown>,
  ): Promise<unknown[]> => {
    const rows: unknown[] = [];
    let offset = 0;

    for (let page = 0; page < MACHINE_MAX_PAGES; page += 1) {
      const raw = await dataRequest<unknown>('/machine/search', 'POST', buildRequestBody(offset));
  ...
  try {
    const aggregateRows = await loadRows((offset) => ({
      calculatorCatalog: true,
      limit: MACHINE_AGGREGATE_PAGE_SIZE,
      offset,
    }));
    ...
    // Older servers do not expose the expanded calculator catalog yet.
  }

  return loadRows((offset) => ({
...
export async function fetchProgrammingCatalogState(
  ...
  const load = async (requestedMode?: MachineMode): Promise<CalculatorCatalogLoadState<ProgrammingEnvironmentOption>> => {
    const raw = await dataRequest<unknown>('/programming/catalog', 'POST', requestedMode ? { mode: requestedMode } : {});
    const record = asRecord(raw);
    const backendSource = readText(record?.source).toLowerCase();
    ...
    if (liveItems.length === 0) {
      const authority = buildJMDieProgrammingCatalogAuthority({
        ...
        liveCount: 0,
        fallbackCount: fallbackItems.length,
        stage: fallbackItems.length ? 'no-compatible-live-results' : 'empty',
      });
      return {
        items: fallbackItems,
        source: fallbackItems.length ? 'fallback' : 'empty',
    ...
    if (backendSource === 'curated') {
      const authority = buildJMDieProgrammingCatalogAuthority({
        mode: requestedMode,
        items: liveItems,
        seedCandidates: fallbackItems,
        liveCount: 0,
        fallbackCount: liveItems.length,
...
function buildFallbackWorkholdingBundle(mode: MachineMode): WorkholdingCatalogBundle {
  return {
...
function normalizeWorkholdingPreset(entry: unknown): WorkholdingPresetOption | null {
  const record = asRecord(entry);
  if (!record) return null;
  ...
  const modes = unwrapCollection(record.modes)
    .map((value) => normalizeToolMode(readText(value).toLowerCase()))
    .filter((value): value is MachineMode => Boolean(value));
  const categoryId = readText(record.categoryId ?? record.category_id);
  const brandId = readText(record.brandId ?? record.brand_id);
  const workholdingId = readText(record.workholdingId ?? record.workholding_id);
  const stabilityId = readText(record.stabilityId ?? record.stability_id);

  if (!id || !label || !detail || !modes.length || !categoryId || !brandId || !workholdingId || !stabilityId) {
    return null;
  }
...
export async function fetchWorkholdingCatalogState(mode?: MachineMode): Promise<WorkholdingCatalogState> {
  if (!mode) {
    return {
      bundle: {
        categoryOptions: [],
  ...
  const fallbackBundle = buildFallbackWorkholdingBundle(mode);

  const load = async (): Promise<WorkholdingCatalogState> => {
    const raw = await dataRequest<unknown>('/workholding/catalog', 'POST', { mode });
    const record = asRecord(raw);
    const backendSource = readText(record?.source).toLowerCase();
    const backendLiveCount = safeNum(record?.liveCount ?? record?.live_count, Number.NaN);
    const backendFallbackCount = safeNum(record?.fallbackCount ?? record?.fallback_count, Number.NaN);
    const backendNote = readText(record?.note);

    const categoryOptions = unwrapCollection(record?.categoryOptions ?? record?.category_options)
      .map((entry) => {
        const option = asRecord(entry);
        if (!option) return null;
        const id = readText(option.id);
        const label = readText(option.label);
        const detail = readText(option.detail);
        return id && label && detail ? { id, label, detail } : null;
      })
      .filter((entry): entry is SelectionOption => Boolean(entry));
    const brandOptions = unwrapCollection(record?.brandOptions ?? record?.brand_options)
      .map((entry) => {
        const option = asRecord(entry);
        if (!option) return null;
        const id = readText(option.id);
      ...
      .filter((entry): entry is SelectionOption => Boolean(entry));
    const presetOptions = unwrapCollection(record?.presetOptions ?? record?.preset_options)
      .map(normalizeWorkholdingPreset)
      .filter((entry): entry is WorkholdingPresetOption => Boolean(entry))
      .filter((entry) => entry.modes.includes(mode));
    const stabilityOptions = unwrapCollection(record?.stabilityOptions ?? record?.stability_options)
      .map((entry) => {
        const option = asRecord(entry);
        if (!option) return null;
        const id = readText(option.id);
    ...
    const hasBackendBundle = categoryOptions.length > 0 || brandOptions.length > 0 || presetOptions.length > 0;
    if (!hasBackendBundle) {
      return {
        ...
        note: fallbackBundle.presetOptions.length
          ? 'Workholding route returned no compatible presets, so the curated fallback workholding catalog is active.'
          : 'No workholding presets were available for this machine slice.',
        sampled: false,
      };
    }

    const resolvedBundle = {
      categoryOptions: categoryOptions.length ? categoryOptions : fallbackBundle.categoryOptions,
      brandOptions: brandOptions.length ? brandOptions : fallbackBundle.brandOptions,
      presetOptions: presetOptions.length ? presetOptions : fallbackBundle.presetOptions,
      stabilityOptions: stabilityOptions.length ? stabilityOptions : fallbackBundle.stabilityOptions,
    };
    const liveCount = Number.isFinite(backendLiveCount) ? backendLiveCount : presetOptions.length;
    const fallbackCount = Number.isFinite(backendFallbackCount) ? backendFallbackCount : Math.max(resolvedBundle.presetOptions.length - presetOptions.length, 0);

    if (backendSource === 'database' || backendSource === 'live') {
      return {
        bundle: resolvedBundle,
        source: 'live',
        liveCount,
        fallbackCount,
        note: backendNote || 'Live workholding and fixture catalog is active for this machine slice.',
        sampled: false,
      };
    }

    if (backendSource === 'hybrid') {
      return {
        bundle: resolvedBundle,
        source: 'hybrid',
        liveCount,
        fallbackCount,
        note: backendNote || 'Live workholding and fixture rows are merged with calculator-native presets for this machine slice.',
        sampled: false,
      };
    }
    ...
    return {
      ...
      source: 'fallback' as CalculatorCatalogSourceState,
      liveCount: 0,
      fallbackCount: resolvedBundle.presetOptions.length,
      note: backendNote || 'Backend-served curated workholding catalog is active. No live workholding registry is wired yet, so fixture posture remains advisory.',
      sampled: false,
    };
  };
  ...
  try {
    ...
    const cached = workholdingCatalogPromiseCache.get(mode);
    if (cached) {
      return await cached;
    }

    const promise = load().catch((error) => {
      workholdingCatalogPromiseCache.delete(mode);
      throw error;
    });
    workholdingCatalogPromiseCache.set(mode, promise);
    return await promise;
  } catch {
    return {
      bundle: fallbackBundle,
      source: fallbackBundle.presetOptions.length ? 'fallback' : 'empty',
      liveCount: 0,
      fallbackCount: fallbackBundle.presetOptions.length,
      note: fallbackBundle.presetOptions.length
        ? 'Workholding route is unavailable, so the local curated workholding catalog is active.'
        : 'No workholding presets were available for this machine slice.',
      sampled: false,
    };
  }
...
export async function fetchToolCatalogState(
  ...
  const load = async (requestedMode?: MachineMode): Promise<CalculatorCatalogLoadState<ToolCatalogItem>> => {
    if (!requestedMode) {
      return {
...
export async function fetchToolHolderCatalogState(
  ...
  const load = async (): Promise<CalculatorCatalogLoadState<HolderPackageOption>> => {
    ...
    if (!raw) {
      return {
        ...
        note: normalizedFallbackItems.length
          ? (
            describeJMDieHolderCatalogAuthority(input.mode, normalizedFallbackItems, 'database-unavailable')
    ...
    if (liveItems.length === 0) {
      return {
        ...
        note: normalizedFallbackItems.length
          ? (
            describeJMDieHolderCatalogAuthority(input.mode, normalizedFallbackItems, 'no-compatible-live-results')
    ...
    return {
      ...
      note: fallbackCount > 0
        ? (
          describeJMDieHolderCatalogAuthority(input.mode, mergedItems, 'hybrid')
  ...
  try {
    ...
    return {
      ...
      note: normalizedFallbackItems.length
        ? (
          describeJMDieHolderCatalogAuthority(input.mode, normalizedFallbackItems, 'failure')
