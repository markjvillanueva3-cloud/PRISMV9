...
export function getRequestHeaders(): Record<string, string> {
  ...
  const token = apiKey || getStoredAuthToken();
  if (token) {
...
// Month-end close calculators + filings. The routes unwrap the prism_business
// envelope server-side (erp.ts rfqRoute) and emit {ok,data}; read body.data.

export async function acctBankReconcile(params: Record<string, unknown>): Promise<PrismResponse> {
  return request('POST', '/erp/acct-bank-reconcile', params);
}

export async function acctWipValuation(params: Record<string, unknown>): Promise<PrismResponse> {
  return request('POST', '/erp/acct-wip-valuation', params);
}

export async function acctVarianceAnalysis(params: Record<string, unknown>): Promise<PrismResponse> {
  return request('POST', '/erp/acct-variance-analysis', params);
}

export async function acctCostToComplete(params: Record<string, unknown>): Promise<PrismResponse> {
  return request('POST', '/erp/acct-cost-to-complete', params);
}

export async function acctMultiPeriodCompare(params: Record<string, unknown>): Promise<PrismResponse> {
  return request('POST', '/erp/acct-multi-period-compare', params);
}

export async function acctQuickbooksSync(params: Record<string, unknown>): Promise<PrismResponse> {
  return request('POST', '/erp/acct-quickbooks-sync', params);
}

export async function form1099NecGenerate(params: Record<string, unknown>): Promise<PrismResponse> {
  return request('POST', '/erp/form-1099nec', params);
}

export async function payrollGenerateW2(params: Record<string, unknown>): Promise<PrismResponse> {
  return request('POST', '/erp/payroll-w2', params);
}

export async function salesUseTaxCalc(params: Record<string, unknown>): Promise<PrismResponse> {
  return request('POST', '/erp/sales-use-tax', params);
}
...
export interface WedmAutonomyStatus {
  ...
  level_label: string;
  /** Optional: AutonomyStatusSnapshot has no confidence field -- absent means "not measured" (render "--"). */
  confidence?: number;
  can_promote: boolean;
...
/**
 * U-WEDM-LIVE-ROUTES (2026-07-05): reshaped to the components the backend ACTUALLY tracks
 * (WEDMDegradationModelEngine: guide_wear/wire_erosion/filter_capacity/filter_clogging/
 * wire_fatigue). The original wire_spool/upper_guide/lower_guide/power_feed shape was
 * speculative (U-P2PFS33, written before any backend existed) and matched no engine.
 */
export interface WedmRulComponent {
  component: string;
  label: string;
  /** Engine health fraction (0-1) as a percent -- remaining life. */
  rul_pct: number;
  /** null = "not aging" (no live usage-rate telemetry yet). */
  hours_remaining: number | null;
  band: 'imminent' | 'soon' | 'planned' | 'healthy';
}

export interface WedmRulStatus {
  components: WedmRulComponent[];
