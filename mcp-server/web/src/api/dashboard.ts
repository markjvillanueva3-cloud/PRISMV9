...
async function fetchRoute(path: string, method: 'GET' | 'POST', body?: Record<string, unknown>): Promise<unknown> {
  // Attach auth on EVERY request: /erp/job-dashboard + /erp/tool-usage are verifyToken-gated
  // (erp.ts:587/596), so a signed-in user without the Bearer 401s -> the dashboard silently falls
  // to DEMO data instead of the shop's live job/tool figures. getRequestHeaders() (client.ts, the
  // canonical shared helper, R8 reuse) reconciles the stored token key. Content-Type only when a body.
  const response = await fetch(`${API_BASE}${path}`, {
    method,
    headers: body ? { 'Content-Type': 'application/json', ...getRequestHeaders() } : getRequestHeaders(),
