...
// ── Per-group override computation ─────────────────────────────────────────
...
function groupOverride(
  ...
  // Per-grade machinability scaling: the SFC returns the ISO-group Vc; scale it by
  // the grade machinability rating so each grade's SFM/RPM/feed actually differ
  // (feed follows RPM; chip load fz + DOC ap/ae stay group-level -- machinability
  // is a SPEED factor, not a chip-load factor).
  const vc = lk.vc * (vcScale > 0 ? vcScale : 1);
  ...
  if (op === "drilling" || op === "reaming") {
    // CUTTING_PARAMS drilling fz is feed-PER-REV; hole-making feeds per rev.
    const fnRevMm = lk.fz;                       // mm/rev
    const fnRev = toUnit(fnRevMm);
  ...
  const fzMm = lk.fz;
  const fzTooth = toUnit(fzMm);
  const fnRev = fzTooth * (flutes > 0 ? flutes : 1);
  const feed = fnRev * rpm;
  ov[COL.feedPerTooth] = fmt(fzTooth, 6);
  ov[COL.feedCuttingRel] = fmt(fnRev, 6);
  ov[COL.feedCutting] = fmt(feed, 4);
  if (lk.ap > 0) { ov[COL.stepdown] = fmt(toUnit(lk.ap), 4); ov[COL.useStepdown] = "true"; }
  if (lk.ae > 0) { ov[COL.stepover] = fmt(toUnit(lk.ae), 4); ov[COL.useStepover] = "true"; }
  return ov;
}
  ...
  for (const file of csvs) {
    ...
    for (let li = 1; li < lines.length; li++) {
      ...
      for (const g of GRADES) {
        ...
        const ov = groupOverride(g.iso, op, dMm, flutes, toolMaterial, unit, vcScale);
        if (!ov) { skippedGroups++; continue; }
        const row = fields.slice();
        // Match the verbatim data-row width exactly. The final header token
        // (CSV_TOOLS_VERSION_1) is a format sentinel with NO data column, so
        // real data rows carry headers.length-1 fields. Pad/trim grade rows to
        // that width so they are byte-width-identical to the originals (every
        // override target index is well below this width, so no field is lost).
        const dataWidth = headers.length - 1;
        while (row.length < dataWidth) row.push("");
        if (row.length > dataWidth) row.length = dataWidth;
        // grade preset name + Fusion stock category (Filter by Type) + HB hardness
        // range (Filter by hardness) so Fusion auto-selects the right grade preset
        // AND distinguishes same-category grades (1018 vs 4140 both 'steel') by the
        // setup's stock hardness.
        const pi = colOf(COL.preset);
        if (pi >= 0) row[pi] = g.name;
    ...
    const outName = basename(file, ".csv") + "-6groups.csv";
...
function writeReadme(stats: RowStat[], srcDir: string): void {
  ...
  out.push("- `*-6groups.csv` -- each JM tool with its original as-run preset PLUS per-GRADE");
  out.push("  presets (1018 / 1045 / 4140-4340 in P; 304-316 / 17-4PH / 2205 in M; Gray /");
  out.push("  Ductile iron in K; 6061-7075 / Brass / Copper in N; Ti-6Al-4V / Inconel in S;");
  out.push("  Hardened tool steel in H) -- ONLY for grades whose ISO group the tool's");
  out.push("  coating+substrate is compatible with. Each grade SFM is scaled from the");
  out.push("  ISO-group base by its canonical machinability rating (4140 ~ -23% vs 1018).");
