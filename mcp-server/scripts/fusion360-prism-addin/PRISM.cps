/**
  ...
  Connects to the PRISM MCP Server for physics-optimized G-code generation.

  Phase A (Server Mode): Sends toolpath + context to PRISM HTTP API,
    receives optimized G-code with variable S/F, controller features, analytics.
  Phase B (Offline Mode): Falls back to standard G-code generation when
    PRISM server is unreachable, exports sidecar JSON for later optimization.
...
properties = {
  ...
  prismServerUrl: {
    ...
    description: "URL of the PRISM optimization server",
    type: "string",
...
function onOpen() {
  // Try to connect to PRISM server
  prismConnected = testPrismConnection();

  if (prismConnected) {
    writeComment("PRISM Post Processor v1.0.0 — Server Mode (optimized)");
  } else {
    writeComment("PRISM Post Processor v1.0.0 — Offline Mode (standard)");
    writeComment("Start PRISM server for physics-optimized output");
    // Fall back to standard post processing
    writeSafeStart();
  }
}
...
function onSection() {
  ...
  if (!prismConnected) {
    // Offline: write standard tool change
...
function onClose() {
  if (prismConnected) {
    // Phase A: Send all collected data to PRISM server
    var payload = buildPrismPayload();
    var response = callPrismServer(payload);

    if (response && response.gcode) {
      // Write optimized G-code from PRISM
      write(response.gcode);

      // Write analytics as comments
      if (properties.includeAnalytics && response.analytics) {
        writeComment("═══ PRISM OPTIMIZATION ANALYTICS ═══");
        writeComment("Total cycle time: " + response.analytics.overall.total_cycle_time_s.toFixed(1) + "s");
        writeComment("Cutting time: " + response.analytics.overall.cutting_time_s.toFixed(1) + "s");
        writeComment("Stages: " + response.stages.length + " executed");
        writeComment("Status: " + response.overall_status);
        if (response.warnings && response.warnings.length > 0) {
          writeComment("─── Warnings ───");
          for (var i = 0; i < Math.min(response.warnings.length, 10); i++) {
            writeComment(response.warnings[i]);
          }
        }
      }
    } else {
      // Server call failed — fall back to offline mode
      writeComment("PRISM server returned no data — writing standard output");
      writeStandardClose();
    }
  } else {
    // Phase B: Write sidecar JSON for offline optimization
    writeStandardClose();
    writeSidecarJSON();
  }
}

// ═══ PRISM Server Communication ═══

function testPrismConnection() {
  try {
    var url = properties.prismServerUrl + "/api/post-process/health";
    var response = httpGet(url, 2000); // 2 second timeout
    return response && response.indexOf("ok") >= 0;
  } catch (e) {
    return false;
  }
}
...
function callPrismServer(payload) {
  try {
    var url = properties.prismServerUrl + "/api/post-process";
    var body = JSON.stringify(payload);
    var response = httpPost(url, body, "application/json", 30000); // 30s timeout
    return JSON.parse(response);
  } catch (e) {
    writeComment("PRISM server error: " + e.message);
    return null;
  }
}
...
function httpPost(url, body, contentType, timeout) {
  ...
  client.setContentType(contentType || "application/json");
