...
// A bare `node sfc-closed-loop-compare.mjs` (cron / engineered loop / ad-hoc / MCP boot) cannot
// resolve the `.ts` engine import below -- Node type-strip won't rewrite a .js specifier to .ts.
...
function run() {
  ...
  for (const cell of CELLS) {
    ...
    rows.push({
      ...
      // U-OSC-VC-UNCAPPED-PARITY: the pre machine/holder RPM-cap surface speed. Vendor cribs are uncapped,
      // so a fair PRISM-vs-vendor comparison uses THIS; the printed prism_vc stays the achievable (capped) Vc.
      prism_vc_uncapped: typeof prism?.vc_uncapped_mpm === "number" ? prism.vc_uncapped_mpm.toFixed(1) : "n/a",
      prism_capped: !!prism?.rpm_capped,
  ...
  for (const r of rows) {
    ...
    console.log(`  ${r.cell}`);
    // Show the uncapped surface speed when the achievable Vc was RPM-capped, so a capped value is never
    // misread as a physics under-prediction vs the (uncapped) vendor baseline. (U-OSC-VC-UNCAPPED-PARITY)
    const prismCell = r.prism_capped
      ? `PRISM ${r.prism_vc} (RPM-capped from ${r.prism_vc_uncapped} uncapped)`
      : `PRISM ${r.prism_vc}`;
    console.log(`      ${prismCell}  |  baseline ${r.baseline_vc}  |  consensus(externals) ${r.consensus}`);
