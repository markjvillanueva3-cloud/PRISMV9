...
async function extractCatalog(cat) {
  ...
  try {
    ...
    const jsonPath = path.join(DIST_DATA, cat.json);
    fs.writeFileSync(jsonPath, jsonStr);

    const sizeMB = (Buffer.byteLength(jsonStr) / 1024 / 1024).toFixed(2);
    console.log(`  OK   ${cat.json} — ${sizeMB} MB`);
...
// ── Main ──
console.log(`[build-catalog-json] Extracting ${CATALOGS.length} catalogs to dist/data/`);
