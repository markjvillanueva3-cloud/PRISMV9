...
// `--mode full` enumerates 69,120 cells x 4 tool materials = 276,480 comparisons; the default
// ~2GB V8 heap OOMs mid-stream (verified FATAL "Reached heap limit Allocation failed" at ~165,656
// ledger rows, 2026-06-25). Relaunch with a 32GB heap (host has 136GB) BEFORE the tsx re-exec so
// the flag survives via NODE_OPTIONS inheritance. Fires on bare-node AND `npx tsx` -- the tsx guard
// no-ops when already under tsx and so can never add heap itself. No-op for prod mode (576 cells).
// [[reference_oscar_full_sweep_heap_oom_2026_06_25]]
reexecForHeapIfNeeded(import.meta.url);

// A bare `node sfc-full-sweep-compare.mjs` (cron / engineered loop / ad-hoc / MCP boot) cannot
...
const { speedFeedExhaustiveCombinationEngine } = await import("../src/engines/SpeedFeedExhaustiveCombinationEngine.js");
const { speedFeedTriComparatorEngine } = await import("../src/engines/SpeedFeedTriComparatorEngine.js");
...
/** PRISM/vendor feed-per-tooth (mm/tooth) from a system opinion, or null when unavailable. */
function axisFz(sys) {
  return sys && sys.available && sys.axes && Number.isFinite(sys.axes.fz_mm) ? sys.axes.fz_mm : null;
}
...
function run() {
  ...
  const byVendor = { cnccookbook: { n: 0, vcDeltas: [], fzDeltas: [], fzByMode: {} }, hsmadvisor: { n: 0, vcDeltas: [], fzDeltas: [], fzByMode: {} } };
  let prismRan = 0;
  let cellErrors = 0;
  let totalCells = 0;
  ...
  for (const c of allCells()) {
    ...
    for (const tm of TOOL_MATERIALS) {
      ...
      const prismFz = axisFz(by.prism); // feed-per-tooth for the india training ledger (priority-4 completeness)
      ...
      byIso[iso] ||= { n: 0, deltas: [], carbideDeltas: [], prismLower: 0, prismHigher: 0, gwSeen: 0, hsmSeen: 0 };
      const a = byIso[iso];
      a.n++;
      let deltaPct = null;
      if (prismVc !== null && baselineVc !== null && baselineVc > 0) {
        ...
        // Carbide-only per-ISO delta = the apples-to-apples accuracy signal. The all-material
        // `deltas` median MIXES hss/ceramic/cbn rows (compared to a CARBIDE-keyed baseline), so a
        // per-ISO aggregate can read "aggressive" (e.g. P +10%) while carbide alone is conservative
        // (P -22.7%). Isolating carbide answers "is PRISM safe per ISO" honestly. (2026-06-25)
        if (tm === "carbide") a.carbideDeltas.push(deltaPct);
      ...
      let gwPubFzDelta = null;
      let hsmPubFzDelta = null;
      ...
      for (const ps of perSource) {
        ...
        if (Number.isFinite(ps.fz_variance_pct)) {
          acc.fzDeltas.push(ps.fz_variance_pct);
          // Per-mode fz split (task #17): the aggregate median mixes cost_batch (low fz) with
          // aggressive_rush (high fz), so a per-mode breakdown is the honest signal for india/analysis.
          const mode = c.input_summary.mode;
          (acc.fzByMode[mode] = acc.fzByMode[mode] || []).push(ps.fz_variance_pct);
        }
        if (ps.source === "cnccookbook") { gwPubDelta = ps.vc_variance_pct; gwPubFzDelta = ps.fz_variance_pct ?? null; }
        if (ps.source === "hsmadvisor") { hsmPubDelta = ps.vc_variance_pct; hsmPubFzDelta = ps.fz_variance_pct ?? null; }
      }
      ...
      const row = {
        ...
        // Uncapped Vc + machine-RPM-cap flag (U-OSC-VC-UNCAPPED-PARITY fields). prism_vc_mpm is the
        // machine-RPM-CAPPED achievable Vc; prism_vc_uncapped_mpm is the engine's true surface-speed
        // recommendation BEFORE the holder/spindle RPM cap. They diverge on small/high-Vc tools (e.g.
        // alu + carbide aggressive). Persisting both lets india + accuracy analysis distinguish a true
        // over/under-speed from a cap artifact WITHOUT a manual probe (see the 2026-06-25 HSS lead).
        prism_vc_uncapped_mpm: by.prism?.vc_uncapped_mpm ?? null,
        prism_rpm_capped: by.prism?.rpm_capped ?? null,
        ...
        // Feed-per-tooth, persisted so the india LoRA/GNN training ledger covers FEED, not only Vc
        // (priority-4 completeness; the +124.7% fz divergence vs G-Wizard published is the queued accuracy unit).
        prism_fz_mm: prismFz,
        gwizard_published_fz_delta_pct: gwPubFzDelta,
        hsmadvisor_published_fz_delta_pct: hsmPubFzDelta,
      };
  ...
  const isoSummary = Object.fromEntries(
    Object.entries(byIso).map(([iso, a]) => [
      iso,
      {
        ...
        prism_carbide_only_median_delta_pct: a.carbideDeltas.length ? Number(median(a.carbideDeltas).toFixed(1)) : null,
  ...
  const perVendorSummary = Object.fromEntries(
    Object.entries(perVendor).map(([label, v]) => [
      label,
      {
        ...
        prism_vs_vendor_fz_delta_by_mode: Object.fromEntries(
          Object.entries(v.fzByMode).map(([m, arr]) => [m, arr.length ? Number(median(arr).toFixed(1)) : null]),
        ),
  ...
  console.log("  NOTE: 'all-mat' median MIXES tool materials vs a CARBIDE-keyed baseline (hss/ceramic/cbn");
  console.log("  rows compare against carbide data) -> it can read 'aggressive' while carbide is conservative;");
  console.log("  'carbide-only' is the apples-to-apples accuracy signal -- read THAT for safety.");
  console.log("  ISO | cells | all-mat median d% | carbide-only d% | conservative/aggressive | GW/HSM");
  for (const [iso, s] of Object.entries(isoSummary)) {
    const d = s.prism_vs_baseline_median_delta_pct;
    const dir = d === null ? "n/a" : d < 0 ? `${d}% (SAFE)` : `+${d}% (aggr)`;
    const c = s.prism_carbide_only_median_delta_pct;
    const cdir = c === null ? "n/a" : c < 0 ? `${c}% (SAFE)` : `+${c}% (aggr)`;
    console.log(
      `  ${iso.padEnd(3)} | ${String(s.cells).padStart(5)} | ${String(dir).padEnd(17)} | ${String(cdir).padEnd(15)} | ` +
  ...
  console.log("  NOTE: the median fz delta% MIXES optimization modes (cost_batch runs much lower fz than");
  console.log("  aggressive_rush) -- a single aggregate is NOT a uniform over-prediction; PRISM's aggressive");
  console.log("  modes are MEANT to exceed a vendor's one conservative published fz (per-mode split: task #17).");
