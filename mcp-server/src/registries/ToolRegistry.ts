...
export class ToolRegistry extends BaseRegistry<CuttingTool> {
  ...
  /**
   * Add tool to USER or LEARNED layer
   * @param tool - Cutting tool to add
   * @param layer - Target layer (USER or LEARNED only)
   * @returns Tool ID
   */
  async addTool(tool: CuttingTool, layer: string = DATA_LAYERS.USER): Promise<string> {
    await this.load();

    const id = tool.id || `TOOL-${Date.now()}`;

    // Validate layer - only USER or LEARNED allowed for additions
    if (layer !== DATA_LAYERS.USER && layer !== DATA_LAYERS.LEARNED) {
      throw new Error(`Can only add tools to USER or LEARNED layer, not ${layer}`);
    }

    // Check for duplicates
    if (this.has(id)) {
      throw new Error(`Tool ${id} already exists. Use enhanceTool() to update.`);
    }

    // Set the ID and layer
    tool.id = id;
    tool.layer = layer;

    // Add to registry
    this.set(id, tool, layer);

    // Incremental index update (avoids full rebuild)
    this.addToIndexes(id, tool);

    // Persist to layer file
    await this.persistToLayer(id, tool, layer);

    log.info(`Added tool ${id} to ${layer} layer`);
    return id;
  }

  /**
   * Enhance existing tool with new data
   * @param id - Tool ID to enhance
   * @param enhancements - Partial tool data to merge
   * @param source - Optional source identifier
   * @returns Enhanced tool
   */
  async enhanceTool(
    id: string,
    enhancements: Partial<CuttingTool>,
    source?: string
  ): Promise<CuttingTool> {
    await this.load();
    ...
    // Rebuild indexes (enhancement may change type/manufacturer/material groups)
    this.buildIndexes();

    // Persist to ENHANCED layer
    await this.persistToLayer(id, enhanced, DATA_LAYERS.ENHANCED);

    log.info(`Enhanced tool ${id}`);
    return enhanced;
  }
  ...
  /**
   * Register a new tool (alias for addTool)
   * @param tool - Cutting tool to register
   * @param layer - Target layer (default: USER)
   * @returns Tool ID
   */
  async register(tool: CuttingTool, layer: string = DATA_LAYERS.USER): Promise<string> {
    return this.addTool(tool, layer);
  }

  /**
   * Update an existing tool (alias for enhanceTool)
   * @param id - Tool ID to update
   * @param updates - Partial tool data to merge
   * @param source - Optional source identifier
   * @returns Updated tool
   */
  async update(id: string, updates: Partial<CuttingTool>, source?: string): Promise<CuttingTool> {
    return this.enhanceTool(id, updates, source);
  }
  ...
  /**
   * Persist tool to layer file
   */
  private async persistToLayer(id: string, tool: CuttingTool, layer: string): Promise<void> {
    // Use TOOLS_DB path with layer subdirectory
    const layerPath = path.join(PATHS.TOOLS_DB, layer.toLowerCase());
    ...
    try {
      ...
      log.debug(`Persisted tool ${id} to ${filePath}`);
    } catch (error) {
      log.error(`Failed to persist tool ${id}: ${error}`);
    }
  }

  /**
   * Add tool to indexes without full rebuild
   */
  private addToIndexes(id: string, tool: CuttingTool): void {
    // Index by type
    if (tool.type) {
      if (!this.indexByType.has(tool.type)) {
        this.indexByType.set(tool.type, new Set());
      }
      this.indexByType.get(tool.type)!.add(id);
    }

    // Index by manufacturer
    const manufacturer = tool.manufacturer || tool.vendor;
    if (manufacturer) {
      if (!this.indexByManufacturer.has(manufacturer)) {
        this.indexByManufacturer.set(manufacturer, new Set());
      }
      this.indexByManufacturer.get(manufacturer)!.add(id);
    }

    // Index by material groups
    if (tool.material_groups) {
      for (const group of tool.material_groups) {
        if (!this.indexByMaterialGroup.has(group)) {
          this.indexByMaterialGroup.set(group, new Set());
    ...
    // Index by diameter (round to nearest 0.5mm)
    const diameter = tool.geometry?.diameter || tool.cutting_diameter_mm;
    if (diameter != null && diameter > 0) {
      const rounded = Math.round(diameter * 2) / 2;
      if (!this.indexByDiameter.has(rounded)) {
        this.indexByDiameter.set(rounded, new Set());
      }
      this.indexByDiameter.get(rounded)!.add(id);
    }

    // Index by coating
    const coatingType = typeof tool.coating === "string"
      ? tool.coating
      : (tool.coating?.type || tool.coating_type);
    if (coatingType) {
      if (!this.indexByCoating.has(coatingType)) {
        this.indexByCoating.set(coatingType, new Set());
      }
      this.indexByCoating.get(coatingType)!.add(id);
    }

    // Index by category
    if (tool.category) {
      if (!this.indexByCategory.has(tool.category)) {
        this.indexByCategory.set(tool.category, new Set());
      }
      this.indexByCategory.get(tool.category)!.add(id);
    }
  }
}
