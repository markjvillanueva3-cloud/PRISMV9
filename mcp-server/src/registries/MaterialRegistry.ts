...
export class MaterialRegistry extends BaseRegistry<Material> {
  ...
  /**
   * Register a new material (alias for addMaterial)
   * @param material - Material to register
   * @param layer - Target layer (default: USER)
   * @returns Material ID
   */
  async register(material: Material, layer: string = DATA_LAYERS.USER): Promise<string> {
    return this.addMaterial(material, layer);
  }

  /**
   * Update an existing material (alias for enhanceMaterial)
   * @param id - Material ID to update
   * @param updates - Partial material data to merge
   * @param source - Optional source identifier
   * @returns Updated material
   */
  async update(id: string, updates: Partial<Material>, source?: string): Promise<Material> {
    return this.enhanceMaterial(id, updates, source);
  }

  /**
   * Upsert: Add if new, update if exists
   * @param material - Material data (must have id or material_id)
   * @param layer - Target layer
   * @returns Material ID
   */
  async upsert(material: Material, layer: string = DATA_LAYERS.USER): Promise<string> {
    const id = material.material_id || material.id;
    if (!id) throw new Error("Material must have id or material_id");

    if (this.has(id)) {
      await this.enhanceMaterial(id, material, layer);
      return id;
    } else {
      return this.addMaterial(material, layer);
    }
