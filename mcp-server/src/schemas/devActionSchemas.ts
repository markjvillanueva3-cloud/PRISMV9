...
// U-WIRE-ERP-IMPORT: ERPImportEngine work-order shape (mirrors WorkOrderImportSchema; the
// engine re-validates strictly via .parse, so the dispatcher boundary stays passthrough-tolerant). slot:papa 2026-06-14.
const _erpSystem = z.enum(["sap", "oracle", "jobboss", "e2", "epicor", "infor", "custom"]);
const _erpRoutingOp = z.object({
  operationNumber: z.number(), workCenter: z.string(), description: z.string(),
  setupHours: z.number(), runHoursPerUnit: z.number(),
  setupComplete: z.boolean().optional(), quantityComplete: z.number().optional(),
}).passthrough();
const _erpBomItem = z.object({
  itemNumber: z.string(), description: z.string(), quantityPer: z.number(),
  unitOfMeasure: z.string(), issued: z.boolean().optional(),
}).passthrough();
const _erpWorkOrder = z.object({
  erpSystem: _erpSystem, workOrderNumber: z.string(), partNumber: z.string(),
  revision: z.string().optional(), description: z.string(), customer: z.string(),
  quantityOrdered: z.number(), quantityComplete: z.number().optional(),
  dueDate: z.string(), releaseDate: z.string().optional(), priority: z.number().optional(),
  routing: z.array(_erpRoutingOp), bom: z.array(_erpBomItem).optional(), notes: z.string().optional(),
}).passthrough();

// U-WIRE-HZPAUDIT: HzpDashAuditEngine envelope sub-schemas (mirror the engine's exported
// AuditEnvelopeRequestSchema; build() re-validates strictly via .parse so the boundary stays
// passthrough-tolerant). slot:papa->golf 2026-06-15.
const _hzpAuditRequest = z.object({
  ...
  ts_iso: z.string().optional(),
}).passthrough();
const _hzpAuditEnvelope = z.object({
  audit_id: z.string(),                 // a real envelope MUST carry a minted id + ts
  ts: z.string(),                       // (reject hand-crafted envelopes missing them ->
  operation: z.string(),                //  no "audit_id":undefined JSONL lines)
  actor: z.string(),
...
// U-WIRE-PACT: PactContractTestEngine sub-schemas (slot:papa->dev 2026-06-15). Mirror the engine's
// plain-object I/O (Interaction/Contract). Matchers + actualResponse stay loose -- the engine
// validates matcher kinds at verify time (matchValue), so over-strict Zod here would reject valid
// 7-matcher-kind payloads the engine accepts.
const PACT_MATCHER_SCHEMA = z.object({ kind: z.string() }).passthrough();
const PACT_INTERACTION_SCHEMA = z.object({
  id: z.string().min(1),
  description: z.string(),
  providerState: z.string().optional(),
  expected: z.record(z.string(), PACT_MATCHER_SCHEMA),
  required: z.array(z.string()),
}).passthrough();
const PACT_CONTRACT_SCHEMA = z.object({
  consumer: z.string(),
  provider: z.string(),
  version: z.string(),
  createdAt: z.number(),
  interactions: z.array(PACT_INTERACTION_SCHEMA),
}).passthrough();
...
export const ACTION_DEV_SCHEMAS: Record<string, z.ZodType<any>> = {
  ...
  code_graph_project: z.object({
    target: z.string().min(1).describe("File or dir (rel to repo root, or absolute) to project to a code graph"),
    center: z.string().min(1).optional().describe("Node id to ego-graph around (RepoGraph retrieval); e.g. file:mcp-server/src/engines/X.ts"),
    hops: z.number().int().min(0).max(12).optional().describe("Ego radius when center is set (default 1)"),
    maxFiles: z.number().int().min(1).optional().describe("Walk cap for dir targets (default 2000)"),
    maxFileBytes: z.number().int().min(1).optional().describe("Per-file size skip cap in bytes (default 5MB)"),
  }).describe("CodeGraphProjectionEngine.project/egoGraph -- TypeScript code graph (files + symbols + imports) for ego-graph retrieval (GRAPH-AS-LLM-CONTEXT-MS0 U-GAC03)."),
  // U-GAC06: CommunitySummaryEngine (GRAPH-AS-LLM-CONTEXT-MS0, slot:sierra 2026-06-15)
  community_summary: z.object({
    domain: z.string().min(1).optional().describe("Single domain to summarize (e.g. Lathe/Mill/WEDM/CAM/CAD/AI-ML); omit for all clusters"),
    useLlm: z.boolean().optional().describe("Opt into a fail-soft Ollama prose pass (default false -> extractive)"),
    // NOTE: .max(300) mirrors CommunitySummaryEngine.HARD_TOKEN_CEIL -- co-change both if the ceiling moves.
    maxTokens: z.number().int().min(1).max(300).optional().describe("Per-summary token ceiling before truncate (default 200)"),
    topK: z.number().int().min(1).optional().describe("Max engine names in an extractive summary (default 12)"),
  }).describe("CommunitySummaryEngine -- cluster the engine catalog by domain into token-bounded community summaries (GRAPH-AS-LLM-CONTEXT-MS0 U-GAC06)."),
  ...
  // U-WIRE-ERP-IMPORT: ERPImportEngine work-order import surfaces (slot:papa 2026-06-14)
  erp_import_work_order: z.object({ workOrder: _erpWorkOrder }),
  erp_import_batch: z.object({ workOrders: z.array(_erpWorkOrder).min(1) }),
  erp_get_work_order: z.object({ erpSystem: _erpSystem, workOrderNumber: z.string().min(1) }),
  erp_list_work_orders: z.object({ erpSystem: _erpSystem.optional() }).optional(),
  erp_validate_work_order: z.object({ workOrder: z.unknown() }),
  erp_field_mappings: z.object({ erpSystem: _erpSystem }),
  // U-WIRE-COHORTSHIM: CohortBridgeShimEngine shim primitives (slot:papa 2026-06-15). Pure transforms +
  // a matrix-read recommender (fail-loud on missing COHORT-COMPAT-MATRIX -> dispatcherError, not silent).
  cohort_shim_nodenext_suffix: z.object({
    spec: z.string().min(1).describe("ESM import specifier to rewrite for NodeNext resolution (e.g. './x')"),
    assumeDirectoryIndex: z.boolean().optional().describe("If true, a directory import './dir' becomes './dir/index.js'"),
  }),
  cohort_shim_rewrite_imports: z.object({
    source: z.string().min(1).describe("Full source-file text; every relative import specifier gets a NodeNext .js suffix"),
  }),
  cohort_shim_build_shape_coerce: z.object({
    fromShape: z.string().min(1).describe("Source API shape name (e.g. 'compute')"),
    toShape: z.string().min(1).describe("Target API shape name (e.g. 'calculate')"),
    methodMap: z.record(z.string(), z.string()).describe("from-method -> to-method name correspondence (engine requires >=1 entry)"),
  }),
  cohort_shim_recommend_bridges: z.object({
    topK: z.number().int().positive().optional().describe("Number of top MEDIUM-cost bridges to return (default 10)"),
    matrixPath: z.string().optional().describe("Override path to COHORT-COMPAT-MATRIX.json"),
  }).optional(),
  // U-WIRE-HZPAUDIT: HzpDashAuditEngine audit-envelope builder (slot:papa->golf 2026-06-15).
  hzp_audit_build: z.object({
    req: _hzpAuditRequest.describe("Audit-envelope request (operation/actor/authorized/authority_reason required)"),
    randHex6: z.string().optional().describe("Deterministic 6-hex id seed (tests); omitted -> random"),
  }),
  hzp_audit_to_jsonl: z.object({
    env: _hzpAuditEnvelope.describe("A built AuditEnvelope to serialize to one JSONL line"),
  }),
  hzp_audit_render_line: z.object({
    env: _hzpAuditEnvelope.describe("A built AuditEnvelope to render as a 1-line human view"),
  }),
  // U-WIRE-PROGPARSE: UnifiedProgramParserEngine.parseContent (slot:papa 2026-06-15). filePath is a
  // dialect/extension hint (path.extname/basename); pass a real name for best format detection.
  program_parse_content: z.object({
    content: z.string().min(1).describe("Raw NC/G-code program text to parse"),
    filePath: z.string().optional().describe("Optional file path/name hint for dialect + extension detection (e.g. 'O1234.nc'); omitted -> content-only sniff"),
  }),
  // U-WIRE-MILLCORPUS: MillProgramCorpusEngine canonical-corpus stats (slot:papa 2026-06-15). No params --
  // reads the server-side canonical corpus only; a caller corpus_path is intentionally NOT accepted.
  mill_corpus_stats: z.object({}).optional(),
  // U-WIRE-D2F: DesignToFloorPipelineEngine pre-flight + read-state (slot:papa 2026-06-15). Mirrors
  // PreFlightInput (gcode required; all tool/machine/cut params optional with engine defaults).
  d2f_preflight: z.object({
    gcode: z.string().min(1).describe("CNC g-code program to pre-flight (physics + safety + Monte-Carlo risk)"),
    machine_id: z.string().optional(),
    material: z.string().optional().describe("Material key (engine default: steel_1045)"),
    tool_diameter_mm: z.number().positive().optional(),
    tool_length_mm: z.number().positive().optional(),
    ...
    mc_samples: z.number().int().positive().optional().describe("Monte-Carlo samples (engine default: 500)"),
  }),
  d2f_job_count: z.object({}).optional(),
  d2f_calibration_state: z.object({ material: z.string().optional() }).optional(),
  // U-WIRE-PACT: PactContractTestEngine consumer-driven contract testing (slot:papa->dev 2026-06-15).
  // 3 actions: define a contract, verify a provider response, diff two contract versions. Pure compute.
  pact_define_contract: z.object({
    consumer: z.string().min(1),
    provider: z.string().min(1),
    version: z.string().min(1),
    interactions: z.array(PACT_INTERACTION_SCHEMA).min(1).describe("At least one interaction (engine throws on empty / duplicate ids)"),
    now: z.number().optional().describe("Override createdAt for deterministic tests; engine defaults to Date.now()"),
  }).passthrough(),
  pact_verify_interaction: z.object({
    contract: PACT_CONTRACT_SCHEMA,
    interactionId: z.string().min(1).describe("Interaction id to verify (engine throws if not found in the contract)"),
    actualResponse: z.unknown().describe("The provider response to check against the interaction's matchers"),
  }).passthrough(),
  pact_check_backward_compat: z.object({
    oldContract: PACT_CONTRACT_SCHEMA,
    newContract: PACT_CONTRACT_SCHEMA,
  }).passthrough(),
  // U-WIRE-ACQUISITION: AcquisitionRecommendationEngine tool/holder/coolant procurement (slot:papa->dev 2026-06-15).
  // 6 read/compute actions over an in-mem catalog. recommend/best require a live machine binding (null if unbound).
  // recordPurchase + getPurchaseHistory excluded (mutator + its empty-store reader). recommend+best share AcquisitionInput.
  acquisition_recommend: z.object({
    machine_id: z.string().min(1),
    category: z.enum(["tooling", "holder", "coolant", "software", "accessory"]),
    item_type: z.string().min(1).describe('Specific item type, or "any" to match all'),
    operation_type: z.string().optional(),
    material_iso_group: z.string().optional(),
    max_budget: z.number().optional(),
    preferred_brands: z.array(z.string()).optional(),
    exclude_owned: z.array(z.string()).optional(),
  }).passthrough(),
  acquisition_best: z.object({
    machine_id: z.string().min(1),
    category: z.enum(["tooling", "holder", "coolant", "software", "accessory"]),
  ...
  acquisition_roi: z.object({
    item_cost: z.number(),
    annual_usage_hours: z.number(),
    hourly_rate: z.number(),
    productivity_gain_pct: z.number(),
    tool_life_improvement_pct: z.number(),
  }).passthrough(),
  acquisition_distributor: z.object({ item_id: z.string().min(1) }).passthrough(),
  acquisition_compare: z.object({ item_ids: z.array(z.string().min(1)).min(1) }).passthrough(),
  acquisition_stats: z.object({}).optional(),
  // U-WIRE-MEASURE: MeasureSummaryEngine quality-measurement aggregation (slot:papa->quality 2026-06-15).
  // 7 actions over an in-mem measurement store (process-lifetime). measure_add ingests a data point;
  // the rest aggregate/read/export. Static methods on the class; pure compute (no fs/network).
  measure_add: z.object({
    partNumber: z.string().min(1),
    source: z.enum(["cmm", "surface", "probe", "manual", "vision"]),
  ...
  measure_generate_summary: z.object({
    partNumber: z.string().min(1),
    workOrderNumber: z.string().optional(),
    serialNumber: z.string().optional(),
  }).passthrough(),
  measure_get_summary: z.object({ id: z.string().min(1) }).passthrough(),
  measure_list_summaries: z.object({ partNumber: z.string().min(1) }).passthrough(),
  measure_quality_trend: z.object({ partNumber: z.string().min(1), days: z.number().int().positive().optional() }).passthrough(),
  measure_parts_with_issues: z.object({}).optional(),
  measure_export: z.object({ id: z.string().min(1), format: z.enum(["json", "csv", "text"]) }).passthrough(),
  ...
  // U-DEV-SCHEMA-COVERAGE-WIRE (slot:bravo 2026-06-22): the runtime schema-coverage
  // reporter actions take no params -- giving them real schemas makes the coverage
  // reporter itself schema-covered (dogfoods the gap it measures: these calls count as
  // `validated`, not `passthrough`).
  dispatcher_schema_coverage_stats: z.object({}).optional()
    .describe("Report live dispatcherMiddleware schema coverage: validated vs unvalidated-passthrough counts + the distinct no-schema actions seen this process"),
  dispatcher_schema_coverage_reset: z.object({}).optional()
    .describe("Zero the runtime schema-coverage counters (fresh measurement window); returns the post-reset stats"),
  ...
  svi_mi_weight_learner: z.object({
    componentTimeseries: z.record(z.string(), z.array(z.number())).describe("Per-component time series of Ψ_k values"),
  ...
  engine_acc_record: z.object({
    engine_id: z.string().min(1).optional().describe("Engine id that made the prediction."),
    engineId: z.string().min(1).optional().describe("camelCase alias for engine_id."),
    metric_name: z.string().min(1).optional().describe("Metric name (e.g., 'cutting_force')."),
    metricName: z.string().min(1).optional().describe("camelCase alias for metric_name."),
    predicted: z.number().finite().describe("The value the engine predicted."),
    actual: z.number().finite().describe("The measured/observed actual value."),
    unit: z.string().optional().describe("Optional unit label."),
    context: z.record(z.string(), z.unknown()).optional().describe("Optional context metadata."),
  }).passthrough().refine(
    (d) =>
      ((typeof d.engine_id === "string" && d.engine_id.length > 0) || (typeof d.engineId === "string" && d.engineId.length > 0))
      && ((typeof d.metric_name === "string" && d.metric_name.length > 0) || (typeof d.metricName === "string" && d.metricName.length > 0)),
    { message: "engine_acc_record requires engine_id (or engineId) AND metric_name (or metricName)" },
  ).describe("WRITE side: record a prediction-vs-actual outcome so the engine_acc_* read actions have real data (closes the meta-learning accuracy feedback loop -- recordOutcome was wired NOWHERE)."),
  ...
  commutative_diagram_validate: z.object({
    specA: z.record(z.string(), z.unknown()).describe("First field-rewrite DSL spec ({kind,field,...}); strictly re-parsed by the engine"),
    specB: z.record(z.string(), z.unknown()).describe("Second field-rewrite DSL spec"),
    inputs: z.array(z.record(z.string(), z.unknown())).describe("Records to test commutation over"),
  }).describe("CommutativeTranslatorValidatorEngine — verify two field-rewrites commute (category-theory naturality); catches silent translator divergence"),
};
