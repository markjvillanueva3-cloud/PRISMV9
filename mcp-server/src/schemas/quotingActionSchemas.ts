...
export const quotingActionEnum = z.enum([
  ...
  // U-QP-CONSUMABLE-COST-BASIS (charlie 2026-07-01) -- surface the advisory per-type tool-cost prior
  // (jm-tool-purchases.json byType spend/count = $/line-item) + fold back the reconciliation ledger
  // multipliers. ALWAYS confidence:"advisory" (no per-type qty -> never customer-grade). Cost basis:
  // admin-gated route + generic-surface deny (anon-leak class).
  "tool_cost_basis",                        // U-QP-CONSUMABLE-COST-BASIS -- advisory per-type consumable cost prior + [0.8,1.2] fold-back
]);
...
export const QUOTING_ACTION_SCHEMAS: Record<QuotingAction, z.ZodTypeAny> = {
  ...
  // U-QP-CONSUMABLE-COST-BASIS (charlie 2026-07-01) -- advisory per-type consumable cost prior + reconciliation fold-back
  tool_cost_basis: z.object({
    type: z.string().min(1).optional().describe("Consumable type key (e.g. insert, drill, carbide-blank); omit to return all types"),
    basisPath: z.string().optional().describe("Override the jm-tool-purchases.json path (testing/override)"),
    ledgerPath: z.string().optional().describe("Override the consumable-feedback-multipliers.json ledger path (testing/override)"),
  }),
  // QUOTING-OPTIMAL-MS0/U4 (charlie 2026-06-30) -- per-consumable-type predicted-vs-actual reconciliation
