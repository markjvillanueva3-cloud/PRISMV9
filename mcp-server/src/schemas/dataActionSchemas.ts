...
// ============================================================================
// BRIDGE-WIRING/U-WIRE-SINKER-LORA -- SinkerEDM LoRA cadence + dataset (5 actions)
// Mirrors grinding_lora_* schemas. CadenceInterval: daily|weekly|biweekly|monthly|on-demand
// ============================================================================

const SINKER_CADENCE_INTERVAL = z.enum(["daily", "weekly", "biweekly", "monthly", "on-demand"]);

const sinker_lora_cadence_config = z.object({
  minNewJobs: z.number().int().positive().optional()
    .describe("Minimum new jobs required to trigger a training run."),
  interval: SINKER_CADENCE_INTERVAL.optional()
    .describe("Training cadence interval: daily, weekly, biweekly, monthly, or on-demand."),
  dayOfMonth: z.number().int().min(1).max(28).optional()
    .describe("Day of month (1..28) for monthly cadence -- capped at 28 to avoid month-end ambiguity."),
  hour: z.number().int().min(0).max(23).optional()
    .describe("UTC hour (0..23) at which the scheduled run fires."),
}).passthrough();

const sinker_lora_cadence_record = z.object({
  n: z.number().int().nonnegative()
    .describe("Number of new sinker-EDM jobs to record toward the next training trigger."),
});

const sinker_lora_cadence_state = z.object({}).passthrough();

const sinker_lora_dataset_build = z.object({
  jobs: z.array(
    z.object({
      id: z.string().min(1).describe("Stable job identifier (program filename + rev)."),
      fingerprint: z.record(z.string(), z.union([z.string(), z.number()]))
        .describe("Feature fingerprint keys for geometry hashing."),
      features: z.record(z.string(), z.unknown())
        .describe("Free-form feature map -- material, electrode, cavity geometry, count."),
      actual: z.record(z.string(), z.unknown())
        .describe("Observed actuals -- total_wear_mm, achieved_ra_um, cycle_time_min."),
      weight: z.number().positive().optional()
        .describe("Optional job weight (default 1.0); deep-cavity jobs auto-boosted to 2.0)."),
      labels: z.array(z.string()).optional()
        .describe("Optional labels (e.g. deep-cavity)."),
    }).passthrough(),
  ).nonempty().describe("Non-empty array of raw sinker-EDM job records to build the LoRA dataset from."),
}).passthrough();

const sinker_lora_dataset_schema = z.object({}).passthrough();
