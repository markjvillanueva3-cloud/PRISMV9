...
export const ACTION_CAM_SCHEMAS: Record<string, z.ZodType> = {
  // U-WIRE-COOLANT-ADAPTER: CoolantStrategyAdapter.selectCoolantOrchestrated (slot:papa->kilo 2026-06-15). Mirrors OrchestratedCoolantRequest.
  coolant_select_orchestrated: z.object({
    decision_point: z.string().min(1).describe("Where in the pipeline this coolant decision is made"),
    domain: z.string().optional().describe("CoolantDomain selector (machining/edm/laser/...); engine default machining"),
    material_iso_group: z.enum(["P","M","K","N","S","H"]).optional(),
    material_name: z.string().optional(),
    operation_type: z.enum(["rough","finish","drill","thread","turn","grind","laser_cut","edm_cut"]).optional(),
    cutting_temp_c: z.number().optional(),
    depth_over_diameter: z.number().optional(),
    hrsa_flag: z.boolean().optional(),
    environmental_bias: z.boolean().optional(),
    pipeline_stage: z.string().optional(),
    caller: z.string().optional(),
    objective: z.enum(["speed","quality","cost","balanced","tool_life"]).optional(),
    explain: z.boolean().optional(),
  }).passthrough(),
  // U-WIRE-ENTRYEXIT-ADAPTER: EntryExitStrategyAdapter.selectEntryExitOrchestrated (slot:papa->kilo 2026-06-15). Mirrors OrchestratedEntryExitRequest; engine re-validates so sub-fields stay loose.
  entryexit_select_orchestrated: z.object({
    decision_point: z.string().min(1).describe("Where in the pipeline this entry/exit decision is made"),
    domain: z.string().optional().describe("EntryExitDomain selector; engine defaults by decision_point prefix"),
    phase: z.string().optional().describe("PhaseKind (entry/exit/...)"),
    operation_type: z.enum(["rough","finish","drill","thread","laser_cut","waterjet_cut"]).optional(),
    tool_diameter_mm: z.number().optional(),
    stepover_ae_mm: z.number().optional(),
    stepdown_ap_mm: z.number().optional(),
    center_cutting: z.boolean().optional(),
    surface_finish_critical: z.boolean().optional(),
    material_thickness_mm: z.number().optional(),
  ...
  // U-WIRE-SEQUENCING-ADAPTER: IntelligentSequencingAdapter.selectSequenceOrchestrated (slot:papa->kilo 2026-06-15). Mirrors OrchestratedSequenceRequest; engine re-validates so op sub-fields stay loose. objective set ends in "safety" (not "tool_life").
  sequence_select_orchestrated: z.object({
    decision_point: z.string().min(1).describe("Pipeline decision point (e.g. p2p.sequence_order)"),
    pipeline: z.enum(["milling","turning","multiaxis","millturn","edm","grinding","laser","waterjet","quote"]).optional(),
    operations: z.array(z.object({
      id: z.string(),
      type: z.string(),
      operation: z.string(),
    }).passthrough()).describe("SequenceableOp[] (id/type/operation required per op); empty array -> no_candidates"),
    avg_tool_change_sec: z.number().optional(),
    precision_critical: z.boolean().optional(),
    thermal_sensitive: z.boolean().optional(),
    objective: z.enum(["speed","quality","cost","balanced","safety"]).optional(),
    pipeline_stage: z.string().optional(),
    caller: z.string().optional(),
    explain: z.boolean().optional(),
  }).passthrough(),
  ...
  master_post_hurco_v11: z.object({
    operations: z.array(z.object({
      ...
      // POST-PROCESSOR/U-PP-HURCO-CYCLE (slot:echo) -- optional drilling canned cycle.
      // When present, the engine emits a modal Fanuc-family canned cycle (G81/G82/G83/G73/G84/G85)
      // using coordinates[] as hole XY positions instead of the long-hand move list.
      // Absent -> byte-identical output to pre-cycle behavior (fully additive, no regression risk).
      // Shape mirrors HurcoDrillCycle + HaasDrillCycle (same Fanuc/ISO canned-cycle dialect).
      cycle: z.object({
        type: z.enum(["drill", "dwell", "peck", "chip_break", "tap", "bore"])
          .describe("Canned cycle variant: drill->G81, dwell/spot->G82, peck->G83 (full retract), chip_break->G73 (high-speed peck), tap->G84 (rigid, no M29), bore->G85"),
        depth_mm: z.number()
          .describe("Final hole depth -- absolute Z in mm (negative below the part top)"),
        retract_mm: z.number()
          .describe("R-plane clearance above the part top in mm (positive value)"),
        peck_mm: z.number().optional()
          .describe("Q peck increment in mm -- required for peck/chip_break; absent or <=0 downgrades to G81 simple drill with a warning"),
        dwell_s: z.number().optional()
          .describe("P dwell at hole bottom in seconds -- required for dwell; absent or <=0 downgrades to G81 simple drill with a warning"),
      }).optional().describe("Optional drilling canned cycle (G81/G82/G83/G73/G84/G85). Present -> emit a modal canned cycle from coordinates[] hole XY positions. Absent -> standard long-hand move list (additive, no change for ops without a cycle)."),
  ...
  /**
   * Hurco WinMax LATHE master post (ISNC / Fanuc-dialect turning). Same TurningOperation
   * shape as master_post_okuma_b250; positive() bounds give the boundary validation the
   * engine relies on (rejects empty ops, non-positive pitch/feed). U-PP-HURCO-WINMAX-LATHE-GENERATOR.
   */
  master_post_hurco_winmax_lathe: z.object({
    operations: z.array(z.object({
      operation_type: z.enum([
        "od_rough", "od_finish", "id_rough", "id_finish",
        "face", "groove", "thread", "drill", "bore", "part_off", "c_mill"
      ...
      tool_description: z.string().optional().describe("Tool description for comments"),
      material_iso: z.enum(["P", "M", "K", "N", "S", "H"]).describe("ISO material group"),
      spindle_rpm: z.number().positive().optional().describe("Direct RPM (G97 fixed)"),
      css_m_min: z.number().positive().optional().describe("G96 constant surface speed m/min"),
      css_max_rpm: z.number().positive().optional().describe("G50 spindle speed clamp"),
      feed_mm_rev: z.number().positive().describe("Feed rate mm/rev"),
      depth_of_cut_mm: z.number().positive().describe("Depth of cut mm"),
      ...
      end_z: z.number().describe("End Z coordinate"),
      thread_pitch_mm: z.number().positive().optional().describe("Thread pitch for G76"),
      thread_depth_mm: z.number().positive().optional().describe("Thread depth for G76 (derived from pitch if absent)"),
      groove_width_mm: z.number().positive().optional().describe("Groove width for grooving ops"),
      coolant: z.enum(["flood", "off"]).optional().describe("Coolant mode"),
    })).min(1).describe("Array of turning operations to post-process"),
    config: z.object({
      ...
      use_css: z.boolean().optional().describe("Use G96 constant surface speed (default: true)"),
      css_max_rpm: z.number().positive().optional().describe("G50 spindle clamp RPM (default: 3500)"),
      machine_label: z.string().optional().describe("Machine label for the (MACHINE:) header line"),
    }).optional().describe("Hurco WinMax ISNC lathe post configuration"),
  }),
