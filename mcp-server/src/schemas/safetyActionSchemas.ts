...
// Shared dual-wire schema (U-GOLF-WHISKEY-WORKTREE-INTEGRATION): same engine +
// same params on prism_turning and prism_safety share ONE const (U-MACRO-LIB pattern).
import { lathe_lora_calibration_gate as latheLoraCalibrationGateShared } from "./turningActionSchemas.js";
...
export const ACTION_SAFETY_SCHEMAS: ActionSchemaMap = {
  ...
  // U-LW-C2 (whiskey 2026-07-05): `now` removed — the dispatcher strips it before every FSM
  // mutation so the 5-day quarantine hold cannot be back-dated (see safetyDispatcher.ts).
  wetrun_start_session: z.object({
    session_id: z.string().min(1), tenant_id: z.string().min(1), part_id: z.string(),
    machine_id: z.string(), operator: z.string(),
  }).passthrough(),
  ...
  // U-GOLF-WHISKEY-WORKTREE-INTEGRATION -- dual-wire of the lathe LoRA calibration
  // gate. ONE shared schema const (imported from turningActionSchemas) serves both
  // prism_turning and prism_safety, per the U-MACRO-LIB shared-const precedent --
  // no behavioural drift between the two wires by construction.
  lathe_lora_calibration_gate: latheLoraCalibrationGateShared
    .describe("LatheLoRAUncertaintyQuantifierEngine.assess -- calibration gate over a LoRA-inference output: accept/review/reject verdict from confidence, epistemic support, fusion conflicts, S(x), and hard hazard flags."),
};
