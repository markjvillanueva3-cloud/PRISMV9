...
export const RoadmapEnvelope = z.object({
  ...
  // @ts-ignore - Zod v4 overload mismatch
  scrutiny_config: ScrutinyConfig.default({} as z.input<typeof ScrutinyConfig>),
...
/** Unit status within the roadmap index (lighter than full RoadmapUnit). */
export const IndexUnitStatus = z.enum(["not_started", "in_progress", "complete"]);
export type IndexUnitStatus = z.infer<typeof IndexUnitStatus>;

/** A lightweight unit entry in the roadmap index (not the full RoadmapUnit). */
export const IndexUnit = z.object({
  /** Unit ID, e.g. "U-TK01". */
  id: z.string().min(1),
  /** Human-readable title. */
  title: z.string().min(1),
  /** Current status. */
  status: IndexUnitStatus.default("not_started"),
});
export type IndexUnit = z.infer<typeof IndexUnit>;
...
export const MilestoneEntry = z.object({
  ...
  notes: z.string().optional(),
  /** Freeform description of milestone scope. */
  description: z.string().optional(),
  /** Decomposed units for task queue execution. */
  units: z.array(IndexUnit).optional(),
  /** Instance ID that has claimed this milestone (e.g. "Claude@DESKTOP/pid-1234"). */
  claimed_by: z.string().optional(),
  /** ISO timestamp of when the claim was made. */
