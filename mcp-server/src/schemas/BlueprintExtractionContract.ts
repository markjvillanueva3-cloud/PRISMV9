...
// A dimensional tolerance (mm, signed deviations). The VLM extract already resolves nominal ± deviation to mm
// (ollama-vision-extract-lib.mjs), but normalizeFusedToContract used to DROP it -- an R12 data loss, because
// the tolerance IS the driving variable's allowed range (the parametric-accuracy dimension). Optional: absent
// when the print states no tolerance (a basic/reference dim), never fabricated.
export const contractToleranceSchema = z
  .object({
    type: z.string().describe("tolerance class (bilateral|unilateral_plus|unilateral_minus|limit|basic|reference); 'unknown' if unresolved"),
    upper: z.number().describe("upper deviation in mm (signed; e.g. +0.05)"),
    lower: z.number().describe("lower deviation in mm (signed; e.g. -0.05)"),
  })
  .describe("dimensional tolerance in mm -- the extracted allowed range around value_mm");
...
export const contractDimensionSchema = z
  .object({
    ...
    // ADDITIVE + OPTIONAL (no schemaVersion bump -- non-strict schema, back-compat both directions: old data
    // lacks it -> valid; new data's field is stripped by an old consumer -> valid). Fixes the dropped-tolerance
    // R12 loss so print-read dims carry their allowed range for parametric/tolerance-aware downstream use.
    tolerance: contractToleranceSchema.optional().describe("dimensional tolerance if the print states one; absent for basic/reference dims"),
  })
...
export function normalizeFusedToContract(fused: unknown, opts: NormalizeOpts = {}): BlueprintExtractionContract {
  ...
  const dimensions: ContractDimension[] = (Array.isArray(f.dimensions) ? f.dimensions : [])
    .map((d: Record<string, unknown>) => {
      ...
      // Thread the extracted tolerance (mm) through -- the VLM lib emits d.tolerance_mm = {upper, lower} (mm)
      // + d.tolerance_type. Attach ONLY when BOTH bounds are finite (fail-open: a partial/absent tolerance is
      // omitted, never fabricated, R12). This was silently dropped before (the data-loss bug this fixes).
      const tmm = (d.tolerance_mm && typeof d.tolerance_mm === "object" ? d.tolerance_mm : d.tolerance) as Record<string, unknown> | undefined;
      const tUpper = tmm ? Number(tmm.upper) : NaN;
      const tLower = tmm ? Number(tmm.lower) : NaN;
      const tolerance = Number.isFinite(tUpper) && Number.isFinite(tLower)
        ? { type: typeof d.tolerance_type === "string" && d.tolerance_type ? (d.tolerance_type as string) : "unknown", upper: tUpper, lower: tLower }
        : undefined;
      return {
