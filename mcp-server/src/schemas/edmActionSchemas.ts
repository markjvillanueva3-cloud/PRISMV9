...
/** wedm_taper_angle_correct -- WEDMWireDeflectionEngine.correctTaperAngle (U-WEDM-TAPER-CORRECT) */
const wedm_taper_angle_correct = z
  .object({
    target_taper_deg: z
      .number()
      .nonnegative()
      .describe("Target (intended) wall taper angle (deg), non-negative."),
    discharge_force_N: z
      .number()
      .nonnegative()
      .describe("Discharge/gap force bowing the wire (N), non-negative."),
    wire_tension_N: z.number().positive().describe("Wire tension (N), positive."),
    wire_diameter_mm: z.number().positive().describe("Wire diameter (mm), positive."),
    modulus_GPa: z
      .number()
      .positive()
      .describe("Wire Young's modulus (GPa), positive (brass ~110)."),
    guide_span_mm: z
      .number()
      .positive()
      .optional()
      .describe("Upper-to-lower guide span (mm); defaults to WEDM_TAPER_SPEC."),
    max_taper_deg: z
      .number()
      .positive()
      .optional()
      .describe("Max programmable taper (deg); defaults to WEDM_TAPER_SPEC standard guide max."),
    bow_direction: z
      .union([z.literal(1), z.literal(-1)])
      .optional()
      .describe("+1 (default) = bow opens outward -> subtract theta_bow; -1 = inward bow."),
    max_iter: z
      .number()
      .int()
      .positive()
      .optional()
      .describe("Fixed-point iteration cap (default 12)."),
    tol_deg: z
      .number()
      .positive()
      .optional()
      .describe("Fixed-point convergence tolerance (deg, default 1e-4)."),
  })
  .describe(
    "TaperAngleCorrectInput -- invert the wire-bow taper error to a corrected programmed taper angle.",
  );
