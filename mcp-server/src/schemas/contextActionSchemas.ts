...
export const ACTION_CONTEXT_SCHEMAS: Record<string, z.ZodTypeAny> = {
  ...
  token_economy_compute_roi: z.object({
    task_class: z.enum(["backend", "web", "cad_python", "roadmap", "audit", "speed_feed", "post_process", "erp", "general"]).describe("Task class label for the ROI computation"),
    tokens_spent: z.number().min(0).describe("Total tokens spent on the task"),
    capabilities_delivered: z.number().int().min(0).describe("Count of distinct capabilities delivered"),
  }),
