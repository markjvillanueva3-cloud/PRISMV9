...
// Chatter-stable RPM optimizer -> ChatterStabilityLobeEngine (Altintas-Budak real-part SLD).
// Tool geometry + material + spindle range are the mill-ergonomic core; measured modal params
// are optional overrides of the engine's beam-theory FRF estimate.
const mill_stable_rpm_select = z
  .object({
    // .finite() on every numeric field: Zod .positive()/.gt() ACCEPT +Infinity (only .finite() rejects it),
    // which would otherwise leak an Infinity spindle-speed recommendation (safety-physics scrutiny P1-a).
    tool_diameter_mm: z.number().finite().positive().describe("Cutter diameter (mm)."),
    toolFlutes: z.number().int().positive().describe("Number of flutes."),
    overhang_mm: z.number().finite().positive().optional().describe("Tool stick-out length (mm); defaults to 3x diameter."),
    tool_material: z.string().optional().describe("Tool material key for the FRF estimate (default 'carbide')."),
    iso_group: z.string().optional().describe("ISO material group P/M/K/N/S/H for Kc lookup (default 'P')."),
    specificCuttingForce_Nmm2: z.number().finite().positive().optional().describe("Specific cutting force Kc / kc1.1 (N/mm^2); else derived from iso_group."),
    rpmMin: z.number().finite().positive().optional().describe("Minimum spindle RPM of the search range (default 2000, must be < rpmMax)."),
    rpmMax: z.number().finite().positive().describe("Maximum spindle RPM of the search range (required)."),
    rpm_points: z.number().int().positive().max(5000).optional().describe("RPM sampling resolution, 1..5000 (default 200). Bounded to prevent runaway sweeps."),
    radial_immersion_ratio: z.number().finite().gt(0).max(1).optional().describe("ae/D radial immersion, (0,1] (default 0.5)."),
    up_milling: z.boolean().optional().describe("Up-milling (true) vs down-milling (false, default)."),
    naturalFrequency_Hz: z.number().finite().positive().optional().describe("Measured dominant modal natural frequency (Hz); overrides beam-theory estimate."),
    dampingRatio: z.number().finite().gt(0).lt(1).optional().describe("Measured modal damping ratio zeta in (0,1); overrides default."),
    stiffness_Nmm: z.number().finite().positive().optional().describe("Measured modal stiffness (N/mm); overrides estimate."),
    machine_id: z.string().optional().describe("Machine id to pull FRF from the machine registry."),
  })
  .passthrough();
...
/** mill_counterfactual_analyze -- CounterfactualMillEngine.analyze(MillingBaselineParams).
 *  WIRING/U-WIRE-COUNTERFACTUAL-MILL (slot:romeo). Validates the baseline BEFORE the Kienzle/Taylor
 *  what-if math runs -- a NaN/missing field would otherwise produce garbage force/tool-life deltas. */
const mill_counterfactual_analyze = z
  .object({
    cutting_speed_mpm: z.number().positive().describe("Baseline cutting speed (m/min)."),
    feed_per_tooth_mm: z.number().positive().describe("Baseline feed per tooth (mm)."),
    axial_depth_mm: z.number().positive().describe("Baseline axial depth of cut ap (mm)."),
    radial_depth_mm: z.number().positive().describe("Baseline radial depth of cut ae (mm)."),
    tool_diameter_mm: z.number().positive().describe("Tool diameter (mm)."),
    number_of_teeth: z.number().int().positive().describe("Number of teeth/flutes."),
    material_iso_group: z.enum(["P", "M", "K", "N", "S", "H"]).describe("ISO material group."),
    hardness_hrc: z.number().positive().optional().describe("Optional workpiece hardness (HRC)."),
    operation: z.enum(["roughing", "semi-finishing", "finishing"]).describe("Operation phase."),
    engagement_type: z.enum(["conventional", "climb", "trochoidal"]).optional().describe("Optional engagement pattern."),
  })
  .passthrough()
  .describe("Counterfactual what-if analysis of a milling baseline (Kienzle force, Taylor life, deflection, cycle-time, risk deltas).");
