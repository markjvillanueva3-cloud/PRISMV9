...
const lathe_orchestrate = turningInputZ.extend({
  workpiece_type: z.enum(["bar_stock", "chucked", "casting", "forging"]).optional().describe("Workpiece form: bar stock, chucked billet, casting, or forging"),
  bar_feeder: z.boolean().optional().describe("Bar feeder installed for continuous bar stock feeding"),
  bar_extension_behind_spindle_mm: z.number().optional().describe("Bar extension behind spindle (mm) for whip hazard calculation"),
  part_catcher: z.boolean().optional().describe("Part catcher or sub-spindle available for cutoff support"),
  prove_out: z.boolean().optional().describe("Enable prove-out mode (25% feed reduction, 80% RPM cap) for first article"),
  safety_only: z.boolean().optional().describe("Run only safety + input stages for quick validation"),
  work_offset: z.string().optional().describe("Work coordinate system (G54, G55, etc.)"),
  controller: z.enum(["fanuc", "haas", "mazak", "okuma", "siemens", "dmg_mori", "citizen", "star"]).optional().describe("Target CNC controller for dialect-specific G-code generation"),
}).passthrough();
