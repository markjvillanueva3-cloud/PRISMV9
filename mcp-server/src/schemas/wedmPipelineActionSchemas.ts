...
// ============================================================================
// WEDMSchedulingEngine — U-WGAP06: Machine Reservation (Book-Ahead)
// ============================================================================

const wedm_reserve_machine = z.object({
  machine_id:       z.string().optional().describe("Machine identifier (default: mitsubishi-fa10s)"),
  machine_name:     z.string().optional().describe("Human-readable machine name"),
  job_id:           z.string().optional().describe("Optional job reference ID"),
  job_name:         z.string().optional().describe("Optional job name, e.g. 'ITW SHAKEPROOF Die Insert'"),
  material:         z.string().optional().describe("Workpiece material, e.g. 'D2'"),
  estimated_hours:  z.number().positive().describe("Estimated cutting time in hours"),
  starts_at:        z.string().describe("ISO 8601 start date-time for the reservation"),
  ends_at:          z.string().optional().describe("ISO 8601 end date-time (computed from estimated_hours if omitted)"),
  created_by:       z.string().optional().describe("Creator identifier"),
}).passthrough();

const wedm_check_availability = z.object({
  machine_id:               z.string().optional().describe("Machine identifier (default: mitsubishi-fa10s)"),
  starts_at:                z.string().describe("ISO 8601 start of window to check"),
  ends_at:                  z.string().describe("ISO 8601 end of window to check"),
  exclude_reservation_id:   z.string().optional().describe("Reservation ID to exclude from conflict check (for rescheduling)"),
}).passthrough();

const wedm_cancel_reservation = z.object({
  reservation_id: z.string().describe("UUID of the reservation to cancel"),
}).passthrough();
