...
export const MILL_ACTIONS = [
  ...
  "mill_strategy_record_outcome",   // MillStrategyNeuralEngine.recordOutcome -- capture actuals (closes the learning loop)
  "mill_strategy_train",            // MillStrategyNeuralEngine.trainFromBuffer -- SGD over buffered outcomes
  ...
  // Counterfactual what-if analysis (WIRING/U-WIRE-COUNTERFACTUAL-MILL slot:romeo) --
  // CounterfactualMillEngine.analyze: Kienzle/Taylor/deflection what-if scenarios (speed/feed +-20/50%,
  // tool geometry, engagement, material substitution) with force/tool-life/cycle-time/risk deltas.
      ...
      try {
        ...
        switch (action) {
          ...
          case "mill_validate_program": {
            ...
            const response = await (await getEngine("facade")).orchestrate({
              ...p,
              gcode: p.gcode ?? p.program ?? p.program_text,
              request_type: "validate",
            });
            if (!response.success) throw new Error(response.warnings?.join("; ") || "mill_validate_program failed");
          ...
          case "mill_counterfactual_analyze": {
            // WIRING/U-WIRE-COUNTERFACTUAL-MILL (slot:romeo) -- CounterfactualMillEngine.analyze:
            // Kienzle/Taylor/deflection what-if scenarios (speed/feed +-20/50%, tool geometry,
            // engagement, material) with force/tool-life/cycle-time/risk deltas. Zero-arg singleton.
            const { counterfactualMillEngine } = await import("../../engines/CounterfactualMillEngine.js");
            result = counterfactualMillEngine.analyze(params as Parameters<typeof counterfactualMillEngine.analyze>[0]);
            break;
          }
          ...
          case "mill_strategy_recommend": {
            // Real predict() round-trip. (Was a dead action: it called a non-existent
            // engine method "recommend" and always threw -- sibling of the dead-action
            // cluster in reference_mill_optimizer_dead_actions_2026_06_01.)
            ...
            const num = (key: string): number => {
              ...
              if (typeof v !== "number" || !Number.isFinite(v)) {
                throw new TypeError(`mill_strategy_recommend: '${key}' must be a finite number`);
              }
              return v;
            };
            result = engine.predict({
              material_iso_group: num("material_iso_group"),
              hardness_normalized: num("hardness_normalized"),
              operation_type: num("operation_type"),
              tolerance_class: num("tolerance_class"),
              feature_complexity: num("feature_complexity"),
              machine_class: num("machine_class"),
              depth_to_diameter: num("depth_to_diameter"),
              wall_thickness_ratio: num("wall_thickness_ratio"),
            });
            break;
          }
          ...
          case "mill_strategy_record_outcome": {
            // Actuals side of the strategy-neural learning loop: operator returns the
            // observed outcome of a previously-predicted strategy -> buffered for training.
            const engine = await getEngine("strategy");
            const p = params as Record<string, unknown>;
            if (typeof p.features !== "object" || p.features === null) {
              throw new TypeError("mill_strategy_record_outcome: 'features' object required (StrategyFeatureVector)");
            }
            if (typeof p.label_strategy_id !== "string") {
              throw new TypeError("mill_strategy_record_outcome: 'label_strategy_id' string required");
            }
            if (p.outcome !== "success" && p.outcome !== "partial" && p.outcome !== "failure") {
              throw new TypeError("mill_strategy_record_outcome: 'outcome' must be success|partial|failure");
            }
          ...
          case "mill_strategy_train": {
            // Train side: SGD over buffered outcomes -> updated weights + bumped model version.
            const engine = await getEngine("strategy");
            const p = params as Record<string, unknown>;
            const opts: { epochs?: number; learningRate?: number; failureWeight?: number } = {};
            if (p.epochs !== undefined) {
              if (typeof p.epochs !== "number" || !Number.isInteger(p.epochs) || p.epochs <= 0) {
                throw new TypeError("mill_strategy_train: 'epochs' must be a positive integer");
              }
              opts.epochs = p.epochs;
            }
            if (p.learning_rate !== undefined) {
              if (typeof p.learning_rate !== "number" || !Number.isFinite(p.learning_rate) || p.learning_rate <= 0) {
                throw new TypeError("mill_strategy_train: 'learning_rate' must be a positive finite number");
              }
              opts.learningRate = p.learning_rate;
            }
            if (p.failure_weight !== undefined) {
              if (typeof p.failure_weight !== "number" || !Number.isFinite(p.failure_weight)) {
                throw new TypeError("mill_strategy_train: 'failure_weight' must be a finite number");
              }
              opts.failureWeight = p.failure_weight;
            }
            result = engine.trainFromBuffer(opts);
            break;
          }
          ...
          case "mill_stable_rpm_select": {
            // Chatter-stable spindle-RPM optimizer. Delegates to the physically-correct
            // ChatterStabilityLobeEngine (Altintas-Budak REAL-PART SLD: a_lim = -1/(2*Ks*alpha_xx*Re[G(wc)])),
            // the SAME engine prism_calc:chatter_stability_sld uses -- NOT a bespoke SLD. Returns the
            // max-stable-DOC (max-MRR) chatter-free spindle speed (= the engine's optimal_rpm, a genuine
            // lobe crest). ADVISORY: returns a recommendation; emits NO G-code, commands no machine.
            // (Rewired 2026-07-04: the prior selectStableRPM used FRF magnitude not Re[G] -> lobeless
            // curve -> recommended the MINIMUM-MRR rpm. Caught by the 3-of-3 safety-physics arm.)
            // Field names are the schema-validated flat contract (safeParse ran before this case);
            // no nested/alias fallbacks -- those were dead code (schema mandates the flat names).
            const p = params as Record<string, any>;
            const diameter_mm = p.tool_diameter_mm;
            const flute_count = p.toolFlutes;
            const max_rpm = p.rpmMax;
            const min_rpm = p.rpmMin ?? 2000; // engine's own default, made explicit so the range guard sees it
            if (!(Number.isFinite(diameter_mm) && diameter_mm > 0)) throw new Error("mill_stable_rpm_select: tool_diameter_mm must be a positive finite number");
            if (!(Number.isFinite(flute_count) && flute_count > 0)) throw new Error("mill_stable_rpm_select: toolFlutes must be a positive finite number");
            if (!(Number.isFinite(max_rpm) && max_rpm > 0)) throw new Error("mill_stable_rpm_select: rpmMax must be a positive finite number");
            // Cross-field fail-loud (the removed selectStableRPM had this; the engine would otherwise
            // silently sweep an inverted range and return an out-of-range advisory -- scrutiny P1-b).
            if (!(Number.isFinite(min_rpm) && min_rpm > 0 && min_rpm < max_rpm)) throw new Error(`mill_stable_rpm_select: require 0 < rpmMin < rpmMax (got rpmMin=${min_rpm}, rpmMax=${max_rpm})`);
            const { chatterStabilityLobeEngine } = await import("../../engines/ChatterStabilityLobeEngine.js");
            const chatterInput = {
              tool: {
                diameter_mm,
                flute_count,
                overhang_mm: p.overhang_mm ?? diameter_mm * 3,
                material: p.tool_material ?? "carbide",
              },
              workpiece: {
                iso_group: p.iso_group ?? "P",
                kc11_mpa: p.specificCuttingForce_Nmm2,
              },
              machine: {
                machine_id: p.machine_id,
                natural_frequency_hz: p.naturalFrequency_Hz,
                damping_ratio: p.dampingRatio,
                stiffness_n_um: p.stiffness_Nmm != null ? p.stiffness_Nmm / 1000 : undefined,
                min_rpm,
                max_rpm,
              },
              cutting: {
                radial_immersion_ratio: p.radial_immersion_ratio ?? 0.5,
                up_milling: p.up_milling,
              },
              rpm_points: p.rpm_points ?? 200,
            };
            const sld = chatterStabilityLobeEngine.compute(chatterInput as any).value;
            const recommended_rpm = sld.optimal_rpm;
            // Output fail-loud: never surface a non-finite or out-of-machine-range spindle-speed rec.
            if (!(Number.isFinite(recommended_rpm) && recommended_rpm >= min_rpm && recommended_rpm <= max_rpm)) {
              throw new Error(`mill_stable_rpm_select: engine returned an out-of-range/non-finite rpm (${recommended_rpm}) for [${min_rpm}, ${max_rpm}]`);
            }
            result = {
              recommended_rpm,
              max_stable_doc_mm: sld.max_stable_ap_mm,
              chatter_frequency_hz: sld.chatter_frequency_hz,
              critical_frequency_hz: sld.critical_frequency_hz,
              lobe_count: Array.isArray(sld.lobes) ? sld.lobes.length : 0,
              stable_pockets: sld.stable_pockets,
              source: "chatter-stability-lobe-engine[altintas-budak-real-part-sld]",
            };
            break;
          }
          ...
          case "mill_validate_safety": {
            ...
            if (!response.success) throw new Error(response.warnings?.join("; ") || "mill_validate_safety failed");
            result = response.result;
            break;
          }
