/**
 * Hook Dispatcher - Consolidates hookToolsV2 (8) + hookToolsV3 (10) + hookManagementTools (10) = 28 tools → 1
 * Tool: prism_hook
 * Actions: list, get, execute, chain, toggle, emit, event_list, event_history,
 *          fire, chain_v2, status, history, enable, disable, coverage, gaps, performance, failures
 */
import { z } from "zod";
import { log } from "../../utils/Logger.js";
import { hookEngine } from "../../orchestration/HookEngine.js";
import { eventBus } from "../../engines/EventBus.js";
import { slimResponse } from "../../utils/responseSlimmer.js";
import { dispatcherError, validateActionParams } from "../../utils/dispatcherMiddleware.js";
import { HOOK_ACTION_SCHEMAS } from "../../schemas/hookActionSchemas.js";

const ACTIONS = [
  "list", "get", "execute", "chain", "toggle",
  "emit", "event_list", "event_history",
  "fire", "chain_v2", "status", "history",
  "enable", "disable", "coverage", "gaps", "performance", "failures",
  "subscribe", "reactive_chains",
  "run_ingestion", "ingestion_status", "ingestion_reset",
  "extraction_maintenance", "extraction_maintenance_status",
  "extraction_enforce", "extraction_enforce_check", "extraction_enforce_autofix",
  "routing_run", "routing_status", "routing_upgrades", "routing_consumers",
  "wiring_process", "wiring_stats", "wiring_queue",
  "ai_reason", "ai_classify", "ai_route", "ai_suggest_upgrades", "ai_feedback", "ai_stats"
] as const;

function ok(data: any) {
  return { content: [{ type: "text" as const, text: JSON.stringify(slimResponse(data)) }] };
}

/** Registers hook dispatcher.
 * @param server - MCP server instance
  * @returns void
 */
export function registerHookDispatcher(server: any): void {
  server.tool(
    "prism_hook",
    `Hook & event management (41 actions, includes AI reasoning). Actions: ${ACTIONS.join(", ")}`,
    { action: z.enum(ACTIONS), params: z.record(z.string(), z.any()).optional() },
    async ({ action, params: rawParams = {} }: { action: typeof ACTIONS[number]; params: Record<string, any> }) => {
      log.info(`[prism_hook] ${action}`);
      // H1-MS2: Auto-normalize snake_case → camelCase params
      let params = rawParams;
      try {
        const { normalizeParams } = await import("../../utils/paramNormalizer.js");
        params = normalizeParams(rawParams);
      } catch { /* normalizer not available */ }
      // Zod schema validation
      const validation = validateActionParams(action, params, HOOK_ACTION_SCHEMAS);
      if (!validation.valid) {
        return dispatcherError(
          `Invalid params for '${action}': ${validation.errorMessage}`,
          action,
          "prism_hook"
        );
      }

      try {
        switch (action) {
          // === V2 Hook Tools ===
          case "list": {
            let hooks = hookEngine.listHooks();
            if (params.event) hooks = hooks.filter((h: any) => h.event === params.event);
            if (params.phase) hooks = hooks.filter((h: any) => h.phase === params.phase);
            if (params.enabled !== undefined) hooks = hooks.filter((h: any) => h.enabled === params.enabled);
            return ok({ count: hooks.length, hooks });
          }
          case "get": {
            const hook = hookEngine.getHook(params.hook_id);
            if (!hook) return ok({ error: `Hook not found: ${params.hook_id}` });
            return ok(hook);
          }
          case "execute": {
            const result = await hookEngine.executeHook(params.hook_id, params.data || {});
            return ok(result);
          }
          case "chain": {
            const results = await (hookEngine as unknown as Record<string, ((...args: unknown[]) => Promise<unknown>) | undefined>).executeHookChain?.(params.event, params.phase || "before", params.data || {}, { stopOnError: params.stop_on_error || false, stopOnHalt: params.stop_on_halt ?? true })
              ?? await hookEngine.executeChain([params.event], params.data || {});
            return ok(results);
          }
          case "toggle": {
            hookEngine.toggleHook(params.hook_id, params.enabled);
            return ok({ hook_id: params.hook_id, enabled: params.enabled });
          }
          // === Event Tools ===
          case "emit": {
            const result = await eventBus.publish(params.event, params.data || {});
            return ok({ event: params.event, result });
          }
          case "event_list": {
            const events = eventBus.listEvents(params.category);
            return ok({ count: events.length, events });
          }
          case "event_history": {
            const history = eventBus.getHistory({ limit: params.limit ?? 20, type: params.event, category: params.category });
            return ok({ count: history.length, history });
          }
          // === V3/Management Tools (unified) ===
          case "fire": {
            const safety = params.validate_safety ?? true;
            if (safety) {
              const hook = hookEngine.getHook(params.hook_id);
              if (!hook) return ok({ error: `Hook not found: ${params.hook_id}` });
            }
            const result = await hookEngine.executeHook(params.hook_id, params.data || params.context || {});
            return ok(result);
          }
          case "chain_v2": {
            const results = await (hookEngine as unknown as Record<string, ((...args: unknown[]) => Promise<unknown>) | undefined>).executeHookChain?.(params.event, params.phase || "before", params.data || {}, { stopOnError: true, parallel: params.parallel || false, enableRollback: params.enable_rollback ?? true })
              ?? await hookEngine.executeChain([params.event], params.data || {});
            return ok(results);
          }
          case "status": {
            let hooks = hookEngine.listHooks();
            if (params.filter_domain) hooks = hooks.filter((h: any) => h.domain === params.filter_domain);
            if (params.filter_enabled !== undefined) hooks = hooks.filter((h: any) => h.enabled === params.filter_enabled);
            const enabled = hooks.filter((h: any) => h.enabled).length;
            const disabled = hooks.length - enabled;
            return ok({ total: hooks.length, enabled, disabled, hooks: params.show_metrics !== false ? hooks : hooks.map((h: any) => ({ id: h.id, enabled: h.enabled, event: h.event })) });
          }
          case "history": {
            let history = eventBus.getHistory({ limit: params.last_n ?? 50 });
            if (params.event) history = history.filter((h: any) => h.event === params.event);
            if (params.hook_id) history = history.filter((h: any) => h.data?.hook_id === params.hook_id);
            return ok({ count: history.length, history });
          }
          case "enable": {
            hookEngine.toggleHook(params.hook_id, true);
            log.info(`[prism_hook] Enabled ${params.hook_id}: ${params.reason || "no reason"}`);
            return ok({ hook_id: params.hook_id, enabled: true, reason: params.reason });
          }
          case "disable": {
            hookEngine.toggleHook(params.hook_id, false);
            log.info(`[prism_hook] Disabled ${params.hook_id}: ${params.reason || "no reason"}`);
            return ok({ hook_id: params.hook_id, enabled: false, reason: params.reason, temporary: params.temporary || false });
          }
          case "coverage": {
            const coverage = hookEngine.getCoverage(params.domain);
            return ok(coverage);
          }
          case "gaps": {
            const gaps = hookEngine.getGaps(params.domain, params.severity || "all");
            return ok(gaps);
          }
          case "performance": {
            const perf = hookEngine.getPerformance(params.hook_id, params.sort_by ?? "avg_duration", params.limit ?? 20);
            return ok(perf);
          }
          case "failures": {
            const failures = hookEngine.getFailures(params.hook_id, params.last_n ?? 100, params.include_stack ?? false);
            return ok(failures);
          }
          // === Pub/Sub Protocol Actions (R3-MS4.5) ===
          case "subscribe": {
            if (!params.event) return ok({ error: "Missing required param: event" });
            if (typeof params.callback !== "function") {
              // When called via MCP tool (not programmatic), store a no-op subscription and return id
              const subId = eventBus.subscribeTyped({
                event: params.event,
                filter: params.filter,
                callback: (_evt) => { /* no-op for MCP-registered subscriptions */ },
                description: params.description,
                active: params.active !== false
              });
              return ok({ subscription_id: subId, event: params.event, filter: params.filter });
            }
            const subId = eventBus.subscribeTyped({
              event: params.event,
              filter: params.filter,
              callback: params.callback,
              description: params.description,
              active: params.active !== false
            });
            return ok({ subscription_id: subId, event: params.event });
          }
          case "reactive_chains": {
            const chains = eventBus.getReactiveChains();
            return ok({ count: chains.length, chains });
          }
          // === Extraction Ingestion Actions ===
          case "run_ingestion": {
            const { runIngestion } = await import("../../hooks/extractionIngestionHook.js");
            const result = await runIngestion();
            return ok(result);
          }
          case "ingestion_status": {
            const { getIngestionState, detectNewExtractions } = await import("../../hooks/extractionIngestionHook.js");
            const state = getIngestionState();
            const pending = detectNewExtractions();
            return ok({
              ...state,
              pending_files: pending.length,
              pending: pending.map(p => ({ path: p.path, type: p.type }))
            });
          }
          case "ingestion_reset": {
            const { resetIngestionState } = await import("../../hooks/extractionIngestionHook.js");
            resetIngestionState();
            return ok({ reset: true, message: "Ingestion state cleared — all files will be re-processed on next run" });
          }
          // === Extraction Maintenance Actions ===
          case "extraction_maintenance": {
            const { runMaintenanceCheck, saveMaintenanceReport } = await import("../../hooks/extractionMaintenanceHook.js");
            const report = runMaintenanceCheck();
            const reportPath = saveMaintenanceReport(report);
            return ok({ ...report, report_saved: reportPath });
          }
          case "extraction_maintenance_status": {
            const { getQuickStatus } = await import("../../hooks/extractionMaintenanceHook.js");
            return ok(getQuickStatus());
          }
          // === Extraction Enforcement Actions ===
          case "extraction_enforce": {
            const { enforceExtractionPipeline } = await import("../../hooks/extractionEnforcementHooks.js");
            const result = await enforceExtractionPipeline({
              operation: params.operation || "build",
              phase: params.phase || "pre",
              metadata: params.metadata,
            });
            if (result.blocked) {
              return {
                content: [{ type: "text", text: JSON.stringify({
                  blocked: true,
                  blocker: result.blocker,
                  reason: result.reason,
                  pending_actions: result.pending_actions,
                  auto_fix_available: result.auto_fix_available,
                }) }]
              };
            }
            return ok({ passed: true, blocked: false });
          }
          case "extraction_enforce_check": {
            const { enforceExtractionPipeline } = await import("../../hooks/extractionEnforcementHooks.js");
            // Dry run - just check, don't block
            const result = await enforceExtractionPipeline({
              operation: "check", // Non-enforced operation
              phase: "pre",
            });
            return ok({
              would_block: false, // check operation doesn't block
              status: result.blocked ? "Would block" : "Would pass",
              reasons: result.reasons,
              hints: "Use extraction_enforce with operation param to test blocking",
            });
          }
          case "extraction_enforce_autofix": {
            const { autoFixIngestion } = await import("../../hooks/extractionEnforcementHooks.js");
            const result = await autoFixIngestion();
            return ok(result);
          }
          // === Extraction Routing Actions ===
          case "routing_run": {
            const { runRoutingPipeline } = await import("../../hooks/extractionRoutingHooks.js");
            const { detectNewExtractions } = await import("../../hooks/extractionIngestionHook.js");
            const { readFileSync } = await import("fs");

            // Get files to route
            let files = detectNewExtractions();
            if (params.source_file) {
              files = files.filter(f => f.path.includes(params.source_file));
            }

            if (files.length === 0) {
              return ok({ message: "No files to route", files_routed: 0 });
            }

            const results = [];
            for (const file of files.slice(0, 5)) { // Limit to 5 files per call
              try {
                const content = JSON.parse(readFileSync(file.path, "utf-8"));
                let items: any[] = [];
                if (Array.isArray(content)) items = content;
                else if (content.items) items = content.items;
                else if (content.tips) items = content.tips;
                else if (content.workflows?.items) items = content.workflows.items;
                else items = [content];

                const result = await runRoutingPipeline(items.slice(0, 50), file.path, file.type);
                results.push({
                  file: file.path,
                  blocked: result.blocked,
                  routes: result.wiring_actions?.length || 0,
                  upgrades: result.upgrades?.length || 0,
                });
              } catch (err: any) {
                results.push({ file: file.path, error: err.message });
              }
            }
            return ok({ files_routed: results.length, results });
          }
          case "routing_status": {
            const { extractionRouter } = await import("../../engines/ExtractionIntelligenceRouter.js");
            const summary = extractionRouter.getRoutingSummary();
            return ok(summary);
          }
          case "routing_upgrades": {
            const { extractionRouter } = await import("../../engines/ExtractionIntelligenceRouter.js");
            let upgrades = extractionRouter.getPendingUpgrades();
            if (params.priority && params.priority !== "all") {
              upgrades = upgrades.filter(u => u.priority === params.priority);
            }
            return ok({ count: upgrades.length, upgrades: upgrades.slice(0, 50) });
          }
          case "routing_consumers": {
            const { extractionRouter } = await import("../../engines/ExtractionIntelligenceRouter.js");
            let consumers = extractionRouter.getConsumerRegistry();
            if (params.content_type) {
              consumers = consumers.filter(c => c.accepts.includes(params.content_type as any));
            }
            if (params.domain) {
              consumers = consumers.filter(c => c.domains.includes(params.domain as any));
            }
            return ok({
              count: consumers.length,
              consumers: consumers.map(c => ({
                id: c.id,
                name: c.name,
                type: c.type,
                accepts: c.accepts,
                domains: c.domains,
                wiring_method: c.wiring_method,
              })),
            });
          }
          // === Extraction Wiring Actions ===
          case "wiring_process": {
            const { extractionWiringEngine } = await import("../../engines/ExtractionWiringEngine.js");
            const report = await extractionWiringEngine.processQueue();
            return ok(report);
          }
          case "wiring_stats": {
            const { extractionWiringEngine } = await import("../../engines/ExtractionWiringEngine.js");
            const stats = extractionWiringEngine.getStats();
            return ok(stats);
          }
          case "wiring_queue": {
            const { existsSync, readFileSync } = await import("fs");
            const { join } = await import("path");
            const queueFile = join(process.cwd(), "data/state/wiring-actions-queue.json");
            if (!existsSync(queueFile)) {
              return ok({ queue_exists: false, actions: [], pending: 0 });
            }
            try {
              const queue = JSON.parse(readFileSync(queueFile, "utf-8"));
              const pending = (queue.actions || []).filter((a: any) => a.status === "pending").length;
              return ok({
                queue_exists: true,
                total: queue.actions?.length || 0,
                pending,
                applied: (queue.actions || []).filter((a: any) => a.status === "applied").length,
                failed: (queue.actions || []).filter((a: any) => a.status === "failed").length,
                updated: queue.updated,
              });
            } catch (err: any) {
              return ok({ queue_exists: true, error: err.message });
            }
          }
          // === AI Reasoning Actions ===
          case "ai_reason": {
            const { aiExtractionReasoner } = await import("../../engines/AIExtractionReasonerEngine.js");
            if (!params.content) {
              return ok({ error: "Missing required param: content" });
            }
            const decision = await aiExtractionReasoner.reason(params.content, params.source_file || "api");
            return ok({
              content_id: decision.content_id,
              classification: decision.classification,
              consumers: decision.recommended_consumers.length,
              upgrades: decision.upgrade_suggestions.length,
              reasoning: decision.reasoning_chain,
              processing_time_ms: decision.processing_time_ms,
            });
          }
          case "ai_classify": {
            const { aiExtractionReasoner } = await import("../../engines/AIExtractionReasonerEngine.js");
            if (!params.content) {
              return ok({ error: "Missing required param: content" });
            }
            const classification = await aiExtractionReasoner.classifyContent(params.content);
            return ok(classification);
          }
          case "ai_route": {
            const { aiExtractionReasoner } = await import("../../engines/AIExtractionReasonerEngine.js");
            if (!params.content || !params.classification) {
              return ok({ error: "Missing required params: content, classification" });
            }
            const consumers = await aiExtractionReasoner.routeContent(params.content, params.classification);
            return ok({ count: consumers.length, consumers });
          }
          case "ai_suggest_upgrades": {
            const { aiExtractionReasoner } = await import("../../engines/AIExtractionReasonerEngine.js");
            if (!params.content || !params.classification) {
              return ok({ error: "Missing required params: content, classification" });
            }
            const upgrades = await aiExtractionReasoner.suggestUpgrades(params.content, params.classification);
            return ok({ count: upgrades.length, upgrades });
          }
          case "ai_feedback": {
            const { aiExtractionReasoner } = await import("../../engines/AIExtractionReasonerEngine.js");
            if (!params.decision_id || !params.consumer_id || params.wiring_successful === undefined) {
              return ok({ error: "Missing required params: decision_id, consumer_id, wiring_successful" });
            }
            aiExtractionReasoner.recordFeedback({
              decision_id: params.decision_id,
              consumer_id: params.consumer_id,
              wiring_successful: params.wiring_successful,
              actual_value_provided: params.actual_value_provided || "medium",
              feedback_notes: params.feedback_notes,
              timestamp: new Date().toISOString(),
            });
            return ok({ recorded: true });
          }
          case "ai_stats": {
            const { aiExtractionReasoner } = await import("../../engines/AIExtractionReasonerEngine.js");
            return ok(aiExtractionReasoner.getStats());
          }
          default: return ok({ error: `Unknown action: ${action}`, available: ACTIONS });
        }
      } catch (err: any) {
        return dispatcherError(err, action, "prism_hook");
      }
    }
  );
}
