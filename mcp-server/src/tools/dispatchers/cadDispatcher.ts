      ...
      try {
        ...
        switch (action) {
          ...
          case "feature_recognize": {
            // U-DELTA-CADGEN-FEATRECOG-FIX: route at the REAL heuristic recognizer.
            // The old getEngine("feature") -> FeatureRecognitionEngine.recognize(features[])
            // expected an ALREADY-recognized feature array, so raw geometry ({holes,pockets,...})
            // hit `params.length === undefined` -> the loop never ran -> silent {features:[]}.
            // That starved cad->cam + feature-learning. CADFeatureRecognitionEngine.extractFeatures
            // takes GeometryInput and returns {features:[{type,id,confidence,details}],confidence,counts}.
            // Mirrors the proven wiring in cadAutomationDispatcher `cad_feature_extract`.
            const { cadFeatureRecognitionEngine } = await import("../../engines/CADFeatureRecognitionEngine.js");
            // Schema declares params.geometry (see featureRecognizeSchema); fall back to raw
            // params for flat direct callers who pass {holes,pockets,...} at top level.
            const geometry = (params as { geometry?: unknown }).geometry ?? params;
            const extracted = cadFeatureRecognitionEngine.extractFeatures(geometry);
            // Preserve the top-level `.features` contract every consumer reads (routes/cad.ts ->
            // frontend, mill pipeline). `total_features` is an additive back-compat field for any
            // legacy caller that read the old engine's total_features. `counts`/`confidence` pass through.
            result = {
              ...extracted,
              total_features: extracted.features.length,
              source: "CADFeatureRecognitionEngine.extractFeatures",
            };
            break;
          }
          case "feature_edit": {
