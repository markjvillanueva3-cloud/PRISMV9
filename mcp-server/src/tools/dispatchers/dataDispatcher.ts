/**
 * Data Access Dispatcher - Consolidates data tools → 1 dispatcher (58 actions)
...
const DataDispatcherSchema = z.object({
  action: z.enum([
    ...
    // Ingestion pipeline — folder scanning + orchestration
    "scan_folder", "ingest_batch", "ingest_status", "ingest_resume",
    // Spreadsheet import — employee, vendor, material data
    "employee_import", "spreadsheet_parse", "spreadsheet_detect_columns",
    // Tool holder catalog — INGEST-MS4
    "holder_catalog_create", "holder_catalog_search", "holder_catalog_get",
    "holder_catalog_update", "holder_catalog_by_machine", "holder_catalog_by_taper",
    "holder_catalog_inventory", "holder_catalog_reorder", "holder_catalog_adjust_stock",
    // Business document extraction — INGEST-MS5
    "extract_document", "extraction_get", "extraction_approve",
    "extraction_reject", "extraction_pending", "extraction_search", "extraction_stats",
    // Material stock — INGEST-MS4B
    "material_stock_create", "material_stock_search", "material_stock_get",
    "material_stock_adjust", "material_stock_import", "material_stock_summary",
    "material_stock_seed_jmdie",
    // Shop completeness / My Shop
    "shop_completeness", "shop_completeness_domain", "shop_recommendations",
    "shop_summary",
    // RES-MS0: Resource harvesting pipeline
    "harvest_queue", "harvest_progress", "harvest_pending",
    "harvest_complete", "harvest_failed", "harvest_retry",
    "harvest_summary", "harvest_routes",
  ]),
...
export function registerDataDispatcher(server: any): void {
      ...
      try {
        switch (action) {
          ...
          case "scan_folder": {
            ...
            result = folderScannerEngine.scan({
              root_path: (params.root_path as string),
              file_types: (params.file_types as string[] | undefined) as any,
              seed_domain: (params.seed_domain as string | undefined) as any,
              include_unchanged: (params.include_unchanged as boolean | undefined),
            });
            break;
          }
          ...
          case "ingest_batch": {
            ...
            result = await ingestionOrchestratorEngine.processBatch({
              files: scanResult.files,
              dry_run: (params.dry_run as boolean | undefined),
              targets: (params.targets as import("../../engines/IngestionOrchestratorEngine.js").IngestionTarget[] | undefined),
              seed_domain: (params.seed_domain as string | undefined) as any,
            });
            break;
          }
          ...
          case "ingest_status": {
            ...
            const { ingestionOrchestratorEngine } = await import("../../engines/IngestionOrchestratorEngine.js");
            result = {
              stats: ingestionOrchestratorEngine.getStats(),
              summary: folderScannerEngine.getSummary(),
            };
            break;
          }
          ...
          case "ingest_resume": {
            const { ingestionOrchestratorEngine } = await import("../../engines/IngestionOrchestratorEngine.js");
            result = await ingestionOrchestratorEngine.retryAllFailed();
            break;
          }
          ...
          case "employee_import": {
            ...
            const csv = String(params?.csv_content ?? params?.content ?? "");
            const parseResult = spreadsheetIngestionEngine.parseCSVContent(csv, "employee");
            result = await spreadsheetIngestionEngine.importEmployees(parseResult);
            break;
          }
          case "spreadsheet_parse": {
            const { spreadsheetIngestionEngine } = await import("../../engines/SpreadsheetIngestionEngine.js");
            const csv = String(params?.csv_content ?? params?.content ?? "");
            const target = String(params?.target ?? "employee") as any;
            result = spreadsheetIngestionEngine.parseCSVContent(csv, target);
            break;
          }
          case "spreadsheet_detect_columns": {
            ...
            const target = String(params?.target ?? "employee") as any;
            result = spreadsheetIngestionEngine.detectColumns(headers, target);
            break;
          }

          // ── Tool Holder Catalog — INGEST-MS4 ──────────────────────────
          case "holder_catalog_create": {
            const { toolHolderCatalogEngine } = await import("../../engines/ToolHolderCatalogEngine.js");
            result = toolHolderCatalogEngine.create({
              type: String(params.type ?? "other") as any,
              brand: String(params.brand ?? ""),
              model: params.model as string | undefined,
              taper: String(params.taper ?? "other") as any,
              bore_diameter_mm: Number(params.bore_diameter_mm ?? 0),
              gauge_length_mm: params.gauge_length_mm as number | undefined,
              projection_length_mm: params.projection_length_mm as number | undefined,
              max_rpm: params.max_rpm as number | undefined,
              runout_um: params.runout_um as number | undefined,
              coolant_through: params.coolant_through as boolean | undefined,
              machine_ids: params.machine_ids as string[] | undefined,
              quantity_on_hand: params.quantity_on_hand as number | undefined,
              min_quantity: params.min_quantity as number | undefined,
              location: params.location as string | undefined,
              supplier: params.supplier as string | undefined,
              unit_cost: params.unit_cost as number | undefined,
              notes: params.notes as string | undefined,
            });
            break;
          }
          case "holder_catalog_search": {
            const { toolHolderCatalogEngine } = await import("../../engines/ToolHolderCatalogEngine.js");
            result = toolHolderCatalogEngine.search({
              query: params.query as string | undefined,
              type: params.type as any,
              taper: params.taper as any,
              brand: params.brand as string | undefined,
              machine_id: params.machine_id as string | undefined,
              bore_min_mm: params.bore_min_mm as number | undefined,
              bore_max_mm: params.bore_max_mm as number | undefined,
              coolant_through: params.coolant_through as boolean | undefined,
              in_stock_only: params.in_stock_only as boolean | undefined,
              limit: params.limit as number | undefined,
            });
            break;
          }
          case "holder_catalog_get": {
            const { toolHolderCatalogEngine } = await import("../../engines/ToolHolderCatalogEngine.js");
            const holderId = String(params.holder_id ?? params.id ?? "");
            result = toolHolderCatalogEngine.get(holderId) ?? { error: `Holder not found: ${holderId}` };
            break;
          }
          case "holder_catalog_update": {
            const { toolHolderCatalogEngine } = await import("../../engines/ToolHolderCatalogEngine.js");
            result = toolHolderCatalogEngine.update(String(params.holder_id ?? params.id ?? ""), {
              quantity_on_hand: params.quantity_on_hand as number | undefined,
          ...
          case "holder_catalog_by_machine": {
            const { toolHolderCatalogEngine } = await import("../../engines/ToolHolderCatalogEngine.js");
            result = toolHolderCatalogEngine.getByMachine(String(params.machine_id ?? ""));
            break;
          }
          case "holder_catalog_by_taper": {
            const { toolHolderCatalogEngine } = await import("../../engines/ToolHolderCatalogEngine.js");
            result = toolHolderCatalogEngine.getByTaper(String(params.taper ?? "") as any);
            break;
          }
          case "holder_catalog_inventory": {
            const { toolHolderCatalogEngine } = await import("../../engines/ToolHolderCatalogEngine.js");
            result = toolHolderCatalogEngine.getInventorySummary(params.machine_ids as string[] | undefined);
            break;
          }
          case "holder_catalog_reorder": {
            const { toolHolderCatalogEngine } = await import("../../engines/ToolHolderCatalogEngine.js");
            result = toolHolderCatalogEngine.getReorderList();
            break;
          }
          case "holder_catalog_adjust_stock": {
            const { toolHolderCatalogEngine } = await import("../../engines/ToolHolderCatalogEngine.js");
            result = toolHolderCatalogEngine.adjustStock(
              String(params.holder_id ?? params.id ?? ""),
          ...
          case "material_stock_create": {
            ...
            result = materialStockEngine.create({
              material_spec: String(params.material_spec ?? ""),
              common_name: params.common_name as string | undefined,
              iso_group: params.iso_group as any,
              form: params.form as any,
              size_label: params.size_label as string | undefined,
              quantity_on_hand: params.quantity_on_hand as number | undefined,
              ...
              supplier: params.supplier as string | undefined,
              unit_cost: params.unit_cost as number | undefined,
              hardness_hrc: params.hardness_hrc as number | undefined,
            });
            break;
          }
          case "material_stock_search": {
            const { materialStockEngine } = await import("../../engines/MaterialStockEngine.js");
            result = materialStockEngine.search({
              query: params.query as string | undefined,
              material_spec: params.material_spec as string | undefined,
              iso_group: params.iso_group as any,
              form: params.form as any,
              supplier: params.supplier as string | undefined,
              in_stock_only: params.in_stock_only as boolean | undefined,
              low_stock_only: params.low_stock_only as boolean | undefined,
            });
            break;
          }
          case "material_stock_get": {
            const { materialStockEngine } = await import("../../engines/MaterialStockEngine.js");
            const stockId = String(params.item_id ?? params.id ?? "");
            result = materialStockEngine.get(stockId) ?? { error: `Stock item not found: ${stockId}` };
            break;
          }
          case "material_stock_adjust": {
            const { materialStockEngine } = await import("../../engines/MaterialStockEngine.js");
            result = materialStockEngine.adjustStock(
              String(params.item_id ?? params.id ?? ""),
              Number(params.delta ?? 0),
              params.reason as string | undefined,
            );
            break;
          }
          case "material_stock_import": {
            const { materialStockEngine } = await import("../../engines/MaterialStockEngine.js");
            result = materialStockEngine.importFromRows(params.rows as any[] ?? []);
            break;
          }
          case "material_stock_summary": {
            const { materialStockEngine } = await import("../../engines/MaterialStockEngine.js");
            result = materialStockEngine.getSummary();
            break;
          }
          case "material_stock_seed_jmdie": {
            const { materialStockEngine } = await import("../../engines/MaterialStockEngine.js");
            const count = materialStockEngine.seedJMDieMaterials();
            result = { seeded: count, total: materialStockEngine.list().length };
            break;
          }
          ...
          case "extract_document": {
            ...
            result = businessDocumentExtractorEngine.extract({
              filename: String(params.filename ?? ""),
              document_type: params.document_type as any,
              text_content: params.text_content as string | undefined,
              po_data: params.po_data as any,
              invoice_data: params.invoice_data as any,
              rfq_data: params.rfq_data as any,
            });
            break;
          }
          case "extraction_get": {
            const { businessDocumentExtractorEngine } = await import("../../engines/BusinessDocumentExtractorEngine.js");
            const extId = String(params.extraction_id ?? params.id ?? "");
            result = businessDocumentExtractorEngine.get(extId) ?? { error: `Extraction not found: ${extId}` };
            break;
          }
          case "extraction_approve": {
            const { businessDocumentExtractorEngine } = await import("../../engines/BusinessDocumentExtractorEngine.js");
            result = businessDocumentExtractorEngine.approve(
              String(params.extraction_id ?? params.id ?? ""),
              String(params.reviewer ?? ""),
              params.corrections as any,
            );
            break;
          }
          case "extraction_reject": {
            const { businessDocumentExtractorEngine } = await import("../../engines/BusinessDocumentExtractorEngine.js");
            result = businessDocumentExtractorEngine.reject(
              String(params.extraction_id ?? params.id ?? ""),
              String(params.reviewer ?? ""),
          ...
          case "extraction_pending": {
            const { businessDocumentExtractorEngine } = await import("../../engines/BusinessDocumentExtractorEngine.js");
            result = businessDocumentExtractorEngine.getPendingReview();
            break;
          }
          case "extraction_search": {
            const { businessDocumentExtractorEngine } = await import("../../engines/BusinessDocumentExtractorEngine.js");
            result = businessDocumentExtractorEngine.search({
              document_type: params.document_type as any,
              status: params.status as any,
          ...
          case "extraction_stats": {
            const { businessDocumentExtractorEngine } = await import("../../engines/BusinessDocumentExtractorEngine.js");
            result = businessDocumentExtractorEngine.getStats();
            break;
          }

          // ── Shop Completeness / My Shop — INGEST-MS6 ───────────────────
          case "shop_completeness": {
            const { shopDataCompletenessEngine } = await import("../../engines/ShopDataCompletenessEngine.js");
            result = await shopDataCompletenessEngine.calculateCompleteness(
              params.profile_id as string | undefined,
            );
            break;
          }
          case "shop_completeness_domain": {
            const { shopDataCompletenessEngine } = await import("../../engines/ShopDataCompletenessEngine.js");
            result = await shopDataCompletenessEngine.getDomainGaps(
              params.domain as any,
              params.profile_id as string | undefined,
            );
            break;
          }
          case "shop_recommendations": {
            const { shopDataCompletenessEngine } = await import("../../engines/ShopDataCompletenessEngine.js");
            result = await shopDataCompletenessEngine.getRecommendations(
              params.profile_id as string | undefined,
            );
            break;
          }
          case "shop_summary": {
            const { shopDataCompletenessEngine } = await import("../../engines/ShopDataCompletenessEngine.js");
            const completeness = await shopDataCompletenessEngine.calculateCompleteness(
              params.profile_id as string | undefined,
            );
            ...
            result = {
              shop_name: profile.company_profile.legal_name,
              overall_completeness: completeness.overall_percentage,
          ...
          // ── RES-MS0: Resource Harvesting Pipeline ───────────────
          case "harvest_queue": {
            const { HarvestPipelineEngine: HP } = await import("../../engines/HarvestPipelineEngine.js");
            const { folderScannerEngine } = await import("../../engines/FolderScannerEngine.js");
            const rootPath = (params.root_path as string) || "H:/prism/resources";
            const scanResult = folderScannerEngine.scan({ root_path: rootPath });
            const queueResult = HP.queueFiles(scanResult.files);
            result = { scan: { total: scanResult.total_files, new: scanResult.new_files }, queue: queueResult };
            break;
          }
          case "harvest_progress": {
            const { HarvestPipelineEngine: HP } = await import("../../engines/HarvestPipelineEngine.js");
            result = HP.getProgress();
            break;
          }
          case "harvest_pending": {
            const { HarvestPipelineEngine: HP } = await import("../../engines/HarvestPipelineEngine.js");
            result = HP.getPendingFiles(params.file_type as any, (params.limit as number) || 100);
            break;
          }
          case "harvest_complete": {
            const { HarvestPipelineEngine: HP } = await import("../../engines/HarvestPipelineEngine.js");
            HP.markComplete(params.file_path as string, (params.items_extracted as number) || 0);
            result = { success: true };
            break;
          }
          case "harvest_failed": {
            const { HarvestPipelineEngine: HP } = await import("../../engines/HarvestPipelineEngine.js");
            HP.markFailed(params.file_path as string, (params.error as string) || "unknown");
            result = { success: true };
            break;
          }
          case "harvest_retry": {
            const { HarvestPipelineEngine: HP } = await import("../../engines/HarvestPipelineEngine.js");
            result = { retried: HP.retryFailed() };
            break;
          }
          case "harvest_summary": {
            const { HarvestPipelineEngine: HP } = await import("../../engines/HarvestPipelineEngine.js");
            result = HP.getSummary();
            break;
          }
          case "harvest_routes": {
            const { HarvestPipelineEngine: HP } = await import("../../engines/HarvestPipelineEngine.js");
  ...
  log.info("[dataDispatcher] Registered prism_data (147 actions)");
}
