...
const ACTIONS = [
  ...
  // ── Process Routing (QUOTE-GROUNDING-MS0/U1) ──
  "quote_route_operations",
  // ── Quote Revisions ──
  "quote_revise",
  "quote_get_history",
  ...
  // ── ERP Autofeed (QUOTING-ERP-AUTOFEED) -- project a completed pipeline
  //    result into the ERP/department/portal field-map, then commit the writes ──
  "quote_to_ship_erp_autofeed",
  "quote_to_ship_erp_commit",
  ...
  "traveler_get",
  "traveler_scan",
  // ── Auto-generated print→shipping traveler + per-dept checklists (U-TRAVGEN/U-JOBCHECKLIST) ──
  "traveler_generate",
  "traveler_checklist_get",
  "traveler_checklist_check",
  "traveler_checklist_uncheck",
  "traveler_checklist_step_complete",
...
/**
 * OPTIONAL producer link: when a quote->order bridge result is produced and the
 * caller opted in (params.generate_traveler truthy) AND supplied a part spec
 * (material_iso_group + stock), auto-generate the print->shipping traveler +
 * per-department checklists and attach it as `result.traveler`. Additive and
 * fail-soft: any error leaves the original bridge result untouched (R12 -- the
 * order create must never be lost because traveler generation failed).
 * @param bridgeResult - the {order, work_orders, bridge} result from the bridge engine
 * @param params - the dispatcher params (carries the opt-in flag + part spec)
 * @returns the bridge result, optionally with a `.traveler` field
 */
async function maybeAttachTraveler(bridgeResult: any, params: any): Promise<any> {
  if (!params || !params.generate_traveler) return bridgeResult;
  const order = bridgeResult?.order;
  if (!order || typeof order.id !== "string") return bridgeResult;
  // A quote/order does NOT carry part features -- the caller must supply them
  // (material_iso_group + stock at minimum) for a feature-driven traveler.
  if (!params.material_iso_group || !params.stock) {
    return { ...bridgeResult, traveler_note: "generate_traveler set but material_iso_group/stock not supplied; traveler skipped" };
  }
  try {
    const { travelerGenerationOrchestratorEngine } = await import("../../engines/TravelerGenerationOrchestratorEngine.js");
    const { jobChecklistEngine } = await import("../../engines/JobChecklistEngine.js");
    const traveler = travelerGenerationOrchestratorEngine.generate({
      job_id: order.id,
      part_number: bridgeResult?.bridge?.part_number ?? order.part_number ?? params.part_number ?? order.id,
      customer: order.customer ?? params.customer,
      material_iso_group: params.material_iso_group,
      material_name: params.material_name,
      features: Array.isArray(params.features) ? params.features : [],
      stock: params.stock,
      batch_size: params.batch_size ?? order.quantity,
      quoted_finish: params.quoted_finish,
      has_proven_program: params.has_proven_program,
      stock_precut: params.stock_precut,
    });
    jobChecklistEngine.attachChecklists(traveler.job_id, traveler.steps);
    return { ...bridgeResult, traveler };
  } catch (e) {
    return { ...bridgeResult, traveler_error: (e as Error).message };
  }
}
    ...
    async ({
      ...
      try {
        ...
        switch (action) {
          ...
          case "operations_kpis": {
            const engine = await getEngine("jobLifecycle");
            result = engine.operationsKpis();
            break;
          }
          ...
          case "timecard_audit_log": {
            const engine = await getEngine("timeClock");
            const p = params as any;
            result = engine.timecardAuditLog({
              employee_id: p.employee_id ?? p.employeeId,
              job_id: p.job_id ?? p.jobId,
              entity_type: p.entity_type ?? p.entityType,
              action: p.action_filter ?? p.actionFilter,
              since: p.since,
              limit: p.limit,
            });
            break;
          }
          case "a3_report_create": {
            const { a3ReportEngine } = await import("../../engines/A3ReportEngine.js");
            result = a3ReportEngine.create(params as any);
            break;
          }
          case "a3_report_list": {
            const { a3ReportEngine } = await import("../../engines/A3ReportEngine.js");
            const p = params as any;
            result = a3ReportEngine.list({
              status: p.status,
              owner_employee_id: p.owner_employee_id ?? p.ownerEmployeeId,
              job_id: p.job_id ?? p.jobId,
              limit: p.limit,
            });
            break;
          ...
          case "a3_report_get": {
            const { a3ReportEngine } = await import("../../engines/A3ReportEngine.js");
            result = a3ReportEngine.get((params as any).id);
            break;
          }
          ...
          // ── Process Routing (QUOTE-GROUNDING-MS0/U1) ──
          case "quote_route_operations": {
            const { processRoutingEngine } = await import("../../engines/ProcessRoutingEngine.js");
            result = processRoutingEngine.route(params as any);
            break;
          }
        ...
        // ── ERP Autofeed: project a completed pipeline result into the
        //    ERP/department/front-office/management field-map + employee-portal
        //    task fan-out. READ-ONLY (no writes). If a `result` is supplied it
        //    is projected directly; otherwise the pipeline is run first. ──
        case "quote_to_ship_erp_autofeed": {
          const p = params as Record<string, any>;
          let pipelineResult = p.result;
          if (pipelineResult == null) {
            const { quoteToShipOrchestratorEngine: qtsAuto } = await import("../../engines/QuoteToShipOrchestratorEngine.js");
            pipelineResult = await qtsAuto.runFullPipeline(p as any);
          }
          const { erpAutofeedProjectionEngine } = await import("../../engines/ErpAutofeedProjectionEngine.js");
          result = erpAutofeedProjectionEngine.project({ ...p, result: pipelineResult });
          break;
        }

        // ── ERP Commit: project THEN write the ERP rows (work order, employee
        //    portal tasks, PO stubs). Crosses into hotel-owned engines with
        //    their role-gating preserved. `dry_run:true` projects without
        //    writing (so the read-only autofeed path never mutates). ──
        case "quote_to_ship_erp_commit": {
          const p = params as Record<string, any>;
          let pipelineResult = p.result;
          if (pipelineResult == null) {
            const { quoteToShipOrchestratorEngine: qtsCommit } = await import("../../engines/QuoteToShipOrchestratorEngine.js");
            pipelineResult = await qtsCommit.runFullPipeline(p as any);
          }
          const { erpAutofeedProjectionEngine: proj } = await import("../../engines/ErpAutofeedProjectionEngine.js");
          const payload = proj.project({ ...p, result: pipelineResult });
          const { erpAutofeedWriterEngine } = await import("../../engines/ErpAutofeedWriterEngine.js");
          result = await erpAutofeedWriterEngine.commit({
            payload,
        ...
        // ── Job Traveler (Session 6-7) ──
          ...
          case "traveler_scan": {
            const { jobTravelerEngine } = await import("../../engines/JobTravelerEngine.js");
            // FIX: scan() takes a ScanInput object {code, operator_id, action}, not a
            // bare code string. Passing (params as any).code dropped operator_id +
            // action so every scanned transition threw "operator_id is required".
            result = jobTravelerEngine.scan(params as any);
            break;
          }

          // ── Auto-generated print→shipping traveler + per-dept checklists ──
          case "traveler_generate": {
            const { travelerGenerationOrchestratorEngine } = await import("../../engines/TravelerGenerationOrchestratorEngine.js");
            const { jobChecklistEngine } = await import("../../engines/JobChecklistEngine.js");
            const traveler = travelerGenerationOrchestratorEngine.generate(params as any);
            // Seed the stateful per-employee checklist store from the generated steps.
            jobChecklistEngine.attachChecklists(traveler.job_id, traveler.steps);
            result = traveler;
            break;
          }
          case "traveler_checklist_get": {
            const { jobChecklistEngine } = await import("../../engines/JobChecklistEngine.js");
            result = jobChecklistEngine.getJobChecklist((params as any).job_id);
            break;
          }
          case "traveler_checklist_check": {
            const { jobChecklistEngine } = await import("../../engines/JobChecklistEngine.js");
            result = jobChecklistEngine.checkItem(params as any);
            break;
          }
          case "traveler_checklist_uncheck": {
            const { jobChecklistEngine } = await import("../../engines/JobChecklistEngine.js");
            result = jobChecklistEngine.uncheckItem(params as any);
            break;
          }
          case "traveler_checklist_step_complete": {
            const { jobChecklistEngine } = await import("../../engines/JobChecklistEngine.js");
            result = {
              job_id: (params as any).job_id,
              step_seq: (params as any).step_seq,
              complete: jobChecklistEngine.stepComplete((params as any).job_id, (params as any).step_seq),
            };
            break;
          }
          ...
          case "oee_trend": {
            ...
            // An operator/integration posting {samples:[...]} (array of daily OEEInput) gets the real series.
            result = oeeCalculatorEngine.trend((params.samples ?? params.days_samples ?? []) as any);
            break;
          }
          ...
          case "quote_to_order": {
            ...
            // OPTIONAL producer link: auto-generate the print->shipping traveler +
            // per-dept checklists when the caller opts in with a part spec. Additive
            // -- existing callers (no generate_traveler flag) are unaffected.
            result = await maybeAttachTraveler(result, params);
            break;
          }
          ...
          case "tool_recondition_quality": {
            const { reconditioningQualityEngine } = await import("../../engines/ReconditioningQualityEngine.js");
            type Input = import("../../engines/ReconditioningQualityEngine.js").ReconditioningInput;
            result = reconditioningQualityEngine.assess(params as Input);
            break;
          }
          // ── Critical Path Method scheduling (G5 — hotel iter16) ──
          case "cpm_schedule": {
          ...
          case "margin_trends": {
            const engine = await getEngine("generalLedger");
            const p = params as any;
            result = engine.marginTrends({ months: p.months, asOf: p.as_of ?? p.asOf, periods: p.periods });
            break;
          }
          case "cash_flow_summary": {
            const engine = await getEngine("generalLedger");
            const p = params as any;
            result = engine.cashFlowSummary({ months: p.months, asOf: p.as_of ?? p.asOf, periods: p.periods });
            break;
          }
          case "revenue_forecast": {
            const engine = await getEngine("generalLedger");
            const p = params as any;
            result = engine.revenueForecast({
              lookbackMonths: p.lookback_months ?? p.lookbackMonths,
              horizonMonths: p.horizon_months ?? p.horizonMonths,
              asOf: p.as_of ?? p.asOf,
            });
            break;
          }
          ...
          case "rfq_assign": {
            const { rfqToOrderOrchestratorEngine } = await import("../../engines/RFQToOrderOrchestratorEngine.js");
            const p = params as { record_id?: string; rfq_id?: string; assignee_id: string; assignee_name?: string };
            result = {
              success: true,
              data: rfqToOrderOrchestratorEngine.assign({
                record_id: p.record_id ?? p.rfq_id ?? "",
          ...
          case "rfq_update_status": {
            const { rfqToOrderOrchestratorEngine } = await import("../../engines/RFQToOrderOrchestratorEngine.js");
            const p = params as { record_id?: string; rfq_id?: string; status?: string; inbox_status?: string };
            result = {
              success: true,
              data: rfqToOrderOrchestratorEngine.updateInboxStatus({
                record_id: p.record_id ?? p.rfq_id ?? "",
                inbox_status: (p.inbox_status ?? p.status) as
          ...
          case "root_cause_list": {
            // Root causes are stored 8D-style on non-conformances (recordRootCause -> nc.d4_root_cause).
            // Surface ONLY the NCs that have a root cause recorded (distinct from nc_list, which lists ALL
            // NCs incl. open/contained ones with no root cause yet). Optional status/severity/source filter.
            const { nonConformanceAndCorrectiveActionEngine } = await import("../../engines/NonConformanceAndCorrectiveActionEngine.js");
            const p = params as any;
            const ncs = nonConformanceAndCorrectiveActionEngine.listNCs({ status: p.status, severity: p.severity, source: p.source });
            const rootCauses = ncs
              .filter((nc: any) => nc.d4_root_cause)
              .map((nc: any) => ({
                ncr_id: nc.id,
                root_cause: nc.d4_root_cause,
                recorded_at: nc.d4_root_cause_at,
                status: nc.status,
                severity: nc.severity,
                source: nc.source,
                description: nc.description,
              }));
            result = { success: true, data: { count: rootCauses.length, root_causes: rootCauses } };
            break;
          }
          case "nc_get": {
            const { nonConformanceAndCorrectiveActionEngine } = await import("../../engines/NonConformanceAndCorrectiveActionEngine.js");
          ...
          case "osha_incidents": {
            ...
            result = rows.map((r) => ({
              ...
              //     the human-readable `status` first.
              // So emit BOTH as strings: `recordable` "true"/"false" (drives the metric) + `status`
              // "recordable"/"non-recordable" (drives the card label). Overriding the spread boolean is safe
              // here -- the result is an inline object literal, not typed OSHAIncident.
              recordable: r.recordable ? "true" : "false",
          ...
          case "osha_300_log": {
            const { oshaComplianceEngine } = await import("../../engines/OSHAComplianceEngine.js");
            const p = params as any;
            result = oshaComplianceEngine.generateOSHA300Log(Number(p.year) || new Date().getFullYear());
            break;
          }
          ...
          case "osha_near_miss": {
            const { oshaComplianceEngine } = await import("../../engines/OSHAComplianceEngine.js");
            const p = params as any;
            // A near miss is an OSHA incident with injury_type "near_miss": no medical treatment, no lost/
            // restricted days -> createIncident computes recordable=false (correct -- a near miss is not a
            // recordable injury). Fill the required-but-FE-omitted fields with safe defaults.
            const created = oshaComplianceEngine.createIncident({
              incident_date: p.incident_date ?? new Date().toISOString().slice(0, 10),
              employee_id: p.employee_id,
              employee_name: p.employee_name,
              location: p.location ?? "Shop floor",
              ...
              reported_by: p.reported_by ?? p.employee_name ?? "Unassigned",
          ...
          case "safety_training_list_all": {
            ...
            result = {
              records: records.map((r) => ({
                ...r,
                employee_name: empName(r.employee_id) ?? r.employee_id,
              })),
            };
            break;
          }
