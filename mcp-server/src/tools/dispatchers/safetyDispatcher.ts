...
// U-GOLF-WHISKEY-WORKTREE-INTEGRATION (2026-07-01) -- dual-wire of the lathe LoRA
// uncertainty calibration gate (also on prism_turning). Lives on prism_safety because
// its reject/review verdict consumes S(x) + hard hazard flags (toolBreakageRisk /
// collisionRisk) -- an uncertainty gate over safety-relevant inference is operator-
// in-the-loop territory. Recovered from slot/whiskey (test shipped there aspirationally;
// the wiring itself never landed -- completed here per R15 wire-to-all-consumers).
const LATHE_LORA_CALIBRATION_ACTIONS = new Set(["lathe_lora_calibration_gate"]);
...
const ALL_ACTIONS = [
  ...
  ...LATHE_LORA_CALIBRATION_ACTIONS,
  ...
  server.tool(
    ...
    async ({ action, params = {} }: { action: string; params: Record<string, any> }) => {
      try {
        ...
        if (COLLISION_ACTIONS.has(action)) {
        ...
        } else if (LATHE_LORA_CALIBRATION_ACTIONS.has(action)) {
          // U-GOLF-WHISKEY-WORKTREE-INTEGRATION -- LoRA-inference uncertainty calibration
          // gate (accept / review / reject verdict; S(x) + hard hazard flags force reject).
          const { latheLoRAUncertaintyQuantifierEngine: uq } = await import("../../engines/LatheLoRAUncertaintyQuantifierEngine.js");
          const uqParams = (params as Record<string, unknown>) ?? {};
          result = {
            success: true,
            data: uq.assess({
              modelConfidence: typeof uqParams.modelConfidence === "number" ? uqParams.modelConfidence : 0.5,
              ...uqParams,
            }),
          };
          ...
          const p = params as any;
          // SAFETY FIX (U-LW-STUB-SAFETY): the engine's real method is gate() -- the prior
          // stub guessed run/evaluate/check (none exist) so this stock-boundary/collision
          // gate ALWAYS returned {note:"method not callable"} success:true = a silent
          // false-pass (the gate NEVER fired). Route to the real gate().
          result = (stockBoundaryGateEngine as any).gate(p);
          ...
          // SECURITY (U-LW-C2, whiskey 2026-07-05): strip any caller-supplied `now` before
          // every FSM mutation. This engine is a SHARED singleton also wired into prism_turning;
          // honoring `now` here would let a caller back-date quarantine_started_at, then clear the
          // 5-day hold early via prism_turning's wet_run_clear_quarantine (real clock). The FSM
          // must always use the real server clock on every dispatcher surface. `.passthrough()`
          // keeps extra keys, so the strip (not the schema) is the load-bearing guard.
          const { now: _wetIgnoredNow, ...wetParams } = params as Record<string, any>;
          if (action === "wetrun_start_session") {
            result = wetRunStateMachineEngine.startSession(wetParams as unknown as Parameters<typeof wetRunStateMachineEngine.startSession>[0]);
          } else if (action === "wetrun_record_part") {
            result = wetRunStateMachineEngine.recordPartOutcome(wetParams as unknown as Parameters<typeof wetRunStateMachineEngine.recordPartOutcome>[0]);
          } else if (action === "wetrun_record_safety_event") {
            result = wetRunStateMachineEngine.recordSafetyEvent(wetParams as unknown as Parameters<typeof wetRunStateMachineEngine.recordSafetyEvent>[0]);
