...
const DOCS_DIR = path.join(import.meta.dirname, "../../data/docs");
...
const ACTIONS = ["list", "read", "write", "append", "roadmap_status", "action_tracker", "migrate", "print_ingest", "print_search", "print_get", "print_link_part", "print_revisions", "print_by_customer", "print_stats"] as const;
...
export function registerDocumentDispatcher(server: any): void {
  server.tool(
    "prism_doc",
    `Document management dispatcher. Actions: list, read, write, append, roadmap_status, action_tracker, migrate, print_ingest, print_search, print_get, print_link_part, print_revisions, print_by_customer, print_stats.
Params: read/write/append need 'name'. write needs 'content'. print_ingest needs 'filename' + 'file_path'. print_search accepts 'query', 'part_number', 'customer'. print_get needs 'print_id'. print_link_part needs 'print_id' + 'part_id'. print_revisions needs 'part_number'. print_by_customer needs 'customer'.`,
    ...
    async ({ action, params: rawParams = {} }: { action: typeof ACTIONS[number]; params?: Record<string, any> }) => {
      ...
      try {
        ...
        switch (action) {
          ...
          // ── Print Library Actions ─────────────────────────────────────
          case "print_ingest": {
            const { printLibraryEngine } = await import("../../engines/PrintLibraryEngine.js");
            result = printLibraryEngine.ingest({
              filename: String(params.filename ?? ""),
              file_path: String(params.file_path ?? ""),
              format: params.format as any,
              title_block: params.title_block as any,
              customer: params.customer as string | undefined,
              tags: params.tags as string[] | undefined,
              notes: params.notes as string | undefined,
              file_size_bytes: params.file_size_bytes as number | undefined,
            });
            break;
          }
          case "print_search": {
            const { printLibraryEngine } = await import("../../engines/PrintLibraryEngine.js");
            result = printLibraryEngine.search({
              query: params.query as string | undefined,
              part_number: params.part_number as string | undefined,
              customer: params.customer as string | undefined,
          ...
          case "print_get": {
            const { printLibraryEngine } = await import("../../engines/PrintLibraryEngine.js");
            const printId = String(params.print_id ?? params.id ?? "");
            result = printLibraryEngine.get(printId) ?? { error: `Print not found: ${printId}` };
            break;
          }
          case "print_link_part": {
            const { printLibraryEngine } = await import("../../engines/PrintLibraryEngine.js");
            result = printLibraryEngine.linkToPart(
              String(params.print_id ?? ""),
              String(params.part_id ?? ""),
            );
            break;
          }
          case "print_revisions": {
            const { printLibraryEngine } = await import("../../engines/PrintLibraryEngine.js");
            result = printLibraryEngine.getRevisionHistory(String(params.part_number ?? ""));
            break;
          }
          case "print_by_customer": {
            const { printLibraryEngine } = await import("../../engines/PrintLibraryEngine.js");
            result = printLibraryEngine.getByCustomer(String(params.customer ?? ""));
            break;
          }
          case "print_stats": {
            const { printLibraryEngine } = await import("../../engines/PrintLibraryEngine.js");
            result = printLibraryEngine.getStats();
            break;
          }
  ...
  log.info("✅ Registered: prism_doc dispatcher (14 actions)");
}
