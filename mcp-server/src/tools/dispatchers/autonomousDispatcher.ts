...
import { ACTION_AUTONOMOUS_SCHEMAS } from "../../schemas/autonomousActionSchemas.js";
...
Actions: ${ACTIONS.join(", ")}`,
    ...
    async ({ action, params: rawParams = {} }: { action: typeof ACTIONS[number]; params: Record<string, any> }) => {
      ...
      const validation = validateActionParams(action, params, ACTION_AUTONOMOUS_SCHEMAS);
      if (!validation.valid) {
        return dispatcherError(`Invalid params for '${action}': ${validation.errorMessage}`, action, "prism_autonomous");
      }
      try {
        switch (action) {
