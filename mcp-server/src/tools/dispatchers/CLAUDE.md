...
## Dispatchers — live counts in PRISM-INVENTORY-LATEST.md / BASELINE_INVENTORY.json
...
- All dispatcher result objects must include schema metadata
- Use `src/schemas/` Zod schemas for input/output validation
- Schema changes require version bump in `schemaVersion` field
- Breaking changes require migration path in `src/migrations/`
- Document schema versions in action-level JSDoc comments

## Distributed Locks for Orchestration
- Orchestration dispatchers (`prism_orchestrate`, `prism_autonomous`, `prism_autopilot_d`) require distributed locks
- Acquire lock via `DistributedLockManager.acquire(resourceId)` before state mutations
- Use `withLock(resourceId, async () => { ... })` for automatic release
- Lock timeout: 30s default — increase for long-running orchestration tasks
- Multi-agent coordination: check lock ownership before cross-dispatcher state access
