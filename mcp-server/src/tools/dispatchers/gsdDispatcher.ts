...
function getSystemCounts(): Record<string, number> {
  ...
  try {
    ...
    const dispatchers = fs.existsSync(dispDir)
      ? fs.readdirSync(dispDir).filter(f => f.endsWith("Dispatcher.ts")).length : 83;
    const engines = fs.existsSync(engDir)
      ? fs.readdirSync(engDir).filter(f => f.endsWith(".ts") && !f.startsWith("index")).length : 1565;
    const hooks = fs.existsSync(hookDir)
      ? fs.readdirSync(hookDir).filter(f => f.endsWith(".ts") && !f.startsWith("index")).length : 35;
    const counts = { dispatchers, engines, hooks };
    _countsCache = { ts: now, counts };
    return counts;
  } catch {
    return { dispatchers: 83, engines: 1565, hooks: 35 };
  }
}
...
const FALLBACK_QUICK = "# PRISM Quick Reference v23.2\n## 83 dispatchers | 3310+ verified actions | 1565 engines | 35 hooks\n## GSD files not found — edit data/docs/gsd/GSD_QUICK.md\n## START: prism_dev session_boot → prism_context todo_update";
  ...
  server.tool(
    ...
    async ({ action, params: rawParams = {} }: { action: string; params: Record<string, any> }) => {
      ...
      try {
        ...
        // Include schema version metadata per SCHEMA_VERSIONING_PROTOCOL
        const response = { ...slimResponse(result), _meta: getVersionMetadata() };
