...
const ACTIONS = [
  ...
  "prediction_validate", "calibration_run", "benchmark_run", "benchmark_list",
  "uncertainty_quantify", "improvement_run",
  "qt_list_cases", "qt_parse_golden", "qt_compare", "qt_summary",
] as const;
...
Params vary by action - see individual action docs.`,
    ...
    async ({ action, params: rawParams = {} }: { action: typeof ACTIONS[number]; params?: Record<string, any> }) => {
      ...
      try {
        ...
        switch (action) {
          ...
          case "qt_list_cases": {
            const { QTValidationSuiteEngine: QT } = await import("../../engines/QTValidationSuiteEngine.js");
            result = QT.getTestCases();
            break;
          }
          case "qt_parse_golden": {
            const { QTValidationSuiteEngine: QT } = await import("../../engines/QTValidationSuiteEngine.js");
            const caseId = params.case_id || params.caseId;
            const content = params.content || params.program;
            if (!caseId || !content) {
              return dispatcherError("qt_parse_golden requires case_id and content", action, "prism_validate");
            }
            result = QT.parseGoldenStandard(caseId, content);
            break;
          }
          case "qt_compare": {
            const { QTValidationSuiteEngine: QT } = await import("../../engines/QTValidationSuiteEngine.js");
            if (!params.golden || !params.candidate) {
              return dispatcherError("qt_compare requires golden and candidate objects", action, "prism_validate");
            }
            result = QT.comparePrograms(params.golden, params.candidate);
            break;
          }
          case "qt_summary": {
            const { QTValidationSuiteEngine: QT } = await import("../../engines/QTValidationSuiteEngine.js");
            result = QT.getSummary();
            break;
          }
  ...
  log.info("Registered: prism_validate dispatcher (17 actions)");
}
