...
// -- Diagnostic Reasoning (LLM-INTEL) --
const DIAGNOSTIC_REASONING_ACTIONS = [
  "diag_fault_tree", "diag_symptom_match", "diag_health_assess",
  "diag_fmea", "diag_alarm_interpret", "diag_stats",
] as const;
...
function diagnosisExtractKeyValues(action: string, result: any): Record<string, any> {
  ...
  switch (action) {
    ...
    case "diag_fault_tree":
      return { root_event: result.root_event, depth: result.depth, gates: result.gates?.length };
    case "diag_symptom_match":
      return { primary: result.primary, confidence: result.confidence, alternatives: result.alternatives?.length };
    case "diag_health_assess":
      return { health_score: result.overall_health, risk_level: result.risk_level };
    case "diag_fmea":
      return { component: result.component, modes: result.failure_modes?.length, rpn_max: result.max_rpn };
    case "diag_alarm_interpret":
      return { alarm_code: result.alarm_code, severity: result.severity, actions: result.recommended_actions?.length };
    case "diag_stats":
      return { engine: result.engine, capabilities: result.capabilities?.length };
    default:
      return result;
  }
}
  ...
  server.tool(
    ...
    async ({ action, params: rawParams = {} }: { action: string; params?: Record<string, any> }) => {
      ...
      try {
        ...
        } else if ((DIAGNOSTIC_REASONING_ACTIONS as readonly string[]).includes(action)) {
          // Diagnostic Reasoning (LLM-INTEL)
          const { DiagnosticReasoningEngine } = await import("../../engines/DiagnosticReasoningEngine.js");
          switch (action) {
            case "diag_fault_tree": {
              if (!params.symptoms) {
                result = { error: "Missing required param: symptoms (array)" };
                break;
              }
              result = DiagnosticReasoningEngine.buildFaultTree(
                params.symptoms,
                params.context
              );
              break;
            ...
            case "diag_symptom_match": {
              ...
              const diagnosis = DiagnosticReasoningEngine.diagnoseFromSymptoms(
                params.symptoms,
                params.context
              );
              result = {
                ...
                alternatives: diagnosis.differential_diagnoses.slice(0, 3),
            ...
            case "diag_health_assess": {
              if (!params.machine_id) {
                result = { error: "Missing required param: machine_id" };
                break;
              }
              result = DiagnosticReasoningEngine.assessMachineHealth(
                params.machine_id,
                params.sensor_data || {},
                params.maintenance_history || []
              );
              break;
            }
            case "diag_fmea": {
              if (!params.component || !params.failure_modes) {
                result = { error: "Missing required params: component, failure_modes" };
                break;
              }
              result = DiagnosticReasoningEngine.generateFMEA(
                params.component,
            ...
            case "diag_alarm_interpret": {
              if (!params.alarm_code || !params.machine_type) {
                result = { error: "Missing required params: alarm_code, machine_type" };
                break;
              }
              result = DiagnosticReasoningEngine.interpretAlarm(
                params.alarm_code,
                params.machine_type,
                params.context
              );
            ...
            case "diag_stats": {
              result = {
                ...
                capabilities: [
                  "fault tree analysis", "symptom-based diagnosis", "machine health assessment",
                  "FMEA generation", "alarm interpretation", "differential diagnosis"
                ]
              };
              break;
            }
        ...
        if (pressure > 50) {
          const keyValues = diagnosisExtractKeyValues(action, result);
          return {
