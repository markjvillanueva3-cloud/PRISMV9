...
let _rest: any, _seq: any, _trans: any, _refine: any, _setup: any, _workflow: any;
async function getEngine(name: string): Promise<any> {
  switch (name) {
    ...
    case "workflow": return _workflow ??= (await import("../../engines/WorkflowTemplateEngine.js")).workflowTemplateEngine;
...
const ACTIONS = [
  ...
  "adaptive_refine",
  "multi_setup_plan",
  "workflow_suggest", "workflow_gap_analysis", "workflow_list_templates",
] as const;
...
Params vary by action — pass relevant fields in params object.`,
      ...
      try {
        ...
        switch (action) {
          ...
          case "workflow_suggest": {
            const eng = await getEngine("workflow");
            result = eng.suggestSequence(params);
            break;
          }
          case "workflow_gap_analysis": {
            const eng = await getEngine("workflow");
            result = eng.analyzeGaps(params.process_type, params.provided_operations || []);
            break;
          }
          case "workflow_list_templates": {
            const eng = await getEngine("workflow");
            if (params.process_type) {
              result = eng.getTemplatesForProcess(params.process_type);
            } else {
              result = {
                process_types: eng.getAvailableProcessTypes(),
                order_of_operations: eng.getOrderOfOperations(),
              };
            }
            break;
          }
  ...
  log.info("Registered: prism_multi_op dispatcher (10 actions)");
}
