...
import { dispatcherError, validateActionParams, getSchemaCoverageStats, resetSchemaCoverageStats } from "../../utils/dispatcherMiddleware.js";
...
"engine_acc_degrading", "engine_acc_list", "engine_acc_stats", "engine_acc_record",
...
"output_truncate", "output_truncate_json", "output_truncate_savings", "output_truncate_auto",
  ...
  // U-GAC03: CodeGraphProjectionEngine (GRAPH-AS-LLM-CONTEXT-MS0, slot:sierra 2026-06-15)
  "code_graph_project",
  // U-GAC06: CommunitySummaryEngine (GRAPH-AS-LLM-CONTEXT-MS0, slot:sierra 2026-06-15)
  "community_summary",
  ...
  "formal_satisfy",
  "formal_ready",
  // U-WIRE-ERP-IMPORT / WIRE-UNWIRED-PAPA: ERPImportEngine work-order import surfaces (6 actions, slot:papa 2026-06-14).
  "erp_import_work_order",
  "erp_import_batch",
  "erp_get_work_order",
  "erp_list_work_orders",
  "erp_validate_work_order",
  "erp_field_mappings",
  // U-WIRE-COHORTSHIM / WIRE-UNWIRED-PAPA: CohortBridgeShimEngine NodeNext/shape-coerce shim primitives (4 actions; applyNodeNextSuffix/rewriteSourceImports/buildShapeCoerceShim pure, recommend reads COHORT-COMPAT-MATRIX). slot:papa 2026-06-15.
  "cohort_shim_nodenext_suffix",
  "cohort_shim_rewrite_imports",
  "cohort_shim_build_shape_coerce",
  "cohort_shim_recommend_bridges",
  // U-WIRE-HZPAUDIT / WIRE-UNWIRED-PAPA: HzpDashAuditEngine audit-envelope builder (3 actions; pure static; galaxy:golf wired into prism_dev). slot:papa->golf 2026-06-15.
  "hzp_audit_build",
  "hzp_audit_to_jsonl",
  "hzp_audit_render_line",
  // U-WIRE-PROGPARSE / WIRE-UNWIRED-PAPA: UnifiedProgramParserEngine.parseContent (pure NC/G-code parse; parseFile/parseArchive are fs-bound, excluded). galaxy:echo/india wired into prism_dev. slot:papa 2026-06-15.
  "program_parse_content",
  // U-WIRE-MILLCORPUS / WIRE-UNWIRED-PAPA: MillProgramCorpusEngine canonical-corpus stats (read-only; corpus_build/persist are fs-write, excluded; caller corpus_path NOT forwarded = no path traversal). galaxy:foxtrot/juliett -> prism_dev. slot:papa 2026-06-15.
  "mill_corpus_stats",
  // U-WIRE-D2F / WIRE-UNWIRED-PAPA: DesignToFloorPipelineEngine pre-flight + read-state (runPreFlightAnalysis physics-validates g-code in-memory; record*/submit* mutators + getSPCDashboard/getJobHistory deferred). galaxy:foxtrot/golf -> prism_dev. slot:papa 2026-06-15.
  "d2f_preflight",
  "d2f_job_count",
  "d2f_calibration_state",
  // U-WIRE-PACT / WIRE-UNWIRED-PAPA: PactContractTestEngine consumer-driven contract testing (3 actions; pure static compute, plain-object I/O; no HTTP/broker). galaxy:dev -> prism_dev. slot:papa 2026-06-15.
  "pact_define_contract",
  "pact_verify_interaction",
  "pact_check_backward_compat",
  // U-WIRE-ACQUISITION / WIRE-UNWIRED-PAPA: AcquisitionRecommendationEngine procurement recommender (6 read/compute actions; in-mem catalog, machine-binding-gated recommend/best). galaxy:hotel/business -> prism_dev. slot:papa 2026-06-15.
  ...
  "acquisition_compare",
  "acquisition_stats",
  // U-WIRE-MEASURE / WIRE-UNWIRED-PAPA: MeasureSummaryEngine quality-measurement aggregation (7 actions; in-mem store, static class methods). galaxy:quality -> prism_dev. slot:papa 2026-06-15.
  "measure_add",
  "measure_generate_summary",
  "measure_get_summary",
  "measure_list_summaries",
  "measure_quality_trend",
  "measure_parts_with_issues",
  "measure_export",
  ...
  "regression_baseline",
  // AI-SYNERGY/U-INDIA-AUTO-STORAGE-CONTRACT (slot:india 2026-06-30): pure
  // discoverability read over the frozen STORAGE_ROUTES registry in
  // scripts/lib/auto-storage-contract.mjs. No mutation, no pipeline execution.
  // Optional dataType filter returns one matching route; omit for full registry.
  "storage_route_map",
  // -- MATH-APPLICATIONS (category theory): naturality/commutation validator --
  "commutative_diagram_validate",
] as const;
    ...
    async ({ action, params: rawParams = {} }: { action: string; params: Record<string, any> }) => {
      ...
      try {
        switch (action) {
          ...
          case "commutative_diagram_validate": {
            // Category-theory naturality/commutation check over a JSON field-rewrite DSL (no eval,
            // wire-safe). Catches silent-divergence translator bugs (the G170/G168 dialect class).
            const { commutativetranslatorEngine } = await import("../../engines/CommutativeTranslatorValidatorEngine.js");
            result = commutativetranslatorEngine.checkFieldRewriteCommutation(params as any);
            break;
          }
          case "code_search": {
            const dirs: string[] = [];
          ...
          case "code_graph_project": {
            const { codeGraphProjectionEngine } = await import("../../engines/CodeGraphProjectionEngine.js");
            const p = params as { target: string; center?: string; hops?: number; maxFiles?: number; maxFileBytes?: number };
            const graph = codeGraphProjectionEngine.project({ target: p.target, maxFiles: p.maxFiles, maxFileBytes: p.maxFileBytes });
            const data = p.center
              ? codeGraphProjectionEngine.egoGraph(graph, p.center, Number.isFinite(p.hops as number) ? (p.hops as number) : 1)
              : graph;
            result = { success: true, data };
            break;
          }
          // U-GAC06: CommunitySummaryEngine -- cluster the engine catalog by domain into
          // token-bounded community summaries (extractive default; useLlm opts into fail-soft Ollama).
          case "community_summary": {
            const { communitySummaryEngine } = await import("../../engines/CommunitySummaryEngine.js");
            const p = params as { domain?: string; useLlm?: boolean; maxTokens?: number; topK?: number };
            const opts = {
              useLlm: p.useLlm,
              maxTokens: p.maxTokens != null ? Number(p.maxTokens) : undefined,
              topK: p.topK != null ? Number(p.topK) : undefined,
            };
            const data = p.domain
              ? await communitySummaryEngine.summarizeDomain(p.domain, opts)
              : await communitySummaryEngine.summarizeAll(opts);
            result = { success: true, data };
            break;
          }
          ...
          // U-WIRE-ENGACC-RECORD (slot:india): the WRITE side. recordOutcome was
          // wired NOWHERE, so the 7 engine_acc_* READ actions always saw an empty
          // tracker -- the cross-engine meta-learning accuracy loop had no feedback
          // arrow. This action lets a caller log predicted-vs-actual so the reads
          // have real data. (Engine is NOT WIRE-EXEMPT -- it already has a full
          // dispatcher surface, so a dispatcher action is the correct closure.)
          case "engine_acc_record": {
            const { engineAccuracyTrackerEngine } = await import("../../engines/EngineAccuracyTrackerEngine.js");
            const ep = typeof params === "object" && params !== null ? params as Record<string, unknown> : {};
            const engineId = typeof ep.engine_id === "string" ? ep.engine_id : (typeof ep.engineId === "string" ? ep.engineId : "");
            const metricName = typeof ep.metric_name === "string" ? ep.metric_name : (typeof ep.metricName === "string" ? ep.metricName : "");
            const predicted = typeof ep.predicted === "number" ? ep.predicted : NaN;
            const actual = typeof ep.actual === "number" ? ep.actual : NaN;
            if (!engineId || !metricName || !Number.isFinite(predicted) || !Number.isFinite(actual)) {
              result = { error: "engine_acc_record requires 'engine_id', 'metric_name' (strings) and finite numeric 'predicted' + 'actual'" };
              break;
            }
            const unit = typeof ep.unit === "string" ? ep.unit : undefined;
            const context = typeof ep.context === "object" && ep.context !== null && !Array.isArray(ep.context)
              ? ep.context as Record<string, unknown>
              : undefined;
            const outcome = engineAccuracyTrackerEngine.recordOutcome(engineId, metricName, predicted, actual, unit, context);
            result = { recorded: true, outcome };
            break;
          }
          ...
          case "wpna_optimize_parameters": {
            ...
            const r = wedmProgramNeuralAnalysisEngine.optimizeParameters(
              wedmParams as unknown as Parameters<typeof wedmProgramNeuralAnalysisEngine.optimizeParameters>[0],
              material,
              thickness_mm,
            );
            result = {
          ...
          // U-DEV-SCHEMA-COVERAGE-WIRE (slot:bravo 2026-06-22): expose the RUNTIME dispatcher
          // schema-coverage counters from dispatcherMiddleware -- closes the
          // U-DISPATCHER-SCHEMA-FAILLOUD orphan (getSchemaCoverageStats had no MCP consumer).
          // Distinct from schema_coverage_audit_* above: that statically scans schema FILES;
          // this reports LIVE per-process validate-vs-passthrough traffic across every
          // dispatcher that shares the dispatcherMiddleware module singleton.
          case "dispatcher_schema_coverage_stats": {
            result = getSchemaCoverageStats();
            break;
          }
          case "dispatcher_schema_coverage_reset": {
            resetSchemaCoverageStats();
            result = { reset: true, ...getSchemaCoverageStats() };
            break;
          }
          ...
          case "token_ledger_most_expensive": {
            ...
            // mostExpensive() returns undefined on an empty ledger. Return a stable,
            // self-describing shape: `found:false` (slimResponse strips the null `entry`,
            // keeps the false boolean + 0) so callers never get a bare undefined.
          ...
          case "regression_baseline": {
            ...
            if (regressionBaselineEngine.listBaseline().length === 0 && fs.existsSync(snapPath)) {
              try { regressionBaselineEngine.loadSnapshot(JSON.parse(fs.readFileSync(snapPath, "utf-8"))); }
              catch { /* corrupt/absent snapshot -> start empty; freeze re-seeds */ }
            }
            const persist = () => safeWriteSync(snapPath, JSON.stringify(regressionBaselineEngine.toSnapshot(), null, 2));
            const mode = String(params.mode || "list");
            try {
              switch (mode) {
                case "freeze": {
                  const entries = Array.isArray(params.entries) ? params.entries : [];
                  const frozen = regressionBaselineEngine.freeze(entries, String(params.frozen_by ?? params.frozenBy ?? ""));
                  persist();
                  result = { success: true, data: { mode, frozen } };
                  break;
                }
                case "evaluate": {
                  const runs = Array.isArray(params.runs) ? params.runs : [];
                  const opts = (params.opts && typeof params.opts === "object") ? params.opts : {};
                  const report = regressionBaselineEngine.evaluate(runs, opts);
                  result = { success: true, data: { mode, report } };
                  break;
                }
                case "quarantine": {
                  const rec = regressionBaselineEngine.quarantineTest({
                    ...
                    expires_at: params.expires_at != null ? Number(params.expires_at) : undefined,
                    ticket_url: typeof params.ticket_url === "string" ? params.ticket_url : undefined,
                  });
                  persist();
                  result = { success: true, data: { mode, quarantined: rec } };
                  break;
                }
                case "lift": {
                  const lifted = regressionBaselineEngine.liftQuarantine(String(params.test_id ?? ""));
                  persist();
                  result = { success: true, data: { mode, lifted } };
                  break;
                }
                case "list": {
                  result = { success: true, data: {
                    mode,
                    baseline: regressionBaselineEngine.listBaseline(),
                    quarantine: regressionBaselineEngine.listQuarantine(),
                  } };
                  break;
                }
                case "observed_p95": {
                  const test_id = String(params.test_id ?? "");
                  result = { success: true, data: { mode, test_id, observed_p95_ms: regressionBaselineEngine.observedP95(test_id) } };
                  break;
                }
                case "snapshot": {
                  result = { success: true, data: { mode, snapshot: regressionBaselineEngine.toSnapshot(), path: snapPath } };
                  break;
                }
                default:
                  result = { success: false, error: "invalid_mode", mode, allowed: ["freeze", "evaluate", "quarantine", "lift", "list", "observed_p95", "snapshot"] };
              }
            } catch (err) {
              result = { success: false, error: "regression_baseline_failed", mode, detail: err instanceof Error ? err.message : String(err) };
            }
            break;
          }
          ...
          case "gnn_deploy_status": {
            ...
            const status = gnnDeployStatusEngine.readStatus({ evalPath, nowMs: Date.now() });
          ...
          // U-WIRE-ERP-IMPORT / WIRE-UNWIRED-PAPA: ERPImportEngine (static methods; in-memory work-order store). slot:papa 2026-06-14.
          case "erp_import_work_order": {
            const { ERPImportEngine } = await import("../../engines/ERPImportEngine.js");
            result = ERPImportEngine.importWorkOrder(params.workOrder as Parameters<typeof ERPImportEngine.importWorkOrder>[0]);
            break;
          }
          case "erp_import_batch": {
            const { ERPImportEngine } = await import("../../engines/ERPImportEngine.js");
            result = ERPImportEngine.importBatch(params.workOrders as Parameters<typeof ERPImportEngine.importBatch>[0]);
            break;
          }
          case "erp_get_work_order": {
            const { ERPImportEngine } = await import("../../engines/ERPImportEngine.js");
            const wo = ERPImportEngine.getImportedWorkOrder(
              params.erpSystem as Parameters<typeof ERPImportEngine.getImportedWorkOrder>[0],
              params.workOrderNumber as string,
            );
            result = { found: wo !== undefined, workOrder: wo ?? null };
            break;
          }
          case "erp_list_work_orders": {
            const { ERPImportEngine } = await import("../../engines/ERPImportEngine.js");
            const list = ERPImportEngine.listImportedWorkOrders(params.erpSystem as Parameters<typeof ERPImportEngine.listImportedWorkOrders>[0]);
            result = { count: list.length, workOrders: list };
            break;
          }
          case "erp_validate_work_order": {
            const { ERPImportEngine } = await import("../../engines/ERPImportEngine.js");
            result = ERPImportEngine.validateWorkOrder(params.workOrder);
            break;
          }
          case "erp_field_mappings": {
            const { ERPImportEngine } = await import("../../engines/ERPImportEngine.js");
            result = { mappings: ERPImportEngine.getFieldMappings(params.erpSystem as Parameters<typeof ERPImportEngine.getFieldMappings>[0]) };
            break;
          }
          // U-WIRE-COHORTSHIM / WIRE-UNWIRED-PAPA: CohortBridgeShimEngine shim primitives (slot:papa 2026-06-15).
          // applyNodeNextSuffix/rewriteSourceImports/buildShapeCoerceShim are pure; recommendShimsForTopBridges
          // reads COHORT-COMPAT-MATRIX.json and throws (fail-loud) on a missing matrix -> dispatcherError catch.
          case "cohort_shim_nodenext_suffix": {
            const { cohortBridgeShimEngine } = await import("../../engines/CohortBridgeShimEngine.js");
            result = cohortBridgeShimEngine.applyNodeNextSuffix(
              params.spec as string,
              { assumeDirectoryIndex: params.assumeDirectoryIndex as boolean | undefined },
            );
            break;
          }
          case "cohort_shim_rewrite_imports": {
            const { cohortBridgeShimEngine } = await import("../../engines/CohortBridgeShimEngine.js");
            result = cohortBridgeShimEngine.rewriteSourceImports(params.source as string);
            break;
          }
          case "cohort_shim_build_shape_coerce": {
            const { cohortBridgeShimEngine } = await import("../../engines/CohortBridgeShimEngine.js");
            result = cohortBridgeShimEngine.buildShapeCoerceShim(
              params.fromShape as string,
              params.toShape as string,
              params.methodMap as Record<string, string>,
            );
            break;
          }
          case "cohort_shim_recommend_bridges": {
            const { cohortBridgeShimEngine } = await import("../../engines/CohortBridgeShimEngine.js");
            result = cohortBridgeShimEngine.recommendShimsForTopBridges(
              params.topK as number | undefined,
              params.matrixPath as string | undefined,
            );
            break;
          }
          // U-WIRE-HZPAUDIT / WIRE-UNWIRED-PAPA: HzpDashAuditEngine audit-envelope builder (slot:papa->golf 2026-06-15).
          // build() re-validates via .parse and throws on a bad ts_iso -> dispatcherError (fail-loud).
          case "hzp_audit_build": {
            const { HzpDashAuditEngine } = await import("../../engines/HzpDashAuditEngine.js");
            result = HzpDashAuditEngine.build(
              params.req as Parameters<typeof HzpDashAuditEngine.build>[0],
              params.randHex6 as string | undefined,
            );
            break;
          }
          case "hzp_audit_to_jsonl": {
            const { HzpDashAuditEngine } = await import("../../engines/HzpDashAuditEngine.js");
            result = { jsonl: HzpDashAuditEngine.toJsonl(params.env as Parameters<typeof HzpDashAuditEngine.toJsonl>[0]) };
            break;
          }
          case "hzp_audit_render_line": {
            const { HzpDashAuditEngine } = await import("../../engines/HzpDashAuditEngine.js");
            result = { line: HzpDashAuditEngine.renderLine(params.env as Parameters<typeof HzpDashAuditEngine.renderLine>[0]) };
            break;
          }
          // U-WIRE-PROGPARSE / WIRE-UNWIRED-PAPA: UnifiedProgramParserEngine.parseContent -- pure NC/G-code
          // parse (string in -> ParsedProgram out; fail-soft: unsupported format -> warnings[], never throws).
          // slot:papa 2026-06-15.
          case "program_parse_content": {
            const { unifiedProgramParser } = await import("../../engines/UnifiedProgramParserEngine.js");
            // filePath is an optional dialect hint; coerce undefined -> "" so path.extname/basename never NPE.
            result = unifiedProgramParser.parseContent(params.content as string, (params.filePath as string | undefined) ?? "");
            break;
          }
          // U-WIRE-MILLCORPUS / WIRE-UNWIRED-PAPA: MillProgramCorpusEngine canonical-corpus stats. slot:papa 2026-06-15.
          case "mill_corpus_stats": {
            const { millProgramCorpusEngine } = await import("../../engines/MillProgramCorpusEngine.js");
            // canonical corpus only -- caller corpus_path intentionally NOT forwarded (no path traversal).
            result = millProgramCorpusEngine.calculate("corpus_stats", {});
            break;
          }
          // U-WIRE-D2F / WIRE-UNWIRED-PAPA: DesignToFloorPipelineEngine pre-flight + read-state. slot:papa 2026-06-15.
          case "d2f_preflight": {
          ...
          case "d2f_job_count": {
            const { designToFloorPipelineEngine } = await import("../../engines/DesignToFloorPipelineEngine.js");
            result = { job_count: designToFloorPipelineEngine.getJobCount() };
            break;
          }
          case "d2f_calibration_state": {
            const { designToFloorPipelineEngine } = await import("../../engines/DesignToFloorPipelineEngine.js");
            result = { calibration: designToFloorPipelineEngine.getCalibrationState(params.material as string | undefined) };
            break;
          }
          // U-WIRE-PACT / WIRE-UNWIRED-PAPA: PactContractTestEngine consumer-driven contract testing. slot:papa->dev 2026-06-15.
          // Static methods on the exported class; pure compute (no HTTP/broker). defineContract/verifyInteraction throw on bad input -> dispatcherError.
          case "pact_define_contract": {
            const { PactContractTestEngine } = await import("../../engines/PactContractTestEngine.js");
            result = PactContractTestEngine.defineContract(params as unknown as Parameters<typeof PactContractTestEngine.defineContract>[0]);
            break;
          }
          case "pact_verify_interaction": {
            const { PactContractTestEngine } = await import("../../engines/PactContractTestEngine.js");
            result = PactContractTestEngine.verifyInteraction(
              params.contract as unknown as Parameters<typeof PactContractTestEngine.verifyInteraction>[0],
              params.interactionId as string,
              params.actualResponse,
            );
            break;
          }
          case "pact_check_backward_compat": {
            const { PactContractTestEngine } = await import("../../engines/PactContractTestEngine.js");
            result = PactContractTestEngine.checkBackwardCompat(
              params.oldContract as unknown as Parameters<typeof PactContractTestEngine.checkBackwardCompat>[0],
              params.newContract as unknown as Parameters<typeof PactContractTestEngine.checkBackwardCompat>[1],
            );
            break;
          }
          // U-WIRE-ACQUISITION / WIRE-UNWIRED-PAPA: AcquisitionRecommendationEngine procurement recommender. slot:papa->dev 2026-06-15.
          // In-mem catalog reads + ROI math. recommend/best return null when machine_id has no binding (wrapped so the null is explicit, not a bare "null" body).
          case "acquisition_recommend": {
            const { acquisitionRecommendationEngine } = await import("../../engines/AcquisitionRecommendationEngine.js");
            result = { recommendations: acquisitionRecommendationEngine.getRecommendations(params as unknown as Parameters<typeof acquisitionRecommendationEngine.getRecommendations>[0]) };
            break;
          }
          case "acquisition_best": {
            const { acquisitionRecommendationEngine } = await import("../../engines/AcquisitionRecommendationEngine.js");
            result = { recommendation: acquisitionRecommendationEngine.getBestRecommendation(params as unknown as Parameters<typeof acquisitionRecommendationEngine.getBestRecommendation>[0]) };
            break;
          }
          case "acquisition_roi": {
            const { acquisitionRecommendationEngine } = await import("../../engines/AcquisitionRecommendationEngine.js");
            result = acquisitionRecommendationEngine.calculateROI(params as unknown as Parameters<typeof acquisitionRecommendationEngine.calculateROI>[0]);
            break;
          }
          case "acquisition_distributor": {
            const { acquisitionRecommendationEngine } = await import("../../engines/AcquisitionRecommendationEngine.js");
            result = { distributor: acquisitionRecommendationEngine.getDistributorInfo(params.item_id as string) };
            break;
          }
          case "acquisition_compare": {
            const { acquisitionRecommendationEngine } = await import("../../engines/AcquisitionRecommendationEngine.js");
            result = { comparison: acquisitionRecommendationEngine.compareItems(params.item_ids as string[]) };
            break;
          }
          case "acquisition_stats": {
            const { acquisitionRecommendationEngine } = await import("../../engines/AcquisitionRecommendationEngine.js");
            result = acquisitionRecommendationEngine.getStats();
            break;
          }
          // U-WIRE-MEASURE / WIRE-UNWIRED-PAPA: MeasureSummaryEngine quality-measurement aggregation. slot:papa->quality 2026-06-15.
          // Static methods on the exported class; in-mem store (process-lifetime). measure_add ingests; the rest read/aggregate/export.
          case "measure_add": {
            const { MeasureSummaryEngine } = await import("../../engines/MeasureSummaryEngine.js");
            MeasureSummaryEngine.addMeasurement(
              params.partNumber as Parameters<typeof MeasureSummaryEngine.addMeasurement>[0],
              params.source as Parameters<typeof MeasureSummaryEngine.addMeasurement>[1],
              params.featureName as Parameters<typeof MeasureSummaryEngine.addMeasurement>[2],
              params.passed as Parameters<typeof MeasureSummaryEngine.addMeasurement>[3],
              params.deviation as Parameters<typeof MeasureSummaryEngine.addMeasurement>[4],
              params.tolerance as Parameters<typeof MeasureSummaryEngine.addMeasurement>[5],
              params.cpk as Parameters<typeof MeasureSummaryEngine.addMeasurement>[6],
              params.serialNumber as Parameters<typeof MeasureSummaryEngine.addMeasurement>[7],
            );
            result = { added: true };
            break;
          }
          case "measure_generate_summary": {
            const { MeasureSummaryEngine } = await import("../../engines/MeasureSummaryEngine.js");
            result = MeasureSummaryEngine.generateSummary(params.partNumber as string, params.workOrderNumber as string | undefined, params.serialNumber as string | undefined);
            break;
          }
          case "measure_get_summary": {
            const { MeasureSummaryEngine } = await import("../../engines/MeasureSummaryEngine.js");
            const measureSummary = MeasureSummaryEngine.getSummary(params.id as string);
            result = { found: measureSummary !== undefined, summary: measureSummary ?? null };
            break;
          }
          case "measure_list_summaries": {
            const { MeasureSummaryEngine } = await import("../../engines/MeasureSummaryEngine.js");
            result = { summaries: MeasureSummaryEngine.listSummaries(params.partNumber as string) };
            break;
          }
          case "measure_quality_trend": {
            const { MeasureSummaryEngine } = await import("../../engines/MeasureSummaryEngine.js");
            result = MeasureSummaryEngine.getQualityTrend(params.partNumber as string, params.days as number | undefined);
            break;
          }
          case "measure_parts_with_issues": {
            const { MeasureSummaryEngine } = await import("../../engines/MeasureSummaryEngine.js");
            result = { parts: MeasureSummaryEngine.getPartsWithIssues() };
            break;
          }
          case "measure_export": {
            const { MeasureSummaryEngine } = await import("../../engines/MeasureSummaryEngine.js");
            const measureExport = MeasureSummaryEngine.exportSummary(params.id as string, params.format as "json" | "csv" | "text");
            result = { found: measureExport !== undefined, export: measureExport ?? null };
            break;
          }
          // AI-SYNERGY/U-INDIA-AUTO-STORAGE-CONTRACT (slot:india 2026-06-30)
          // Pure discoverability read -- no mutation, no pipeline execution.
          // The contract lives in scripts/lib/ (a .mjs file outside TS rootDir).
          // We resolve its path at runtime via PATHS.PRISM_ROOT so both src-run
          // (tsx) and dist-run (node dist/) find the same file on disk.
          case "storage_route_map": {
            const { StorageRouteMapInputSchema } = await import("../../schemas/devSchema.js");
            const parsed = StorageRouteMapInputSchema.safeParse(params);
            if (!parsed.success) {
              result = { success: false, error: "invalid_params", detail: parsed.error.message };
              break;
            }
            // pathToFileURL is required for Windows absolute paths in dynamic import()
            const { pathToFileURL } = await import("url");
            const contractPath = path.join(PROJECT_ROOT, "scripts", "lib", "auto-storage-contract.mjs");
            const contractUrl = pathToFileURL(contractPath).href;
            // Dynamic import via URL string -- TS treats the string as `any`
            // so no @ts-ignore is needed; the runtime resolves the .mjs module.
            const contractMod = await import(contractUrl) as {
              STORAGE_ROUTES: ReadonlyArray<Record<string, unknown>>;
              findRoute: (dt: string) => Record<string, unknown> | null;
              validateRoutes: (r?: unknown) => { ok: boolean; errors: string[] };
            };
            const { STORAGE_ROUTES, findRoute, validateRoutes } = contractMod;
            const { dataType } = parsed.data;
            if (dataType) {
              const route = findRoute(dataType);
              result = route
                ? { success: true, data: { route } }
                : { success: false, error: "not_found", dataType };
            } else {
              const validation = validateRoutes(STORAGE_ROUTES);
              result = {
                success: true,
                data: {
                  routes: Array.from(STORAGE_ROUTES),
                  count: STORAGE_ROUTES.length,
                  valid: validation.ok,
                },
              };
            }
