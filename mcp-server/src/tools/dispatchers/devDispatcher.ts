    ...
    async ({ action, params: rawParams = {} }: { action: string; params: Record<string, any> }) => {
      ...
      try {
        switch (action) {
          ...
          // ── P9-U02: AutoChain orchestrator (SI → AutoFix wiring) ──
          case "auto_chain_run": {
            const { autoChainOrchestratorEngine } = await import("../../engines/AutoChainOrchestratorEngine.js");
            const opts: { autoPromoteMinPriority?: number; dryRun?: boolean } = {};
            if (typeof params.auto_promote_min_priority === "number") {
              opts.autoPromoteMinPriority = params.auto_promote_min_priority;
            }
            if (typeof params.dry_run === "boolean") {
              opts.dryRun = params.dry_run;
            }
            result = autoChainOrchestratorEngine.run(opts);
            break;
          }
          case "auto_chain_read": {
            const { autoChainOrchestratorEngine: acRead } = await import("../../engines/AutoChainOrchestratorEngine.js");
            const cached = acRead.read();
            result = cached ?? { error: "No auto-chain run yet. Run auto_chain_run first." };
            break;
          }
          case "auto_chain_summary": {
            const { autoChainOrchestratorEngine: acSum } = await import("../../engines/AutoChainOrchestratorEngine.js");
            result = { summary: acSum.summary() };
            break;
          }
          ...
          // ── INTEL-OLLAMA-OBSIDIAN-MS0/P4-U02: shared-directive summarizer ──
          case "directive_summarize_build": {
            const { directiveSummarizerEngine } = await import("../../engines/DirectiveSummarizerEngine.js");
            const useOllama = params.use_ollama === true;
            const model = typeof params.model === "string" ? params.model : undefined;
            const built = await directiveSummarizerEngine.buildAndPersist({ useOllama, model });
            result = {
              ok: built.ok,
              total_files: built.cache.total_files,
              ollama_used: built.cache.ollama_used,
              heuristic_used: built.cache.heuristic_used,
              md_path: built.md_path,
              json_path: built.json_path,
              error: built.error ?? null,
            };
            break;
          }
          case "directive_summarize_read": {
            const { directiveSummarizerEngine } = await import("../../engines/DirectiveSummarizerEngine.js");
            const cache = directiveSummarizerEngine.read();
            if (cache === null) {
              result = { ok: false, error: "cache missing — run directive_summarize_build first" };
            } else {
              result = { ok: true, cache };
            }
            break;
          }
          case "directive_summarize_stats": {
            const { directiveSummarizerEngine } = await import("../../engines/DirectiveSummarizerEngine.js");
            const cache = directiveSummarizerEngine.read();
            if (cache === null) {
              result = { ok: false, error: "cache missing", total_files: 0 };
            } else {
              result = {
                ok: true,
                ...
                titles: cache.summaries.map((s) => ({ file_name: s.file_name, title: s.title, mode: s.mode })),
              };
            }
            break;
          }

          // ── INTEL-OLLAMA-OBSIDIAN-MS0/P9-U04: pre-edit Quality scoring consumer ──
          case "build_guard_quality_score": {
            const { qualityScoreEngine } = await import("../../engines/QualityScoreEngine.js");
            // Resolve engine name from explicit params first, else derive from a file path.
            // We strip directories + extensions ("src/engines/Foo.ts" → "Foo") so callers
            // can pass either Edit-tool style file_path or an explicit engine_name.
            let engineName: string | undefined =
              (params.engine_name as string | undefined) ?? undefined;
            if (!engineName && typeof params.file_path === "string" && params.file_path.length > 0) {
              const base = params.file_path.split(/[\\/]/).pop() || "";
              engineName = base.replace(/\.(ts|tsx|js)$/, "") || undefined;
            }
            const report = await qualityScoreEngine.compute(engineName);
            result = {
          ...
          // ── Utilization Contract (MXU-MS0A) ──────────────────────
          case "utilization_map": {
            const { utilizationContractEngine } = await import("../../engines/UtilizationContractEngine.js");
            result = utilizationContractEngine.runCensus([], new Map(), [], [], 0, 0);
            break;
          }
          case "utilization_gaps": {
            const { utilizationContractEngine } = await import("../../engines/UtilizationContractEngine.js");
            const caps = utilizationContractEngine.mapCapabilities([], new Map(), [], []);
            result = utilizationContractEngine.findGaps(caps);
            break;
          }
          case "utilization_report": {
            const { utilizationContractEngine } = await import("../../engines/UtilizationContractEngine.js");
            result = utilizationContractEngine.generateReport([], 0, 0, 0);
            break;
          }
          ...
          // ── Model telemetry (INTEL-OLLAMA-OBSIDIAN-MS0 / P23-U01) ─────
          case "model_telemetry_report": {
            const { modelTelemetryEngine } = await import("../../engines/ModelTelemetryEngine.js");
            const windowMs = typeof params.windowMs === "number" && params.windowMs > 0
              ? params.windowMs
              : 7 * 24 * 60 * 60 * 1000;
            try {
              result = await modelTelemetryEngine.summary(windowMs);
            } catch (e) {
              result = { error: `model_telemetry_report: ${(e as Error).message}` };
            }
            break;
          }
          case "model_telemetry_recent": {
            const { modelTelemetryEngine } = await import("../../engines/ModelTelemetryEngine.js");
            const limit = typeof params.limit === "number" && params.limit > 0 && Number.isInteger(params.limit)
              ? params.limit
              : 50;
            try {
              result = await modelTelemetryEngine.getRecent(limit);
            } catch (e) {
              result = { error: `model_telemetry_recent: ${(e as Error).message}` };
            }
            break;
          }
          case "model_telemetry_record": {
            const { modelTelemetryEngine } = await import("../../engines/ModelTelemetryEngine.js");
            if (typeof params.model !== "string" || params.model.length === 0) {
              result = { error: "model_telemetry_record requires 'model' string" };
              break;
            }
            try {
              const r = await modelTelemetryEngine.recordCall({
                model: params.model,
                tier: typeof params.tier === "number" ? params.tier : 1,
                outcome: typeof params.outcome === "string" ? params.outcome : "ok",
                latencyMs: typeof params.latencyMs === "number" ? params.latencyMs : 0,
                ...
                qualityScore: typeof params.qualityScore === "number" ? params.qualityScore : undefined,
                caller: typeof params.caller === "string" ? params.caller : undefined,
                error: typeof params.error === "string" ? params.error : undefined,
              });
              result = r;
            } catch (e) {
              result = { error: `model_telemetry_record: ${(e as Error).message}` };
            }
            break;
          }

          case "catalog_extract": {
            // INTEL-OLLAMA-OBSIDIAN-MS0/P18-U02 — schema-driven extractor framework.
            const { ExtractionFrameworkEngine } = await import("../../engines/ExtractionFrameworkEngine.js");
            const input = params.input;
            const schema = params.schema;
            if (typeof input !== "string") {
              result = { error: "catalog_extract requires 'input' string" };
              break;
            }
            if (!schema || typeof schema !== "object") {
              result = { error: "catalog_extract requires 'schema' object" };
              break;
            }
            try {
              ...
              result = { error: `catalog_extract: ${(e as Error).message}` };
            }
            break;
          }

          case "audit_chain": {
            // INTEL-OLLAMA-OBSIDIAN-MS0/P10-U03 — single-entry router over 13 audit engines.
            const { auditChainEngine } = await import("../../engines/AuditChainEngine.js");
            const type = params.type;
            if (typeof type !== "string" || type.length === 0) {
              result = { error: "audit_chain requires 'type' string (code|machine|data|hook|system|security|operator|library|memory)" };
              break;
            }
            ...
            const options = (params.options && typeof params.options === "object" && !Array.isArray(params.options))
              ...
              : undefined;
            result = await auditChainEngine.run({ type: type as any, scope, options });
            break;
          }

          case "validate": {
            // INTEL-OLLAMA-OBSIDIAN-MS0/P10-U02 — single-entry router over 19+ validation engines.
            const { validationOrchestratorEngine } = await import("../../engines/ValidationOrchestratorEngine.js");
            const domain = params.domain;
            const target = params.target;
            if (typeof domain !== "string" || domain.length === 0) {
              result = { error: "validate requires 'domain' string (code|physics|data|post|formula|machine|coolant|gcode|catalog|dimensional|macro|controller)" };
              break;
            }
            if (typeof target !== "string" || target.length === 0) {
              result = { error: "validate requires 'target' string (file path / id / formula name)" };
              break;
            }
            const options = (params.options && typeof params.options === "object" && !Array.isArray(params.options))
              ? (params.options as Record<string, unknown>)
              : undefined;
            result = await validationOrchestratorEngine.run({ domain: domain as any, target, options });
            break;
          }

          case "vision_extract": {
            // INTEL-OLLAMA-OBSIDIAN-MS0/P21-U01 — wraps llama3.2-vision:11b for image+text understanding.
            const { visionExtractionEngine } = await import("../../engines/VisionExtractionEngine.js");
            const prompt = params.prompt;
            if (typeof prompt !== "string" || prompt.length === 0) {
              result = { error: "vision_extract requires 'prompt' string" };
              break;
            }
            result = await visionExtractionEngine.extract({
              prompt,
