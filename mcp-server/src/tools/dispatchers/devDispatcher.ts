...
"schema_coverage_audit"] as const;
    ...
    async ({ action, params: rawParams = {} }: { action: string; params: Record<string, any> }) => {
      ...
      try {
        switch (action) {
          ...
          // INTEL-OLLAMA-OBSIDIAN-MS0/P8-U05 — schema quality (z.any() residue + describe coverage + drift)
          case "schema_coverage_audit": {
            const { schemaQualityAuditEngine } = await import("../../engines/SchemaQualityAuditEngine.js");
            result = schemaQualityAuditEngine.audit();
            break;
          }
