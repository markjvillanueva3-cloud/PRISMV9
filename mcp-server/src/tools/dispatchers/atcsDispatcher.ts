...
export function registerAtcsDispatcher(server: any): void {
  server.tool(
    ...
    async ({ action, params: rawParams = {} }: { action: typeof ACTIONS[number]; params: Record<string, any> }) => {
      ...
      const validation = validateActionParams(action, params, ACTION_ATCS_SCHEMAS);
      if (!validation.valid) {
        return dispatcherError(`Invalid params for '${action}': ${validation.errorMessage}`, action, "prism_atcs");
      }
      try {
        switch (action) {
