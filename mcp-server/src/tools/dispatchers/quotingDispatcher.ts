    ...
    async ({ action, params = {} }: { action: string; params?: Record<string, unknown> }) => {
      ...
      try {
        switch (action) {
          ...
          case "tool_cost_basis": {
            // U-QP-CONSUMABLE-COST-BASIS -- advisory per-type consumable cost prior (jm-tool-purchases.json
            // byType spend/count = $/line-item), folded with the reconciliation-ledger [0.8,1.2] multipliers.
            // ALWAYS confidence:"advisory" (source has no per-type qty -> never customer-grade). Read-only;
            // admin-gated route + generic-surface deny (this is cost basis -- anon-leak class).
            const { consumableCostBasisEngine } = await import("../../engines/ConsumableCostBasisEngine.js");
            const p = parsed.data as { type?: string; basisPath?: string; ledgerPath?: string };
            result = consumableCostBasisEngine.toolCostBasis(p.type, {
              basisPath: p.basisPath,
          ...
          case "consumable_reconcile": {
            ...
            // U-QP-CONSUMABLE-COST-BASIS -- close the loop: persist the bounded [0.8,1.2]
            // feedback_multipliers to the durable ledger so tool_cost_basis folds them back
            // into the forward advisory prior. Append-only, reads no threshold, writes no gate.
            // Fail-soft: a thrown recordMultipliers() never alters the reconcile result (R12).
            try {
              const { consumableCostBasisEngine } = await import("../../engines/ConsumableCostBasisEngine.js");
              await consumableCostBasisEngine.recordMultipliers(report.feedback_multipliers, {
                updatedAt: new Date().toISOString(),
              });
            } catch {
              /* ledger write is best-effort telemetry -- never alters the reconciliation result (R12) */
            }
            result = report;
            break;
          }
