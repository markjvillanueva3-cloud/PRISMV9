...
// ---------------------------------------------------------------------------
// PROGRAMMATIC API (for use by AutomatedResourceHarvestingPipeline and other engines)
// ---------------------------------------------------------------------------

/**
 * Call a document learning action programmatically (bypasses MCP transport).
 * Supports the same actions as the MCP tool: doc_upload, doc_extract, doc_list, doc_get, doc_delete.
...
export async function callDocumentAction(
  ...
  if (!handler) {
    return { error: `Unknown action: ${action}`, available: Object.keys(ACTION_HANDLERS) };
  }
