...
// QDRANT-DENSE-ARM: async fetch-based 5th RRF substrate (U-INDIA-QDRANT-ARM-WIRE).
// The engine lives at src/engines/ai-training/QdrantDenseRetrievalEngine.ts; the
// dist/ singleton is imported at runtime so the dispatcher stays cross-tree-clean.
export interface QdrantDenseModule {
  qdrantDenseRetrievalEngine: {
    search: (opts: {
      ...
      limit?: number;
      timeoutMs?: number;
    }) => Promise<{ ok: boolean; hits: Array<{ id: string; score: number; payload: Record<string, unknown> | null }>; error?: string; durationMs?: number }>;
  };
}
...
export interface HybridSearchDeps {
  ...
  importQdrantDense?: () => Promise<QdrantDenseModule>; // QDRANT-DENSE-ARM
  curlSend?: CurlSend;
}
...
// QDRANT-DENSE-ARM: load the compiled singleton from dist/ at runtime via
// file:// URL so esbuild does not attempt static bundling of the import
// (same pattern as defaultImportLib / defaultImportLocalStore above).
async function defaultImportQdrantDense(): Promise<QdrantDenseModule> {
  return (await import(
    "file:///H:/prism/mcp-server/dist/engines/ai-training/QdrantDenseRetrievalEngine.js" as string
  )) as QdrantDenseModule;
}
...
export async function runHybridSearchAction(
  ...
  const importQdrantDense = deps.importQdrantDense ?? defaultImportQdrantDense; // QDRANT-DENSE-ARM
  ...
  // QDRANT-DENSE-ARM: resolve singleton only when the env gate is on, so the
  // import is a no-op for callers that never set PRISM_RAG_DENSE_QDRANT=1.
  const qdrantDenseEnvOn = (process.env["PRISM_RAG_DENSE_QDRANT"] ?? "").trim() === "1";
  let qdrantDenseSearchFn: ((opts: unknown) => Promise<unknown>) | undefined;
  if (qdrantDenseEnvOn) {
    try {
      const qdrantMod = await importQdrantDense();
      qdrantDenseSearchFn = (opts: unknown) =>
        qdrantMod.qdrantDenseRetrievalEngine.search(
          opts as Parameters<typeof qdrantMod.qdrantDenseRetrievalEngine.search>[0],
        );
    } catch {
      // Degrade gracefully -- hybrid-retrieval will record "no-impl" skip
    }
  }
  ...
  return lib.hybridSearch(query, {
    ...
    // QDRANT-DENSE-ARM: 5th RRF substrate -- active when PRISM_RAG_DENSE_QDRANT=1
    qdrantDenseSearch: qdrantDenseSearchFn,
