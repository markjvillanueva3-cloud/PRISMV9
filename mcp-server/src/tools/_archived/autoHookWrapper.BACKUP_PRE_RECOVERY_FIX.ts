    ...
    if (cadence.actions.length > 0 && result?.content?.[0]?.text) {
      try {
        ...
        try {
          ...
          fs.writeFileSync(cadenceFiresPath, JSON.stringify(fires, null, 2));
        } catch {
        }
        try {
        ...
        if (compactionDetectedThisCall) {
          ...
          try {
            ...
            if (fs.existsSync(manifestPath)) {
              ...
              if (ageMs > 4 * 60 * 60 * 1000) recoveryManifest = null;
            }
          } catch {
          }
          let survivalInfo = {};
          ...
          let reasoningTrail = [];
          try {
            const jPath = path.join(STATE_DIR12, "SESSION_JOURNAL.jsonl");
          ...
          const hijacked = {
            ...
            recovery_context: {
              ...
              phase: (() => {
                ...
                try {
                  const cs = JSON.parse(fs.readFileSync(path.join(STATE_DIR12, "CURRENT_STATE.json"), "utf-8"));
