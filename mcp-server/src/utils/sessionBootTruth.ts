...
export function buildSessionBootInstanceId(options: {
  ...
  const family = sanitizeToken(
    options.agentFamily ?? process.env.PRISM_AGENT_FAMILY ?? process.env.CODEX_AGENT_FAMILY ?? "prism",
    "prism",
  );
  const machine = sanitizeToken(
    options.machineName ?? process.env.COMPUTERNAME ?? os.hostname(),
    "unknown",
  );
...
function describeFileFreshness(filePath: string, staleAfterMinutes: number, now: Date = new Date()) {
  try {
...
function summarizeRoadmapCollaborationState(markdown: string, freshness: any) {
...
function summarizeTaskQueue(markdown: string, freshness: any) {
...
function summarizeGsdQuick(markdown: string, freshness: any) {
  ...
  return {
    ...
    key_rules: ["MCP first", "No placeholders", "Build via npm run build", "Persist state before session end"],
...
function sanitizeResumeDetection(resumeDetection: any, currentStateFreshness: any) {
...
function sanitizeEnhancedStartup(enhancedStartup: any) {
  if (!enhancedStartup || typeof enhancedStartup !== "object" || Array.isArray(enhancedStartup)) return enhancedStartup;
...
function sanitizeKeyMemories(keyMemories: any, freshness: any) {
...
export function applySessionBootTruthfulness<T extends Record<string, any>>(
  result: T,
  options: {
    stateDir: string;
    mcpRoot: string;
  ...
  const sharedActionItems =
    sharedRoadmap.delivery_priority.length > 0
      ? sharedRoadmap.delivery_priority
      : sharedTaskQueue.available_preview.map((item: any) => `${item.id}: ${item.title}`);
  ...
  if (currentStateFreshness.stale) {
    integrity.push(`CURRENT_STATE.json is ${formatAgeMinutes(currentStateFreshness.age_minutes)} old; shared state is canonical`);
  }
