  ...
  handler: (context: HookContext): HookResult => {
    ...
    if (safety === undefined) {
      ...
      const READONLY_PATTERNS = [
        ...
        // ── AI stats, registry queries, and reasoning introspection (read-only) ──
        "stats", "_stats", "ai_registry_", "profile_list", "profile_get",
        "chain_sharing_stats", "neural_stats", "opus_stats", "cot_stats",
        "uncertainty_stats", "decision_stats", "learning_stats",
        "list_", "get_", "search_", "query_", "analyze_", "compare_",
        ...
        // ── Post processor analysis and verification (read-only) ──
        "pp_verify", "pp_analyze", "pp_backplot", "pp_capability",
        "cps_parse", "cps_summary", "cps_search", "cps_map",
        "post_coverage", "post_browse", "post_get_", "post_compare",
        "post_list_", "post_match_", "post_detect_",
        // ── EDM analysis, planning, and generation (non-destructive) ──
        "wedm_plan", "wedm_assess", "wedm_calculate", "wedm_estimate",
        "wedm_select", "wedm_check", "wedm_generate", "wedm_full",
        "wedm_monitor", "wedm_verify", "wedm_evaluate", "wedm_advanced",
        "wedm_record", "wedm_get", "wedm_run_pipeline", "wedm_studio",
        "wedm_bimaterial", "wedm_calculator", "wedm_optimize",
        "wedm_solve", "wedm_predict", "wedm_interpret", "wedm_classify",
        "wedm_parse", "wedm_validate",
        "electrode_design", "wire_settings", "surface_integrity",
        "sinker_calculate", "sinker_recommend", "sinker_materials",
...
/**
 * Pre-page-create: Frontend Feature Audit Gate
 *
 * BLOCKING HOOK: Prevents creation of new frontend pages without:
 * 1. Checking existing 102+ pages for similar functionality
 * 2. Running gap analysis on existing pages
 * 3. Documenting coverage mapping
 * 4. Following Calculator Studio design language
 *
 * This hook enforces the Codex page protection protocol.
 */
const preFrontendPageCreateAudit: HookDefinition = {
  id: "pre-frontend-page-create-audit",
  name: "Frontend Feature Audit Gate",
  description: "BLOCKS new page creation without gap analysis. Enforces Codex page protection protocol.",

  phase: "pre-page-create",
  category: "enforcement",
  mode: "blocking",
  priority: "critical",
  enabled: true,

  tags: ["frontend", "codex", "audit", "gap-analysis", "calculator-studio"],

  handler: (context: HookContext): HookResult => {
    const hook = preFrontendPageCreateAudit;

    const pageData = context.target?.data as {
      pageName?: string;
      filePath?: string;
      features?: string[];
      auditReport?: { recommendation: string; coveragePercent: number };
    };

    if (!pageData) {
      return hookBlock(hook, "No page data provided for audit");
    }

    // Check if audit report exists
    if (!pageData.auditReport) {
      return hookBlock(hook,
        `🚫 BLOCKED: No frontend audit report for "${pageData.pageName || 'unknown'}".\n` +
        `Run frontendFeatureAuditHook.analyze() first to check existing page coverage.`,
        {
          issues: [
            "Missing audit report",
            "Must check existing pages before creating new",
            "See: data/state/WIRE-MS0/frontend-coverage-audit.json"
          ]
        }
      );
    }
    ...
    const { recommendation, coveragePercent } = pageData.auditReport;

    if (recommendation === "blocked") {
      return hookBlock(hook,
        `🚫 BLOCKED: Existing page coverage too high (${coveragePercent}%). Enhance existing page instead.`,
        {
          score: coveragePercent / 100,
          issues: [
            `Coverage: ${coveragePercent}%`,
            "Recommendation: enhance_existing",
            "Do NOT create new page - improve existing page"
          ]
        }
      );
    }

    if (recommendation === "enhance_existing" && coveragePercent >= 50) {
      return hookBlock(hook,
        `🚫 REDIRECT: Existing page covers ${coveragePercent}% of functionality.\n` +
        `Enhance the existing page instead of creating new.`,
        {
          score: coveragePercent / 100,
          issues: [
            `Coverage: ${coveragePercent}%`,
            "Recommendation: enhance_existing",
            "Add missing features to existing page"
          ]
        }
      );
    }

    // Check for Calculator Studio design elements
    if (pageData.filePath) {
      ...
      // This would be checked against actual file content in real implementation
      log.info(`Frontend audit passed for ${pageData.pageName}: recommendation=${recommendation}, coverage=${coveragePercent}%`);
    }

    if (recommendation === "create_new") {
      return hookSuccess(hook,
        `✅ Frontend audit PASSED: New page "${pageData.pageName}" approved (existing coverage: ${coveragePercent}%)`,
        {
          score: coveragePercent / 100,
          data: {
            recommendation,
            designLanguage: "calculator-studio"
          }
        }
      );
    }
    ...
    return hookWarning(hook,
      `⚠️ Frontend audit: ${pageData.pageName} - low coverage (${coveragePercent}%). New page allowed with caution.`,
      {
        score: coveragePercent / 100,
        warnings: ["Check for existing pages with partial coverage"]
      }
    );
  }
};
