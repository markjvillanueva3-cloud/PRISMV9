...
/**
 * KAR-MS1 U-KAR05: Session-start resource scan hook
 * Scans resources folder on session start, queues new/modified for extraction.
 * Uses FolderScannerEngine for mtime-based change detection.
 */
const onSessionStartResourceScan: HookDefinition = {
  id: "on-session-start-resource-scan",
  name: "Resource Scan on Session Start",
  description: "Scans resources folder, detects new/modified files, logs pending count for KnowledgeIngestionOrchestrator.",
  ...
  tags: ["session", "resources", "ingestion", "kar"],

  handler: (context: HookContext): HookResult => {
    const hook = onSessionStartResourceScan;

    // Resource roots to scan
    ...
    for (const rootPath of RESOURCE_ROOTS) {
      try {
        const scanResult = folderScannerEngine.scan({
          root_path: rootPath,
          max_depth: 15,
          include_unchanged: false, // Only new/changed
        });

        totalNew += scanResult.new_files;
        totalChanged += scanResult.changed_files;

        // Count extractable files by type
        const extractableTypes = ["pdf", "cnc_program", "cam_file", "spreadsheet"];
        for (const file of scanResult.files) {
          if ((file.is_new || file.is_changed) && extractableTypes.includes(file.file_type)) {
            pendingByType[file.file_type] = (pendingByType[file.file_type] || 0) + 1;
          }
        }

        // Track scan actions
        if (scanResult.new_files > 0 || scanResult.changed_files > 0) {
          actions.push(`scanned:${rootPath.split("/").pop()}`);
        }

        if (scanResult.errors.length > 0) {
          errors.push(...scanResult.errors.slice(0, 5)); // Limit errors
        }

      } catch (e) {
        const errMsg = e instanceof Error ? e.message : String(e);
        errors.push(`Scan failed for ${rootPath}: ${errMsg}`);
        log.warn(`[ResourceScan] Failed to scan ${rootPath}: ${errMsg}`);
      }
    }

    // Calculate total pending for extraction
    const totalPending = Object.values(pendingByType).reduce((a, b) => a + b, 0);

    // Build summary
    const summary = `Found ${totalNew} new, ${totalChanged} changed files. ${totalPending} pending extraction.`;
    log.info(`[ResourceScan] ${summary}`);

    if (totalPending > 0) {
      actions.push(`pending_extraction:${totalPending}`);
      // Add type breakdown to actions
      for (const [type, count] of Object.entries(pendingByType)) {
        actions.push(`pending_${type}:${count}`);
      }
    }
