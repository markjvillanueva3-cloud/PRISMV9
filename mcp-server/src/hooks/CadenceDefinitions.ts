...
/** #224 hourly_knowledge_reindex — KAR-MS1 U-KAR08: periodic knowledge re-index */
const hourlyKnowledgeReindex: HookDefinition = {
  id: "cadence-hourly-knowledge-reindex",
  name: "Hourly Knowledge Re-Index",
  description:
    "Scans resource folders for new/changed files and triggers knowledge re-indexing. Part of Knowledge-Augmented Reasoning infrastructure.",
  phase: "on-session-checkpoint",
  event: "phase.post-data-query",
  category: "automation",
  mode: "logging",
  priority: "low",
  enabled: true,
  tags: ["cadence", "hourly", "knowledge", "kar", "reindex"],
  handler: (ctx: HookContext): HookResult => {
    const d = (ctx.target?.data ?? {}) as Record<string, any>;
    const stats = d.reindexStats ?? { newFiles: 0, changedFiles: 0, indexed: 0, errors: 0 };

    const summary: string[] = [];
    if (stats.newFiles > 0) summary.push(`${stats.newFiles} new`);
    if (stats.changedFiles > 0) summary.push(`${stats.changedFiles} changed`);
    if (stats.indexed > 0) summary.push(`${stats.indexed} indexed`);

    if (stats.errors > 0) {
      return hookWarning(hourlyKnowledgeReindex, `Re-index: ${summary.join(", ")} (${stats.errors} errors)`, {
        score: 0.8,
        warnings: [`${stats.errors} indexing errors occurred`],
        data: stats,
      });
    }

    if (summary.length === 0) {
      return hookSuccess(hourlyKnowledgeReindex, "No new knowledge to index", {
        score: 1.0,
        data: stats,
      });
    }

    log.info(`[cadence] Knowledge re-index: ${summary.join(", ")}`);
    return hookSuccess(hourlyKnowledgeReindex, `Re-indexed: ${summary.join(", ")}`, {
      score: 1.0,
      data: stats,
    });
  },
