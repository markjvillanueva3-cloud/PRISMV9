...
const wedmSyntheticBlock: HookDefinition = {
  ...
  handler: async (ctx: HookContext): Promise<HookResult> => {
    const files = ((ctx.target?.data?.files as unknown) ?? []) as string[];
...
/**
 * Hook: hook_wedm_collision_gate
 * Trigger: PreTool on wedm program emit / generate_gcode / execute
 * HARD BLOCK: Any wire-path collision with head, fixture, workpiece, or tank wall.
 *
 * MS-P1.5-ONESHOT/U-P1.5-OS-05 exit criterion:
 *   "wedm-collision-gate hook BUILT here: blocks program emit on any collision"
 *
 * Reads collision check result from ctx.target.data.collisionCheck
 * (populated by WEDMWirePathCollisionEngine.check upstream). If the caller has
 * not run the collision engine, the hook performs the check when wire_paths +
 * machine + obstacles are present in the payload.
 *
 * S(x) contribution: +0.15 on pass, 0.0 + hard block on any collision.
 */
const wedmCollisionGate: HookDefinition = {
  id: "wedm-collision-gate",
  name: "WEDM Wire-Path Collision Gate",
  description:
    "HARD BLOCK on any wire-path collision (upper/lower head, fixture, workpiece, tank wall) before program emit.",
  phase: "pre-tool",
  category: "safety",
  mode: "blocking",
  priority: "critical",
  enabled: true,
  tags: ["wedm", "collision", "safety", "blocking", "gate"],
  handler: async (ctx: HookContext): Promise<HookResult> => {
    const d = (ctx.target?.data ?? {}) as Record<string, any>;
    const action = ctx.target?.action ?? "";

    // Scope: program-emit critical actions only.
    const emitActions = ["emit_program", "generate_gcode", "generate_program", "execute", "run_program", "post_program"];
    const matches = emitActions.some((a) => action.includes(a)) || action.startsWith("wedm_emit");
    if (!matches) {
      return hookSuccess(wedmCollisionGate, "Not a program-emit action", { data: { skipped: true } });
    }

    // Prefer an upstream collision result (canonical path).
    const provided = d.collisionCheck ?? d.collision_check ?? d.collisionResult ?? null;

    let result: { pass: boolean; collision_count: number; critical_count: number; summary: string; safety_score_contribution: number } | null = null;

    if (provided && typeof provided === "object") {
      result = {
        pass: Boolean(provided.pass),
        collision_count: Number(provided.collision_count ?? 0),
        critical_count: Number(provided.critical_count ?? 0),
        summary: String(provided.summary ?? ""),
        safety_score_contribution: Number(provided.safety_score_contribution ?? 0),
      };
    } else if (Array.isArray(d.wire_paths) && d.machine && Array.isArray(d.obstacles)) {
      // Fallback: run the engine inline if the caller supplied geometry.
      try {
        const { wedmWirePathCollisionEngine } = await import("../engines/WEDMWirePathCollisionEngine.js");
        const r = wedmWirePathCollisionEngine.check({
          wire_paths: d.wire_paths,
          machine: d.machine,
          obstacles: d.obstacles,
          workpiece_thickness_mm: Number(d.workpiece_thickness_mm ?? 0),
          safety_margin_mm: d.safety_margin_mm,
        });
        result = {
          pass: r.pass,
          ...
          summary: r.summary,
          safety_score_contribution: r.safety_score_contribution,
        };
      } catch (err) {
        log.warn("wedm-collision-gate", `Inline collision check failed: ${String(err)}`);
      }
    }

    if (!result) {
      // Missing both upstream result and geometry → block: cannot verify safety.
      log.error("wedm-collision-gate", "BLOCKED: program emit attempted without collision evidence");
      return hookBlock(
        wedmCollisionGate,
        "WEDM program emit requires collisionCheck from WEDMWirePathCollisionEngine (missing).",
        { data: { reason: "no_collision_evidence", action } }
      );
    }

    if (result.collision_count > 0 || !result.pass) {
      log.error("wedm-collision-gate", `HARD BLOCK: ${result.collision_count} collision(s) — ${result.summary}`);
      return hookBlock(
        wedmCollisionGate,
        `WEDM collision gate FAILED: ${result.collision_count} collision(s), ${result.critical_count} critical. ${result.summary}`,
        {
          data: {
            collision_count: result.collision_count,
            critical_count: result.critical_count,
            summary: result.summary,
            safety_score_contribution: 0.0,
          },
        }
      );
    }

    return hookSuccess(wedmCollisionGate, `No collisions detected (+${result.safety_score_contribution.toFixed(2)} S(x))`, {
      data: {
        collision_count: 0,
        safety_score_contribution: result.safety_score_contribution,
      },
    });
  },
};
...
/**
 * Hook: hook_wedm_program_safety_gate
 * Trigger: PreTool on wedm program emit / generate_gcode
 * HARD BLOCK: composite S(x) < 0.70 across 7 weighted safety components.
 *
 * MS-P2.5-SAFETY/U-P2.5-SAFE-01 exit criterion:
 *   "wedm-program-safety-gate hook BUILT here: CI blocks merge on any bypass"
 *
 * Reads either an upstream {safetyGate | safety_gate | safetyBreakdown} result
 * (SafetyGateResult from WEDMProgramSafetyGateEngine) or composes one inline
 * from individual component inputs on ctx.target.data. Refuses to pass the
 * program if S(x) < threshold with a component-by-component failure breakdown.
 *
 * Weighted components:
 *   collision(0.20) | head_clearance(0.15) | flushing(0.15) | thermal(0.15)
 *   dialect(0.10)   | unit_tag(0.10)       | deflection(0.15)
 */
const wedmProgramSafetyGate: HookDefinition = {
  id: "wedm-program-safety-gate",
  name: "WEDM Program Safety Gate (S(x) ≥ 0.70)",
  description:
    "HARD BLOCK: composite S(x) < 0.70 across collision/head-clearance/flush/thermal/dialect/unit-tag/deflection before program emit.",
  phase: "pre-tool",
  category: "safety",
  mode: "blocking",
  priority: "critical",
  enabled: true,
  tags: ["wedm", "safety", "gate", "sx", "blocking"],
  handler: async (ctx: HookContext): Promise<HookResult> => {
    const d = (ctx.target?.data ?? {}) as Record<string, any>;
    const action = ctx.target?.action ?? "";
    ...
    const matches = emitActions.some((a) => action.includes(a)) || action.startsWith("wedm_emit");
    if (!matches) {
      return hookSuccess(wedmProgramSafetyGate, "Not a program-emit action", { data: { skipped: true } });
    }

    // Prefer an upstream SafetyGateResult (canonical path).
    const provided = d.safetyGate ?? d.safety_gate ?? d.safetyBreakdown ?? null;
    let result: { pass: boolean; s_of_x: number; threshold: number; hard_block: boolean; failing_components: number; failure_reasons: string[]; summary: string } | null = null;

    if (provided && typeof provided === "object" && typeof provided.s_of_x === "number") {
      result = {
        pass: Boolean(provided.pass),
        s_of_x: Number(provided.s_of_x),
        threshold: Number(provided.threshold ?? 0.70),
        hard_block: Boolean(provided.hard_block),
        failing_components: Number(provided.failing_components ?? 0),
        failure_reasons: Array.isArray(provided.failure_reasons) ? provided.failure_reasons : [],
        summary: String(provided.summary ?? ""),
      };
    } else {
      // Fallback: compose SafetyGateInput inline from component data present on payload.
      const inlineComponents = {
        collision: d.collision ?? d.collisionCheck ?? undefined,
        head_clearance: d.head_clearance ?? d.headClearance ?? undefined,
        flushing: d.flushing ?? d.flushAdequacy ?? undefined,
        thermal: d.thermal ?? d.thermalRelease ?? undefined,
        dialect: d.dialect ?? d.dialectVerification ?? undefined,
        unit_tag: d.unit_tag ?? d.unitTag ?? undefined,
        deflection: d.deflection ?? undefined,
      };
      const anyComponent = Object.values(inlineComponents).some((v) => v !== undefined);
      if (anyComponent) {
        try {
          const { wedmProgramSafetyGateEngine } = await import("../engines/WEDMProgramSafetyGateEngine.js");
          const r = wedmProgramSafetyGateEngine.evaluate(inlineComponents as any);
          result = {
            pass: r.pass,
            s_of_x: r.s_of_x,
          ...
          log.warn("wedm-program-safety-gate", `Inline gate evaluation failed: ${String(err)}`);
        }
      }
    }

    if (!result) {
      log.error("wedm-program-safety-gate", "BLOCKED: program emit attempted without safety-gate evidence");
      return hookBlock(
        wedmProgramSafetyGate,
        "WEDM program emit requires SafetyGateResult (S(x) evaluation missing).",
        { data: { reason: "no_safety_evidence", action } }
      );
    }

    if (result.hard_block || !result.pass || result.s_of_x < result.threshold) {
      log.error(
        "wedm-program-safety-gate",
        `HARD BLOCK: S(x)=${result.s_of_x.toFixed(3)} < ${result.threshold} (${result.failing_components} failing)`
      );
      return hookBlock(
        wedmProgramSafetyGate,
        `WEDM safety gate FAILED: S(x)=${result.s_of_x.toFixed(3)} < ${result.threshold}. ${result.failing_components} failing component(s). ${result.summary}`,
        {
          data: {
            s_of_x: result.s_of_x,
    ...
    return hookSuccess(
      wedmProgramSafetyGate,
      `S(x)=${result.s_of_x.toFixed(3)} ≥ ${result.threshold} — safety gate PASS`,
      {
        data: {
          s_of_x: result.s_of_x,
          threshold: result.threshold,
...
/**
 * Hook: hook_wedm_unit_tag_gate
 * Trigger: PreTool on WEDM program-emit actions
 *
 * MS-P2.5-SAFETY/U-P2.5-SAFE-02 exit criterion:
 *   "wedm-unit-tag-gate hook BUILT here: blocks mixed-unit / 25.4× confusion
 *    emits at CI and at runtime."
 *
 * HARD BLOCK when any of:
 *   - Program header lacks G20 or G21
 *   - Header contradicts declared input.units
 *   - Shop-profile unit system differs from declared
 *   - Max coordinate magnitude is > 5× or < 1/5 of the declared stock bbox
 *     (catching the 25.4× imperial↔metric confusion class)
 *
 * Reads an upstream `unitTag` / `unit_tag` result from the pipeline OR runs
 * WEDMUnitTagGateEngine inline when `program_text` + `declared_unit` are on
 * the payload.
 */
const wedmUnitTagGate: HookDefinition = {
  id: "wedm-unit-tag-gate",
  name: "WEDM Unit-Tag & 25.4× Confusion Gate",
  description:
    "HARD BLOCK on unit-tag mismatch or 25.4× imperial↔metric coordinate confusion before program emit.",
  phase: "pre-tool",
  category: "safety",
  mode: "blocking",
  priority: "critical",
  enabled: true,
  tags: ["wedm", "unit", "safety", "blocking", "gate", "25.4x"],
  handler: async (ctx: HookContext): Promise<HookResult> => {
    const d = (ctx.target?.data ?? {}) as Record<string, any>;
    const action = ctx.target?.action ?? "";
    ...
    const matches =
      emitActions.some((a) => action.includes(a)) || action.startsWith("wedm_emit");
    if (!matches) {
      return hookSuccess(wedmUnitTagGate, "Not a program-emit action", {
        data: { skipped: true },
      });
    }

    // Prefer an upstream unit_tag gate result.
    const provided = d.unitTag ?? d.unit_tag ?? null;
    ...
    if (provided && typeof provided === "object") {
      pass = Boolean(provided.pass);
      declaredUnit = provided.declared_unit ?? null;
      codeUnit = provided.code_unit ?? null;
      summary = String(provided.summary ?? "");
    } else if (typeof d.program_text === "string" && typeof d.declared_unit === "string") {
      // Fallback: run the engine inline.
      try {
        const { wedmUnitTagGateEngine } = await import(
          "../engines/WEDMUnitTagGateEngine.js"
        );
        const r = wedmUnitTagGateEngine.evaluate({
          program_text: d.program_text,
          declared_unit: d.declared_unit === "imperial" ? "imperial" : "metric",
          shop_unit_system: d.shop_unit_system,
          expected_max_coord:
            typeof d.expected_max_coord === "number" ? d.expected_max_coord : undefined,
          magnitude_tolerance_ratio:
            typeof d.magnitude_tolerance_ratio === "number"
              ? d.magnitude_tolerance_ratio
              : undefined,
        });
        pass = r.pass;
        declaredUnit = r.declared_unit;
        codeUnit = r.metrics.header_found;
        summary = r.summary;
      } catch (err) {
        log.warn("wedm-unit-tag-gate", `Inline unit evaluation failed: ${String(err)}`);
      }
    }
    ...
    if (pass === null) {
      log.error(
        "wedm-unit-tag-gate",
        "BLOCKED: program emit attempted without unit-tag evidence",
      );
      return hookBlock(
        wedmUnitTagGate,
        "WEDM program emit requires unit-tag evidence (WEDMUnitTagGateEngine result or program_text + declared_unit).",
        { data: { reason: "no_unit_evidence", action } },
      );
    }
    ...
    if (!pass) {
      log.error(
        "wedm-unit-tag-gate",
        `HARD BLOCK: unit-tag gate failed — declared=${declaredUnit}, code=${codeUnit}`,
      );
      return hookBlock(
        wedmUnitTagGate,
        `WEDM unit-tag gate FAILED: declared ${declaredUnit}, code_unit ${codeUnit}. ${summary}`,
        {
          data: {
    ...
    return hookSuccess(
      wedmUnitTagGate,
      `Unit-tag OK — declared ${declaredUnit}, code ${codeUnit}`,
      {
        data: {
          declared_unit: declaredUnit,
          code_unit: codeUnit,
...
export const wedmSafetyHooks: HookDefinition[] = [
  // Blocking (9) — citation + collision gate + S(x) program safety gate
  wedmCalibrationValidate,
  wedmNeuralSanity,
  wedmGcodeInjection,
  wedmEcodeConsistency,
  ...
  wedmCollisionGate, // MS-P1.5-ONESHOT/U-P1.5-OS-05: HARD BLOCK on wire-path collisions
  wedmProgramSafetyGate, // MS-P2.5-SAFETY/U-P2.5-SAFE-01: HARD BLOCK when S(x) < 0.70
  wedmUnitTagGate, // MS-P2.5-SAFETY/U-P2.5-SAFE-02: HARD BLOCK on unit-tag / 25.4× confusion
  // Quality/Warning (5)
  wedmCostConfidence,
  wedmBatchDeps,
  wedmScheduleConflict,
