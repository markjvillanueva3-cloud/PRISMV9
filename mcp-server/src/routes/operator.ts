...
export function createOperatorRouter(callTool: CallToolFn): Router {
  ...
  router.post("/feedback", async (req: { body?: unknown }, res: Response, next: NextFunction) => {
    try {
      const r: any = await callTool("prism_session", "operator_feedback_record", { feedback: req.body ?? {} });
