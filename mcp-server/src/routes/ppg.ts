...
export function createPpgRouter(callTool: CallToolFn): Router {
  ...
  // ── PPG-REAL S5: Generate Post + Optimize Program ───────────────────

  // POST /ppg/generate-post — Generate machine-specific CPS file
  // Input: {machine_model, controller_family, features[], material?, aggressiveness?}
  // Output: {cps_content, filename, validation:{pass, warnings[]}}
  router.post("/generate-post", async (req, res) => {
    try {
      const { machine_model, controller_family, features, aggressiveness } = req.body;
      if (!controller_family) {
        res.status(400).json({ ok: false, error: "controller_family is required" });
        return;
      }

      const { masterPostProcessorEngine } = await import("../engines/MasterPostProcessorEngine.js");
      const { postLibraryConfiguratorEngine } = await import("../engines/PostLibraryConfiguratorEngine.js");

      // Generate CPS config from MasterPostProcessorEngine
      const machine = {
        manufacturer: machine_model?.split(" ")[0] ?? "Generic",
        model: machine_model ?? "Machine",
        controller: controller_family,
        max_rpm: req.body.max_rpm ?? 0,
        max_feed: req.body.max_feed ?? 0,
        axis_count: req.body.axis_count ?? 3,
        has_probing: features?.includes("probing") ?? false,
        has_tsc: features?.includes("tsc") ?? false,
        taper: req.body.taper ?? "CAT40",
        tool_capacity: req.body.tool_capacity ?? 20,
      };

      const cpsConfig = masterPostProcessorEngine.generateMasterCpsConfig(machine as any);

      // Read the Master Post CPS template and customize
      const fs = await import("fs");
      const path = await import("path");
      ...
      try {
        cpsContent = fs.readFileSync(templatePath, "utf-8");
        // Customize default controller
        cpsContent = cpsContent.replace(
          /value: "haas_ngc"\s*\n\s*\}/,
          `value: "${cpsConfig.controller}"\n  }`
        );
        ...
        // Template not found — generate from engine
        const result = masterPostProcessorEngine.process([], {
          controller: controller_family as any,
          program_number: 1001,
        });
        cpsContent = result.gcode;
      }

      const filename = `PRISM_${(machine_model ?? "Machine").replace(/\s+/g, "_")}.cps`;
      const warnings: string[] = [];

      // Validate
      if (cpsContent.includes("HTTPClient")) {
        warnings.push("CPS contains HTTPClient reference — review for compliance");
      }
      if (cpsContent.length < 100) {
        warnings.push("CPS content appears too short — may be incomplete");
      }

      res.json({
        ok: true,
        data: {
          cps_content: cpsContent,
          filename,
          controller: cpsConfig.controller,
          properties: cpsConfig.properties,
          validation: {
  ...
  // POST /ppg/optimize-program — Optimize existing NC program (Journey B)
  // Input: {gcode, material (REQUIRED), machine?, controller?, aggressiveness?}
  // Output: {optimized_gcode, stages, analytics, diff_summary}
  router.post("/optimize-program", async (req, res) => {
    try {
      const { gcode, material, machine, controller } = req.body;
      if (!gcode) {
        res.status(400).json({ ok: false, error: "gcode is required" });
        return;
      }
      if (!material) {
        res.status(400).json({ ok: false, error: "material is required for program optimization — select from MaterialRegistry" });
        return;
      }
      ...
      const result = await postProcessorPipelineEngine.process({
        gcode,
        material: typeof material === "string" ? { name: material } : material,
      ...
      const optimizedLines = (result as any).output?.split("\n").length ?? originalLines;

      res.json({
        ok: true,
        data: {
          optimized_gcode: (result as any).output ?? gcode,
          stages: (result as any).stages ?? [],
          analytics: (result as any).analytics ?? {},
          diff_summary: {
            original_lines: originalLines,
            optimized_lines: optimizedLines,
            blocks_modified: (result as any).blocks?.filter((b: any) => b.modified).length ?? 0,
          },
        },
      });
    } catch (e: any) {
