...
export function createQuotingRouter(callTool: CallToolFn): Router {
  ...
  // U-QP-CONSUMABLE-COST-BASIS (2026-07-01): the shop's own per-consumable-type procurement cost basis
  // (jm-tool-purchases.json $/line-item) + reconciliation fold-back. The tool-side twin of material_cost_basis
  // -> admin-tier, same as the cost-basis priors above. Denied on the generic surface; reachable ONLY here.
  router.post("/tool-cost-basis", verifyToken, requireRole("admin"), async (req, res, next) => {
    try { res.json(await callTool("prism_quoting", "tool_cost_basis", req.body)); } catch (e) { next(e); }
  });
  // U-QP-SECONDARY-OPS-AUTH-MIGRATION (2026-07-01): the PLAIN secondary-ops price returns the shop's
  // real per-op cost basis (total_secondary_ops_usd). Denied on the generic surface (403 anon);
  // reachable ONLY here behind verifyToken -- any SIGNED-IN operator (a quoting-workbench figure, NOT
  // admin-tier, unlike the cost-basis priors above). Anon -> 401; QuotingWorkbenchPage attaches its Bearer.
  router.post("/secondary-ops-price", verifyToken, async (req, res, next) => {
    try { res.json(await callTool("prism_quoting", "quoting_secondary_ops_price", req.body)); } catch (e) { next(e); }
  });
