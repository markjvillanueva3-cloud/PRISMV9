...
/** Deprecated endpoints (path → deprecation info) */
export const DEPRECATED_ENDPOINTS: Record<string, { since: string; sunset: string; replacement: string }> = {
  // Example: "/api/v1/old-endpoint": { since: "0.9.0", sunset: "2.0.0", replacement: "/api/v1/new-endpoint" },
};

/**
 * API versioning middleware — sets X-API-Version header on all responses.
 * Also adds Deprecation + Sunset headers for deprecated endpoints.
 */
export function apiVersioning(req: Request, res: Response, next: NextFunction): void {
  res.setHeader("X-API-Version", API_VERSION);

  // Check for deprecated endpoints
  const deprecation = DEPRECATED_ENDPOINTS[req.path];
  if (deprecation) {
    res.setHeader("Deprecation", `version="${deprecation.since}"`);
...
/** Creates open api router with Swagger UI.
 * @returns router
 */
export function createOpenApiRouter(): Router {
  ...
  // GET /api/docs/ui — Swagger UI (served from CDN, no npm dependency)
  router.get("/docs/ui", (_req, res) => {
    res.setHeader("Content-Type", "text/html");
    res.send(getSwaggerUIHtml());
  });

  // GET /api/docs/version — API version info
  router.get("/docs/version", (_req, res) => {
    res.json({
      version: API_VERSION,
      deprecated_endpoints: Object.keys(DEPRECATED_ENDPOINTS).length,
      spec_url: "/api/docs",
      ui_url: "/api/docs/ui",
    });
  });
...
/** Swagger UI HTML page using CDN resources */
...
function getOpenApiSpec() {
  return {
    ...
    paths: {
      ...
      "/dev/svi/compute": {
        post: { summary: "Compute SVI from live system state", tags: ["Dev"], requestBody: jsonBody([]), responses: stdResponse() }
      },
      "/dev/svi/read": {
        get: { summary: "Read live SVI report with auto-refresh", tags: ["Dev"], responses: stdResponse() }
      },
      "/dev/svi/summary": {
        get: { summary: "Read compact SVI summary with auto-refresh", tags: ["Dev"], responses: stdResponse() }
      },
      "/sp/brainstorm": {
