...
export function createEdmRouter(callTool: CallToolFn): Router {
  ...
  // ── Feedback Loop: Program Parsing, Calibration, Feedback ────────

  router.post("/parse-program",
    requirePermission("edm:read"),
    async (req, res, next) => {
      try { res.json(await invoke("wedm_parse_program", req.body)); } catch (e) { next(e); }
    },
  );
  router.post("/calibration-report",
    requirePermission("edm:read"),
    async (req, res, next) => {
      try { res.json(await invoke("wedm_calibration_report", req.body)); } catch (e) { next(e); }
    },
  );
  router.post("/submit-feedback",
    requirePermission("edm:write"),
    async (req, res, next) => {
      try { res.json(await invoke("wedm_submit_feedback", req.body)); } catch (e) { next(e); }
    },
  );
   ...
   * POST /photo-to-quote
   *
   * Send a photo of a blueprint → get a cost estimate WITHOUT generating
   * a full NC program. Fast path for quoting.
   *
   * Input: { image_base64: string, material?: string, thickness_mm?: number }
   * Returns: { material_detected, thickness_mm, passes, cost_estimate, ... }
   */
  router.post("/photo-to-quote",
    requirePermission("edm:read"),
    rateLimitMiddleware("RL-EDM-COMPUTE", "user"),
    async (req, res, next) => {
      try { res.json(await invoke("wedm_photo_to_quote", req.body)); } catch (e) { next(e); }
    },
  );
   ...
   *   step_content?: string,          // STEP/AP203/AP214 ASCII content
   *   iges_content?: string,          // IGES ASCII content (entity 110/100)
  ...
  router.post("/photo-to-program",
    ...
    async (req, res, next) => {
      try {
        ...
        // Skip OCR if any geometry source is already provided
        if (req.body.image_base64 && !req.body.dxf_content && !req.body.contours && !req.body.step_content && !req.body.iges_content) {
          const { blueprintVisionOCREngine } = await import("../engines/BlueprintVisionOCREngine.js");
