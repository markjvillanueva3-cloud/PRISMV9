 ...
 *
 * Response contract: a SUCCESS result is returned VERBATIM (callTool already parses the MCP text
 * payload), so the client's unwrapBusiness() sees the dispatcher's real shape — either
...
export function createBusinessRouter(callTool: CallToolFn): Router {
  ...
  router.post("/dispatch", verifyToken, async (req, res) => {
    ...
    try {
      const result = await callTool("prism_business", action, params);
