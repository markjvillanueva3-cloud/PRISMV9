...
export function createTravelerRouter(): Router {
  ...
  // SECURITY (U-ERP-SHOPCONFIG-AUTH): gate the entire traveler + dispatch
  // surface. Mounted under the global optionalToken (index.ts) which never
  // rejects anon, so before this line the dispatch board (whole-shop
  // machine-queue/job state) and every mutation was anonymously reachable.
  // Any authed operator may work the traveler + read the board; the dispatch
  // WRITE mutations (assign/reorder/remove) carry requireRole("lead"+).
  ...
  /** Assign (queue) a job to a machine. (floor scheduling: lead+) */
  router.post("/dispatch/assign", requireRole("lead", "hr_manager", "admin"), (req, res) => {
    try {
  ...
  /** Reorder the queue for a machine. (floor scheduling: lead+) */
  router.post("/dispatch/reorder", requireRole("lead", "hr_manager", "admin"), (req, res) => {
    try {
  ...
  /** Remove a queued job from a machine. (floor scheduling: lead+) */
  router.post("/dispatch/remove", requireRole("lead", "hr_manager", "admin"), (req, res) => {
    try {
