...
export function createErpRouter(callTool: CallToolFn): Router {
  ...
  // and hands the route the RAW {type,text}. So we MUST parse the envelope first, then read .success/.data
  // -- otherwise res.data is the {type,text} object, the FE's Array.isArray is false, and the inbox is
  // permanently empty. A {success:false} (engine threw) is surfaced as 400 so the FE catch fires.
  const unwrapEnvelope = (r: any): { success?: boolean; data?: unknown; error?: string } => {
  ...
  router.get("/kaizen-suggestions", verifyToken, async (req, res) => {
    try {
      const result = await callTool("prism_business", "kaizen_list_suggestions", req.query);
      res.json({ ok: true, data: result });
    } catch (e: any) { res.status(500).json({ ok: false, error: e.message }); }
  });
  ...
  router.get("/dispatch-board", verifyToken, async (req, res) => {
    try {
      const result = await callTool("prism_business", "dispatch_get_all_queues", req.query);
      res.json({ ok: true, data: result });
    } catch (e: any) { res.status(500).json({ ok: false, error: e.message }); }
  });
  ...
  router.get("/a3-report-list", verifyToken, async (req, res) => {
    try {
      const result = await callTool("prism_business", "a3_report_list", {
  ...
  router.get("/a3-report/:id", verifyToken, async (req, res) => {
    try {
      const result = await callTool("prism_business", "a3_report_get", { id: req.params.id });
      res.json({ ok: true, data: result });
    } catch (e: any) { res.status(500).json({ ok: false, error: e.message }); }
  });
  ...
  router.get("/revenue-forecast", verifyToken, requireRole("admin"), async (req, res) => {
    try {
      const result = await callTool("prism_business", "revenue_forecast", {
  ...
  router.get("/cash-flow", verifyToken, requireRole("admin"), async (req, res) => {
    try {
      const result = await callTool("prism_business", "cash_flow_summary", { months: req.query.months, as_of: req.query.as_of });
      res.json({ ok: true, data: result });
    } catch (e: any) { res.status(500).json({ ok: false, error: e.message }); }
  });
  router.get("/top-customers", verifyToken, requireRole("admin"), async (req, res) => {
    try {
      ...
      const result = await callTool("prism_business", "jm_db_top_customers", { n: req.query.n, limit: req.query.limit });
      res.json({ ok: true, data: result });
    } catch (e: any) { res.status(500).json({ ok: false, error: e.message }); }
  });
  ...
  router.get("/margin-trends", verifyToken, requireRole("admin"), async (req, res) => {
    try {
      const result = await callTool("prism_business", "margin_trends", { months: req.query.months, as_of: req.query.as_of });
      res.json({ ok: true, data: result });
    } catch (e: any) { res.status(500).json({ ok: false, error: e.message }); }
  });
  ...
  router.get("/oee-six-losses", verifyToken, async (req, res) => {
    try {
      ...
      const result = await callTool("prism_business", "oee_calculate", input);
      res.json({ ok: true, data: result });
    } catch (e: any) { res.status(500).json({ ok: false, error: e.message }); }
  });
  ...
  router.get("/tool-crib-search", verifyToken, async (req, res) => {
    ...
    try {
      const result = await callTool("prism_business", "erp_tool_search", { query, category: req.query.category });
      res.json({ ok: true, data: result });
    } catch (e: any) { res.status(500).json({ ok: false, error: e.message }); }
  });
  router.get("/tool-crib-substitutes", verifyToken, async (req, res) => {
    try {
      const result = await callTool("prism_business", "tool_inv_suggest_substitutes", {
  ...
  router.get("/tool-cost-per-part", verifyToken, async (req, res) => {
    try {
      ...
      const result = await callTool("prism_business", "tool_cost_per_part", input);
      res.json({ ok: true, data: result });
    } catch (e: any) { res.status(500).json({ ok: false, error: e.message }); }
  });
  router.get("/tool-life-forecast", verifyToken, async (req, res) => {
    try {
      ...
      const result = await callTool("prism_business", "report_tool_life_forecast", {
  ...
  router.get("/quality-metrics", verifyToken, async (req, res) => {
    ...
    try {
      const result = await callTool("prism_business", "erp_quality_metrics", { work_order_number: wo });
      res.json({ ok: true, data: result });
    } catch (e: any) { res.status(500).json({ ok: false, error: e.message }); }
  });
  router.get("/quality-inspections-by-type", verifyToken, async (req, res) => {
    ...
    if (!wo) { res.json({ ok: true, data: { data_available: false, note: "provide ?work_order_number" } }); return; }
    try {
      const result = await callTool("prism_business", "erp_quality_inspections_by_type", { work_order_number: wo });
      res.json({ ok: true, data: result });
    } catch (e: any) { res.status(500).json({ ok: false, error: e.message }); }
  });
  router.get("/quality-open-ncrs", verifyToken, async (req, res) => {
    try {
      const result = await callTool("prism_business", "erp_quality_open_ncrs", { work_order_number: req.query.work_order_number ?? req.query.wo });
      res.json({ ok: true, data: result });
    } catch (e: any) { res.status(500).json({ ok: false, error: e.message }); }
  });
  ...
  router.get("/schedule-job-status", verifyToken, async (req, res) => {
    ...
    try {
      const result = await callTool("prism_business", "milestone_get_timeline", { job_id: jobId });
      res.json({ ok: true, data: result });
    } catch (e: any) { res.status(500).json({ ok: false, error: e.message }); }
  });
