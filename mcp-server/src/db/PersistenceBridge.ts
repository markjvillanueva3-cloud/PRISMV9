...
import { writeAheadLog, type WALEntry, type WALHealth } from "./WriteAheadLog.js";
...
interface PendingWrite {
  ...
  /** WAL sequence number for crash recovery (INTEG-MS2) */
  walSeq?: number;
}
...
class PersistenceBridgeImpl {
   ...
   * INTEG-MS2: Initializes WAL and replays uncommitted entries.
   */
  async loadAll(): Promise<{ mode: "postgres" | "memory"; loaded: Record<string, number>; walRecovered: number }> {
    ...
    // INTEG-MS2: Initialize WAL and get uncommitted entries
    if (mode === "postgres") {
      const uncommitted = await writeAheadLog.init();
      walRecovered = uncommitted.length;

      // Replay uncommitted WAL entries
      if (uncommitted.length > 0) {
        console.error(`[PersistenceBridge] Replaying ${uncommitted.length} uncommitted WAL entries`);
        for (const entry of uncommitted) {
          this.pendingWrites.push({
            entity: entry.entity,
            key: entry.key,
            value: entry.op === "delete" ? null : (entry.data as StoreRecord),
            retries: 0,
            walSeq: entry.seq,
          });
        }
        // Schedule immediate flush for recovery
        this.scheduleFlush();
      }
    }

    if (mode === "memory") {
      return { mode, loaded, walRecovered: 0 };
    }
    ...
    return { mode, loaded, walRecovered };
  }
   ...
   * INTEG-MS2: Writes to WAL first for crash recovery, then queues for PostgreSQL.
   */
  persist(entity: string, key: string, value: StoreRecord | null): Promise<boolean> {
    ...
    // INTEG-MS2: Write to WAL first (crash-safe)
    const walSeq = writeAheadLog.append(
    ...
    return new Promise<boolean>((resolve) => {
      this.pendingWrites.push({ entity, key, value, retries: 0, resolve, walSeq });
      this.scheduleFlush();
    });
  }
  ...
  async flushAll(): Promise<{ flushed: number; errors: number }> {
    ...
    for (const write of writes) {
      try {
        ...
        // INTEG-MS2: Checkpoint WAL entry after successful persistence
        if (write.walSeq !== undefined && write.walSeq > 0) {
          writeAheadLog.checkpoint(write.walSeq);
        }
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        console.error(
  ...
  /** Health metrics for monitoring (C2 fix, INTEG-MS2 WAL). */
  getHealth(): {
    mode: string;
    initialized: boolean;
  ...
  async gracefulShutdown(timeoutMs: number = SHUTDOWN_TIMEOUT_MS): Promise<boolean> {
    console.error(`[PersistenceBridge] Graceful shutdown: ${this.pendingWrites.length} pending writes, timeout=${timeoutMs}ms`);
    ...
    console.error(`[PersistenceBridge] Shutdown complete: flushed=${this.totalFlushed}, errors=${this.totalErrors}`);
    return allFlushed;
  }

  /** M8 fix: full reset for test isolation. INTEG-MS2: also reset WAL. */
  reset(): void {
    if (this.flushTimer) {
      clearTimeout(this.flushTimer);
      this.flushTimer = null;
