...
export interface UpgradeV2Input {
  ...
  operation?: "turning" | "drilling" | "tapping" | "reaming" | "milling" | "boring" | "thread_milling";
...
export class JMDieLatheProgramUpgraderV2Engine {
  ...
  static async upgradeOne(input: UpgradeV2Input): Promise<UpgradeV2Result> {
    ...
    const variants: UpgradeV2Variant[] = JM_DIE_LATHES.map((lathe) => {
      ...
      const sfmEff = result?.cutting_speed?.value ? Math.round(result.cutting_speed.value * 3.281) : 180;
      const feedPerRev = result?.feed_per_rev?.value ?? 0.13;
      const feedrate = result?.feed_rate?.value ?? Math.round(rpmValue * 0.13);
      const feedConfidence = result?.feed_rate?.confidence ?? 0;
      const doc = result?.axial_depth?.value ?? 1.5;
