...
class QuoteAnalyticsEngine {
  ...
  recordQuote(record: QuoteRecord): QuoteRecord {
    ...
    record.part_name = sanitizeText(record.part_name, 200);
    record.customer = sanitizeText(record.customer, 200);
    if (record.part_number) record.part_number = sanitizeText(record.part_number, 100);
    if (record.material) record.material = sanitizeText(record.material, 100);
    if (record.complexity) record.complexity = sanitizeText(record.complexity, 50);
    if (record.estimator) record.estimator = sanitizeText(record.estimator, 100);
    this.quotes.set(record.quote_id, record);
    persistenceBridge.persist("quote_analytics", record.quote_id, record as any);
    return record;
  }
  ...
  updateOutcome(
    ...
    if (details?.loss_notes) q.loss_notes = sanitizeText(details.loss_notes, 500);
    if (details?.competing_price != null) q.competing_price = details.competing_price;
    persistenceBridge.persist("quote_analytics", quoteId, q as any);
    return q;
  }
...
persistenceBridge.registerMap({
  entity: "quote_analytics",
  getMap: () => (quoteAnalyticsEngine as any).quotes as Map<string, any>,
  keyField: "quote_id",
});
