...
export class GrindingWheelDressingOptimizationEngine {
  ...
  creepFeedSetup(input: CreepFeedInput): CreepFeedResult {
    ...
    // Burn risk assessment based on specific energy and contact length
    const contactLength = Math.sqrt(input.slot_depth_mm * input.wheel_diameter_mm);
    const heatFluxProxy = specificEnergy * input.slot_depth_mm * table_speed / (60 * contactLength);
