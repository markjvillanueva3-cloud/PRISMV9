...
export class PPSinkerEDMPostEngine {
  ...
  generate(input: SinkerProgramInput): SinkerProgram {
    ...
    // Machine setup
    lines.push(`G90 G21 (ABSOLUTE, METRIC)`);
    ...
    for (let i = 0; i < input.operations.length; i++) {
      ...
      // Position to start XY
      if (op.start_x !== undefined && op.start_y !== undefined) {
        lines.push(`G0 X${op.start_x.toFixed(3)} Y${op.start_y.toFixed(3)} (MOVE TO XY)`);
      }

      // Move to retract plane Z
      const startZ = op.start_z ?? 5.0;
      lines.push(`G0 Z${startZ.toFixed(3)} (RETRACT PLANE)`);
      ...
      if (orbitPat !== "none" && orbitR > 0) {
        lines.push(`(ORBIT: ${orbitPat.toUpperCase()} RADIUS=${orbitR.toFixed(3)}mm)`);
        lines.push(`${this.getOrbitCode(orbitPat)} R${orbitR.toFixed(3)} (ORBIT ENABLE)`);
      }
      ...
      if (flushCycle > 0) {
        ...
        lines.push(`G83 Z${finalZ.toFixed(3)} Q${retract.toFixed(3)} F${flushCycle.toFixed(1)} (PECK BURN W/ FLUSH)`);
      } else {
        // Straight servo plunge
        lines.push(`G81 Z${finalZ.toFixed(3)} (SINKER SERVO PLUNGE)`);
      }
      ...
      if (orbitPat !== "none" && op.orbit_revolutions && op.orbit_revolutions > 0) {
        ...
        for (let r = 0; r < op.orbit_revolutions; r++) {
          lines.push(`G03 I${orbitR.toFixed(3)} J0 (ORBIT REV ${r + 1})`);
        }
      }
      ...
      // Retract
      lines.push(`G0 Z${startZ.toFixed(3)} (RETRACT)`);
    ...
    // Program end
    lines.push(``);
    lines.push(`G0 Z50 (FULL RETRACT)`);
