...
export interface MillingTraceEntry {
  schemaVersion: 1;
  id: string;
  at: string;
  dispatcher: string;
  ...
  reasoning_steps?: MillingReasoningStep[];
  physics_validated?: boolean;
  physics_warnings?: string[];
  error?: string;
}

export interface MillingLedgerStats {
  totalTraces: number;
...
export interface MillingLedgerAppendResult {
  ok: boolean;
  entry?: MillingTraceEntry;
  errors?: string[];
}
...
export class MillingReasoningTraceLedgerEngine {
  private ring: MillingTraceEntry[] = [];
  private nextId = 1;
  ...
  validate(candidate: Partial<MillingTraceEntry>): { ok: true } | { ok: false; errors: string[] } {
    const errors: string[] = [];
  ...
  async recordTrace(
    partial: Omit<MillingTraceEntry, "schemaVersion" | "id" | "at">,
  ): Promise<MillingLedgerAppendResult> {
    const entry: MillingTraceEntry = {
      schemaVersion: 1,
      id: this.generateId(),
    ...
    if (this.diskWritesEnabled) {
      try {
        ...
        log.warn(`[milling-trace-ledger] disk write failed (in-memory preserved): ${(e as Error).message}`);
      }
    }
  ...
  recordTraceSync(
    ...
    if (this.diskWritesEnabled) {
      void appendToFile(this.ledgerPath, JSON.stringify(entry) + "\n").catch((e) => {
        log.warn(`[milling-trace-ledger] async disk write failed: ${(e as Error).message}`);
      });
    }
    return { ok: true, entry };
  }

  getRecent(limit = 100): MillingTraceEntry[] {
    return this.ring.slice(-limit);
  }

  queryByDispatcher(dispatcher: string, limit = 100): MillingTraceEntry[] {
    return this.ring.filter((e) => e.dispatcher === dispatcher).slice(-limit);
  }

  queryByAction(action: string, limit = 100): MillingTraceEntry[] {
    return this.ring.filter((e) => e.action === action).slice(-limit);
  }

  queryByKeyword(keyword: string, limit = 100): MillingTraceEntry[] {
    const kw = keyword.toLowerCase();
    return this.ring.filter((e) => e.keywords.some((k) => k.toLowerCase().includes(kw))).slice(-limit);
  }

  queryWithReasoning(limit = 100): MillingTraceEntry[] {
    return this.ring.filter((e) => e.reasoning_steps && e.reasoning_steps.length > 0).slice(-limit);
  }

  getStats(): MillingLedgerStats {
    const now = Date.now();
    ...
    for (const e of this.ring) {
      ...
      if (e.reasoning_steps && e.reasoning_steps.length > 0) withReasoning++;
    }
    ...
    return {
      ...
      reasoningCoverage: this.ring.length === 0 ? 0 : Math.round((withReasoning / this.ring.length) * 1000) / 10,
      lastTraceAt,
      silentMinutes,
    };
  }
...
export const millingReasoningTraceLedgerEngine = new MillingReasoningTraceLedgerEngine();
