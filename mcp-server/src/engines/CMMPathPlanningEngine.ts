...
export class CMMPathPlanningEngine {
   ...
   * Uses the characteristic polynomial and inverse iteration.
   */
  private _smallestEigenvector3x3(
    a11: number, a12: number, a13: number,
    a22: number, a23: number, a33: number,
  ): Point3D {
    // Characteristic polynomial coefficients: λ³ - pλ² + qλ - r = 0
    const p = a11 + a22 + a33; // trace
    const q = a11 * a22 + a11 * a33 + a22 * a33 - a12 * a12 - a13 * a13 - a23 * a23;
    const r = a11 * a22 * a33 + 2 * a12 * a23 * a13
            - a11 * a23 * a23 - a22 * a13 * a13 - a33 * a12 * a12; // determinant

    // Solve cubic for eigenvalues using trigonometric method
    const p3 = p / 3;
    const q2 = (p * p - 3 * q) / 9;
    const r2 = (2 * p * p * p - 9 * p * q + 27 * r) / 54;

    let eigenvalues: number[];
    const disc = r2 * r2 - q2 * q2 * q2;
    ...
    if (disc <= 0) {
      // Three real roots
      const theta = Math.acos(Math.max(-1, Math.min(1, r2 / Math.pow(q2, 1.5))));
      const sqrtQ = Math.sqrt(q2);
      eigenvalues = [
      ...
      // One real root (shouldn't happen for symmetric matrix, but handle)
      const A = -Math.sign(r2) * Math.pow(Math.abs(r2) + Math.sqrt(disc), 1 / 3);
      const B = Math.abs(A) > 1e-15 ? q2 / A : 0;
      eigenvalues = [A + B + p3];
    }

    // Find smallest eigenvalue
    const lambda = Math.min(...eigenvalues);

    // Inverse iteration: solve (A - λI)v = b for eigenvector
    // Shift matrix
    const m11 = a11 - lambda, m22 = a22 - lambda, m33 = a33 - lambda;

    // Try cross-product of two rows to find normal
    // Row 1 × Row 2
    let nx = a12 * a23 - a13 * m22;
    let ny = a13 * a12 - m11 * a23;
    let nz = m11 * m22 - a12 * a12;
