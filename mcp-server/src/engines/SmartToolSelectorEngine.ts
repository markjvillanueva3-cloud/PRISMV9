...
let _toolCatalogEngine: any = null;
let _toolSelectionEngine: any = null;
let _machineProfileEngine: any = null;
let _kienzleForceEngine: any = null;
let _playbookEngine: any = null;
let _insertGradeEngine: any = null;

function getToolCatalog() {
  if (!_toolCatalogEngine) {
    const m = require("./ToolCatalogEngine.js");
    _toolCatalogEngine = m.toolCatalogEngine ?? m.default ?? new m.ToolCatalogEngine();
  }
  return _toolCatalogEngine;
}
function getToolSelection() {
  if (!_toolSelectionEngine) {
    const m = require("./ToolSelectionEngine.js");
    _toolSelectionEngine = m.toolSelectionEngine ?? m.default ?? new m.ToolSelectionEngine();
  }
  return _toolSelectionEngine;
}
function getMachineProfile() {
  if (!_machineProfileEngine) {
    const m = require("./MachineProfileEngine.js");
    _machineProfileEngine = m.machineProfileEngine ?? m.default ?? new m.MachineProfileEngine();
  }
  return _machineProfileEngine;
}
function getKienzleForce() {
  if (!_kienzleForceEngine) {
    const m = require("./KienzleForceModelEngine.js");
    _kienzleForceEngine = m.kienzleForceModelEngine ?? m.default ?? new m.KienzleForceModelEngine();
  }
  return _kienzleForceEngine;
}
function getPlaybook() {
  if (!_playbookEngine) {
    const m = require("./MachiningPlaybookEngine.js");
    _playbookEngine = m.machiningPlaybookEngine ?? m.default ?? new m.MachiningPlaybookEngine();
  }
  return _playbookEngine;
}
function getInsertGrade() {
  if (!_insertGradeEngine) {
    const m = require("./InsertGradeSelectionEngine.js");
    _insertGradeEngine = m.insertGradeSelectionEngine ?? m.default ?? new m.InsertGradeSelectionEngine();
  }
  return _insertGradeEngine;
}

// ── Kienzle coefficients by ISO group ──────────────────────────
const KIENZLE_TABLE: Record<string, { kc1_1: number; mc: number }> = {
  P: { kc1_1: 1780, mc: 0.26 },  // Carbon/alloy steel
  ...
  H: { kc1_1: 4000, mc: 0.22 },  // Hardened steel
};
...
export class SmartToolSelectorEngine {
  ...
  select(req: SmartToolRequest): SmartToolSelectionOutput {
    ...
    const kienzle = KIENZLE_TABLE[iso] || KIENZLE_TABLE.P;
  ...
  private _scoreTool(
    ...
    // Tool deflection: δ = FL³/(3EI), E=580GPa carbide, I=πd⁴/64
