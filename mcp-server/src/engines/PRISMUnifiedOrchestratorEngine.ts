...
export interface TribalKnowledgeContext {
  ...
  tribal_risks: Array<{ risk: string; severity: "low" | "medium" | "high"; source: string }>;
...
export class PRISMUnifiedOrchestratorEngine {
  ...
  async assessPhysicalFeasibility(
    ...
    return feasibilityOrchestratorEngine.assessForPUOA(input);
  }
  ...
  async assessSequenceFeasibility(
    ...
    return sequenceFeasibilityEngine.assessSequenceForPUOA(input);
  }
  ...
  async suggestResequencing(
    ...
    return sequenceFeasibilityEngine.suggestResequencingForPUOA(input);
  }
  ...
  private buildTribalNLQuery(input: PUOAInput, routing: TierRoutingResult): string {
    ...
    if (input.context?.material) {
      parts.push(input.context.material);
    }

    // Add machine from context
    if (input.context?.machine) {
      parts.push(input.context.machine);
    }
  ...
  consultTribalKnowledge(input: PUOAInput, routing: TierRoutingResult): TribalKnowledgeContext {
    ...
    const modifiers = tribalKnowledgeAdvisorEngine.getModifiers(queryContext);
    const constraints = tribalKnowledgeAdvisorEngine.getConstraints(queryContext);
    const advisory = tribalKnowledgeAdvisorEngine.getAdvisory(queryContext);
