...
export class AdaptiveCalibrationEngine {
  ...
  taylorCoefficientTracker(input: TaylorTrackerInput): TaylorTrackerResult {
    ...
    // State: [ln(C), n]
    let x0 = Math.log(priorC);
    let x1 = priorN;

    // Initial state covariance — moderate uncertainty
    let P00 = 1.0, P01 = 0.0, P10 = 0.0, P11 = 0.1;

    // Process noise (diagonal)
    const Q00 = processNoise;
    const Q11 = processNoise * 0.1; // n drifts less than C
    ...
    for (const obs of observations) {
      ...
      const z = Math.log(obs.toolLife_min);
      ...
      // Innovation: y = z - H·x
      const zPred = H0 * x0 + H1 * x1;
      const innovation = z - zPred;
      residuals.push(innovation);

      // Innovation covariance: S = H·P⁻·Hᵀ + R
      const S = H0 * (P00 * H0 + P01 * H1) + H1 * (P10 * H0 + P11 * H1) + R;

      // Kalman gain: K = P⁻·Hᵀ / S
      const K0 = (P00 * H0 + P01 * H1) / S;
      const K1 = (P10 * H0 + P11 * H1) / S;
      kalmanGains.push(Math.sqrt(K0 * K0 + K1 * K1)); // gain magnitude
      ...
      const newP00 = IKH00 * P00 + IKH01 * P10;
      const newP01 = IKH00 * P01 + IKH01 * P11;
      const newP10 = IKH10 * P00 + IKH11 * P10;
      const newP11 = IKH10 * P01 + IKH11 * P11;

      P00 = newP00; P01 = newP01;
      P10 = newP10; P11 = newP11;

      stateHistory.push({ C: Math.exp(x0), n: x1 });
    }

    const updatedC = Math.exp(x0);
    const updatedN = x1;
    // Standard deviations from diagonal of P
    // For C: σ_C ≈ C · σ_lnC (delta method)
    const cStdDev = updatedC * Math.sqrt(Math.max(0, P00));
    const nStdDev = Math.sqrt(Math.max(0, P11));
    ...
    return {
      ...
      residuals,
      stateHistory,
      predictionAtSpeed: (v: number) => updatedC / Math.pow(v, updatedN)
    };
  }
