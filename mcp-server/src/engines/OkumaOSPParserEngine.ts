...
/** Enhanced S/F extraction with per-operation detail (KAR-MS2 U-KAR11) */
export interface DetailedSpeedFeed {
  /** Source file path */
  filePath: string;
  ...
  /** G50 max RPM clamp */
  maxRPM?: number;
  /** All feed rates found in this operation */
  feedRates: Array<{
    value: number;
    gcode: string;
    line: number;
    context: "roughing" | "finishing" | "drilling" | "threading" | "general";
  }>;
  /** Feed mode */
  feedMode: "per_rev" | "per_min";
  /** Has canned cycle (G85/G87/G71/G72/G74) */
  hasCannedCycle: boolean;
  /** Canned cycle parameters if present */
  cycleParams?: {
    depthOfCut?: number;
    finishStock?: number;
    peckDepth?: number;
...
/** Batch parse result for JM Die archive processing */
export interface BatchParseResult {
  ...
  /** Aggregated speed/feed data */
  speedFeeds: DetailedSpeedFeed[];
...
class OkumaOSPParserEngineImpl {
  ...
  /**
   * Extract detailed speed/feed data per operation (KAR-MS2 U-KAR11)
   * Returns all S/F occurrences with full context for proven parameter mining.
   */
  extractDetailedSpeedFeeds(program: OkumaProgram, filePath: string = ""): DetailedSpeedFeed[] {
    const results: DetailedSpeedFeed[] = [];

    for (const section of program.toolSections) {
      // Classify operation type from comment
      const opType = this.classifyOperationType(section.comment);

      // Collect all feed rates from operations
      const feedRates: DetailedSpeedFeed["feedRates"] = [];
      let hasCannedCycle = false;
      const cycleParams: DetailedSpeedFeed["cycleParams"] = {};

      for (const op of section.operations) {
        // Track feed rates
        if (op.params.feed !== undefined) {
          feedRates.push({
            value: op.params.feed as number,
            gcode: op.gcode,
            line: op.line,
            context: this.classifyFeedContext(op.type),
          });
        }

        // Track canned cycle params
        if (["od_rough", "id_rough"].includes(op.type)) {
          hasCannedCycle = true;
          if (op.params.depth_of_cut) cycleParams.depthOfCut = op.params.depth_of_cut as number;
          if (op.params.finish_x) cycleParams.finishStock = op.params.finish_x as number;
        }
        if (op.type === "peck_drill" && op.params.peck) {
          hasCannedCycle = true;
          cycleParams.peckDepth = op.params.peck as number;
        }
        if (op.type === "thread" && op.params.pitch) {
          hasCannedCycle = true;
          cycleParams.pitch = op.params.pitch as number;
        }
      }
      ...
      results.push({
        ...
        offsetNumber: section.offsetNumber,
        operationDescription: section.comment,
        operationType: opType,
        cssSpeed: section.cssValue,
        directRPM: section.rpmValue,
        maxRPM: section.maxRPM,
        feedRates,
        feedMode: "per_rev", // Okuma G95 default
        hasCannedCycle,
        cycleParams: Object.keys(cycleParams).length > 0 ? cycleParams : undefined,
      });
    }
  ...
  /**
   * Classify operation type from NAT comment
   */
  private classifyOperationType(comment: string): OkumaOpType {
    const lower = comment.toLowerCase();
    if (lower.includes("rough") && lower.includes("od")) return "od_rough";
    if (lower.includes("fin") && lower.includes("od")) return "od_finish";
    if (lower.includes("rough") && lower.includes("id")) return "id_rough";
    if (lower.includes("fin") && lower.includes("id")) return "id_finish";
    if (lower.includes("bore")) return "bore";
    if (lower.includes("drill") && lower.includes("center")) return "center_drill";
    if (lower.includes("drill")) return "drill";
    if (lower.includes("thread")) return "thread";
    if (lower.includes("groove") || lower.includes("grv")) return "groove";
    if (lower.includes("cutoff") || lower.includes("part")) return "cutoff";
    if (lower.includes("face")) return "face";
    return "unknown";
  }

  /**
   * Classify feed context from operation type
   */
  private classifyFeedContext(opType: OkumaOpType): "roughing" | "finishing" | "drilling" | "threading" | "general" {
    if (["od_rough", "id_rough"].includes(opType)) return "roughing";
    if (["od_finish", "id_finish", "bore_finish"].includes(opType)) return "finishing";
    if (["drill", "peck_drill", "center_drill"].includes(opType)) return "drilling";
    if (opType === "thread") return "threading";
    return "general";
  }

  /**
   * Batch parse multiple files for JM Die archive processing (KAR-MS2 U-KAR11)
   */
  batchParse(filePaths: string[]): BatchParseResult {
    ...
    for (const filePath of filePaths) {
      try {
        ...
        const program = this.parse(source, filePath);
        const extracted = this.extractDetailedSpeedFeeds(program, filePath);
  ...
  /**
   * Parse a sample set of files to estimate parse success rate
   */
  estimateParseRate(filePaths: string[], sampleSize: number = 100): { successRate: number; sampleSize: number; errors: string[] } {
    const sample = filePaths.slice(0, Math.min(sampleSize, filePaths.length));
    const result = this.batchParse(sample);
    return {
      successRate: result.successCount / result.totalFiles,
