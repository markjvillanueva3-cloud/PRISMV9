...
export class ProspectiveCustomerEngine {
  ...
  /**
   * Per-stage close probability for the OPEN funnel stages (cold..quoted).
   * Standard B2B-sales-funnel weighted-pipeline convention: confidence rises as a prospect
   * advances toward close. Values are conventional CRM stage-weights (e.g. HubSpot/Salesforce
   * default deal-stage probabilities), not measured constants -- documented so they are not magic.
   * won/lost are terminal (excluded from the open pipeline) and carry no forecast probability.
   */
  private static readonly STAGE_PROBABILITY: Readonly<Record<Exclude<ProspectStatus, "won" | "lost">, number>> = {
    cold: 0.05,
    researched: 0.10,
    first_contact: 0.20,
    engaged: 0.40,
  ...
  /**
   * FE Forecast object backing SalesPipelinePage's summary tiles.
   * Composes pipelineReport() (R8 -- single source) into the page's exact shape:
   *   pipeline_value   = open pipeline (won/lost excluded) -- total_pipeline_value_usd
   *   conversion_rate  = won / (won + lost), the historical close rate (0 when no closed deals)
   *   forecast_30d/90d = probability-weighted open value; the nearer-close stages (quoted/engaged)
   *                      dominate, so 30d weights ONLY quoted+engaged, 90d weights the whole open funnel
   *   backlog_value    = closed-won annual spend (revenue already committed)
   */
  pipelineForecast(): {
    pipeline_value: number; conversion_rate: number; forecast_30d: number;
    forecast_90d: number; backlog_value: number;
  } {
    const r = this.pipelineReport();
    const bs = r.by_status;
    const round2 = (x: number): number => Math.round(x * 100) / 100;
    const wonCount = bs.won.count;
    const closed = wonCount + bs.lost.count;
    const conversion = closed === 0 ? 0 : wonCount / closed;
    const P = ProspectiveCustomerEngine.STAGE_PROBABILITY;
    // 90-day forecast: every open stage weighted by its close probability.
    const forecast90 =
      bs.cold.est_spend_total * P.cold +
      bs.researched.est_spend_total * P.researched +
      bs.first_contact.est_spend_total * P.first_contact +
      bs.engaged.est_spend_total * P.engaged +
      bs.quoted.est_spend_total * P.quoted;
    // 30-day forecast: only the near-close stages (engaged + quoted) realistically close inside a month.
    const forecast30 = bs.engaged.est_spend_total * P.engaged + bs.quoted.est_spend_total * P.quoted;
    return {
      pipeline_value: r.total_pipeline_value_usd,
      conversion_rate: Number(conversion.toFixed(4)),
      forecast_30d: round2(forecast30),
      forecast_90d: round2(forecast90),
      backlog_value: round2(bs.won.est_spend_total),
    };
  }

  /**
   * FE PipelineStage[] backing SalesPipelinePage's stage-breakdown funnel.
   * One row per OPEN stage (cold..quoted; won/lost excluded as terminal), shape:
   *   { stage, count, value (est_spend_total), weighted_value (value*probability), probability_pct }
   * Ordered along the funnel (cold -> quoted) so the page renders a descending funnel.
   */
  pipelineStages(): ReadonlyArray<{
    stage: ProspectStatus; count: number; value: number; weighted_value: number; probability_pct: number;
  }> {
    const bs = this.pipelineReport().by_status;
    const P = ProspectiveCustomerEngine.STAGE_PROBABILITY;
    const order: Array<Exclude<ProspectStatus, "won" | "lost">> = [
      "cold", "researched", "first_contact", "engaged", "quoted",
    ];
    return Object.freeze(order.map((stage) => {
      const prob = P[stage];
      const value = bs[stage].est_spend_total;
      return Object.freeze({
        stage,
        count: bs[stage].count,
        value,
        weighted_value: Math.round(value * prob * 100) / 100,
