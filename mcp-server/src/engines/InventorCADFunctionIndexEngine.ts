 ...
 *      Machining post-weld features) wrapped around Convert-to-Weldment.
 *
 * This engine is pure: lazy JSON load + in-process cache, no I/O beyond
 * initial read, no state mutation, no dispatcher imports.
 *
 * Coverage state: FOUNDATION — 1 module / 28 operations / 138 parameters.
 *   U-CAD-FIDX-INV-01 sketch_operations    28 ops / 138 params (this unit)
 *   U-CAD-FIDX-INV-02..08                  pending (see future_modules)
 *
 * @see data/cad-functions/inventor/function-index.json
 * @see HyperCADCADFunctionIndexEngine (sister CAD function-index, 8/8 modules complete)
 * @see Fusion360CADFunctionIndexEngine (sister CAD function-index, 5/5 modules complete)
 */
...
export interface InventorCADFunctionIndex {
...
// data root sits at mcp-server/data/cad-functions/inventor either way (relative to src or dist)
const CATALOG_ROOT = resolve(__dirname, "..", "..", "data", "cad-functions", "inventor");
...
/**
 * InventorCADFunctionIndexEngine — static API for Autodesk Inventor function/parameter lookup.
 *
 * Mirrors the API surface of HyperCADCADFunctionIndexEngine + Fusion360CADFunctionIndexEngine:
 ...
 *   - getTotalParameterCount / getLoadErrors — discovery / drift surface
 *   - clearCache — test isolation
 */
export class InventorCADFunctionIndexEngine {
  private static indexCache: InventorCADFunctionIndex | null = null;
   ...
   */
  static getIndex(): InventorCADFunctionIndex {
    if (!this.indexCache) {
      if (!existsSync(INDEX_PATH)) {
        throw new Error(`Autodesk Inventor CAD function index not found: ${INDEX_PATH}`);
      }
      this.indexCache = readJson<InventorCADFunctionIndex>(INDEX_PATH);
    }
