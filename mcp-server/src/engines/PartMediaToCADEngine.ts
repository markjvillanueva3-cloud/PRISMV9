 ...
 *
 * Reuses:
 *   • free Ollama-first `llmEngine.queryVision` (same pattern BlueprintVisionOCREngine + VisionActionAnalyzerEngine now use)
 ...
 *   2. Per-frame vision LLM call (Ollama-first)→ coarse part-primitive hypothesis
...
export interface PartMediaInput {
  ...
  /** Advisory model hint (the llmEngine vision ladder selects the provider). */
  model?: string;
...
const MAX_FRAMES = 24;
/** Bounded timeout (ms) for fetching a kind:"url" image, so a hung/slow remote host degrades
 *  to a per-frame warning instead of stalling. The old Anthropic server-side URL fetch had its
 *  own timeout budget; the in-engine fetch must supply its own. */
...
export class PartMediaToCADEngine {
  ...
  async generateFromMedia(input: PartMediaInput): Promise<PartMediaResult> {
    ...
    // FREE-AI-MIGRATION/U-PARTMEDIA-TO-CAD-LLM-ROUTE: no ANTHROPIC_API_KEY gate, no Anthropic
    // client. Each frame routes through the free Ollama-first llmEngine.queryVision substrate
    // (Ollama vision -> Claude backup -> offline). input.model is advisory; the ladder picks the
    // provider, and we report the REAL provider in meta.model (R12). Per-frame offline is caught
    // -> warning + emptyPrimitive (this engine's always-returns-degraded contract).
    ...
    // ── 2. Per-frame vision LLM pass (Ollama-first) ──
    const perFrame: PartPrimitive[] = [];
    let visionCalls = 0;
    const hintsBlock = this.buildHintsBlock(input.hints);
    for (let i = 0; i < capped.length; i++) {
      try {
        const { primitive, model: usedModel } = await this.analyzeFrame(capped[i], hintsBlock, i);
        perFrame.push(primitive);
    ...
    if (visionCalls === 0) {
      warnings.push(
        "NO vision provider answered any frame (offline) -- this scaffold is NOT based on real part data; do not machine from it (R12)",
      );
    }
  ...
  private async analyzeFrame(
    ...
    hintsBlock: string,
    frameIdx: number,
  ): Promise<{ primitive: PartPrimitive; model: string }> {
    const { llmEngine } = await import("./LLMEngine.js");
    const image = await this.imageSourceToVisionImage(source);
    ...
    if (res.model === "offline") {
      throw new Error(
        "No vision AI provider available (Ollama vision model down and no Claude backup key) -- PartMediaToCAD requires a real provider for part-photo analysis.",
      );
    }
    return { primitive: this.parseVisionJson(res.answer), model: res.model };
  }

  /** Resolve an ImageSource to a VisionImage ({data: base64, media_type}) for llmEngine.queryVision.
   *  URL sources are fetched here -> base64 (the old Anthropic server-side URL fetch is gone; the
   *  Ollama vision path needs the raw bytes). Pure except the file read / URL fetch. */
  private async imageSourceToVisionImage(source: ImageSource): Promise<{ data: string; media_type: string }> {
    if (source.kind === "base64") {
      return { data: source.data, media_type: source.mediaType };
    }
    if (source.kind === "file") {
    ...
    // url -- fetch the bytes -> base64 (Ollama vision cannot fetch a remote URL itself).
    // Bounded timeout: a hung host -> AbortError -> caught per-frame -> warning (never stalls).
    const resp = await fetch(source.url, { signal: AbortSignal.timeout(URL_FETCH_TIMEOUT_MS) });
    if (!resp.ok) throw new Error(`image URL fetch failed: ${resp.status} ${source.url}`);
    const buf = Buffer.from(await resp.arrayBuffer());
    return { data: buf.toString("base64"), media_type: source.mediaType ?? "image/png" };
  }
