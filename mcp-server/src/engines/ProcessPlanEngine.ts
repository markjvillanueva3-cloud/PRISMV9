...
export class ProcessPlanEngine {
  generate(input: ProcessPlanInput): ProcessPlan {
    ...
    // TK-MS3 U-TK18: Query tribal modifiers for speed adjustments
    const tribalMods = tribalKnowledgeAdvisorEngine.getModifiers({
      iso_group: input.material_iso_group,
      material: input.material_name,
    });

    // Apply tribal Vc modifier to base speed
    const baseSpeed = (ISO_SPEEDS[input.material_iso_group] || 200) * tribalMods.vc_modifier;
    ...
    // TK-MS2-U14: Query tribal knowledge for process planning tips
    let tribalTips: string[] = [];
    try {
      const tips = tribalKnowledgeEngine.search({
        category: "setup",
        operation_type: "process_planning",
        material_iso_group: input.material_iso_group,
        limit: 8,
      });
      for (const tip of tips) {
        tribalTips.push(`TK: ${tip.title}`);
      }
    } catch { /* tribal knowledge unavailable */ }

    // TK-MS3 U-TK18: Add tribal modifier info if modifiers were applied
    if (tribalMods.evidence_count > 0 && tribalMods.vc_modifier !== 1.0) {
      tribalTips.unshift(`TK Modifier: Vc×${tribalMods.vc_modifier.toFixed(2)} (${tribalMods.evidence_count} tips, conf=${(tribalMods.confidence * 100).toFixed(0)}%)`);
    }
