 ...
 *
 * Reference: GibbsCAM 14 Reference / 3D Systems / Hexagon Cimatron docs.
 */

import { readFileSync, existsSync } from "fs";
import { join } from "path";

export interface GibbsCAMParameter {
  type: string;
  description?: string;
  unit?: string;
  default?: unknown;
...
export interface GibbsCAMOperation {
  display_name: string;
  category: string;
  description: string;
  dialog_tabs: string[];
  parameter_count?: number;
  parameters: Record<string, Record<string, GibbsCAMParameter>>;
}

export interface GibbsCAMSection {
  ...
  operations: Record<string, GibbsCAMOperation>;
...
export interface GibbsCAMFunctionIndex {
  system_id: "gibbscam";
  sections: Record<string, GibbsCAMSection>;
  total_operations: number;
  total_parameters: number;
  categories: string[];
}
...
const SECTION_FILES: Array<{ file: string; key: string }> = [
  ...
  { file: "vmc.json", key: "vmc" },
];

export class GibbsCAMFunctionIndexEngine {
  private static index: GibbsCAMFunctionIndex | null = null;

  private static readonly DATA_DIR = join(
    __dirname,
    "../../data/cam-functions/gibbscam"
  );

  private static loadIndex(): GibbsCAMFunctionIndex {
    if (this.index) return this.index;

    const sections: Record<string, GibbsCAMSection> = {};
    let totalOps = 0;
    let totalParams = 0;
    ...
    for (const { file, key } of SECTION_FILES) {
      ...
      try {
        const data = JSON.parse(readFileSync(path, "utf-8")) as GibbsCAMSection;
        sections[key] = data;
        ...
        throw new Error(
          `GibbsCAMFunctionIndexEngine: failed to parse ${file}: ${(err as Error).message}`
        );
      }
    }
  ...
  static getIndex(): GibbsCAMFunctionIndex {
  ...
  static getSection(
    sectionKey: string
  ): GibbsCAMSection | { error: string; available: string[] } {
  ...
  /**
   * VoluMill / high-efficiency operations — GibbsCAM's licensed Celeritive
   * adaptive clearing surface. Equivalent of HSM/Waveform/Profit on peers.
   */
  static getVoluMillOperations(): Array<{
    operation_id: string;
    display_name: string;
    section: string;
    description: string;
    ...
    for (const [sectionKey, section] of Object.entries(idx.sections)) {
      for (const [opId, op] of Object.entries(section.operations || {})) {
        if (
          opId.startsWith("vomotion") ||
          op.display_name.toLowerCase().includes("volumill") ||
  ...
  /**
   * Virtual Machining Center operations — GibbsCAM's distinctive surface.
   * Returns all ops in the vmc section.
   */
  static getVMCOperations(): Array<{
    operation_id: string;
    display_name: string;
    section: string;
    category: string;
    ...
    const vmc = idx.sections["vmc"];
    if (!vmc) return out;
    for (const [opId, op] of Object.entries(vmc.operations || {})) {
      out.push({
        operation_id: opId,
        display_name: op.display_name,
        section: "vmc",
  ...
  static getOperation(operationId: string):
    | { operation_id: string; section: string; operation: GibbsCAMOperation }
    | { error: string } {
