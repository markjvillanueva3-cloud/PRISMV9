...
export interface AlphacamParameter {
  type: string;
  description?: string;
  unit?: string;
  default?: unknown;
...
export interface AlphacamOperation {
  display_name: string;
  category: string;
  description: string;
  dialog_tabs: string[];
  parameter_count?: number;
  parameters: Record<string, Record<string, AlphacamParameter>>;
}

export interface AlphacamSection {
  ...
  operations: Record<string, AlphacamOperation>;
...
export interface AlphacamFunctionIndex {
  system_id: "alphacam";
  sections: Record<string, AlphacamSection>;
  total_operations: number;
  total_parameters: number;
  categories: string[];
}
...
const SECTION_FILES: Array<{ file: string; key: string }> = [
  { file: "routing.json", key: "routing" },
  { file: "drilling.json", key: "drilling" },
  { file: "cutting.json", key: "cutting" },
  { file: "nesting.json", key: "nesting" },
];

export class AlphacamFunctionIndexEngine {
  private static index: AlphacamFunctionIndex | null = null;

  private static readonly DATA_DIR = join(
    __dirname,
    "../../data/cam-functions/alphacam"
  );

  private static loadIndex(): AlphacamFunctionIndex {
    if (this.index) return this.index;

    const sections: Record<string, AlphacamSection> = {};
    let totalOps = 0;
    let totalParams = 0;
    ...
    for (const { file, key } of SECTION_FILES) {
      ...
      try {
        const data = JSON.parse(readFileSync(path, "utf-8")) as AlphacamSection;
        sections[key] = data;
        ...
        throw new Error(
          `AlphacamFunctionIndexEngine: failed to parse ${file}: ${(err as Error).message}`
        );
      }
    }
  ...
  static getIndex(): AlphacamFunctionIndex {
  ...
  static getSection(
    sectionKey: string
  ): AlphacamSection | { error: string; available: string[] } {
  ...
  /**
   * LR32 / drilling operations — Alphacam's signature furniture surface.
   */
  static getDrillingOperations(): Array<{
    operation_id: string;
    display_name: string;
    section: string;
    category: string;
    ...
    const ds = idx.sections["drilling"];
    if (!ds) return out;
  ...
  /**
   * Wood-grain nesting operations — Alphacam's secondary specialty.
   */
  static getNestingOperations(): Array<{
    operation_id: string;
    display_name: string;
    section: string;
    category: string;
    ...
    const ns = idx.sections["nesting"];
    if (!ns) return out;
    for (const [opId, op] of Object.entries(ns.operations || {})) {
      out.push({
        operation_id: opId,
        display_name: op.display_name,
        section: "nesting",
  ...
  static getOperation(operationId: string):
    | { operation_id: string; section: string; operation: AlphacamOperation }
    | { error: string } {
