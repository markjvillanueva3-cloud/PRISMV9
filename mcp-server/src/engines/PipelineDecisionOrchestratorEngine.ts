...
let _decisionTreeEngine: any = null;
let _xaiEngine: any = null;
let _bayesianInferenceEngine: any = null;

function getDecisionTree() {
  if (!_decisionTreeEngine) {
    const m = require("./DecisionTreeEngine.js");
    _decisionTreeEngine = m.decisionTreeEngine ?? m.default ?? m;
  }
  return _decisionTreeEngine;
}

function getXAI() {
  if (!_xaiEngine) {
    const m = require("./XAIEngine.js");
    _xaiEngine = m.xaiEngine ?? m.default ?? m;
  }
  return _xaiEngine;
}

function getBayesianInference() {
  if (!_bayesianInferenceEngine) {
    const m = require("./BayesianInferenceEngine.js");
    _bayesianInferenceEngine = m.bayesianInferenceEngine ?? m.default ?? new m.BayesianInferenceEngine();
  }
  return _bayesianInferenceEngine;
}
