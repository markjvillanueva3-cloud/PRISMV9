...
function getBayesianInference() {
    return bayesianInferenceEngine;
}
// ============================================================================
// TK-MS2-U10: TRIBAL KNOWLEDGE INTEGRATION
// ============================================================================
/**
 * Maps the 12 DecisionCategory values to tribal knowledge categories.
 * Used to query relevant tips for each decision type.
 */
const DECISION_TO_TRIBAL_CATEGORY = {
    tool_select: ["tooling", "speeds_feeds", "tool_life"],
    strategy_select: ["speeds_feeds", "tooling", "quality"],
    parameter_optimize: ["speeds_feeds", "quality", "tooling"],
    coolant_select: ["coolant", "machine_setup", "quality"],
    entry_exit_select: ["toolpath", "tooling", "quality"],
    sequence_optimize: ["toolpath", "workholding", "quality"],
    workholding_validate: ["workholding", "machine_setup", "quality"],
    controller_select: ["machine_setup", "controller"],
    holder_select: ["tooling", "workholding"],
    coating_select: ["tooling", "tool_life", "speeds_feeds"],
    work_offset_select: ["machine_setup", "workholding"],
    safety_check: ["safety", "machine_setup", "troubleshooting"],
};
/**
 * Query tribal tips relevant to a decision candidate.
 * Returns a score adjustment (0 to 0.15) based on matching tips.
 */
function getTribalBonus(category, context, candidateData) {
    try {
        // Build search query from context and candidate
        const queryParts = [];
        if (context.material?.name)
            queryParts.push(context.material.name);
        if (context.material?.iso_group)
            queryParts.push(context.material.iso_group);
        if (context.operation)
            queryParts.push(context.operation);
        if (candidateData.type)
            queryParts.push(String(candidateData.type));
        if (candidateData.label)
            queryParts.push(String(candidateData.label));
        if (queryParts.length === 0)
            return { bonus: 0, tips: [] };
        // Get relevant tribal categories for this decision
        const tribalCategories = DECISION_TO_TRIBAL_CATEGORY[category] || [];
        // Search for tips
        const tips = tribalKnowledgeEngine.search({
            query: queryParts.join(" "),
            category: tribalCategories[0], // Primary category
            limit: 10,
        });
        if (tips.length === 0)
            return { bonus: 0, tips: [] };
        // Filter to high-confidence tips (>70)
        const highConfidenceTips = tips.filter(t => t.confidence >= 70);
        // Calculate bonus: 0.03 per matching tip, max 0.15
        const bonus = Math.min(highConfidenceTips.length * 0.03, 0.15);
        return { bonus, tips: highConfidenceTips.slice(0, 3) };
    }
    catch (e) {
        log.debug(`[PipelineDecisionOrchestrator] Tribal bonus query failed: ${e}`);
        return { bonus: 0, tips: [] };
    }
}
...
const PARAMETER_OPTIMIZE_RUBRIC = {
    mrr: 0.25,
    force_margin: 0.25,
...
const ENTRY_EXIT_RUBRIC = {
    tool_stress: 0.30,
    surface_quality: 0.25,
    cycle_time: 0.20,
    collision_safety: 0.25,
};
const SEQUENCE_RUBRIC = {
...
const WORK_OFFSET_RUBRIC = {
    accuracy: 0.30,
    repeatability: 0.25,
    ease_of_setup: 0.20,
    collision_clearance: 0.25,
};
const SAFETY_CHECK_RUBRIC = {
    force_margin: 0.25,
...
function computeEffectiveWeights(category, objective) {
...
function compositeScore(axes, weights) {
    return (axes.optimal_performance * weights[0] +
        axes.logical_consistency * weights[1] +
        axes.safety * weights[2] +
        axes.cost_efficiency * weights[3] +
        axes.robustness * weights[4]) * 100;
}
...
function numVal(data, key, fallback = 0.5) {
    const v = data[key];
...
function computeSafetyScore(data, constraints, context) {
    if (!constraints)
        return 0.7; // no constraints = assume moderately safe
    let safetySum = 0;
    let safetyCount = 0;
    // Force margin
...
function computeCostEfficiency(data, constraints) {
...
function computeRobustness(data, context) {
    let robustScore = 0.5;
    let factors = 0;
...
function computeLogicalConsistency(data, context, category) {
    ...
    if (category === "tool_select" && context.feature) {
        ...
        if (toolDia > 0 && featDia && featDia > 0) {
            ...
            if (ratio > 0.5 && ratio < 1.0)
                ...
                score -= 0.25; // tool larger than feature
        }
    }
    ...
    if (context.operation && data.operation_types) {
        const ops = data.operation_types;
...
function computePerformance(data, context, category, preScores) {
    // If upstream provided a performance score, weight it heavily
    if (preScores?.optimal_performance !== undefined) {
        // TK-MS2-U10: Still apply tribal bonus even with pre-scores
        const { bonus } = getTribalBonus(category, context, data);
        return clamp(preScores.optimal_performance + bonus, 0, 1);
    }
    ...
    if (totalWeight === 0) {
        ...
        if (genericScore >= 0) {
            baseScore = genericScore > 1 ? norm100(genericScore) : genericScore;
        }
        else {
            baseScore = 0.5;
        }
    ...
    // TK-MS2-U10: Add tribal knowledge bonus (0-0.15 based on matching tips)
    const { bonus } = getTribalBonus(category, context, data);
...
function generateJustification(choice, axes, weights, category, objective, context, alternativeCount) {
    const lines = [];
    const composite = compositeScore(axes, weights);
    lines.push(`Selected "${choice.label}" (score: ${composite.toFixed(1)}/100) for ${category.replace(/_/g, " ")}.`);
    // Top scoring axis
    const axisNames = [
    ...
    lines.push(`Primary strength: ${axisLabels[topAxis.key]} at ${(topAxis.val * 100).toFixed(0)}% (weight: ${(topAxis.weight * 100).toFixed(0)}%).`);
    // Weakest axis (if notably low)
    const weakest = sorted[sorted.length - 1];
    if (weakest.val < 0.5) {
        lines.push(`Note: ${axisLabels[weakest.key]} is below average at ${(weakest.val * 100).toFixed(0)}% — monitor this aspect.`);
    }
    // Objective alignment
...
function generateWhyNot(candidate, axes, choiceScore, candidateScore, vetoed) {
    if (vetoed) {
...
export class PipelineDecisionOrchestratorEngine {
    ...
    decide(input) {
        const t0 = Date.now();
        const warnings = [];
        const { category, context, candidates, constraints, objective = "balanced", explain = false, pipeline_stage, caller, } = input;
        // ── Validation ──────────────────────────────────────────────
        if (!candidates || candidates.length === 0) {
            throw new Error("[PipelineDecisionOrchestrator] No candidates provided.");
        }
        if (candidates.length > MAX_CANDIDATES_WARN) {
            warnings.push(`Large candidate set (${candidates.length}). Consider pre-filtering for performance.`);
        }
        ...
        for (const cand of candidates) {
            ...
            if (vetoed) {
                warnings.push(`Candidate "${cand.label}" vetoed: safety ${(axes.safety * 100).toFixed(0)}% < ${SAFETY_VETO_THRESHOLD * 100}%.`);
            }
        ...
        if (viable.length === 0) {
            ...
            warnings.push("ALL candidates failed safety veto. Returning least-unsafe option. MANUAL REVIEW REQUIRED.");
        }
        ...
        // ── Justification ───────────────────────────────────────────
        const justification = generateJustification(best.candidate, best.axes, weights, category, objective, context, alternatives.length);
        // ── XAI feature importance (optional) ───────────────────────
        let featureImportance;
        if (explain) {
            featureImportance = this._computeFeatureImportance(best.candidate, scored, category, context);
        }
        // ── Decision tree path (optional) ───────────────────────────
        let decisionTreePath;
     ...
     */
    compare(a, b, category, context, constraints, objective = "balanced") {
        const weights = computeEffectiveWeights(category, objective);
    ...
    getStats() {
        ...
        for (const entry of this._auditLog) {
            if (!stats[entry.category])
                stats[entry.category] = { count: 0, total_score: 0 };
     ...
     */
    prescreen(candidate, constraints, context) {
        const reasons = [];
     ...
     */
    _scoreCandidate(candidate, context, category, constraints) {
        const data = candidate.data;
     ...
     */
    _computeFeatureImportance(choice, allScored, category, context) {
        const importance = {};
        ...
        try {
            ...
            if (xai?.computeShap) {
                const features = {};
                if (context.material?.hardness_hrc)
                    features.hardness = context.material.hardness_hrc;
                if (context.feature?.depth_mm)
                    features.depth = context.feature.depth_mm;
                if (context.feature?.diameter_mm)
                    features.diameter = context.feature.diameter_mm;
                if (context.machine?.max_rpm)
                    features.max_rpm = context.machine.max_rpm;
                if (context.machine?.max_power_kw)
                    features.max_power = context.machine.max_power_kw;
        ...
        for (const key of keys) {
            ...
            if (typeof val === "number") {
                ...
                const perturbedCand = { ...choice, data: perturbedData };
     ...
     */
    _traceDecisionTreePath(category, context) {
        try {
     ...
     */
    bayesianAssess(category, context, priorProbabilities) {
        try {
            ...
            const evidence = {};
            if (context.material?.iso_group)
                evidence.iso_group = context.material.iso_group;
            if (context.material?.hardness_hrc)
                evidence.hardness = context.material.hardness_hrc;
            if (context.operation)
...
//# sourceMappingURL=PipelineDecisionOrchestratorEngine.js.map