...
class OkumaParametricProgramEngineImpl {
  ...
  convertToHardcode(programText: string, decimalPlaces = 4): {
    ...
    const evalExpr = (expr: string): number => {
      ...
      // Safe numeric evaluation helper — uses safeMathEval utility for RCE prevention (MS1 U-LAT09)
      const safeNumEval = (s: string): number => {
        try {
          ...
          // Import safeMathEval dynamically to avoid circular deps
          const { safeMathEval } = require("../utils/safeMathEval.js");
          return safeMathEval(cleaned, {});
        } catch { return 0; }
      };
    ...
    const evalCondition = (cond: string): boolean => {
      ...
      e = e.replace(/\[/g, "(").replace(/\]/g, ")");
      // MS1 U-LAT10: Use safeMathEval for RCE prevention (replaces raw eval)
      try {
        const { safeMathEval } = require("../utils/safeMathEval.js");
        // safeMathEval returns number; comparison expressions evaluate to 0/1
        return Boolean(safeMathEval(e, {}));
      } catch { return false; }
    };
