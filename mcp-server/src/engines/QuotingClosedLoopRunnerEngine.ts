...
import {
  ...
  QuotingOutcomeCaptureWireEngine,
} from "./QuotingOutcomeCaptureWireEngine.js";
import type {
  ClosedLoopDeps,
...
export interface BuildLiveDepsOptions {
  ...
  /** Mirror every observed outcome to the OutcomeCaptureBus so india's
   *  cross-process learner (via the P0-U04 OutcomeCaptureBusToFeedbackBridge)
   *  trains on quoting outcomes. Default true; set false to disable (e.g. an
   *  offline replay that must not pollute the live learning signal).
   *  U-CHARLIE-QUOTING-OUTCOME-WIRE (2026-06-27). */
  emitToCaptureBus?: boolean;
  /** DI override for the capture-bus wire (tests inject an isolated bus). */
  captureWire?: QuotingOutcomeCaptureWireEngine;
}
...
export function buildLiveDeps(opts: BuildLiveDepsOptions): ClosedLoopDeps {
  ...
  const emitToCaptureBus = opts.emitToCaptureBus ?? true;
  const captureWire = opts.captureWire ?? quotingOutcomeCaptureWireEngine;

  return {
    fetchOutcomes: async (sinceIso?: string) => {
      const records = await opts.loadOutcomes(sinceIso);
      // U-CHARLIE-QUOTING-OUTCOME-WIRE: mirror every observed outcome to the
      // OutcomeCaptureBus -> P0-U04 bridge -> india's cross-process learner.
      // Best-effort + idempotent: the wire never throws, so a bus failure can
      // never break the cycle's observe stage (invariant #1).
      if (emitToCaptureBus && Array.isArray(records) && records.length > 0) {
        captureWire.recordOutcomes(records);
      }
      return records;
    },
    ...
    deriveWithCoV: async (report) => {
      const synthetic: SubstrateAccuracyReport = {
        ...
        total_skipped: 0,
        total_skipped_contaminated: 0, // CoV-derivation placeholder runs no contamination pass
        metrics: {
          mae_usd: 0,
          rmse_usd: 0,
        ...
        worst_5_records: [],
        best_5_records: [],
        // CoV-derivation placeholder carries no per-record predictions -- empty
        // arrays are the engine's own canonical defaults (QuotingTrainingLoopEngine
        // not-ok path), consistent with the empty fields above. No fabricated values.
