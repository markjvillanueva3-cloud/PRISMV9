...
export class AgentExecutor {
  ...
  private async executeAgentReal(task: TaskDefinition): Promise<unknown> {
    ...
    const requestedProfileId = typeof metadata.runtimeProfileId === "string"
      ? metadata.runtimeProfileId
      : undefined;
    const requestedProvider = metadata.reasoningProvider === "anthropic"
      || metadata.reasoningProvider === "openai"
      ? metadata.reasoningProvider
      : undefined;
    const profile = resolvePrismRuntimeProfile({
      profileId: requestedProfileId as PrismRuntimeProfileId | undefined,
      providerPreference: requestedProvider,
    });

    // ENFORCE real API execution - no simulation allowed
    if (!hasAnthropicApiKey() && !hasOpenAIApiKey()) {
      throw new Error(
        `A live reasoning key is required for agent execution. ` +
        `Configure ANTHROPIC_API_KEY and/or OPENAI_API_KEY in the MCP server environment. ` +
        `Simulation mode DISABLED for safety-critical manufacturing.`
      );
    }

    let provider: ReasoningProvider = profile.executionProvider;
    let fallbackFrom: ReasoningProvider | undefined;

    if (provider === "anthropic" && !hasAnthropicApiKey() && hasOpenAIApiKey()) {
      fallbackFrom = provider;
      provider = "openai";
    } else if (provider === "openai" && !hasOpenAIApiKey() && hasAnthropicApiKey()) {
      fallbackFrom = provider;
      provider = "anthropic";
    }

    if (provider === "openai") {
      return this.executeWithOpenAIAPI(task, agent, fallbackFrom);
    }

    return this.executeWithClaudeAPI(task, agent, fallbackFrom);
  }
  ...
  private async executeWithClaudeAPI(
    ...
    fallbackFrom?: ReasoningProvider,
  ): Promise<unknown> {
    ...
    const model = getModelForTier(tier as 'opus' | 'sonnet' | 'haiku', "anthropic");
  ...
  private async executeWithOpenAIAPI(
    task: TaskDefinition,
    agent: AgentDefinition,
    fallbackFrom?: ReasoningProvider,
  ): Promise<unknown> {
    const tier = ((agent as unknown as Record<string, unknown>).tier as string)?.toLowerCase() || "sonnet";
    const model = getModelForTier(tier as 'opus' | 'sonnet' | 'haiku', "openai");
    const systemPrompt = this.buildAgentSystemPrompt(agent);
    const userMessage = this.buildTaskMessage(task);
    const startTime = Date.now();

    log.info(`[AgentExecutor] Calling OpenAI Responses API (${model}) for agent ${agent.name}`);
    ...
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        model,
        reasoning: {
          effort: tier === "opus" ? "high" : tier === "sonnet" ? "medium" : "low",
        },
        max_output_tokens: agent.config?.max_tokens || 4096,
        temperature: agent.config?.temperature || 0.3,
        input: [
          {
            role: "system",
        ...
        metadata: {
          prism_agent_id: task.agentId,
          prism_task_id: task.id,
        },
      }),
    });
    ...
    if (!response.ok) {
      ...
      log.error(`[AgentExecutor] OpenAI API error: ${response.status} ${response.statusText} ${errorText}`);
    ...
    const outputItems = Array.isArray(data.output)
      ...
      : [];
    const textContent = outputItems.flatMap((item) => {
    ...
    return {
      agent: agent.name,
      category: agent.category,
      response: textContent || String(data.output_text ?? ""),
      model,
      provider: "openai",
      fallbackFrom,
      usage: {
  ...
  private buildAgentSystemPrompt(agent: AgentDefinition): string {
    ...
    prompt += "Reasoning Stack: ScientificReasoningEngine, MultiPathReasoningEngine, DecisionReasoningEngine, ";
    prompt += "ChainOfThoughtEngine, ExtendedThinkingBridgeEngine, SpeedFeedDeepLearningEngine, PostProcessorDeepLearningEngine.\n";
