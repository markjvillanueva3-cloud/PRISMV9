...
## ERP/business anon-leak security sweep (slot:hotel, 2026-06-24)

Operator directive: apply the quoting anon-cost/PII-leak hardening pass to the business/ERP surface. Per-route audit (NOT the headline verifyToken count -- R12 "existence != complete") of every business/ERP HTTP route:

- **hotel-portal.ts (`/api/v1/hotel-portal`, 31 routes): had ZERO auth -> ALL anon-reachable.** Employee PII (digest/dashboard/pto-balance/payroll-gross-wages/osha-injury) + privileged MUTATIONS (pto-approve, timeclock-edit, po-create/transition/receipt, cofc-issue, role-hire). FIXED **U-HOTEL-PORTAL-AUTH (18f8da8ed9):** /health open, then `router.use(verifyToken)` gates all 31 + `requireRole` tiers on 14 privileged routes (mirroring erp.ts: payroll/timeclock-edit/osha/hire->hr_manager+admin; dashboard/pto-approve/po-*/cofc->lead+; exec->admin). FE HotelPortalPage wired to send the token (R15). 66/66 tests, 3-of-3. [[reference_hotel_portal_auth_2026_06_24]]
- **wedm-erp.ts `GET /quote/rates`: intentionally anon (prospect rate card) but LEAKED overhead_pct + margin_pct.** FIXED **U-WEDMERP-RATES-REDACT (5538ba5c38):** redact-when-anon -- authed gets the full card incl. percentages; anon gets only customer-facing machine/operator/wire rates. 16/16 u04, 3-of-3.
- **VERIFIED CLEAN (not touched):** erp.ts (205 auth refs, verifyToken+requireRole on every route), business.ts (1 route /dispatch, verifyToken+allowlist), wedm-erp.ts /quote/estimate (intentional prospect public, no margin in response).

**OPEN follow-ups (separate units):** (1) **IDOR** -- hotel-portal self-service routes take an arbitrary employee_id with no self-scoping; needs an erp.ts-style requireSelfOrAdmin BUT blocked on a user<->employee_id mapping (AuthUser has no employee_id field; req.userId is the auth user id, not EMP-*; the same gap likely makes erp.ts's own requireSelfOrAdmin subtly broken). Needs an operator/india auth-data-model decision. (2) /simulation/run lead+ tier review.

The anon-cost/PII-leak class is now CLOSED across BOTH the quoting (charlie) and business/ERP (hotel) HTTP surfaces.
