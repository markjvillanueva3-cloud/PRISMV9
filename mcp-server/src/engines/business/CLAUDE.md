# Business/ERP Galaxy — slot:hotel (Kienzle Tool Crib redesign active)

> **KIENZLE REDESIGN DIRECTIVE (2026-06-26):** The Claude Design project "Kienzle Tool Crib" (https://claude.ai/design/p/9e002608-540b-4214-81ff-446dd2409274) is the new canonical design. All hotel/business/ERP/employee portal surfaces (HR, CRM, ERP, Accounting, CustomerPortal, QuoteToOrderBridge, ERPCostFeedback, etc.) must be wired to match the new Kienzle UI/UX (iOS-style tokens, SF typography, new CSS vars, rebranded flows). Current build must change to align. Fleet memory + state/shared/KIENZLE-TOOL-CRIB-DESIGN-DIRECTIVE.md is the source of truth. Continuous auto-firing loops running.
...
- **Reasoning bridge** (`scripts/lib/galaxy-reasoning-bridge.mjs`, PSN leg #10): **CAG** + **RAG** hybrid
  ...
  local Ollama stack -- `node scripts/lib/galaxy-reasoning-bridge.mjs business "<question>"`.
- **Vault -> LoRA**: this galaxy's Obsidian **synthesis** brain (`knowledge/memories/patterns/business_synthesis.md`)
...
**Domain angle:** ERP / forecasting + document classification draw on this shared substrate (deep-reasoning over financial / HR / quote text).
