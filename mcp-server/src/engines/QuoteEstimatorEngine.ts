...
class QuoteEstimatorEngine {
  ...
  estimate(input: QuoteEstimateInput): QuoteEstimateResult {
    ...
    // ── 0. Query Tribal Knowledge Modifiers (TK-MS3 U-TK18) ──
    const tribalMods = tribalKnowledgeAdvisorEngine.getModifiers({
      material: input.material,
      operation: input.machine_type?.includes("lathe") ? "turning" : "milling",
      complexity: input.complexity,
    });
    if (tribalMods.evidence_count > 0) {
      confidence.push(`tribal: ${tribalMods.evidence_count} tips applied (conf=${(tribalMods.confidence * 100).toFixed(0)}%)`);
    }
    ...
    const machiningCost = this.calcMachiningCost(input, confidence, tribalMods);
    ...
    // ── 3. Setup Cost ──
    const setupCost = this.calcSetupCost(input, tribalMods);
  ...
  private calcMachiningCost(
    ...
    // TK-MS3 U-TK18: Apply tribal machine rate override if available
    const rate = tribalMods?.machine_rate_override ?? MACHINE_RATE_HR[machType] ?? 85;
    const qty = input.quantity;
    // TK-MS3 U-TK18: Apply tribal machinability factor override if available
    const machinability = tribalMods?.machinability_factor_override
      ?? MACHINABILITY_FACTOR[input.material.toLowerCase()] ?? 1.0;
    ...
    // TK-MS3 U-TK18: Apply tribal cycle time base override if available
    if (tribalMods?.cycle_time_base_override !== undefined && tribalMods?.cycle_time_base_override !== null) {
      cycleTimeMin = tribalMods.cycle_time_base_override;
      source = "parametric_estimate";
      confidence.push("machining: tribal cycle time override applied");
    }
  ...
  private calcSetupCost(input: QuoteEstimateInput, tribalMods?: TribalModifiers): QuoteEstimateResult["costs"]["setup"] {
    // TK-MS3 U-TK18: Apply tribal setup count override if available
    const numSetups = tribalMods?.setup_count_override
      ?? input.num_setups
      ?? (input.complexity === "simple" ? 1
    ...
    // TK-MS3 U-TK18: Apply tribal setup time per setup override if available
    const baseMinPerSetup = tribalMods?.setup_time_per_setup_min ?? 25;
