...
class QuoteEstimatorEngine {
  ...
  private calcNRECost(input: QuoteEstimateInput): QuoteEstimateResult["costs"]["nre"] {
    ...
    const results = items.map(item => {
      ...
      const amortQty = item.amortize_over_qty ?? 0;
      return {
        type: item.type,
        description: item.description,
        cost: round2(cost),
        amortized_per_part: amortQty > 0 ? round2(cost / amortQty) : 0,
      };
    });
  ...
  private quickEstimateCostPerPart(input: QuoteEstimateInput): number {
    ...
    // Learning curve for volume
    const learningFactor = input.quantity > 1
      ? Math.pow(input.quantity, Math.log2(LEARNING_CURVE_FACTOR)) / input.quantity
      : 1;
