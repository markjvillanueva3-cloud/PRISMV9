...
export class EDMWireSlugCornerTaperEngine {
  ...
  private calculateWireLag(
    ...
    // δ = F × L² / (8T)
    // Convert guide distance mm to same unit (keep mm)
    return (dischargeForceN * guideDistanceMm * guideDistanceMm) / (8 * wireTensionN);
  }
  ...
  solveTaper(input: TaperInput): TaperSolution {
    ...
    const geoError = (input.wire_diameter_mm / 2) * input.workpiece_height_mm / input.guide_distance_mm;
