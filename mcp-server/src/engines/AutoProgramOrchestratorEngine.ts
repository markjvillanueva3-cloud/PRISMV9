...
import { CANONICAL_KIENZLE, CANONICAL_TAYLOR, CANONICAL_MATERIAL_DB, CANONICAL_TURNING_SPEEDS, kienzleForce, taylorLife, } from "../physics/constants.js";
...
function resolveIsoGroup(material, override) {
    if (override)
        return override;
    const m = material.toLowerCase();
    // Common material patterns → ISO group
    if (/aluminum|aluminium|6061|7075|2024/.test(m))
        return "N";
    if (/stainless|304|316|17-4|inox/.test(m))
        return "M";
    if (/steel|4140|4340|1018|1045|8620/.test(m))
        return "P";
    if (/cast.?iron|gray.?iron|ductile/.test(m))
        return "K";
    if (/titanium|ti-6|ti6al4v/.test(m))
        return "S";
    if (/inconel|hastelloy|waspaloy|rene/.test(m))
        return "S";
    if (/hardened|hrc.?[4-6]/.test(m))
        return "H";
    return "P"; // default to steel
}
/** Default workholding stiffness per type. */
const WORKHOLDING_STIFFNESS = {
...
const WORKHOLDING_DEFAULT_FORCE_KN = {
 ...
 */
function edmPassCountForRa(targetRa) {
    if (targetRa >= 3.2)
        return 1; // rough only → Ra ~3.2 µm
    if (targetRa >= 1.6)
        return 2; // rough + 1 trim → Ra ~1.6 µm
    if (targetRa >= 0.8)
        return 3; // rough + 2 trim → Ra ~0.8 µm
 ...
 */
const TURNING_APPROACH_RETRACT = {
    turning_rough: {
 ...
 */
function computeTurningPasses(opType, totalDepth) {
...
const WORKHOLDING_CLAMP_POINTS = {
...
export class AutoProgramOrchestratorEngine {
    ...
    static async run(input) {
        ...
        const operations = ctx.planned_operations ?? [];
        const dfmIssues = ctx.dfm_issues ?? [];
        const dfmScore = ctx.dfm_score ?? 100;
        const gcodeFile = ctx.gcode_file;
        const gcodeLines = ctx.gcode_lines;
        const setupSheet = ctx.setup_sheet;
        const machType = ctx.machine_type ?? "vmc_3axis";
     ...
     */
    static validateStageOutput(result, ctx) {
        const ops = ctx.planned_operations;
        if (!ops)
            return;
        for (const op of ops) {
    ...
    // ── S1: Model Intake ──────────────────────────────────────────────
    static async stageModelIntake(ctx) {
        const t0 = Date.now();
        const warnings = [];
        const engines = ["Fusion360LiveBridgeEngine"];
        try {
            const BridgeClass = await loadBridge();
            const bridge = new BridgeClass(ctx.fusion_url);
    ...
    static async stageFeatureRecognition(ctx) {
        const t0 = Date.now();
        const engines = ["Fusion360LiveBridgeEngine", "FeatureRecognitionEngine"];
        const warnings = [];
        try {
            const bridge = ctx.bridge;
            const candidates = await bridge.getFeatureCandidates();
            const features = (candidates.candidates ?? []).map((f) => ({
                type: f.type ?? "unknown",
    ...
    // ── S3: DFM Analysis ──────────────────────────────────────────────
    static async stageDfmAnalysis(ctx) {
        const t0 = Date.now();
        const engines = ["DFMPipelineEngine"];
        const warnings = [];
        try {
            const dfmEngine = await loadDFMPipeline();
            const features = ctx.features ?? [];
            const isoGroup = ctx.iso_group;
            ...
            const criticalIssues = (dfmResult.issues ?? []).filter((i) => i.severity === "critical");
            if (criticalIssues.length > 0) {
                warnings.push(`${criticalIssues.length} critical DFM issue(s) — review before machining.`);
            }
            // TK-MS2-U14: Query tribal knowledge for DFM tips
            try {
                const dfmTips = tribalKnowledgeEngine.search({
                    category: "setup",
                    operation_type: "dfm",
                    material_iso_group: isoGroup,
                    limit: 5,
                });
                for (const tip of dfmTips) {
                    warnings.push(`TK: ${tip.title}`);
                }
                engines.push("TribalKnowledgeEngine");
            }
            catch {
                // Tribal knowledge unavailable — continue without
            }
            return {
                stage: "dfm_analysis",
                stage_index: 2,
                ...
                data: {
                    score: ctx.dfm_score,
                    issue_count: ctx.dfm_issues.length,
    ...
    // ── S4: Process Planning ──────────────────────────────────────────
    static async stageProcessPlanning(ctx) {
        const t0 = Date.now();
        const engines = ["OperationSequencerEngine"];
        try {
            const features = ctx.features ?? [];
            const machType = ctx.machine_type ?? "vmc_3axis";
            ...
            operations.sort((a, b) => (typeOrder[a.type] ?? 10) - (typeOrder[b.type] ?? 10));
            const whType = ctx.workholding_type ?? "vise";
            const planWarnings = [];
            ...
            if (whType === "tombstone" || whType === "pallet") {
                ...
                for (const op of operations) {
                    ...
                    if (feat?.position) {
                        ...
                        if (whType === "tombstone") {
                            ...
                            op.setup_face = Math.floor(normalized / 90) % 4;
                        }
                        else {
                            op.setup_face = feat.position.z > 0 ? 0 : 1;
                        }
                    }
                    else {
                        op.setup_face = 0;
                ...
                operations.sort((a, b) => {
                    const faceA = a.setup_face ?? 0;
                    const faceB = b.setup_face ?? 0;
                    if (faceA !== faceB)
                        return faceA - faceB;
                    return (typeOrder[a.type] ?? 10) - (typeOrder[b.type] ?? 10);
                });
                planWarnings.push(`${whType}: ${face_count} faces detected, operations grouped by face.`);
            }
            ...
            if (whType === "4th_axis") {
                ...
                operations.sort((a, b) => {
                    const angleA = a.a_axis_deg ?? 0;
                    const angleB = b.a_axis_deg ?? 0;
                    if (angleA !== angleB)
                        return angleA - angleB;
                    return (typeOrder[a.type] ?? 10) - (typeOrder[b.type] ?? 10);
                });
                planWarnings.push(`4th-axis: ${unique_angles} unique index angle(s) detected.`);
            }
            ...
            const singularity_warnings = [];
            if (machType === "five_axis_3plus2" || machType === "five_axis_simultaneous") {
                const kinModel = ctx.kinematic_model ?? "trunnion_table";
                ...
                for (const op of operations) {
                    ...
                    if (n && (n.x !== 0 || n.y !== 0 || Math.abs(n.z) < 0.9999)) {
                        ...
                        if (Math.abs(A_deg) < limits.singularity_zone_deg) {
                            singularity_warnings.push(`Feature ${op.feature_index} (${feat.type}): A=${A_deg.toFixed(1)}° near singularity zone (±${limits.singularity_zone_deg}°). C-axis may exhibit tracking errors on ${kinModel}.`);
                        }
                        op.work_plane_a_deg = A_deg;
                        op.work_plane_c_deg = C_deg;
                        op.kinematic_model = kinModel;
                        ...
                        op.work_plane_a_deg = 0;
                        op.work_plane_c_deg = 0;
                    }
                }
                work_plane_count = planeSet.size;
                // Re-sort: group by work plane (A then C), then by type within each plane
                operations.sort((a, b) => {
                    const aA = a.work_plane_a_deg ?? 0;
                    const bA = b.work_plane_a_deg ?? 0;
                    if (aA !== bA)
                        return aA - bA;
                    const aC = a.work_plane_c_deg ?? 0;
                    const bC = b.work_plane_c_deg ?? 0;
                    if (aC !== bC)
                        return aC - bC;
                    return (typeOrder[a.type] ?? 10) - (typeOrder[b.type] ?? 10);
                });
                ...
                planWarnings.push(`5-axis (${kinModel}): ${work_plane_count} unique work plane(s), mode=${isSimultaneous ? "simultaneous" : "3+2 indexed"}.`);
            }
            ...
            if (machType === "lathe" || machType === "mill_turn") {
                const isoGroup = ctx.iso_group ?? "P";
                const maxRpm = ctx.max_rpm ?? MACHINE_DEFAULTS[machType].max_rpm;
                const canonVc = CANONICAL_TURNING_SPEEDS[isoGroup];
                for (const op of operations) {
                    const spindleMode = TURNING_SPINDLE_MODE[op.type];
                    if (!spindleMode)
                        continue; // milling ops on mill-turn — no spindle mode override
                    turning_op_count++;
                    op.spindle_mode = spindleMode;
                    if (spindleMode === "G96") {
                        ...
                        op.calculated_rpm_at_od = Math.min(calcRpm, maxRpm);
                        g96_count++;
                    }
                    else {
                        ...
                        const cssVc = op.type === "turning_thread"
                            ? canonVc.finish * 0.5
                            : canonVc.rough;
                        const fixedRpm = Math.min(Math.round((1000 * cssVc) / (Math.PI * wpDia)), maxRpm);
                        op.fixed_rpm = fixedRpm;
                        g97_count++;
                    }
                }
                if (turning_op_count > 0) {
                    planWarnings.push(`Lathe spindle: ${g96_count} G96 (CSS) + ${g97_count} G97 (constant RPM) operation(s).`);
                }
                ...
                let multiPassOps = 0;
                for (const op of operations) {
                    if (!op.type.startsWith("turning_"))
                        continue;
                    const pattern = TURNING_APPROACH_RETRACT[op.type];
                    if (pattern) {
                        op.approach_type = pattern.approach;
                        op.retract_type = pattern.retract;
                        // Multi-pass: split deep features
                        const totalDepth = op.depth_mm ?? 5;
                        const passes = computeTurningPasses(op.type, totalDepth);
                        op.pass_count = passes.pass_count;
                        op.depth_per_pass_mm = passes.depth_per_pass_mm;
            ...
            if (machType === "mill_turn") {
                ...
                if (channel_count > 1) {
                    ...
                    for (let oi = 0; oi < operations.length; oi++) {
                        ...
                        const ch = op.channel_id;
                        // Sub-spindle transfer: sync before and after
                        if (op.type === "sub_spindle_transfer") {
                            op.sync_before = MILL_TURN_SYNC_CODES.pre_transfer.code;
                            op.sync_after = MILL_TURN_SYNC_CODES.post_transfer.code;
                        }
                        // Channel transition: C1→Cm (milling start)
                        else if (prevChannel === "C1" && ch === "Cm") {
                            op.sync_before = MILL_TURN_SYNC_CODES.milling_start.code;
                        }
                        // Channel transition: Cm→C1 (milling end, resume turning)
                        else if (prevChannel === "Cm" && ch === "C1") {
                            op.sync_before = MILL_TURN_SYNC_CODES.milling_end.code;
                        }
                    ...
                    if (lastOp && channel_count > 1) {
                        lastOp.sync_after = MILL_TURN_SYNC_CODES.program_end.code;
                    }
                    planWarnings.push(`Mill-turn: ${channel_count} channels (C1=${operations.filter(o => o.channel_id === "C1").length} main, ` +
                    ...
                    const syncCount = operations.filter(o => o.sync_before != null ||
                        o.sync_after != null).length;
            ...
            if (machType === "wire_edm") {
                for (const op of operations) {
                    if (!op.type.startsWith("wire_"))
                        continue;
                    wire_edm_op_count++;
                    const thickness = op.depth_mm ?? 25; // workpiece thickness mm
                    op.wire_diameter_mm = WIRE_DEFAULTS.wire_diameter_mm;
                    op.workpiece_thickness_mm = thickness;
                    if (op.type === "wire_4axis_taper") {
                        ...
                        // Ref: Mitsubishi MV/FA series programming manual, AgieCharmilles CUT E series
                        const angle = op.taper_angle_deg ?? 3; // default 3°
                        ...
                        op.taper_angle_deg = angle;
                        op.uv_offset_mm = Math.round(uvOffset * 1000) / 1000;
                        op.upper_guide_z = thickness;
                        op.lower_guide_z = 0;
                    }
                    else {
                        // Straight cut: no taper
                        op.taper_angle_deg = 0;
                        op.uv_offset_mm = 0;
                    }
                ...
                if (wire_edm_op_count > 0) {
                    const taperOps = operations.filter(o => o.taper_angle_deg > 0).length;
                    planWarnings.push(`Wire EDM: ${wire_edm_op_count} operation(s), ${taperOps} with taper (UV offset).`);
                }
            }
            ...
            if (machType !== "lathe" && machType !== "wire_edm") {
                const hasBottomFeatures = features.some(f => f.normal != null && f.normal.z < -0.5);
                if (hasBottomFeatures && machType !== "mill_turn") {
                    ...
                    operations.sort((a, b) => {
                        const sA = a.setup_index ?? 0;
                        const sB = b.setup_index ?? 0;
                        if (sA !== sB)
                            return sA - sB;
                        return (typeOrder[a.type] ?? 10) - (typeOrder[b.type] ?? 10);
                    });
                    planWarnings.push(`Multi-setup: Op1 (top) = ${operations.filter(o => (o.setup_index ?? 0) === 0).length} ops, ` +
                        `Op2 (flip) = ${operations.filter(o => o.setup_index === 1).length} ops.`);
                }
                    ...
                    const setups = new Set(operations.map(o => o.setup_index));
            ...
            if (skippedCount > 0) {
                planWarnings.push(`${skippedCount} operation(s) filtered — not supported by ${machType}.`);
            }
            return {
                ...
                data: {
                    ...
                    kinematic_model: (machType === "five_axis_3plus2" || machType === "five_axis_simultaneous")
                        ? (ctx.kinematic_model ?? "trunnion_table") : undefined,
                    ...
                    multi_pass_ops: (() => {
                        const mp = operations.filter(o => (o.pass_count ?? 1) > 1).length;
                    ...
                    sync_point_count: (() => {
                        const sc = operations.filter(o => o.sync_before != null ||
                            o.sync_after != null).length;
    ...
    // ── S5: Tool Selection ────────────────────────────────────────────
    static async stageToolSelection(ctx) {
        const t0 = Date.now();
        const engines = ["SmartToolSelectorEngine"];
        const warnings = [];
        try {
            const operations = ctx.operation_plan ?? [];
            const isoGroup = ctx.iso_group;
            const kc = CANONICAL_KIENZLE[isoGroup];
            const toolRecommendations = operations.map(op => {
    ...
    static async stageStrategySelection(ctx) {
        const t0 = Date.now();
        const engines = ["OptimalStrategySelectionEngine"];
        try {
            const operations = ctx.operation_plan ?? [];
    ...
    static async stageSpeedFeedOptimization(ctx) {
        const t0 = Date.now();
        const engines = ["SpeedFeedOrchestratorEngine"];
        const warnings = [];
        try {
            const isoGroup = ctx.iso_group;
            const maxRpm = ctx.max_rpm;
            const maxPower = ctx.max_power_kw;
            const targetRa = ctx.target_ra_um;
            const materialName = ctx.material;
            const machineName = ctx.machine;
            const optimizeFor = ctx.optimize_for;
            const tools = ctx.tool_recommendations ?? [];
            const operations = ctx.operation_plan ?? [];
            // Map AutoProgram optimize_for → Orchestrator optimize_for
            const orchOptimize = optimizeFor === "time" ? "productivity"
            ...
            const physicsLog = [];
            const planned = operations.flatMap((op, i) => {
                ...
                if (isWireEdm) {
                    ...
                    const thickness = op.workpiece_thickness_mm ?? 25;
                    ...
                    return EDM_MULTI_CUT_TABLE.slice(0, passCount).map((pass, pi) => {
                        ...
                        return {
                            ...
                            tool,
                            strategy: (ctx.strategies?.[i]?.strategy) ?? op.type,
                ...
                const sfoInput = {
                    ...
                    workholding_type: isTurning ? "chuck" : WORKHOLDING_TO_SFO[ctx.workholding_type ?? "vise"],
                    workholding_stiffness: ctx.workholding_stiffness,
                    clamping_force_kN: ctx.clamping_force_kN,
                    // Turning: pass workpiece diameter for CSS RPM calculation
                    ...(isTurning ? {
                        workpiece_diameter_mm: op.diameter_mm ?? 50,
                        machine_type: "lathe",
                    } : {}),
                };
                try {
                    ...
                    return {
                        ...
                        operation_type: op.type,
                        tool,
                        strategy: (ctx.strategies?.[i]?.strategy) ?? "adaptive",
                        ...
                        ...(isTurning ? {
                            spindle_mode: op.spindle_mode,
                            css_surface_speed_m_min: op.css_surface_speed_m_min,
                            max_rpm_clamp: op.max_rpm_clamp,
                            fixed_rpm: op.fixed_rpm,
                            approach_type: op.approach_type,
                            retract_type: op.retract_type,
                            pass_count: op.pass_count,
                            depth_per_pass_mm: op.depth_per_pass_mm,
                        } : {}),
                        // Forward S4-assigned fields: channel, setup, sync
                        channel_id: op.channel_id,
                        spindle_id: op.spindle_id,
                        setup_index: op.setup_index,
                        sync_before: op.sync_before,
                        sync_after: op.sync_after,
                    };
                }
                catch (sfErr) {
     ...
     */
    static machineToSetupType(machType) {
        switch (machType) {
    ...
    static async stageCamCreation(ctx, input) {
        const t0 = Date.now();
        const engines = ["Fusion360LiveBridgeEngine"];
        const warnings = [];
        try {
            ...
            const whType = ctx.workholding_type ?? "vise";
            const setupCount = ctx.setup_count ?? 1;
            // ── Stock configuration per machine type ──
            const stockConfig = machType === "lathe" || machType === "mill_turn"
                ? { mode: "cylindrical", bar_diameter_mm: 80, bar_length_mm: 200 }
                : machType === "wire_edm"
                    ? { mode: "fixed_size", thickness_mm: 25 }
                    : { mode: "relative", offset_top_mm: 2, offset_sides_mm: 2, offset_bottom_mm: 0 };
            ...
            for (let si = 0; si < setupCount; si++) {
                ...
                if (!setupResult.success) {
                    warnings.push(`Setup ${si} creation warning: ${setupResult.error ?? "unknown"}`);
                }
            ...
            if (Object.keys(channelOps).length > 1) {
                warnings.push(`Mill-turn multi-channel: ${Object.entries(channelOps).map(([ch, n]) => `${ch}=${n}`).join(", ")} operations.`);
            }
            return {
                stage: "cam_creation",
                stage_index: 7,
    ...
    //
    static async stageVerification(ctx) {
        const t0 = Date.now();
        const engines = ["CAMKernelValidationEngine"];
        const warnings = [];
        try {
            const operations = ctx.planned_operations ?? [];
            const maxRpm = ctx.max_rpm;
            ...
            const isoGroup = ctx.iso_group;
            const whType = ctx.workholding_type ?? "vise";
            const clampForceKN = ctx.clamping_force_kN
            ...
            for (const op of operations) {
                ...
                if (isWireEdm) {
                    ...
                    if (op.wire_tension_N != null && (op.wire_tension_N < 2 || op.wire_tension_N > 25)) {
                        warnings.push(`Op ${op.sequence}: wire tension ${op.wire_tension_N}N outside safe range [2..25] N.`);
                    }
                    ...
                    if (op.wire_speed_m_min != null && (op.wire_speed_m_min < 1 || op.wire_speed_m_min > 20)) {
                        warnings.push(`Op ${op.sequence}: wire speed ${op.wire_speed_m_min} m/min outside range [1..20].`);
                    }
                    ...
                    if (op.flushing_pressure_bar != null && (op.flushing_pressure_bar < 1 || op.flushing_pressure_bar > 15)) {
                        warnings.push(`Op ${op.sequence}: flushing pressure ${op.flushing_pressure_bar} bar outside range [1..15].`);
                    }
                ...
                if (isTurning && op.spindle_mode === "G96" && op.max_rpm_clamp != null) {
                    ...
                    if (calcRpm != null && calcRpm > op.max_rpm_clamp) {
                        warnings.push(`Op ${op.sequence}: calculated RPM ${calcRpm} exceeds G50 clamp ${op.max_rpm_clamp}.`);
                    }
                }
                ...
                if (op.speed_rpm > 0 && op.tool.flute_count > 0) {
                    ...
                    if (power_kw > maxPower) {
                        warnings.push(`Op ${op.sequence}: estimated ${power_kw.toFixed(1)} kW exceeds ${maxPower} kW limit.`);
                    }
                }
            }
            ...
            if (opsWithForce.length > 0) {
                try {
                    ...
                    const cuttingForces = opsWithForce.map(op => ({
                    ...
                    verificationReport = workholdingVerificationEngine.verifyAllOperations(cuttingForces, whConfig);
                    // Propagate verification warnings
                    if (verificationReport.overall_verdict === "CRITICAL") {
                        warnings.push(`WORKHOLDING CRITICAL: ${verificationReport.limiting_operation} — ` +
                        ...
                        warnings.push(`WORKHOLDING WARNING: ${verificationReport.limiting_operation} — ` +
                    ...
                    for (const opResult of verificationReport.operations) {
                        if (opResult.resonance_warning) {
                            warnings.push(`${opResult.operation}: resonance risk — cutting freq near workholding natural freq.`);
                        }
                    }
                }
                catch {
                ...
                try {
                    ...
                    for (const op of opsWithForce) {
                        ...
                        if (result.required_force.value > clampForceKN * 1000) {
                            warnings.push(`Op ${op.sequence}: required clamp force ${result.required_force.value}N ` +
            ...
            const kinModel = ctx.kinematic_model
            ...
            if (is5Axis && kinModel) {
                ...
                for (const op of operations) {
                    ...
                    if (op.tool.overall_length_mm > FIVE_AXIS_COLLISION_ZONE.max_safe_tool_stickout_mm) {
                        warnings.push(`Op ${op.sequence}: tool L=${op.tool.overall_length_mm}mm exceeds ` +
                    ...
                    const featureNormal = ctx.features?.[op.feature_index]?.normal;
                    if (featureNormal) {
                        ...
                        const tiltDeg = Math.acos(Math.max(-1, Math.min(1, featureNormal.z))) * (180 / Math.PI);
                        ...
                        if (tiltDeg > maxTilt - FIVE_AXIS_COLLISION_ZONE.axis_limit_margin_deg) {
                            warnings.push(`Op ${op.sequence}: required tilt ${tiltDeg.toFixed(1)}° near ` +
                        ...
                        if (tiltDeg < limits.singularity_zone_deg && machType === "five_axis_simultaneous") {
                            warnings.push(`Op ${op.sequence}: tilt ${tiltDeg.toFixed(1)}° within singularity zone ` +
                    ...
                    for (const op of operations) {
                        ...
                        if (op.operation_type === "turning_drill" || op.operation_type === "turning_bore") {
                            ...
                            if (opDepth > tailstockZ - LATHE_TAILSTOCK_DEFAULTS.min_clearance_mm) {
                                warnings.push(`Op ${op.sequence} (${op.operation_type}): Z-depth ${opDepth.toFixed(1)}mm ` +
                        ...
                        if (op.operation_type === "turning_part_off" && tailstockPresent) {
                            warnings.push(`Op ${op.sequence}: part-off with tailstock present — ` +
            ...
            if (machType === "mill_turn") {
                ...
                if (MILL_TURN_INTERFERENCE.transfer_requires_sync) {
                    for (const op of operations) {
                        if (op.operation_type === "sub_spindle_transfer") {
                            if (!op.sync_before) {
                                warnings.push(`Op ${op.sequence}: sub_spindle_transfer missing sync_before — ` +
                                    `main spindle may still be rotating during transfer.`);
                                millTurnInterferenceCount++;
                            }
                            if (!op.sync_after) {
                                warnings.push(`Op ${op.sequence}: sub_spindle_transfer missing sync_after — ` +
                ...
                if (MILL_TURN_INTERFERENCE.milling_requires_orient && byChannel["Cm"]) {
                    for (const op of byChannel["Cm"]) {
                        if (!op.sync_before) {
                            warnings.push(`Op ${op.sequence} (${op.operation_type}, channel Cm): milling without ` +
                ...
                if (channelIds.length > 1) {
                    const lastOp = operations[operations.length - 1];
                    const hasProgramEndSync = operations.some(op => op.sync_after === MILL_TURN_SYNC_CODES.program_end.code);
                    if (!hasProgramEndSync) {
                        warnings.push(`Multi-channel program (${channelIds.join(",")}): no M299 program_end sync — ` +
    ...
    // ── S10: Output Package ───────────────────────────────────────────
    static async stageOutputPackage(ctx, input) {
        const t0 = Date.now();
        const engines = ["Fusion360LiveBridgeEngine", "SetupSheetEngine"];
        const warnings = [];
        try {
            const bridge = ctx.bridge;
            const operations = ctx.planned_operations ?? [];
            // Generate toolpaths
            if (bridge && ctx.ops_created > 0) {
                    ...
                    if (status !== "complete") {
                        ...
                        try {
                            ...
                            for (const op of operations) {
                                ...
                                }, { kc1_1: ctx.kc1_1 ?? 1800, mc: ctx.mc ?? 0.25, T_melt_C: ctx.T_melt_C ?? 1450 }, { max_rpm: ctx.max_rpm ?? 12000, max_power_kW: ctx.max_power_kw ?? 15 }, { material: "carbide", tensile_strength_MPa: 3500 }, { grip_force_N: 20000, friction_coefficient: 0.15 });
            ...
            if (isWireEdm) {
                ...
                sheetLines.push("SEQ | OPERATION           | WIRE SPD | TENSION | FLUSH | UV OFFSET", "----|--------------------|---------:|--------:|------:|----------:");
                for (const op of operations) {
                    const uv = op.uv_offset_mm;
                    sheetLines.push(`${String(op.sequence).padStart(3)} | ${op.operation_type.padEnd(18)} | ${(op.wire_speed_m_min ?? 0).toFixed(0).padStart(5)} m/m | ${(op.wire_tension_N ?? 0).toFixed(0).padStart(4)} N | ${(op.flushing_pressure_bar ?? 0).toFixed(0).padStart(3)} bar | ${uv != null && uv > 0 ? uv.toFixed(3) + " mm" : "—"}`);
                }
            }
            else if (isLatheType) {
                // Turning setup sheet: G96/G97, CSS speed, RPM clamp
                sheetLines.push("SEQ | OPERATION       | MODE | CSS/RPM  | CLAMP | TOOL                  | DOC" +
                ...
                for (const op of operations) {
                    ...
                    const chCol = machType === "mill_turn"
                        ? ` | ${op.channel_id ?? "C1"}`
                        : "";
                    sheetLines.push(`${String(op.sequence).padStart(3)} | ${op.operation_type.padEnd(14)} | ${mode} | ${speedCol.padStart(7)} | ${clampCol.padStart(5)} | D${op.tool.diameter_mm} ${op.tool.tool_type.substring(0, 12).padEnd(12)} | ${op.doc_mm.toFixed(1).padStart(5)}${chCol}`);
                }
            }
            else {
                // Milling setup sheet (original format)
                sheetLines.push("SEQ | OPERATION       | TOOL                  | RPM   | FEED    | DOC   | WOC", "----|----------------|-----------------------|-------|---------|-------|------");
                for (const op of operations) {
                    sheetLines.push(`${String(op.sequence).padStart(3)} | ${op.operation_type.padEnd(14)} | D${op.tool.diameter_mm} ${op.tool.flute_count}FL ${op.tool.tool_type.substring(0, 10).padEnd(10)} | ${String(op.speed_rpm).padStart(5)} | ${String(op.feed_mm_min).padStart(7)} | ${op.doc_mm.toFixed(1).padStart(5)} | ${op.woc_mm.toFixed(1).padStart(5)}`);
                }
            }
            ...
            const toolChangeBlocks = [];
            const operationBodyBlocks = [];
            for (let oi = 0; oi < operations.length; oi++) {
                const op = operations[oi];
     ...
     */
    static generateMachineHeader(machType, programName, ops) {
        const lines = [];
        const pgm = programName || "O1001";
        switch (machType) {
            case "vmc_3axis":
            case "hmc_4axis":
                lines.push(`%`, `${pgm} (PRISM AUTO-PROGRAM)`, `G17 G21 G40 G49 G80 G90 (INIT: XY PLANE, METRIC, CANCEL COMP)`, `G28 G91 Z0. (HOME Z)`);
                if (machType === "hmc_4axis") {
                    lines.push(`G28 B0. (HOME B-AXIS)`);
                }
                break;
            case "five_axis_3plus2":
                lines.push(`%`, `${pgm} (PRISM 5-AXIS 3+2)`, `G17 G21 G40 G49 G80 G90`, `G28 G91 Z0.`, `G43.4 H0 (RTCP ENABLE — TOOL CENTER POINT CONTROL)`);
                break;
            case "five_axis_simultaneous":
                lines.push(`%`, `${pgm} (PRISM 5-AXIS SIMULTANEOUS)`, `G17 G21 G40 G49 G80 G90`, `G28 G91 Z0.`, `G43.5 H0 (RTCP ENABLE — 5-AXIS SIMULTANEOUS TCPC)`);
                break;
            case "lathe": {
                ...
                lines.push(`%`, `${pgm} (PRISM TURNING)`, `G18 G21 G40 G80 G99 (INIT: XZ PLANE, METRIC, FEED/REV)`, `G28 U0. W0. (HOME X Z)`, `G50 S${clamp} (RPM CLAMP FOR CSS)`);
                break;
            }
            case "mill_turn": {
                ...
                lines.push(`%`, `${pgm} (PRISM MILL-TURN)`, `$1 (CHANNEL 1 — MAIN SPINDLE)`, `G18 G21 G40 G80 G99`, `G28 U0. W0.`, `G50 S${clamp}`);
                if (hasSubSpindle) {
                    lines.push(`$2 (CHANNEL 2 — SUB-SPINDLE)`, `G18 G21 G40 G80 G99`);
                }
                if (hasLiveTooling) {
                    lines.push(`$3 (CHANNEL 3 — MILLING SPINDLE)`, `G17 G21 G40 G49 G80 G90`);
                }
                break;
            }
            case "wire_edm":
                lines.push(`%`, `${pgm} (PRISM WIRE EDM)`, `G90 G92 X0. Y0. (ABS MODE, DATUM SET)`, `G21 (METRIC)`, `M50 (WIRE THREADING)`, `M01 (OPTIONAL STOP — VERIFY WIRE THREADED)`);
                break;
        }
        return lines;
    }
     ...
     */
    static generateMachineFooter(machType) {
        switch (machType) {
            case "vmc_3axis":
            case "hmc_4axis":
                return [
     ...
     */
    static generateApproachRetract(op) {
        const approach = [];
        const retract = [];
     ...
     */
    static generateOperationBody(op, machType) {
        const lines = [];
        ...
        switch (op.operation_type) {
            ...
            case "trochoidal":
                ...
                if (passes > 5)
                    lines.push(`(... ${passes - 5} more passes)`);
            ...
            case "turning_rough": {
                ...
                if (op.max_rpm_clamp)
                    lines.push(`G50 S${op.max_rpm_clamp} (RPM CLAMP)`);
     ...
     */
    static generateToolChange(machType, op, toolIndex) {
        const lines = [];
        const tNum = toolIndex + 1; // 1-based tool number
        switch (machType) {
            case "vmc_3axis":
            case "hmc_4axis":
                lines.push(`G28 G91 Z0. (TOOL CHANGE SAFE Z)`, `T${tNum} M06 (TOOL ${tNum}: ${op.tool.tool_type} D${op.tool.diameter_mm})`, `G43 H${tNum} Z50. (LENGTH COMP + RAPID TO CLEARANCE)`);
                break;
            case "five_axis_3plus2":
                lines.push(`G49 (CANCEL RTCP BEFORE TOOL CHANGE)`, `G28 G91 Z0.`, `T${tNum} M06 (TOOL ${tNum}: ${op.tool.tool_type} D${op.tool.diameter_mm})`, `G43.4 H${tNum} (RTCP RE-ENGAGE)`);
                break;
            case "five_axis_simultaneous":
                lines.push(`G49 (CANCEL TCPC BEFORE TOOL CHANGE)`, `G28 G91 Z0.`, `T${tNum} M06 (TOOL ${tNum}: ${op.tool.tool_type} D${op.tool.diameter_mm})`, `G43.5 H${tNum} (TCPC RE-ENGAGE)`);
                break;
            case "lathe": {
                ...
                lines.push(`G28 U0. (SAFE X HOME BEFORE TURRET INDEX)`, `T${turretPos}${offsetNum} (TURRET ${tNum}: ${op.tool.tool_type})`);
                break;
            }
            case "mill_turn": {
                const ch = op.channel_id ?? "C1";
                if (ch === "Cm") {
                    ...
                    lines.push(`G28 G91 Z0.`, `T${tNum} M06 (MILLING TOOL ${tNum}: ${op.tool.tool_type} D${op.tool.diameter_mm})`, `G43 H${tNum}`);
                }
                else {
                    // Turning channels (C1/C2): turret index
                    const turretPos = String(tNum).padStart(2, "0");
                    lines.push(`T${turretPos}${turretPos} (TURRET ${tNum}: ${op.tool.tool_type})`);
                }
                break;
            }
            case "wire_edm":
     ...
     */
    static generateCoolantOn(machType, op) {
        if (machType === "wire_edm") {
    ...
    static generateCoolantOff(machType) {
        if (machType === "wire_edm") {
     ...
     */
    static generateChannelSyncLines(ops) {
        const result = [];
     ...
     */
    static estimateCycleTime(operations, machType, numSetups) {
        ...
        const toolSet = new Set(operations.map(o => `${o.tool.tool_type}|${o.tool.diameter_mm}`));
    ...
    static featureToOperationType(featureType) {
        const map = {
            face: "face_mill",
    ...
    static operationToToolType(opType) {
        const map = {
            face_mill: "face mill",
    ...
    static selectStrategy(opType, _featureType) {
        if (opType === "face_mill")
            return "face";
        if (opType === "adaptive_clear")
            return "adaptive";
        if (opType === "pocket_2d")
            return "pocket2d";
        if (opType === "contour_2d")
            return "contour2d";
        if (opType === "drill_peck")
            return "drill";
        if (opType === "bore")
            return "bore";
        if (opType === "chamfer")
        ...
        if (opType === "swarf")
            return "swarf_5axis";
        if (opType === "flow_cut")
            return "flowline_5axis";
        if (opType === "blade_machining")
            return "blade_5axis";
        if (opType === "impeller")
            return "impeller_5axis";
        if (opType === "port_machining")
            return "port_5axis";
        if (opType === "indexed_5axis")
            return "indexed_3plus2";
        if (opType === "multiaxis_contour")
            return "multiaxis_contour_5axis";
        // Turning strategies
        if (opType === "turning_rough")
            return "turning_rough";
        if (opType === "turning_finish")
            return "turning_finish";
        if (opType === "turning_bore")
            return "turning_bore";
        if (opType === "turning_face")
            return "turning_face";
        if (opType === "turning_groove")
            return "turning_groove";
        if (opType === "turning_thread")
            return "turning_thread";
        if (opType === "turning_part_off")
            return "turning_part_off";
        if (opType === "turning_drill")
            return "turning_drill";
        // Mill-turn
        if (opType === "live_tool_mill")
            return "adaptive";
        if (opType === "sub_spindle_transfer")
            return "sub_spindle_transfer";
        // Wire EDM strategies
        if (opType === "wire_profile")
            return "wire_profile";
        if (opType === "wire_4axis_taper")
            return "wire_4axis_taper";
        if (opType === "wire_no_core")
            return "wire_no_core";
        if (opType === "wire_open_pocket")
            return "wire_open_pocket";
        return "adaptive";
    }
    /** Compute overall confidence from stage results (0-1). */
    static computeConfidence(stages) {
        if (stages.length === 0)
            return 0;
...
//# sourceMappingURL=AutoProgramOrchestratorEngine.js.map