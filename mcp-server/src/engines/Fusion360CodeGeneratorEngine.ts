...
const DESCRIPTION_PATTERNS: DescriptionPattern[] = [
  {
    regex: /(?:box|block|rect(?:angle)?)\s+(\d+(?:\.\d+)?)\s*[x×]\s*(\d+(?:\.\d+)?)\s*(?:[x×]\s*(\d+(?:\.\d+)?))?/i,
    actions: (m) => {
    ...
    for (const pattern of DESCRIPTION_PATTERNS) {
      ...
      if (match) {
        ...
        for (const a of parsed) {
          // Avoid duplicate step 99 fillets if already added via another pattern
          if (a.step_number === 99 && allActions.some((e) => e.action_type === "fillet")) continue;
          allActions.push(a);
        }
      }
    }
