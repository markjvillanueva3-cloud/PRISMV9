...
class PayrollEngine {
  ...
  configureDeductions(config: DeductionConfig): DeductionConfig {
    ...
    persistenceBridge.persist("payroll_deductions", config.employee_id, config as any);
    return config;
  }
  ...
  createPeriod(input: {
    ...
    persistenceBridge.persist("payroll_periods", id, period as any);
    return period;
  }
  ...
  calculatePayStub(employeeId: string, periodId: string): PayStub {
    ...
    persistenceBridge.persist("payroll_ytd", employeeId, { id: employeeId, ...ytd } as any);
  ...
  runPayroll(periodId: string): PayrollSummary {
    ...
    persistenceBridge.persist("payroll_periods", periodId, period as any);
...
persistenceBridge.registerMap({
  ...
  getMap: () => (payrollEngine as any).periods as Map<string, any>,
  keyField: "id",
});
persistenceBridge.registerMap({
  entity: "payroll_deductions",
  getMap: () => (payrollEngine as any).deductions as Map<string, any>,
  keyField: "employee_id",
});
persistenceBridge.registerMap({
  entity: "payroll_ytd",
  getMap: () => (payrollEngine as any).ytdEarnings as Map<string, any>,
  keyField: "id",
});
