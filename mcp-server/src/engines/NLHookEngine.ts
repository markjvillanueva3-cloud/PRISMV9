...
function safeEvalCondition(conditionCode: string, ctx: Record<string, any>): boolean {
  ...
  try {
    // MS1 U-LAT11: Use safeMathEval for simple conditions, fall back to validated Function for complex
    // The BLOCKED pattern above catches dangerous code; this is validated dynamic execution
    const frozenCtx = Object.freeze({ ...ctx });
    const { safeMathEval } = require("../utils/safeMathEval.js");
    // Try safeMathEval first for simple numeric conditions
    try {
      const result = safeMathEval(conditionCode, frozenCtx as Record<string, number>);
      return !!result;
    } catch {
      // Fall back to validated Function for complex boolean logic (already BLOCKED-checked above)
      const fn = new Function(
        'ctx',
        `"use strict"; return !!(${conditionCode});`
      );
      return !!fn(frozenCtx);
    }
  } catch (e: any) {
...
export class NLHookEngine {
  ...
  private buildHookDefinition(hookId: string, spec: HookSpec, compileResult: CompileResult): HookDefinition {
    // Safety: validate compiled code before building functions (same blocklist as safeEvalCondition)
    const BLOCKED = /\b(eval|import|require|process|globalThis|window|document|fetch|XMLHttpRequest|fs|child_process|exec|spawn)\b|\.\s*constructor/;
    if (BLOCKED.test(compileResult.condition_code)) {
      throw new Error(`Unsafe condition code blocked for hook ${hookId}`);
    }
    // MS1 U-LAT11: Build condition function with strict mode (BLOCKED-validated above)
    // Note: new Function() here is intentional - hook code is validated by BLOCKED pattern
    const condFn = new Function('ctx', `"use strict"; ${compileResult.condition_code}`) as (ctx: HookContext) => boolean;
    // Build handler function
    ...
    if (BLOCKED.test(handlerTemplate)) {
      throw new Error(`Unsafe handler code blocked for hook ${hookId}`);
    }
    const handlerFn = new Function('context', `"use strict"; ${handlerTemplate}`) as (ctx: HookContext) => HookResult;
