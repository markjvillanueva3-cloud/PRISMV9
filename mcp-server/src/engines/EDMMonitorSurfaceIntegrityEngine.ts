 ...
 *   - Residual stress: tensile, 200–800 MPa thermal contraction model
...
const MATERIAL_ALPHA: Record<string, number> = {
  ...
  "Al 7075":      70.0,
  "Cu":          116.0,
  "WC":           10.0,   // cemented carbide (cobalt binder)
  "default":       6.0,
};
...
function recastLayerPredictor(
  ...
  const d_rough_m = 2 * Math.sqrt(alpha_mm2_s * 1e-6 * t_on_s); // metres
...
function hazDepthCalculator(
  ...
  const depth_um = recast_depth_um * 3;
...
function residualStressEstimator(
  ...
  // Linear model: 200 MPa baseline, +600 MPa over 0–5 mJ range
  const magnitude_MPa = Math.round(
    Math.min(800, 200 + (Math.min(pulse_energy_mj, 5) / 5) * 600)
  );
...
export class EDMMonitorSurfaceIntegrityEngine {
  ...
  assessSurfaceIntegrity(input: SurfaceIntegrityInput): SurfaceIntegrityResult {
    ...
    // Unit 2: HAZ
    const haz = hazDepthCalculator(recast.depth_um, hardness);
    ...
    // Unit 4: Residual stress
    const residual_stress = residualStressEstimator(input.pulse_energy_mj, haz.depth_um);
