...
/**
 * Kienzle coefficients for 12 common ISO material groups.
...
const KIENZLE_TABLE: KienzleCoefficients[] = [
  ...
  { kc1_1: 1650, mc: 0.19, iso_group: "S", description: "Ti-6Al-4V" },
  { kc1_1: 2800, mc: 0.18, iso_group: "S", description: "Inconel 718" },
  { kc1_1: 3800, mc: 0.20, iso_group: "H", description: "Hardened steel 55 HRC" },
  { kc1_1: 4500, mc: 0.17, iso_group: "H", description: "Hardened steel 62 HRC" },
];
...
export class KienzleForceModelEngine {
  ...
  calculateForceComponents(params: ForceComponentsInput): ForceComponentsResult {
    ...
    if (operation === "turning") {
      ...
      // Average chip thickness over engagement: h_avg = fz * ae / (pi * D) * sin(kappa_r)
      // More precisely: h_avg = fz * sin(engagement/2) * sin(kappa_r) for slot/partial
      const h_avg = feed_mm * (ae / (Math.PI * D)) * sin_kappa;
  ...
  predictFromMaterialHardness(params: HardnessPredictionInput): HardnessPredictionResult {
    ...
    if (material_family === "aluminum") {
      // Aluminum: different correlation
