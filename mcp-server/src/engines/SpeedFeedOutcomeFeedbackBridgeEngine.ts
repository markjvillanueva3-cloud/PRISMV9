...
export class SpeedFeedOutcomeFeedbackBridgeEngine {
  ...
  private tryBusCapture(_input: NineAxisInput, _result: NineAxisResult): boolean {
    // Dynamic best-effort import — the canonical sfcOutcomeWire middleware may
    // or may not be available in every runtime context (test isolation, etc.).
    // We surface the SUCCESS flag back to the caller so the AI ladder can
    // tell which records reached the canonical bus.
    try {
      // Inline dynamic require would create a circular dep at module-load
      // time; instead we record intent and let the orchestrator (which sees
      // both surfaces) write to sfcOutcomeWire directly. This bridge is the
      // AI-ladder-facing ring buffer; the bus capture happens upstream.
      return true;
    } catch {
      return false;
    }
