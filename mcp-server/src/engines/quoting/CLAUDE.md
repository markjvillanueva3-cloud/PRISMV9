...
- Training loop coverage: 2 of 5 sources (40%); unconsumed next: `jm-vendor-cost-index.json` (units-gated, see below), `docustrata-invoices.curated.json`. NOTE: `jm-tool-purchases.json` is now SURFACED as an advisory per-type cost prior via **`prism_quoting:tool_cost_basis`** (`ConsumableCostBasisEngine`, U-QP-CONSUMABLE-COST-BASIS 2026-07-01) -- but that is a forward COST-prior + reconciliation fold-back, NOT a training source (per-type cost-input grain != the loop's per-part-job revenue grain).
