...
- **`quoting_secondary_ops_price` (PLAIN variant) auth-migration** (P2): has a shipped token-less caller
  ...
  (the U-MKTPRICE01 pattern), then deny the generic path. A frontend-touching unit, not a pure deny -- defer
  to a page-aware pass. (Its `_for_profile` sibling -- no caller -- was DENIED in U-MKTPRICE02.)
