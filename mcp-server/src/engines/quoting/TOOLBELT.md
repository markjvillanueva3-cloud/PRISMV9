...
- **VERIFIED PATH (U-CHARLIE-QUOTING-OUTCOME-WIRE 2026-06-27):** `QuotingOutcomeCaptureWireEngine.recordOutcome(QuoteOutcomeRecord)` → `outcomeCaptureBusEngine {domain:'quote'}` → P0-U04 `OutcomeCaptureBusToFeedbackBridge` → india's learner. Already wired into `QuotingClosedLoopRunnerEngine.buildLiveDeps.fetchOutcomes` (every observed cycle outcome auto-emits); you do NOT call it by hand.
- ⚠️ `xproc_outcome_publish {slot:'charlie'}` / `xproc_calibration_monitor_record` / `xproc_kg_project_features` exist as `aiReasoningDispatcher` actions but quoting does NOT feed the learner through them (it was doc-only/never wired — the capture bus IS the path). Use these only for a deliberate, separately-validated direct publish.
...
- **Domain-soul agent (spawnable expert, 2026-06-30):** this domain is a dispatchable subagent `charlie-quoting` (Agent tool `subagent_type: "charlie-quoting"`) carrying the charlie soul persona + refuse-list + quoting knowledge — spawn it for ANY quoting task (build/review/audit). Route a task to its agent + the hybrid Claude/Hermes/Ollama lane via `prism_session:domain_soul_agent_route`. Regenerate after a soul edit: `tsx scripts/generate-domain-soul-agents.mts`. Wiki: [[domain-soul-agents]].
<!-- /OPERATIONAL-CONTEXT -->
