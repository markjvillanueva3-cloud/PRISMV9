...
export class MultiProcessCAMBridgeEngine {
  ...
  processAll(
    ...
    const combinedGcode = results.map(r => r.gcode).join("\n\n");
  ...
  private _processTurning(
    ...
    const vcMap: Record<string, number> = { P: 200, M: 120, K: 250, N: 400, S: 50, H: 100 };
    const vc = vcMap[iso] || 200;
    const maxRpm = machine?.max_rpm || 4000;
    const rpm = Math.min(maxRpm, Math.round((vc * 1000) / (Math.PI * od)));

    // Feed and DOC
    const fMap: Record<string, number> = { P: 0.25, M: 0.20, K: 0.30, N: 0.35, S: 0.12, H: 0.08 };
    const f = fMap[iso] || 0.25; // mm/rev
    ...
    return {
      ...
      physics: {
        ...
        surface_finish_Ra: Math.round((f * f * 1000) / (8 * 0.8) * 100) / 100, // Ra ≈ f²/(8R)
  ...
  private _processMillingStub(
    ...
    return {
      process: "milling",
      feature_id: feature.id,
      gcode: `(MILLING — delegate to UnifiedCAMPipeline for ${feature.type})`,
      gcode_lines: 1,
      parameters: {},
      physics: {},
