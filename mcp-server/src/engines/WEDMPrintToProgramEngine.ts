...
import { selectECodeFamily, getECodeForPass, getShopFeedForPass, getShopOffsetForPass } from "../data/jm-die-wedm-tech-tables.js";
import type { ECodeFamily } from "../data/jm-die-wedm-tech-tables.js";
...
export interface WEDMProgramInput {
  ...
  /** STEP file content (AP203/AP214 ASCII) — geometry extracted via StepImportEngine */
  step_content?: string;
  /** IGES file content (ASCII) — geometry extracted via IgesImportEngine or inline fallback parser */
  iges_content?: string;
...
function passDetailsToEDMPasses(
  ...
  // Shop-calibrated tech table lookup (JM Die Mitsubishi FA-10S)
  const shopFamily = selectECodeFamily({
    material,
    taper_angle_deg: input.taper_angle_deg,
    tolerance_mm: input.target_accuracy_mm,
  ...
  return details.map((d, idx) => {
    ...
    // If shop-calibrated offset exists and no manual override, prefer it
    const shopOffset = shopFamily ? getShopOffsetForPass(shopFamily, d.pass_number) : 0;

    if (input.offset_overrides_mm?.[idx] != null) {
      offset = input.offset_overrides_mm[idx]!;
    } else if (shopOffset > 0) {
      // Use shop-calibrated offset (already includes wire radius + spark gap)
      offset = shopOffset + wireCompensation + stockAdj;
      offset = Math.max(0.005, offset);
    } else {
      // Fall back to physics-derived offset
      offset += wireCompensation + stockAdj;
      offset = Math.max(0.005, offset);
    }

    // Feed rate: prefer shop-calibrated, then physics-derived, then override
    let feedRate = d.cutting_speed_mm_min > 0 ? d.cutting_speed_mm_min : d.wire_speed_m_min;
    const shopFeed = shopFamily ? getShopFeedForPass(shopFamily, d.pass_number) : null;
    if (input.feed_overrides_mm_min?.[idx] != null) {
      feedRate = input.feed_overrides_mm_min[idx]!;
    } else if (shopFeed != null) {
      // Blend: shop-calibrated feed weighted 60%, physics 40% for robustness
      feedRate = shopFeed * 0.6 + feedRate * 0.4;
    }

    // E-code: prefer shop-calibrated (machine-specific), fall back to generic
    const eCode = shopFamily
      ? getECodeForPass(shopFamily, d.pass_number)
      : generateEPackCode(
...
export class WEDMPrintToProgramEngine {
  ...
  async generate(input: WEDMProgramInput): Promise<WEDMProgramResult> {
    ...
    } else if (input.step_content) {
      const stepResult = await this.extractContoursFromStep(input.step_content);
      if (!stepResult.success || !stepResult.contours) {
        return { ...this._failResult(input, [stepResult.error ?? "STEP geometry extraction failed"]), stages_completed: stages };
      }
      contours = stepResult.contours;
      warnings.push("STEP import: 3D geometry projected to 2D XY plane — verify contour orientation before cutting");
      stages.push("step_imported");
    } else if (input.iges_content) {
      const igesResult = await this.extractContoursFromIges(input.iges_content);
      if (!igesResult.success || !igesResult.contours) {
        return { ...this._failResult(input, [igesResult.error ?? "IGES geometry extraction failed"]), stages_completed: stages };
      }
      contours = igesResult.contours;
      warnings.push("IGES import: geometry projected to 2D XY plane — verify contour orientation before cutting");
      stages.push("iges_imported");
    } else {
      return { ...this._failResult(input, ["No geometry provided — supply dxf_content, step_content, iges_content, or contours"]), stages_completed: stages };
    }
  ...
  /**
   * Extract 2D contours from a STEP file via bounding-box projection.
   * Uses StepImportEngine to parse 3D geometry, then projects the XY
   * bounding box into a closed rectangular WireEDMContour.
   */
  private async extractContoursFromStep(
    stepContent: string,
  ): Promise<{ success: boolean; contours?: WireEDMContour[]; error?: string }> {
    let tempDir: string | null = null;
    let tempFile: string | null = null;
    try {
      tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "prism-step-"));
      tempFile = path.join(tempDir, "import.step");
      fs.writeFileSync(tempFile, stepContent, "utf8");

      const { stepImportEngine } = await import("./StepImportEngine.js");
      const importResult = await stepImportEngine.importStep({ file_path: tempFile });

      const bb = importResult.bounding_box;
      if (!bb || !Number.isFinite(bb.min.x) || !Number.isFinite(bb.max.x)) {
        return { success: false, error: "STEP import failed: bounding box could not be determined. Use DXF format for reliable geometry import." };
      }

      const minX = bb.min.x, minY = bb.min.y, maxX = bb.max.x, maxY = bb.max.y;
      const width = maxX - minX, height = maxY - minY;
      if (width <= 0 || height <= 0) {
        return { success: false, error: `STEP import failed: degenerate XY footprint (${width.toFixed(3)} x ${height.toFixed(3)} mm). Use DXF format.` };
      }

      const bl = { x: minX, y: minY }, br = { x: maxX, y: minY };
      const tr = { x: maxX, y: maxY }, tl = { x: minX, y: maxY };
      ...
      const contour: WireEDMContour = {
        id: "step_bbox_contour_0", segments, is_closed: true, is_exterior: true,
        area_mm2: width * height, perimeter_mm: 2 * (width + height),
        bbox: { min_x: minX, min_y: minY, max_x: maxX, max_y: maxY },
      };
      return { success: true, contours: [contour] };
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      return { success: false, error: `STEP import failed: ${msg}. Use DXF format for reliable geometry import.` };
    } finally {
      try { if (tempFile) fs.unlinkSync(tempFile); } catch { /* ignore */ }
      try { if (tempDir) fs.rmdirSync(tempDir); } catch { /* ignore */ }
    }
  }

  /**
   * Extract 2D contours from an IGES file via bounding-box projection.
   * Parses IGES entity 110 (LINE) and entity 100 (CIRCULAR ARC) from the
   * parameter data section to build basic 2D contours. Falls back to
   * bounding-box rectangle if entity extraction fails.
   */
  private async extractContoursFromIges(
    igesContent: string,
  ): Promise<{ success: boolean; contours?: WireEDMContour[]; error?: string }> {
    try {
      // Parse IGES parameter data section for entity 110 (LINE) segments
      const lines = igesContent.split("\n");
      const paramLines = lines.filter(l => l.length >= 73 && l[72] === "P");
      if (paramLines.length === 0) {
        return { success: false, error: "IGES import failed: no parameter data section found. Verify file is valid IGES format." };
      }

      // Extract LINE entities (type 110) — columns give start/end XY
      const segments: GeometrySegment[] = [];
      let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;

      // Join parameter continuation lines and split by record delimiter ";"
      const paramText = paramLines.map(l => l.substring(0, 64).trim()).join("");
      const records = paramText.split(";").filter(r => r.trim().length > 0);

      for (const record of records) {
        const values = record.split(",").map(v => v.trim());
        if (values.length < 1) continue;
        const entityType = parseInt(values[0], 10);

        if (entityType === 110 && values.length >= 7) {
          // Line entity: type,x1,y1,z1,x2,y2,z2
          const x1 = parseFloat(values[1]), y1 = parseFloat(values[2]);
          const x2 = parseFloat(values[4]), y2 = parseFloat(values[5]);
          if (Number.isFinite(x1) && Number.isFinite(y1) && Number.isFinite(x2) && Number.isFinite(y2)) {
            segments.push({ type: "line", start: { x: x1, y: y1 }, end: { x: x2, y: y2 } });
      ...
      // If we extracted at least 3 line segments, try to build a closed contour
      if (segments.length >= 3) {
        ...
        const contour: WireEDMContour = {
          id: "iges_extracted_contour_0", segments, is_closed: true, is_exterior: true,
          area_mm2: width * height, perimeter_mm: perimeter,
          bbox: { min_x: minX, min_y: minY, max_x: maxX, max_y: maxY },
        };
        return { success: true, contours: [contour] };
      }

      // Fallback: if no line entities found, try bounding box from any numeric data
      if (!Number.isFinite(minX) || minX === Infinity) {
        return { success: false, error: "IGES import failed: no LINE entities (type 110) found. Convert to DXF for reliable import." };
      }

      const width = maxX - minX, height = maxY - minY;
      if (width <= 0 || height <= 0) {
        return { success: false, error: `IGES import failed: degenerate geometry (${width.toFixed(3)} x ${height.toFixed(3)} mm).` };
      }

      // Build bounding-box rectangle as fallback
      const bl = { x: minX, y: minY }, br = { x: maxX, y: minY };
      const tr = { x: maxX, y: maxY }, tl = { x: minX, y: maxY };
      const fallbackSegments: GeometrySegment[] = [
        { type: "line", start: bl, end: br },
        { type: "line", start: br, end: tr },
        { type: "line", start: tr, end: tl },
        { type: "line", start: tl, end: bl },
      ...
      const contour: WireEDMContour = {
        id: "iges_bbox_contour_0", segments: fallbackSegments, is_closed: true, is_exterior: true,
        area_mm2: width * height, perimeter_mm: 2 * (width + height),
        bbox: { min_x: minX, min_y: minY, max_x: maxX, max_y: maxY },
      };
      return { success: true, contours: [contour] };
      ...
      const msg = err instanceof Error ? err.message : String(err);
      return { success: false, error: `IGES import failed: ${msg}. Convert to DXF for reliable import.` };
    }
  }
