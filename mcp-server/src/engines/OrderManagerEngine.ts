...
class OrderManagerEngineImpl {
  ...
  createOrder(spec: OrderSpec): Order {
    ...
    persistenceBridge.persist("order_manager_orders", id, order as any);
    return { ...order };
  }
  ...
  updateOrderStatus(
    ...
    persistenceBridge.persist("order_manager_orders", orderId, order as any);
    return { ...order };
  }
  ...
  createWorkOrder(spec: WorkOrderSpec): WorkOrder {
    ...
    persistenceBridge.persist("order_manager_work_orders", id, wo as any);
    order.workOrders.push(id);
    order.updatedAt = now;
    persistenceBridge.persist("order_manager_orders", spec.orderId, order as any);
    return { ...wo };
  }
  ...
  updateWorkOrderStatus(woId: string, status: WorkOrderStatus): WorkOrder {
    ...
    persistenceBridge.persist("order_manager_work_orders", woId, wo as any);
    return { ...wo };
  }
  ...
  logProduction(
    ...
    if (order) {
      ...
      persistenceBridge.persist("order_manager_orders", wo.orderId, order as any);
    }
...
persistenceBridge.registerMap({
  entity: "order_manager_orders",
  getMap: () => (orderManagerEngine as any).orders as Map<string, any>,
  keyField: "id",
});
persistenceBridge.registerMap({
  entity: "order_manager_work_orders",
  getMap: () => (orderManagerEngine as any).workOrders as Map<string, any>,
  keyField: "id",
});
