...
const DEFAULT_RULES: RateLimitRule[] = [
  ...
  { id: "RL-BILLING", name: "Billing Endpoints", algorithm: "fixed_window", scope: "per_ip", max_requests: 30, window_sec: 300, burst_allowance: 5, cooldown_sec: 60 },
...
export class RateLimitEngine {
  ...
  check(ruleId: string, key: string): RateLimitCheckResult {
    ...
    const now = Date.now();
    /** If.
     * @param bucket.cooldown_until - bucket.cooldown_until
     * @returns void
     */
  ...
  consume(ruleId: string, key: string, count: number = 1): RateLimitCheckResult {
    ...
    const rule = this.rules.get(ruleId);
    if (!rule) return checkResult;

    const bucketKey = `${ruleId}:${key}`;
    const bucket = this.buckets.get(bucketKey);
    if (!bucket) return checkResult;
    ...
    /** If.
     * @param bucket.tokens - bucket.tokens
     * @returns void
     */
  ...
  private refillBucket(rule: RateLimitRule, bucket: BucketState): void {
    ...
    /** If.
     * @param rule.algorithm - rule.algorithm
     * @returns void
     */
