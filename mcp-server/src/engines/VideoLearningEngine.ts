...
export interface VideoProcessResult {
  ...
  playbook_rules_extracted: number;
  playbook_rules: ExtractedPlaybookRule[];
}

/** Auto-extracted playbook rule candidate from a video */
export interface ExtractedPlaybookRule {
  id: string;
  category: string;       // mapped to RuleCategory
  severity: "critical" | "important" | "recommended" | "tip";
  title: string;
  rule: string;
  reasoning: string;
  source: string;          // "VideoLearning — <channel> — <title> (<URL|path>)"
  added_to_playbook: boolean;
  dedup_reason?: string;
}
...
export interface VideoProcessOptions {
  ...
  channel?: string;                // YouTube channel or creator name, used for playbook provenance
  video_url?: string;              // original URL/source, used for playbook provenance
}
...
[{"timestamp": <seconds>, "description": "...", "extracted_values": {"param": "value"}, "ui_elements": ["..."], "category": "..."}]`;
  ...
  /**
   * Extract PlaybookRule candidates from VideoKnowledgeItems by scanning for
   * sequencing/best-practice/anti-pattern phrasing. Uses keyword heuristics to
   * classify rule category and severity.
   *
   * Dedup is delegated to the caller (via playbook.addRule throwing on ID clash)
   * plus a title-similarity check against existing rules.
   */
  extractPlaybookRules(
    knowledgeItems: VideoKnowledgeItem[],
    videoPath: string,
    channel?: string,
    videoUrl?: string
  ): ExtractedPlaybookRule[] {
    const videoName = path.basename(videoPath, path.extname(videoPath));
    const provenance = videoUrl
      ? `VideoLearning — ${channel ?? "unknown"} — ${videoName} (${videoUrl})`
      : `VideoLearning — ${channel ?? "unknown"} — ${videoName} (${videoPath})`;

    const rules: ExtractedPlaybookRule[] = [];
    const usedSlugs = new Set<string>();

    const ANTI_PATTERN_MARKERS = /\b(never|avoid|don't|do not|shouldn't|should not|bad practice|pitfall|wrong)\b/i;
    const SEQUENCING_MARKERS = /\b(before|after|first|then|next|sequence|order|ordering|precedes|follows)\b/i;
    const SETUP_MARKERS = /\b(fixture|workholding|clamp|datum|jaw|vise|chuck|collet|setup strategy)\b/i;
    const FINISHING_MARKERS = /\b(finish pass|finishing|surface finish|ra\s*\d|surface quality)\b/i;
    const ROUGHING_MARKERS = /\b(roughing|stock removal|hog out|bulk removal|adaptive clearing)\b/i;
    const THIN_WALL_MARKERS = /\b(thin wall|thin floor|deflection|chatter|harmonics)\b/i;
    const MATERIAL_MARKERS = /\b(aluminum|steel|stainless|titanium|inconel|brass|copper|tool steel|4140|6061|316)\b/i;
    const TOOLPATH_MARKERS = /\b(toolpath|adaptive|trochoidal|climb|conventional|spiral|contour|ramp)\b/i;
    const SAFETY_MARKERS = /\b(crash|collision|safe height|rapid|g00|safe plane)\b/i;
    const COOLANT_MARKERS = /\b(coolant|flood|mist|mql|through spindle|through tool|air blast)\b/i;
    ...
    for (const item of knowledgeItems) {
      ...
      let category: string;
      let severity: "critical" | "important" | "recommended" | "tip";

      if (ANTI_PATTERN_MARKERS.test(text)) {
        category = "anti_pattern";
        severity = "critical";
      } else if (SAFETY_MARKERS.test(text)) {
        category = "safety";
        severity = "critical";
      } else if (SEQUENCING_MARKERS.test(text) && !FINISHING_MARKERS.test(text)) {
        category = "sequencing";
        severity = "important";
      } else if (SETUP_MARKERS.test(text)) {
        category = "setup_strategy";
        severity = "important";
      } else if (THIN_WALL_MARKERS.test(text)) {
        category = "thin_wall";
        severity = "important";
      } else if (FINISHING_MARKERS.test(text)) {
        category = "finishing";
        severity = "recommended";
      } else if (ROUGHING_MARKERS.test(text)) {
        category = "roughing";
        severity = "recommended";
      } else if (COOLANT_MARKERS.test(text)) {
        category = "coolant_strategy";
        severity = "recommended";
      } else if (TOOLPATH_MARKERS.test(text)) {
        category = "toolpath_strategy";
        severity = "recommended";
      } else if (MATERIAL_MARKERS.test(text)) {
        category = "material_tip";
        severity = "tip";
      } else {
        continue; // not a rule-shaped item
      }
      ...
      const slugBase = item.title
        ...
        .replace(/^-+|-+$/g, "")
        .substring(0, 40);
      if (!slugBase || usedSlugs.has(slugBase)) continue;
      usedSlugs.add(slugBase);

      const id = `PB-VL-${slugBase}-${Date.now().toString(36).slice(-4)}`;
      const reasoning = item.transcript_excerpt
        ? `Observed in ${videoName}: "${item.transcript_excerpt.substring(0, 200)}"`
        : `Derived from video frame analysis at ${item.frame_reference ?? "unknown"}.`;
      ...
      rules.push({
        ...
        rule: item.body.substring(0, 300),
        reasoning,
        source: provenance,
        added_to_playbook: false,
      });
    }
    return rules;
  }
  ...
  async processVideo(
    ...
    // Step 6: Extract playbook rule candidates from knowledge items
    const extractedRules = this.extractPlaybookRules(
      knowledgeItems,
      videoPath,
      opts.channel,
      opts.video_url
    ...
    // Attempt to register unique rules with MachiningPlaybookEngine (dedup by title + ID)
    let addedCount = 0;
    if (extractedRules.length > 0 && !opts.dry_run) {
      try {
        const { machiningPlaybookEngine } = require("./MachiningPlaybookEngine.js");
        // Build title set from all known categories (scan all rules)
        const allCategories = [
          "sequencing", "setup_strategy", "tool_selection", "toolpath_strategy",
          "anti_pattern", "material_tip", "thin_wall", "finishing", "roughing",
          "safety", "coolant_strategy",
        ];
        const existingTitles = new Set<string>();
        for (const cat of allCategories) {
          try {
            for (const r of machiningPlaybookEngine.byCategory(cat)) {
              existingTitles.add(r.title.toLowerCase());
            }
          } catch { /* category may not exist */ }
        }
        for (const ex of extractedRules) {
          const titleKey = ex.title.toLowerCase();
          if (existingTitles.has(titleKey)) {
            ex.dedup_reason = "title_exists_in_playbook";
            continue;
          }
          try {
            ...
            ex.added_to_playbook = true;
            existingTitles.add(titleKey);
            addedCount++;
          } catch (err: any) {
            ex.dedup_reason = err.message.includes("already exists") ? "id_collision" : "add_failed";
          }
        }
        if (addedCount > 0) {
          log.info(`[VideoLearning] Added ${addedCount} new playbook rules from video`);
        }
      } catch (err: any) {
        log.warn(`[VideoLearning] Playbook rule registration skipped: ${err.message}`);
      }
    }
    ...
    const result: VideoProcessResult = {
      ...
      playbook_rules_extracted: addedCount,
      playbook_rules: extractedRules,
    };

    log.info(`[VideoLearning] Complete: ${knowledgeItems.length} knowledge items, ${addedCount} playbook rules added, est. cost $${totalCost.toFixed(3)}`);
