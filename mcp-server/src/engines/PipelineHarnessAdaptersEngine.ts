...
export interface MinimalLatheInput {
  ...
  /** Optimization target (e.g. "min_cost" enables the Gilbert economic Vc cap in the generator). */
  optimization_target?: TurningInput["optimization_target"];
  /** Cost-economic params for the Gilbert minimum-cost Vc cap (only consumed under "min_cost"). */
  economics?: TurningInput["economics"];
  // --- Lathe Wizard UI form keys (LatheWizardPage submit payload) -----------
  // The web wizard posts a flat, INCH-unit shape that does NOT use the canonical
  // mm/material-object keys above. Accepting them here is what makes the wizard
  // actually HONOR the user's part size + material instead of silently falling
  // back to the 31.75mm/50mm/1018 defaults (units-first: *_in => *_mm via x25.4).
  /** Flat material string from the wizard (e.g. "4140 steel"); maps to material.material_name. */
  material?: string;
  /** Bar/stock OD in INCHES from the wizard; converted to bar_stock_od_mm (x25.4). */
  diameter_in?: number;
  /** Part length in INCHES from the wizard; converted to part_length_mm (x25.4). */
  length_in?: number;
}
...
export function normalizeLatheInput(input: unknown): TurningInput {
  ...
  return {
    ...
    material: {
      // Honor the wizard's flat `material` string (the engine resolves the ISO group +
      // Kienzle/Taylor coefficients from material_name); canonical material_name still wins.
      material_name: i.material_name ?? i.material ?? "1018 Steel",
      iso_group: i.iso_group ?? "P",
      ...(typeof i.hardness_hrc === "number" ? { hardness_hrc: i.hardness_hrc } : {}),
    },
    // Canonical mm key wins; else the wizard's INCH diameter/length -> mm (x25.4, units-first);
    // else the 1.25in bar x 50mm JM Die default. A user-entered 2"x6" now reaches the physics
    // instead of being silently replaced by the default (was a silent variable-input drop).
    bar_stock_od_mm: i.bar_stock_od_mm
      ?? (typeof i.diameter_in === "number" && i.diameter_in > 0 ? i.diameter_in * 25.4 : 31.75),
    part_length_mm: i.part_length_mm
      ?? (typeof i.length_in === "number" && i.length_in > 0 ? i.length_in * 25.4 : 50),
    ...
    // Forward economic-generation fields when supplied (P2-B hardening): "min_cost" + economics must
    // survive normalize so any caller -- not only the post-normalize runner -- can drive the Gilbert cap.
    ...(i.optimization_target ? { optimization_target: i.optimization_target } : {}),
