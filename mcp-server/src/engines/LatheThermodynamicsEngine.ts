...
export class LatheThermodynamicsEngine {
  ...
  private getMaterialProps(spec: MaterialSpec): MaterialPhysics & {
    ...
    const base = resolveMaterial(spec.material_id);
    const phase = PHASE_TRANSFORMATION_DB[spec.material_id];

    return {
      ...base,
      k_thermal: spec.k_thermal_override ?? base.k_thermal,
      cp_J_kgK: spec.cp_override ?? base.cp_J_kgK,
      density_kg_m3: spec.density_override ?? base.density_kg_m3,
