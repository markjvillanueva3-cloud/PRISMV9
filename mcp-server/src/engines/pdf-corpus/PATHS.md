# pdf-corpus galaxy — PATHS atlas (auto-derived baseline)
...
## Engines (name-matched to this domain, 73 found — verify ownership)
...
- `mcp-server/src/engines/CAMTrainingExtractionAggregatorEngine.ts`
...
- `mcp-server/src/engines/FusionCAMExtractorEngine.ts`
...
- `mcp-server/src/engines/GroundTruthFeatureTreeExtractor.ts`
...
- `mcp-server/src/engines/HyperMillOmCyclesExtractor.ts`
- `mcp-server/src/engines/HyperMillXmlExtractor.ts`
...
- `mcp-server/src/engines/PDFFormulaExtractionEngine.ts`
...
- `mcp-server/src/engines/PDFStructureEngine.ts`
- `mcp-server/src/engines/PDFTableExtractionEngine.ts`
- `mcp-server/src/engines/PhysicsFeatureExtractorEngine.ts`
- `mcp-server/src/engines/PrintCorpusOrchestratorEngine.ts`
- `mcp-server/src/engines/PrintCorpusTableWriter.ts`
- …and 13 more (keyword match — prune false positives)
...
- `../CLAUDE.md`
- `../academy/CLAUDE.md`
- `../mit-curriculum/CLAUDE.md`
- `../post-processor/CLAUDE.md`
- `./MEMORY.md`
- `/state/shared/specs/DOMAIN-GALAXY-DOCTRINE-2026-05-26.md`
- `CLAUDE.md`
- `MEMORY.md`
...
- **Domain-relevant for pdf-corpus:** `resources/PDF` · `resources/RESOURCE PDFS` · `resources/OKUMA MULTUS PDFS` · `Docustrata/manifest.json`
...
- **wiki (domain):** `knowledge/wiki/pdf-corpus/` (4 entries) -- query before re-deriving
- **synthesis brain (Obsidian):** `knowledge/memories/patterns/pdf-corpus_synthesis.md` -- compounded domain patterns; refresh via `galaxy-synthesis-refresh.mjs`
- **galaxy brain:** `CLAUDE.md` · `MEMORY.md` · `SOUL.md` · `AWARENESS.md` · `TOOLBELT.md` (this dir)

**Uniform recall routing (same for every domain):**
- memories (domain-tagged, flat corpus): `prism_memory:semantic_search query="pdf-corpus" topK=20` -- auto-fed every Stop by `stop-obsidian-memory-feed.mjs`
...
- AI reasoning over ALL of the above (PSN leg #10): `node scripts/lib/galaxy-reasoning-bridge.mjs pdf-corpus "<question>"` (hybrid CAG+RAG, local Ollama, $0)
