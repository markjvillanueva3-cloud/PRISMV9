...
interface PassBaseline {
  ...
  open_voltage_V: number;
  servo_voltage_V: number;
  wire_speed_factor: number; // of wire max
  wire_tension_factor: number; // of wire tensile × area
}

const PASS_BASELINES: Record<PassType, PassBaseline> = {
  rough: {
    t_on_us: 4.0, t_off_us: 16.0, peak_current_A: 20.0,
    open_voltage_V: 80, servo_voltage_V: 50,
    wire_speed_factor: 0.7, wire_tension_factor: 0.55,
  },
  semi_finish: {
    t_on_us: 1.5, t_off_us: 10.0, peak_current_A: 10.0,
    open_voltage_V: 80, servo_voltage_V: 45,
    wire_speed_factor: 0.5, wire_tension_factor: 0.50,
  },
  finish: {
    t_on_us: 0.4, t_off_us: 6.0, peak_current_A: 4.0,
    open_voltage_V: 100, servo_voltage_V: 40,
    wire_speed_factor: 0.35, wire_tension_factor: 0.45,
  },
  super_finish: {
    t_on_us: 0.15, t_off_us: 4.0, peak_current_A: 1.5,
    open_voltage_V: 120, servo_voltage_V: 35,
    wire_speed_factor: 0.25, wire_tension_factor: 0.40,
  },
};
...
class EDMCuttingParamFlushEngine {
  ...
  private optimizePulse(input: CuttingParamInput, mat: MaterialEDMProps): CuttingParamResult["pulse"] {
    const base = PASS_BASELINES[input.pass_type];

    // --- Material-corrected t_on / t_off / I_p ---
    let t_on = base.t_on_us * mat.t_on_factor;
    let t_off = base.t_off_us * mat.t_off_factor;
    let I_p = base.peak_current_A * mat.current_factor;

    // --- Thickness compensation: thicker parts need more off-time for flushing ---
    const thicknessFactor = Math.max(1, Math.sqrt(input.thickness_mm / 30));
    t_off *= thicknessFactor;

    // --- Wire diameter compensation: thinner wire → less energy capacity ---
    const wireDiamFactor = input.wire_diameter_mm / 0.25; // 0.25 mm baseline
    I_p *= Math.min(1, Math.sqrt(wireDiamFactor));

    // --- Target Ra reverse-solve (empirical: Ra ∝ t_on^0.38 × I_p^0.32) ---
    if (input.target_ra_um !== undefined && input.target_ra_um > 0) {
      ...
      if (currentRa > input.target_ra_um) {
        ...
        // Reduce t_on and I_p proportionally
        t_on *= Math.pow(ratio, 1 / 0.38);
        I_p *= Math.pow(ratio, 1 / 0.32);
      }
    }

    // --- Target MRR forward-adjust ---
    if (input.target_mrr_mm2_min !== undefined && input.target_mrr_mm2_min > 0) {
      const currentMRR = this.estimateMRR(t_on, t_off, I_p, base.open_voltage_V, input.thickness_mm, mat);
      if (currentMRR < input.target_mrr_mm2_min && input.pass_type === "rough") {
        const mrrRatio = input.target_mrr_mm2_min / currentMRR;
        // Increase current and reduce off-time to gain MRR
    ...
    const servo_V = base.servo_voltage_V * (1 + 0.1 * (thicknessFactor - 1));

    return {
      on_us: parseFloat(t_on.toFixed(3)),
      off_us: parseFloat(t_off.toFixed(2)),
      peak_current_A: parseFloat(I_p.toFixed(2)),
      open_voltage_V: base.open_voltage_V,
      servo_voltage_V: parseFloat(Math.min(80, servo_V).toFixed(1)),
    };
  }
  ...
  private optimizeWireSpeed(
    ...
    // Base speed from wire DB and pass type
    let speed = wire.max_speed_m_min * base.wire_speed_factor;
    ...
    // Wire tension: fraction of breaking load
    ...
    let tension_N = breakingForce_N * base.wire_tension_factor;
   ...
   *   Ra ≈ K × E^0.38 (empirical, ISO 4287)
  ...
  calculateEnergy(
    ...
    // Material removal rate: MRR = f × V_crater × η_removal
    const eta_removal = 0.45; // fraction of crater volume actually removed
    const freq_hz = freq_khz * 1000;
    const MRR_mm3_min = craterVol_mm3 * freq_hz * eta_removal * 60;
    ...
    // Surface roughness Ra (empirical: Ra = K3 × E^0.38), E in mJ, Ra in μm
    const K3 = 1.6 * (1 / mat.machinability_index);
    const Ra_um = K3 * Math.pow(E_mj, 0.38);
  ...
  mapTechnologyTable(
    ...
    const values = [
      ...
      pulse.open_voltage_V, wire.speed_m_min, wire.tension_N, 0, // placeholder for flush
    ];
  ...
  private optimizeCapacitance(
    ...
    // Desired energy for target Ra: Ra = K × E^0.38 → E = (Ra/K)^(1/0.38)
    const K = 1.6 * (1 / mat.machinability_index);
    const E_target_mj = Math.pow(targetRa / K, 1 / 0.38);
  ...
  private estimateRa(t_on_us: number, I_p_A: number, mat: MaterialEDMProps): number {
    // Empirical: Ra = K × t_on^0.38 × I_p^0.32 (μm)
    const K = 0.8 / mat.machinability_index;
    return K * Math.pow(t_on_us, 0.38) * Math.pow(I_p_A, 0.32);
  }
