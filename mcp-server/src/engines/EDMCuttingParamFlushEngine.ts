...
/**
 * Wire break recovery parameters — calculated per break event.
 * Based on tribal knowledge wedm-kb-003: "re-thread 2-3mm behind break point".
 * Reference: Sodick operation manual, AWT retry protocol.
 */
export interface WireBreakRecovery {
  /** Distance to back up from break point before re-threading (mm) */
  backup_mm: number;
  /** Maximum automatic retry attempts before operator alarm */
  max_retries: number;
  /** Reduced power parameters for restart (20% power reduction) */
  reduced_params: {
  ...
  strategy: string;
  /** Risk assessment for the recovery */
