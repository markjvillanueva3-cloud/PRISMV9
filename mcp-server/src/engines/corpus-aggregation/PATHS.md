# corpus-aggregation galaxy — PATHS atlas (auto-derived baseline)
...
## Engines (name-matched to this domain, 46 found — verify ownership)
- `mcp-server/src/engines/AdditiveManufacturingTribalCorpusEngine.ts`
- `mcp-server/src/engines/ArchiveToPartsCatalogIngesterEngine.ts`
...
- `mcp-server/src/engines/JMDieDocustrataIngestEngine.ts`
- `mcp-server/src/engines/JMDieFleetWideIngestEngine.ts`
...
- `mcp-server/src/engines/SourceCatalogAggregator.ts`
- `mcp-server/src/engines/SpeedFeedPDFCorpusBridgeEngine.ts`
- `mcp-server/src/engines/SpreadsheetIngestionEngine.ts`
- `mcp-server/src/engines/TribalCorpusOrchestratorEngine.ts`
- `mcp-server/src/engines/WEDMFeedbackIngestionEngine.ts`
- `mcp-server/src/engines/WaterjetCuttingTribalCorpusEngine.ts`
- `mcp-server/src/engines/WeldingTribalCorpusEngine.ts`
...
- **Domain-relevant for corpus-aggregation:** `resources/PDF` · `resources/RESOURCE PDFS` · `JM DIE/TRIBAL + WIKI` · `Docustrata/.index`
...
## 📥 Registered DB intake (juliett-owned stores the corpus-aggregation galaxy consumes — DB-EXPANSION-BRIDGE-MS0)
...
- **wiki (domain):** `knowledge/wiki/corpus-aggregation/` (4 entries) -- query before re-deriving
- **synthesis brain (Obsidian):** `knowledge/memories/patterns/corpus-aggregation_synthesis.md` -- compounded domain patterns; refresh via `galaxy-synthesis-refresh.mjs`
- **galaxy brain:** `CLAUDE.md` · `MEMORY.md` · `SOUL.md` · `AWARENESS.md` · `TOOLBELT.md` (this dir)

**Uniform recall routing (same for every domain):**
- memories (domain-tagged, flat corpus): `prism_memory:semantic_search query="corpus-aggregation" topK=20` -- auto-fed every Stop by `stop-obsidian-memory-feed.mjs`
...
- AI reasoning over ALL of the above (PSN leg #10): `node scripts/lib/galaxy-reasoning-bridge.mjs corpus-aggregation "<question>"` (hybrid CAG+RAG, local Ollama, $0)
