# knowledge-conversion galaxy — PATHS atlas (auto-derived baseline)
...
## Engines (name-matched to this domain, 126 found — verify ownership)
...
- `mcp-server/src/engines/BatchMacroConversionEngine.ts`
...
- `mcp-server/src/engines/CAMFunctionRouterEngine.ts`
...
- `mcp-server/src/engines/ComplexityAwareRouterEngine.ts`
...
- `mcp-server/src/engines/EmbeddingRouterEngine.ts`
...
- `mcp-server/src/engines/IntentRouterEngine.ts`
...
- …and 66 more (keyword match — prune false positives)
...
- **Domain-relevant for knowledge-conversion:** `resources/MIT COURSES` · `resources/MACHINING KNOWLEDGE FORMULAS AND ALGORITHMS` · `JM DIE/TRIBAL + WIKI`
...
## 📥 Registered DB intake (juliett-owned stores the knowledge-conversion galaxy consumes — DB-EXPANSION-BRIDGE-MS0)
...
- **wiki (domain):** `knowledge/wiki/knowledge-conversion/` (4 entries) -- query before re-deriving
- **synthesis brain (Obsidian):** `knowledge/memories/patterns/knowledge-conversion_synthesis.md` -- compounded domain patterns; refresh via `galaxy-synthesis-refresh.mjs`
- **galaxy brain:** `CLAUDE.md` · `MEMORY.md` · `SOUL.md` · `AWARENESS.md` · `TOOLBELT.md` (this dir)

**Uniform recall routing (same for every domain):**
- memories (domain-tagged, flat corpus): `prism_memory:semantic_search query="knowledge-conversion" topK=20` -- auto-fed every Stop by `stop-obsidian-memory-feed.mjs`
...
- AI reasoning over ALL of the above (PSN leg #10): `node scripts/lib/galaxy-reasoning-bridge.mjs knowledge-conversion "<question>"` (hybrid CAG+RAG, local Ollama, $0)
