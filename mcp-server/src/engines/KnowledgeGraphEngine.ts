...
export type NodeType =
  ...
  | "tribal_tip";  // TK-MS6 U-TK29: Tribal knowledge tips as graph nodes
...
export type EdgeType =
  ...
  | "relates_to";  // TK-MS6 U-TK29: Edge connecting tips with 2+ shared tags
...
// ─── TK-MS6 U-TK29: Tribal Knowledge Graph Integration ─────────────────────

let tribalGraphSeeded = false;

/**
 * Seed tribal knowledge tips into the graph.
 * Tips become nodes; 2+ shared applicability tags = edge between tips.
 * @returns Count of tips and edges added
 */
export async function seedTribalTips(): Promise<{ tips_added: number; edges_added: number }> {
  if (tribalGraphSeeded) return { tips_added: 0, edges_added: 0 };
  seedGraph();  // Ensure base graph exists
  ...
  for (const tip of allTips) {
    const tipId = `tip_${tip.id.replace(/[^a-zA-Z0-9]/g, "_")}`;
    const tags = [
      ...(tip.tags || []),
      ...(tip.material_groups || []).map((m: string) => `material:${m}`),
      ...(tip.operation_types || []).map((o: string) => `op:${o}`),
      tip.category,
      tip.domain || "general",
    ].filter(Boolean);

    addNode(tipId, "tribal_tip", tip.title || tip.id, {
      body: (tip.body || "").slice(0, 500),  // Truncate for graph storage
    ...
    // Link to existing material nodes if material_groups specified
    for (const matGroup of tip.material_groups || []) {
      const matNode = findNodeByMaterialGroup(matGroup);
      if (matNode) {
        addEdge(tipId, matNode.id, "relates_to", 0.7, `Tip applies to ${matGroup}`);
        edgesAdded++;
      }
    }
  }

  // Create edges between tips that share 2+ tags
  const tipNodes = Array.from(nodes.values()).filter(n => n.type === "tribal_tip");
  for (let i = 0; i < tipNodes.length; i++) {
    for (let j = i + 1; j < tipNodes.length; j++) {
      const tags1 = (tipNodes[i].properties.tags as string[]) || [];
      const tags2 = (tipNodes[j].properties.tags as string[]) || [];
      const sharedTags = tags1.filter(t => tags2.includes(t));

      if (sharedTags.length >= 2) {
        addEdge(tipNodes[i].id, tipNodes[j].id, "relates_to",
          Math.min(0.3 + sharedTags.length * 0.1, 0.9),
          `Shared: ${sharedTags.slice(0, 3).join(", ")}`
        );
        edgesAdded++;
      }
    }
  ...
  tribalGraphSeeded = true;
  return { tips_added: tipsAdded, edges_added: edgesAdded };
}

/** Find material node by ISO group code (helper for tip linking). */
function findNodeByMaterialGroup(group: string): GraphNode | undefined {
  const groupMap: Record<string, string> = {
    P: "mat_4140", M: "mat_304ss", K: "mat_7075", S: "mat_ti64",
    H: "mat_inconel718", N: "mat_7075",
  };
  const nodeId = groupMap[group.toUpperCase()];
  return nodeId ? nodes.get(nodeId) : undefined;
}

/**
 * Query tribal knowledge graph for a specific context.
 * @param context - Material, machine, operation query context
 * @returns Related tips and traversal paths
 */
export async function queryTribalGraph(context: {
  material?: string;
  machine?: string;
  operation?: string;
}): Promise<{
  tips: Array<{ id: string; title: string; confidence: number; related_count: number }>;
  traversal_depth: number;
  total_nodes: number;
}> {
  await seedTribalTips();

  const tipNodes = findNodesByType("tribal_tip");
  const results: Array<{ id: string; title: string; confidence: number; related_count: number }> = [];
  ...
  for (const tip of tipNodes) {
    ...
    if (context.material && tags.some(t => t.toLowerCase().includes(context.material!.toLowerCase()))) {
      relevance += 2;
    }
    if (context.operation && tags.some(t => t.toLowerCase().includes(context.operation!.toLowerCase()))) {
      relevance += 2;
    }
    if (context.machine && tags.some(t => t.toLowerCase().includes(context.machine!.toLowerCase()))) {
      relevance += 1;
    }

    if (relevance > 0 || (!context.material && !context.machine && !context.operation)) {
      const neighbors = getNeighbors(tip.id);
      results.push({
        id: tip.id.replace("tip_", ""),
        title: tip.name,
        confidence: (tip.properties.confidence as number) || 70,
        related_count: neighbors.length,
      });
    }
  }

  return {
    tips: results.sort((a, b) => b.confidence - a.confidence).slice(0, 50),
    traversal_depth: 2,
    total_nodes: tipNodes.length,
  };
}
...
// ─── Exported Graph API (KAR-MS3 U-KAR19) ───────────────────────────────────

/**
 * Add a node to the knowledge graph (exported for engine wiring)
 * @param id - Unique node identifier
 * @param type - Node type (material, tool, machine, etc.)
 * @param name - Human-readable name
 * @param props - Additional properties
 */
export function graphAddNode(id: string, type: NodeType, name: string, props: Record<string, unknown> = {}): void {
  seedGraph();
  addNode(id, type, name, props);
}

/**
 * Add an edge to the knowledge graph (exported for engine wiring)
 * @param source - Source node ID
 * @param target - Target node ID
 * @param type - Edge type
 * @param weight - Relationship strength (0-1)
 * @param evidence - How we know this relationship
 * @param jobCount - Optional job count supporting this edge
 */
export function graphAddEdge(
  ...
  seedGraph();
  addEdge(source, target, type, weight, evidence, jobCount);
}
 ...
 */
export function graphHasNode(id: string): boolean {
  seedGraph();
  return nodes.has(id);
}
 ...
 * @param id - Node ID
 */
export function graphGetNode(id: string): GraphNode | undefined {
  seedGraph();
  return nodes.get(id);
}
