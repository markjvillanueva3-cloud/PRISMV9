...
eventBus.registerAction("update_capacity_from_estimate", async (params) => {
  try {
    const { machine_id, estimated_seconds } = params;
    if (!machine_id || !estimated_seconds) {
      return { error: "Missing machine_id or estimated_seconds" };
    }

    // Get current machine load (getMachineLoad is the available method)
    const loadInfo = capacityPlanningEngine.getMachineLoad(machine_id);

    // Emit capacity.updated event with available info
    await eventBus.publish(EventTypes.CAPACITY_UPDATED, {
      machine_id,
      machine_name: loadInfo.machine_name,
      time_window: loadInfo.period,
      available_hours: loadInfo.available_hours,
      scheduled_hours: loadInfo.loaded_hours,
      utilization_pct: loadInfo.utilization_pct,
      pending_jobs: loadInfo.jobs.length,
      status: loadInfo.status,
    });

    log.info(`[Scheduling Chain] Capacity check for ${machine_id}: ${loadInfo.utilization_pct.toFixed(1)}% utilization`);
    return { updated: true, utilization_pct: loadInfo.utilization_pct };
  } catch (err) {
...
eventBus.registerAction("reoptimize_schedule", async (params) => {
  try {
    ...
    if (utilization_pct > 85 || utilization_pct < 20) {
      // Use optimize with actual signature: (jobs: Job[], machines: MachineSlot[])
      // For now, use empty arrays to get default optimization
      const result = schedulingEngine.optimize([], []);
      const bestResult = result.results[result.best_strategy];

      // Emit schedule.optimized event with available data
      await eventBus.publish(EventTypes.SCHEDULE_OPTIMIZED, {
        schedule_id: `OPT-${Date.now()}`,
        affected_machines: machine_id ? [machine_id] : [],
        affected_jobs: bestResult?.assignments?.map((a: { job_id: string }) => a.job_id) ?? [],
        optimization_type: utilization_pct > 85 ? "rebalance" : "consolidate",
        makespan_days: bestResult?.total_makespan_days ?? 0,
        schedule_score: bestResult?.schedule_score ?? 0,
      });

      const improvement = bestResult?.schedule_score ?? 0;
      log.info(`[Scheduling Chain] Schedule optimized: score ${improvement.toFixed(1)}`);
      return { optimized: true, schedule_score: improvement };
    }
