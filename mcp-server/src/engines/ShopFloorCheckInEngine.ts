...
export interface DepartmentCheckInResult {
  duplicate: boolean;
  message: string;
  checkIns: DepartmentCheckIn[];
  entry?: DepartmentCheckIn;
  tasks: TrackedTask[];
}

export type TaskEventAction = "start" | "pause" | "complete";
...
function runningSeconds(task: TrackedTask, nowMs: number) {
...
export class ShopFloorCheckInEngine {
  ...
  static checkIntoDepartment(input: {
    ...
    if (!trackedJob) {
      return {
        duplicate: false,
        message: "No job registered. Scan a job first.",
    ...
    if (!employee) {
      return {
        duplicate: false,
        message: "No employee identified. Badge in first.",
        checkIns: existingCheckIns,
        tasks: existingTasks,
      };
    }
  ...
  /**
   * Apply a task-event update through one canonical shop-floor contract instead
   * of re-deriving task state only in the browser.
   */
