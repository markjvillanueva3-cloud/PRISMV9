...
class FeatureSequenceReplicatorEngine {
  ...
  private generateACPythonScript(record: FeatureSequenceRecord): string {
    ...
    for (const op of record.operations) {
      ...
      // NOTE: Milling speed/feed come from tool technology, not CfgParameters.
      // Only turning jobs use TCUTTINGSPEED/FVORSCHUB/TFEEDRATE in CfgParameters.
      const cfgEntries: string[] = [];
      const isTurning = /turn|lathe|groo|thread|bore/i.test(op.cycleCode ?? "");
      ...
      if (isTurning && typeof rpm === "number") {
        cfgEntries.push(`"TCUTTINGSPEED": "${rpm}"`);
      }
      if (isTurning && typeof feed === "number") {
        cfgEntries.push(`"FVORSCHUB": "${feed}"`);
      }
  ...
  /** Translate internal param keys to hyperMILL CfgParameter keys.
   *  Keys derived from C:/Program Files/OPEN MIND/hyperMILL/31.0/Metric.cfg/ */
  private translateParamKey(key: string): string {
    const map: Record<string, string> = {
      stepdown_mm: "VERTZUSTEL", stepdown: "VERTZUSTEL", depth_of_cut: "VERTZUSTEL",
      stepover_mm: "HORIZUSTEL", stepover: "HORIZUSTEL", radial_depth: "HORIZUSTEL",
      depth_mm: "TIEF", total_depth: "TIEF",
      tolerance_mm: "PRECISION", tolerance: "PRECISION",
      scallop_height: "HORIZUSTEL", cusp_height: "HORIZUSTEL",
      clearance_z: "SICHDIST", clearance_z_mm: "SICHDIST",
      retract_z: "SICHEBENE", retract_z_mm: "SICHEBENE",
      peck_depth_mm: "TIEF", peck_depth: "TIEF",
      thread_pitch_mm: "PITCH", thread_pitch: "PITCH",
      bore_diameter: "DIAMETER", hole_diameter: "DIAMETER",
      allowance_mm: "AUFMASS", stock_leave: "AUFMASS",
    };
    return map[key] ?? key;
  }
