...
- [[reference_sierra_regen_fast_registration_gap_2026_05_29]] — 9 *-features.mjs absent from regen-viz FAST[]; merge loads by output-json name not generator filename (U-VIZ-FAST-REGISTER-9). **UPDATE 2026-06-21 (verified): 5+/9 now wired in FAST[] (quoting-pipeline / run-hotel-domain / milling-tribal-tip-bridge / svi-component / vendor-catalog); the "blocked on merge-OOM" reason is OBSOLETE — the merge-augmentations OOM is RESOLVED (streaming I/O, see "Merge OOM" below). Re-verify the remaining generators before re-flagging this gap.**
...
- **Merge OOM / exit 134 (merge path RESOLVED 2026-06; class PRESERVED)** — merge-augmentations WAS hitting the V8 external-allocation / ~512MB string cap on the (then-548MB, now ~770MB) graph (`.last-regen-failure.json` 2026-05-29T01:47 exit 134). **FIXED:** `merge-augmentations.mjs` now reads via `readGraphStreaming` + writes via `writeGraphStreamingAtomic` (`scripts/lib/graph-io.mjs`) — it no longer string-parses/stringifies the merged graph, so the merge itself no longer OOMs. The OOM **class** stays a hard rail for ANY new graph code (CLAUDE.md §6): never raw `JSON.parse`/`stringify(...,null,2)` the merged graph — use `scripts/lib/{graph-io,system-viz-graph}.mjs`. `regen-viz-merge-guard` still fail-louds on a genuine merge failure; leftover `.tmp.system-graph.json.<pid>` + `_node-embeddings.jsonl.partial` are orphaned scratch (golf reaps).
...
**The NODE-ACCESS MAP** (cheapest token path per vault node type) lives in wiki [[obsidian-vault-node-access-map]] (`knowledge/wiki/architecture/obsidian-vault-node-access-map.md`) — the core deliverable. Every populated node type has a free ≤200-token path; remaining GAP: empty wiki dirs / tribal→wiki coverage — `memory-rag-inject` recall RESTORED (U-VAULT-RAG-WIRE) + canvas seek path CLOSED (U-CANVAS-READ-REFLECT `47fe91554f`).

**Gap ladder (A→B→C, R13)** — all in the self-maintenance/write-back layer, none block reading:
- **A (P0 silent failures) — SHIPPED 2026-06-15, ladder CLEAR:** `memory-rag-inject.mjs` keyword-recall restored (U-VAULT-RAG-WIRE `9e4376b3b2`; stale-claim fix `47fe91554f`); `obsidian-memory-sync.mjs` per-file try/catch+retry (U-VAULT-SYNC-RESILIENT `f5573940c8`).
- **B (P1 works-but-manual):** U-VAULT-INDEX-META SHIPPED (`ea21008f25`); U-VAULT-MAINT-CRON installers SHIPPED (`8c4dff660a`) but **NOT YET ARMED** (promote-memory-to-wiki + vault-rot-sentinel crons installed, disabled) — arming is the one open B follow-up.
- **C (P2 net-new write-back):** U-VAULT-LINK-HEAL SHIPPED — broken-wikilink scorer recalibrated (`984313825e`) + short-token auto-apply hole closed (`d948b85a74`, auto-apply deliberately disarmed). Remaining C: tribal→wiki 31.5%; inbox/mistakes daily-process writer; DailyFlashReportEngine.ts:149 email is a `console.log` stub.
...
## Dual-registration auditor + 3 dropped echo roosts (2026-06-22, slot:sierra, claude-358cf225)
Built `scripts/lib/viz-dual-registration-audit.mjs` (+ CLI `scripts/audit-viz-dual-registration.mjs`,
12 tests, wired as a `regen-viz.mjs` preflight) -- pure static auditor for the FAST[]+merge-splice
"both-or-neither" invariant. Classes: crashRisks(P0, missing generator -> regen crash) / silentDiscards(P1,
in FAST no merge splice) / orphanGenerators(P2) / danglingConsumers / unverifiable(R12). Run:
`node scripts/audit-viz-dual-registration.mjs [--json] [--strict]`.
It FOUND 3 echo POST-PDF roosts (cited-tips/tribal-wiki/post-pdf) in FAST[] since 2026-05-26 but NEVER
spliced -> 117 corpus nodes silently dropped every regen. FIXED via merge-time class-name->node-id
resolver (`foldRoostAug` in merge-augmentations): +117 nodes, 185/210 edges recovered, 0 danglers
(heeded [[reference_orphan_augmentation_dangling_diagnosis_2026_06_10]] -- bare-class-name bridge edges).
Live audit now: crashRisks 0, silentDiscards 0, orphans 3 (advisory: core/fs-inventory stale-fold +
vault-atomic dead), dangling 1 (documented slot-queue). Follow-up U-VIZ-ROOST-BRIDGE-RESOLVE = move the
resolver into the 3 generators + graph the 25 un-graphed engines. Full: [[reference_viz_dual_registration_audit_2026_06_22]].
