...
- NEVER raise the fleet hook heap (`PRISM_HOOK_HEAP_MB` / portable-node default / ensure-heap-floor)
  to "fix" the 267MB master-index sidecar rejection. On WINDOWS `--max-old-space-size` is a COMMIT
  RESERVATION -> a blanket raise re-creates the MCP-FLEET-CAPACITY commit-storm outage. The 384MB
  cap + architecture-graph fallback is DELIBERATE; the only safe full-coverage path is sharding the
  sidecar (load only the needed shard per query). -> [[windows-commit-reservation-hook-heap]].
...
/ `blast-radius` / `dispatcher-summary` / `coverage-by-domain` / `build-order` / `octopus [<caller>]`
(octopus consensus audit summary -- cheap, reads consensus-decisions.jsonl not the graph; backed by
`scripts/lib/octopus-consensus-query.mjs`, the MCP-down sibling of `aiReasoning:consensus_audit_query`)

To INVOKE (not just query) the octopus on a real crossroad for FREE:
`PRISM_OCTOPUS_LIVE_DISPATCH=1 PRISM_SLOT=sierra node H:/prism/scripts/octopus-with-hermes-rag.mjs --local-only --json --prompt "<q>"`.
`--local-only` (alias `PRISM_OCTOPUS_LOCAL_ONLY=1`; impl `scripts/lib/octopus-dispatch.mjs localOnlyOverrides`)
disables every PAID cloud voice -> consensus runs on the local Ollama Blackwell panel only ($0, no external
send), with full Obsidian PSN RAG, and writes the decision to consensus-decisions.jsonl + octopus-outcomes/.
This is the un-dormant path -- cron/loop-safe. Default (no flag) stays cloud-inclusive. Verified 2026-06-23
(2-voice cloud-free consensus, anyCloud:false).
...
- **Reasoning bridge** (`scripts/lib/galaxy-reasoning-bridge.mjs`, PSN leg #10): **CAG** + **RAG** hybrid
  ...
  local Ollama stack -- `node scripts/lib/galaxy-reasoning-bridge.mjs system-viz "<question>"`.
- **Vault -> LoRA**: this galaxy's Obsidian **synthesis** brain (`knowledge/memories/patterns/system-viz_synthesis.md`)
