...
import { getJMDieSelectorCatalog } from "../utils/jmDieSelectorCatalog.js";
...
export class ProgramReleaseCatalogEngine {
  ...
  static getCatalog(): ProgramReleaseCatalog {
    const selectorCatalog = getJMDieSelectorCatalog();

    return {
      partClasses: DEFAULT_PART_CLASSES,
      machines: ProgramReleaseCatalogEngine.getMachineCatalog(),
      toolholders: selectorCatalog.toolholders.length > 0 ? selectorCatalog.toolholders : DEFAULT_TOOLHOLDERS,
      toolingPackages: selectorCatalog.toolingPackages.length > 0 ? selectorCatalog.toolingPackages : DEFAULT_TOOLING,
      fixtures: DEFAULT_FIXTURES,
      stockProfiles: selectorCatalog.stockProfiles.length > 0 ? selectorCatalog.stockProfiles : DEFAULT_STOCK,
