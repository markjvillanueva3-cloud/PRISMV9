...
import {
  JM_DIE_SHELL_ROLE_DEFINITIONS,
  type JMDieShellRoleDefinition,
  type JMDieShellRoleId,
} from "../data/jm-die-employees.js";
import {
  employeeEngine,
  type EmployeeMachineAuthoritySummary,
} from "./EmployeeEngine.js";
...
type RoleId = JMDieShellRoleId;
type RoleDefinition = JMDieShellRoleDefinition;

const ROLE_DEFINITIONS: Record<RoleId, RoleDefinition> = JM_DIE_SHELL_ROLE_DEFINITIONS;
...
export class ShellBootstrapEngine {
  ...
  static getEmployeeShellBootstrap(
    ...
    const machineAuthority = employeeEngine.getMachineAuthority(role.employeeId);
    ...
    return {
      ...
      accessCards: ShellBootstrapEngine.buildAccessCards(bootstrap.deskCounts, machineAuthority),
      ...
      policyNote:
        `Role-filtered view for ${role.department}. Some surfaces are restricted based on role permissions. `
        + `Machine authority is mapped from canonical JM Die skills, certifications, and the shared machine registry.`,
    };
  }
  ...
  private static buildAccessCards(
    counts: DeskCounts,
    machineAuthority: EmployeeMachineAuthoritySummary,
  ): AccessCard[] {
    ...
    if (machineAuthority.machine_count > 0) {
      cards.push({
        id: "ac-machine-authority",
        label: "Machine Authority",
        value: String(machineAuthority.machine_count),
        detail:
          `${machineAuthority.program_machine_count} program / `
          + `${machineAuthority.release_machine_count} release / `
          + `${machineAuthority.setup_machine_count} setup`,
        tone: machineAuthority.release_machine_count > 0 ? "sky" : "emerald",
      });
    }
