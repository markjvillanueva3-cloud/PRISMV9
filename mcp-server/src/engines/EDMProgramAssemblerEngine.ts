 ...
 */
function dischargeEnergy_J(V_gap_V, I_peak_A, t_on_us) {
 ...
 */
function satoWireCuttingSpeed_mm_min(K, I_peak_A, t_on_us, h_mm, D_wire_mm) {
    if (h_mm <= 0 || D_wire_mm <= 0)
        return 0;
 ...
 */
function sinkerMRR_mm3_min(C_mrr, I_peak_A, t_on_us, a, b, density_g_cm3) {
 ...
 */
function recastLayerThickness_um(C_recast, E_discharge_J) {
 ...
 */
function hazDepth_um(thermal_diffusivity_mm2_s, t_on_us) {
 ...
 */
function wireOffset_mm(D_wire_mm, V_gap_V, I_peak_A) {
 ...
 */
function sinkerSparkGap_mm(V_gap_V, I_peak_A) {
...
let _edmResolvedMaterial = null;
/** Cached machine context from registry bridge (U-ARCH3). */
let _edmResolvedMachine = null;
/**
...
function getMaterial(materialKey) {
    // Tier 1: exact key
    if (EDM_MATERIALS[materialKey])
        return EDM_MATERIALS[materialKey];
...
function contourPerimeter_mm(contour) {
    let len = 0;
...
const DIALECTS = {
    fanuc_wedm: {
        ...
        flushOnCode: "M08",
        flushOffCode: "M09",
        powerSetting: (pass, I, ton, Vg) => `E${String(pass).padStart(3, "0")} (IP=${I}A TON=${ton}US VG=${Vg}V)`,
    ...
    mitsubishi_wedm: {
        ...
        flushOnCode: "M08",
        flushOffCode: "M09",
        powerSetting: (pass, I, ton, Vg) => `C${String(pass * 10).padStart(3, "0")} E${String(Math.round(I)).padStart(4, "0")} (TON=${ton}US VG=${Vg}V)`,
    ...
    sodick: {
        ...
        flushOnCode: "FON",
        flushOffCode: "FOFF",
        powerSetting: (pass, I, ton, Vg) => `T${String(pass * 5 + 1).padStart(3, "0")} ; IP=${I}A TON=${ton}US VG=${Vg}V`,
    ...
    agiecharmilles: {
        ...
        flushOnCode: "M08",
        flushOffCode: "M09",
        powerSetting: (pass, I, ton, Vg) => `P${String(pass).padStart(3, "0")} (IP=${I}A ON=${ton} GAP=${Vg})`,
    ...
    makino_edm: {
        ...
        flushOnCode: "M08",
        flushOffCode: "M09",
        powerSetting: (pass, I, ton, Vg) => `E${String(pass).padStart(3, "0")} (PEAK=${I}A PULSE=${ton}US VOLT=${Vg}V)`,
    ...
    generic_edm: {
        ...
        flushOnCode: "M08",
        flushOffCode: "M09",
        powerSetting: (pass, I, ton, Vg) => `(PASS ${pass}: I=${I}A T_ON=${ton}US V=${Vg}V)`,
...
function autoDesignElectrodes(profile, mat) {
...
function getWirePassSettings(passNum, mat) {
    const configs = [
        {
...
export class EDMProgramAssemblerEngine {
    engineName = "EDMProgramAssemblerEngine";
    /**
    ...
    assembleWireEDM(input) {
        ...
        if (input.thickness_mm > 400)
            warnings.push("Workpiece height > 400mm: consider special flushing strategy");
        // Wire setup
        const wireSetup = this._buildWireSetup(wireProp, D_wire, contourPerim, totalPasses);
        // Build operations (one per pass)
        const operations = [];
        // Optional start hole sequence
        if (input.start_hole) {
            operations.push(this._buildStartHoleOp(input.start_hole, dialect, mat));
        }
        // Auto-thread
        ...
        let raTargetFinishOverride = null;
        if (input.target_Ra_um && input.target_Ra_um > 0) {
            ...
            if (input.target_Ra_um < defaultFinishRa * 0.9) {
                ...
                warnings.push(`Finish pass tuned for Ra≤${input.target_Ra_um}µm: I=${raTargetFinishOverride.I_peak_A}A, t_on=${raTargetFinishOverride.t_on_us}µs (default was ${defaultFinishRa.toFixed(2)}µm)`);
            }
        }
        // Cutting passes
        for (let pass = 1; pass <= totalPasses; pass++) {
            ...
            const op = this._buildWirePass(pass, passSettings, input, mat, D_wire, contourPerim, dialect, controller);
            operations.push(op);
        }
        ...
        const gcode = this._assembleWireGCode(programStr, headerComments, operations, dialect, input);
        // Uncertainty
        ...
        if (input.target_Ra_um &&
            ...
            warnings.push(`Monte Carlo: ${(uncertainty.p_exceed_Ra * 100).toFixed(1)}% probability of exceeding Ra ${input.target_Ra_um}µm — add extra skim pass`);
        }
    ...
    assembleSinkerEDM(input) {
        ...
        for (const electrode of electrodes) {
            for (const feature of input.features) {
                // Main burn operation
                const op = this._buildSinkerOp(electrode, feature, mat, jumpHeight, jumpFreq, dialect, controller);
                operations.push(op);
                ...
                if ((electrode.stage === "finishing" ||
                    ...
                    const orbitOp = this._buildOrbitingOp(electrode, feature, dialect);
        ...
        const gcode = this._assembleSinkerGCode(programStr, headerComments, operations, dialect);
        // Warnings
        for (const feat of input.features) {
            if (feat.depth_mm > 100)
                warnings.push(`Feature "${feat.name}" depth ${feat.depth_mm}mm > 100mm: use through-flush or vacuum flushing`);
            if (feat.area_mm2 > 5000)
                warnings.push(`Feature "${feat.name}" area ${feat.area_mm2}mm² is large: expect long rough time`);
        }
    ...
    assembleMicroEDM(input) {
        ...
        if (aspectRatio > 30)
            warnings.push(`Aspect ratio ${aspectRatio.toFixed(1)} > 30: use ultrasonic-assisted EDM or peck drilling`);
        if (D_um < 10)
        ...
        const Ra_um = surfaceRoughness_Ra(mat.ra_C, I_peak, t_on, mat.ra_alpha, mat.ra_beta);
        // Micro MRR is very low: scale down sinker formula
        const mrr_um3_min = sinkerMRR_mm3_min(mat.sinker_C_mrr, I_peak, t_on, mat.sinker_a, mat.sinker_b, mat.density_g_cm3) *
            1e9 *
            0.01; // tiny fraction for micro
        // Volume of each feature
        const vol_um3 = (Math.PI / 4) * D_um * D_um * depth_um;
        const time_per_hole_s = mrr_um3_min > 0 ? (vol_um3 / mrr_um3_min) * 60 : 600;
        // Build G-code
        const lines = [];
        const D = dialect;
        const programStr = `O${String(progNum).padStart(4, "0")}`;
        ...
        if (input.feature_type === "wedg_electrode_prep") {
            ...
            for (let i = 0; i < positions.length; i++) {
                ...
                lines.push(`G00 X${pos.x_mm.toFixed(4)} Y${pos.y_mm.toFixed(4)}`);
                lines.push("G00 Z2.");
                lines.push(D.powerSetting(1, I_peak, t_on, V_gap));
                if (input.feature_type === "micro_hole") {
                    ...
                    while (z_current < depth_mm) {
                        ...
                        lines.push(`G01 Z-${z_next.toFixed(4)} F${(I_peak * 0.5).toFixed(2)} (PECK ${peck})`);
                    ...
                    for (let py = 0; py < passes_y; py++) {
                        ...
                        lines.push(`G01 X${(pos.x_mm - D_mm / 2).toFixed(4)} Y${(pos.y_mm + y_off).toFixed(4)} F20`);
                        lines.push(`G01 X${(pos.x_mm + D_mm / 2).toFixed(4)} Y${(pos.y_mm + y_off).toFixed(4)} F20`);
                    }
                }
    ...
    estimateCycleTime(program) {
        ...
        const cutting_s = ops
            .filter((o) => !o.name.toLowerCase().includes("setup") &&
    ...
    computeUncertainty(input) {
        ...
        for (let i = 0; i < N; i++) {
            ...
            if (input.edm_type === "wire") {
                const v_cut = satoWireCuttingSpeed_mm_min(mat.sato_K, I, ton, h, D_wire);
                mrr = v_cut * D_wire * h; // mm²/min → proxy as mm³/min
            }
            else {
                mrr = sinkerMRR_mm3_min(mat.sinker_C_mrr, I, ton, mat.sinker_a, mat.sinker_b, mat.density_g_cm3);
            }
            // Electrode wear (log-normal scatter, CV 30%)
            const stage = I > mat.rough_current_A * 0.7 ? "roughing" : "finishing";
            const wearTable = ELECTRODE_WEAR["graphite"][mat.iso_group] ??
                ELECTRODE_WEAR["graphite"]["P"];
            const wearBase = stage === "roughing" ? wearTable.roughing : wearTable.finishing;
        ...
        const dominant_source = cv_ra >= cv_mrr && cv_ra >= cv_recast
    ...
    // PRIVATE HELPERS
    // ============================================================================
    _buildWireSetup(wireProp, D_wire_mm, perim_mm, totalPasses) {
    ...
    _buildStartHoleOp(startHole, dialect, mat) {
        const D = dialect;
        const lines = [
    ...
    _buildWirePass(passNum, ps, input, mat, D_wire_mm, perim_mm, dialect, controller) {
        const D = dialect;
        ...
        const v_cut = satoWireCuttingSpeed_mm_min(mat.sato_K * wireProp.machinability_factor * ps.feed_factor, I, ton, input.thickness_mm, D_wire_mm);
        ...
        lines.push(D.comment(`V_CUT=${v_cut.toFixed(2)}mm/min  Ra≈${Ra_um.toFixed(2)}µm  RECAST≈${recast_um.toFixed(2)}µm`));
        ...
        lines.push(D.wireOnCode);
        // Offset register
        lines.push(`${D.offsetRegister(passNum)} (WIRE OFFSET: ${offset_mm.toFixed(4)}MM)`);
        // Set WCS
        ...
        for (let i = 1; i < input.contour.length; i++) {
            ...
            if (pt.arc_radius_mm && pt.arc_radius_mm > 0) {
                ...
                lines.push(`${arcDir} X${pt.x_mm.toFixed(4)} Y${pt.y_mm.toFixed(4)} ` +
                ...
                lines.push(`G01 X${pt.x_mm.toFixed(4)} Y${pt.y_mm.toFixed(4)} ` +
                ...
                const taper_offset_factor = Math.tan((input.taper_angle_deg * Math.PI) / 180) *
                    input.thickness_mm;
                lines.push(`G01 X${pt.x_mm.toFixed(4)} Y${pt.y_mm.toFixed(4)} ` +
                ...
                lines.push(`G01 X${pt.x_mm.toFixed(4)} Y${pt.y_mm.toFixed(4)} F${v_cut.toFixed(2)}`);
            }
            // Corner slow-down strategy
            if (input.corner_strategy === "slow_down" &&
                ...
                if (angle_deg > 45) {
                    lines.push(D.comment(`CORNER ${angle_deg.toFixed(0)}° — speed reduced by 40%`));
                    lines.push(`G01 X${pt.x_mm.toFixed(4)} Y${pt.y_mm.toFixed(4)} F${(v_cut * 0.6).toFixed(2)}`);
                }
            }
        }
        // Close contour back to start
        lines.push(`G01 X${startPt.x_mm.toFixed(4)} Y${startPt.y_mm.toFixed(4)} F${v_cut.toFixed(2)}`);
        lines.push(D.wireOffCode);
        lines.push(D.flushOffCode);
        lines.push(D.comment(`PASS ${passNum} COMPLETE`));
        const opType = passNum === 1
            ? "rough_cut"
            : passNum === 2
                ? "skim_1"
                : passNum === 3
    ...
    /**
     * TK-MS2-U11: Query tribal tips for EDM sinker operations.
     */
    _queryEDMTribalTips(material, domain, limit = 5) {
        try {
            const query = `EDM sinker ${material}${domain ? ` ${domain}` : ""}`;
            const tips = tribalKnowledgeEngine.search({
                query,
                domain: "edm",
                limit: limit * 2,
            });
            return tips.filter(t => t.confidence >= 60).slice(0, limit);
        }
        catch (e) {
            log.debug(`[EDMProgramAssembler] Tribal tip query failed: ${e}`);
            return [];
        }
    }
    _buildSinkerOp(electrode, feature, mat, jumpHeight_mm, jumpFreq_hz, dialect, controller) {
        const D = dialect;
        const { peak_current_A: I, pulse_on_us: ton, gap_voltage_V: Vg } = electrode;
        const elecMat = electrode.material;
        const isoGroup = mat.iso_group;
        const wearEntry = ELECTRODE_WEAR[elecMat]?.[isoGroup] ?? ELECTRODE_WEAR["graphite"]["P"];
        let wear = electrode.stage === "roughing"
            ? wearEntry.roughing
            : electrode.stage === "semi_finishing"
                ? wearEntry.semi_finishing
                : wearEntry.finishing;
        // TK-MS2-U11: Check tribal knowledge for electrode wear adjustments
        const tribalTips = this._queryEDMTribalTips(mat.name, "electrode wear");
        for (const tip of tribalTips) {
            const lower = tip.body.toLowerCase();
            // Look for explicit wear ratio recommendations
            const wearMatch = lower.match(/wear.*?(\d+(?:\.\d+)?)\s*%/);
            if (wearMatch) {
                const tribalWear = parseFloat(wearMatch[1]) / 100;
                // Blend tribal knowledge with physics (40% tribal, 60% physics)
                wear = wear * 0.6 + tribalWear * 0.4;
                log.debug(`[EDMProgramAssembler] Applied tribal wear adjustment: ${wearMatch[1]}%`);
                break;
            }
        }
        const mrr = sinkerMRR_mm3_min(mat.sinker_C_mrr, I, ton, mat.sinker_a, mat.sinker_b, mat.density_g_cm3);
        const E_disch = dischargeEnergy_J(Vg, I, ton);
        const Ra = surfaceRoughness_Ra(mat.ra_C, I, ton, mat.ra_alpha, mat.ra_beta);
        ...
        const volFraction = electrode.stage === "roughing"
            ? 0.75
            : electrode.stage === "semi_finishing"
                ? 0.20
                : 0.05;
        ...
        const opType = electrode.stage === "roughing"
            ? "roughing"
            : electrode.stage === "semi_finishing"
                ? "semi_finishing"
                : "finishing";
        return {
            ...
            mrr_mm3_min: {
                ...
                unit: "mm³/min",
                formula: "MRR = (C_mrr × I^a × t_on^b) / ρ [Koenig & Klocke 1997]",
                confidence: 0.82,
            },
    ...
    _buildOrbitingOp(electrode, feature, dialect) {
        const D = dialect;
        ...
        if (feature.orbit_pattern === "circular") {
            lines.push(`G02 X${(feature.x_mm + orbitR).toFixed(4)} Y${feature.y_mm.toFixed(4)} I${orbitR.toFixed(4)} J0. F${(I * 0.2).toFixed(1)} (CIRCULAR ORBIT)`);
            lines.push(`G02 X${(feature.x_mm + orbitR).toFixed(4)} Y${feature.y_mm.toFixed(4)} I${(-orbitR).toFixed(4)} J0. F${(I * 0.2).toFixed(1)} (RETURN)`);
        }
    ...
    _buildElectrodeChangeOp(electrode, dialect) {
        const D = dialect;
        const nextStage = electrode.stage === "roughing" ? "semi-finishing" : "finishing";
        const lines = [
        ...
        const stubElectrode = {
            ...
            stage: electrode.stage === "roughing"
                ? "semi_finishing"
                : "finishing",
        };
        return {
    ...
    _assembleWireGCode(programStr, headerComments, operations, dialect, input) {
        const D = dialect;
        const lines = [];
        lines.push(D.programStart.trimEnd());
        lines.push(programStr);
        for (const hc of headerComments)
            lines.push(D.comment(hc));
        lines.push("");
    ...
    _assembleSinkerGCode(programStr, headerComments, operations, dialect) {
        const D = dialect;
        const lines = [];
        lines.push(D.programStart.trimEnd());
        lines.push(programStr);
...
//# sourceMappingURL=EDMProgramAssemblerEngine.js.map