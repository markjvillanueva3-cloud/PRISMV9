 ...
 */
// TK-MS0-U4: Import for tribal knowledge bridge
...
const knowledgeBase = [];
// ─── Functions ───────────────────────────────────────────────────────────────
function explainParameter(params) {
    ...
    if (parameter === "cutting_speed" || parameter === "vc" || parameter === "rpm") {
        factors.push({ factor: "Thermal conductivity", impact: matInfo.thermal_conductivity, physics: matInfo.speed_reasoning }, { factor: "Key challenge", impact: matInfo.key_challenge, physics: `For ${matInfo.name}, the main concern at the cutting edge is: ${matInfo.key_challenge}` });
        if (depth === "detailed") {
        ...
        factors.push({ factor: "Chip formation", impact: "Must exceed minimum chip thickness", physics: matInfo.feed_reasoning });
    }
    else if (parameter === "coolant") {
        factors.push({ factor: "Coolant strategy", impact: "Depends on material and operation", physics: matInfo.coolant_reasoning });
    }
    return {
        parameter,
        value: String(value),
...
function assessSkills(params) {
    ...
    const level = total >= 80 ? "advanced" : total >= 50 ? "intermediate" : "beginner";
    const assessment = {
        assessment_id: id,
        level,
        scores,
...
function captureKnowledge(params) {
    knowledgeCounter++;
    ...
    // TK-MS0-U4: Bridge to TribalKnowledgeEngine for unified knowledge spine
    try {
        const tribalTip = {
            id: `tk-app-${entry.knowledge_id}-${Date.now().toString(36)}`,
            title: `${entry.topic}: ${entry.material}`,
            body: entry.insight,
            category: mapTopicToCategory(entry.topic),
            tags: [entry.material.toLowerCase(), entry.topic.toLowerCase(), "apprentice"],
            confidence: entry.confidence === "high" ? 90 : entry.confidence === "medium" ? 70 : 50,
            source: `apprentice:${entry.source}`,
            created_at: entry.timestamp.slice(0, 10),
            usage_count: 0,
            material_groups: [mapMaterialToIsoGroup(entry.material)],
        };
        tribalKnowledgeEngine.ingest([tribalTip]);
    }
    catch { /* tribal knowledge bridge is best-effort */ }
    return entry;
}
/** Map apprentice topics to tribal knowledge categories */
function mapTopicToCategory(topic) {
    const topicLower = topic.toLowerCase();
    if (topicLower.includes("speed") || topicLower.includes("feed"))
        return "speeds_feeds";
    if (topicLower.includes("tool") || topicLower.includes("insert"))
        return "tooling";
    if (topicLower.includes("setup") || topicLower.includes("fixture"))
        return "setup";
    if (topicLower.includes("surface") || topicLower.includes("finish"))
        return "surface_finish";
    if (topicLower.includes("coolant"))
        return "material_handling";
    if (topicLower.includes("thread"))
        return "thread";
    if (topicLower.includes("safety"))
        return "safety";
    if (topicLower.includes("maintenance"))
        return "maintenance";
    return "general";
}
/** Map material names to ISO groups */
function mapMaterialToIsoGroup(material) {
    const m = material.toLowerCase();
    if (m.includes("steel") && !m.includes("stainless"))
        return "P";
    if (m.includes("stainless"))
        return "M";
    if (m.includes("cast iron") || m.includes("graphite"))
        return "K";
    if (m.includes("aluminum") || m.includes("copper") || m.includes("brass"))
        return "N";
    if (m.includes("titanium") || m.includes("inconel") || m.includes("superalloy"))
        return "S";
    if (m.includes("hardened") || m.includes("hrc"))
        return "H";
    return "P"; // default
}
function getChallenge(params) {
    ...
    const lesson = params.lesson_ref;
    let filtered = CHALLENGES;
    if (difficulty)
        filtered = filtered.filter(c => c.difficulty === difficulty);
    if (lesson)
        filtered = filtered.filter(c => c.lesson_ref === lesson);
 ...
 */
export function apprenticeEngine(action, params) {
    switch (action) {
        ...
        case "apprentice_get": {
            ...
            if (!assessment)
                return { error: "Assessment not found", id };
            return assessment;
        }
        default:
 ...
 */
export const APPRENTICE_SOURCE_FILE_CATALOG = {
...
export function getApprenticeSourceFileCatalog() {
    return APPRENTICE_SOURCE_FILE_CATALOG;
}
//# sourceMappingURL=ApprenticeEngine.js.map