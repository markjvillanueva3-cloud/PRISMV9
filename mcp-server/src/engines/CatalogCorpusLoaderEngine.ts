...
export class CatalogCorpusLoaderEngine {
  ...
  /**
   * `*-extracted.json` vendor files that are 100%-REDUNDANT with a richer `.ts`-getter cache
   * already loaded into ToolCatalogEngine via `_loadStandardTools()`. Verified 2026-06-12
   * (slot:romeo): each is identical row-count + 100% edp/part-number overlap + identical
   * present-geometry vs its cache twin (`osg-tools.json`, etc.). Loading them through the corpus
   * DOUBLE-COUNTS the same physical tools (17,389 = osg 11,550 + guhring 3,421 + sandvik 2,418).
   * The getter copy is RICHER (it computes per-ISO `cutting_data`, coolant, series), so the getter
   * wins and the extracted twin is skipped here. This is the EXACT removal; natural-key dedup in
   * `ToolCatalogEngine.addTools` is the general backstop, but it cannot catch these specific dups
   * because the two pipelines default missing oal/loc divergently (cache -> dia*5/dia*2; corpus
   * -> 0), so the same physical part ends up with different normalized geometry.
   */
