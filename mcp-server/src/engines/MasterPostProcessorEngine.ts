...
export class MasterPostProcessorEngine {
  ...
  /**
   * Generate a PRISM Master Post CPS configuration snippet for a specific machine.
   * This wires MasterPostProcessorEngine data into the CPS property defaults.
   * The physical CPS file is scripts/fusion360-post/PRISM-Master.cps.
   */
  generateMasterCpsConfig(machine: MachineProfile): {
    controller: string;
    ...
    const controller = controllerMap[machine.controller] ?? "fanuc_31i";
    const features = machine.features ?? MACHINE_FEATURE_DB[machine.controller] ?? {};
    ...
    return {
      ...
      properties: {
        ...
        maxSpindleSpeed: machine.max_rpm ?? 0,
        maxFeedRate: machine.max_feed ?? 0,
        proveOutMode: true,
        proveOutFeedPct: 50,
        proveOutSpeedPct: 80,
        useSmoothing: !!features.hsm,
        smoothingMode: "medium",
      },
      features,
      cps_file: "PRISM-Master.cps",
    };
  }

  /**
   * Validate that a controller family is supported by the PRISM Master Post.
   */
  isMasterPostController(controller: string): boolean {
    return [
  ...
  stats(): {
    ...
    sub_engines: number;
    master_post_controllers: number;
  } {
    return {
      controllers: 6,
