...
/**
 * U-TK31: Knowledge domain → human-readable module name.
 * Domains from TribalKnowledgeEngine.KnowledgeDomain.
 */
const DOMAIN_LABELS: Record<string, string> = {
  shop_floor: "Shop Floor Expertise",
  cam_software: "CAM Software Tips",
  drawing_standards: "Drawing & GD&T Standards",
  safety: "Safety Procedures",
  maintenance: "Machine Maintenance",
  process_engineering: "Process Engineering",
  quality_inspection: "Quality & Inspection",
  tooling_technology: "Tooling Technology",
  controller_specific: "Controller-Specific Tips",
  workholding: "Workholding & Fixturing",
  video_learned: "Video-Based Learning",
  document_learned: "Document-Based Knowledge",
...
export class CourseBuilderEngine {
   ...
   * U-TK31: Added domain-aware grouping and cam_software preference.
   */
  courseBuild(params: {
    camSystem: string;
    level: CourseLevel;
    maxModules?: number;
    groupBy?: "category" | "domain" | "domain_category";
    preferDomain?: string;
  }): GeneratedCourse {
    const {
      camSystem,
    ...
    const cacheKey = `tips-${camSystem}-${level}-${maxModules}-${groupBy}-${preferDomain ?? "none"}`;
    if (this.cache.has(cacheKey)) return this.cache.get(cacheKey)!;

    let tips = this.getTipsForCam(camSystem);
    if (tips.length === 0) {
      return {
    ...
    // U-TK31: Prefer cam_software domain for CAM courses (sort to front)
    const effectivePreferDomain = preferDomain ?? "cam_software";
    tips = [...tips].sort((a, b) => {
      const aDomain = a.domain === effectivePreferDomain ? 0 : 1;
      const bDomain = b.domain === effectivePreferDomain ? 0 : 1;
    ...
    // U-TK31: Group by category, domain, or domain_category
    const groups = new Map<string, typeof filtered>();
    for (const tip of filtered) {
      let key: string;
      if (groupBy === "domain") {
        key = tip.domain || "general";
      } else if (groupBy === "domain_category") {
        const domain = tip.domain || "general";
        const cat = tip.category || "general";
        key = `${domain}/${cat}`;
      } else {
        key = tip.category || "general";
      }
    ...
    let quizCount = 0;
    for (const [groupKey, groupTips] of entries) {
      const lessons: CourseLesson[] = groupTips
        .slice(0, 8)
        .map(tip => {
      ...
      // U-TK31: Format module title based on groupBy mode
      let moduleTitle: string;
      let moduleDesc: string;
      if (groupBy === "domain_category" && groupKey.includes("/")) {
        const [domain, cat] = groupKey.split("/");
        const domainLabel = DOMAIN_LABELS[domain] || this.titleCase(domain);
        const catLabel = CATEGORY_LABELS[cat] || this.titleCase(cat);
        moduleTitle = `${domainLabel}: ${catLabel}`;
        moduleDesc = `${lessons.length} lessons from ${domain} domain covering ${cat}`;
      } else if (groupBy === "domain") {
        moduleTitle = DOMAIN_LABELS[groupKey] || this.titleCase(groupKey);
        moduleDesc = `${lessons.length} lessons from the ${groupKey} knowledge domain`;
      } else {
        moduleTitle = CATEGORY_LABELS[groupKey] || this.titleCase(groupKey);
        moduleDesc = `${lessons.length} lessons covering ${groupKey} topics in ${camSystem}`;
      }
    ...
    switch (action) {
      case "course_build":
        return this.courseBuild({
          ...
          groupBy: params.groupBy ?? params.group_by ?? "category",
          preferDomain: params.preferDomain ?? params.prefer_domain,
        });
