 ...
 *
 * Audit note (slot:xray 2026-06-23): this module is INTENTIONALLY left in the unwired-audit's
 * UNWIRED bucket (NOT tagged `// WIRE-EXEMPT`). WIRE-EXEMPT is never reclassified by
 * scripts/audit-unwired-engines.mjs, so tagging it would permanently HIDE the genuine wiring
 * work that is due once a concrete backend (U-OCR-EDOCR2-IMPL / U-OCR-PADDLEOCR-IMPL) + the
 * PrintToProgramPipelineEngine.validateIntake() consumer are built. "Unwired-pending-impl" is
 * the honest, still-visible state. Do not re-flag, re-chase, or re-tag -- see
 * reference_xray_blueprint_ocr_adapter_deferred_2026_06_23.
 */
