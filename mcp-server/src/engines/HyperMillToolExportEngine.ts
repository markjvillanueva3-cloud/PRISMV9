 ...
 *   - C:/PRISM/HYPERMILL/Tool Database/33.0/template database/sqlite.sql v1.53
 *   - hyperMILL v33 CAM Manual (C:/PRISM/HYPERMILL/doc/33.0/PDF/CAM/CAM_Manual-en-US.pdf)
 *
...
function getToolCatalog(): any {
  if (!_toolCatalog) {
    try {
      ...
      _toolCatalog = m.toolCatalogEngine ?? m.default ?? null;
  ...
  exportToHMT(
    ...
    if (!prismTools || prismTools.length === 0) {
      try {
        ...
        if (tc?.search) {
          prismTools = tc.search({ max_results: 5000 }) || [];
        }
      } catch { /* ignore */ }
      if (!prismTools || prismTools.length === 0) {
