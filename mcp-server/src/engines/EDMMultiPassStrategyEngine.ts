...
import { EDM_MULTIPASS_MATERIALS, resolveEDMMaterial } from "../data/edm-material-db.js";
import type { EDMMultiPassMaterialProps } from "../data/edm-material-db.js";
...
export interface MultiPassInput {
  ...
  /** Taper angle in degrees (0 = straight cut). Affects skim cascade direction. */
...
/**
 * Material properties for multi-pass strategy.
 * Canonical source: src/data/edm-material-db.ts (EDM_MULTIPASS_MATERIALS)
 */
type MaterialProps = EDMMultiPassMaterialProps;

/** Material DB — imported from canonical source (edm-material-db.ts). */
const MATERIAL_DB: Record<string, MaterialProps> = EDM_MULTIPASS_MATERIALS;
...
/**
 * Base rough cutting speed for steel at 50 mm thickness [mm/min].
 * Calibrated from published data: D2 tool steel @50mm rough = 3.0 mm/min [Klocke/Toenshoff 2013]
 * and real shop data (ITW SHAKEPROOF D2 @25.4mm rough ≈ 3.05 mm/min = F0.12 in/min).
 * Steel base = 3.0 / mrr_factor_tool_steel(0.9) = 3.33 mm/min.
 */
const BASE_SPEED_MM_MIN = 3.33;
...
function planRoughCut(
  ...
  // Cutting speed — piecewise model calibrated to published + real shop data:
  //   h ≤ 50mm: speed ≈ BASE × mrr_factor × (50/h)^0.1  [nearly flat; thin parts slightly faster]
  //   h > 50mm: speed ≈ BASE × mrr_factor × (50/h)^0.74 [flushing-limited steep drop]
  //
  // Calibration points (tool_steel, mrr_factor=0.9):
  //   D2 @25.4mm rough: 3.05 mm/min (ITW SHAKEPROOF shop NC, F0.12 in/min)
  //   D2 @50mm rough:   3.0  mm/min (Klocke 2013 Table 8.2; PPC-006)
  //   D2 @100mm rough:  1.8  mm/min (Klocke 2013 Table 8.3; PPC-011)
  // Old sqrt(50/h)/h model gave 9.94 mm/min @25mm — calibrated values are 3-4 mm/min.
  const h = Math.max(input.thickness_mm, 1);
  const thicknessFactor = h <= 50
    ? Math.pow(50 / h, 0.1)   // nearly flat: slight speed bonus for thinner parts
    : Math.pow(50 / h, 0.74); // flushing-limited: sharp drop for thick sections
  const cuttingSpeed = BASE_SPEED_MM_MIN * mat.mrr_factor * thicknessFactor;
...
function buildTrimCascade(
  ...
  for (let p = 2; p <= totalPasses; p++) {
    ...
    // Skim speed cascade — direction depends on taper vs. straight:
    //
    // STRAIGHT (taper=0): DECREASING cascade (ITW SHAKEPROOF 500-30540-24000-04.NC, Mar 2022)
    //   D2 @25.4mm — Rough F.12, Skim1 F.24 (2.0×), Skim2 F.21 (1.75×), Skim3 F.20 (1.67×)
    //   Fit: factor = max(2.0 - 0.15×(p-2), 1.2)
    //
    // TAPER (taper>0): INCREASING cascade (NOZE TEST.NC — SS taper, 5-pass)
    //   Skim speeds: F.16 → .23 → .26 → .30 in/min (INCREASING pass-over-pass)
    //   PHYSICS CAVEAT: empirically observed, not first-principles derived.
    //   Hypothesized: taper reduces effective spark contact area per pass,
    //   requiring higher feed to maintain MRR.
    //   Fit: factor = 1.3 + 0.20×(p-2), where p=2 is skim1.
    const isTaper = (input.taper_angle_deg ?? 0) > 0;
    const speedFactor = isTaper
      ? Math.min(1.3 + 0.20 * (p - 2), 2.5)            // taper: increasing
      : Math.max(2.0 - 0.15 * (p - 2), 1.2);            // straight: decreasing
...
function estimatePassTimes(
  ...
  for (let p = 2; p <= totalPasses; p++) {
    const passType = assignPassType(p, totalPasses);
    // Same cascade logic as buildTrimCascade — taper uses increasing, straight uses decreasing
    const isTaper = (input.taper_angle_deg ?? 0) > 0;
    const speedFactor = isTaper
      ? Math.min(1.3 + 0.20 * (p - 2), 2.5)
      : Math.max(2.0 - 0.15 * (p - 2), 1.2);
...
function estimateTime(input: MultiPassInput): {
  ...
  // Compute rough cutting speed — same piecewise calibrated model as computeRoughPass
  const hEst = Math.max(input.thickness_mm, 1);
  const thicknessFactor = hEst <= 50
    ? Math.pow(50 / hEst, 0.1)
    : Math.pow(50 / hEst, 0.74);
  const roughSpeed = BASE_SPEED_MM_MIN * mat.mrr_factor * thicknessFactor;
