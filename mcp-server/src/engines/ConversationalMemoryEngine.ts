...
/**
 * Singleton wrapper for ConversationalMemoryEngine.
 * Provides a class-style API for the PUOA integration tests.
 */
class ConversationalMemoryEngineFacade {
  private sessionId: string = "default";

  reset(): void {
    resetConversation(this.sessionId);
  }

  getContext(): ConversationContext {
    return getConversationContext(this.sessionId);
  }

  transitionTo(state: ConversationState): ConversationContext {
    transitionState(this.sessionId, state);
    return getConversationContext(this.sessionId);
  }

  setMaterial(material: string): void {
    this.ensureActiveJob();
    updateJob(this.sessionId, { material });
  }

  setMachine(machine: string): void {
    this.ensureActiveJob();
    updateJob(this.sessionId, { machine });
  }

  addRecommendation(action: string, result: string): void {
    this.ensureActiveJob();
    updateJob(this.sessionId, { recommendation: { action, result_summary: result } });
  }
  ...
  private ensureActiveJob(): void {
    const ctx = getConversationContext(this.sessionId);
    if (!ctx.active_job) {
      startJob(this.sessionId, {});
    }
  }
}

export const conversationalMemoryEngine = new ConversationalMemoryEngineFacade();
