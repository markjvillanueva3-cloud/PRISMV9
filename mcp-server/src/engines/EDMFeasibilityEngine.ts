...
import type { WEDMMachineSpec } from "../data/wedm-published-machines.js";
 ...
 * Derived from median of published WEDM machine specs.
 */
const DEFAULT_UV_TRAVEL_MM = (() => {
  if (WEDM_MACHINE_SPECS.length === 0) return 60;
  const uvValues = WEDM_MACHINE_SPECS.map(m => Math.min(m.uv_travel_mm.u, m.uv_travel_mm.v));
  uvValues.sort((a, b) => a - b);
  return uvValues[Math.floor(uvValues.length / 2)]; // median
})();
...
/**
 * Find the best matching published machine spec for feasibility validation.
 * Uses workpiece dimensions to filter machines that can physically hold the part.
 */
function findMatchingMachine(workpiece: FeasibilityInput["workpiece"]): WEDMMachineSpec | null {
  const candidates = WEDM_MACHINE_SPECS.filter(m =>
    m.max_workpiece_mm.w >= workpiece.length_mm &&
    m.max_workpiece_mm.d >= workpiece.width_mm &&
    m.max_workpiece_mm.h >= workpiece.thickness_mm &&
    m.travels_mm.x >= workpiece.length_mm * 0.8 &&
    m.travels_mm.y >= workpiece.width_mm * 0.8
  );
  if (candidates.length === 0) return null;
  // Prefer smallest machine that fits (cost-effective)
  candidates.sort((a, b) =>
    (a.travels_mm.x * a.travels_mm.y) - (b.travels_mm.x * b.travels_mm.y)
  );
  return candidates[0];
}
...
class EDMFeasibilityEngine {
  ...
  assess(input: FeasibilityInput): FeasibilityResult {
    ...
    const result: FeasibilityResult = {
      ...
      warnings,
      recommendations,
      machine_availability: machineAvail,
    };
  ...
  private _checkTaper(
    ...
    // Use explicit machine specs → published machine match → defaults
    let uvTravel = DEFAULT_UV_TRAVEL_MM;
    if (input.machine?.uv_travel_mm) {
      uvTravel = input.machine.uv_travel_mm;
    } else {
      const pubMachine = findMatchingMachine(input.workpiece);
      if (pubMachine) {
        uvTravel = Math.min(pubMachine.uv_travel_mm.u, pubMachine.uv_travel_mm.v);
      }
    }
  ...
  // ==========================================================================
  // PRIVATE — U-WGAP05 MACHINE QUEUE AWARENESS
  // ==========================================================================
