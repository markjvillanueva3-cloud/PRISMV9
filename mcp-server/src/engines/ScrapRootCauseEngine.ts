...
/** Thermal growth estimate: δ = α · ΔT · L, using steel α ≈ 12e-6 /°C, L ≈ 300mm workpiece */
function estimateThermalGrowth(tempDelta?: number): number | undefined {
  if (tempDelta === undefined || tempDelta === 0) return undefined;
  const alpha = 12e-6; // /°C for steel
  const L = 300; // mm typical workpiece
    ...
    },
    {
      ...
      check: (input) => {
        if (input.process_data?.spindle_load_pct !== undefined && input.process_data.spindle_load_pct > 60) {
          return { probability: 0.4, evidence: ["High spindle load suggests aggressive feed — Ra ∝ f²/(8·r_nose)"] };
        }
        return { probability: 0.3, evidence: ["Feed rate is primary driver of theoretical Ra — verify f vs Ra requirement"] };
      },
      corrective_action: "Reduce feed per revolution — Ra ≈ f²/(8·r), halving feed reduces Ra by 4×.",
    },
    {
...
export class ScrapRootCauseEngine {
  ...
  analyzeScrapEvent(input: ScrapEventInput): ScrapAnalysisResult {
    const physics: PhysicsAnalysis = {
      thermal_growth_mm: estimateThermalGrowth(input.process_data?.temperature_delta_C),
    ...
    return { probable_causes, physics_analysis: physics, pattern_type, recommended_checks };
  }
