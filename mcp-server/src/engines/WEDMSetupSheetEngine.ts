...
// ============================================================================
// AI-POWERED SETUP SHEET ENHANCEMENTS
// ============================================================================

/**
 * AI-powered setup sheet enhancement with intelligent recommendations.
 *
 * Uses PRISM Intelligence Layer to:
 * - Generate context-aware safety notes
 * - Optimize operator instructions based on part complexity
 * - Add machine-specific guidance
 * - Suggest pre-flight checks
 *
 * @param data Generated setup sheet data
 * @returns AI reasoning result with enhanced setup guidance
 */
export async function enhanceSetupSheetWithAI(data: WEDMSetupSheetData): Promise<AIReasoningResult> {
  return prismIntelligence.reason({
    domain: "wedm_setup",
    intent: "Enhance Wire EDM setup sheet with intelligent operator guidance and safety recommendations",
    context: {
      part_name: data.header.part_name,
      material: data.header.material,
      hardness_hrc: data.header.hardness_hrc,
      thickness_mm: data.header.thickness_mm,
      wire_type: data.header.wire_type,
      wire_diameter_mm: data.header.wire_diameter_mm,
      controller: data.header.controller,
      num_passes: data.machine_setup.num_passes,
      flush_pressure_bar: data.machine_setup.flush_pressure_bar,
      submerged: data.machine_setup.submerged,
      cycle_time_min: data.cycle_time.total_min,
      wire_needed_m: data.consumables.wire_needed_m,
      existing_warnings: data.warnings,
      existing_safety_notes: data.safety_notes.slice(0, 5),
...
/**
 * AI-powered operator skill-level adaptation.
 *
 * @param data Setup sheet data
 * @param skillLevel Operator skill level
 * @returns AI reasoning result with skill-appropriate guidance
 */
export async function adaptToOperatorSkillWithAI(
  data: WEDMSetupSheetData,
  skillLevel: "beginner" | "intermediate" | "expert"
): Promise<AIReasoningResult> {
  return prismIntelligence.reason({
    domain: "wedm_setup",
    intent: `Adapt Wire EDM setup sheet instructions for ${skillLevel} operator skill level`,
    context: {
      ...
      wire_type: data.header.wire_type,
      controller: data.header.controller,
      cycle_time_hours: data.cycle_time.total_min / 60,
      pass_parameters: data.pass_table.slice(0, 3).map(p => ({
        pass: p.pass_number,
        type: p.pass_type,
...
/**
 * AI-powered troubleshooting guide generation.
 *
 * @param data Setup sheet data
 * @param potentialIssues Known potential issues for this job
 * @returns AI reasoning result with troubleshooting guidance
 */
export async function generateTroubleshootingGuideWithAI(
  data: WEDMSetupSheetData,
  potentialIssues?: string[]
): Promise<AIReasoningResult> {
  // Identify potential issues based on job parameters
  const autoIssues: string[] = [];

  if (data.header.thickness_mm > 80) {
    autoIssues.push("thick_section_flushing");
  }
  if (data.header.hardness_hrc >= 60) {
    autoIssues.push("hard_material_wire_wear");
  }
  if (data.header.material.toLowerCase().includes("carbide")) {
    autoIssues.push("carbide_wire_break_risk");
  }
  if (data.machine_setup.num_passes >= 5) {
    autoIssues.push("multi_pass_surface_quality");
  }
  if (data.cycle_time.total_min > 240) {
    autoIssues.push("long_cycle_wire_supply");
  }

  return prismIntelligence.reason({
    domain: "wedm_troubleshoot",
    intent: "Generate troubleshooting guide for Wire EDM job setup and operation",
    context: {
      material: data.header.material,
      hardness_hrc: data.header.hardness_hrc,
      thickness_mm: data.header.thickness_mm,
      ...
      submerged: data.machine_setup.submerged,
      flush_pressure_bar: data.machine_setup.flush_pressure_bar,
      potential_issues: [...autoIssues, ...(potentialIssues ?? [])],
      cycle_time_min: data.cycle_time.total_min,
    },
    options: {
      require_explanation: true,
