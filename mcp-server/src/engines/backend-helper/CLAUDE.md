...
- **Reasoning bridge** (`scripts/lib/galaxy-reasoning-bridge.mjs`, PSN leg #10): **CAG** + **RAG** hybrid
  ...
  local Ollama stack -- `node scripts/lib/galaxy-reasoning-bridge.mjs backend-helper "<question>"`.
- **Vault -> LoRA**: this galaxy's Obsidian **synthesis** brain (`knowledge/memories/patterns/backend-helper_synthesis.md`)
...
## 14. Cross-galaxy work + commit to their worktrees (PERMANENT, operator 2026-06-15)

**Papa is never idle.** When papa's own queue is exhausted (WIRE-UNWIRED + H-DRIVE + build health),
papa AUTONOMOUSLY picks up OTHER galaxies' backend-fittable work across all 34 galaxies — wiring
dispatcher-unwired engines, TSC triage, Zod schema validation, vitest coverage, build health — and
commits each to the OWNING slot's branch/worktree. Drive it with a cron + harnessed `/loop` + Hermes
agentic coding (Agent fan-out for triage + per-engine wiring). Full rule:
[[feedback_papa_cross_galaxy_work_commit_to_their_worktrees]].

**Protocol (every cross-galaxy pickup):**
1. **Re-audit, never trust a stale worklist** — `node H:/prism/scripts/audit-unwired-engines.mjs` for the
   LIVE backlog. (The 2026-06-15 "no candidates" miss came from a fixed worklist without re-auditing.)
2. **dup-check EACH engine across ALL branches** — `git -C H:/prism log --all --oneline | grep -i <engine>`
   BEFORE wiring. Peers build on slot branches (BarRemnant->romeo, MeasureSummary->romeo, PactContract->november
   were already-built-unmerged; XProcNeuralAutoFire was already LIVE). Re-wiring = a merge collision.
3. **Commit to the owning slot's worktree/branch when free:**
   `cd H:/prism-slot-<X> && git add ... && git commit -m "[<SCOPE>]/U-... (slot:papa->X): ..."`.
   **Safe fallback:** if slot X is LIVE (fresh `chat-slots.json` heartbeat) OR its worktree is contended
   (`index.lock`/`CHERRY_PICK_HEAD`), commit to the shared integration tree with X's `[SCOPE]` tag +
   `(slot:papa->X)` attribution and post to the chat bus. **NEVER clobber a peer's live worktree.**
4. **Universal rails still bind** — per-file 2-agent scrutiny, `tsc` 0-new, affected vitest, no stubs,
   no inlined physics constants. Papa does the WIRING; **domain-PHYSICS edits defer to the domain slot.**
5. Read-method-only for engines whose write/persist methods do fs I/O — expose the JSON-pure reads,
   leave the fs-mutating ones out (or guard them).

**Live cross-galaxy worklist:** `state/shared/specs/PAPA-WIRE-UNWIRED-WORKLIST-2026-06-15.md`
(11 CLEAN from the 2026-06-15 fleet re-audit; 13 DEFERRED transport/closure; 4 already-wired/redundant).
