  ...
  runPipeline(input: SecondaryOpsInput): SecondaryOpsResult {
    ...
    for (const op of input.operations) {
      ...
      switch (op.type) {
        ...
        default: {
          gcode.push(`(${op.type} — not yet implemented)`);
          toolDesc = "Unknown";
          cycleTime = 10;
          warnings.push({ stage: "planning", severity: "warning", message: `Op ${op.id}: unsupported type "${op.type}"` });
        }
      }
    ...
    // Assemble program
    const programText = this.assembleProgram(results, input);
