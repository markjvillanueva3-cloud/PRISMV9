...
**Two findings for oscar to action:**
1. **`SpeedFeedOutcomeFeedbackBridgeEngine.tryBusCapture()` is a hardwired `return true`** -> `stats().bus_capture_success_rate_pct` is a FAKE constant 100% (pre-existing oscar code 2026-05-26; the new `speedfeed_outcome_stats` action now EXPOSES this constant). Wire a real bus-capture when convenient so the metric is truthful (R12).
2. **False `// WIRE-EXEMPT` markers** -- several SFC engines carried a line-1 WIRE-EXEMPT comment whose alleged consumers were PHANTOM (a code comment / a `surfaces_into` metadata string / a reverse-direction ref), hiding a real orphan from the unwired-engine audit. Worth a fleet sweep for WIRE-EXEMPT markers that NAME no wrapper singleton (CLAUDE.md ENGINE-WIRING requires the tag to name the wrapper).
