...
**SFC FRONTEND — oscar OWNS it (operator directive 2026-06-22, overrides the default `frontend → quebec` gate).**
The SFC web/app page suite (`mcp-server/web/` SpeedFeedPage `/speed-feed`, CalculatorPage `/calculator`,
SfcCalculatorPage `/speed-feed-calc`, plus future Electron/iOS/Android shells of the SFC product) is oscar's
to build + harden — "SFC is your specialty so you'll understand it better than quebec." quebec is NOT a gate
for SFC-product frontend work. After building, run full CLOSED-LOOP testing of the entire SFC app page suite
(verify the calculations are correct end-to-end), **JM Die fleet machines FIRST**. See
[[reference_oscar_sfc_frontend_ownership_2026_06_22]]. (Cross-app shell infra still coordinates with quebec;
the SFC-specific pages + their calc correctness are oscar's.)
...
| Outcome feedback / bus capture | `SpeedFeedOutcomeFeedbackBridgeEngine.ts` (real bus wired 2026-06-22 — see §5.5) |
...
5. **`SpeedFeedOutcomeFeedbackBridgeEngine.tryBusCapture()` — FIXED 2026-06-22 (`U-SFC-OUTCOME-BUS-REAL`).** Was
   hardwired `return true` (fake 100% `bus_capture_success_rate_pct`); now calls the real `captureSFC`
   (sfcOutcomeWire) and returns its `ok` flag, so `speedfeed_outcome_stats.bus_capture_success_rate_pct`
   is a truthful metric. The NineAxis layer now actually reaches the canonical bus (the orchestrator does
   not emit captureSFC for that layer, so no double-capture). Originally flagged by bravo's 2026-06-11 sweep.
...
- **`speedfeed_outcome_stats` bus_capture_success_rate_pct is now a REAL metric** (fixed 2026-06-22, `U-SFC-OUTCOME-BUS-REAL`) — was a hardwired fake 100%; now reflects actual `captureSFC` success.
...
- ~~**`SpeedFeedOutcomeFeedbackBridgeEngine.tryBusCapture()` hardwired `return true`**~~ — FIXED 2026-06-22
  (`U-SFC-OUTCOME-BUS-REAL`): now calls real `captureSFC` + returns its `ok`; `bus_capture_success_rate_pct`
  is truthful. 8-test proof in `SpeedFeedOutcomeFeedbackBridge-bus-capture.test.ts`.
...
- **Reasoning bridge** (`scripts/lib/galaxy-reasoning-bridge.mjs`, PSN leg #10): **CAG** + **RAG** hybrid
  ...
  local Ollama stack -- `node scripts/lib/galaxy-reasoning-bridge.mjs speed-feed "<question>"`.
- **Vault -> LoRA**: this galaxy's Obsidian **synthesis** brain (`knowledge/memories/patterns/speed-feed_synthesis.md`)
