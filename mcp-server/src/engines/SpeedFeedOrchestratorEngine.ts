...
import { CANONICAL_TAYLOR, CANONICAL_TOOL_MODULUS, CANONICAL_MATERIAL_DB, CANONICAL_KIENZLE, kcHardnessFactor } from "../physics/constants.js";
...
export class SpeedFeedOrchestratorEngine {
  ...
  private resolveMaterial(input: OrchestratorInput): ResolvedMaterial {
    ...
    // Adjust kc1_1 based on hardness deviation from DB default -- via the canonical
    // kcHardnessFactor (constants.ts, U-OSC-SFC-HARDNESS-KC-PARITY): identical math to the
    // previous inline (hb/rec.hb)^0.4 for valid positive inputs; a non-positive/non-finite
    // user hardness now fail-neutrals to factor 1.0 (record kc) instead of ZEROING or
    // NaN-ing kc as the old inline did (a hb=0 input used to publish zero cutting force).
    // Single-sourced so this engine and the customer ProductEngine sfc_* paths can never
    // drift apart on the hardness->kc physics.
    const kcFactor = kcHardnessFactor(hb, rec.hb);
    const hardnessRatio = kcFactor === 1 && !(hb > 0 && Number.isFinite(hb)) ? 1 : hb / rec.hb;
    const adjustedKc = rec.kc1_1 * kcFactor;
