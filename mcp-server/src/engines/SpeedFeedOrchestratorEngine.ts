...
import { tribalKnowledgeAdvisorEngine, type TribalModifiers } from "./TribalKnowledgeAdvisorEngine.js";
import { speedFeedResourceIntegrationEngine, type ResourceInsights } from "./SpeedFeedResourceIntegrationEngine.js";
...
export interface OrchestratorInput {
  ...
  machine_rate_usd_hr?: number;     // shop rate in $/hr
  tool_change_time_min?: number;
  part_volume_cm3?: number;         // for cycle time estimation
...
export interface OrchestratorResult {
  ...
  // ── LLM-Level AI Reasoning (LLM-INTEL) ──
  ai_reasoning?: SpeedFeedAIReasoning;

  // ── PDF Resource Integration (SpeedFeedResourceIntegrationEngine) ──
  resource_insights?: ResourceInsights;
}

// ============================================================================
// LLM-LEVEL AI REASONING TYPES
// ============================================================================

/** Decision trace for a single parameter choice */
...
/** Hypothesis about optimal parameters */
export interface SpeedFeedHypothesis {
  id: string;
  statement: string;
...
export interface OptimizationObjectives {
  /** Productivity weight (MRR, cycle time) */
  productivity: number;
  /** Tool cost weight (tool life, wear rate) */
  tool_cost: number;
  /** Quality weight (surface finish, accuracy) */
  quality: number;
  /** Safety weight (force limits, chatter avoidance) */
  safety: number;
  /** Explanation of optimization balance */
  balance_explanation: string;
}

/** Pre-machining risk assessment */
export interface MachiningRiskAssessment {
  /** Overall risk level */
  ...
  safety_factors: {
    speed_derating: number;
    feed_derating: number;
    depth_derating: number;
  };
...
/** Complete AI reasoning for speed/feed calculation */
export interface SpeedFeedAIReasoning {
  /** Decision trace for each parameter */
  decision_trace: ParameterDecision[];

  /** Hypotheses evaluated */
  hypotheses: SpeedFeedHypothesis[];
  ...
  /** Pre-machining risk assessment */
  risk_assessment: MachiningRiskAssessment;
  ...
  uncertainty_analysis: {
    ...
    /** Suggested calibration to reduce uncertainty */
    suggested_calibration: string;
    /** Confidence intervals */
    confidence_intervals: {
      speed_ci95_pct: [number, number];
      feed_ci95_pct: [number, number];
      life_ci95_pct: [number, number];
    };
  };
  ...
  cost_benefit: {
    ...
    /** Machine time per part (min) */
    cycle_time_min: number;
    /** Break-even volume for optimization */
...
const MATERIAL_DB: Record<string, MaterialRecord> = {
  ...
  // Tool steels in ANNEALED state (28-35 HRC) — JM Die's primary working condition
  // These are ISO P group materials until hardened. D2, A2, S7, M2 are typically
  // received at 28-32 HRC from steel suppliers and machined before heat treatment.
  tool_steel_annealed: {
    iso_group: "P",
    hb: 280,  // ~30 HRC converted
    sigma_y_MPa: 700,
    kc1_1: 2000,  // Between alloy steel (2100) and mild steel (1800)
    mc: 0.25,
    k_thermal: 28,
    machinability_factor: 0.55,
    vc_base: { roughing: 100, finishing: 160 },  // Conservative for tool steels
  ...
  hardened_steel: {
    ...
    aliases: ["hrc 50", "hrc 55", "hrc 60", "hrc60", "hardened", "hardened steel", "hardened tool steel"],
  },
  plastic: {
    iso_group: "N",
    hb: 20,
...
const SFO_CANONICAL_MAP: Record<string, string> = {
  ...
  // MILL-HARD-MS1: Tool steel annealed uses alloy_steel Kienzle constants (kc1_1=2100)
  // since annealed tool steels have similar cutting characteristics to alloy steels
  tool_steel_annealed: "alloy_steel",
};
...
export class SpeedFeedOrchestratorEngine {
  ...
  private resolveMaterial(input: OrchestratorInput): ResolvedMaterial {
    ...
    // ── MILL-HARD-MS1: Tool Steel State Classification ──
    // Tool steels (D2, A2, S7, M2, H13) require hardness-based classification.
    // JM Die typically receives these at 28-32 HRC (annealed) and machines before hardening.
    // Without explicit hardness, default to annealed state for better productivity.
    if (input.material !== undefined) {
      const matLower = input.material.toLowerCase();
      const toolSteelGrades = ["d2", "a2", "s7", "m2", "h13", "d-2", "a-2", "s-7", "m-2", "h-13"];
      const isToolSteel = toolSteelGrades.some(g => matLower.includes(g));

      if (isToolSteel) {
        const hrc = input.hardness_hrc;
        if (hrc !== undefined && hrc >= 45) {
          // Hardened state: use ISO H group parameters
          matKey = "hardened_steel";
          matchSource = `tool_steel_hardened:${input.material}@${hrc}HRC`;
        } else {
          // Annealed state (default): use tool_steel_annealed (ISO P-like)
          matKey = "tool_steel_annealed";
          matchSource = hrc !== undefined
            ? `tool_steel_annealed:${input.material}@${hrc}HRC`
            : `tool_steel_annealed:${input.material}(assumed_annealed)`;
        }
      }
    }

    // Standard fuzzy match for non-tool-steel materials
    if (input.material !== undefined && matKey === undefined) {
  ...
  private queryProvenParameters(input: OrchestratorInput): {
    ...
    try {
      ...
      const { provenSpeedFeedAggregatorEngine } = require("./ProvenSpeedFeedAggregatorEngine.js");

      // Map input to proven parameter categories
      const materialGroup = this.mapToProvenMaterial(input);
      const opCategory = this.mapToProvenOperation(input);

      const proven = provenSpeedFeedAggregatorEngine.getProvenParams(materialGroup, opCategory);

      if (proven && proven.sampleCount >= 3 && proven.confidence >= 0.6) {
        const source = `proven_program:${proven.id}:n=${proven.sampleCount}`;

        return {
          cssSpeed: proven.cssSpeed ? {
            value: proven.cssSpeed.median,
            confidence: proven.confidence * 0.88, // Scale to 0.88 max
            source,
          } : undefined,
          feedRate: proven.feedRate ? {
            value: proven.feedRate.median,
            confidence: proven.confidence * 0.88,
            source,
          } : undefined,
          source,
          found: true,
      ...
      // ProvenSpeedFeedAggregatorEngine not loaded or no data
    }
  ...
  private mapToProvenMaterial(input: OrchestratorInput): string {
    ...
    if (input.iso_group) {
      return isoMap[input.iso_group] || "unknown";
    }

    // Infer from material name
    const mat = (input.material || "").toLowerCase();
    if (mat.includes("m2") || mat.includes("d2") || mat.includes("a2") || mat.includes("s7") || mat.includes("h13")) {
      return "tool_steel";
    }
    ...
    return "tool_steel"; // JM Die default
  }

  private mapToProvenOperation(input: OrchestratorInput): string {
    const op = (input.operation || "").toLowerCase();
    const cut = (input.cut_type || "").toLowerCase();

    if (op === "turning") {
      return cut === "finishing" ? "od_finishing" : "od_roughing";
    }
    if (op === "boring") {
      return cut === "finishing" ? "id_finishing" : "id_roughing";
    }
    if (op === "drilling") return "drilling";
    if (op === "threading" || op === "tapping") return "threading";
    if (op === "facing") return "facing";
    if (op === "grooving") return "grooving";
    if (op === "parting") return "parting";
    if (op === "milling") {
      return cut === "finishing" ? "milling_finishing" : "milling_roughing";
    }
  ...
  public compute(input: OrchestratorInput & { resumeFromStage?: number; checkpointRunId?: string }): AtomicValue<OrchestratorResult> {
    ...
    t0 = Date.now();
    const proven = this.queryProvenParameters(input);
    if (proven.found) {
      engines_called.push("ProvenSpeedFeedAggregatorEngine");
    ...
    if (tribalMods.evidence_count > 0) {
      engines_called.push("TribalKnowledgeAdvisorEngine");
      formulas_used.push(`Tribal modifiers: Vc×${tribalMods.vc_modifier.toFixed(2)}, fz×${tribalMods.fz_modifier.toFixed(2)} (${tribalMods.evidence_count} tips, conf=${(tribalMods.confidence * 100).toFixed(0)}%)`);
    }
    if (resumeFrom <= 9) cpm.checkpoint('query_tribal_mods', 9, tribalMods, Date.now() - t0);
    ...
    const tribalVcMod = tribalMods.vc_modifier;
    ...
    // ── KAR-MS2 U-KAR14: Proven Program Integration ──
    // If proven parameters exist with high confidence, use them as validation
    // or apply a weighted blend with physics-based Vc.
    let provenVcAdjustment = 1.0;
    let provenFeedAdjustment = 1.0;
    if (proven.found && proven.cssSpeed && proven.cssSpeed.confidence >= 0.7) {
    ...
    const tribalFzMod = tribalMods.fz_modifier;
    fz *= tribalFzMod;
    ...
    const tribalApMod = tribalMods.ap_modifier;
    ap *= tribalApMod;
    ...
    const tribalAeMod = tribalMods.ae_modifier;
    ae *= tribalAeMod;
    ...
    const tribalTlMod = tribalMods.tool_life_modifier;
    ...
    // ── Step 9c: Resource Integration (PDF knowledge + JM Die materials) ──
    let resourceInsights: ResourceInsights | undefined;
    try {
      resourceInsights = speedFeedResourceIntegrationEngine.buildResourceInsights(
        material.name.value,
        material.iso_group.value as ISOGroup,
        tool.diameter_mm.value,
        (input.operation ?? "milling") as any,
        input.strategy,
        input.machine_name,
      );
      engines_called.push("SpeedFeedResourceIntegrationEngine");
    } catch (e) {
      log.warn(`[SpeedFeedOrchestrator] SpeedFeedResourceIntegrationEngine skipped: ${e}`);
    }
    ...
    // ── LLM-INTEL: Build AI reasoning metadata ──
    result.ai_reasoning = this.buildAIReasoning(
      input,
      result,
      fullUQ,
      sortedFactors,
    ...
    // ── Resource Integration: attach PDF-sourced insights ──
    if (resourceInsights) {
      result.resource_insights = resourceInsights;
    }
  ...
  // ============================================================================
  // LLM-LEVEL AI REASONING METHODS
  // ============================================================================

  /**
   * Build comprehensive AI reasoning metadata for speed/feed calculation.
   * Provides LLM-level transparency into parameter selection.
   */
  private buildAIReasoning(
    input: OrchestratorInput,
    result: OrchestratorResult,
    uq: ReturnType<SpeedFeedOrchestratorEngine["runFullUQ"]>,
    limitingFactors: LimitingFactor[],
    recommendations: string[],
    formulas: string[],
    tribalMods: TribalModifiers,
  ): SpeedFeedAIReasoning {
    // Build decision trace for each parameter
    const decisionTrace = this.buildDecisionTrace(input, result, limitingFactors);

    // Generate and evaluate hypotheses
    const hypotheses = this.generateSpeedFeedHypotheses(input, result, uq);
    const bestHypothesis = hypotheses.find(h => h.status === "confirmed")?.id ?? null;

    // Build optimization objectives
    const optimization = this.buildOptimizationObjectives(input, result);

    // Assess machining risks
    const riskAssessment = this.assessMachiningRisks(input, result, uq);

    // Build uncertainty analysis
    const uncertaintyAnalysis = this.buildUncertaintyAnalysis(result, uq);

    // Cost/benefit analysis
    const costBenefit = this.buildCostBenefit(input, result);

    // Generate natural language explanation
    const explanation = this.generateSpeedFeedExplanation(
      input, result, decisionTrace, hypotheses, riskAssessment, tribalMods
    );

    // Calculate meta-confidence
    const metaConfidence = this.calculateSpeedFeedMetaConfidence(
      result, hypotheses, riskAssessment, tribalMods
    );

    // Counterfactual analysis
    const counterfactual = this.buildCounterfactual(input, result, limitingFactors);
  ...
  /**
   * Build decision trace showing why each parameter was chosen.
   */
  private buildDecisionTrace(
    ...
    // Cutting speed decision
    const speedFactor = limitingFactors.find(f =>
      f.parameter.includes("speed") || f.parameter.includes("Vc")
    );
    decisions.push({
      parameter: "cutting_speed_mpm",
      chosen_value: result.cutting_speed_mpm,
      unit: "m/min",
      reason: speedFactor
        ? `Limited by ${speedFactor.constraint} (${speedFactor.utilization_pct}% utilization)`
        : "Based on material machinability and tool capability",
      alternatives_considered: [
        {
          value: result.cutting_speed_mpm * 1.2,
          source: "aggressive_mrr",
          why_rejected: "Would exceed tool life threshold or machine power",
        },
        {
          value: result.cutting_speed_mpm * 0.8,
          source: "conservative",
          why_rejected: "Unnecessarily reduces productivity without safety benefit",
        },
      ],
      confidence: result.resolved_material.vc_base_roughing.confidence,
      limiting_factor: speedFactor?.constraint,
    });

    // Feed per tooth decision
    const feedFactor = limitingFactors.find(f =>
      f.parameter.includes("feed") || f.parameter.includes("fz")
    );
    decisions.push({
      parameter: "feed_per_tooth_mm",
      chosen_value: result.feed_per_tooth_mm,
      unit: "mm/tooth",
      reason: feedFactor
        ? `Limited by ${feedFactor.constraint}`
        : "Optimized for chip thickness and surface finish",
      alternatives_considered: [
        {
          value: result.feed_per_tooth_mm * 1.3,
          source: "high_mrr",
          why_rejected: "Would degrade surface finish beyond acceptable Ra",
        },
      ],
      confidence: 0.85,
      limiting_factor: feedFactor?.constraint,
    });

    // Depth of cut decision
    const depthFactor = limitingFactors.find(f =>
      f.parameter.includes("depth") || f.parameter.includes("ap")
    );
    decisions.push({
      parameter: "axial_depth_mm",
      chosen_value: result.axial_depth_mm,
      unit: "mm",
      reason: depthFactor
        ? `Limited by ${depthFactor.constraint}`
        : "Set for stable cutting with acceptable force levels",
      alternatives_considered: [
        {
          value: result.axial_depth_mm * 2,
          source: "full_depth",
          why_rejected: result.stability_assessment.zone !== "stable"
            ? "Would cause chatter instability"
            : "Would exceed tool deflection limits",
        },
      ],
      confidence: 0.82,
      limiting_factor: depthFactor?.constraint,
    });

    // Spindle RPM decision
    decisions.push({
      parameter: "spindle_rpm",
      chosen_value: result.spindle_rpm,
      unit: "RPM",
      reason: result.stability_assessment.suggested_rpm_pocket
        ? `Adjusted to stable pocket (lobe ${result.stability_assessment.lobe_index})`
        : "Calculated from cutting speed and tool diameter",
      alternatives_considered: result.stability_assessment.suggested_rpm_pocket
        ? [{
            value: result.stability_assessment.suggested_rpm_pocket,
            source: "lobe_pocket",
            why_rejected: "Current RPM within stability zone",
          }]
        : [],
      confidence: 0.9,
    });
  ...
  /**
   * Generate hypotheses about optimal parameter selection.
   */
  private generateSpeedFeedHypotheses(
    input: OrchestratorInput,
    result: OrchestratorResult,
    uq: ReturnType<SpeedFeedOrchestratorEngine["runFullUQ"]>,
  ): SpeedFeedHypothesis[] {
    const hypotheses: SpeedFeedHypothesis[] = [];

    // Hypothesis 1: Parameters are within safe operating envelope
    const safetyHyp: SpeedFeedHypothesis = {
      id: "safe_envelope",
      statement: "Selected parameters are within safe operating envelope",
    ...
    const passedChecks = result.safety_checks.filter(c => c.passed).length;
    const totalChecks = result.safety_checks.length;
    if (totalChecks > 0) {
      const passRate = passedChecks / totalChecks;
      safetyHyp.posterior_probability = 0.5 + passRate * 0.45;
      if (passRate === 1) {
        safetyHyp.supporting_evidence.push(`All ${totalChecks} safety checks passed`);
        safetyHyp.status = "confirmed";
      } else {
        safetyHyp.contradicting_evidence.push(
          `${totalChecks - passedChecks} of ${totalChecks} safety checks failed`
        );
        if (passRate < 0.5) safetyHyp.status = "refuted";
      }
    }
    hypotheses.push(safetyHyp);

    // Hypothesis 2: Cutting is stable (no chatter)
    const stabilityHyp: SpeedFeedHypothesis = {
      id: "stable_cutting",
      statement: "Cutting conditions are dynamically stable (no chatter)",
    ...
    if (uq.p_chatter < 0.1) {
      stabilityHyp.supporting_evidence.push(`Low chatter probability: ${(uq.p_chatter * 100).toFixed(1)}%`);
      stabilityHyp.posterior_probability = 0.9;
      stabilityHyp.status = "confirmed";
    } else if (uq.p_chatter > 0.4) {
      stabilityHyp.contradicting_evidence.push(`High chatter probability: ${(uq.p_chatter * 100).toFixed(1)}%`);
      stabilityHyp.posterior_probability = 0.3;
      stabilityHyp.status = "refuted";
    } else {
      stabilityHyp.supporting_evidence.push(`Marginal chatter probability: ${(uq.p_chatter * 100).toFixed(1)}%`);
      stabilityHyp.posterior_probability = 0.6;
    }
    hypotheses.push(stabilityHyp);

    // Hypothesis 3: Tool life is adequate
    const toolLifeHyp: SpeedFeedHypothesis = {
      id: "adequate_tool_life",
      statement: "Expected tool life is economically viable (>15 min)",
    ...
    if (result.tool_life_min >= 30) {
      toolLifeHyp.supporting_evidence.push(`Good tool life: ${result.tool_life_min} min`);
      toolLifeHyp.posterior_probability = 0.9;
      toolLifeHyp.status = "confirmed";
    } else if (result.tool_life_min < 10) {
      toolLifeHyp.contradicting_evidence.push(`Short tool life: ${result.tool_life_min} min`);
      toolLifeHyp.posterior_probability = 0.3;
      toolLifeHyp.status = "refuted";
    } else {
      toolLifeHyp.supporting_evidence.push(`Acceptable tool life: ${result.tool_life_min} min`);
    }
    hypotheses.push(toolLifeHyp);

    // Hypothesis 4: Surface finish meets typical requirements
    const finishHyp: SpeedFeedHypothesis = {
      id: "acceptable_finish",
      statement: "Surface finish meets typical machining requirements",
      prior_probability: 0.8,
      posterior_probability: 0.8,
      supporting_evidence: [],
      contradicting_evidence: [],
    ...
    if (result.surface_finish_Ra_um <= 1.6) {
      finishHyp.supporting_evidence.push(`Excellent finish: Ra ${result.surface_finish_Ra_um} µm`);
      finishHyp.posterior_probability = 0.95;
      finishHyp.status = "confirmed";
    } else if (result.surface_finish_Ra_um <= 3.2) {
      finishHyp.supporting_evidence.push(`Good finish: Ra ${result.surface_finish_Ra_um} µm`);
      finishHyp.posterior_probability = 0.85;
      finishHyp.status = "confirmed";
    } else if (result.surface_finish_Ra_um > 6.3) {
      finishHyp.contradicting_evidence.push(`Rough finish: Ra ${result.surface_finish_Ra_um} µm`);
      finishHyp.posterior_probability = 0.4;
    }
    hypotheses.push(finishHyp);

    return hypotheses;
  }

  /**
   * Build optimization objectives and weights.
   */
  private buildOptimizationObjectives(
    input: OrchestratorInput,
    result: OrchestratorResult,
  ): OptimizationObjectives {
    // Determine optimization balance from cut type and operation
    const operation = input.operation?.toLowerCase() ?? "milling";
    const cutType = input.cut_type?.toLowerCase() ?? "roughing";

    let productivity = 0.4;
    let tool_cost = 0.2;
    let quality = 0.2;
    let safety = 0.2;
    let balance = "Balanced optimization";

    if (cutType === "roughing") {
      productivity = 0.5;
      quality = 0.1;
      balance = "Productivity-focused (roughing)";
    } else if (cutType === "finishing") {
      productivity = 0.2;
      quality = 0.5;
      balance = "Quality-focused (finishing)";
    } else if (cutType === "semi-finishing") {
      productivity = 0.35;
      quality = 0.35;
      balance = "Balanced (semi-finishing)";
    }

    // Safety weight increases for difficult materials
    const isoGroup = result.resolved_material.iso_group.value;
    if (isoGroup === "S" || isoGroup === "H") {
      safety = 0.3;
      tool_cost = 0.25;
      productivity -= 0.1;
      balance += "; elevated safety for difficult material";
    }
  ...
  /**
   * Assess machining risks before cutting.
   */
  private assessMachiningRisks(
    input: OrchestratorInput,
    result: OrchestratorResult,
    uq: ReturnType<SpeedFeedOrchestratorEngine["runFullUQ"]>,
  ): MachiningRiskAssessment {
    const risks: MachiningRiskAssessment["risks"] = [];
    let riskLevel: MachiningRiskAssessment["risk_level"] = "low";
    const blockers: string[] = [];

    // Chatter risk
    if (uq.p_chatter > 0.4) {
      risks.push({
        risk: "High probability of chatter vibration",
        severity: "high",
        probability: uq.p_chatter,
        mitigation: result.stability_assessment.suggested_rpm_pocket
          ? `Switch to RPM ${result.stability_assessment.suggested_rpm_pocket} (stable pocket)`
          : "Reduce depth of cut by 30-50%",
      });
      riskLevel = "high";
    } else if (uq.p_chatter > 0.1) {
      risks.push({
        risk: "Marginal chatter stability",
        severity: "medium",
        probability: uq.p_chatter,
        mitigation: "Monitor for vibration, reduce depth if chatter occurs",
      });
      if (riskLevel === "low") riskLevel = "medium";
    }

    // Tool breakage risk (short tool life + high force)
    if (result.tool_life_min < 10 && result.tangential_force_N > 1000) {
      risks.push({
        risk: "Elevated tool breakage risk",
        severity: "high",
        probability: 0.15,
        mitigation: "Reduce feed rate by 20% or use higher grade insert",
      });
      riskLevel = "high";
    }

    // Deflection risk
    if (result.deflection_um > 50) {
      risks.push({
        risk: `High tool deflection (${result.deflection_um} µm)`,
        severity: result.deflection_um > 100 ? "critical" : "high",
        probability: 0.8,
        mitigation: "Reduce stickout, use larger diameter tool, or reduce depth",
      });
      if (result.deflection_um > 100) {
        riskLevel = "critical";
        blockers.push("Tool deflection exceeds safe limits");
      }
    }

    // Power limit risk
    const powerCheck = result.safety_checks.find(c => c.name.includes("power"));
    if (powerCheck && !powerCheck.passed) {
      risks.push({
        risk: "Machine power limit exceeded",
        severity: "critical",
        probability: 1.0,
        mitigation: "Reduce MRR by lowering depth or feed",
      });
      riskLevel = "critical";
      blockers.push("Power requirement exceeds machine capacity");
    }

    // Material difficulty risk
    const isoGroup = result.resolved_material.iso_group.value;
    if (isoGroup === "S") {
      risks.push({
        risk: "Titanium/superalloy - high cutting temperature expected",
        severity: "medium",
        probability: 0.9,
        mitigation: "Ensure adequate high-pressure coolant, use sharp tools",
      });
    } else if (isoGroup === "H") {
      risks.push({
        risk: "Hardened material - accelerated tool wear expected",
        severity: "medium",
        probability: 0.85,
        mitigation: "Use CBN/ceramic tooling, maintain conservative parameters",
      });
    }
    ...
    return {
      ...
      risks,
      safety_factors: {
        speed_derating: speedDerate,
        feed_derating: feedDerate,
        depth_derating: depthDerate,
      },
   ...
   * Build uncertainty analysis.
   */
  private buildUncertaintyAnalysis(
    result: OrchestratorResult,
    uq: ReturnType<SpeedFeedOrchestratorEngine["runFullUQ"]>,
  ): SpeedFeedAIReasoning["uncertainty_analysis"] {
    const avgCv = (
      result.uncertainty.speed_cv_pct +
      result.uncertainty.feed_cv_pct +
      result.uncertainty.life_cv_pct
    ) / 3;

    const level: "low" | "medium" | "high" =
      avgCv < 10 ? "low" : avgCv < 25 ? "medium" : "high";

    return {
      level,
      dominant_source: uq.dominant_uncertainty_source,
      suggested_calibration: uq.suggested_measurement,
      confidence_intervals: {
        speed_ci95_pct: [
          -result.uncertainty.speed_cv_pct * 2,
          result.uncertainty.speed_cv_pct * 2,
        ],
        feed_ci95_pct: [
          -result.uncertainty.feed_cv_pct * 2,
          result.uncertainty.feed_cv_pct * 2,
        ],
        life_ci95_pct: [
          -result.uncertainty.life_cv_pct * 2,
          result.uncertainty.life_cv_pct * 2,
        ],
      },
    };
   ...
   */
  private buildCostBenefit(
    input: OrchestratorInput,
    result: OrchestratorResult,
  ): SpeedFeedAIReasoning["cost_benefit"] {
    // Estimate cycle time (assume 100 cm³ part volume if not specified)
    const partVolume = input.part_volume_cm3 ?? 100;
    const cycleTime = partVolume / Math.max(0.1, result.mrr_cm3min);

    // Tool cost estimation (assume $20 insert, resharpening $5)
    const toolCost = 20;
    const partsPerTool = result.tool_life_min / Math.max(0.1, cycleTime);
    const toolCostPerPart = toolCost / Math.max(1, partsPerTool);

    // Machine rate (assume $75/hr if not specified)
    const machineRate = input.machine_rate_usd_hr ?? 75;
    const machineCostPerPart = (cycleTime / 60) * machineRate;

    return {
      estimated_cost_per_part: Math.round((toolCostPerPart + machineCostPerPart) * 100) / 100,
  ...
  /**
   * Generate natural language explanation of speed/feed selection.
   */
  private generateSpeedFeedExplanation(
    input: OrchestratorInput,
    result: OrchestratorResult,
    decisions: ParameterDecision[],
    hypotheses: SpeedFeedHypothesis[],
    risks: MachiningRiskAssessment,
    tribalMods: TribalModifiers,
  ): string {
    const parts: string[] = [];

    // Opening
    const material = result.resolved_material.name.value;
    const operation = input.operation ?? "milling";
    parts.push(
      `I calculated speed/feed for ${operation} ${material} (ISO ${result.resolved_material.iso_group.value}).`
    );

    // Key parameters
    parts.push(
      `Recommended: Vc=${result.cutting_speed_mpm} m/min, fz=${result.feed_per_tooth_mm} mm/tooth, ` +
      `ap=${result.axial_depth_mm} mm, giving MRR=${result.mrr_cm3min} cm³/min.`
    );

    // Limiting factor
    const primaryLimit = decisions.find(d => d.limiting_factor)?.limiting_factor;
    if (primaryLimit) {
      parts.push(`Primary constraint: ${primaryLimit}.`);
    }

    // Stability
    if (result.stability_assessment.zone !== "stable") {
      parts.push(result.stability_assessment.message);
    }

    // Risk summary
    if (risks.risk_level === "high" || risks.risk_level === "critical") {
      parts.push(`Warning: ${risks.risk_level} risk level. ${risks.risks[0]?.mitigation ?? "Review parameters."}`);
    }

    // Tribal knowledge
    if (tribalMods.evidence_count > 0) {
      const modSummary: string[] = [];
      if (tribalMods.vc_modifier !== 1) modSummary.push(`Vc×${tribalMods.vc_modifier.toFixed(2)}`);
      if (tribalMods.fz_modifier !== 1) modSummary.push(`fz×${tribalMods.fz_modifier.toFixed(2)}`);
      if (modSummary.length > 0) {
        parts.push(`Applied tribal knowledge adjustments: ${modSummary.join(", ")} (${tribalMods.evidence_count} tips).`);
      }
    }

    // Confidence
    parts.push(`Confidence: ${(result.overall_confidence * 100).toFixed(0)}%.`);

    return parts.join(" ");
  }

  /**
   * Calculate meta-confidence in the speed/feed reasoning.
   */
  private calculateSpeedFeedMetaConfidence(
    result: OrchestratorResult,
    hypotheses: SpeedFeedHypothesis[],
    risks: MachiningRiskAssessment,
    tribalMods: TribalModifiers,
    ...
    meta += result.overall_confidence * 0.2;

    // Hypothesis confirmation rate
    const confirmed = hypotheses.filter(h => h.status === "confirmed").length;
    meta += (confirmed / Math.max(1, hypotheses.length)) * 0.15;

    // Risk level penalty
    if (risks.risk_level === "low") meta += 0.1;
    else if (risks.risk_level === "medium") meta += 0.0;
    else if (risks.risk_level === "high") meta -= 0.1;
    else meta -= 0.2;

    // Tribal knowledge boost
    if (tribalMods.auto_apply_approved) meta += 0.05;

    return Math.max(0.1, Math.min(0.95, meta));
  }
  ...
  private buildCounterfactual(
    ...
    result: OrchestratorResult,
    limitingFactors: LimitingFactor[],
  ): SpeedFeedAIReasoning["counterfactual"] {
    const primaryFactor = limitingFactors[0];

    if (!primaryFactor) {
      return {
        question: "What would happen with higher productivity parameters?",
        answer: "Without a limiting factor, increasing speed/feed would eventually hit machine power or stability limits.",
      };
    }

    if (primaryFactor.parameter.includes("power")) {
      return {
        question: "What if machine power were higher?",
        answer: `With 50% more spindle power, MRR could increase to ~${(result.mrr_cm3min * 1.3).toFixed(1)} cm³/min, but tool life would decrease by ~20%.`,
      };
    }

    if (primaryFactor.parameter.includes("chatter") || primaryFactor.parameter.includes("stability")) {
      return {
        question: "What if the machine were more rigid?",
        answer: `Higher rigidity (e.g., box guideways) would allow ~40% deeper cuts with the same stability margin, increasing MRR to ~${(result.mrr_cm3min * 1.4).toFixed(1)} cm³/min.`,
      };
    }

    if (primaryFactor.parameter.includes("life") || primaryFactor.parameter.includes("wear")) {
      return {
        question: "What if tool wear weren't a constraint?",
        answer: `Without tool life limits, speed could increase to ~${(result.cutting_speed_mpm * 1.5).toFixed(0)} m/min, but thermal damage to the workpiece would become the new limit.`,
      };
    }

    return {
      question: `What if ${primaryFactor.parameter} weren't limiting?`,
      answer: `Removing this constraint would allow ~30% higher productivity, with the next limiting factor being ${limitingFactors[1]?.parameter ?? "machine capability"}.`,
    };
  }
}
