 ...
 *
 * @module engines/MastercamInHostRunnerEngine
 * @milestone CAM-EXHAUST-MS0 U-CAMTEST04
 *
 * WIRE-EXEMPT: dispatcher wiring deferred to a single follow-up unit that
 * lands all four cam_inhost_*_{register,plan,summarize,stats,reset} action
 * sets together (Inventor HSM + Mastercam). Splitting the wiring commit
 * from the engine commits avoids the lint-staged tsc cascade triggered by
 * pre-existing baseline errors in camDispatcher.ts transitive deps.
 *
 * shared-pattern: HyperMillInHostRunnerEngine, Fusion360InHostRunnerEngine,
 * InventorHSMInHostRunnerEngine. With four siblings now in tree, the
 * shared-core extraction is queued for a dedicated refactor unit.
 */
...
// Constants (Mastercam specific)

export const MASTERCAM_RUNNER_PLUGIN_ID = "mastercam-inhost-runner";
export const MASTERCAM_RUNNER_VERSION = "1.0.0";
export const MASTERCAM_RUNNER_ENDPOINT = "ws://localhost:7421/inhost/mastercam";
...
export function buildRegistration(
  plugin_id: string = MASTERCAM_RUNNER_PLUGIN_ID,
  version: string = MASTERCAM_RUNNER_VERSION,
): HubPluginRegistration {
  return {
    plugin_id,
    target: "mastercam",
    transport: "websocket",
    endpoint: MASTERCAM_RUNNER_ENDPOINT,
    version,
...
export class MastercamInHostRunnerEngine {
  static readonly MASTERCAM_RUNNER_PLUGIN_ID = MASTERCAM_RUNNER_PLUGIN_ID;
  static readonly MASTERCAM_RUNNER_VERSION = MASTERCAM_RUNNER_VERSION;
  static readonly MASTERCAM_RUNNER_ENDPOINT = MASTERCAM_RUNNER_ENDPOINT;
...
export const mastercamInHostRunnerEngine = MastercamInHostRunnerEngine;
