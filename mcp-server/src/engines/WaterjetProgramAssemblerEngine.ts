...
export class WaterjetProgramAssemblerEngine {
  ...
  assembleAbrasiveWJ(input: AbrasiveWJProfile): WaterjetProgram {
    ...
    warnings.push(...this._queryWaterjetTribalTips(input.material, "waterjet_cutting"));
  ...
  /** TK-MS2-U15: Query tribal knowledge for waterjet tips. */
  private _queryWaterjetTribalTips(materialName?: string, opType?: string): string[] {
    const tips: string[] = [];
    try {
      ...
      const results = tribalKnowledgeEngine.search({
        category: "setup",
        operation_type: opType ?? "waterjet",
