...
import { getKienzleByISO, getTaylor, chipThinningFactor, correctedCuttingForce, } from "./MachiningKnowledgeBaseEngine.js";
...
function getIntelligentSequencingEngine() { return intelligentSequencingEngine; }
...
function mapToCoolantOp5ax(opType) {
    if (opType.includes("drill"))
        return "drilling";
...
function effectiveDiameter(D, ap, _cornerR, isBall) {
    if (isBall) {
...
function scallopHeight(ae, R) {
    // h = ae² / (8R) for ball endmill
    if (R <= 0)
        return 0;
    return (ae * ae) / (8 * R);
}
function singularityRisk(A, B) {
...
export class MultiAxisPrintToProgramEngine {
    name = "MultiAxisPrintToProgramEngine";
    version = "1.0.0";
    ...
    classifyFeatures(features) {
        return features.map(f => {
            const c = { ...f };
            if (!c.priority)
                c.priority = this.priority(f.type);
            if (!c.required_operations?.length)
                c.required_operations = this.autoOps(f);
            return c;
        });
    }
    priority(type) {
    ...
    selectTool(opType, feat, toolNum) {
    ...
    calcParams(opType, feat, mat, tool, maxRPM, target) {
        ...
        const canonicalMat = CANONICAL_MATERIAL_DB[matKey]
            ...
            m.iso_group === iso && (m.name.toLowerCase().includes(matNameLower)
        ...
        if (!this._resolvedMaterial || this._cachedMaterialName !== currentMatName) {
            ...
            resolveMaterial({ material_name: mat.material_name, iso_group: iso })
    ...
    estimateCycleTime(opType, feat, params) {
    ...
    generateGCode(operations, input) {
        const lines = [];
        let lineNum = 10;
        ...
        for (const op of operations) {
            if (op.tool.tool_number !== currentTool) {
                ...
                if (op.coolant === "through_tool")
                    lines.push(`${ln()} M88`);
                else if (op.coolant !== "off")
                    lines.push(`${ln()} M08`);
    ...
    runPipeline(input) {
        ...
        try {
            ...
            for (const tip of multiAxisTips) {
                warnings.push({
                    stage: "tribal",
                    severity: "info",
        ...
        const cpm = new PipelineCheckpointManager("multiaxis-print-to-program", input.runId);
        ...
        try {
            ...
            const idOrder = new Map(seqResult.operations.map((o, i) => [o.id, i]));
        ...
        const operations = [];
        const singularityFeedReductions = [];
        let opNum = 1;
        let toolNum = 1;
        ...
        for (const feat of classified) {
            for (const opType of (feat.required_operations || [])) {
                ...
                else if (opType.startsWith("3plus2") || opType.startsWith("indexed")) {
                    maxAxes = Math.max(maxAxes, 4);
                }
        ...
        if (maxFc > 0) {
            try {
                ...
                const whResult = wve.verify({ Fc_N: maxFc, operation_name: "max_force_5ax" }, { type: "5ax_vise", clamping_force_N: 25000, clamp_points: 2, clamping_method: "hydraulic" });
        ...
        if (hasCriticalWarnings)
            confidence -= 0.5;
        if (!hasCriticalWarnings && needs5ax && input.has_rtcp)
            confidence += 0.1;
    ...
    selectCoolant(iso, opType) {
        if (iso === "S" || iso === "H")
            return "through_tool";
        if (opType.includes("drill"))
            return "through_tool";
        if (opType.includes("port"))
            return "through_tool";
        if (iso === "N")
            return "mist";
        return "flood";
...
//# sourceMappingURL=MultiAxisPrintToProgramEngine.js.map