 ...
 * - Surface roughness from pulse parameters
...
export class EDMParameterEngine {
  calculate(input: EDMParameterInput): EDMParameterResult {
    ...
    // Surface roughness: Ra ≈ K × I^0.4 × ton^0.3
    const ra = 0.5 * Math.pow(peakI, 0.4) * Math.pow(ton, 0.3);
    ...
    return {
      ...
      surface_roughness: mkAv(r2(actualRa), "µm Ra", actualRa * 0.15, "I^0.4 × ton^0.3"),
