...
export interface SprutCAMParameter {
  type: string;
  description?: string;
  unit?: string;
  default?: unknown;
...
export interface SprutCAMOperation {
  display_name: string;
  category: string;
  description: string;
  dialog_tabs: string[];
  parameter_count?: number;
  parameters: Record<string, Record<string, SprutCAMParameter>>;
}

export interface SprutCAMSection {
  ...
  operations: Record<string, SprutCAMOperation>;
...
export interface SprutCAMFunctionIndex {
  system_id: "sprutcam";
  sections: Record<string, SprutCAMSection>;
  total_operations: number;
  total_parameters: number;
  categories: string[];
}
...
const SECTION_FILES: Array<{ file: string; key: string }> = [
  ...
  { file: "robot.json", key: "robot" },
];

export class SprutCAMFunctionIndexEngine {
  private static index: SprutCAMFunctionIndex | null = null;

  private static readonly DATA_DIR = join(
    __dirname,
    "../../data/cam-functions/sprutcam"
  );

  private static loadIndex(): SprutCAMFunctionIndex {
    if (this.index) return this.index;

    const sections: Record<string, SprutCAMSection> = {};
    let totalOps = 0;
    let totalParams = 0;
    ...
    for (const { file, key } of SECTION_FILES) {
      ...
      try {
        const data = JSON.parse(readFileSync(path, "utf-8")) as SprutCAMSection;
        sections[key] = data;
        ...
        throw new Error(
          `SprutCAMFunctionIndexEngine: failed to parse ${file}: ${(err as Error).message}`
        );
      }
    }
  ...
  static getIndex(): SprutCAMFunctionIndex {
  ...
  static getSection(
    sectionKey: string
  ): SprutCAMSection | { error: string; available: string[] } {
  ...
  /**
   * Robot operations — SprutCAM Robot's flagship surface.
   * Returns all operations from the robot section.
   */
  static getRobotOperations(): Array<{
    operation_id: string;
    display_name: string;
    section: string;
    category: string;
    ...
    const rs = idx.sections["robot"];
    if (!rs) return out;
    for (const [opId, op] of Object.entries(rs.operations || {})) {
      out.push({
        operation_id: opId,
        display_name: op.display_name,
        section: "robot",
  ...
  /**
   * Mill-turn operations — SprutCAM's secondary specialty surface.
   */
  static getMillTurnOperations(): Array<{
    ...
    const mts = idx.sections["mill_turn"];
    if (!mts) return out;
    for (const [opId, op] of Object.entries(mts.operations || {})) {
      out.push({
        operation_id: opId,
        display_name: op.display_name,
        section: "mill_turn",
  ...
  static getOperation(operationId: string):
    | { operation_id: string; section: string; operation: SprutCAMOperation }
    | { error: string } {
