...
export interface JohnsonCookMaterial {
  ...
  Tr: number;  // reference temperature (C)
...
class AdvancedCuttingPhysicsEngineImpl {
  ...
  rechtShearInstability(input: RechtInput): RechtOutput {
    ...
    // Estimate shear angle (Merchant approximation)
    const phi = (45 - 15 + input.rake_angle_deg / 2) * DEG; // rough Merchant
    const phiClamped = Math.max(phi, 10 * DEG);
