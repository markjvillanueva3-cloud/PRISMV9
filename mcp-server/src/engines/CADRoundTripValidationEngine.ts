...
export interface PrintDimension {
  ...
  unit?: string;
  /**
   * Optional 0-based source page index for multi-page PDF prints. Set by the
   * live OCR adapter when a print bundle (cover/table page + the drawing on a
   * later page) is rasterized all-pages and unioned. Undefined for single
   * images. Additive -- existing consumers ignore it.
   */
  sourcePage?: number;
}
...
export interface PrintFeature {
  ...
  detail?: string;
  /**
   * ASME Y14.5 FCF syntax verdict for a GD&T feature (from FCFSyntaxValidatorEngine via the
   * blueprint OCR path -- U-XRAY-GDT-FCF-VALIDATE). true = validated OK; false = a structural
   * violation (e.g. a datum-deficient position/orientation/runout/concentricity/symmetry frame).
   * Absent for non-GD&T features or when the symbol could not be classified. Informational
   * operator-review signal -- mutates no numeric/cost field.
   */
  fcfValid?: boolean;
  /** Human-readable FCF issues when fcfValid is false, e.g. "[error] POSITION_NO_DATUM: ...". */
  fcfIssues?: string[];
}
...
export interface PrintOcrResult {
  ...
  /**
   * Total pages in the source. 1 for single images; the PDF page count for
   * multi-page prints. Undefined when not set by the producer.
   */
  pagesTotal?: number;
  /**
   * Pages that were successfully OCR'd and unioned into this result
   * (<= pagesTotal). Lets the operator see partial reads honestly (R12) -- e.g.
   * pagesOcrd 2 / pagesTotal 4 means 2 pages failed to render or extract.
   */
  pagesOcrd?: number;
}
