...
// ── Timoshenko Integration ─────────────────────────────────────────
// For L/D > 4, shear deformation becomes significant. The Timoshenko model
// includes shear: delta_total = delta_bending + delta_shear
// delta_shear = F × L / (kappa × G × A)
// Reference: TimoshenkoDeflectionEngine for full implementation

import { timoshenkoDeflectionEngine } from "./TimoshenkoDeflectionEngine.js";

// Poisson's ratio by bar material
const POISSON: Record<string, number> = {
  steel: 0.30,
  carbide: 0.22,
  dampened: 0.30,
  heavy_metal: 0.28,
...
export class BoringBarDeflectionEngine {
  /**
   * Calculate boring bar deflection using appropriate model.
   * Automatically uses Timoshenko for L/D > 4 where shear is significant.
   */
  calculate(input: BoringBarDeflectionInput): BoringBarDeflectionResult {
    const LD = input.overhang_mm / input.bar_diameter_mm;

    // For L/D > 4, use Timoshenko model which includes shear deformation
    if (LD > 4) {
      return this.calculateTimoshenko(input);
    }

    // For L/D <= 4, Euler-Bernoulli is sufficient
    return this.calculateEulerBernoulli(input);
  }

  /**
   * Calculate using Euler-Bernoulli model (bending only).
   * Accurate for L/D <= 4.
   */
  calculateEulerBernoulli(input: BoringBarDeflectionInput): BoringBarDeflectionResult {
