...
class StatisticalProcessMonitoringEngineImpl {
  ...
  pcaProcessMonitoring(input: PCAMonitoringInput): PCAMonitoringResult {
    ...
    const eig = this._eigenDecomp(S, p);
    const totVar = eig.reduce((s, e) => s + e.value, 0);
    let cum = 0, k = 0;
    const explained: number[] = [];
    for (const e of eig) {
      cum += e.value / totVar; explained.push(e.value / totVar);
      k++; if (cum >= variance_threshold) break;
    }
    const loadings = eig.slice(0, k).map(e => e.vector);
    const lambdas = eig.slice(0, k).map(e => e.value);
  ...
  bootstrapCI(input: BootstrapCIInput): BootstrapCIResult {
    ...
    const calc = (d: number[]): number => {
      ...
      return Math.sqrt(v) > 0 ? mu / (3 * Math.sqrt(v)) : 0; // cpk
    };
    const est = calc(data);
  ...
  combinedSPCScheme(input: CombinedSPCInput): CombinedSPCResult {
    ...
    const ewUCL = target + 3 * sigma * Math.sqrt(ewL / (2 - ewL));
    const ewLCL = target - 3 * sigma * Math.sqrt(ewL / (2 - ewL));
  ...
  responseSurfaceMethodology(input: RSMInput): RSMResult {
    ...
    try {
      ...
      const eig = this._eigenDecomp(B, k);
      sType = eig.every(e => e.value > 0) ? "minimum"
        : eig.every(e => e.value < 0) ? "maximum" : "saddle";
  ...
  private _eigenDecomp(
    M: number[][], dim: number
  ): Array<{ value: number; vector: number[] }> {
    const result: Array<{ value: number; vector: number[] }> = [];
    const A = M.map(r => [...r]);
    for (let comp = 0; comp < dim; comp++) {
      let v: number[] = Array.from({ length: dim },
        (_, i): number => i === comp ? 1 : 0.5);
      let ev = 0;
      for (let iter = 0; iter < 200; iter++) {
        const Av = A.map(r => r.reduce((s, x, j) => s + x * v[j], 0));
        ev = Math.sqrt(Av.reduce((s, x) => s + x * x, 0));
        if (ev < 1e-12) break;
        v = Av.map(x => x / ev);
      }
      const Av = A.map(r => r.reduce((s, x, j) => s + x * v[j], 0));
      ev = Av.reduce((s, x, j) => s + x * v[j], 0);
      result.push({ value: ev, vector: v });
      for (let i = 0; i < dim; i++)
        for (let j = 0; j < dim; j++) A[i][j] -= ev * v[i] * v[j];
    }
