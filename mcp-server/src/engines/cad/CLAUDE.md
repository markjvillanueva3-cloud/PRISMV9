...
- NEVER reference `cad-fleet-verify.mjs`, `cad-fleet-regen-valid.mjs` — do NOT exist on disk (Glob-verified 2026-06-13). (`cad-analyze-step.mjs` was BUILT 2026-06-26 -- STEP inspector + manifold-solid validity gate over `cad-gen-validate-check.py`; it now exists, is the gen-loop learning-signal analyzer, and is safe to reference.)
...
This galaxy is a first-class AI-substrate **participant** -- it OWNS 5 AI engine(s) (e.g. `CADEmbeddingIndexOrchestratorEngine`, `CADFeatureEmbeddingEngine`, `CADReasoningChainEngine`), wired to PSN leg #10 via `cad_reasoning_generate`, `cad_reasoning_why`, `cad_reasoning_get`.
...
- **Reasoning bridge** (`scripts/lib/galaxy-reasoning-bridge.mjs`, PSN leg #10): **CAG** + **RAG** hybrid
  ...
  local Ollama stack -- `node scripts/lib/galaxy-reasoning-bridge.mjs cad "<question>"`.
- **Vault -> LoRA**: this galaxy's Obsidian **synthesis** brain (`knowledge/memories/patterns/cad_synthesis.md`)
