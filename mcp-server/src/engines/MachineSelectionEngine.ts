...
const FALLBACK_MACHINES: MachineSpec[] = [
...
// Service-backed machine list — warm-loaded from MachineService on startup.
let serviceMachines: MachineSpec[] | null = null;
function getMachines(): MachineSpec[] { return serviceMachines ?? FALLBACK_MACHINES; }

void (async () => {
  try {
    const count = machineService.getCount();
    if (count > 0) {
      const types = ["VMC", "HMC", "5-Axis", "Lathe", "Multi-Tasking", "Swiss-Lathe"];
      const loaded: MachineSpec[] = [];
      for (const t of types) {
        const mats = machineService.getByType(t);
        for (const m of mats.slice(0, 5)) {
          loaded.push({
            ...
            axes: m.simultaneous_axes ?? 3,
            pallet: m.automation_ready ?? false,
          });
        }
      }
      if (loaded.length > 0) serviceMachines = loaded;
    }
  } catch { /* registry unavailable — FALLBACK_MACHINES used */ }
})();
...
export class MachineSelectionEngine {
  ...
  compare(machineIds: string[]): MachineComparisonResult {
    ...
    for (const id of machineIds) {
      const m = getMachines().find(mc => mc.id === id) || getMachines()[0];
      machines.push({
  ...
  validate(machineId: string, req: MachineRequirements): MachineValidationResult {
    const m = getMachines().find(mc => mc.id === machineId);
