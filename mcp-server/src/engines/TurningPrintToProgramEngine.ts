...
import { latheAIReasoningEngine, type LatheRecommendation, type ThreadingAIResult } from "./LatheAIReasoningEngine.js";
import { threadTurningEngine, getG76Dialect, generateG76Code, type G76Dialect } from "./ThreadTurningEngine.js";
...
export interface TurningProgramResult {
  ...
  g71_type?: "I" | "II";
  // LATHE-AI-MS1: AI-enhanced intelligence fields
  ai_recommendations?: LatheRecommendation[];
  ai_tribal_tips?: Array<{ tip_id: string; category: string; body: string; confidence: number }>;
  ai_risk_assessment?: {
    overall_risk: "low" | "medium" | "high" | "critical";
    risk_factors: Array<{ factor: string; severity: string; mitigation: string }>;
  };
  ai_confidence?: number;
  threading_dialect?: G76Dialect;
  threading_dialect_notes?: string[];
}
...
export class TurningPrintToProgramEngine {
  ...
  async calculate(action: string, params: Record<string, unknown>): Promise<TurningProgramResult> {
    switch (action) {
  ...
  async runPipeline(input: TurningInput): Promise<TurningProgramResult> {
    ...
    // TK-MS2-U13: Query tribal knowledge for turning-specific tips
    const isoGroup = input.material?.iso_group || "P";
    try {
      const machineIds = rmach?.manufacturer ? [`${rmach.manufacturer}-${rmach.model ?? ""}`] : [];
      const turningTips = tribalKnowledgeEngine.search({
        category: "setup",
      ...
      for (const tip of turningTips) {
        warnings.push({
          stage: "tribal",
          severity: "info",
          message: `TK: ${tip.title}`,
        });
      }
    } catch {
    ...
    // LATHE-AI-MS1: AI-enhanced intelligence layer (synchronous path for pipeline compatibility)
    let aiRecommendations: LatheRecommendation[] = [];
    let aiTribalTips: TurningProgramResult["ai_tribal_tips"] = [];
    let aiRiskAssessment: TurningProgramResult["ai_risk_assessment"];
    let aiConfidence = 0.85;
    let threadingDialect: G76Dialect | undefined;
    let threadingDialectNotes: string[] = [];

    try {
      // Run AI reasoning for the primary operation type
      const hasThreading = operations.some(op => op.operation_type.includes("thread"));

      // Controller-specific AI recommendations (synchronous)
      if (input.controller) {
        aiRecommendations = latheAIReasoningEngine.getControllerRecommendations(input.controller);
      }
      ...
      if (input.material.iso_group === "S" || input.material.iso_group === "H") {
        aiRecommendations.push({
          category: "speed_feed",
          priority: "critical",
          title: `Difficult material (${input.material.iso_group})`,
          description: "Use reduced cutting speeds and high-pressure coolant",
          rationale: input.material.iso_group === "S"
            ? "Titanium/superalloys have low thermal conductivity"
            : "Hardened steel requires conservative parameters",
          confidence: 0.9,
        });
      }
      ...
      if (ld_ratio > 4) {
        aiRecommendations.push({
          ...
          priority: ld_ratio > 8 ? "critical" : "important",
          title: `Slender part (L/D=${ld_ratio.toFixed(1)})`,
          description: ld_ratio > 8
            ? "Steady rest or follow rest required — part will deflect under cutting forces"
            : "Tailstock support recommended to prevent chatter",
          rationale: "Part deflection risk increases with L/D ratio",
          confidence: 0.95,
        });
      }

      // Gather tribal tips (synchronous)
      const searchTerms = `${input.material.material_name || input.material.iso_group} turning ${input.optimization_target || "balanced"}`;
      const tips = tribalKnowledgeEngine.search(searchTerms, 8);
      aiTribalTips = tips.map(t => ({
        tip_id: t.tip_id,
        category: t.category,
        body: t.body,
      ...
      // Risk assessment
      const riskFactors: NonNullable<TurningProgramResult["ai_risk_assessment"]>["risk_factors"] = [];
      if (ld_ratio > 4) {
        riskFactors.push({ factor: `High L/D ratio (${ld_ratio.toFixed(1)})`, severity: ld_ratio > 8 ? "high" : "medium", mitigation: "Use tailstock/steady rest" });
      }
      if (input.material.iso_group === "S") {
        riskFactors.push({ factor: "Titanium/superalloy material", severity: "medium", mitigation: "Reduce speeds, use high-pressure coolant" });
      }
      if (hasThreading) {
        riskFactors.push({ factor: "Threading operation", severity: "low", mitigation: "Verify pitch, use spring passes" });
      }
      ...
      if (hasThreading && input.controller) {
        const threadFeature = input.features.find(f => f.type.includes("thread"));
        if (threadFeature?.thread_pitch_mm) {
          const threadingAI = latheAIReasoningEngine.selectG76Dialect({
            controller: input.controller,
            thread_pitch_mm: threadFeature.thread_pitch_mm,
            major_diameter_mm: threadFeature.od_mm || input.bar_stock_od_mm,
            thread_length_mm: threadFeature.length_mm || 20,
            material_iso: input.material.iso_group,
            is_external: threadFeature.type === "thread_od",
          });
          threadingDialect = threadingAI.recommended_dialect;
          threadingDialectNotes = threadingAI.warnings;

          // Log dialect selection
          log.info(`[TurningPrintToProgram] AI threading dialect: ${threadingDialect} (${(threadingAI.dialect_confidence * 100).toFixed(0)}% confidence)`);
        }
      }

      // Add critical AI recommendations as warnings
      for (const rec of aiRecommendations.filter(r => r.priority === "critical")) {
        warnings.push({
          stage: "ai_reasoning",
          severity: "warning",
          message: `AI: ${rec.title} — ${rec.description}`,
        });
      }

      aiConfidence = 0.85 + (aiTribalTips.length > 3 ? 0.05 : 0) + (input.controller ? 0.05 : 0);
      log.info(`[TurningPrintToProgram] AI reasoning: ${aiRecommendations.length} recommendations, ${overallRisk} risk`);
    } catch (aiError: any) {
      log.debug?.(`[TurningPrintToProgram] AI reasoning unavailable: ${aiError?.message}`);
      // Continue without AI — non-blocking enhancement
    }
    ...
    return {
      ...
      g71_type: g71Type,
      // LATHE-AI-MS1: AI intelligence fields
      ai_recommendations: aiRecommendations.length > 0 ? aiRecommendations : undefined,
      ai_tribal_tips: aiTribalTips.length > 0 ? aiTribalTips : undefined,
      ai_risk_assessment: aiRiskAssessment,
      ai_confidence: aiConfidence,
      threading_dialect: threadingDialect,
      threading_dialect_notes: threadingDialectNotes.length > 0 ? threadingDialectNotes : undefined,
    };
  }
