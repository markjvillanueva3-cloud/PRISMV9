...
let _sheetNestingEngine = null;
async function getSheetNestingEngine() {
...
function fillMCode(template, vars) {
    let out = template;
 ...
 */
function computeCutPhysics(mat, power_w, thickness_mm, focus_d_mm, gas_pressure_bar, nozzle_d_mm, laser_type, process) {
 ...
 */
function runMonteCarlo(mat, base, power_w, thickness_mm, focus_d_mm, gas_pressure_bar, nozzle_d_mm, laser_type, process, n_samples = 500) {
    const speeds = [];
    const kerfs = [];
    const hazs = [];
    const rzs = [];
...
// G-CODE BUILDERS
// ============================================================================
function buildGCodeCut(dialect, prog_no, part_name, mat, phys, profile, processLabel) {
    const cs = dialect.comment_style;
    const C = (t) => comment(t, cs);
    ...
    if (geom && geom.points.length >= 2) {
        ...
        if (geom.closed) {
            ...
            if (lenA > 0.001 && lenB > 0.001) {
                ...
                const angleDeg = Math.acos(Math.max(-1, Math.min(1, cosAngle))) * (180 / Math.PI);
                if (angleDeg > 45)
                    closeFeed = Math.round(F_feed * 0.7);
            }
...
function buildGCodeMark(dialect, prog_no, part_name, mat, profile, processLabel) {
    const cs = dialect.comment_style;
    const C = (t) => comment(t, cs);
...
function buildGCodeWeld(dialect, prog_no, part_name, mat, profile, pen_mm, processLabel) {
    const cs = dialect.comment_style;
    const C = (t) => comment(t, cs);
    ...
    if (profile.joint_type)
        lines.push(C(`JOINT TYPE : ${profile.joint_type.toUpperCase()}`));
    lines.push("");
    lines.push("G90 G21 G54");
    lines.push(dialect.feed_mode);
    lines.push(powerLine);
...
function buildGCodeDrill(dialect, prog_no, part_name, mat, profile, drill_time_s, processLabel) {
    const cs = dialect.comment_style;
    const C = (t) => comment(t, cs);
...
export class LaserProgramAssemblerEngine {
    ...
    assembleLaserCut(input) {
        ...
        // Physics
        const phys = computeCutPhysics(mat, input.power_w, input.thickness_mm, focus_d, input.gas_pressure_bar, nozzle_d, input.laser_type, input.process);
        // Override speed if explicitly specified
        if (input.speed_override_mmpm)
            phys.V_cut_mmpm = input.speed_override_mmpm;
        ...
        // Monte Carlo
        const unc = runMonteCarlo(mat, phys, input.power_w, input.thickness_mm, focus_d, input.gas_pressure_bar, nozzle_d, input.laser_type, input.process);
        // Warnings
        const warnings = [];
        const recs = [];
        ...
        if (unc.p_dross > 0.20)
            recs.push(`P(dross)=${(unc.p_dross * 100).toFixed(0)}% — increase gas pressure or reduce speed by 10%`);
        if (unc.p_incomplete_cut > 0.10)
            recs.push(`P(incomplete cut)=${(unc.p_incomplete_cut * 100).toFixed(0)}% — reduce speed or increase power`);
        if (phys.haz_mm > input.thickness_mm * 0.15)
            recs.push(`HAZ ${fmt(phys.haz_mm, 2)}mm is >15% of thickness — consider pulsed mode or faster traverse`);
        recs.push(mat.notes);
        // G-code
        ...
        // TK-MS2-U15: Add tribal knowledge tips
        warnings.push(...this._queryLaserTribalTips(input.material, "laser_cutting"));
        return {
    ...
    assembleLaserDrill(input) {
        ...
        const recs = [];
        if (recast_mm > 0.05)
            recs.push(`Recast layer ~${fmt(recast_mm, 3)}mm — consider EDM/chemical post-processing for turbine blades`);
        if (haz_mm > input.hole_diameter_mm * 0.3)
            warnings.push(`HAZ ${fmt(haz_mm, 2)}mm >30% of hole diameter — reduce pulse energy or use USP (ultra-short pulses)`);
    ...
    computeUncertainty(input) {
        ...
        const nozzle_d = input.nozzle_diameter_mm ?? 1.5;
        const phys = computeCutPhysics(mat, input.power_w, input.thickness_mm, focus_d, input.gas_pressure_bar, nozzle_d, input.laser_type, input.process);
        return runMonteCarlo(mat, phys, input.power_w, input.thickness_mm, focus_d, input.gas_pressure_bar, nozzle_d, input.laser_type, input.process);
    }
    // -------------------------------------------------------------------------
    // STATS
    // -------------------------------------------------------------------------
     ...
     */
    async assembleNestedLaserCut(input) {
        // Determine if nesting is needed
        const nesting = input.nesting;
        const quantity = input.quantity ?? 1;
        const needsNesting = nesting && ((nesting.parts && nesting.parts.length > 0) || quantity > 1);
        if (!needsNesting) {
        ...
        const nestResult = nestingEngine.optimizeNesting(nestParts.map(p => ({
            id: p.id,
            vertices: p.vertices,
            quantity: p.quantity,
        })), sheet, {
        ...
        const C = (t) => comment(t, cs);
        const prog_no = input.program_number ?? 1001;
        const phys = computeCutPhysics(mat, input.power_w, input.thickness_mm, input.focus_diameter_mm ?? (input.laser_type === "co2" ? 0.30 : 0.10), input.gas_pressure_bar, input.nozzle_diameter_mm ?? 1.5, input.laser_type, input.process);
        if (input.speed_override_mmpm)
            phys.V_cut_mmpm = input.speed_override_mmpm;
        const F_feed = Math.round(phys.V_cut_mmpm);
        const pierce_ms = Math.round(phys.t_pierce_s * 1000);
        ...
        if (tabWidth > 0)
            lines.push(C(`TABS     : ${tabWidth}mm WIDE EVERY ${tabSpacing}mm`));
        if (commonLinePairs.length > 0)
            lines.push(C(`COMMON LINES: ${commonLinePairs.length} PAIR(S) DETECTED`));
        lines.push("");
        // Safe start
        lines.push("G90 G21 G54");
        lines.push(dialect.feed_mode);
    ...
    /** TK-MS2-U15: Query tribal knowledge for laser tips. */
    _queryLaserTribalTips(materialName, opType) {
        const tips = [];
        try {
    ...
    static _ISO_LASER_FALLBACK = {
        P: "mild_steel", M: "stainless_steel", K: "mild_steel",
        N: "aluminum", S: "titanium", H: "mild_steel",
    };
    _resolveMaterial(name) {
        // Tier 1: exact match
        if (LASER_MAT_DB[name])
            return LASER_MAT_DB[name];
    ...
    _contourLength(geom) {
        if (!geom || geom.points.length < 2)
            return 100; // default 100mm for cycle time
        let len = 0;
...
//# sourceMappingURL=LaserProgramAssemblerEngine.js.map