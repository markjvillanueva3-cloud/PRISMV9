 ...
 */
import { calculateKienzleCuttingForce, calculateTaylorToolLife, calculateSpeedFeed, calculateSurfaceFinish, calculateMRR, calculateSpindlePower, getDefaultKienzle, getDefaultTaylor, } from "./ManufacturingCalculations.js";
import { calculateStabilityLobes, calculateToolDeflection, calculateCuttingTemperature, calculateMinimumCostSpeed, optimizeCuttingParameters, } from "./AdvancedCalculations.js";
import { calculateMultiPassStrategy, recommendCoolantStrategy, estimateCycleTime, } from "./ToolpathCalculations.js";
...
import { formatByLevel } from "../types/ResponseLevel.js";
 ...
 */
export const INTELLIGENCE_SOURCE_FILE_CATALOG = {
...
async function jobPlan(params) {
    ...
    let mat = undefined;
    try {
        mat = await materialRegistry.getByIdOrName(params.material);
    }
    catch (err) {
    ...
    if (mat) {
        ...
        confidence *= 0.70;
        safetyWarnings.push(`Material "${params.material}" not found in registry -- using conservative P-group defaults`);
    }
    ...
    if (needsMultiPass) {
        ...
        multiPassStrategy = calculateMultiPassStrategy(depth, toolDiameter, kienzle.kc1_1, 
        /* machine_power_kw */ 15, roughSpeed.cutting_speed, finishSpeed.cutting_speed, roughSpeed.feed_per_tooth, finishSpeed.feed_per_tooth, targetRa, length);
    }
    for (const phase of phases) {
        ...
        if (multiPassStrategy?.strategy?.phases) {
            const matchedPhase = multiPassStrategy.strategy.phases.find((p) => p.phase === phase);
            if (matchedPhase) {
        ...
        // -- 6. Tool life --
        const toolLifeResult = calculateTaylorToolLife(sf.cutting_speed, taylor, sf.feed_per_tooth, axialDepth);
        ...
        if (toolLifeResult.tool_life_minutes < INTELLIGENCE_SAFETY_LIMITS.MIN_TOOL_LIFE_HARD) {
            safetyWarnings.push(`[${phase}] CRITICAL: Tool life ${toolLifeResult.tool_life_minutes} min < ${INTELLIGENCE_SAFETY_LIMITS.MIN_TOOL_LIFE_HARD} min hard limit`);
        }
        else if (toolLifeResult.tool_life_minutes < INTELLIGENCE_SAFETY_LIMITS.MIN_TOOL_LIFE_WARN) {
            safetyWarnings.push(`[${phase}] WARNING: Tool life ${toolLifeResult.tool_life_minutes} min < ${INTELLIGENCE_SAFETY_LIMITS.MIN_TOOL_LIFE_WARN} min recommended minimum`);
        }
        // -- 7. Surface finish (only for finishing) --
        let surfaceFinish;
        if (phase === "finishing") {
            const finishResult = calculateSurfaceFinish(sf.feed_per_tooth, noseRadius, 
            /* is_milling */ true, radialDepth, toolDiameter, "milling");
            surfaceFinish = { Ra: finishResult.Ra, Rz: finishResult.Rz };
            if (finishResult.Ra > targetRa) {
                safetyWarnings.push(`Predicted Ra ${finishResult.Ra} um exceeds target ${targetRa} um -- consider reducing feed or increasing nose radius`);
            }
        }
        // Estimate cutting distance for this phase
        const passCount = multiPassStrategy?.strategy?.phases?.find((p) => p.phase === phase)?.passes ?? 1;
        ...
        if (powerKw > INTELLIGENCE_SAFETY_LIMITS.MAX_POWER_KW) {
            safetyWarnings.push(`[${phase}] Power ${powerKw.toFixed(1)} kW exceeds absolute max ${INTELLIGENCE_SAFETY_LIMITS.MAX_POWER_KW} kW`);
        }
        operations.push({
            sequence,
            type: phase,
    ...
    const cycleTimeResult = estimateCycleTime(totalCuttingDistance, avgFeedRate > 0 ? avgFeedRate : 500, rapidDistance, 
    /* number_of_tools */ 1, 
    /* tool_change_time */ 0.5);
    // -- 10. Coolant strategy --
    const coolantResult = recommendCoolantStrategy(isoGroup, feature, operations[0].params.cutting_speed, 
    /* tool_has_coolant_through */ false, thermalConductivity);
    // -- 11. Toolpath strategy recommendation --
    let toolpath;
    try {
        const tpResult = toolpathRegistry.getBestStrategy(feature, materialName, "roughing", { priority: "balanced" });
        if (tpResult?.strategy) {
            toolpath = {
                ...
                alternatives: (tpResult.alternatives || []).slice(0, 3).map((a) => a.name || a.id),
            };
        }
    }
    catch (err) {
    ...
    if (params.machine_id) {
        try {
            const machine = machineRegistry.getByIdOrModel(params.machine_id);
            if (machine) {
                const spindleRec = machine.spindle;
                const maxSpindlePower = spindleRec?.max_power_kw
                    ?? spindleRec?.power
                    ?? undefined;
                if (maxSpindlePower) {
                    for (const op of operations) {
                        if (op.force.power_kW > maxSpindlePower) {
                            safetyWarnings.push(`[${op.type}] Power ${op.force.power_kW} kW exceeds machine "${params.machine_id}" rating of ${maxSpindlePower} kW`);
                        }
                    }
                }
            }
    ...
    let allChecksPassed = !safetyWarnings.some((w) => w.includes("CRITICAL") || w.includes("SAFETY BLOCK"));
    const elapsedMs = Date.now() - startMs;
    log.info(`[IntelligenceEngine] job_plan completed in ${elapsedMs}ms, ` +
    ...
    if (!stability.is_stable && stability.spindle_speeds?.length) {
        ...
        safetyWarnings.push(`Chatter risk: current axial depth exceeds stability limit ` +
...
function mapIsoToKienzleGroup(isoGroup) {
    const map = {
...
function mapIsoToTaylorGroup(isoGroup) {
    const map = {
        P: "steel",
        M: "stainless",
        K: "cast_iron",
 ...
 */
function validateRequiredFields(actionName, params, fields) {
    for (const field of fields) {
        if (params[field] === undefined || params[field] === null) {
            throw new Error(`[IntelligenceEngine] ${actionName}: missing required field "${field}"`);
        }
    }
}
/** Throw a standard NOT_IMPLEMENTED error for stub actions. */
function throwNotImplemented(actionName) {
    throw new Error(`[IntelligenceEngine] ${actionName} is not yet implemented (R3-MS1)`);
}
/**
 ...
 */
async function setupSheet(params) {
    ...
    const opsDetail = planResults.map((plan, i) => ({
        ...
        phases: plan.operations.map((phase) => ({
            type: phase.type,
    ...
    if (format === "markdown" || format === "text") {
        ...
        for (const op of opsDetail) {
            ...
            for (const phase of op.phases) {
                lines.push(`  - **${phase.type}**: Vc=${phase.cutting_speed} m/min, ` +
    ...
    log.info(`[IntelligenceEngine] setup_sheet: ${operations.length} ops, ` +
 ...
 */
async function processCost(params) {
    ...
    for (const op of operations) {
        ...
        const phaseToolLife = plan.operations.reduce((min, o) => Math.min(min, o.tool_life_min), Infinity);
    ...
    log.info(`[IntelligenceEngine] process_cost: $${result.total_cost_per_part}/part, ` +
...
async function materialRecommend(params) {
    ...
    if (materials.length === 0 && params.iso_group) {
        const broader = await materialRegistry.search({
            iso_group: params.iso_group,
            hardness_min: params.hardness_range?.min,
            hardness_max: params.hardness_range?.max,
    ...
    if (materials.length === 0) {
        const all = await materialRegistry.search({ query: "*", limit: 30 });
    ...
    const scored = materials.map((m) => {
        ...
        if (machinability > 70)
            reasons.push("High machinability");
        ...
        if (hasKienzle) {
            score += 0.15;
            reasons.push("Kienzle calibrated");
        }
        if (hasTaylor) {
            score += 0.15;
            reasons.push("Taylor calibrated");
        }
        ...
        return {
            ...
            name: m.name,
            iso_group: m.classification?.iso_group || m.iso_group || "?",
    ...
    log.info(`[IntelligenceEngine] material_recommend: ${materials.length} searched, ` +
 ...
 * @param params.limit - Max candidates to return (default: 5)
 */
async function toolRecommend(params) {
    ...
    if (!isoGroup) {
        try {
            const mat = await materialRegistry.getByIdOrName(params.material);
            isoGroup = mat?.classification?.iso_group || mat?.iso_group || "P";
        }
        catch {
            isoGroup = "P";
        }
    ...
    // Search tools
    const searchResult = toolRegistry.search({
    ...
    if (tools.length === 0) {
        const broader = toolRegistry.search({
            material_group: isoGroup,
            diameter_min: params.diameter_range?.min,
            diameter_max: params.diameter_range?.max,
            limit: Math.max(limit * 4, 20),
    ...
    if (tools.length === 0) {
        const all = toolRegistry.search({ query: "*", limit: 30 });
        tools = all.tools;
    }
    // Score each tool
    const scored = tools.map((t) => {
        ...
        const hasGroupMatch = materialGroups.includes(isoGroup) ||
            cpMaterials.some((k) => k.startsWith(isoGroup));
        if (hasGroupMatch) {
            score += 0.30;
        ...
        if (flutes) {
            ...
            const idealFlutes = { P: 4, M: 4, K: 4, N: 3, S: 5, H: 4 };
    ...
    scored.sort((a, b) => b.score - a.score);
    const candidates = scored.slice(0, limit);
    log.info(`[IntelligenceEngine] tool_recommend: ${tools.length} searched, ` +
        `${candidates.length} returned, iso=${isoGroup}, feature=${feature}`);
    // TK-MS2-U14: Query tribal knowledge for tooling tips
    let tribalTips = [];
    try {
        ...
        tribalTips = tips.map(t => `TK: ${t.title}`);
    }
    catch {
        // Tribal knowledge unavailable
    }
    return {
        ...
        tribal_tips: tribalTips.length > 0 ? tribalTips : undefined,
    };
}
/**
...
async function machineRecommend(params) {
    ...
    if (machines.length < limit) {
        const relaxed = machineRegistry.search({
            type: params.type,
            min_spindle_rpm: params.min_spindle_rpm,
    ...
    const scored = machines.map((m) => {
        ...
        if (xFits && yFits && zFits) {
            ...
            if (zFits)
                score += 0.10;
            if (!xFits)
                reasons.push(`X travel ${xTravel}mm < ${Math.round(reqX)}mm needed`);
            if (!yFits)
                reasons.push(`Y travel ${yTravel}mm < ${Math.round(reqY)}mm needed`);
            if (!zFits)
                reasons.push(`Z travel ${zTravel}mm < ${Math.round(reqZ)}mm needed`);
        }
    ...
    scored.sort((a, b) => b.score - a.score);
    const candidates = scored.slice(0, limit);
    log.info(`[IntelligenceEngine] machine_recommend: ${machines.length} searched, ` +
...
async function whatIf(params) {
    ...
    if (params.material || baseline.material) {
        ...
        let mat;
        try {
            mat = await materialRegistry.getByIdOrName(matName);
        }
        catch { /* skip */ }
        if (mat) {
            materialName = mat.name;
    ...
    const insights = [];
    if (deltas.tool_life_min.percent < -30)
        insights.push("Significant tool life reduction — consider cost impact");
    if (deltas.cutting_force_N.percent > 25)
        insights.push("Force increase may cause chatter or deflection");
    if (deltas.surface_finish_Ra.percent > 20)
        insights.push("Surface finish degradation — verify against tolerance");
    if (deltas.mrr_cm3_min.percent > 15)
        insights.push("Productivity gain from higher MRR");
    if (deltas.power_kW.percent > 40)
        insights.push("Power spike — verify machine capacity");
    if (scenarioMetrics.tool_life_min < 5)
        insights.push("CRITICAL: Tool life below 5 min — Taylor cliff zone");
    ...
    if (params.sweep) {
        const sweepParam = params.sweep.parameter;
        ...
        if (baseValue !== undefined && typeof baseValue === "number" && baseValue > 0) {
            ...
            for (const pt of sweepPoints) {
                if (pt.power_kW > machineMaxPower) {
                    constraintViolations.push(`${sweepParam}=${pt[sweepParam]}: power ${pt.power_kW.toFixed(1)} kW exceeds machine limit ${machineMaxPower} kW`);
                }
                if (pt.tool_life_min < 5) {
                    constraintViolations.push(`${sweepParam}=${pt[sweepParam]}: tool life ${pt.tool_life_min.toFixed(1)} min below safe minimum`);
                }
            }
            sensitivity = {
 ...
 */
async function failureDiagnose(params) {
    // At least one of symptoms or alarm_code must be provided
    if (!params.symptoms && !params.alarm_code) {
        throw new Error('[IntelligenceEngine] failure_diagnose: at least one of "symptoms" or "alarm_code" is required');
    }
        ...
        try {
            ...
            if (alarm) {
                alarmResult = {
                    ...
                    fix_procedures: alarm.fix_procedures?.map((fp) => ({
                        step: fp.step,
                        action: fp.action,
                        details: fp.details,
                ...
                const searchResult = await alarmRegistry.search({
                ...
                if (searchResult.alarms.length > 0) {
                    alarmResult = {
                        ...
                        matches: searchResult.alarms.map((a) => ({
                            code: a.code,
                            name: a.name,
                            category: a.category,
 ...
 * IMPLEMENTED R3-MS0
 */
async function parameterOptimize(params) {
    ...
    const result = optimizeCuttingParameters(constraints, weights, kienzle.kc1_1, taylor.C, taylor.n, toolDiameter, numTeeth);
    // Also compute the minimum-cost speed as a reference
    const minCost = calculateMinimumCostSpeed(taylor.C, taylor.n, { machine_rate: 75 / 60, tool_cost: 45, tool_change_time: 1 });
    log.info(`[IntelligenceEngine] parameter_optimize: ${params.material} → ` +
 ...
 * IMPLEMENTED R3-MS0
 */
async function cycleTimeEstimate(params) {
    ...
    let mat;
    try {
        mat = await materialRegistry.getByIdOrName(materialName);
    }
    catch { /* skip */ }
    const isoGroup = mat?.iso_group || mat?.classification?.iso_group || "P";
    ...
    const warnings = [];
    if (utilization < 50)
        warnings.push("Low spindle utilization — consider consolidating tool paths");
    if (totalTime > 120)
        warnings.push("Cycle time exceeds 2 hours — consider batch splitting or parallel fixturing");
    log.info(`[IntelligenceEngine] cycle_time_estimate: ${operations.length} ops, ` +
 ...
 * IMPLEMENTED R3-MS0
 */
async function qualityPredict(params) {
    ...
    let deflection;
    try {
        deflection = calculateToolDeflection(Fc, D, overhang, 600);
    }
    catch {
    ...
    try {
        ...
        thermal = calculateCuttingTemperature(Vc, fz, // feed per tooth (mm)
    ...
    const qualityWarnings = [...surfaceFinish.warnings];
    ...
    log.info(`[IntelligenceEngine] quality_predict: ${materialName} → Ra=${surfaceFinish.Ra}, ` +
...
function isIntelligenceAction(action) {
    return INTELLIGENCE_ACTIONS.includes(action);
}
/**
 ...
 */
export async function executeIntelligenceAction(action, params) {
    if (!isIntelligenceAction(action)) {
        throw new Error(`[IntelligenceEngine] Unknown action "${action}". ` +
    ...
    try {
        ...
        switch (action) {
            case "job_plan":
                ...
                if (!params.dimensions?.depth) {
                    throw new Error('[IntelligenceEngine] job_plan: dimensions.depth is required');
                }
                result = await jobPlan(params);
                break;
            case "setup_sheet":
        ...
        const elapsedMs = Date.now() - startMs;
        log.error(`[IntelligenceEngine] ${action} failed after ${elapsedMs}ms: ${err.message}`);
        throw err;
    }
}
// ============================================================================
