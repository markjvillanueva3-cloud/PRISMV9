...
export class WeeklySynthesisEngine {
  ...
  constructor(opts: { loader?: LoaderFn; summarizer?: SummarizerFn } = {}) {
    // P5 imported composeOctopusLoader but never applied it — the octopus→WeeklySynthesis→Obsidian
    // consumption loop was dormant (the loader fell straight through to defaultLoader). Compose it
    // here so the consensus brief + the per-domain rollup (from the U-FLEET-CONSUME feeds) actually
    // fold into the weekly retro. The composer is default-OFF internally: it returns the base loader
    // UNCHANGED unless PRISM_WEEKLY_SYNTHESIS_OCTOPUS=1, so this is byte-identical when the knob is
    // unset. Passing outcomesDir enables the per-domain rollup source (off without it).
    const baseLoader = opts.loader ?? defaultLoader;
    this.loader = composeOctopusLoader(baseLoader, { outcomesDir: OCTOPUS_OUTCOMES_DIR });
