...
import { CANONICAL_KIENZLE, CANONICAL_TAYLOR, kienzleForce, taylorLife, toolDeflection, predictedRa, cuttingPower, spindleTorque, rpmFromVc, } from "../physics/constants.js";
// ─── Constants ───────────────────────────────────────────────────────
...
function getCAMPostStrategy(controller) {
    // Direct match
    if (CAM_POST_STRATEGIES[controller])
        return CAM_POST_STRATEGIES[controller];
    // Family match: "fanuc_31i" → "fanuc", "siemens_840d" → "siemens"
    for (const family of Object.keys(CAM_POST_STRATEGIES)) {
        if (controller.startsWith(family))
            return CAM_POST_STRATEGIES[family];
    }
 ...
 */
function helixLinearizationSegments(sweepRad, radiusMm, toleranceMm) {
 ...
 */
const getCanonicalKc = (iso) => CANONICAL_KIENZLE[iso]?.kc1_1 ?? 1800;
const getCanonicalMc = (iso) => CANONICAL_KIENZLE[iso]?.mc ?? 0.25;
// ─── G-code Parser ───────────────────────────────────────────────────
...
function parseGCode(gcode) {
    ...
    let lastFeed, lastSpindle;
    for (const raw of lines) {
        const line = raw.trim();
        if (!line || line.startsWith("(") || line.startsWith(";") || line.startsWith("%") || line.startsWith("O"))
            continue;
        // Tool change
        const toolMatch = line.match(/T(\d+)/);
        if (toolMatch) {
            currentTool = parseInt(toolMatch[1], 10);
            if (!tools.has(currentTool))
                tools.set(currentTool, {});
        }
        // Spindle
        const sMatch = line.match(/S(\d+\.?\d*)/);
        if (sMatch)
            lastSpindle = parseFloat(sMatch[1]);
        // Feed
        const fMatch = line.match(/F(\d+\.?\d*)/);
        if (fMatch)
            lastFeed = parseFloat(fMatch[1]);
        // Move type
        ...
        if (iMatch)
            block.i = parseFloat(iMatch[1]);
        if (jMatch)
            block.j = parseFloat(jMatch[1]);
        if (kMatch)
            block.k = parseFloat(kMatch[1]);
        if (rMatch)
            block.r = parseFloat(rMatch[1]);
        blocks.push(block);
        lastX = x;
        lastY = y;
        lastZ = z;
...
function parseCLData(clData) {
    ...
    for (const raw of lines) {
        ...
        if (fedMatch)
            lastFeed = parseFloat(fedMatch[1]);
        // SPINDL
        const spindlMatch = line.match(/SPINDL\s*\/\s*(\d+\.?\d*)/);
        if (spindlMatch)
            lastSpindle = parseFloat(spindlMatch[1]);
        // LOADTL / TURRET
...
class PostProcessorPipelineEngineImpl {
    // Lazy-loaded engines
    _ultimateSF = null;
    _instantaneousEng = null;
    _engagementAdaptive = null;
    _safetyAnalyzer = null;
    _playbook = null;
    _motionDynamics = null;
    _toolpathSmoothing = null;
    _cycleTimeEst = null;
    _thermalEngine = null;
    _energyOpt = null;
    _toolSelection = null;
    _constitutive = null;
    _stochasticChatter = null;
    _stochasticDeflection = null;
    _chipMorphology = null;
    _coolantDynamics = null;
    _fixtureClamping = null;
    _processCapability = null;
    _controllerDialect = null;
    _speedFeedOrchestrator = null;
    _postSelection = null;
    _advancedPost = null;
    _probeGenerator = null;
    _lineByLine = null;
    _stabilityRewriter = null;
    _probingCycle = null;
    _threadMilling = null;
    _omegaSafety = null;
    _predictionCalibration = null;
    _thermalWearCoupling = null;
    _rlPostProcessor = null;
    _sustainabilityLCA = null;
    /** Get a lazily-loaded engine by name */
    async _getEngine(name) {
        switch (name) {
            case "ultimateSF":
    ...
    async process(input = {}) {
        ...
        const _localRunStage = ((name, phase, stgs, fn) => {
            if (!_pastResume && name !== _resumeTarget) {
                const cp = _cpm.resumeFrom(_stageIdx);
                if (cp) {
                    stgs.push({ stage: name, phase, status: "pass", duration_ms: 0, summary: "Resumed from checkpoint", data: cp.data });
                    _stageIdx++;
                    return cp.data;
                }
            }
        ...
        const _localRunStageAsync = (async (name, phase, stgs, fn) => {
            if (!_pastResume && name !== _resumeTarget) {
                const cp = _cpm.resumeFrom(_stageIdx);
                if (cp) {
                    stgs.push({ stage: name, phase, status: "pass", duration_ms: 0, summary: "Resumed from checkpoint", data: cp.data });
        ...
        const parsedTools = parseResult?.tools ?? new Map();
            ...
            featureSelectionResult = await _localRunStageAsync("0.7_feature_selection", 0, stages, async () => {
                try {
                    ...
                    if (input.features && input.features.length > 0) {
                        ...
                        result.selected_features = result.selected_features.filter((f) => userFeatures.has(f.id));
                    }
                    return result;
                }
                catch {
        ...
        let calibratedKc1_1Factor = null;
        let calibratedTaylorCFactor = null;
        let calibratedTaylorNFactor = null;
        ...
        if (stageFlags.speed_feed && material) {
            await _localRunStageAsync("1.1_base_speed_feed", 1, stages, async () => {
                ...
                try {
                    ...
                    const coolType = input.coolant_type ?? (material.iso_group === "N" ? "mist" : "flood");
                    const COOLANT_K = {
                ...
                for (const [toolNum, toolBlocks] of toolGroups) {
                    ...
                    const tool = tools.find(t => t.id === String(toolNum))
                        ...
                        ?? tools.find(t => t.tool_number === toolNum)
                        ?? tools[0];
                    if (!tool)
                        continue;
                    ...
                    try {
                        ...
                        try {
                            ...
                            const provenMatch = userProvenCuttingData.find((r) => Math.abs(r.diameter - toolDia) < 0.5 &&
                        ...
                        for (const block of toolBlocks) {
                            ...
                            const reasons = [`Base S/F: Vc=${Vc_val.toFixed?.(0) ?? Vc_val}m/min, fz=${fz_val.toFixed?.(3) ?? fz_val}mm`];
            ...
            await _localRunStageAsync("1.2_constitutive_model", 1, stages, async () => {
                ...
                if (material.jc_A && material.jc_B) {
                    ...
                    for (const [toolNum, toolBlocks] of toolGroups) {
                        ...
                        for (const block of toolBlocks) {
                            if (block.move_type === "G0" || !block.forces)
                                continue;
                            ...
                            if (needsShift) {
                                ...
                                if (newRpm !== rpm && newRpm > 0) {
                                    if (block.optimization) {
                                        ...
                                        block.optimization.reasons.push(`Spindle harmonics: ftp=${ftp.toFixed(1)}Hz resonant → S=${newRpm}`);
                                    }
        ...
        if (stageFlags.thread_milling_physics) {
            await _localRunStageAsync("1.5b_thread_milling", 1, stages, async () => {
                try {
                    ...
                    const threadOps = (input.operations ?? []).filter((op) => /thread/i.test(op.type ?? '') || /thread_mill|tap/i.test(op.tool_type ?? ''));
                    ...
                    for (const tool of targetTools) {
                        if (!tool)
                            continue;
                        const threadDia = threadOps[0]?.diameter_mm ?? (tool.diameter_mm * 1.5);
                        const pitch = threadOps[0]?.pitch_mm ?? 1.5;
        ...
        if (stageFlags.coolant_strategy !== false && material) {
            _localRunStage("1.7_coolant_strategy", 1, stages, () => {
                ...
                const recommended = (() => {
                    switch (isoGroup) {
                        case "N": return { type: "mql", flow_rate_l_min: 0.05 };
                        case "S": return { type: "flood", pressure_bar: 70, flow_rate_l_min: 20 };
                        case "H": return { type: "flood", pressure_bar: 40, flow_rate_l_min: 15 };
                        case "M": return { type: "flood", pressure_bar: 20, flow_rate_l_min: 12 };
                        case "K": return { type: "mist", flow_rate_l_min: 0.5 };
                        default: return { type: "flood", flow_rate_l_min: 10 };
                    }
                })();
                    ...
                    for (let bi = 0; bi < cuttingBlocks.length; bi++) {
                        ...
                        if (prev && block.z !== undefined && prev.z !== undefined) {
                            ...
                            if (zDelta > 0.01) {
                                ...
                                const xyDelta = Math.sqrt(((block.x ?? prev.x ?? 0) - (prev.x ?? 0)) ** 2 +
                        ...
                        if (isPlunge)
                            ...
                            if (prev && pprev) {
                                ...
                                if (m1 > 0.01 && m2 > 0.01) {
                                    ...
                                    if (cos < 0.5)
                                        classification = "corner"; // > 60° direction change
                                }
                            }
                        }
                        try {
            ...
            await _localRunStageAsync("2.3_adaptive_feed", 2, stages, async () => {
                try {
                    ...
                    for (const [toolNum, toolBlocks] of toolGroups) {
                        ...
                        for (const block of cuttingBlocks) {
                            if (!block.engagement || !block.optimization)
                                continue;
                            // Skip if Stage 2.1 already applied chip thinning compensation
                            if (block.engagement.chip_thinning_factor > 1.05)
                                continue;
        ...
        if (stageFlags.plunge_detection && !lineByLineActive) {
            _localRunStage("2.5_plunge_ramp", 2, stages, () => {
                ...
                for (let i = 1; i < blocks.length; i++) {
                    const prev = blocks[i - 1];
                    const curr = blocks[i];
                    if (curr.move_type === "G0" || !curr.optimization)
                        continue;
                    const dz = Math.abs((curr.z ?? 0) - (prev.z ?? 0));
                    const dxy = Math.sqrt(((curr.x ?? 0) - (prev.x ?? 0)) ** 2 +
                        ((curr.y ?? 0) - (prev.y ?? 0)) ** 2);
                    if (dz < 0.01)
                        continue; // no Z motion
        ...
        if (stageFlags.wear_progression && !lineByLineActive) {
            _localRunStage("2.6_wear_progression", 2, stages, () => {
                ...
                for (const [toolNum, toolBlocks] of toolGroups) {
                    ...
                    for (const block of toolBlocks) {
                        if (block.move_type === "G0" || !block.feed_mm_min)
                            continue;
                        // Estimate cutting time for this block
                        const prevBlock = blocks[block.id - 1];
                        const dist = prevBlock ? Math.sqrt(((block.x ?? 0) - (prevBlock.x ?? 0)) ** 2 +
                            ((block.y ?? 0) - (prevBlock.y ?? 0)) ** 2 +
                            ((block.z ?? 0) - (prevBlock.z ?? 0)) ** 2) : 0;
                ...
                try {
                    ...
                    for (const [toolNum, toolBlocks] of toolGroups) {
                        ...
                        for (let i = 1; i < toolBlocks.length; i++) {
                            if (toolBlocks[i].move_type === "G0" || !toolBlocks[i].feed_mm_min)
                                continue;
                            const prev = toolBlocks[i - 1];
                            const d = Math.sqrt(((toolBlocks[i].x ?? 0) - (prev.x ?? 0)) ** 2 +
                                ((toolBlocks[i].y ?? 0) - (prev.y ?? 0)) ** 2 +
                                ((toolBlocks[i].z ?? 0) - (prev.z ?? 0)) ** 2);
                            if (d > 0 && toolBlocks[i].feed_mm_min > 0)
                                totalTime += d / toolBlocks[i].feed_mm_min;
                        }
                        if (totalTime <= 0)
                            continue;
                        ...
                        const result = twcEng.analyze({
                            ...
                            tool_overhang_mm: tool?.overhang_mm ?? 40,
                            tool_E_GPa: tool?.modulus_GPa ?? 600,
                        ...
                        if (trajectory.length > 0) {
                            ...
                            for (let i = 0; i < toolBlocks.length; i++) {
                                ...
                                const d = Math.sqrt(((block.x ?? 0) - (prev.x ?? 0)) ** 2 +
                                    ((block.y ?? 0) - (prev.y ?? 0)) ** 2 +
                                    ((block.z ?? 0) - (prev.z ?? 0)) ** 2);
                                if (d > 0 && block.feed_mm_min > 0)
                                    cumulativeTime += d / block.feed_mm_min;
                                // Find closest trajectory point
                                const tIdx = Math.min(trajectory.length - 1, Math.max(0, Math.round((cumulativeTime / totalTime) * (trajectory.length - 1))));
            ...
            await _localRunStageAsync("3.1_toolpath_smoothing", 3, stages, async () => {
                try {
                    ...
                    for (let i = 1; i < blocks.length - 1; i++) {
                        ...
                        if (curr.move_type === "G0" || next.move_type === "G0")
                            continue;
            ...
            await _localRunStageAsync("3.2_motion_dynamics", 3, stages, async () => {
                try {
                    ...
                    for (let i = 1; i < blocks.length; i++) {
                        const prev = blocks[i - 1];
                        const curr = blocks[i];
                        if (curr.move_type === "G0" || !curr.feed_mm_min)
                            continue;
                        const dist = Math.sqrt(((curr.x ?? 0) - (prev.x ?? 0)) ** 2 +
                            ((curr.y ?? 0) - (prev.y ?? 0)) ** 2 +
                            ((curr.z ?? 0) - (prev.z ?? 0)) ** 2);
                        if (dist < 0.001)
                            continue;
                        ...
                        if (maxAchievable_mm_min < curr.feed_mm_min * 0.95) {
                            if (curr.optimization) {
                                ...
                                curr.optimization.reasons.push(`Motion: seg ${dist.toFixed(1)}mm too short for F${curr.feed_mm_min} → F${Math.round(maxAchievable_mm_min)}`);
                            }
        ...
        // Source: fusion-post-strategies.json + cimco-post-strategies.json (PP-MOAT-MS3 U04)
        {
            ...
            for (const block of blocks) {
                ...
                const sweep = block.arc_sweep_deg;
        ...
        if (stageFlags.look_ahead) {
            await _localRunStageAsync("3.3_look_ahead", 3, stages, async () => {
                try {
                    ...
                    const maxVels = new Array(count).fill(0); // mm/s
                    for (let i = 1; i < count; i++) {
                        const prev = blocks[i - 1];
                        const curr = blocks[i];
                        distances[i] = Math.sqrt(((curr.x ?? 0) - (prev.x ?? 0)) ** 2 +
                            ((curr.y ?? 0) - (prev.y ?? 0)) ** 2 +
                            ((curr.z ?? 0) - (prev.z ?? 0)) ** 2);
                        maxVels[i] = (curr.feed_mm_min ?? 0) / 60;
                    }
                    // Forward pass: limit entry velocity based on previous exit + accel + distance
                    const entryVels = new Array(count).fill(0);
                    entryVels[0] = 0;
                    for (let i = 1; i < count; i++) {
                    ...
                    const exitVels = new Array(count).fill(0);
                    exitVels[count - 1] = 0;
                    ...
                    for (let i = 1; i < count; i++) {
                        if (blocks[i].move_type === "G0" || !blocks[i].feed_mm_min)
                            continue;
                        ...
                        if (originalFeed > 0 && plannedFeed < originalFeed * 0.95) {
                            if (blocks[i].optimization) {
                                ...
                                blocks[i].optimization.reasons.push(`Look-ahead: F${originalFeed} → F${plannedFeed} (accel/decel limited)`);
                            }
                ...
                try {
                    ...
                    for (const block of blocks) {
                        ...
                        if (delta > 0.0001) { // only apply if > 0.1 µm
                            ...
                            offsetCount++;
                            if (block.optimization) {
                                block.optimization.reasons.push(`Thermal comp: Z offset ${(delta * 1000).toFixed(2)}µm (ΔT=${deltaT}°C)`);
                            }
                        }
                    }
                    return {
        ...
        if (stageFlags.stability_rewrite) {
            await _localRunStageAsync("3.4b_stability_rpm_rewrite", 3, stages, async () => {
                try {
                    ...
                    const sldData = sldStage?.data;
                    const lobes = sldData?.sld_lobes ?? sldData?.lobes ?? [];
                    if (lobes.length === 0) {
                    ...
                    for (const block of blocks) {
                        ...
                        if (block.x != null)
                            line += ` X${block.x.toFixed(3)}`;
                        if (block.y != null)
                            line += ` Y${block.y.toFixed(3)}`;
                        if (block.z != null)
                            line += ` Z${block.z.toFixed(3)}`;
                        if (block.feed_mm_min != null && block.move_type !== "G0")
                            line += ` F${block.feed_mm_min}`;
                        if (block.spindle_rpm != null)
                            line += ` S${block.spindle_rpm}`;
        ...
        if (stageFlags.controller_features && (machine?.controller || input.controller)) {
            await _localRunStageAsync("3.5_controller_features", 3, stages, async () => {
                try {
                    ...
                    const featureSelected = featureSelectionResult?.selected_features?.some((f) => f.id === "hsm_smoothing" || f.id === "corner_rounding" || f.id === "nurbs_interpolation");
                    ...
                    if (isFinishing || isHSM || tightTolerance || featureSelected) {
                        try {
                            ...
                            const hsmConfig = {
                                ...
                                corner_rounding: featureSelectionResult?.selected_features?.some((f) => f.id === "corner_rounding") ?? tightTolerance,
                                nurbs: featureSelectionResult?.selected_features?.some((f) => f.id === "nurbs_interpolation") ?? false,
                            };
                    ...
                    const rtcpSelected = featureSelectionResult?.selected_features?.some((f) => f.id === "rtcp" || f.id === "tcpc") || isMultiAxis;
                    if (rtcpSelected) {
                        const ctrlKey = ctrl.toLowerCase();
                        const rtcpCodes = {
                    ...
                    const ssvSelected = featureSelectionResult?.selected_features?.some((f) => f.id === "ssv" || f.id === "spindle_speed_variation") || stageFlags.stability_rewrite;
                    if (ssvSelected) {
                        const ctrlKey = ctrl.toLowerCase();
                        const ssvCodes = {
                        ...
                        if (ssv.on)
                            injectedCodes.unshift(`${dialect.comment_open ?? "("}SSV ON${dialect.comment_close ?? ")"} ${ssv.on}`);
                        if (ssv.off)
                            injectedCodes.push(`(HSM_OFF) ${ssv.off}`);
                    }
                    // Store codes for Stage 6.1 to inject into output
                    this._featureInjectionCodes = injectedCodes;
                    return {
        ...
        if (stageFlags.uncertainty_propagation === true) {
            _localRunStage("4.2_uncertainty_propagation", 4, stages, () => {
                ...
                if (blocksWithCI.length === 0)
                    return { propagated_blocks: 0, max_dim_uncertainty_um: 0, note: "no confidence data" };
                ...
                for (const block of blocksWithCI) {
                    ...
                    if (dimUncertaintyUm > maxDimUncertainty)
                        maxDimUncertainty = dimUncertaintyUm;
                }
                return {
        ...
        if (stageFlags.dimensional_verification === true) {
            _localRunStage("4.3_dimensional_verification", 4, stages, () => {
                ...
                const maxDimUncertaintyUm = stage42?.data?.max_dim_uncertainty_um ?? 0;
        ...
        if (stageFlags.surface_finish_verification === true) {
            _localRunStage("4.4_surface_finish_verification", 4, stages, () => {
                ...
                if (cuttingBlocks.length === 0)
                    return { avg_ra_predicted: 0, max_ra: 0, blocks_exceeding_target: 0, note: "no cutting blocks" };
                let sumRa = 0;
                let maxRa = 0;
                ...
                for (const block of cuttingBlocks) {
                    ...
                    sumRa += raPredicted;
                    if (raPredicted > maxRa)
                        maxRa = raPredicted;
                    if (raPredicted > targetRa)
                        blocksExceeding++;
                }
        ...
        if (stageFlags.robustness_score === true) {
            _localRunStage("4.7_robustness_score", 4, stages, () => {
                const cuttingBlocks = blocks.filter(b => b.move_type !== "G0" && b.forces);
                if (cuttingBlocks.length === 0)
                    return { score: 100, note: "no cutting data" };
        ...
        // Stage 5.3: Tribal knowledge — CAM-system-specific tips (TK-MS2-U12)
        if (stageFlags.tribal_knowledge && material) {
            _localRunStage("5.3_tribal_knowledge", 5, stages, () => {
                ...
                // Also query for tooling tips
                const toolingQuery = tribalKnowledgeEngine.search({
                    category: "tooling",
                    material_name: material.name,
                    iso_group: isoGroup,
                    limit: 5,
                });
                // Add dynamic tribal tips from engine
                for (const tip of materialQuery) {
                    tips.push(`TK: ${tip.title ?? tip.insight}`);
                }
                for (const tip of toolingQuery) {
                    tips.push(`TK: ${tip.title ?? tip.insight}`);
                }
                // Fallback hardcoded tips if no tribal knowledge found
                if (tips.length === 0) {
                    if (isoGroup === "S") {
                ...
                // Tool-based tips (L/D ratio and ball endmill)
                for (const tool of tools) {
                    const ld = (tool.flute_length_mm ?? tool.diameter_mm * 3) / tool.diameter_mm;
                    if (ld > 5)
                        tips.push(`TK: T${tool.id} L/D=${ld.toFixed(1)} — use light cuts, peck strategy for deep features`);
                    if (tool.type === "ball_endmill")
                        tips.push(`TK: T${tool.id} ball nose — maintain min 5° tilt for effective cutting`);
                }
                if (tips.length > 0)
                    warnings.push(...tips);
                return { tips_applied: tips.length, material_group: isoGroup, tribal_engine_hits: materialQuery.length + toolingQuery.length };
            });
        }
        else {
                ...
                try {
                    ...
                    for (let i = 1; i < blocks.length; i++) {
                        if (blocks[i].move_type === "G0" || !blocks[i].feed_mm_min)
                            continue;
                        const prev = blocks[i - 1];
                        const dist = Math.sqrt(((blocks[i].x ?? 0) - (prev.x ?? 0)) ** 2 +
                            ((blocks[i].y ?? 0) - (prev.y ?? 0)) ** 2 +
                            ((blocks[i].z ?? 0) - (prev.z ?? 0)) ** 2);
                        if (dist > 0 && blocks[i].feed_mm_min > 0)
                            cuttingTimeSec += (dist / blocks[i].feed_mm_min) * 60;
                    }
        ...
        if (stageFlags.reliability_check === true) {
            _localRunStage("5.6_reliability_check", 5, stages, () => {
                const cuttingBlocks = blocks.filter(b => b.move_type !== "G0" && b.feed_mm_min);
                if (cuttingBlocks.length === 0)
                    return { reliability_pct: 100, note: "no cutting blocks" };
        ...
        if (stageFlags.acoustic_check === true) {
            _localRunStage("5.7_acoustic_check", 5, stages, () => {
                ...
                if (cuttingBlocks.length === 0)
                    return { noise_level_dba: 70, hearing_protection_required: false, note: "no cutting blocks" };
                let maxNoise = 0;
                const machineBackground = 70; // dBA
                for (const block of cuttingBlocks) {
                    const rpm = block.spindle_rpm ?? 1000;
                    const toolDia = block.tool_diameter_mm ?? 10;
                    const Vc = (Math.PI * toolDia * rpm) / 1000; // m/min
                    const ap = block.ap_mm ?? 1;
                    const ae = block.ae_mm ?? toolDia * 0.5;
        ...
        if (machine?.axes === 5 || blocks.some(b => b.a !== undefined || b.b !== undefined || b.c !== undefined)) {
            await _localRunStageAsync("5.9_five_axis_post", 5, stages, async () => {
                try {
                    ...
                    const fiveAxisBlocks = blocks.filter(b => b.a !== undefined || b.b !== undefined || b.c !== undefined).map(b => ({
                    ...
                    if (fiveAxisBlocks.length > 0) {
                        const singularity = eng.detectSingularities(fiveAxisBlocks, {
                            ...
                            kinematics: machine?.kinematics ?? "table_table",
        ...
        if (stageFlags.gcode_generation && omegaSafetyPassed) {
            await _localRunStageAsync("6.1_gcode_generation", 6, stages, async () => {
                ...
                const featureCodes = this._featureInjectionCodes ?? [];
                ...
                this._featureInjectionCodes = undefined;
        ...
        const sisterToolSelected = featureSelectionResult?.selected_features?.some((f) => f.id === "sister_tooling" || f.id === "tool_break_detect");
            ...
            await _localRunStageAsync("6.1b_tool_management", 6, stages, async () => {
                try {
                    ...
                    const toolMgmtConfig = {
                        sister_tooling: featureSelectionResult?.selected_features?.some((f) => f.id === "sister_tooling") ?? false,
                        max_tool_life_minutes: 30,
                        auto_offset_update: true,
                        wear_offset_increment: 0.005,
                        break_detection: featureSelectionResult?.selected_features?.some((f) => f.id === "tool_break_detect") ?? false,
        ...
        if (stageFlags.rl_formatting && outputGcode) {
            await _localRunStageAsync("6.1c_rl_formatting", 6, stages, async () => {
                try {
                    ...
                    const moves = blocks
                        .filter((b) => b.x != null && b.y != null && b.z != null)
                        .map((b) => ({
                        type: b.move_type === "G0" ? "rapid"
                            : (b.move_type === "G2" || b.move_type === "G3") ? "arc"
                                : "linear",
                        x: b.x ?? 0,
                        y: b.y ?? 0,
                        z: b.z ?? 0,
        ...
        if (stageFlags.probe_routines === true) {
            await _localRunStageAsync("6.2_probe_routines", 6, stages, async () => {
                const ctrl = machine?.controller ?? input.controller ?? "fanuc";
                const toleranceMm = input.tolerance_mm ?? DEFAULT_TOLERANCE_MM;
                const probeFeatureSelected = featureSelectionResult?.selected_features?.some((f) => f.id === "in_process_probing" || f.id === "auto_compensation");
                ...
                try {
                    ...
                    try {
                        ...
                        const probeCtrl = ctrlStr.includes("heidenhain") ? "heidenhain"
                            : ctrlStr.includes("siemens") ? "siemens"
                                : ctrlStr.includes("haas") ? "renishaw_haas"
                                    : "renishaw_fanuc";
                        const probeConfig = {
                        ...
                        const cornerResult = probeCycle.generate({ operation: "corner", x: 0, y: 0, z: 0, approach_z: 50 }, probeConfig);
                        // Generate surface-Z probe
                        const surfResult = probeCycle.generate({ operation: "surface_z", z: 0, approach_z: 50 }, probeConfig);
                        // Infer in-process probes from tight-tolerance features
                        const cuttingBlocks = blocks.filter(b => b.move_type !== "G0");
                        const inProcessProbes = [];
                        const seenZ = new Set();
                        for (const block of cuttingBlocks.slice(0, 20)) {
                            if (block.z != null && toleranceMm < 0.05) {
                                ...
                                if (seenZ.has(key))
                                    continue;
                                seenZ.add(key);
                                const r = probeCycle.generate({ operation: "surface_z", z: block.z, approach_z: block.z + 5, tolerance_mm: toleranceMm }, { ...probeConfig, print_result: true });
                                inProcessProbes.push(r.gcode);
                                if (inProcessProbes.length >= 10)
                                    break;
                            }
                        }
        ...
        if (stageFlags.setup_sheet === true) {
            _localRunStage("6.3_setup_sheet", 6, stages, () => {
                ...
                const safetyWarnings = safetyStage?.data?.critical_issues ?? 0;
                const setupSheet = {
                    part_number: input.part_id ?? "UNKNOWN",
                    material: input.material ?? "Unknown",
                    tools_used: toolsUsed,
                    work_offset: "G54",
                    fixture_notes: input.fixture_notes ?? "See setup drawing",
                    estimated_cycle_time: stages.find(s => s.stage === "6.6_cycle_time")?.data?.total_seconds ?? null,
        ...
        if (stageFlags.analytics_report || input.include_analytics) {
            _localRunStage("6.5_analytics_report", 6, stages, () => {
                ...
                if (analytics)
                    analytics.calibration_source = calibrationSource;
                // PP-MOAT-MS2 U04: Inject sustainability data into analytics
                if (analytics && sustainabilityData)
                    analytics.sustainability = sustainabilityData;
                return analytics;
            });
        }
        ...
        if (stageFlags.digital_twin === true) {
            _localRunStage("6.7_digital_twin", 6, stages, () => {
                ...
                const thermalProfile = cuttingBlocks.filter(b => b.thermal).map(b => ({ id: b.id, temp_C: b.thermal?.temperature_C ?? 0 }));
            ...
            await _localRunStageAsync("6.8_output_verification", 6, stages, async () => {
                try {
                    ...
                    if (criticals.length > 0) {
                        warnings.push(`VERIFY: ${criticals.length} critical issue(s) — ${criticals.map((c) => c.message).join("; ")}`);
                    }
                    return {
                        valid: result.valid,
        ...
        return {
            ...
            feature_selection: featureSelectionResult ? {
                selected: (featureSelectionResult.selected_features ?? []).map((f) => ({
                    id: f.id, name: f.name, score: f.score,
                })),
                rejected: (featureSelectionResult.rejected_features ?? []).map((f) => ({
    ...
    _resolveContexts(input, _parsedTools) {
        ...
        if (input.machine && "max_rpm" in input.machine) {
            ...
            const machineName = input.machine.name;
            let resolvedController = input.controller ?? "fanuc";
            let confidence = 0.3;
            ...
            try {
                ...
                if (machineRegistry?.isLoaded?.() || machineRegistry?.entries?.size > 0) {
                    ...
                    if (regMatch) {
                        ...
                        const controllerFamily = (ctrl?.manufacturer?.toLowerCase() ?? "fanuc");
            ...
            if (!machine) {
                try {
                    ...
                    if (postProcessorRegistry?.count && postProcessorRegistry.count() > 0) {
                        ...
                        const match = allPosts.find((p) => p.machine_examples?.some((m) => m.toLowerCase().includes(nameLower)) ||
                            p.controller_model?.toLowerCase().includes(nameLower));
                        if (match?.controller_family) {
                            resolvedController = match.controller_family;
                            confidence = 0.8;
                        }
                    }
                }
        ...
        if (input.material && "iso_group" in input.material && "id" in input.material) {
            ...
            if (!resolved) {
                const isoGroup = input.material.iso_group ?? this._inferISOGroup(name);
                material = {
                    id: "resolved",
                    name,
                    iso_group: isoGroup,
        ...
        if (input.tools) {
            ...
            for (const t of input.tools) {
                if ("flute_count" in t && "type" in t) {
                    ...
                    const inputDia = t.diameter_mm ?? 10;
                    const inputType = t.type;
                    const inputId = t.tool_number ?? t.id;
                    let resolved = false;
                    ...
                    if (toolRegistry?.entries?.size > 0) {
                        try {
                            ...
                            for (const entry of toolRegistry.entries.values()) {
                                ...
                                if (diaDelta > 0.5)
                                    continue; // within 0.5mm of requested diameter
                                if (inputType && ct.type && !ct.type.toLowerCase().includes(inputType.toLowerCase()))
                                    continue;
                    ...
                    if (!resolved) {
                        resolvedTools.push({
                            ...
                            flute_count: t.flute_count ?? (inputDia <= 6 ? 3 : inputDia <= 16 ? 4 : 5),
    ...
    _applySmartDefaults(input, machine, material, tools, blocks, warnings) {
        const defaults = {};
        // Default tolerance
        if (!input.tolerance_mm) {
            input.tolerance_mm = DEFAULT_TOLERANCE_MM;
            defaults.tolerance = `${DEFAULT_TOLERANCE_MM}mm (ISO 2768-m)`;
        }
        // Default surface finish
        if (!input.surface_finish_Ra) {
            input.surface_finish_Ra = DEFAULT_RA_SEMI;
        ...
        if (!input.coolant && material) {
            ...
            input.coolant = coolantMap[material.iso_group] ?? { type: "flood" };
            defaults.coolant = input.coolant.type;
        }
    ...
    _resolveDefaultAxialDepthMm(toolDiameterMm, operation) {
        if (operation?.ap_mm != null)
            return operation.ap_mm;
        // Keep the generic fallback conservative and comparable across tool sizes.
        return Math.min(Math.max(toolDiameterMm * 0.5, 2), 5);
    }
    _resolveDefaultRadialDepthMm(toolDiameterMm, operation) {
        if (operation?.ae_mm != null)
            return operation.ae_mm;
    ...
    _mapToolTypeForEngagement(type) {
        if (type === "ball_endmill")
            return "ball_endmill";
        if (type === "bull_nose")
            return "bull_nose";
        return "flat_endmill";
    ...
    _inferISOGroup(name) {
        ...
        if (/alum|6061|7075|2024/.test(lower))
            return "N";
        if (/stainless|316|304|303|17-4/.test(lower))
            return "M";
        if (/cast.?iron|grey|ductile|ggg/.test(lower))
            return "K";
        if (/inconel|hastelloy|waspaloy|nimonic|rene/.test(lower))
            return "S";
        if (/hardened|d2|m2|h13|hrc/.test(lower))
            return "H";
        return "P"; // Default to general steel
    }
    _blocksToGCode(blocks, controller) {
        const lines = [];
            ...
            if (isHeidenhain) {
                ...
                if (block.move_type === "G0") {
                    ...
                    if (block.x !== undefined)
                        parts.push(`X${this._fmt(block.x)}`);
                    if (block.y !== undefined)
                        parts.push(`Y${this._fmt(block.y)}`);
                    if (block.z !== undefined)
                        parts.push(`Z${this._fmt(block.z)}`);
                    parts.push("FMAX");
                }
                ...
                if (block.move_type === "G2" || block.move_type === "G3") {
                    if (block.i !== undefined)
                        parts.push(`I${this._fmt(block.i)}`);
                    if (block.j !== undefined)
                        parts.push(`J${this._fmt(block.j)}`);
                    if (block.k !== undefined)
                        parts.push(`K${this._fmt(block.k)}`);
                    if (block.r !== undefined)
     ...
     */
    _blocksToGCodeWithDialect(blocks, dialectEng, controllerKey, outputMode = "pipeline_optimized") {
        ...
        if (dialect.safe_start)
            lines.push(dialect.safe_start);
        let lastTool = -1;
        let lastRpm = -1;
        let lastFeed = -1;
        ...
        for (const block of blocks) {
            ...
            if (isHeidenhain) {
                ...
                if (block.z !== undefined)
                    parts.push(`Z${this._fmt(block.z)}`);
                if (block.a !== undefined)
                    parts.push(`A${this._fmt(block.a)}`);
                if (block.b !== undefined)
                    parts.push(`B${this._fmt(block.b)}`);
                if (block.c !== undefined)
                    parts.push(`C${this._fmt(block.c)}`);
                if (block.move_type === "G2" || block.move_type === "G3") {
            ...
            if (parts.length > 0) {
                ...
                if (annotate && block.optimization?.reasons?.length && !isHeidenhain) {
                    ...
                    if (reason)
                        lines.push(`${dialect.comment_open ?? "("}PRISM: ${reason}${dialect.comment_close ?? ")"}`);
                }
            }
        }
        ...
        if (coolantActive)
            lines.push(dialect.coolant_off ?? "M9");
        if (!isHeidenhain)
            lines.push(dialect.spindle_stop ?? "M5");
    ...
    _generateAnalytics(blocks, tools, machine) {
        const cuttingBlocks = blocks.filter(b => b.move_type !== "G0");
    ...
    _estimateCycleTime(blocks, machine) {
        ...
        for (let i = 1; i < blocks.length; i++) {
            ...
            if (curr.tool_number !== undefined)
                toolChanges.add(curr.tool_number);
        }
    ...
    _runStage(name, phase, stages, fn) {
        const t0 = Date.now();
        try {
            const result = fn();
            stages.push({
    ...
    // Async stage runner
    async _runStageAsync(name, phase, stages, fn) {
        const t0 = Date.now();
        try {
...
//# sourceMappingURL=PostProcessorPipelineEngine.js.map