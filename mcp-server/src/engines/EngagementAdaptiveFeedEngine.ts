...
/** ISO material group identifier for Kienzle constant lookup. */
...
/**
 * Default Kienzle constants per ISO material group.
 * kc1_1: specific cutting force at h=1mm, b=1mm (N/mm²)
 * mc: Kienzle exponent (dimensionless)
 * Source: Klocke (2011) Table 7.2, averaged per group
 */
const KIENZLE_DEFAULTS: Record<ISOGroup, { kc1_1: number; mc: number }> = {
  P: { kc1_1: 1800, mc: 0.25 },   // Steel
  M: { kc1_1: 2100, mc: 0.26 },   // Stainless steel
  K: { kc1_1: 1200, mc: 0.24 },   // Cast iron
  N: { kc1_1: 700, mc: 0.23 },    // Aluminum / non-ferrous
  S: { kc1_1: 2800, mc: 0.28 },   // Superalloys (Inconel, Ti)
  H: { kc1_1: 3500, mc: 0.30 },   // Hardened steel (>45 HRC)
};
...
export class EngagementAdaptiveFeedEngineImpl {
  ...
  adaptFeed(input: AdaptiveFeedInput): AdaptiveFeedResult {
    ...
    const nominalChipThickness = fz_nom * Math.sin((nominalEngDeg * DEG2RAD) / 2);
