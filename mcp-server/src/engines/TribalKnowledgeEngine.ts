...
// ============================================================================
// TK-MS6 U-TK30: Master Machinist Types
// ============================================================================
...
export interface MasterMachinistTip {
  message: string;              // "Senior machinists say: ..."
  validation_count: number;     // Times validated/applied
...
export interface MasterMachinistRecommendation {
  ...
  master_machinist_says: string;  // Top recommendation as simple string
}
...
// ============================================================================
// TK-MS7-U34: LLM LEARNING LOOP TYPES
// ============================================================================
...
// ============================================================================
// TK-MS8: OUTCOME TRACKING & REINFORCEMENT LEARNING TYPES
// ============================================================================

/** Outcome of using a tribal tip. */
export type TipOutcome = "success" | "partial" | "failure" | "not_applicable";

/** Context in which a tip was used. */
...
/** Record of a tip outcome for reinforcement learning. */
export interface OutcomeRecord {
  ...
  timestamp: string;
  session_id: string;
  confidence_delta?: number;  // How much this outcome affected confidence
  notes?: string;
}

/** Reinforcement summary for a tip. */
...
/** Proactive suggestion from tribal knowledge. */
export interface ProactiveTribalSuggestion {
  tip_id: string;
  title: string;
  body: string;
  category: string;
...
const KNOWLEDGE_BASE: KnowledgeTip[] = [
  ...
  // --- Document-learned tips (source: AU2013 Mighty Macros handout — AutoCAD automation) ---
  { id: "TK-DL-cad-macro-01", title: "Macro special characters for CAD/CAM automation", body: "When writing macros for CAD/CAM software: ^C^C cancels previous actions (equivalent to double-Escape), semicolon (;) issues Enter/Return, backslash (\\) pauses for user input, underscore (_) enables command translation across localized versions. These patterns apply broadly: AutoCAD, hyperMILL scripting, Mastercam toolpath macros. Always start macros with ^C^C to ensure clean state before executing commands.", category: "automation", tags: ["macro", "special-characters", "cad", "cam", "automation", "scripting"], confidence: 75, source: "document:AC1337_Mighty_Macros@table1", created_at: "2026-04-15", usage_count: 0 },
  { id: "TK-DL-cad-macro-02", title: "Cancel-Command-Pause-Input macro pattern", body: "Robust macro structure for CAD/CAM automation: (1) Cancel any previous action (^C^C), (2) Issue command with dialog suppression (-command or underscore prefix), (3) Pause for user input (\\) where needed, (4) Issue returns (;) to confirm selections. Example: ^C^C_-COMMAND;\\;option; This pattern ensures macros work regardless of the software's initial state and across different language versions.", category: "automation", tags: ["macro", "pattern", "automation", "best-practice", "cad", "cam"], confidence: 75, source: "document:AC1337_Mighty_Macros@table2-4", created_at: "2026-04-15", usage_count: 0 },
  ...
  // --- PDF-learned tips (source: Okuma MULTUS Fixed Cycles Manual) ---
  { id: "okuma-fc-001", title: "Okuma G181 drilling cycle uses I/K for rapid shift and Q for hole repeat count", body: "G181 (Drilling Cycle with repeat function) format: G181,X,Z,C,R,I(K),F,Q,E. X/Z define cycle endpoint, R is infeed amount from start point (sign indicates direction), I is X-axis rapid shift, K is Z-axis rapid shift, F is feed rate, Q is number of equally spaced holes, E is optional dwell at bottom. For end face drilling, X is start point and Z is endpoint. For OD drilling, flip axis roles.", category: "cnc_programming", tags: ["okuma", "g-code", "drilling", "fixed-cycle", "g181", "lathe"], operation_types: ["drilling"], confidence: 95, source: "document:okuma-fixed-cycles-manual", created_at: "2026-04-15", usage_count: 0 },
  { id: "okuma-fc-002", title: "Okuma G183 deep hole drilling with peck uses D for depth per peck, L for relief", body: "G183 (Deep Hole Drilling with repeat) adds D and L parameters: D specifies depth of cut per peck feed (chip breaking), L specifies axis relieving amount (retract distance for chip evacuation). For holes >3xD deep, always use G183 instead of G181. Break chips frequently to prevent chip packing and drill breakage. Set D to 1-2x drill diameter, L to 0.5-1mm for carbide drills.", category: "cnc_programming", tags: ["okuma", "g-code", "deep-drilling", "peck", "g183", "chip-breaking"], operation_types: ["drilling"], confidence: 94, source: "document:okuma-fixed-cycles-manual", created_at: "2026-04-15", usage_count: 0 },
  { id: "okuma-fc-003", title: "Okuma G178/G179 synchronized tapping uses D for start position, J for thread count", body: "G178 (sync tap forward) and G179 (sync tap reverse) enable rigid tapping on Okuma lathes. D specifies M-tool spindle start position, J specifies number of threads. These cycles synchronize spindle rotation with Z-axis feed for accurate threading without a floating tap holder. Use G178 for right-hand threads, G179 for left-hand. Always verify with a thread gauge on first article.", category: "cnc_programming", tags: ["okuma", "g-code", "tapping", "rigid-tap", "synchronized", "g178", "g179"], operation_types: ["tapping", "threading"], confidence: 93, source: "document:okuma-fixed-cycles-manual", created_at: "2026-04-15", usage_count: 0 },
  { id: "okuma-fc-004", title: "Okuma G185-G188 thread cutting cycles use SA= for C-axis rotation speed", body: "Thread cutting cycles G185 (longitudinal), G186 (transverse), G187 (straight longitudinal), G188 (straight face) include SA= parameter for C-axis rotation speed command. This calculates C-axis movement for thread cutting. G185/G186 are for single-point threading, G187/G188 are for continuous straight threads. I/K define taper endpoints for tapered threads. Always use G180 to cancel fixed cycle mode after completion.", category: "cnc_programming", tags: ["okuma", "g-code", "threading", "single-point", "c-axis", "g185", "g186"], operation_types: ["threading"], confidence: 92, source: "document:okuma-fixed-cycles-manual", created_at: "2026-04-15", usage_count: 0 },
  { id: "okuma-fc-005", title: "Okuma G296/G297 fine boring retracts tool edge before extraction", body: "G296 (Fine Boring) retracts the tool edge using HS parameter after finishing cut, then extracts tool cleanly. G297 (Back Boring) does the opposite — retracts edge before insertion for machining the back side of holes. HS specifies the spindle orientation angle for edge retraction. These cycles prevent tool drag marks on precision bore finishes. Always use on holes requiring Ra < 1.6μm surface finish.", category: "cnc_programming", tags: ["okuma", "g-code", "boring", "fine-boring", "back-boring", "g296", "g297"], operation_types: ["boring"], confidence: 91, source: "document:okuma-fixed-cycles-manual", created_at: "2026-04-15", usage_count: 0 },
  { id: "okuma-fc-006", title: "Okuma G190 keyway cutting cycle uses M211-M214 for spindle orientation", body: "G190 (Key Way Cutting with repeat) uses M211/M212 for spindle CW/CCW orientation and M213/M214 for spindle clamp/release. Format: G190,X,Z,C,I(K),D,U(W),E,F,Q,M211,M213. D is depth of cut per pass, U/W are finish allowances for side/face. The cycle indexes to each C position (equally spaced by Q), orients spindle, clamps, and cuts keyway. Critical: verify spindle clamp pressure before cutting deep keyways.", category: "cnc_programming", tags: ["okuma", "g-code", "keyway", "milling", "spindle-orient", "g190"], operation_types: ["milling", "keyway"], confidence: 90, source: "document:okuma-fixed-cycles-manual", created_at: "2026-04-15", usage_count: 0 },

  // --- PDF-learned tips (source: Mastercam Dynamic Milling Tutorial) ---
  { id: "dyn-001", title: "Dynamic Mill uses full flute length to maximize material removal and tool life", body: "Mastercam Dynamic Mill utilizes the entire flute length of cutting tools (not just the tip) for maximum material removal efficiency. Key benefits: tool burial avoidance (constant chip load), minimum heat buildup (chip carries heat away), better chip evacuation (tool never dwells in chips). Always use full flute length tools — short-flute tools defeat the purpose. Set stepover to 5-15% of tool diameter.", category: "cam_strategy", tags: ["mastercam", "dynamic-mill", "flute-length", "chip-load", "roughing", "efficiency"], operation_types: ["roughing", "pocketing"], confidence: 94, source: "document:mastercam-dynamic-milling", created_at: "2026-04-15", usage_count: 0 },
  { id: "dyn-002", title: "Dynamic Mill Stay Inside strategy machines pockets from inside to outside efficiently", body: "Dynamic Mill with 'Stay Inside' strategy (default) machines pockets starting from the inside and working outward. Requires at least one closed chain as Machining region. Additional geometry becomes Avoidance, Containment, or Air regions. Best for: enclosed pockets, complex multi-island geometry. The toolpath maintains constant engagement angle throughout, preventing tool overload at corners and narrow areas.", category: "cam_strategy", tags: ["mastercam", "dynamic-mill", "stay-inside", "pocket", "engagement-angle", "strategy"], operation_types: ["pocketing", "roughing"], confidence: 93, source: "document:mastercam-dynamic-milling", created_at: "2026-04-15", usage_count: 0 },
  { id: "dyn-003", title: "Dynamic Mill From Outside strategy cuts standing cores from the boundary inward", body: "Dynamic Mill 'From Outside' strategy machines open pocket shapes or standing cores by treating the outermost chain as stock boundary. Tool moves freely outside this area, starting from outside and working toward inner boundaries. Use for: boss machining, standing features, open-sided pockets. Not suitable for fully enclosed cavities — use Stay Inside strategy instead.", category: "cam_strategy", tags: ["mastercam", "dynamic-mill", "from-outside", "core", "boss", "open-pocket"], operation_types: ["roughing", "pocketing"], confidence: 92, source: "document:mastercam-dynamic-milling", created_at: "2026-04-15", usage_count: 0 },
  { id: "dyn-004", title: "Dynamic Mill Rest Mill strategy targets material left by previous operations", body: "Dynamic Mill 'Rest Mill' strategy specifically targets material left from previous roughing operations. Uses dynamic motion to rapidly remove corner material and narrow areas the larger roughing tool could not reach. Specify the previous tool diameter or reference previous operation — Mastercam calculates remaining stock automatically. Always run after main roughing, before finishing passes.", category: "cam_strategy", tags: ["mastercam", "dynamic-mill", "rest-mill", "corner-cleanup", "previous-operation", "remaining-stock"], operation_types: ["roughing", "rest_machining"], confidence: 93, source: "document:mastercam-dynamic-milling", created_at: "2026-04-15", usage_count: 0 },
  { id: "dyn-005", title: "Dynamic Contour converts to dynamic motion where tool would bind or overload", body: "Dynamic Contour mills material off walls supporting closed or open chains. The toolpath automatically converts to dynamic motion in areas where a standard contour would cause tool binding or encounter too much material. Use for walls with small radii, variable stock conditions, or forged/cast parts with inconsistent allowance. Does not require closed chains — works on open geometry.", category: "cam_strategy", tags: ["mastercam", "dynamic-contour", "walls", "small-radii", "binding", "variable-stock"], operation_types: ["contouring", "finishing"], confidence: 91, source: "document:mastercam-dynamic-milling", created_at: "2026-04-15", usage_count: 0 },

  // --- PDF-learned tips (source: hyperMILL Manual v2022) ---
  { id: "hm-001", title: "hyperMILL collision check requires pre-defined stock model and tool holder definition", body: "hyperMILL collision checking analyzes tool holder against current stock model (tracked stock) and turning model. Preparation: 1) Define accurate tool holder geometry in Tool Management, 2) Create initial stock model before first operation, 3) Enable 'Generate resulting stock' on each operation so stock updates progressively. Without proper stock tracking, collision check reports false positives or misses real collisions.", category: "cam_strategy", tags: ["hypermill", "collision-check", "stock-model", "tool-holder", "simulation", "safety"], operation_types: ["setup", "simulation"], confidence: 93, source: "document:hypermill-manual", created_at: "2026-04-15", usage_count: 0 },
  { id: "hm-002", title: "hyperMILL turning clearance radius/plane defines safe rapid traverse zone", body: "In hyperMILL turning cycles, Clearance radius (X) and Clearance plane (Z) define the safe zone for G0 rapid movements. All approach and retract rapids stay within this envelope. Clearance distance adds a feedrate buffer before rapids begin. Set clearance values outside workpiece + allowance + expected thermal growth. For internal operations (boring), clearance radius must be smaller than bore diameter.", category: "cnc_programming", tags: ["hypermill", "turning", "clearance", "rapid", "safety", "approach-retract"], operation_types: ["turning", "boring"], confidence: 92, source: "document:hypermill-manual", created_at: "2026-04-15", usage_count: 0 },
  { id: "hm-003", title: "hyperMILL approach/retract macros add tangential or circular entry for smooth cutting", body: "hyperMILL approach/retract macros define tool movement from clearance zone to cutting zone. Options: Radial (straight line), Tangential (smooth tangent entry), Circular (arc entry with defined radius/angle), Ramp (angled descent). Tangential and circular entries reduce tool shock at cut start. Ramp angle references turning axis accounting for infeed direction. Always verify macro geometry doesn't collide with fixtures.", category: "cam_strategy", tags: ["hypermill", "approach", "retract", "macro", "tangential", "circular", "ramp"], operation_types: ["turning", "milling"], confidence: 91, source: "document:hypermill-manual", created_at: "2026-04-15", usage_count: 0 },
  { id: "hm-004", title: "hyperMILL turning model must be closed planar contour in X-Z plane of turning frame", body: "A hyperMILL turning model requires a closed, planar contour located exactly in the X-Z plane of the defined turning frame. Select surfaces or solid → hyperMILL auto-generates the 2D turning contour. Resolution parameter controls contour fineness for complex surfaces. The turning contour creates a Contour feature in the feature browser. For multi-setup parts, define separate turning models per setup/frame.", category: "setup", tags: ["hypermill", "turning-model", "contour", "x-z-plane", "frame", "closed-contour"], operation_types: ["turning", "setup"], confidence: 92, source: "document:hypermill-manual", created_at: "2026-04-15", usage_count: 0 },
  { id: "hm-005", title: "hyperMILL Feature Based Machining auto-recognizes holes, pockets, slots from solid models", body: "hyperMILL Feature technology automatically identifies prismatic features (holes, pockets, slots, threads) on solid models and generates appropriate machining cycles. Feature catalogue contains standard templates for common feature types. Generate feature → hyperMILL analyzes geometry and proposes machining operations. Best for: plate work, fixture components, parts with many similar features. Manual intervention still needed for complex 3D surfaces.", category: "automation", tags: ["hypermill", "feature-based", "fbm", "automatic", "holes", "pockets", "slots"], operation_types: ["drilling", "pocketing", "automation"], confidence: 90, source: "document:hypermill-manual", created_at: "2026-04-15", usage_count: 0 },
  { id: "hm-006", title: "hyperMILL Optimised Roughing maintains constant tool engagement for extended tool life", body: "hyperMILL Optimised Roughing (3D Machining module) maintains constant tool engagement angle throughout the toolpath, similar to Mastercam Dynamic. Parameters: stepover percentage (typically 10-25%), radial engagement limit, entry method (helical, ramp, pre-drilled). The strategy avoids full-width cuts that overload tools. Best for hardened materials, high-speed machining, and deep cavities where consistent chip load matters.", category: "cam_strategy", tags: ["hypermill", "optimised-roughing", "constant-engagement", "chip-load", "hsm", "tool-life"], operation_types: ["roughing", "3d_roughing"], confidence: 93, source: "document:hypermill-manual", created_at: "2026-04-15", usage_count: 0 },
  { id: "hm-007", title: "hyperMILL Z-Level Finishing creates horizontal slice passes for steep walls", body: "hyperMILL Z Level Finishing creates constant-Z passes down steep walls and vertical surfaces. Parameters: Z-step (typically 0.1-0.5mm for finishing), limit angle (cuts only surfaces steeper than angle), smoothing (blend between Z passes). Combine with Complete Finishing for shallow areas. For large parts, split Z-level into multiple depth ranges for better tool control and easier restart after tool change.", category: "cam_strategy", tags: ["hypermill", "z-level", "finishing", "steep-walls", "constant-z", "vertical"], operation_types: ["finishing", "3d_finishing"], confidence: 92, source: "document:hypermill-manual", created_at: "2026-04-15", usage_count: 0 },
  { id: "hm-008", title: "hyperMILL Probing cycles auto-measure workpiece alignment before machining", body: "hyperMILL Probing module includes: 3D Point Probing, Workpiece Alignment (edge/holes), Rectangular Probing, Slot-Rib Probing, Circular Probing, Axis-dependent Probing. Cycles output probe results to controller variables for automatic WCS adjustment. Run probing after fixture loading, before machining. Critical for production automation where manual touch-off is eliminated. Requires Renishaw or compatible touch probe.", category: "automation", tags: ["hypermill", "probing", "alignment", "renishaw", "automatic", "wcs"], operation_types: ["probing", "setup", "automation"], confidence: 91, source: "document:hypermill-manual", created_at: "2026-04-15", usage_count: 0 },

  // --- PDF-learned tips (source: Hurco WinMax Cutter Compensation) ---
  { id: "hurco-001", title: "Hurco WinMax cutter comp Left = climb milling, Right = conventional milling", body: "In Hurco WinMax Mill, cutter compensation Left performs climb milling (tool cuts on downhill side of rotation), Right performs conventional milling (tool cuts on uphill side). Without cutter comp, tool centerline follows programmed path. With comp, tool edge follows finished contour. Milling Parameters > Conventional/Climb setting can override programmed direction. Verify with dry run before cutting.", category: "cnc_programming", tags: ["hurco", "winmax", "cutter-comp", "climb", "conventional", "milling-type"], operation_types: ["milling", "contouring"], confidence: 93, source: "document:hurco-winmax-cutter-comp", created_at: "2026-04-15", usage_count: 0 },
  { id: "hurco-002", title: "Hurco WinMax Inside/Outside milling with Blend Moves uses 180-degree arc entry", body: "When Enable Blend Moves = YES, Hurco WinMax Inside and Outside milling types enter/exit the cut using a 180-degree arc blend. This prevents tool marks at entry/exit points. For closed contours (circles, frames, ellipses), blend arc is automatic unless Blend Offset is 0. Use Profile Inside/Outside with No Blend Moves for tangent entry when blend marks are unacceptable in finish surface.", category: "cnc_programming", tags: ["hurco", "winmax", "blend-moves", "arc-entry", "inside", "outside"], operation_types: ["milling", "contouring", "pocketing"], confidence: 92, source: "document:hurco-winmax-cutter-comp", created_at: "2026-04-15", usage_count: 0 },
  { id: "hurco-003", title: "Hurco WinMax Pocket Boundary/Island handles complex pocket geometry automatically", body: "Hurco WinMax Pocket Boundary defines the outer pocket limit, Pocket Island defines internal features to avoid. Pocket Type = Outward starts from center and spirals out (circles/frames without islands). Pocket Type = Inward cuts from outside boundary, avoiding islands. Define as many islands as needed within available memory. Island cannot follow Outward boundary — change to Inward. Order of segment programming determines initial tool direction.", category: "cnc_programming", tags: ["hurco", "winmax", "pocket", "island", "boundary", "spiral"], operation_types: ["pocketing"], confidence: 91, source: "document:hurco-winmax-cutter-comp", created_at: "2026-04-15", usage_count: 0 },

  // --- PDF-learned tips (source: hyperMILL AUTOMATION Center Manual) ---
  { id: "ac-001", title: "hyperMILL Automation Center scripts automate repetitive CAM workflows", body: "hyperMILL AUTOMATION Center provides scripting for automating repetitive CAM tasks: model preparation, colouring, feature recognition, toolpath generation. Scripts can be saved as toolbar buttons or menu items for one-click execution. Server controller mode runs scripts across multiple parts without user interaction. Critical for production environments processing many similar parts.", category: "automation", tags: ["hypermill", "automation-center", "scripting", "workflow", "production"], operation_types: ["automation", "setup"], confidence: 92, source: "document:hypermill-automation-center", created_at: "2026-04-15", usage_count: 0 },
  { id: "ac-002", title: "Automation Center colouring functions identify feature types by surface geometry", body: "hyperMILL Automation Center includes smart colouring functions: Colouring holes, Colouring planes, Colouring lines/arcs, Colouring fillets (inner/outer), Colouring rotational elements, Colouring cylinders/chamfers. These automatically assign colours based on geometry analysis, enabling feature-based machining rules. Use Search inner/outer fillet to find blend radii. Colour assignments persist and drive downstream toolpath selection.", category: "automation", tags: ["hypermill", "automation-center", "colouring", "feature-recognition", "geometry-analysis"], operation_types: ["automation", "setup"], confidence: 90, source: "document:hypermill-automation-center", created_at: "2026-04-15", usage_count: 0 },
  { id: "ac-003", title: "Automation Center variables store and reuse values across script functions", body: "hyperMILL Automation Center Set Variable/Get Variable functions enable data persistence within scripts. Variable from spreadsheet reads values from Excel files for batch processing. Use variables to store: tool numbers, feed rates, stock allowances, feature counts, iteration indices. Variables scope to the current script execution — use external files for cross-session persistence. Helper functions manipulate strings, numbers, and lists.", category: "automation", tags: ["hypermill", "automation-center", "variables", "spreadsheet", "batch-processing"], operation_types: ["automation"], confidence: 89, source: "document:hypermill-automation-center", created_at: "2026-04-15", usage_count: 0 },
  { id: "ac-004", title: "Automation Center layer management organizes model data for CAM processing", body: "hyperMILL Automation Center layer functions: Create layer, Delete layer, Set layer visible/invisible, Set layer current, Layer description. Use layers to organize: raw stock, finished model, fixtures, toolpath boundaries, construction geometry. Check layer collision validates that programmed operations do not intersect restricted layers. Delete unused hyperMILL data cleans up orphaned CAM data from failed calculations.", category: "automation", tags: ["hypermill", "automation-center", "layers", "organization", "cleanup"], operation_types: ["automation", "setup"], confidence: 88, source: "document:hypermill-automation-center", created_at: "2026-04-15", usage_count: 0 },

  // --- PDF-learned tips (source: hyperMILL VIRTUAL Machining Center Manual) ---
  { id: "vmc-001", title: "hyperMILL VIRTUAL Machining simulates NC code post-postprocessor for collision detection", body: "hyperMILL VIRTUAL Machining Center simulates actual NC code AFTER postprocessor output, not just toolpaths. This catches collision issues that only appear in posted code (tool changes, rapid moves, retract heights). Simulation includes holder, fixtures, clamps, and tracked stock. Each machining operation is collision-checked before machine movement. Essential for 5-axis work where complex motions create hidden collision risks.", category: "simulation", tags: ["hypermill", "virtual-machining", "nc-code", "collision", "post-simulation"], operation_types: ["simulation", "5axis"], confidence: 94, source: "document:hypermill-vmc", created_at: "2026-04-15", usage_count: 0 },
  { id: "vmc-002", title: "Best Fit calculates optimal part placement within machine workspace", body: "hyperMILL VIRTUAL Machining Best Fit function automatically calculates the optimal placement of a part within available machine workspace. This prevents time-consuming manual setup adjustments when part orientation causes axis travel limit issues. Run Best Fit before posting to ensure all toolpaths execute within machine limits. Critical for large parts on smaller machines or parts with extreme orientations.", category: "setup", tags: ["hypermill", "virtual-machining", "best-fit", "workspace", "axis-limits"], operation_types: ["setup", "5axis"], confidence: 92, source: "document:hypermill-vmc", created_at: "2026-04-15", usage_count: 0 },
  { id: "vmc-003", title: "Axis diagrams reveal unusual traverse movements before machine execution", body: "hyperMILL VIRTUAL Machining axis diagrams display detailed traversing movements of each individual axis over time. Use to identify: unexpected rapid movements, axis reversals, near-limit conditions, excessive rotary unwinding. Unusual patterns often indicate post-processor issues or suboptimal toolpath planning. Correct before transferring program to machine to avoid crashes and premature axis wear.", category: "simulation", tags: ["hypermill", "virtual-machining", "axis-diagram", "analysis", "movement"], operation_types: ["simulation", "analysis"], confidence: 91, source: "document:hypermill-vmc", created_at: "2026-04-15", usage_count: 0 },
  { id: "vmc-004", title: "CONNECTED Machining synchronizes CAM workstation with machine controller bidirectionally", body: "hyperMILL CONNECTED Machining enables real-time bidirectional communication with the machine controller. Before running: zero-point definitions, tool data, and machine parameters are read from controller and validated against CAM data. Machine run only starts if data matches and collision check passes. NC programs load directly to controller memory, preventing mixup and unauthorized access. Use for lights-out production.", category: "automation", tags: ["hypermill", "connected-machining", "synchronization", "controller", "production"], operation_types: ["automation", "production"], confidence: 93, source: "document:hypermill-vmc", created_at: "2026-04-15", usage_count: 0 },
  { id: "vmc-005", title: "VIRTUAL Machining Optimizer calculates best tool positions for multi-axis machining", body: "hyperMILL VIRTUAL Machining Optimizer module calculates technically optimal tool positions that enable efficient traverse movements during 5-axis machining. All selected tool positions are collision-checked, guaranteeing safe machine run. This eliminates programming errors that would require machine-side correction. Use Optimizer when standard indexing produces suboptimal results or near-collision conditions.", category: "multiaxis", tags: ["hypermill", "virtual-machining", "optimizer", "5axis", "tool-position"], operation_types: ["5axis", "multiaxis"], confidence: 92, source: "document:hypermill-vmc", created_at: "2026-04-15", usage_count: 0 },

  // --- PDF-learned tips (source: hyperMILL TOOL Builder Manual) ---
  { id: "tb-001", title: "TOOL Builder prepares 3D tool models from manufacturers for CAM collision checking", body: "hyperMILL TOOL Builder interactively prepares 3D tool models from tool suppliers for use in calculation, collision checking, and simulation. Import 3D data (STEP, IGES), run toolbar steps to label entities (spindle, holder, extension, tool), auto-create appropriate layers. Required for accurate collision detection on tools with non-standard holder geometry. Tool holders with rotationally symmetrical support in spindle are supported.", category: "setup", tags: ["hypermill", "tool-builder", "3d-model", "collision", "holder"], operation_types: ["setup"], confidence: 90, source: "document:hypermill-tool-builder", created_at: "2026-04-15", usage_count: 0 },
  { id: "tb-002", title: "Tool holder positional orientation follows specific conventions per spindle type", body: "hyperMILL TOOL Builder requires correct positional orientation: Z-axis coaxial with main flange pointing toward spindle. BT: X-axis toward center of one slot. SK/CAT: X-axis toward center of flattest slot. HSK: X-axis toward flattest slot on notch side, coupling at flange upper side. CAPTO: Y-axis toward reference notch. KM: Y-axis through hole axis toward recess. Incorrect orientation causes simulation misalignment and false collision reports.", category: "setup", tags: ["hypermill", "tool-builder", "orientation", "bt", "hsk", "capto"], operation_types: ["setup"], confidence: 89, source: "document:hypermill-tool-builder", created_at: "2026-04-15", usage_count: 0 },

  // --- PDF-learned tips (source: Hurco 5-Axis Post Processor Notes) ---
  { id: "5ax-001", title: "M31 rotary axis encoder reset prevents unwinding to zero on Hurco 5-axis", body: "On Hurco 5-axis machines, output M31 early in program header to reset rotary axis encoder to current machine position. Without M31, commanding rotary axes to return to zero degrees can cause 'unwinding' — the axis takes the long way around instead of staying put. Place M31 before any rotary positioning commands. Critical for programs that use rotary indexing or A0 C0 retract positions.", category: "cnc_programming", tags: ["hurco", "5axis", "m31", "encoder", "rotary", "unwinding"], operation_types: ["5axis", "multiaxis"], confidence: 95, source: "document:hurco-5axis-post", created_at: "2026-04-15", usage_count: 0 },
  { id: "5ax-002", title: "M126 Shortest Angular Traverse prevents long rotary moves on Hurco", body: "Hurco M126 (Shortest Angular Traverse) forces the control to take the shortest rotational path to commanded position regardless of sign. Output M126 in program header after safety line. Cancel with M127 when specific rotary direction is required (e.g., winding around a helix). Without M126, a C-axis move from C350 to C10 takes 340 degrees the long way instead of 20 degrees the short way.", category: "cnc_programming", tags: ["hurco", "5axis", "m126", "m127", "rotary", "shortest-path"], operation_types: ["5axis", "multiaxis"], confidence: 94, source: "document:hurco-5axis-post", created_at: "2026-04-15", usage_count: 0 },
  { id: "5ax-003", title: "G68.2 Transform Plane enables tilted workplane machining on Hurco with TCPM", body: "Hurco G68.2 activates Transform Plane (tilted workplane) and enables Tool Center Point Management. Format: G68.2 X Y Z A C where XYZ repositions origin and AC defines tilt angles using ISO rotation conventions. G68.2 does NOT command axis motion — output rotary position separately or use G08.2 (ASR). Cancel with G69. Transform planes can stack; each G69 cancels one in reverse order.", category: "cnc_programming", tags: ["hurco", "5axis", "g68.2", "transform-plane", "tcpm", "tilted-workplane"], operation_types: ["5axis", "multiaxis", "3plus2"], confidence: 93, source: "document:hurco-5axis-post", created_at: "2026-04-15", usage_count: 0 },
  { id: "5ax-004", title: "M128/M129 control TCPM for simultaneous 5-axis on Hurco", body: "Hurco M128 enables Tool Center Point Management (TCPM) for simultaneous 5-axis machining. M129 cancels TCPM. With M128 active, all XYZ coordinates reference the un-rotated workpiece system while the machine compensates for rotary axis position. Position XY and Z to initial point BEFORE M128 to avoid travel limit errors. TCPM is essential for smooth 5-axis surface machining with IJK tool vectors.", category: "cnc_programming", tags: ["hurco", "5axis", "m128", "m129", "tcpm", "simultaneous"], operation_types: ["5axis", "multiaxis"], confidence: 94, source: "document:hurco-5axis-post", created_at: "2026-04-15", usage_count: 0 },
  { id: "5ax-005", title: "M140 retracts tool along current tool vector on Hurco 5-axis", body: "Hurco M140 retracts tool along current tool vector to machine limit position. Add L parameter for specific retract distance: M140 L3.0 retracts 3 inches. Use M140 instead of G28 Z0 after 5-axis moves to avoid collision during retract when tool is tilted. For Z-home in program header, prefer G53 Z0 (machine coordinates) over G91 G28 Z0 to avoid G90/G91 mode confusion.", category: "cnc_programming", tags: ["hurco", "5axis", "m140", "retract", "tool-vector", "safe-retract"], operation_types: ["5axis", "multiaxis"], confidence: 93, source: "document:hurco-5axis-post", created_at: "2026-04-15", usage_count: 0 },
  { id: "5ax-006", title: "G43.4 enables toolpath linearization for 5-axis simultaneous on Hurco", body: "Hurco G43.4 activates toolpath linearization for 5-axis simultaneous machining with IJK tool vectors. Output G43.4 after TCPM (M128) and before cutting moves. The control interpolates smooth 5-axis motion using the IJK direction cosines. Required for machining with tool perpendicular to sculptured surfaces. Without G43.4, 5-axis moves may produce faceted surface finish.", category: "cnc_programming", tags: ["hurco", "5axis", "g43.4", "linearization", "ijk", "simultaneous"], operation_types: ["5axis", "multiaxis"], confidence: 92, source: "document:hurco-5axis-post", created_at: "2026-04-15", usage_count: 0 },
  { id: "5ax-007", title: "DO NOT output G17 plane call in 5-axis program safety line on Hurco", body: "On Hurco 5-axis, avoid G17/G18/G19 plane designation in the program safety line when using Transform Planes or 5-axis simultaneous. Plane codes can conflict with G68.2 Transform Planes and cause unexpected behavior. The traditional G0 G20 G40 G80 G54 G90 safety line is sufficient. Re-establish plane only when returning to 3-axis operations after cancelling transform planes.", category: "cnc_programming", tags: ["hurco", "5axis", "g17", "safety-line", "transform-plane", "best-practice"], operation_types: ["5axis", "multiaxis", "3plus2"], confidence: 91, source: "document:hurco-5axis-post", created_at: "2026-04-15", usage_count: 0 },
  { id: "5ax-008", title: "ISO standard rotary conventions: front/right positive, CCW around Z positive", body: "When programming 5-axis Transform Planes, ISO standard rotary conventions apply: rotations toward front or right side of block are positive, toward back or left are negative. Counter-clockwise (CCW) rotations around Z-axis are positive, clockwise (CW) are negative. Verify machine parameter 'ISO Standard = YES' in Hurco rotary axis settings. Non-ISO machines invert some conventions.", category: "cnc_programming", tags: ["iso", "5axis", "rotary", "conventions", "positive", "negative", "ccw"], operation_types: ["5axis", "multiaxis", "3plus2"], confidence: 90, source: "document:hurco-5axis-post", created_at: "2026-04-15", usage_count: 0 },
];
...
/** Allowed base directories for path validation (TK-MS5 U-TK28 security). */
const ALLOWED_BASE_DIRS = [
  path.resolve(import.meta.dirname, "../.."),  // mcp-server/
  path.resolve(import.meta.dirname, "../../../cad-engine"),  // cad-engine/ (sibling)
  path.resolve("H:/prism"),
  path.resolve(process.cwd()),
].map(p => p.replace(/\\/g, "/"));  // Normalize to forward slashes for Windows

/** Validate that a path stays within allowed base directories. */
function validatePath(p: string, label: string): string {
  const resolved = path.resolve(p).replace(/\\/g, "/");  // Normalize for Windows
  const withinAllowed = ALLOWED_BASE_DIRS.some(base => resolved.startsWith(base));
  if (!withinAllowed) {
    log.warn(`[TribalKnowledge] ${label} path escapes allowed base: ${resolved}`);
    throw new Error(`${label} path must be within allowed project directories`);
  }
  return resolved;
}

/** Path for persisted captured tips (survives server restarts). */
const CAPTURED_TIPS_PATH = (() => {
  const envPath = process.env.PRISM_TRIBAL_TIPS_PATH;
  const defaultPath = path.join(import.meta.dirname, "../../state/tribal_captured_tips.json");
  return validatePath(envPath || defaultPath, "PRISM_TRIBAL_TIPS_PATH");
})();

/** Load previously captured tips from disk. Returns empty array on any error. */
function loadCapturedTips(): KnowledgeTip[] {
  try {
    // TK-MS5 U-TK28: Use try/catch instead of existsSync to avoid TOCTOU race
...
const DOC_KNOWLEDGE_DIR = (() => {
  const envPath = process.env.PRISM_KNOWLEDGE_DIR;
  const defaultPath = path.join(import.meta.dirname, "../../../cad-engine/knowledge_store");
  return validatePath(envPath || defaultPath, "PRISM_KNOWLEDGE_DIR");
})();

/** TK-MS5 U-TK28: Filename sanitization regex — only allow alphanumeric, dash, underscore. */
...
function loadDocumentLearnedTips(): KnowledgeTip[] {
  ...
  try {
    // TK-MS5 U-TK28: Use try/catch instead of existsSync to avoid TOCTOU race
    const allFiles = fs.readdirSync(DOC_KNOWLEDGE_DIR);
    // TK-MS5 U-TK28: Sanitize filenames to prevent directory traversal
    const files = allFiles.filter(f =>
      f.endsWith(".json") &&
      f !== "_registry.json" &&
      SAFE_FILENAME_RE.test(f)
    );
    for (const file of files) {
      try {
        ...
        // Sanitize docId to prevent injection via filename
        const docId = file.replace(".json", "").replace(/[^a-zA-Z0-9_-]/g, "_");
    ...
    // TK-MS5 U-TK28: ENOENT is expected if directory doesn't exist yet
    if (err.code !== "ENOENT") {
...
function loadExtractedTips(): KnowledgeTip[] {
  ...
  try {
    ...
    if (match) {
      // MS1 U-LAT12: Replace new Function() with JSON.parse for RCE prevention
      // Convert TS array literal to JSON: unquoted keys → quoted, single quotes → double
      let jsonStr = match[1]
      ...
      try {
        ...
        log.warn("[TribalKnowledge] Could not parse auto-ingested-tips.ts as JSON, skipping");
        tipsArray = [];
      }
    ...
    // TK-MS5 U-TK28: ENOENT is expected if file doesn't exist yet
    if (err.code !== "ENOENT") {
...
function saveCapturedTips(tips: KnowledgeTip[]): void {
  try {
    ...
    // TK-MS5 U-TK28: Use recursive:true which handles existence check atomically
...
function notifySVITribalChange(tipCount: number): void {
  try {
    ...
    // TK-MS5 U-TK28: Use safeWriteSync for atomic write
    safeWriteSync(sviPath, JSON.stringify(status, null, 2));
  } catch { /* SVI notification is best-effort — ENOENT or other errors are silently ignored */ }
}
...
export class TribalKnowledgeEngine {
  ...
  /**
   * Get top recommendations in "Master Machinist" format.
   * Returns highest-confidence tips with provenance chain for a given context.
   *
   * @param context - Material, machine, operation, tolerance context
   * @returns Top 3 tips with provenance in "Senior machinists say..." format
   */
  masterMachinistRecommend(context: {
    ...
    // Score and rank by: confidence (50%) + usage_count (30%) + evidence_count (20%)
    const scored = candidates.map(tip => {
      ...
      return {
        tip,
        score: confidenceScore + usageScore + evidenceScore,
      };
    ...
    // Format as Master Machinist recommendations
    const recommendations: MasterMachinistTip[] = top3.map(({ tip, score }) => ({
      ...
      provenance: {
        ...
        success_rate: tip.evidence_count ? Math.round((tip.evidence_count / Math.max(tip.usage_count || 1, 1)) * 100) : undefined,
      },
      tip_id: tip.id,
      tip_title: tip.title,
  ...
  // =========================================================================
  // TK-MS7-U32: Natural Language Tribal Query for LLM/PUOA
  // =========================================================================

  /**
   * Query tribal knowledge using natural language intent.
   * Parses the intent to extract material, operation, machine, and category keywords,
   * then returns relevant tips with confidence scores and provenance for LLM consumption.
   *
   * @param intent - Natural language query (e.g., "tips for roughing hardened D2 steel")
   * @returns Structured result with tips, modifiers, and formatted summary
   */
    ...
    return {
      ...
      tips: tips.slice(0, 10).map(t => ({
        ...
        provenance: {
          ...
          captured_by: t.captured_by || "system",
   ...
   * Extracts materials, operations, machines, controllers, and categories.
   */
  private parseNaturalLanguageIntent(intent: string): ParsedTribalIntent {
    ...
    // Determine if query is ambiguous (no clear parameters extracted)
    const ambiguous = !material && !operation && !machine && !category;

    // Extract raw keywords for fallback search
    const stopWords = new Set(["the", "a", "an", "for", "to", "in", "on", "with", "and", "or", "of", "tips", "advice", "help", "how", "what", "when", "why", "best", "good", "any"]);
  ...
  /**
   * Search tips using parsed intent parameters.
   */
  private searchByParsedIntent(parsed: ParsedTribalIntent): (KnowledgeTip & { relevance_score: number })[] {
    ...
    for (const tip of this.tips) {
      ...
      if (parsed.material && tipText.includes(parsed.material.toLowerCase())) {
        score += 30;
      }
      if (parsed.material_iso && tip.material_iso === parsed.material_iso) {
        score += 20;
      }

      // Operation match
      if (parsed.operation && tipText.includes(parsed.operation)) {
        score += 25;
      }

      // Machine/Controller match
      if (parsed.machine && tipText.includes(parsed.machine.toLowerCase())) {
        score += 20;
      }
      if (parsed.controller && tipText.includes(parsed.controller.toLowerCase())) {
        score += 15;
      }

      // Category match
      if (parsed.category && tip.category === parsed.category) {
        score += 20;
      }

      // Keyword matches (fallback)
      for (const kw of parsed.keywords) {
        if (tipText.includes(kw)) {
          score += 5;
        }
      }

      // Boost by tip confidence
      ...
      if (score > 0) {
        results.push({ ...tip, relevance_score: score });
      }
    }
  ...
  /**
   * Extract modifier suggestions from tips for calculation adjustments.
   */
  private extractModifiersFromTips(tips: KnowledgeTip[]): TribalModifier[] {
    ...
    for (const tip of tips.slice(0, 5)) {
      ...
      if (feedMatch) {
        modifiers.push({
          parameter: "feed_rate",
          adjustment: -parseInt(feedMatch[1]) / 100,
          reason: tip.title,
          source_tip_id: tip.id,
          confidence: tip.confidence,
        });
  ...
  /**
   * Extract constraint warnings from tips.
   */
  ...
  /**
   * Extract warning messages from tips.
   */
  private extractWarningsFromTips(tips: KnowledgeTip[]): string[] {
    const warnings: string[] = [];

    for (const tip of tips.slice(0, 5)) {
      if (tip.category === "safety" || tip.knowledge_type === "danger_zone") {
        warnings.push(`[SAFETY] ${tip.title}`);
      }
      if (tip.knowledge_type === "gotcha") {
  ...
  /**
   * Generate clarification suggestions for ambiguous queries.
   */
  private generateClarifications(parsed: ParsedTribalIntent): string[] {
    const suggestions: string[] = [];

    if (!parsed.material) {
      suggestions.push("Specify the material (e.g., 'D2 steel', '6061 aluminum', 'Inconel')");
    }
    if (!parsed.operation) {
      suggestions.push("Specify the operation (e.g., 'roughing', 'finishing', 'drilling')");
    }
    if (!parsed.machine) {
      suggestions.push("Specify the machine or controller (e.g., 'Okuma', 'Haas', 'Fanuc')");
    }
    if (!parsed.category) {
      suggestions.push("Specify the category (e.g., 'speeds/feeds', 'tooling', 'fixturing')");
    }
  ...
  // =========================================================================
  // TK-MS7-U34: LLM Interaction Learning Loop
  // =========================================================================

  /**
   * Capture new tribal knowledge from LLM reasoning interactions.
   * Analyzes reasoning traces to identify parameter overrides, issue
   * identifications, and successful workarounds that should be captured.
   *
   * @param reasoning - LLM reasoning trace with decisions and outcomes
   * @returns Array of capture candidates with auto-categorization
   */
  captureFromLLMReasoning(reasoning: LLMReasoningTrace): CaptureCandidate[] {
    ...
    for (const decision of reasoning.decisions) {
      if (decision.type === "parameter_override" && decision.outcome === "success") {
        const candidate = this.createCaptureCandidate({
  ...
  /**
   * Calculate confidence score for LLM-sourced capture.
   */
  private calculateLLMCaptureConfidence(data: {
    ...
    let confidence = 50; // Base confidence for LLM captures
    ...
    return Math.min(confidence, 90); // Cap at 90% for LLM-sourced
  }

  /**
   * Infer category from LLM capture content.
   */
  private inferCategoryFromLLMContent(input: {
    ...
    if (text.includes("speed") || text.includes("feed") || text.includes("rpm")) {
      return "speeds_feeds";
    }
    if (text.includes("tool") || text.includes("insert") || text.includes("cutter")) {
      return "tooling";
    }
    if (text.includes("fixture") || text.includes("clamp") || text.includes("vise")) {
      return "fixturing";
    }
    if (text.includes("safety") || text.includes("danger") || text.includes("warning")) {
      return "safety";
    }
    if (text.includes("quality") || text.includes("tolerance") || text.includes("inspection")) {
      return "quality";
    }
    if (text.includes("troubleshoot") || text.includes("problem") || text.includes("fix")) {
  ...
  /**
   * Infer domain from LLM capture content.
   */
  private inferDomainFromLLMContent(input: {
    ...
    if (input.context.machine || input.context.controller) {
      return "controller_specific";
    }
    if (text.includes("cam") || text.includes("toolpath") || text.includes("mastercam") || text.includes("fusion")) {
      return "cam_software";
    }
    if (text.includes("safety") || text.includes("danger")) {
      return "safety";
    }
  ...
  /**
   * Infer knowledge type from LLM capture content.
   */
  private inferKnowledgeTypeFromLLMContent(input: {
    ...
    if (input.type === "workaround") {
      return "workaround";
    }
    if (text.includes("never") || text.includes("don't") || text.includes("danger")) {
      return "danger_zone";
    }
    if (text.includes("always") || text.includes("must") || text.includes("required")) {
      return "mandatory_practice";
    }
    if (input.type === "issue_identification") {
      return "gotcha";
    }
  ...
  /**
   * Validate and optionally capture an LLM-sourced tip.
   * Returns the captured tip if validation passes.
   */
  validateAndCaptureLLMTip(candidate: CaptureCandidate): {
    captured: boolean;
    tip_id?: string;
    reason: string;
  } {
    // Check for duplicates
    const existingTips = this.search({
      query: candidate.title,
      limit: 5,
    });

    for (const existing of existingTips) {
      const similarity = this.calculateSimilarity(candidate.body, existing.body);
      if (similarity > 0.8) {
        return {
          captured: false,
          reason: `Similar tip already exists: ${existing.id} (${Math.round(similarity * 100)}% similar)`,
        };
      }
    }
    ...
    if (candidate.confidence < 50) {
      return {
        captured: false,
        reason: `Confidence too low: ${candidate.confidence}% (minimum 50%)`,
      };
    }

    // Capture the tip
    const tip = this.capture({
      title: candidate.title,
      body: candidate.body,
      category: candidate.category,
      tags: candidate.tags,
      confidence: candidate.confidence,
      source: `llm_reasoning:${candidate.capture_type}`,
    ...
    if (tip) {
      return {
        captured: true,
        tip_id: tip.id,
        reason: `Captured as ${tip.id} (confidence: ${candidate.confidence}%)`,
      };
    }

    return {
      captured: false,
      reason: "Capture failed - unknown error",
    };
  }
  ...
  // =========================================================================
  // TK-MS8-U35: Outcome Tracking & Reinforcement Learning
  // =========================================================================

  /** In-memory outcome records (persisted on write). */
  private outcomeRecords: OutcomeRecord[] = [];
  private outcomeRecordsLoaded = false;

  /**
   * Track the outcome of using a tribal tip.
   * Records success/failure to adjust future tip relevance.
   */
  trackTipOutcome(
    tipId: string,
    outcome: TipOutcome,
    ...
    const delta = outcome === "success" ? 2 : outcome === "failure" ? -3 : 0;
    record.confidence_delta = delta;

    this.outcomeRecords.push(record);

    // Update tip usage count and confidence
    const tip = this.tips.find(t => t.id === tipId);
    if (tip) {
      tip.usage_count = (tip.usage_count || 0) + 1;
      // Apply small confidence adjustment
      tip.confidence = Math.max(30, Math.min(98, tip.confidence + delta * 0.5));
    }

    // Persist outcomes
    this.saveOutcomeRecords();
  }

  /**
   * Get tips with positive outcomes in similar contexts (reinforced tips).
   */
  getReinforcedTips(context: OutcomeContext, limit = 10): (KnowledgeTip & { reinforcement: ReinforcementSummary })[] {
    this.loadOutcomeRecords();

    // Build reinforcement summaries
    const summaries = this.buildReinforcementSummaries();

    // Filter tips with positive reinforcement
    const reinforcedTipIds = summaries
      .filter(s => s.success_rate >= 0.6 && s.total_uses >= 2)
      .map(s => s.tip_id);

    // Get matching tips
    const results: (KnowledgeTip & { reinforcement: ReinforcementSummary })[] = [];

    for (const tipId of reinforcedTipIds) {
      const tip = this.tips.find(t => t.id === tipId);
      const summary = summaries.find(s => s.tip_id === tipId);
      if (tip && summary) {
        // Check context similarity
        const contextMatch = this.checkContextMatch(tipId, context);
        if (contextMatch > 0.3) {
          results.push({ ...tip, reinforcement: summary });
        }
      }
    }

    // Sort by success rate × context match
    results.sort((a, b) => b.reinforcement.success_rate - a.reinforcement.success_rate);

    return results.slice(0, limit);
  }

  /**
   * Get reinforcement summary for a specific tip.
   */
  getTipReinforcement(tipId: string): ReinforcementSummary | null {
    this.loadOutcomeRecords();
    const summaries = this.buildReinforcementSummaries();
    return summaries.find(s => s.tip_id === tipId) || null;
  }

  /**
   * Build reinforcement summaries from outcome records.
   */
  private buildReinforcementSummaries(): ReinforcementSummary[] {
    const byTip = new Map<string, OutcomeRecord[]>();

    for (const record of this.outcomeRecords) {
      if (!byTip.has(record.tip_id)) {
        byTip.set(record.tip_id, []);
      }
      byTip.get(record.tip_id)!.push(record);
    }

    const summaries: ReinforcementSummary[] = [];

    for (const [tipId, records] of byTip) {
      ...
      const successRate = totalUses > 0 ? (successCount + partialCount * 0.5) / totalUses : 0;
      ...
      if (recentSuccessRate > olderSuccessRate + 0.15) trend = "improving";
      else if (recentSuccessRate < olderSuccessRate - 0.15) trend = "declining";

      // Calculate confidence adjustment
      const confidenceAdj = (successRate - 0.5) * 10; // -5 to +5
  ...
  /**
   * Check how well a tip's past outcomes match a given context.
   */
  private checkContextMatch(tipId: string, context: OutcomeContext): number {
    const tipRecords = this.outcomeRecords.filter(r => r.tip_id === tipId);
    if (tipRecords.length === 0) return 0;
    ...
    for (const record of tipRecords) {
      ...
      if (context.material && record.context.material) {
        factors++;
        if (record.context.material.toLowerCase().includes(context.material.toLowerCase()) ||
            context.material.toLowerCase().includes(record.context.material.toLowerCase())) {
          matchScore++;
        }
      }
      if (context.operation && record.context.operation) {
        factors++;
        if (record.context.operation === context.operation) matchScore++;
      }
      if (context.machine && record.context.machine) {
        factors++;
        if (record.context.machine === context.machine) matchScore++;
      }

      if (factors > 0) {
        totalMatch += matchScore / factors;
      }
    }

    return tipRecords.length > 0 ? totalMatch / tipRecords.length : 0;
  }

  /**
   * Load outcome records from persistent storage.
   */
  private loadOutcomeRecords(): void {
    if (this.outcomeRecordsLoaded) return;

    try {
      const outcomesPath = path.join(path.dirname(CAPTURED_TIPS_PATH), "tribal-outcomes.json");
      const content = fs.readFileSync(outcomesPath, "utf-8");
      this.outcomeRecords = JSON.parse(content);
    } catch {
      // No existing outcomes file
      this.outcomeRecords = [];
    }
    this.outcomeRecordsLoaded = true;
  }

  /**
   * Save outcome records to persistent storage.
   */
  private saveOutcomeRecords(): void {
    try {
      const outcomesPath = path.join(path.dirname(CAPTURED_TIPS_PATH), "tribal-outcomes.json");
      safeWriteSync(outcomesPath, JSON.stringify(this.outcomeRecords, null, 2));
    } catch (err) {
      log.warn("[TribalKnowledge] Failed to save outcome records:", err);
    }
  }

  // =========================================================================
  // TK-MS8-U36: Confidence Calibration Engine
  // =========================================================================

  /**
   * Calibrate tip confidence based on usage outcomes.
   * Uses Bayesian-style update: prior confidence + outcome evidence.
   */
  calibrateTipConfidence(tipId: string): number {
    const tip = this.tips.find(t => t.id === tipId);
    if (!tip) return 0;

    const reinforcement = this.getTipReinforcement(tipId);
    if (!reinforcement || reinforcement.total_uses < 2) {
      // Not enough evidence to calibrate
      return tip.confidence;
    }

    // Bayesian-style update
    const priorConfidence = tip.confidence;
    const evidenceWeight = Math.min(reinforcement.total_uses / 10, 1); // Max weight at 10 uses
    const outcomeConfidence = reinforcement.success_rate * 100;

    // Weighted combination
    const calibrated = priorConfidence * (1 - evidenceWeight * 0.5) +
                       outcomeConfidence * (evidenceWeight * 0.5);

    // Apply bounds
    const bounded = Math.max(30, Math.min(98, calibrated));

    // Update the tip
    tip.confidence = bounded;

    return bounded;
  }

  /**
   * Calibrate all tips and return a report.
   */
  calibrateAllTips(): CalibrationEntry[] {
    this.loadOutcomeRecords();
    const summaries = this.buildReinforcementSummaries();
    const entries: CalibrationEntry[] = [];

    for (const summary of summaries) {
      if (summary.total_uses < 2) continue;

      const tip = this.tips.find(t => t.id === summary.tip_id);
      if (!tip) continue;

      const originalConfidence = tip.confidence;
      const calibratedConfidence = this.calibrateTipConfidence(summary.tip_id);
      const delta = calibratedConfidence - originalConfidence;

      if (Math.abs(delta) > 1) {
        let reason: string;
        if (delta > 0) {
          reason = `Success rate ${(summary.success_rate * 100).toFixed(0)}% (${summary.success_count}/${summary.total_uses})`;
        } else {
          reason = `Failure rate ${((1 - summary.success_rate) * 100).toFixed(0)}% (${summary.failure_count}/${summary.total_uses})`;
        }
        ...
        entries.push({
          ...
          delta,
          reason,
          evidence_count: summary.total_uses,
        });
      }
    }
  ...
  /**
   * Get tips with significant confidence drops (potential issues).
   */
  getFlaggedTips(threshold = -5): CalibrationEntry[] {
    const calibration = this.calibrateAllTips();
    return calibration.filter(e => e.delta <= threshold);
  }

  // =========================================================================
  // TK-MS8-U37: Proactive Tribal Suggestions
  // =========================================================================

  /** Cooldown tracking for recently shown suggestions. */
  private suggestionCooldowns = new Map<string, Date>();

  /**
   * Generate proactive tribal suggestions based on context.
   * Returns tips that should be shown before user asks.
   */
  ...
  ): ProactiveTribalSuggestion[] {
    const maxSuggestions = options?.maxSuggestions ?? 5;
    const minConfidence = options?.minConfidence ?? 70;
    const cooldownMinutes = options?.cooldownMinutes ?? 30;

    const suggestions: ProactiveTribalSuggestion[] = [];
    const now = new Date();

    // Build context query
    const contextParts: string[] = [];
    if (context.material) contextParts.push(context.material);
    if (context.operation) contextParts.push(context.operation);
    if (context.machine) contextParts.push(context.machine);
    if (context.controller) contextParts.push(context.controller);

    if (contextParts.length === 0) {
      return []; // No context, can't suggest
    }

    // Search for matching tips
    const query = contextParts.join(" ");
    const matchingTips = this.queryTribalNaturalLanguage(query);

    for (const tip of matchingTips.tips) {
      // Check confidence threshold
      if (tip.confidence < minConfidence) continue;

      // Check cooldown
      const cooldownEnd = this.suggestionCooldowns.get(tip.id);
      ...
      if (isSafety) {
        priority = "critical";
      } else if (tip.confidence >= 90) {
        priority = "high";
      } else if (tip.confidence >= 80) {
        priority = "medium";
      } else {
        priority = "low";
      }
      ...
      let reason: string;
      if (isSafety) {
        reason = `Safety tip for ${context.operation || "operation"}`;
      } else if (context.material && tip.body.toLowerCase().includes(context.material.toLowerCase())) {
        reason = `Relevant for ${context.material}`;
      } else if (context.operation) {
        reason = `Recommended for ${context.operation}`;
      } else {
        reason = `Based on context match`;
      }
      ...
      // Set cooldown
      const cooldownTime = new Date(now.getTime() + cooldownMinutes * 60000);
      this.suggestionCooldowns.set(tip.id, cooldownTime);

      if (suggestions.length >= maxSuggestions) break;
    }

    // Sort: safety first, then by priority and relevance
    suggestions.sort((a, b) => {
      if (a.is_safety !== b.is_safety) return a.is_safety ? -1 : 1;
  ...
  /**
   * Clear suggestion cooldowns (for testing or reset).
   */
  clearSuggestionCooldowns(): void {
    this.suggestionCooldowns.clear();
  }

  /**
   * Get proactive suggestions integrated with PUOA context.
   */
  getProactiveSuggestionsForPUOA(
    material?: string,
    operation?: string,
    machine?: string
  ): ProactiveTribalSuggestion[] {
    return this.generateProactiveSuggestions({
      material,
      operation,
      machine,
    });
