...
import { JMDieScanLedgerEngine, canonScanPath, type AppendInput } from "./JMDieScanLedgerEngine.js";
...
export class JMDieScanCoordinatorEngine {
  ...
  plan(opts: PlanOptions): PlanSummary {
    ...
    // CANONICAL membership keys (forward-slash + lowercase). The ledger holds
    // "H:/prism/..." rows while the walker emits "H:\PRISM\..." absolutes -- a raw
    // Set never intersected, so to_scan == walked_files (the 2026-07-02 full-corpus
    // re-scan bug). canonScanPath is shared with the ledger engine so planner and
    // recorder can never drift into different key formats again.
    const alreadyScanned = new Set<string>();
    for (const p of ledgerPaths) alreadyScanned.add(canonScanPath(p));
    if (opts.extraAlreadyScanned) {
      for (const p of opts.extraAlreadyScanned) alreadyScanned.add(canonScanPath(p));
    }
    ...
    for (const r of ingest.records) {
      if (alreadyScanned.has(canonScanPath(r.abs_path))) continue;
      delta.push(r);
    }
