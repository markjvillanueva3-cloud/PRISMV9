...
export interface WireEDMProgram {
  ...
  /** H-variable declarations: H1=0.0085, H175=0.0000, etc. */
  offset_declarations: Record<number, number>;
  /** Adaptive control detected (M90/M91) */
  hasAdaptiveControl: boolean;
  /** Tank fill/drain detected (M78/M58) */
...
const DIALECT_MARKERS: Record<WireEDMDialect, RegExp[]> = {
  mitsubishi: [
    /\bE\d{4}\b/,          // E-code condition (E1221, E2821, E0100, etc.)
    /\bM20\b/,             // Thread wire (Mitsubishi specific)
    /\bM21\b/,             // Cut wire (Mitsubishi specific)
    /\bM78\b/,             // Fill tank (Mitsubishi specific)
    /\bM58\b/,             // Drain tank (Mitsubishi specific)
    /\bM90\b/,             // Adaptive control on (Mitsubishi specific)
    /H\d+\s*=.*\+\s*H/,   // H-variable declarations (H1=.0085 + H175)
    /G51\s+W/,             // Taper with W-axis
    /MITSUBISHI/i,
  ],
  fanuc: [
    /E000[1-5]\b/,         // E0001-E0005 condition codes
    /\bH\d{1,3}\b/,        // H-register wire offset (H01, H1, H021)
...
export class WireEDMProgramParserEngine {
  ...
  private _extractTaper(lines: string[], program: WireEDMProgram): void {
    for (let i = 0; i < lines.length; i++) {
      ...
      // G51 with U or W: explicit taper command
      ...
      // Inline UV taper: G1 X... Y... U... V... (Mitsubishi M800 4-axis taper)
      // If U or V axis values are non-zero on cutting moves, it's a taper program
      if (!program.hasTaper) {
        const beforeComment = line.split("(")[0];
        if (/\bG0?1\b/.test(beforeComment)) {
          const uMatch = beforeComment.match(/U([+-]?[\d.]+)/);
          const vMatch = beforeComment.match(/V([+-]?[\d.]+)/);
          if (uMatch || vMatch) {
            const uVal = uMatch ? parseFloat(uMatch[1]) : 0;
            const vVal = vMatch ? parseFloat(vMatch[1]) : 0;
            if (Math.abs(uVal) > 0.0001 || Math.abs(vVal) > 0.0001) {
              program.taper.enabled = true;
              program.taper.taper_on_line = i + 1;
              program.hasTaper = true;
            }
  ...
  private _extractFlushing(lines: string[], program: WireEDMProgram): void {
    for (let i = 0; i < lines.length; i++) {
      ...
      // M80: upper flush on / water on (Mitsubishi)
      if (/\bM80\b/.test(line)) {
      ...
      // M81: lower flush on / fluid off (Mitsubishi)
      if (/\bM81\b/.test(line)) {
      ...
      // M82/M83: wire on/off (Mitsubishi) or flush off (other)
      if (/\bM82\b/.test(line) || /\bM83\b/.test(line)) {
        program.flushing.flush_off_line = i + 1;
      }

      // Submerged cutting detection (M78 = fill tank in Mitsubishi)
      if (/SUBMERGE|DIELECTRIC\s+FILL|TANK\s+FILL/i.test(line) || /\bM78\b/.test(line)) {
  ...
  /**
   * Extract H-variable offset declarations (Mitsubishi style: H1=.0085 + H175)
   */
  private _extractOffsetDeclarations(lines: string[], program: WireEDMProgram): void {
    for (const line of lines) {
      const trimmed = line.trim();
      // Match: H1 =.0085 + H175  or  H175 = 0.0000  or  H2 =0. + H175
      const declMatch = trimmed.match(/^H(\d+)\s*=\s*([+-]?[\d.]+)/);
      if (declMatch) {
        const reg = parseInt(declMatch[1], 10);
        const val = parseFloat(declMatch[2]);
        program.offset_declarations[reg] = val;
      }
    }
  }

  /**
   * Extract Mitsubishi-specific features: M20/M21 (thread/cut wire),
   * M78/M58 (tank fill/drain), M90/M91 (adaptive control)
   */
  private _extractMitsubishiSpecific(lines: string[], program: WireEDMProgram): void {
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i].trim();

      // M20: Thread wire (Mitsubishi)
      if (/\bM20\b/.test(line)) {
        program.wire_settings.has_auto_thread = true;
        if (program.wire_settings.thread_line === null) {
          program.wire_settings.thread_line = i + 1;
        }
      }

      // M21: Cut wire (Mitsubishi)
      if (/\bM21\b/.test(line)) {
        program.wire_settings.wire_stop_line = i + 1;
      }

      // M82: Wire on (Mitsubishi)
      if (/\bM82\b/.test(line) && /WIRE\s+ON/i.test(line)) {
        program.wire_settings.wire_run_on = true;
        if (program.wire_settings.wire_start_line === null) {
          program.wire_settings.wire_start_line = i + 1;
        }
      ...
      // M90/M91: Adaptive control on/off (Mitsubishi)
      if (/\bM90\b/.test(line)) {
        program.hasAdaptiveControl = true;
      }

      // M78: Fill tank, M58: Drain tank (Mitsubishi)
      if (/\bM78\b/.test(line) || /\bM58\b/.test(line)) {
        program.hasTankManagement = true;
      }
    }
  }
  ...
  private _extractMovesAndPasses(lines: string[], program: WireEDMProgram): void {
    ...
    // Real Mitsubishi programs use E#### codes (E1221, E2821, etc.) with embedded PASS comments
    const conditionPatterns: RegExp[] = [
      /\b(E\d{4})\b/,                      // Any 4-digit E-code (Mitsubishi/Fanuc)
      /\b(C[1-5]00)\b/,                    // Sodick C-codes
      /\b(C000[1-5])\b/,                   // Mitsubishi C-codes
    ];

    /**
     * Determine pass phase from condition code and sequential pass count.
     * Real Mitsubishi programs use technology table E-codes (E1221 = material group 12, condition 21).
     * The pass number is determined by the (PASS=N) comment or by sequential order.
     */
    const phaseFromPassNumber = (passNum: number, totalPasses: number): WireEDMPass["phase"] => {
      if (passNum === 1) return "rough";
      if (passNum === 2) return "skim1";
      if (passNum === 3) return "skim2";
      if (passNum === 4) return "skim3";
      if (passNum >= 5) return "skim4";
      return "unknown";
    };

    const phaseFromCondition = (code: string, passNum: number): WireEDMPass["phase"] => {
      ...
      // Mitsubishi canonical: E0100=rough, E0200-500=skim
      ...
      // Real Mitsubishi technology table E-codes (E1221, E2821, etc.):
      // phase is determined by sequential pass number
      return phaseFromPassNumber(passNum, 0);
    };

    const finishPass = (endLine: number) => {
      if (passStartLine >= 0 && passMoveCount > 0) {
        // Resolve offset value from declarations
        const offsetValue = currentOffset !== null && program.offset_declarations[currentOffset] !== undefined
          ? program.offset_declarations[currentOffset]
          : null;
        program.passes.push({
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Also extract H-register and F-feed from same line (Mitsubishi: E1221 H1 F.12)
      for (const pat of conditionPatterns) {
        ...
        if (condMatch) {
          ...
          currentPhase = phaseFromCondition(currentCondition, currentPass);

          // Extract H-register from E-code line (Mitsubishi: E1221 H1 F.12)
          const hMatch = line.match(/\bH(\d+)/);
          if (hMatch) {
            const hReg = parseInt(hMatch[1], 10);
            currentOffset = hReg;
            // Look up the declared offset value
            if (program.offset_declarations[hReg] !== undefined) {
              currentFeed = null; // reset
            }
          }

          // Extract feed rate from E-code line (F.12 = 0.12)
      ...
      // Wire offset: G41/G42 (with or without D/H register on same line)
      const offsetMatch = line.match(/\bG4([12])\b/);
      if (offsetMatch) {
        currentOffsetDir = offsetMatch[1] === "1" ? "left" : "right";
        // Check for D/H register on same line (e.g., G42 H1, G41 D02)
        const dhMatch = line.match(/\bG4[12]\s+[DH](\d+)/);
        if (dhMatch) {
          currentOffset = parseInt(dhMatch[1], 10);
        }
        if (passStartLine < 0) {
      ...
      // Motion commands
      // Match G0-G3 or G00-G03 (real programs use both formats)
      const motionMatch = line.match(/\b(G0?[0-3])\b/);
      if (motionMatch) {
        const gCode = motionMatch[1];
        const type: WireEDMMove["type"] =
          (gCode === "G00" || gCode === "G0") ? "rapid" :
          (gCode === "G01" || gCode === "G1") ? "linear" :
          (gCode === "G02" || gCode === "G2") ? "cw_arc" : "ccw_arc";
  ...
  private _validateSafety(lines: string[], program: WireEDMProgram): void {
    ...
    // M51 (Fanuc/Sodick) or M21 (Mitsubishi) = wire stop/cut
    program.safety.has_wire_stop = /\bM51\b|\bM21\b/.test(content);
    program.safety.has_offset_cancel = /\bG40\b/.test(content);
    // For inline UV taper programs, no G50 needed — taper ends with program
    program.safety.has_taper_cancel = program.hasTaper
      ? (/\bG50\b/.test(content) || program.taper.angle_deg === null) : true;
    // M81 (Mitsubishi fluid off) or M82/M83 in off-context
    program.safety.has_flush_off = program.flushing.upper_on || program.flushing.lower_on
      ? /\bM81\b|\bM82\b|\bM83\b/.test(content) : true;
