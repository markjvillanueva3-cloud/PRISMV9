...
import { capacityPlanningEngine, type MachineLoad } from "./CapacityPlanningEngine.js";
...
eventBus.registerAction("update_capacity_forecast", async (params) => {
  try {
    ...
    const loads = capacityPlanningEngine.getAllMachineLoads(1);  // 1 week forward
    const machineLoad = loads.find((l: MachineLoad) => l.machine_id === machine_id);
    ...
    // Get updated load
    const newLoads = capacityPlanningEngine.getAllMachineLoads(1);  // 1 week forward
    const newMachineLoad = newLoads.find((l: MachineLoad) => l.machine_id === machine_id);
    ...
    await eventBus.publishTyped({
      ...
      source: "cycle_scheduling_bridge",
      payload: capacityPayload as Record<string, unknown>,
    });
...
eventBus.registerAction("reoptimize_schedule", async (params) => {
  try {
    ...
    // Get current job assignments for the affected machine
    // Note: SchedulingEngine.optimize returns results per strategy
    const optimizeResult = schedulingEngine.optimize([], []);
    const bestResult = optimizeResult.results[optimizeResult.best_strategy];
    const affectedJobs = bestResult?.assignments?.filter((a: { machine_id: string }) => a.machine_id === machine_id) ?? [];
    ...
    await eventBus.publishTyped({
      ...
      source: "cycle_scheduling_bridge",
      payload: schedulePayload as Record<string, unknown>,
    });
