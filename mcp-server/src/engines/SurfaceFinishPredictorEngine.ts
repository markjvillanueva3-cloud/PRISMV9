 ...
 *
 * Models:
 * - Brammertz: Ra = f²/(32R) + re/2 (theoretical roughness from feed & geometry)
 * - Scallop height: h = R - √(R² - (ae/2)²) for ball; ae²/(8R) for flat end
 * - Waviness: Wt from vibration amplitude, tool deflection, thermal drift
 * - Composite: combines kinematic roughness + dynamic effects
 *
 * References:
 * - Brammertz (1961): Kinematic surface roughness model
 * - Whitehouse (2002): Handbook of Surface & Nanometrology
 * - Benardos & Vosniakos (2003): Predicting surface roughness in machining (review)
 */
...
class SurfaceFinishPredictorEngine {
  ...
  wavinessEstimate(ap_mm: number, ae_mm: number, rpm: number, feed_mmmin: number, toolDia_mm: number): number {
    ...
    const stiffProxy = Math.pow(toolDia_mm, 4) / 64;
