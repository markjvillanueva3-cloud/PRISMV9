...
export class HoningProcessEngine {
  ...
  designHoningProcess(input: HoningDesignInput): HoningDesignResult {
    ...
    const Vc_rotation = Math.PI * input.bore_diameter_mm * rotation_rpm / 1000; // mm/min
    ...
    // Cycle time estimate based on Preston's equation
    // MRR proportional to pressure × velocity × machinability
    const stockToRemove = Math.max(0.005, (input.initial_Ra_um - input.target_Ra_um) * 0.005); // mm
    const mrrFactor = expansion_pressure * targetVc * machinability * 0.0001; // mm/min empirical
