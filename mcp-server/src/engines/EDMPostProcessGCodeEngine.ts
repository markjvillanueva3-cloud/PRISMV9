...
export interface RestartMarker {
  ...
  label: string;
  /** Recommended backup distance (mm) if wire breaks near this marker */
  backup_mm?: number;
}
...
function emitRestartMarker(
  ...
  // Wire break recovery info comment (if backup distance provided)
  if (backupMm !== undefined) {
    markerLines.push(buildComment(cfg, `BREAK RECOVERY: BACKUP ${backupMm.toFixed(1)}mm, RE-THREAD ${cfg.thread_code}, RETRY MAX 3`));
  }
  return {
    lines: markerLines,
    marker: { n_block: nBlock, profile_index: profileIdx, pass_index: passIdx, label, backup_mm: backupMm },
  };
}
...
function emitInterProfileThreading(
  ...
  result.push(`${buildLineNumber(cfg, lineNum)} ${cfg.rapid_code} X${formatCoord(nextStartHole.x, dp, cs)} Y${formatCoord(nextStartHole.y, dp, cs)} ${buildComment(cfg, "RAPID TO NEXT START HOLE")}`);
  lineNum += lineInc;
...
function formatCoord(value: number, decimals: number, scale = 1.0): string {
  return (value * scale).toFixed(decimals);
}

/** Returns coordinate scale factor: 1.0 for metric, 1/25.4 for imperial */
function coordScale(units?: "metric" | "imperial"): number {
  return units === "imperial" ? 1.0 / 25.4 : 1.0;
}

/** Returns feed rate scale: 1.0 for metric (mm/min), 1/25.4 for imperial (IPM) */
function feedScale(units?: "metric" | "imperial"): number {
  return units === "imperial" ? 1.0 / 25.4 : 1.0;
}

/** Returns G20 for imperial, G21 for metric */
function unitCode(units?: "metric" | "imperial"): string {
  return units === "imperial" ? "G20" : "G21";
}

/** Returns unit label for comments */
...
function buildMoveLine(
  ...
  if (pt.type === "arc" && pt.i !== undefined && pt.j !== undefined) {
    ...
    return `${buildLineNumber(cfg, lineNum)} ${arcCode} X${formatCoord(pt.x, dp, cs)} Y${formatCoord(pt.y, dp, cs)} I${formatCoord(pt.i, dp, cs)} J${formatCoord(pt.j, dp, cs)}`;
  }
  if (pt.type === "arc" && (pt as MovePoint).direction !== undefined) {
    ...
    return `${buildLineNumber(cfg, lineNum)} ${arcCode} X${formatCoord(pt.x, dp, cs)} Y${formatCoord(pt.y, dp, cs)}`;
  }
  const moveCode = pt.type === "rapid" ? cfg.rapid_code : cfg.linear_code;
  return `${buildLineNumber(cfg, lineNum)} ${moveCode} X${formatCoord(pt.x, dp, cs)} Y${formatCoord(pt.y, dp, cs)}`;
}
...
function generateFanucGCode(input: EDMGCodeInput): EDMGCodeResult {
  ...
  const dp = input.units === "imperial" ? cfg.decimal_places + 1 : cfg.decimal_places;
  ...
  lineNum += 10;
  lines.push(`${buildLineNumber(cfg, lineNum)} G90 ${unitCode(input.units)} ${buildComment(cfg, `ABSOLUTE, ${unitLabel(input.units)}`)}`);
  lineNum += 10;
  ...
  for (let pi = 0; pi < input.profiles.length; pi++) {
    ...
    if (pi > 0) {
      const threading = emitInterProfileThreading(cfg, profile.start_hole, lineNum, 10, dp, cs);
    ...
    for (let passIdx = 0; passIdx < input.passes.length; passIdx++) {
      ...
      const rm = emitRestartMarker(cfg, pi, passIdx, passType, isRough ? 2.5 : 2.0);
      lines.push(...rm.lines);
      ...
      if (passIdx === 0 || pi > 0) {
        lines.push(`${buildLineNumber(cfg, lineNum)} ${cfg.rapid_code} X${formatCoord(profile.start_hole.x, dp, cs)} Y${formatCoord(profile.start_hole.y, dp, cs)} ${buildComment(cfg, "RAPID TO START HOLE")}`);
        lineNum += 10;
      ...
      if (profile.contour_points.length > 0) {
        ...
        for (const pt of approachPts) {
          lines.push(buildMoveLine(cfg, lineNum, pt, dp, cs));
          lineNum += 10;
        }
      }

      // Contour cutting — G01 for lines, G02/G03 for arcs; UV computed per move for taper
      ...
      for (let ci = 0; ci < profile.contour_points.length; ci++) {
        ...
        if (tabPositions.has(ci)) {
          ...
          lines.push(`${buildLineNumber(cfg, lineNum)} ${cfg.rapid_code} X${formatCoord(nextPt.x, dp, cs)} Y${formatCoord(nextPt.y, dp, cs)} ${buildComment(cfg, "SKIP TAB")}`);
          lineNum += 10;
          continue;
        }

        // Compute UV offsets for taper mode (per-block incremental UV)
        const prevPt = ci > 0 ? profile.contour_points[ci - 1] : profile.start_hole;
        const uv = profileHasTaper ? computeUVOffsets(prevPt, pt, taperDeg, guideDist) : { u: 0, v: 0 };
        if (profileHasTaper) {
          const uvWarn = validateUVTravel(uv.u, uv.v, uvLimit);
          if (uvWarn) warnings.push(uvWarn);
        }

        // Build move with UV words for taper
        if (pt.type === "arc" && pt.i !== undefined && pt.j !== undefined) {
          const arcCode = pt.direction === "cw" ? cfg.cw_arc_code : cfg.ccw_arc_code;
          if (profileHasTaper) {
            lines.push(`${buildLineNumber(cfg, lineNum)} ${arcCode} X${formatCoord(pt.x, dp, cs)} Y${formatCoord(pt.y, dp, cs)} I${formatCoord(pt.i, dp, cs)} J${formatCoord(pt.j, dp, cs)} U${formatCoord(uv.u, dp, cs)} V${formatCoord(uv.v, dp, cs)}`);
          } else {
            lines.push(`${buildLineNumber(cfg, lineNum)} ${arcCode} X${formatCoord(pt.x, dp, cs)} Y${formatCoord(pt.y, dp, cs)} I${formatCoord(pt.i, dp, cs)} J${formatCoord(pt.j, dp, cs)}`);
          }
        } else {
          if (profileHasTaper) {
            lines.push(`${buildLineNumber(cfg, lineNum)} ${cfg.linear_code} X${formatCoord(pt.x, dp, cs)} Y${formatCoord(pt.y, dp, cs)} U${formatCoord(uv.u, dp, cs)} V${formatCoord(uv.v, dp, cs)}`);
          } else {
            lines.push(buildMoveLine(cfg, lineNum, { ...pt, type: pt.type === "arc" ? "arc" : "linear" } as MovePoint, dp, cs));
          }
        }
        lineNum += 10;
        if (ci > 0) {
      ...
      if (profile.contour_points.length >= 2) {
        ...
        for (const dpt of depPts) {
          lines.push(buildMoveLine(cfg, lineNum, dpt, dp, cs));
          lineNum += 10;
        }
      }
      ...
      // Reset UV axes to zero (taper off — no G50 needed, UV=0 implies no taper)
      if (profileHasTaper) {
        lines.push(`${buildLineNumber(cfg, lineNum)} G1 U0. V0. ${buildComment(cfg, "RESET UV - TAPER OFF")}`);
        lineNum += 10;
      }
    ...
    if (profile.tabs && profile.tabs.length > 0) {
      ...
      for (const tab of profile.tabs) {
        if (tab.position_index < profile.contour_points.length) {
          ...
          lineNum += 10;
          lines.push(`${buildLineNumber(cfg, lineNum)} ${cfg.rapid_code} X${formatCoord(tabPt.x - 2, dp, cs)} Y${formatCoord(tabPt.y, dp, cs)}`);
          lineNum += 10;
          ...
          lineNum += 10;
          lines.push(`${buildLineNumber(cfg, lineNum)} ${cfg.linear_code} X${formatCoord(tabPt.x + tab.width_mm, dp, cs)} Y${formatCoord(tabPt.y, dp, cs)}`);
          lineNum += 10;
...
function generateSodickGCode(input: EDMGCodeInput): EDMGCodeResult {
  ...
  for (let pi = 0; pi < input.profiles.length; pi++) {
    ...
    for (let passIdx = 0; passIdx < input.passes.length; passIdx++) {
      ...
      // Contour — G01 for lines, G02/G03 for arcs; tabs retained on ALL passes; UV per move for taper
      const tabPositions = new Set((profile.tabs ?? []).map(t => t.position_index));
      for (let ci = 0; ci < profile.contour_points.length; ci++) {
        const pt = profile.contour_points[ci];
        if (tabPositions.has(ci)) {
          ...
          const nextIdx = Math.min(ci + 1, profile.contour_points.length - 1);
          const nextPt = profile.contour_points[nextIdx];
          lines.push(`${buildLineNumber(cfg, lineNum)} ${cfg.rapid_code} X${formatCoord(nextPt.x, dp, cs)} Y${formatCoord(nextPt.y, dp, cs)}`);
          lineNum += 10;
          continue;
        }
...
function generateMakinoGCode(input: EDMGCodeInput): EDMGCodeResult {
  ...
  for (let pi = 0; pi < input.profiles.length; pi++) {
    ...
    for (let passIdx = 0; passIdx < input.passes.length; passIdx++) {
      ...
      // Contour — G01/G02/G03; tabs retained on ALL passes; UV per move for taper
      const tabPositions = new Set((profile.tabs ?? []).map(t => t.position_index));
      for (let ci = 0; ci < profile.contour_points.length; ci++) {
        const pt = profile.contour_points[ci];
        if (tabPositions.has(ci)) {
...
function generateMitsubishiGCode(input: EDMGCodeInput): EDMGCodeResult {
  ...
  lines.push(cfg.program_start);
  // Mitsubishi programs always use L001 in the NC body.
  // The program_number is the FILE NAME on the machine controller, not embedded in the body.
  // Real shop data: ITW SHAKEPROOF (Mar 2022) = L001, NOZE TEST (May 2022) = L001.
  lines.push("L001");
  ...
  lineNum += LINE_INC;
  lines.push(`${buildLineNumber(cfg, lineNum)} G1 X${formatCoord(0, dp, cs)} Y${formatCoord(0, dp, cs)} F${(25.0 * feedScale(input.units)).toFixed(4)}`);
  lineNum += LINE_INC;
  ...
  for (let pi = 0; pi < input.profiles.length; pi++) {
    ...
    if (pi > 0) {
      ...
      lineNum += LINE_INC;
      // Rapid to next start hole
      lines.push(`${buildLineNumber(cfg, lineNum)} G0 X${formatCoord(profile.start_hole.x, dp, cs)} Y${formatCoord(profile.start_hole.y, dp, cs)}`);
      lineNum += LINE_INC;
      // Re-thread wire
    ...
    for (let passIdx = 0; passIdx < input.passes.length; passIdx++) {
      ...
      // U-WGAP10: Safety verification before tank fill
      lines.push(buildComment(cfg, "VERIFY: Tank valve AUTO, dielectric level OK, interlocks engaged"));
      ...
      lineNum += LINE_INC;
      // U-WGAP10: Safety verification before power on
      lines.push(buildComment(cfg, "VERIFY: Wire path clear, workpiece clamped, door closed"));
      ...
      // Wire comp alternation per skim pass for better surface finish:
      //   Pass 1 (rough):  G42 (internal) or G41 (external) — standard direction
      //   Pass 2 (skim 1): same direction
      //   Pass 3 (skim 2): REVERSE direction for error averaging
      //   Pass 4 (skim 3): back to standard direction
      //   Pattern: G42/G42/G41/G42 for internal (matches real Mitsubishi programs)
      const useReverse = passIdx % 2 === 0 && passIdx >= 2; // Pass 3, 5 reverse
      ...
      if (pass.offset_mm > 0 && cutPoints.length > 0) {
        ...
        if (approachPts.length > 0 && approachPts[0].type === "rapid") {
          lines.push(`${buildLineNumber(cfg, lineNum)} G0 X${formatCoord(approachPts[0].x, dp, cs)} Y${formatCoord(approachPts[0].y, dp, cs)}`);
          lineNum += LINE_INC;
        }
        ...
        for (let ai = 1; ai < approachPts.length; ai++) {
          ...
          if (apt.type === "arc" && apt.i !== undefined && apt.j !== undefined) {
            ...
            lines.push(`${buildLineNumber(cfg, lineNum)} ${arcDir} X${formatCoord(apt.x, dp, cs)} Y${formatCoord(apt.y, dp, cs)} I${formatCoord(apt.i, dp, cs)} J${formatCoord(apt.j, dp, cs)}`);
          } else {
            const prefix = !compEmitted ? `${compCode} ` : "";
            compEmitted = true;
            lines.push(`${buildLineNumber(cfg, lineNum)} ${prefix}G1 X${formatCoord(apt.x, dp, cs)} Y${formatCoord(apt.y, dp, cs)}`);
          }
          lineNum += LINE_INC;
        }
        ...
        if (!compEmitted) {
          lines.push(`${buildLineNumber(cfg, lineNum)} ${compCode} G1 X${formatCoord(firstPt.x, dp, cs)} Y${formatCoord(firstPt.y, dp, cs)}`);
          lineNum += LINE_INC;
        }
      }
      ...
      for (let ci = 1; ci < cutPoints.length; ci++) {
        ...
        if (pt.type === "arc" && pt.i !== undefined && pt.j !== undefined) {
          ...
          if (profileHasTaper) {
            lines.push(`${buildLineNumber(cfg, lineNum)} G1 X${formatCoord(pt.x, dp, cs)} Y${formatCoord(pt.y, dp, cs)} U${formatCoord(uv.u, dp, cs)} V${formatCoord(uv.v, dp, cs)}`);
          } else {
            lines.push(`${buildLineNumber(cfg, lineNum)} X${formatCoord(pt.x, dp, cs)} Y${formatCoord(pt.y, dp, cs)}`);
          }
        }
        lineNum += LINE_INC;
      ...
      if (cutPoints.length >= 2) {
        ...
        if (profileHasTaper && cutPoints.length > 1) {
          ...
          lines.push(`${buildLineNumber(cfg, lineNum)} G1 X${formatCoord(firstPt.x, dp, cs)} Y${formatCoord(firstPt.y, dp, cs)} U${formatCoord(uv.u, dp, cs)} V${formatCoord(uv.v, dp, cs)}`);
        } else {
          lines.push(`${buildLineNumber(cfg, lineNum)} X${formatCoord(firstPt.x, dp, cs)} Y${formatCoord(firstPt.y, dp, cs)}`);
        }
        lineNum += LINE_INC;
      }
      ...
      if (pass.offset_mm > 0 && cutPoints.length >= 2) {
        ...
        for (const dpt of depPts) {
          if (dpt.type === "arc" && dpt.i !== undefined && dpt.j !== undefined) {
            ...
            lines.push(`${buildLineNumber(cfg, lineNum)} ${arcDir} X${formatCoord(dpt.x, dp, cs)} Y${formatCoord(dpt.y, dp, cs)} I${formatCoord(dpt.i, dp, cs)} J${formatCoord(dpt.j, dp, cs)}`);
          } else {
            lines.push(`${buildLineNumber(cfg, lineNum)} G1 X${formatCoord(dpt.x, dp, cs)} Y${formatCoord(dpt.y, dp, cs)}`);
          }
          lineNum += LINE_INC;
        }

        // Cancel compensation on move back toward start hole
        lines.push(`${buildLineNumber(cfg, lineNum)} G40 X${formatCoord(profile.start_hole.x, dp, cs)} Y${formatCoord(profile.start_hole.y, dp, cs)}`);
        lineNum += LINE_INC;
      }
...
function generateAgieCharmillesGCode(input: EDMGCodeInput): EDMGCodeResult {
  ...
  for (let pi = 0; pi < input.profiles.length; pi++) {
    ...
    for (let passIdx = 0; passIdx < input.passes.length; passIdx++) {
      ...
      // TAPER-EXPERT — UV offsets computed per contour point (not G51 which is scaling)
      const profileHasTaper = (profile.taper_angle_deg ?? 0) > 0;
      const taperDeg = profile.taper_angle_deg ?? 0;
      const guideDist = input.guide_distance_mm ?? 60;
      const uvLimit = input.uv_travel_limit_mm ?? 75;
      ...
      for (let ci = 0; ci < profile.contour_points.length; ci++) {
        ...
        // Build move with UV words for taper (TAPER-EXPERT manages parameters, we emit UV)
        if (pt.type === "arc" && pt.i !== undefined && pt.j !== undefined) {
          const arcCode = pt.direction === "cw" ? cfg.cw_arc_code : cfg.ccw_arc_code;
          if (profileHasTaper) {
            lines.push(`${buildLineNumber(cfg, lineNum)} ${arcCode} X${formatCoord(pt.x, dp, cs)} Y${formatCoord(pt.y, dp, cs)} I${formatCoord(pt.i, dp, cs)} J${formatCoord(pt.j, dp, cs)} U${formatCoord(uv.u, dp, cs)} V${formatCoord(uv.v, dp, cs)}`);
      ...
      // Reset UV axes to zero (taper off — TAPER-EXPERT manages, UV=0 implies no taper)
      if (profileHasTaper) {
        lines.push(`${buildLineNumber(cfg, lineNum)} G1 U0. V0. ${buildComment(cfg, "RESET UV - TAPER OFF")}`);
        lineNum += 10;
      }
