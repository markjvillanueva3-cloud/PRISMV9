...
export interface NineAxisResult {
  ...
  /** All 9 axis sections resolved (with defaults applied). part_volume_cm3 is excluded:
   *  it is a job quantity read directly from input (never resolved/defaulted -- "never fabricated"). */
  resolved_axes: Required<Omit<NineAxisInput, "tool_library" | "mode" | "batch_size" | "part_volume_cm3">>;
}
...
export class SpeedFeedNineAxisOrchestratorEngine {
  ...
  private translateToUltimate(
    ...
    // Mode → optimize_for translation. The prism_optimized DEFAULT uses shop_recommended (the
    // 80% balanced->aggressive blend on Vc+fz, ap/ae held at balanced) ONLY for ISO P (steel) +
    // M (stainless) milling-roughing -- the cases where the balanced default genuinely UNDER-shoots
    // modern coated-carbide catalog (validated: 1018 -27%->-5%, 304 -26%->-2%). Everything else stays
    // at balanced: turning/finishing/drilling AND K (cast iron) / N (aluminum) / S (Ti/superalloy) /
    // H (hardened) milling already sit at/near the table's aggressive column, so the blend OVERSHOOTS
    // them (validated +26%..+56% on turning/ceramic/CBN, +35% cast-iron mill, +27% Ti mill,
    // 2026-06-19). Operation+group-scoped default per operator decision.
    // iso_group: explicit caller input wins; otherwise resolve it from material.name via the
    // engine's CANONICAL MATERIAL_DB resolver (getMaterialProfile -- exact alias match, NOT a name
    // substring heuristic, so e.g. "tool_steel" resolves to H and is NEVER mis-read as P). An
    // unresolved name yields undefined -> balanced (safe; the aggressive blend never fires on an
    // unknown group). This lets the P/M-milling-roughing default fire for name-only callers too.
    const grp = input.material.iso_group
      ?? this.ultimate.getMaterialProfile(input.material.name)?.iso_group;
    const isShopRecDefault =
      (tp.operation ?? "milling") === "milling"
      && (tp.cut_type ?? "roughing") === "roughing"
      && (grp === "P" || grp === "M");
    const optimizeFor: UltimateSpeedFeedInput["optimize_for"] =
      mode === "cost_batch"
        ...
        : isShopRecDefault
        ? "shop_recommended"   // product DEFAULT for P/M milling-roughing (catalog-matched)
        : "balanced";           // everything else -- balanced avoids the blend's overshoot
  ...
  private buildModeRecommendation(
    ...
    if (mode === "cost_batch") {
      ...
      // PRISM-optimized (product DEFAULT) -- the SFC engine ran the operation-scoped optimize_for
      // chosen in translateToUltimate (shop_recommended for milling-roughing, else balanced), so
      // sfc's PRIMARY vc/rpm/fz/ap/ae ALREADY carry that setpoint and sfc.forces were computed at
      // that SAME chip load. They were initialized from those primary values above, so we DON'T
      // override them with sfc.alternatives.balanced anymore -- and because the workholding +
      // spindle-power clamps read sfc.forces (checkWorkholding / spindle-power clamp), they protect
      // at the recommended load (no under-protection; the 2026-06-10 force-collapse class). When an
      // aggressive milling-roughing default exceeds the workholding retention margin, checkWorkholding
      // HONESTLY reports feasible=false + warns (operator decision 2026-06-19: flag, NOT auto-derate).
      // Below we layer ONLY the 9-axis MRR multipliers + controller smoothing onto the primary.
      // ae stays at sfc.radial_depth.value (initialized above) -- the engine-resolved engagement:
      // the operator's radial input if given, else the table ae for the active goal (shop_recommended
      // holds ap/ae at balanced; balanced uses its own column). Either way the engine computed
      // sfc.forces at this SAME ae, so the workholding/spindle-power clamps stay force-consistent with
      // no re-derivation here. (The prior alt.ae_pct fallback was dropped with the
      // sfc.alternatives.balanced read.)
      ...
      // life stays at sfc.tool_life.life_minutes -- the active-goal primary value
      explanation =
        `PRISM-optimized mode -- operation-scoped setpoint (shop-recommended 80% blend for ` +
        `milling-roughing; balanced for turning/finishing to avoid catalog overshoot) with 9-axis ` +
        `MRR multipliers (rigidity ${factors.machine_rigidity_factor.toFixed(2)}, ` +
        `coolant ${factors.coolant_effectiveness.toFixed(2)}, controller ${factors.controller_smoothing_factor.toFixed(2)}).`;
    }
    ...
    const kien = input.material.iso_group !== undefined ? CANONICAL_KIENZLE[input.material.iso_group] : undefined;
    if (
  ...
  private computeROIPopup(
    ...
    // Part volume may be absent -> rec.cost_per_part_usd is null. The ROI tooling
    // suggestions below (coated insert, shrink-fit, balance class, HSM, TSC) do NOT
    // require a part cost -- they carry fixed price ranges + tool-life/MRR multipliers;
    // only the payback math reads current cost. A prior tsc-hardening early-returned an
    // empty popup whenever cost was null, silently killing the WHOLE ROI capability in
    // the common no-part-volume case (regression, papa c516a27aa6). Coerce null cost to 0
    // (indeterminate) so suggestions still surface and payback stays finite/positive.
    const current_cost_per_part_usd = rec.cost_per_part_usd ?? 0;
  ...
  private checkWorkholding(
    ...
    // End-of-life flank-wear workholding consequence (U-OSC-SFC-FLANK-WEAR-FORCE step 2, cross-engine):
    // the clamp check above uses the FRESH cutting force; as flank wear grows the in-plane force rises
    // (sfc.forces.cutting_force_worn_N, computed by UltimateSpeedFeedEngine) so the required clamp grows
    // and the retention safety factor falls. ADDITIVE note + a flip warning when the FRESH tool is held
    // but the WORN tool is not -- the headline feasible/safety_factor stay on the fresh force (non-regression).
    const FcutWorn = sfc.forces.cutting_force_worn_N?.value;
    if (typeof FcutWorn === "number" && Number.isFinite(FcutWorn) && FcutWorn > Fcut) {
      const requiredWornKn = ((FcutWorn * SF) / Math.max(friction, 0.05)) / 1000;
      const safetyWorn = available_clamp_force_kn / Math.max(requiredWornKn, 0.001);
      const wornFlip = feasible && safetyWorn < 1.5;
      notes.push(
        `End-of-life flank wear: cutting force rises to ${FcutWorn.toFixed(0)} N -> required clamp ${requiredWornKn.toFixed(1)} kN, retention SF ${safetyWorn.toFixed(2)}x.` +
          (wornFlip
            ? ` WARNING: workholding holds the FRESH tool but is INADEQUATE at the wear limit -- change the tool before the VB limit or increase clamp force.`
            : ``),
      );
    }
