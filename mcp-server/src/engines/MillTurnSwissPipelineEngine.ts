 ...
 */
import { CANONICAL_KIENZLE, CANONICAL_TAYLOR, taylorLife, } from "../physics/constants.js";
...
// ============================================================================
function getSmartToolSelector() { return smartToolSelectorEngine; }
function getCoolantStrategyEngine() { return coolantStrategyEngine; }
function getEntryExitStrategyEngine() { return entryExitStrategyEngine; }
function getWorkholdingVerificationEngine() { return workholdingVerificationEngine; }
/** Map ISO to CoolantStrategy material */
function mtCoolantMat(iso) {
    const m = {
...
const KIENZLE_ISO = CANONICAL_KIENZLE;
...
export class MillTurnSwissPipelineEngine {
     ...
     */
    _fireResolveMaterial(materialName, iso) {
    ...
    _getKienzle(iso) {
        ...
        if (rm)
            return { kc1_1: rm.kc1_1, mc: rm.mc };
    ...
    calculate(input) {
        switch (input.action) {
            case "live_tool_calc":
                return this.calculateLiveTool(input.params);
            case "sub_spindle_transfer":
                return this.calculateSubSpindleTransfer(input.params);
            case "multi_channel_program":
                return this.calculateMultiChannel(input.params);
            case "bar_feeder_calc":
                return this.calculateBarFeeder(input.params);
            case "swiss_machining":
                return this.calculateSwissMachining(input.params);
            case "mill_turn_assemble_program":
                return this.assembleProgram(input.params);
            default:
                throw new Error(`Unknown mill-turn action: ${input.action}`);
        }
    }
    // ═══════════════════════════════════════════════════════════════════
     ...
     */
    calculateLiveTool(input) {
        const { operation, tool_diameter_mm: D, depth_of_cut_mm: ap, width_of_cut_mm: ae = D * 0.5, workpiece_diameter_mm: Dw, offset_from_centerline_mm: offset = 0, iso_group: iso = "P", material_name: matName, live_spindle_power_kW: maxPower, num_flutes: z_teeth = 2, c_axis_resolution_deg = 0.001, y_axis_travel_mm = 0, hole_depth_mm, interpolation_type: interpOverride, } = input;
        // Fire async per-material physics resolution (U-ARCH3)
        this._fireResolveMaterial(matName, iso);
        const recs = [];
        const warnings = [];
        // TK-MS2-U13: Query tribal knowledge for mill-turn live tooling tips
        try {
            const machineIds = this._resolvedMachine?.manufacturer
                ? [`${this._resolvedMachine.manufacturer}-${this._resolvedMachine.model ?? ""}`]
                : [];
            const tips = tribalKnowledgeEngine.search({
                category: "setup",
                operation_type: "mill_turn",
                material_iso_group: iso,
                machine_ids: machineIds.length > 0 ? machineIds : undefined,
                limit: 5,
            });
            for (const tip of tips) {
                recs.push(`TK: ${tip.title}`);
            }
        }
        catch {
     ...
     */
    calculateSubSpindleTransfer(input) {
        const { transfer_mode, workpiece_diameter_mm: Dw, workpiece_length_mm: Lw, main_spindle_rpm: mainRPM, sub_spindle_grip_diameter_mm: gripD = Dw, sub_spindle_grip_length_mm: gripL = Math.min(Lw * 0.3, 15), cut_off_tool_width_mm: cutWidth = 3.0, cut_off_feed_mm_rev: cutFeed = 0.05, cutting_force_tangential_N: Fc_ext, cutting_force_axial_N: Fa_ext = 0, collet_friction_coeff: mu = 0.25, sub_spindle_max_rpm: subMaxRPM = 5000, acceleration_time_s: accelTime = 0.8, overlap_mm: overlap = 2.0, back_work_operations: backOps = [], iso_group: iso = "P", material_name: matName, } = input;
        // Fire async per-material physics resolution (U-ARCH3)
        this._fireResolveMaterial(matName, iso);
        const recs = [];
        const warnings = [];
     ...
     */
    calculateMultiChannel(input) {
        const { channels, sync_style, collision_zones = [], optimize_overlap = true, max_simultaneous_cuts = 2, } = input;
        const recs = [];
        const syncPoints = [];
            ...
            for (const a of opsA) {
                for (const b of opsB) {
                    ...
                    if (timeOverlap && zOverlap) {
                        recs.push(`COLLISION RISK: ${a.op_id} (turret ${zone.turret_a}) and ${b.op_id} (turret ${zone.turret_b}) ` +
        ...
        if (optimize_overlap) {
            ...
            if (maxSimul > max_simultaneous_cuts) {
                recs.push(`Peak simultaneous cuts: ${maxSimul} exceeds limit of ${max_simultaneous_cuts}. ` +
     ...
     */
    calculateBarFeeder(input) {
        const { bar_diameter_mm: Dbar, bar_length_mm: Lbar, bar_shape = "round", part_length_mm: Lpart, cut_off_width_mm: cutWidth = 3.0, facing_allowance_mm: faceAllow = 0.5, grip_length_mm: gripL = 15, collet_type, guide_bushing = false, guide_bushing_bore_mm: bushingBore, remnant_min_length_mm: remnantMin = 30, bar_end_waste_mm: endWaste = 20, feed_time_s: feedTime = 3.0, spindle_orient_time_s: orientTime = 1.0, bar_material_density_kg_m3: density = 7850, } = input;
        const recs = [];
        const warnings = [];
        // ── 1. Material per part ──
     ...
     */
    calculateSwissMachining(input) {
        const { machine_config: config, guide_bushing, bushing_bore_mm, workpiece_diameter_mm: Dw, overhang_from_bushing_mm: L_overhang, operation, tool_diameter_mm: Dtool, cutting_force_N: F_ext, depth_of_cut_mm: ap = 0.5, feed_mm_rev: f = 0.08, cutting_speed_m_min: Vc_in, workpiece_modulus_GPa: E_GPa = 210, l_over_d_ratio: ld_override, b_axis_angle_deg: bAngle = 0, gang_slide = false, num_gang_positions: gangPos, thread_pitch_mm, hole_depth_mm, iso_group: iso = "P", material_name: matName, } = input;
        // Fire async per-material physics resolution (U-ARCH3)
        this._fireResolveMaterial(matName, iso);
        const recs = [];
        const warnings = [];
        ...
        if (guide_bushing && Math.abs(bushing_bore_mm - Dw) > 0.1) {
            warnings.push(`Bushing bore ${bushing_bore_mm}mm vs bar ${Dw}mm — gap ${(bushing_bore_mm - Dw).toFixed(3)}mm. ` +
     ...
     * @returns Cutting speed in m/min
     */
    _defaultCuttingSpeedTurning(iso) {
        const base = {
     ...
     */
    _selectInterpolation(op, fallback) {
        switch (op) {
     ...
     */
    _estimateBackWorkTime(op, Dw, iso) {
     ...
     */
    _generateSyncCode(style, afterOp, waitCh, signalCh) {
     ...
     */
    _generateSyncCodeBlock(style, syncPoints, numChannels) {
        const lines = [];
     ...
     */
    _getOpsInZone(channels, turret, zStart, zEnd) {
        const result = [];
        for (const ch of channels) {
            if (ch.turret !== turret)
                continue;
     ...
     */
    _countMaxSimultaneousCuts(timelines, scheduled) {
    ...
    assembleProgram(input) {
        ...
        this._fireResolveMaterial(input.material?.name, input.material?.iso_group);
        ...
        const cpm = new PipelineCheckpointManager("millturn-assemble", input.runId);
        ...
        for (const op of input.turning_ops) {
            ...
            if (feedMmMin > 0)
                totalCycleMin += (cutLength * passes) / feedMmMin;
            lines.push("");
        }
        ...
        for (const ltop of input.live_tool_ops) {
            ...
            if (ltop.type === "cross_drill") {
                ...
                lines.push(`G00 C0.`);
                if (ltop.y_position_mm !== undefined)
                    lines.push(`G00 Y${round4(ltop.y_position_mm)}`);
                lines.push(`G00 Z${round4(ltop.z_start_mm)}`);
            ...
            if (ltop.feed_mm_min > 0)
                totalCycleMin += ltop.depth_mm / ltop.feed_mm_min;
            lines.push("");
        }
        ...
        if (input.sub_spindle_ops && input.sub_spindle_ops.length > 0) {
            ...
            for (const subOp of input.sub_spindle_ops) {
                ...
                if (subFeedMmMin > 0)
                    totalCycleMin += subCutLen / subFeedMmMin;
                syncPoints++;
            }
            lines.push("");
        }
        ...
        if (input.sub_spindle_ops && input.sub_spindle_ops.length > 0) {
            try {
                ...
                if (maxSubFc > 0) {
                    const whResult = wve.verify({ Fc_N: maxSubFc, operation_name: "sub_spindle_max_force" }, { type: "sub_spindle_collet", clamping_force_N: 15000, clamp_points: 1, clamping_method: "collet" });
        ...
        try {
            ...
            for (const op of input.turning_ops) {
                ...
                if (life < cycleMin * 2) {
                    warnings.push(`T${op.tool_number} (${op.type}): Taylor tool life ${life.toFixed(0)}min — may not complete 2 parts per insert at Vc=${op.cutting_speed_m_min} m/min`);
                }
            }
        }
        catch (e) {
