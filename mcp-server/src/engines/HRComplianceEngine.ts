...
class HRComplianceEngine {
  ...
  constructor() {
    for (let i = 0; i < DEFAULT_BENEFIT_PLANS.length; i++) {
      ...
      const fullPlan = { ...plan, id };
      this.benefitPlans.set(id, fullPlan);
      persistenceBridge.persist("hr_benefit_plans", id, fullPlan as any);
    }
  }
  ...
  enrollEmployee(employee_id: string, plan_ids: string[]): EmployeeBenefitEnrollment {
    ...
    persistenceBridge.persist("hr_enrollments", employee_id, enrollment as any);
    return enrollment;
  }
  ...
  initializePTO(employee_id: string, years_of_service: number): PTOBalance {
    ...
    persistenceBridge.persist("hr_pto_balances", employee_id, pto as any);
    return pto;
  }
  ...
  approvePTO(request_id: string, approved_by: string): PTORequest {
    ...
    if (balance) {
      ...
      persistenceBridge.persist("hr_pto_balances", req.employee_id, balance as any);
    }
    return req;
  }
  ...
  createReview(params: Omit<PerformanceReview, 'id'>): PerformanceReview {
    ...
    if (params.compensation_change) {
      ...
      persistenceBridge.persist("hr_compensation_history", params.employee_id, history as any);
    }
...
persistenceBridge.registerMap({
  entity: "hr_benefit_plans",
  getMap: () => (hrComplianceEngine as any).benefitPlans as Map<string, any>,
  keyField: "id",
});
persistenceBridge.registerMap({
  entity: "hr_enrollments",
  getMap: () => (hrComplianceEngine as any).enrollments as Map<string, any>,
  keyField: "employee_id",
});
persistenceBridge.registerMap({
  entity: "hr_pto_balances",
  getMap: () => (hrComplianceEngine as any).ptoBalances as Map<string, any>,
  keyField: "employee_id",
});
persistenceBridge.registerMap({
  entity: "hr_compensation_history",
  getMap: () => (hrComplianceEngine as any).compensationHistory as Map<string, any>,
  keyField: "employee_id",
});
