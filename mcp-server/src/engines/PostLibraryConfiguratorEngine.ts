...
// ---- Machine Limits (from MachineRegistry at generation time) ----
// PPG-REAL S3a U-PPR10: Limits sourced from machine catalog, not hardcoded.
var maxSpindleSpeed = ${config.machine.max_rpm ?? (config.aggressiveness >= 70 ? 24000 : 18000)};
var maxFeedrate = spatial(${config.machine.max_feed_mm_min ?? (config.aggressiveness >= 70 ? 20000 : 15000)}, MM);
var maxPower_kW = ${config.machine.max_power_kW ?? 30};
var travelX = ${config.machine.travel_x_mm ?? 762};
var travelY = ${config.machine.travel_y_mm ?? 406};
var travelZ = ${config.machine.travel_z_mm ?? 508};
...
// ---- Spindle Speed (clamped to machine max) ----
function onSpindleSpeed(spindleSpeed) {
  var rpm = spindleSpeed;
  if (rpm > maxSpindleSpeed) {
    writeComment("PRISM LIMIT: S" + rpm + " clamped to machine max " + maxSpindleSpeed + " RPM");
    rpm = maxSpindleSpeed;
  }
...
// ---- Feed Clamping (PPG-REAL S3a U-PPR10) ----
function clampFeed(feed) {
  if (feed > getProperty("maxFeedrate", maxFeedrate)) {
    writeComment("PRISM LIMIT: F" + feed.toFixed(0) + " clamped to machine max " + maxFeedrate);
    return getProperty("maxFeedrate", maxFeedrate);
  }
  return feed;
}
...
function onLinear(x, y, z, feed) {
  ...
  var adjustedFeed = clampFeed(feed * chipFactor);
...
// ============================================================================
// MASTER POST TEMPLATE GENERATOR (PPG-REAL S5 U-PPR20)
// ============================================================================

/** Controller families supported by the PRISM Master Post CPS template. */
const MASTER_POST_FAMILIES: Record<string, string> = {
  Haas: "haas_ngc", Fanuc: "fanuc_31i", Siemens: "siemens_840d",
  Heidenhain: "heidenhain", Mazak: "mazak", Okuma: "okuma",
  Hurco: "hurco", "DMG MORI": "dmg_mori", Brother: "brother", Doosan: "doosan",
};

/**
 * Generate a machine-specific CPS by customizing the PRISM-Master.cps template.
 * Replaces default property values with machine-specific ones from the configuration.
 * Returns null if the controller family is not supported by the Master Post.
 */
function generateFromMasterPost(config: PostConfiguration): string | null {
  const family = MASTER_POST_FAMILIES[config.controller.family];
  if (!family) return null;

  try {
    const fs = require("fs");
    const path = require("path");
    const templatePath = path.resolve(__dirname, "../../scripts/fusion360-post/PRISM-Master.cps");
    let template = fs.readFileSync(templatePath, "utf-8");

    // Customize the header description
    const machineId = `${config.machine.brand} ${config.machine.model}`;
    template = template.replace(
      /description = "PRISM Master Post — Multi-Controller";/,
      `description = "PRISM ${machineId} — ${config.controller.variant}";`
    );

    // Set controller family default
    template = template.replace(
      /value: "haas_ngc"\s*\n\s*\}/,
      `value: "${family}"\n  }`
    );

    // Set machine name default
    template = template.replace(
      /machineName:[\s\S]*?value: ""/,
      `machineName: {\n    title: "Machine Name",\n    description: "Machine model for PRISM catalog lookup",\n    type: "string",\n    value: "${machineId}"`
    );

    // Set machine limits
    if (config.machine.max_rpm) {
      template = template.replace(
        /maxSpindleSpeed:[\s\S]*?value: 0/,
        `maxSpindleSpeed: {\n    title: "Max Spindle Speed (RPM)",\n    description: "Machine maximum RPM. 0 = no limit.",\n    type: "integer",\n    range: [0, 100000],\n    value: ${config.machine.max_rpm}`
      );
    }
    if (config.machine.max_feed_mm_min) {
      template = template.replace(
        /maxFeedRate:[\s\S]*?value: 0\n\s*\}/,
        `maxFeedRate: {\n    title: "Max Feed Rate (mm/min)",\n    description: "Machine maximum feed rate. 0 = no limit.",\n    type: "integer",\n    range: [0, 100000],\n    value: ${config.machine.max_feed_mm_min}\n  }`
      );
    }

    // Set aggressiveness-based prove-out defaults
    if (config.aggressiveness >= 75) {
      // Production mode — disable prove-out
      template = template.replace(
        /proveOutMode:[\s\S]*?value: true/,
        `proveOutMode: {\n    title: "Prove-Out Mode",\n    description: "Derate feeds/speeds for first article proving. ON by default.",\n    type: "boolean",\n    value: false`
      );
    }

    // Add PRISM config ID as a comment at the top
    template = template.replace(
      "Version: 1.0.0",
      `Version: 1.0.0\n  PRISM Config: ${config.id}\n  Tier: ${config.tier} | Aggressiveness: ${config.aggressiveness}/100`
    );

    return template;
  } catch {
    return null; // Template file not available — fall back to legacy generator
  }
}
...
function exportPost(input: ExportPostInput): PostExportResult {
  ...
  switch (config.output_format) {
    case "cps": {
      // Try Master Post template first, fall back to legacy generator
      const masterCps = generateFromMasterPost(config);
      content = masterCps ?? generateCpsContent(config);
