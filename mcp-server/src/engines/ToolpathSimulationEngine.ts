...
export class ToolpathSimulationEngine {
  simulate(moves: SimMove[], config: SimConfig = {}): SimResult {
    ...
    for (let i = 0; i < moves.length; i++) {
      ...
      if (m.type === "dwell") {
        ...
        dist = this.arcLength(pos, to, m.i ?? 0, m.j ?? 0, m.r);
  ...
  private arcLength(from: { x: number; y: number; z: number }, to: { x: number; y: number; z: number }, i: number, j: number, r?: number): number {
    // Center = from + (i, j)
    const cx = from.x + i;
    const cy = from.y + j;
    ...
    if (sweep < 0.001) sweep = 2 * Math.PI; // full circle
