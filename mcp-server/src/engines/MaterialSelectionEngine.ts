...
// Fallback catalog — used when MaterialService is unavailable or during cold start.
// Primary path: materialsFromService (populated async on module load).
...
// Service-backed material list — warm-loaded from MaterialService on startup.
let serviceMaterials: MaterialData[] | null = null;
function getMaterials(): MaterialData[] { return serviceMaterials ?? FALLBACK_MATERIALS; }

void (async () => {
  try {
    const count = await materialService.getCount();
    if (count > 0) {
      // Load a representative set from the registry covering common ISO groups
      const isoGroups = ["P", "M", "K", "N", "S", "H"];
      const loaded: MaterialData[] = [];
      for (const iso of isoGroups) {
        const mats = await materialService.getByISO(iso);
        for (const m of mats.slice(0, 10)) {
          const ts = typeof m.mechanical?.tensile_strength === "number" ? m.mechanical.tensile_strength : 0;
          const ys = typeof m.mechanical?.yield_strength === "number" ? m.mechanical.yield_strength : 0;
          const hv = m.mechanical?.hardness?.rockwell_c ?? m.mechanical?.hardness?.vickers ?? 0;
          loaded.push({
            name: m.name, iso: m.iso_group, cat: m.category,
            UTS: ts, YS: ys, HRC: typeof hv === "number" ? hv : 0,
            density: m.physical?.density ?? 7.85,
            thermal_k: m.thermal?.thermal_conductivity ?? 50,
            max_temp: m.thermal?.max_service_temp ?? 500,
            elong: m.mechanical?.elongation ?? 10,
            machinability: m.machining?.machinability_rating ?? 50,
            cost: 2.0, // registry doesn't track unit cost
            chip: (m.machining?.chip_formation === "continuous" ? "long" :
                   m.machining?.chip_formation === "discontinuous" ? "short" :
                   m.machining?.chip_formation ?? "long") as MaterialData["chip"],
            wear: (m.machining?.abrasiveness === "extreme" ? "severe" :
                   m.machining?.abrasiveness ?? "moderate") as MaterialData["wear"],
            coolant: (m.machining?.recommended_coolant ?? "flood") as MaterialData["coolant"],
            corrosion: m.category?.includes("stainless") || m.category?.includes("titanium") || false,
            weldable: true,
          });
        }
      }
      if (loaded.length > 0) serviceMaterials = loaded;
    }
  } catch { /* registry unavailable — FALLBACK_MATERIALS used */ }
})();
...
export class MaterialSelectionEngine {
  ...
  compare(materialIds: string[]): MaterialComparisonResult {
    ...
    for (const id of materialIds) {
      const mat = getMaterials().find(m => m.name.toLowerCase().replace(/[\s-]+/g, "_") === id) || getMaterials()[0];
  ...
  machinability(materialId: string): MachinabilityReport {
    const mat = getMaterials().find(m => m.name.toLowerCase().replace(/[\s-]+/g, "_") === materialId) || getMaterials()[0];
