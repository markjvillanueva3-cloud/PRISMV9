 ...
 * tests pass a fake so every path runs with no live vault. v1 issues only GET and
 * a query-string POST (no request body) — hence no `body` field.
 */
 ...
 */
const defaultTransport: ObsidianTransport = ({ method, url, path, apiKey, accept, timeoutMs }) =>
