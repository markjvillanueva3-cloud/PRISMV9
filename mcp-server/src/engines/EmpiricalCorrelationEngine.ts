...
export class EmpiricalCorrelationEngine {
  ...
  productivityCorrelations(params: ProductivityInput): ProductivityResult {
    ...
    let kc_base = 2000; // MPa default
    const mt = material_type.toLowerCase();
    if (mt.includes("aluminum")) kc_base = 800;
    else if (mt.includes("cast_iron") || mt.includes("cast iron")) kc_base = 1200;
    else if (mt.includes("titanium")) kc_base = 1800;
    else if (mt.includes("stainless")) kc_base = 2400;
    else if (mt.includes("steel")) kc_base = 2000;
    else if (mt.includes("inconel") || mt.includes("nickel")) kc_base = 3000;
