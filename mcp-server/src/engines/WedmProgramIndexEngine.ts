...
export class WedmProgramIndexEngine {
  ...
  /**
   * AI-powered pattern recognition across the program archive.
   *
   * Analyzes filename patterns, customer program distributions,
   * and identifies naming conventions, common part types, and trends.
   *
   * @param result Harvest result to analyze
   * @returns AI reasoning result with pattern insights
   */
  static async analyzePatternWithAI(result: WedmProgramIndexResult): Promise<AIReasoningResult> {
    // Extract filename patterns
    const filenamePatterns = new Map<string, number>();
    result.programs.forEach(p => {
      // Extract common prefixes/patterns
      const name = p.filename.replace(/\.[^.]+$/, "").toUpperCase();
      // Look for part number patterns like "XXX-XXXXX-XXXXX"
      if (/^\d{3}-\d{5}-\d{5}/.test(name)) {
        filenamePatterns.set("CUSTOMER_PART_NUMBER", (filenamePatterns.get("CUSTOMER_PART_NUMBER") ?? 0) + 1);
      } else if (/^[A-Z]{2,4}\d{4,}/.test(name)) {
        filenamePatterns.set("ALPHANUMERIC_CODE", (filenamePatterns.get("ALPHANUMERIC_CODE") ?? 0) + 1);
      } else if (name.includes("DIE") || name.includes("PUNCH") || name.includes("INSERT")) {
        filenamePatterns.set("TOOL_TYPE_NAME", (filenamePatterns.get("TOOL_TYPE_NAME") ?? 0) + 1);
      } else {
        filenamePatterns.set("OTHER", (filenamePatterns.get("OTHER") ?? 0) + 1);
      }
    });

    // File size distribution for complexity estimation
    const fileSizes = result.programs.map(p => p.fileSize);
    ...
    return prismIntelligence.reason({
      domain: "wedm_analysis",
      intent: "Analyze Wire EDM program archive patterns for naming conventions, part types, and shop insights",
      context: {
        total_programs: result.totalPrograms,
        wire_edm_count: result.wireEdmCount,
        electrode_mill_count: result.electrodeMillCount,
        customer_count: result.customerCount,
        top_customers: WedmProgramIndexEngine.getTopCustomers(result, 10),
        extension_distribution: result.byExtension,
        filename_patterns: Object.fromEntries(filenamePatterns),
        file_size_stats: {
          avg_bytes: Math.round(avgFileSize),
          max_bytes: maxFileSize,
          min_bytes: minFileSize,
        },
      },
      options: {
  ...
  /**
   * AI-powered customer program analysis.
   *
   * @param result Harvest result
   * @param customer Customer name to analyze
   * @returns AI reasoning result with customer-specific insights
   */
  static async analyzeCustomerWithAI(
    result: WedmProgramIndexResult,
    customer: string
  ): Promise<AIReasoningResult> {
    const customerPrograms = WedmProgramIndexEngine.getCustomerPrograms(result, customer);

    // Analyze program characteristics
    const extensionDist = new Map<string, number>();
    let totalSize = 0;
    customerPrograms.forEach(p => {
      extensionDist.set(p.extension, (extensionDist.get(p.extension) ?? 0) + 1);
      totalSize += p.fileSize;
    });

    // Machine type distribution
    const machineTypes = new Map<string, number>();
    customerPrograms.forEach(p => {
      machineTypes.set(p.machine, (machineTypes.get(p.machine) ?? 0) + 1);
    });

    return prismIntelligence.reason({
      domain: "wedm_analysis",
      intent: `Analyze Wire EDM program history for customer ${customer} to identify patterns and opportunities`,
      context: {
        customer,
        program_count: customerPrograms.length,
        extension_distribution: Object.fromEntries(extensionDist),
        machine_type_distribution: Object.fromEntries(machineTypes),
        total_file_size_mb: Math.round(totalSize / 1024 / 1024 * 100) / 100,
        sample_filenames: customerPrograms.slice(0, 10).map(p => p.filename),
      },
      options: {
        require_explanation: true,
        include_alternatives: true,
  ...
  /**
   * AI-powered program recommendation based on similar jobs.
   *
   * @param result Harvest result
   * @param query Search parameters (customer, material, part type hints)
   * @returns AI reasoning result with recommended similar programs
   */
  static async findSimilarProgramsWithAI(
    result: WedmProgramIndexResult,
    query: {
      customer?: string;
      partNameHints?: string[];
      programType?: "wire_edm" | "electrode_mill";
    }
  ): Promise<AIReasoningResult> {
    // Filter programs based on query
    let candidates = result.programs;
    if (query.customer) {
      candidates = candidates.filter(
        p => p.customer.toLowerCase().includes(query.customer!.toLowerCase())
      );
    }
    if (query.programType) {
      candidates = candidates.filter(p => p.programType === query.programType);
    }

    // Score programs by filename similarity to hints
    if (query.partNameHints && query.partNameHints.length > 0) {
      const scored = candidates.map(p => {
        let score = 0;
        const nameLower = p.filename.toLowerCase();
        query.partNameHints!.forEach(hint => {
          if (nameLower.includes(hint.toLowerCase())) score += 10;
        });
    ...
    return prismIntelligence.reason({
      domain: "wedm_analysis",
      intent: "Find and recommend similar Wire EDM programs based on job parameters",
      context: {
        query,
        candidate_count: candidates.length,
        top_candidates: candidates.slice(0, 15).map(p => ({
          filename: p.filename,
          customer: p.customer,
