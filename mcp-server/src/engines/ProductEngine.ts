...
function resolveMaterial(material?: string, hardness?: number, group?: string) {
  ...
  if (lookup) {
    return {
      hardness: resolvedHardness ?? lookup.hardness,
  ...
  if (ext) {
    return {
      hardness: resolvedHardness ?? ext.hardness_HB ?? 200,
...
function sfcCalculate(params: SFCInput): { result: SFCResult } | { error: string } {
  ...
  const ap = params.depth_of_cut ?? params.depth ?? toolDiam * 0.5;
...
function sfcCompare(params: SFCInput): { result: SFCCompareResult } | { error: string } {
  ...
  for (const tool of toolMaterials) {
    ...
    // same derated frame the calculator publishes (fz is unaffected by coolant/coating).
    const vc = sf.cutting_speed * vcFactor;
...
function sfcOptimize(params: SFCInput & { objective?: string }): { result: SFCOptimizeResult } | { error: string } {
  ...
  const derates = sfcToolingDerates(params, isoGroup);
  const vcFactor = derates.coatingSpeedFactor * derates.coolantVcFactor;
  const baseVc = sf.cutting_speed * vcFactor;
...
function sfcSafety(params: SFCInput): any {
  ...
  // workpiece -> carbon diffusion / rapid crater wear). sfcSafety recomputes the operating point
  // INDEPENDENTLY of sfcCalculate, so without this it would SILENTLY pass an incompatible tool/material
  // pair -- the safety-critical residual of the customer-path coating wire (U-OSC-SFC-SAFETY-COATING-GAP).
  // Additive: surfaces the same warning sfcCalculate:1014-1015 raises; the score/formulas are unchanged.
  const coatingKey = normalizeCoatingKey(params.coating);
  const isoGroup = groupToISO(mat.group) as ISOGroup;
  if (isCoatingMaterialIncompatible(coatingKey, isoGroup)) {
