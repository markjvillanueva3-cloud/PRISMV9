...
// Material cost per kg (approximate)
const MATERIAL_COST_KG: Record<string, number> = {
...
export class StockSizeOptimizerEngine {
  ...
  optimize(input: StockOptimizeInput): StockOptimizeResult {
    ...
    const density = MATERIAL_DENSITY[input.material];
    const costKg = MATERIAL_COST_KG[input.material];
    if (!density || !costKg) throw new Error(`Unknown material: ${input.material}. Supported: ${Object.keys(MATERIAL_COST_KG).join(", ")}`);
  ...
  catalog(material: string): { round_bars: number[]; flat_bars: number[][]; plate_thicknesses: number[]; cost_per_kg: number; density_kg_m3: number } {
    const costKg = MATERIAL_COST_KG[material] ?? 0;
    const dens = MATERIAL_DENSITY[material] ?? 0;
    if (!costKg) throw new Error(`Unknown material: ${material}`);
    return {
