...
export interface AccuracyReport {
  ...
  /**
   * U2 (QUOTING-OPTIMAL): of total_skipped, how many were dropped specifically because
   * they are synthetic/placeholder/non-quote contamination (vs a legit non-finite/zero
   * actual). Surfaced so calibration can refuse to activate when the corpus is mostly
   * contamination, and so the skip is auditable (R12 -- never silently dropped).
   */
  total_skipped_contaminated: number;
  metrics: {
...
export interface RunOptions {
  ...
  /**
   * Override the placeholder-revenue floor ($) below which a record is treated as
   * synthetic contamination. Default DEFAULT_PLACEHOLDER_REVENUE_FLOOR_USD ($20).
   */
  placeholderRevenueFloor?: number;
}
...
/**
 * Below this dollar amount an `actual_revenue_usd` is a synthetic placeholder, not a
 * real invoiced price. The legacy bootstrap baseline stamped 19 rows at exactly $10
 * (size_bytes x $ stub) + sub-$5 OCR misreads; real JM part prices floor well above
 * this (measured p05 = $15, p25 = $295). Tunable via RunOptions.placeholderRevenueFloor.
 */
export const DEFAULT_PLACEHOLDER_REVENUE_FLOOR_USD = 20;

/**
 * Synthetic / non-quote `customer` markers that polluted the bootstrap baseline:
 *   - "MACHINE MODELS FOR LEARNING ENGINE AND SIMULATION" (machine-catalog rows whose
 *     part_id is a machine model, not a part -- 15 rows)
 *   - "CLAUDE - ..." (AI-generated test rows -- 5 rows)
 *   - "BASEBALL PARTS" (synthetic test rows -- 2 rows)
 * A record whose customer matches any of these is NOT a real quote and must never
 * reach calibration (it fabricated the +2108% bias that clamped the global factor).
 */
const SYNTHETIC_CUSTOMER_MARKERS = [
  "MACHINE MODELS FOR LEARNING",
  "BASEBALL PARTS",
] as const;

/**
 * True when a baseline record is a synthetic/placeholder/non-quote row that must be
 * excluded from accuracy measurement + calibration. Exported + pure so the filter is
 * unit-testable and reusable by the corpus builders. R12: contamination is reported
 * (total_skipped_contaminated), never silently dropped.
 */
export function isContaminatedBaselineRecord(
  rec: QuoteBaselineRecord | null | undefined,
  placeholderFloorUsd = DEFAULT_PLACEHOLDER_REVENUE_FLOOR_USD,
): boolean {
  if (!rec) return true;
  const rev = rec.actual_revenue_usd;
  if (typeof rev !== "number" || !isFinite(rev) || rev < placeholderFloorUsd) return true;
  const cust = String(rec.customer ?? "").toUpperCase();
  if (cust.startsWith("CLAUDE")) return true;
  for (const marker of SYNTHETIC_CUSTOMER_MARKERS) {
    if (cust.includes(marker)) return true;
  }
  return false;
}
...
export class QuotingTrainingLoopEngine {
  ...
  run(records: QuoteBaselineRecord[], opts: RunOptions = {}): AccuracyReport {
    ...
    const placeholderFloor = typeof opts.placeholderRevenueFloor === "number"
      ? opts.placeholderRevenueFloor
    ...
    for (const rec of records) {
      // Synthetic/placeholder/non-quote contamination -- never reaches calibration.
      // (The legacy bootstrap baseline's $10 placeholders + machine-catalog rows
      // fabricated the +2108% bias that clamped the global correction factor.)
      if (isContaminatedBaselineRecord(rec, placeholderFloor)) {
        skipped++;
        skippedContaminated++;
        continue;
      }
    ...
    if (predictions.length === 0) {
      return { ...empty, total_records: records.length, total_skipped: skipped, total_skipped_contaminated: skippedContaminated, reason: "all-records-skipped" };
    }
    ...
    return {
      ...
      total_skipped_contaminated: skippedContaminated,
      metrics,
