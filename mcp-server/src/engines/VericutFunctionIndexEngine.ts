 ...
 * VRC file format integration. FunctionIndexEngine catalogues VERICUT's
 * user-facing operation taxonomy.
 *
 * Reference: CGTech VERICUT 9.4 Reference / NC verification training.
 */

import { readFileSync, existsSync } from "fs";
import { join } from "path";

export interface VericutParameter {
  type: string;
  description?: string;
  unit?: string;
  default?: unknown;
...
export interface VericutOperation {
  display_name: string;
  category: string;
  description: string;
  dialog_tabs: string[];
  parameter_count?: number;
  parameters: Record<string, Record<string, VericutParameter>>;
}

export interface VericutSection {
  ...
  operations: Record<string, VericutOperation>;
...
export interface VericutFunctionIndex {
  system_id: "vericut";
  sections: Record<string, VericutSection>;
  total_operations: number;
  total_parameters: number;
  categories: string[];
}
...
const SECTION_FILES: Array<{ file: string; key: string }> = [
  { file: "verification.json", key: "verification" },
  { file: "optimization.json", key: "optimization" },
  { file: "simulation.json", key: "simulation" },
  { file: "review.json", key: "review" },
];

export class VericutFunctionIndexEngine {
  private static index: VericutFunctionIndex | null = null;

  private static readonly DATA_DIR = join(
    __dirname,
    "../../data/cam-functions/vericut"
  );

  private static loadIndex(): VericutFunctionIndex {
    if (this.index) return this.index;

    const sections: Record<string, VericutSection> = {};
    let totalOps = 0;
    let totalParams = 0;
    ...
    for (const { file, key } of SECTION_FILES) {
      ...
      try {
        const data = JSON.parse(readFileSync(path, "utf-8")) as VericutSection;
        sections[key] = data;
        ...
        throw new Error(
          `VericutFunctionIndexEngine: failed to parse ${file}: ${(err as Error).message}`
        );
      }
    }
  ...
  static getIndex(): VericutFunctionIndex {
  ...
  static getSection(
    sectionKey: string
  ): VericutSection | { error: string; available: string[] } {
  ...
  /**
   * Verification operations — VERICUT's signature surface
   * (collision/gouge/holder/fixture detection).
   */
  static getVerificationOperations(): Array<{
    operation_id: string;
    display_name: string;
    section: string;
    category: string;
    ...
    const vs = idx.sections["verification"];
    if (!vs) return out;
    for (const [opId, op] of Object.entries(vs.operations || {})) {
      out.push({
        operation_id: opId,
        display_name: op.display_name,
  ...
  /**
   * Optimisation operations — OptiPath secondary surface.
   */
  static getOptimizationOperations(): Array<{
    operation_id: string;
    display_name: string;
    section: string;
    category: string;
    ...
    const os = idx.sections["optimization"];
    if (!os) return out;
    for (const [opId, op] of Object.entries(os.operations || {})) {
      out.push({
        operation_id: opId,
        display_name: op.display_name,
  ...
  static getOperation(operationId: string):
    | { operation_id: string; section: string; operation: VericutOperation }
    | { error: string } {
