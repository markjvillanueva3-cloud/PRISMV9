...
export class SPCProcessCapabilityEngine {
  ...
  private checkNelsonRules(data: number[], mean: number, stdDev: number): NelsonViolation[] {
    ...
    const rule1Points = data.map((v, i) => Math.abs(v - mean) > 3 * stdDev ? i : -1).filter(i => i >= 0);
    if (rule1Points.length > 0) {
      violations.push({ rule: 1, description: "Point(s) beyond 3σ from mean", points: rule1Points });
    }
    ...
    for (let i = 0; i <= data.length - 9; i++) {
      ...
      if (slice.every(v => v > mean) || slice.every(v => v < mean)) {
        violations.push({ rule: 2, description: "9 consecutive points on same side of mean", points: Array.from({ length: 9 }, (_, j) => i + j) });
        break;
      }
    }
    ...
    for (let i = 0; i <= data.length - 6; i++) {
      ...
      if (increasing || decreasing) {
        violations.push({ rule: 3, description: "6 consecutive points trending monotonically", points: Array.from({ length: 6 }, (_, j) => i + j) });
        break;
      }
    }
