...
export const SubtaskSchema = z.object({
  ...
  /** IDs of subtasks this depends on; empty = leaf (parallelizable). */
...
export class HermesParallelFanoutPlannerEngine {
  static plan(req: FanoutPlanRequest): FanoutPlan {
    ...
    // Assign leaves to slots, preferring highest-score candidate for the leaf's domain.
    const assigned = new Set<string>();
    const wave_1: AgentAssignment[] = [];
    const unrouted: string[] = [];

    // Bucket candidates by domain for fast lookup; preserve score ordering.
    const sortedCands = [...v.candidates].sort((a, b) => b.score - a.score);

    for (const leaf of leaves) {
      // Prefer first un-assigned candidate whose primary_domain matches the leaf's domain.
      let pick = sortedCands.find((c) => !assigned.has(c.slot) && c.primary_domain === leaf.domain && c.score > 0);
      // Fallback: any un-assigned candidate with positive score.
      if (!pick) pick = sortedCands.find((c) => !assigned.has(c.slot) && c.score > 0);
      if (!pick) {
        unrouted.push(leaf.subtask_id);
        continue;
      }
      assigned.add(pick.slot);
      wave_1.push({
        subtask_id: leaf.subtask_id,
        slot: pick.slot,
        hermes_role: pick.hermes_role,
        score: pick.score,
        reason: pick.primary_domain === leaf.domain ? "domain-match" : "best-available",
      });
      if (wave_1.length >= v.max_parallel) break;
    }

    // Anything that didn't fit in max_parallel becomes deferred.
