...
// ============================================================================
// PUOA INTEGRATION TYPES
// ============================================================================

/** Risk entry for PUOA sequence assessment. */
export interface PUOASequenceRisk {
  category: "accessibility" | "workholding" | "rigidity" | "datum_available" | "sequence";
  operation_id: string;
...
/** Fallback strategy for PUOA. */
export interface PUOASequenceFallback {
  strategy: string;
  trigger: string;
  confidence: number;
  action: "resequence" | "modify_operation" | "add_setup" | "change_tooling";
}

/** PUOA-compatible sequence assessment for LLM/CLI. */
export interface PUOASequenceAssessment {
  feasible: boolean;
  risk_score: number;
  risks: PUOASequenceRisk[];
  dead_ends: Array<{ blocking_op: string; blocked_op: string; reason: string }>;
  missing_prerequisites: string[];
  fallback_strategies: PUOASequenceFallback[];
...
/** PUOA resequence suggestion. */
export interface PUOAResequenceSuggestion {
  can_resequence: boolean;
  reason: string;
  suggestions: Array<{
    order: string[];
    improvement: number;
...
export class SequenceFeasibilityEngine {
  ...
  // --- PUOA Integration Methods ---

  /**
   * PUOA-compatible assessment for LLM/CLI integration.
   * Converts simulation result to PUOA pre-flight format.
   */
  assessSequenceForPUOA(input: SimulateSequenceInput): PUOASequenceAssessment {
    const sim = this.simulateSequence(input);
    const risks: PUOASequenceRisk[] = [];
    const fallbacks: PUOASequenceFallback[] = [];

    // Convert per-operation failures to risks
    for (const opResult of sim.per_operation) {
      if (opResult.status === "fail" || opResult.status === "warning") {
        const severity = opResult.status === "fail" ? "high" : "medium";
        for (const [check, result] of Object.entries(opResult.checks)) {
          if (!result.pass) {
            risks.push({
              category: check as "accessibility" | "workholding" | "rigidity" | "datum_available",
              operation_id: opResult.operation_id,
              severity,
              description: result.detail,
              risk_score: opResult.risk_score,
            });
          }
        }
      }
    ...
    // Convert dead-ends to high-severity risks
    for (const deadEnd of sim.dead_ends) {
      risks.push({
        category: "sequence",
        operation_id: deadEnd.blocked_op,
        severity: "critical",
        description: `Dead-end: ${deadEnd.blocking_op} blocks ${deadEnd.blocked_op} — ${deadEnd.reason}`,
        risk_score: 1.0,
      });
      // Add resequencing fallback
      fallbacks.push({
        strategy: `Reorder operations: execute ${deadEnd.blocked_op} before ${deadEnd.blocking_op}`,
        trigger: `Dead-end detected at ${deadEnd.blocking_op}`,
        confidence: 0.7,
        action: "resequence",
      });
    }
    ...
    const successProb = Math.max(0, 1 - sim.cumulative_risk * 0.2 - sim.fail_count * 0.15);

    // Generate summary
    let summary: string;
    if (sim.feasible && sim.warning_count === 0) {
      summary = `Sequence feasibility PASSED: all ${sim.pass_count} operations clear. Success probability: ${(successProb * 100).toFixed(0)}%`;
    } else if (sim.feasible) {
      summary = `Sequence feasibility PASSED with warnings: ${sim.pass_count} pass, ${sim.warning_count} warnings. Review flagged operations.`;
    } else {
      summary = `Sequence feasibility FAILED: ${sim.fail_count} operations blocked, ${sim.dead_ends.length} dead-ends detected. Resequencing recommended.`;
    }

    return {
      feasible: sim.feasible,
      risk_score: sim.cumulative_risk,
      risks,
      dead_ends: sim.dead_ends.map(d => ({
        blocking_op: d.blocking_op,
        blocked_op: d.blocked_op,
        reason: d.reason,
      })),
      ...
      summary,
      operation_results: sim.per_operation.map(op => ({
  ...
  /**
   * Quick resequencing assessment for PUOA.
   * Returns whether resequencing can solve detected issues.
   */
  suggestResequencingForPUOA(input: ResequenceInput): PUOAResequenceSuggestion {
    const result = this.resequence(input);

    if (result.proof_of_infeasibility) {
      return {
        can_resequence: false,
        reason: result.proof_of_infeasibility,
        suggestions: [],
        confidence: 0.95,
      };
    }
    ...
    if (!result.best_sequence) {
      return {
        can_resequence: false,
        reason: "No valid sequence found within search limit",
        suggestions: [],
        confidence: 0.7,
      };
    }

    const suggestions = result.valid_sequences.slice(0, 3).map((seq, i) => ({
      order: seq.order,
      improvement: result.improvement_pct ?? 0,
      score: seq.score,
      reason: i === 0
        ? `Optimal sequence: ${seq.cycle_time_sec}s cycle, ${seq.tool_changes} tool changes`
        : `Alternative ${i + 1}: ${seq.cycle_time_sec}s cycle, ${seq.tool_changes} tool changes`,
    }));

    return {
      can_resequence: true,
      reason: `Found ${result.valid_sequences.length} valid sequences`,
      suggestions,
      confidence: 0.85,
    };
  }
