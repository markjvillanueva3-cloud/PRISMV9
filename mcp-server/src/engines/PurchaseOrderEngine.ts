...
class PurchaseOrderEngine {
  ...
  threeWayMatch(po_id: string, invoice_total: number, invoice_line_prices?: { line_id: string; price: number }[]): ThreeWayMatch {
    ...
    // Compare receipt value against the invoice's material value, not the tax-bearing grand total.
    // This preserves a clean 3-way match when the invoice total legitimately includes PO tax/shipping.
    const invoice_net_total = Math.max(0, invoice_total - po.tax - po.shipping);
    const receipt_vs_invoice_discrepancy = Math.abs(receiving_total - invoice_net_total);
    if (receipt_vs_invoice_discrepancy > 0.01) {
      discrepancies.push({ field: 'receipt_vs_invoice_subtotal', po_value: receiving_total, invoice_value: invoice_net_total, diff: invoice_net_total - receiving_total });
    }
