...
export class MastercamToolBridgeEngine extends CAMToolBridgeBase {
   ...
   * Delegates full physics-backed export to MastercamToolExportEngine (E1123)
   * when available; returns a lightweight format stub otherwise.
   */
  exportTools(prism_tools: ToolRecord[]): Record<string, unknown> {
    // Attempt delegation to E1123
    try {
      const { mastercamToolExportEngine } = require("./MastercamToolExportEngine.js");
      if (mastercamToolExportEngine?.exportLibrary) {
        return mastercamToolExportEngine.exportLibrary({ tools: prism_tools });
      }
    } catch { /* engine not loaded — fall back */ }

    // Lightweight stub output
    return {
      format: "mcam-tools",
      version: "2024",
      source: "PRISM E1143 (stub — E1123 not available)",
      ...
      tools: prism_tools.map((t) => ({
        ...
        coating: t.coating,
        material_group: Object.entries(MastercamToolBridgeEngine.MC_GROUP_TO_ISO)
          .find(([, v]) => v === t.iso_group)?.[0] ?? "1",
        cutting_data: {
...
export class HyperMillToolBridgeEngine extends CAMToolBridgeBase {
   ...
   * when available; returns lightweight format stub otherwise.
   */
  exportTools(prism_tools: ToolRecord[]): Record<string, unknown> {
    // Attempt delegation to E1127
    try {
      const { hyperMillToolExportEngine } = require("./HyperMillToolExportEngine.js");
      if (hyperMillToolExportEngine?.exportToHMT) {
        return hyperMillToolExportEngine.exportToHMT(prism_tools);
      }
    } catch { /* engine not loaded — fall back */ }

    // Lightweight stub: return SQL-ready row data
    return {
      format: "hypermill-hmt",
      version: "1.53",
      source: "PRISM E1143 (stub — E1127 not available)",
      tool_count: prism_tools.length,
      Tools: prism_tools.map((t) => {
        const typeId = Object.entries(HyperMillToolBridgeEngine.HM_TYPE_ID_MAP)
          .find(([, v]) => v === t.tool_type)?.[0] ?? "2";
        return {
          tool_id: this._coerceNum(t.cam_native_id) || t.tool_number,
          name: t.description,
          tool_type_id: Number(typeId),
          dbl_param1: t.diameter_mm,
          dbl_param2: t.corner_radius_mm,
          dbl_param3: t.flute_length_mm,
          dbl_param4: t.oal_mm,
          int_param1: t.flutes,
          manufacturer: t.manufacturer,
          article_number: t.manufacturer_part_no,
          cutting_material: t.material,
          coating: t.coating,
          cutting_speed: t.vc_recommended_m_min,
          feed_per_tooth: t.fz_recommended_mm,
          coolant_through: t.coolant_through ? 1 : 0,
          chipping_class: Object.entries(HyperMillToolBridgeEngine.CC_TO_ISO)
            .find(([, v]) => v === t.iso_group)?.[0] ?? "1",
        };
      }),
    };
