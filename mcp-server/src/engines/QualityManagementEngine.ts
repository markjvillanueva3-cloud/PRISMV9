...
export interface NCR {
  ...
  closed_at?: string;
  source?: 'receiving_inspection' | 'in_process' | 'final_inspection' | 'customer_complaint' | 'audit_finding';
}
...
export interface EightDReport {
  ...
  ncr_id: string;
  d1_team: string[];
  d2_problem_statement: string | undefined;
  d3_containment: string | undefined;
  d4_root_cause: string | undefined;
  d5_corrective_actions: string[] | undefined;
  d6_implementation_date: string | undefined;
  d7_prevention: string | undefined;
  d8_congratulate: string | undefined;
...
class QualityManagementEngine {
  ...
  private eightDReports: Map<string, EightDReport> = new Map();
  ...
  createSPCChart(params: {
    ...
    persistenceBridge.persist("quality_spc_data", key, { id: key, data: params.data } as any);
  ...
  addCalibration(params: Omit<CalibrationRecord, 'id' | 'status'>): CalibrationRecord {
    ...
    persistenceBridge.persist("quality_calibrations", id, record as any);
    return record;
  }
  ...
  addMaterialCert(params: Omit<MaterialCert, 'id'>): MaterialCert {
    ...
    persistenceBridge.persist("quality_material_certs", id, cert as any);
    return cert;
  }
  ...
  createNCR(params: Omit<NCR, 'id' | 'status' | 'closed_at'>): NCR {
    ...
    persistenceBridge.persist("quality_ncrs", id, ncr as any);
    return ncr;
  }
  ...
  createFAI(params: Omit<FAIRecord, 'id' | 'overall_pass'>): FAIRecord & { cascade?: Record<string, any> } {
    ...
    persistenceBridge.persist("quality_fais", id, fai as any);
  ...
  // --- 8D Workflow (U-BIZ42) ---
  createEightD(params: { ncr_id: string; d1_team: string[]; source: EightDReport['source'] }): EightDReport {
    if (!this.ncrs.has(params.ncr_id)) {
      throw new Error(`NCR not found: ${params.ncr_id}`);
    }
    const report: EightDReport = {
      id: `8D-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      ncr_id: params.ncr_id,
      d1_team: params.d1_team,
      d2_problem_statement: undefined,
      d3_containment: undefined,
      d4_root_cause: undefined,
      d5_corrective_actions: undefined,
      d6_implementation_date: undefined,
      d7_prevention: undefined,
      d8_congratulate: undefined,
    ...
    this.eightDReports.set(report.id, report);
    persistenceBridge.persist("eight_d_reports", report.id, report as any);
    return report;
  }

  advanceEightD(reportId: string, disciplineData: Partial<EightDReport>): EightDReport {
    const report = this.eightDReports.get(reportId);
    if (!report) throw new Error(`8D report not found: ${reportId}`);

    const nextD = report.current_discipline + 1;
    // Validate required fields per discipline
    if (nextD >= 2 && !disciplineData.d2_problem_statement && !report.d2_problem_statement) {
      throw new Error('D2 requires d2_problem_statement before advancing');
    }
    if (nextD >= 3 && !disciplineData.d3_containment && !report.d3_containment) {
      throw new Error('D3 requires d3_containment before advancing');
    }
    if (nextD >= 4 && !disciplineData.d4_root_cause && !report.d4_root_cause) {
      throw new Error('D4 requires d4_root_cause before advancing');
    }
    if (nextD >= 5) {
      const actions = disciplineData.d5_corrective_actions ?? report.d5_corrective_actions;
      if (!actions || actions.length === 0) {
        throw new Error('D5 requires d5_corrective_actions with at least one item');
      }
    }
    if (nextD >= 6 && !disciplineData.d6_implementation_date && !report.d6_implementation_date) {
      throw new Error('D6 requires d6_implementation_date before advancing');
    }
    if (nextD >= 7 && !disciplineData.d7_prevention && !report.d7_prevention) {
      throw new Error('D7 requires d7_prevention before advancing');
    }

    // Merge discipline data
    if (disciplineData.d2_problem_statement) report.d2_problem_statement = disciplineData.d2_problem_statement;
    if (disciplineData.d3_containment) report.d3_containment = disciplineData.d3_containment;
    if (disciplineData.d4_root_cause) report.d4_root_cause = disciplineData.d4_root_cause;
    if (disciplineData.d5_corrective_actions) report.d5_corrective_actions = disciplineData.d5_corrective_actions;
    if (disciplineData.d6_implementation_date) report.d6_implementation_date = disciplineData.d6_implementation_date;
    if (disciplineData.d7_prevention) report.d7_prevention = disciplineData.d7_prevention;
    if (disciplineData.d8_congratulate) report.d8_congratulate = disciplineData.d8_congratulate;
    if (disciplineData.d1_team) report.d1_team = disciplineData.d1_team;

    report.current_discipline = Math.min(nextD, 8);

    if (report.current_discipline >= 8 && report.d8_congratulate) {
  ...
  getEightDReport(reportId: string): EightDReport {
    const report = this.eightDReports.get(reportId);
    if (!report) throw new Error(`8D report not found: ${reportId}`);
    return report;
  }

  listEightDByNCR(ncrId: string): EightDReport[] {
    return Array.from(this.eightDReports.values()).filter(r => r.ncr_id === ncrId);
  }
}
...
persistenceBridge.registerMap({
  entity: "quality_spc_data",
  getMap: () => (qualityManagementEngine as any).spcData as Map<string, any>,
  keyField: "id",
});
persistenceBridge.registerMap({
  entity: "quality_calibrations",
  getMap: () => (qualityManagementEngine as any).calibrations as Map<string, any>,
  keyField: "id",
});
persistenceBridge.registerMap({
  entity: "quality_material_certs",
  getMap: () => (qualityManagementEngine as any).materialCerts as Map<string, any>,
  keyField: "id",
});
persistenceBridge.registerMap({
  entity: "quality_ncrs",
  getMap: () => (qualityManagementEngine as any).ncrs as Map<string, any>,
  keyField: "id",
});
persistenceBridge.registerMap({
  entity: "quality_fais",
  getMap: () => (qualityManagementEngine as any).fais as Map<string, any>,
  keyField: "id",
});
persistenceBridge.registerMap({
  entity: "eight_d_reports",
  getMap: () => (qualityManagementEngine as any).eightDReports as Map<string, any>,
  keyField: "id",
});
