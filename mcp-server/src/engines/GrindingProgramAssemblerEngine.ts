...
function genSurfaceGrindGCode(
  ...
  if (profile.operation_subtype === "creep_feed") {
    ...
    // Horizontal reciprocating (and rotary table)
    lines.push("");
    lines.push(`(--- ${passes} PASSES, ae=${a_e_mm.toFixed(4)}mm, Ycross=${cross_feed.toFixed(3)}mm ---)`);
    let depth = 0;
    for (let i = 0; i < passes; i++) {
      depth += a_e_mm;
      // Plunge infeed at end of stroke
      lines.push(`G01 ${fmtCoord("Z", -depth, ctrl)} ${fmtF(50, ctrl)} (INFEED PASS ${i + 1})`);
      const x_end = i % 2 === 0 ? profile.part_length_mm : 0;
      lines.push(`G01 ${fmtCoord("X", x_end, ctrl)} ${fmtF(v_w_mm_min, ctrl)}`);
      // Cross-feed step at each stroke reversal
      if (i < passes - 1) {
        lines.push(`G01 ${fmtCoord("Y", (Math.floor(i / 2) + 1) * cross_feed, ctrl)} ${fmtF(v_w_mm_min * 0.5, ctrl)}`);
      }
    }
    // Spark-out passes (2 passes at zero infeed)
    lines.push("(--- SPARK-OUT ---)")
    lines.push(`G01 ${fmtCoord("X", 0, ctrl)} ${fmtF(v_w_mm_min, ctrl)}`);
...
function genCylindricalGrindGCode(
  ...
  if (subtype === "od_plunge") {
    ...
    for (let i = 0; i < passes; i++) {
      const x_pos = (profile.workpiece_od_mm / 2) - (infeed_mm * (i + 1));
      lines.push(`G01 ${fmtCoord("X", x_pos, ctrl)} ${fmtF(30, ctrl)} (INFEED PASS ${i + 1})`);
    ...
    lines.push(`G01 ${fmtCoord("Z", z_start, ctrl)} ${fmtF(traverse_feed, ctrl)}`);
