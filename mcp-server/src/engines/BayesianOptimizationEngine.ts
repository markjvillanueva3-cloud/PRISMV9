...
class BayesianOptimizationEngineImpl {
  ...
  private _fitGP(obs: BOObservation[], ls: number, noise: number) {
    ...
    // Cholesky-like inverse via simple inversion for small n
    const Kinv = this._invertMatrix(K);

    // Alpha = K^(-1) * Y
    const alpha = Kinv.map(row => row.reduce((s, v, j) => s + v * Y[j], 0));

    return { X, Y, Kinv, alpha, ls, noise };
  }

  private _gpPredict(
    gp: { X: number[][]; Kinv: number[][]; alpha: number[]; ls: number; noise: number },
    x: number[]
  ): GPPrediction {
    ...
    const variance = Math.max(0, kxx - kStar.reduce((s, k, i) => s + k * v[i], 0));
