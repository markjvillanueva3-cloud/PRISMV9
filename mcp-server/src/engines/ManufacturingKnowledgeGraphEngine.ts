...
export type MfgNodeType = "material" | "tool" | "machine" | "operation" | "strategy" | "part" | "tribal_tip";
...
export class ManufacturingKnowledgeGraphEngine {
  ...
  calculate(action: string, params: Record<string, unknown> = {}): unknown {
    switch (action) {
      ...
      case "tribal_graph":
        return this.tribalGraph(params);
      case "tribal_traverse":
        return this.tribalTraverse(params);
      default:
        return { error: `Unknown action: ${action}`, validActions: ["kg_schema", "kg_populate", "kg_query", "kg_recommend", "kg_gap", "tribal_graph", "tribal_traverse"] };
    }
  }
  ...
  private getSchema(): {
    ...
    return {
      nodeTypes: ["material", "tool", "machine", "operation", "strategy", "part", "tribal_tip"],
      edgeTypes: ["compatible_with", "recommended_for", "proven_on", "requires", "produces", "related_to", "supports"],
  ...
  linkTip(
    ...
    // Create tip node with proper tribal_tip type (TK-MS6)
    this.addNode({
      id: nodeId,
      type: "tribal_tip",
  ...
  // ─── TK-MS6: Tribal Knowledge Graph Methods ──────────────────────────────

  /**
   * Populate the knowledge graph with tribal tips from TribalKnowledgeEngine.
   * Creates tip nodes and edges between tips with 2+ shared applicability tags.
   *
   * @param params - Optional: { min_shared_tags?: number, max_tips?: number }
   * @returns Population statistics
   * @milestone TK-MS6 U-TK29
   */
  tribalGraph(params: Record<string, unknown> = {}): TribalGraphResult {
    this.ensurePopulated();

    const minSharedTags = Number(params.min_shared_tags ?? 2);
    const maxTips = Number(params.max_tips ?? 10000);

    // Try to load tips from TribalKnowledgeEngine
    let tips: Array<{ id: string; title: string; body: string; tags: string[]; confidence: number; source: string }> = [];
    try {
      const { tribalKnowledgeEngine } = require("./TribalKnowledgeEngine.js");
      const allTips = tribalKnowledgeEngine.getTips?.() ?? tribalKnowledgeEngine.listTips?.() ?? [];
      tips = allTips.slice(0, maxTips).map((t: any) => ({
        id: t.id ?? t.tip_id ?? `tip_${Math.random().toString(36).slice(2, 10)}`,
        title: t.title ?? t.name ?? "",
        body: t.body ?? t.text ?? t.content ?? "",
        tags: t.tags ?? t.applicability_tags ?? [],
        confidence: t.confidence ?? 0.5,
        source: t.source ?? "tribal",
      }));
    } catch {
      // TribalKnowledgeEngine not available — return empty result
      return {
        tips_populated: 0,
        edges_created: 0,
        tip_nodes: 0,
        ...
        material_edges: 0,
        operation_edges: 0,
        error: "TribalKnowledgeEngine not available",
      };
    }
    ...
    let relatedEdges = 0;
    let materialEdges = 0;
    let operationEdges = 0;

    // Create tip nodes
    const tipNodeIds: string[] = [];
    for (const tip of tips) {
      const nodeId = `tip_${tip.id}`;
      ...
      this.addNode({
        id: nodeId,
        type: "tribal_tip",
        name: tip.title.substring(0, 80) || tip.body.substring(0, 80),
        properties: {
          tipId: tip.id,
          title: tip.title,
          body: tip.body.substring(0, 500),
          tags: tip.tags,
          confidence: tip.confidence,
          source: tip.source,
        },
      });
      tipNodeIds.push(nodeId);
      tipsPopulated++;

      // Link to material nodes based on tags
      for (const tag of tip.tags) {
        const [category, value] = tag.split(":");
        if (category === "material" && value) {
          const matNodeId = `mat_${value}`;
          if (this.nodes.has(matNodeId)) {
            this.addEdge({
              source: nodeId,
              target: matNodeId,
              type: "recommended_for",
              weight: tip.confidence * 0.8,
              metadata: { basis: "tribal_graph", source: tip.source },
            });
            materialEdges++;
          }
        } else if (category === "operation" && value) {
          const opNodeId = `op_${value.replace(/\s+/g, "_")}`;
          if (this.nodes.has(opNodeId)) {
            this.addEdge({
              source: nodeId,
              target: opNodeId,
              type: "compatible_with",
              weight: tip.confidence * 0.7,
              metadata: { basis: "tribal_graph", source: tip.source },
            });
            operationEdges++;
          }
    ...
    // Create related_to edges between tips with 2+ shared tags
    for (let i = 0; i < tipNodeIds.length; i++) {
      const nodeA = this.nodes.get(tipNodeIds[i]);
      if (!nodeA) continue;
      const tagsA = (nodeA.properties.tags as string[]) || [];

      for (let j = i + 1; j < tipNodeIds.length; j++) {
        const nodeB = this.nodes.get(tipNodeIds[j]);
        if (!nodeB) continue;
        const tagsB = (nodeB.properties.tags as string[]) || [];
        ...
        if (sharedTags.length >= minSharedTags) {
          this.addEdge({
            ...
            target: tipNodeIds[j],
            type: "related_to",
            weight: Math.min(1.0, 0.3 + sharedTags.length * 0.15),
            metadata: { shared_tags: sharedTags, count: sharedTags.length },
          });
          relatedEdges++;
        }
      }
    ...
    return {
      tips_populated: tipsPopulated,
      edges_created: relatedEdges + materialEdges + operationEdges,
      tip_nodes: [...this.nodes.values()].filter(n => n.type === "tribal_tip").length,
      related_edges: relatedEdges,
      material_edges: materialEdges,
      operation_edges: operationEdges,
    };
  }

  /**
   * Traverse the tribal knowledge graph for a given context.
   * Query like: "D2 on L300-M" finds all tips related to D2 tool steel on L300 machine.
   *
   * @param params - { query: string, material?: string, machine?: string, operation?: string, limit?: number }
   * @returns Matching tips with relevance scores
   * @milestone TK-MS6 U-TK29
   */
  tribalTraverse(params: Record<string, unknown>): TribalTraverseResult {
    this.ensurePopulated();

    const query = String(params.query || "").toLowerCase();
    const materialFilter = params.material ? String(params.material).toUpperCase() : undefined;
    const machineFilter = params.machine ? String(params.machine).toLowerCase() : undefined;
    const operationFilter = params.operation ? String(params.operation).toLowerCase() : undefined;
    const limit = Number(params.limit ?? 20);

    // Find all tribal_tip nodes
    const tipNodes = [...this.nodes.values()].filter(n => n.type === "tribal_tip");
    const results: TribalTraverseItem[] = [];

    for (const tip of tipNodes) {
      const tags = (tip.properties.tags as string[]) || [];
      const body = String(tip.properties.body || tip.name).toLowerCase();
      const title = String(tip.properties.title || tip.name).toLowerCase();
      let score = 0;
      const matchReasons: string[] = [];

      // Material filter match
      if (materialFilter) {
        const matTag = tags.find(t => t.startsWith("material:"));
        if (matTag && matTag.includes(materialFilter.toLowerCase())) {
          score += 0.4;
          matchReasons.push(`material:${materialFilter}`);
        } else if (body.includes(materialFilter.toLowerCase()) || title.includes(materialFilter.toLowerCase())) {
          score += 0.2;
          matchReasons.push(`text_material:${materialFilter}`);
        }
      }

      // Machine filter match
      if (machineFilter) {
        const machTag = tags.find(t => t.startsWith("machine:"));
        if (machTag && machTag.toLowerCase().includes(machineFilter)) {
          score += 0.3;
          matchReasons.push(`machine:${machineFilter}`);
        } else if (body.includes(machineFilter) || title.includes(machineFilter)) {
          score += 0.15;
          matchReasons.push(`text_machine:${machineFilter}`);
        }
      }

      // Operation filter match
      if (operationFilter) {
        const opTag = tags.find(t => t.startsWith("operation:"));
        if (opTag && opTag.toLowerCase().includes(operationFilter)) {
          score += 0.3;
          matchReasons.push(`operation:${operationFilter}`);
        } else if (body.includes(operationFilter) || title.includes(operationFilter)) {
          score += 0.15;
          matchReasons.push(`text_operation:${operationFilter}`);
        }
      }
      ...
      if (query) {
        ...
        for (const term of queryTerms) {
          if (body.includes(term) || title.includes(term)) {
            score += 0.1;
            matchReasons.push(`query:${term}`);
          }
          if (tags.some(t => t.toLowerCase().includes(term))) {
            score += 0.15;
            matchReasons.push(`tag:${term}`);
          }
        }
      }

      // Base confidence boost
      const confidence = Number(tip.properties.confidence ?? 0.5);
      score = score * (0.5 + confidence * 0.5);

      if (score > 0 || (!query && !materialFilter && !machineFilter && !operationFilter)) {
        results.push({
          tip_id: String(tip.properties.tipId || tip.id.replace("tip_", "")),
          title: String(tip.properties.title || tip.name),
          body: String(tip.properties.body || ""),
          score,
          confidence,
          match_reasons: matchReasons,
          tags,
          source: String(tip.properties.source || "unknown"),
        });
      }
    }
    ...
    return {
      query: { text: query, material: materialFilter, machine: machineFilter, operation: operationFilter },
      results: topResults,
      total_matches: results.length,
      tip_nodes_searched: tipNodes.length,
    };
  }
}

/** Result from tribalGraph() population */
export interface TribalGraphResult {
  tips_populated: number;
  edges_created: number;
  tip_nodes: number;
  related_edges: number;
  material_edges: number;
  operation_edges: number;
  error?: string;
}

/** Result from tribalTraverse() */
export interface TribalTraverseResult {
  query: { text: string; material?: string; machine?: string; operation?: string };
  results: TribalTraverseItem[];
  total_matches: number;
  tip_nodes_searched: number;
}

/** Individual item in tribalTraverse() results */
export interface TribalTraverseItem {
  tip_id: string;
  title: string;
  body: string;
  score: number;
