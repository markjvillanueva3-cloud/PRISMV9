...
export class ProcessFingerprintEngine {
  ...
  monitorDrift(params: DriftInput): DriftResult {
    ...
    // Hotelling T² (diagonal covariance approximation)
    let t_squared = 0;
    ...
    for (let j = 0; j < p; j++) {
      ...
      // Individual feature out of control (z > 3)
      if (Math.abs(z) > 3) {
        out_of_control_features.push(featureNames[j] || `feature_${j}`);
      }
    }

    // T² limit (chi-squared approximation: p * F(alpha, p, n-p))
    // Simplified: use p * (1 + 2/sqrt(n)) as approximate UCL
