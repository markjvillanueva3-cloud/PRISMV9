...
/** Thermal damage threshold: temp must be below 70% of melting point */
const THERMAL_DERATING = 0.7;
...
function computeChatterProbability(
  ...
  if (!natural_freqs_Hz || natural_freqs_Hz.length === 0) {
    // No modal data: assume moderate risk
    return 0.05;
  }
...
export class PipelineSafetyOrchestratorEngine {
  ...
  assess(
    ...
    const T_limit = material.T_melt_C * THERMAL_DERATING;
