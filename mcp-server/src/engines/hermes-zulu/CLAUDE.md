...
**LEADS ELSEWHERE (not EXCLUDED):** domain-specialist work is *led* by the domain slot
(mill→foxtrot, lathe→whiskey, wedm→mike, quoting→charlie). Under the multi-domain fleet policy
(operator 2026-06-30 — every chat has full codebase access), bravo/zulu MAY work in any domain;
the line above marks *who leads*, not a wall. Coordinate via chat-bus + `[MAIN-FORCE]`.
Still: do NOT treat zulu as a 13th worker slot — it is orchestration above the worker slots;
its multi-domain reach is for fleet synthesis + cross-slot coordination, not solo domain builds.
The Hermes parallel-agent work-loop (`HermesWorkSourceFeederEngine`) feeds units from ALL domains.
...
This galaxy is a first-class AI-substrate **participant** -- it OWNS 2 AI engine(s) (e.g. `ConsensusAIBridgeEngine`, `ConsensusNeuralFeedbackEngine`).
...
- **Reasoning bridge** (`scripts/lib/galaxy-reasoning-bridge.mjs`, PSN leg #10): **CAG** + **RAG** hybrid
  ...
  local Ollama stack -- `node scripts/lib/galaxy-reasoning-bridge.mjs hermes-zulu "<question>"`.
- **Vault -> LoRA**: this galaxy's Obsidian **synthesis** brain (`knowledge/memories/patterns/hermes-zulu_synthesis.md`)
