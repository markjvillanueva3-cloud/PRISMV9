...
export interface BobCADParameter {
  type: string;
  description?: string;
  unit?: string;
  default?: unknown;
...
export interface BobCADOperation {
  ...
  parameters: Record<string, Record<string, BobCADParameter>>;
}

export interface BobCADSection {
  ...
  operations: Record<string, BobCADOperation>;
...
export interface BobCADFunctionIndex {
  system_id: "bobcad";
  sections: Record<string, BobCADSection>;
  total_operations: number;
  total_parameters: number;
  categories: string[];
}
...
const SECTION_FILES: Array<{ file: string; key: string }> = [
  ...
  { file: "multi_axis.json", key: "multi_axis" },
  { file: "nesting.json", key: "nesting" },
  { file: "turning.json", key: "turning" },
];

export class BobCADCAMFunctionIndexEngine {
  private static index: BobCADFunctionIndex | null = null;

  private static readonly DATA_DIR = join(
    __dirname,
    "../../data/cam-functions/bobcad"
  );

  private static loadIndex(): BobCADFunctionIndex {
    if (this.index) return this.index;

    const sections: Record<string, BobCADSection> = {};
    let totalOps = 0;
    let totalParams = 0;
    ...
    for (const { file, key } of SECTION_FILES) {
      ...
      try {
        const data = JSON.parse(readFileSync(path, "utf-8")) as BobCADSection;
        sections[key] = data;
        ...
        throw new Error(
          `BobCADCAMFunctionIndexEngine: failed to parse ${file}: ${(err as Error).message}`
        );
      }
    }
  ...
  static getIndex(): BobCADFunctionIndex {
  ...
  static getSection(
    sectionKey: string
  ): BobCADSection | { error: string; available: string[] } {
  ...
  /**
   * DMT (Dynamic Motion Technology) operations — BobCAD's signature HSM surface.
   */
  static getDMTOperations(): Array<{
    operation_id: string;
    display_name: string;
    section: string;
    description: string;
    ...
    for (const [sectionKey, section] of Object.entries(idx.sections)) {
      for (const [opId, op] of Object.entries(section.operations || {})) {
        if (op.category.toLowerCase().includes("dmt")) {
          out.push({
            operation_id: opId,
            display_name: op.display_name,
            section: sectionKey,
  ...
  /**
   * Nesting operations — BobCAD's strong laser/plasma/waterjet workflow.
   */
  ...
  static getOperation(operationId: string):
    | { operation_id: string; section: string; operation: BobCADOperation }
    | { error: string } {
