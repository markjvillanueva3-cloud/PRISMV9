 ...
 *   - C:/PRISM/HYPERMILL/NcGenerator/33.0/ (post-processor configs)
...
function buildHeader(params: HMGenerateParams, opCount: number): string {
  ...
  return [
    `# hyperMILL Automation Center Python Script`,
    `# Part: ${partName}  |  Job: ${jobName}`,
    `# Generated by PRISM HyperMillCodeGeneratorEngine (E1120)`,
    `# hyperMILL v${ver} — ${opCount} operation(s)`,
    `#`,
    `# Usage: Run via hyperMILL Automation Center > Execute Script`,
    ``,
    `import hypermill as hm`,
    `import os`,
    ``,
  ].join("\n");
}
...
function buildErrorHandlerClose(): string {
  return [
    ...
    `    hm.MessageBox(f"Script error: {e}", "PRISM AC Error")`,
    `    raise`,
  ].join("\n");
}
...
function buildJobSetup(params: HMGenerateParams): string {
  ...
  lines.push(`${indent}# ── Job Setup ──────────────────────────────────────`);
  lines.push(`${indent}app = hm.Application.GetInstance()`);
  lines.push(`${indent}doc = app.ActiveDocument`);
  if (params.job_name) {
    lines.push(`${indent}job_list = doc.CreateJobList("${params.job_name}")`);
  } else {
    lines.push(`${indent}job_list = doc.ActiveJobList`);
  }
  lines.push(`${indent}job_list.SetMachine("${machineName}")`);
  lines.push(`${indent}job_list.SetWCS("${wcsName}")`);
  if (stockName) {
    lines.push(`${indent}job_list.SetStockModel("${stockName}")`);
  }
  lines.push(``);
  return lines.join("\n");
}
...
function buildToolSetup(tools: HMTool[], params: HMGenerateParams): string {
  ...
  const indent = params.add_error_handling ? "    " : "";
  const lines: string[] = [
    `${indent}# ── Tool Library Setup ──────────────────────────────`,
    `${indent}tool_db = hm.ToolDatabase.GetInstance()`,
    `${indent}tool_db.BeginTransaction()`,
  ];
  ...
  for (const t of tools) {
    ...
    lines.push(`${indent}t${t.number} = tool_db.FindOrCreateTool(${t.number}, "${t.type}")`);
    lines.push(`${indent}t${t.number}.SetNumber(${t.number})`);
    lines.push(`${indent}t${t.number}.SetDiameter(${t.diameter_mm})`);
    lines.push(`${indent}t${t.number}.SetDescription("${label}")`);
    if (t.corner_radius_mm != null) {
      lines.push(`${indent}t${t.number}.SetCornerRadius(${t.corner_radius_mm})`);
    }
    if (t.flute_length_mm != null) {
      lines.push(`${indent}t${t.number}.SetFluteLength(${t.flute_length_mm})`);
    }
    if (t.overall_length_mm != null) {
      lines.push(`${indent}t${t.number}.SetOverallLength(${t.overall_length_mm})`);
    }
    if (t.flutes != null) {
      lines.push(`${indent}t${t.number}.SetNumberOfTeeth(${t.flutes})`);
    }
    if (t.material) {
      lines.push(`${indent}t${t.number}.SetSubstrate("${t.material.toUpperCase()}")`);
    }
    if (t.coating) {
      lines.push(`${indent}t${t.number}.SetCoating("${t.coating}")`);
    }
  }

  lines.push(`${indent}tool_db.CommitTransaction()`);
  lines.push(``);
  return lines.join("\n");
}
...
function buildOperation(op: HMOperation, idx: number, params: HMGenerateParams): string {
  ...
  if (op.tool_number != null) {
    lines.push(`${indent}op${idx}.SetTool(${op.tool_number})`);
  }
  if (op.spindle_rpm != null) {
    lines.push(`${indent}op${idx}.SetSpindleSpeed(${op.spindle_rpm})`);
  }
  if (op.feed_mm_min != null) {
    lines.push(`${indent}op${idx}.SetFeedRate(${op.feed_mm_min})`);
  }
  if (op.stepdown_mm != null) {
    lines.push(`${indent}op${idx}.SetParameter("ap", ${op.stepdown_mm})`);
  }
  if (op.stepover_mm != null) {
    lines.push(`${indent}op${idx}.SetParameter("ae", ${op.stepover_mm})`);
  }
  if (op.scallop_height_mm != null) {
    lines.push(`${indent}op${idx}.SetParameter("scallop_height", ${op.scallop_height_mm})`);
  }
  if (op.stock_leave_mm != null) {
    lines.push(`${indent}op${idx}.SetParameter("stock_leave", ${op.stock_leave_mm})`);
  }
  if (op.floor_stock_mm != null) {
    lines.push(`${indent}op${idx}.SetParameter("floor_stock", ${op.floor_stock_mm})`);
  }
  if (op.angle_deg != null) {
    lines.push(`${indent}op${idx}.SetParameter("direction", ${op.angle_deg})`);
  }
  if (op.pass_count != null) {
    lines.push(`${indent}op${idx}.SetParameter("pass_count", ${op.pass_count})`);
  }
  if (op.cut_direction) {
