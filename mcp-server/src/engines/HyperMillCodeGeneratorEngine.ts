...
function buildOperation(op: HMOperation, idx: number, params: HMGenerateParams): string {
  ...
  // Only turning jobs use TCUTTINGSPEED/FVORSCHUB in CfgParameters.
  const isTurning = /turn|lathe|groo|thread|bore/i.test(cycleCode ?? "");
  if (isTurning && op.spindle_rpm != null) {
    cfgEntries.push(`"TCUTTINGSPEED": "${op.spindle_rpm}"`);
  }
  if (isTurning && op.feed_mm_min != null) {
    cfgEntries.push(`"FVORSCHUB": "${op.feed_mm_min}"`);
  }
  if (op.stepdown_mm != null) {
    cfgEntries.push(`"VERTZUSTEL": "${op.stepdown_mm}"`);
  }
  if (op.stepover_mm != null) {
    cfgEntries.push(`"HORIZUSTEL": "${op.stepover_mm}"`);
  }
  if (op.scallop_height_mm != null) {
    cfgEntries.push(`"HORIZUSTEL": "${op.scallop_height_mm}"`);
  }
  if (op.stock_leave_mm != null) {
    cfgEntries.push(`"AUFMASS": "${op.stock_leave_mm}"`);
  }
  if (op.floor_stock_mm != null) {
    cfgEntries.push(`"AUFMASS_Z": "${op.floor_stock_mm}"`);
  }
