...
export interface CombinationRunInput {
  ...
  /** Axial depth-of-cut (ap) grid in mm. When set, the sweep adds an ap axis --
   *  each value is forwarded to NineAxisToolpath.axial_depth_mm. Default omitted ->
   *  orchestrator derives ap from diameter (dia*0.5). Enables the operator's .001in
   *  (0.0254mm) length-of-cut x depth-of-cut fine sweep. */
  axial_depths_mm?: number[];
  /** Radial depth-of-cut / length-of-engagement (ae) grid in mm. When set, the sweep
   *  adds an ae axis -> NineAxisToolpath.radial_depth_mm. Default omitted ->
   *  orchestrator derives ae from diameter (dia*0.4). */
  radial_depths_mm?: number[];
}
...
export interface CombinationCellResult {
  ...
  input_summary: {
    ...
    /** Axial depth-of-cut (ap, mm) when swept on the LOC x DOC axis; omitted -> orchestrator default. */
    axial_depth_mm?: number;
    /** Radial depth-of-cut (ae, mm) when swept; omitted -> orchestrator default. */
...
const DOMAIN_DEFAULTS = {
  mill: {
    ...
    strategies_full: ["conventional", "adaptive", "trochoidal"] as const,
    coolants: ["flood"] as const,
    coolants_full: ["flood", "mist", "dry"] as const,
    holders: ["cat40"] as const,
    holders_full: ["cat40", "hsk_a63", "er_collet"] as const,
  },
  lathe: {
    diameters_mm: [25],
...
export class SpeedFeedExhaustiveCombinationEngine {
  ...
  *runStreaming(req: CombinationRunInput): Generator<CombinationCellResult> {
  ...
  private enumerateCells(req: CombinationRunInput, mode: SampleMode): NineAxisInput[] {
    ...
    // LOC x DOC axis (operator .001in fine sweep): when ap/ae grids are provided, sweep
    // them; otherwise a single [undefined] pass keeps the orchestrator's diameter-derived
    // default ap/ae -- byte-identical to the pre-axis behavior.
    const apGrid = req.axial_depths_mm && req.axial_depths_mm.length ? req.axial_depths_mm : [undefined];
    const aeGrid = req.radial_depths_mm && req.radial_depths_mm.length ? req.radial_depths_mm : [undefined];
                  ...
                  for (const coolant of coolantGrid) {
                    for (const holder of holderGrid) {
                      for (const optMode of modes) {
                        for (const ap of apGrid) {
                          for (const ae of aeGrid) {
                            cells.push({
                              ...
                              toolpath: {
                                ...
                                cut_type: cutType,
                                strategy: strat,
                                ...(ap != null ? { axial_depth_mm: ap } : {}),
                                ...(ae != null ? { radial_depth_mm: ae } : {}),
                              },
                              mode: optMode,
                            });
                          }
