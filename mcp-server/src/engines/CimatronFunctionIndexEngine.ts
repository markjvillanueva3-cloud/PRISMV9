 ...
 *                   tap_thread_5x)
 *
 * Reference: Cimatron 16 Reference / Hexagon mold-die training docs.
 */

import { readFileSync, existsSync } from "fs";
import { join } from "path";

export interface CimatronParameter {
  type: string;
  description?: string;
  unit?: string;
  default?: unknown;
...
export interface CimatronOperation {
  ...
  parameters: Record<string, Record<string, CimatronParameter>>;
}

export interface CimatronSection {
  ...
  operations: Record<string, CimatronOperation>;
...
export interface CimatronFunctionIndex {
  system_id: "cimatron";
  sections: Record<string, CimatronSection>;
  total_operations: number;
  total_parameters: number;
  categories: string[];
}
...
const SECTION_FILES: Array<{ file: string; key: string }> = [
  ...
  { file: "drill_grid.json", key: "drill_grid" },
];

export class CimatronFunctionIndexEngine {
  private static index: CimatronFunctionIndex | null = null;

  private static readonly DATA_DIR = join(
    __dirname,
    "../../data/cam-functions/cimatron"
  );

  private static loadIndex(): CimatronFunctionIndex {
    if (this.index) return this.index;

    const sections: Record<string, CimatronSection> = {};
    let totalOps = 0;
    let totalParams = 0;
    ...
    for (const { file, key } of SECTION_FILES) {
      ...
      try {
        const data = JSON.parse(readFileSync(path, "utf-8")) as CimatronSection;
        sections[key] = data;
        ...
        throw new Error(
          `CimatronFunctionIndexEngine: failed to parse ${file}: ${(err as Error).message}`
        );
      }
    }
  ...
  static getIndex(): CimatronFunctionIndex {
  ...
  static getSection(
    sectionKey: string
  ): CimatronSection | { error: string; available: string[] } {
  ...
  /**
   * Mold/die operations — Cimatron's flagship surface (MoldDesign + DieDesign).
   */
  static getMoldDieOperations(): Array<{
    ...
    for (const [sectionKey, section] of Object.entries(idx.sections)) {
      const isMoldDie =
        sectionKey === "mold_die" ||
        sectionKey === "electrode" ||
        section.summary?.categories?.some((c) =>
          ["core_cavity", "parting_design", "sliding_components", "die_design", "quick_electrode", "electrode_design"].includes(c)
        );
      if (!isMoldDie) continue;
      for (const [opId, op] of Object.entries(section.operations || {})) {
        out.push({
          operation_id: opId,
          display_name: op.display_name,
  ...
  /**
   * Drill5x operations — Cimatron's flagship 5-axis hole-pattern automation.
   */
  static getDrill5xOperations(): Array<{
    operation_id: string;
    display_name: string;
    section: string;
    category: string;
    ...
    const ds = idx.sections["drill_grid"];
    if (!ds) return out;
    for (const [opId, op] of Object.entries(ds.operations || {})) {
      out.push({
        operation_id: opId,
        display_name: op.display_name,
        section: "drill_grid",
  ...
  static getOperation(operationId: string):
    | { operation_id: string; section: string; operation: CimatronOperation }
    | { error: string } {
