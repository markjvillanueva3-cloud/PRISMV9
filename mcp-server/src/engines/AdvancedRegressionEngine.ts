...
/** Solve (A + αI)x = b via Cholesky or LU-like pivoted Gaussian elimination */
...
export class AdvancedRegressionEngine {
  ...
  huberRegression(params: HuberInput): HuberResult {
    const { X, y, delta = 1.35, max_iter = 50 } = params;
    const n = X.length;
    const p = X[0].length;
  ...
  private predictXGBTree(node: any, x: number[]): number {
