...
export class MultiAxisKinematicEngine {
  ...
  private singularityProximity(
    ...
    switch (kinematics.type) {
      case "head-table": {
        // Singularity when primary (A or B tilt) is at 0 — tool axis
        // parallel to spindle, no unique C solution
        const distFromZero = Math.abs(primaryDeg);
        const distFrom90 = Math.abs(Math.abs(primaryDeg) - 90);
        // Also near +-90 for some configurations
        return Math.min(distFromZero, distFrom90);
      }
      case "table-table": {
        // Singularity when primary tilt puts workpiece vertical — tool
        // vector has no unique decomposition into two table rotations
        const distFromCritical = Math.abs(Math.abs(primaryDeg) - 90);
        return distFromCritical;
      }
      case "head-head": {
  ...
  private inverseKinematics(
    ...
    // Compensate pivot point offset
    const tipPos: Vec3 = {
      x: move.x + kin.pivot_point.x * (1 - Math.cos(pRad)),
      y: move.y + kin.pivot_point.y * (1 - Math.cos(sRad)),
      z: move.z + kin.pivot_point.z * (1 - Math.cos(pRad)),
    };
