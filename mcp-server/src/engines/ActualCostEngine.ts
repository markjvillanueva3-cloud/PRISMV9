...
class ActualCostEngine {
  ...
  recordMaterialCost(jobId: string, cost: number, scrapCost: number = 0, materialType?: string): void {
    ...
    persistenceBridge.persist("actual_cost_material", jobId, { id: jobId, job_id: jobId, cost, scrap_cost: scrapCost } as any);
    if (materialType) {
      this.materialTypes.set(jobId, materialType);
      persistenceBridge.persist("actual_cost_material_types", jobId, { id: jobId, job_id: jobId, material_type: materialType } as any);
    }
  }
  ...
  recordMachineTime(jobId: string, hours: number, ratePerHour: number = 85): void {
    ...
    persistenceBridge.persist("actual_cost_machine", jobId, { id: jobId, job_id: jobId, hours, rate: ratePerHour } as any);
  }

  /** Set overhead rate for a job. */
  setOverheadRate(jobId: string, ratePct: number): void {
    this.overheadRates.set(jobId, ratePct);
    persistenceBridge.persist("actual_cost_overhead", jobId, { id: jobId, job_id: jobId, rate_pct: ratePct } as any);
  }
  ...
  recordEstimate(jobId: string, estimates: Record<string, number>): void {
    ...
    persistenceBridge.persist("actual_cost_estimates", jobId, { id: jobId, job_id: jobId, estimates } as any);
  }
  ...
  recordRevenue(jobId: string, revenue: number): void {
    ...
    persistenceBridge.persist("actual_cost_revenues", jobId, { id: jobId, job_id: jobId, revenue } as any);
  }
...
persistenceBridge.registerMap({
  entity: "actual_cost_material",
  getMap: () => (actualCostEngine as any).materialCosts as Map<string, any>,
  keyField: "job_id",
});
persistenceBridge.registerMap({
  entity: "actual_cost_machine",
  getMap: () => (actualCostEngine as any).machineCosts as Map<string, any>,
  keyField: "job_id",
});
persistenceBridge.registerMap({
  entity: "actual_cost_overhead",
  getMap: () => (actualCostEngine as any).overheadRates as Map<string, any>,
  keyField: "job_id",
  fromRecord: (record) => record.rate_pct as number,
});
persistenceBridge.registerMap({
  entity: "actual_cost_estimates",
  getMap: () => (actualCostEngine as any).estimates as Map<string, any>,
  keyField: "job_id",
});
persistenceBridge.registerMap({
  entity: "actual_cost_revenues",
  getMap: () => (actualCostEngine as any).revenues as Map<string, any>,
  keyField: "job_id",
  fromRecord: (record) => record.revenue as number,
});
persistenceBridge.registerMap({
  entity: "actual_cost_material_types",
  getMap: () => (actualCostEngine as any).materialTypes as Map<string, any>,
  keyField: "job_id",
});
