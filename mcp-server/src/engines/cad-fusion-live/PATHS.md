# cad-fusion-live galaxy — PATHS atlas (auto-derived baseline)
...
## Engines (name-matched to this domain, 236 found — verify ownership)
...
- `mcp-server/src/engines/AutodeskFusionMCPProxyEngine.ts`
- `mcp-server/src/engines/BatchDeliverableEngine.ts`
...
- `mcp-server/src/engines/CADAIStateMachineEngine.ts`
...
- `mcp-server/src/engines/CADArgEncoderEngine.ts`
...
- `mcp-server/src/engines/CADBundleSigningVersioningEngine.ts`
- `mcp-server/src/engines/CADCAMGenerationTestEngine.ts`
- `mcp-server/src/engines/CADCanonicalTreeAdapterEngine.ts`
...
- `mcp-server/src/engines/CADContentAddressableStoreEngine.ts`
...
- `mcp-server/src/engines/CADCrashRecoveryEngine.ts`
...
- `mcp-server/src/engines/CADExecutionOutcomeBusEngine.ts`
- `mcp-server/src/engines/CADFailureTriageEngine.ts`
- `mcp-server/src/engines/CADFallbackRoutingEngine.test.ts`
- `mcp-server/src/engines/CADFallbackRoutingEngine.ts`
...
- `mcp-server/src/engines/CADFormatConversionMatrixEngine.ts`
- `mcp-server/src/engines/CADFoundationEncoderEngine.ts`
- `mcp-server/src/engines/CADFunctionParameterEmitterEngine.ts`
- `mcp-server/src/engines/CADGeometricAugmentationEngine.ts`
- `mcp-server/src/engines/CADGeometryComparisonEngine.ts`
- `mcp-server/src/engines/CADHeadReplayBufferEngine.ts`
- `mcp-server/src/engines/CADInstallationProbeEngine.ts`
- `mcp-server/src/engines/CADIntentDecomposerEngine.ts`
- `mcp-server/src/engines/CADJMDieArchetypeFrequencyEngine.ts`
- …and 176 more (keyword match — prune false positives)
...
- **Domain-relevant for cad-fusion-live:** `resources/FUSION360` · `resources/FUSION POSTS` · `resources/fusion-addin` · `JM DIE/FUSION CAD AND CAM FILES`
...
- **wiki (domain):** `knowledge/wiki/cad-fusion-live/` (5 entries) -- query before re-deriving
- **synthesis brain (Obsidian):** `knowledge/memories/patterns/cad-fusion-live_synthesis.md` -- compounded domain patterns; refresh via `galaxy-synthesis-refresh.mjs`
- **galaxy brain:** `CLAUDE.md` · `MEMORY.md` · `SOUL.md` · `AWARENESS.md` · `TOOLBELT.md` (this dir)

**Uniform recall routing (same for every domain):**
- memories (domain-tagged, flat corpus): `prism_memory:semantic_search query="cad-fusion-live" topK=20` -- auto-fed every Stop by `stop-obsidian-memory-feed.mjs`
...
- AI reasoning over ALL of the above (PSN leg #10): `node scripts/lib/galaxy-reasoning-bridge.mjs cad-fusion-live "<question>"` (hybrid CAG+RAG, local Ollama, $0)
