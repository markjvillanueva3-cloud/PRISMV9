...
export class EDMStartHoleSetupEngine {
  ...
  planStartHoles(
    ...
    // Sinker EDM start hole recommendation for thick stock
    // Thick hardened material (>80mm) is commonly pre-holed by sinker EDM before wire EDM
    if (workpiece.thickness_mm > 80 && (workpiece.is_hardened || workpiece.hardness_hrc >= 50)) {
      const sinkerTimePerHole = workpiece.thickness_mm * 0.3; // ~0.3 min/mm for sinker EDM hole
      const totalSinkerTime = round2(sinkerTimePerHole * holes.length);
      notes.push(
        `SINKER EDM RECOMMENDED for start holes: ${workpiece.thickness_mm}mm × ${workpiece.hardness_hrc} HRC ` +
        `is beyond efficient EDM hole popper range. Use sinker EDM with Cu or graphite electrode ` +
        `(est. ${totalSinkerTime} min for ${holes.length} hole${holes.length > 1 ? "s" : ""}). ` +
        `Two-step process: (1) sinker pre-holes, (2) wire EDM cutting.`
      );
    } else if (workpiece.thickness_mm > 120) {
      notes.push(
        `Very thick stock (${workpiece.thickness_mm}mm): consider sinker EDM for start holes — ` +
        `faster and more reliable than hole popper at this depth.`
      );
    }
