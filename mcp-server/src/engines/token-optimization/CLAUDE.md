...
6. **Subagent fan-out prompt overflow — MEASURED non-problem (2026-06-21).** The ">200K cold-cache anchors into every subagent" was an unverified assumption: `cag-cold-cache-anchor.mjs` emits only a ~4KB SUMMARY (not the doctrine text it catalogs), and the live per-subagent (Task/Agent-spawn) injection ceiling measures **3.65 KB** via `scripts/measure-subagent-injection.mjs` (cf40d23901). Subagents get context via `subagent-start-context.mjs` (SubagentStart), itself bounded. No "gate SessionStart off for subagents" fix needed. [[reference_subagent_injection_measured_2026_06_21]]
...
node scripts/measure-subagent-injection.test.mjs   # subagent-injection instrument (15 cases)
node scripts/measure-subagent-injection.mjs        # live per-subagent (Task/Agent-spawn) injection ceiling (~3.65KB)
node scripts/audit-injection-surface.mjs --bytes   # per-turn (SessionStart+UserPromptSubmit) injection census
```
...
- **Subagent SessionStart overflow — RESOLVED 2026-06-21 (MEASURED non-problem):** the ">200K anchor tokens into every subagent" claim was an unverified assumption. `scripts/measure-subagent-injection.mjs` (U-SUBAGENT-INJECTION-MEASURE, cf40d23901) measures the live per-subagent (Task/Agent-spawn) injection ceiling = **3.65 KB** (cag-cold-cache-anchor=4KB summary, agent-rules-inject=3.5KB; both capped). NO "gate SessionStart off" fix needed. Surfaced + logged: `agent-rules-inject` is Task-name-gated (dead for the current "Agent" tool) but HARMLESS — `subagent-start-context.mjs` (SubagentStart) provides the subagent bundle; agent-rules-inject is a legacy-redundant cleanup candidate. [[reference_subagent_injection_measured_2026_06_21]]
