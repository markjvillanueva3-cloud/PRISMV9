 ...
 */
export const QUOTING_DISPATCH_DENY_SCHEMA_VERSION = "1.1.1";
...
export const QUOTING_GENERIC_DISPATCH_DENY_SET: ReadonlySet<string> = new Set<string>([
  ...
  //   (the shop's own per-op rates) into total_secondary_ops_usd + per-op setup/per_part cost. The PLAIN
  //   `quoting_secondary_ops_price` (caller-supplied overrides only) stays OUT -- it has a shipped
  //   token-less caller (QuotingWorkbenchPage) and needs an auth-migration, not a blunt deny.
