...
describe("Section 1: Tool Steel Material Coverage", () => {
  ...
  // [S3][real-shop] D2 25mm rough: real ITW SHAKEPROOF = F0.12 in/min = 3.05 mm/min ≈ 50mm rate
  // Wire EDM feed is nearly flat across thicknesses (not exponential) — confirmed by real programs
  it("T002: D2 25mm rough speed in real-world range [S3]", () => {
    ...
    // Both should be in the same ballpark — feed is nearly flat for moderate thicknesses
    expect(thin.first_cut_speed_mm_per_min).toBeGreaterThanOrEqual(2);
    expect(thin.first_cut_speed_mm_per_min).toBeLessThanOrEqual(6);
    // Thin should not be dramatically slower than thick
    expect(thin.first_cut_speed_mm_per_min).toBeGreaterThanOrEqual(thick.first_cut_speed_mm_per_min * 0.7);
  });
  ...
  // [S6][calibrated] D2 10mm: thin stock slightly faster due to flushing, ~3-5 mm/min
  // Old claim of 8-15 mm/min was based on inflated thickness-scaling model (h^-1.5).
  // Calibrated to real shop data: feed is flat ~3mm/min across moderate thicknesses.
  it("T004: D2 10mm rough speed in valid range [S6]", () => {
    const r = wireEDMSettingsEngine.calculate(settingsInput({
...
describe("Section 2: Non-Ferrous & Exotic Materials", () => {
  ...
  // --- Aluminum 6061 ---
  // [S2] Makino: Al 6061 25mm → faster than tool steel (mrr_factor 1.5 vs 0.9)
  // Ratio ~1.5-2.0× calibrated: aluminum cuts ~67% faster than D2 due to MRR factor difference
  it("T021: Al 6061 25mm rough speed faster than D2 steel [S2]", () => {
