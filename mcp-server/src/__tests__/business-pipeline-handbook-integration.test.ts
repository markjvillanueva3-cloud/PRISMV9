...
describe("ShopConfigurationEngine — selectCapableMachines (U02)", () => {
  ...
  it("filters by capability tags", () => {
    ...
    // Roku-Roku and Haas OM-2 have engraving capability
    expect(capable.length).toBeGreaterThanOrEqual(1);
    expect(capable.some((m: any) => m.machine_name.includes("Roku-Roku") || m.machine_name.includes("OM-2"))).toBe(true);
    // Lathes and EDMs should be rejected — don't have engraving
  ...
  it("filters by turning capability", () => {
    ...
    const capable = result.filter((m: any) => m.rejection_reasons.length === 0);
    // Multiple Okuma lathes + manual lathe + Multus should pass
    expect(capable.length).toBeGreaterThanOrEqual(7);
  ...
  it("rejects wire EDM for milling capability", () => {
    const result = shopConfigurationEngine.selectCapableMachines({
      capabilities: ["milling"],
    });
    const wedm = result.find((m: any) => m.machine_id === "WEDM-01");
    expect(wedm).toBeDefined();
    expect(wedm.rejection_reasons.length).toBeGreaterThan(0);
    expect(wedm.rejection_reasons[0]).toContain("milling");
  });
});
...
describe("Cost comparison — handbook-sourced rates vs actual (U03)", () => {
  ...
  it("VMC base rate is within industry range ($50-$150/hr)", () => {
    const rate = shopConfigurationEngine.getMachineRate("default", "VMC-01");
    expect(rate).toBeGreaterThanOrEqual(50);
    expect(rate).toBeLessThanOrEqual(150);
  });

  it("Wire EDM base rate is within industry range ($70-$120/hr)", () => {
    const rate = shopConfigurationEngine.getMachineRate("default", "WEDM-01");
    expect(rate).toBeGreaterThanOrEqual(70);
    expect(rate).toBeLessThanOrEqual(120);
  });

  it("Lathe base rate is within industry range ($60-$130/hr)", () => {
    const rate = shopConfigurationEngine.getMachineRate("default", "LTH-01");
    expect(rate).toBeGreaterThanOrEqual(60);
    expect(rate).toBeLessThanOrEqual(130);
  });
