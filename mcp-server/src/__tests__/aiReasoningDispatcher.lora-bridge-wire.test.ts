...
describe("aiReasoningDispatcher → blueprint_lora_prepare_set", () => {
  ...
  // U-BPA-LORA-PAIRS-WIRE: precomputedPairs[] is now OPTIONAL on BOTH dispatchers
  // (clone-don't-fork of the cadDispatcher rewire). An absent precomputedPairs no
  // longer rejects -- it DEFAULTS the training data from the closed-loop ledger,
  // surfaced via pairSource (R12 -- no silent source swap).
  // NB: prism_ai wraps as {success:true, data: slimResponse(selection)}, so the
  // selection (with pairSource/empty) is at r.data.data.* (one level deeper than
  // cadDispatcher). slimResponse preserves shallow scalars + drops empty arrays.
  it("precomputedPairs[] optional: defaults from the closed-loop ledger (pairSource=ledger)", async () => {
    const r = await call(server, "blueprint_lora_prepare_set", { confidenceTier: "operator_verified" });
    expect(r.ok).toBe(true);
    expect((r.data.data as Record<string, unknown>).pairSource).toBe("ledger");
  });

  it("backward-compat: caller-supplied precomputedPairs[] still accepted (pairSource=caller)", async () => {
    const r = await call(server, "blueprint_lora_prepare_set", {
      confidenceTier: "operator_verified",
      precomputedPairs: [
        { pairId: "p1", customer: "", partNumber: "X", pdfPath: "/x.pdf", extractionType: "diameter", groundTruthValue: ".500", context: "c" },
      ],
    });
    expect(r.ok).toBe(true);
    expect((r.data.data as Record<string, unknown>).pairSource).toBe("caller");
  });

  it("empty-tier loud signal: a tier with no confirmed ground truth flags empty:true (R12)", async () => {
    // single_backend has 0 trainable ledger rows today -> ledger default is empty.
    const r = await call(server, "blueprint_lora_prepare_set", { confidenceTier: "single_backend" });
    expect(r.ok).toBe(true);
    const inner = r.data.data as Record<string, unknown>;
    expect(inner.pairSource).toBe("ledger");
