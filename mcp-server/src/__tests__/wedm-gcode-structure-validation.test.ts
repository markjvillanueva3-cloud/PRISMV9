...
describe("Program-level structural integrity", () => {
  ...
  it("line numbers are sequential", () => {
    // Mitsubishi uses restart markers at fixed N-blocks (N1100, N1200, etc.)
    // These are intentional jump-points for operator restart — they break global
    // monotonicity. Verify: (a) no duplicate N-numbers, (b) sequential within each
    // segment between restart markers, (c) restart markers have expected N-values.
    const result = engine.generate_gcode(makeITWShakeproofInput());

    // Restart markers: N{(profile+1)*1000 + (pass+1)*100}
    // Profile 1, Passes 1-4 → N1100, N1200, N1300, N1400
    const RESTART_BLOCKS = new Set([1100, 1200, 1300, 1400]);
    ...
    // (b) Restart markers present for all 4 passes
    for (const rb of RESTART_BLOCKS) {
      expect(seen.has(rb)).toBe(true);
    }

    // (c) Sequential within segments (between restart markers)
    let segNums: number[] = [];
    for (const n of lineNums) {
      if (RESTART_BLOCKS.has(n)) {
        // End of segment — check it was sequential
        for (let i = 1; i < segNums.length; i++) {
          expect(segNums[i]).toBeGreaterThan(segNums[i - 1]);
        }
        segNums = [];
      } else {
        segNums.push(n);
    ...
    // (d) Total line count is substantial (not empty program)
    expect(lineNums.length).toBeGreaterThan(20);
  });
