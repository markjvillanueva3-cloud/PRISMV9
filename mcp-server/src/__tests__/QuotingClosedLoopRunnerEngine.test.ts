...
import { QuotingOutcomeCaptureWireEngine } from "../engines/QuotingOutcomeCaptureWireEngine.js";
import {
...
// U-CHARLIE-QUOTING-OUTCOME-WIRE: prove fetchOutcomes mirrors observed outcomes
// all the way through the REAL wire to the OutcomeCaptureBus (-> P0-U04 bridge
// -> india's learner). Round-trips THROUGH the consumer seam end-to-end (R15):
// loadOutcomes -> fetchOutcomes -> real QuotingOutcomeCaptureWireEngine -> bus.
describe("buildLiveDeps fetchOutcomes -> OutcomeCaptureBus wire", () => {
  const sampleOutcomes: CycleOutcome[] = [
    { quote_id: "Q-A", predicted_quote_usd: 1000, accepted: true },
    { quote_id: "Q-B", predicted_quote_usd: 2000, actual_invoice_usd: 2100 },
  ];

  /** A real wire injected with a capturing fake bus -- proves the whole chain. */
  function realWireWithCapturingBus() {
    const events: RecordOutcomeInput[] = [];
    const fakeBus = {
      record: (input: RecordOutcomeInput): RecordOutcomeResult => {
        events.push(input);
        return { ok: true, lineage_id: input.lineage_id ?? "", event_id: `e${events.length}` };
      },
    };
    const wire = new QuotingOutcomeCaptureWireEngine(
      fakeBus as unknown as OutcomeCaptureBusEngine,
    );
    return { wire, events };
  }

  it("emits each loaded outcome to the capture bus with the right kind by default", async () => {
    const { wire, events } = realWireWithCapturingBus();
    const deps = buildLiveDeps({ loadOutcomes: async () => sampleOutcomes, captureWire: wire });
    const out = await deps.fetchOutcomes();
    expect(out).toEqual(sampleOutcomes); // records pass through unchanged
    expect(events.map((e) => e.lineage_id)).toEqual(["Q-A", "Q-B"]);
    expect(events.map((e) => e.kind)).toEqual(["quote_accepted", "quote_vs_actual"]);
    expect(events.every((e) => e.domain === "quote" && e.source === "system")).toBe(true);
  });

  it("emitToCaptureBus:false disables the wire (records still pass through)", async () => {
    const { wire, events } = realWireWithCapturingBus();
    const deps = buildLiveDeps({
      loadOutcomes: async () => sampleOutcomes,
      emitToCaptureBus: false,
      captureWire: wire,
    });
    const out = await deps.fetchOutcomes();
    expect(out).toEqual(sampleOutcomes);
    expect(events).toHaveLength(0);
  });

  it("an empty outcome batch never touches the bus", async () => {
    const { wire, events } = realWireWithCapturingBus();
    const deps = buildLiveDeps({ loadOutcomes: async () => [], captureWire: wire });
    await deps.fetchOutcomes();
    expect(events).toHaveLength(0);
  });
});
