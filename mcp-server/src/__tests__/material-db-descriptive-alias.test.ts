 ...
 * while Object.keys/entries/values still enumerate ONLY the 16 canonical
...
describe("CANONICAL_MATERIAL_DB descriptive-name aliasing", () => {
  ...
  describe("non-enumerable invariant: integrity of the canonical 16 is preserved", () => {
    // Count is 16 since U-SFC-DUCTILE-IRON-KC added "ductile_iron" (nodular/SG iron) as a real
    // canonical material distinct from gray iron (kc1_1 1300 vs the K-group 1100).
    it("Object.keys enumerates exactly the 16 canonical materials (aliases hidden)", () => {
      expect(Object.keys(CANONICAL_MATERIAL_DB).length).toBe(16);
    });
    it("Object.values / Object.entries also see exactly 16 (no alias double-count)", () => {
      expect(Object.values(CANONICAL_MATERIAL_DB).length).toBe(16);
      expect(Object.entries(CANONICAL_MATERIAL_DB).length).toBe(16);
    });
    ...
    it("the 16 canonical short-code keys remain present and enumerable", () => {
      for (const key of ["1018", "1045", "4140", "304", "316", "6061", "7075",
        "Ti-6Al-4V", "Inconel 718", "D2", "A2", "tungsten_carbide", "gray_iron",
        "ductile_iron", "C11000", "C26000"]) {
