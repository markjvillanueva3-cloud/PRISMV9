...
const PROBE_PARAMS: Record<string, Record<string, any>> = {
  ...
  // lathe/billing reads added to the allowlist (whiskey/charlie) -- verified REAL wires (return real
  // structured data with empty params), so a {} probe is a valid round-trip. Closes the guard's
  // "no probe" coverage gap (U-ERP-ANALYTICS-WIRE-GUARD follow-up).
