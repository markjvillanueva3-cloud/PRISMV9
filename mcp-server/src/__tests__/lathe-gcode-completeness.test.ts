...
describe("G-Code Completeness — CSS Rapid Safety", () => {
  ...
  it("the G50 max-RPM clamp is set before the home retract (overspeed-safe ordering)", () => {
    // Was a now-vacuous "G97 before G53 retract" loop (the emitter retracts via G28, not G53). Assert the
    // REAL overspeed guarantee: the G50 max-RPM clamp is emitted before the final G28 home rapid, so the
    // clamp bounds RPM through the whole program including the non-cutting retract.
    const lines = programLines(basicInput());
    const clampIdx = lines.findIndex((l) => /G50\s+S\d+.*Max RPM clamp/.test(l));
    const retractIdxs = lines.map((l, i) => ({ l, i })).filter((x) => /G28\s+U0\s+W0/.test(x.l)).map((x) => x.i);
    expect(clampIdx).toBeGreaterThanOrEqual(0);
    expect(retractIdxs.length).toBeGreaterThan(0);
    expect(retractIdxs[retractIdxs.length - 1]).toBeGreaterThan(clampIdx);
  });
