...
describe("UltimateSpeedFeedEngine — Physics Validation", () => {
  ...
  // U-PF-COATING liveness (trunk port 2026-07-01, absorption arm-B P1): before this wire the
  // tool_coating input was a silent Vc NO-OP on trunk (only a thermal warning), so the
  // fullspace sweep's 8-coating overlay measured nothing. Factor is RELATIVE TO the regime's
  // tabulated baseline coating (P_milling baseline = AlTiN), so the baseline coating pins 1.0
  // (no double count) and lesser coatings DERATE. Live-probed reference values 2026-07-01.
  describe("coating -> Vc wire (U-PF-COATING)", () => {
    const base = {
      iso_group: "P" as const, operation: "milling" as const,
      tool_diameter_mm: 12, flutes: 4, tool_material: "carbide" as const,
    };
    const vc = (extra: object) =>
      ultimateSpeedFeedEngine.calculate({ ...base, ...extra }).cutting_speed.value;

    it("baseline coating (ALTIN on ISO-P) is 1.0 -- identical to unspecified (no double count)", () => {
      expect(vc({ tool_coating: "ALTIN" })).toBe(vc({}));
    });

    it("UNCOATED derates Vc below the coated baseline (live-probed 114 vs 160 on ISO-P)", () => {
      const none = vc({});
      const uncoated = vc({ tool_coating: "UNCOATED" });
      expect(uncoated).toBeLessThan(none);
      expect(uncoated).toBeCloseTo(114, 0);
      expect(none).toBeCloseTo(160, 0);
    });

    it("coating ordering: baseline >= TIN >= UNCOATED (monotone with coating capability)", () => {
      const altin = vc({ tool_coating: "ALTIN" });
      const tin = vc({ tool_coating: "TIN" });
      const uncoated = vc({ tool_coating: "UNCOATED" });
      expect(altin).toBeGreaterThanOrEqual(tin);
      expect(tin).toBeGreaterThanOrEqual(uncoated);
    });
  });
});
