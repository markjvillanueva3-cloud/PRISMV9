...
describe("FeatureRecognitionEngine — PHASE17 type extension", () => {
  it("recognizes pocket_complex with very_complex difficulty", () => {
    const r = featureRecognitionEngine.recognize([
      { type: "pocket_complex", dimensions: { length_mm: 80, width_mm: 60, depth_mm: 25 }, position: { x: 0, y: 0, z: 0 } },
    ]);
    expect(r.features[0].type).toBe("pocket_complex");
    expect(r.features[0].confidence).toBeCloseTo(0.95, 5);
    const c = featureRecognitionEngine.classify(r.features[0]);
    expect(c.manufacturing_difficulty).toBe("very_complex");
    expect(c.required_operations).toContain("adaptive_roughing");
    expect(c.required_operations).toContain("rest_machining");
  });

  it("recognizes slot_dovetail with restricted accessibility + dovetail_cutter", () => {
    const r = featureRecognitionEngine.recognize([
      { type: "slot_dovetail", dimensions: { width_mm: 12, depth_mm: 6, angle_deg: 60, length_mm: 40 }, position: { x: 0, y: 0, z: 0 } },
    ]);
    expect(r.features[0].type).toBe("slot_dovetail");
    const c = featureRecognitionEngine.classify(r.features[0]);
    expect(c.manufacturing_difficulty).toBe("complex");
    expect(c.required_operations).toEqual(["slotting", "dovetail_cut"]);
    expect(c.tool_requirements).toEqual(["end_mill", "dovetail_cutter"]);
    expect(c.accessibility).toBe("restricted");
  });

  it("recognizes slot_t_shaped with two-stage operation (through_slot + t_slot_undercut)", () => {
    const r = featureRecognitionEngine.recognize([
      { type: "slot_t_shaped", dimensions: { width_mm: 14, depth_mm: 18, length_mm: 100 }, position: { x: 0, y: 0, z: 0 } },
    ]);
    expect(r.features[0].type).toBe("slot_t_shaped");
    const c = featureRecognitionEngine.classify(r.features[0]);
    expect(c.required_operations).toEqual(["through_slot", "t_slot_undercut"]);
    expect(c.tool_requirements).toContain("t_slot_cutter");
  });

  it("recognizes thread_internal with drilling+tapping ops + restricted accessibility", () => {
    const r = featureRecognitionEngine.recognize([
      { type: "thread_internal", dimensions: { diameter_mm: 6, depth_mm: 12, pitch_mm: 1.0 }, position: { x: 0, y: 0, z: 0 } },
    ]);
    expect(r.features[0].type).toBe("thread_internal");
    const c = featureRecognitionEngine.classify(r.features[0]);
    expect(c.manufacturing_difficulty).toBe("complex");
    expect(c.required_operations).toEqual(["drilling", "tapping"]);
    expect(c.tool_requirements).toContain("tap");
    expect(c.accessibility).toBe("restricted");
  });

  it("complexity_score reflects very_complex pocket_complex (≥ chamfer baseline)", () => {
    const baseline = featureRecognitionEngine.recognize([
      { type: "chamfer", dimensions: { length_mm: 20, depth_mm: 2 }, position: { x: 0, y: 0, z: 0 } },
    ]);
    const complex = featureRecognitionEngine.recognize([
      { type: "pocket_complex", dimensions: { length_mm: 80, width_mm: 60, depth_mm: 25 }, position: { x: 0, y: 0, z: 0 } },
    ]);
    expect(complex.complexity_score).toBeGreaterThan(baseline.complexity_score);
  });

  it("estimated_operations counts all four orphan-ported ops correctly", () => {
    const r = featureRecognitionEngine.recognize([
      { type: "pocket_complex", dimensions: { length_mm: 50, width_mm: 50, depth_mm: 10 }, position: { x: 0, y: 0, z: 0 } },     // 4 ops
      { type: "slot_dovetail", dimensions: { width_mm: 10, depth_mm: 5 }, position: { x: 0, y: 0, z: 0 } },                       // 2 ops
      { type: "slot_t_shaped", dimensions: { width_mm: 12, depth_mm: 15 }, position: { x: 0, y: 0, z: 0 } },                      // 2 ops
      { type: "thread_internal", dimensions: { diameter_mm: 8, depth_mm: 16, pitch_mm: 1.25 }, position: { x: 0, y: 0, z: 0 } },  // 2 ops
    ]);
    expect(r.estimated_operations).toBe(4 + 2 + 2 + 2);
  });
});
