 ...
 */
// R9: the REAL callTool wraps prism_business engine output in a BARE
// { type:"text", text:"<json>" } slimResponse (NOT content[]), which the route
// must unwrapDispatcherEnvelope back to the real object. This harness now emits
// that exact envelope so the tests prove the route's peel -- previously it
// returned the post-peel object directly and could not have caught the
// U-WIRE-HOTELPORTAL-ENVELOPE dead-panel.
async function realCallTool(tool: string, action: string, params: any): Promise<any> {
  const value = await dispatchEngine(tool, action, params);
  return { type: "text", text: JSON.stringify(value) };
}

async function dispatchEngine(tool: string, action: string, params: any): Promise<any> {
