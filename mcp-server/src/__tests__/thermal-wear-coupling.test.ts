...
describe("wearRate", () => {
  it("increases with temperature (Arrhenius)", () => {
    const r1 = engine.wearRate(1e-5, 5000, 200, 700);
    const r2 = engine.wearRate(1e-5, 5000, 200, 900);
    expect(r2).toBeGreaterThan(r1);
  });

  it("zero at zero temperature", () => {
    expect(engine.wearRate(1e-5, 5000, 200, 0)).toBe(0);
  });

  it("proportional to speed", () => {
    const r1 = engine.wearRate(1e-5, 5000, 100, 800);
    const r2 = engine.wearRate(1e-5, 5000, 200, 800);
