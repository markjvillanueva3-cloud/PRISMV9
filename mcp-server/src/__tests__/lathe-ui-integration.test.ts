...
async function runPipeline(input: TurningInput): Promise<TurningProgramResult> {
...
describe("Workflow 1: Simple Shaft (OD turning)", () => {
  ...
  it("material parsing resolves AISI 4140", async () => {
  ...
  it("stock selection finds appropriate bar stock", async () => {
  ...
  it("full pipeline produces valid shaft program", async () => {
  ...
  it("shaft program includes facing and OD rough/finish", async () => {
    const result = await runPipeline(input);
  ...
  it("shaft cycle time is reasonable (< 10 minutes for simple part)", async () => {
    const result = await runPipeline(input);
...
describe("Workflow 2: Bore (ID boring)", () => {
  ...
  it("full pipeline produces valid bore program", async () => {
    const result = await runPipeline(input);
    assertValidResult(result, "Bore");
    assertGCodeStructure(result.program_text, "Bore");
  });

  it("bore program includes drilling and boring operations", async () => {
    const result = await runPipeline(input);
  ...
  it("bore program has boring bar deflection checks for deep bore", async () => {
    ...
    const result = await runPipeline(deepInput);
...
describe("Workflow 3: Thread (external threading)", () => {
  ...
  it("full pipeline produces valid threaded part program", async () => {
    const result = await runPipeline(input);
    assertValidResult(result, "Thread");
    assertGCodeStructure(result.program_text, "Thread");
  });

  it("thread program includes threading operation in G-code", async () => {
    const result = await runPipeline(input);
  ...
  it("thread program has multi-pass threading (not single depth-of-cut)", async () => {
    const result = await runPipeline(input);
...
describe("Workflow 4: Groove (grooving + part-off)", () => {
  ...
  it("full pipeline produces valid grooved part program", async () => {
    const result = await runPipeline(input);
    assertValidResult(result, "Groove");
    assertGCodeStructure(result.program_text, "Groove");
  });

  it("groove program includes grooving and cutoff operations", async () => {
    const result = await runPipeline(input);
  ...
  it("groove program has correct tool changes", async () => {
    const result = await runPipeline(input);
...
describe("Workflow 5: Complex multi-feature (shaft + bore + thread + groove)", () => {
  ...
  it("complex part G-code has threading cycle", async () => {
    const result = await runPipeline(input);
    expect(result.program_text).toMatch(/G(32|33|76|92)/);
  });

  it("complex part has multiple tool changes for different operations", async () => {
    const result = await runPipeline(input);
    // Minimum: facing, OD, groove, thread, bore = 5 different tools
    expect(result.total_tool_changes).toBeGreaterThanOrEqual(4);
  });

  it("complex part generates setup notes", async () => {
    const result = await runPipeline(input);
...
describe("Error paths: pipeline handles bad input", () => {
  it("rejects input with no features", async () => {
  ...
  it("handles zero-length part gracefully", async () => {
    const input = buildInput({
      part_length_mm: 0,
  ...
  it("handles bar stock smaller than finished diameter", async () => {
    const input = buildInput({
      bar_stock_od_mm: 20,
      finished_od_mm: 30,
      features: [
  ...
  it("handles extremely deep bore with deflection warning", async () => {
    const input = buildInput({
      bar_stock_od_mm: 50,
      part_length_mm: 200,
      features: [
  ...
  it("handles unknown controller dialect gracefully", async () => {
    const input = buildInput({
...
describe("Download artifacts: G-code, setup sheet, physics report", () => {
  ...
  it("G-code output is valid text content for .nc file", async () => {
    const result = await runPipeline(input);
  ...
  it("setup notes can be joined into a text file", async () => {
    const result = await runPipeline(input);
    const setupSheet = result.setup_notes.join("\n");
    expect(typeof setupSheet).toBe("string");
  });

  it("result has all fields required by LatheResultData interface", async () => {
    const result = await runPipeline(input);
