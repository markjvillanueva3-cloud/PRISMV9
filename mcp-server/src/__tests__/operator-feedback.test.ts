...
const callTool: CallToolFn = async (tool, action, params) => {
  ...
  if (!fb.operatorId) return { success: false, error: "operator_feedback_record requires operatorId, tenantId, ..." };
...
describe("/api/operator/feedback route", () => {
  ...
  it("POST /feedback wraps the body as {feedback} for prism_session:operator_feedback_record", async () => {
    ...
    expect(lastForwarded?.params.feedback.operatorId).toBe("op-1");
  ...
  it("maps a rejected record ({success:false}) to 400 so the SPA offline-queue retries", async () => {
    const { status, json } = await httpRequest("POST", "/api/operator/feedback", { feedbackType: "thumbs_up" });
