...
describe("nine-axis radial engagement: radial_depth is LIVE (U-OSC-RADIAL-ENGAGEMENT)", () => {
  ...
  it("NOT INERT: changing radial_depth_pct changes MRR (the exact pre-fix bug); higher ae -> higher MRR", () => {
    // Pre-fix these were IDENTICAL (ae frozen at the table value). The doubling of ae must move MRR.
    expect(r50.mrr_cm3min).not.toBe(r25.mrr_cm3min);
    expect(r50.mrr_cm3min).toBeGreaterThan(r25.mrr_cm3min);
    // ae doubled (3.0->6.0) with everything else equal -> MRR ratio ~2x (no clamp on aluminum).
    expect(r50.mrr_cm3min / r25.mrr_cm3min).toBeGreaterThan(1.8);
    expect(r50.mrr_cm3min / r25.mrr_cm3min).toBeLessThan(2.2);
  });
