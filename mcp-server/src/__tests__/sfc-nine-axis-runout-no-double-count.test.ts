...
function orchLife(holder?: Record<string, unknown>): number {
  const input = {
...
describe("nine-axis runout: tool_holder.type LIVE + no double-count (U-OSC-HOLDER-RUNOUT-DEDUP)", () => {
  it("tool_holder.type moves tool_life_min (type -> default TIR -> engine derate -> shorter life)", () => {
    const erCollet = orchLife({ type: "er_collet" }); // HOLDER_RUNOUT_TIR_UM.er_collet = 12um
    const hsk = orchLife({ type: "hsk_a63" });         // HOLDER_RUNOUT_TIR_UM.hsk_a63 = 3um
