 ...
 *
 * R9: real reference value — corpus:Accupro:ACCU-0.0625 has cutting_diameter_mm
 * 1.587 (verified from the live extracted catalog). The test fails if SFC stops
 * resolving by id and reverts to the default.
 */
...
describe("SFC catalog-id tool resolution", () => {
  ...
  it("resolves the cataloged tool's REAL diameter from tool_catalog_id (no diameter given)", async () => {
    ...
    // Accupro ACCU-0.0625 = 0.0625" drill = 1.587 mm in the corpus.
    const r = eng.compute({
      material: "steel",
      operation: "drilling",
      tool_catalog_id: "corpus:Accupro:ACCU-0.0625",
    ...
    // the real cataloged 1.587mm resolves.
    expect(tool.diameter_mm.value).toBeCloseTo(1.587, 2);
