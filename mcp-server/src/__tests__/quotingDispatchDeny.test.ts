...
describe("U-MKTPRICE01 generic quoting dispatch -- cost-basis deny-list", () => {
  ...
  const U_MKTPRICE02_DENIED = [
    ...
    "quoting_secondary_ops_price",             // U-QP-SECONDARY-OPS-AUTH-MIGRATION -- PLAIN variant now DENIED on the generic
                                               // surface + migrated to the verifyToken verb POST /secondary-ops-price (was LEFT).
    "tool_cost_basis",                         // U-QP-CONSUMABLE-COST-BASIS -- the shop's own per-consumable-type procurement
                                               // cost basis (jm-tool-purchases $/line-item). Tool-side twin of material_cost_basis;
                                               // admin verb POST /tool-cost-basis. No token-less caller (internal-only).
  ] as const;

  it("the deny-set contains EXACTLY the 16 known cost-basis actions (anti-regression count contract)", () => {
    // 6 from U-MKTPRICE01 + 8 from U-MKTPRICE02 + 1 from U-QP-SECONDARY-OPS-AUTH-MIGRATION (the PLAIN
    // secondary_ops_price -- last open member of the anon cost-basis leak class, migrated to a verifyToken
    // typed verb so the QuotingWorkbenchPage keeps working with its Bearer token) + 1 from
    // U-QP-CONSUMABLE-COST-BASIS (tool_cost_basis -- the tool-side twin of material_cost_basis). R12.
    expect(QUOTING_GENERIC_DISPATCH_DENY_SET.size).toBe(16);
  ...
  const U_MKTPRICE02_LEFT = [
    ...
    // NOTE: quoting_secondary_ops_price was HERE (LEFT) until U-QP-SECONDARY-OPS-AUTH-MIGRATION (2026-07-01).
    // It IS now denied on the generic surface -- moved to U_MKTPRICE02_DENIED above -- because its cost basis
    // is now reachable via the verifyToken verb POST /secondary-ops-price (the page attaches a Bearer token).
  ] as const;
...
describe("U-QP-SECONDARY-OPS-AUTH-MIGRATION -- /secondary-ops-price verifyToken verb (any signed-in operator)", () => {
  it("401s an ANONYMOUS request (verifyToken blocks before the engine -- no cost basis leaks)", async () => {
    const before = calls.length;
    const { status } = await post("/secondary-ops-price", { ops: ["grinding"], quantity: 10 }); // no x-test-roles => anon
    expect(status).toBe(401);
    expect(calls.length).toBe(before); // the shop's per-op cost basis is NEVER computed for an anon caller
  });

  it("200s ANY authenticated operator (NOT admin-tier -- a workbench figure) and forwards to the dispatcher", async () => {
    const before = calls.length;
    // 'quoter' is a NON-admin role: it 403s on the admin cost-basis verbs above, but PASSES here (verifyToken only).
    const { status, body } = await post("/secondary-ops-price", { ops: ["grinding"], quantity: 10, base_material_cost_usd: 50 }, "quoter");
    expect(status).toBe(200);
    expect(body.ok).toBe(true);
    expect(calls.length).toBe(before + 1);
    expect(calls[calls.length - 1].action).toBe("quoting_secondary_ops_price");
  });

  it("the PLAIN quoting_secondary_ops_price is 403'd on the GENERIC surface (the migration's deny half)", async () => {
    const before = calls.length;
    const { status, body } = await post("/", { action: "quoting_secondary_ops_price", params: { ops: ["grinding"] } });
    expect(status).toBe(403); // was 200 pre-migration -- the anon leak is now closed
    expect(body.error).toMatch(/not browser-dispatchable on the generic quoting surface/);
    expect(calls.length).toBe(before); // never reached the dispatcher via the anon generic path
  });
});
