...
const callTool: CallToolFn = vi.fn(async (tool: string, action: string, params: any = {}) => {
  ...
  if (action === "handoff_list") {
    ...
    // R9: prism_business emits a BARE { type:"text", text:"<json>" } slimResponse that the REAL
    // callTool CANNOT peel (no content[] wrapper) -> the route receives the raw envelope. This is
    // the PRODUCTION wire shape (verified LIVE :3100), NOT the convenient post-peel {success,data}
    // the other branches simulate. The route's unwrapBusinessEnvelope must peel it so the client
    // sees {success,data}; without it every business/portal page dead-pans.
    return {
      type: "text",
      text: JSON.stringify({ success: true, data: [{ id: "H-1", status: "pending" }] }),
    };
  }
...
describe("POST /api/v1/business/dispatch — allowlisted passthrough", () => {
  ...
  it("PEELS the { type:'text', text } slimResponse so the client sees {success,data}, not the raw envelope (R9 — handoff_list mock returns the real prism_business wire shape)", async () => {
    const { status, body } = await dispatch({ action: "handoff_list", params: { status: "pending" } });
    expect(status).toBe(200);
    // The raw envelope keys must NOT survive to the wire — if they do, every
    // business/portal page reads {type,text} and dead-pans.
    expect(body.type).toBeUndefined();
    expect(body.text).toBeUndefined();
    // The real inner payload IS surfaced.
    expect(body.success).toBe(true);
    expect(Array.isArray(body.data)).toBe(true);
    expect(body.data[0].id).toBe("H-1");
  });
