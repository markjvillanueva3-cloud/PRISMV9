...
vi.mock("../middleware/auth.js", () => ({
  // Defaults unchanged (userId "test-user", roles ["lead"]) so every pre-existing test is untouched.
  // x-test-userid / x-test-roles headers let the U-ERP-CLOCK-SELFGATE-HARDEN tests exercise the
  // no-mapping DEGRADE path and the admin override deterministically (hotel-portal-auth.test.ts pattern).
  verifyToken: (req: any, _res: any, next: () => void) => {
    req.userId = req.headers["x-test-userid"] ?? "test-user";
    const roles = req.headers["x-test-roles"];
    req.userRoles = roles ? String(roles).split(",").map((s: string) => s.trim()) : ["lead"];
    next();
  },
...
// U-ERP-CLOCK-SELFGATE-HARDEN: the hardened requireSelfOrAdmin resolves the caller's OWN employee id
// from EmployeeEngine.auth_user_id (auth ids are USR-*/test-user, targets EMP-*). To exercise the REAL
// production 403 path (not a fabricated userId===employee_id coincidence), seed an employee whose
// auth_user_id === the stubbed verifyToken userId ("test-user"). create() mints the EMP id and forces
// auth_user_id:null, so link it via update(). (Same seed pattern as hotel-portal-auth.test.ts.)
const seededClockEmp = employeeEngine.create({
  first_name: "Clock", last_name: "Selfgate", email: "erp-clock-selfgate@jmdie.test",
  department: "shop", hourly_rate: 25,
} as Parameters<typeof employeeEngine.create>[0]);
employeeEngine.update(seededClockEmp.id, { auth_user_id: "test-user" } as Parameters<typeof employeeEngine.update>[1]);
const SELF_EMP_ID = seededClockEmp.id;
...
const callTool: CallToolFn = vi.fn(async (tool: string, action: string, params: any = {}) => {
  ...
  // --- U-WIRE-ERP-ENVELOPE: prism_business actions the 27-site unwrap fix must peel. Each returns a
  // DISTINCTIVE inner field the route surfaces; a dropped unwrapEnvelope would leave res.data = {type,text}
  // (the inner field undefined), which is exactly what these assertions catch. ---
  if (action === "dispatch_get_all_queues") {
    return env({ machines: [{ machine_id: "VMC-1", queued: 3 }], total_queued_jobs: 3 });
  }
  if (action === "cash_flow_summary") {
    return env({ months: Number(params.months ?? 3), net_cash_flow: 42500, opening_balance: 10000 });
  }
  if (action === "erp_tool_search") {
    return env({ tools: [{ tool_id: "T-101", description: "1/2 4FL carbide EM", on_hand: 6 }], query: params.query });
  }
  if (action === "margin_trends") {
    return env({ trend: [{ month: "2026-06", margin_pct: 34.2 }], months: Number(params.months ?? 3) });
  }
...
async function http(method: string, path: string, body?: any, extraHeaders?: Record<string, string>): Promise<{ status: number; json: any }> {
...
// ---------------------------------------------------------------------------
// U-WIRE-ERP-ENVELOPE: the 27 `const result = await callTool("prism_business", ...)` sites in erp.ts
// sent the un-peeled { type:"text", text } slimResponse straight to res.json({ data: result }) -> the FE
// (api/erp.ts reads json.data as the typed inner object) got the envelope, not the parsed payload -> every
// one of these ERP panels (dispatch board / cash flow / tool inventory / margin trends / ...) rendered dead
// for a signed-in user. The fix wraps each with the existing local unwrapEnvelope. These tests mount the REAL
// router with the PRODUCTION env() mock and assert the DISTINCTIVE inner field is surfaced at res.data --
// dropping unwrapEnvelope on any of these routes leaves res.data = {type,text} and fails the assertion.
// (Note: /oee-six-losses' prism_business oee_calculate is covered by the same class; /analytics/oee routes
//  to prism_calc which callTool ALREADY peels -- correctly NOT wrapped, so it is not re-tested here.)
// ---------------------------------------------------------------------------
describe("U-WIRE-ERP-ENVELOPE: prism_business routes surface the PARSED inner payload (not the {type,text} envelope)", () => {
  it("GET /dispatch-board peels the envelope -> board fields live at res.data (dead-panel regression)", async () => {
    const { status, json } = await http("GET", "/api/v1/erp/dispatch-board");
    expect(status).toBe(200);
    expect(json.ok).toBe(true);
    // If unwrapEnvelope were dropped, res.data would be { type:"text", text:"..." } and these would be undefined.
    expect(json.data.total_queued_jobs).toBe(3);
    expect(Array.isArray(json.data.machines)).toBe(true);
    expect(json.data.machines[0].machine_id).toBe("VMC-1");
    // The raw envelope keys must NOT leak through to the FE.
    expect(json.data.type).toBeUndefined();
    expect(json.data.text).toBeUndefined();
  });

  it("GET /cash-flow (admin) peels the envelope -> financial fields live at res.data", async () => {
    const { status, json } = await http("GET", "/api/v1/erp/cash-flow?months=6");
    expect(status).toBe(200);
    expect(json.data.net_cash_flow).toBe(42500);
    expect(json.data.months).toBe(6); // route forwards the ?months query into params, dispatcher echoes it
    expect(json.data.text).toBeUndefined();
  });

  it("GET /tool-crib-search peels the envelope -> tool inventory rows live at res.data", async () => {
    const { status, json } = await http("GET", "/api/v1/erp/tool-crib-search?query=carbide%20em");
    expect(status).toBe(200);
    expect(Array.isArray(json.data.tools)).toBe(true);
    expect(json.data.tools[0].tool_id).toBe("T-101");
    expect(json.data.query).toBe("carbide em"); // route forwards ?query -> params.query
    expect(json.data.type).toBeUndefined();
  });

  it("GET /margin-trends (admin) peels the envelope -> trend array live at res.data", async () => {
    const { status, json } = await http("GET", "/api/v1/erp/margin-trends?months=3");
    expect(status).toBe(200);
    expect(Array.isArray(json.data.trend)).toBe(true);
    expect(json.data.trend[0].margin_pct).toBe(34.2);
    expect(json.data.text).toBeUndefined();
  });
});

// ---------------------------------------------------------------------------
// U-ERP-CLOCK-SELFGATE-HARDEN: the old requireSelfOrAdmin compared employee_id (EMP-*) directly
// against req.userId (auth USR-*) -- different namespaces, so every legitimate non-admin
// self-service clock call 403'd while proving nothing about peers. The hardened gate (clone of
// hotel-portal.ts requireSelfOrPrivileged) resolves the caller's own employee record via
// EmployeeEngine.auth_user_id, 403s only a PROVEN peer, and degrades to verifyToken-only when
// the mapping cannot resolve. These run the REAL local gate (it is not part of the auth.js mock).
// ---------------------------------------------------------------------------
describe("U-ERP-CLOCK-SELFGATE-HARDEN: /shift-clock-in self-gate (real requireSelfOrAdmin)", () => {
  it("SELF: the caller's resolved EMP id clocks in (the old namespace bug 403'd this)", async () => {
    calls.length = 0;
    const { status, json } = await http("POST", "/api/v1/erp/shift-clock-in", { employee_id: SELF_EMP_ID });
    expect(status).toBe(200);
    expect(json.ok).toBe(true);
    const c = calls.find((x) => x.action === "clock_in");
    expect(c?.params.employee_id).toBe(SELF_EMP_ID);
  });

  it("PEER (deny WITH TEETH): a proven-peer EMP target 403s AND the engine is never invoked", async () => {
    calls.length = 0;
    const { status, json } = await http("POST", "/api/v1/erp/shift-clock-in", { employee_id: "EMP-PEER-9999" });
    expect(status).toBe(403);
    expect(json.ok).toBe(false);
    // Teeth: a role->200 allow-test passes with or without the gate; only proving the DENY
    // short-circuits the handler catches a dropped gate (U-INBOX-INTEGRATIONS-AUTH lesson).
    expect(calls.some((x) => x.action === "clock_in")).toBe(false);
  });

  it("DEGRADE: an auth user with NO employee mapping passes through (documented verifyToken-only trade)", async () => {
    calls.length = 0;
    const { status } = await http("POST", "/api/v1/erp/shift-clock-in",
      { employee_id: "EMP-PEER-9999" }, { "x-test-userid": "usr-unmapped-000" });
    expect(status).toBe(200);
    expect(calls.some((x) => x.action === "clock_in")).toBe(true);
  });

  it("ADMIN override: an admin may clock a peer", async () => {
    calls.length = 0;
    const { status } = await http("POST", "/api/v1/erp/shift-clock-in",
      { employee_id: "EMP-PEER-9999" }, { "x-test-roles": "admin" });
    expect(status).toBe(200);
    expect(calls.some((x) => x.action === "clock_in")).toBe(true);
  });

  it("NO TARGET: omitting employee_id passes (downstream defaults to self)", async () => {
    calls.length = 0;
    const { status } = await http("POST", "/api/v1/erp/shift-clock-in", {});
    expect(status).toBe(200);
    expect(calls.some((x) => x.action === "clock_in")).toBe(true);
  ...
  it("LEGACY compat: sending the auth id AS employee_id still counts as self", async () => {
    calls.length = 0;
    const { status } = await http("POST", "/api/v1/erp/shift-clock-in", { employee_id: "test-user" });
    expect(status).toBe(200);
    expect(calls.some((x) => x.action === "clock_in")).toBe(true);
  });
});
