...
describe("U-BRIDGE-WIRE-TURNING — turning_sensitivity_analysis", () => {
  ...
  it("CASCADE HEALTHY — baseline feasible: no infeasible error, real cpk_baseline arrives (flipped sentinel; the U-FIX-TURNING-CASCADE-API pin fired as designed)", async () => {
    // Formerly the KNOWN-ENGINE-BUG sentinel pinning error.toContain("infeasible") --
    // the cascade fix landed, the pin fired, this now asserts the HEALTHY contract.
    const r = await invokeHandler(turningHandler, "turning_sensitivity_analysis", SENS_INPUT);
    const data = r.data as { error?: string; cpk_baseline?: number; source?: string };
    expect(data.error).toBeUndefined();
    expect(typeof data.source).toBe("string");
    expect(Number.isFinite(data.cpk_baseline)).toBe(true);
  });
...
describe("U-BRIDGE-WIRE-TURNING — turning_stochastic_production_plan", () => {
  ...
  it("CASCADE HEALTHY — all 30 trials feasible: trials_feasible honours n_trials (flipped sentinel; the trials_feasible=0 pin fired as designed)", async () => {
    // Formerly the KNOWN-ENGINE-BUG sentinel pinning trials_feasible === 0 --
    // the cascade fix landed, the pin fired, this now asserts the HEALTHY contract.
    const r = await invokeHandler(turningHandler, "turning_stochastic_production_plan", PLAN_INPUT);
    const data = r.data as { trials_feasible: number };
    expect(data.trials_feasible).toBe(30); // = PLAN_INPUT.n_trials, observed live post-fix
  });
