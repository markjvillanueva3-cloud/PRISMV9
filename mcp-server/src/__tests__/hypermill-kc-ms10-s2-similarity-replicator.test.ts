...
describe("HM-KC-MS10-S2/U-HKC54: FeatureSequenceReplicatorEngine", () => {
  ...
  describe("Material Change — H13 to P20", () => {
    ...
    it("AC script contains all 5+ operations", () => {
      // Real API: operations are listed as "# Op N:" comments with param dicts
      const opCount = (result.acPythonScript.match(/# Op \d+:/g) ?? []).length;
  ...
  describe("AC Python Script Quality", () => {
    ...
    it("script sets all operation parameters via CfgParameters dict", () => {
      ...
      // Real hyperMILL API uses CfgParameters dict with real keys from Metric.cfg
      // Milling speed/feed come from tool tech, not CfgParameters
      expect(result.acPythonScript).toContain('"VERTZUSTEL"');
      expect(result.acPythonScript).not.toContain('"spindle_speed"');
    });
  });
