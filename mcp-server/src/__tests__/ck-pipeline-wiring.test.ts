...
describe("ACTION_CK_PIPELINE_SCHEMAS", () => {
  ...
  it("edm_wire_program validates required material + thickness", async () => {
    ...
    expect(s.safeParse({ material: "D2 Steel", thickness_mm: 50 }).success).toBe(true);
  ...
  it("laser_cut_program rejects unknown laser_type", async () => {
    ...
    expect(s.safeParse({ material: "Steel", thickness_mm: 3, laser_type: "fiber" }).success).toBe(true);
  ...
  it("waterjet_abrasive_program rejects unknown quality_level", async () => {
    ...
    expect(s.safeParse({ material: "Steel", thickness_mm: 20, quality_level: "Q3" }).success).toBe(true);
...
describe("camDispatcher CK-MS7 wiring integrity", () => {
  const DISPATCHER_PATH = "C:/prism/mcp-server/src/tools/dispatchers/camDispatcher.ts";
