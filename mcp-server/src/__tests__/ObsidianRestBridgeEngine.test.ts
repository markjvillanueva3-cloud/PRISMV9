...
const WRITEK = "PRISM_OBSIDIAN_WRITE";
...
describe("ObsidianControlBridgeEngine -- default-DENY write gate", () => {
  beforeEach(() => {
    process.env[KEY] = "test-key";        // a key is present...
    delete process.env[WRITEK];           // ...but writes are OFF by default
  });
  afterEach(() => {
    delete process.env[WRITEK];
  });

  it("every MUTATING method returns write-disabled WITHOUT opening a socket (default deny)", async () => {
    const t = ok(200, "{}");
    expect((await ObsidianControlBridgeEngine.create("a.md", "x", t)).reason).toBe("write-disabled");
    expect((await ObsidianControlBridgeEngine.append("a.md", "x", t)).reason).toBe("write-disabled");
    expect((await ObsidianControlBridgeEngine.deleteNote("a.md", t)).reason).toBe("write-disabled");
    expect((await ObsidianControlBridgeEngine.runCommand("editor:toggle-bold", t)).reason).toBe("write-disabled");
    expect((await ObsidianControlBridgeEngine.open("a.md", t)).reason).toBe("write-disabled");
    expect(t).not.toHaveBeenCalled();
  });

  it("the write gate beats the no-key check: write-disabled even with NO key", async () => {
    delete process.env[KEY];
    const t = ok(200, "{}");
    expect((await ObsidianControlBridgeEngine.create("a.md", "x", t)).reason).toBe("write-disabled");
    expect(t).not.toHaveBeenCalled();
  ...
  it("READ methods (listCommands / list / periodic) work WITHOUT the write flag", async () => {
    expect((await ObsidianControlBridgeEngine.listCommands(ok(200, '{"commands":[{"id":"a:b","name":"A B"}]}'))).ok).toBe(true);
    expect((await ObsidianControlBridgeEngine.list("", ok(200, '{"files":["x.md"]}'))).ok).toBe(true);
    expect((await ObsidianControlBridgeEngine.periodic("daily", ok(200, "# today"))).ok).toBe(true);
  });
});

describe("ObsidianControlBridgeEngine -- gated ON: request shaping + fail-soft", () => {
  beforeEach(() => {
    process.env[KEY] = "test-key";
    process.env[WRITEK] = "1"; // writes enabled in-process
  });
  afterEach(() => {
    delete process.env[WRITEK];
  });

  it("create issues a PUT with the markdown body to /vault/<path>", async () => {
    const t = ok(200, "");
    const res = await ObsidianControlBridgeEngine.create("memories/x.md", "# hi", t);
    expect(res.ok).toBe(true);
    const args = (t as unknown as { mock: { calls: [Record<string, unknown>][] } }).mock.calls[0][0];
    expect(args.method).toBe("PUT");
    expect(args.path).toBe("/vault/memories/x.md");
    expect(args.body).toBe("# hi");
    expect(args.contentType).toBe("text/markdown");
  });

  it("append issues a POST with the body", async () => {
    const t = ok(200, "");
    expect((await ObsidianControlBridgeEngine.append("x.md", "more", t)).ok).toBe(true);
    const args = (t as unknown as { mock: { calls: [Record<string, unknown>][] } }).mock.calls[0][0];
    expect(args.method).toBe("POST");
    expect(args.body).toBe("more");
  });

  it("runCommand POSTs to /commands/<id>/ and 204 is ok", async () => {
    const t = ok(204, "");
    const res = await ObsidianControlBridgeEngine.runCommand("editor:toggle-bold", t);
    expect(res.ok).toBe(true);
    const args = (t as unknown as { mock: { calls: [Record<string, unknown>][] } }).mock.calls[0][0];
    expect(args.method).toBe("POST");
    expect(args.path).toBe("/commands/editor%3Atoggle-bold/");
  });

  it("deleteNote DELETEs and maps 404 to not-found", async () => {
    expect((await ObsidianControlBridgeEngine.deleteNote("x.md", ok(200, ""))).ok).toBe(true);
    expect((await ObsidianControlBridgeEngine.deleteNote("gone.md", ok(404, ""))).reason).toBe("not-found");
  });

  it("open POSTs to /open/<path>", async () => {
    const t = ok(200, "");
    expect((await ObsidianControlBridgeEngine.open("memories/x.md", t)).ok).toBe(true);
    const args = (t as unknown as { mock: { calls: [Record<string, unknown>][] } }).mock.calls[0][0];
    expect(args.path).toBe("/open/memories/x.md");
  });

  it("listCommands parses {commands:[...]}, bad-json, and non-array -> []", async () => {
    const good = await ObsidianControlBridgeEngine.listCommands(ok(200, '{"commands":[{"id":"a:b","name":"A B"},{"noid":1}]}'));
    expect(good.ok).toBe(true);
    expect(good.data).toEqual([{ id: "a:b", name: "A B" }]); // the no-id entry is dropped
    expect((await ObsidianControlBridgeEngine.listCommands(ok(200, "<<bad"))).reason).toBe("bad-json");
    expect((await ObsidianControlBridgeEngine.listCommands(ok(200, '{"x":1}'))).data).toEqual([]);
  });

  it("list parses {files:[...]} and maps 404 to not-found", async () => {
    const res = await ObsidianControlBridgeEngine.list("memories", ok(200, '{"files":["a.md","b/"]}'));
    expect(res.data).toEqual(["a.md", "b/"]);
    expect((await ObsidianControlBridgeEngine.list("nope", ok(404, ""))).reason).toBe("not-found");
  });

  it("rejects path traversal + invalid command id + invalid period WITHOUT a socket", async () => {
    const t = ok(200, "");
    expect((await ObsidianControlBridgeEngine.create("../../etc/passwd", "x", t)).reason).toBe("unsafe-path");
    expect((await ObsidianControlBridgeEngine.deleteNote("/abs.md", t)).reason).toBe("unsafe-path");
    expect((await ObsidianControlBridgeEngine.runCommand("bad id!", t)).reason).toBe("invalid-command-id");
    expect((await ObsidianControlBridgeEngine.periodic("fortnightly", t)).reason).toBe("invalid-period");
    expect(t).not.toHaveBeenCalled();
  });

  it("fail-soft: a transport error surfaces a reason, never throws", async () => {
    expect((await ObsidianControlBridgeEngine.create("x.md", "y", throws("ECONNREFUSED"))).reason).toContain("unreachable");
    expect((await ObsidianControlBridgeEngine.runCommand("a:b", throws("AbortTimeout"))).reason).toContain("timeout");
    expect((await ObsidianControlBridgeEngine.create("x.md", "y", ok(403, ""))).reason).toBe("http-403");
  });

  it("non-loopback URL is refused for control writes too (no socket, no key sent)", async () => {
    process.env[URLK] = "https://evil.example.com";
    delete process.env[REMOTE];
    const t = ok(200, "");
    expect((await ObsidianControlBridgeEngine.create("x.md", "y", t)).reason).toBe("non-loopback-url");
    expect(t).not.toHaveBeenCalled();
  });
});
