/**
  ...
  $Revision: 44005 Enhanced for VM30i $
  $Date: 2025-12-22 $
...
longDescription = "Enhanced post for HURCO VM30i with WinMax control. Features: iMachining-style variable feedrate (arc correction, direction change detection, feed ramping), adaptive roughing feed control, minimum Z retract between work offsets, M98 subprogram calls for air through spindle, automatic G05.3 smoothing. Supports both ISNC and BNC modes.";
...
properties = {
  ...
  // iMachining-Style Variable Feedrate Control
  useIMachiningFeed: {
    title      : "Use iMachining-style variable feed",
    description: "Enable intelligent feedrate adjustment similar to SolidCAM iMachining. Automatically reduces feed in corners and arcs to maintain constant chip thickness, and ramps feed smoothly between different cutting conditions.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  iMachiningLevel: {
    ...
    description: "Similar to SolidCAM's level slider. Level 1 = conservative (reduced MRR, longer tool life). Level 8 = aggressive (maximum MRR). Affects how much feed reduction is applied in corners.",
    group      : "preferences",
    type       : "integer",
    range      : [1, 8],
    value      : 5,
  ...
  arcFeedCorrection: {
    ...
    description: "Reduce feedrate on arcs/corners by this percentage to maintain constant chip thickness. 100% = full correction (constant chip load), 0% = no correction (constant feed). SolidCAM default is 100%.",
    group      : "preferences",
    type       : "integer",
    range      : [0, 100],
    value      : 100,
...
/** Variables for iMachining-style feed control */
var previousPosition = null;
var previousDirection = null;
var previousFeed = 0;
var currentArcRadius = 0;
var iMachiningActive = false;
...
/**
  ...
  These functions implement variable feedrate logic similar to SolidCAM iMachining:
  ...
  4. Level-based aggressiveness - similar to iMachining's 1-8 level slider
*/

/**
  Calculate the aggressiveness factor based on iMachining level (1-8).
...
function calculateArcFeedFactor(radius, toolRadius) {
  ...
  var levelFactor = getIMachiningLevelFactor();
...
function calculateDirectionChangeFactor(currentDir, previousDir) {
  if (!getProperty("useIMachiningFeed") || previousDir == null) {
    return 1.0;
  }
...
/**
  Calculate iMachining-style adjusted feedrate.
...
function applyIMachiningFeed(f, isArc, arcRadius, currentPos, moveDistance) {
  ...
  if (isArc && arcRadius > 0) {
    ...
    currentArcRadius = arcRadius; // Store for potential comments
  } else {
    currentArcRadius = 0;
  }
...
// SolidCAM iMachining Technology Wizard calculates optimal feeds based on:
...
function getFeed(f) {
  ...
  // Apply iMachining-style feed adjustments (arc correction, direction changes, ramping)
  if (getProperty("useIMachiningFeed")) {
  ...
  if (getProperty("showFeedComments") && feedChanged) {
    var extraInfo = "";
    if (getProperty("useIMachiningFeed") && iMachiningIsArc && iMachiningArcRadius > 0) {
      extraInfo = " R" + xyzFormat.format(iMachiningArcRadius);
    }
    if (getProperty("useDynamicDepthFeed") && dynamicDepthEnabled) {
...
/** Global variables for iMachining feed calculations in motion functions */
