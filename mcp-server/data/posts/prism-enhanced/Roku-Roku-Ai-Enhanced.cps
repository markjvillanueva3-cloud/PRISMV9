/**
  ...
  FANUC 31i-MODEL B5 Enhanced Post Processor
  Optimized for Roku-Roku HC-658 II

  $Revision: 44207 Enhanced Edition $
  $Date: 2025-12-22 $
...
///////////////////////////////////////////////////////////////////////////////
//                    ROKU-ROKU HC-658 II WITH FANUC 31i-B5
//                         ENHANCED POST PROCESSOR
//
// OPTIMIZED FOR COPPER TRILOBE MACHINING
//
///////////////////////////////////////////////////////////////////////////////
//
// FANUC 31i-MODEL B5 SPECIFIC FEATURES:
//
//   AI CONTOUR CONTROL II (AICC II) - G05.1 Q1 Rx
//     - Advanced look-ahead and acceleration control
//     - R1-R10 smoothing levels (R1=fastest, R10=smoothest)
//     - Automatic level selection based on operation type
//     - Trilobe-optimized: Prioritizes arc accuracy
//
//   NANO SMOOTHING - G5.1 Q3
//     - Sub-micron interpolation for ultra-fine finishes
//     - Essential for precision copper work
//     - Automatic enable for finishing operations
//
//   HIGH-SPEED MACHINING MODES
//     - G05 P10000 - High-speed mode ON
//     - G05 P0 - High-speed mode OFF
//     - Optimized for copper's high machinability
//
//   CORNER DECELERATION CONTROL
//     - Automatic corner slowdown for accuracy
//     - Configurable angle threshold
//     - Prevents overcuts on trilobe lobes
//
///////////////////////////////////////////////////////////////////////////////
//
// CYCLE TIME REDUCTION FEATURES:
//
//   iMACHINING-STYLE VARIABLE FEED CONTROL
//     - Arc feed correction (CRITICAL for trilobes!)
//     - Direction change detection
//     - Smooth feed ramping
//     - 8-level aggressiveness slider
//
//   DYNAMIC DEPTH FEED ADJUSTMENT
//     - INCREASES feed when cutting shallow
//     - Never reduces below programmed for adaptive
//     - KEY to fast 3D adaptive clearing
//
//   CHIP THINNING COMPENSATION (COPPER-OPTIMIZED)
//     - Increases feed at low radial engagement
//     - Copper machines fast - take advantage of it!
//     - Up to 2x feed increase at light cuts
//
//   MINIMUM Z RETRACT BETWEEN WORK OFFSETS
//     - Only retract to clearance, not full home
//     - Saves significant time on multi-WCS jobs
//
//   ARC OPTIMIZATION FOR TRILOBES
//     - Enhanced arc interpolation
//     - Tight arc feed limiting
//     - Smooth arc-to-arc transitions
//
///////////////////////////////////////////////////////////////////////////////
//
// COPPER-SPECIFIC OPTIMIZATIONS:
//
//   - Higher default feeds (copper machines 3-5x faster than steel)
//   - Chip evacuation awareness (copper makes stringy chips)
//   - Coolant optimization (flood preferred for chip clearing)
//   - Tool engagement monitoring (copper grabs tools)
...
longDescription = "Ultra-enhanced post for Roku-Roku HC-658 II with Fanuc 31i-MODEL B5 controller. COPPER/TRILOBE OPTIMIZED: AI Contour Control II (AICC II) with R-level smoothing, nano smoothing for finishing, arc-optimized iMachining feed control, chip thinning compensation, dynamic depth feed for adaptive, minimum Z retract, corner deceleration control. All features configurable on/off.";
...
properties = {
  ...
  // =============== FANUC 31i-B5 SPECIFIC FEATURES ===============
  useAICC2: {
    title      : "Use AI Contour Control II (AICC II)",
    description: "Enable G05.1 Q1 Rx for advanced smoothing. HIGHLY RECOMMENDED for trilobes and copper.",
    group      : "fanuc31i",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  aiccRoughingLevel: {
    title      : "AICC roughing R-level (1-10)",
    description: "R-level for roughing operations. Lower = faster, Higher = smoother. R1-R3 typical for roughing.",
    group      : "fanuc31i",
    type       : "integer",
    range      : [1, 10],
    value      : 2,
  ...
  aiccFinishingLevel: {
    title      : "AICC finishing R-level (1-10)",
    description: "R-level for finishing operations. R7-R10 typical for finishing copper.",
    group      : "fanuc31i",
    type       : "integer",
    range      : [1, 10],
    value      : 8,
  ...
  useNanoSmoothing: {
    title      : "Use nano smoothing (G5.1 Q3)",
    description: "Enable sub-micron interpolation for ultra-fine finishes. WARNING: Requires Fanuc Nano CNC option - will cause PS0343 alarm if not installed!",
    group      : "fanuc31i",
    type       : "boolean",
    value      : false,
    scope      : "post"
  },
  nanoSmoothingFinishingOnly: {
    title      : "Nano smoothing only for finishing",
    description: "Only enable nano smoothing for finishing operations (stock to leave = 0).",
    group      : "fanuc31i",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  useSGISmoothing: {
    title      : "Use SGI precision modes (M250/M251/M252)",
    description: "Enable Smooth GI interpolation precision control. M252=finest (finish), M251=medium (rough). Verified from hyperMILL HC-658 certified post.",
    group      : "fanuc31i",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  useHighSpeedMode: {
    title      : "Use high-speed mode (G05 P10000)",
    description: "Enable high-speed machining mode. Good for copper's high machinability.",
    group      : "fanuc31i",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  useCornerDecel: {
    title      : "Use corner deceleration control",
    description: "Automatically slow at corners to prevent overcuts. Critical for trilobe accuracy.",
    group      : "fanuc31i",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  cornerDecelAngle: {
    title      : "Corner deceleration angle (deg)",
    description: "Start decelerating when direction change exceeds this angle.",
    group      : "fanuc31i",
    type       : "number",
    value      : 30,
    scope      : "post"
  ...
  useLookAhead: {
    ...
    description: "Number of blocks for look-ahead buffer. 0 = use controller default.",
    group      : "fanuc31i",
    type       : "integer",
    value      : 100,
    scope      : "post"
  ...
  useMinimumZRetract: {
    ...
    description: "When changing work offsets (NOT tool changes), retract only to clearance above stock.",
    group      : "cycleTime",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  minimumZRetractClearance: {
    ...
    description: "Distance above stock top for minimum retract between work offsets.",
    group      : "cycleTime",
    type       : "number",
    value      : 5,
    scope      : "post"
  ...
  // =============== iMACHINING-STYLE VARIABLE FEED CONTROL ===============
  useIMachiningFeed: {
    title      : "Use iMachining-style variable feed",
    description: "Adjusts feedrate based on cutting conditions. ESSENTIAL for trilobe arc accuracy!",
  ...
  iMachiningLevel: {
    ...
    description: "1 = conservative (50%), 8 = aggressive (100%). Start at 5 for copper.",
    group      : "feedOptimization",
    type       : "integer",
    range      : [1, 8],
    value      : 6,
  ...
  arcFeedCorrection: {
    ...
    description: "Feed percentage for arcs. CRITICAL FOR TRILOBES. 80-90% recommended.",
    group      : "feedOptimization",
    type       : "integer",
    range      : [50, 100],
    value      : 85,
  ...
  tightArcFeedReduction: {
    title      : "Tight arc extra reduction (%)",
    description: "Extra feed reduction for arcs smaller than tool diameter.",
    group      : "feedOptimization",
    type       : "integer",
    range      : [0, 50],
    value      : 20,
  ...
  feedRampingDistance: {
    ...
    description: "Distance over which to smoothly transition feedrates.",
    group      : "feedOptimization",
    type       : "number",
    value      : 0,
    scope      : "post"
  ...
  minChipLoadFeed: {
    ...
    description: "Never reduce feedrate below this percentage.",
    group      : "feedOptimization",
    type       : "integer",
    range      : [20, 100],
    value      : 50,
  ...
  // =============== DYNAMIC DEPTH FEED FOR ADAPTIVE ===============
  useDynamicDepthFeed: {
    title      : "Use dynamic depth feed adjustment",
    description: "INCREASE feedrate when cutting shallow. KEY to fast adaptive!",
    group      : "feedOptimization",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  adaptiveBaseFeed: {
    ...
    description: "Override base feed for adaptive. 0 = use Fusion's programmed feed.",
    group      : "feedOptimization",
    type       : "number",
    value      : 0,
    scope      : "post"
  },
  // =============== CHIP THINNING COMPENSATION (COPPER!) ===============
  useChipThinningCompensation: {
    title      : "Use chip thinning compensation",
    description: "HIGHLY RECOMMENDED FOR COPPER. Increase feed at low radial engagement.",
    group      : "feedOptimization",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  maxChipThinningMultiplier: {
    ...
    description: "Maximum feed increase for chip thinning. Copper can handle 2.0-2.5x!",
    group      : "feedOptimization",
    type       : "number",
    value      : 2.0,
    scope      : "post"
  ...
  useAdvancedFeedOptimization: {
    ...
    description: "Enable tool stickout analysis and comprehensive feed adjustment.",
    group      : "feedOptimization",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  showOptimizationNotes: {
    ...
    description: "Add comments showing cutting parameters and adjustments.",
    group      : "feedOptimization",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  toolStickoutMultiplier: {
    ...
    description: "How aggressively to reduce feed for long stickout. 1.0 = standard.",
    group      : "feedOptimization",
    type       : "number",
    value      : 1.0,
    scope      : "post"
  ...
  maxStickoutRatio: {
    title      : "Max safe stickout ratio",
    description: "Maximum L/D ratio before feed reduction.",
    group      : "feedOptimization",
    type       : "number",
    value      : 4.0,
    scope      : "post"
  ...
  roughingFeedMultiplier: {
    ...
    description: "Scale roughing feeds. Copper can often handle 120-150%!",
    group      : "feedOptimization",
    type       : "integer",
    range      : [10, 200],
    value      : 100,
  ...
  minimumFeedrate: {
    ...
    description: "Never go below this feed. 0 = no limit.",
    group      : "feedOptimization",
    type       : "number",
    value      : 0,
    scope      : "post"
  ...
  forceFeedOutput: {
    ...
    description: "Output F on every motion line. Required for variable feed.",
    group      : "feedOptimization",
    type       : "boolean",
    value      : false,
    scope      : "post"
  },
  // =============== TRILOBE-SPECIFIC SETTINGS ===============
  trilobeMode: {
    title      : "Trilobe optimization mode",
    description: "Enable special optimizations for trilobe machining.",
    group      : "trilobe",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  trilobeArcTolerance: {
    title      : "Trilobe arc tolerance",
    description: "Tighter tolerance for trilobe arcs. 0 = use standard.",
    group      : "trilobe",
    type       : "number",
    value      : 0.005,
    scope      : "post"
  },
  trilobeTransitionSmoothing: {
    title      : "Trilobe arc-to-arc smoothing",
    description: "Extra smoothing at arc transitions (lobe junctions).",
    group      : "trilobe",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  copperMode: {
    title      : "Copper machining mode",
    description: "Enable copper-specific optimizations (higher speeds, chip handling).",
    group      : "copper",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  copperFeedBoost: {
    title      : "Copper feed boost (%)",
    description: "Extra feed boost for copper material. Copper machines 3-5x faster than steel!",
    group      : "copper",
    type       : "integer",
    range      : [100, 200],
    value      : 100,
  ...
  copperChipBreaking: {
    title      : "Copper chip breaking dwells",
    description: "Add brief dwells to help break stringy copper chips.",
    group      : "copper",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
function onSection() {
  ...
  // Reset iMachining state for new section
  previousPosition = null;
  previousDirection = null;
  previousFeed = 0;
  
  // Initialize dynamic depth tracking for this section
  if (getProperty("useDynamicDepthFeed") || getProperty("useAdvancedFeedOptimization") || getProperty("useChipThinningCompensation")) {
    var tempParams = getCuttingParameters();
    if (getProperty("useDynamicDepthFeed")) {
      initializeDynamicDepth(tempParams);
    }
    cachedCuttingParams = tempParams;
  }
  ...
  // ENHANCED: Use minimum Z retract for work offset changes (not tool changes)
  if (newWorkOffset && !insertToolCall && !isFirstSection() && getProperty("useMinimumZRetract")) {
    writeMinimumRetract();
  } else if (insertToolCall || newWorkOffset || newWorkPlane || smoothing.cancel || state.tcpIsActive || currentSection.isMultiAxis()) {
  ...
  // Show operation type and optimization info
  if (getProperty("showOptimizationNotes") && cachedCuttingParams != null) {
    writeComment("=== OPERATION INFO ===");
    writeComment("Type: " + (cachedCuttingParams.isRoughing ? "ROUGHING" : "FINISHING") + 
                 (cachedCuttingParams.isAdaptive ? " (ADAPTIVE)" : "") +
                 (getProperty("copperMode") ? " [COPPER]" : "") +
                 (getProperty("trilobeMode") ? " [TRILOBE]" : ""));
    if (cachedCuttingParams.radialDepth > 0) {
      var engagement = Math.round((cachedCuttingParams.radialDepth / tool.diameter) * 100);
      writeComment("Radial: " + xyzFormat.format(cachedCuttingParams.radialDepth) + " (" + engagement + "% WOC)");
      if (getProperty("useChipThinningCompensation") && engagement < 50) {
        var chipThin = calculateChipThinningFactor(cachedCuttingParams.radialDepth, tool.diameter);
        if (chipThin.factor > 1.1) {
          writeComment(chipThin.suggestion);
        }
      }
    }
    writeComment("======================");
  ...
  // ENHANCED: Output AICC II control for Fanuc 31i
  var isFinishing = cachedCuttingParams != null && cachedCuttingParams.isFinishing;
  writeAICCControl(isFinishing);
  
  // ENHANCED: Output high-speed mode
  writeHighSpeedMode(true);
  
  // ENHANCED: Output nano smoothing for finishing
  writeNanoSmoothing(true, isFinishing);

  // ENHANCED: Output SGI interpolation precision (hyperMILL-verified M252/M251)
  writeSGISmoothing(true, isFinishing);
...
function onSectionEnd() {
  // Cancel Fanuc 31i enhanced features
  cancelAICCControl();
  writeHighSpeedMode(false);
  writeNanoSmoothing(false, false);
...
///////////////////////////////////////////////////////////////////////////////
//        FANUC 31i-B5 SPECIFIC CONTROL FUNCTIONS
///////////////////////////////////////////////////////////////////////////////

/**
  Output AICC II (AI Contour Control II) settings.
  G05.1 Q1 Rx - R1=fastest, R10=smoothest
*/
function writeAICCControl(isFinishing) {
  if (!getProperty("useAICC2")) {
    return;
  }
  
  var rLevel = isFinishing ? getProperty("aiccFinishingLevel") : getProperty("aiccRoughingLevel");
  writeBlock(gFormat.format(5.1), "Q1", "R" + rLevel);
  
  if (getProperty("showOptimizationNotes")) {
    writeComment("AICC II LEVEL R" + rLevel + (isFinishing ? " (FINISHING)" : " (ROUGHING)"));
  }
}

/**
  Cancel AICC II control.
*/
function cancelAICCControl() {
  if (getProperty("useAICC2")) {
    writeBlock(gFormat.format(5.1), "Q0");
  }
}
...
/**
  Output nano smoothing for ultra-fine finishes.
  G5.1 Q3 - Nano smoothing ON
*/
function writeNanoSmoothing(enable, isFinishing) {
  if (!getProperty("useNanoSmoothing")) {
    return;
  }
  
  // Only enable for finishing if that option is set
  if (getProperty("nanoSmoothingFinishingOnly") && !isFinishing) {
    return;
  }
  
  if (enable) {
    writeBlock(gFormat.format(5.1), "Q3");
    if (getProperty("showOptimizationNotes")) {
      writeComment("NANO SMOOTHING ON");
    }
  } else {
    writeBlock(gFormat.format(5.1), "Q0");
  }
...
/**
  Output SGI (Smooth GI) interpolation precision.
  M252 = finest (≤0.00002"), M250 = fine (≤0.00004"), M251 = medium (≤0.0004")
  Source: hyperMILL certified Roku-Roku HC-658 post package (F_R10b_E20)
*/
function writeSGISmoothing(enable, isFinishing) {
  if (!getProperty("useSGISmoothing")) {
    return;
  }
  if (enable) {
    if (isFinishing) {
      writeBlock(mFormat.format(252), formatComment("SGI FINEST - FINISH"));
    } else {
      writeBlock(mFormat.format(251), formatComment("SGI MEDIUM - ROUGH"));
    }
  }
  // SGI modes persist until changed — no explicit OFF needed
}

/**
  Output high-speed machining mode.
  G05 P10000 - High-speed ON, G05 P0 - OFF
*/
function writeHighSpeedMode(enable) {
  ...
  if (enable) {
    ...
    if (getProperty("showOptimizationNotes")) {
      writeComment("HIGH-SPEED MODE ON");
    }
  } else {
...
///////////////////////////////////////////////////////////////////////////////
//        iMACHINING-STYLE VARIABLE FEED CONTROL FUNCTIONS
///////////////////////////////////////////////////////////////////////////////
...
function calculateArcFeedFactor(arcRadius, toolRadius) {
  ...
  // Extra reduction for tight arcs (smaller than tool diameter)
  if (radiusRatio < 1) {
    var extraReduction = getProperty("tightArcFeedReduction") / 100;
    reductionFactor = correctionPercent * (1.0 - extraReduction);
  } else if (radiusRatio < 2) {
    reductionFactor = correctionPercent * (0.8 + (radiusRatio - 1) * 0.2);
  } else if (radiusRatio < 5) {
    reductionFactor = correctionPercent * (1.0 - (5 - radiusRatio) * 0.033);
  } else {
    reductionFactor = correctionPercent;
  }
  
  return Math.max(reductionFactor, getProperty("minChipLoadFeed") / 100);
}
...
function calculateDirectionChangeFactor(currentDir, prevDir) {
  ...
  // No reduction for small direction changes (< 15 degrees)
  if (angle < Math.PI / 12) {
    return 1.0;
  }
  
  // Linear reduction based on angle
  var angleRatio = (angle - Math.PI / 12) / (Math.PI - Math.PI / 12);
  var reductionFactor = 1.0 - (maxReduction * angleRatio);
...
*/
function applyFeedRamping(targetFeed, prevFeed, distance) {
  var rampDist = getProperty("feedRampingDistance");
  if (rampDist <= 0 || prevFeed <= 0 || distance <= 0) {
    return targetFeed;
  }
  ...
  var ratio = distance / rampDist;
  return prevFeed + (targetFeed - prevFeed) * ratio;
}

/**
  Update iMachining state after each move.
*/
function updateIMachiningState(x, y, z) {
...
*/
function applyIMachiningFeed(f, isArc, arcRadius, currentPos, moveDistance) {
  ...
  // Apply base level factor
  adjustedFeed = adjustedFeed * getIMachiningLevelFactor();
  
  // Apply arc feed correction (CRITICAL FOR TRILOBES!)
  if (isArc && arcRadius > 0) {
    var arcFactor = calculateArcFeedFactor(arcRadius, toolRadius);
    adjustedFeed = adjustedFeed * arcFactor;
    currentArcRadius = arcRadius;
  } else {
    currentArcRadius = 0;
  }
  
  // Apply direction change factor for linear moves
...
///////////////////////////////////////////////////////////////////////////////
//        ADVANCED FEED OPTIMIZATION FUNCTIONS
///////////////////////////////////////////////////////////////////////////////
...
function getCuttingParameters() {
  ...
  params.effectiveTopZ = zRange.getMaximum();
  params.effectiveBottomZ = zRange.getMinimum();
...
/**
  ...
  CRITICAL FOR COPPER - it machines fast, use it!
*/
function calculateChipThinningFactor(radialDepth, toolDiameter) {
  ...
  // No chip thinning needed above 50% engagement
  ...
  if (result.factor > 1.2) {
    result.suggestion = "CHIP THINNING: +" + Math.round((result.factor - 1) * 100) + "% feed increase";
  }
...
/**
  Calculate tool stickout factor.
*/
function calculateStickoutFactor(toolDiameter, toolLength, isFinishing) {
  ...
  var threshold = getProperty("maxStickoutRatio");
...
function initializeDynamicDepth(params) {
  ...
  if (getProperty("showOptimizationNotes")) {
    writeComment("DYNAMIC DEPTH FEED: Z range " + xyzFormat.format(dynamicDepthStockTop) + 
                 " to " + xyzFormat.format(dynamicDepthStockBottom));
  }
}

/**
  Calculate dynamic feed based on current Z position.
*/
function calculateDynamicDepthFeed(currentZ, baseFeed) {
  ...
  // At very shallow depths, maximum increase
  if (depthRatio < 0.1) {
...
function applyFeedMultiplier(f, movementType) {
  ...
  // Get roughing/finishing multiplier
  switch (movementType) {
  ...
  case MOVEMENT_PREDRILL:
    multiplier = getProperty("roughingFeedMultiplier") / 100;
    break;
  case MOVEMENT_FINISH_CUTTING:
    multiplier = getProperty("finishingFeedMultiplier") / 100;
    break;
  default:
    multiplier = 1.0;
  }
  
  // Apply copper feed boost if enabled
  if (getProperty("copperMode")) {
    multiplier = multiplier * (getProperty("copperFeedBoost") / 100);
  }
...
/**
  Write minimum retract for work offset changes.
*/
function writeMinimumRetract() {
  var clearance = getProperty("minimumZRetractClearance");
  var zRange = currentSection.getGlobalZRange();
  var stockTop = zRange.getMaximum();
  var retractZ = stockTop + clearance;
  
  writeComment("MIN RETRACT TO Z" + xyzFormat.format(retractZ));
...
function getFeed(f) {
  ...
  if (cachedCuttingParams == null && (getProperty("useAdvancedFeedOptimization") || getProperty("useChipThinningCompensation"))) {
  ...
  // Apply chip thinning compensation (CRITICAL FOR COPPER!)
  if (getProperty("useChipThinningCompensation") && cachedCuttingParams != null && cachedCuttingParams.radialDepth > 0) {
    var chipThin = calculateChipThinningFactor(cachedCuttingParams.radialDepth, tool.diameter);
    // Only INCREASE feed for adaptive/roughing (chip thinning), never reduce
    if (chipThin.factor > 1.0 || !cachedCuttingParams.isAdaptive) {
      f = f * chipThin.factor;
    }
  }
  
  // Apply feedrate multiplier based on movement type (and copper mode)
  f = applyFeedMultiplier(f, movement);
  
  // Apply iMachining-style feed adjustments (arc correction is CRITICAL for trilobes!)
  if (getProperty("useIMachiningFeed")) {
    f = applyIMachiningFeed(f, iMachiningIsArc, iMachiningArcRadius, 
                            iMachiningCurrentPos, iMachiningMoveDistance);
  }
  ...
  if (getProperty("useG95")) {
    return feedOutput.format(f / spindleSpeed);
  }
  
  // Force feedrate output on every line if enabled or using variable feed
  if (getProperty("forceFeedOutput") || getProperty("useIMachiningFeed") || 
      (getProperty("useDynamicDepthFeed") && dynamicDepthEnabled)) {
    feedOutput.reset();
    return feedOutput.format(f);
  }
  
  // Standard parametric feed handling
...
function onLinear(_x, _y, _z, feed) {
  ...
  // Calculate move distance and position for iMachining
  if (getProperty("useIMachiningFeed")) {
    var start = getCurrentPosition();
    var dx = _x - start.x;
    var dy = _y - start.y;
    var dz = _z - start.z;
    iMachiningMoveDistance = Math.sqrt(dx*dx + dy*dy + dz*dz);
    iMachiningCurrentPos = [_x, _y, _z];
    iMachiningIsArc = false;
    iMachiningArcRadius = 0;
  }
  ...
  // Update iMachining state for direction tracking
  if (getProperty("useIMachiningFeed")) {
    updateIMachiningState(_x, _y, _z);
  }
}
// <<<<< INCLUDED FROM include_files/onLinear_fanuc.cpi
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  // Set up iMachining variables for arc feed correction (CRITICAL FOR TRILOBES!)
  if (getProperty("useIMachiningFeed")) {
    var dx = start.x - cx;
    var dy = start.y - cy;
    iMachiningArcRadius = Math.sqrt(dx*dx + dy*dy);
    iMachiningIsArc = true;
    iMachiningCurrentPos = [x, y, z];
    var sweep = getCircularSweep();
    iMachiningMoveDistance = Math.abs(sweep * iMachiningArcRadius);
  }
