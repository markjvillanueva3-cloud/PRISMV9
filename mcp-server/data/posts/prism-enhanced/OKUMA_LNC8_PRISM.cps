/**
  ...
  Manufacturer: Okuma
  Control: LNC (Legacy)
  Type: 2-Axis CNC Lathe (Turning Only)
  ============================================================================
  PRISM OPTIMIZED TURNING TECHNOLOGY

  Legacy controller - limited macro variables, no collision avoidance,
  no thermal compensation. Use conservative S/F values. This machine is
  a workhorse for basic turning operations with excellent rigidity.
  64KB program memory hard limit - PRISM monitors program size continuously.

  Machine Specifications:
    - Max Spindle Speed: 3500 RPM
    - Spindle Motor: 11 kW / 300 Nm continuous
    - Chuck Size: 10" (254 mm) hydraulic 3-jaw
    - Turret: 8-station VDI40 turret (static tools only)
    ...
    - Bar Capacity: 65 mm
    - Tailstock: Manual/semi-automatic
    - Coolant: Standard flood coolant
    - Bed Type: Slant bed, high rigidity box ways
    - Program Memory: 64KB (CRITICAL LIMIT)

  Controller Features (LNC Legacy):
    - Basic absolute/incremental programming
    - Standard canned cycles: G70-G76, G85 (stock removal)
    - LIMITED custom macro support (32 variables max)
    - NO tool life management
    - NO thermal compensation
    - NO collision avoidance
    - NO nano smoothing
    - Fixed acceleration/deceleration profiles
    - Basic diagnostics only
    - 64KB program memory (limited program size)
    - NOTE: G83/G84 (advanced drilling/tapping) NOT available on LNC

  Supported G-Codes (LNC):
    ...
    G70  - Finishing cycle
    G71  - OD/ID roughing cycle (multi-pass threading)
    ...
    G96  - Constant surface speed (CSS) - use conservatively on LNC
    G97  - Constant RPM
    G98  - Feed per minute
    G99  - Feed per revolution
    NOTE: G83/G84 (advanced drilling/tapping) NOT available on LNC
    NOTE: G181-G189 (Okuma drilling canned cycles) NOT available on LNC
    NOTE: No polar interpolation (G12.1/G13.1 not available)
    NOTE: No Y-axis interpolation (G112 not available)

  Supported M-Codes (LNC):
    M00  - Program stop
    M01  - Optional stop
    M02  - Program end
    M03  - Spindle forward
    ...
    NOTE: No live tool M-codes (M23/M24/M25 not available)
    NOTE: No C-axis M-codes (M83/M84 not available)
    NOTE: No high-pressure chuck M-codes (M68/M69 not available)
  ...
  Kienzle Force Model (Turning):
    ...
    NOTE: Legacy machine with box ways - excellent rigidity allows deeper
    cuts despite older controller. High torque (300 Nm) at lower RPM range.
    PRISM leverages the high torque for aggressive roughing at moderate speeds.

  CSS Optimization:
    Vc = pi * D * n / 1000
    G50 clamp: n_max = 1000 * Vc_max / (pi * D_min)
    Machine limit: 3500 RPM
    CAUTION: Legacy controller has slower axis response. Use conservative
    CSS transitions - avoid small diameter face cuts at high Vc.
    PRISM applies conservative derating (default 10%) for LNC dynamics.

  Tool Life (Extended Taylor Equation):
    T = C / (Vc^n * f^a * ap^b)
    NOTE: No controller-side tool life tracking. PRISM provides the only
    tool life monitoring for this machine. Critical to track manually.

  Program Size Budget:
    Controller limit: 64KB
    Estimated bytes/line: ~40
    At 1500 lines: ~58KB (approaching limit)
    PRISM warns at configurable threshold (default 60KB).
    Use M98/M99 subprograms for repetitive patterns to save memory.

  Material Group Speed Recommendations (Carbide, coated):
    ISO P (Steel):        Vc 150-280 m/min, f 0.15-0.45 mm/rev
    ISO M (Stainless):    Vc 100-200 m/min, f 0.10-0.35 mm/rev
    ISO K (Cast Iron):    Vc 120-320 m/min, f 0.15-0.50 mm/rev
    ISO N (Non-ferrous):  Vc 200-800 m/min, f 0.10-0.40 mm/rev
    ISO S (Super alloys): Vc 20-60 m/min,   f 0.08-0.22 mm/rev
    ISO H (Hard steel):   Vc 60-150 m/min,  f 0.05-0.18 mm/rev
    NOTE: Conservative speeds due to legacy controller dynamics.
    High torque (300 Nm) supports heavy roughing at moderate Vc.

  ============================================================================
*/

description = "PRISM Enhanced - Okuma LNC8";
vendor = "Okuma";
...
longDescription = "PRISM-enhanced post processor for the Okuma LNC8 2-axis CNC lathe with legacy LNC controller. " +
  "Legacy LNC controller with limited macro support (32 variables). " +
  "8-station VDI40 turret with static tools only - no live tooling, no C-axis, no Y-axis. " +
  "High rigidity box way construction with excellent torque (300Nm) for heavy roughing. " +
  "CRITICAL: 64KB program memory limit - PRISM monitors program size and warns before M30. " +
  "PRISM provides physics-based optimization using Kienzle force model with conservative derating " +
  "and Taylor tool life monitoring (only tool life tracking available - no controller TLM). " +
  "Supports OD/ID turning, facing, grooving, threading (G33/G71), stock removal (G85), and part-off. " +
  "NOTE: G83/G84 advanced drilling NOT available. Use G74 peck drill only.";
...
tolerance = spatial(0.005, MM); // Wider tolerance for legacy controller
...
properties = {
  ...
  machineModel: {
    title      : "Machine Model",
    description: "Okuma LNC8 machine model identifier",
    group      : "prism",
    type       : "string",
    value      : "LNC8",
    scope      : "post"
  ...
  controllerModel: {
    title      : "Controller",
    description: "LNC legacy controller",
    group      : "prism",
    type       : "string",
    value      : "LNC",
    scope      : "post"
  ...
  prismConservativeMode: {
    title      : "PRISM Conservative Mode",
    description: "Apply conservative derating for legacy controller dynamics",
    group      : "prism",
    type       : "boolean",
    value      : true,
    scope      : "post"
  },
  prismConservativeDerating: {
    title      : "Conservative Derating (%)",
    description: "Speed/feed derating for legacy controller lag",
    group      : "prism",
    type       : "number",
    value      : 10.0,
    scope      : "post"
  ...
  prismToolLifeTracking: {
    ...
    description: "Enable PRISM Taylor-equation tool life monitoring (no controller-side TLM on LNC)",
    group      : "prism",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  maxProgramSize: {
    title      : "Max Program Size Warning (KB)",
    description: "Warn when program approaches this size - LNC controller 64KB hard limit",
    group      : "prism",
    type       : "integer",
    value      : 60,
    scope      : "post"
  ...
  maximumSpindleSpeed: {
    ...
    description: "Defines the maximum spindle speed allowed by the LNC8 (3500 RPM).",
    group      : "configuration",
    type       : "integer",
    range      : [0, 3500],
    value      : 3500,
...
var fpmCode = 94;
var fprCode = 95;
var programLineCount = 0; // PRISM: track lines for 64KB limit monitoring

// ============================================================================
// PRISM PHYSICS ENGINE INTERFACE — Okuma LNC8
// Legacy 2-axis turning: 11kW / 300Nm, 3500 RPM max, conservative controller
// ============================================================================

/**
 * PRISM Kienzle force calculation for turning operations.
 * LNC8 variant: leverages high torque (300Nm) capability.
 *
...
function prismCalculateCuttingForce(kc11, mc, ap, f) {
  ...
  var Fc = kc11 * ap * Math.pow(f, (1 - mc));
  return Fc;
}

/**
 * PRISM CSS optimization with legacy controller speed limit and derating.
 * LNC max: 3500 RPM. Conservative clamp strongly recommended.
 * Applies prismConservativeDerating to prevent axis lag-induced overcutting.
 *
 ...
 * @returns {number} Recommended G50 clamp RPM (conservatively derated)
 */
function prismCalculateG50Clamp(vc, dMin, machineMax) {
  ...
  // Apply conservative derating for legacy controller response lag
  if (getProperty("prismConservativeMode")) {
    var derating = 1 - (getProperty("prismConservativeDerating") / 100);
    nCalc = Math.round(nCalc * derating);
  }
 ...
 * LNC8: 11kW motor with 300Nm torque - optimized for low-speed heavy cuts.
 * Uses conservative 75% safety margin for legacy machine.
 */
function prismCheckSpindlePower(Fc, vc, availablePower) {
  ...
  return {
    ...
    safe: utilization <= 75 // Conservative 75% for legacy machine
  };
}

/**
 * PRISM torque check.
 * LNC8 excels in torque - validate we stay within the torque curve.
 */
 ...
 * Only tool life monitoring available - no controller TLM on LNC.
 * This is critical for LNC8 operations.
 */
...
/**
 * PRISM program size monitor.
 * LNC controller has 64KB program memory hard limit.
 * CRITICAL: This must be called BEFORE M30 to warn operator in time.
 * Estimates ~40 bytes per line average for Okuma LNC format.
 *
 * @param {number} lineCount - Current estimated line count
 * @returns {number} Estimated program size in KB
 */
function prismCheckProgramSize(lineCount) {
  var estimatedKB = (lineCount * 40) / 1024;
  if (estimatedKB > getProperty("maxProgramSize")) {
    warning("PRISM: Program size estimate (" + Math.round(estimatedKB) +
      "KB) exceeds " + getProperty("maxProgramSize") + "KB warning threshold." +
      " LNC controller hard limit is 64KB. Consider using M98/M99 subprograms.");
  } else if (estimatedKB > (getProperty("maxProgramSize") * 0.8)) {
    warning("PRISM: Program size estimate (" + Math.round(estimatedKB) +
      "KB) approaching " + getProperty("maxProgramSize") + "KB warning threshold.");
  }
  return estimatedKB;
}
...
function writeBlock() {
  programLineCount++; // PRISM: track lines for 64KB memory monitoring
...
function writeComment(text) {
  programLineCount++; // PRISM: comments also consume memory
...
function onOpen() {
  ...
  writeComment("PRISM ENHANCED - OKUMA LNC8");
  writeComment("CONTROLLER: LNC LEGACY");
  writeComment("TYPE: 2-AXIS TURNING LATHE - NO LIVE TOOLING");
  writeComment("CAUTION: 64KB PROGRAM MEMORY LIMIT");
  if (getProperty("prismConservativeMode")) {
    writeComment("PRISM CONSERVATIVE MODE: " + getProperty("prismConservativeDerating") + "PCT DERATING");
  }
  ...
  if (getProperty("writeTools")) {
    ...
    if (tools.getNumberOfTools() > 0) {
      for (var i = 0; i < tools.getNumberOfTools(); ++i) {
        ...
        // On LNC8, warn about non-turning tools
        if (!tool.isTurningTool()) {
          warning("LNC8: Tool T" + tool.number + " appears to be non-turning. LNC8 has no live tooling.");
        }
  ...
  // Global G50 spindle speed clamp (machine max 3500 RPM)
...
function onSection() {
  if (currentSection.getType() != TYPE_TURNING) {
    if (!hasParameter("operation-strategy") || (getParameter("operation-strategy") != "drill")) {
      if (currentSection.getType() == TYPE_MILLING) {
        error(localize("Milling toolpath is not supported. Okuma LNC8 is a 2-axis turning lathe with no live tooling."));
      } else {
  ...
  if (insertToolCall) {
    ...
    if (tool.number > 8) {
      warning("LNC8: Tool number " + tool.number + " exceeds 8-station turret capacity.");
    }
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    switch (cycleType) {
    case "drilling":
      // LNC8: use G74 peck drill - G181 not available on LNC
      writeBlock(
    ...
    case "counter-boring":
    case "deep-drilling":
      // LNC8: use G74 with incremental peck
      writeBlock(
        gCycleModal.format(74),
        getCommonCycle(x, y, z, cycle.retract - cycle.stock),
      ...
      break;
    case "chip-breaking":
      // LNC8: expand chip-breaking - no G183 available
      expandCyclePoint(x, y, z);
      break;
    case "tapping":
    case "left-tapping":
    case "right-tapping":
      // LNC8: G84 not available - expand to G01 tapping sequence
      expandCyclePoint(x, y, z);
      break;
    case "reaming":
    case "boring":
      // LNC8: use G74 for boring, no G189 available
      writeBlock(
        gCycleModal.format(74),
        getCommonCycle(x, y, z, cycle.retract - cycle.stock),
        conditional(E > 0, "E" + secFormat.format(E)),
    ...
    case "fine-boring":
    case "stop-boring":
      error(localize("Unsupported cycle " + cycleType + " on Okuma LNC8."));
      return;
    default:
      expandCyclePoint(x, y, z);
    }
...
function onClose() {
  ...
  // PRISM: Check program size BEFORE M30 — operator must see warning before program loads
  // LNC controller 64KB hard limit. This warning appears in Fusion output window.
  var finalSizeKB = prismCheckProgramSize(programLineCount);
  writeComment("PRISM EST SIZE: " + Math.round(finalSizeKB) + "KB / 64KB LNC LIMIT");
