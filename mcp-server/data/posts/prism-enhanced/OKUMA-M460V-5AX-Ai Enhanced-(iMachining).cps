/**
  ...
  OKUMA M460V-5AX Enhanced Post Processor

  $Revision: 44100 Enhanced Edition $
  $Date: 2025-12-22 $
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
//                        CYCLE TIME REDUCTION FEATURES (NEW!)
//
//   C-AXIS ROTARY REPOSITIONING
//     - Instead of Z retract → XY move → Z plunge, rotate C-axis to bring part to tool
//     - Can reduce repositioning time by 50-80% on rotary table operations
//     - Calculates optimal repositioning method automatically
//     - Configurable maximum rotation angle and minimum distance threshold
//
//   iMACHINING-STYLE VARIABLE FEED CONTROL
//     - Arc feed correction (slower in tight arcs)
//     - Direction change detection (slower at corners)
//     - Smooth feed ramping (prevents load spikes)
//     - 8-level aggressiveness slider
//
//   DYNAMIC DEPTH FEED ADJUSTMENT
//     - For 3D adaptive: INCREASES feedrate when cutting shallow
//     - Never reduces feed below programmed for adaptive
//     - Enables faster cycle times on variable-depth toolpaths
//
//   ADVANCED FEED OPTIMIZATION
//     - Tool stickout analysis (deflection risk)
//     - Chip thinning compensation (radial engagement)
//     - Axial depth adjustment
//     - Speed-up suggestions in G-code comments
//
//   MINIMUM Z RETRACT BETWEEN WORK OFFSETS
//     - Save cycle time by retracting only to clearance above stock
//     - Full retract still used for tool changes
//
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
longDescription = "Ultra-enhanced milling post for OKUMA Genos M460V-5AX with OSP-P300MA-H control. NEW CYCLE TIME FEATURES: C-axis rotary repositioning (rotate part to tool instead of XY moves), iMachining-style variable feed, dynamic depth feed for adaptive, advanced feed optimization. EXISTING: 5-axis simultaneous with TCP, high precision mode (G08), look-ahead control, corner rounding, singularity avoidance, rotary feed limiting, Super NURBS smoothing (G131), fixture offset (CALL OO88), chip conveyor, spindle warm-up, air blast, safe start, tool breakage detection, auto door.";
...
properties = {
  ...
  useTCPOutput: {
    ...
    description: "Output G169 (TCP on) and G168 (TCP off) for 5-axis operations.",
    group      : "multiAxis",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  // =============== C-AXIS ROTARY REPOSITIONING (CYCLE TIME SAVER!) ===============
  useCAxisRepositioning: {
    title      : "Use C-axis rotary repositioning",
    description: "Instead of Z retract → XY move → Z plunge, rotate C-axis to bring part to tool. Can save 50-80% on repositioning time!",
    group      : "cycleTime",
    type       : "boolean",
    value      : false,
    scope      : "post"
  },
  cAxisRepositionMaxAngle: {
    title      : "Max C-axis reposition angle (deg)",
    description: "Maximum C-axis rotation allowed for repositioning. Larger = more opportunities but longer rotations.",
    group      : "cycleTime",
    type       : "number",
    value      : 180,
    scope      : "post"
  },
  cAxisRepositionMinDistance: {
    title      : "Min XY distance for C reposition",
    description: "Minimum XY distance (mm) before considering C-axis repositioning. Below this, normal XY move is used.",
    group      : "cycleTime",
    type       : "number",
    value      : 25,
    scope      : "post"
  ...
  cAxisRepositionFeed: {
    title      : "C-axis reposition feed (deg/min)",
    description: "Feedrate for C-axis repositioning moves. 0 = use rapid.",
    group      : "cycleTime",
    type       : "number",
    value      : 0,
    scope      : "post"
  },
  showRepositionComments: {
    title      : "Show repositioning comments",
    description: "Add comments showing repositioning method selection and time savings.",
    group      : "cycleTime",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  useMinimumZRetract: {
    ...
    description: "When changing work offsets (NOT tool changes), retract only to clearance above stock instead of full retract.",
    group      : "cycleTime",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  useIMachiningFeed: {
    ...
    description: "Adjusts feedrate based on cutting conditions similar to SolidCAM iMachining.",
  ...
  iMachiningLevel: {
    title      : "iMachining aggressiveness level (1-8)",
    description: "Level 1 is most conservative (50% of programmed feed), Level 8 is most aggressive (100%).",
    group      : "feedOptimization",
    type       : "integer",
    range      : [1, 8],
    value      : 5,
  ...
  arcFeedCorrection: {
    ...
    description: "Percentage of programmed feed to use when cutting arcs. Smaller arcs get more reduction.",
    group      : "feedOptimization",
    type       : "integer",
    range      : [0, 100],
    value      : 100,
  ...
  feedRampingDistance: {
    ...
    description: "Distance over which to smoothly transition feedrates. Prevents sudden load spikes.",
    group      : "feedOptimization",
    type       : "number",
    value      : 0,
    scope      : "post"
  ...
  minChipLoadFeed: {
    ...
    description: "Never reduce feedrate below this percentage of programmed feed.",
    group      : "feedOptimization",
    type       : "integer",
    range      : [20, 100],
    value      : 50,
  ...
  useDynamicDepthFeed: {
    ...
    description: "For 3D adaptive: automatically INCREASE feedrate when cutting shallow. KEY to fast adaptive!",
    group      : "feedOptimization",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  adaptiveBaseFeed: {
    ...
    description: "Base feedrate for adaptive toolpaths. 0 = use Fusion's programmed feed.",
    group      : "feedOptimization",
    type       : "number",
    value      : 0,
    scope      : "post"
  ...
  useAdvancedFeedOptimization: {
    ...
    description: "Automatically adjust feedrates based on tool stickout, radial engagement, and axial depth.",
    group      : "feedOptimization",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  showOptimizationNotes: {
    ...
    description: "Add comments showing cutting parameters, adjustments, and speed-up suggestions.",
    group      : "feedOptimization",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  useChipThinningCompensation: {
    ...
    description: "Increase feedrate at low radial engagements to maintain target chip load.",
    group      : "feedOptimization",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  maxChipThinningMultiplier: {
    ...
    description: "Maximum feed increase for chip thinning. 2.0 = allow up to 2x feed increase.",
    group      : "feedOptimization",
    type       : "number",
    value      : 2,
    scope      : "post"
  ...
  maxStickoutRatio: {
    ...
    description: "Maximum stickout-to-diameter ratio before feed reduction.",
    group      : "feedOptimization",
    type       : "number",
    value      : 4.0,
    scope      : "post"
  ...
  roughingFeedMultiplier: {
    ...
    description: "Scale roughing/adaptive feeds. 120 = 20% faster than programmed.",
    group      : "feedOptimization",
    type       : "integer",
    range      : [10, 200],
    value      : 100,
  ...
  finishingFeedMultiplier: {
    ...
    description: "Scale finishing feeds. 100 = use programmed feed.",
    group      : "feedOptimization",
    type       : "integer",
    range      : [10, 200],
    value      : 100,
  ...
  forceFeedOutput: {
    ...
    description: "Output F on every motion line. Essential for variable feed toolpaths.",
    group      : "feedOptimization",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
function defineMachine() {
  ...
  if (getProperty("rotaryTableAxis") != "none") {
    ...
    if (getProperty("rotaryTableAxis") == "5axis") {
      ...
      var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-110, 20], preference:0, tcp:useTCP});
      ...
      if (getProperty("useTableDirectionCodes")) {
        aAxis = createAxis({coordinate:0, table:true, axis:rotaryVector, cyclic:true, range:[-110, 20], preference:0, tcp:useTCP});
      } else {
        aAxis = createAxis({coordinate:0, table:true, axis:rotaryVector, cyclic:true, range:[-110, 20], preference:0, tcp:useTCP});
      }
    ...
    if (false) { // note: setup your machine here
      var aAxis = createAxis({coordinate:X, table:true, axis:[1, 0, 0], offset:[0, 0, 0], range:[-110, 20], cyclic:false, preference:0, tcp:useTCP});
  ...
  if (!receivedMachineConfiguration) {
    ...
    /* home positions */
    machineConfiguration.setHomePositionX((unit == IN) ? 15 : 380); // M460V-5AX X travel 762mm
    machineConfiguration.setHomePositionY((unit == IN) ? 9 : 230); // M460V-5AX Y travel 460mm
  }
  if (getProperty("safePositionMethod") == "G0") {
    machineConfiguration.setRetractPlane((unit == IN) ? 18 : 450); // M460V-5AX Z travel 460mm
  }
...
///////////////////////////////////////////////////////////////////////////////
//        C-AXIS ROTARY REPOSITIONING FOR CYCLE TIME REDUCTION
//
// Instead of the traditional Z retract → XY move → Z plunge sequence,
// this feature rotates the C-axis to bring the part to the tool position.
// This can reduce repositioning time by 50-80% on rotary table operations!
///////////////////////////////////////////////////////////////////////////////

/** Global tracking for C-axis repositioning */
var lastCuttingPosition = null;
var cAxisRepositionCount = 0;
var cAxisTimeSaved = 0;

/**
  Calculate if C-axis repositioning is more efficient than XY move.
  Returns {useRotary: boolean, angle: number, timeSavings: number}
*/
function calculateCAxisReposition(fromX, fromY, toX, toY, partCenterX, partCenterY) {
  var result = {
    useRotary: false,
    angle: 0,
    timeSavings: 0,
  ...
  if (!getProperty("useCAxisRepositioning")) {
    result.reason = "C-axis repositioning disabled";
    return result;
  }
  ...
  var dx = toX - fromX;
  var dy = toY - fromY;
  var xyDistance = Math.sqrt(dx*dx + dy*dy);
  
  // Check minimum distance threshold
  if (xyDistance < getProperty("cAxisRepositionMinDistance")) {
    result.reason = "XY distance below threshold (" + xyzFormat.format(xyDistance) + "mm)";
    return result;
  }
  
  // Calculate angle from part center to both positions
  var fromAngle = Math.atan2(fromY - partCenterY, fromX - partCenterX);
  var toAngle = Math.atan2(toY - partCenterY, toX - partCenterX);
  
  // Calculate rotation needed
  var deltaAngle = (toAngle - fromAngle) * 180 / Math.PI;
  
  // Normalize to -180 to +180
  while (deltaAngle > 180) deltaAngle -= 360;
  while (deltaAngle < -180) deltaAngle += 360;
  
  // Check against maximum allowed rotation
  if (Math.abs(deltaAngle) > getProperty("cAxisRepositionMaxAngle")) {
    result.reason = "Rotation exceeds max (" + Math.abs(deltaAngle).toFixed(1) + " > " + getProperty("cAxisRepositionMaxAngle") + " deg)";
    return result;
  }
  
  // Calculate radii (distance from part center)
  var fromRadius = Math.sqrt(Math.pow(fromX - partCenterX, 2) + Math.pow(fromY - partCenterY, 2));
  var toRadius = Math.sqrt(Math.pow(toX - partCenterX, 2) + Math.pow(toY - partCenterY, 2));
  
  // If radii are very different, XY move is needed anyway
  var radiusDiff = Math.abs(fromRadius - toRadius);
  if (radiusDiff > xyDistance * 0.5) {
    result.reason = "Large radius difference requires XY move";
    return result;
  }
  
  // Estimate time savings
  // Assume: XY rapid ~500 mm/s, C-axis rapid ~100 deg/s, Z retract/plunge ~200 mm/s
  var zRetract = 50; // typical retract distance
  var xyTime = xyDistance / 500 + (zRetract * 2) / 200; // XY move + Z retract + Z plunge
  var cTime = Math.abs(deltaAngle) / 100 + radiusDiff / 500; // C rotation + small radial adjust
  
  if (cTime < xyTime * 0.8) { // Only use if significantly faster (20%+ savings)
    result.useRotary = true;
    result.angle = deltaAngle;
    result.timeSavings = xyTime - cTime;
    result.reason = "C-axis faster by " + (result.timeSavings * 1000).toFixed(0) + "ms";
  } else {
    result.reason = "XY move is faster or similar";
  }
...
/**
  Output C-axis repositioning move.
*/
function writeCAxisReposition(deltaAngle, radialAdjust, currentZ) {
  var cAxisFeed = getProperty("cAxisRepositionFeed");
  
  if (getProperty("showRepositionComments")) {
    writeComment("C-AXIS REPOSITION: " + deltaAngle.toFixed(2) + " deg");
  }
  
  // Get current C position and calculate new position
  var currentC = cOutput.getCurrent();
  if (currentC === undefined) currentC = 0;
  var newC = currentC + deltaAngle;
  
  // Output the rotation
  cOutput.reset();
  if (cAxisFeed > 0) {
    writeBlock(gMotionModal.format(1), cOutput.format(newC), "F" + feedFormat.format(cAxisFeed));
  } else {
    writeBlock(gMotionModal.format(0), cOutput.format(newC));
  }
  
  cAxisRepositionCount++;
}
...
function calculateArcFeedFactor(arcRadius, toolRadius) {
  ...
  if (radiusRatio < 1) {
    reductionFactor = 0.5 * correctionPercent;
  } else if (radiusRatio < 2) {
    reductionFactor = (0.5 + (radiusRatio - 1) * 0.2) * correctionPercent;
  } else if (radiusRatio < 5) {
    reductionFactor = (0.7 + (radiusRatio - 2) * 0.1) * correctionPercent;
  } else {
    reductionFactor = 1.0;
  }
...
function getFeed(f) {
  ...
  if (getProperty("useChipThinningCompensation") && cachedCuttingParams != null) {
    var chipThin = calculateChipThinningFactor(cachedCuttingParams.radialDepth, tool.diameter);
    // Only INCREASE feed for adaptive (chip thinning), never reduce
  ...
  // Force feedrate output on every line if enabled
...
function onSection() {
  ...
  if (newWorkOffset && !insertToolCall && !isFirstSection() && getProperty("useMinimumZRetract")) {
    writeMinimumRetract();
  } else if (insertToolCall || newWorkOffset || newWorkPlane) {
  ...
  if (getProperty("useAdvancedFeedOptimization") && getProperty("showOptimizationNotes")) {
    ...
    if (cuttingParams.isAdaptive || cuttingParams.is3D) {
      writeComment("=== FEED OPTIMIZATION ===");
      writeComment("Operation: " + (cuttingParams.isRoughing ? "ROUGHING" : "FINISHING") + 
                   (cuttingParams.isAdaptive ? " (ADAPTIVE)" : ""));
      if (cuttingParams.radialDepth > 0) {
        writeComment("Radial DOC: " + xyzFormat.format(cuttingParams.radialDepth) + 
                     " (" + Math.round((cuttingParams.radialDepth / tool.diameter) * 100) + "% of D)");
      }
      writeComment("=========================");
    }
...
function onLinear(_x, _y, _z, feed) {
  ...
  // Calculate move distance for iMachining
  if (getProperty("useIMachiningFeed")) {
  ...
  // Track position for C-axis repositioning
  lastCuttingPosition = [_x, _y, _z];
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  // Set up iMachining variables for arc feed correction
  if (getProperty("useIMachiningFeed")) {
    var dx = start.x - cx;
    var dy = start.y - cy;
...
function onSectionEnd() {
  ...
  if (operationSupportsTCP) {
    ...
    // Disable TCP
    if (getProperty("useTCPOutput")) {
      writeBlock(gFormat.format(168), formatComment("TCP OFF"));
    }
  }
  if (currentSection.isMultiAxis()) {
    if (getProperty("useCAS")) {
...
function onMoveToSafeRetractPosition() {
  ...
  if (currentSection.isMultiAxis() && operationSupportsTCP) {
    ...
    if (getProperty("useTCPOutput")) {
      writeBlock(gFormat.format(168), formatComment("TCP OFF FOR REWIND"));
    }
  }
}
...
function onClose() {
  ...
  writeRetract(Z);
  writeBlock(gMotionModal.format(0), gFormat.format(30), "P10", formatComment("RETURN TO REFERENCE"));
  gAbsIncModal.reset();
