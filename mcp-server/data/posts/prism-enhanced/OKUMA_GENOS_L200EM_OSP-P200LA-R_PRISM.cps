/**
  ...
  ============================================================================
  Machine: Okuma Genos L200E-M
  Manufacturer: Okuma
  Control: OSP-P200LA-R
  Type: Economy CNC Lathe (Turning Only - No Live Tooling)
  ============================================================================
  PRISM OPTIMIZED TURNING TECHNOLOGY

  Economy 2-axis turning lathe. OSP-P200LA-R has basic macro variable set
  and no collision avoidance. C-axis is physically absent - this is a pure
  turning machine. No live tooling, no milling capability.

  Machine Specifications:
    - Max Spindle Speed: 4200 RPM
    - Spindle Motor: 11 kW / 220 Nm continuous
  ...
  Controller Features (OSP-P200LA-R):
    - P200 generation controller (reduced macro variable set)
    - Absolute/incremental programming
    - Canned cycles: G70-G76, G83, G84, G181-G189 (drilling), G85 (stock removal)
    - Custom macro B compatible (limited variables)
    - Tool life management (up to 200 tools)
    - Basic thermal compensation
    - NO collision avoidance system
    - Standard acceleration/deceleration

  Supported G-Codes (OSP-P200LA-R):
    ...
    G70  - Finishing cycle
    G71  - Threading / OD-ID roughing (multi-pass)
    ...
    NOTE: G112 (Y-axis interpolation) NOT available - no Y-axis
    NOTE: G12.1/G13.1 (polar interpolation) NOT available - no C-axis

  Supported M-Codes (OSP-P200LA-R):
    M00  - Program stop
    M01  - Optional stop
    M02  - Program end
  ...
  Kienzle Force Model (Turning):
    ...
    NOTE: Economy machine - lower power (11kW) means tighter force budget.
    PRISM auto-derates parameters to keep power utilization under 70%.

  CSS Optimization:
    Vc = pi * D * n / 1000
    G50 clamp: n_max = 1000 * Vc_max / (pi * D_min)
    Machine limit: 4200 RPM

  Tool Life (Extended Taylor Equation):
    T = C / (Vc^n * f^a * ap^b)

  Material Group Speed Recommendations (Carbide, coated):
    ISO P (Steel):        Vc 150-300 m/min, f 0.12-0.40 mm/rev
    ISO M (Stainless):    Vc 100-220 m/min, f 0.08-0.30 mm/rev
    ISO K (Cast Iron):    Vc 130-350 m/min, f 0.12-0.45 mm/rev
    ISO N (Non-ferrous):  Vc 250-1200 m/min, f 0.10-0.35 mm/rev
    ISO S (Super alloys): Vc 20-70 m/min,   f 0.06-0.20 mm/rev
    ISO H (Hard steel):   Vc 70-180 m/min,  f 0.05-0.18 mm/rev
    NOTE: Derated 10-15% vs higher-end machines due to lower spindle power/torque.

  ============================================================================
*/

description = "PRISM Enhanced - Okuma Genos L200E-M";
vendor = "Okuma";
...
longDescription = "PRISM-enhanced post processor for the Okuma Genos L200E-M economy 2-axis CNC lathe. " +
  "OSP-P200LA-R controller with basic macro support and limited variable set. " +
  "12-station VDI40 turret with static tools only - no live tooling, no C-axis, no Y-axis. " +
  "Economy variant with lower power (11kW/220Nm) - PRISM auto-derates cutting parameters " +
  "to prevent spindle overload. Supports turning, facing, grooving, threading (G33/G71), " +
  "drilling canned cycles (G181-G189), stock removal (G85), and part-off operations. " +
  "Kienzle force model with 15% power derating. Taylor tool life monitoring active.";
...
properties = {
  ...
  machineModel: {
    title      : "Machine Model",
    description: "Okuma Genos L200E-M machine model identifier",
    group      : "prism",
    type       : "string",
    value      : "GENOS-L200EM",
    scope      : "post"
  ...
  controllerModel: {
    title      : "Controller",
    description: "OSP-P200LA-R controller generation",
    group      : "prism",
    type       : "string",
    value      : "OSP-P200LA-R",
    scope      : "post"
  ...
  prismPowerDerating: {
    title      : "PRISM Power Derating (%)",
    description: "Derate cutting parameters by this percentage for economy spindle protection",
    group      : "prism",
    type       : "number",
    value      : 15.0,
    scope      : "post"
  ...
  prismToolLifeTracking: {
    ...
    description: "Enable PRISM Taylor-equation tool life monitoring",
    group      : "prism",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  maximumSpindleSpeed: {
    ...
    description: "Defines the maximum spindle speed allowed by the Genos L200E-M (4200 RPM).",
    group      : "configuration",
    type       : "integer",
    range      : [0, 4200],
    value      : 4200,
...
// ============================================================================
// PRISM PHYSICS ENGINE INTERFACE — Okuma Genos L200E-M
// Economy variant: 11kW / 220Nm, 4200 RPM max, pure 2-axis turning
// ============================================================================

/**
 * PRISM Kienzle force calculation for turning operations.
 * Economy variant: auto-derates by prismPowerDerating percentage.
 *
 * @param {number} kc11 - Specific cutting force at 1mm^2 (N/mm^2)
 * @param {number} mc   - Kienzle exponent (material-dependent, 0.17-0.35)
...
/**
 * PRISM CSS optimization with economy spindle limit.
 * Genos L200E-M max: 4200 RPM. OSP-P200LA-R standard acceleration.
 *
...
/**
 * PRISM power check with economy derating.
 * Applies automatic derating factor for the lower-powered L200E-M spindle.
 *
 * @param {number} Fc             - Cutting force (N)
 * @param {number} vc             - Cutting speed (m/min)
 * @param {number} availablePower - Machine spindle power (kW)
 * @returns {object} {required, available, utilization, safe}
 */
function prismCheckSpindlePower(Fc, vc, availablePower) {
  var deratingFactor = 1 - (getProperty("prismPowerDerating") / 100);
  var effectivePower = availablePower * deratingFactor;
  var requiredPower = (Fc * vc) / (60 * 1000);
  var utilization = (requiredPower / effectivePower) * 100;
  return {
    required: requiredPower,
    available: effectivePower,
    nominal: availablePower,
    derating: getProperty("prismPowerDerating"),
 ...
 *
 * @param {number} C  - Taylor constant
 * @param {number} vc - Cutting speed (m/min)
 * @param {number} n  - Speed exponent
 * @param {number} f  - Feed (mm/rev)
 * @param {number} a  - Feed exponent
 * @param {number} ap - Depth of cut (mm)
 * @param {number} b  - DOC exponent
...
/**
 * PRISM Y-axis guard.
 * Genos L200E-M has NO Y-axis. This function raises an error if any
 * Y-axis motion is detected in the CAM output, preventing machine crashes.
 *
 * @param {number} y - Y coordinate value from CAM
 */
function prismValidateNoYAxis(y) {
  if (y !== undefined && Math.abs(y) > 1e-6) {
    error("Okuma Genos L200E-M does NOT have a Y-axis. Y-axis move detected: Y=" + y +
      ". This machine is a pure 2-axis turning lathe. Remove Y-axis toolpaths from the operation.");
  }
}
...
function onOpen() {
  ...
  writeComment("PRISM ENHANCED - OKUMA GENOS L200E-M");
  writeComment("CONTROLLER: OSP-P200LA-R");
  writeComment("TYPE: 2-AXIS TURNING LATHE - NO LIVE TOOLING");
  writeComment("PRISM POWER DERATING: " + getProperty("prismPowerDerating") + "PCT");
  ...
  // Global G50 spindle speed clamp (machine max 4200 RPM)
  ...
  if (currentSection.getType() != TYPE_TURNING) {
    if (!hasParameter("operation-strategy") || (getParameter("operation-strategy") != "drill")) {
      if (currentSection.getType() == TYPE_MILLING) {
        error(localize("Milling toolpath is not supported. Okuma Genos L200E-M is a 2-axis turning lathe."));
      } else {
...
function onRapid(_x, _y, _z) {
  // PRISM Y-axis guard: error on any Y motion
  prismValidateNoYAxis(_y);
