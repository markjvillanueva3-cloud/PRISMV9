...
/**
  ...
  $Revision: 44800 Ultra Enhanced Edition v2 - Spring Pass Fix $
  $Date: 2026-01-15 $
  ...
  10. SPRING PASS RAPID FIX (NEW!)
     - Added "Use G0 for rapids" property (default: enabled)
     - Fixes issue where retracts and repositions during spring passes
       used G1 feed moves instead of G0 rapids
     - Significantly reduces cycle time on turning profile operations
     - Applies to both 2-axis and 5-axis rapid positioning moves
...
properties = {
  ...
  useG0: {
    title      : "Use G0 for rapids",
    description: "Enable to use G0 rapid traverse for non-cutting moves (retracts, repositions, spring pass transitions). Disable to use G1 with high feed rate instead.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
...
function onRapid(_x, _y, _z) {
  ...
  if (x || y || z) {
    // SPRING PASS FIX: When useG0 is enabled (default), use G0 for rapid moves even when
    // multiple axes are moving simultaneously. This ensures retracts and repositions
    // during spring passes use true rapids instead of G1 feed moves.
    // Only fall back to G1 if useG0 is disabled or if using polar interpolation.
...
function onRapid5D(_x, _y, _z, _a, _b, _c) {
  ...
  if (x || y || z || a || b || c) {
    // Use G0 if useG0 property is enabled, otherwise use G1 with high feedrate for multi-axis moves
