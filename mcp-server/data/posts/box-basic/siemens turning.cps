...
function onSection() {
  ...
  writeBlock(gAbsIncModal.format(90), formatFeedMode(currentSection.feedMode), gPlaneModal.format(18));
...
function onCyclePoint(x, y, z) {
  ...
  case "thread-turning":
    ...
    if (isLastCyclePoint() && threadNumber == numberOfThreads) {
      ...
      var MPIT = "";
      var SPL = z - cycle.incrementalZ;
      var FPL = z;
      var DM1 = (x - cycle.incrementalX) + (threadHeight * (id ? -1 : 1));
      var DM2 = x + (threadHeight * (id ? -1 : 1));
      var APP = 0;
