...
var feedFormat = fprFormat;
var pitchFormat = createFormat({decimals:4, forceDecimal:true});
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    writeToolBlock("T" + toolFormat.format(tool.number * 100 + tool.compensationOffset));
    if (tool.comment) {
  ...
  writeBlock(formatFeedMode(currentSection.feedMode),
    feedOutput.format(currentSection.feedMode == FEED_PER_REVOLUTION ? (unit == MM) ? 1 : 0.1 : (unit == MM) ? 100 : 10) // G94/G95 requires a feedrate
  );
