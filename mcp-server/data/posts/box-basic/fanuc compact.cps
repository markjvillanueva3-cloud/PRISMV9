/**
  ...
  FORKID {60C10204-0780-4dbb-954A-146D518D8E3F}
*/

description = "FANUC (Compact)";
vendor = "Fanuc";
...
longDescription = "Fanuc post illustrating how to generate very compact programs for old CNCs.";
...
var settings = {
  ...
  workPlaneMethod: {
    useTiltedWorkplane: false // not supported
  },
  comments: {
...
function onOpen() {
  ...
  settings.comments.maximumLineLength = 0; // disable comment output
  writeProgramHeader();
...
function stopSubprogram() {
  if (isRedirecting()) {
    var text = getRedirectionBuffer();
    closeRedirection();
    if (text) {
      if (blockCount < MINIMUM_SUBPROGRAM_SIZE) { // too small
        write(text);
      } else {
        // find subprogram
        var id;
        ...
        for (var i = 0; i < subprograms.length; ++i) {
          ...
          if (text == sp) {
            id = i;
            break;
          } else if (text.indexOf(sp) == 0) {
            id = i;
            partial = true;
            text = text.substr(sp.length);
            break;
          }
        }
        if ((id == undefined) || partial) {
          if (partial) {
            redirectToBuffer();
            writeBlock("P" + (subprogramId + id + 1), mFormat.format(98));
            text = getRedirectionBuffer() + text;
            closeRedirection();
          ...
          id = subprograms.length;
          subprograms.push(text);
        }
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      writeBlock(gAbsIncModal.format(90), gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3), iOutput.format(cx - start.x), jOutput.format(cy - start.y), getFeed(feed));
      break;
    case PLANE_ZX:
      writeBlock(gAbsIncModal.format(90), gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3), iOutput.format(cx - start.x), kOutput.format(cz - start.z), getFeed(feed));
      break;
    case PLANE_YZ:
      writeBlock(gAbsIncModal.format(90), gPlaneModal.format(19), gMotionModal.format(clockwise ? 2 : 3), jOutput.format(cy - start.y), kOutput.format(cz - start.z), getFeed(feed));
      break;
    default:
      linearize(tolerance);
    }
    ...
    case PLANE_XY:
      ...
      writeBlock(gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), _z, iOutput.format(cx - start.x), jOutput.format(cy - start.y), getFeed(feed));
      break;
    case PLANE_ZX:
      var _z = zOutput.format(z);
      if (_z) {
      ...
      writeBlock(gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), _z, iOutput.format(cx - start.x), kOutput.format(cz - start.z), getFeed(feed));
      break;
    case PLANE_YZ:
      var _z = zOutput.format(z);
      if (_z) {
      ...
      writeBlock(gPlaneModal.format(19), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), _z, jOutput.format(cy - start.y), kOutput.format(cz - start.z), getFeed(feed));
      break;
    default:
      linearize(tolerance);
    }
    ...
    case PLANE_XY:
      ...
      writeBlock(gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), _z, "R" + rFormat.format(r), getFeed(feed));
      break;
    case PLANE_ZX:
      var _z = zOutput.format(z);
      if (_z) {
      ...
      writeBlock(gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), _z, "R" + rFormat.format(r), getFeed(feed));
      break;
    case PLANE_YZ:
      var _z = zOutput.format(z);
      if (_z) {
      ...
      writeBlock(gPlaneModal.format(19), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), _z, "R" + rFormat.format(r), getFeed(feed));
      break;
    default:
      linearize(tolerance);
    }
...
function onClose() {
  ...
  for (var i = 0; i < subprograms.length; ++i) {
    var program = subprograms[i];
    writeln("");
    writeln("O" + oFormat.format(subprogramId + i + 1));

    var id = 0;
    for (var j = i + 1; j < subprograms.length; ++j) {
      var otherProgram = subprograms[j];
      if (program.indexOf(otherProgram) == 0) {
        id = j;
        program = program.substr(otherProgram.length);
        break;
      }
    }
    if (id > 0) {
      redirectToBuffer();
      writeBlock("P" + (subprogramId + id + 1), mFormat.format(98));
      program = getRedirectionBuffer() + program;
      closeRedirection();
    }
...
// add blockCount to existing writeBlock function
var cachedFunction = this.writeBlock;
this.writeBlock = function() {
