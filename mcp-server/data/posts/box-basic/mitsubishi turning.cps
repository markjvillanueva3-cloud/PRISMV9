...
properties = {
  ...
  type: {
    title      : "Type",
    description: "Specifies the G-code list type",
    group      : "formats",
    type       : "enum",
    values     : [
      {title:"2", id:"2"},
...
function onOpen() {
  ...
  case "7":
    ...
    break;
  default:
    error(localize("Unsupported G-code list type. Only types 2 - 7 are allowed."));
    return;
  }

  fpmCode = activeGCodeList == 2 ? 98 : 94;
  fprCode = activeGCodeList == 2 ? 99 : 95;
...
function startSpindle(tappingMode, forceRPMMode, initialPosition, rpm) {
  ...
  if (tappingMode) {
    writeBlock(mFormat.format(29), sOutput.format(_spindleSpeed));
    return;
  } else {
