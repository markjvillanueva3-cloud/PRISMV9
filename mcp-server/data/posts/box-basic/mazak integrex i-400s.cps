var modelType = "Integrex i-400S";
description = "Mazak Integrex i-400S";
...
/**
  ...
  $Revision: 44199 5a7fc143bd3dc3cca562058791d89c3375097bb9 $
  $Date: 2025-10-14 10:23:33 $
...
if (typeof (revision) == "number" && revision >= 50279) {
...
var zOutput = createOutputVariable({onchange:function () {retracted = false; machineState.isPrepositioned = false;}, prefix:"Z"}, zFormat);
...
function onSection() {
  ...
  currentTurret = isFirstSection() ? activeTurret : activateMachine(currentSection);
  ...
  if (insertToolCall && partCutoff && partCutoffTool != undefined) {
    // avoid redundant tool change if the part cutoff tool is already set
    var toolProperties = ["number", "compensationOffset", "diameterOffset", "lengthOffset", "productId"];
    insertToolCall = toolProperties.some(function(criteria) {return partCutoffTool[criteria] != tool[criteria];});
    partCutoffTool = undefined;
  }
  ...
  if (insertToolCall || newSpindle || newWorkOffset || newWorkPlane) {
    ...
    if (!machineState.spindlesAreAttached) {
      ...
      if (isFirstSection()) {
        onCommand(COMMAND_UNLOCK_MULTI_AXIS);
        gMotionModal.reset();
        writeBlock(gMotionModal.format(0), gAbsIncModal.format(90), gFormat.format(53), bOutput.format(0));
      }
    } else {
      if (insertToolCall) {
        onCommand(COMMAND_UNLOCK_MULTI_AXIS);
      ...
      writeRetract(X, Y);
      if (!machineState.isPrepositioned && !partCutoff) {
        writeRetract(ZB);
      }
    }
    forceAny();
  ...
  machineState.isPrepositioned = false; // reset prepositioned state as it is only used for stock transfer operations
  ...
  if (machineConfiguration.isMultiAxisConfiguration()) {
    if (machineState.isTurningOperation || machineState.axialCenterDrilling) { // turning toolpath
      if (gotBAxis) {
        if (!machineState.spindlesAreAttached && (insertToolCall || bFormat.areDifferent(bAxisOrientationTurning.y, bOutput.getCurrent()))) {
          writeBlock(mFormat.format(getCode("UNLOCK_B_AXIS")));
          writeBlock(gMotionModal.format(0), gFormat.format(53), bOutput.format(bAxisOrientationTurning.y));
          writeBlock(mFormat.format(getCode("LOCK_B_AXIS")));
        }
      } else {
  ...
  // workaround, synchronize main and subspindle to be able to use TCP on the subspindle
  if (currentSection.isMultiAxis() && operationSupportsTCP && currentSection.spindle == SPINDLE_SECONDARY) {
    cOutput.setPrefix("C");
    writeBlock(mFormat.format(getCode("ENABLE_C_AXIS", SPINDLE_MAIN)), mFormat.format(getCode("ENABLE_C_AXIS", SPINDLE_SUB)));
    writeBlock(gFormat.format(0), gFormat.format(53), cOutput.format(0), uOutput.format(0));
    writeBlock(mFormat.format(getCode("SPINDLE_SYNCHRONIZATION_PHASE")), "(SYNC MAIN AND SUB SPINDLES)");
  }
  ...
  if (currentSection.isMultiAxis() && operationSupportsTCP) {
    ...
    if (currentSection.isOptimizedForMachine()) {
      abc = currentSection.getInitialToolAxisABC();
      writeBlock(gMotionModal.format(0), gAbsIncModal.format(90), aOutput.format(abc.x), bOutput.format(abc.y), cOutput.format(abc.z));
    }
    ...
    if (insertToolCall || retracted || newWorkPlane || newWorkOffset || newSpindle) {
      ...
      if (machineState.usePolarCoordinates) {
        ...
        writeBlock(gMotionModal.format(0), xOutput.format(polarPosition.first.x), conditional(gotYAxis, yOutput.format(0)));
      } else {
        if (machineState.usePolarInterpolation) {
          ...
          writeBlock(gMotionModal.format(0), xOutput.format(polarPosition.first.x), conditional(gotYAxis, yOutput.format(polarPosition.first.y)));
        } else if (machineState.isTurningOperation || machineState.axialCenterDrilling) { // turning
          // in case spindles are attached, we need to position in B and Z axes simultaneously
          var b = machineState.spindlesAreAttached && bFormat.areDifferent(bOutput.getCurrent(), bAxisOrientationTurning.y) ? bOutput.format(bAxisOrientationTurning.y) : "";

          writeBlock(gPlaneModal.format(18));
          writeBlock(conditional(b, mFormat.format(getCode("UNLOCK_B_AXIS"))));
          writeBlock(gMotionModal.format(0), gFormat.format(offsetCode), "P1", b, zOutput.format(initialPosition.z), offset);
          writeBlock(conditional(b, mFormat.format(getCode("LOCK_B_AXIS"))));
          ...
          if (!machineState.spindlesAreAttached) {
            ...
            writeBlock(
              ...
              aOutput.format(abc.x), bOutput.format(abc.y), offset
            );
            onCommand(COMMAND_LOCK_MULTI_AXIS);
          }
        }
      ...
      if (machineState.usePolarInterpolation) {
        writeBlock(gMotionModal.format(0), xOutput.format(polarPosition.first.x), conditional(gotYAxis, yOutput.format(polarPosition.first.y)), zOutput.format(polarPosition.first.z));
      }
    }
  }
...
function setPolarInterpolation(activate) {
  if (activate) {
    ...
    writeBlock(gPlaneModal.format(getG17Code()), (getSpindle(PART) == SPINDLE_MAIN) ? "XC" : "XU");// XC halts Sub Spindle Operations
...
function writeRetract() {
  ...
  for (var i = 0; i < arguments.length; ++i) {
    ...
    case ZB:
      zOutput.reset();
      var zHome = "Z#" + ((currentSection.spindle == SPINDLE_SECONDARY) ? zSubHomeParameter : zHomeParameter);
      if (arguments[0] == ZB) {
        bOutput.reset();
        outputClampCodes = bOutput.getCurrent() != 0;
        words.push(formatWords(bOutput.format(0), zHome));
      } else {
        words.push(zHome);
      }
      retracted = true; // specifies that the tool has been retracted to the safe plane
...
function writeToolCall(_tool) {
  if (arguments[0] == undefined) {
    _tool = tool; // use current tool if no argument is passed
  }
  forceWorkPlane();
  forceModals();
  setCoolant(COOLANT_OFF, activeTurret);
  ...
  if (!retracted) {
    ...
    if (!machineState.spindlesAreAttached) {
      writeRetract(Z);
    } else if (!machineState.isPrepositioned) {
      writeRetract(ZB);
    }
  }
  var toolCall = "T" + toolFormat.format(_tool.number) + conditional(_tool.productId, "." + _tool.productId.replace(".", ""));
  var nextToolNumber = "";
  if (getProperty("preloadTool")) {
    var nextTool = getNextTool(_tool.number);
    if (nextTool) {
      nextToolNumber = "T" + toolFormat.format(nextTool.number);
    } else {
      var firstTool = getSection(0).getTool().number;
      nextToolNumber = "T" + toolFormat.format(_tool.number != firstTool ? firstTool : 0);
    }
  }
  writeToolBlock(toolCall, nextToolNumber, mFormat.format(6));
  bOutput.reset(); // tool change happens at B0
  if (_tool.comment) {
    writeComment(_tool.comment);
  }
  ...
  writeRetract(X, Y);
  writeRetract(machineState.spindlesAreAttached ? ZB : Z);
  activeTurret = currentTurret;
}

var partCutoffTool = undefined;
function onCycle() {
  if (cycleType == "tool-call") {
    ...
    if (machineState.spindlesAreAttached) {
      error(subst(localize("A Tool Call operation is not allowed during part transfers or when spindles are attached at operation \"%1\"."), getOperationComment()));
    }
    validate(hasNextSection() && getNextSection().getParameter("operation-strategy") == "turningSecondarySpindleGrab",
      localize("The 'Tool Call' operation must be placed directly before a 'Subspindle Grab' operation."));

    writeln("");
    writeComment(getParameter("operation-comment", ""));
    if (!insertToolCall) {
      writeComment(localize("No tool change is required for this operation."));
    }
    if (insertToolCall || cycle.toolCallPositionWorkCS || cycle.toolCallPositionMachineCS) {
      onCommand(COMMAND_COOLANT_OFF);
      onCommand(COMMAND_STOP_SPINDLE);
      writeRetract(X, Y);
      if (!machineState.spindlesAreAttached) {
        writeRetract(isFirstSection() ? ZB : Z);
      }
      if (getProperty("optionalStop")) {
        onCommand(COMMAND_OPTIONAL_STOP);
        gMotionModal.reset();
      ...
      if (cycle.toolCallPositionWorkCS || cycle.toolCallPositionMachineCS) {
        var bAxisAngle = 0;
        if (hasNextSection()) {
          for (var i = getNextSection().getId(); i < getNumberOfSections(); ++i) {
            var section = getSection(i);
            var operationStrategy = section.getParameter("operation-strategy");
            if (operationStrategy != "turningSecondarySpindleGrab" && operationStrategy != "turningSecondarySpindleReturn" && operationStrategy != "turningSecondarySpindlePull") {
              if (operationStrategy == "turningPart" && (tool.number == section.getTool().number)) {
                bAxisAngle = toRad(90); // only in case of a cutoff operation and when the cutoff tool is called we want to preposition the B-axis to B90
                warning(subst(localize("The B-axis is prepositioned to B90 in operation '%1' due to the following cutoff operation."), section.getParameter("operation-comment", "")));
              }
              break;
            }
          }
        ...
        zOutput.reset();
        gMotionModal.reset();
        if (cycle.useMachineFrame) {
          writeBlock(gMotionModal.format(0), gFormat.format(53), "Z" + zFormat.format(cycle.toolCallPositionMachineCS), bOutput.format(bAxisAngle));
        } else {
          var offset = getProperty("useFixedOffset") ? "H#3020" : "H" + (tool.isTurningTool() ? tool.compensationOffset : tool.lengthOffset);
          writeBlock(currentSection.wcs);
          currentWorkOffset = undefined;
          writeBlock(gMotionModal.format(0), gFormat.format(43), zOutput.format(cycle.toolCallPositionWorkCS.z), bOutput.format(bAxisAngle), offset);
        }
        retracted = false;
        machineState.isPrepositioned = true;
        forceWorkPlane();
      }
    }
    return;
  ...
  if ((typeof isSubSpindleCycle == "function") && isSubSpindleCycle(cycleType)) {
    ...
    var useLegacyTransfer = false;
    if (cycleType != "secondary-spindle-return" && cycleType != "secondary-spindle-pull") {
      if (cycleType == "secondary-spindle-grab" && (isFirstSection() || getPreviousSection().getParameter("operation-strategy") != "turningToolCall")) {
        if (hasNextSection()) {
          for (var i = getNextSection().getId(); i < getNumberOfSections(); ++i) {
            var section = getSection(i);
            var operationStrategy = section.getParameter("operation-strategy");
            if (operationStrategy != "turningSecondarySpindleGrab" && operationStrategy != "turningSecondarySpindleReturn" && operationStrategy != "turningSecondarySpindlePull") {
              writeRetract(ZB);
              if (operationStrategy == "turningPart") {
                writeToolCall(section.getTool());
                partCutoffTool = section.getTool();
                useLegacyTransfer = true;
              }
              break;
            }
          }
      ...
      if (!machineState.isPrepositioned) {
        writeRetract(bOutput.getCurrent() != 0 ? ZB : Z);
      }

      gMotionModal.reset();
      gAbsIncModal.reset();
      if (useLegacyTransfer) {
        bOutput.reset();
        writeBlock(mFormat.format(getCode("UNLOCK_B_AXIS")));
...
function onCycleEnd() {
  if (!cycleExpanded && !machineState.stockTransferIsActive && cycleType != "tool-call") {
    switch (cycleType) {
    case "thread-turning":
      forceFeed();
      xOutput.reset();
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    if (!(machineState.usePolarInterpolation || machineState.usePolarCoordinates || machineState.spindlesAreAttached || currentSection.isMultiAxis())) {
  ...
  case COMMAND_START_SPINDLE:
    forceSpindleSpeed = false;
    writeBlock(gSpindleModeModal.format(getCode("CONSTANT_SURFACE_SPEED_OFF")), mFormat.format(getCode(tool.clockwise ? "START_SPINDLE_CW" : "START_SPINDLE_CCW", getSpindle(TOOL))), sOutput.format(tool.spindleRPM));
    break;
...
function getSpindleTransferCodes(_transferType) {
  ...
  if (isLastSection() || (!isFirstSection() && getPreviousSection().getParameter("operation-strategy") == "turningToolCall")) {
...
function onSectionEnd() {
  ...
  if ((tool.getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED) && !machineState.spindlesAreAttached && getParameter("operation-strategy") != "turningToolCall") {
  ...
  if (currentSection.isMultiAxis() && operationSupportsTCP && currentSection.spindle == SPINDLE_SECONDARY) {
    writeBlock(mFormat.format(getCode("SPINDLE_SYNCHRONIZATION_PHASE_OFF")), "(UNSYNC MAIN AND SUB SPINDLES)");
  }
  forcePolarCoordinates = false;
  forcePolarInterpolation = false;
  partCutoff = false;
...
// <<<<< INCLUDED FROM ../common/mazak integrex i.cps
properties.maximumSpindleSpeed.value = 3300;
