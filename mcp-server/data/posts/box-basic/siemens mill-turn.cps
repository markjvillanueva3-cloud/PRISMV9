...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    writeToolBlock("T" + (getProperty("toolAsName") ? "=" + "\"" + (tool.description.toUpperCase()) + "\"" : toolFormat.format(tool.number)), dFormat.format(toolLengthOffset));
...
function writeRetract() {
  ...
  if (words.length > 0) {
    writeBlock(gFormat.format(53), gMotionModal.format(0), words, dOutput.format(0)); // retract
    writeBlock(dOutput.format(toolLengthOffset));
  }
  zOutput.reset();
}
