/**
  ...
  HWACHEON Hi-TECH 230AL Mill/Turn post processor configuration.

  $Revision: 44191 10f6400eaf1c75a27c852ee82b57479e7a9134c0 $
  $Date: 2025-08-21 13:23:15 $

  FORKID {2F8367FA-2D1F-4306-B2E4-D00064BC57E1}
*/
...
description = "Hwacheon Hi-TECH 230AL Mill/Turn with Fanuc control";
vendor = "Hwacheon";
vendorUrl = "http://www.hwacheon.com/";
legal = "Copyright (C) 2012-2025 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45909;

longDescription = "Hwacheon Hi-TECH 230AL post (Fanuc control) with support for mill-turn.";
...
var oFormat = createFormat({minDigitsLeft:4});
...
var yAxisMinimum = toPreciseUnit(-60.0, MM); // specifies the minimum range for the Y-axis
var yAxisMaximum = toPreciseUnit(60.0, MM); // specifies the maximum range for the Y-axis
...
*/
  case "ENABLE_C_AXIS":
    ...
    return (spindle == SPINDLE_MAIN) ? 28 : 228;
  case "DISABLE_C_AXIS":
    machineState.cAxisIsEngaged = false;
    return (spindle == SPINDLE_MAIN) ? 29 : 229;
  ...
  case "LOCK_MULTI_AXIS":
    return (spindle == SPINDLE_MAIN) ? 38 : 238;
  case "UNLOCK_MULTI_AXIS":
    return (spindle == SPINDLE_MAIN) ? 39 : 239;
  case "CLAMP_CHUCK":
...
function activateMachine(section) {
  ...
  machineConfiguration.setVendor("Hwacheon");
  machineConfiguration.setModel("Hi-TECH 230AL");
...
function onOpen() {
  ...
  writeBlock(gMotionModal.format(0), gFormat.format(40), gFormat.format(80), gFeedModeModal.format(99));
...
function onSection() {
  ...
  if (spindleChanged) {
    ...
    if (machineState.isTurningOperation || machineState.axialCenterDrilling) {
      if (spindleSpeed > getProperty("maximumSpindleSpeed", 6000)) {
...
*/
  ...
  if (currentSection.isMultiAxis()) {
    ...
    } else if (!machineState.isTurningOperation && !machineState.usePolarInterpolation && !machineState.usePolarCoordinates) {
      setWorkPlane(abc);
    }
  }
  ...
  if ((typeof isSubSpindleCycle == "function") && isSubSpindleCycle(cycleType)) {
    ...
    if (!machineState.stockTransferIsActive) {
      if (cycleType != "secondary-spindle-return" && cycleType != "secondary-spindle-pull") {
        writeBlock(gMotionModal.format(0), gFormat.format(28), subOutput.format(0)); // retract Sub Spindle
        goHome();
      }
      onCommand(COMMAND_STOP_SPINDLE);
      onCommand(COMMAND_COOLANT_OFF);
    ...
    case "secondary-spindle-pull":
      ...
      if (pullMethod.home) {
        ...
        writeBlock(gMotionModal.format(0), gFormat.format(28), subOutput.format(0), formatComment("SUB SPINDLE RETURN"));
      } else {
        writeBlock(mFormat.format(getCode("CLAMP_CHUCK", getSpindle(PART))), formatComment("CLAMP MAIN CHUCK"));
        onDwell(cycle.dwell);
...
function onCommand(command) {
  ...
  case COMMAND_START_CHIP_TRANSPORT:
    writeBlock(mFormat.format(55), formatComment("CHIP CONVEYOR START"));
    break;
  case COMMAND_STOP_CHIP_TRANSPORT:
    writeBlock(mFormat.format(57), formatComment("CHIP CONVEYOR STOP"));
    break;
...
function getG17Code() {
  return (machineState.usePolarInterpolation || !gotYAxis) ? 17 : 17;
}
