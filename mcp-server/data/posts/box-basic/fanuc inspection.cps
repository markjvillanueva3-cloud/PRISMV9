...
// variables used to import probing results via Live Connection
inspectionVariables.macroVariables = JSON.parse(JSON.stringify(settings.probing.probeAngleVariables));
inspectionVariables.macroVariables.z = "#137";
inspectionVariables.macroVariables.size = "#138";
inspectionVariables.systemVariableWCSOffset = 5200;
inspectionVariables.systemVariableWCSOffsetExt = 14000;
// >>>>> INCLUDED FROM inspection/common/fanuc live connection.cps
...
inspectionVariables.bufferAddress = 800; // the start address of the live connection buffer, must be set to 800. Do not change.
...
function liveConnectionHeader(count) {
  if (inspectionVariables.bufferAddress <= count) {
    error("The macro variables used for inspection overlap with the reserved macro variable range required for live connection (" +
      inspectionVariables.bufferAddress + "-" + (inspectionVariables.bufferAddress + 49) + ")."
    );
  }
  // initialize variables for live connection
  var prefix = inspectionVariables.localVariablePrefix;
  var addr = inspectionVariables.bufferAddress;
  inspectionVariables.liveConnectionVersion = prefix + addr; // #800
  ...
  inspectionVariables.commandID = prefix + ++addr; // #810
  ...
  inspectionVariables.probeResultsStartAddress = ++addr; // #820
  ...
  writeBlock("IF [" + inspectionVariables.liveConnectionStatus, "NE -1] GOTO" + skipNLines(2));
  writeBlock("IF [" + inspectionVariables.liveConnectionStatus, "EQ -1] GOTO" + skipNLines(3));
  ...
  writeBlock(formatComment("WAITING FOR FUSION CONNECTION"));
  ...
  for (var i = 0; i < getNumberOfSections(); ++i) {
    ...
    if (section.hasParameter("autodeskcam:operation-id")) {
      ...
      writeln(pathTypeID + " = 0"); // Path type set to 0 as this is an Information data block
      writeln(toolpathID + " = " + inspectionGetToolpathId(section));
      writeln(toolpathInfo1 + " = 0"); // notice type record
      writeln(toolpathInfo2 + " = 0 " + formatComment("tool length")); // length
      writeln(toolpathInfo3 + " = 0 " + formatComment("tool radius")); // radius
      writeln(toolpathInfo4 + " = 0");
      ...
      writeln(inspectionVariables.liveConnectionWritePointer + " = " + inspectionVariables.probeResultsBufferIndex);
    }
  }
}
...
function liveConnectionStoreResults() {
  writeBlock(inspectionVariables.commandArg8, "=", inspectionVariables.macroVariables.x); // store x position
  writeBlock(inspectionVariables.commandArg9, "=", inspectionVariables.macroVariables.size); // store x size
}
...
function liveConnectionWriteData(type) {
  ...
  case "toolpathStart":
    ...
    writeln(pathTypeID + " = 0"); // record type
    writeln(toolpathID + " = " + inspectionGetToolpathId(currentSection));
    writeln(toolpathInfo1 + " = 1"); // information type, 1 = "start" record, 2 = end record, 3 = part alignment
    writeln(toolpathInfo2 + " = #3012"); // HH-MM-SS 235,959 (max value)
    writeln(toolpathInfo3 + " = 0");
    writeln(toolpathInfo4 + " = 0");
    inspectionVariables.probeResultsBufferIndex += 1;
    break;
  case "toolpathEnd":
    writeComment("Toolpath End Live Import");
    writeln(pathTypeID + " = 0");
    writeln(toolpathID + " = " + inspectionGetToolpathId(currentSection));
    writeln(inspectionVariables.commandArg1 + " = " + inspectionGetToolpathId(currentSection)); // store toolpath ID in cmd arg 1 for part alignment
    writeln(toolpathInfo1 + " = 2");
    writeln(toolpathInfo2 + " = #3012");
    writeln(toolpathInfo3 + " = 0");
    writeln(toolpathInfo4 + " = 0");
    inspectionVariables.probeResultsBufferIndex += 1;
    break;
  case "toolpathAlignment":
    ...
    writeln(pathTypeID + " = 0");
    writeln(toolpathID + " = " + inspectionVariables.commandArg1);
    writeln(toolpathInfo1 + " = 3");
    writeln(toolpathInfo2 + " = " + inspectionVariables.liveAlignmentCapability);
    writeln(toolpathInfo3 + " = 0");
    writeln(toolpathInfo4 + " = 0");
    inspectionVariables.probeResultsBufferIndex += 1;
    break;
  case "inspectSurfacePoint":
    ...
    writeln(pathTypeID + " = 1");
    writeln(toolpathID + " = " + inspectionGetToolpathId(currentSection));
    writeln(toolpathInfo1 + " = " + cycle.pointID);
    writeln(toolpathInfo2 + " = " + inspectionVariables.xMeasured);
    writeln(toolpathInfo3 + " = " + inspectionVariables.yMeasured);
    writeln(toolpathInfo4 + " = " + inspectionVariables.zMeasured);
    inspectionVariables.probeResultsBufferIndex += 1;
    break;
  case "inspectSurfaceAlarm":
    onDwell(1);
    writeBlock("IF [" + inspectionVariables.commandID, "NE 2] GOTO" + skipNLines(2));
    writeBlock(inspectionVariables.commandID, "= -2");
    writeBlock("IF [" + inspectionVariables.commandID, "NE -2] GOTO" + skipNLines(3));
    writeBlock("IF [" + inspectionVariables.commandArg1, "NE 1] GOTO" + skipNLines(2));
    writeBlock(inspectionVariables.userMessage, "(OUT_OF_TOLERANCE)");
    writeBlock("IF [" + inspectionVariables.commandID, "NE -2] GOTO" + skipNLines(3));
    writeBlock("IF [" + inspectionVariables.commandArg1, "NE 2] GOTO" + skipNLines(2));
    writeBlock(inspectionVariables.userMessage, "(-OUT_OF_TOLERANCE-)"); // point unprojected alarm
    writeBlock("IF [" + inspectionVariables.commandID, "NE -2] GOTO" + skipNLines(3));
    writeBlock("IF [" + inspectionVariables.commandArg1, "NE 3] GOTO" + skipNLines(2));
    writeBlock(inspectionVariables.userMessage, "(PART_ALIGNMENT_ALARM)");
    writeBlock("IF [" + inspectionVariables.commandID, "NE -2] GOTO" + skipNLines(3));
    writeBlock("IF [" + inspectionVariables.commandArg1, "NE 4] GOTO" + skipNLines(2));
    writeBlock(inspectionVariables.userMessage, "(UNSPECIFIED)");
    writeBlock("IF [" + inspectionVariables.commandID, "NE -2] GOTO" + skipNLines(2));
    writeBlock(inspectionVariables.commandID, "= 0");
    writeBlock(" ");
    break;
  case "macroEnd":
    ...
    case "probing-x":
      macroType = 0.000;
      data2 = inspectionVariables.macroVariables.x;
      break;
    case "probing-y":
      macroType = 0.000;
      data3 = inspectionVariables.macroVariables.y;
      break;
    case "probing-z":
      macroType = 0.000;
      data4 = inspectionVariables.macroVariables.z;
      break;
    case "probing-x-wall":
    case "probing-x-channel":
    case "probing-x-channel-with-island":
      macroType = 0.001;
      data1 = inspectionVariables.macroVariables.x; // x position
      data2 = inspectionVariables.macroVariables.size; // x size
      break;
    case "probing-y-wall":
    case "probing-y-channel":
    case "probing-y-channel-with-island":
      macroType = 0.001;
      data3 = inspectionVariables.macroVariables.y; // y position
      data4 = inspectionVariables.macroVariables.size; // y size
      break;
    case "probing-xy-circular-boss":
    case "probing-xy-circular-hole":
    case "probing-xy-circular-partial-boss":
    ...
    case "probing-xy-circular-partial-hole-with-island":
      macroType = 0.002;
      data1 = inspectionVariables.macroVariables.size; // diameter
      data2 = inspectionVariables.macroVariables.x; // x position
      data3 = inspectionVariables.macroVariables.y; // y position
      break;
    case "probing-xy-inner-corner":
    case "probing-xy-outer-corner":
      macroType = 0.003;
      data3 = inspectionVariables.macroVariables.x; // x position
      data4 = inspectionVariables.macroVariables.y; // y position
      break;
    case "probing-x-plane-angle":
    case "probing-y-plane-angle":
      macroType = 0.005;
      data4 = inspectionVariables.macroVariables.r; // angle error
      break;
    case "probing-xy-rectangular-boss":
    case "probing-xy-rectangular-hole":
    case "probing-xy-rectangular-hole-with-island":
    ...
    writeln(pathTypeID + " = " + (2 + macroType + 0.00001));
    writeln(toolpathID + " = " + inspectionGetToolpathId(currentSection));
    writeln(toolpathInfo1 + " = " + data1);
    writeln(toolpathInfo2 + " = " + data2);
    writeln(toolpathInfo3 + " = " + data3);
    writeln(toolpathInfo4 + " = " + data4);
    inspectionVariables.probeResultsBufferIndex += 1;
    break;
  }
...
function liveConnectorInterface(type) {
  ...
  case "overwriteProtection":
    writeBlock(formatComment("WAITING FOR FUSION CONNECTION-OVERWRITE PROTECTION"));
    writeBlock(gFormat.format(53));
    writeBlock("IF [" + inspectionVariables.liveConnectionStatus, "NE -1] GOTO" + skipNLines(3));
    writeBlock("IF [" + inspectionVariables.liveConnectionReadPointer, "NE " + inspectionVariables.liveConnectionWritePointer + "] GOTO" + skipNLines(2));
    writeBlock("GOTO" + skipNLines(-3));
    break;
  case "WORKPLANE":
    var orientation = machineConfiguration.isMultiAxisConfiguration() ? machineConfiguration.getOrientation(getCurrentDirection()) : currentSection.workPlane;
    var abc = orientation.getEuler2(EULER_XYZ_S);
    writeBlock(formatComment("WAITING FOR FUSION CONNECTION WORKPLANE READ"));
    writeBlock(gFormat.format(53));
    writeBlock("IF [" + inspectionVariables.workplaneStartAddress, "EQ 0] GOTO" + skipNLines(4));
    writeBlock("IF [" + inspectionVariables.liveConnectionWPA, "NE", abcFormat.format(abc.x) + "] GOTO" + skipNLines(-2));
    writeBlock("IF [" + inspectionVariables.liveConnectionWPB, "NE", abcFormat.format(abc.y) + "] GOTO" + skipNLines(-3));
    writeBlock("IF [" + inspectionVariables.liveConnectionWPC, "NE", abcFormat.format(abc.z) + "] GOTO" + skipNLines(-4));
    writeBlock(" ");
    writeBlock("IF [" + inspectionVariables.liveConnectionWPA, "NE", abcFormat.format(abc.x) + "] GOTO" + skipNLines(4));
    writeBlock("IF [" + inspectionVariables.liveConnectionWPB, "NE", abcFormat.format(abc.y) + "] GOTO" + skipNLines(3));
    writeBlock("IF [" + inspectionVariables.liveConnectionWPC, "NE", abcFormat.format(abc.z) + "] GOTO" + skipNLines(2));
    writeBlock("GOTO" + skipNLines(2));
...
function onLiveAlignment() {
  if (!getProperty("useLiveConnection")) {
    writeBlock(inspectionVariables.userMessage, formatComment("IMPORT_THE_RESULT_FILE_FOR_PART_ALIGNMENT_FIRST"));
    writeBlock(inspectionVariables.userMessage, formatComment("THEN_REGENERATE_THE_NC_PROGRAM_OF_THE_FOLLOWING_MILLING_OPERATIONS."));
    onCommand(COMMAND_STOP);
    return;
  }
  ...
  setProperty("showSequenceNumbers", "true"); // force sequence numbers to be output
  writeBlock(formatComment("WAITING FOR WCS UPDATE"));
  writeBlock(gFormat.format(53));
  writeBlock("IF [" + inspectionVariables.commandID, "NE 2] GOTO" + skipNLines(2));
  writeBlock(inspectionVariables.commandID, "= -2");
  writeBlock("IF [" + inspectionVariables.commandID, "NE -2] GOTO" + skipNLines(4));
  writeBlock("IF [" + inspectionVariables.commandArg1, "NE 3] GOTO" + skipNLines(3));
  writeBlock(inspectionVariables.userMessage, "(PART_ALIGNMENT_ALARM)");
  ...
  writeBlock("IF [" + inspectionVariables.liveConnectionStatus, "NE 2] GOTO" + skipNLines(3));
  writeBlock("IF [" + inspectionVariables.commandArg1, "NE 3] GOTO" + skipNLines(2));
  writeBlock(mFormat.format(30)); // program end
  writeBlock("IF [" + inspectionVariables.commandID, "NE 3] GOTO " + skipNLines(-13));
  ...
  writeBlock("IF [" + inspectionVariables.commandID, "NE 3] GOTO" + skipNLines(2));
  writeBlock(inspectionVariables.commandID, "= 0");
  writeBlock(" "); // do not remove, required for GOTO functionality
}
