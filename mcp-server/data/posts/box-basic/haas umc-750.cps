...
function onSection() {
  ...
  if (getProperty("useMaximumMachineZRetract") && getProperty("safePositionMethod") == "G53" && (insertToolCall || operationNeedsSafeStart)) {
    state.retractedZ = false; // force retract to move to maximum machine retract plane after a tool change
  }
