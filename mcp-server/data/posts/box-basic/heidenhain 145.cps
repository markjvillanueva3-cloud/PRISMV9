/**
  ...
  FORKID {DFDC9502-40DC-4D97-928C-BFAF41D9B8CB}
*/

description = "Heidenhain TNC 145";
vendor = "Heidenhain";
...
longDescription = "Generic milling post for Heidenhain TNC 145.";
...
properties = {
  ...
  useCoolant: {
    title      : "Use Coolant",
    description: "Enable to output coolant codes or disable if the coolant is externally handled.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post",
...
var xyzFormat = createFormat({decimals:(unit == MM ? 3 : 4), type:FORMAT_REAL, minDigitsRight:(unit == MM ? 3 : 4), forceSign:true, separator:","});
...
var radiusCompensationTable = new Table(
  ["R0", "RL", "RR"],
  {initial:RADIUS_COMPENSATION_OFF, force:true},
...
function writeInitialPositioning(position, isRequired, codes1, codes2) {
  ...
  var feedCode = {single:"R0 F9999 M", multi:"R0 F9999"};
...
function writeDrillCycle(cycle, x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    if (cycle.clearance != undefined) {
      if (xyzFormat.getResultingValue(getCurrentPosition().z) < xyzFormat.getResultingValue(cycle.clearance)) {
        writeBlock("L", zOutput.format(cycle.clearance), radiusCompensationTable.lookup(radiusCompensation), "F9999 M");
  ...
  if (cycleExpanded || expandCurrentCycle) {
    ...
    if (spatialFormat.areDifferent(getCurrentPosition().z, cycle.clearance)) { // old heidenhain cycles do not support "2nd set-up clearance"
      if (_x || _y) {
        writeBlock("L", _x, _y, radiusCompensationTable.lookup(radiusCompensation), "F9999");
      }
      writeBlock("L", zOutput.format(cycle.clearance), radiusCompensationTable.lookup(radiusCompensation), "F9999", mFormat.format(99));
    } else {
      writeBlock("L", _x, _y, radiusCompensationTable.lookup(radiusCompensation), "F9999", mFormat.format(99));
    }
  }
}
...
function onRapid(_x, _y, _z) {
  ...
  if (x || y || z) {
    ...
    writeBlock("L", x, y, z, radiusCompensationTable.lookup(radiusCompensation), "F9999 M");
    forceFeed();
  }
}
...
function onLinear(_x, _y, _z, feed) {
  ...
  if (x || y || z) {
    ...
    writeBlock("L", x, y, z, radiusCompensationTable.lookup(radiusCompensation), _feed);
  } else if (f) {
    if (getNextRecord().isMotion()) { // try not to output feed without motion
      forceFeed(); // force feed on next line
    } else {
      pendingRadiusCompensation = -1;
      writeBlock("L", radiusCompensationTable.lookup(radiusCompensation), _feed);
    }
  }
}
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (!isHelical() && (Math.abs(getCircularSweep()) <= 2 * Math.PI * 0.9)) { // use IPA to avoid radius compensation errors
    ...
    case PLANE_XY:
      writeBlock(motionCode, xOutput.format(x), yOutput.format(y), dir, radiusCompensationTable.lookup(radiusCompensation), _feed);
      break;
    case PLANE_ZX:
      writeBlock(motionCode, xOutput.format(x), zOutput.format(z), dir, radiusCompensationTable.lookup(radiusCompensation), _feed);
      break;
    case PLANE_YZ:
      writeBlock(motionCode, yOutput.format(y), zOutput.format(z), dir, radiusCompensationTable.lookup(radiusCompensation), _feed);
      break;
    default:
      linearize(tolerance);
    }
  ...
  if (isHelical()) {
    if (getCircularPlane() == PLANE_XY) {
      writeBlock("CP IPA" + paFormat.format(sweep), zOutput.format(z), dir, _feed);
      forceModals(xOutput, yOutput);
    }
  } else {
    // IPA must have same sign as DR
    writeBlock("CP IPA" + paFormat.format(sweep), dir, _feed);
    switch (getCircularPlane()) {
    case PLANE_XY:
      forceModals(xOutput, yOutput);
      break;
...
function onCommand(command) {
  switch (command) {
  case COMMAND_STOP:
    writeBlock("STOP", mFormat.format(0));
...
function writeRetract() {
  ...
  if (retract && retract.words.length > 0) {
    for (var i in retract.words) {
      ...
      case "M91":
      case "M92":
        writeBlock("L", words, "R0 F9999", mFormat.format(retract.method == "M92" ? 92 : 91));
        break;
      default:
...
function onClose() {
  ...
  writeBlock("STOP", mFormat.format(0)); // stop program, spindle stop, coolant of
}
