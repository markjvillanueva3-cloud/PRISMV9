/**
  ...
  FORKID {11A65633-41C0-2317-AD33-7E4AF333EDEB}
*/

// >>>>> INCLUDED FROM generic_posts/haas.cps
...
var settings = {
  ...
  programNumber                 : {min:1, max:99999, reserved:[9010, 9019]}, // specifies the program number range and reserved numbers
...
function defineMachine() {
  ...
  if (false) { // note: setup your machine here
    ...
    var bAxis = createAxis({coordinate:1, table:true, axis:[0, 1, 0], range:[0 - 0.0001, 110 + 0.0001], preference:1, reset:1, tcp:useTCP});
    var cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], cyclic:false, range:[-13320, 13320], preference:0, reset:1, tcp:useTCP});
...
function onSection() {
  ...
  if (gFeedModeModal.getCurrent() == 94) {
    gFeedModeModal.format(94); // re-apply G94 to avoid undesired output for pre-NGC controls
  }
...
function writeDrillCycle(cycle, x, y, z) {
  ...
  if (forceCycle || isFirstCyclePoint()) {
    ...
    feedOutput.setFormat(useG95 ? feedPerRevFormat : feedFormat);
...
longDescription += " This postprocessor has Inspection & Live connection capabilities.";

var controlType = "Classic"; // Choose the correct control model
