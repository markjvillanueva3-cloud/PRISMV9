var modelType = "Slim3n";
description = "Makino Slim3n";
...
/**
  ...
  FORKID {BD823E6F-CBFC-4275-8757-DA8DAE6F38C2}
*/

// >>>>> INCLUDED FROM ../common/makino vertical 5 axis.cps
if (!description) {
  description = "Makino Vertical 5 Axis";
}
...
if (!longDescription) {
  longDescription = subst("Preconfigured %1 post processor with 5 axis support.", description);
}
...
var settings = {
  coolant: {
    ...
    coolants: [
      ...
      {id:COOLANT_MIST, on:399, off:9},
      {id:COOLANT_THROUGH_TOOL, on:88, off:89},
      {id:COOLANT_AIR, on:7},
      {id:COOLANT_AIR_THROUGH_TOOL},
      {id:COOLANT_SUCTION},
      {id:COOLANT_FLOOD_MIST, on:399},
...
function defineMachine() {
  if (receivedMachineConfiguration) {
    warning(localize("The postprocessor machine configuration is overwritten by the provided CAM machine configuration."));
  } else {
    var useTCP = true; //TCP is supported for D200Z,D300,D500 machines
    if (modelType == "D200Z") {
      var bAxis = createAxis({coordinate:1, table:true, axis:[0, 0.71325, -0.700909], range:[0, 180], preference:1, tcp:useTCP});
      var cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], cyclic:true, preference:0, tcp:useTCP});
      machineConfiguration = new MachineConfiguration(bAxis, cAxis);
    } else if (modelType == "D300") {
      var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-120, 120], preference:0, tcp:useTCP});
      var cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], cyclic:true, preference:0, tcp:useTCP});
      machineConfiguration = new MachineConfiguration(aAxis, cAxis);
    } else if (modelType == "D500") {
      var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-120, 30], preference:-1, tcp:useTCP});
      var cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], cyclic:true, preference:0, tcp:useTCP});
      machineConfiguration = new MachineConfiguration(aAxis, cAxis);
    } else if (modelType == "a500Z") {
      var bAxis = createAxis({coordinate:1, table:true, axis:[0, 1, 0], cyclic:true, preference:0, tcp:useTCP});
      var cAxis = createAxis({coordinate:2, table:true, axis:[0, 0.707107, 0.707107], range:[-180, 0], preference:-1, tcp:useTCP});
      machineConfiguration = new MachineConfiguration(cAxis, bAxis);
    } else if (modelType == "Slim3n") {
      useTCP = false;
      var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-110, 110], preference:-1, tcp:useTCP});
      var cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], cyclic:true, preference:0, tcp:useTCP});
      machineConfiguration = new MachineConfiguration(aAxis, cAxis);
      settings.workPlaneMethod.useTiltedWorkplane = false;
    }
...
function setSmoothing(mode) {
  ...
  if (mode) { // enable smoothing
    writeBlock(mFormat.format(250 + smoothing.level));
  }
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    if (!is3D() && machineConfiguration.isMultiAxisConfiguration()) {
      writeBlock(fourthAxisClamp.format(10), conditional(machineConfiguration.getNumberOfAxes() > 4, fifthAxisClamp.format(12))); // lock 4th + 5th axis
    }
    return;
  case COMMAND_UNLOCK_MULTI_AXIS:
    if (!is3D() && machineConfiguration.isMultiAxisConfiguration()) {
      writeBlock(fourthAxisClamp.format(11), conditional(machineConfiguration.getNumberOfAxes() > 4, fifthAxisClamp.format(13))); // unlock 4th + 5th axis
    }
    return;
