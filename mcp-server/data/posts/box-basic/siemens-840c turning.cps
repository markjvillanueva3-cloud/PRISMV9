...
function onCyclePoint(x, y, z) {
  ...
  case "thread-turning":
    ...
    if (isLastCyclePoint()) {
      ...
      var cuttingAngle = getParameter("operation:infeedAngle", 30); // Angle is not stored with tool. toDeg(tool.getTaperAngle());
      ...
      var r21 = "R21=" + xFormat.format((x - cycle.incrementalX) + (threadHeight * (id ? -1 : 1)));
      var r22 = "R22=" + spatialFormat.format(z - cycle.incrementalZ);
      ...
      var r27 = "R27=" + spatialFormat.format(chamferWidth);
      var r28 = "R28=" + spatialFormat.format(steps);
      var r29 = "R29=" + spatialFormat.format(cuttingAngle);
      var r31 = "R31=" + xFormat.format(x + (threadHeight * (id ? -1 : 1)));
      var r32 = "R32=" + spatialFormat.format(z);
      if (getProperty("cycleComments")) {
      ...
      // Siemens thread cycles do not return to start point
      var _x = xOutput.getCurrent();
      var _y = yOutput.isEnabled() ? yOutput.getCurrent() : 0;
      var _z = zOutput.getCurrent();
      forceXYZ();
      gMotionModal.reset();
