/**
  ...
  Mazak Lathe post processor configuration.
...
description = "Mazak Turning";
vendor = "Mazak";
vendorUrl = "https://www.mazak.com/";
legal = "Copyright (C) 2012-2025 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45845;
longDescription = "Generic turning post for Mazak Matrix and Smooth controls. Use Turret 0 for Positional Turret, Turret 101 for QCTP on X- Post, Turret 102 for QCTP on X+ Post, Turret 103 for Gang Tooling on X- Post, Turret 104 for Gang Tooling on X+ Tool Post.";
...
properties = {
  ...
  useParametricFeed: {
    ...
    description: "Specifies the feed value that should be output using #500 variables.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
function getCode(code) {
  ...
  case "PART_CATCHER_ON":
    return mFormat.format(48);
  case "PART_CATCHER_OFF":
    return mFormat.format(49);
  case "TAILSTOCK_ON":
...
function onOpen() {
  ...
  writeBlock(gFeedModeModal.format(99), gPlaneModal.format(18));
...
function onSection() {
  ...
  writeBlock(getCode(currentSection.feedMode == FEED_PER_REVOLUTION || isTappingCycle() ? "FEED_MODE_UNIT_REV" : "FEED_MODE_UNIT_MIN"), gPlaneModal.format(18));
...
function onCyclePoint(x, y, z) {
  ...
  if (forceCycle || isFirstCyclePoint()) {
    ...
    case "deep-drilling":
      ...
      break;
      //All Tapping Need F per Rev;
      //G84 = Float G84.2 = Rigid;
    case "tapping":
    ...
    case "right-tapping-with-chip-breaking":
      ...
      while (u > cycle.bottom) {
        ...
        var depth = u * (getSpindle(true) == SPINDLE_SECONDARY ? -1 : 1);
