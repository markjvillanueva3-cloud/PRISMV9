...
properties = {
  ...
  useG99: {
    title      : "Use G99",
    description: "Enable to override G98 with G99. Use carefully as toolpath will have different behavior!",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
function defineMachine() {
  ...
  if (getProperty("fifthAxis")) {
    var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-90, 90], tcp:useTCP});
...
function writeDrillCycle(cycle, x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    var cycleReturnMode = getProperty("useG99") ? 99 : 98;
    var forceCycle = false;
    switch (cycleType) {
    case "drilling":
      writeBlock(
        gRetractModal.format(cycleReturnMode), gCycleModal.format(81),
    ...
    case "counter-boring":
      if (P > 0) {
        writeBlock(
          gRetractModal.format(cycleReturnMode), gCycleModal.format(82),
          getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
    ...
    case "chip-breaking":
      if ((cycle.accumulatedDepth < cycle.depth) || (P > 0)) {
        ...
        writeBlock(
          gRetractModal.format(cycleReturnMode), gCycleModal.format(73),
          getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
    ...
    case "deep-drilling":
      if (P > 0) {
        ...
        writeBlock(
          gRetractModal.format(cycleReturnMode), gCycleModal.format(83),
          getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
          peckOutput.format(cycle.incrementalDepth),
    ...
    case "right-tapping":
      ...
      writeBlock(
        gRetractModal.format(cycleReturnMode), gCycleModal.format(cycleCode),
        getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
        "P" + milliFormat.format(P),
    ...
    case "fine-boring":
      writeBlock(
        gRetractModal.format(cycleReturnMode), gCycleModal.format(76),
        getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
    ...
    case "back-boring":
      ...
      writeBlock(
        gRetractModal.format(cycleReturnMode), gCycleModal.format(87),
    ...
    case "reaming":
      ...
      if (P > 0) {
        writeBlock(
          gRetractModal.format(cycleReturnMode), gCycleModal.format(89),
          getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
          "P" + milliFormat.format(P),
          feedOutput.format(F)
        );
        ...
        writeBlock(
          gRetractModal.format(cycleReturnMode), gCycleModal.format(85),
          getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
          feedOutput.format(F)
        );
      }
    ...
    case "stop-boring":
      if (P > 0) {
        ...
        writeBlock(
          gRetractModal.format(cycleReturnMode), gCycleModal.format(86),
          getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
          feedOutput.format(F)
        );
      }
    ...
    case "manual-boring":
      writeBlock(
        gRetractModal.format(cycleReturnMode), gCycleModal.format(88),
        getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
        "P" + milliFormat.format(P), // not optional
        feedOutput.format(F)
      );
