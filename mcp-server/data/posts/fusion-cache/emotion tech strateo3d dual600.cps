/**
  ...
  eMotion Tech Strateo3D DUAL600 printer post configuration.

  $Revision: 44106 85222cccf9cee3e216d9ee111aa5823d026a774f $
  $Date: 2024-01-03 15:11:55 $

  FORKID {6292D346-DE36-4232-BBC9-6AE2D4A4A2C3}
*/

description = "eMotion Tech Strateo3D DUAL600";
vendor = "eMotion Tech";
vendorUrl = "https://www.emotion-tech.com/";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Post for exporting toolpath to the eMotion Tech Strateo3D DUAL600.";
...
highFeedrate = toPreciseUnit(16980, MM);
...
properties = {
  enclosureTemperature: {
    ...
    description: "Set the temperature to use with the enclosure temperature command (M141) in the program start.",
    type       : "integer",
    value      : 0,
    range      : [0, 60],
    scope      : "post"
  ...
  filamentRunoutRight: {
    title      : "Filament runout right detector",
    description: "Specifies whether to use the filament runout detector or not, or let the IHM setting decide, for the right extruder.",
    type       : "enum",
    values     : [
      {title:"Disabled", id:"disabled"},
      {title:"Enabled", id:"enabled"},
      {title:"Handled by IHM", id:"unused"}
    ],
    value: "unused",
    scope: "post",
  },
  filamentRunoutLeft: {
    title      : "Filament runout left detector",
    description: "Specifies whether to use the filament runout detector or not, or let the IHM setting decide, for the left extruder.",
    type       : "enum",
    values     : [
      {title:"Disabled", id:"disabled"},
      {title:"Enabled", id:"enabled"},
...
var commands = {
  ...
  enclosureTemperature  : mFormat.format(141) // command to set the enclosure temperature
};
...
function onOpen() {
  setFormats(unit); // Smoothieware is supporting inches
  if (typeof writeProgramHeader == "function") {
    writeProgramHeader();
  }
  writeBlock(gFormat.format(unit == MM ? 21 : 20)); // set unit
  // homing is automatically done by the firmware at program start
}
...
function onSection() {
  ...
  if (getProperty("enclosureTemperature") > 0) {
    writeBlock(getCode(commands.enclosureTemperature), sOutput.format(getProperty("enclosureTemperature")));
    sOutput.reset();
  }
  var rightDetector = getProperty("filamentRunoutRight");
  var leftDetector = getProperty("filamentRunoutLeft");

  writeBlock(conditional(rightDetector != "unused", mFormat.format(rightDetector == "enabled" ? 406 : 405) + " S0"));
  writeBlock(conditional(leftDetector != "unused", mFormat.format(leftDetector == "enabled" ? 406 : 405) + " S1"));

  // split the first move
  feedOutput.reset();
...
function onClose() {
  ...
  gMotionModal.reset();
  writeBlock(gMotionModal.format(0), zOutput.format(1));
  writeBlock(gAbsIncModal.format(90));
  writeBlock(gFormat.format(28));
  writeBlock("M801.0");
  writeBlock(mFormat.format(84));
  writeBlock(mFormat.format(192));
