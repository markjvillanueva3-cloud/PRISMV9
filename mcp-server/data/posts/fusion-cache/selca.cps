...
wcsDefinitions = {
  useZeroOffset: false,
  wcs          : [
    {name:"Standard", format:"O", range:[1, 99]}
  ]
};
...
var pitchFormat = createFormat({decimals:6, forceDecimal:true, scale:1000});
var toolFormat = createFormat({decimals:0});
var rpmFormat = createFormat({decimals:0});
var kFormat = createFormat({decimals:3, forceDecimal:true}); // seconds - range 0.001-99999.999
var taperFormat = createFormat({decimals:1, scale:DEG});
var angularFormat = createFormat({decimals:1, scale:DEG});  // for spindle orientation
  ...
  if (isFirstCyclePoint()) {
    ...
    case "fine-boring":
      ...
      writeBlock(
        ...
        getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
        conditional(K > 0, "K" + kFormat.format(K)), // optional
        "D0=" + angularFormat.format(cycle.shiftOrientation),
        "D7=" + xyzFormat.format(cycle.shift),
        feedOutput.format(F)
      );
      break;
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      ...
      case RADIUS_COMPENSATION_LEFT:
        writeBlock(gFormat.format(49), "K" + d);
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      ...
      if (isHelical()) {
        forceXYZ();
        writeBlock(gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), iOutput.format(cx), jOutput.format(cy), zOutput.format(z), feedOutput.format(feed));
      } else {
