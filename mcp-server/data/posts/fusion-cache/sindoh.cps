/**
  ...
  Sindoh post processor configuration.

  $Revision: 44106 85222cccf9cee3e216d9ee111aa5823d026a774f $
  $Date: 2024-01-03 15:11:55 $

  FORKID {6626730B-E586-4CA7-ABE9-B266DFD1D8E3}
*/

description = "Sindoh";
vendor = "Sindoh";
vendorUrl = "https://3dprinter.sindoh.com/";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Post for exporting toolpath to the Sindoh range of printers in gcode format (3DWOX 1, 1X, 2X, 7X DP201, DP203.";
...
properties = {
  calibration: {
    title      : "Calibration",
    description: "Output calibration command at start. (M299)",
    type       : "boolean",
    value      : "false",
    scope      : "machine"
  }
...
var ignoreMove;
function onSection() {
  // storing the last position for filtering in onLinear, onRapid
  lastPosition = currentSection.getFinalPosition();
  ignoreMoves = false;
  if (getProperty("calibration")) {
    writeBlock(mFormat.format(299), formatComment(localize("calibration")));
  }
  writeBlock(gFormat.format(200), formatComment(localize("nozzle cleaning")));
  ...
  writeBlock(mFormat.format(298), formatComment(localize("move nozzle to the build plate")));
  eOutput.reset();
  writeBlock(gFormat.format(92), eOutput.format(0));
  forceXYZE();
  var initialPosition = getFramePosition(currentSection.getInitialPosition());
  // split the first move, z then xy.
...
function onClose() {
  writeBlock(mFormat.format(117), localize(" Print Completed"));
