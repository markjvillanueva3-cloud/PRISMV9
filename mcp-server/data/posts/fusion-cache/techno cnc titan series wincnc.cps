/**
  ...
  Techno CNC Titan Series post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {12D8357F-C510-4B53-8BB4-986E7227727E}
*/

description = "Techno CNC Systems Titan Series CNC Router with a WinCNC controller";
vendor = "Techno CNC";
...
longDescription = "Generic post for Techno CNC Systems Titan Series CNC Router with an optional B-axis and WinCNC controller.";
...
function defineMachine() {
  ...
  if (masterAxis >= 0) { // note: setup your machine here
    ...
    var aAxis = createAxis({coordinate:1, table:true, axis:rotaryVector, range:[0, 360], cyclic:true, preference:0, tcp:useTCP});
...
/*
  Writes a blank line (with sequence number)
*/
function writeBlankLine() {
  if (getProperty("showSequenceNumbers") == "true") {
    writeln("N" + sequenceNumber);
    ...
    if (pendingRadiusCompensation >= 0) {
      ...
      case RADIUS_COMPENSATION_LEFT:
        writeBlock(gMotionModal.format(1), gFormat.format(41),
          x, y, z, "O" + xyzFormat.format(tool.diameter / 2), f);
        break;
      case RADIUS_COMPENSATION_RIGHT:
        writeBlock(gMotionModal.format(1), gFormat.format(42),
          x, y, z, "O" + xyzFormat.format(tool.diameter / 2), f);
        break;
      default:
