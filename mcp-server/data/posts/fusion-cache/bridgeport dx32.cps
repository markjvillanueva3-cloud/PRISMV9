...
var gFeedModeModal = createOutputVariable({}, gFormat); // modal group 4 // G94-95
var gUnitModal = createOutputVariable({}, gFormat); // modal group 5 // G70-71
var gCycleModal = createOutputVariable({}, gFormat); // modal group 6 // G81, ...
var gDecelModal = createOutputVariable({}, gFormat); // modal group 7 // G8-9
...
var settings = {
  ...
  maximumSequenceNumber       : 10000, // the maximum sequence number (Nxxx), use 'undefined' for unlimited
...
function onOpen() {
  ...
  if (getProperty("writeHeader")) {
    ...
    writeComment(getGlobalParameter("generated-by", ""));
    ...
    writeComment(getGlobalParameter("username", ""));
...
function onSection() {
  ...
  if (getProperty("showSequenceNumbers") == "operation") {
    setProperty("showSequenceNumbers", "true");
    writeBlock(formatComment(getParameter("operation-comment", "")));
    setProperty("showSequenceNumbers", "operation");
  } else {
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    if (getProperty("useM26")) {
      gMotionModal.reset();
      writeToolBlock(gMotionModal.format(0), "T" + toolFormat.format(tool.number), mFormat.format(26));
    ...
    if (getProperty("useToolCall") && getProperty("preloadTool") && preloadTool) {
...
function writeRetract() {
  // dummy function, Bridgeport does not support retracts with ie G28/G53
}
