/**
  ...
  FORKID {7E507705-C566-400E-9ACD-FF3C88277E77}
*/

description = "STEPCRAFT WinPC-NC";
vendor = "WinPC-NC";
...
longDescription = "Generic post for STEPCRAFT machines using WinPC-NC control software. If you are using a fourth axis, make sure to select which axis it is mounted along. If you have an automatic tool changer, enable the property 'Use tool changer'. If you need the feedrates in mm/s, please set the property 'Feedrate output per second' to true. For laser cutting, set your desired power setting by using property 'Laser power percentage'.";
...
var settings = {
  ...
  supportsInverseTimeFeed     : false // default: true, specifies if inverse time feedrates are supported by the postprocessor
};
...
function onOpen() {
  zFormat = createFormat({decimals:(unit == MM ? 3 : 4), forceDecimal:true, scale:getProperty("reverseZAxis") ? -1 : 1});
  zOutput = createOutputVariable({onchange:function() {state.retractedZ = false;}, prefix:"Z"}, zFormat);
  ...
  if (getProperty("feedUnit")) {
    ...
    feedOutput.setScale(1 / 60); // mm/min
  }
...
function onSection() {
  ...
  var isRequired = !jetMode && (insertToolCall || state.retractedZ || (!isFirstSection() && getPreviousSection().isMultiAxis()));
...
function getCommonCycle(x, y, z, r) {
  ...
  return [xOutput.format(x), yOutput.format(y),
    zOutput.format(z),
    (r !== undefined) ? "R" + zFormat.format(r) : ""];
}
    ...
    if (pendingRadiusCompensation >= 0) {
      ...
      case RADIUS_COMPENSATION_LEFT:
        writeBlock(
          ...
          conditional(getProperty("useToolCall"), "O" + xyzFormat.format(tool.diameter / 2)), f
        );
        break;
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    if (getProperty("useToolCall")) {
      ...
      writeBlock((!isFirstSection() ? mFormat.format(0) : ""), formatComment(localize("CHANGE TOOL")));
      writeToolBlock("T" + toolFormat.format(tool.number), mFormat.format(6) + " (" + getToolTypeName(tool.type).toUpperCase() + " D=" + xyzFormat.format(tool.diameter) + (tool.description ? " " + tool.description.toUpperCase() : "") + ")");
    }
