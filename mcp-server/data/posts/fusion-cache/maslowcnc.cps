/**
  ...
  MaslowCNC post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {695FCA46-F545-4E34-8856-D8F81BBD96A8}
*/

description = "MaslowCNC";
vendor = "MaslowCNC";
vendorUrl = "http://www.maslowcnc.com/";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic post for MaslowCNC.";
...
properties = {
  ...
  useWorkOffsets: {
    ...
    description: "Specifies to output work offset code (eg. G54).",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    if (getProperty("safePositionMethod") == "G28" && isFirstSection()) { // avoid initial retract at program start since no tool is loaded at this point
...
function onCommand(command) {
  ...
  case COMMAND_START_SPINDLE:
    if (getProperty("useActiveSpindle")) {
      forceSpindleSpeed = false;
      writeToolBlock(mFormat.format(tool.clockwise ? 3 : 4), sOutput.format(spindleSpeed));
    } else {
      writeComment(mFormat.format(tool.clockwise ? 3 : 4) + " " + sOutput.format(spindleSpeed));
      wait = true;
    }
    return;
  case COMMAND_LOAD_TOOL:
    writeToolBlock("T" + toolFormat.format(tool.number), getProperty("useM06") ? mFormat.format(6) : "");
