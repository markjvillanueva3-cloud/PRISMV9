...
var threadPQFormat = createFormat({decimals:4, type:FORMAT_LZS, minDigitsLeft:0, minDigitsRight:1});
...
var barOutput = createOutputVariable({prefix:"A", control:CONTROL_FORCE}, spatialFormat);
...
function activateMachine(section) {
  ...
  if (section.getSpindle() == SPINDLE_PRIMARY) {
    ...
    cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], preference:0, tcp:operationSupportsTCP});
  } else {
  ...
  machineConfiguration.setVendor("Takisawa");
  machineConfiguration.setModel("TS-4000YS");
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    if (getProperty("useTailStock") && !machineState.axialCenterDrilling) {
      if (getSpindle(PART) == "SPINDLE_MAIN" && (getSpindleAxis() != POSZ)) {
...
function onLinear(_x, _y, _z, feed) {
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      pendingRadiusCompensation = -1;
      getCompensationPlane(getCurrentDirection(), false, true);
      switch (radiusCompensation) {
      case RADIUS_COMPENSATION_LEFT:
        writeBlock(
          gMotionModal.format(isSpeedFeedSynchronizationActive() ? 32 : 1),
          gFormat.format((getSpindle(PART) == SPINDLE_MAIN) ? 41 : 42),
          x, y, z, getFeed(feed)
        );
        break;
      case RADIUS_COMPENSATION_RIGHT:
        writeBlock(
          gMotionModal.format(isSpeedFeedSynchronizationActive() ? 32 : 1),
          gFormat.format((getSpindle(PART) == SPINDLE_MAIN) ? 42 : 41),
          x, y, z, getFeed(feed)
        );
        break;
      default:
...
function onCycle() {
  if ((typeof isSubSpindleCycle == "function") && isSubSpindleCycle(cycleType)) {
    ...
    if (!machineState.stockTransferIsActive) {
      ...
      writeBlock(
        feedMode, gPlaneModal.format(18), cAxisEngageModal.format(getCode("DISABLE_C_AXIS", getSpindle(PART))),
    ...
    case "secondary-spindle-grab":
      ...
      if (cycle.stopSpindle) { // no spindle rotation
        ...
        cAxisEngageModal.reset();
        writeBlock(cAxisEngageModal.format(getCode("ENABLE_C_AXIS", getSpindle(PART))), formatComment("C-AXIS MODE"));
        ...
        writeBlock(mFormat.format(5), "S0", pOutput.format(getCode("SELECT_SPINDLE", getSpindle(PART))),
        ...
        var spindleDir = mFormat.format(tool.clockwise ? getCode("START_SPINDLE_CW", getSpindle(TOOL)) : getCode("START_SPINDLE_CCW", getSpindle(TOOL)));
        writeBlock(
          gSpindleModeModal.format(97),
          sOutput.format(100),
          spindleDir,
          pOutput.format(getCode("SELECT_SPINDLE", getSpindle(TOOL)))
        );
        ...
        lastSpindleDirection = spindleDir;
      }
      // approach part
      gMotionModal.reset();
    ...
    case "secondary-spindle-pull":
      ...
      if (pullMethod.pull) {
        ...
        onDwell(cycle.dwell);
        writeBlock(
          conditional(chuckMachineFrame == 1, gFormat.format(53)),
          gMotionModal.format(1),
      ...
      if (pullMethod.home) {
        ...
        writeBlock(mFormat.format(getCode("INTERLOCK_BYPASS_OFF", getSpindle(PART))), formatComment("INTERLOCK BYPASS OFF"));
        writeBlock(gFeedModeModal.format(99));
        if (transferType == TRANSFER_SPEED) {
          writeBlock(mFormat.format(getCode("SPINDLE_SYNCHRONIZATION_SPEED_OFF", getSpindle(PART))), formatComment("SYNCRHONIZATION OFF"));
          writeBlock(mFormat.format(61), formatComment("PARKING OFF"));
        } else if (transferType == TRANSFER_PHASE) {
          writeBlock(mFormat.format(getCode("SPINDLE_SYNCHRONIZATION_PHASE_OFF", getSpindle(PART))), formatComment("SYNCRHONIZATION OFF"));
        }
        machineState.stockTransferIsActive = false;
      } else {
        writeBlock(mFormat.format(getCode("CLAMP_CHUCK", getSpindle(PART))), formatComment("CLAMP MAIN CHUCK"));
...
function ejectPart() {
  ...
  writeBlock(
    ...
    pOutput.format(getCode("SELECT_SPINDLE", getSpindle(PART)))
  );
  ...
  writeBlock(mFormat.format(getCode("STOP_SPINDLE", getSpindle(PART))), pOutput.format(getCode("SELECT_SPINDLE", getSpindle(PART))));
  setCoolant(COOLANT_OFF);
...
function onClose() {
  ...
  if (getSpindle(PART) == SPINDLE_SUB && previousSpindle == SPINDLE_LIVE) {
    writeBlock(mFormat.format(getCode("SPINDLE_SYNCHRONIZATION_SPEED_OFF")), mFormat.format(61));
    writeBlock(mFormat.format(getCode("DISABLE_C_AXIS", SPINDLE_MAIN)), cAxisEngageModal.format(getCode("DISABLE_C_AXIS", SPINDLE_SUB)));
  }
  writeBlock(cAxisEngageModal.format(getCode("DISABLE_C_AXIS", getSpindle(PART))));
