/**
  ...
  Prusa post processor configuration.
...
highFeedrate = toPreciseUnit(12000, MM);
...
// <<<<< INCLUDED FROM ../common/propertyRelativeExtrusion.cpi
// >>>>> INCLUDED FROM ../common/propertyPause.cpi
properties.pauseLayers = {
  title      : "Layer number(s) pause print",
  description: "Comma separated list containing the layer numbers to pause printing.",
  type       : "string",
  value      : "",
  scope      : "post",
  group      : "pauseFeature"
};
properties.pauseMessage = {
  title      : "Message for pause print",
  description: "The message displayed when pausing the print to inform the user.",
  type       : "string",
  value      : "",
  scope      : "post",
  group      : "pauseFeature"
...
groupDefinitions.pauseFeature = {
  title      : "Pause Print",
  description: "Pauses the print at the start of the user defined layer(s) and displays a message. Used for pausing the print to insert / embed objects to the part during printing.",
  collapsed  : true,
  order      : 1
};
// <<<<< INCLUDED FROM ../common/propertyPause.cpi
// >>>>> INCLUDED FROM ../common/propertyChangeFilament.cpi
properties.changeLayers = {
  title      : "Layer number(s) filament change",
  description: "Comma separated list containing the layer numbers to stop printing and changing the filament.",
  type       : "string",
  value      : "",
  scope      : "post",
  group      : "changeFilament"
};
properties.changeMessage = {
  title      : "Message for filament change",
  description: "Message displayed when pausing the print to inform the user.",
  type       : "string",
  value      : "",
  scope      : "post",
  group      : "changeFilament"
...
/*
properties.useFirmwareConfiguration = {
  title      : "Use Firmware configuration",
  description: "Specifies to use the firmware default options for a filament change.",
  type       : "boolean",
  value      : true,
  scope      : "post",
  group      : "changeFilament"
};
properties.initialRetract = {
  title      : "Extruder initial retract",
  description: "Specifies the initial extruder retract.",
  type       : "number",
  value      : 0,
  scope      : "post",
  group      : "changeFilament"
};
properties.removalRetract = {
  title      : "Extruder removal retract",
  description: "Specifies the retract distance for removing the filament.",
  type       : "number",
  value      : 0,
  scope      : "post",
  group      : "changeFilament"
};
properties.zPosition = {
  title      : "Z position relative",
  description: "Z relative lift for the filament change.",
  type       : "number",
  value      : 0,
  scope      : "post",
  group      : "changeFilament"
...
groupDefinitions.changeFilament = {
  title      : "Filament Change",
  description: "Pauses the print at the start of the user defined layer(s) and displays a message. Used for switching to a different color filament of the same material.",
  collapsed  : true,
  order      : 2
};
// <<<<< INCLUDED FROM ../common/propertyChangeFilament.cpi
...
var commands = {
  ...
  pauseCommand          : mFormat.format(601), // command to pause the print, waiting for user manual restart
  changeFilament        : {
    command: mFormat.format(600)
    // initialRetract: "E",
    // removalRetract: "L"
  }
};
...
function onOpen() {
  ...
  writeBlock("M115 U3.0.10", formatComment("tell printer latest fw version"));
  if (getProperty("printerModel") == "i3mk2mk3") {
    writeBlock(gFormat.format(28), "W", formatComment("home all without mesh bed level"));
  }  else if (getProperty("printerModel") == "mini") {
    writeBlock(gFormat.format(28), formatComment("home all without mesh bed level"));
  }
}
...
function onSection() {
  ...
  if (getProperty("printerModel") == "i3mk2mk3") {
    writeBlock(gFormat.format(80), formatComment("mesh bed leveling"));
  }  else if (getProperty("printerModel") == "mini") {
    writeBlock(gFormat.format(29), formatComment("mesh bed leveling"));
  }
...
function onClose() {
  ...
  yOutput.reset();
  if (getProperty("printerModel") == "i3mk2mk3") {
    writeBlock(gMotionModal.format(1), xOutput.format(0), yOutput.format(toPreciseUnit(200, MM)), formatComment("home X axis"));
  } else if (getProperty("printerModel") == "mini") {
    writeBlock(gMotionModal.format(1), xOutput.format(0), yOutput.format(toPreciseUnit(150, MM)), formatComment("home X axis"));
  }
  writeBlock(mFormat.format(84), formatComment("disable motors"));
}
