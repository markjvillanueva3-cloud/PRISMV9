/**
  ...
  BRESSER post processor configuration.

  $Revision: 44106 85222cccf9cee3e216d9ee111aa5823d026a774f $
  $Date: 2024-01-03 15:11:55 $

  FORKID {35875924-78F9-4BF9-A73D-25D50699D4C9}
*/

description = "Bresser";
vendor = "BRESSER";
vendorUrl = "https://www.bresser.de/en/Weather-Time/3D-Printer/BRESSER-WIFI-3D-Printer.html";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Post for exporting toolpath to the BRESSER range of printers in gcode format";
...
var commands = {
  extruderChangeCommand : mFormat.format(108), // command to change the extruder
...
function onSection() {
  ...
  writeBlock(mFormat.format(132), "X Y Z A B"); // read home position from eeprom

  // lower build plate
  var initialPosition = getFramePosition(currentSection.getInitialPosition());
  writeBlock(gMotionModal.format(1), zOutput.format(initialPosition.z), feedOutput.format(highFeedrate));
  writeBlock(gFormat.format(161), "X Y", feedOutput.format(highFeedrate));
  writeBlock(mFormat.format(651));

  // apparently A100 for ABS, 80 for PLA 20 for idle
  // Check the material name for changing the A value
  var aValue = 100;
  var materialName = "";

  materialName = getExtruder(activeExtruder + 1).materialName;
  materialName = materialName.substr(0, 3).toUpperCase();
  if (materialName == "PLA") {
    aValue = 80;
  }
  writeBlock(mFormat.format(907), xOutput.format(100), yOutput.format(100), zOutput.format(40), "A" + integerFormat.format(aValue), "B" + integerFormat.format(20));
...
function onClose() {
  writeBlock(gFormat.format(162), "Z", feedOutput.format(toPreciseUnit(1800, MM)));
  writeBlock(gFormat.format(28), "X Y");
  writeBlock(mFormat.format(132), "X Y A B");
  writeBlock(mFormat.format(652));
...
function onExtruderTemp(temp, wait, id) {
  ...
  if (wait) {
    writeBlock(mFormat.format(6), tFormat.format(id));
  } else {
    writeBlock(getCode(commands.setExtruderTemperature), sOutput.format(temp), tFormat.format(id));
  }
}

// include("../common/onBedTemp.cpi");  // printer specific function used instead
// include("../common/onExtruderTemp.cpi");  // printer specific function used instead
