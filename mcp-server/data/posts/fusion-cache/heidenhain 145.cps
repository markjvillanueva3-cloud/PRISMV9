...
var xyzFormat = createFormat({decimals:(unit == MM ? 3 : 4), forceDecimal:true, forceSign:true, trim:false, separator:","});
...
var mFormat = createFormat({prefix:"M", decimals:0, width:2, trim:false, zeropad:true});
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    retracted = true;
    writeBlock("STOP " + mFormat.format(0)); // stop program, spindle stop, coolant off
  ...
  if (!retracted && !insertToolCall) {
    if (getCurrentPosition().z < initialPosition.z) {
      writeBlock("L" + zOutput.format(initialPosition.z) + " F9999" + " M");
    }
  }

  if (!machineConfiguration.isHeadConfiguration()) {
    writeBlock("L" + xOutput.format(initialPosition.x) + yOutput.format(initialPosition.y) + " R0 F9999 " + mFormat.format(13));
    z = zOutput.format(initialPosition.z);
    if (z) {
      writeBlock("L" + z + " R0 F9999 M");
    }
  } else {
    writeBlock("L" + xOutput.format(initialPosition.x) + yOutput.format(initialPosition.y) + zOutput.format(initialPosition.z) + " R0 F9999 " + mFormat.format(13));
  }
...
function onCycle() {
  ...
  if (pendingRadiusCompensation >= 0) {
    error(localize("Radius compensation cannot be activated/deactivated for cycle."));
    return;
  }
...
function onCyclePoint(x, y, z) {
  if (!expandCurrentCycle) {
    ...
    if (spatialFormat.areDifferent(getCurrentPosition().z, cycle.clearance)) { // old heidenhain cycles do not support "2nd set-up clearance"
      if (xy) {
        writeBlock("L" + xy + radiusCompensationTable.lookup(radiusCompensation) + " F9999");
      }
      writeBlock("L" + zOutput.format(cycle.clearance) + radiusCompensationTable.lookup(radiusCompensation) + " F9999 " + mFormat.format(99));
    } else {
      writeBlock("L" + xy + radiusCompensationTable.lookup(radiusCompensation) + " F9999 " + mFormat.format(99));
    }
  } else {
...
function onRapid(x, y, z) {
  ...
  if (xyz) {
    ...
    writeBlock("L" + xyz + radiusCompensationTable.lookup(radiusCompensation) + " F9999" + " M");
  }
  forceFeed();
}
...
function onLinear(x, y, z, feed) {
  ...
  if (xyz) {
    pendingRadiusCompensation = -1;
    writeBlock("L" + xyz + radiusCompensationTable.lookup(radiusCompensation) + f + " M");
  } else if (f) {
    if (getNextRecord().isMotion()) { // try not to output feed without motion
      forceFeed(); // force feed on next line
    } else {
      pendingRadiusCompensation = -1;
      writeBlock("L" + radiusCompensationTable.lookup(radiusCompensation) + f + " M");
    }
  }
}
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isHelical()) {
    if (getCircularPlane() == PLANE_XY) {
      ...
      var block = "CP" + " PR" + xyzFormat.format(r) + " IPA" + paFormat.format(sweep) + zOutput.format(z);
    ...
    var r = getCircularRadius();
    var sweep = (clockwise ? -1 : 1) * Math.abs(getCircularSweep());
    var block = "CP" + " PR" + xyzFormat.format(r) + " IPA" + paFormat.format(sweep);
    block += clockwise ? " DR-" : " DR+";
    block += /*radiusCompensationTable.lookup(radiusCompensation) +*/ getFeed(feed);
    block += " M";
    writeBlock(block);
...
function writeRetract() {
  ...
  */
  ...
  if (words.length > 0) {
    ...
    case "M91":
    case "M92":
      writeBlock("L " + words.join(getWordSeparator()) + SP + "R0 F9999 " +  mFormat.format(method == "M92" ? 92 : 91));
      break;
    default:
