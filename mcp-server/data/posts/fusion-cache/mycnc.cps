/**
  ...
  myCNC post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {529C3C79-CF04-481E-9D10-D781663A086F}
*/

description = "myCNC Mill Controller";
vendor = "Puruvesi Automation Inc";
vendorUrl = "http://www.pv-automation.com";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic milling post for myCNC by Puruvesi Automation.";
...
properties = {
  ...
  useTrunnion: {
    title      : "Use BC-trunnion",
    description: "Enables a trunnion table with a B and C-axis.",
    group      : "configuration",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  useTCP: {
    title      : "TCP is supported",
    description: "Specifies whether to use TCP on the control.",
    group      : "multiAxis",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
var settings = {
  ...
  subprograms: {
    initialSubprogramNumber: 1001, // specifies the initial number to be used for subprograms. 'undefined' uses the main program number
...
function defineMachine() {
  ...
  if (receivedMachineConfiguration) {
    if (machineConfiguration.isMultiAxisConfiguration() && machineConfiguration.getMultiAxisFeedrateMode() == FEED_INVERSE_TIME) {
      error(localize("Inverse time feedrates are not supported by the controller."));
      return;
    }
  }
  if (getProperty("rotaryTableAxis") != "none" || getProperty("useTrunnion")) {
    if (receivedMachineConfiguration) {
      error(localize("You can only select either a machine in the CAM setup or use the properties to define your kinematics."));
      return;
    }
  }
  if (getProperty("useTrunnion")) { // 5-axis trunnion
    var bAxis = createAxis({coordinate:1, table:true, axis:[0, 1, 0], range:[-35, 110], preference:1, tcp:useTCP});
    var cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], cyclic:true, tcp:useTCP});
    machineConfiguration = new MachineConfiguration(bAxis, cAxis);
    setMachineConfiguration(machineConfiguration);
  } else if (getProperty("rotaryTableAxis") != "none") { // 4-axis rotary table
    var rotary = parseChoice(getProperty("rotaryTableAxis"), "-Z", "-Y", "-X", "NONE", "X", "Y", "Z");
    if (rotary < 0) {
      error(localize("Valid 'Rotary table axis' values are: None, X, Y, Z, -X, -Y, -Z"));
      return;
    }
    rotary -= 3;
...
function onSection() {
  ...
  settings.outputToolLengthOffset =  !tcp.isSupportedByOperation;
  if (isRequired) {
    disableLengthCompensation(false);
    if (tcp.isSupportedByOperation) {
      writeBlock(mFormat.format(102), hFormat.format(tool.lengthOffset), formatComment("TCP ON"));
    }
  }
...
function getOffsetCode() {
  var offsetCode = 43;  // tool length compensation only, TCP is turned on/off external
  return offsetCode;
}
...
function onCyclePoint(x, y, z) {
  if (currentSection.isOptimizedForMachine()) {
    forward = machineConfiguration.getOptimizedDirection(currentSection.workPlane.forward, getCurrentABC(), false, false);
  } else {
...
function onMoveToSafeRetractPosition() {
  ...
  if (tcp.isSupportedByOperation) {
    writeBlock(mFormat.format(101), formatComment("TCP OFF"));
  }
  writeRetract(Z);
}
