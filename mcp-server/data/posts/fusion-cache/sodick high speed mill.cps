/**
  ...
  Sodick post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {86ED64AD-BF10-4555-9A1B-8806ABF2CCD8}
*/

description = "Sodick High Speed Milling";
vendor = "Sodick";
vendorUrl = "http://www.sodick.com";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic post for Sodick High Speed Mills, such as the UH430L and UG650L.";
...
properties = {
  ...
  material: {
    title      : "Type of material to mill",
    description: "Set the material type to 'Hard', 'Graphite', or 'Standard'. 'Standard' disables control of the dust and mist collectors.",
    group      : "preferences",
    type       : "enum",
    values     : [
      {title:"Hard", id:"hard"},
      {title:"Graphite", id:"graphite"},
...
// fixed settings
var minSpeedChange = 5000; // ramp up/down spindle speed
...
function writeToolMeasureBlock(tool) {
  ...
  case TOOL_MILLING_END_BALL:
    var tipDiameter = tool.diameter - (tool.cornerRadius * 2);
    var height = tool.cornerRadius;
    var width = (tipDiameter / 2) - (tipDiameter / 2 * 0.075);
    writeBlock(
      gFormat.format(65),
      "P9603",
      hFormat.format(tool.lengthOffset),
      dFormat.format(tool.diameterOffset),
      "Z" + xyzFormat.format(height),
      "X" + xyzFormat.format(width),
      formatComment(subst("LASER MEASUREMENT FOR TOOL %1", tool.number))
    );
    break;
  default:
    writeBlock(
...
function toolBreakControl(tool) {
  writeBlock(mFormat.format(66), formatComment("AIR BLOW ON"));
  onDwell(10);
  writeBlock(mFormat.format(67), formatComment("AIR BLOW OFF"));
  writeBlock(
    gFormat.format(65),
    "P" + 9608,
    "H" + toolFormat.format(tool.lengthOffset),
    "Q" + xyzFormat.format(getProperty("toolBreakageTolerance")),
    formatComment(subst("TOOL BREAKAGE CHECK FOR TOOL %1", tool.number))
  );
...
function onOpen() {
  ...
  writeComment(subst(localize("=== This program is programmed in %1. ==="), unit == IN ? "Inches" : "Millimeters"));
  writeComment(localize("=== Make sure the units on the control are set the same. ==="));
  writeProgramHeader();

  // absolute coordinates and feed per min
  writeBlock(gPlaneModal.format(17), gFormat.format(40), gAbsIncModal.format(90), gFormat.format(49), gFormat.format(80));
...
function enableLengthCompensation(force) {
  if (!state.lengthCompensationActive || force) {
...
function setSmoothing(mode) {
  ...
  if (mode) {
    writeBlock(gFormat.format(5), "P10000", "Q301");
  } else {
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane || smoothing.cancel || breakControl ||
      (!isFirstSection() && isProbeOperation(getPreviousSection()))) {
    if ((insertToolCall && !isFirstSection()) || smoothing.cancel || breakControl) {
      setSmoothing(false);
      if (getProperty("material") != "graphite") {
    ...
    if (isFirstSection()) {
      if (getProperty("material") == "hard") {
        writeBlock(mFormat.format(64), formatComment("MIST COLLECTOR ON"));
      } else if (getProperty("material") == "graphite") {
        writeBlock(mFormat.format(65), formatComment("MIST COLLECTOR OFF"));
      }
    }

    // tool break control
    breakControl = breakControl ? true :
      (insertToolCall && !isFirstSection() && getPreviousSection().getTool().breakControl);
    if (breakControl) {
      toolBreakControl(getPreviousSection().getTool());
      forceSpindleSpeed = true;
    // stop spindle during tool change
    } else if (insertToolCall && !isFirstSection()) {
      onCommand(COMMAND_STOP_SPINDLE);
    }
  ...
  if (insertToolCall && !isProbeOperation()) {
    writeToolMeasureBlock(tool); // measure tool
  }
  ...
  } else { // turn on dust collector
    if (isFirstSection()) {
      writeBlock(mFormat.format(68), formatComment("DUST COLLECTOR ON"));
    }
    writeBlock(mFormat.format(66), formatComment("AIR BLOW ON"));
  }
...
function onDwell(seconds) {
  ...
  writeBlock(gFormat.format(4), "X" + xyzFormat.format(seconds));
  xOutput.reset();
}
...
function printProbeResults() {
  return currentSection.getParameter("printResults", 0);
}
...
function protectedProbeMove(_cycle, x, y, z) {
  ...
  if (_z && z >= getCurrentPosition().z) {
    writeBlock(gMotionModal.format(1), _z, getFeed(_cycle.feedrate)); // protected positioning move
  }
  if (_x || _y) {
    writeBlock(gMotionModal.format(1), _x, _y, getFeed(highFeedrate)); // protected positioning move
  }
  if (_z && z < getCurrentPosition().z) {
    writeBlock(gMotionModal.format(1), _z, getFeed(_cycle.feedrate)); // protected positioning move
  }
...
function writeProbeCycle(cycle, x, y, z) {
  ...
  var F = cycle.feedrate;
  var minProbeDistance = toPreciseUnit(0.003, IN); // minimum feed step distance for probing

  switch (cycleType) {
  case "probing-x":
    protectedProbeMove(cycle, x, y, z - cycle.depth);
    writeBlock(
      gFormat.format(65), "P" + 9149,
      "X" + approach(cycle.approach1), // direction of probe movement
      "T" + printProbeResults(), // T0 = "0 set" is made, T1 = "0 set" is not made (origin)
      "I" + xyzFormat.format(cycle.probeClearance), // return distance
      feedOutput.format(F),
      "Q" + xyzFormat.format(minProbeDistance) // step feed distance from start point
    );
    enableLengthCompensation(false);
    break;
  case "probing-y":
    protectedProbeMove(cycle, x, y, z - cycle.depth);
    writeBlock(
      gFormat.format(65), "P" + 9149,
      "Y" + approach(cycle.approach1), // direction of probe movement
      "T" + printProbeResults(), // T0 = "0 set" is made, T1 = "0 set" is not made (origin)
      "I" + xyzFormat.format(cycle.probeClearance), // return distance
      feedOutput.format(F),
      "Q" + xyzFormat.format(minProbeDistance) // step feed distance from start point
  ...
  case "probing-z":
    ...
    if (cycle.outOfPositionAction != undefined) { // measure tool sensor length
      writeBlock(
        gFormat.format(65), "P" + 9141,
        "X" + xyzFormat.format(x), // x-position
        "Y" + xyzFormat.format(y), // y-position
        "Z" + xyzFormat.format(z - cycle.depth + cycle.probeClearance), // initial z-position
        "R" + xyzFormat.format(z - cycle.depth), // top of part
        "Q" + xyzFormat.format(minProbeDistance), // step feed distance from start point
        feedOutput.format(F)
      );
      enableLengthCompensation(false);
      ...
      writeBlock(
        gFormat.format(65), "P" + 9149,
        "Z" + approach(cycle.approach1), // direction of probe movement
        "T" + printProbeResults(), // T0 = "0 set" is made, T1 = "0 set" is not made (origin)
        "I" + xyzFormat.format(cycle.probeClearance), // return distance
        feedOutput.format(F),
        "Q" + xyzFormat.format(minProbeDistance) // step feed distance from start point
  ...
  case "probing-x-wall":
    ...
    writeBlock(
      gFormat.format(65), "P" + 9146,
      "X" + xyzFormat.format(cycle.probeClearance + (cycle.width1 / 2) + (tool.diameter / 2)), // start along the X-axis
      "Y" + xyzFormat.format(0), // start along the Y-axis
      "Z" + xyzFormat.format(cycle.depth), // probing depth from current position
      "T" + printProbeResults(), // T0 = "0 set" is made, T1 = "0 set" is not made (origin)
      "I" + xyzFormat.format(cycle.probeClearance), // return distance
      "K" + xyzFormat.format(cycle.probeClearance), // return distance in Z
      feedOutput.format(F),
      "Q" + xyzFormat.format(minProbeDistance), // step feed distance from start point
      "S1", // S1 = Z is not measured, S0 = Z is measured
      "B1", // number of times measurement is made 1-3
      "C" + xyzFormat.format(cycle.tolerancePosition), // allowable tolerance when B > 1
      "A0" // rotation angle
    );
    enableLengthCompensation(false);
    break;
  case "probing-y-wall":
    ...
    writeBlock(
      gFormat.format(65), "P" + 9146,
      "X" + xyzFormat.format(0), // start along the X-axis
      "Y" + xyzFormat.format(cycle.probeClearance + (cycle.width1 / 2) + (tool.diameter / 2)), // start along the Y-axis
      "Z" + xyzFormat.format(cycle.depth), // probing depth from current position
      "T" + printProbeResults(), // T0 = "0 set" is made, T1 = "0 set" is not made (origin)
      "I" + xyzFormat.format(cycle.probeClearance), // return distance
      "K" + xyzFormat.format(cycle.probeClearance), // return distance in Z
  ...
  case "probing-xy-circular-partial-boss":
    cycle.partialCircleAngleA = cycle.partialCircleAngleA < 0 ? cycle.partialCircleAngleA + 360 : cycle.partialCircleAngleA;
    cycle.partialCircleAngleB = cycle.partialCircleAngleB < 0 ? cycle.partialCircleAngleB + 360 : cycle.partialCircleAngleB;
    cycle.partialCircleAngleC = cycle.partialCircleAngleC < 0 ? cycle.partialCircleAngleC + 360 : cycle.partialCircleAngleC;
    protectedProbeMove(cycle, x, y, z);
    writeBlock(
      gFormat.format(65), "P" + 9144,
      "Z" + xyzFormat.format(cycle.depth), // probing depth from current position
      "T" + printProbeResults(), // T0 = "0 set" is made, T1 = "0 set" is not made (origin)
      "I" + xyzFormat.format(cycle.probeClearance), // return distance
      feedOutput.format(F),
      ...
      "B1", // number of times measurement is made 1-3
      "C" + xyzFormat.format(cycle.tolerancePosition), // allowable tolerance when B > 1
      "U" + xyzFormat.format(cycle.partialCircleAngleA), // measurement angle 1
      "V" + xyzFormat.format(cycle.partialCircleAngleB), // measurement angle 2
      "W" + xyzFormat.format(cycle.partialCircleAngleC), // measurement angle 3
      "R" + xyzFormat.format((cycle.width1 / 2) + (tool.diameter / 2) + cycle.probeClearance)
    );
    enableLengthCompensation(false);
    break;
  case "probing-xy-circular-hole":
    ...
    if (cycle.outOfPositionAction != undefined) { // measure tool sensor length
      writeBlock(
        gFormat.format(65), "P" + 9142,
        "R" + xyzFormat.format(cycle.width1 / 2), // radius of ring gauge
        "Q" + xyzFormat.format(minProbeDistance), // step feed distance from start point
        feedOutput.format(F)
      );
    } else {
      writeBlock(
        gFormat.format(65), "P" + 9145,
        "X" + xyzFormat.format((cycle.width1 / 2) - (tool.diameter / 2) - cycle.probeClearance), // rapid distance in X
        "Y" + xyzFormat.format((cycle.width1 / 2) - (tool.diameter / 2) - cycle.probeClearance), // rapid distance in Y
        "Z" + xyzFormat.format(0), // probing depth from current position
        "T" + printProbeResults(), // T0 = "0 set" is made, T1 = "0 set" is not made (origin)
        "I" + xyzFormat.format(cycle.probeClearance), // return distance
        feedOutput.format(F),
        "Q" + xyzFormat.format(minProbeDistance), // step feed distance from start point
  ...
  case "probing-xy-rectangular-hole":
    ...
    writeBlock(
      gFormat.format(65), "P" + 9145,
      "X" + xyzFormat.format((cycle.width1 / 2) - (tool.diameter / 2) - cycle.probeClearance), // rapid distance in X
      "Y" + xyzFormat.format((cycle.width2 / 2) - (tool.diameter / 2) - cycle.probeClearance), // rapid distance in Y
      "Z" + xyzFormat.format(0), // probing depth from current position
      "T0", // T0 = "0 set" is made, T1 = "0 set" is not made (origin)
      "I" + xyzFormat.format(cycle.probeClearance), // return distance
      feedOutput.format(F),
      "Q" + xyzFormat.format(minProbeDistance), // step feed distance from start point
      "B1", // number of times measurement is made 1-3
  ...
  case "probing-xy-rectangular-boss":
    ...
    writeBlock(
      gFormat.format(65), "P" + 9146,
      "X" + xyzFormat.format(cycle.probeClearance + (cycle.width1 / 2) + (tool.diameter / 2)), // start along the X-axis
      "Y" + xyzFormat.format(cycle.probeClearance + (cycle.width2 / 2) + (tool.diameter / 2)), // start along the Y-axis
      "Z" + xyzFormat.format(cycle.depth), // probing depth from current position
      "T" + printProbeResults(), // T0 = "0 set" is made, T1 = "0 set" is not made (origin)
      "I" + xyzFormat.format(cycle.probeClearance), // return distance
      "K" + xyzFormat.format(cycle.probeClearance), // return distance in Z
  ...
  case "probing-xy-inner-corner":
    var quadrant;
    var dir1 = approach(cycle.approach1);
    var dir2 = approach(cycle.approach2);
    if (dir1 == 1) {
      quadrant = dir2 == 1 ? 1 : 4;
    } else {
      quadrant = dir2 == 1 ? 2 : 3;
    ...
    protectedProbeMove(cycle, x, y, z - cycle.depth);
    writeBlock(
      gFormat.format(65), "P" + 9147,
      "X" + xyzFormat.format(0), // rapid distance in X
      "Y" + xyzFormat.format(0), // rapid distance in X
      "Z" + xyzFormat.format(0), // probing depth from current position
      "T" + printProbeResults(), // T0 = "0 set" is made, T1 = "0 set" is not made (origin)
      "I" + xyzFormat.format(cycle.probeClearance), // return distance
      feedOutput.format(F),
...
function onCommand(command) {
  ...
  case COMMAND_START_SPINDLE:
    forceSpindleSpeed = false;
    if ((Math.abs(spindleSpeed - sOutput.getCurrent()) > minSpeedChange) && (spindleSpeed != minSpeedChange)) {
      writeBlock(sOutput.format(minSpeedChange), mFormat.format(tool.clockwise ? 3 : 4));
      onDwell(10);
    }
  ...
  case COMMAND_STOP_SPINDLE:
    if (sOutput.getCurrent() > 5000) {
      writeBlock(sOutput.format(5000), mFormat.format(tool.clockwise ? 3 : 4));
      onDwell(10);
    }
    sOutput.setCurrent(0);
...
function onSectionEnd() {
  ...
  // tool break control
  if (isLastSection() && tool.breakControl) {
    writeRetract(Z);
...
function onClose() {
  ...
  if (getProperty("material") == "hard") {
    ...
    writeBlock(mFormat.format(67), formatComment("AIR BLOW OFF"));
    writeBlock(mFormat.format(70), formatComment("DUST COLLECTOR OFF"));
  }
  writeRetract(Z); // retract
  onCommand(COMMAND_STOP_SPINDLE);
