/**
  ...
  $Revision: 44086 3db02772c580fa626f6d59f7a48e3477eb72c563 $
  $Date: 2023-08-25 21:42:35 $

  FORKID {065403A3-A589-43ef-B345-90B5274EE274}
*/

description = "ProtoTRAK GCD";
...
longDescription = "Milling post for ProtoTRAK common machine tools like RMX and TMC, GCD format.";
...
properties = {
  ...
  preferredTilt: {
    title      : "Use M22",
    description: "RMX machine tools can use M22, which moves the table towards the operator.",
    group      : "multiAxis",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
var gCompModal = createModal({}, gFormat); // modal group 1 // G0-G3, ...
...
function onSection() {
  ...
  // cutter compensation must be output before z (at depth) move and before it is turned on in toolpath stream.
  // get comp usage/state from parameters
  if (hasParameter("operation:compensationType") && hasParameter("operation:compensation") &&
      getParameter("operation:compensationType", "") != "computer" && getParameter("operation:compensationType", "") != "off") {
    pendingRadiusCompensation = getParameter("operation:compensation") == "right" ?  RADIUS_COMPENSATION_RIGHT : RADIUS_COMPENSATION_LEFT;
  }
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    case "right-tapping":
      ...
      var tapCode = ((tool.type == TOOL_TAP_LEFT_HAND) || (cycleType == "left-tapping")) ? 74 : 84;
      if (getProperty("usePitchForTapping")) {
        writeBlock(
          gRetractModal.format(98), gAbsIncModal.format(90), gCycleModal.format(tapCode),
          getCommonCycle(x, y, z, cycle.retract),
          pitchOutput.format(tool.threadPitch), sOutput.format(spindleSpeed)
        );
      } else {
        writeBlock(
          gRetractModal.format(98), gAbsIncModal.format(90), gCycleModal.format(tapCode),
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      ...
      case RADIUS_COMPENSATION_LEFT:
        writeBlock(gMotionModal.format(1), gCompModal.format(41), x, y, z, f);
        break;
      case RADIUS_COMPENSATION_RIGHT:
        writeBlock(gMotionModal.format(1), gCompModal.format(42), x, y, z, f);
        break;
      default:
        writeBlock(gCompModal.format(40), gMotionModal.format(1), x, y, z, f);
      }
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  gMotionModal.reset(); // added due to TRAK Machines stating that all circular moves should have the G2/G3 output included
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      writeBlock(
        ...
        iOutput.format(circle.offset.x, 0), jOutput.format(circle.offset.y, 0), kOutput.format(0, 0), feedOutput.format(feed)
      );
      break;
    case PLANE_ZX:
      writeBlock(
        gAbsIncModal.format(90), gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3),
        iOutput.format(circle.offset.x, 0), jOutput.format(0, 0), kOutput.format(circle.offset.z, 0), feedOutput.format(feed)
      );
      break;
    case PLANE_YZ:
      writeBlock(
        gAbsIncModal.format(90), gPlaneModal.format(19), gMotionModal.format(clockwise ? 2 : 3),
        iOutput.format(0, 0), jOutput.format(circle.offset.y, 0), kOutput.format(circle.offset.z, 0), feedOutput.format(feed)
      );
      break;
    default:
      var t = tolerance;
...
function onSectionEnd() {
  writeBlock(gPlaneModal.format(17));
  if (gCompModal.getCurrent() != 40) {
    writeBlock(gCompModal.format(40));
  }
