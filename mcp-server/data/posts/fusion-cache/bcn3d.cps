/**
  ...
  BCN3D post processor configuration.

  $Revision: 44106 85222cccf9cee3e216d9ee111aa5823d026a774f $
  $Date: 2024-01-03 15:11:55 $

  FORKID {C36EACE6-2207-48E6-A42C-9061B02A9BED}
*/

description = "BCN3D";
vendor = "BCN3D";
vendorUrl = "https://www.bcn3d.com/";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Post for exporting toolpath to the BCN3D range of printers in gcode format (Sigma, Epsilon...)";
...
var settings = {
  ...
  maximumExtruderTemp: 300, // specifies the maximum extruder temperature
...
function onOpen() {
  ...
  writeComment("FLAVOR:Marlin");
  if (typeof writeProgramHeader == "function") {
    writeProgramHeader();
  }
  if (getProperty("printMode") != "regular") {
    if (hasGlobalParameter("ext2-extrusion-len")) {
      if (getExtruder(2).extrusionLength > 0) {
        error(localize("When using the duplication or the mirror mode, you can only generate a toolpath for the extruder 1 (T0)."));
        return;
      }
    }
  }
...
function onSection() {
  ...
  // dwell for ensuring pressure is reached
  writeBlock(gFormat.format(4), pFormat.format(2000));
  switch (getProperty("printMode")) {
  case "duplication":
    writeBlock(mFormat.format(605), sOutput.format(5));
    writeBlock(gFormat.format(4), pFormat.format(1));
    writeBlock(gFormat.format(4), pFormat.format(2));
    writeBlock(gFormat.format(4), pFormat.format(3));
    break;
  case "mirror":
    writeBlock(mFormat.format(605), sOutput.format(6));
    writeBlock(gFormat.format(4), pFormat.format(1));
    writeBlock(gFormat.format(4), pFormat.format(2));
    writeBlock(gFormat.format(4), pFormat.format(3));
    break;
...
function onClose() {
  xOutput.reset();
  yOutput.reset();
  writeBlock(gAbsIncModal.format(91), gFormat.format(28), xOutput.format(0), yOutput.format(0));
  writeBlock(gAbsIncModal.format(90)); // absolute spatial coordinates
