...
properties = {
  ...
  useTappingCycle: {
    title      : "Use tapping cycles",
    description: "Specifies that tapping cycles are supported on the machine.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
var feedXYOutput = createOutputVariable({prefix:"F"}, feedXYFormat);
var feedZOutput = createOutputVariable({prefix:"F"}, feedZFormat);
...
function defineMachine() {
  ...
  if (masterAxis >= 0) { // note: setup your machine here
...
function onOpen() {
  ...
  writeBlock(gUnitModal.format(unit == MM ? 22 : 20));
...
function onSection() {
  ...
  if (insertToolCall && !isFirstSection() && !getProperty("useToolCall")) {
    error(localize("Having different tools in a single program is not allowed without a tool changer."));
    return;
  }
...
function onCyclePoint(x, y, z) {
  ...
  if (!getProperty("useTappingCycle") && isTappingCycle()) {
  ...
  if (isFirstCyclePoint() || (isTappingCycle() && !cycleExpanded) || cycleRequiresRetract) {
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      ...
      case RADIUS_COMPENSATION_LEFT:
        writeBlock(gFormat.format(41), conditional(!getProperty("useToolCall"), "O" + xyzFormat.format(tool.diameter / 2)));
        writeBlock(gMotionModal.format(1), x, y, z, f);
        break;
      case RADIUS_COMPENSATION_RIGHT:
        writeBlock(gFormat.format(42), conditional(!getProperty("useToolCall"), "O" + xyzFormat.format(tool.diameter / 2)));
        writeBlock(gMotionModal.format(1), x, y, z, f);
        break;
      default:
...
function onClose() {
  ...
  // writeBlock(mFormat.format(15)); // activate notification light
}
