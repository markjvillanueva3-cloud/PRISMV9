...
description = "Tekna";
vendor = "Tekna";
vendorUrl = "http://www.tekna.it";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic post for Tekna.";
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    if (getProperty("useRadius")) { // radius mode does not support full arcs
      linearize(tolerance);
      return;
    }
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    writeToolBlock(gPlaneModal.format(17), "T" + toolFormat.format(tool.number), "DL" + xyzFormat.format(tool.bodyLength));
