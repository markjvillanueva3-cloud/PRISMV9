/**
  ...
  Bravoprodigy post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {8E81B1AA-D64A-4042-B2AA-4B5E9C14D83A}
*/

description = "Bravoprodigy";
vendor = "Bravoprodigy";
vendorUrl = "https://www.bravoprodigy.com";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic post for Bravoprodigy.";
...
function onOpen() {
  ...
  if (getProperty("splitFile") != "none") {
    writeln(localize("***THIS FILE DOES NOT CONTAIN NC CODE***"));
    return;
  }
...
function onSection() {
  ...
  var newWorkOffset = isNewWorkOffset() || splitHere;
  var newWorkPlane = isNewWorkPlane() || splitHere;
  ...
  if (splitHere) {
    if (!isFirstSection()) {
      ...
      writeBlock(mFormat.format(30)); // home XYZ
      if (isRedirecting()) {
        closeRedirection();
      }
    }
    ...
    if (getProperty("splitFile") == "toolpath") {
      var comment = getParameter("operation-comment", getCurrentSectionId());
    ...
    writeln(localize("Load tool number " + tool.number + " and subprogram " + subprogram));
  ...
  startSpindle(tool, true); // force spindle speed output
  ...
  if (isRequired) {
    writeBlock(gMotionModal.format(0), zOutput.format(initialPosition.z)); // position Z first
    forceXYZ();
  }
...
function onCommand(command) {
  ...
  case COMMAND_OPTIONAL_STOP:
    if (!isFirstSection() && getProperty("optionalStop") && getProperty("splitFile") == "none") {
