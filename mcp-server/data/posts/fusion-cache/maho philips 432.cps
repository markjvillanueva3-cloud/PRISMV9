...
function onOpen() {
  ...
  gFeedModeModal.disable(); // Feedmode is not supported
...
function getPlane() {
  return getProperty("vertical") ? 18 : 17;
}
...
function writeInitialPositioning(initialPosition, isRequired) {
  ...
  var modalCodes = formatWords(gAbsIncModal.format(90), gPlaneModal.format(getPlane()));
  if (!isRequired) {
    ...
    writeBlock(modalCodes, gMotionModal.format(motionCode.multi), xOutput.format(initialPosition.x), yOutput.format(initialPosition.y), feed);
  } else {
    if (machineConfiguration.isHeadConfiguration()) {
      writeBlock(modalCodes, gMotionModal.format(motionCode.multi),
        xOutput.format(initialPosition.x), yOutput.format(initialPosition.y), zOutput.format(initialPosition.z), feed);
    } else {
      writeBlock(modalCodes, gMotionModal.format(motionCode.multi), xOutput.format(initialPosition.x), yOutput.format(initialPosition.y), feed);
      writeBlock(gMotionModal.format(motionCode.single), zOutput.format(initialPosition.z));
    }
  }
}
