...
properties = {
  ...
  hasCAxis: {
    title      : "Got C axis",
    description: "Defines whether the machine has a C axis.",
    group      : "configuration",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
var settings = {
  coolant: {
    ...
    coolants: [
      {id:COOLANT_FLOOD, on:106},
      {id:COOLANT_MIST, on:106},
      {id:COOLANT_THROUGH_TOOL, on:106},
      {id:COOLANT_AIR, on:106},
      {id:COOLANT_AIR_THROUGH_TOOL, on:106},
      {id:COOLANT_SUCTION, on:106},
      {id:COOLANT_FLOOD_MIST, on:106},
      {id:COOLANT_FLOOD_THROUGH_TOOL, on:106},
      {id:COOLANT_OFF, off:107}
    ],
...
function defineMachine() {
  ...
  if (getProperty("fourthAxis") != "none") {
    ...
    if (getProperty("fourthAxis") == "x") {
      var aAxis = createAxis({coordinate:0, table:true, axis:[-1, 0, 0], cyclic:true, tcp:useTCP, preference:0});
    } else if (getProperty("fourthAxis") == "y") {
      var aAxis = createAxis({coordinate:1, table:true, axis:[0, -1, 0], cyclic:true, tcp:useTCP, preference:0});
    }
    if (getProperty("hasCAxis")) {
      var cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], cyclic:true, tcp:useTCP, preference:0});
    }
    if (aAxis) {
      if (cAxis) {
  ...
  if (!receivedMachineConfiguration) {
    ...
    if (machineConfiguration.isMultiAxisConfiguration()) {
      machineConfiguration.setMultiAxisFeedrate(
        FEED_FPM,
        999999, // maximum output value for inverse time feed rates
...
function onOpen() {
  ...
  gFeedModeModal.disable(); // disable feed mode modal
...
function onSection() {
  ...
  if (currentSection.type == TYPE_MILLING) {
    ...
    var toolNumber = getProperty("laserToolNumber"); // laser
...
function writeJetCodes(mode) {
  if (getProperty("useLaserM3M5") && currentSection.type == TYPE_JET && tool.type == TOOL_LASER_CUTTER) {
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      writeBlock(gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), iOutput.format(cx - start.x), jOutput.format(cy - start.y), feedOutput.format(feed), conditional(currentSection.type == TYPE_JET, powerOutput.format(power ? getPower() : 0)));
      break;
    case PLANE_ZX:
      writeBlock(gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3), zOutput.format(z), iOutput.format(cx - start.x), kOutput.format(cz - start.z), feedOutput.format(feed), conditional(currentSection.type == TYPE_JET, powerOutput.format(power ? getPower() : 0)));
      break;
    case PLANE_YZ:
      writeBlock(gPlaneModal.format(19), gMotionModal.format(clockwise ? 2 : 3), yOutput.format(y), jOutput.format(cy - start.y), kOutput.format(cz - start.z), feedOutput.format(feed), conditional(currentSection.type == TYPE_JET, powerOutput.format(power ? getPower() : 0)));
      break;
    default:
      linearize(tolerance);
    }
    ...
    switch (getCircularPlane()) {
    case PLANE_XY:
      writeBlock(gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x), jOutput.format(cy - start.y), feedOutput.format(feed), conditional(currentSection.type == TYPE_JET, powerOutput.format(power ? getPower() : 0)));
      break;
    case PLANE_ZX:
      writeBlock(gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x), kOutput.format(cz - start.z), feedOutput.format(feed), conditional(currentSection.type == TYPE_JET, powerOutput.format(power ? getPower() : 0)));
      break;
    case PLANE_YZ:
      writeBlock(gPlaneModal.format(19), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), jOutput.format(cy - start.y), kOutput.format(cz - start.z), feedOutput.format(feed), conditional(currentSection.type == TYPE_JET, powerOutput.format(power ? getPower() : 0)));
      break;
    default:
      linearize(tolerance);
    }
