...
longDescription = "Post for ShopBot using ISO format. Note that ShopBot recommends using the Shopbot OpenSBP post instead.";
deprecatedDescription = "This post has been deprecated. ShopBot recommends using the Shopbot OpenSBP post exclusively.";
...
allowedCircularPlanes = 1 << PLANE_XY; // allow ZX plane only
...
properties = {
  ...
  useSpindleDirection: {
    title      : "Spindle direction code",
    description: "Sets the available spindle direction code.",
    group      : "preferences",
    type       : "integer",
    values     : [
      {id:3, title:"M03"},
...
function onOpen() {
  ...
  if ((getProperty("useSpindleDirection") != 3) &&
      (getProperty("useSpindleDirection") != 4) &&
      (getProperty("useSpindleDirection") != 34)) {
    error(localize("The property 'Spindle Direction Code' must be 3, 4, or 34."));
    return;
  }
...
function onCyclePoint(x, y, z) {
  ...
  expandCyclePoint(x, y, z); // expand all cycles
  return;
}
...
function onCommand(command) {
  ...
  case COMMAND_START_SPINDLE:
    ...
    writeBlock(sOutput.format(spindleSpeed));
    if (getProperty("useSpindleDirection") == 34) {
      writeBlock(mFormat.format(tool.clockwise ? 3 : 4));
    } else {
      writeBlock(mFormat.format(getProperty("useSpindleDirection")));
    }
    return;
