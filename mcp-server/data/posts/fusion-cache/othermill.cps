...
description = "Othermill (Otherplan)";
vendor = "Other Machine";
vendorUrl = "https://othermachine.co";
legal = "Copyright (C) 2012-2022 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45702;

longDescription = "Generic milling post for Othermill (Otherplan). This post also works with Bantam Tools desktop milling machines.";
...
wcsDefinitions = {
  useZeroOffset: false,
  wcs          : [
    {name:"Standard", format:"G", range:[55, 55]}
  ]
};
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    writeToolBlock("T" + toolFormat.format(tool.number), conditional(getProperty("useToolCall"), mFormat.format(6)));
    if (tool.comment) {
