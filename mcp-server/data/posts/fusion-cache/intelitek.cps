...
properties = {
  ...
  useG43: {
    title      : "Use compensation offsets",
    description: "Specifies to output tool length and compensation offsets.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  machineModel: {
    ...
    type       : "enum",
    values     : [
      {id:"Benchman", title:"Benchman"},
      {id:"Benchmill", title:"Benchmill"},
      {id:"Promill", title:"Promill"}
    ],
    value: "Benchman",
    scope: "post"
  }
...
var gUnitModal = createOutputVariable({}, gFormat); // modal group 6 // G70-71 or G20-21
...
var settings = {
  ...
  outputToolDiameterOffset    : true, // specifies if tool diameter offset code should be output for tool radius compensation (Dxx)
...
function onOpen() {
  ...
  case "Benchman":
    settings.maximumSpindleRPM = 45000;
    maximumFeed = toPreciseUnit(200, IN);
    // tolerance = spatial(0.002, MM);
    break;
  case "Benchmill":
    settings.maximumSpindleRPM  = 5000;
  ...
  if (!getProperty("useG43")) {
    ...
    settings.outputToolDiameterOffset = false;
  }
...
function onSection() {
  ...
  if (!getProperty("useToolCall")) {
    setProperty("optionalStop", false);
  }
  // tool change
  ...
  if (currentSection.getMaximumFeedrate() > maximumFeed) {
    warning(subst(localize("The feedrate exceeds the machines maximum feedrate in operation \"%1\"."), getParameter("operation-comment", "")));
  }
