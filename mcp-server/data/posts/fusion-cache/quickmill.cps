...
description = "QUICKMILL";
vendor = "QUICKMILL";
vendorUrl = "https://quickmill.com/";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic post for QUICKMILL.The machine must be defined in the setup to access Motion Clamps during the drilling cycles";
...
properties = {
  ...
  clampXYaxes: {
    title      : "Clamp XY axes for drilling",
    description: "Enable XY Motion Clamps with drilling cycles.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "operation",
...
var mClampModal = createOutputVariable({}, mFormat); // M15-M16
...
var settings = {
  coolant: {
    ...
    coolants: [
      {id:COOLANT_FLOOD, on:11},
      {id:COOLANT_MIST},
      {id:COOLANT_THROUGH_TOOL, on:12, off:9},
      {id:COOLANT_AIR},
...
function clampXYaxes(lock) {
  if (isDrillingCycle() && getProperty("clampXYaxes")) {
    writeBlock(mClampModal.format(lock ? 15 : 16));
  }
}

function onCycle() {
  writeBlock(gPlaneModal.format(17));
  clampXYaxes(true); // clamp XY axes if required
}
...
function writeDrillCycle(cycle, x, y, z) {
  ...
  // K0 tells the machine to NOT execute the cycle at the current position, it will execute the cycle on the following motion block
  var K = isDrillingCycle() && getProperty("showSequenceNumbers") == "true" ? "K0" : "";
  ...
  if (isFirstCyclePoint()) {
    ...
    case "tapping":
      ...
      if (getProperty("usePitchForTapping")) {
        writeBlock(
          ...
          getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
          "P" + milliFormat.format(P),
          pitchOutput.format(tool.threadPitch), K
        );
        forceFeed();
      } else {
    ...
    if (K != "") {
      writeBlock("GOTO" + (sequenceNumber + getProperty("sequenceNumberIncrement")));
      xOutput.reset(); // XY positions are always required when using GOTO
      yOutput.reset(); // XY positions are always required when using GOTO
      writeBlock(xOutput.format(x), yOutput.format(y), zOutput.format(z)); // repeat first cycle position when using GOTO
    }
  } else {
    if (cycleExpanded) {
      expandCyclePoint(x, y, z);
...
function onSectionEnd() {
  clampXYaxes(false); // disable XY axes clamps if required
