/**
  ...
  FLASHFORGE Creator Pro post processor configuration.
...
description = "Flashforge Creator Pro";
vendor = "Flashforge";
...
longDescription = "Post for exporting toolpath to the FlashForge Creator Pro in gcode format. The GPX PostProcessor add-in can be downloaded from here: https://autode.sk/3nPOEj8";
...
function onOpen() {
  ...
  writeBlock(mFormat.format(136)); // start print
}

function onSection() {
  writeBlock(gFormat.format(162), "X Y", feedOutput.format(3000)); // homing in xy
  writeBlock(gFormat.format(161), "Z", feedOutput.format(1000)); // homing in Z

  writeBlock(gMotionModal.format(1), gAbsIncModal.format(91), zOutput.format(5));
  writeBlock(gAbsIncModal.format(90));

  writeBlock(gFormat.format(161), "Z", feedOutput.format(100)); // home z axis minimum

  writeBlock(mFormat.format(132), "X Y Z A B"); // recall stored home offsets
  writeBlock(getCode(getProperty("relativeExtrusion") ? commands.extrusionMode.relative : commands.extrusionMode.absolute));

  writeBlock(getCode(commands.extruderChangeCommand), tFormat.format(0)); // load right extruder offsets
  writeBlock(gFormat.format(130), xOutput.format(20), yOutput.format(20), zOutput.format(20), "A" + xyzFormat.format(20), "B" + xyzFormat.format(20)); // lower stepper Vrefs while heating
  writeBlock(getCode(commands.fan.off)); // disable fan
  writeBlock(mFormat.format(140), sOutput.format(bedTemp), tFormat.format(activeExtruder)); // set bed temperature
  writeBlock(mFormat.format(134), tFormat.format(activeExtruder)); // stabilize bed temperature
  ...
  if (getExtruder(1).temperature > 0.05) {
    writeBlock(mFormat.format(133), tFormat.format(0)); // stabilize right extruder temperature
  }
  if (getExtruder(2).temperature > 0.05) {
    writeBlock(mFormat.format(133), tFormat.format(1)); // stabilize left extruder temperature
  }

  writeBlock(gFormat.format(130), xOutput.format(127), yOutput.format(127), zOutput.format(40), "A" + xyzFormat.format(127), "B" + xyzFormat.format(127)); // reset stepper Vrefs while heating

  var initialPosition = getFramePosition(currentSection.getInitialPosition());
  writeBlock(gMotionModal.format(1), xOutput.format(initialPosition.x), yOutput.format(initialPosition.y), zOutput.format(initialPosition.z), feedOutput.format(toPreciseUnit(3000, MM)));

  writeBlock(gFormat.format(92), eOutput.format(0)); // zero extruders
  eOutput.reset();

  // writeBlock(gMotionModal.format(1), zOutput.format(unit == MM ? 0.4 : 0.4 / 25.4));

  writeBlock(getCode(commands.extruderChangeCommand), tFormat.format(activeExtruder)); // left or right tool
  eOutput.reset();
  writeBlock(gFormat.format(92), eOutput.format(0)); // zero extruders
...
function onClose() {
  ...
  writeBlock(gFormat.format(162), "X Y", feedOutput.format(toPreciseUnit(3000, MM))); // home XY maximum
  writeBlock(mFormat.format(18)); // disable stepper
  writeBlock(mFormat.format(72), pFormat.format(1));
  writeBlock(mFormat.format(137)); // play sound
...
function onLayer(num) {
  ...
  writeComment(localize("Layer : ") + integerFormat.format(num) + localize(" of ") + integerFormat.format(layerCount));
...
// <<<<< INCLUDED FROM ../common/onFanSpeed.cpi
// include("../common/onLayer.cpi");  // progress indicator at each layer
