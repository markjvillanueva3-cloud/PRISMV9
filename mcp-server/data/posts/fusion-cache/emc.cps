...
longDescription = "Generic milling post for Enhanced Machine Controller (EMC).";
...
function onOpen() {
  ...
  settings.outputToolLengthCompensation =  getProperty("useToolCall");
  settings.outputToolLengthOffset =  getProperty("useToolCall");
  ...
  if (insertToolCall) {
    if (getProperty("useToolCall")) {
      ...
      if (!isFirstSection()) {
        ...
        writeComment(localize("Pause program for tool change"));
      }
      writeToolBlock(formatComment("T" + toolFormat.format(tool.number) + " " + mFormat.format(6)));
      writeComment(getToolTypeName(tool.type) + " D=" + xyzFormat.format(tool.diameter) + (tool.description ? " " + tool.description : ""));
    }
  }
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    ...
    if (getProperty("preloadTool") && preloadTool && getProperty("useToolCall")) {
