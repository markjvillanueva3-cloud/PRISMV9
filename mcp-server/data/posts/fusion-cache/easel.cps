/**
  ...
  Inventables Easel post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {ACF761D6-7247-4780-8EB9-02E989EA34AF}
*/

description = "Easel by Inventables";
vendor = "Inventables";
vendorUrl = "https://www.inventables.com";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Milling post for Easel by Inventables (Grbl).";
...
// Easel g code sender does not take G28 or G53
...
properties = {
  ...
  clampOffsetX: {
    title      : "Clamp offset X",
    description: "Adds an offset for the clamps in the X direction.",
    group      : "preferences",
    type       : "number",
    value      : 0,
    scope      : "post"
  ...
  clampOffsetY: {
    title      : "Clamp offset Y",
    description: "Adds an offset for the clamps in the Y direction.",
    group      : "preferences",
    type       : "number",
    value      : 0,
    scope      : "post"
...
function writeTool(tool) {
  ...
  var toolString = "TOOL/MILL"
    + "," + xyzFormat.format(tool.diameter)
    + "," + xyzFormat.format(tool.cornerRadius)
    + "," + xyzFormat.format(tool.fluteLength)
    + "," + xyzFormat.format(tool.taperAngle);

  writeComment(toolString);
}

function writeStockAndOffset(wcs) {
  try {
    // STOCK/BLOCK, Length, Width, Height, Origin X, Origin Y, Origin Z
    var workpiece = getWorkpiece();
    var blockString = "STOCK/BLOCK"
      + "," + xyzFormat.format(workpiece.upper.x - workpiece.lower.x)
      + "," + xyzFormat.format(workpiece.upper.y - workpiece.lower.y)
      + "," + xyzFormat.format(workpiece.upper.z - workpiece.lower.z)
      + "," + wcs;
    writeComment(blockString);
  } catch (e) {
...
function onSection() {
  ...
  if (insertToolCall) {
    writeTool(tool);
    writeStockAndOffset(currentSection.wcsOrigin);
  }
  // tool change
  ...
  // translate the co-ordinates to allow for clamps
  setTranslation(new Vector(getProperty("clampOffsetX"), getProperty("clampOffsetY"), 0));
