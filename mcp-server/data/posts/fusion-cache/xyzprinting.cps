/**
  ...
  XYZprinting post processor configuration.
...
longDescription = "Post for exporting toolpath to XYZprinting fff range of printers in gcode format (daVinci, nano, mini, junior, color, PartPro)";
...
highFeedrate = toPreciseUnit(7200, MM);
...
properties = {
  printerModel: {
    ...
    type       : "enum",
    values     : [
      {title:"PartPro 300", id:"partpro300"},
  ...
  enclosureTemperature: {
    ...
    description: "Set the temperature to use with the enclosure temperature command (M103) for PartPro 300.",
    type       : "integer",
    value      : 0,
    range      : [0, 70],
    scope      : "post"
...
var commands = {
  ...
  setEnclosureTemperature: mFormat.format(103), // command to set the enclosure temperature PartPro only
  waitEnclosure          : mFormat.format(108), // wait command for the enclosure temperature PartPro only
};
...
function onOpen() {
  ...
  if (getProperty("printerModel") == "partpro300") {
    settings.maximumExtruderTemp = 350;
  ...
  if (getProperty("printerModel") == "partpro300") {
    if (getProperty("enclosureTemperature") > 0) {
      writeBlock(getCode(commands.setEnclosureTemperature), sOutput.format(getProperty("enclosureTemperature")));
    }
  }
}
...
function onSection() {
  ...
  if (getProperty("printerModel") == "partpro300") {
    if (getProperty("enclosureTemperature") > 0) {
      writeBlock(getCode(commands.waitEnclosure), sOutput.format(getProperty("enclosureTemperature")));
    }
  }
...
function onClose() {
  ...
  writeBlock(gMotionModal.format(1), eOutput.format(toPreciseUnit(-1, MM)), feedOutput.format(toPreciseUnit(300, MM)));

  if (getProperty("printerModel") == "partpro300") {
    if (getProperty("enclosureTemperature") > 0) {
      writeBlock(getCode(commands.setEnclosureTemperature), sOutput.format(0));
    }
  }
