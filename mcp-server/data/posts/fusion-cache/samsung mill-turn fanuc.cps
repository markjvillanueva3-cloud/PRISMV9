...
//     usePolarCoordinates        - Force Polar coordinates mode for next operation
//     usePolarInterpolation      - Force Polar interpolation for next operation
//
///////////////////////////////////////////////////////////////////////////////

description = "Samsung Mill/Turn with Fanuc control";
...
properties = {
  ...
  useG28: {
    title       : "Use G28 home position",
    description : "Use G28 instead of G53 to move to home position.",
    group       : "homePositions",
    type        : "boolean",
    presentation: "yesno",
    value       : true,
...
var coolants = [
  ...
  {id:COOLANT_AIR, spindle1:{on:51, off:53}, spindle2:{on:52, off:54}},
...
function getCode(code, spindle) {
  ...
  case "UNLOCK_MULTI_AXIS":
    return (spindle == SPINDLE_MAIN) ? 67 : 67;
  case "CLAMP_CHUCK":
...
function activateMachine(section) {
  ...
  machineConfiguration.setVendor("Samsung");
  machineConfiguration.setModel("SL 2500");
...
function invertAxes(activate, polarMode) {
  ...
  var yAxisPrefix = polarMode ? (activate ? "A" : "C") : "Y";
  ...
  if (polarMode) {
    ...
    cOutput = createVariable({prefix:(activate ? "A" : "C")}, cFormat);
    if (!yIsEnabled) {
      yOutput.disable();
    }
  }
...
function onSection() {
  ...
  // Turning is active
    ...
    if ((insertToolCall || wcsOut || feedMode) && !machineState.stockTransferIsActive) {
      ...
      writeBlock(wcsOut);
      writeBlock(feedMode, gPlaneModal.format(plane), cAxisEngageModal.format(getCode("DISABLE_C_AXIS", getSpindle(PART))));
    } else {
      writeBlock(feedMode);
    }
  }
  ...
  if (insertToolCall || retracted || (tool.getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED)) {
    ...
    if (machineState.usePolarCoordinates) {
      ...
      writeBlock(gMotionModal.format(0), zOutput.format(initialPosition.z));
      writeBlock(gMotionModal.format(0), xOutput.format(initialPosition.x), conditional(gotYAxis, yOutput.format(initialPosition.y)));
    }
  }
...
function goHome(forceY) {
  ...
  if (getProperty("useG28")) {
    ...
    if (gotYAxis && (yAxisIsEnabled || forceY)) {
      setYAxisMode(true);
      var yAxis = "V" + yFormat.format(getProperty("homePositionY"));
    }
    gMotionModal.reset();
    writeBlock(gMotionModal.format(0), gFormat.format(53), "X" + xFormat.format(getProperty("homePositionX")), yAxis);
    writeBlock(
      gMotionModal.format(0), gFormat.format(53),
      "Z" + zFormat.format((getSpindle(PART) == SPINDLE_MAIN) ? getProperty("homePositionZ") : getProperty("homePositionSubZ"))
    );
  }
}
