...
longDescription = "Generic milling post for Biemmepi.";
...
var gPlaneModal = createOutputVariable({}, gFormat);  // planes are not supported, added to be able to use writeInitialPositioning function
...
var settings = {
  ...
  retract: {
    ...
    useZeroValues             : ["G30"], // enter property value id(s) for using "0" value instead of machineConfiguration axes home position values (ie G30 Z0)
...
function onOpen() {
  ...
  gPlaneModal.disable(); // planes are not supported on this control
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      writeBlock(gAbsIncModal.format(90), gMotionModal.format(clockwise ? 2 : 3), iOutput.format(cx - start.x), jOutput.format(cy - start.y));
      break;
    default:
      linearize(tolerance);
    }
    ...
    case PLANE_XY:
      if (isHelical()) {
        ...
        writeBlock(gAbsIncModal.format(90), gMotionModal.format(clockwise ? 22 : 23), xOutput.format(x), yOutput.format(y), iOutput.format(cx - start.x), jOutput.format(cy - start.y), "K" + xyzFormat.format(getHelicalPitch()));
      } else {
        writeBlock(gAbsIncModal.format(90), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), iOutput.format(cx - start.x), jOutput.format(cy - start.y));
      }
      break;
    default:
      linearize(tolerance);
      ...
      case "G28":
        ...
        if (retract.retractAxes[2]) {
          ...
          if (machineConfiguration.hasHomePositionX() || machineConfiguration.hasHomePositionY()) {
            writeBlock(gFormat.format(28), gAbsIncModal.format(90), words);
          } else {
