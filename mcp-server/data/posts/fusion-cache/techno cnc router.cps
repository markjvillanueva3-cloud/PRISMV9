...
function writeInitialPositioning(initialPosition, isRequired) {
  ...
  if (getProperty("machineModel") == "hd2") {
    validate(state.lengthCompensationActive, "Length compensation is not active.");
  }
...
function onSection() {
  ...
  var isRequired = (getProperty("machineModel") == "hd2" || (getProperty("machineModel") == "hds" &&  tcp.isSupportedByOperation)) &&
    (insertToolCall || !state.lengthCompensationActive || state.retractedZ || (!isFirstSection() && getPreviousSection().isMultiAxis()));
...
function writeRetract() {
  ...
  if (retract && retract.words.length > 0) {
    ...
    for (var i in retract.words) {
      ...
      gMotionModal.reset();
      writeBlock(gFormat.format(79), words);
...
function onReturnFromSafeRetractPosition(_x, _y, _z) {
  ...
  if (!state.lengthCompensationActive && getOffsetCode() != undefined) {
    if (getProperty("machineModel") == "hd2") {
