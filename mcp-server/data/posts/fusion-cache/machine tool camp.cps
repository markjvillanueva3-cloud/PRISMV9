/**
  ...
  Machine Tool Camp post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {8E87A215-A75F-4A8D-9AD0-A47765B4A42B}
*/

description = "Machine Tool Camp";
vendor = "Machine Tool Camp";
vendorUrl = "https://www.machinetoolcamp.com";

legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic milling post for Machine Tool Camp.";
...
properties = {
  writeHeaderInfo: {
    ...
    description: "Output document path, job description, time, date, and programmer in the header of the code.",
    group      : "formats",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  beginWithLoadingCodes: {
    title      : "Begin program with vacuum codes",
    description: "Toggles output of Sweeper functions at end of program.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  endWithSweeperDown: {
    title      : "End program with sweeper down",
    description: "Toggles output of Sweeper functions at end of program.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
  ...
  endWithVacuumOff: {
    title      : "End program with vacuum off",
    description: "Toggles output of vacuum at program end.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  pinCodes: {
    title      : "Output the pin control codes",
    description: "Toggles output of Sweeper functions at end of program.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
...
var dateFormat = createFormat({width:2, zeropad:true, decimals:0});
...
var dOutput = createOutputVariable({}, diameterOffsetFormat);
...
var settings = {
  ...
  retract: {
    ...
    homeXY                    : {onIndexing:{axes:[Y]}, onToolChange:false, onProgramEnd:{axes:[X]}} // Specifies when the machine should be homed in X/Y. Sample: onIndexing:{axes:[X, Y], singleLine:false}
  },
...
function onOpen() {
  ...
  if (machineConfiguration.getAxisU().isEnabled()) { // When post is set for B axis (4th axis) skip G43 and H values, the G43 length offset is non-functional when using B-axis according the MTC staff
  ...
  // Dust brush up *this should happen always
  writeBlock(mFormat.format(5), mFormat.format(602), formatComment("Spindle off and Dust Brush up"));
  writeBlock(mFormat.format(100), mFormat.format(607), mFormat.format(608), mFormat.format(603), formatComment("Drills off, drill slide up, drill motor  off, part pins down"));

  if (getProperty("beginWithLoadingCodes")) {
    writeBlock(mFormat.format(604), formatComment("Vacuum off"));
    if (getProperty("pinCodes")) {
      writeBlock(mFormat.format(503), formatComment("Pins up"));
    }
    writeBlock(mFormat.format(0), formatComment("Stop to check part hold down"));
    writeBlock(mFormat.format(504), formatComment("Vacuum on"));
    if (getProperty("pinCodes")) {
      writeBlock(mFormat.format(603), formatComment("Pins down"));
    }
    writeBlock(mFormat.format(0), formatComment("Stop to position part"));
  }
...
function writeHeaderInfo() {
  if (getProperty("writeHeaderInfo")) {
    // Header Info
    var docPath = getGlobalParameter("document-path", "");
    writeComment(docPath ? localize("Project: ") + docPath  : "");

    var jobDescription = getGlobalParameter("job-description", "");
    writeComment(jobDescription ? localize("Setup Name: ") + jobDescription : "");

    // Show date in ISO date format
    var date = new Date();
    writeComment(localize("Date: ") + date.getFullYear() + "-" + dateFormat.format((date.getMonth() + 1)) + "-"
    + dateFormat.format(date.getDate()) + localize("   Time: ") + date.toLocaleTimeString());

    var userName = getGlobalParameter("username", "");
    writeComment(userName ? localize("Programmer: ") + userName : "");

    var generatedBy = getGlobalParameter("generated-by", "");
    writeComment(localize(generatedBy));
    writeComment("Post file name: " + FileSystem.replaceExtension(FileSystem.getFilename(getConfigurationPath()), ""));

    if (machineConfiguration.getAxisU().isEnabled()) {
      writeComment("Note : This code is for machine with rotary axis on head.");
    } else {
      writeComment("Note : This code is for 3 axis machine.");
    }
  }
}
...
function onSection() {
  ...
  if (machineConfiguration.getAxisU().isEnabled()) {
    writeComment("Note : This code is for machine with rotary axis on head.");
    writeComment("Total from spindle face to tool tip for this tool is: " + xyzFormat.format(getBodyLength(currentSection.getTool())));
    writeComment("Coordinates will be at the spindle face, not the tool tip.");
  }
  ...
  if (insertToolCall) {
    writeBlock(mFormat.format(502), formatComment("Dust Brush down"));
    forceWorkPlane();
  }
  ...
  setCoolant(tool.coolant); // set coolant after we have positioned at Z
}
...
function onCycle() {
  if (isTappingCycle()) {
    error(localize("Tapping Cycles are not supported by the machine."));
  }
  writeBlock(gPlaneModal.format(17));
}
...
function onCyclePoint(x, y, z) {
  if (isInspectionOperation()) {
    error(localize("Inspection functions are not supported in this post processor"));
  } else if (isProbeOperation()) {
    error(localize("Probing functions are not supported in this post processor"));
  } else {
...
function onClose() {
  ...
  writeBlock(gUnitModal.format(unit == MM ? 71 : 70), gFormat.format(17), gFormat.format(40), gFormat.format(49), gFormat.format(90));

  writeBlock(mFormat.format(5), mFormat.format(602), formatComment("Spindle off and Dust Brush up"));
  writeBlock(mFormat.format(607), mFormat.format(608), mFormat.format(603));
  writeRetract(Z); // retract
  disableLengthCompensation(true);

  if (getProperty("endWithVacuumOff")) {
    writeBlock(mFormat.format(604), formatComment("Vacuum off"));
  }
  ...
  if (getProperty("endWithSweeperDown")) {
    writeBlock(mFormat.format(517), formatComment("Sweeper down"));
    writeBlock(mFormat.format(809), formatComment("Check Sweeper extend sensor"));
    writeBlock(mFormat.format(617), formatComment("Sweeper up"));
    writeBlock(mFormat.format(810), formatComment("Check Sweeper retract sensor"));
  }
