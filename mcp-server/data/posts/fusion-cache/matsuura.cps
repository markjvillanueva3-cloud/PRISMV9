...
properties = {
  ...
  probingMacros: {
    ...
    description: "Select the probing software installed on your control.",
    group      : "probing",
    type       : "enum",
    values     : [
...
var mClampModal = createOutputVariable({}, mFormat); // M131-132
...
function onOpen() {
  ...
  probeBaseNumber = getProperty("probingMacros") == "renishaw" ? 9800 : 9500;
...
function initializeSmoothingPrefix() {
  ...
  if (smoothing.prefix != previousPrefix) {
    smoothing.level      = -1; // reset smoothing level to force 'isDifferent' parameter
    smoothing.tolerance  = -1; // reset smoothing tolerance to force 'isDifferent' parameter
  }
}
