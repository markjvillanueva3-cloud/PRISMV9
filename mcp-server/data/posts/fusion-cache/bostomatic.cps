...
var settings = {
  coolant: {
    ...
    coolants: [
      ...
      {id:COOLANT_MIST, on:81, off:9},
      {id:COOLANT_THROUGH_TOOL},
      {id:COOLANT_AIR, on:83, off:9},
...
function onSection() {
  ...
  if (insertToolCall || state.retractedZ) {
    writeBlock(gAbsIncModal.format(90), gFormat.format(48), "R"); // enable tool length compensation
  }
  // prepositioning
...
function onCommand(command) {
  ...
  case COMMAND_START_SPINDLE:
    ...
    writeBlock(gFormat.format(141), sOutput.format(spindleSpeed), mFormat.format(tool.clockwise ? 3 : 20));
    return;
...
function onClose() {
  ...
  writeBlock(gAbsIncModal.format(90), gFormat.format(49), "R"); // disable tool length compensation
