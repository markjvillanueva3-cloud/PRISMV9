/**
  ...
  FORKID {E381342B-84BD-4434-9481-F4447F7E592D}
*/

description = "RepRap fff";
vendor = "RepRap";
vendorUrl = "https://reprap.org";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic post for exporting toolpath to a printer with a RepRap firmware in gcode format";
...
properties = {
  functionSettingTemperature: {
    title      : "Function code for temperature",
    description: "Define the function used for setting the temperature for the extruder.",
    type       : "enum",
    values     : [
      {title:"M104", id:"M104"},
      {title:"G10", id:"G10"}
...
function onSection() {
  ...
  writeBlock(gFormat.format(28), "Z"); // homing Z
  writeBlock(gFormat.format(28), "X Y"); // homing XY
  ...
  writeBlock(gMotionModal.format(1), zOutput.format(initialPosition.z));
  forceXYZE();
}
...
function onExtruderTemp(temp, wait, id) {
  ...
  if (wait) {
    if (getProperty("functionSettingTemperature") == "M104") {
      ...
      writeBlock(mFormat.format(116), pFormat.format(id));
    }
  } else {
    if (getProperty("functionSettingTemperature") == "M104") {
...
// <<<<< INCLUDED FROM ../common/onBedTemp.cpi
// include("../common/onExtruderTemp.cpi");  // Specific function needed for that firmware
