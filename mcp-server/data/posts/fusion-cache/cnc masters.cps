/**
  ...
  CNC Masters post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {561A1D04-2834-495C-B3C7-047D948B2F98}
*/

description = "CNC Masters Mill";
vendor = "CNC Masters";
vendorUrl = "https://www.cncmasters.com/";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic post for the CNC Masters mill with the CNC Machine Master MX Software.";
...
allowedCircularPlanes = 1 << PLANE_XY; // circular in XY plane only
...
properties = {
  ...
  useComments: {
    ...
    description: "Enable to allow comments to be output to the NC file.",
    group      : "formats",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  useParkPosition: {
    title      : "Force home position at end",
    description: "Force home position (X0 Y0) at end of program.",
    group      : "homePositions",
    type       : "boolean",
    value      : true,
    scope      : "post"
...
var aOutput = createOutputVariable({prefix:"W"}, abcFormat);
...
function onOpen() {
  ...
  if (receivedMachineConfiguration) {
    if (machineConfiguration.getNumberOfAxes() > 4) {
      error(localize("The CNC Machine Master MX Software only supports a single rotary axis."));
      return;
    }
  }
...
function writeInitialPositioning(position, isRequired, insertToolCall) {
  ...
  writeStartBlocks(isRequired, function() {
    ...
    if (machineConfiguration.isHeadConfiguration()) {
      writeBlock(modalCodes, gMotionModal.format(motionCode.multi),
        xOutput.format(position.x), yOutput.format(position.y), zOutput.format(position.z), feed);
    } else {
      if (insertToolCall) {
        writeBlock(gMotionModal.format(motionCode.single), zOutput.format(position.z), feed);
        writeBlock(modalCodes, gMotionModal.format(motionCode.multi), xOutput.format(position.x), yOutput.format(position.y));
      }
      writeBlock(modalCodes, gMotionModal.format(motionCode.multi), xOutput.format(position.x), yOutput.format(position.y), feed);
  ...
  validate(state.lengthCompensationActive, "Tool length compensation is not active."); // make sure that lenght compensation is enabled
...
function onSection() {
  ...
  // tool change
  writeToolCall(tool, insertToolCall && !isFirstSection());
  ...
  writeInitialPositioning(initialPosition, isRequired, insertToolCall);
    ...
    case "chip-breaking":
      if (cycle.accumulatedDepth < cycle.depth) {
        ...
        writeBlock(
          ...
          getCommonCycle(x, y, z, cycle.chipBreakDistance),
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    if (!getProperty("useToolCall")) {
      var comment = formatComment(localize("CHANGE TO T") + toolFormat.format(tool.number));
  ...
  if (getProperty("useParkPosition")) {
    if (getProperty("safePositionMethod") == "clearanceHeight") {
      writeBlock(
        ...
        xOutput.format(machineConfiguration.hasHomePositionX() ? machineConfiguration.getHomePositionX() : toPreciseUnit(0, MM)),
        yOutput.format(machineConfiguration.hasHomePositionY() ? machineConfiguration.getHomePositionY() : toPreciseUnit(0, MM))
      );
    } else {
