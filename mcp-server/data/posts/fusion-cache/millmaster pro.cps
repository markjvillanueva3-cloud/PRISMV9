...
longDescription = "Generic MicroKinetics MillMaster Pro 2014. You can set the property 'Retract Z level' to a non-zero value to make the machine retract to the given Z-level between operations.";
...
properties = {
  ...
  safePositionMethod: {
    ...
    values     : [
      ...
      {title:"Use safe Z-level", id:"safeZLevel"}
    ],
    value: "safeZLevel",
    scope: "post"
  },
...
var settings = {
  ...
  retract: {
    ...
    useZeroValues             : ["M29"], // enter property value id(s) for using "0" value instead of machineConfiguration axes home position values (ie G30 Z0)
...
function onOpen() {
  ...
  writeStarComment(" {DEFVAR}: TCPX, " + xyzFormat.format(getProperty("toolChangePositionX")) + " ");
  writeStarComment(" {DEFVAR}: TCPY, " + xyzFormat.format(getProperty("toolChangePositionY")) + " ");
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint() || cycleRequiresRetract) {
    if (!isFirstCyclePoint() && !cycleExpanded) {
...
function writeRetract() {
  ...
  if (retract && retract.words.length > 0) {
    ...
    for (var i in retract.words) {
      ...
      case "M29":
        writeBlock(mFormat.format(29), String(words).split("0")[0].toLowerCase(), "(varname)");
        break;
      case "safeZLevel":
        if (getProperty("retractZlevel") != 0) {
          writeBlock(gMotionModal.format(0), "Z" + xyzFormat.format(getProperty("retractZlevel")));
        }
        break;
      default:
