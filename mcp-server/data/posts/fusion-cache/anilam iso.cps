/**
  ...
  Anilam ISO post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {1C5E3999-6726-48ea-A1A1-B9037FEF1B52}
*/

description = "Anilam ISO";
vendor = "ACU-RITE";
...
longDescription = "Generic milling post for Anilam ISO.";
...
properties = {
  ...
  spindleControl: {
    title      : "Direct Spindle Control is supported",
    description: "Determines the type of tapping cycle supported.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
...
wcsDefinitions = {
  useZeroOffset: false,
  wcs          : [
    {name:"Standard", format:"G53 O", range:[1, 99]}
  ]
};
  ...
  if (isFirstCyclePoint()) {
    ...
    case "right-tapping":
      ...
      writeBlock(
        ...
        getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
        getProperty("spindleControl") ? "S1" : "S0",
    ...
    case "fine-boring":
      writeBlock(
        ...
        conditional(P > 0, "D" + secFormat.format(P)),
        conditional(cycle.shiftOrientation != 0, "C" + xyzFormat.format(toDeg(cycle.shiftOrientation)))
      );
      break;
