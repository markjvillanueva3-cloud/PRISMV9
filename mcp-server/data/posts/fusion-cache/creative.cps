/**
  ...
  Creative Evolution post processor configuration.

  $Revision: 44104 165ed1a9c77e5601dab85a96f32381c0b4a83b25 $
  $Date: 2023-12-14 12:10:48 $

  FORKID {6A0E3DDD-DC9F-48bc-AE99-B97410EA44A5}
*/

// ATTENTION: Make sure to set the line 71 setting to Fanuc compatible mode

description = "Creative Evolution";
vendor = "Geonics";
vendorUrl = "http://www.geonicsinc.com";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic milling post for Creative Evolution.";
...
wcsDefinitions = {
  useZeroOffset: false,
  wcs          : [
    {name:"Standard", format:"G99 O", range:[1, 99]}
  ]
};
...
var milliFormat = createFormat({decimals:0}); // milliseconds // range 1-2147418112
...
var gUnitModal = createOutputVariable({}, gFormat); // modal group 6 // G170-171
...
var settings = {
  coolant: {
    ...
    coolants: [
      {id:COOLANT_FLOOD, on:[gFormat.format(4), mFormat.format(8)]},
      {id:COOLANT_MIST, on:[gFormat.format(4), mFormat.format(7)]},
      ...
      {id:COOLANT_OFF, off:[gFormat.format(4), mFormat.format(9)]}
    ],
...
function onOpen() {
  ...
  gPlaneModal.format(17); // default to G17
  ...
  writeProgramHeader();
  writeBlock(gUnitModal.format(unit == MM ? 171 : 170));
  writeBlock(gFormat.format(191)); // incremental arc centers
  writeWCS(getSection(0), true);

  // writeBlock(gFormat.format(398), "A" + xyzFormat.format(diameter)); // set part diameter for mapping feeds for 4-axis machining
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    if (!isFirstSection() && (newWorkOffset || newWorkPlane || tool.manualToolChange)) { // not required on tool change
      writeRetract(Z);
    }
  }
  ...
  if (tool.manualToolChange) {
    writeToolBlock(gFormat.format(40), mFormat.format(56), "T" + toolFormat.format(tool.number)); // G X Y Z1 retract?
  }
  ...
  onCommand(COMMAND_COOLANT_ON); // writes the required coolant codes
...
function onDwell(seconds) {
  var maxValue = 2147418112 / 1000.0;
  if (seconds > maxValue) {
  ...
  milliseconds = clamp(1, seconds * 1000, 2147418112);
  writeBlock(gFormat.format(4), "D" + milliFormat.format(milliseconds));
}
...
function getCommonCycle(x, y, z, retract, clearance) {
  forceXYZ();
  return [xOutput.format(x), yOutput.format(y),
    zOutput.format(clearance),
    "P" + xyzFormat.format(retract),
    "Q" + xyzFormat.format(z)];
}
...
function onCyclePoint(x, y, z) {
  ...
  var P = (cycle.dwell == 0) ? 0 : clamp(1, cycle.dwell * 1000, 2147418112); // in milliseconds
  if (isFirstCyclePoint()) {
    ...
    case "counter-boring":
      ...
      writeBlock(
        ...
        conditional(P > 0, "D" + milliFormat.format(P)),
        feedOutput.format(F)
      );
      break;
    case "chip-breaking":
      if ((cycle.accumulatedDepth < cycle.depth) || (P > 0)) {
        ...
        writeBlock(
          gCycleModal.format(cycleCode),
          getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
          "E" + xyzFormat.format(cycle.incrementalDepth),
          conditional(P > 0, "D" + milliFormat.format(P)),
          feedOutput.format(F)
        );
      }
    ...
    case "deep-drilling":
      ...
      writeBlock(
        ...
        getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
        "E" + xyzFormat.format(cycle.incrementalDepth),
        "W" + xyzFormat.format(toPreciseUnit(0.2, MM)),
        conditional(P > 0, "D" + milliFormat.format(P)),
        feedOutput.format(F)
      );
      break;
    case "tapping":
      cycleCode = (tool.type == TOOL_TAP_LEFT_HAND) ? 83 : 84;
      cycleCode += getProperty("useRigidTapping") ? 0.1 : 0;
      writeBlock(
        gCycleModal.format(cycleCode),
        getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
        conditional(getProperty("useRigidTapping"), "L" + pitchFormat.format(tool.threadPitch)),
        feedOutput.format(F)
      );
      break;
    case "left-tapping":
    ...
    case "right-tapping":
      ...
      writeBlock(
        gCycleModal.format(cycleCode),
        getCommonCycle(x, y, z, cycle.retract, cycle.clearance),
        "L" + pitchFormat.format(tool.threadPitch),
        feedOutput.format(F)
      );
      break;
    case "fine-boring":
      ...
      writeBlock(gFormat.format(4), mFormat.format(tool.clockwise ? 3 : 4));
      gCycleModal.reset();
      break;
    case "stop-boring":
      cycleCode = 86;
    ...
    if (cycleExpanded) {
      ...
      writeBlock(gCycleModal.format(cycleCode), _x, _y, conditional(P > 0, "D" + milliFormat.format(P)));
    }
  }
}
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      ...
      case RADIUS_COMPENSATION_LEFT:
        writeBlock(gFormat.format(41), "T" + toolFormat.format(tool.number));
        writeBlock(gMotionModal.format(1), x, y, z, f);
        break;
      case RADIUS_COMPENSATION_RIGHT:
        writeBlock(gFormat.format(42), "T" + toolFormat.format(tool.number));
        writeBlock(gMotionModal.format(1), x, y, z, f);
        break;
      default:
        writeBlock(gFormat.format(40), "T" + toolFormat.format(tool.number));
        writeBlock(gMotionModal.format(1), x, y, z, f);
      }
    } else {
      writeBlock(gMotionModal.format(1), x, y, z, f);
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  // make sure control has been setup to run in Fanuc compatible mode
  if (pendingRadiusCompensation >= 0) {
...
function onCommand(command) {
  ...
  case COMMAND_START_SPINDLE:
    ...
    writeBlock(
      gFormat.format(4), sOutput.format(spindleSpeed), mFormat.format(tool.clockwise ? 3 : 4)
    );
    return;
  case COMMAND_LOAD_TOOL:
    writeToolBlock(gFormat.format(40), mFormat.format(6), "T" + toolFormat.format(tool.number)); // G X Y Z1 retract?
  ...
  if (mcode != undefined) {
    writeBlock(gFormat.format(4), mFormat.format(mcode));
  } else {
...
function writeRetract() {
  ...
  if (retract && retract.words.length > 0) {
    ...
    for (var i in retract.words) {
      ...
      case "G100":
        gMotionModal.reset();
        writeBlock(gFormat.format(100), words);
        break;
      default:
...
function onClose() {
  ...
  writeBlock(gFormat.format(4), mFormat.format(30)); // stop program, spindle stop, coolant off
  writeln("%");
}
