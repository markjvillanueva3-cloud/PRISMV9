...
*/
  ...
  if (insertToolCall) {
    // writeBlock("T" + toolFormat.format(tool.number) + "|" + rpmFormat.format(spindleSpeed) + "|" + getToolTypeName(tool.type));
    writeBlock("M" + toolFormat.format(tool.number) + "|" +  "D" + xyzFormat.format(tool.diameter) + " " + getToolTypeName(tool.type) + "<" + rpmFormat.format(spindleSpeed) + " R.P.M>");
  }
  ...
  // writeBlock("A" + xyzFormat.format(initialPosition.x), xyzFormat.format(initialPosition.y), zFormat.format(getCurrentPosition().z));
