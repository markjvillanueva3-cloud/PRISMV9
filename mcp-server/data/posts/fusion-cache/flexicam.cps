...
longDescription = "Generic milling post for FlexiCAM.";
...
var gFeedModeModal = createOutputVariable({}, gFormat); // modal group 5 // 94
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  case PLANE_XY:
    ...
    break;
  case PLANE_ZX:
    writeBlock(gPlaneModal.format(18), gMotionModal.format(clockwise ? 3 : 2), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x), kOutput.format(cz - start.z), feedOutput.format(feed));
    break;
  case PLANE_YZ:
    writeBlock(gPlaneModal.format(19), gMotionModal.format(clockwise ? 3 : 2), xOutput.format(x), yOutput.format(y), zOutput.format(z), jOutput.format(cy - start.y), kOutput.format(cz - start.z), feedOutput.format(feed));
    break;
  default:
    var t = getParameter("operation:tolerance", tolerance);
    linearize(t);
...
function onCommand(command) {
  ...
  case COMMAND_START_SPINDLE:
    ...
    writeBlock(sOutput.format(spindleSpeed));
    validate(tool.clockwise, "Counterclockwise spindle rotation direction is not supported.");
    return;
