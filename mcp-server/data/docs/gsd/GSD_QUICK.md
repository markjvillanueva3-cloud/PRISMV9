# PRISM Quick Reference v23.2
## 83 dispatchers | 4296 verified actions | 101 cadence auto-functions | 1592 engines | 220 hooks
...
## DECISION TREE — What Tool For What Task (83 dispatchers)
### Core Manufacturing
...
## AI-FIRST REASONING — prism_ai (87 actions)
AI reasoning → prism_ai (87 actions) — USE FOR ALL COMPLEX WORK

### Core Reasoning (12)
reason, speed_feed, tool_select, sequence, strategy, quote, resolve_error, safety_check, feasibility, process_plan, gcode, stats

### Multi-Agent (14)
register_session, update_session, end_session, list_sessions, execute_chain, claim_chain, release_chain, share_chain, query_chains, get_chain, token_status, allocate_tokens, get_activity, detect_conflicts

### Chain Sharing (7)
register_reasoning_chain, share_reasoning_chain, find_similar_chain, validate_chain, query_shared_chains, extract_tribal, chain_sharing_stats

### Opus Capability (6)
opus_execute, opus_assess_complexity, opus_validate_physics, opus_translate_nl, opus_generate_hypotheses, opus_stats

### Neural Bridge (8)
neural_index_entity, neural_batch_index, neural_semantic_search, neural_find_similar, neural_graph_reasoning, neural_detect_gaps, neural_infer_relations, neural_stats

### Agent Profiles (10)
profile_register, profile_update, profile_get, profile_list, profile_remove, profile_match, profile_best, team_compose, team_get, profile_stats

### Chain-of-Thought + Advanced (30)
cot_reason, cot_adversarial, cot_tree_search, cot_stats, uncertainty_analytical, uncertainty_monte_carlo, uncertainty_sobol, uncertainty_stats, learning_update, learning_status, learning_patterns, learning_calibration, learning_stats, decision_select, decision_pareto, decision_sensitivity, decision_stats, roadmap_intelligence, roadmap_diagnose, roadmap_explain, roadmap_stats, proactive_analyze, proactive_warnings, proactive_suggestions, proactive_opportunities, proactive_blockers, proactive_dependencies, proactive_status, proactive_subscribe, proactive_stats

### When to Use:
| Situation | Action |
|-----------|--------|
| Complex reasoning | opus_execute category=deep_reasoning |
| Physics validation | opus_validate_physics |
| Past solutions | neural_semantic_search |
| Multi-step logic | cot_reason or cot_tree_search |
| Uncertainty | uncertainty_monte_carlo |
| Team for task | team_compose |
| Extract learning | extract_tribal |
...
|----------|----------|---------|
| Build | `protocols/BUILD_PROTOCOL.md` | Build commands, verification, CI integration |
| Schema Versioning | `protocols/SCHEMA_VERSIONING_PROTOCOL.md` | SemVer, migrations, entity versions |
| Orchestration | `protocols/ORCHESTRATION_PROTOCOL.md` | Distributed locking, plugin registration |

## SCHEMA VERSIONING (Summary)
- Current version: `2.0.0` (in `src/schemas/schemaVersioning.ts`)
- All state files MUST have `schemaVersion` field
- Breaking changes require migration in `src/migrations/`
- Entity versions: material=3.0.0, machine=2.5.0, tool=2.0.0, alarm=1.5.0

## DISTRIBUTED LOCKING (Summary)
- Lock manager: `src/orchestration/DistributedLockManager.ts`
- Pattern: `withLock(resourceId, async () => { ... })`
- Default timeout: 30s (configurable per resource)
- Required for: orchestration dispatchers, state mutations

## Changelog
- 2026-04-12: v23.1 — Added protocol reference section (BUILD, SCHEMA_VERSIONING, ORCHESTRATION)
