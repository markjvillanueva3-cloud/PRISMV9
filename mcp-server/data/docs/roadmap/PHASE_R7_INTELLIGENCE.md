...
### RECOMMENDED_SKILLS: prism-master-equation, prism-material-physics, prism-cutting-mechanics, prism-process-optimizer
### HOOKS_EXPECTED: ALL
### DATA_PATHS: C:\PRISM\data\materials, C:\PRISM\mcp-server\src\engines

<!-- ANCHOR: r7_wire_extracted_engines_algorithms_mit_courses_into_live_manufacturing_intelligence -->
## Wire Extracted Engines, Algorithms & MIT Courses Into Live Manufacturing Intelligence
<!-- ANCHOR: r7_v14_2_prerequisites_r1_registries_loaded_r3_campaign_actions_live -->
## v14.2 | Prerequisites: R1 (registries loaded), R3 (campaign actions live)
# DEPENDS ON: R1 complete (registries loaded), R3 complete (campaign actions live), R2 complete (safety validation)

---

<!-- ANCHOR: r7_quick_reference_standalone_after_compaction_no_other_doc_needed -->
...
<!-- ANCHOR: r7_knowledge_contributions_what_this_phase_feeds_into_the_hierarchical_index -->
...
<!-- ANCHOR: r7_execution_model -->
### EXECUTION MODEL
```
Environment: Hybrid â€” Claude Code (catalog extraction, wiring) + Claude.ai MCP (physics design)
Model: Haiku (catalog parsing/grep) â†’ Sonnet (wiring) â†’ Opus (coupled physics modeling)

PARALLEL EXECUTION:
- R7-MS6 (catalog extraction): BACKGROUND AGENTS â€” this takes hours
...
- R7-MS0 (coupled physics): Claude.ai MCP with Opus
  ...
  Do NOT attempt with Sonnet â€” physics model design is not implementation.
```


<!-- ANCHOR: r7_phase_objective -->
## PHASE OBJECTIVE
...
manufacturer catalogs** into the live MCP server â€” transforming PRISM from a calculation engine
into an **intelligence platform** that learns, predicts, optimizes, and recommends.

**Data Utilization Target**: 22% (current) â†’ 65% (after R1) â†’ **90%+** (after R7)

âš¡ **CLAUDE CODE:** R7 involves wiring 495+ JS modules â€” bulk file reading, parsing, and integration
...
<!-- ANCHOR: r7_dependency_map -->
## DEPENDENCY MAP

```
R1 (Registry Loading) â”€â”€â”
                         â”œâ”€â”€ R3 (Campaign Actions) â”€â”€ R7 (Intelligence Evolution)
R2 (Safety Validation) â”€â”€â”˜
```
...
<!-- ANCHOR: r7_ms0_physics_informed_predictions_surface_integrity_thermal_chatter -->
## MS0: PHYSICS-INFORMED PREDICTIONS (Surface Integrity, Thermal, Chatter)
<!-- ANCHOR: r7_role_intelligence_architect_model_opus_physics_models_formulas_sonnet_engine_wiring_effort_l_20_calls_sessions_1_2 -->
### Role: Intelligence Architect | Model: Opus (physics models + formulas) â†’ Sonnet (engine wiring) | Effort: L (20 calls) | Sessions: 1-2
<!-- ANCHOR: r7_sessions_1_effort_l_prerequisites_r1_ms5_materials_loaded_r3_ms1_advanced_calcs -->
...
<!-- ANCHOR: r7_source_assets_to_wire -->
...
2. Calculate cutting forces via Merchant/Kienzle (ManufacturingCalculations.ts â€” already exists)
3. Calculate heat generation from force Ã   velocity (PRISM_CUTTING_THERMAL_ENGINE.js)
...
**Failure handling**: If Johnson-Cook data missing for material â†’ fall back to empirical
...
**Failure handling**: If machine dynamics unknown â†’ use generic stiffness by machine type
(VMC/HMC/lathe/5-axis). Flag result as "estimated â€” tap test recommended for accuracy."
...
<!-- ANCHOR: r7_test_protocol -->
### Test Protocol
| Test | Input | Expected | Source |
|------|-------|----------|--------|
| Ra prediction Ti-6Al-4V | Vc=60, f=0.15, ap=2.0, carbide, flood | Ra â   ¤ 1.6Î¼m | Machining Data Handbook |
| Chatter stability 20mm endmill | 4-flute, 60mm overhang, BT40, 8000rpm | stable=true for aeâ   ¤4mm | Altintas textbook example |
| White layer Inconel 718 | Vc=80 (too fast), ceramic, dry | white_layer.risk=true | Published research |
| Thermal growth VMC after 30min | 10,000rpm, 7.5kW, 22Â  C ambient | Z offset 15-25Î¼m | Typical VMC specification |

<!-- ANCHOR: r7_validation -->
### Validation
- S(x) â   ¥ 0.70 on all predictions (uncertainty bounds must be included)
...
<!-- ANCHOR: r7_coupled_physics_models_v14_2_gap_6_the_real_breakthrough -->
### COUPLED PHYSICS MODELS (v14.2 â€” Gap 6, the real breakthrough)
...
F-HYB-005: Force-Thermal Coupling
  Higher cutting force â†’ more heat generation â†’ faster tool wear
  Iterate until force-temperature-wear converge (typically 3-5 iterations)

F-HYB-006: Wear-Surface Coupling
  As tool wears (flank wear VB increases) â†’ Ra increases nonlinearly
  Fresh tool Ra=0.4Î¼m â†’ worn tool Ra=1.2Î¼m for same parameters

F-HYB-007: Vibration-Wear Coupling
  Chatter events destroy tools 3x faster than stable cutting
  If stability lobe predicts marginal stability â†’ apply 3x wear factor
...
F-HYB-017: Unified Machining Model (THE key model)
  ...
  Material + Tool + Machine + Operation â†’
    Force â†’ Temperature â†’ Wear â†’ Surface Finish â†’ Dimensional Accuracy
...
unified_machining_model(material, tool, machine, operation)
  â†’ Runs ALL couplings simultaneously for integrated prediction
  â†’ Returns: { force, temperature, wear_rate, surface_finish, dimensional_accuracy,
               convergence_iterations, coupling_sensitivities }
  â†’ Safety: S(x) computed on the COUPLED result, not individual models

coupling_sensitivity(model_result, parameter)
  â†’ Shows which input most affects each output
  â†’ "Increasing speed 10% increases temperature 15% but only wear 8%"
  â†’ Enables what_if analysis on the coupled model
```

**CONVERGENCE HOOK:**
```
coupled_physics_convergence â€” blocking, post-calculation
  Checks: did the iterative solver converge?
  If iterations > 20 or residual > threshold â†’ BLOCK result
...
coupled_physics_divergence_alert â€” warning
  If sensitivity analysis shows >50% output change for <5% input change â†’ ALERT
  Indicates numerically unstable regime â€” recommend different parameters
```

---

<!-- ANCHOR: r7_ms1_constrained_optimization_cost_quality_productivity_tradeoffs -->
## MS1: CONSTRAINED OPTIMIZATION (Cost, Quality, Productivity Tradeoffs)
<!-- ANCHOR: r7_role_intelligence_architect_model_opus_optimization_algorithm_sonnet_impl_effort_l_18_calls_sessions_1 -->
### Role: Intelligence Architect | Model: Opus (optimization algorithm) â†’ Sonnet (impl) | Effort: L (18 calls) | Sessions: 1
<!-- ANCHOR: r7_sessions_1_effort_l_prerequisites_r3_ms0_job_plan_r7_ms0_physics_predictions -->
...
2. Define constraints from input + safety limits (S(x) â   ¥ 0.70)
...
**Source**: PRISM_ACO_SEQUENCER.js (197KB) â€” ant colony optimization for sequencing.

<!-- ANCHOR: r7_test_protocol -->
### Test Protocol
| Test | Input | Expected |
|------|-------|----------|
| Optimize pocket in 4140 steel | Raâ   ¤3.2, tol Â  0.05, min_cost | Vc within handbook range, cost < brute force |
| Optimize face mill Inconel 718 | Raâ   ¤0.8, min_time | Selects ceramic insert at high Vc |
| Sequence 8-operation part | 3 tools, 2 setups | Reduces tool changes by â   ¥30% vs input order |

<!-- ANCHOR: r7_sustainability_esg_metrics_v14_2_gap_3 -->
### SUSTAINABILITY / ESG METRICS (v14.2 â€” Gap 3)
...
sustainability: {
  ...
  chip_waste_kg: number,     // from MRR Ã   material density Ã   time
...
sustainability_report(job_plan_result)
  â†’ Full environmental impact of a machining job
  â†’ Energy, CO2, coolant, waste, water, tool material usage
  â†’ Comparison vs "typical" parameters (baseline from registry)
  â†’ ISO 14955 compliance notes where applicable

eco_optimize(job_plan, weight_eco: 0.0-1.0)
  â†’ Rebalance optimization with sustainability weight
  â†’ weight_eco=0.0 â†’ pure cost/quality/productivity optimization
  â†’ weight_eco=1.0 â†’ minimize environmental impact (may be slower/costlier)
  â†’ weight_eco=0.3 â†’ recommended balance (30% eco consideration)
```
...
**NEW HOOK:**
  sustainability_bounds â€” warning, post-optimization
...
<!-- ANCHOR: r7_ms2_workholding_intelligence_fixture_selection_clamping_force_deflection -->
## MS2: WORKHOLDING INTELLIGENCE (Fixture Selection, Clamping Force, Deflection)
<!-- ANCHOR: r7_role_intelligence_architect_model_sonnet_fixture_logic_opus_clamping_force_safety_review_effort_m_12_calls_sessions_1 -->
### Role: Intelligence Architect | Model: Sonnet (fixture logic) â†’ Opus (clamping force safety review) | Effort: M (12 calls) | Sessions: 1
<!-- ANCHOR: r7_sessions_1_effort_m_prerequisites_r1_material_machine_data_loaded -->
...
and provide intelligent workholding recommendations â€” not just "use a vise" but "use a
...
| WorkholdingEngine.ts | mcp-server/src/engines/ | 46KB | **Already live** â€” has physics, needs data |

<!-- ANCHOR: r7_implementation -->
### Implementation

**New data file**: `data/workholding/FIXTURES.json` â€” merged from extracted databases
...
2. Determine minimum clamping force: F_clamp > (safety_factor Ã   F_cutting) / Î¼
...
| Test | Input | Expected |
|------|-------|----------|
| Small prismatic part, 6061-T6 | 100Ã  50Ã  25mm, Fc=800N, tol Â  0.02 | Vise, 3500N clamp, defl < 0.01mm |
| Large plate, mild steel | 500Ã  300Ã  12mm, Fc=2000N, tol Â  0.1 | Fixture plate with toe clamps |
| Round bar, Inconel 718 | Ã˜50Ã  200mm, turning, Fc=3000N | 3-jaw chuck, 6000N+ grip, soft jaws |

---

<!-- ANCHOR: r7_ms3_learning_from_jobs_accumulate_analyze_improve_recommendations -->
## MS3: LEARNING FROM JOBS (Accumulate, Analyze, Improve Recommendations)
<!-- ANCHOR: r7_role_intelligence_architect_model_sonnet_learning_pipeline_impl_effort_m_12_calls_sessions_1 -->
### Role: Intelligence Architect | Model: Sonnet (learning pipeline impl) | Effort: M (12 calls) | Sessions: 1
<!-- ANCHOR: r7_sessions_1_effort_l_prerequisites_r3_ms0_job_plan_generating_structured_outputs -->
...
intelligence** â€” the system gets smarter with use.
...
| MemoryGraphEngine.ts | mcp-server/src/engines/ | exists | **Already live** â€” use for persistence |

<!-- ANCHOR: r7_implementation -->
### Implementation

**New file**: `src/engines/JobLearningEngine.ts`
**New data directory**: `data/job_history/` â€” structured job outcome storage
...
**Learning pipeline**:
1. job_plan generates parameters â†’ user runs the job â†’ user records outcome via job_record
...
| Test | Expected |
|------|----------|
| Record 10 identical jobs, vary tool life Â  20% | job_insights identifies mean and std dev |
...
<!-- ANCHOR: r7_ms4_advanced_algorithms_mit_course_integration_numerical_methods -->
## MS4: ADVANCED ALGORITHMS (MIT Course Integration, Numerical Methods)
<!-- ANCHOR: r7_role_intelligence_architect_model_opus_algorithm_selection_sonnet_integration_effort_m_12_calls_sessions_1 -->
### Role: Intelligence Architect | Model: Opus (algorithm selection) â†’ Sonnet (integration) | Effort: M (12 calls) | Sessions: 1
<!-- ANCHOR: r7_sessions_1_effort_m_prerequisites_r7_ms0_physics_engines_wired -->
...
and computational geometry â€” most of which are directly applicable to manufacturing problems.
...
| PRISM_COURSE_GATEWAY_GENERATOR.js | extracted/mit/ | varies | Route generator for coursesâ†’engines |
...
<!-- ANCHOR: r7_implementation_strategy -->
...
algorithms to namespaces â€” we register the manufacturing-relevant ones.
...
| FFT / spectral analysis | MIT 6.003 | Chatter frequency identification (â†’ MS0) |
| Gradient descent + Newton | MIT 6.079 | Parameter optimization (â†’ MS1) |
| Bayesian inference | MIT 6.041 | Learning from jobs (â†’ MS3) |
...
| Kalman filter (EKF) | MIT 6.832 | Adaptive machining â€” estimate tool wear in real-time |
...
<!-- ANCHOR: r7_ms5_shop_floor_optimization_scheduling_machine_assignment_utilization -->
## MS5: SHOP FLOOR OPTIMIZATION (Scheduling, Machine Assignment, Utilization)
<!-- ANCHOR: r7_role_intelligence_architect_model_opus_scheduling_algorithm_sonnet_impl_effort_l_15_calls_sessions_1 -->
### Role: Intelligence Architect | Model: Opus (scheduling algorithm) â†’ Sonnet (impl) | Effort: L (15 calls) | Sessions: 1
<!-- ANCHOR: r7_sessions_1_effort_l_prerequisites_r1_machine_capabilities_loaded -->
...
interface ShopScheduleResult {
  ...
  bottlenecks: string[];                 // "Machine X is overloaded â€” consider outsourcing op Y"
}
```
...
<!-- ANCHOR: r7_ms6_manufacturer_catalog_extraction_9_7gb_structured_data -->
## MS6: MANUFACTURER CATALOG EXTRACTION (9.7GB â†’ Structured Data)
<!-- ANCHOR: r7_role_data_architect_model_haiku_bulk_pdf_parsing_116_catalogs_sonnet_normalization_effort_xl_30_calls_sessions_2_3 -->
### Role: Data Architect | Model: Haiku (bulk PDF parsing 116 catalogs) â†’ Sonnet (normalization) | Effort: XL (30+ calls) | Sessions: 2-3
<!-- ANCHOR: r7_sessions_1_2_effort_xl_prerequisites_none_independent_track -->
...
<!-- ANCHOR: r7_source_assets -->
### Source Assets
```
C:\PRISM\CATALOGS\                      44 PDFs, 9.7GB total
Key catalogs:
- Sandvik Coromant (Drilling, Milling, Turning, Tooling) â€” 4 files
- ISCAR Part 1 â€” cutting tools
- Kennametal Master Catalog â€” 2 volumes
- OSG â€” taps and drills
- SGS Global Catalog â€” solid carbide
- Guhring â€” full catalog + tool holders
- Emuge â€” threading
- MA Ford â€” end mills
- REGO-FIX â€” toolholding
...
1. **Phase 1 (MS6a)**: Build the extraction pipeline
   - PDF â†’ text extraction (per page)
...
**New data directory**: `data/tools/catalogs/` â€” per-manufacturer extracted JSON
...
| Test | Expected |
|------|----------|
| Extract 1 page of Sandvik milling catalog | Valid JSON with â   ¥5 tool entries |
| Cross-validate extracted data against known Sandvik specs | â   ¥95% accuracy |
...
<!-- ANCHOR: r7_novel_differentiators_enabled_by_r7 -->
...
<!-- ANCHOR: r7_1_adaptive_parameters_based_on_machine_condition -->
...
- This is currently done by expensive testing â€” PRISM does it computationally

<!-- ANCHOR: r7_4_self_improving_recommendations -->
...
<!-- ANCHOR: r7_5_thread_engine_expansion -->
...
<!-- ANCHOR: r7_6_intelligent_alarm_diagnosis -->
...
- Predict alarms before they occur (spindle overload trending upward â†’ warn operator)

---

<!-- ANCHOR: r7_cross_reference_r7_actions_source_assets_engines -->
## CROSS-REFERENCE: R7 ACTIONS â†’ SOURCE ASSETS â†’ ENGINES
...
<!-- ANCHOR: r7_phase_gate_r7_complete -->
## PHASE GATE: R7 â†’ COMPLETE

<!-- ANCHOR: r7_definition_of_done -->
### Definition of Done
- [ ] All 11 actions respond correctly to valid inputs
- [ ] S(x) â   ¥ 0.70 on ALL predictions (surface integrity, chatter, thermal)
...
- [ ] Î© â   ¥ 0.70 for overall R7 quality
- [ ] At least 1 manufacturer catalog extracted and loaded into ToolRegistry
- [ ] Data utilization metric â   ¥ 85% (measured against ASSET_INVENTORY.md baseline)

<!-- ANCHOR: r7_fault_injection_tests -->
...
<!-- ANCHOR: r7_estimated_total_effort -->
...
<!-- ANCHOR: r7_r7_companion_assets_v14_5_built_per_ms_verified_at_r7_gate -->
## R7 COMPANION ASSETS (v14.5 -- built per-MS, verified at R7 gate)

<!-- ANCHOR: r7_per_ms_companion_schedule -->
### PER-MS COMPANION SCHEDULE:
``r
MS0 PRODUCES:
  SKILL: prism-coupled-physics (unified machining model, convergence troubleshooting)
  HOOK: coupled_physics_convergence (blocking, post-calc, verifies solver converged)

MS1 PRODUCES:
  SKILL: prism-sustainability-advisor (ESG metrics, ISO 14955, eco-optimization)
  HOOK: sustainability_bounds (warning, alerts if eco_efficiency < 0.3)

MS2 PRODUCES:
  SKILL: prism-workholding-guide (fixture selection, clamping force interpretation)

MS3 PRODUCES:
  HOOK: learning_data_quality (warning, validates recorded job data quality)

MS4 PRODUCES: (algorithm integration -- knowledge captured in existing course-derived skills)

MS5 PRODUCES:
  SKILL: prism-scheduling-advisor (shop floor scheduling, machine assignment guidance)

MS6 PRODUCES:
  SCRIPT: catalog_extraction_report (progress + quality metrics for catalog parsing)

GATE VERIFIES:
  PARALLEL TRACK CHECK: SKILL_INDEX.json count >= 3500 (all tiers complete)
  Cross-reference check: no duplicate skills between course-derived and split-derived
  Skill dependency graph complete and validated
  All hooks fire, all 4 skills load, scripts run, SKILL_INDEX count >= expected
``r

<!-- ANCHOR: r7_r7_companion_assets_detail_v14_2_build_after_r7_gate_passes -->
## R7 COMPANION ASSETS DETAIL (v14.2 â€” build AFTER R7 gate passes)

```
HOOKS (3 new â€” plus coupled_physics hooks from MS0):
  sustainability_bounds       â€” warning, post-optimization, alerts if eco_efficiency < 0.3
  optimization_convergence    â€” blocking, post-optimize, checks optimizer converged
  learning_data_quality       â€” warning, post-job-record, validates recorded job data quality

SCRIPTS (2 new):
  coupled_physics_demo        â€” Runs unified_machining_model on 5 representative scenarios.
                                Shows convergence behavior and coupling sensitivities.
  sustainability_comparison   â€” Compares eco metrics for conventional vs optimized params
                                across 3 materials. Shows energy/coolant/CO2 savings.

SKILLS (2 new):
  prism-sustainability-advisor â€” Teaches Claude ESG metrics, ISO 14955, eco-optimization
                                 tradeoffs. Maps SUSTAINABILITY formulas to use cases.
  prism-coupled-physics        â€” Teaches Claude the unified machining model, when to use
