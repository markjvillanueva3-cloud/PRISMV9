...
### RECOMMENDED_SKILLS: prism-anti-regression, prism-safety-framework, prism-perf-patterns
### HOOKS_EXPECTED: ALL (production = every hook active)
### DATA_PATHS: C:\PRISM\mcp-server

# Status: not-started | Sessions: 2-3 | Role: Production Safety Officer
# DEPENDS ON: R4 complete (API), R5 complete (UI), R2 complete (safety baselines for load validation)
...
# v13.3: Test suite growth path documented (R2â†’R3â†’R4â†’R5â†’R6 cumulative npm test).
...
<!-- ANCHOR: r6_quick_reference_standalone_after_compaction_no_other_doc_needed -->
...
SAFETY:     S(x) >= 0.70 is HARD BLOCK. R6 elevates to   >= 0.85 for production.
...
ENV:        R6 = Claude Code 80% + MCP 20% | Sonnetâ†’Opus. SRE. After R8.
```

---

<!-- ANCHOR: r6_knowledge_contributions_what_this_phase_feeds_into_the_hierarchical_index -->
...
FAULT INJECTION SUITE (XA-13    R6 runs ALL fault tests, not just one):
  Run ALL prior phase fault tests (R1-R8) under production-like conditions:
    R1 test: Registry unavailable â†’ Tier 2 degradation
    R2 test: NaN input â†’ safety block
    R3 test: Batch task timeout â†’ partial results
    R4 test: Tenant disconnect â†’ no data leakage
    R5 test: Malformed API data â†’ error display
    R7 test: Coupled physics divergence â†’ fallback to uncoupled with warning
    R8 test: Intent engine misparse â†’ safe default (explicit mode), no silent wrong action
  PLUS R6-specific fault tests:
    Dispatcher death mid-calculation â†’ graceful error, no operator-visible garbage
    Memory pressure (heap > 3GB) â†’ system remains responsive, degrades gracefully
    Concurrent write contention â†’ atomicWrite prevents corruption

---

<!-- ANCHOR: r6_mandatory_requirements_brainstorm_must_include_all_of_these -->
...
1. strictNullChecks: ALREADY ENABLED from P0 (SL-4). Verify no @ts-expect-error comments remain.
   If any remain from P0 â†’ fix them in R6-MS0. Zero tolerance for null-unsafe code in production.
...
10. SAFETY-SCORE-UNDER-LOAD VALIDATION (SL-5):
    ...
    Delta must be < 0.02. If > 0.02 â†’ investigate if adaptive thinking degrades under pressure.
...
<!-- ANCHOR: r6_stub_objectives_expand_at_phase_start_using_phase_template_md -->
...
2. Stress test: 1000+ concurrent requests with MIXED query patterns (v14.5 QA-3):
...
<!-- ANCHOR: r6_test_suite_growth_path_how_npm_test_evolves_from_r2_r6 -->
## TEST SUITE GROWTH PATH (how npm test evolves from R2 â†’ R6)

```
The test suite is NOT static. Each phase adds tests. By R6, npm test is the FULL regression gate.

R2-MS3 CREATES (baseline):
  src/__tests__/safetyMatrix.test.ts    50 calcs (10 materials Ã— 5 operations)
...
R6 RUNS ALL OF THE ABOVE:
  npm test â†’ vitest run â†’ ALL test files execute â†’ ALL must pass â†’ build gate.
  If ANY test fails â†’ build FAILS â†’ cannot deploy to production.
  This now covers the COMPLETE system: data â†’ calcs â†’ intelligence â†’ UX â†’ production.
...
<!-- ANCHOR: r6_milestone_assignments_apply_during_brainstorm_to_ship_expansion -->
...
<!-- ANCHOR: r6_tool_anchors_concrete_starting_points_for_brainstorm_to_ship_expansion -->
## TOOL ANCHORS (concrete starting points for Brainstorm-to-Ship expansion)

```
MS0    FULL REGRESSION:
  npm test  â†’ ALL tests from R2+R3+R4+R5 must pass.
  prism_dev action=build target=mcp-server  [effort=medium]
  If any test fails â†’ fix before proceeding. This is the gate.
...
MS1    STRESS TEST:
  ...
  prism_orchestrate action=swarm_execute pattern="parallel_batch"  [effort=max]
    tasks=[1000 Ã— random material/operation combinations]
  ...
  For EACH of the 10 R2 baseline materials (4140, 1045, D2, 316SS, Ti-6Al-4V, 6061-T6, GG25, C360, 718, 2205):
    ...
    Delta must be < 0.02. If > 0.02 â†’ STOP DEPLOYMENT.
...
MS2    SECURITY AUDIT:
  ...
  â†’ Find all input access points. Verify each has validation (zod/joi schema).
...
MS3    PERFORMANCE BASELINE:
  For each safety calc (speed_feed, cutting_force, tool_life, spindle_speed):
    Run 100 iterations â†’ measure mean response time.
...
MS4    OPUS 4.6 FEATURE AUDIT:
  ...
  â†’ Each must match expected pattern. No deprecated patterns found.
...
MS5    AGENT TEAMS VALIDATION (parallel vs sequential diff):
  ...
  Parallel execution may reorder operations: (a+b)+c â‰  a+(b+c).
  ...
  Compare: every field within tolerance â†’ PASS. Any field beyond tolerance â†’ debug.

QUALITY GATE:
  prism_ralph action=assess target="R6 Production Hardening"  [effort=max]  â†’ Ralph >= A
  prism_omega action=compute target="R6 complete"  [effort=max]  â†’ Omega >= 0.85
```

<!-- ANCHOR: r6_opus_4_6_patterns_for_r6 -->
...
FULL OPUS 4.6 FEATURE AUDIT: Verify all features configured in P0-MS0 still active.
  Compaction API: âœ“ still triggering correctly
  Adaptive Thinking: âœ“ effort tiers applied correctly
  Structured Outputs: âœ“ all safety calcs schema-validated
  Agent Teams: âœ“ parallel execution producing same results as sequential
  Fine-grained Streaming: âœ“ large outputs stream correctly
  Data Residency: âœ“ ITAR calcs route to US inference
  Context Editing: âœ“ tool result clearing functioning
  Prefilling: âœ“ no assistant prefilling anywhere in codebase
...
<!-- ANCHOR: r6_additional_exit_criteria_v14_3 -->
## ADDITIONAL EXIT CRITERIA (v14.3)

<!-- ANCHOR: r6_plugin_packaging_gate -->
...
<!-- ANCHOR: r6_r6_companion_assets_v14_5_built_per_ms_verified_at_r6_ms5_gate -->
## R6 COMPANION ASSETS (v14.5 -- built per-MS, verified at R6-MS5 gate)

<!-- ANCHOR: r6_per_ms_companion_schedule -->
### PER-MS COMPANION SCHEDULE:
```
MS0 PRODUCES:
  SCRIPT: performance_baseline (captures response times, memory, throughput baseline)

MS1 PRODUCES:
  HOOK: fault_recovery_verify (blocking, post-fault-injection, verifies clean recovery)

MS2 PRODUCES:
  HOOK: safety_degradation_alert (blocking, continuous, alerts if S(x) drops under load)
  SKILL: prism-production-runbook (deployment, monitoring, incident response)

MS3 PRODUCES:
  SKILL: prism-alert-interpretation (alert thresholds, escalation, root cause)

MS4 PRODUCES:
  HOOK: deployment_rollback_trigger (blocking, post-deploy, auto-rollback on failure spike)
  SCRIPT: load_test_runner (automated load test with S(x) verification)
