# PHASE R3: INTELLIGENCE + DATA CAMPAIGNS — v14.2
# Status: not-started | Sessions: 4-6 | MS: 7 (MS0-MS5, with MS0.5 new) | Role: Principal Architect
# DEPENDS ON: R1 complete (data foundation), R2 complete (safety validated, golden values locked)
...
<!-- ANCHOR: r3_quick_reference_standalone_after_compaction_no_other_doc_needed -->
...
<!-- ANCHOR: r3_knowledge_contributions_what_this_phase_feeds_into_the_hierarchical_index -->
...
AT PHASE GATE: RELATIONSHIP_GRAPH.json grows by 200+ edges. DATA_TAXONOMY.json updated.
```

---

<!-- ANCHOR: r3_execution_model -->
### EXECUTION MODEL
```
...
<!-- ANCHOR: r3_mandatory_requirements_brainstorm_must_include_all_of_these_from_v13_9 -->
...
<!-- ANCHOR: r3_r3_ms0_job_planner_setup_sheet -->
## R3-MS0: Job Planner + Setup Sheet
<!-- ANCHOR: r3_role_systems_architect_model_opus_cross_system_design_then_sonnet_impl_effort_l_14_calls_sessions_1_2 -->
### Role: Systems Architect | Model: Opus (cross-system design) then Sonnet (impl) | Effort: L (14 calls) | Sessions: 1-2
...
1a. Design the job_plan action handler (add to dataDispatcher or new intelligenceDispatcher):
    ...
    INPUT: {
      ...
      target_Ra?: number,         //  m — surface finish target (triggers finishing pass)
    ...
    ```json
    {
      "job_plan": {
        ...
        "gcode_header": "G90 G54\### RECOMMENDED_SKILLS: prism-formula-evolution, prism-cam-strategies, prism-combination-engine, prism-signal-processing\n### HOOKS_EXPECTED: DISPATCH, FILE, STATE, CALC, FORMULA, INTEL\n### DATA_PATHS: C:\PRISM\data\materials\, C:\PRISM\mcp-server\src\engines\\nnT1 M6\nG43 H1\nM3 S796\nM8\nG0 X0 Y0\nG0 Z5.0\n...",
...
<!-- ANCHOR: r3_r3_ms0_5_formula_registry_integration_new_in_v14_2 -->
## R3-MS0.5: FORMULA REGISTRY INTEGRATION ← NEW in v14.2
<!-- ANCHOR: r3_role_systems_architect_model_sonnet_wiring_then_opus_integration_validation_effort_m_10_calls_sessions_1 -->
### Role: Systems Architect | Model: Sonnet (wiring) then Opus (integration validation) | Effort: M (10 calls) | Sessions: 1
...
<!-- ANCHOR: r3_r3_ms1_advanced_calculations -->
## R3-MS1: Advanced Calculations
<!-- ANCHOR: r3_role_safety_engineer_model_opus_formula_verification_safety_critical_effort_m_12_calls_sessions_1 -->
### Role: Safety Engineer | Model: Opus (formula verification, SAFETY-CRITICAL) | Effort: M (12 calls) | Sessions: 1
...
<!-- ANCHOR: r3_r3_ms2_toolpath_intelligence -->
## R3-MS2: Toolpath Intelligence
<!-- ANCHOR: r3_role_intelligence_architect_model_sonnet_impl_then_opus_strategy_selection_logic_effort_m_8_calls_sessions_1 -->
### Role: Intelligence Architect | Model: Sonnet (impl) then Opus (strategy selection logic) | Effort: M (8 calls) | Sessions: 1
...
<!-- ANCHOR: r3_r3_ms3_cross_system_intelligence -->
## R3-MS3: Cross-System Intelligence
<!-- ANCHOR: r3_role_intelligence_architect_model_opus_cross_system_chain_design_then_sonnet_impl_effort_m_12_calls_sessions_1_2 -->
### Role: Intelligence Architect | Model: Opus (cross-system chain design) then Sonnet (impl) | Effort: M (12 calls) | Sessions: 1-2
...
<!-- ANCHOR: r3_r3_ms4_data_enrichment_campaigns_workholding_alarms -->
## R3-MS4: Data Enrichment Campaigns + Workholding + Alarms
<!-- ANCHOR: r3_role_data_architect_model_haiku_bulk_extraction_then_sonnet_enrichment_validation_effort_l_20_calls_sessions_2 -->
### Role: Data Architect | Model: Haiku (bulk extraction) then Sonnet (enrichment + validation) | Effort: L (20 calls) | Sessions: 2
...
<!-- ANCHOR: r3_r3_ms4_5_wiring_verification_audit -->
## R3-MS4.5: WIRING VERIFICATION AUDIT (v15.0)
### Role: Platform Engineer | Model: Sonnet | Effort: S (5-8 calls) | Sessions: 0.5
# PURPOSE: R3 builds intelligence engines, proof chains, cross-system queries.
# Verify all agent configs connected, proof validators called, enrichment pipelines wired.

PROCEDURE (follow PHASE_TEMPLATE.md § WIRING VERIFICATION AUDIT):
  1. INVENTORY all new intelligence engines, agent configs, proof chains from R3
  2. CALLER CHECK: every cross-query path has end-to-end callers
  3. DOWNSTREAM CHECK: R4-R6 docs reference R3 intelligence capabilities
  4. DISPOSITION for orphans
  5. VERIFY with smoke tests

SPECIFIC R3 CHECKS:
  - Job planner (MS0) outputs are consumed by downstream rendering (R5)
  - Formula registry (MS0.5) entries all callable and returning correct results
  - Cross-system queries (MS3) exercised by at least 1 realistic scenario
  - Data enrichment (MS4) results stored where downstream phases expect them
  - Agent configs created during R3 are in agent recommendation system

GAP ASSESSMENT (Step 6 — what R4-R6 will need):
  - CADENCE: Does R4 multi-tenant need tenant-aware cadence functions?
  - ENGINES: Are intelligence engines exposing APIs R5 rendering layer can consume?
  - SCRIPTS: Does R4 need tenant provisioning scripts? Does R5 need rendering test scripts?
  - NL HOOKS: Should R4 tenant boundaries trigger isolation-check NL hooks?
  - SKILLS: Are intelligence skills updated with R3's actual cross-query capabilities?
  Output: append findings to R3_WIRING_AUDIT.md → feed as dependencies into R4-MS0

GATE: <10% orphaned artifacts with documented disposition

---

<!-- ANCHOR: r3_r3_ms5_phase_gate -->
## R3-MS5: Phase Gate
<!-- ANCHOR: r3_role_systems_architect_model_sonnet_tests_then_opus_gate_review_effort_m_10_calls_sessions_1 -->
### Role: Systems Architect | Model: Sonnet (tests) then Opus (gate review) | Effort: M (10 calls) | Sessions: 1
...
<!-- ANCHOR: r3_opus_4_6_patterns_for_r3 -->
...
<!-- ANCHOR: r3_atcs_tracking_task_management_across_r3_sessions -->
...
<!-- ANCHOR: r3_r3_companion_assets_v14_5_built_per_ms_verified_at_r3_ms5_gate -->
## R3 COMPANION ASSETS (v14.5 -- built per-MS, verified at R3-MS5 gate)

<!-- ANCHOR: r3_per_ms_companion_schedule -->
### PER-MS COMPANION SCHEDULE:
``r
MS0 PRODUCES:
  HOOK: unit_consistency_check (blocking, pre-calc, detects mixed units)
  Verify: Submit speed in m/min + feed in IPR -> hook blocks

MS0.5 PRODUCES:
  HOOK: formula_registry_consistency (warning, verifies formula implementations)
  Verify: Check registry -> hook reports stub formulas

MS1 PRODUCES: (advanced calcs -- covered by existing safety skills from R2)

MS2 PRODUCES:
  SKILL: prism-toolpath-advisor (strategy selection guidance for common features)
  Verify: Ask about pocket strategy for Ti-6Al-4V -> skill loads with trochoidal rec

MS3 PRODUCES:
  HOOK: wiring_registry_check (warning, pre-impl, reminds to check D2F/F2E/E2S)
  SKILL: prism-controller-expert (alarm diagnosis per controller family)
  Verify: Ask about Fanuc alarm 0090 -> skill loads with diagnosis

MS4 PRODUCES:
  SKILL: prism-tolerance-advisor (tolerance analysis, stack-up, IT grades)
  SKILL: prism-formula-navigator (discover and explain 490+ formulas)
  Verify: Ask about Cpk calculation -> formula-navigator loads

MS5 GATE VERIFIES:
  PARALLEL TRACK CHECK: SKILL_INDEX.json count >= 1200 (300 from R1 + T3/T4 course skills + CNC/CAM)
  If count < 1200: T3/T4/CNC extraction behind schedule, allocate catch-up sessions
  PARALLEL: R7-MS6 (Manufacturer Catalog Extraction) can begin as independent track
  All 3 hooks fire, all 4 skills load, both scripts run
``r

<!-- ANCHOR: r3_r3_companion_assets_detail_v14_2_build_after_r3_ms5_gate_passes -->
## R3 COMPANION ASSETS DETAIL (v14.2 — build AFTER R3-MS5 gate passes)
