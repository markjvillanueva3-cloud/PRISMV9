# 24 dispatchers | ~304 actions | MCP-native | esbuild
...
### Cadence (auto-fired by autoHookWrapper — zero manual effort)
- Every 5 tool calls → auto todo refresh
- Every 8 tool calls → auto context pressure check + adaptive truncation + attention scoring
- Every 10 tool calls → auto checkpoint reminder
- Every 12 tool calls → auto compaction detection + compaction prediction (Python)
- Every 20 tool calls → auto variation check
- On first call → auto pre-task recon + warm start + knowledge enrichment + compaction rehydration
- On ANY error → auto error learn + on-error hooks (circuit breaker, retry, rollback)
- On ANY success → on-outcome hooks (performance, quality trends, usage tracking)
...
- On file writes → pre/post-file-write hooks (backup, anti-regression, rollback)
- At ≥70% pressure → auto-compress context snapshot
- At ≥75% pressure → auto compaction survival (saves task/findings/decisions to COMPACTION_SURVIVAL.json)
- At <40% pressure → auto context pull-back (restores externalized data)
...
- Layer 2: Slimming — results >2KB at ≥50% pressure get field-stripped (responseSlimmer)
...
## 4. HOOK SYSTEM — 73/112 hooks LIVE, auto-firing
...
### 39 Dead Hooks (data-mutation phases — not yet needed)
pre-material-add(7), pre-alarm-add(5), post-material-add(3), pre-batch-import(2),
post-batch-import(2), pre-machine-add(2), on-audit(2), on-anomaly(1), on-force-limit(1),
on-thermal-limit(1), on-machine-limit(1), on-tool-life-warning(1), on-tool-call(1),
on-pattern-match(1), on-learning-update(1), on-decision(2), pre-file-delete(1),
pre-material-update(1), post-material-update(1), post-alarm-add(1), post-alarm-batch(1),
post-machine-add(1)
...
prism:prism_context action=attention_score       → D2: Score content segments for attention priority
...
## 6. AUTO-FIRING SYSTEMS REFERENCE

### autoHookWrapper (wraps ALL 24 dispatchers)
| Trigger | What Fires | Behavior |
|---|---|---|
| Every call | on-tool-call-frequency tracking | Non-blocking |
| Error caught | on-error (5 hooks) | Non-blocking: error-log, circuit-breaker, retry, rollback |
| Success | on-outcome (9 hooks) | Non-blocking: performance, quality trends, usage |
| Before return | pre-output (2 hooks) | **BLOCKING**: omega-gate + safety-hard-gate |
| Result >cap | Auto-truncate + externalize | Adaptive: 20KB→5KB→500B by pressure |
| Result >2KB @≥50% | Response slimming | Strip low-value fields, cap arrays |
| Safety score <0.70 | CALC-SAFETY-VIOLATION-001 | Hard block notification |

### cadenceExecutor (23 auto-functions)
| Function | Trigger | Effect |
|---|---|---|
| autoTodoRefresh | @5 calls | Attention anchor |
| autoCheckpoint | @10 calls | State persistence |
| autoContextPressure | @8 calls | Buffer zone + adaptive caps |
