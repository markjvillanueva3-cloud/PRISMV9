...
- Every 8 tool calls → auto context pressure check + adaptive truncation
- Every 10 tool calls → auto checkpoint reminder
- Every 12 tool calls → auto compaction detection
- Every 20 tool calls → auto variation check
- On first call → auto pre-task recon + warm start + knowledge enrichment
...
### Context Pressure Management (3-layer defense, auto-firing)
...
### cadenceExecutor (17 auto-functions)
