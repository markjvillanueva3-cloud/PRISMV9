# PRISM — Universal Skills / Scripts / Hooks / Stop-Hooks Build Plan
**Date:** 2026-04-15
**Scope:** EVERYTHING — every dispatcher, every engine, every machine family, every workflow
**Goal:** Complete coverage of user-facing skills, automation scripts, validation hooks, and session-lifecycle stop-hooks across PRISM's entire surface area.
...
| U-PLG4 | `.mcp.json` update — register claude-flow MCP server | **Activate claude-flow's 80+ tools** — swarm_init, agent_spawn, task_orchestrate, memory_usage, consensus primitives, workflow_create. Health-check on SessionStart |
| U-PLG5 | **Delegation waypoints during Phase 0 buildout** (documented in Phase 0 execution rubric) | During Phase 0.6 auto-wiring, invoke `dispatcher-wirer` agent; during 0.7 indexes, `perf-analyzer`; during 0.13 awareness, `consensus-coordinator` for cross-terminal ordering; during 0.16 retrofit, `queen-coordinator` + `implementer` team; during 0.11 exit gate, `verifier` + `silent-failure-hunter` + `type-design-analyzer` in parallel |
| U-PLG6 | `state/shared/SPARC_OPT_IN.flag` + `/sparc` router skill | **SPARC methodology** — opt-in spec→pseudo→arch→refine→complete chain via `sparc-coord` agent for any "design a new subsystem" request. Default off |
| U-PLG7 | `/commands-audit` skill + `scripts/commands-audit.ts` | **Weekly report** — which of 175 slash commands were invoked 0 times / ≤2 times in last 30d; flag unused; propose archival or visibility fix |
| U-PLG8 | `/pr-swarm` skill binding | **Multi-agent PR review orchestration** — runs `pr-review-toolkit:code-reviewer` + `silent-failure-hunter` + `pr-test-analyzer` + `comment-analyzer` + `type-design-analyzer` in parallel, aggregates. Replaces ad-hoc single-agent reviews |
| U-PLG9 | `AGENT_UTILIZATION_LEDGER.jsonl` | **Append-only telemetry** — `{ulid, ts, agent, subagent_type, durationMs, exitCode, correlationId, invokedBy}`. Feeds `/commands-audit` and the recommender's ranking |
| U-PLG10 | `CAPABILITY_MANIFEST.json` extension (Phase 0.13) | Add `agents[]`, `mcps[]`, `slashCommands[]` to existing session-scope manifest. `/aware` skill surfaces these in boot brief |
