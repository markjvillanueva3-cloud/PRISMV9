...
description = "Milltronics";
vendor = "Milltronics";
vendorUrl = "http://milltronics.net";
legal = "Copyright (C) 2012-2025 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Generic milling post for Milltronics.";
...
properties = {
  ...
  G18: {
    title      : "G18 plane",
    description: "Set to \"ZX\" (right handed) or \"XZ\" (left handed) in accordance with your control setting - byte 79 - G18 plane select: XZ=0, ZX=1.",
    group      : "preferences",
    type       : "enum",
    values     : [
  ...
  useRigidTapping: {
    ...
    description: "Select 'Yes' to enable G88 rigid tapping or 'No' to select G84 tapping.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  useExtendedWCS: {
    title      : "Use extended WCS codes",
    description: "If yes is selected, there are 60 available WCS, 540-590.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
var diameterOffsetFormat = createFormat({prefix:"D"});
...
var settings = {
  ...
  retract: {
    ...
    methodZ                   : "G32", // special condition, overwrite retract behavior per axis
...
function onParameter(name, value) {
  if (name == "display") {
    writeBlock("PRINT" + "[" + filterText(String(value).toUpperCase(), settings.comments.permittedCommentChars) + "]");
  }
}
...
function onSection() {
  ...
  if (workOffset > 0) {
    if (!getProperty("useExtendedWCS")) {
      if (workOffset > 6) {
        error(localize("Work offset out of range.  Maximum of 6 unless extended WCS codes are enabled."));
      } else {
      ...
      if (workOffset > 60) {
        error(localize("Work offset out of range.  Maximum of 60."));
      } else {
        if (workOffset != currentWorkOffset) {
          var v1 = workOffset % 6;
          var v2 = Math.floor(workOffset / 6);
          var wcs = v1 == 0 ? 590 + v2 - 1 : (53 + v1) * 10 + v2;
          writeBlock(gFormat.format(wcs)); // G540->G590
          currentWorkOffset = workOffset;
        }
      }
    }
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    var P = !cycle.dwell ? 0 : clamp(0.01, cycle.dwell, 99999.999); // in seconds
    ...
    case "deep-drilling":
      ...
      break;
    case "tapping":
      F = getProperty("useRigidTapping") ? pitchOutput.format(tool.getThreadPitch()) : feedOutput.format(tool.getThreadPitch() * rpmFormat.getResultingValue(spindleSpeed));
      var tappingCode = getProperty("useRigidTapping") ? 88 : ((tool.type == TOOL_TAP_LEFT_HAND) ? 74 : 84);
      writeBlock(
        gRetractModal.format(98), gAbsIncModal.format(90), gCycleModal.format(tappingCode),
        getCommonCycle(x, y, z, cycle.retract),
        "P" + secFormat.format(P),
        F
      );
    ...
    case "left-tapping":
      F = getProperty("useRigidTapping") ? pitchOutput.format(tool.getThreadPitch()) : feedOutput.format(tool.getThreadPitch() * rpmFormat.getResultingValue(spindleSpeed));
      var tappingCode = getProperty("useRigidTapping") ? 88 : 74;
      writeBlock(
        gRetractModal.format(98), gAbsIncModal.format(90), gCycleModal.format(tappingCode),
        getCommonCycle(x, y, z, cycle.retract),
        "P" + secFormat.format(P),
    ...
    case "right-tapping":
      F = getProperty("useRigidTapping") ? pitchOutput.format(tool.getThreadPitch()) : feedOutput.format(tool.getThreadPitch() * rpmFormat.getResultingValue(spindleSpeed));
      var tappingCode = getProperty("useRigidTapping") ? 88 : 84;
      writeBlock(
        gRetractModal.format(98), gAbsIncModal.format(90), gCycleModal.format(tappingCode),
        getCommonCycle(x, y, z, cycle.retract),
        "P" + secFormat.format(P),
    ...
    case "right-tapping-with-chip-breaking":
      ...
      if (!getProperty("useRigidTapping")) {
        error(localize("Canned tapping cycles with chip breaking are only supported with rigid tapping."));
        return;
      }
      F = getProperty("useRigidTapping") ? pitchOutput.format(tool.getThreadPitch()) : feedOutput.format(tool.getThreadPitch() * rpmFormat.getResultingValue(spindleSpeed));
      writeBlock(
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      writeBlock(gAbsIncModal.format(90), gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3), iOutput.format(cx - start.x), jOutput.format(cy - start.y), feedOutput.format(feed));
      break;
    case PLANE_ZX:
      if (getProperty("G18") == "ZX") { // right handed
        writeBlock(gAbsIncModal.format(90), gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3), iOutput.format(cx - start.x), kOutput.format(cz - start.z), feedOutput.format(feed));
      } else if (getProperty("G18") == "XZ") { // left handed
        writeBlock(gAbsIncModal.format(90), gPlaneModal.format(18), gMotionModal.format(clockwise ? 3 : 2), iOutput.format(cx - start.x), kOutput.format(cz - start.z), feedOutput.format(feed));
      } else {
      ...
      break;
    case PLANE_YZ:
      writeBlock(gAbsIncModal.format(90), gPlaneModal.format(19), gMotionModal.format(clockwise ? 2 : 3), jOutput.format(cy - start.y), kOutput.format(cz - start.z), feedOutput.format(feed));
      break;
    default:
      linearize(tolerance);
    }
    ...
    switch (getCircularPlane()) {
    case PLANE_XY:
      writeBlock(gAbsIncModal.format(90), gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x), jOutput.format(cy - start.y), feedOutput.format(feed));
      break;
    case PLANE_ZX:
      if (getProperty("G18") == "ZX") { // right handed
        writeBlock(gAbsIncModal.format(90), gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x), kOutput.format(cz - start.z), feedOutput.format(feed));
      } else if (getProperty("G18") == "XZ") { // left handed
        writeBlock(gAbsIncModal.format(90), gPlaneModal.format(18), gMotionModal.format(clockwise ? 3 : 2), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x), kOutput.format(cz - start.z), feedOutput.format(feed));
      } else {
        warningOnce("ATTENTION: Set the G18 property to either 'ZX' (right handed) or 'XZ' left handed.", WARNING_G18);
        warningOnce("ATTENTION: For more information see programming manual - about byte 79 - G18 plane select: XZ=0, ZX=1.", WARNING_G18_2);
        linearize(tolerance);
      ...
      break;
    case PLANE_YZ:
      writeBlock(gAbsIncModal.format(90), gPlaneModal.format(19), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), jOutput.format(cy - start.y), kOutput.format(cz - start.z), feedOutput.format(feed));
      break;
    default:
      linearize(tolerance);
    }
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    if (machineConfiguration.isMultiAxisConfiguration()) {
      writeBlock(fourthAxisClamp.format(10));
...
function writeRetractCustom(retract) {
  ...
  case "G32":
    writeBlock(gFormat.format(32)); // Z retract only
    break;
  default:
    error(subst(localize("Unsupported safe position method '%1'"), retract.method));
  }
  machineSimulation({z:retract.singleLine || words.indexOf("Z") != -1 ? retract.positions.z : undefined, coordinates:MACHINE});
}
