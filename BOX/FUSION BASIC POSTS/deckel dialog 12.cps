/**
  ...
  Deckel Dialog 12 post processor configuration.

  $Revision: 44207 3c29ebc507e8e59a0898c47558016451f197da94 $
  $Date: 2025-12-17 08:29:21 $

  FORKID {4EE83F9D-E7AB-467b-8585-C930FE13FF32}
*/

description = "Deckel Dialog 12";
vendor = "Deckel";
...
longDescription = "Post for Deckel Dialog 12. Note that there are quite some difference between the Dialog 12 controls so this post would most likely need further customization to work properly for the specific control.";
...
wcsDefinitions = {
  useZeroOffset: false,
  wcs          : [
    {name:"Standard", format:"G51*", range:[1, 999]}
  ]
};
...
var settings = {
  ...
  maximumToolNumber           : 50, // specifies the maximum allowed tool number
  programNumber               : {min:1, max:99999}, // specifies the program number range and reserved numbers
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    writeToolBlock(gMotionModal.format(0), zOutput.format(getProperty("safeZHeight")), "T" + toolFormat.format(tool.number), mFormat.format(6));
