/**
  ...
  $Revision: 44206 2b1f7582515f2a58db4264aebbe6165bf26e0c15 $
  $Date: 2025-12-04 14:30:34 $
...
var coolants = [
  ...
  {id:COOLANT_AIR, on:20, off:21},
  {id:COOLANT_AIR_THROUGH_TOOL, on:56, off:57},
  {id:COOLANT_SUCTION},
...
function onOpen() {
  ...
  previousMaximumSpeed = getProperty("maximumSpindleSpeed");

  if (gotSecondarySpindle) {
    writeBlock(gMotionModal.format(0), gFormat.format(28), subOutput.format(0)); // retract Sub Spindle if applicable
  }
  ...
  if (machineConfiguration.isMultiAxisConfiguration()) {
    if (machineState.isTurningOperation) {
      ...
      if (_section.isMultiAxis() || isPolarModeActive()) {
        ...
        abc = getWorkPlaneMachineABC(_section, _section.workPlane);
        if (!machineState.usePolarCoordinates /*&& !machineState.usePolarInterpolation*/ && !machineState.isTurningOperation && !machineState.axialCenterDrilling) {
          if (_setWorkPlane) {
            setWorkPlane(abc);
          }
        }
...
function onSection() {
  ...
  if (tempSpindle == SPINDLE_LIVE) {
    ...
    if ((maximumSpindleSpeed > 0) && (currentSection.getTool().getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED)) {
      if (rpmFormat.areDifferent(maximumSpindleSpeed, previousMaximumSpeed) && !machineState.stockTransferIsActive) {
  ...
  var abc = defineWorkPlane(currentSection, !machineState.usePolarInterpolation);
  ...
  if (insertToolCall || retracted || (tool.getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED) || machineState.usePolarInterpolation) {
    // gPlaneModal.reset();
    gMotionModal.reset();
    ...
    if (pendingRadiusCompensation >= 0) {
      ...
      case RADIUS_COMPENSATION_LEFT:
        writeBlock(
          ...
          x, y, z, getFeed(feed), coolantCode
        );
        break;
      case RADIUS_COMPENSATION_RIGHT:
        writeBlock(
      ...
      default:
        writeBlock(gMotionModal.format(isSpeedFeedSynchronizationActive() ? 32 : 1), gFormat.format(40), x, y, z,
          getFeed(feed), coolantCode);
      }
      resetCoolantCode();
    } else {
      writeBlock(gMotionModal.format(isSpeedFeedSynchronizationActive() ? 32 : 1), x, y, z, getFeed(feed), coolantCode);
      resetCoolantCode();
    }
  }
}
...
function onLinear5D(_x, _y, _z, _a, _b, _c, feed) {
  ...
  if (x || y || z || a || b || c) {
    writeBlock(gMotionModal.format(1), x, y, z, a, b, c, getFeed(feed), coolantCode);
    resetCoolantCode();
  }
}
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      writeBlock(gPlaneModal.format(getG17Code()), gMotionModal.format(directionCode), iOutput.format(cx - start.x), jOutput.format(cy - start.y), getFeed(feed), coolantCode);
      resetCoolantCode();
      break;
    case PLANE_ZX:
      if (machineState.usePolarInterpolation) {
      ...
      writeBlock(gPlaneModal.format(18), gMotionModal.format(directionCode), iOutput.format(cx - start.x), kOutput.format(cz - start.z), getFeed(feed), coolantCode);
      resetCoolantCode();
      break;
    case PLANE_YZ:
      if (machineState.usePolarInterpolation) {
      ...
      writeBlock(gPlaneModal.format(19), gMotionModal.format(directionCode), jOutput.format(cy - start.y), kOutput.format(cz - start.z), getFeed(feed), coolantCode);
      resetCoolantCode();
      break;
    default:
      linearize(toler);
    ...
    switch (getCircularPlane()) {
    case PLANE_XY:
      writeBlock(gPlaneModal.format(getG17Code()), gMotionModal.format(directionCode), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x), jOutput.format(cy - start.y), getFeed(feed), coolantCode);
      resetCoolantCode();
      break;
    case PLANE_ZX:
      if (machineState.usePolarInterpolation) {
      ...
      writeBlock(gPlaneModal.format(18), gMotionModal.format(directionCode), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x), kOutput.format(cz - start.z), getFeed(feed), coolantCode);
      resetCoolantCode();
      break;
    case PLANE_YZ:
      if (machineState.usePolarInterpolation) {
      ...
      writeBlock(gPlaneModal.format(19), gMotionModal.format(directionCode), xOutput.format(x), yOutput.format(y), zOutput.format(z), jOutput.format(cy - start.y), kOutput.format(cz - start.z), getFeed(feed), coolantCode);
      resetCoolantCode();
      break;
    default:
      linearize(toler);
...
function onCyclePoint(x, y, z) {
  ...
  var lockCode = "";
  var lockCAxis = machineState.usePolarCoordinates || machineState.usePolarInterpolation;
