...
var inverseTimeFormat = createFormat({decimals:(unit == MM ? 2 : 3), type:FORMAT_REAL});
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    if (machineConfiguration.isMultiAxisConfiguration()) {
      writeBlock(fourthAxisClamp.format(32)); // lock 4th axis
  ...
  case COMMAND_UNLOCK_MULTI_AXIS:
    if (machineConfiguration.isMultiAxisConfiguration()) {
      writeBlock(fourthAxisClamp.format(33)); // unlock 4th axis
      if (machineConfiguration.getNumberOfAxes() > 4) {
