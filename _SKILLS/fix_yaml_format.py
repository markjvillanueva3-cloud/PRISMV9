...
DESCRIPTIONS = {
    ...
    "prism-safety-framework": "7 failure modes and 7 defense layers for S(x) score. CRITICAL for manufacturing safety. Hard constraint S(x) >= 0.70.",
    ...
    "prism-cognitive-enhancement": "Overview of cognitive enhancement system. Integration guide for all cognitive skills.",
    ...
    "prism-prompt-engineering": "Expert prompt engineering for Claude 4.5 Opus optimization.",
    "prism-hook-system": "Hook-based automatic enforcement with 147 hook points.",
    "prism-uncertainty-propagation": "Mandatory uncertainty quantification for all PRISM outputs.",
    "prism-mathematical-planning": "Mathematical proof required before execution with MATHPLAN gate.",
    "prism-formula-evolution": "Continuous mathematical evolution for PRISM formulas.",
    "prism-predictive-thinking": "Anticipatory thinking and proactive problem-solving.",
    "prism-maximum-completeness": "Enforces 100% theoretical and mathematical completeness.",
    "prism-life-safety-mindset": "Critical mindset for maximum thoroughness in safety-critical work.",
    "prism-anti-regression": "Mandatory protocol for preventing content loss during updates.",
    "prism-codebase-packaging": "Pack codebases into AI-friendly single files using Repomix.",
    "prism-code-complete-integration": "Steve McConnell's Code Complete principles for PRISM.",
    "prism-tdd-enhanced": "Enhanced TDD with manufacturing-specific test patterns.",
    "prism-tool-holder-schema": "Complete 85-parameter tool holder schema.",
}

def get_skill_name(filename):
    """Extract skill name from filename like prism-xyz_SKILL.md"""
    name = filename.replace("_SKILL.md", "")
    # Handle v6 orchestrator special case
...
def fix_yaml_format(filepath):
    """Fix a skill file to have YAML frontmatter on line 1"""
    ...
    # Check if already has valid YAML on line 1
    if content.startswith('---\n'):
        # Check if it has proper name and description
        yaml_match = re.match(r'^---\n(.*?)\n---', content, re.DOTALL)
        if yaml_match:
            yaml_content = yaml_match.group(1)
            if 'name:' in yaml_content and 'description:' in yaml_content:
                print(f"  OK: {filename}")
                return False  # Already good
    
    # Remove existing YAML frontmatter if present (anywhere in file)
    content_clean = re.sub(r'^---\s*\n.*?\n---\s*\n', '', content, flags=re.DOTALL)
    
    # Remove leading comment blocks (lines starting with #)
    ...
    # Keep the actual content (may include markdown headers)
    main_content = '\n'.join(lines[content_start:]).strip()
    
    # If content starts with markdown header, keep it
    # Otherwise preserve the original structure
    if not main_content:
        main_content = content_clean.strip()
...
def main():
    print("=" * 60)
    print("Fixing YAML format for all skills in UPLOAD_ALL")
    print("=" * 60)
