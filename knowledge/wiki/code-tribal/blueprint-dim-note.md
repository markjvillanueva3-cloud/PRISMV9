---
title: Blueprint pattern — note
slug: blueprint-dim-note
kind: code-tribal
status: shipped
date: 2026-05-21
...
# Reading `note` regions on blueprints

Observed in the corpus: **0 regions** across all customer families.

## Extraction guidance

Free text — extract verbatim then classify via downstream NER. Don't normalise units in notes.

## RAG hooks

When extracting a `note` region, the RAG retrieval should pull from:
...
**OR** `confidenceFloor !== "normal"`. A `note` extraction with
