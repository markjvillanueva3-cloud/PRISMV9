---
title: Blueprint pattern — radius
slug: blueprint-dim-radius
kind: code-tribal
status: shipped
date: 2026-05-21
...
# Reading `radius` regions on blueprints

Observed in the corpus: **0 regions** across all customer families.

## Extraction guidance

`R` prefix. Surface-radius (outer) vs corner-radius (inner) distinguished by callout location.

## RAG hooks

When extracting a `radius` region, the RAG retrieval should pull from:
...
**OR** `confidenceFloor !== "normal"`. A `radius` extraction with
