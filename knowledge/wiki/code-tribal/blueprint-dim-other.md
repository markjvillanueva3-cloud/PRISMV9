---
title: Blueprint pattern — other
slug: blueprint-dim-other
kind: code-tribal
status: shipped
date: 2026-05-21
...
# Reading `other` regions on blueprints

Observed in the corpus: **0 regions** across all customer families.

## Extraction guidance

Unclassified — these are the highest-value items for U4 pattern-mining. Cluster + propose new dimTypes.

## RAG hooks

When extracting a `other` region, the RAG retrieval should pull from:
...
**OR** `confidenceFloor !== "normal"`. A `other` extraction with
