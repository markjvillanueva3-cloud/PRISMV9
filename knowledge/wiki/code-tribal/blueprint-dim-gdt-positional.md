---
title: Blueprint pattern — gdt_positional
slug: blueprint-dim-gdt-positional
kind: code-tribal
status: shipped
date: 2026-05-21
...
# Reading `gdt_positional` regions on blueprints

Observed in the corpus: **0 regions** across all customer families.

## Extraction guidance

FCF anatomy: symbol | tol | [Ø?] | M/L/F? | [datums]. `M` = MMC, `L` = LMC, `F` = RFS-equivalent in Y14.5-2018.

## RAG hooks

When extracting a `gdt_positional` region, the RAG retrieval should pull from:
...
**OR** `confidenceFloor !== "normal"`. A `gdt_positional` extraction with
