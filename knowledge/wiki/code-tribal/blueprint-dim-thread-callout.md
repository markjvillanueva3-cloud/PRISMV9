---
title: Blueprint pattern — thread_callout
slug: blueprint-dim-thread-callout
kind: code-tribal
status: shipped
date: 2026-05-21
...
# Reading `thread_callout` regions on blueprints

Observed in the corpus: **0 regions** across all customer families.

## Extraction guidance

Pattern: `<series>-<size> <tolerance> [LH] [<class>]` e.g. `1/4-20 UNC-2B LH`. Tap-drill table lookup in `prism_thread:calculate_tap_drill`.

## RAG hooks

When extracting a `thread_callout` region, the RAG retrieval should pull from:
...
**OR** `confidenceFloor !== "normal"`. A `thread_callout` extraction with
