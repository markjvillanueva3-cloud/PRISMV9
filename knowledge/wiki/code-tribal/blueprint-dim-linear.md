---
title: Blueprint pattern — linear
slug: blueprint-dim-linear
kind: code-tribal
status: shipped
date: 2026-05-21
...
# Reading `linear` regions on blueprints

Observed in the corpus: **0 regions** across all customer families.

## Extraction guidance

Look for `mm` / `in` suffix or no-unit (drawing default applies). Watch for limit-pair `12.5 / 12.3` per ISO 14405.

## RAG hooks

When extracting a `linear` region, the RAG retrieval should pull from:
...
**OR** `confidenceFloor !== "normal"`. A `linear` extraction with
