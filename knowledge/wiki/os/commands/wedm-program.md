---
kind: command
slug: wedm-program
status: stub
generated_at: 2026-05-23T00:34:38.924Z
generator: scripts/backfill-wiki-os-commands-entities.mjs
source: .claude/commands/wedm-program.md
description: "Generate a complete, production-ready Wire EDM NC program with physics-derived parameters, per-pass optimization, and full traceability."
---

# /wedm-program

Generate a complete, production-ready Wire EDM NC program with physics-derived parameters, per-pass optimization, and full traceability.

## Source command

See `.claude/commands/wedm-program.md` for the live executable surface.

## Status

This entity is a U-CK11 Phase 2 backfill stub. Richer content (purpose,
composes_with chain, invocation examples, related skills) lands in a
later phase per the U-CK11 verdicts doc remediation queue.
