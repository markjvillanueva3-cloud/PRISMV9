---
kind: command
slug: train-lora
status: stub
generated_at: 2026-05-23T00:34:38.924Z
generator: scripts/backfill-wiki-os-commands-entities.mjs
source: .claude/commands/train-lora.md
description: "Kick off a LoRA fine-tune of the local base model using recent shop outcomes as training signal. Queries `OutcomeTrackingEngine` for good/adjusted records, emits a training manifest, and shells out to the Python trainer."
---

# /train-lora

Kick off a LoRA fine-tune of the local base model using recent shop outcomes as training signal. Queries `OutcomeTrackingEngine` for good/adjusted records, emits a training manifest, and shells out to the Python trainer.

## Source command

See `.claude/commands/train-lora.md` for the live executable surface.

## Status

This entity is a U-CK11 Phase 2 backfill stub. Richer content (purpose,
composes_with chain, invocation examples, related skills) lands in a
later phase per the U-CK11 verdicts doc remediation queue.
