---
kind: command
slug: shop-setup
status: stub
generated_at: 2026-05-23T00:34:38.924Z
generator: scripts/backfill-wiki-os-commands-entities.mjs
source: .claude/commands/shop-setup.md
description: "Configure shop-wide rates, machines, and overhead for accurate costing across all PRISM engines."
---

# /shop-setup

Configure shop-wide rates, machines, and overhead for accurate costing across all PRISM engines.

## Source command

See `.claude/commands/shop-setup.md` for the live executable surface.

## Status

This entity is a U-CK11 Phase 2 backfill stub. Richer content (purpose,
composes_with chain, invocation examples, related skills) lands in a
later phase per the U-CK11 verdicts doc remediation queue.
