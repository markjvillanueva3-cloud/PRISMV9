---
kind: command
slug: traveler
status: stub
generated_at: 2026-05-23T00:34:38.924Z
generator: scripts/backfill-wiki-os-commands-entities.mjs
source: .claude/commands/traveler.md
description: "Manage E2-style job travelers with ordered routing steps, dual time tracking (setup + cycle), QR scan, and machine dispatch queues."
---

# /traveler

Manage E2-style job travelers with ordered routing steps, dual time tracking (setup + cycle), QR scan, and machine dispatch queues.

## Source command

See `.claude/commands/traveler.md` for the live executable surface.

## Status

This entity is a U-CK11 Phase 2 backfill stub. Richer content (purpose,
composes_with chain, invocation examples, related skills) lands in a
later phase per the U-CK11 verdicts doc remediation queue.
