---
kind: command
slug: startup
status: stub
generated_at: 2026-05-23T00:34:38.924Z
generator: scripts/backfill-wiki-os-commands-entities.mjs
source: .claude/commands/startup.md
description: "Full session bootstrap via psk — 4 syscalls (checkin composite + position + handoff read + pick). Heavyweight; use /boot for 1-2s orientation."
---

# /startup

Full session bootstrap via psk — 4 syscalls (checkin composite + position + handoff read + pick). Heavyweight; use /boot for 1-2s orientation.

## Source command

See `.claude/commands/startup.md` for the live executable surface.

## Status

This entity is a U-CK11 Phase 2 backfill stub. Richer content (purpose,
composes_with chain, invocation examples, related skills) lands in a
later phase per the U-CK11 verdicts doc remediation queue.
