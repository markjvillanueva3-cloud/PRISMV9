...
description: Obsidian brain ≡ PSN leg #1 — PRISM's cross-session persistent memory. Auto-fed every Stop by `stop-obsidian-memory-feed.mjs` (C: memory/*.md → H: knowledge/memories/<type>/). The reason you "always utilize memories with virtually no token usage". Distinct from the wiki (project-lifetime architecture).
aliases: [obsidian-brain, Obsidian Brain, feedback-obsidian-brain]
metadata:
  type: feedback
---
