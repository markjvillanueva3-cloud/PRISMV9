...
description: With 6 concurrent chats on H:/PRISM, each chat must own a worktree+branch+milestone scope, must not edit/commit files outside its lane or files claimed by peer chats, and must use the chat bus to broadcast intent before non-trivial work.
type: feedback
