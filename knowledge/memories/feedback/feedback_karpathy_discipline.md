...
description: Karpathy 5-step pre-coding checklist — CLASSIFY → TECHNIQUE → EDGE CASES → FAILURE MODES → THEN WRITE. The mechanism behind PRISM rule R1 (Think Before Coding). Cited by every hook header, every commit, every code review. Auto-injector anchor.
aliases: [Karpathy 5-step, R1, R1-mechanism, classify-technique-edge-failure-write, Karpathy-discipline, pre-coding-checklist]
metadata:
  type: feedback
---
