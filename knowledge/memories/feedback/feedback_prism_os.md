...
description: PRISM OS ≡ PSN leg #2 — the shell + desk + program-release + scheduling-studies + shop-floor surface exposed via `prism_operating_system` dispatcher (45+ actions). The "operating system" feel of PRISM that lets one query bootstrap an entire role-aware workspace. Distinct from the OS-level filesystem.
aliases: [prism-os, Prism OS, feedback-prism-os]
metadata:
  type: feedback
---
