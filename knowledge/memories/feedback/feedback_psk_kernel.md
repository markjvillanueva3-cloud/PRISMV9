...
description: PSK ≡ PRISM Syscall Kernel — composite syscall layer behind /startup /checkin /handoff /pick /precompact. 10 fail-soft syscalls (whoami, manifest, position, delta, tools, pick, checkin, handoff, record, recommend) at `.claude/kernel/psk.mjs`, wired to `prism_session:psk`. The COMMAND-KERNEL-MS0 substrate that lets every lifecycle slash-command compose the same primitives. Auto-injector anchor.
aliases: [PSK, PRISM Syscall Kernel, prism_session-psk, COMMAND-KERNEL-MS0, syscall-kernel]
metadata:
  type: feedback
---
