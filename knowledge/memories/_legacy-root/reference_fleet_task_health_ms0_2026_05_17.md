...
metadata:  
source: prism-memory
synced: 2026-05-18T01:02:09.339Z
...
- `.claude/hooks/critical-memory-compact-nudge.mjs` — UserPromptSubmit hook;
  when fleet-memory-monitor telemetry is fresh + `critical` AND this chat's slot
...
- B matches the largest tree BY SLOT — it fires only when fleet-memory-monitor
...
- chat-slots.json path order MUST mirror the telemetry producer
  (fleet-memory-monitor: `.claude/state/` primary, `state/shared/` fallback) —
...
## Related
[[skills/fleet-task-health-watch|/fleet-task-health-watch]] • [[skills/disabled|/disabled]] • [[skills/failing|/failing]] • [[skills/stale|/stale]] • [[skills/never-ran|/never-ran]] • [[skills/warn|/warn]] • [[skills/critical|/critical]] • [[skills/hooks|/hooks]] • [[skills/fleet-task-health-stop|/fleet-task-health-stop]] • [[skills/critical-memory-compact-nudge|/critical-memory-compact-nudge]]