...
metadata:  
source: prism-memory
synced: 2026-05-18T01:02:09.225Z
...
2. **node `fetch` to localhost:11434 FAILS on this Windows box** ("fetch failed") — Ollama distill MUST use a `curl` subprocess (documented OLLAMA-PIPELINE-MS0 pattern). A fetch-based distill silently falls back to raw every call → the "use Ollama for token cost" directive would be silently unmet. Verify: distill via curl produces real LLM bullets in ~1.4s.
...
## Related
[[engines/MemoryConflictResolverEngine|MemoryConflictResolverEngine]] • [[dispatchers/prism_safety|prism_safety]] • [[skills/checkin|/checkin]] • [[skills/prism|/prism]] • [[skills/checkin-recall|/checkin-recall]] • [[skills/graph|/graph]] • [[skills/source|/source]] • [[skills/system-viz-query|/system-viz-query]] • [[skills/autopilot-full|/autopilot-full]] • [[skills/yolo-mode|/yolo-mode]]