---
name: reference_code_schema_toolpathactionschemas
description: "Toolpath Dispatcher Action Schemas"
metadata:
  type: reference
  kind: code-schema
  galaxy: unassigned
  sourcePath: mcp-server/src/schemas/toolpathActionSchemas.ts
  sourceHash: 0c8be883
  loc: 225
aliases: toolpathActionSchemas.ts
---

# toolpathActionSchemas.ts -- Schema SUMMARY

> Auto-generated by `scripts/code-to-vault.mjs` (CODE-VAULT-BRIDGE, slot:sierra). Re-run to refresh.
> generatedAt: 2026-07-03T03:00:00Z  |  source: `mcp-server/src/schemas/toolpathActionSchemas.ts` (225 LOC)

## What it is
```
Toolpath Dispatcher Action Schemas
====================================
Per-action Zod schemas for prism_toolpath actions (9 actions).
Validated AFTER normalizeParams(), BEFORE engine dispatch.

Design:
- `.passthrough()` on all schemas: extra params flow through (hooks, metadata, debug)
- Only enforce fields the engine/handler actually reads
- Aliases resolved by normalizeParams BEFORE validation

Actions: strategy_select, params_calculate, strategy_search, strategy_list,
         strategy_info, stats, material_strategies, prism_novel, generate

@module schemas/toolpathActionSchemas
@version 1.0.0
@milestone SYS-MS6-U03
```


## Exports
`ACTION_TOOLPATH_SCHEMAS` (const)

## Key methods
_(none extracted)_

## Depends on
_(no sibling-engine imports)_

## Where it lives
- **Source:** `mcp-server/src/schemas/toolpathActionSchemas.ts`
- **Galaxy:** _(cross-cutting / unassigned)_
- **Kind:** Schema

## How to query it
Recall by meaning via `/system-viz find toolpathActionSchemas.ts` or `prism_session:master_index_query`; open the source at `mcp-server/src/schemas/toolpathActionSchemas.ts`. This note IS the brain connection -- a vault memory node under `knowledge/memories/reference/`, graphed by `generate-memories-atomic` and indexed by `build-memory-index-sidecar`. Related: [[feedback_read_full_content_not_titles]].
