---
name: reference_code_schema_tolerances
description: "PRISM MCP Server - Canonical Tolerance Table"
metadata:
  type: reference
  kind: code-schema
  galaxy: unassigned
  sourcePath: mcp-server/src/schemas/tolerances.ts
  sourceHash: 8b4587a2
  loc: 70
aliases: tolerances.ts
---

# tolerances.ts -- Schema SUMMARY

> Auto-generated by `scripts/code-to-vault.mjs` (CODE-VAULT-BRIDGE, slot:sierra). Re-run to refresh.
> generatedAt: 2026-07-03T03:00:00Z  |  source: `mcp-server/src/schemas/tolerances.ts` (70 LOC)

## What it is
```
PRISM MCP Server - Canonical Tolerance Table
Single source of truth for R2 test matrix validation thresholds.

S(x) >= 0.70 is the SAFETY gate (is this cut safe?).
Tolerances are the ACCURACY gate (is the math correct?).
Both must pass. A calc can be "safe" (S(x)=0.85) but "wrong" (delta=40%).

SOURCE OF TRUTH: PRISM_PROTOCOLS_CORE.md §Canonical Tolerance Table.
This file is the compile-time implementation of those protocol values.

@module schemas/tolerances
@safety HIGH — Controls pass/fail thresholds for calculation validation.
```


## Exports
`R2_TOLERANCES` (const), `ToleranceCategory` (type), `withinTolerance` (function), `toleranceDelta` (function)

## Key methods
_(none extracted)_

## Depends on
_(no sibling-engine imports)_

## Where it lives
- **Source:** `mcp-server/src/schemas/tolerances.ts`
- **Galaxy:** _(cross-cutting / unassigned)_
- **Kind:** Schema

## How to query it
Recall by meaning via `/system-viz find tolerances.ts` or `prism_session:master_index_query`; open the source at `mcp-server/src/schemas/tolerances.ts`. This note IS the brain connection -- a vault memory node under `knowledge/memories/reference/`, graphed by `generate-memories-atomic` and indexed by `build-memory-index-sidecar`. Related: [[feedback_read_full_content_not_titles]].
