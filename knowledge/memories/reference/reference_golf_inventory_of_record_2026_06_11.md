...
## UPDATE — later 2026-06-11 (slot:golf cc814d81, handoff-mine + Ollama-miner pass)

Independent corroboration via a 2nd ultracode Workflow `wf_cc1f3500-64f` (6 sonnet agents over the **46 `HANDOFF-golf-*.md`** session handoffs) + the now-working Ollama galaxy-miner. Evidence-backed appendix: `state/shared/specs/GOLF-CONTEXT-INVENTORY-2026-06-11.md`. **Two NET-NEW facts the wf_2c7ce362 record did not carry:**

1. **🔴 NEW P0 — the reaper is CURRENTLY DISABLED.** `PRISM_FLEET_REAPER_DISABLE=1` **and** `PRISM_GOLF_GUARDIAN_DISABLE=1` are both set in `C:/Users/wompu/.claude/settings.json` right now (operator, 2026-06-11). Root cause: `stale-node-hunter findStaleOrphanedNodes` (`01220f8a5f`) reaped **legit idle fleet `node.exe`** (RSS=0 / sub-5MB). Operator directive: do NOT re-launch any reaper until the hunter gets cmdline-allowlist + higher age-floor + deeper ancestry. **Golf's core duty is offline** → co-P0 with the AGENT-TIER merge. Evidence: `HANDOFF-golf-golf-context-regain.md`.
2. **🟢 Ollama galaxy-miner is now OPERATIONAL** — the prior exit-255 "registry key mismatch" did NOT reproduce; `mine-galaxy-transcripts.mjs --galaxy fleet-hygiene` mined 23 sessions (258MB JSONL, qwen2.5-coder:32b MAP → gpt-oss:120b SYNTH), refreshing `reference_fleet-hygiene_transcript_synthesis.md`. The local-LLM deep-read path the prior record called broken is back.

Handoff-mine also confirmed the **stale "Next:" corrections**: INFRA-CONSENSUS-WIRE-MS0 (8 commits) + INFRA-AGI-ROUTER-MS2 (13) + U-BIBRYAM-3 noise-filter + gpt-oss:120b pull are all SHIPPED — do NOT re-queue. Genuinely-pending head: **L8-P0/P1/P2-MS2, U-FD02→05, U-SR05/06**.
...
## ⚠️ CORRECTION — 2026-06-20 (slot:golf /checkin-golf)
The `🔴 NEW P0 — reaper CURRENTLY DISABLED` above is **half-true and stale** — superseded by [[reference_golf_reaper_state_correction_2026_06_20]]. Verified live: the disable flags are **unset at Machine/User scope**, so the **SYSTEM scheduled task never saw them** and has been running **LIVE + SAFE for 10 days** (fixed hunter, `LastResult=0`, 0 live false-positives in a dry-run sweep, 44/44 tests). The settings.json flag only gated the in-session path, which is now in **dry-run-observe** per operator. Do **NOT** re-queue "reaper disabled" as an open P0.
