...
`PRISM Fleet Reaper` Windows scheduled task is INSTALLED + Ready (5-min cadence, [[reference_fleet_reaper|FLEET-REAPER]]-MS0) — bounds orphan accumulation; this is why the storm self-drained rather than wedging. The gap is per-spike *amplitude*, not unbounded growth.
