---
name: reference_code_engine_managerdailydashboardengine
description: "ManagerDailyDashboardEngine — foreman/manager team rollup."
metadata:
  type: reference
  kind: code-engine
  galaxy: unassigned
  sourcePath: mcp-server/src/engines/ManagerDailyDashboardEngine.ts
  sourceHash: b041b1d8
  loc: 242
aliases: managerDailyDashboardEngine
---

# managerDailyDashboardEngine -- Engine SUMMARY

> Auto-generated by `scripts/code-to-vault.mjs` (CODE-VAULT-BRIDGE, slot:sierra). Re-run to refresh.
> generatedAt: 2026-07-03T03:00:00Z  |  source: `mcp-server/src/engines/ManagerDailyDashboardEngine.ts` (242 LOC)

## What it is
```
ManagerDailyDashboardEngine — foreman/manager team rollup.

Natural counterpart to EmployeeDailyDigestEngine: aggregates the digests of
the manager's direct reports into one team-view + adds manager-only signals:
  • team-coverage gaps (machines unstaffed, employees unqualified, overdue rest)
  • PTO requests awaiting THIS manager's approval
  • overdue management-review action items (the §9.3 action items where this
    manager is the owner)
  • team performance rollup (avg quality/throughput/safety/learning)
  • top-performer + needs-attention lists

Pass-in pattern (no rebuild — pure transform over upstream signals).

Hotel-soul invariants: PII-free (employee_id only), Object.frozen returns,
R12 fail-loud, audit-trail (generated_at).

@module ManagerDailyDashboardEngine
@milestone HOTEL/U-MANAGER-DAILY-DASHBOARD (2026-05-25, slot:hotel iter21)
```


## Exports
`DirectReportSummary` (interface), `PendingPTORequest` (interface), `CoverageGapSummary` (interface), `OverdueActionItem` (interface), `ManagerDashboardInput` (interface), `TeamPerformanceRollup` (interface), `ManagerDashboard` (interface), `managerDailyDashboardEngine` (const)

## Key methods
`buildDashboard`

## Depends on
_(no sibling-engine imports)_

## Where it lives
- **Source:** `mcp-server/src/engines/ManagerDailyDashboardEngine.ts`
- **Galaxy:** _(cross-cutting / unassigned)_
- **Kind:** Engine

## How to query it
Recall by meaning via `/system-viz find managerDailyDashboardEngine` or `prism_session:master_index_query`; open the source at `mcp-server/src/engines/ManagerDailyDashboardEngine.ts`. This note IS the brain connection -- a vault memory node under `knowledge/memories/reference/`, graphed by `generate-memories-atomic` and indexed by `build-memory-index-sidecar`. Related: [[feedback_read_full_content_not_titles]].
