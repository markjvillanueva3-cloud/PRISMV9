...
The lib reads `state/shared/system-viz/system-graph.json` via mtime cache. When the peer chat (`claude-b6c4b196`, slot ?) expanding system-viz to cover **all files on H: drive** (the [[reference_system_viz_fs_coverage_ms0|SYSTEM-VIZ-FS-COVERAGE-MS0]] milestone, adding L12 filesystem leaves) completes its work, the cache invalidates automatically on the next subagent spawn — no manual refresh required.
