...
**Q2 SHIPPED (`89146678bf`, U-OBS-EMBED-PROGRESS-HONEST).** Fixed the lying
embed-progress marker: a fleet-reaper SIGKILL / crash before the first batch flush left
`state:"running" done:0` forever (the "aborted" flip only fires in the Ollama-error
catch). The live marker was 24.5h stale yet read "running". Now stamps `pid` (schema
v1->2) + exports pure `classifyEmbedProgress(marker,{nowMs,isPidAlive,stalenessMs})`
(running + dead-pid OR stale-heartbeat -> "stale"; terminal verbatim) + `isPidAlive` +
a `--status` CLI consumer. 11 tests (mutation-verified real, incl. the exact 24h-marker
repro), 3-of-3 PASS, LIVE `--status` reports the real marker as stale. P3 follow-ups
(deferred, non-block): (a) a future-dated + no-pid v1 marker could read "running" --
moot for future pid-stamped runs; a one-line `ageMs<0 -> stale "clock skew"` guard closes
it; (b) pid-reuse over-trust, bounded by the 15m time-staleness OR-clause.
Also shipped this session: [[reference_ollama_vision_single_source_2026_06_09]]
(Ollama vision-model single-source + xray dangling-dep fix).

**Next fire:** Q3 (subagent-turn recall -- biggest uncovered turn) is the cheapest
remaining TIER-1 ship. Pairs with
