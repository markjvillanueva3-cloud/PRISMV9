---
name: reference_code_dispatcher_localdispatcher
description: "prism_local — Local LLM Dispatcher — LOCAL-LLM-MS0"
metadata:
  type: reference
  kind: code-dispatcher
  galaxy: unassigned
  sourcePath: mcp-server/src/tools/dispatchers/localDispatcher.ts
  sourceHash: 47f34213
  loc: 519
aliases: localDispatcher
---

# localDispatcher -- Dispatcher SUMMARY

> Auto-generated by `scripts/code-to-vault.mjs` (CODE-VAULT-BRIDGE, slot:sierra). Re-run to refresh.
> generatedAt: 2026-07-03T16:37:47.621Z  |  source: `mcp-server/src/tools/dispatchers/localDispatcher.ts` (519 LOC)

## What it is
```
prism_local — Local LLM Dispatcher — LOCAL-LLM-MS0
===================================================

Dispatch actions to local Ollama/Qwen for token savings:
- validate_code: Validate code against CLAUDE.md rules (FREE via Ollama)
- local_health: Check Ollama/Docker stack health
- offload_classify: Classify if task can be offloaded to local LLM

Token savings: 40-60% for typical development sessions by:
- Running code validation locally instead of via Claude hooks
- Offloading explanations/summaries to local models
- Using local embeddings for pattern matching

@module tools/dispatchers/localDispatcher
@milestone LOCAL-LLM-MS0 Session 2
```


## Exports
`LocalDispatcherResult` (interface), `localDispatcher` (function), `registerLocalDispatcher` (function)

## Key methods
`async`

## Depends on
`Logger`

## Where it lives
- **Source:** `mcp-server/src/tools/dispatchers/localDispatcher.ts`
- **Galaxy:** _(cross-cutting / unassigned)_
- **Kind:** Dispatcher

## How to query it
Recall by meaning via `/system-viz find localDispatcher` or `prism_session:master_index_query`; open the source at `mcp-server/src/tools/dispatchers/localDispatcher.ts`. This note IS the brain connection -- a vault memory node under `knowledge/memories/reference/`, graphed by `generate-memories-atomic` and indexed by `build-memory-index-sidecar`. Related: [[feedback_read_full_content_not_titles]].
