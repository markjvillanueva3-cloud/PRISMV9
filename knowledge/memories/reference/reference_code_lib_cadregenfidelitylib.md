---
name: reference_code_lib_cadregenfidelitylib
description: "scripts/lib/cad-regen-fidelity-lib.mjs -- pure core of the CAD regen-fidelity runner (slot:delta)."
metadata:
  type: reference
  kind: code-lib
  galaxy: unassigned
  sourcePath: scripts/lib/cad-regen-fidelity-lib.mjs
  sourceHash: 7fe7bc1e
  loc: 177
aliases: cad-regen-fidelity-lib.mjs
---

# cad-regen-fidelity-lib.mjs -- Lib Module SUMMARY

> Auto-generated by `scripts/code-to-vault.mjs` (CODE-VAULT-BRIDGE, slot:sierra). Re-run to refresh.
> generatedAt: 2026-07-03T03:00:00Z  |  source: `scripts/lib/cad-regen-fidelity-lib.mjs` (177 LOC)

## What it is
```
scripts/lib/cad-regen-fidelity-lib.mjs -- pure core of the CAD regen-fidelity runner (slot:delta).

THE GOAL (T3 geometry half, honest scope per the 2026-06-26 recon).
  A TRUE NURBS regen-fidelity run (re-emit a blisk-class B_SPLINE_SURFACE net) is MERGE-GATED --
  no surface emitter exists on trunk or in slot/delta. What IS achievable headless on trunk is a
  PRISMATIC dimensional-fidelity + SELF-CONSISTENCY run over the 240 cadquery-generated STEPs, plus
  reference-corpus measurement. This file holds the PURE math/parse; the .mts driver does the I/O
  (cadGeometryComparisonEngine.extractMetrics/compare) + writes CAD-TRAIN-TEST-RESULT.

PURE: no I/O, no Date.now. Fully node:test-able.

Karpathy discipline:
  CLASSIFY: parse (NL spec -> intended bbox) + transform (sorted dim-delta) + validate (band gate).
  TECHNIQUE: orientation-invariant comparison via SORTED dimension sets (a cylind
```


## Exports
`INCH_TO_MM` (const), `DEFAULT_BAND` (const), `parseInchesFromSpec` (function), `dimFidelity` (function), `bandPass` (function), `aggregate` (function)

## Key methods
_(none extracted)_

## Depends on
_(no sibling-engine imports)_

## Where it lives
- **Source:** `scripts/lib/cad-regen-fidelity-lib.mjs`
- **Galaxy:** _(cross-cutting / unassigned)_
- **Kind:** Lib Module

## How to query it
Recall by meaning via `/system-viz find cad-regen-fidelity-lib.mjs` or `prism_session:master_index_query`; open the source at `scripts/lib/cad-regen-fidelity-lib.mjs`. This note IS the brain connection -- a vault memory node under `knowledge/memories/reference/`, graphed by `generate-memories-atomic` and indexed by `build-memory-index-sidecar`. Related: [[feedback_read_full_content_not_titles]].
