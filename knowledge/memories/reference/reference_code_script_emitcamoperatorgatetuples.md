---
name: reference_code_script_emitcamoperatorgatetuples
description: "scripts/emit-cam-operator-gate-tuples.mjs"
metadata:
  type: reference
  kind: code-script
  galaxy: unassigned
  sourcePath: scripts/emit-cam-operator-gate-tuples.mjs
  sourceHash: 20683c5f
  loc: 134
aliases: emit-cam-operator-gate-tuples.mjs
---

# emit-cam-operator-gate-tuples.mjs -- Script SUMMARY

> Auto-generated by `scripts/code-to-vault.mjs` (CODE-VAULT-BRIDGE, slot:sierra). Re-run to refresh.
> generatedAt: 2026-07-03T03:00:00Z  |  source: `scripts/emit-cam-operator-gate-tuples.mjs` (134 LOC)

## What it is
```
scripts/emit-cam-operator-gate-tuples.mjs
  CAM-AI-TRAINING-MS0/U-CAMT-OPERATOR-GATE-TUPLES

Encodes the 12-item CAMOperatorGate pre-cut checklist as LoRA tuples:
  (a) Per-item "why does this matter?" Q&A (12 items)
  (b) Severity classification Q&A
  (c) Sample failure scenarios with computed S(x)

S(x) penalties + 0.98 shop-floor gate threshold verbatim from engine.
```


## Exports
_(no top-level exports detected)_

## Key methods
`join`

## Depends on
_(no sibling-engine imports)_

## Where it lives
- **Source:** `scripts/emit-cam-operator-gate-tuples.mjs`
- **Galaxy:** _(cross-cutting / unassigned)_
- **Kind:** Script

## How to query it
Recall by meaning via `/system-viz find emit-cam-operator-gate-tuples.mjs` or `prism_session:master_index_query`; open the source at `scripts/emit-cam-operator-gate-tuples.mjs`. This note IS the brain connection -- a vault memory node under `knowledge/memories/reference/`, graphed by `generate-memories-atomic` and indexed by `build-memory-index-sidecar`. Related: [[feedback_read_full_content_not_titles]].
